...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/app/convert.py
# Compiled at: 2018-10-19 09:36:00
# Size of source mod 2**32: 14490 bytes
...
def convert_one(input_filename, output_filename, options):
    ...
    if output_exists:
        ...
        if options.no_clobber:
            ...
            if options.force or options.remove_destination:
                remove_file = True
            else:
                if options.update:
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='fabio-convert', description=__doc__, epilog=epilog)
    parser.add_argument('IMAGE', nargs='*', help='Input file images')
    parser.add_argument('-V', '--version', action='version', version=fabio.version, help='output version and exit')
    parser.add_argument('-v', '--verbose', action='store_true', dest='verbose', default=False, help='show information for each conversions')
    parser.add_argument('--debug', action='store_true', dest='debug', default=False, help='show debug information')
    group = parser.add_argument_group('main arguments')
    group.add_argument('-l', '--list', action='store_true', dest='list', default=None, help='show the list of available formats and exit')
    group.add_argument('-o', '--output', dest='output', type=str, help='output file or directory')
    group.add_argument('-F', '--output-format', dest='format', type=str, default=None, help='output format')
    group = parser.add_argument_group('optional behaviour arguments')
    group.add_argument('-f', '--force', dest='force', action='store_true', default=False, help='if an existing destination file cannot be' + ' opened, remove it and try again (this option' + ' is ignored when the -n option is also used)')
    group.add_argument('-n', '--no-clobber', dest='no_clobber', action='store_true', default=False, help='do not overwrite an existing file (this option' + ' is ignored when the -i option is also used)')
    group.add_argument('--remove-destination', dest='remove_destination', action='store_true', default=False, help='remove each existing destination file before' + ' attempting to open it (contrast with --force)')
    group.add_argument('-u', '--update', dest='update', action='store_true', default=False, help='copy only when the SOURCE file is newer' + ' than the destination file or when the' + ' destination file is missing')
    group.add_argument('-i', '--interactive', dest='interactive', action='store_true', default=False, help='prompt before overwrite (overrides a previous -n' + ' option)')
    ...
    try:
        ...
        if args.format is None or not args.format.endswith('image'):
            ...
            if filename.format is None or len(filename.format) > 1:
