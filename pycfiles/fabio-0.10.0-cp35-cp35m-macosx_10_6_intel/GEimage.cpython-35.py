...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/GEimage.py
# Compiled at: 2019-03-04 08:01:16
# Size of source mod 2**32: 11988 bytes
from __future__ import with_statement, print_function, division
__authors__ = [
 'Antonino Miceli', 'Jon Wright', 'Jérôme Kieffer']
__date__ = '14/11/2018'
__status__ = 'production'
__copyright__ = '2007 APS; 2010-2015 ESRF'
__licence__ = 'MIT'
import numpy, struct, logging
...
class GeImage(FabioImage):
    ...
    def _readframe(self, filepointer, img_num):
        ...
        imgstart = self.header['StandardHeaderSizeInBytes'] + self.header['UserHeaderSizeInBytes'] + img_num * self.header['NumberOfRowsInFrame'] * self.header['NumberOfColsInFrame'] * self.header['ImageDepthInBits'] // 8
        filepointer.seek(imgstart, 0)
        bpp = self.header['ImageDepthInBits'] // 8
        if bpp != 2:
            logger.warning('Using uint16 for GE but seems to be wrong, bpp=%s' % bpp)
        imglength = self.header['NumberOfRowsInFrame'] * self.header['NumberOfColsInFrame'] * bpp
        data = numpy.frombuffer(filepointer.read(imglength), numpy.uint16).copy()
        if not numpy.little_endian:
            data.byteswap(True)
        data.shape = (
         self.header['NumberOfRowsInFrame'],
