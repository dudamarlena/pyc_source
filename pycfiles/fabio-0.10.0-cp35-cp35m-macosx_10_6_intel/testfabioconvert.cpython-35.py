...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/test/testfabioconvert.py
# Compiled at: 2020-04-03 09:02:03
# Size of source mod 2**32: 8220 bytes
"""
Test for fabio-convert
"""
import numpy, os.path, shutil, sys, subprocess, time, unittest, logging
...
class TestFabioConvert(unittest.TestCase):
    ...
    def setUp(self):
        self._TestFabioConvert__oldPath = os.getcwd()
        self._TestFabioConvert__testPath = self.create_test_env()
        os.chdir(self._TestFabioConvert__testPath)
        env = dict((str(k), str(v)) for k, v in os.environ.items())
        env['PYTHONPATH'] = os.pathsep.join(sys.path)
        self._TestFabioConvert__env = env
        self._TestFabioConvert__script = fabio.app.convert.__file__

    def tearDown(self):
        os.chdir(self._TestFabioConvert__oldPath)
        self.clean_test_env(self._TestFabioConvert__testPath)
        self.exe, self.env = (None, None)

    def subprocessFabioConvert(self, *args):
        commandLine = [
         sys.executable, self._TestFabioConvert__script]
        commandLine.extend(args)
        _logger.info('Execute: %s', ' '.join(commandLine))
        p = subprocess.Popen(commandLine, env=self._TestFabioConvert__env, shell=False, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return p
