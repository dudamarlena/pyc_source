...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/dm3image.py
# Compiled at: 2020-04-03 09:02:03
# Size of source mod 2**32: 11012 bytes
...
class Dm3Image(FabioImage):
    __doc__ = ' Read and try to write the dm3 data format '
    ...
    def _readheader(self):
        self.infile.seek(0)
        file_format = self.readbytes(4, numpy.uint32)[0]
        assert file_format == 3, 'Wrong file type '
        self.bytes_in_file = self.readbytes(4, numpy.uint32)[0]
        self.byte_order = self.readbytes(4, numpy.uint32)[0]
    ...
    def read(self, fname, frame=None):
        ...
        try:
            ...
            dim1_binning, dim2_binning = map(lambda x: x * int(binning_raw) * x, (1,
                                                                                  1))

        self._shape = (
         dim2_raw // dim2_binning, dim1_raw // dim1_binning)
    ...
    def read_tag_type(self):
        ...
        if self.tag_data_type == 1:
            return self.readbytes(DATA_BYTES[self.tag_encoded_type], DATA_TYPES[self.tag_encoded_type], swap=self.swap)[0]
