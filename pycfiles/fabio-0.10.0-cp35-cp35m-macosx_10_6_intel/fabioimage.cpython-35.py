...
# [GCC 8.4.0]
# Embedded file name: /Users/kieffer/workspace/fabio/build/lib.macosx-10.6-intel-3.5/fabio/fabioimage.py
...
__copyright__ = 'ESRF'
__date__ = '07/03/2019'
import os, logging, sys, tempfile, weakref
...
class _FabioArray(object):
    __doc__ = '"Abstract class providing array API used by :class:`FabioImage` and\n    :class:`FabioFrame`.'
    ...
    def integrate_area(self, coords):
        ...
        if len(coords) == 4:
            ...
            if sli == self.slice and self.area_sum is not None:
                ...
                if sli == self.slice and self.roi is not None:
...
class FabioImage(_FabioArray):
    ...
    def add(self, other):
        ...
        if not hasattr(other, 'data'):
            logger.warning('edfimage.add() called with something that does not have a data field')
    ...
    def rebin(self, x_rebin_fact, y_rebin_fact, keep_I=True):
        ...
        if dim1 % x_rebin_fact != 0 or dim2 % y_rebin_fact != 0:
            raise RuntimeError('image size is not divisible by rebin factor - skipping rebin')
        else:
    ...
    def readheader(self, filename):
        ...
        if isinstance(filename, fabioutils.PathTypes):
            ...
            save_state = (
             self._need_a_real_file, self._need_a_seek_to_read)
    ...
    def _open(self, fname, mode='rb'):
        ...
        if hasattr(fname, 'read') and hasattr(fname, 'write'):
            ...
            if comp_type == '.gz':
                fileObject = self._compressed_stream(fname, fabioutils.COMPRESSORS['.gz'], fabioutils.GzipFile, mode)
            else:
                if comp_type == '.bz2':
                    fileObject = self._compressed_stream(fname, fabioutils.COMPRESSORS['.bz2'], fabioutils.BZ2File, mode)
                else:
    ...
    def _compressed_stream(self, fname, system_uncompress, python_uncompress, mode='rb'):
        """
    ...
    def convert(self, dest):
        ...
        if type(dest) in fabioutils.StringTypes:
            ...
            if isinstance(dest, FabioImage):
                other = dest.__class__()
            else:
                if '__new__' in dir(dest) and isinstance(dest(), FabioImage):
                    other = dest()
                else:
