...
# Embedded file name: D:\Workspace\py34\py34_projects\ctmatching-project\ctmatching\core.py
# Compiled at: 2016-09-08 04:09:12
# Size of source mod 2**32: 11698 bytes
...
def exam_input(control, treatment, stratify_order=None):
    ...
    if len(control_sample) != len(treatment_sample):
        raise InputError('control sample and treatment sample are different in size!')
    if stratify_order:
        for chunk in stratify_order:
            if len(chunk) != len(set(chunk)):
                raise InputError('duplicate column index is not allowed in one order!')
            for index in chunk:
...
def stratified_matching(control, treatment, stratify_order):
    ...
    for chunk in zip(*distmatrix_list):
        ...
        nn_index.append(df.sort(columns=list(range(len(stratify_order)))).index.values)
...
def non_repeat_index_matching(nn_indices, k=1):
    ...
    if k * num_of_treatment > num_of_control:
        raise InputError("There's no enough samples in control group to perform non repeat matching. Use independent matching instead.")
    selected_control_index = OrderedSet(list())
    selected_control_index_for_each_treatment = list()
    for indice in nn_indices:
        counter = 0
        selected = list()
        for ind in indice:
            if ind not in selected_control_index:
    ...
    return (
     selected_control_index, selected_control_index_for_each_treatment)
...
def psm(control, treatment, use_col=None, stratify_order=None, independent=True, k=1):
    ...
    if stratify_order:
        nn_index = stratified_matching(control_used, treatment_used, stratify_order)
    else:
        nn_index = non_stratified_matching(control_used, treatment_used)
    if independent:
        selected_control_index, selected_control_index_for_each_treatment = independent_index_matching(nn_index, k)
    else:
        selected_control_index, selected_control_index_for_each_treatment = non_repeat_index_matching(nn_index, k)
    return (
     selected_control_index, selected_control_index_for_each_treatment)
