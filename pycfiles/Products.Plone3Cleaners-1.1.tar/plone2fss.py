...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Products/Plone2FSS/browser/plone2fss.py
# Compiled at: 2009-01-22 15:21:13
import os
...
class Plone2FSSExport(BrowserView):
    ...
    def _getSubObjects(self, content_types):
        ...
        results = catalog(portal_type=content_types, path=physical_path)
        return results
    ...
    def _createImageFormat(self, obj, brain, format, fs_path):
        ...
        if not traversed:
            print "The object at %s (%s) doesn't have the %s image format" % (brain.getPath(), obj.portal_type, format)
            return
    ...
    def _exportStrategyFlat(self, brain, filesystem_output_dir):
        ...
        if obj.portal_type == 'File':
            if obj.restrictedTraverse(brain.getPath() + '/file', default=None):
                self._createFile(str(obj.getFile().data), os.path.join(filesystem_output_dir, uid + '_file'))
        elif obj.portal_type == 'Image' or obj.portal_type == 'News Item':
            if obj.restrictedTraverse(brain.getPath() + '/image', default=None):
                self._createFile(str(obj.restrictedTraverse(brain.getPath() + '/image').data), os.path.join(filesystem_output_dir, uid + '_image'))
                ...
                for format in image_field_formats:
                    self._createImageFormat(obj, brain, format, os.path.join(filesystem_output_dir, uid + '_image_%s' % format))
    ...
    def _exportStrategyDirectory(self, brain, filesystem_output_dir):
        ...
        if obj.portal_type == 'File':
            if obj.restrictedTraverse(brain.getPath() + '/file', default=None):
                self._createFile(str(obj.getFile().data), os.path.join(filesystem_output_dir, d1, d2, uid + '_file'))
        elif obj.portal_type == 'Image' or obj.portal_type == 'News Item':
            if obj.restrictedTraverse(brain.getPath() + '/image', default=None):
                self._createFile(str(obj.restrictedTraverse(brain.getPath() + '/image').data), os.path.join(filesystem_output_dir, d1, d2, uid + '_image'))
                image_field_formats = self.request.get('image_field_formats', self.default_image_field_formats)
                for format in image_field_formats:
                    self._createImageFormat(obj, brain, format, os.path.join(filesystem_output_dir, d1, d2, uid + '_image_%s' % format))
        ...
        if brain.portal_type == 'File':
            ...
            try:
                ...
                try:
                    ...
                    if not fileContent or not obj.getFilename():
                        ...
                        return
                    self._createFile(fileContent, os.path.join(dirpath2, obj.getFilename(ct_type)))
            ...
            for ct_type in ct_types:
                ...
                if not fileContent or not obj.getFilename('image'):
                    ...
                    continue
                self._createFile(fileContent, os.path.join(dirpath2, obj.getFilename('image')))
