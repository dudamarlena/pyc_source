...
# [GCC 8.4.0]
# Embedded file name: /Users/Seth/Programming/input_reader/input_reader/key_adder.py
...
class _KeyAdder(_KeyLevel):
    ...
    def _ensure_default_has_a_value(self, kwargs):
        if b'default' not in kwargs:
            kwargs[b'default'] = self._default

    def _check_keyname(self, keyname, strid):
        """Run the given keyname through a few checks"""
        if not isinstance(keyname, py23_basestring):
            raise ValueError((b'{0}: {1} must be str').format(repr(keyname), strid))
        if keyname in self._keys:
            raise ReaderError((b'The {0} "{1}" has been defined twice').format(strid, keyname))
        if not self._case:
    ...
    def _check_case(self, case, keyname):
        ...
        if not isinstance(case, bool):
            raise ValueError(keyname + b': case must be bool, given ' + repr(case))
        return case
    ...
    def add_block_key(self, keyname, end=b'end', case=None, ignoreunknown=None, **kwargs):
        ...
        if not isinstance(end, py23_basestring):
            raise ValueError(keyname + b': end must be str, given ' + repr(end))
        if ignoreunknown is None:
            ignoreunknown = self._ignoreunknown
        if not isinstance(ignoreunknown, bool):
            raise ValueError(keyname + b': ignoreunknown must be bool, given ' + repr(ignoreunknown))
    ...
    def add_regex_line(self, handle, regex, case=None, **kwargs):
        ...
        :type repeat: bool
        """
        handle = self._check_keyname(handle, b'handle')
        self._ensure_default_has_a_value(kwargs)
    ...
    def add_mutually_exclusive_group(self, dest=None, default=None, required=False):
        ...
        if dest is not None and not isinstance(dest, py23_basestring):
            raise ValueError(b'dest must be a str, given ' + repr(dest))
        if not isinstance(required, bool):
            raise ValueError(b'required value must be a bool, given ' + repr(required))
        self._meg.append(MutExGroup(self._case, dest, default, required, self._ignoreunknown))
        return self._meg[(-1)]
    ...
    def _find_keys_in_input(self, f, i, namespace):
        ...
        while i < len(f) and notend:
            if f[i]:
                try:
                    ...
                    if self._ignoreunknown:
                        if b'Unrecognized' not in str(e):
                            raise ReaderError(self.name + b': ' + str(e))
                    else:
                        raise ReaderError(self.name + b': ' + str(e))
            ...
            try:
                ...
                if i == len(f) and self.name != b'main':
                    raise ReaderError(self.name + b': Unterminated block.')
    ...
    def _find_key(self, f, i, namespace):
        ...
        raise ReaderError(self.name + b': Unrecognized key: "' + f[i] + b'"')
    ...
    def _post(self, namespace):
        ...
        for meg in self._meg:
            ...
            if nkeys == 0:
                if meg._required:
                    ...
                    msg = b': One and only one of '
                    msg += (b', ').join([ repr(x) for x in keys[:-1] ])
                    msg += b', or ' + repr(keys[(-1)]) + b' must be included.'
                ...
                msg = b': Only one of '
                msg += (b', ').join([ repr(x) for x in keys[:-1] ])
                msg += b', or ' + repr(keys[(-1)]) + b' may be included.'
                raise ReaderError(self.name + msg)
            elif meg._dest:
        ...
        for key, val in py23_items(self._keys)():
            ...
            if val._required and name not in namespace:
                msg = b': The key "' + key + b'" is required but not found'
                raise ReaderError(self.name + msg)
        ...
        for key in namespace:
            for val in py23_values(self._keys)():
                name = val._dest if val._dest is not None else val.name
                if key == name:
                    depends = getattr(val, b'_depends', None)
                    break
            else:
                depends = None

            if depends and depends not in namespace:
                msg = b': The key "' + key + b'" requires that "' + depends
                msg += b'" is also present, but it is not'
                raise ReaderError(self.name + msg)
...
class BlockKey(_KeyAdder):
    ...
    def _parse(self, f, i, namespace):
        ...
        raise ReaderError(b'The block "' + self.name + b'" was given arguments, this is illegal')