...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\stacklesslib\test\test_futures.py
# Compiled at: 2017-12-11 20:12:50
from __future__ import print_function
import test.test_support, sys, threading, unittest, weakref, logging
...
def create_future(state=PENDING, exception=None, result=None):
    ...
    if state == FINISHED:
        if exception:
            f._result = (
             False, (type(exception), exception, None))
        else:
            f._result = (
             True, result)
...
class ExecutorMixin(unittest.TestCase):
    ...
    def _prime_executor(self):
        futures = [ self.executor.submit(time.sleep, 0.1) for _ in range(self.worker_count)
                  ]
        for f in futures:
            f.result()
...
class WaitTests(object):
    ...
    def test_first_completed_some_already_completed(self):
        ...
        finished, pending = futures.wait([
         CANCELLED_AND_NOTIFIED_FUTURE, SUCCESSFUL_FUTURE, future1], return_when=futures.FIRST_COMPLETED)
    ...
    def test_first_exception_some_already_complete(self):
        ...
        finished, pending = futures.wait([
         ...
         future1, future2], return_when=futures.FIRST_EXCEPTION)
    ...
    def test_all_completed(self):
        ...
        finished, pending = futures.wait([
         ...
         EXCEPTION_FUTURE,
         future1,
         future2], return_when=futures.ALL_COMPLETED)
        self.assertEqual(set([SUCCESSFUL_FUTURE,
         CANCELLED_AND_NOTIFIED_FUTURE,
         EXCEPTION_FUTURE,
         future1,
    ...
    def test_timeout(self):
        ...
        finished, pending = futures.wait([
         ...
         EXCEPTION_FUTURE,
         SUCCESSFUL_FUTURE,
         future1, future2], timeout=0.5, return_when=futures.ALL_COMPLETED)
...
class AsCompletedTests(object):
    ...
    def test_zero_timeout(self):
        ...
        try:
            for future in futures.as_completed([CANCELLED_AND_NOTIFIED_FUTURE,
             EXCEPTION_FUTURE,
             SUCCESSFUL_FUTURE,
             future1], timeout=0):
...
class FutureTests(unittest.TestCase):
    ...
    def _test_result_with_cancel(self):
    ...
    def _test_exception_with_success(self):
