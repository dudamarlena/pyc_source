...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\stacklesslib\test\testasync.py
# Compiled at: 2017-12-11 20:12:50
import stackless, unittest, logging, time, sys
...
def GetSleepFunc(min=1e-06, max=0.01):
...
def GetErrFunc(exception=ZeroDivisionError, min=1e-06, max=0.01):
    t = random.random() * (max - min) + min
...
def GetTask(args, min=1e-06, max=0.01):
    return async.create_future(GetSleepFunc(min, max), args)


def GetErrTask(exception=ZeroDivisionError, min=1e-06, max=0.01):
