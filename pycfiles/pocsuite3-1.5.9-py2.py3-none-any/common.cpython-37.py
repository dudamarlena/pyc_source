...
# [GCC 8.4.0]
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/lib/core/common.py
# Compiled at: 2019-06-16 22:29:50
# Size of source mod 2**32: 26708 bytes
import hashlib, inspect, logging, os, re, select, shlex, socket, struct, subprocess, sys, time
...
from pocsuite3.lib.core.settings import BANNER, BOLD_PATTERNS, IS_WIN, URL_DOMAIN_REGEX, LOCAL_IP_ADDRESS_REGEX, IP_ADDRESS_WITH_PORT_REGEX, IPV6_URL_REGEX, TIMESTAMP, OS_SYSTEM
...
import pocsuite3.thirdparty.colorama.initialise as coloramainit
import pocsuite3.thirdparty.ifcfg as ifcfg
...
def data_to_stdout(data, bold=False):
    ...
    """
    if not 'quiet' not in conf:
        message = conf.quiet or ''
...
def get_latest_revision():
    ...
    try:
        ...
        ret = extract_regex_result('__version__\\s*=\\s*[\\"\'](?P<result>[\\d.]+)', content)
    except Exception:
        pass
...
def parse_target_url(url):
    ...
    ret = url
    if conf.ipv6:
        if is_ipv6_address_format(url):
            ret = '[' + ret + ']'
    elif not re.search('^http[s]*://', ret, re.I):
        if not re.search('^ws[s]*://', ret, re.I):
...
def is_url_format(value):
    if value:
        if re.match(URL_ADDRESS_REGEX, value):
            return True
    return False
...
def is_domain_format(value):
    if value:
        if re.match(URL_DOMAIN_REGEX, value):
            return True
    return False
...
def is_ip_address_with_port_format(value):
    if value:
        if re.match(IP_ADDRESS_WITH_PORT_REGEX, value):
            return True
    return False
...
def is_ipv6_address_format(value):
    if value:
        if re.match(IPV6_ADDRESS_REGEX, value):
            return True
    return False
...
def is_ipv6_url_format(value):
    if value:
        if re.match(IPV6_URL_REGEX, value):
            return True
    return False
...
def parse_target(address):
    target = None
    if not is_domain_format(address):
        if is_url_format(address) or is_ip_address_with_port_format(address):
            target = address
    elif is_ipv6_url_format(address):
        conf.ipv6 = True
        target = address
    else:
        if is_ip_address_format(address):
            try:
...
def is_local_ip(ip_string):
    ...
    if ip_string:
        if isinstance(ip_string, str):
            if re.match(LOCAL_IP_ADDRESS_REGEX, ip_string):
                ret = True
    return ret
...
def create_shellcode(asm_code, os_target, os_target_arch, make_exe=0, debug=0, filename='', dll_inj_funcs=[]):
    ...
    if not is_os_64bit():
        if os_target_arch == OS_ARCH.X64:
            print('ERR: can not create shellcode for this os_target_arch ({0}) on os_arch ({1})'.format(os_target_arch, OS_ARCH.X64))
            return
