...
# [GCC 8.4.0]
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/thirdparty/ifcfg/ifcfg/parser.py
# Compiled at: 2019-06-19 22:30:14
# Size of source mod 2**32: 11970 bytes
from __future__ import unicode_literals
import os, re, socket
from .tools import exec_cmd, hex2dotted, minimal_logger
Log = minimal_logger(__name__)
DEVICE_PROPERTY_DEFAULTS = {'inet':None, 
 'inet4':'LIST', 
 'ether':None, 
 'inet6':'LIST', 
 'netmask':None}
            ...
            for pattern in patterns:
                ...
                if not m:
                    ...
                    if 'device' in groupdict:
                        ...
                        if cur is None:
                            raise RuntimeError("Got results that don't belong to a device")
                for k, v in groupdict.items():
                    if k in self._interfaces[cur]:
                        if self._interfaces[cur][k] is None:
                            self._interfaces[cur][k] = v
                        else:
                            if hasattr(self._interfaces[cur][k], 'append'):
                                self._interfaces[cur][k].append(v)
                            else:
                                if self._interfaces[cur][k] == v:
                                    continue
                                else:
                                    raise RuntimeError('Tried to add {}={} multiple times to {}, it was already: {}'.format(k, v, cur, self._interfaces[cur][k]))
                    else:
                        self._interfaces[cur][k] = v
    ...
    def alter(self, interfaces):
        ...
        for device, device_dict in interfaces.items():
            ...
            if 'inet' in device_dict:
                if device_dict['inet'] is not None:
                    try:
...
class WindowsParser(Parser):
    ...
    def get_patterns(cls):
        return [
         '(?P<device>\\w.+):',
         '.*?(?:Physical Address|物理地址).*?(?P<ether>[ABCDEFabcdef\\d-]+)',
         '.*?IPv4.*?(?P<inet4>(?:(?:\\d+\\.){3}\\d+))',
         '.*?IPv6.*?(?P<inet6>(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){7}|[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,7}::[a-f0-9]{0,4}(?::[a-f0-9]{1,4}){0,7}%?[a-z0-9]*))']
