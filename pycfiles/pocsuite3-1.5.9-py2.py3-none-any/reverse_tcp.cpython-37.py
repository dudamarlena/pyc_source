...
# [GCC 8.4.0]
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/modules/listener/reverse_tcp.py
# Compiled at: 2019-08-19 22:10:17
# Size of source mod 2**32: 10742 bytes
...
def list_clients():
    ...
    for i, client in enumerate(kb.data.clients):
        try:
            ...
            if ret:
                ...
                if 'darwin' in ret.lower():
                    ...
                    if 'linux' in ret.lower():
                        system = 'Linux'
                    else:
                        if 'uname' in ret.lower():
                            system = 'Windows'
        ...
        results += str(i) + '   ' + str(client.address[0]) + '    ' + str(client.address[1]) + ' ({0})'.format(system) + '\n'
...
def send_shell_commands_for_console(client):
    module_prompt_default_template = '\x01\x1b[4m\x02SHELL\x01\x1b[0m\x02 (\x01\x1b[91m\x02{hostname}\x01\x1b[0m\x02) > '
    while True:
        cmd = None
        try:
            ...
            if not cmd:
                ...
                if cmd.lower() == 'clear':
                    ...
                    if cmd.lower() in ('x', 'q', 'exit', 'quit', 'bye'):
                        break
