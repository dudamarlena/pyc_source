...
class PrettyTable(object):
    ...
    def _validate_option(self, option, val):
        if option in 'field_names':
            ...
            if option in ('start', 'end', 'max_width', 'min_width', 'min_table_width',
                        ...
                        if option in 'hrules':
                            ...
                            if option in 'vrules':
                                ...
                                if option in 'fields':
                                    ...
                                    if option in ('header', 'border', 'reversesort',
                                                  'xhtml', 'print_empty', 'oldsortslice'):
                                        self._validate_true_or_false(option, val)
                                    else:
                                        if option in 'header_style':
                                            self._validate_header_style(val)
                                        else:
                                            if option in 'int_format':
                                                self._validate_int_format(option, val)
                                            else:
                                                if option in 'float_format':
                                                    self._validate_float_format(option, val)
                                                else:
                                                    if option in ('vertical_char',
                                                                  'horizontal_char',
                                                                  'junction_char'):
    ...
    def align(self, val):
        if not self._field_names:
            ...
            if not (val is None or isinstance)(val, dict) or len(val) is 0:
    ...
    def _stringify_header(self, options):
        ...
        if options['border']:
            if options['hrules'] in (ALL, FRAME):
                ...
                bits.append(' ')
        elif (self._field_names or options['vrules']) in (ALL, FRAME):
            bits.append(options['vertical_char'])
        else:
            bits.append(' ')
        for field, width in zip(self._field_names, self._widths):
            if options['fields']:
                ...
                if self._header_style == 'title':
                    fieldname = field.title()
                else:
                    if self._header_style == 'upper':
                        fieldname = field.upper()
                    else:
                        if self._header_style == 'lower':
    ...
    def _get_formatted_html_string(self, options):
        ...
        if options['border']:
            if options['hrules'] == ALL and options['vrules'] == ALL:
                ...
                if options['hrules'] == FRAME and options['vrules'] == FRAME:
                    open_tag.append(' frame="box"')
                else:
                    if options['hrules'] == FRAME and options['vrules'] == ALL:
