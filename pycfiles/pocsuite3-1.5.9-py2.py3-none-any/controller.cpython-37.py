...
# [GCC 8.4.0]
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/lib/controller/controller.py
# Compiled at: 2019-11-05 00:20:44
            ...
            if hasattr(poc_module, '_options'):
                ...
                for opt, v in poc_module.options.items():
                    if opt in CMD_PARSE_WHITELIST:
                        info_msg = "Poc:'{0}' You can't customize this variable '{1}' because it is already taken up by the pocsuite.".format(poc_name, opt)
                        logger.error(info_msg)
                        raise SystemExit
                    if v.require and v.value == '':
                        info_msg = "Poc:'{poc}' Option '{key}' must be set,please add parameters '--{key}'".format(poc=poc_name,
                          key=opt)
                        logger.error(info_msg)
                        raise SystemExit
