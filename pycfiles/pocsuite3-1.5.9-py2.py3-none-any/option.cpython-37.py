...
# [GCC 8.4.0]
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/lib/core/option.py
# Compiled at: 2019-11-11 22:57:25
# Size of source mod 2**32: 20580 bytes
import copy, glob, logging, os, re, socket
from queue import Queue
from urllib.parse import urlsplit
from pocsuite3.lib.core.clear import remove_extra_log_message
from pocsuite3.lib.core.common import boldify_message, check_file, get_file_items, parse_target, get_public_type_members, data_to_stdout
...
import pocsuite3.lib.core.update as update
    ...
    if conf.headers:
        ...
        for header_value in conf.headers:
            ...
            if header_value.count(':') >= 1:
                ...
                if header and value and header not in conf.http_headers:
...
def _set_network_proxy():
    if conf.proxy:
        ...
        if not all((scheme, hasattr(PROXY_TYPE, scheme), hostname, port)):
            err_msg = "proxy value must be in format '({0})://address:port'".format('|'.join((_[0].lower() for _ in get_public_type_members(PROXY_TYPE))))
            raise PocsuiteSyntaxException(err_msg)
        else:
            if conf.proxy_cred:
                _ = re.search('\\A(.*?):(.*?)\\Z', conf.proxy_cred)
                if not _:
            ...
            if scheme in (PROXY_TYPE.SOCKS4, PROXY_TYPE.SOCKS5, PROXY_TYPE.SOCKS5H):
                socks.set_default_proxy((socks.PROXY_TYPE_SOCKS4 if scheme == PROXY_TYPE.SOCKS4 else socks.PROXY_TYPE_SOCKS5),
                  hostname,
                  port,
                  username=username,
                  password=password,
                  rdns=(True if scheme == PROXY_TYPE.SOCKS5H else False))
                conf.origin_socks = copy.deepcopy(socket.socket)
                socket.socket = socks.socksocket
            if conf.proxy_cred:
                proxy_string = '{0}@'.format(conf.proxy_cred)
            else:
                proxy_string = ''
        proxy_string = '{scheme}://{proxy_string}{hostname}:{port}'.format(scheme=(scheme.lower()), proxy_string=proxy_string, hostname=hostname,
          port=port)
        conf.proxies = {'http':proxy_string, 
         'https':proxy_string}
...
def _set_connect_back():
    ...
    if conf.mode == 'shell':
        if conf.connect_back_host is None:
    ...
    if conf.poc:
        ...
        for poc in conf.poc:
            ...
            if any([poc in exists_poc_with_ext, poc in exists_pocs]):
                ...
                if load_poc_sucess or poc.startswith('ssvid-'):
...
def _cleanup_options():
    ...
    if conf.agent:
        ...
        if isinstance(conf.cookie, str):
            ...
            if not isinstance(conf.cookie, dict):
...
def _create_directory():
    if not os.path.isdir(paths.POCSUITE_OUTPUT_PATH):
        ...
        os.path.isfile(paths.POCSUITE_RC_PATH) or open(paths.POCSUITE_RC_PATH, 'a').close()
