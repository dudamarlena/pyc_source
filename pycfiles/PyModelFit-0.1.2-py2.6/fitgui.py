...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymodelfit/fitgui.py
# Compiled at: 2011-05-10 05:53:13
"""
This module contains the internals for the FitGui gui.
"""
from __future__ import division, with_statement
import numpy as np
from enthought.traits.api import HasTraits, Instance, Int, Float, Bool, Button, Event, Property, on_trait_change, Array, List, Tuple, Str, Dict, cached_property, Color, Enum, TraitError, Undefined, DelegatesTo
from enthought.traits.ui.api import View, Handler, Item, Label, Group, VGroup, HGroup, InstanceEditor, EnumEditor, ListEditor, TupleEditor, spring
from enthought.traits.ui.menu import ModalButtons
from enthought.chaco.api import Plot, ArrayPlotData, jet, ColorBar, HPlotContainer, ColorMapper, LinearMapper, ScatterInspectorOverlay, LassoOverlay, AbstractOverlay, ErrorBarPlot, ArrayDataSource
...
def _cmapblack(range, **traits):
    _data = {'red': ((0, 1, 1), (0.3, 0.8, 0.8), (0.5, 0, 0), (0.75, 0.75, 0.75), (0.875, 0.2, 0.2),
             (1, 0, 0)), 
       'blue': (
              (0.0, 0, 0), (0.3, 0, 0), (0.5, 0, 0), (0.75, 0.75, 0.75),
              (0.875, 0.75, 0.75), (1, 1, 1)), 
       'green': (
               (0.0, 0, 0), (0.3, 0.8, 0.8), (0.4, 0.4, 0.4), (0.5, 1, 1), (0.65, 0.75, 0.75), (0.75, 0.1, 0.1),
               (1, 0, 0))}
...
class TraitedModel(HasTraits):
    ...
    def default_traits_view(self):
        if self.model is None:
            ...
            hg = HGroup(Item('fittype', label='Fit Technique', editor=EnumEditor(name='fittypes')))
            g.content.append(hg)
    ...
    def _fitdata_fired(self, new):
        ...
        if self.model is not None:
            if isSequenceType(new) and len(new) == 2:
                ...
                if isSequenceType(new) and len(new) == 3:
...
class NewModelSelector(HasTraits):
    ...
    traits_view = View(Item('selectedname', label='Model Name:', editor=EnumEditor(name='modelnames')), Item('modelargnum', label='Extra Parameters:', enabled_when='isvarargmodel'), buttons=[
     'OK', 'Cancel'])
...
class FGHandler(Handler):
    ...
    def object_datasymb_changed(self, info):
        kind = info.ui.rebuild.__name__.replace('ui_', '')
        info.object.plot.plots['data'][0].edit_traits(parent=info.ui.control, kind=kind)

    def object_modline_changed(self, info):
        kind = info.ui.rebuild.__name__.replace('ui_', '')
        info.object.plot.plots['model'][0].edit_traits(parent=info.ui.control, kind=kind)
...
class FitGui(HasTraits):
    ...
    nmod = Int(1024)
    modelpanel = View
    panel_view = View(VGroup(Item('plot', editor=ComponentEditor(), show_label=False), HGroup(Item('tmodel.modelname', show_label=False, style='readonly'), Item('nmod', label='Number of model points'), Item('updatemodelplot', show_label=False, enabled_when='not autoupdate'), Item('autoupdate', label='Auto?'))), title='Model Data Fitter')
    selection_view = View(Group(Item('scattertool', label='Selection Mode', editor=EnumEditor(values={None: '1:No Selection', 'clicktoggle': '3:Toggle Select', 
       'clicksingle': '2:Single Select', 
       'clickimmediate': '7:Immediate', 
       'lassoadd': '4:Add with Lasso', 
       'lassoremove': '5:Remove with Lasso', 
       'lassoinvert': '6:Invert with Lasso'})), Item('unselectonaction', label='Clear Selection on Action?'), Item('clearsel', show_label=False), Item('weightchangesel', show_label=False), Item('weightchangeto', label='Weight'), Item('delsel', show_label=False)), title='Selection Options')
    traits_view = View(VGroup(HGroup(Item('object.plot.index_scale', label='x-scaling', enabled_when='object.plot.index_mapper.range.low>0 or object.plot.index_scale=="log"'), spring, Item('ytype', label='y-data'), Item('object.plot.value_scale', label='y-scaling', enabled_when='object.plot.value_mapper.range.low>0 or object.plot.value_scale=="log"')), Item('plotcontainer', editor=ComponentEditor(), show_label=False), HGroup(VGroup(HGroup(Item('weighttype', label='Weights:'), Item('savews', show_label=False), Item('loadws', enabled_when='_savedws', show_label=False)), Item('weights0rem', label='Remove 0-weight points for fit?'), HGroup(Item('newmodel', show_label=False), Item('fitmodel', show_label=False), Item('showerror', show_label=False, enabled_when='tmodel.lastfitfailure'), VGroup(Item('chi2', label='Chi2:', style='readonly', format_str='%6.6g', visible_when='tmodel.model is not None'), Item('chi2r', label='reduced:', style='readonly', format_str='%6.6g', visible_when='tmodel.model is not None'))), springy=False), spring, VGroup(HGroup(Item('autoupdate', label='Auto?'), Item('updatemodelplot', show_label=False, enabled_when='not autoupdate')), Item('nmod', label='Nmodel'), HGroup(Item('datasymb', show_label=False), Item('modline', show_label=False)), springy=False), springy=True), '_', HGroup(Item('scattertool', label='Selection Mode', editor=EnumEditor(values={None: '1:No Selection', 'clicktoggle': '3:Toggle Select', 
       'clicksingle': '2:Single Select', 
       'clickimmediate': '7:Immediate', 
       'lassoadd': '4:Add with Lasso', 
       'lassoremove': '5:Remove with Lasso', 
       'lassoinvert': '6:Invert with Lasso'})), Item('unselectonaction', label='Clear Selection on Action?'), Item('clearsel', show_label=False), Item('weightchangesel', show_label=False), Item('weightchangeto', label='Weight'), Item('delsel', show_label=False), layout='flow'), Item('tmodel', show_label=False, style='custom', editor=InstanceEditor(kind='subpanel'))), handler=FGHandler(), resizable=True, title='Data Fitting', buttons=[
     'OK', 'Cancel'], width=700, height=900)

    def __init__(self, xdata=None, ydata=None, weights=None, model=None, include_models=None, exclude_models=None, fittype=None, **traits):
        """
        ...
        self.scatter = plot.plot(('xdata', 'ydata', 'weights'), name='data', color_mapper=_cmapblack if self.weights0rem else _cmap, type='cmap_scatter', marker='circle')[0]
        ...
        self.scatter.overlays.append(ScatterInspectorOverlay(self.scatter, hover_color='black', selection_color='black', selection_outline_color='red', selection_line_width=2))
        self.colorbar = colorbar = ColorBar(index_mapper=LinearMapper(range=plot.color_mapper.range), color_mapper=plot.color_mapper.range, plot=plot, orientation='v', resizable='v', width=30, padding=5)
    ...
    def _newmodel_fired(self, newval):
        from inspect import isclass
        if isinstance(newval, basestring) or isinstance(newval, FunctionModel1D) or isclass(newval) and issubclass(newval, FunctionModel1D):
            self.tmodel = TraitedModel(newval)
        elif self.modelselector.edit_traits(kind='modal').result:
    ...
    def _showerror_fired(self, evt):
        if self.tmodel.lastfitfailure:
            ...
            view = View(Item('s', style='custom', show_label=False), resizable=True, buttons=['OK'], title='Fitting error message')
    ...
    def _scattertool_changed(self, old, new):
        ...
        if old is not None and 'lasso' in old:
            if new is not None and 'lasso' in new:
                ...
                return
            self.scatter.tools[(-1)].on_trait_change(self._lasso_handler, 'selection_changed', remove=True)
            del self.scatter.overlays[-1]
            del self.lassomode
        elif old == 'clickimmediate':
            self.scatter.index.on_trait_change(self._immediate_handler, 'metadata_changed', remove=True)
        self.scatter.tools = []
        if new is None:
            pass
        elif 'click' in new:
            smodemap = {'clickimmediate': 'single', 'clicksingle': 'single', 'clicktoggle': 'toggle'}
            self.scatter.tools.append(ScatterInspector(self.scatter, selection_mode=smodemap[new]))
            if new == 'clickimmediate':
                self.clearsel = True
                self.scatter.index.on_trait_change(self._immediate_handler, 'metadata_changed')
        elif 'lasso' in new:
            lasso_selection = LassoSelection(component=self.scatter, selection_datasource=self.scatter.index)
            self.scatter.tools.append(lasso_selection)
            lasso_overlay = LassoOverlay(lasso_selection=lasso_selection, component=self.scatter)
            self.scatter.overlays.append(lasso_overlay)
            self.lassomode = new.replace('lasso', '')
            lasso_selection.on_trait_change(self._lasso_handler, 'selection_changed')
            lasso_selection.on_trait_change(self._lasso_handler, 'selection_completed')
            lasso_selection.on_trait_change(self._lasso_handler, 'updated')
        else:
    ...
    def weightsChanged(self):
        ...
        if 'errorplots' in self.trait_names():
            ...
            if len(weights.shape) == 2 and weights.shape[0] == 2:
                ...
                ebpx = ErrorBarPlot(orientation='v', value_high=high, value_low=low, index=self.scatter.value, value=self.scatter.index, index_mapper=self.scatter.value_mapper, value_mapper=self.scatter.index_mapper)
                self.plot.add(ebpx)
                high = ArrayDataSource(self.scatter.value.get_data() + yerr)
                low = ArrayDataSource(self.scatter.value.get_data() - yerr)
                ebpy = ErrorBarPlot(value_high=high, value_low=low, index=self.scatter.index, value=self.scatter.value, index_mapper=self.scatter.index_mapper, value_mapper=self.scatter.value_mapper)
                self.plot.add(ebpy)
                self.errorplots = (
                 ebpx, ebpy)
    ...
    def _weighttype_changed(self, name, old, new):
        if old == 'custom':
            self._customweights = self.weights
        if new == 'custom':
            self.weights = self._customweights
        elif new == 'equal':
    ...
    def getModelInitStr(self):
        ...
        if mod is None:
            ...
            if mod.__class__._pars is None:
