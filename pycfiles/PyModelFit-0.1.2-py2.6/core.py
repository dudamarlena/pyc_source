...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymodelfit/core.py
# Compiled at: 2011-05-10 05:52:20
"""
    ...
    def _weights_to_err(self, weights):
        if weights is None:
            ...
            if self._weightstype == 'ierror':
                ...
                return 1.0 / weights
                if self._weightstype == 'ivar':
                    return 1.0 / np.abs(weights) ** 0.5
                if self._weightstype == 'error':
                    return weights
                if self._weightstype == 'var':
                    return np.abs(weights) ** 0.5
                raise False or AssertionError, 'should be unreachable'
            return
    ...
    def __call__(cls, *args, **kwargs):
        if cls._pars is None:
            ...
            try:
                ...
                cls._currnparams = nparams
                obj = super(AutoParamsMeta, AutoParamsMeta).__call__(cls, **objkwargs)
...
class FunctionModel(ParametricModel):
    ...
    * :meth:`_filterfunc`
    ...
    fittype = 'leastsq'
    _optfittypes = (
     'leastsq', 'fmin', 'fmin_powell', 'fmin_cg', 'fmin_bfgs',
    ...
    def fitData(self, x=None, y=None, fixedpars='auto', weights=None, savedata=True, updatepars=True, fitf=False, contraction='sumsq', **kwargs):
        """
    ...
    def chi2Data(self, x=None, y=None, weights=None):
        ...
        return (
         chi2, chi2 / dof, chisqprob(chi2, dof))
    ...
    def resampleFit(self, x=None, y=None, xerr=None, yerr=None, bootstrap=False, modely=False, n=250, prefit=True, medianpars=False, plothist=False, **kwargs):
        """
        ...
        return (
         d, np.cov([ d[p] for p in self.params ]))
...
class CompositeModel(FunctionModel):
    ...
    def __init__(self, models=[], operation='+', parnames={}, autoshorten=True, **parvals):
        """
        :param models: 
        ...
        for (i, p) in enumerate(self._pars):
            ...
            parlistd[modi][indinmodel] = mods[modi].parvals[indinmodel]
...
class FunctionModel1D(FunctionModel):
    ...
    * :attr:`rangehint`
    ...
    def plot(self, lower=None, upper=None, n=100, clf=True, logplot='', data='auto', errorbars=True, *args, **kwargs):
        """
    ...
    def pixelize(self, xorxl, xu=None, n=None, edge=False, sampling=None):
        ...
        if xu is None:
            if n is None:
                ...
                dx = edge or (xorxl[(-1)] - xorxl[0]) / (len(xorxl) - 1)
...
class ModelSequence(object):
    ...
    def __init__(self, models, extraparams=None, outputcontraction=None, interpolation='linear', interpolationdirection='y', offgrid=None):
        """
...
class FunctionModel2DScalar(FunctionModel, InputCoordinateTransformer):
    ...
    _inputtransforms = {'cartesian': {'polar': cartesian_to_polar}, 'polar': {'cartesian': polar_to_cartesian}}
    ...
    def pixelize(self, xl, xu, yl, yu, nx=None, ny=None, sampling=None):
        ...
        try:
            ...
            if sampling <= 1:
                (xg, yg) = np.mgrid[xl:xu:complex(0.0, 1.0) * nx, yl:yu:complex(0.0, 1.0) * ny]
                return self((xg, yg)) * da
            (xg, yg) = np.mgrid[xl:xu:complex(0.0, 1.0) * nx * sampling, yl:yu:complex(0.0, 1.0) * ny * sampling]
            res = self((xg, yg))
    ...
    def getFluxSize(self, flux=0.5, frac=True, mode='radial', cen=(0, 0), v0=1, minfunc='fmin', intkwargs=None, **kwargs):
        """
    ...
    def plot(self, datarange=None, nx=100, ny=100, clf=True, cb=True, data='auto', log=False, **kwargs):
        """
        ...
        with mpl_context(clf=clf) as (plt):
            ...
            if data:
                ...
                if datarange is None:
                    datarange = (
                     np.min(xd), np.max(xd), np.min(yd), np.max(yd))
                maxmind = (
                 np.max(dataval), np.min(dataval))
            elif datarange is None:
                if self.rangehint is not None:
                    datarange = self.rangehint
                else:
                    raise ValueError("Can't choose limits for plotting without data or a range hint")
            grid = np.mgrid[datarange[0]:datarange[1]:complex(0.0, 1.0) * nx, datarange[2]:datarange[3]:complex(0.0, 1.0) * ny]
            res = self(grid)
            if log:
...
class FunctionModel2DScalarSeperable(FunctionModel2DScalar):
    ...
    def __getattr__(self, name):
        if name in self.params:
            ...
            if hasattr(self._thetamodel, name):
...
def list_models(include=None, exclude=None, baseclass=None, showabstract=False):
    ...
    res = [ k for (k, c) in __model_registry.iteritems() if baseclass is None or issubclass(c, baseclass) if showabstract or not isabstract(c)
          ]
...
def intersect_models(m1, m2, bounds=None, nsample=1024, full_output=False, **kwargs):
    ...
    transis = np.convolve(diff > 0, [1, -1])
    transis[0] = 0
    transis[-1] = 0
    inters = []
    reses = []
...
def scale_model(model, scaleparname='A'):
    ...
    return compclass((model, 'constant'), operation='*', parnames={'C1': scaleparname})
...
def offset_model(model, offsetparname='C'):
    ...
    if isinstance(model, FunctionModel1D):
        ...
        compclass = CompositeModel
    return compclass((model, 'constant'), operation='+', parnames={'C1': offsetparname})
...
def scale_and_offset_model(model, scaleparname='A', offsetparname='C'):
    ...
    if isinstance(model, FunctionModel1D):
        ...
        compclass = CompositeModel
    return compclass((model, 'constant', 'constant'), operation=['*', '+'], parnames={'C1': scaleparname, 'C2': offsetparname})