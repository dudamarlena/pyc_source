...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/pacemaker.py
...
class Schema(object):
    ...
    def tmp_schema_f(self):
        ...
        for schema_doc_name in self.schema_str_docs:
            ...
            fd = os.open(schema_doc_filename, os.O_RDWR | os.O_CREAT | os.O_TRUNC, 420)
...
class RngSchema(Schema):
    ...
    def find_start_rng_node(self, validate_name, filename):
        ...
        crm_schema = self.get_schema_fn(validate_name, os.path.join(self.local_dir, filename))
        if not crm_schema:
    ...
    def rng_xpath(self, xpath, namespaces=None):
        return [ grammar.xpath(xpath, namespaces=namespaces) for grammar, _ in self.rng_docs.values()
               ]
    ...
    def get_sub_rng_nodes(self, grammar, rng_node):
        ...
        for child_node in rng_node.iterchildren():
            ...
            elif local_tag in ('interleave', 'optional', 'zeroOrMore', 'choice', 'group',
                               'oneOrMore'):
        ...
        if rng_node is None:
            ...
            for node in sub_rng_nodes:
                ...
                if selected.count(name):
                    continue
                optional = any(self.find_decl(node, opt) != 0 for opt in ('optional',
                                                                          'zeroOrMore'))
