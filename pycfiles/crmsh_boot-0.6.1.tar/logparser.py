...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/logparser.py
# Compiled at: 2016-05-20 12:15:05
import bz2, gzip, re, os, sys, collections, json, time
...
class Transition(object):
    __slots__ = ('loc', 'dc', 'start_ts', 'trans_num', 'pe_file', 'pe_num', 'end_ts',
                 'end_state', 'end_actions', 'tags')
...
class LogParser(object):
    ...
    def get_events(self, event=None, nodes=None, resources=None):
        ...
        if nodes:
            ...
            if resources:
                ...
                def clonify(r):
                    if r in self.cib.cloned_resources:
                        return r + '(?::[0-9]+)?'
                    return r
    ...
    def _build_re(self, etype, args):
        ...
        if not patt_l:
            ...
            if not args:
                ...
                re_l = mk_re_list(patt_l, '(%s)' % ('|').join(args))
    ...
    def to_dict(self):
        ...
        """
        o = {'version': _METADATA_VERSION, 
           'events': self.events, 
           'transitions': [ t.to_dict() for t in self.transitions ], 'cib': {'nodes': self.cib.nodes, 
    ...
    def count(self):
        ...
        """
        return (
         len(self.transitions), sum(len(e) for e in self.events.values()))
