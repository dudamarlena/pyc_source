...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/utils.py
# Compiled at: 2016-05-20 12:10:07
import os, sys
from tempfile import mkstemp
import subprocess, re, glob, time, datetime, shutil, bz2, fnmatch, gc
...
def verify_boolean(opt):
    return opt.lower() in ('yes', 'true', 'on', '1') or opt.lower() in ('no', 'false',
                                                                        'off', '0')
...
def filter_string(cmd, s, stderr_on=True, shell=True):
    ...
    if options.regression_tests:
        print '.EXT', cmd
    p = subprocess.Popen(cmd, shell=shell, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=stderr)
    try:
        ret = p.communicate(s)
        if stderr_on == 'stdout':
            outp = ('\n').join(ret)
...
def get_stdout(cmd, input_s=None, stderr_on=True, shell=True):
    ...
    if options.regression_tests:
        print '.EXT', cmd
    proc = subprocess.Popen(cmd, shell=shell, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=stderr)
    stdout_data, stderr_data = proc.communicate(input_s)
    return (
     proc.returncode, stdout_data.strip())
...
def get_stdout_stderr(cmd, input_s=None, shell=True):
    ...
    if options.regression_tests:
        print '.EXT', cmd
    proc = subprocess.Popen(cmd, shell=shell, stdin=input_s and subprocess.PIPE or None, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout_data, stderr_data = proc.communicate(input_s)
    return (
     proc.returncode, stdout_data.strip(), stderr_data.strip())
...
def run_ptest(graph_s, nograph, scores, utilization, actions, verbosity):
    ...
    if re.search('simulate', ptest) and not re.search('-[RS]', ptest):
        ptest = '%s -S' % ptest
    if verbosity:
        if actions:
...
def cluster_stack():
    if is_process('heartbeat:.[m]aster'):
        return 'heartbeat'
    if is_process('[a]isexec'):
        return 'openais'
    if os.path.exists('/etc/corosync/corosync.conf') or is_program('corosync-cfgtool'):
        return 'corosync'
    return ''
...
def cibadmin_features():
    ...
    if rc == 0:
        m = re.match('Pacemaker\\s(\\S+)\\s\\(Build: ([^\\)]+)\\):\\s(.*)', outp.strip())
...
def list_corosync_nodes():
    ...
    try:
        cfg = os.getenv('COROSYNC_MAIN_CONFIG_FILE', '/etc/corosync/corosync.conf')
        lines = open(cfg).read().split('\n')
        addr_re = re.compile('\\s*ring0_addr:\\s+(.*)')
        nodes = []
        for line in lines:
...
def cluster_copy_file(local_path, nodes=None):
    ...
    ok = True
    for host, result in parallax.copy(nodes, local_path, local_path, opts).iteritems():
        if isinstance(result, parallax.Error):
