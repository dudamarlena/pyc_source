...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/cibconfig.py
# Compiled at: 2016-05-11 07:50:35
import copy
from lxml import etree
import os, sys, re, fnmatch, time, collections
from . import config
from . import options
...
class CibObjectSet(object):
            ...
            while not rc:
                ...
                if hash(s) != filehash:
                    ...
                    if not ok and config.core.force:
                        common_err('Save failed and --force is set, ' + 'aborting edit to avoid infinite loop')
    ...
    def _check_unique_clash(self, set_obj_all):
        ...
        check_set = set([ o.obj_id for o in self.obj_set if o.obj_type == 'primitive'
                        ])
        if not check_set:
            return 0
...
class CibObjectSetCli(CibObjectSet):
    ...
    def repr(self, format_mode=1):
        ...
        if not self.obj_set:
            return ''
        return ('\n').join(obj.repr_cli(format_mode=format_mode) for obj in processing_sort_cli(list(self.obj_set)))
...
class CibObject(object):
    ...
    def _get_oldnode(self):
        ...
        if self.obj_type in constants.defaults_tags:
    ...
    def find_child_in_node(self, child):
        for c in self.node.iterchildren():
            if c.tag == child.node.tag and c.get('id') == child.obj_id:
                return c
...
class CibPrimitive(CibObject):
    ...
    Primitives.
    """
    set_names = {'instance_attributes': 'params', 
    ...
    def check_sanity(self):
        ...
        if self.node is None:
            ...
            rc1 = ra.sanity_check_params(self.obj_id, params, existence_only=self.obj_type != 'primitive')
...
class CibContainer(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        children = []
        for c in self.node.iterchildren():
            if self.obj_type == 'group' and is_primitive(c) or is_child_rsc(c):
    ...
    def repr_gv(self, gv_obj, from_grp=False):
        ...
        if self.obj_type != 'group':
            return
        sg_obj = gv_obj.group([ x.obj_id for x in self.children ], 'cluster_%s' % self.obj_id)
...
class CibLocation(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        ...
        if 'rsc' in self.node.keys():
            ...
            if 'rsc-pattern' in self.node.keys():
...
class CibSimpleConstraint(CibObject):
    ...
    def _mk_one_edge(self, gv_obj, e):
        ...
        e = [
         gv_last_rsc(e[0]), gv_first_rsc(e[1])]
        e_id = gv_obj.new_edge(e)
...
class CibFencingOrder(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        ...
        for c in self.node.iterchildren('fencing-level'):
            if 'target-attribute' in c.attrib:
                target = (
                 c.get('target-attribute'), c.get('target-value'))
            else:
                target = c.get('target')
            if target not in d:
                d[target] = {}
        ...
        return cli_format([s] + [ '%s %s' % (fmt_target(x), (' ').join(dd[x])) for x in dd.keys()
    ...
    def check_sanity(self):
        ...
        if self.node is None:
            ...
            stonith_rsc_l = [ x.obj_id for x in cib_factory.get_elems_on_type('type:primitive') if is_stonith_rsc(x.node) ]
...
class CibTag(CibObject):
    ...
    def _repr_cli_head(self, fmt):
        return (' ').join([clidisplay.keyword(self.obj_type),
         clidisplay.ident(self.obj_id)] + [ clidisplay.rscref(c.get('id')) for c in self.node.iterchildren() if not is_comment(c)
                                          ])
...
class CibAlert(CibObject):
    ...
    def _repr_cli_head(self, fmt):
        ret = [
         clidisplay.keyword(self.obj_type),
...
cib_object_map = {'node': (
          'node', CibNode, 'nodes'), 
   'primitive': (
               'primitive', CibPrimitive, 'resources'), 
   'group': (
           'group', CibContainer, 'resources'), 
   'clone': (
           'clone', CibContainer, 'resources'), 
   'master': (
            'ms', CibContainer, 'resources'), 
   'template': (
              'rsc_template', CibPrimitive, 'resources'), 
   'rsc_location': (
                  'location', CibLocation, 'constraints'), 
   'rsc_colocation': (
                    'colocation', CibSimpleConstraint, 'constraints'), 
   'rsc_order': (
               'order', CibSimpleConstraint, 'constraints'), 
   'rsc_ticket': (
                'rsc_ticket', CibRscTicket, 'constraints'), 
   'cluster_property_set': (
                          'property', CibProperty, 'crm_config', 'cib-bootstrap-options'), 
   'rsc_defaults': (
                  'rsc_defaults', CibProperty, 'rsc_defaults', 'rsc-options'), 
   'op_defaults': (
                 'op_defaults', CibProperty, 'op_defaults', 'op-options'), 
   'fencing-topology': (
                      'fencing_topology', CibFencingOrder, 'configuration'), 
   'acl_role': (
              'role', CibAcl, 'acls'), 
   'acl_user': (
              'user', CibAcl, 'acls'), 
   'acl_target': (
                'acl_target', CibAcl, 'acls'), 
   'acl_group': (
               'acl_group', CibAcl, 'acls'), 
   'tag': (
         'tag', CibTag, 'tags'), 
   'alert': (
           'alert', CibAlert, 'alerts')}
...
class CibDiff(object):
    ...
    def add(self, item):
        ...
        if obj_id is None:
            common_err('element %s has no id!' % etree.tostring(item, pretty_print=True))
            return False
        else:
            if is_node and obj_id in self._node_set:
                common_err('Duplicate node: %s' % obj_id)
                return False
            if not is_node and obj_id in self._rsc_set:
    ...
    def _obj_nodes(self):
        return orderedset.oset([ n for n in self.objset.obj_ids if self._is_node(n)
                               ])

    def _obj_resources(self):
        return orderedset.oset([ n for n in self.objset.obj_ids if self._is_resource(n)
                               ])
    ...
    def _is_edit_valid(self, id_set, existing):
        ...
        rscstat = RscState()
        if not_allowed:
            common_err('Elements %s already exist' % (', ').join(list(not_allowed)))
            rc = False
        delete_set = existing - id_set
        cannot_delete = [ x for x in delete_set if not rscstat.can_delete(x)
                        ]
        if cannot_delete:
            common_err('Cannot delete running resources: %s' % (', ').join(cannot_delete))
            rc = False
        return rc
...
class CibFactory(object):
    ...
    def _copy_cib_attributes(self, src_cib, cib):
        ...
        """
        attrs = ((attr, src_cib.get(attr)) for attr in self.cib_attrs if attr not in constants.cib_user_attrs)
    ...
    def is_current_cib_equal(self, silent=False):
        ...
        if cib_elem is None:
            ...
            rc = self._attr_match(cib_elem, 'epoch') and self._attr_match(cib_elem, 'admin_epoch')
    ...
    def _state_header(self):
        ...
        print CibObject.state_fmt % ('', 'origin', 'updated', 'parent', 'children')
    ...
    def _patch_cib(self, force):
        ...
        if current_cib is None:
            ...
            rc, cib_diff = filter_string('%s -o %s -n -' % (
             self._crm_diff_cmd, tmpf), etree.tostring(self.cib_elem))
    ...
    def _push_state(self):
        ...
        """
        self._state.append([ copy.deepcopy(x) for x in (
         self.cib_elem,
         self.cib_attrs,
         self.cib_objects,
         self.remove_queue,
    ...
    def _pop_state(self):
        try:
            ...
            self.cib_elem, self.cib_attrs, self.cib_objects, self.remove_queue, self.id_refs = self._state.pop()
        except KeyError:
            return False
    ...
    def rsc_id_list(self):
        ...
        return [ x.obj_id for x in self.cib_objects if x.obj_type in constants.resource_tags
               ]

    def top_rsc_id_list(self):
        """List of top resource ids (for constraint completion)."""
        return [ x.obj_id for x in self.cib_objects if x.obj_type in constants.resource_tags and not x.parent
               ]

    def node_id_list(self):
        """List of node ids."""
        return sorted([ x.node.get('uname') for x in self.cib_objects if x.obj_type == 'node'
                      ])

    def f_prim_free_id_list(self):
        """List of possible primitives ids (for group completion)."""
        return [ x.obj_id for x in self.cib_objects if x.obj_type == 'primitive' and not x.parent
               ]

    def f_group_id_list(self):
        """List of group ids."""
        return [ x.obj_id for x in self.cib_objects if x.obj_type == 'group'
               ]

    def rsc_template_list(self):
        """List of templates."""
        return [ x.obj_id for x in self.cib_objects if x.obj_type == 'rsc_template'
               ]

    def f_children_id_list(self):
        """List of possible child ids (for clone/master completion)."""
        return [ x.obj_id for x in self.cib_objects if x.obj_type in constants.children_tags and not x.parent
               ]
    ...
    def modified_elems(self):
        return [ x for x in self.cib_objects if x.updated or x.origin == 'user'
               ]
    ...
    def _verify_rsc_children(self, obj):
        ...
        for other in [ x for x in self.cib_objects if x != obj and is_container(x.node) ]:
    ...
    def _cli_set_update(self, edit_d, mk_set, upd_set, del_set, method):
        ...
        if not self.delete(*del_constraints):
            ...
            for cli in processing_sort([ edit_d[x] for x in mk_set ]):
                ...
                if not obj:
                    common_debug("create_from_cli '%s' failed" % etree.tostring(cli, pretty_print=True))
                    return False
                test_l.append(obj)
    ...
    def _add_element(self, obj, node):
        if not node is not None:
            ...
            return self._adjust_children(obj) or None
        else:
            pnode.append(node)
    ...
    def delete(self, *args):
        ...
        for obj_id in args:
            ...
            if is_template(obj.node):
                ...
                prim_l = [ x for x in prim_l if x not in l and x.obj_id not in args
                         ]
