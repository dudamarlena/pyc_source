...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_node.py
...
def unpack_node_xmldata(node, is_offline):
    ...
    inst_attr = [ cli_nvpairs(nvpairs2list(elem)) for elem in node.xpath('./instance_attributes')
                ]
    return (
     uname, ident, typ, other, inst_attr, is_offline)
...
class NodeMgmt(command.UI):
    ...
    dc = 'crmadmin -D'
    node_attr = {'set': "crm_attribute -t nodes -N '%s' -n '%s' -v '%s'", 
       'delete': "crm_attribute -D -t nodes -N '%s' -n '%s'", 
       'show': "crm_attribute -G -t nodes -N '%s' -n '%s'"}
    node_status = {'set': "crm_attribute -t status -N '%s' -n '%s' -v '%s'", 
       'delete': "crm_attribute -D -t status -N '%s' -n '%s'", 
       'show': "crm_attribute -G -t status -N '%s' -n '%s'"}
    node_utilization = {'set': "crm_attribute -z -t nodes -N '%s' -n '%s' -v '%s'", 
    ...
    def do_status(self, context, node=None):
        """usage: status [<node>]"""
        a = node and '--xpath "//nodes/node[@uname=\'%s\']"' % node or '-o nodes'
    ...
    def do_fence(self, context, node):
        ...
        if not utils.is_name_sane(node):
            ...
            if not config.core.force and not utils.ask('Fencing %s will shut down the node and migrate any resources that are running on it! Do you want to fence %s?' % (node, node)):
                return False
    ...
    def do_clearstate(self, context, node=None):
        ...
        if not utils.is_name_sane(node):
            ...
            if not config.core.force and not utils.ask('Do you really want to drop state for node %s?' % node):
                return False
            if utils.is_pcmk_118():
                ...
                return utils.ext_cmd(self.node_clear_state % ('-M -c', node, node)) == 0 and utils.ext_cmd(self.node_clear_state % ('-R', node, node)) == 0
            return
    ...
    def do_delete(self, context, node):
        ...
        if not self._call_delnode(node):
            return False
        if utils.ext_cmd(self.node_delete % node) != 0 or utils.ext_cmd(self.node_delete_status % node) != 0:
    ...
    def do_attribute(self, context, node, cmd, rsc, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.node_attr, node, cmd, rsc, value)
    ...
    def do_utilization(self, context, node, cmd, rsc, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.node_utilization, node, cmd, rsc, value)
    ...
    def do_status_attr(self, context, node, cmd, rsc, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.node_status, node, cmd, rsc, value)
