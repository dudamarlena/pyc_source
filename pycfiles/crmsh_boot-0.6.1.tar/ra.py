...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ra.py
...
class RaOS(object):
    ...
    def types(self, ra_class='ocf', ra_provider=''):
        ...
        if ra_class == 'ocf':
            ...
            l = [ x.replace('check_', '') for x in os_types_list('%s/check_*' % config.path.nagios_plugins) ]
...
def ra_providers_all(ra_class='ocf'):
    ...
    if os.path.isdir(ocf):
        return cache.store(ident, sorted(s for s in os.listdir(ocf) if os.path.isdir(os.path.join(ocf, s))))
    return []
...
def mk_monitor_name(role, depth):
    ...
    return role and role != 'Started' and 'monitor_%s%s' % (role, depth) or 'monitor%s' % depth
...
class RAInfo(object):
    ...
    def ra_string(self):
        return self.ra_class == 'ocf' and '%s:%s:%s' % (self.ra_class, self.ra_provider, self.ra_type) or '%s:%s' % (self.ra_class, self.ra_type)
    ...
    def param_type_default(self, n):
        try:
            ...
            return (
             content.get('type'), content.get('default'))
        except:
            return (None, None)
    ...
    def completion_params(self):
        ...
        if self.mk_ra_node() is None:
            ...
            return [ c.get('name') for c in self.ra_elem.xpath('//parameters/parameter') if c.get('name') and c.get('name') not in self.excluded_from_completion ]
        ...
        if cache.is_cached(ident):
            ...
            for op in d.keys():
                if re.match('monitor_[^0-9]', op):
                    norole_op = re.sub('monitor_[^0-9_]+_(.*)', 'monitor_\\1', op)
    ...
    def unreq_param(self, p):
        ...
        """
        if self.ra_class == 'stonith' and (self.ra_type.startswith('rhcs/') or self.ra_type.startswith('fence_')):
            if p in ('action', 'port'):
                return True
        return False
...
def validate_agent(agentname, params):
    ...
    if not can_validate_agent(agentname):
        ...
        cmd = [
         os.path.join(config.path.ocf_root, 'resource.d', agent.ra_provider, agent.ra_type), 'validate-all']
        if options.regression_tests:
            print '.EXT', (' ').join(cmd)
