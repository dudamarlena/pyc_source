...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/corosync.py
# Compiled at: 2016-05-25 05:16:10
...
def check_tools():
    return all(utils.is_program(p) for p in ['corosync-cfgtool', 'corosync-quorumtool', 'corosync-cmapctl'])
...
def make_section(path, contents=None):
    ...
    return [
     Token(_tBEGIN, sp, name)] + contents + [Token(_tEND, [])]
...
class Parser(object):
    ...
    def set(self, path, value):
        ...
        if i < 0:
            spath = path.split('.')
            return self.add(('.').join(spath[:-1]), make_value(path, value))
...
def add_node(addr, name=None):
    ...
    if addr in nodenames + coronodes or ipaddr and ipaddr in coronodes:
        ...
        node_name = name
        node_value = make_value('nodelist.node.ring0_addr', node_addr) + make_value('nodelist.node.nodeid', str(node_id))
        if node_name:
...
def del_node(addr):
    ...
    if nodes:
        utils.ext_cmd(['corosync-cmapctl', '-D', 'nodelist.node.%s.nodeid' % nth], shell=False)
        utils.ext_cmd(['corosync-cmapctl', '-D', 'nodelist.node.%s.ring0_addr' % nth], shell=False)
        utils.ext_cmd(['corosync-cmapctl', '-D', 'nodelist.node.%s.name' % nth], shell=False)