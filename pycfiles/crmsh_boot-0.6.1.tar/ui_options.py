...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_options.py
...
_legacy_map = {'editor': ('core', 'editor'), 
   'pager': ('core', 'pager'), 
   'user': ('core', 'user'), 
...
def _getprefs(opt):
    ...
    opt = opt.replace('-', '_')
    if opt == 'colorscheme':
        return ('black', 'blue', 'green', 'cyan', 'red', 'magenta', 'yellow', 'white',
                'normal')
