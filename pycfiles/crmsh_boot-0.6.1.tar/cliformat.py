...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/cliformat.py
...
def cli_nvpair(nvp):
    ...
    if idref is not None:
        ...
        if nodeid is not None and cib_factory.is_id_refd(nvp.tag, nodeid):
...
def cli_op(node):
    ...
    if not action:
        return ''
    ret = [
     '%s %s' % (clidisplay.keyword('op'), action)]
    ...
    if operation in utils.olist(constants.simple_date_ops):
        ...
        for c in node.iterchildren():
            if c.tag in ('duration', 'date_spec'):
                l.extend([ nvpair_format(name, c.get(name)) for name in c.keys() if name != 'id'
                         ])
...
def rsc_set_constraint(node, obj_type):
    ...
    if not is_location and (is_seq_all and not is_ticket and cnt <= 2 or is_ticket and cnt <= 1):
...
def build_exp_re(exp_l):
    return [ x.replace('@@', '([a-zA-Z_][a-zA-Z0-9_.-]*)') for x in exp_l ]
...
def cli_acl_permission(node):
    s = [
     clidisplay.keyword(node.get('kind'))]
