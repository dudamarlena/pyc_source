...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/command.py
...
def _cd_completer(args, context):
    ...
    if context.previous_level():
        ret += ['..']
    return ret + [ l for l in context.current_level().get_completions() if context.current_level().is_sublevel(l)
                 ]
    ...
    if found:
        ...
        def fuzzy_match(rx):
            try:
                ...
                matches = [ c for m, c in items.iteritems() if matcher.match(m)
                          ]
                if len(matches) == 1:
                    return matches[0]
...
class UI(object):
    ...
    def init_ui(cls):
        ...
        def get_if_command(attr):
            ...
            if attr.startswith('do_') and inspect.ismethod(child):
                return child
            else:
                return
        ...
        def add_help(info):
            ...
            if info.short_help:
                ...
                entry = help_module.HelpEntry('Help for command ' + info.name, 'Note: This command is not documented.\n' + 'Usage: %s %s' % (info.name,
                 ui_utils.pretty_arguments(info.function, nskip=2)), generated=True)
            elif info.type == 'level':
                entry = help_module.HelpEntry('Help for level ' + info.name, 'Note: This level is not documented.\n', generated=True)
...
def _check_args(fn, expected):
    ...
    if argnames != expected:
        raise ValueError(fn.__name__ + ': Expected method with signature ' + repr(expected))