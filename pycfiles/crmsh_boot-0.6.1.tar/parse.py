...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/parse.py
# Compiled at: 2016-05-11 07:50:35
import shlex, re, inspect
...
from . import xmlutil
_NVPAIR_RE = re.compile('([^=@$][^=]*)=(.*)$')
_NVPAIR_ID_RE = re.compile('\\$([^:=]+)(?::(.+))?=(.*)$')
_NVPAIR_REF_RE = re.compile('@([^:]+)(?::(.+))?$')
_NVPAIR_KEY_RE = re.compile('([^:=]+)$', re.IGNORECASE)
_IDENT_RE = re.compile('([a-z0-9_#$-][^=]*)$', re.IGNORECASE)
_DISPATCH_RE = re.compile('[a-z0-9_]+$', re.IGNORECASE)
_DESC_RE = re.compile('description=(.+)$', re.IGNORECASE)
_ATTR_RE = re.compile('\\$?([^=]+)=(.*)$')
_ALERT_PATH_RE = re.compile('(.*)$')
_RESOURCE_RE = re.compile('([a-z_#$][^=]*)$', re.IGNORECASE)
_IDSPEC_RE = re.compile('(\\$id-ref|\\$id)=(.*)$', re.IGNORECASE)
_ID_RE = re.compile('\\$id=(.*)$', re.IGNORECASE)
_ID_NEW_RE = re.compile('([\\w-]+):$', re.IGNORECASE)
_SCORE_RE = re.compile('([^:]+):$')
_ROLE_RE = re.compile('\\$?role=(.+)$', re.IGNORECASE)
_BOOLOP_RE = re.compile('(%s)$' % ('|').join(constants.boolean_ops), re.IGNORECASE)
_UNARYOP_RE = re.compile('(%s)$' % ('|').join(constants.unary_ops), re.IGNORECASE)
_ACL_RIGHT_RE = re.compile('(%s)$' % ('|').join(constants.acl_rule_names), re.IGNORECASE)
_ROLE_REF_RE = re.compile('role:(.+)$', re.IGNORECASE)
_PERM_RE = re.compile('([^:]+)(?::(.+))?$', re.I)
_UNAME_RE = re.compile('([^:]+)(:(normal|member|ping|remote))?$', re.IGNORECASE)
_TEMPLATE_RE = re.compile('@(.+)$')
_RA_TYPE_RE = re.compile('[a-z0-9_:-]+$', re.IGNORECASE)
_TAG_RE = re.compile('([a-zA-Z_][^\\s:]*):?$')
_ROLE2_RE = re.compile('role=(.+)$', re.IGNORECASE)
_TARGET_RE = re.compile('([\\w=-]+):$')
_TARGET_ATTR_RE = re.compile('attr:([\\w-]+)=([\\w-]+)$', re.IGNORECASE)
...
class Validation(object):
    ...
    def classify_role(self, role):
        if not role:
            ...
            if role in olist(self.resource_roles()):
    ...
    def classify_action(self, action):
        if not action:
            ...
            if action in olist(self.resource_actions()):
...
class BaseParser(object):
    ...
    def match_nvpairs_bykey(self, valid_keys, minpairs=1):
        ...
        """
        _KEY_RE = re.compile('(%s)=(.+)$' % ('|').join(valid_keys))
        _NOVAL_RE = re.compile('(%s)$' % ('|').join(valid_keys))
        ret = []
        while True:
    ...
    def match_nvpairs(self, terminator=None, minpairs=1):
        ...
        while True:
            ...
            if tok is not None and tok.lower() in terminator:
                ...
                ret.append(xmlutil.nvpair_ref(self.matched(1), self.matched(2)))
            elif self.try_match(_NVPAIR_ID_RE):
                ret.append(xmlutil.nvpair_id(self.matched(1), self.matched(2), self.matched(3)))
    ...
    def match_attr_lists(self, name_map, implicit_initial=None, terminator=None):
        ...
        if self.try_match(to_match):
            ...
            if implicit_initial is not None:
                attrs = self.match_attr_list(implicit_initial, name_map[implicit_initial], allow_empty=False, terminator=terminator)
    ...
    def _match_simple_exp(self):
        if self.try_match('date'):
            ...
            val = self.match_any()
            node = xmlutil.new('expression', operation=binop, attribute=attr, value=val)
    ...
    def validate_score(self, score, noattr=False):
        if not noattr and score in olist(constants.score_types):
            ...
            if re.match('^[+-]?(inf(inity)?|INF(INITY)?|[0-9]+)$', score):
    ...
    def match_arguments(self, out, name_map, implicit_initial=None, terminator=None):
        ...
        while self.has_tokens():
            ...
            if t in names:
                ...
                initial = False
                for attr_list in self.match_attr_lists(name_map, implicit_initial=implicit_initial, terminator=terminator):
                    out.append(attr_list)
...
def parse_node(self, cmd):
    ...
    self.match_arguments(out, {'attributes': 'instance_attributes', 'utilization': 'utilization'}, implicit_initial='attributes')
    return out
...
class ResourceParser(BaseParser):
    ...
    def _primitive_or_template(self):
        ...
        xmlutil.maybe_set(out, 'description', self.try_match_description())
        self.match_arguments(out, {'params': 'instance_attributes', 'meta': 'meta_attributes', 
    ...
    def _master_or_clone(self):
        ...
        xmlutil.maybe_set(out, 'description', self.try_match_description())
        self.match_arguments(out, {'params': 'instance_attributes', 'meta': 'meta_attributes'}, implicit_initial='params')
        out.append(child)
        return out
...
class ConstraintParser(BaseParser):
    ...
    def parse_order(self):
        ...
        if self.try_match('(%s):$' % ('|').join(validator.rsc_order_kinds())):
            out.set('kind', validator.canonize(self.matched(1), validator.rsc_order_kinds()))
    ...
    def try_match_rscset(self, out, suffix_type, simple_count=2):
        ...
        if simple:
            ...
            self.err('Expected %s | resource_sets' % (' ').join(repeat('<rsc>[:<%s>]' % suffix_type, simple_count)))
    ...
    def match_resource_set(self, suffix_type, simple_count=2):
        ...
        if self.remaining_tokens() == simple_count:
            ...
            return (
             True, self.match_simple_action_set())
...
def property_parser(self, cmd):
    ...
    """
    from .cibconfig import cib_factory
    setmap = {'property': 'cluster_property_set', 'rsc_defaults': 'meta_attributes', 
    ...
    def _postprocess_levels(self, raw_levels):
        ...
        for target, devices in lvl_generator():
            if isinstance(target, tuple):
                c = xmlutil.child(out, 'fencing-level', index=str(targets[target[0]]), devices=devices)
                c.set('target-attribute', target[0])
                c.set('target-value', target[1])
                targets[target[0]] += 1
            else:
                xmlutil.child(out, 'fencing-level', target=target, index=str(targets[target]), devices=devices)
                targets[target] += 1
...
def parse_alert(self, cmd):
    ...
    while self.has_tokens():
        if self.current_token() in ('attributes', 'meta'):
            self.match_arguments(out, {'attributes': 'instance_attributes', 'meta': 'meta_attributes'}, terminator=[
             'attributes', 'meta', 'to'])
            continue
        self.match('to')
        ...
        self.match_arguments(elem, {'attributes': 'instance_attributes', 'meta': 'meta_attributes'}, terminator=terminators)
        if bracer:
            self.match('}')
        out.append(elem)
...
class ResourceSet(object):
    ...
    def parseattr(self, p, tokpos):
        ...
        l = p.split('=')
        if len(l) != 2:
            self.err('Extra = in %s' % p, token=self.tokens[tokpos])
        if l[0] not in attrs:
            self.err('Unknown attribute', token=self.tokens[tokpos])
        k, v = l
        if not verify_boolean(v):
            self.err('Not a boolean: %s' % v, token=self.tokens[tokpos])
    ...
    def splitrsc(self, p):
        ...
        if len(l) == 2:
            if self.q_attr == 'action':
                l[1] = validator.canonize(l[1], validator.resource_actions())
            else:
                l[1] = validator.canonize(l[1], validator.resource_roles())
            if not l[1]:
    ...
    def update_attrs(self, bracket, tokpos):
        if bracket in ('(', '['):
            if self.opened:
                self.err('Cannot nest resource sets', token=self.tokens[tokpos])
            ...
            if not self.opened:
                self.err('Unmatched closing bracket', token=self.tokens[tokpos])
            if bracket != self.matching[self.opened]:
                self.err('Mismatched closing bracket', token=self.tokens[tokpos])
            self.sequential = True
            self.require_all = True
            self.opened = ''
    ...
    def parse(self):
        ...
        for p in self.tokens:
            ...
            if p in self.close_set:
                if not len(self.set_pl):
                    self.err('Empty resource set', token=self.tokens[tokpos])
                self.save_set()
                self.update_attrs(p, tokpos)
                continue
            if '=' in p:
        ...
        if self.opened:
            self.err('Unmatched opening bracket', token=self.tokens[tokpos])
        if len(self.set_pl):
            self.save_set()
        ret = self.cli_list
        self.cli_list = []
