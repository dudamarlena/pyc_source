...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/help.py
...
class HelpFilter(object):
    ...
    def _filter(self, line):
        ...
        if block_edge and not self.in_block:
            ...
            if block_edge and self.in_block:
                self.in_block = False
                return ''
            if not self.in_block:
                if self._TOPIC2.match(line):
                    return clidisplay.help_topic(line[1:])
                line = self._QUOTED.sub(clidisplay.help_keyword('\\1'), line)
                line = self._MONO.sub(clidisplay.help_block('\\1'), line)
                line = self._TOPIC.sub(clidisplay.help_topic('\\1'), line)
                return line
...
def _load_help():
    ...
    if _LOADED:
        ...
        """
            return _REFERENCE_RE.sub('\\1', line)
