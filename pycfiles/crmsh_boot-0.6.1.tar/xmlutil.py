...
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/xmlutil.py
# Compiled at: 2016-05-26 00:38:21
...
def cibdump2elem(section=None):
    ...
    if rc == 0:
        ...
        if rc != constants.cib_no_section_rc:
...
def is_crmuser():
    crmusers = (
     'root', config.path.crm_daemon_user)
...
attr_defaults = {'rule': (('boolean-op', 'and'), ), 
...
def is_cloned(node):
    return node.getparent().tag in constants.clonems_tags or node.getparent().tag == 'group' and node.getparent().getparent().tag in constants.clonems_tags
...
def rsc_constraint(rsc_id, con_elem):
    for attr in con_elem.keys():
        if attr in constants.constraint_rsc_refs and rsc_id == con_elem.get(attr):
            return True
...
def sort_container_children(e_list):
    ...
    """
    for node in e_list:
        children = [ x for x in node.iterchildren() if x.tag in constants.children_tags
                   ]
        for c in children:
            node.remove(c)
...
match_list = defaultdict(tuple, {'node': ('uname', ), 'nvpair': ('name', ), 
   'op': ('name', 'interval'), 
...
def find_operation(rsc_node, name, interval=None):
    ...
    for ops in rsc_node.findall('operations'):
        matching_name.extend([ op for op in ops.iterchildren('op') if op.get('name') == name
                             ])
...
_sort_xml_order = make_sort_map('node', 'template', 'primitive', 'group', 'master', 'clone', 'op', 'tag', [
 'rsc_location', 'rsc_colocation', 'rsc_order'], [
 'rsc_ticket', 'fencing-topology'], 'cluster_property_set', 'rsc_defaults', 'op_defaults', 'acl_role', ['acl_target', 'acl_group', 'acl_user'], 'alert')
_sort_cli_order = make_sort_map('node', 'rsc_template', 'primitive', 'group', [
 'ms', 'master'], 'clone', 'op', 'tag', [
 'location', 'colocation', 'collocation', 'order'], [
 'rsc_ticket', 'fencing_topology'], 'property', 'rsc_defaults', 'op_defaults', 'role', ['acl_target', 'acl_group', 'user'], 'alert')
_SORT_LAST = 1000
...
def processing_sort(nl):
    ...
    if config.core.sort_elements:
        sortfn = lambda k: (
         _sort_xml_order.get(k.tag, _SORT_LAST), k.get('id'))
    else:
...
def processing_sort_cli(cl):
    ...
    if config.core.sort_elements:
        sortfn = lambda k: (
         _sort_cli_order.get(k.obj_type, _SORT_LAST), k.obj_id)
    else:
...
def referenced_resources(node):
    if not is_constraint(node):
        ...
        if xml_obj_type == 'rsc_location' and node.get('rsc'):
            ...
            rsc_list = [ x.get('id') for x in node.xpath('resource_set/resource_ref')
                       ]
        elif xml_obj_type == 'rsc_colocation':
            rsc_list = [
             node.get('rsc'), node.get('with-rsc')]
        elif xml_obj_type == 'rsc_order':
            rsc_list = [
             node.get('first'), node.get('then')]
...
def rename_rscref_simple(c_obj, old_id, new_id):
    ...
    for attr in c_obj.node.keys():
        if attr in constants.constraint_rsc_refs and c_obj.node.get(attr) == old_id:
...
def delete_rscref_simple(c_obj, rsc_id):
    c_modified = False
    for attr in c_obj.node.keys():
        if attr in constants.constraint_rsc_refs and c_obj.node.get(attr) == rsc_id:
...
def rename_rscref(c_obj, old_id, new_id):
    if rename_rscref_simple(c_obj, old_id, new_id) or rename_rscref_rset(c_obj, old_id, new_id):
        err_buf.info('modified %s from %s to %s' % (str(c_obj), old_id, new_id))


def delete_rscref(c_obj, rsc_id):
    return delete_rscref_simple(c_obj, rsc_id) or delete_rscref_rset(c_obj, rsc_id)
...
def silly_constraint(c_node, rsc_id):
    ...
    if c_node.xpath('resource_set/resource_ref'):
        ...
        if c_node.tag in ('rsc_location', 'rsc_ticket'):
            return cnt < 1
        return cnt <= 1
    else:
        cnt = 0
...
def is_climove_location(node):
    ...
    return len(rule_l) == 1 and len(expr_l) == 1 and node.get('id').startswith('cli-') and expr_l[0].get('attribute') == '#uname' and expr_l[0].get('operation') == 'eq'
...
def is_pref_location(node):
    ...
    rule_l = node.findall('rule')
    expr_l = node.xpath('.//expression')
    return len(rule_l) == 1 and len(expr_l) == 1 and expr_l[0].get('attribute') == '#uname' and expr_l[0].get('operation') == 'eq'


def get_rsc_ref_ids(node):
    return [ x.get('id') for x in node.xpath('./resource_ref')
           ]


def get_rsc_children_ids(node):
    return [ x.get('id') for x in node.iterchildren() if is_child_rsc(x)
           ]
