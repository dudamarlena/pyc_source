...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/urlographer/views.py
# Compiled at: 2013-07-25 14:18:13
...
def route(request):
    ...
    try:
        url = URLMap.objects.cached_get(site, canonicalized, force_cache_invalidation=force_cache_invalidation(request))
    ...
    if handler:
        if callable(handler) or hasattr(handler, 'as_view'):
            ...
            raise ImproperlyConfigured('URLOGRAPHER_HANDLERS values must be views or import strings')
        if hasattr(view, 'as_view'):
...
def sitemap(request, invalidate_cache=False):
    r"""
    Constructs a `GenericSitemap <https://docs.djangoproject.com/en/dev/ref/    contrib/sitemaps/#django.contrib.sitemaps.GenericSitemap>`_ containing all
    ...
    response = contrib_sitemap(request, {'urlmap': GenericSitemap({'queryset': URLMap.objects.filter(site=site, status_code=200, on_sitemap=True)})})
    response.render()
