...
# Embedded file name: build/bdist.linux-x86_64/egg/profitbricks/client.py
# Compiled at: 2018-07-12 06:04:26
# Size of source mod 2**32: 97296 bytes
import getpass, json, logging, os, re, time, requests, six
try:
    import configparser
except ImportError:
...
from profitbricks import API_HOST, __version__
from profitbricks.errors import PBNotAuthorizedError, PBNotFoundError, PBValidationError, PBRateLimitExceededError, PBError, PBFailedRequest, PBTimeoutError
...
class ProfitBricksService(object):
    ...
    def __init__(self, username=None, password=None, host_base=API_HOST, host_cert=None, ssl_verify=True, headers=None, client_user_agent=None, use_config=True, use_keyring=HAS_KEYRING, config_filename=None):
        if headers is None:
            headers = dict()
        self._config = None
    ...
    def _read_config(self, filename=None):
        ...
        if filename:
            ...
            try:
                ...
                raise Exception("Missing dependency for determining config path. Please install the 'appdirs' Python module.")
            ...
            if HAS_KEYRING:
                ...
                if password is None:
                    ...
                    try:
                        ...
                        logger.warning("Storing password in keyring '%s' failed: %s", self.keyring_identificator, error)

            else:
                logger.warning("Install the 'keyring' Python module to store your password securely in your keyring!")
                password = self._config.get('credentials', 'password', fallback=None)
                if password is None:
                    password = getpass.getpass(question)
                    store_plaintext_passwords = self._config.get('preferences',
                      'store-plaintext-passwords', fallback=None)
                    if store_plaintext_passwords != 'no':
                        question = 'Do you want to store your password in plain text in ' + self._config_filename()
    ...
    def get_datacenter(self, datacenter_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s?depth=%s' % (datacenter_id, str(depth)))
        return response
    ...
    def get_datacenter_by_name(self, name, depth=1):
        ...
        if len(data_center) == 0:
            raise NameError("No data center found with name containing '{name}'.".format(name=name))
        if len(data_center) > 1:
            raise NameError("Found {n} data centers with the name '{name}': {names}".format(n=(len(data_center)),
              name=name,
    ...
    def delete_datacenter(self, datacenter_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s' % datacenter_id),
          method='DELETE')
        return response
    ...
    def create_datacenter(self, datacenter):
        ...
        loadbalancer_items = []
        entities = dict()
        properties = {'name': datacenter.name}
        if datacenter.location:
            ...
            if datacenter.description:
                ...
                if len(datacenter.servers) > 0:
                    ...
                    servers = {'items': server_items}
                    server_entities = {'servers': servers}
                    ...
                    if len(datacenter.loadbalancers) > 0:
                        for loadbalancer in datacenter.loadbalancers:
                            loadbalancer_items.append(self._create_loadbalancer_dict(loadbalancer))

                        loadbalancers = {'items': loadbalancer_items}
                        loadbalancer_entities = {'loadbalancers': loadbalancers}
                        entities.update(loadbalancer_entities)
                if len(datacenter.lans) > 0:
                    for lan in datacenter.lans:
                        lan_items.append(self._create_lan_dict(lan))

                    lans = {'items': lan_items}
                    lan_entities = {'lans': lans}
        ...
        data = json.dumps(raw)
        response = self._perform_request(url='/datacenters',
          method='POST',
          data=data)
        return response
    ...
    def get_firewall_rule(self, datacenter_id, server_id, nic_id, firewall_rule_id):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s/nics/%s/firewallrules/%s' % (
         datacenter_id,
         server_id,
         nic_id,
         firewall_rule_id))
    ...
    def get_firewall_rules(self, datacenter_id, server_id, nic_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s/nics/%s/firewallrules?depth=%s' % (
         datacenter_id,
         server_id,
         nic_id,
         str(depth)))
    ...
    def delete_firewall_rule(self, datacenter_id, server_id, nic_id, firewall_rule_id):
        """
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s/nics/%s/firewallrules/%s' % (
         datacenter_id,
         server_id,
         nic_id,
         firewall_rule_id)),
    ...
    def create_firewall_rule(self, datacenter_id, server_id, nic_id, firewall_rule):
        ...
        """
        properties = {'name': firewall_rule.name}
        ...
        response = self._perform_request(url=('/datacenters/%s/servers/%s/nics/%s/firewallrules' % (
         datacenter_id,
         server_id,
         nic_id)),
          method='POST',
    ...
    def update_firewall_rule(self, datacenter_id, server_id, nic_id, firewall_rule_id, **kwargs):
        """
    ...
    def delete_image(self, image_id):
        ...
        """
        response = self._perform_request(url=('/images/' + image_id), method='DELETE')
        return response
    ...
    def update_image(self, image_id, **kwargs):
        ...
        response = self._perform_request(url=('/images/' + image_id), method='PATCH',
          data=(json.dumps(data)))
        return response
    ...
    def get_lan(self, datacenter_id, lan_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/lans/%s?depth=%s' % (
         datacenter_id,
         lan_id,
         str(depth)))
        return response
    ...
    def list_lans(self, datacenter_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/lans?depth=%s' % (
         datacenter_id,
         str(depth)))
        return response
    ...
    def delete_lan(self, datacenter_id, lan_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/lans/%s' % (
    ...
    def create_lan(self, datacenter_id, lan):
        ...
        response = self._perform_request(url=('/datacenters/%s/lans' % datacenter_id),
          method='POST',
          data=data)
        return response

    def update_lan(self, datacenter_id, lan_id, name=None, public=None, ip_failover=None):
        """
        Updates a LAN
        ...
        response = self._perform_request(url=('/datacenters/%s/lans/%s' % (datacenter_id, lan_id)),
          method='PATCH',
          data=(json.dumps(data)))
        return response
    ...
    def get_lan_members(self, datacenter_id, lan_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/lans/%s/nics?depth=%s' % (
         datacenter_id,
         lan_id,
         str(depth)))
        return response
    ...
    def get_loadbalancer(self, datacenter_id, loadbalancer_id):
        ...
        """
        response = self._perform_request('/datacenters/%s/loadbalancers/%s' % (
    ...
    def list_loadbalancers(self, datacenter_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/loadbalancers?depth=%s' % (
    ...
    def delete_loadbalancer(self, datacenter_id, loadbalancer_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/loadbalancers/%s' % (
         datacenter_id, loadbalancer_id)),
          method='DELETE')
        return response
    ...
    def create_loadbalancer(self, datacenter_id, loadbalancer):
        ...
        response = self._perform_request(url=('/datacenters/%s/loadbalancers' % datacenter_id),
          method='POST',
          data=data)
        return response
    ...
    def update_loadbalancer(self, datacenter_id, loadbalancer_id, **kwargs):
        ...
        response = self._perform_request(url=('/datacenters/%s/loadbalancers/%s' % (datacenter_id,
         loadbalancer_id)),
          method='PATCH',
          data=(json.dumps(data)))
        return response

    def get_loadbalancer_members(self, datacenter_id, loadbalancer_id, depth=1):
        """
        ...
        """
        response = self._perform_request('/datacenters/%s/loadbalancers/%s/balancednics?depth=%s' % (
         datacenter_id, loadbalancer_id, str(depth)))
        return response

    def add_loadbalanced_nics(self, datacenter_id, loadbalancer_id, nic_id):
        """
        ...
        """
        data = '{ "id": "' + nic_id + '" }'
        response = self._perform_request(url=('/datacenters/%s/loadbalancers/%s/balancednics' % (
         datacenter_id,
         loadbalancer_id)),
          method='POST',
          data=data)
    ...
    def get_loadbalanced_nic(self, datacenter_id, loadbalancer_id, nic_id, depth=1):
        """
        ...
        """
        response = self._perform_request('/datacenters/%s/loadbalancers/%s/balancednics/%s?depth=%s' % (
         datacenter_id,
         loadbalancer_id,
         nic_id,
         str(depth)))
    ...
    def remove_loadbalanced_nic(self, datacenter_id, loadbalancer_id, nic_id):
        """
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/loadbalancers/%s/balancednics/%s' % (
         datacenter_id,
         loadbalancer_id,
         nic_id)),
          method='DELETE')
    ...
    def get_nic(self, datacenter_id, server_id, nic_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s/nics/%s?depth=%s' % (
         datacenter_id,
         server_id,
         nic_id,
         str(depth)))
    ...
    def list_nics(self, datacenter_id, server_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s/nics?depth=%s' % (
         datacenter_id,
         server_id,
         str(depth)))
        return response
    ...
    def delete_nic(self, datacenter_id, server_id, nic_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s/nics/%s' % (
         datacenter_id,
         server_id,
         nic_id)),
          method='DELETE')
    ...
    def create_nic(self, datacenter_id, server_id, nic):
        ...
        response = self._perform_request(url=('/datacenters/%s/servers/%s/nics' % (
         datacenter_id,
         server_id)),
          method='POST',
          data=data)
    ...
    def get_server(self, datacenter_id, server_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s?depth=%s' % (
         datacenter_id,
         server_id,
         str(depth)))
        return response
    ...
    def list_servers(self, datacenter_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers?depth=%s' % (datacenter_id, str(depth)))
        return response
    ...
    def delete_server(self, datacenter_id, server_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s' % (
         datacenter_id,
         server_id)),
          method='DELETE')
        return response
    ...
    def create_server(self, datacenter_id, server):
        ...
        response = self._perform_request(url=('/datacenters/%s/servers' % datacenter_id),
          method='POST',
          data=data)
        return response
    ...
    def update_server(self, datacenter_id, server_id, **kwargs):
        ...
        for attr in kwargs.keys():
            if attr == 'boot_volume':
                boot_volume_properties = {'id': kwargs[attr]}
                boot_volume_entities = {'bootVolume': boot_volume_properties}
    ...
    def get_attached_volumes(self, datacenter_id, server_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s/volumes?depth=%s' % (
         datacenter_id,
         server_id,
         str(depth)))
        return response
    ...
    def get_attached_volume(self, datacenter_id, server_id, volume_id):
        ...
        """
        response = self._perform_request('/datacenters/%s/servers/%s/volumes/%s' % (
         datacenter_id,
         server_id,
         volume_id))
        return response
    ...
    def attach_volume(self, datacenter_id, server_id, volume_id):
        ...
        """
        data = '{ "id": "' + volume_id + '" }'
        response = self._perform_request(url=('/datacenters/%s/servers/%s/volumes' % (
         datacenter_id,
         server_id)),
          method='POST',
          data=data)
    ...
    def detach_volume(self, datacenter_id, server_id, volume_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s/volumes/%s' % (
         datacenter_id,
         server_id,
         volume_id)),
          method='DELETE')
    ...
    def start_server(self, datacenter_id, server_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s/start' % (
         datacenter_id,
         server_id)),
          method='POST-ACTION')
        return response
    ...
    def stop_server(self, datacenter_id, server_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s/stop' % (
         datacenter_id,
         server_id)),
          method='POST-ACTION')
        return response
    ...
    def reboot_server(self, datacenter_id, server_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/servers/%s/reboot' % (
         datacenter_id,
         server_id)),
          method='POST-ACTION')
        return response
    ...
    def get_user(self, user_id, depth=1):
        ...
        """
        response = self._perform_request('/um/users/%s?depth=%s' % (user_id, str(depth)))
        return response
    ...
    def create_user(self, user):
        ...
        response = self._perform_request(url='/um/users',
          method='POST',
          data=(json.dumps(data)))
        return response
    ...
    def update_user(self, user_id, **kwargs):
        ...
        data = {'properties': properties}
        response = self._perform_request(url=('/um/users/%s' % user_id),
          method='PUT',
          data=(json.dumps(data)))
        return response
    ...
    def list_group_users(self, group_id, depth=1):
        ...
        """
        response = self._perform_request('/um/groups/%s/users?depth=%s' % (group_id, str(depth)))
        return response
    ...
    def add_group_user(self, group_id, user_id):
        ...
        """
        data = {'id': user_id}
        response = self._perform_request(url=('/um/groups/%s/users' % group_id),
          method='POST',
          data=(json.dumps(data)))
        return response
    ...
    def remove_group_user(self, group_id, user_id):
        ...
        """
        response = self._perform_request(url=('/um/groups/%s/users/%s' % (group_id, user_id)),
          method='DELETE')
        return response
    ...
    def get_volume(self, datacenter_id, volume_id):
        ...
        """
        response = self._perform_request('/datacenters/%s/volumes/%s' % (datacenter_id, volume_id))
        return response
    ...
    def list_volumes(self, datacenter_id, depth=1):
        ...
        """
        response = self._perform_request('/datacenters/%s/volumes?depth=%s' % (datacenter_id, str(depth)))
        return response
    ...
    def delete_volume(self, datacenter_id, volume_id):
        ...
        """
        response = self._perform_request(url=('/datacenters/%s/volumes/%s' % (
         datacenter_id, volume_id)),
          method='DELETE')
        return response
    ...
    def create_volume(self, datacenter_id, volume):
        ...
        response = self._perform_request(url=('/datacenters/%s/volumes' % datacenter_id),
          method='POST',
          data=data)
        return response
        ...
        while True:
            ...
            if request['metadata']['status'] == 'DONE':
                ...
                if request['metadata']['status'] == 'FAILED':
                    raise PBFailedRequest('Request {0} failed to complete: {1}'.format(response['requestId'], request['metadata']['message']), response['requestId'])
            current_time = time.time()
            if timeout:
                if current_time > timeout:
                    raise PBTimeoutError('Timed out waiting for request {0}.'.format(response['requestId']), response['requestId'])
    ...
    def _wrapped_request(self, method, url, params=None, data=None, headers=None, cookies=None, files=None, auth=None, timeout=None, allow_redirects=True, proxies=None, hooks=None, stream=None):
        headers.update(self.headers)
        session = requests.Session()
        return session.request(method, url, params, data, headers, cookies, files, auth, timeout, allow_redirects, proxies, hooks, stream, self.verify, self.host_cert)
    ...
    def _perform_request(self, url, method='GET', data=None, headers=None):
        if headers is None:
            ...
            if method == 'POST' or method == 'PUT':
                response = self._wrapped_request(method, url, auth=auth, data=data, headers=headers)
                headers.update({'Content-Type': 'application/json'})
            else:
                if method == 'POST-ACTION-JSON' or method == 'POST-ACTION':
                    headers.update({'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'})
                    response = self._wrapped_request('POST', url, auth=auth, data=data, headers=headers)
                    ...
                    if method == 'PATCH':
                        ...
                        headers.update({'Content-Type': 'application/json'})
                        response = self._wrapped_request(method, url, auth=auth, params=data, headers=headers)
                    if method == 'DELETE':
                        if response.status_code == 202:
                            return True
                    try:
    ...
    def _request_id(headers):
        ...
        raise Exception("Failed to extract request ID from response header 'location': '{location}'".format(location=(headers['location'])))
...
class FirewallRule(object):
    ...
    def __init__(self, name=None, protocol=None, source_mac=None, source_ip=None, target_ip=None, port_range_start=None, port_range_end=None, icmp_type=None, icmp_code=None):
        """
    ...
    def __repr__(self):
        return '<FirewallRule: name=%s, protocol=%s, source_mac=%s, source_ip=%s, target_ip=%s, port_range_start=%s, port_range_end=%s, icmp_type=%s, icmp_code=%s> ...>' % (
...
class LoadBalancer(object):
    ...
    def __init__(self, name=None, ip=None, dhcp=None, balancednics=None):
        """
...
class NIC(object):
    ...
    def __init__(self, name=None, ips=None, dhcp=None, lan=None, firewall_active=None, firewall_rules=None, nat=None):
        """
    ...
    def __repr__(self):
        return '<NIC: name=%s, ips=%s, dhcp=%s,lan=%s, firewall_active=%s> ...>' % (
...
class Server(object):
    ...
    def __init__(self, name=None, cores=None, ram=None, availability_zone=None, boot_volume_id=None, boot_cdrom=None, cpu_family=None, create_volumes=None, attach_volumes=None, nics=None):
        """
    ...
    def __repr__(self):
        return '<Server: name=%s, cores=%s, ram=%s, availability_zone=%s, boot_volume_id=%s, boot_cdrom=%s, ...>' % (
...
class Volume(object):
    ...
    def __init__(self, name=None, size=None, bus='VIRTIO', image=None, image_alias=None, disk_type='HDD', licence_type='UNKNOWN', image_password=None, ssh_keys=None, availability_zone='AUTO'):
        """
    ...
    def __repr__(self):
        return '<Volume: name=%s, size=%s, image=%s, image_alias=%s,bus=%s, disk_type=%s, ...>' % (
...
class Snapshot(object):
    ...
    def __init__(self, name=None, description=None, licence_type='UNKNOWN', size=None, location=None):
        """
    ...
    def __repr__(self):
        return '<Snapshot: name={}, description={}, size={},location={}, ...>'.format(self.name, self.description, str(self.size), self.location)


class Group(object):

    def __init__(self, name=None, create_datacenter=None, create_snapshot=None, reserve_ip=None, access_activity_log=None):
        """
    ...
    def __repr__(self):
        return '<Group: name=%s, create_datacenter=%s, create_snapshot=%s, reserve_ip=%s, access_activity_log=%s>' % (
...
class User(object):
    ...
    def __init__(self, firstname=None, lastname=None, email=None, password=None, administrator=None, force_sec_auth=None):
        """
    ...
    def __repr__(self):
        return '<Group: firstname=%s, lastname=%s, email=%s, password=%s, administrator=%s, force_sec_auth=%s>' % (
