...
# Embedded file name: /Users/weisburd/code/ConfigArgParse/tests/test_configargparse.py
# Compiled at: 2020-03-15 07:07:55
import argparse, configargparse
from contextlib import contextmanager
import inspect, logging, os, sys, tempfile, types, unittest
try:
    import mock
except ImportError:
...
class TestCase(unittest.TestCase):
    ...
    def assertParseArgsRaises(self, regex, *args, **kwargs):
        self.assertRaisesRegex(argparse.ArgumentError, regex, self.parse, *args, **kwargs)
...
class TestBasicUseCases(TestCase):
    ...
    def testBasicCase1(self):
        ...
        self.assertParseArgsRaises('too few arg' if sys.version_info.major < 3 else 'the following arguments are required', args='')
        self.assertParseArgsRaises('argument -y/--arg-y is required' if sys.version_info.major < 3 else 'the following arguments are required: -y/--arg-y', args='-x --arg-z 11 file1.txt')
        self.assertParseArgsRaises('argument --arg-z is required' if sys.version_info.major < 3 else 'the following arguments are required: --arg-z', args='file1.txt file2.txt file3.txt -x -y 1')
        ns = self.parse(args='file1.txt --arg-x -y 3 --arg-z 10', config_file_contents='')
        ...
        self.assertRegex(self.format_values(), 'Command Line Args:   file1.txt --arg-x -y 3 --arg-z 10')
        ...
        self.assertRegex(self.format_values(), 'Command Line Args: \\s+ file1.txt file2.txt\nConfig File \\(method arg\\):\n  arg-x: \\s+ True\n  arg-y: \\s+ 10\n  arg-z: \\s+ 40\n')
        ns = self.parse(args='file1.txt file2.txt --arg-x -y 3 --arg-z 100 ', config_file_contents='arg-y = 31.5\n                                    arg-z = 30\n                                 ')
        self.format_help()
        self.format_values()
        self.assertListEqual(ns.filenames, ['file1.txt', 'file2.txt'])
        self.assertEqual(ns.arg_x, True)
        ...
        self.assertRegex(self.format_values(), 'Command Line Args:   file1.txt file2.txt --arg-x -y 3 --arg-z 100')
    ...
    def testBasicCase2(self, use_groups=False):
        ...
        if not use_groups:
            ...
            self.add_arg('-g', '--my-cfg-file', required=True, is_config_file=True)
            self.add_arg('-d', '--dbsnp', env_var='DBSNP_PATH')
            self.add_arg('-f', '--format', choices=[
             'BED', 'MAF', 'VCF', 'WIG', 'R'], dest='fmt', metavar='FRMT', env_var='OUTPUT_FORMAT', default='BED')
        else:
            ...
            g.add_arg('-g', '--my-cfg-file', required=True, is_config_file=True)
            g = p.add_argument_group(title='g2')
            g.add_arg('-d', '--dbsnp', env_var='DBSNP_PATH')
            g.add_arg('-f', '--format', choices=[
             'BED', 'MAF', 'VCF', 'WIG', 'R'], dest='fmt', metavar='FRMT', env_var='OUTPUT_FORMAT', default='BED')
        self.assertParseArgsRaises('too few arg' if sys.version_info.major < 3 else 'the following arguments are required: vcf, -g/--my-cfg-file', args='--genome hg19')
        ...
        self.assertRegex(self.format_values(), 'Command Line Args:   --genome hg19 -g [^\\s]+ bla.vcf\nDefaults:\n  --format: \\s+ BED\n')
        ...
        self.assertRegex(self.format_values(), 'Command Line Args:   --genome hg19 -g [^\\s]+ f.vcf\nConfig File \\([^\\s]+\\):\n  --format: \\s+ MAF\n')
        config_file2.write('--format VCF')
        config_file2.flush()
        ns = self.parse(args='--genome hg19 -g %s f.vcf ' % config_file2.name)
        self.assertEqual(ns.fmt, 'VCF')
        self.assertRegex(self.format_values(), 'Command Line Args:   --genome hg19 -g [^\\s]+ f.vcf\nConfig File \\([^\\s]+\\):\n  --format: \\s+ VCF\n')
        ns = self.parse(env_vars={'OUTPUT_FORMAT': 'R', 'DBSNP_PATH': '/a/b.vcf'}, args='--genome hg19 -g %s f.vcf ' % config_file2.name)
        self.assertEqual(ns.fmt, 'R')
        self.assertEqual(ns.dbsnp, '/a/b.vcf')
        self.assertRegex(self.format_values(), 'Command Line Args:   --genome hg19 -g [^\\s]+ f.vcf\nEnvironment Variables:\n  DBSNP_PATH: \\s+ /a/b.vcf\n  OUTPUT_FORMAT: \\s+ R\n')
        ns = self.parse(env_vars={'OUTPUT_FORMAT': 'R', 'DBSNP_PATH': '/a/b.vcf', 'ANOTHER_VAR': 'something'}, args='--genome hg19 -g %s --format WIG f.vcf' % config_file2.name)
        self.assertEqual(ns.fmt, 'WIG')
        self.assertEqual(ns.dbsnp, '/a/b.vcf')
        self.assertRegex(self.format_values(), 'Command Line Args:   --genome hg19 -g [^\\s]+ --format WIG f.vcf\nEnvironment Variables:\n  DBSNP_PATH: \\s+ /a/b.vcf\n')
        if not use_groups:
            self.assertRegex(self.format_help(), 'usage: .* \\[-h\\] --genome GENOME \\[-v\\] -g MY_CFG_FILE\n?\\s+\\[-d DBSNP\\]\\s+\\[-f FRMT\\]\\s+vcf \\[vcf ...\\]\n\n' + 9 * '(.+\\s+)' + 'positional arguments:\n  vcf \\s+ Variant file\\(s\\)\n\noptional arguments:\n  -h, --help \\s+ show this help message and exit\n  --genome GENOME \\s+ Path to genome file\n  -v\n  -g MY_CFG_FILE, --my-cfg-file MY_CFG_FILE\n  -d DBSNP, --dbsnp DBSNP\\s+\\[env var: DBSNP_PATH\\]\n  -f FRMT, --format FRMT\\s+\\[env var: OUTPUT_FORMAT\\]\n')
        else:
            self.assertRegex(self.format_help(), 'usage: .* \\[-h\\] --genome GENOME \\[-v\\] -g MY_CFG_FILE\n?\\s+\\[-d DBSNP\\]\\s+\\[-f FRMT\\]\\s+vcf \\[vcf ...\\]\n\n' + 9 * '.+\\s+' + 'positional arguments:\n  vcf \\s+ Variant file\\(s\\)\n\noptional arguments:\n  -h, --help \\s+ show this help message and exit\n\ng1:\n  --genome GENOME \\s+ Path to genome file\n  -v\n  -g MY_CFG_FILE, --my-cfg-file MY_CFG_FILE\n\ng2:\n  -d DBSNP, --dbsnp DBSNP\\s+\\[env var: DBSNP_PATH\\]\n  -f FRMT, --format FRMT\\s+\\[env var: OUTPUT_FORMAT\\]\n')
        self.assertParseArgsRaises("invalid choice: 'ZZZ'", args='--genome hg19 -g %s --format ZZZ f.vcf' % config_file2.name)
        self.assertParseArgsRaises('unrecognized arguments: --bla', args='--bla --genome hg19 -g %s f.vcf' % config_file2.name)
    ...
    def testMutuallyExclusiveArgs(self):
        ...
        g = p.add_mutually_exclusive_group(required=True)
        g.add_arg('-f', '--format', choices=['BED', 'MAF', 'VCF', 'WIG', 'R'], dest='fmt', metavar='FRMT', env_var='OUTPUT_FORMAT', default='BED')
        g.add_arg('-b', '--bam', dest='fmt', action='store_const', const='BAM', env_var='BAM_FORMAT')
        ns = self.parse(args='--genome hg19 -f1 %s --bam' % config_file.name)
        self.assertEqual(ns.genome, 'hg19')
        self.assertEqual(ns.verbose, False)
        self.assertEqual(ns.fmt, 'BAM')
        ns = self.parse(env_vars={'BAM_FORMAT': 'true'}, args='--genome hg19 -f1 %s' % config_file.name)
        self.assertEqual(ns.genome, 'hg19')
        self.assertEqual(ns.verbose, False)
        self.assertEqual(ns.fmt, 'BAM')
        self.assertRegex(self.format_values(), 'Command Line Args:   --genome hg19 -f1 [^\\s]+\nEnvironment Variables:\n  BAM_FORMAT: \\s+ true\nDefaults:\n  --format: \\s+ BED\n')
        self.assertRegex(self.format_help(), 'usage: .* \\[-h\\] --genome GENOME \\[-v\\]\\s+ \\(-f1 TYPE1_CFG_FILE \\| \\s*-f2 TYPE2_CFG_FILE\\)\\s+\\(-f FRMT \\| -b\\)\n\n' + 7 * '.+\\s+' + 'optional arguments:\n  -h, --help            show this help message and exit\n  -f1 TYPE1_CFG_FILE, --type1-cfg-file TYPE1_CFG_FILE\n  -f2 TYPE2_CFG_FILE, --type2-cfg-file TYPE2_CFG_FILE\n  -f FRMT, --format FRMT\\s+\\[env var: OUTPUT_FORMAT\\]\n  -b, --bam\\s+\\[env var: BAM_FORMAT\\]\n\ngroup1:\n  --genome GENOME       Path to genome file\n  -v\n')
        config_file.close()
    ...
    def testSubParsers(self):
        ...
        parent_parser.add_argument('-p', '--p', type=int, required=True, help='set db parameter')
        create_p = subparsers.add_parser('create', parents=[parent_parser], help='create the orbix environment')
        create_p.add_argument('--i', env_var='INIT', choices=['A', 'B'], default='A')
        create_p.add_argument('-config', is_config_file=True)
        update_p = subparsers.add_parser('update', parents=[parent_parser], help='update the orbix environment')
    ...
    def testAddArgsErrors(self):
        self.assertRaisesRegex(ValueError, "arg with is_write_out_config_file_arg=True can't also have is_config_file_arg=True", self.add_arg, '-x', '--X', is_config_file=True, is_write_out_config_file_arg=True)
        self.assertRaisesRegex(ValueError, "arg with is_write_out_config_file_arg=True must have action='store'", self.add_arg, '-y', '--Y', action='append', is_write_out_config_file_arg=True)
    ...
    def testConfigFileSyntax(self):
        ...
        self.assertRegex(self.format_values(), 'Command Line Args: \\s+ -x 1\nConfig File \\(method arg\\):\n  y: \\s+ 12.1\n  b: \\s+ True\n  a: \\s+ 33\n  z: \\s+ z 1\n')
        self.assertParseArgsRaises('argument -x is required' if sys.version_info.major < 3 else 'the following arguments are required: -x, --y', config_file_contents='-x 3')
        self.assertParseArgsRaises("invalid float value: 'abc'", args='-x 5', config_file_contents='y: abc')
        self.assertParseArgsRaises('argument --y is required' if sys.version_info.major < 3 else 'the following arguments are required: --y', args='-x 5', config_file_contents='z: 1')
        self.assertParseArgsRaises('bla', args='-x 1 --y 2.3', config_file_contents='bla=3')
        ns, args = self.parse_known('-x 10 --y 3.8', config_file_contents='bla=3', env_vars={'bla': '2'})
        self.assertListEqual(args, ['--bla=3'])
        self.initParser(ignore_unknown_config_file_keys=False)
        ns, args = self.parse_known(args='-x 1', config_file_contents='bla=3', env_vars={'bla': '2'})
    ...
    def testConfigOrEnvValueErrors(self):
        ...
        self.assertParseArgsRaises("Unexpected value for VERBOSE: 'bla'. Expecting 'true', 'false', 'yes', 'no', '1' or '0'", env_vars={'VERBOSE': 'bla'})
        ns = self.parse('', config_file_contents='verbose=true', env_vars={'HEIGHT': 'true'})
        self.assertEqual(ns.verbose, True)
        ns = self.parse('', config_file_contents='verbose', env_vars={'HEIGHT': 'true'})
        self.assertEqual(ns.verbose, True)
    ...
    def testValuesStartingWithDash(self):
        ...
        self.add_arg('--arg6')
        ns = self.parse('--arg0=-foo --arg3=-foo --arg3=-bar --arg6=-test-more-dashes', config_file_contents='arg2: -foo\narg5: [-foo, -bar]', env_vars={'ARG1': '-foo', 'ARG4': '[-foo, -bar]'})
    ...
    def testPriorityKnown(self):
        ...
        ns = self.parse('--arg command_line_val', config_file_contents='arg: config_val', env_vars={'ARG': 'env_val'})
        self.assertEqual(ns.arg, 'command_line_val')
        ns = self.parse('--arg=command_line_val', config_file_contents='arg: config_val', env_vars={'ARG': 'env_val'})
        self.assertEqual(ns.arg, 'command_line_val')
        ns = self.parse('', config_file_contents='arg: config_val', env_vars={'ARG': 'env_val'})
        self.assertEqual(ns.arg, 'env_val')

    def testPriorityUnknown(self):
        self.initParser()
        ns, args = self.parse_known('--arg command_line_val', config_file_contents='arg: config_val', env_vars={'arg': 'env_val'})
        self.assertListEqual(args, ['--arg', 'command_line_val'])
        ns, args = self.parse_known('--arg=command_line_val', config_file_contents='arg: config_val')
        self.assertListEqual(args, ['--arg=command_line_val'])
    ...
    def testAutoEnvVarPrefix(self):
        ...
        ns = self.parse('', env_vars={'TEST_ARG0': '0', 
           'TEST_ARG1': '1', 
           'TEST_ARG2': '2', 
           'TEST2': '22', 
    ...
    def testEnvVarLists(self):
        ...
        ns = self.parse('', env_vars={'TEST2': '22', 'TEST3': '22', 
...
class TestMisc(TestCase):
    ...
    def testGlobalInstances(self, name=None):
        ...
        self.assertRaisesRegex(ValueError, "kwargs besides 'name' can only be passed in the first time", configargparse.getArgumentParser, name, prog='prog')
    ...
    def testAddArguments_IsConfigFilePathArg(self):
        self.assertRaises(ValueError, self.add_arg, 'c', action='store_false', is_config_file=True)
    ...
    def testConstructor_ConfigFileArgs(self):
        ...
        temp_cfg.flush()
        self.initParser(args_for_setting_config_path=['-c', '--config'], config_arg_is_required=True, config_arg_help_message='my config file', default_config_files=[
         temp_cfg.name])
        self.add_arg('--genome', help='Path to genome file', required=True)
        self.assertParseArgsRaises('argument -c/--config is required' if sys.version_info.major < 3 else 'arguments are required: -c/--config')
        ...
        self.assertRegex(self.format_help(), 'usage: .* \\[-h\\] -c CONFIG_FILE --genome GENOME\n\n' + 7 * '.+\\s+' + 'optional arguments:\n  -h, --help\\s+ show this help message and exit\n  -c CONFIG_FILE, --config CONFIG_FILE\\s+ my config file\n  --genome GENOME\\s+ Path to genome file\n')
        self.assertIsNone(self.parser.print_values(file=sys.stderr))
        self.initParser(ignore_unknown_config_file_keys=False)
        self.assertRaisesRegex(argparse.ArgumentError, 'unrecognized arguments', self.parse, config_file_contents='arg1 = 3')
    ...
    def test_AbbrevConfigFileArgs(self):
        ...
        temp_cfg.flush()
        self.initParser()
        self.add_arg('-c', '--config_file', required=False, is_config_file=True, help='config file path')
    ...
    def testConstructor_WriteOutConfigFileArgs(self):
        ...
        self.initParser(args_for_writing_out_config_file=['-w'], write_out_config_file_arg_help_message='write config')
        ...
        self.assertEqual(cfg_f.read().strip(), expected_config_file_contents.strip())
        self.assertRaisesRegex(ValueError, "Couldn't open / for writing:", self.parse, args=command_line_args + ' -w /')
    ...
    def testConstructor_WriteOutConfigFileArgs2(self):
        ...
        self.add_arg('-d', '--arg4')
        self.add_arg('-e', '--arg5')
        self.add_arg('--config-file-settable-flag', action='store_true', env_var='FLAG_ARG')
        self.add_arg('-l', '--config-file-settable-list', action='append')
        command_line_args = '-w %s ' % cfg_f.name
        command_line_args += '-l a -l b -l c -l d '
        self.assertFalse(self.parser._exit_method_called)
        ns = self.parse(command_line_args, env_vars={'ARG1': '10', 'FLAG_ARG': 'true', 'SOME_OTHER_ENV_VAR': '2'}, config_file_contents='arg3 = bla3\narg4 = bla4')
        self.assertTrue(self.parser._exit_method_called)
        cfg_f.seek(0)
        expected_config_file_contents = 'config-file-settable-list = [a, b, c, d]\n'
    ...
    def testConstructor_WriteOutConfigFileArgsLong(self):
        ...
        """
        cfg_f = tempfile.NamedTemporaryFile(mode='w+', delete=True)
        self.initParser(args_for_writing_out_config_file=['--write-config'], write_out_config_file_arg_help_message='write config')
        self.add_arg('-not-config-file-settable')
        self.add_arg('--config-file-settable-arg', type=int)
        self.add_arg('--config-file-settable-arg2', type=int, default=3)
        self.add_arg('--config-file-settable-flag', action='store_true')
        ...
        expected_config_file_contents += 'config-file-settable-arg2 = 3\n'
        self.assertEqual(cfg_f.read().strip(), expected_config_file_contents.strip())
        self.assertRaisesRegex(ValueError, "Couldn't open / for writing:", self.parse, args=command_line_args + ' --write-config /')
...
class TestConfigFileParsers(TestCase):
    ...
    def testDefaultConfigFileParser_Basic(self):
        ...
        self.assertEqual(input_config_str.getvalue().replace(': ', ' = '), output_config_str)
    ...
    def testDefaultConfigFileParser_All(self):
        ...
        self.assertDictEqual(parsed_obj, {'_a': '3', 
           '_b': 'c', 
           '_list_arg1': [
                        'a', 'b', 'c'], 
           '_str_arg': 'true', 
    ...
    def testYAMLConfigFileParser_Basic(self):
        try:
            ...
            logging.warning("WARNING: PyYAML not installed. Couldn't test YAMLConfigFileParser")
            return
...
try:
    ...
    test_argparse_source_code = test_argparse_source_code.replace('argparse.ArgumentParser', 'configargparse.ArgumentParser')
