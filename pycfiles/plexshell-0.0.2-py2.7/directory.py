...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/plexshell/commands/directory.py
# Compiled at: 2011-09-18 11:46:53
...
class DirectoryCmd(PlexCmd):
    ...
    def complete_cd(self, text, line, begidx, endidx):
        ...
        for index, name in enumerate(components) or (0, '..'):
            if index == len(components) - 1:
                ...
                return [ s.display_name for s in subdirs if s.display_name.startswith(text) and isinstance(s, Directory)
                       ]
    ...
    def complete_get(self, text, line, begidx, endidx):
        ...
        result = [ node.name for node in listing if node.name.startswith(text) and isinstance(node, Track)
                 ]
        path = line.replace('get ', '')
        if not path:
            return [self.cwd.path]
        return [ node.path.split('/')[(-1)] for node in listing if node.path.startswith(path)
               ]
...
class SearchCmd(DirectoryCmd):
    ...
    def do_search(self, term):
        ...
        self.cwd = Search(self.parse_response(response), self.cwd, term, search_type, path)
