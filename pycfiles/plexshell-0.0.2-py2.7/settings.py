...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/plexshell/commands/settings.py
# Compiled at: 2011-09-18 04:51:22
...
class SettingsCmd(DirectoryCmd):
    ...
    def __init__(self, conn, directory, stdin=None):
        ...
        self.settings = [ s for s in self.list_directory(directory) if isinstance(s, Setting)
                        ]
        self.dirty = False
        self.prompt_context = colorize('%s[settings]' % self.directory, self.directory.color)
        self.update_prompt()