...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wtforms_jsonschema2/fab.py
# Compiled at: 2018-10-15 04:56:42
# Size of source mod 2**32: 6575 bytes
from flask_appbuilder.fields import QuerySelectField, QuerySelectMultipleField, EnumField
from flask_appbuilder.upload import ImageUploadField
from collections import OrderedDict
import logging
from .base import BaseConverter, converts
from .utils import _get_pretty_name, _get_related_view_property, _is_parent_related_view_property, _get_view_name
...
class FABConverter(BaseConverter):
    ...
    def convert_enum_field(self, field):
        fieldtype = 'object'
        options = {'enum': [{'id': c[0], 'label': str(c[1])} for c in field.iter_choices()]}
        required = False
    ...
    def convert_view(self, view, form_type='add', parentView=None):
        ...
        view_definition = super().convert(self._get_form(view, form_type))
        ...
        conditional_views = [cv for cond in conditions for cv in cond.affected_views]
        if view.related_views is not None:
            for v in view.related_views:
                rel_name, rel_schema = self.convert_view(v, form_type, parentView=view)
