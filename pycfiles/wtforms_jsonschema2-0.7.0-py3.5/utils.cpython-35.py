...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wtforms_jsonschema2/utils.py
...
def _get_related_view_property(view, related_view, field):
    ...
    """
    defin = {}
    log.debug('Checking view {}, related view {} and field {}'.format(view, related_view, field))
    if view.datamodel.is_relation_one_to_one(field):
        log.debug('Got a one-to-one relation')
        title = obj_name = _get_pretty_name(related_view, 'show').replace(' ', '')
        defin['$ref'] = '#/definitions/%s' % obj_name
    else:
        if view.datamodel.is_relation_one_to_many(field):
...
def _is_parent_related_view_property(view, parent_view, field):
    log.debug('Checking parent related property for {}, parent {} and field {}'.format(view, parent_view, field))
...
def _get_pretty_name(view, form_type):
    ...
    if form_type == 'add' and view.add_title != '':
        ...
        if form_type == 'show' and view.show_title != '':
            return view.show_title
        if form_type == 'list' and view.list_title != '':
            return view.list_title
        if form_type == 'edit' and view.edit_title != '':
