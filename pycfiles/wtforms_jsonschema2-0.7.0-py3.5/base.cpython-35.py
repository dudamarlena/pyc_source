...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wtforms_jsonschema2/base.py
# Compiled at: 2018-10-15 04:56:42
# Size of source mod 2**32: 6321 bytes
from wtforms.form import FormMeta
from wtforms.fields.core import StringField, IntegerField, DateTimeField, SelectField, DecimalField, FormField, BooleanField
from wtforms.validators import Required, InputRequired, NumberRange, Length, Email, DataRequired
...
class BaseConverter(object):
 ...
 'csrf_token']):
    ...
    def _is_required(self, vals):
        return InputRequired in vals.keys() or Required in vals.keys() or DataRequired in vals.keys()
    ...
    def select_field(self, field):
        ...
        if all([isinstance(c, int) for c in choices]):
            ...
            if all([isinstance(c, (float, Decimal, int)) for c in choices]):
                fieldtype = 'number'
            else:
                fieldtype = 'string'
    ...
    def convert(self, form):
        ...
        if isinstance(form, FormMeta):
            form = form()
        fields = OrderedDict([(f.name, f) for f in form if f.name not in self.skip_fields])
        schema = OrderedDict([
         ('type', 'object'),
         (
