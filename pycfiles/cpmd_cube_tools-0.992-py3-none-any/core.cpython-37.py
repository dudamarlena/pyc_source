...
# Compiled at: 2020-04-25 20:09:34
# Size of source mod 2**32: 15711 bytes
import os, time, scipy, pkg_resources, pandas as pd, numpy as np
...
class cube:
    ...
    def toorigin(self):
        if not float(self.origin[0]) != float(0):
            if float(self.origin[1]) != float(0) or float(self.origin[2]) != float(0):
                print('Moving structure and volumne data to origin point')
                self.translate_cube(-self.origin)
                self.origin = np.asarray([0, 0, 0])
                print('Original point has been set to [0,0,0]')
        else:
    ...
    def cube2vdd(self, gero, inpf, alists):
        ...
        return (
         pd.to_numeric(vddpd.loc[alist].charge).values, vddcube)
