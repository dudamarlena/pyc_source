...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_metadata.py
# Compiled at: 2020-04-23 10:30:11
# Size of source mod 2**32: 13059 bytes
import os, glob, sys, shutil, logging
...
def test_query(requests_mock):
    ...
    def _mock_request(url, **kwargs):
        if not '/metadata' in url:
            raise AssertionError
        else:
            assert 'foo.bar=fizzbuzz' in url
            assert 'data=False' in url
        mocked_response = MagicMock(requests.Response)
        mocked_response.status_code = 200
        mocked_response.json.return_value = expected_response
        mocked_response.raise_for_status.side_effect = lambda *args: None
...
def test_query_full_metadata(requests_mock):
    ...
    """
    metadata = Gen3Metadata('https://example.com')
    expected_response = {'1cfd6767-7775-4e0d-a4a7-d0fc9db02e1d': {'dbgap':{'sex':'male', 
                                               'body_site':'Blood', 
                                               'repository':'TOPMed_WGS_Amish', 
                                               'sample_use':[],  'analyte_type':'DNA', 
...
def test_batch_create(requests_mock):
    ...
    metadata = Gen3Metadata('https://example.com')
    metadata_list = [
     {'guid':'3c42c819-1dfe-4c3e-8d46-c3ec7eb99bf4', 
      'data':{'foo': 'bar'}},
     {'guid':'dfa1a1dc-98f4-46be-ba8f-ae9b42b0ee50', 
      'data':{'foo': 'bar'}}]
