...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/metadata.py
# Compiled at: 2020-04-23 10:30:11
# Size of source mod 2**32: 13312 bytes
"""
Contains class for interacting with Gen3's Metadata Service.
"""
import aiohttp, backoff, requests, urllib.parse, logging, sys
...
class Gen3Metadata:
    ...
    def __init__(self, endpoint, auth_provider=None, service_location='mds', admin_endpoint_suffix='-admin'):
        """
 ...
 url, data=return_full_metadata, limit=limit, offset=offset, **kwargs)
    ...
    def batch_create(self, metadata_list, overwrite=True, **kwargs):
        ...
        if len(metadata_list) > 1:
            if 'guid' not in metadata_list[0]:
                if 'data' not in metadata_list[0]:
                    logging.warning(f"it looks like your metadata list for bulk create is malformed. the expected format is a list of dicts that have 2 keys: 'guid' and 'data', where 'guid' is a string and 'data' is another dict. The first element doesn't match that pattern: {metadata_list[0]}")
        ...
        response = requests.post(url_with_params,
          json=metadata_list, auth=(self._auth_provider))
        response.raise_for_status()
        return response.json()
    ...
    def create(self, guid, metadata, overwrite=False, **kwargs):
        ...
        response = requests.post(url_with_params,
          json=metadata, auth=(self._auth_provider))
        response.raise_for_status()
        return response.json()
    ...
    async def async_create(self, guid, metadata, overwrite=False, _ssl=None, **kwargs):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url_with_params,
              json=metadata, headers=headers, ssl=_ssl) as response:
                response.raise_for_status()
