...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/tools/indexing/download_manifest.py
# Compiled at: 2020-04-23 10:30:11
# Size of source mod 2**32: 11541 bytes
"""
...
"""
import asyncio, click, time, csv, glob, logging, os, sys, shutil, math
...
async def async_download_object_manifest(commons_url, output_filename='object-manifest.csv', num_processes=4, max_concurrent_requests=MAX_CONCURRENT_REQUESTS):
    """
    ...
    result = await _write_all_index_records_to_file(commons_url, output_filename, num_processes, max_concurrent_requests)
    end_time = time.perf_counter()
    logging.info(f"end time: {end_time}")
    logging.info(f"run time: {end_time - start_time}")


async def _write_all_index_records_to_file(commons_url, output_filename, num_processes, max_concurrent_requests):
    """
    ...
    logging.debug('requesting indexd stats...')
    ...
    for x in range(len(page_chunks)):
        ...
        command = f"python {CURRENT_DIR}/download_manifest.py --commons_url {commons_url} --pages {pages} --num_processes {num_processes} --max_concurrent_requests {max_concurrent_requests}"
...
@click.option('--num_processes',
  type=int,
  help='number of processes you are running so we can make sure we don\'t open too many http connections. ex: "4"')
@click.option('--max_concurrent_requests',
  type=int,
  help='number of processes you are running so we can make sure we don\'t open too many http connections. ex: "4"')
def write_page_records_to_files(commons_url, pages, num_processes, max_concurrent_requests):
    """
    ...
    result = loop.run_until_complete(_get_records_and_write_to_file(commons_url, pages, num_processes, max_concurrent_requests))
    return result


async def _get_records_and_write_to_file(commons_url, pages, num_processes, max_concurrent_requests):
    """
    ...
    await (asyncio.gather)(*(_put_records_from_page_in_queue(page, commons_url, lock, queue) for page in pages))
...
async def _put_records_from_page_in_queue(page, commons_url, lock, queue):
    ...
    async with lock:
        ...
        if 'https' not in commons_url:
            ssl = False
        records = await index.async_get_records_on_page(page=page,
          limit=INDEXD_RECORD_PAGE_SIZE,
          _ssl=ssl)
