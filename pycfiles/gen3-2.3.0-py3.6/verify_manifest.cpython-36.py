...
# Size of source mod 2**32: 13643 bytes
"""
...
Attributes:
    ...
    MAX_CONCURRENT_REQUESTS (int): maximum number of desired concurrent requests across
        processes/threads
"""
import asyncio, csv, logging, os, time
...
async def async_verify_object_manifest(commons_url, manifest_file, max_concurrent_requests=MAX_CONCURRENT_REQUESTS, manifest_row_parsers=manifest_row_parsers, manifest_file_delimiter=None, output_filename=f"verify-manifest-errors-{time.time()}.log"):
    """
    ...
    if not manifest_file_delimiter:
        ...
        if file_ext[(-1)].lower() == '.tsv':
            ...
            manifest_file_delimiter = ','
    await _verify_all_index_records_in_file(commons_url, manifest_file, manifest_file_delimiter, max_concurrent_requests, output_filename.split('/')[(-1)])
    end_time = time.perf_counter()
    logging.info(f"end time: {end_time}")
    logging.info(f"run time: {end_time - start_time}")


async def _verify_all_index_records_in_file(commons_url, manifest_file, manifest_file_delimiter, max_concurrent_requests, output_filename):
    """
    ...
    await (asyncio.gather)(*(_parse_from_queue(queue, lock, commons_url, output_queue) for x in range(0, int(max_concurrent_requests + max_concurrent_requests / 4))))
