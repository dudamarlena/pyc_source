...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/tools/indexing/index_manifest.py
# Compiled at: 2020-04-23 10:30:11
# Size of source mod 2**32: 16084 bytes
"""
...
import threading, re, uuid, copy, sys
...
def _get_and_verify_fileinfos_from_tsv_manifest(manifest_file, manifest_file_delimiter='\t'):
    """
...
def _index_record(indexclient, replace_urls, thread_control, fi):
    ...
    try:
        if 'url' in fi:
            urls = [element.strip().replace("'", '') for element in _standardize_str(fi['url']).strip()[1:-1].split(' ')] if fi['url'] != '[]' else []
            authz = [element.strip().replace("'", '') for element in _standardize_str(fi['authz']).strip()[1:-1].split(' ')] if ('authz' in fi and fi['authz'] != '[]') else ([])
            if 'acl' in fi:
                if fi['acl'].strip().lower() in frozenset({"['open']", "[u'open']"}):
                    acl = [
                     '*']
                else:
                    acl = [element.strip().replace("'", '') for element in _standardize_str(fi['acl']).strip()[1:-1].split(' ')] if ('acl' in fi and fi['acl'] != '[]') else ([])
            else:
                acl = []
        else:
            doc = None
        ...
        if doc is not None:
            if doc.size != fi.get('size') or doc.hashes.get('md5') != fi.get('md5'):
                logging.error('The guid {} with different size/hash already exist. Can not index it without getting a new GUID'.format(fi.get('GUID')))
            else:
                need_update = False
                if replace_urls:
            ...
            if fi.get('GUID'):
                ...
                guid = None
            doc = indexclient.create(did=guid,
        ...
        logging.error('Can not update/create an indexd record with guid {}. Detail {}'.format(fi.get('GUID'), e))
    ...
    if thread_control.num_processed_files * 10 % thread_control.num_total_files == 0:
        logging.info('Progress: {}%'.format(thread_control.num_processed_files * 100.0 / thread_control.num_total_files))
    thread_control.mutexLock.release()


def index_object_manifest(commons_url, manifest_file, thread_num, auth=None, replace_urls=True, manifest_file_delimiter=None):
    """
    ...
    if not manifest_file_delimiter:
        ...
        if file_ext[(-1)].lower() == '.tsv':
            ...
            manifest_file_delimiter = ','
    try:
        files, headers = _get_and_verify_fileinfos_from_tsv_manifest(manifest_file, manifest_file_delimiter)
    except Exception as e:
...
@click.option('--manifest_file_delimiter',
...
def index_object_manifest_cli(commons_url, manifest_file, thread_num, api_key, auth, replace_urls, manifest_file_delimiter, out_manifest_file):
    """
    ...
    files, headers = index_object_manifest(commons_url + '/index', manifest_file, int(thread_num), auth, replace_urls, manifest_file_delimiter)
