...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/submission_test.py
...
def test_create_program(sub):
    with patch('gen3.submission.requests') as (mock_request):
        ...
        p = sub.create_program({'dbgap_accession_number':'programmjm', 
         'name':'programmjm', 
         'type':'program'})
        assert p
...
def test_create_project(sub):
    with patch('gen3.submission.requests') as (mock_request):
        mock_request.status_code = 200
        mock_request.json.return_value = '{ "key": "value" }'
        pj = sub.create_project('programmjm', {'code':'projectmjm', 
...
def test_submit_record(sub):
    with patch('gen3.submission.requests') as (mock_request):
        mock_request.status_code = 200
        mock_request.json.return_value = '{ "key": "value" }'
        rec = sub.submit_record('prog1', 'proj1', {'projects':[
          {'code': 'proj1'}], 
