...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gen3/tools/metadata/ingest_manifest.py
# Compiled at: 2020-04-23 10:30:11
# Size of source mod 2**32: 18180 bytes
"""
...
"""
import aiohttp, asyncio, csv, json, logging, os, time, urllib.parse
...
async def _query_for_associated_indexd_record_guid(commons_url, row, lock, output_queue):
    """
    ...
    if 'urls' in mapping:
        if len(mapping.items()) > 1:
            msg = f"You cannot pattern match 'urls' and exact match other fields for mapping from indexd record to metadata columns. You can match by URL pattern *OR* match exact record fields like size, hash, uploader, url, acl, authz.\nYou provided mapping: {mapping}"
            logging.error(msg)
    ...
    if 'urls' in mapping:
        ...
        params = {mapping_key:row.get(mapping_value) for mapping_key, mapping_value in mapping.items()}
    ...
    if len(records) > 1:
        msg = f"Multiple records were found with the given search criteria, this is assumed to be unintentional so the metadata will NOT be linked to these records:\n{records}"
        logging.warning(msg)
        await output_queue.put(msg)
        records = []
    guid = None
...
manifest_row_parsers = {'guid_for_row':_get_guid_for_row, 
 'indexed_file_object_guid':_query_for_associated_indexd_record_guid}

async def async_ingest_metadata_manifest(commons_url, manifest_file, metadata_source, auth=None, max_concurrent_requests=MAX_CONCURRENT_REQUESTS, manifest_row_parsers=manifest_row_parsers, manifest_file_delimiter=None, output_filename=f"ingest-metadata-manifest-errors-{time.time()}.log", get_guid_from_file=True):
    """
    ...
    if not manifest_file_delimiter:
        ...
        if file_ext[(-1)].lower() == '.tsv':
            ...
            manifest_file_delimiter = ','
    await _ingest_all_metadata_in_file(commons_url, manifest_file, metadata_source, auth, manifest_file_delimiter, max_concurrent_requests, output_filename.split('/')[(-1)], get_guid_from_file)


async def _ingest_all_metadata_in_file(commons_url, manifest_file, metadata_source, auth, manifest_file_delimiter, max_concurrent_requests, output_filename, get_guid_from_file):
    """
    ...
    await (asyncio.gather)(*(_parse_from_queue(queue, lock, commons_url, output_queue, auth, get_guid_from_file, metadata_source) for x in range(0, int(max_concurrent_requests + max_concurrent_requests / 4))))
    ...
    logging.info(f"done processing, writing output queue to single file {output_filename}")
...
async def _parse_from_queue(queue, lock, commons_url, output_queue, auth, get_guid_from_file, metadata_source):
    """
    ...
    while not queue.empty():
        ...
        if get_guid_from_file:
            ...
            is_indexed_file_object = await _is_indexed_file_object(guid, commons_url, lock)
        else:
            guid = await manifest_row_parsers['indexed_file_object_guid'](commons_url, row, lock, output_queue)
        ...
        if guid:
            ...
            for key, value in row.items():
                try:
                    ...
                    if '}' in value or '{' in value or '[' in value or ']' in value:
                        msg = f"Unable to json.loads a string that looks like json: {value}. adding as a string instead of nested json. Exception: {exc}"
                        logging.warning(msg)
                        await output_queue.put(msg)
                    new_value = value
            ...
            metadata['_guid_type'] = GUID_TYPE_FOR_INDEXED_FILE_OBJECT if is_indexed_file_object else GUID_TYPE_FOR_NON_INDEXED_FILE_OBJECT
            ...
            try:
                ...
                logging.debug(f"Got conflict for {guid}. Let's update instead of create...")
            ...
            msg = f"Did not add a metadata object for row because an invalid GUID was parsed or no record with this GUID was found in indexd: {guid}.\nRow: {row}"
            logging.warning(msg)
            await output_queue.put(msg)
