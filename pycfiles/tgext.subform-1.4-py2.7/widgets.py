...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/tgext/subform/widgets.py
# Compiled at: 2013-04-20 14:24:25
import inspect
...
select_field_with_add_js = JSLink(modname='tgext.subform', filename='static/javascript/select_field_with_add.js', javascript=[
 dojo_js])
select_field_with_add_css = CSSLink(modname='tgext.subform', filename='static/css/select_field_with_add.css')
opener_img = CSSLink(modname='tgext.subform', filename='static/images/triangle-solid-up.png')
...
class SelectFieldWithAdd(PropertySingleSelectField):
    subform_params = ('__field_widget_types__', '__field_validator_types__', '__field_validators__',
                      '__base_widget_args__', '__add_fields__', '__omit_fields__')
    params = {'__field_widget_types__': {}, '__field_validator_types__': {}, '__base_widget_args__': {}, '__add_fields__': {}, '__omit_fields__': None, 
       'related_subforms': '', 
    ...
    def __subform__(self):
        ...
        if 'submit' not in self.__add_fields__:
            self.__add_fields__['submit'] = SubmitButton('submit', attrs={'onclick': 'javascript:return function(){%s_helper.validate(); return false;}();' % subform_id})
    ...
    def _subform_js(self, form_id, field_name, related_subforms, pk_names, display_field_name):
        return SubformJS(form_id=form_id, subtype=field_name, pk_names=("','").join(pk_names), related_subforms=related_subforms, display_field_name=display_field_name)
    ...
    def _update_subform(self, d):
        ...
        d['subform_js'] = self._subform_js(form_id, self.field_name, related_subforms, pk_names, display_field_name)
        if 'subform_value' in d:
            value = d['subform_value']
            self.__subform__.__widget__.update_params(value)
        return d
