...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/russell/cli/run.py
# Compiled at: 2019-02-14 01:19:25
from __future__ import print_function
import shutil, click, sys, os, re, russell
...
from russell.cli.utils import get_task_url, get_module_task_instance_id, get_mode_parameter, wait_for_url, force_unicode, get_files_in_current_directory, save_dir, copy_files, get_size, sizeof_fmt
from russell.client.files import FsClient, SOCKET_STATE, copy_files, DataConfigManager
...
@click.option('--data', multiple=True, help='Data source id(s) to use')
@click.option('--mode', help='Different russell modes', default='job', type=click.Choice(['job', 'jupyter', 'serve']))
@click.option('--env', help='Environment type to use')
@click.option('--message', '-m', help='Message to commit', type=click.STRING, default='')
@click.option('--param', '-p', help='Param add to env variables', type=click.STRING, multiple=True, default='')
@click.option('--version', '-v', help='Version to run', type=click.INT)
@click.option('--tensorboard/--no-tensorboard', help='Run tensorboard')
...
def run(ctx, gpu, env, data, mode, command, message, version, tensorboard, param):
    ...
    if version:
        ...
        try:
            ...
            if size > 104857600:
                sys.exit(('Total size: {}. Code size too large to sync, please keep it under 100MB.If you have data files in the current directory, please upload them separately using "russell data" command and remove them from here.\n').format(sizeof_fmt(size)))
        ...
        if module_resp:
            ...
            module = Module(name=experiment_config.name, description=message, family_id=experiment_config.family_id, version=version, module_type='code', entity_id=experiment_config.project_id)
            ...
            try:
                try:
                    ...
                    fc.socket_upload(file_type='code', filename=CUR_DIR, access_token=access_token.token, file_id=module_id, user_name=access_token.username, data_name=experiment_config.name, is_increment=True, temp_dir=_TEMP_DIR, entity_id=project_id)
                except Exception as e:
    ...
    instance_type = GPU_INSTANCE_TYPE if gpu else CPU_INSTANCE_TYPE
    experiment_request = ExperimentRequest(name=experiment_config.name, description=message, module_id=module_id, data_ids=data_ids, command=command_str, full_command=get_command_line(instance_type=instance_type, data=data, open_notebook=True, env=env, message=message, mode=mode, tensorboard=tensorboard, command_str=command_str, params=param), mode=get_mode_parameter(mode), predecessor=experiment_config.experiment_predecessor, family_id=experiment_config.family_id, project_id=experiment_config.project_id, version=version, instance_type=instance_type, environment=env, enable_tensorboard=tensorboard, params=list(param))
    ...
    ExperimentConfigManager.set_config(experiment_config)
    experiment_name = ('{}/{}:{}').format(access_token.username, experiment_config.name, version)
    table_output = [
     [
      'RUN ID', 'NAME', 'VERSION'],
     [
    ...
    if mode in ('jupyter', 'serve'):
        ...
        if mode == 'serve':
            russell_logger.info(('Path to service endpoint: {}').format(get_task_url(experiment.id, gpu, view='serve')))
    if tensorboard:
...
def validate_env(env, instance_type, enable_tensorboard, enable_jupyter):
    try:
        ...
        if envs_info:
            for env_info in envs_info:
                if env_info['name'] == env:
                    if enable_jupyter and env_info['support_jupyter'] is False:
                        ...
                        russell_logger.error('Jupyter mode is not supported in `caffe` for the moment, you could try `caffe:py2`')
                        return False
                    else:
            ...
            russell_logger.error(('{} is not in the list of supported environments:\n{}').format(env, tabulate([ [env.get('name')] for env in sorted(envs_info, key=lambda x: x.get('name')) ])))
            return False
...
def process_data_ids(data):
    if len(data) > 5:
        russell_logger.error('Cannot attach more than 5 datasets to a task')
        return (
         False, None)
    else:
        data_ids = []
        ...
        for data_id_and_path in data:
            ...
            if path in used_path:
                russell_logger.error(('duplicate mount point {}. Please use different mount point in same task').format(path))
                return (
                 False, None)
            used_path.add(path)
