...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/loopfunction/loopfunction.py
# Compiled at: 2016-01-02 13:38:08
# Size of source mod 2**32: 4751 bytes
import threading

class Loop:
    __doc__ = 'Makes it easy for one function to go into an infinite loop\n    '

    def __init__(self, target, args=(), kwargs={}, on_start=lambda : None, on_start_args=(), on_start_kwargs={}, on_stop=lambda : None, on_stop_args=(), on_stop_kwargs={}):
    ...
    def start(self, subthread=True):
        ...
        if self.is_running():
            ...
            if subthread:
                self._loop_thread = threading.Thread(target=self._loop, args=self.args, kwargs=self.kwargs)
