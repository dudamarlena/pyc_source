...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/filter.py
    ...
    for (ev, item) in stream:
        if ev == START:
            ...
            for i in range(0, len(templates)):
                ...
                if match(item):
                    item = stream.expand()
                    newstream = _coalesce(call(template, item, apply_func), template._get_assume_encoding())
                    if len(templates) < 2:
                        for (ev, item) in newstream:
                            yield (
                             ev, item)

                    for (ev, item) in apply_matches(ElementStream(newstream), template, templates[:i] + templates[i + 1:], apply_func):
                        yield (
                         ev, item)
