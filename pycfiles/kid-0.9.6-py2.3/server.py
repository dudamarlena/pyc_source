...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/server.py
...
class HTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def run_kid(self):
        ...
        if not os.path.isfile(scriptfile):
            self.send_error(403, 'Kid template is not a plain file (%r)' % scriptname)
            return
        env = {}
        ...
        try:
            ...
            template = template_module.Template(request=self, environ=env, FieldStorage=FieldStorage(self.rfile, environ=env))
            s = str(template)
...
class HTTPServer(BaseHTTPServer):
    ...
    def __init__(self, server_address=None, RequestHandlerClass=HTTPRequestHandler):
        if server_address is None:
            server_address = (
             default_host, default_port)
        BaseHTTPServer.__init__(self, server_address, HTTPRequestHandler)
        return


def test(server_address=None, HandlerClass=HTTPRequestHandler, ServerClass=HTTPServer, protocol='HTTP/1.0'):
