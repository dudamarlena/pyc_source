...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/format.py
...
class Format(object):
    ...
    re_leading_blanks = re.compile('^[ \\t]+', re.MULTILINE)
    re_trailing_blanks = re.compile('[ \\t]+$', re.MULTILINE)
    re_duplicate_blanks = re.compile('[ \\t]{2,}')
    re_duplicate_newlines = re.compile('\\n[ \\t\\n\\r]*\\n')
    re_whitespace_with_newline = re.compile('[ \\t]*\\n[ \\t\\n\\r]*')
    re_indentation = re.compile('\\n[ \\t]*')
    re_squotes = re.compile("'")
    re_dquotes = re.compile('"')
    re_sbackticks = re.compile('`')
    re_dbackticks = re.compile('(?<![\\w`])``(?!`)')
    re_squote_decade = re.compile("(?<=\\W)'(?=\\d\\d\\D)")
    re_squote_left = re.compile("((?<=\\W)'(?=\\w))|((?<=\\s)'(?=\\S))", re.UNICODE)
    re_squote_right = re.compile("((?<=\\w)'(?=\\W))|((?<=\\S)'(?=\\s))|((?<=\\W)')", re.UNICODE)
    re_dquote_left = re.compile('((?<=\\W)"(?=\\w))|((?<=\\s)"(?=\\S))', re.UNICODE)
    re_dquote_right = re.compile('((?<=\\w)"(?=\\W))|((?<=\\S)"(?=\\s))|((?<=\\W)")', re.UNICODE)
    re_endash = re.compile('(?<!-)--(?!-)')
    re_emdash = re.compile('(?<!-)---(?!-)')
    re_hyphen_between_blanks = re.compile('(?<!\\S)-(?!\\S)', re.UNICODE)
    ...
    def __init__(self, *args, **kw):
        ...
        if wrap:
            ...
            if simple_whitespace:
                ...
                simple_newlines = kw.get('simple_newlines', kw.get('no_empty_lines'))
        ...
        if strip or strip_lines:
            lstrip_blanks = rstrip_blanks = strip_blanks = None
        else:
        ...
        educate = not stupefy and kw.get('educate', kw.get('educated', kw.get('nice')))
        ...
        self.entity_map = kw.get('entity_map', kw.get('named_entities', kw.get('named')))
    ...
    def __repr__(self):
        ...
        if 'educate_quotes' in args and 'educate_backticks' in args and 'educate_dashes' in args and 'educate_ellipses' in args:
        ...
        attrs = args.keys()
        attrs.sort()
        args = [ f.__name__ for f in self.custom_filters ] + [ '%s=%r' % (attr, args[attr]) for attr in attrs ]
        return '%s(%s)' % (self.__class__.__name__, (', ').join(args))
        return

    def filter(self, s, last_char=None, next_char=None):
        """Run all filters."""
        return self.context_filter(self.text_filter(s), last_char, next_char)
