...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/compile.py
...
def main():
    ...
    for f in files:
        if isdir(f):
            for res in kid.compiler.compile_dir(f, force=force, source=source, strip_dest_dir=strip_dest_dir):
            ...
            try:
                stat = kid.compiler.compile_file(f, force=force, source=source, strip_dest_dir=strip_dest_dir)
            except Exception, e:
                (stat, err) = (
                 e, True)
