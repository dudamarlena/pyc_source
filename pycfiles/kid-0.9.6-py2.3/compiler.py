...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/compiler.py
...
import os, os.path, imp, stat, struct, marshal, kid
from kid.codewriter import raise_template_error
__all__ = [
 'KID_EXT', 'compile', 'compile_file', 'compile_dir']
KID_EXT = '.kid'
...
class KidFile(object):
    ...
    def __init__(self, kid_file, force=False, encoding=None, strip_dest_dir=None, entity_map=None):
        ...
        if self._code is None:
            if self.stale:
                ...
                try:
                    ...
                    raise_template_error(filename=self.kid_file, encoding=self.encoding)
    ...
    def python(self):
        ...
        if self._python is None:
            py = kid.codewriter.parse_file(self.kid_file, self.encoding, self.entity_map)
            self._python = py
        return self._python
        return
...
def compile_file(file, force=False, source=False, encoding=None, strip_dest_dir=None, entity_map=None):
...
def compile_dir(dir, maxlevels=10, force=False, source=False, encoding=None, strip_dest_dir=None, entity_map=None):
    ...
    for name in names:
        ...
        if os.path.isfile(fullname):
            ...
            if ext == KID_EXT:
                try:
                    stat = compile_file(fullname, force, source, encoding, strip_dest_dir, entity_map)
                except Exception, e:
                    stat = e
                else:
                    yield (
            ...
            for res in compile_dir(fullname, maxlevels - 1, force, source, encoding, strip_dest_dir, entity_map):
                yield res