...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/test/test_kid.py
...
def test_load_template_func():
    t = kid.load_template(joinpath(template_dir, 'test_if.kid'), name='', cache=False)
    assert_template_interface(t)
    t2 = kid.load_template(joinpath(template_dir, 'test_if.kid'), name=template_package + 'test_if', cache=True)
    assert not t is t2
    t3 = kid.load_template(joinpath(template_dir, 'test_if.kid'), name=template_package + 'test_if', cache=True)
    assert t3 is t2


def test_load_template_func_with_ns():
    t = kid.load_template(joinpath(template_dir, 'templates.kid'), name='', cache=False)
...
def test_kid_files(test='*'):
    ...
    for f in test_files:
        ...
        try:
            ...
            print '\nTemplate: %s' % f
            try:
                kid.compiler.compile_file(f, source=True, force=True, encoding='utf-8')
            except:
