...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/test/test_attribute_interpolation.py
...
def test_simple_interpolation():
    ...
    s = kid.Template('<div x="$a $b $c" />', a='hello', b=123, c=0.5).serialize()
    assert s.endswith('<div x="hello 123 0.5" />')


def test_expression_interpolation():
    """Expression substitution."""
    s = kid.Template('<div x="${a+b+c}" />', a=1, b=2, c=3).serialize()
...
def test_interpolated_xml():
    ...
    sources = (
     '<meta content="$xml" />', '<meta xmlns:py="http://purl.org/kid/ns#"\n        content="temp" py:attrs="content=xml"/>')
