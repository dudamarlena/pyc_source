...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/test/test_templatepath.py
...
def setup_module(module):
    ...
    _create(_create(tmpdir1, 'shared', 'message.kid error.kid'), 'errors', 'error1.kid error2.kid error3.kid')
...
def test_file_in_directory():
    assert kfind(joinpath('members', 'index.kid')) == normpath(joinpath(tmpdir1, 'members', 'index.kid'))
...
def test_mod_python_bug():
    ...
    assert kfind('base.kid', rel=joinpath(tmpdir3, 'test.kid')) == joinpath(tmpdir3, 'base.kid')