...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/importer.py
...
__all__ = [
 'install', 'uninstall', 'import_template', 'get_template_name']
...
def install(ext=None, path=None):
    ...
    if path:
        ...
        for path in paths:
            kidpath = 'kid::%s::' % path
            syspath = [ path for path in syspath if not path.startswith(kidpath) ]
...
def uninstall(path=None):
    ...
    if path:
        ...
        for path in sys.path:
            p = path.split(':')
            if len(p) >= 5 and p[0] == 'kid' and not p[1] and not p[(-2)]:
        ...
        if remove_hook:
            if KidImporter in sys.path_hooks:
                sys.path_hooks = [ hook for hook in sys.path_hooks if hook != KidImporter ]
                sys.path_importer_cache.clear()
    else:
        sys.meta_path = [ importer for importer in sys.meta_path if not isinstance(importer, KidImporter) ]
...
def _create_module(code, name, filename, store=True, ns={}, exec_module=None):
    for recompiled in range(2):
        ...
        if getattr(mod, 'kid_version', None) == __version__:
            break
        if recompiled:
            raise TemplateImportError('Cannot recompile template file %r for Kid version %s' % (filename, __version__))
        template = KidFile(filename)
        template.stale = True
