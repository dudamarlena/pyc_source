...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/parser.py
...
__all__ = [
 'ElementStream', 'XML', 'document', 'Parser', 'ExpatParser', 'START', 'END', 'TEXT', 'COMMENT', 'PI', 'XML_DECL', 'DOCTYPE', 'LOCATION']
default_entity_map = {}
...
class ExpatParser(object):
    ...
    def _push_location(self, ev, stuff):
        self._push(LOCATION, (self._parser.CurrentLineNumber, self._parser.CurrentColumnNumber))
        self._push(ev, stuff)
    ...
    def __iter__(self):
        ...
        for (ev, stuff) in self._expat_stream():
            if ev == TEXT:
                ...
                (tag, attrib) = stuff
                tag = clarkname(tag)
                attrib = dict([ (clarkname(key), value) for (key, value) in attrib.items() ])
                parent = current
    ...
    def feed(self, data, isfinal=False):
        try:
            ...
            try:
                e = 'Error parsing XML%s:\n%s%s' % (e.filename and e.filename != '<string>' and ' file %r' % e.filename or '', get_expat_error(e), str(e))
            except Exception:
                pass
            else:
