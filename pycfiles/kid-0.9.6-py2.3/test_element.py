...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/test/test_element.py
...
import re
from kid.element import Element, SubElement, Comment, ProcessingInstruction, Fragment, QName, namespaces, encode_entity
...
def test_element_repr():
    element = Element('test0')
    print `element`
    match = re.match('^<Element test0 at [\\-a-f\\d]*>$', `element`)
...
def test_element_children():
    ...
    parent[:2] = (
     Element('firstone'), Element('firsttwo'))
...
def test_namespaces():
    element = Element('test', {'xmlns': 'default ns', 'xmlns:junk-ns0': 'junk-ns value 0', 'xmlns:junk-ns1': 'junk-ns value 1', 'py:pyattr': 'py value', 'attr': 'a new attr'})
    ns = namespaces(element)
    expected = {'': 'default ns', 'junk-ns0': 'junk-ns value 0', 'junk-ns1': 'junk-ns value 1'}
    assert ns == expected
