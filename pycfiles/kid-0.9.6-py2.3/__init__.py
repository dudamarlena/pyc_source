...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/__init__.py
...
import sys, os
assert sys.hexversion >= 33751040, 'Kid templates need Python 2.3 or later'
...
from kid.element import Element, SubElement, Comment, ProcessingInstruction, Fragment
from kid.parser import ElementStream, XML, document, _coalesce
from kid.filter import transform_filter
from kid.serialization import Serializer, PlainSerializer, XMLSerializer, HTMLSerializer, XHTMLSerializer
from kid.format import Format, output_formats
import kid.template_util as template_util
__all__ = [
 'KID_XMLNS', 'BaseTemplate', 'Template', 'enable_import', 'import_template', 'load_template', 'Element', 'SubElement', 'XML', 'document', 'Namespace', 'Serializer', 'XMLSerializer', 'HTMLSerializer', 'XHTMLSerializer', 'output_methods', 'Format', 'output_formats', 'filter', 'format', 'namespace', 'serialization', 'util']
...
if os.environ.get('KID_IMPORT_PATH'):
    enable_import(os.environ.get('KID_IMPORT_EXT'), os.environ['KID_IMPORT_PATH'])
...
def load_template(file, name='', cache=True, encoding=None, ns={}, entity_map=None, exec_module=None):
    ...
    if filename != '<string>':
        ...
        if not abs_filename:
            raise template_util.TemplateNotFound('%s (in %s)' % (filename, (', ').join(path.paths)))
    ...
    if filename == '<string>':
        ...
        template = compiler.KidFile(filename, force=False, encoding=encoding, entity_map=entity_map)
        code = template.compile(dump_code=cache, dump_source=os.environ.get('KID_OUTPUT_PY'))
    mod = importer._create_module(code, name, filename, store=cache, ns=ns, exec_module=exec_module)
    return mod
    return


output_methods = {'xml': XMLSerializer(decl=True), 'wml': XMLSerializer(decl=True, doctype='wml'), 'xhtml-strict': XHTMLSerializer(decl=False, doctype='xhtml-strict'), 'xhtml': XHTMLSerializer(decl=False, doctype='xhtml'), 'xhtml-frameset': XHTMLSerializer(decl=False, doctype='xhtml-frameset'), 'html-strict': HTMLSerializer(doctype='html-strict'), 'html': HTMLSerializer(doctype='html'), 'html-frameset': HTMLSerializer(doctype='html-frameset'), 'html-quirks': HTMLSerializer(doctype='html-quirks'), 'html-frameset-quirks': HTMLSerializer(doctype='html-frameset-quirks'), 'HTML-strict': HTMLSerializer(doctype='html-strict', transpose=True), 'HTML': HTMLSerializer(doctype='html', transpose=True), 'HTML-frameset': HTMLSerializer(doctype='html-frameset', transpose=True), 'HTML-quirks': HTMLSerializer(doctype='html-quirks', transpose=True), 'HTML-frameset-quirks': HTMLSerializer(doctype='html-frameset-quirks', transpose=True), 'plain': PlainSerializer()}
...
def Template(file=None, source=None, name=None, encoding=None, **kw):
    ...
    if name:
        ...
        mod = load_template(QuickTextReader(source), name=name or hex(id(source)), encoding=encoding)
    else:
        raise template_util.TemplateError('Must specify one of name, file, or source.')
    try:
        mod.Template.module = mod
    except Exception:
        raise template_util.TemplateImportError('Template could not be initialized.')
...
class BaseTemplate(object):
    ...
    def __init__(self, *args, **kw):
        ...
        for k in kw:
            if hasattr(BaseTemplate, k):
                raise template_util.TemplateAttrsError('Keyword argument %r is a reserved name.' % k)

        self.__dict__.update(kw)
        self._filters = [transform_filter]
        self._layout_classes = []

    def write(self, file, encoding=None, fragment=False, output=None, format=None):
        """
    ...
    def serialize(self, encoding=None, fragment=False, output=None, format=None):
        """
    ...
    def generate(self, encoding=None, fragment=False, output=None, format=None):
        """
