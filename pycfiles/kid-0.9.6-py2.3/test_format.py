...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kid/test/test_format.py
...
def serialize(source, format=None, output='html', encoding='utf-8'):
    ...
    return t.serialize(encoding=encoding, format=format, output=output, fragment=True)
...
def test_educate_quotes():
    ...
    assert educate_quotes('\'Hello\' "World"') == '‘Hello’ “World”'
    assert educate_quotes('\'Hello\', "World"!') == '‘Hello’, “World”!'
    assert educate_quotes('\'Hello,\' "World!"') == '‘Hello,’ “World!”'
    assert educate_quotes("('Hello World')") == '(‘Hello World’)'
    assert educate_quotes("'(Hello World)'") == '‘(Hello World)’'
    assert educate_quotes('"Isn\'t this fun?"') == '“Isn’t this fun?”'
    assert educate_quotes("The 70's and '80s weren't fun.") == 'The 70’s and ’80s weren’t fun.'


def test_educate_backticks():
    educate_backticks = Format().educate_backticks
    assert educate_backticks('`Hello\' ``World"') == '‘Hello’ “World”'


def test_educate_dashes():
    educate_dashes = Format().educate_dashes
    assert educate_dashes('Hello--World') == 'Hello–World'
    assert educate_dashes('Hello---World') == 'Hello—World'
...
def test_educate_ellipses():
    ...
    assert educate_ellipses('Hello... World. . .') == 'Hello… World…'
    assert educate_ellipses('..... --- . . . . .') == '..... --- . . . . .'


def test_stupefy():
    stupefy = Format().stupefy
    assert stupefy('‘Hello’—“World”…') == '\'Hello\'---"World"...'
...
def test_format_strip_blanks():
    ...
    assert s.endswith('<p> \t \n \t Hello, World \n</p>')
    xml = '<body><p> \t </p><p>\n n</p><p>n \n</p><p>\t \t</p></body>'
    for format in (format_strip_blanks, format_lrstrip_blanks, format_lstrip_blanks, format_rstrip_blanks):
        s = serialize(xml, format, 'xml')
...
def test_format_strip_lines():
    ...
    assert s.endswith('<p>\n \t Hello, World\n</p>')
    xml = '<body><p> \t </p><p> \nn\n\t\n </p><p>\t \t</p></body>'
    for format in (format_strip_lines, format_lrstrip_lines, format_lstrip_lines, format_rstrip_lines):
        s = serialize(xml, format, 'xml')
        assert s.endswith('<body><p /><p>\nn\n\n</p><p /></body>')
...
def test_format_indent():
    xml = '<html><body><h1>Hello, World</h1><div><p>Hello, <b>Kids</b>!</p></div></body></html>'
...
def test_format_wrap():
    ...
    s = serialize(xml, format)
    assert s.endswith("<body> It's a long way to Tipperary, It's a long way to go. It's a long way to\nTipperary To the sweetest girl I know. </body>")
...
def test_format_custom():
    ...
    s = serialize(xml, format)
    assert s.endswith('<body>Sometimes you need to be Dutch to understand Python.</body>')
    ...
    s = serialize(xml, format)
    assert s.endswith('<body>Sometimes you need to be a Python to understand a Python.</body>')
...
def test_predefined_formats():
    ...
    assert s.endswith("<body>\n<h1>Hello, World!</h1>\n<div>\n<p>This is a test page only.</p>\n<p><b>To be, or not to be:</b> that is the question: Whether 'tis nobler in the mind to\nsuffer The slings and arrows of outrageous fortune, Or to take arms against a\nsea of troubles, And by opposing <em>end</em> them?\n</p>\n<pre><div>Hello</div>\n\n<div>World</div></pre>\n</div>\n</body>\n</html>")
...
def test_custom_quotes():
    ...
    format = Format(educate=True, squotes='{}', dquotes=['<<', '>>'], apostrophe='`', dashes='-~', ellipsis='++')
...
def test_not_formatted():
    ...
    for tag in tags:
        ...
        xml = '<html><body><h1>Title</h1>%s<p>The\t\t\tend</p></body></html>' % formatted
        s = serialize(xml)
...
def test_nbsp():
    ...
    for output in ('xml', 'html', 'xhtml'):
        ...
        format = Format(entity_map={'\xa0': ' Mooney '})
        r = t.serialize(output=output, format=format, encoding='ascii')
...
def test_for_loops_with_formatting():
    ...
    expected = '<html>\n<head>\n\t<meta content="text/html; charset=utf-8" http-equiv="content-type">\n\t<title>Ticket 59</title>\n</head>\n<body>\n\t<div><img src="img1.png"><img src="img2.png"><img src="img3.png"></div>\n\t<ul>\n\t\t<li>Item 1</li>\n\t\t<li>Item 2</li>\n\t\t<li>Item 3</li>\n\t\t<li>Item 4</li>\n\t</ul>\n</body>\n</html>'
