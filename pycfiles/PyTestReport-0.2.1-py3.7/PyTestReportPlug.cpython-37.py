...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pytestreport\PyTestReportPlug.py
# Compiled at: 2020-02-21 04:15:26
# Size of source mod 2**32: 10429 bytes
import os, time, datetime, bisect
...
def pytest_addoption(parser):
    ...
    group.addoption('--pytest_report', action='store', dest='pytest_report', metavar='path', default=None, help='create html report file at given path.')
    group.addoption('--pytest_title', action='store', dest='pytest_title', metavar='path', default='PyTestReport', help='given title for report.')
    group.addoption('--pytest_desc', action='store', dest='pytest_desc', metavar='path', default='', help='given desc for report..')
    group.addoption('--pytest_theme', action='store', dest='pytest_theme', metavar='path', default=None, help='given theme for report.')
    group.addoption('--pytest_stylesheet', action='store', dest='pytest_stylesheet', metavar='path', default=None, help='given css file path for report.')
    group.addoption('--pytest_htmltemplate', action='store', dest='pytest_htmltemplate', metavar='path', default=None, help='given html file path for report.')
    group.addoption('--pytest_javascript', action='store', dest='pytest_javascript', metavar='path', default=None, help='given js file path for report.')
...
class TestResult:
    ...
    def output(self, cid, tid):
        ...
        return {'has_output':output and True or False, 
...
class HTMLReport(object):
        ...
        for cid, (cls, cls_results) in enumerate(sorted_result, 1):
            ...
            for result in cls_results:
                if result.outcome == 'Passed':
                    np += 1
                elif result.outcome in ('Failed', 'XPassed', 'XFailed'):
                    nf += 1
                else:
    ...
    def _generate_report(self, session):
        ...
        report_content = {'generator':'PyTestReport %s' % HTMLTestRunner.__version__, 
         'title':'%s' % self.config.getoption('pytest_title'), 
         'description':'%s' % self.config.getoption('pytest_desc'), 
         'environment':environment, 
         'report_summary':{'start_time':self.suite_start_time.strftime('%Y-%m-%d %H:%M:%S'), 
          'duration':duration, 
          'suite_count':len(tests), 
          'status':{'pass':self.passed, 
           'fail':self.failed + self.xfailed + self.xpassed, 
           'error':self.errors, 
           'skip':self.skipped, 
           'count':count}}, 
         'report_detail':{'tests':tests, 
          'count':count, 
          'pass':self.passed, 
          'fail':self.failed + self.xfailed + self.xpassed, 
          'error':self.errors, 
    ...
    def _save_report(self, report_content, theme=None, stylesheet=None, htmltemplate=None, javascript=None):
        ...
        with open(self.html_file, 'wb') as (fp):
            test_runner = HTMLTestRunner.HTMLTestRunner(fp, theme=theme, stylesheet=stylesheet, htmltemplate=htmltemplate,
