...
# Embedded file name: /Users/haoxun/Data/Project/pyenv-mirror-download/tests/test_misc.py
# Compiled at: 2016-03-31 05:16:58
# Size of source mod 2**32: 1855 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, os
from pyenv_mirror_download.locate_scripts import check_directory, check_script
from pyenv_mirror_download.parse_script import extract_filename, extract_urls
...
def test_missing_path():
    ...
    if not @py_assert1:
        @py_format5 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py4)s\n{%(py4)s = %(py2)s()\n}', ), (TARGET_DIR_PATH, @py_assert3)) % {'py0': @pytest_ar._saferepr(TARGET_DIR_PATH) if 'TARGET_DIR_PATH' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(TARGET_DIR_PATH) else 'TARGET_DIR_PATH', 'py4': @pytest_ar._saferepr(@py_assert3), 'py2': @pytest_ar._saferepr(check_directory) if 'check_directory' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(check_directory) else 'check_directory'}
...
def test_normal_path():
    ...
    if not @py_assert4:
        @py_format7 = @pytest_ar._call_reprcompare(('is', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n} is %(py6)s', ), (@py_assert2, @py_assert5)) % {'py6': @pytest_ar._saferepr(@py_assert5), 'py3': @pytest_ar._saferepr(@py_assert2), 'py1': @pytest_ar._saferepr(TEST_DIR_PATH) if 'TEST_DIR_PATH' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(TEST_DIR_PATH) else 'TEST_DIR_PATH', 'py0': @pytest_ar._saferepr(check_directory) if 'check_directory' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(check_directory) else 'check_directory'}
...
def test_check_script():
    ...
    if not @py_assert4:
        @py_format6 = ('' + 'assert %(py5)s\n{%(py5)s = %(py0)s(%(py1)s, %(py3)s)\n}') % {'py3': @pytest_ar._saferepr(@py_assert2), 'py1': @pytest_ar._saferepr(TARGET_DIR_PATH) if 'TARGET_DIR_PATH' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(TARGET_DIR_PATH) else 'TARGET_DIR_PATH', 'py0': @pytest_ar._saferepr(check_script) if 'check_script' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(check_script) else 'check_script', 'py5': @pytest_ar._saferepr(@py_assert4)}
...
def test_extract_filename():
    ...
    if not @py_assert2:
        @py_format7 = @pytest_ar._call_reprcompare(('==', ), (@py_assert2,), ('%(py1)s == %(py6)s\n{%(py6)s = %(py3)s(%(py4)s)\n}', ), (@py_assert0, @py_assert5)) % {'py6': @pytest_ar._saferepr(@py_assert5), 'py3': @pytest_ar._saferepr(extract_filename) if 'extract_filename' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(extract_filename) else 'extract_filename', 'py1': @pytest_ar._saferepr(@py_assert0), 'py4': @pytest_ar._saferepr(parsed_url) if 'parsed_url' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(parsed_url) else 'parsed_url'}
        @py_format9 = 'assert %(py8)s' % {'py8': @py_format7}
        raise AssertionError(@pytest_ar._format_explanation(@py_format9))
...
def test_extract_2_7():
 ...
 'Python-2.7.tgz')]
    ...
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert2,), ('%(py1)s == %(py3)s', ), (@py_assert0, url_filename)) % {'py3': @pytest_ar._saferepr(url_filename) if 'url_filename' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(url_filename) else 'url_filename', 'py1': @pytest_ar._saferepr(@py_assert0)}
