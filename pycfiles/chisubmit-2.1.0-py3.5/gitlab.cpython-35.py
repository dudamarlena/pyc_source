...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/repos/gitlab.py
# Compiled at: 2018-10-02 19:35:34
# Size of source mod 2**32: 16289 bytes
...
class GitLabConnection(RemoteRepositoryConnectionBase):
    ...
    def init_course(self, course, fail_if_exists=True):
        group = self._GitLabConnection__get_group(course)
        group_name = self._GitLabConnection__get_group_name(course)
        ...
        if group is None:
            ...
            if isinstance(new_group, gitlab.exceptions.HttpError):
                raise ChisubmitException("Could not create group '%s' (%s)" % (self._GitLabConnection__get_group_name(course), str(new_group)), new_group)
            return True
        else:
            return False
    ...
    def exists_user(self, course, course_user):
        ...
        user = self._GitLabConnection__get_user_by_username(gitlab_username)
        if user is None:
            return False
        else:
            return True
    ...
    def create_user(self, course, course_user):
        ...
        self.gitlab.createuser(name=gitlab_user_name, username=gitlab_user_username, password=gitlab_user_password, email=gitlab_user_email, provider='ldapmain', confirm=False, extern_uid=gitlab_extern_uid)
    ...
    def update_instructors(self, course):
        ...
        self._GitLabConnection__add_users_to_course_group(course, usernames, 'owner')
    ...
    def update_graders(self, course):
        ...
        self._GitLabConnection__add_users_to_course_group(course, usernames, 'developer')

    def create_team_repository(self, course, team, fail_if_exists=True, private=True):
        repo_name = self._GitLabConnection__get_team_namespaced_project_name(course, team)
        ...
        if not self.staging:
            ...
            for tm in team_members:
                gitlab_student = self._GitLabConnection__get_user_by_username(self._get_user_git_username(course, tm.student))
        ...
        project = self._GitLabConnection__get_team_project(course, team)
        ...
        if project is None:
            ...
            gitlab_project = self.gitlab.createproject(team.team_id, namespace_id=group['id'], description=repo_description, public=0)
            ...
            if not self.staging:
                for gitlab_student in gitlab_students:
                    rc = self.gitlab.addprojectmember(gitlab_project['id'], gitlab_student['id'], 'developer')
                    if rc == False:
    ...
    def update_team_repository(self, course, team):
        repo_name = self._GitLabConnection__get_team_namespaced_project_name(course, team)
        team_members = team.get_team_members()
        gitlab_project = self._GitLabConnection__get_team_project(course, team)
        for tm in team_members:
            gitlab_student = self._GitLabConnection__get_user_by_username(self._get_user_git_username(course, tm.student))
            if gitlab_student is None:
                raise ChisubmitException("GitLab user '%s' does not exist " % self._get_user_git_username(course, tm.student))
    ...
    def exists_team_repository(self, course, team):
        repo = self._GitLabConnection__get_team_project(course, team)
        if repo is None:
            return False
        else:
            return True
    ...
    def get_commit(self, course, team, commit_sha):
        project_api_id = self._GitLabConnection__get_team_project_api_id(course, team)
        ...
        if gitlab_commit == False:
            ...
            commit = GitCommit(gitlab_commit['id'], gitlab_commit['title'], gitlab_commit['author_name'], gitlab_commit['author_email'], parse(gitlab_commit['authored_date']), committer_name, committer_email, parse(gitlab_commit['committed_date']))
            return commit
    ...
    def get_submission_tag(self, course, team, tag_name):
        project_name = self._GitLabConnection__get_team_namespaced_project_name(course, team)
        gitlab_tag = self._GitLabConnection__get_tag(project_name, tag_name)
        if gitlab_tag is None:
            return
        tag = GitTag(name=gitlab_tag['name'], commit=self.get_commit(course, team, gitlab_tag['commit']['id']))
        return tag
    ...
    def __get_group(self, course):
        group = self.gitlab.getgroups(group_id=self._GitLabConnection__get_group_name(course))
        if group == False:
            return
        else:
            return group
    ...
    def __get_team_project(self, course, team):
        namespaced_project_name = self._GitLabConnection__get_team_namespaced_project_name(course, team)
    ...
    def __add_users_to_course_group(self, course, usernames, access_level):
        ...
        for username in usernames:
            user = self._GitLabConnection__get_user_by_username(username)
            if user is None:
    ...
    def __has_tag(self, project_name, tag_name):
        return self._GitLabConnection__get_tag(project_name, tag_name) is not None