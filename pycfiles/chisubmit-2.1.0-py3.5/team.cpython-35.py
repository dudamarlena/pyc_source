...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/client/team.py
# Compiled at: 2020-01-15 11:47:40
# Size of source mod 2**32: 19940 bytes
from builtins import str
from chisubmit.client.types import ChisubmitAPIObject, Attribute, APIStringType, APIIntegerType, APIBooleanType, APIObjectType, APIDateTimeType, APIDictType, APIDecimalType, Relationship
...
class Grade(ChisubmitAPIObject):
    _api_attributes = {'rubric_component_id': Attribute(name='rubric_component_id', attrtype=APIIntegerType, editable=False), 
     
     'rubric_component': Attribute(name='rubric_component', attrtype=APIObjectType(RubricComponent), editable=False), 
...
class Submission(ChisubmitAPIObject):
    _api_attributes = {'id': Attribute(name='id', attrtype=APIIntegerType, editable=False), 
     
     'extensions_used': Attribute(name='extensions_used', attrtype=APIIntegerType, editable=True), 
     
     'commit_sha': Attribute(name='commit_sha', attrtype=APIStringType, editable=True), 
     
     'submitted_at': Attribute(name='submitted_at', attrtype=APIDateTimeType, editable=True), 
     
     'submitted_by': Attribute(name='submitted_by', attrtype=APIStringType, editable=True), 
     
     'in_grace_period': Attribute(name='in_grace_period', attrtype=APIBooleanType, editable=True)}
    _api_relationships = {}


class SubmissionResponse(ChisubmitAPIObject):
    _api_attributes = {'submission': Attribute(name='registration', attrtype=APIObjectType(Submission), editable=False), 
     
     'extensions_before': Attribute(name='extensions_before', attrtype=APIIntegerType, editable=False), 
     
     'extensions_after': Attribute(name='extensions_after', attrtype=APIIntegerType, editable=False), 
     
     'extensions_needed': Attribute(name='extensions_needed', attrtype=APIIntegerType, editable=False), 
     
     'extensions_override': Attribute(name='extensions_override', attrtype=APIIntegerType, editable=False), 
...
class Registration(ChisubmitAPIObject):
    _api_attributes = {'assignment_id': Attribute(name='assignment_id', attrtype=APIStringType, editable=False), 
     
     'assignment': Attribute(name='assignment', attrtype=APIObjectType(Assignment), editable=False), 
     
     'grader_username': Attribute(name='grader_username', attrtype=APIStringType, editable=True), 
     
     'grader': Attribute(name='grader', attrtype=APIObjectType(Grader), editable=False), 
     
     'final_submission_id': Attribute(name='final_submission_id', attrtype=APIIntegerType, editable=True), 
     
     'final_submission': Attribute(name='final_submission', attrtype=APIObjectType(Submission), editable=False), 
     
     'grade_adjustments': Attribute(name='grade_adjustments', attrtype=APIDictType(APIDecimalType), editable=True), 
     
     'grading_started': Attribute(name='grading_started', attrtype=APIBooleanType, editable=True), 
     
     'gradescope_uploaded': Attribute(name='gradescope_uploaded', attrtype=APIBooleanType, editable=True)}
    _api_relationships = {'submissions': Relationship(name='submissions', reltype=APIObjectType(Submission)), 
     
     'grades': Relationship(name='grades', reltype=APIObjectType(Grade))}
    ...
    def get_submission(self, submission):
        ...
        headers, data = self._api_client._requester.request('GET', self.submissions_url + str(submission))
    ...
    def add_submission(self, commit_sha, extensions_used=None, submitted_at=None):
        ...
        headers, data = self._api_client._requester.request('POST', self.submissions_url, data=post_data)
        return Submission(self._api_client, headers, data)
    ...
    def add_grade(self, rubric_component, points=None):
        ...
        headers, data = self._api_client._requester.request('POST', self.grades_url, data=post_data)
    ...
    def submit(self, commit_sha, extensions_override=None, dry_run=False):
        ...
        headers, data = self._api_client._requester.request('POST', self.url + '/submit' + qs, data=post_data)
    ...
    def is_ready_for_grading(self):
        if self.final_submission is None:
            ...
            return is_submission_ready_for_grading(assignment_deadline=self.assignment.deadline, submission_date=self.final_submission.submitted_at, extensions_used=self.final_submission.extensions_used)
...
class TeamMember(ChisubmitAPIObject):
    _api_attributes = {'username': Attribute(name='username', attrtype=APIStringType, editable=False), 
     ...
     'student': Attribute(name='student', attrtype=APIObjectType(Student), editable=False), 
     
     'confirmed': Attribute(name='confirmed', attrtype=APIBooleanType, editable=True)}
    _api_relationships = {}


class Team(ChisubmitAPIObject):
    _api_attributes = {'team_id': Attribute(name='team_id', attrtype=APIStringType, editable=True), 
     
     'extensions': Attribute(name='extensions', attrtype=APIIntegerType, editable=True), 
     
     'active': Attribute(name='active', attrtype=APIBooleanType, editable=True)}
    _api_relationships = {'students': Relationship(name='students', reltype=APIObjectType(TeamMember)), 
     
     'assignments': Relationship(name='assignments', reltype=APIObjectType(Registration))}
    ...
    def get_team_member(self, username):
        ...
        headers, data = self._api_client._requester.request('GET', self.students_url + username)
    ...
    def add_team_member(self, user_or_username, confirmed=None):
        ...
        if not isinstance(user_or_username, (str, str)):
            ...
            headers, data = self._api_client._requester.request('POST', self.students_url, data=post_data)
            return TeamMember(self._api_client, headers, data)
    ...
    def get_assignment_registration(self, assignment_id):
        ...
        headers, data = self._api_client._requester.request('GET', self.assignments_url + assignment_id)
    ...
    def add_assignment_registration(self, assignment_or_assignment_id, grader_or_grader_username=None):
        ...
        :rtype: :class:`chisubmit.client.team.Registration`
        """
        if not isinstance(assignment_or_assignment_id, (str, str)):
            assert isinstance(assignment_or_assignment_id, Assignment)
            if not grader_or_grader_username is None:
                if not isinstance(grader_or_grader_username, (str, str)):
                    assert isinstance(grader_or_grader_username, Grader)
            ...
            if grader_or_grader_username is not None:
                if isinstance(grader_or_grader_username, (str, str)):
                    ...
                    if isinstance(grader_or_grader_username, User):
                        grader_username = grader_or_grader_username.user.username
                    post_data['grader_username'] = grader_username
                headers, data = self._api_client._requester.request('POST', self.assignments_url, data=post_data)
                return Registration(self._api_client, headers, data)