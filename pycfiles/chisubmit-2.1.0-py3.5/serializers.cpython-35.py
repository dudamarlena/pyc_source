...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/backend/api/serializers.py
# Compiled at: 2020-01-15 11:47:40
# Size of source mod 2**32: 27469 bytes
from rest_framework import serializers
from chisubmit.backend.api.models import Course, GradersAndStudents, AllExceptAdmin, Students, Student, Instructor, Grader, Team, Assignment, ReadWrite, OwnerPermissions, Read, RubricComponent, TeamMember, Registration, Submission, Grade
            ...
            if hasattr(self, 'hidden_fields'):
                ...
                for f in fields:
                    if f in self.hidden_fields:
                        if is_owner and OwnerPermissions.READ in owner_override.get(f, []) or roles.issubset(self.hidden_fields[f]):
                            data.pop(f)
        ...
        if course is not None and user is not None:
            ...
            for f in fields:
                if hasattr(self, 'hidden_fields') and f in self.hidden_fields:
                    ...
                    if f in self.readonly_fields and (is_owner and OwnerPermissions.WRITE in owner_override.get(f, []) or roles.issubset(self.readonly_fields[f])):
                        internal_value.pop(f)
...
class PersonRelatedField(RelatedField):
    default_error_messages = {'does_not_exist': ugettext_lazy('Object with username={value} does not exist.'), 
...
class InstructorSerializer(ChisubmitSerializer):
    url = serializers.SerializerMethodField()
    username = serializers.SlugRelatedField(source='user', queryset=User.objects.all(), slug_field='username')
...
class GraderSerializer(ChisubmitSerializer):
    ...
    git_username = serializers.CharField(max_length=64, required=False)
    git_staging_username = serializers.CharField(max_length=64, required=False)
    conflicts_usernames = PersonRelatedField(source='conflicts', queryset=Student.objects.all(), many=True, required=False)
...
class TeamMemberSerializer(ChisubmitSerializer):
    url = serializers.SerializerMethodField()
    username = PersonRelatedField(source='student', queryset=Student.objects.all())
...
class SubmissionSerializer(ChisubmitSerializer):
    ...
    submitted_by = serializers.SlugRelatedField(queryset=User.objects.all(), slug_field='username', required=False)
    readonly_fields = {'extensions_used': GradersAndStudents, 
...
class RegistrationSerializer(ChisubmitSerializer):
    url = serializers.SerializerMethodField()
    assignment_id = serializers.SlugRelatedField(source='assignment', queryset=Assignment.objects.all(), slug_field='assignment_id')
    assignment = AssignmentSerializer(read_only=True, required=False)
    grader_username = PersonRelatedField(source='grader', queryset=Grader.objects.all(), required=False, allow_null=True)
    grader = GraderSerializer(read_only=True, required=False)
    submissions_url = serializers.SerializerMethodField()
    final_submission_id = serializers.PrimaryKeyRelatedField(source='final_submission', queryset=Submission.objects.all(), required=False, allow_null=True)
    ...
    grade_adjustments = serializers.DictField(required=False, child=serializers.DecimalField(max_digits=5, decimal_places=2))
    ...
    hidden_fields = {'grader_username': Students, 
     'grader': Students, 
...
class RegistrationResponseSerializer(serializers.Serializer):
    ...
    team = TeamSerializer()
    team_members = serializers.ListField(child=TeamMemberSerializer())
...
class GradeSerializer(ChisubmitSerializer):
    rubric_component_id = serializers.PrimaryKeyRelatedField(source='rubric_component', queryset=RubricComponent.objects.all())
