...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/chisubmit/repos/grading.py
...
class GradingGitRepo(object):
    ...
    def push_grading_branch_to_students(self):
        ...
        self._GradingGitRepo__push_grading_branch('origin')

    def push_grading_branch_to_staging(self):
        if self.staging_only:
            self._GradingGitRepo__push_grading_branch('origin', push_master=True)
        else:
            self._GradingGitRepo__push_grading_branch('staging', push_master=True)

    def pull_grading_branch_from_students(self):
        assert not self.staging_only
        self._GradingGitRepo__pull_grading_branch('origin', pull_master=True)

    def pull_grading_branch_from_staging(self):
        if self.staging_only:
            self._GradingGitRepo__pull_grading_branch('origin')
        else:
            self._GradingGitRepo__pull_grading_branch('staging')
