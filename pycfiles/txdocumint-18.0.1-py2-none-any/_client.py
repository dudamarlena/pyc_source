...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathan/Coding/txdocumint/txdocumint/_client.py
# Compiled at: 2016-02-08 14:56:52
import json
from operator import itemgetter
import treq
from twisted.python.url import URL
from txdocumint.error import DocumintError, DocumintErrorCause, MalformedDocumintError
...
def post_json(request, url, **kw):
    ...
    return json_request(request, 'POST', url, headers=headers, **kw)
    ...
    def _raise_error(data, response):
        if content_type(response.headers) == 'application/json':
            try:
                ...
                raise DocumintError(causes=[ DocumintErrorCause(cause.get('type'), cause.get('reason'), cause.get('description')) for cause in causes
                                           ])
            except ValueError:
                pass
...
def content_type(headers):
    ...
    """
    return next(iter(headers.getRawHeaders('Content-Type', default=[])), 'application/octet-stream').split(';', 1)[0]
...
class Session(object):
    ...
    def store_content(self, fileobj, content_type):
        ...
        """
        d = post_json(self._request, self._session_info['store-content'], data=fileobj, headers={'Content-Type': content_type})
        d.addCallback(itemgetter('links'))
        d.addCallback(itemgetter('self'))
        return d
    ...
    def perform_action(self, action_and_parser):
        ...
        d = post_json(self._request, self._session_info['perform'], data=json.dumps(action))
    ...
    def delete(self):
        ...
        """
        d = self._request('DELETE', self._session_info['self'])
...
def create_session(endpoint, request=treq.request):
    ...
    :type request: Callable mimicing the signature of `treq.request`.
    """
    uri = URL.fromText(endpoint).child('sessions').child('')
...
def get_session(uri, request=treq.request):
    ...
    """
    request = documint_request_factory(request)
    d = json_request(request, 'GET', uri.encode('utf-8'))
    d.addCallback(itemgetter('links'))
    d.addCallback(Session, request)
    return d
