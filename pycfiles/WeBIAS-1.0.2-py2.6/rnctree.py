...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/gnosis/xml/relax/rnctree.py
...
PAIRS = {'BEG_BODY': ('END_BODY', BODY), 'BEG_PAREN': (
               'END_PAREN', GROUP), 
   'BEG_ANNO': (
              'END_ANNO', ANNOTATION)}
TAGS = {ONE: 'group', SOME: 'oneOrMore', 
   MAYBE: 'optional', 
   ANY: 'zeroOrMore'}
...
class Node(object):
    ...
    def xmlnode(self, indent=0):
        ...
        for x in self.value:
            if not isinstance(x, Node):
                ...
                write('  ' * indent + '<a:documentation>%s</a:documentation>' % x.value)
    ...
    def add_ns(self, xml):
        ...
        global OTHER_NAMESPACE
        lines = xml.split('\n')
        self.nest_annotations(lines)
...
def type_bodies(nodes):
    ...
    while 1:
        if i >= len(nodes):
            break
        if nodetypes(nodes[i:i + 3]) == (ELEM, NAME, BODY) or nodetypes(nodes[i:i + 3]) == (ATTR, NAME, BODY):
...
def scan_NS(nodes):
    ...
    for node in nodes:
        if node.type == DEFAULT_NS:
            ...
            OTHER_NAMESPACE['a'] = '"http://relaxng.org/ns/compatibility/annotations/1.0"'
