...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/data.py
# Compiled at: 2015-04-13 16:10:38
import sqlalchemy.ext.declarative, hashlib
from util import *
import cherrypy
...
class User(Base):
    ...
    def authenticate(self, passwd):
        if self.status not in ('OK', 'FORGOTTEN'):
            return False
...
class File(Base):
    ...
    __table_args__ = (
     UniqueConstraint('request_id', 'run_id', 'path', 'name'),)
...
class Scheduler(Base):
    ...
    apps = relationship('Application', secondary=Table('sched_apps', Base.metadata, Column('app_id', String(50), ForeignKey('applications.id')), Column('sched_id', String(50), ForeignKey('schedulers.id'))))
...
class ACL(Base):
    ...
    __mapper_args__ = {'polymorphic_on': acl_class}
...
class ReportSetting(Base):
    ...
    __mapper_args__ = {'polymorphic_on': report_class}
    ...
    def is_due(self, date, last=None):
        if self.frequency == 'N':
            ...
            if last is None:
                return True
            delay_dict = {'M': 60, 'H': 3600, 'D': 86400, 'W': 604800}
...
class ReportSettingApp(ReportSetting):
    app_id = Column(String(50), ForeignKey('applications.id', onupdate='cascade', ondelete='cascade'), nullable=True)
    __mapper_args__ = {'polymorphic_identity': 'ReportSettingApp'}
...
class ReportSettingSched(ReportSetting):
    sched_id = Column(String(50), ForeignKey('schedulers.id', onupdate='cascade', ondelete='cascade'), nullable=True)
    __mapper_args__ = {'polymorphic_identity': 'ReportSettingSched'}
...
class ReportSettingServ(ReportSetting):
    __mapper_args__ = {'polymorphic_identity': 'ReportSettingServ'}
