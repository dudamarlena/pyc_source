...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/field.py
# Compiled at: 2015-04-13 16:10:44
import tempfile, subprocess, os, webias.gnosis.xml.objectify as objectify, cherrypy, data, util, auth
    ...
    def validate(self, kwds):
        if self.getValue(kwds) == '':
            if self.isOptional():
                ...
                return (
                 'INVALID', [Message('INVALID', self, 'Parameter %s is missing!' % self.info)])
        else:
...
class Integer(Field):
    ...
    def check_value(self, val):
        try:
            ...
            return (
             'INVALID', [Message('INVALID', self, 'Parameter %s should be an integer.' % self.info)])
...
class Float(Field):
    ...
    def check_value(self, val):
        try:
            ...
            return (
             'INVALID', [Message('INVALID', self, 'Parameter %s should be an decimal number.' % self.info)])
...
class Text(Field):
    ...
    def check_value(self, val):
        if str(val).isalnum():
            ...
            return (
             'INVALID', [Message('INVALID', self, 'Parameter %s should be alphanumeric.' % self.info)])
...
class Email(Text):
    ...
    def check_value(self, val):
        try:
            ...
            return (
             'INVALID', [Message('INVALID', self, 'Parameter %s should be an e-mail address.' % self.info)])
...
class Select(Field):
    ...
    def check_value(self, val):
        try:
            ...
            return (
             'INVALID', [Message('INVALID', self, 'Parameter %s - invalid option value.' % self.info)])
        except ValueError:
            return (
             'INVALID', [Message('INVALID', self, 'Parameter %s - option value should be integer.' % self.info)])
