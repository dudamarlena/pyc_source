...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/application.py
# Compiled at: 2015-09-22 08:31:31
import cherrypy, webias.gnosis.xml.objectify as objectify, config, time, data, query, field, sys, traceback
try:
    import biofield
except ImportError:
...
class Application(objectify._XO_):
    ...
    def validate_acl(self, *args, **kwargs):
        ...
        cherrypy.session['validate_result'] = [
         status, messages, pars]
        if status != 'VALID':
            return self._acl
        else:
    ...
    def result(self, uuid, runid=None):
        ...
        if request.status == 'FINISHED' or runid != None:
            ...
            if request.status in ('READY', 'PROCESSING'):
