...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/biofield.py
# Compiled at: 2015-04-13 16:10:38
import tempfile, subprocess, os, webias.gnosis.xml.objectify as objectify, cherrypy, data, util, Bio.PDB, auth, field, biodb
...
class StructureCode(field.Text):
    ...
    def check_value(self, val):
        if self.getDB(val) != None:
            ...
            return (
             'INVALID', [field.Message('INVALID', self, 'Parameter %s is not a valid structure code.' % self.info)])
    ...
    def check_file(self, fieldstorage):
        ...
        try:
            try:
                ...
                return (
                 'INVALID', [field.Message('INVALID', self, 'Problem with %s: %s\n' % (self.info, e.message))])
