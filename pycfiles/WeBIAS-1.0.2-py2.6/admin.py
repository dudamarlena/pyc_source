...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/admin.py
# Compiled at: 2015-04-13 16:10:38
import cherrypy, config, data, sqlalchemy, auth, statistics
from util import *
...
class Applications(object):
    ...
    def _acl(self):
        ...
        user_list = map(lambda x: x.login, data.ApplicationACL.get_allowed_users(session, ('ADMIN',
                                                                                           'VIEW')).all())
        return [
         user_list, 'admin']
...
class Schedulers:
    ...
    def _acl(self):
        session = cherrypy.request.db
        user_list = map(lambda x: x.login, data.SchedulerACL.get_allowed_users(session, ('ADMIN',
                                                                                         'VIEW')).all())
        return [
         user_list, 'admin']
