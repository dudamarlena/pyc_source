...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webias/statistics.py
# Compiled at: 2015-04-25 08:10:39
import cherrypy, config, time, datetime, data, auth, sys, traceback
from util import *
...
class ServerLog:
    ...
    def render(self, p=1, sched_id=None, title=''):
        ...
        if sched_id is None:
            filter = [
             self._class.date >= date]
        else:
            filter = [
             self._class.date >= date, self._class.sched_id == sched_id]
