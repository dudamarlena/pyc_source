...
# Embedded file name: /home/jjmontes/git/codenamize/codenamize/codenamize.py
# Compiled at: 2018-06-21 15:10:14
# Size of source mod 2**32: 35927 bytes
import six, argparse, hashlib, functools, sys
ADJECTIVES = [
...
def codenamize_particles(obj=None, adjectives=1, max_item_chars=0, hash_algo='md5'):
    ...
    if max_item_chars > 9:
        max_item_chars = 0
    particles = [
     NOUNS] + [ADJECTIVES for _ in range(0, adjectives)]
    if max_item_chars > 0:
...
def print_test():
    ...
    print('TESTS')
    l1 = list(set([codenamize(a, 1, 3) for a in range(0, 2777)]))
    l2 = list(set([codenamize(a, 2, 3) for a in range(0, 66257)]))
    ...
    print("  ('100001', 1 adj, max 5) => %s (must be 'funny-boat')" % codenamize('100001', 1, 5))
    print("  (u'100001', 1 adj, max 5) => %s (must be 'funny-boat')" % codenamize('100001', 1, 5))
...
def main():
    ...
    parser.add_argument('-a', '--hash_algorithm', dest='hash_algo', action='store', type=str, default='md5', help='the algorithm to use to hash the input value (default: md5)')
    ...
    parser.add_argument('--list_algorithms', dest='list_algorithms', action='store_true', help='List the hash algorithms available')
    parser.add_argument('--version', action='version', version='codenamize 1.2.3')
