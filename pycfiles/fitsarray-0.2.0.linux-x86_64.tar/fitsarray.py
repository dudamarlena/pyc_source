...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/fitsarray/fitsarray.py
# Compiled at: 2010-12-12 22:05:55
import numpy as np, pyfits, copy
...
class InfoArray(np.ndarray):
    ...
    def __new__(subtype, shape=None, data=None, dtype=float, buffer=None, offset=0, strides=None, order=None, header=None):
...
class FitsArray(InfoArray):
    ...
    def __new__(subtype, shape=None, data=None, file=None, ext=0, dtype=float, buffer=None, offset=0, strides=None, order=None, header=None):
        if shape is not None:
            obj = np.ndarray.__new__(subtype, shape, dtype, buffer, offset, strides, order)
        elif data is not None:
            obj = np.array(data).view(subtype)
