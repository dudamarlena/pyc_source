...
# [GCC 8.4.0]
# Embedded file name: F:\GitCode\trangevi-azurecli\azure\cli\command_modules\ml\service\_realtimeutilities.py
# Compiled at: 2017-10-11 11:42:50
"""
Utilities to create and manage realtime web services.

"""
from __future__ import print_function
import docker, os, tarfile, uuid, requests, tempfile
from azure.cli.core.util import CLIError
from ._docker_utils import get_docker_client
from .._constants import AZ_CONTAINER_NAME
...
def get_sample_data(swagger_url, headers=None, verbose=False):
    ...
    if swagger_spec_response.status_code == 404:
        ...
        return ''
    if swagger_spec_response.status_code == 503:
        if verbose:
            print('[Debug] Received a 503 - no such service.')
        return default_retval
    if swagger_spec_response.status_code != 200:
        if verbose:
    ...
    try:
        ...
        if verbose:
            print(("[Debug] Could not deserialize the service's swagger specification. Malformed json {}.").format(swagger_spec_response))
        return default_retval
...
def get_local_webservices(service_name=None, verb=False, context=cli_context):
    ...
    try:
        ...
        if not containers and service_name:
            raise CLIError(('No service with id {} is running locally.').format(service_name))
        services = [ {'Name': container.attrs['Config']['Labels']['amlid'], 'Id': container.attrs['Config']['Labels']['amlid'], 'Image': container.attrs['Config']['Image'] if 'Image' in container.attrs else 'Unknown', 'State': container.attrs['State']['Status'], 'UpdatedAt': container.attrs['Created']} for container in containers
                   ]
    except docker.errors.DockerException as exc:
        raise CLIError(('Unable to list containers: {}').format(exc))
...
def get_local_realtime_service_port(service_name, verbose):
    ...
    if len(containers) == 1:
        ...
        raise CLIError(('Unable to locate expected port info in Ports dictionary: {}').format(port_info_dict))
    raise CLIError(('Expected exactly one container with label {} and found {}.').format(('amlid={}').format(service_name), len(containers)))