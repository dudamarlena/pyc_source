...
# [GCC 8.4.0]
# Embedded file name: F:\GitCode\trangevi-azurecli\azure\cli\command_modules\ml\service\_batchutilities.py
# Compiled at: 2017-05-26 14:46:36
"""
...
BATCH_EXTENSION_TO_ASSET_DICT = {'.py': BATCH_PYTHON_ASSET, '.jar': BATCH_JAR_ASSET}
...
def batch_get_url(context, fmt, *args):
    ...
    elif not context.in_local_mode():
...
def batch_get_parameter_str(param_dict):
    ...
    """
    letter = '--out' if param_dict['Direction'] == 'Output' else '--in' if param_dict['Kind'] == 'Reference' else '--param'
...
def batch_get_job_description(context, http_content):
    ...
    if 'YarnAppId' in json_obj:
        ...
        return_str += ('Logs available at: https://{}.azurehdinsight.net/yarnui/hn/cluster/app/{}\n').format(context.hdi_domain, json_obj['YarnAppId'])
...
def batch_create_parameter_entry(name, kind, direction):
    ...
    """
    return_value = {'Id': name, 'IsRuntime': True, 
       'IsOptional': False, 
       'Kind': kind, 
       'Direction': direction}
    if ':' in name:
...
def batch_create_parameter_list(arg_list):
    ...
    """
    return [ batch_create_parameter_entry(name, kind, direction) for name, direction, kind in arg_list
           ]
...
def batch_get_acceptable_storage(context):
    ...
    try:
        success, content = get_success_and_resp_str(context, context.http_call('get', url, auth=context.get_batch_auth()))
    except requests.ConnectionError:
        raise InvalidStorageException(('Error connecting to {}. Please confirm SparkBatch app is healthy.').format(url))
...
def batch_env_is_valid(context):
    ...
    if hdi_exists:
        ...
        if app_ping_return_code is None or app_ping_return_code == 404:
            print(('AML Batch is not currently installed on {0}. Please install the app.').format(batch_get_url(context, BATCH_URL_BASE_FMT, context.hdi_domain)))
        elif app_ping_return_code == 200:
            app_present = True
        elif app_ping_return_code == 403:
            print(('Authentication failed on {}. Check your AML_HDI_USER and AML_HDI_PW environment variables.').format(batch_get_url(context, BATCH_URL_BASE_FMT, context.hdi_domain)))
            print('For help setting up environment, run')
            print('  az ml env about')
            print('')
        else:
            print(('Unexpected return code {} when querying AzureBatch at {}.').format(app_ping_return_code, batch_get_url(context, BATCH_URL_BASE_FMT, context.hdi_domain)))
            print('If this error persists, contact the SparkBatch team for more information.')
...
def batch_env_and_storage_are_valid(context):
    ...
    if context.az_account_name not in acceptable_storage:
        print(('Environment storage account {0} not found when querying server for acceptable storage. Available accounts are: {1}').format(context.az_account_name, (', ').join(acceptable_storage)))
        return False
    return True
...
class ScoringUrlFunction(ValueFunction):
    ...
    def evaluate(self, context):
        return batch_get_url(context, BATCH_SINGLE_JOB_FMT, self.json_obj['Id'], '<job_id>')
...
batch_view_service_usage_header_to_fn_dict = OrderedDict([
 ...
 (
  'Inputs',
  ConditionalListTraversalFunction(('Parameters', ), condition=lambda param: param['Kind'] == 'Reference' and param['Direction'] == 'Input', action=lambda param: param['Id'])),
 (
  'Outputs',
  ConditionalListTraversalFunction(('Parameters', ), condition=lambda param: param['Kind'] == 'Reference' and param['Direction'] == 'Output', action=lambda param: param['Id'])),
 (
  'Parameters',
  ConditionalListTraversalFunction(('Parameters', ), condition=lambda param: param['Kind'] == 'Value' and param['Direction'] == 'Input', action=lambda param: param['Id']))])
...
def validate_and_split_run_param(raw_param):
    ...
    if ':' not in raw_param:
        ...
        return (
         True, raw_param.split(':')[0], (':').join(raw_param.split(':')[1:]))
        return
...
def create_batch_docker_image(score_file, dependencies, service_name, target_runtime, custom_ice_url, verb, context):
    ...
    if context.acr_home is None or context.acr_user is None or context.acr_pw is None:
        ...
        acr_exists = True
    if not match('[a-zA-Z0-9\\.-]+', service_name):
        ...
        if dependencies is not None:
            ...
            for dependency in dependencies:
                status, location, filename = upload_dependency(context, dependency, verbose)
                if status < 0:
                    print(('Error resolving dependency: no such file or directory {}').format(dependency))
                    return
                dependency_count += 1
                new_asset = {'mimeType': 'application/octet-stream', 'id': str(dependency), 
                   'location': location}
        ...
        if target_runtime == 'spark-py':
            ...
            wasb_config = "spark.sparkContext._jsc.hadoopConfiguration().set('fs.azure.account.key." + context.az_account_name + ".blob.core.windows.net','" + context.az_account_key + "')"
            code = ('{}\n{}\n{}\n{}\n\n\n').format(preamble, wasb_config, dependency_injection_code, code)
        if verbose:
            print(code)
        az_container_name = 'amlbdpackages'
        az_blob_name = str(uuid.uuid4()) + '.py'
        bbs = BlockBlobService(account_name=context.az_account_name, account_key=context.az_account_key)
        bbs.create_container(az_container_name)
        bbs.create_blob_from_text(az_container_name, az_blob_name, code, content_settings=ContentSettings(content_type='application/text'))
        blob_sas = bbs.generate_blob_shared_access_signature(az_container_name, az_blob_name, BlobPermissions.READ, datetime.utcnow() + timedelta(days=30))
        package_location = ('http://{}.blob.core.windows.net/{}/{}?{}').format(context.az_account_name, az_container_name, az_blob_name, blob_sas)
        if verbose:
        ...
        while try_number < max_retries:
            try:
                ice_put_result = requests.put(create_url, headers=headers, data=json.dumps(json_payload), timeout=ice_connection_timeout)
                break
            except (requests.ConnectionError, requests.exceptions.ReadTimeout):
                if try_number < max_retries:
                    try_number += 1
                    continue
                print('Error: could not connect to Azure ML. Please try again later. If the problem persists, please contact deployml@microsoft.com')
                return

        if ice_put_result.status_code == 401:
            print("Invalid API key. Please update your key by running 'az ml env key -u'.")
            return
        if ice_put_result.status_code != 201:
        ...
        while 'Provisioning State' in response_payload:
            ...
            if response_payload['Provisioning State'] == 'Running':
                ...
                if verbose:
                    ...
                    sys.stdout.flush()
                continue
            elif response_payload['Provisioning State'] == 'Succeeded':
                acs_payload = response_payload['ACS_PayLoad']
                acs_payload['container']['docker']['image'] = json_payload['properties']['registryInfo']['location'] + '/' + service_name
