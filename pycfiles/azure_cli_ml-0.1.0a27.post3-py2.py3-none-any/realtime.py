...
# [GCC 8.4.0]
# Embedded file name: F:\GitCode\trangevi-azurecli\azure\cli\command_modules\ml\service\realtime.py
# Compiled at: 2017-10-02 12:58:28
"""
...
def realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, target_runtime, app_insights_logging_enabled, model_data_collection_enabled, model_files, num_replicas, image_id, conda_file, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context=cli_context):
    _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, target_runtime, app_insights_logging_enabled, model_data_collection_enabled, model_files, num_replicas, image_id, conda_file, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context)


def _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, target_runtime, app_insights_logging_enabled, model_data_collection_enabled, model_files, num_replicas, image_id, conda_file, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context=cli_context):
    ...
    if app_insights_logging_enabled:
        ...
        if app_insights_bool is None:
            raise CLIError(('Error, invalid value provided for app_insights_logging_enabled: {}').format(app_insights_logging_enabled))
    ...
    if not image_id:
        image_create_success, image_id = _image_create(service_name[:30], '', None, driver_file, schema_file, dependencies, target_runtime, requirements, conda_file, model_files, verb, context)
        if image_create_success is not SUCCESS_RETURN_CODE:
            return
    if context.in_local_mode():
        return local_service_create(service_name, image_id, app_insights_logging_enabled, model_data_collection_enabled, verb, context)
    else:
        return mms_service_create(image_id, service_name, app_insights_logging_enabled, model_data_collection_enabled, num_replicas, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context)
...
def realtime_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context=cli_context):
    _realtime_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context)


def _realtime_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context=cli_context):
    if context.in_local_mode():
        raise CLIError('Error, service update not supported for local services.')
    return mms_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests, verb, context)
...
def _realtime_service_show(service_name, service_id, verb, context):
    ...
    if not service_id and not service_name:
        raise CLIError('Error, one of service name or service id must be provided')
    if context.in_local_mode():
...
def _realtime_service_keys_handling(service_id, regen, key, verb, context=cli_context):
    if context.in_local_mode():
        raise CLIError('Key operations not supported for local services')
...
def realtime_service_logs(service_id, request_id, kube_config, verb, context=cli_context):
    _realtime_service_logs(service_id, request_id, kube_config, verb, context)


def _realtime_service_logs(service_id, request_id, kube_config, verb, context):
    if not service_id:
        raise CLIError('Service ID must be provided.')
    if context.in_local_mode():
        return local_service_logs(service_id, request_id, verb)
    return mms_service_logs(service_id, request_id, kube_config, verb, context)