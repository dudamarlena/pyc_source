...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tickle/lambdas.py
# Compiled at: 2017-08-18 15:54:20
"""
Lambda serialization for tikcle.

Provides the classes required for serialization of lambdas and functions.
"""
import new, uuid, types, marshal, __builtin__
...
class LambdaTicklable(Ticklable):
    ...
    def dis_cells(func):
        ...
        for k, n in enumerate(func.__code__.co_freevars):
            ...
            obj = type(lambda : 0)((lambda x: lambda : x)(0).__code__, {}, None, None, (cell,))()
            values[n] = obj
    ...
    def __getstate__(self):
        ...
        for k, v in self.data.__globals__.items():
            if v is self.data:
                continue
            if not (isinstance(v, types.ModuleType) or isinstance(v, types.FunctionType) or isinstance(v, type) or isinstance(v, object) or type(v) in [int, long, float, str, unicode]):
                continue
