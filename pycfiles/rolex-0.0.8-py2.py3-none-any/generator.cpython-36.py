...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/rolex-project/rolex/generator.py
# Compiled at: 2019-01-28 15:42:47
# Size of source mod 2**32: 13834 bytes
import random
...
from .util import from_utctimestamp, to_utctimestamp, from_ordinal, to_ordinal
_valid_freq = [
 'days', 'day', 'd',
 'hours', 'hour', 'h',
...
def time_series(start=None, end=None, periods=None, freq='1day', normalize=False, return_date=False):
    """
...
def _randn(size, rnd_generator, *args, **kwargs):
    ...
    try:
        ...
        raise ValueError("'size' has to be int or tuple. e.g. 6 or (2, 3)")
...
def _rnd_datetime(start, end):
    ...
    """
    return from_utctimestamp(random.randint(int(to_utctimestamp(start)), int(to_utctimestamp(end))))
...
def rnd_datetime_list_high_performance(size, start=datetime(1970, 1, 1), end=None):
    ...
    if has_np:
        return [from_utctimestamp(v * interval_range + start_ts) for v in np.random.random(size)]
    else:
        return [from_utctimestamp(random.random() * interval_range + start_ts) for _ in range(size)]
