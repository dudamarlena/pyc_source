...
def add_months(datetime_like_object, n, return_date=False):
    ...
    **中文文档**
    ...
    try:
        a_datetime = datetime(year,
          month, (a_datetime.day), (a_datetime.hour),
          (a_datetime.minute), (a_datetime.second), (a_datetime.microsecond),
          tzinfo=(a_datetime.tzinfo))
    except ValueError:
        month_from_ordinary += 1
        year, month = divmod(month_from_ordinary, 12)
        a_datetime = datetime(year,
          month, 1, (a_datetime.hour),
          (a_datetime.minute), (a_datetime.second), (a_datetime.microsecond),
          tzinfo=(a_datetime.tzinfo))
...
def add_years(datetime_like_object, n, return_date=False):
    ...
    **中文文档**
    ...
    a_datetime = parser.parse_datetime(datetime_like_object)
    try:
        a_datetime = datetime((a_datetime.year + n),
          (a_datetime.month), (a_datetime.day), (a_datetime.hour),
          (a_datetime.minute), (a_datetime.second), (a_datetime.microsecond),
          tzinfo=(a_datetime.tzinfo))
    except ValueError:
