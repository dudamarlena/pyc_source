...
# Compiled at: 2015-04-15 07:02:34
import shutil, socket, struct, tempfile, threading, unittest
...
class TFTPServerTestCase(unittest.TestCase):
    ...
    def setUp(self, handler=FileSystemHandler, handler_args=None):
        ...
        self.server = TFTPServer(host='127.0.0.1', port=0, root=self.tftp_root, handler=handler, handler_args=handler_args)
    ...
    def get_file(self, filename, mode='octet', options=None):
        ...
        if options is not None:
            request += ('').join([ '%s\x00%s\x00' % (k, v) for k, v in options.items() ])
