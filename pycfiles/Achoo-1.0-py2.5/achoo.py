...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/achoo.py
# Compiled at: 2008-08-09 10:37:29
r"""
...
class ValueAssertionBuilder(object):
    ...
    def equal_to(self, other):
        ...
        if self.value != other and not self.invert_sense:
            raise self._error(_("Value `%s' expected to equal `%s'"), _("Value `%s' not expected to equal `%s'"), other)
    ...
    def same_as(self, other):
        ...
        if self.value is not other and not self.invert_sense:
            raise self._error(_("Value `%s' expected to be `%s'"), _("Value `%s' not expected to be `%s'"), other)
        self.invert_sense = False
        return self
    ...
    def is_not_none(self):
        ...
        if self.value is None and not self.invert_sense:
            raise self._error(_("Value `%s' expected to be `%s'"), _("Value `%s' not expected to be `%s'"), None)
        self.invert_sense = False
        return self
    ...
    def is_a(self, clazz):
        ...
        if not isinstance(self.value, clazz) and not self.invert_sense:
            raise self._error(_("Value `%s' expected to be a `%s'"), _("Value `%s' not expected to be a `%s'"), clazz)
        self.invert_sense = False
        return self
    ...
    def length(self, length):
        ...
        if len(self.value) != length and not self.invert_sense:
            raise self._error(_("Length of `%s' expected to equal `%s'"), _("Length of `%s' not expected to equal `%s'"), length)
        self.invert_sense = False
        return self
    ...
    def contains(self, element):
        ...
        if element not in self.value and not self.invert_sense:
            raise self._error(_("Value `%s' expected to contain `%s'"), _("Value of `%s' not expected to contain `%s'"), element)
        self.invert_sense = False
        return self
    ...
    def index(self, index):
        ...
        if self.invert_sense:
            raise AssertionError(_("A call to `index' cannot be preceded by `is_not'"))
        try:
            return ValueAssertionBuilder(self.value[index])
        except KeyError:
            raise self._error(_("Value `%s' expected to contain key `%s'"), None, index)
        except IndexError:
            raise self._error(_("Value `%s' expected to contain index `%s'"), None, index)
    ...
    def _error(self, message, inverse_message, other):
        ...
        """
        return AssertionError((message if not self.invert_sense else inverse_message) % (self.value, other))
...
class CallableAssertionBuilder(object):
    ...
    def raises(self, error):
        ...
        try:
            ...
            raise AssertionError(_("Calling `%s' did not raise any error") % self.callable)
