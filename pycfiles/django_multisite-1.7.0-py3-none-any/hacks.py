...
# [GCC 8.4.0]
# Embedded file name: /home/padraic/eco/src/django-multisite/multisite/hacks.py
# Compiled at: 2020-03-06 10:31:39
...
class SiteCache(object):
    ...
    def __init__(self, cache=None):
        ...
        if cache is None:
            cache_alias = getattr(settings, b'CACHE_MULTISITE_ALIAS', b'default')
            self._key_prefix = getattr(settings, b'CACHE_MULTISITE_KEY_PREFIX', settings.CACHES[cache_alias].get(b'KEY_PREFIX', b''))
            cache = caches[cache_alias]
        else:
            self._key_prefix = getattr(settings, b'CACHE_MULTISITE_KEY_PREFIX', cache.key_prefix)
        self._cache = cache
        return

    def _get_cache_key(self, key):
        return b'sites.%s.%s' % (self.key_prefix, key)
    ...
    def set(self, key, value, *args, **kwargs):
        self._cache.set(key=self._get_cache_key(key), value=self._clean_site(value), *args, **kwargs)
