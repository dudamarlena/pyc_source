...
# [GCC 8.4.0]
# Embedded file name: /home/sachin/random/codechef-cli/codechefcli/problems.py
# Compiled at: 2018-03-28 06:03:18
# Size of source mod 2**32: 18375 bytes
import math, re
from datetime import datetime
from bs4 import BeautifulSoup
from .decorators import login_required, sort_it
from .utils.constants import BASE_URL, DEFAULT_NUM_LINES, PROBLEM_LIST_TABLE_HEADINGS, RATINGS_TABLE_HEADINGS, RESULT_CODES, SERVER_DOWN_MSG
...
def get_description(problem_code, contest_code=None):
    ...
    if req_obj.status_code == 200:
        ...
        if contest_code is None:
            resps.append({'data': 'Maybe, the problem exists only in the contest.\nAdd ' + color_text('--search <contest code>', 'BOLD') + ' to search in the contest specific problems.'})
        return resps
...
def submit_problem(problem_code, solution_file, language):
    ...
    try:
        ...
        post_data = {'language':language_code, 
         'problem_code':problem_code, 
         'form_id':'problem_submission', 
         'form_token':form_token}
        post_files = {'files[sourcefile]': solution_file_obj}
        req_obj = request(session, 'POST', url, data=post_data, files=post_files)
        if req_obj.status_code == 200:
            ...
            while True:
                ...
                if result_code != 'wait':
                    ...
                    if result_code == 'compile':
                        ...
                        compile_error_msg = 'Compilation error.\n{0}'.format(error_msg)
                        resp['data'] = color_text(compile_error_msg, 'FAIL')
                    else:
                        if result_code == 'runtime':
                            error_msg = status_json['signal']
                            runtime_error_msg = 'Runtime error. {0}\n'.format(error_msg)
    ...
    if req_obj.status_code == 200:
        ...
        if is_contest:
            ...
            if timer_calc_script:
                ...
                end_date_str = re.search('new Date\\("([a-zA-Z0-9 ,:]*)"\\)', script_text).group(1)
                ...
                time_left_text = color_text('Contest ends in ', 'BOLD') + days + ' days, ' + hours + ' hours, ' + minutes + ' minutes, ' + seconds + ' seconds.'
            else:
...
def get_all_tags():
    ...
    if req_obj.status_code == 200:
        ...
        resp = {'code':200, 
         'data':data_rows,  'data_type':'table'}
    return resp
...
def get_ratings(sort, order, country, institution, institution_type, page, lines):
    ...
    if req_obj.status_code == 200:
        ...
        if len(ratings) == 0:
            resp = {'code':404,  'data':"Oops! we don't have data.", 
             'data_type':'text'}
        else:
            resp = {'code':200,  'data':data_rows, 
             'data_type':'table'}
        return resp
    else:
        return resp
...
def get_contests(skip_past_contests):
    ...
    if req_obj.status_code == 200:
        ...
        for i in range(1, limit):
            resps.append({'data': color_text(labels[(i - 1)] + ' Contests:\n', 'BOLD')})
            data_rows = html_to_list(str(tables[i]))
            resps.append({'data':data_rows, 
             'data_type':'table'})
...
def get_solution(solution_code):
    ...
    if req_obj.status_code == 200:
        if 'Solution: ' + solution_code in req_obj.text:
            ...
            resps = [
             {'data':'Invalid Solution Code.', 
              'code':404}]
    else:
        if req_obj.status_code == 403:
            resps = [
             {'code':400, 
              'data':'Access Denied!'}]
        else:
