...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/3.7/site-packages/tripgo_parser/parse.py
# Compiled at: 2020-02-22 20:04:28
# Size of source mod 2**32: 24009 bytes
import numpy as np, json, time, pandas as pd, numpy as np
from os import path
import os
...
class Parse:
    ...
    def __init__(self, jsonData, startime='', tripid=''):
        ...
        self.missingBus = 0
        self.missingTram = 0
        self.bestIndex = {'driving':self.getBestDriving(), 
    ...
    def getCompiledData(self):
        ...
        compiled_data = {**{'startime':self.startime, 
         'car':abs(self.missingDriving - 1), 
         'taxi':abs(self.missingTaxi - 1),  'cycling':abs(self.missingCycling - 1), 
         'walking':abs(self.missingWalking - 1), 
         'transit':abs(self.missingTransit - 1)}, **transitData, **drivingData, **taxiData, **cyclingData, **walkingData}
        if self.tripid != '':
    ...
    def getBusData(self):
        ...
        parsedGroupData = {'bus_travelTime':groupData.totalTravelTime, 
         'bus_frequency':groupData.frequency, 
         'bus_weightedScore':groupData.weightedScore, 
    ...
    def getTrainData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'train_travelTime':groupData.totalTravelTime, 
         'train_frequency':groupData.frequency, 
         'train_weightedScore':groupData.weightedScore, 
    ...
    def getTramData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'tram_travelTime':groupData.totalTravelTime, 
         'tram_frequency':groupData.frequency, 
         'tram_weightedScore':groupData.weightedScore, 
    ...
    def getDrivingData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'car_travelTime':groupData.totalTravelTime, 
         'car_weightedScore':groupData.weightedScore, 
         ...
         'car_walkingDistance':groupData.segmentData['Walking Distance'],  'car_hassleCost':groupData.hassleCost,  'car_drivingDistances':groupData.segmentData['Distances'].get('me_car')}
        return parsedGroupData
    ...
    def getCyclingData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'cycling_travelTime':groupData.totalTravelTime, 
         'cycling_weightedScore':groupData.weightedScore, 
    ...
    def getWalkingData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'walking_travelTime':groupData.totalTravelTime, 
         'walking_weightedScore':groupData.weightedScore, 
    ...
    def getTaxiData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'taxi_travelTime':groupData.totalTravelTime, 
         'taxi_weightedScore':groupData.weightedScore, 
    ...
    def getTransitData(self):
        ...
        group = self.allGroups[index]
        groupData = Group(group, self.jsonData)
        parsedGroupData = {'transit_totalTravelTime':groupData.totalTravelTime, 
...
class Trip:
    ...
    def getSegmentData(self):
        segmentsData = {'Local Cost':0, 
         'Fuel Cost':0, 
         'Parking Cost':0, 
         'Parking Exists':False, 
         'Modes':[],  'Distances':{},  'Start':{},  'End':{},  'Travel Times':{},  'Walking Distance':0, 
    ...
    def filterStartEnd(self, data):
        dictionary = {k:v for k, v in dict.items() if v is not None if v is not None}
        return dictionary
