...
# Embedded file name: C:\Users\keurf\Documents\GitHub\Pycker\pycker\gui\gui.py
# Compiled at: 2017-10-13 05:28:38
# Size of source mod 2**32: 28182 bytes
...
class PyckerGUI:
    ...
    def init_frame1(self):
        ...
        data_entry = ttk.Entry(self.frame1, width=25, textvariable=self.input_dirname, takefocus=True)
        data_import_button = ttk.Button(self.frame1, text='Import', command=self.import_traces, takefocus=False)
        norm_button = ttk.Checkbutton(self.frame1, text='Normalize', variable=self.normalize, takefocus=False)
        norm_spinbox = Spinbox(self.frame1, from_=0.01, to_=1.0, increment=0.01, textvariable=self.perc, width=7, justify='right', takefocus=True)
        fs_button = ttk.Checkbutton(self.frame1, text='Sampling rate (Hz)', variable=self.enforce_fs, takefocus=False)
        fs_entry = ttk.Entry(self.frame1, width=10, textvariable=self.sampling_rate, justify='right', takefocus=True)
        low_button = ttk.Checkbutton(self.frame1, text='Lowpass (Hz)', variable=self.lowpass, takefocus=False)
        low_entry = ttk.Entry(self.frame1, width=10, textvariable=self.lpcut, justify='right', takefocus=True)
        high_button = ttk.Checkbutton(self.frame1, text='Highpass (Hz)', variable=self.highpass, takefocus=False)
        high_entry = ttk.Entry(self.frame1, width=10, textvariable=self.hpcut, justify='right', takefocus=True)
        delay_button = ttk.Checkbutton(self.frame1, text='Delay', variable=self.delay, takefocus=False)
        delay_entry = ttk.Entry(self.frame1, width=6, textvariable=self.delay_val, justify='right', takefocus=True)
        ...
        apply_button = ttk.Button(self.frame1, text='Apply', command=self.apply, takefocus=False)
    ...
    def import_traces(self):
        dirname = tkfile.askdirectory(title='Open data directory', initialdir=os.getcwd())
        if len(dirname) > 0:
            dirname += '/'
    ...
    def view_seismogram(self):
        ...
        if self.plot_type.get() == 0:
            ...
            if self.normalize.get() and self.perc.get() < 1.0:
                clip = np.percentile(np.abs(self._traces.ravel()), self.perc.get() * 100.0)
            ...
            self.ax1 = wiggle(self._traces, perc=self.perc.get(), taxis=t, norm=self.normalize.get(), fill=self.fill.get(), axes=self.ax1)
            self.ax1.set_ylabel(ylabel)
            self.ax1.set_ylim(max(tmin, 0.0), t[(-1)])
            self.ax1.invert_yaxis()
            self.ax1.set_picker(True)
        self.fig.suptitle(self._starttime, fontsize=8, va='bottom', ha='left', position=(0.01,
                                                                                         0.01))
            ...
            for k, pick in enumerate(self.picks[self._current_index]):
                if pick is not None and pick.index is not None:
                    ...
                    if len(self.ax1[k].patches) != 0:
                        self.ax1[k].patches = []
                    self.ax1[k].set_title(title, fontsize=6, va='top', ha='right', position=(1,
                                                                                             1.05))
    ...
    def export_current_pick(self):
        if self.picks is not None and self._current_index is not None and self.picks[self._current_index] is not None:
            filename = tkfile.asksaveasfilename(title='Export current pick', initialdir=os.getcwd(), filetypes=[
             ('ASCII', '.txt')], defaultextension='.txt')
            if len(filename) > 0:
                idx = [pick.index if pick is not None and pick.index is not None else -0.005 for pick in self.picks[self._current_index]]
    ...
    def export_all_picks(self):
        if self.picks is not None and not np.all([pick is None for pick in self.picks]):
            filename = tkfile.asksaveasfilename(title='Export all picks', initialdir=os.getcwd(), filetypes=[
             ('Pickle', '.pickle')], defaultextension='.pickle')
            if len(filename) > 0:
    ...
    def import_all_picks(self):
        if self.picks is not None:
            filename = tkfile.askopenfilename(title='Import all picks', initialdir=os.getcwd(), filetypes=[
             ('Pickle', '.pickle')], defaultextension='.pickle')
            if len(filename) > 0:
    ...
    def _read_traces(self):
        ...
        if self.picks[self._current_index] is None:
            self.picks[self._current_index] = [
             None] * self._shape[0]
    ...
    def _filter_traces(self):
        if self.lpcut.get() > self.sampling_rate.get():
            ...
            if self.hpcut.get() > self.sampling_rate.get():
    ...
    def _delay2samples(self):
        ...
        if delay_unit == 's':
            return delay_val * fs
        if delay_unit == 'ms':
            return delay_val * fs * 0.001
        if delay_unit == 'us':
            return delay_val * fs * 1e-06
