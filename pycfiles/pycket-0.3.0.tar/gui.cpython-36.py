...
# Embedded file name: ../pycker/gui/gui.py
# Compiled at: 2017-09-08 11:18:24
# Size of source mod 2**32: 24754 bytes
...
class PyckerGUI:
    ...
    def init_frame1(self):
        self.frame1 = ttk.LabelFrame((self.data_container), text='Data', borderwidth=2, relief='groove', width=300, height=220)
        ...
        norm_spinbox = Spinbox((self.frame1), from_=0.01, to_=1.0, increment=0.01, textvariable=(self.perc), width=7,
          justify='right',
          takefocus=True)
        fs_button = ttk.Checkbutton((self.frame1), text='Sampling rate (Hz)', variable=(self.enforce_fs))
        fs_entry = ttk.Entry((self.frame1), width=10, textvariable=(self.sampling_rate), justify='right')
        low_button = ttk.Checkbutton((self.frame1), text='Lowpass (Hz)', variable=(self.lowpass))
        low_entry = ttk.Entry((self.frame1), width=10, textvariable=(self.lpcut), justify='right')
        high_button = ttk.Checkbutton((self.frame1), text='Highpass (Hz)', variable=(self.highpass))
        high_entry = ttk.Entry((self.frame1), width=10, textvariable=(self.hpcut), justify='right')
        delay_button = ttk.Checkbutton((self.frame1), text='Delay', variable=(self.delay))
        delay_entry = ttk.Entry((self.frame1), width=6, textvariable=(self.delay_val), justify='right')
        ...
        data_label.grid(row=0, column=0, padx=5, sticky='w')
        ...
        norm_spinbox.grid(row=2, column=2, padx=5, pady=1)
    ...
    def init_frame2(self):
        self.frame2 = ttk.LabelFrame((self.data_container), text='Files', borderwidth=2, relief='groove', width=300, height=300)
    ...
    def view_seismogram(self):
        ...
        if self.plot_type.get() == 0:
            ...
            for k, (ax, tr) in enumerate(zip(self.ax1, self._traces)):
            ...
            self.ax1 = wiggle((self._traces), perc=(self.perc.get()), tmin=tmin, norm=(self.normalize.get()),
              fill=(self.fill.get()),
              axes=(self.ax1))
                ...
                if pick is not None:
                    if pick.index is not None:
                        ...
                        if len(self.ax1[k].patches) != 0:
                            self.ax1[k].patches = []
                    title = 'Pick = ' + self._tobs2str(idx / self.sampling_rate.get())
    ...
    def export_current_pick(self):
        if self.picks is not None and self._current_index is not None and self.picks[self._current_index] is not None:
            filename = tkfile.asksaveasfilename(title='Export current pick', filetypes=[
             ('ASCII', '.txt')],
              defaultextension='.txt')
            if len(filename) > 0:
                idx = [pick.index if pick.index is not None else -0.005 for pick in self.picks[self._current_index] if pick is not None]
    ...
    def export_all_picks(self):
        if self.picks is not None and not np.all([pick is None for pick in self.picks]):
            filename = tkfile.asksaveasfilename(title='Export all picks', filetypes=[
             ('Pickle', '.pickle')],
    ...
    def import_all_picks(self):
        if self.picks is not None:
            filename = tkfile.askopenfilename(title='Import all picks', filetypes=[
             ('Pickle', '.pickle')],
              defaultextension='.pickle')
            if len(filename) > 0:
