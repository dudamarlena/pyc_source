...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/snipsskills/utils/systemd.py
# Compiled at: 2017-09-27 13:23:39
""" Systemd utilities. """
...
class Systemd:
    ...
    def setup(use_default_values=None):
        if ask_yes_no('Would you like Snips to start on boot (using systemd)?', use_default_values) == False:
            return
        username, snips_home_path, snipsskills_path, snips_path = Systemd.get_snipsskills_params(use_default_values=use_default_values)
    ...
    def get_snipsskills_params(use_default_values=None):
        ...
        return (
         username, snips_home_path, snipsskills_path, snips_path)
    ...
    def write_snipsskills_file(username, snips_home_path, snipsskills_path):
        ...
        if contents is None:
            ...
            contents = contents.replace('{{SNIPS_HOME_PATH}}', snips_home_path).replace('{{SNIPSSKILLS_PATH}}', snipsskills_path)
