...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/snipsskills/commands/run.py
# Compiled at: 2017-10-30 12:27:22
"""The run command."""
    ...
    def handle_intent(self, intent, payload=None):
        ...
        for skilldef in self.skilldefs:
            ...
            if intent_def.action.startswith('{%'):
                action = intent_def.action.replace('{%', '').replace('%}', '').replace('skilldef', '_snips_eejycfyrdfzilgfb').replace('intent_def', '_snips_jkqdruouzuahmgns').replace('snipsfile', '_snips_pdzdcpaygyjklngz').strip()
                exec action
            else:
