...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/snipsskills/commands/scaffold.py
# Compiled at: 2017-09-29 14:31:23
import os
from jinja2 import Environment, PackageLoader
from .base import Base
from ..utils.os_helpers import create_dir_verbose, write_text_file_verbose, ask_yes_no, ask_for_input, get_user_email_git, email_is_valid
...
class Scaffold(Base):
    ...
    def __init__(self):
        self.jinja_env = Environment(loader=PackageLoader('snipsskills', 'templates'))
        self.wizard = Wizard()
        self.wizard.add_question(description='Give your skill a name. For instance: lightskill, gardeningskill, etc ...', text='Project name? ', input_function=ask_for_input, input_validation=lambda x: len(x) > 0)
        self.wizard.add_question(description='A short sentence to describe what your skill does.', text='Description? ', input_function=ask_for_input, input_validation=lambda x: len(x) > 0)
        self.wizard.add_question(description='', text='Author? ', input_function=ask_for_input, input_validation=lambda x: True)
        self.wizard.add_question(description='', text='Email address? ', input_function=ask_for_input, input_validation=email_is_valid, default_value=get_user_email_git())
    ...
    def run(self):
        ...
        try:
            ...
            self.create_files(current_directory=current_directory, project_name=project_name, description=description, author=author, email=email)
        except IOError as e:
            pass
    ...
    def write_setup(self, root_directory, project_name, email, description, author):
        ...
        setup_content = SETUP_template.render(project_name=project_name, email=email, description=description, author=author)
