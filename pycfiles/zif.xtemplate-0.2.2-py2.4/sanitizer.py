...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zif/xtemplate/sanitizer.py
# Compiled at: 2007-08-25 13:41:36
from lxml.etree import HTML, XMLSyntaxError, fromstring, tounicode, Comment, tostring
import logging
allowed_tags = set(('a', 'abbr', 'acronym', 'address', 'area', 'b', 'big', 'blockquote',
...
class HTMLSanitizer(object):
    ...
    def sanitize(self, element, bad='remove'):
        ...
        for attr in element.keys():
            if attr.lower() not in self.allowedAttributes:
                del element.attrib[attr]

        if element.tag.lower() not in self.allowedTags:
            if bad == 'remove':
