...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zif/xtemplate/lxmlhtmlutils.py
...
def fixTDs(element):
    r"""set td text non-none so pretty-print does not add extra space
...
if os.path.isfile(localfile):
    ...
    logger = logging.getLogger()
    logger.log(logging.INFO, 'loading external DTD. Please place %s in %s.' % (dtdfile, os.path.join(thisfilepath, 'dtds')))
