...
# [GCC 8.4.0]
# Embedded file name: /home/cornish/dev/envs/flextime/lib/python3.5/site-packages/flextime/scheduler.py
# Compiled at: 2017-11-10 13:09:31
# Size of source mod 2**32: 6684 bytes
...
class TimeBlock:
    ...
    def __str__(self):
        return '{} - {} ({}-{}) [{} / {}]'.format(self.day, self.name, '{:02d}:{:02d}'.format(*divmod(self.start, 60)), '{:02d}:{:02d}'.format(*divmod(self.end, 60)), self.effective_minutes(), self.num_minutes())

    def toordinal(self):
        return (
         dateutil.parser.parse(self.day), self.start)
    ...
    def cost(self, task):
        ...
        if days_between < early_threshold:
            ...
            cost /= early_multiplier
        cost = int(cost)
        if cost <= TimeBlock.max_cost:
            return cost
        return TimeBlock.max_cost
...
class Scheduler:
    ...
    def expand_day(self, day_abbrev):
        if day_abbrev in self.abbrev_map:
            return self.abbrev_map[day_abbrev]
        return 'unknown'
    ...
    def scheduled_tasks(self):
        ...
        for i, tb in enumerate(time_blocks):
            ...
            for j, task in enumerate(tasks):
                if tb.can_complete(task):
                    graph.add_edge('time.{}'.format(i), 'task.{}'.format(j), weight=tb.cost(task))
        ...
        for i, tb in enumerate(time_blocks):
            ...
            tb.tasks = [(flowDict[time_key]['task.{}'.format(j)], task) for j, task in enumerate(tasks) if 'task.{}'.format(j) in flowDict[time_key] and flowDict[time_key]['task.{}'.format(j)] > 0]

        unscheduled_tasks = [(flowDict['source']['task.{}'.format(i)], task) for i, task in enumerate(tasks) if 'task.{}'.format(i) in flowDict['source'] and flowDict['source']['task.{}'.format(i)] > 0]
        return (
         unscheduled_tasks, time_blocks)