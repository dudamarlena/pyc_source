...
# [GCC 8.4.0]
# Embedded file name: /home/cornish/dev/envs/flextime/lib/python3.5/site-packages/flextime/interface.py
# Compiled at: 2017-11-02 16:57:04
# Size of source mod 2**32: 9947 bytes
import click, yaml, flextime
...
class Menu:
    ...
    def __init__(self, tasktree, pagify=True, input_type='char'):
        ...
        self._items = []
        self.page_offset = 0
        self.char_options = {'q': ('[q]uit', self.set_quit), 
        ...
        self.cond_options = [
         (
          lambda x: x.isdigit(), self.select_item)]
    ...
    def option_str(self):
        ...
        if len(strs) == 0:
            strs = [
             list(map(option_to_str, self.char_options.values()))]
        return '\n'.join([' | '.join(filter(lambda s: s, line)) for line in filter(any, strs)])
...
class Add(Menu):
    ...
    def __init__(self, tasktree, path, merge_files, **kwargs):
        ...
        self.char_options.update({'a': ('easy [a]dd', self.add_interactive), 
    ...
    def merge_files(self):
        if self.merge_files_present():
            merger = reduce(lambda acc, f: {**acc, **(flextime.utils.file_to_dict(f))}, self._merge_files, {})
