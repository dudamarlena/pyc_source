...
# [GCC 8.4.0]
# Embedded file name: /home/cornish/dev/envs/flextime/lib/python3.5/site-packages/flextime/tasktree.py
# Compiled at: 2017-11-10 13:08:53
...
class TaskLeaf:
    ...
    def __str__(self):
        if '_d' in self.data:
            ...
            due_str = 'No due date'
        return '({}) {}'.format(due_str, ' > '.join([str(n) for n in self.path]))
    ...
    def due(self):
        if '_d' in self.data:
            return dateutil.parser.parse(self.data['_d'])
        return datetime.today() + timedelta(days=7)

    def available(self):
        if '_a' in self.data:
            return dateutil.parser.parse(self.data['_a'])
...
class TaskTree:
    ...
    def leaves(self):
        ...
        def recursive_find(data, path=[], attrs={}):
            ...
            new_attrs = dict(attrs, **{k:v for k, v in data.items() if re.match('^_.*', str(k)) if re.match('^_.*', str(k))})
            if len(children) > 0:
    ...
    def props_from_path(self, path):
        return {k:v for k, v in self.branch_from_path(path).items() if re.match('^_.*', str(k)) if re.match('^_.*', str(k))}
