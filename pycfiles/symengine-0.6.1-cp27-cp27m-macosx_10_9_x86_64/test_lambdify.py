...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/wheel/symengine/tests/test_lambdify.py
...
import math, sys, symengine as se
...
def test_Lambdify_Piecewise():
    _test_Lambdify_Piecewise(lambda *args: se.Lambdify(backend='lambda', *args))
    if se.have_llvm:
        _test_Lambdify_Piecewise(lambda *args: se.Lambdify(backend='llvm', *args))
...
def test_Lambdify_LLVM_with_opt_level():
    for opt_level in range(4):
        ...
        if not se.have_llvm:
            raises(ValueError, lambda : se.Lambdify(args, [x + y + z, x ** 2,
             (x - y) / z, x * y * z], backend='llvm', opt_level=opt_level))
            raise SkipTest('No LLVM support')
...
def test_broadcast():
    ...
    inp = np.ascontiguousarray(np.vstack((np.cos(a), np.sin(a))).T)
...
def test_broadcast_multiple_extra_dimensions():
    ...
    out = out.squeeze()
    assert abs(out[(2, 1, 0)] - 49) < 1e-14
    assert abs(out[(2, 1, 1)] - 343) < 1e-14
    assert abs(out[(-1, -1, 0)] - 121) < 1e-14
    assert abs(out[(-1, -1, 1)] - 1331) < 1e-14
...
def _get_cse_exprs():
    ...
    inp = [11, 13]
    ref = [134, 0.10743801652892562, 1584]
...
def _get_cse_exprs_big():
    x, p = (
     se.symarray('x', 14), se.symarray('p', 14))
    exp = se.exp
    exprs = [
    ...
    return (
     tuple(x) + tuple(p), exprs, np.ones(len(x) + len(p)))
...
def test_2dim_Matrix_broadcast_multiple_extra_dim():
    L, check = _get_1_to_2by3_matrix()
    inp = np.arange(1, 121).reshape((4, 5, 6))
    out = L(inp)
...
def test_unsafe_complex():
    ...
    assert not L.real
    inp = np.array([complex(13.0, 11.0), complex(7.0, 4.0)], dtype=np.complex128)
...
def test_complex_1():
    x = se.Symbol('x')
    lmb = se.Lambdify([x], [complex(0.0, 1.0) + x], real=False)
    assert abs(lmb([complex(11.0, 13.0)])[0] - complex(11.0, 14.0)) < 1e-15


@unittest.skipUnless(have_numpy, 'Numpy not installed')
def test_complex_2():
    x = se.Symbol('x')
    lmb = se.Lambdify([x], [3 + x - complex(0.0, 1.0)], real=False)
    assert abs(lmb([complex(11.0, 13.0)])[0] - complex(14.0, 12.0)) < 1e-15
...
def test_Lambdify_scalar_vector_matrix():
    _test_Lambdify_scalar_vector_matrix(lambda *args: se.Lambdify(backend='lambda', *args))
    if se.have_llvm:
        _test_Lambdify_scalar_vector_matrix(lambda *args: se.Lambdify(backend='llvm', *args))


def test_Lambdify_scalar_vector_matrix_cse():
    _test_Lambdify_scalar_vector_matrix(lambda *args: se.Lambdify(backend='lambda', cse=True, *args))
    if se.have_llvm:
        _test_Lambdify_scalar_vector_matrix(lambda *args: se.Lambdify(backend='llvm', cse=True, *args))
...
def _get_Ndim_args_exprs_funcs(order):
    ...
    return (
     args, nd_exprs_a, nd_exprs_b, f_a, f_b)
...
def test_Lambdify_inp_exceptions():
    ...
    assert np.all(lmb2([2, 3, 4, 5]) == [13, 41])
...
def test_llvm_long_double():
    ...
    exp_out = np.array([2, 0.6666666666666666], dtype=np.longdouble)
    out = l(inp)
