...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/wheel/symengine/tests/test_functions.py
# Compiled at: 2020-03-16 01:41:57
from symengine import Symbol, sin, cos, sqrt, Add, Mul, function_symbol, Integer, log, E, symbols, I, Rational, EulerGamma, Function, Subs, Derivative, LambertW, zeta, dirichlet_eta, zoo, pi, KroneckerDelta, LeviCivita, erf, erfc, oo, lowergamma, uppergamma, exp, loggamma, beta, polygamma, digamma, trigamma, sign, floor, ceiling, conjugate, nan, Float
import unittest
try:
    import sympy
...
def test_Subs():
    ...
    assert Subs(Derivative(function_symbol('f', x, y), x), [x, y], [_x, x]) == Subs(Derivative(function_symbol('f', x, y), x), [y, x], [x, _x])
    s = f.diff(x) / 2
    _xi_1 = Symbol('_xi_1')
