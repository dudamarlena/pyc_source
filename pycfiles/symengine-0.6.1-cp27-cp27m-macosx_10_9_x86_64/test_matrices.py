...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/wheel/symengine/tests/test_matrices.py
# Compiled at: 2020-03-16 01:41:57
from symengine import symbols
from symengine.lib.symengine_wrapper import DenseMatrix, Symbol, Integer, Rational, function_symbol, I, NonSquareMatrixError, ShapeError, zeros, ones, eye, ImmutableMatrix
...
def test_dump_complex():
    if not HAVE_NUMPY:
        return
    ref = [
     complex(0.0, 1.0), complex(0.0, 2.0), complex(0.0, 3.0), complex(0.0, 4.0)]
    A = DenseMatrix(2, 2, ref)
...
def test_row_join():
    assert eye(3).row_join(DenseMatrix([7, 7, 7])) == DenseMatrix([[1, 0, 0, 7],
     [
      0, 1, 0, 7],
     [
      0, 0, 1, 7]])
...
def test_col_join():
    assert eye(3).col_join(DenseMatrix([[7, 7, 7]])) == DenseMatrix([[1, 0, 0],
     [
      0, 1, 0],
     [
      0, 0, 1],
...
def test_cross():
    ...
    raises(ShapeError, lambda : DenseMatrix(1, 2, [1, 1]).cross(DenseMatrix(1, 2, [1, 1])))
