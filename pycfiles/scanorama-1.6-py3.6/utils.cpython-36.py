...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scanorama/utils.py
# Compiled at: 2020-03-29 12:25:59
# Size of source mod 2**32: 4385 bytes
import errno
...
def visualize_cluster(coords, cluster, cluster_labels, cluster_name=None, size=1, viz_prefix='vc', image_suffix='.svg'):
    ...
    plt.figure()
    plt.scatter((coords[(nc_idx, 0)]), (coords[(nc_idx, 1)]), c=(colors[0]),
      s=size)
    plt.scatter((coords[(c_idx, 0)]), (coords[(c_idx, 1)]), c=(colors[1]),
      s=size)
...
def visualize_expr(X, coords, genes, viz_gene, image_suffix='.svg', new_fig=True, size=1, viz_prefix='ve'):
    ...
    if viz_gene.upper() not in genes:
