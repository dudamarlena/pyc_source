...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scanorama/scanorama.py
# Compiled at: 2020-05-03 14:10:59
# Size of source mod 2**32: 37033 bytes
from annoy import AnnoyIndex
from intervaltree import IntervalTree
from itertools import cycle, islice
import numpy as np, operator, random, scipy
...
def correct(datasets_full, genes_list, return_dimred=False, batch_size=BATCH_SIZE, verbose=VERBOSE, ds_names=None, dimred=DIMRED, approx=APPROX, sigma=SIGMA, alpha=ALPHA, knn=KNN, return_dense=False, hvg=None, union=False, seed=0):
    ...
    datasets, genes = merge_datasets(datasets_full, genes_list, ds_names=ds_names,
      union=union)
...
def integrate(datasets_full, genes_list, batch_size=BATCH_SIZE, verbose=VERBOSE, ds_names=None, dimred=DIMRED, approx=APPROX, sigma=SIGMA, alpha=ALPHA, knn=KNN, union=False, hvg=None, seed=0, sketch=False, sketch_method='geosketch', sketch_max=10000):
    ...
    datasets_dimred, genes = process_data(datasets, genes, hvg=hvg, dimred=dimred)
    if sketch:
        datasets_dimred = integrate_sketch(datasets_dimred,
          sketch_method=sketch_method, N=sketch_max, integration_fn=assemble,
          integration_fn_args={'verbose':verbose, 
         'knn':knn,  'sigma':sigma,  'approx':approx, 
...
def correct_scanpy(adatas, **kwargs):
    ...
    """
    if 'return_dimred' in kwargs:
        if kwargs['return_dimred']:
            datasets_dimred, datasets, genes = correct(
             [adata.X for adata in adatas], 
             [adata.var_names.values for adata in adatas], **kwargs)
    else:
...
def plot_clusters(coords, clusters, s=1, colors=None):
    ...
    if colors is None:
        colors = np.array(list(islice(cycle([
         ...
         '#808080', '#fabebe', '#a3f4ff']), int(max(clusters) + 1))))
    plt.figure()
    plt.scatter((coords[:, 0]), (coords[:, 1]), c=(colors[clusters]),
      s=s)
...
def check_datasets(datasets_full):
    ...
    for i, ds in enumerate(datasets_full):
        if issubclass(type(ds), np.ndarray):
            ...
            sys.stderr.write('ERROR: Data sets must be numpy array or scipy.sparse.csr_matrix, received type {}.\n'.format(type(ds)))
            exit(1)
...
def visualize(assembled, labels, namespace, data_names, gene_names=None, gene_expr=None, genes=None, n_iter=N_ITER, perplexity=PERPLEXITY, verbose=VERBOSE, learn_rate=200.0, early_exag=12.0, embedding=None, shuffle_ds=False, size=1, multicore_tsne=True, image_suffix='.svg', viz_cluster=False, colors=None, random_state=None):
    if embedding is None:
        ...
        plt.title('Panorama ({} iter, perplexity: {}, sigma: {}, knn: {}, hvg: {}, dimred: {}, approx: {})'.format(n_iter, perplexity, SIGMA, KNN, HVG, DIMRED, APPROX))
        plt.savefig((namespace + image_suffix), dpi=500)
        if viz_cluster and not shuffle_ds:
            for i in range(len(data_names)):
                visualize_cluster(embedding, i, labels, cluster_name=(data_names[i]),
                  size=size,
            ...
            if gene_expr is not None:
                if genes is not None:
                    ...
                    for gene_name in gene_names:
                        visualize_expr(gene_expr, embedding, genes,
                          gene_name, size=size, viz_prefix=namespace,
                          image_suffix=image_suffix)
...
def find_alignments_table(datasets, knn=KNN, approx=APPROX, verbose=VERBOSE, prenormalized=False):
    ...
    for i in range(len(datasets)):
        if len(datasets[:i]) > 0:
            fill_table(table, i, (datasets[i]), (datasets[:i]), knn=knn, approx=approx)
        if len(datasets[i + 1:]) > 0:
            fill_table(table, i, (datasets[i]), (datasets[i + 1:]), knn=knn,
              base_ds=(i + 1),
              approx=approx)
    ...
    for i in range(len(datasets)):
        for j in range(len(datasets)):
            if i >= j:
                continue
            if (
             i, j) not in table or (j, i) not in table:
                pass
            else:
                match_ij = table[(i, j)]
                match_ji = set([(b, a) for a, b in table[(j, i)]])
                matches[(i, j)] = match_ij & match_ji
                table1[(i, j)] = max(float(len(set([idx for idx, _ in matches[(i, j)]]))) / datasets[i].shape[0], float(len(set([idx for _, idx in matches[(i, j)]]))) / datasets[j].shape[0])
                if verbose > 1:
...
def find_alignments(datasets, knn=KNN, approx=APPROX, verbose=VERBOSE, alpha=ALPHA, prenormalized=False):
    table1, _, matches = find_alignments_table(datasets,
      knn=knn, approx=approx, verbose=verbose, prenormalized=prenormalized)
    alignments = [(i, j) for (i, j), val in reversed(sorted((table1.items()), key=(operator.itemgetter(1)))) if val > alpha]
    return (
     alignments, matches)


def connect(datasets, knn=KNN, approx=APPROX, alpha=ALPHA, verbose=VERBOSE):
    alignments, _ = find_alignments(datasets,
...
def transform(curr_ds, curr_ref, ds_ind, ref_ind, sigma=SIGMA, cn=False, batch_size=None):
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            sys.stderr.write('WARNING: Oversmoothing detected, refusing to batch correct, consider lowering sigma value.\n')
            return csr_matrix((curr_ds.shape), dtype=float)
        except MemoryError:
            if batch_size is None:
                sys.stderr.write('WARNING: Out of memory, consider turning on batched computation with batch_size parameter.\n')
            else:
...
def assemble(datasets, verbose=VERBOSE, view_match=False, knn=KNN, sigma=SIGMA, approx=APPROX, alpha=ALPHA, expr_datasets=None, ds_names=None, batch_size=None, alignments=None, matches=None):
    if len(datasets) == 1:
        return datasets
    else:
        if alignments is None:
            if matches is None:
                alignments, matches = find_alignments(datasets,
                  knn=knn, approx=approx, alpha=alpha, verbose=verbose)
        ds_assembled = {}
        panoramas = []
        for i, j in alignments:
            if verbose:
                ...
                if i not in ds_assembled:
                    ds_assembled[i] = 0
                ds_assembled[i] += 1
                if j not in ds_assembled:
                    ds_assembled[j] = 0
                ds_assembled[j] += 1
                if ds_assembled[i] > 3:
                    if ds_assembled[j] > 3:
                ...
                if len(panoramas_i) == 0:
                    ...
                    if len(panoramas_j) == 0:
                        ...
                        if expr_datasets:
                            ...
                            bias = transform(curr_ds, curr_ref, ds_ind, ref_ind, sigma=sigma, cn=True,
                              batch_size=batch_size)
...
def integrate_sketch(datasets_dimred, sketch_method='geosketch', N=10000, integration_fn=assemble, integration_fn_args={}):
    from geosketch import gs, uniform
    if sketch_method.lower() == 'geosketch' or sketch_method.lower() == 'gs':
        sampling_fn = gs
    else:
        sampling_fn = uniform
    sketch_idxs = [sorted(set(sampling_fn(X, N, replace=False))) if X.shape[0] > N else list(range(X.shape[0])) for X in datasets_dimred]
...
def assemble_accum(datasets, verbose=VERBOSE, knn=KNN, sigma=SIGMA, approx=APPROX, batch_size=None):
    if len(datasets) == 1:
        return datasets
    else:
        for i in range(len(datasets) - 1):
            ...
            ds_ind = [a for a, _ in match]
            ref_ind = [b for _, b in match]
            bias = transform(ds1, ds2, ds_ind, ref_ind, sigma=sigma, batch_size=batch_size)
            datasets[j] = ds1 + bias
