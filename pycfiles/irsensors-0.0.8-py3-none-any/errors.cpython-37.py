...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ISEN\AppData\Local\Temp\pip-install-whg15vss\serial\serial\errors.py
...
class UnmarshalError(Exception):
    ...
    def __init__(self, data, types=None, item_types=None, value_types=None):
        """
            ...
            if item_types:
                ...
                if value_types:
                    ...
                    if types is None:
                        ...
                        error_message_lines.append('The data provided does not match any of the expected types and/or property definitions:')
                error_message_lines.append(' - data: %s' % '\n'.join(data_lines))
                if types is None:
                ...
                types_lines.append('   )')
                error_message_lines.append(' - %s: %s' % (types_label, '\n'.join(types_lines)))
    ...
    def assemble_message(self):
        messages = []
        if self.paramater:
            messages.append('Errors encountered in attempting to un-marshal %s:' % self.paramater)
        if self.index is not None:
            messages.append('Errors encountered in attempting to un-marshal the value at index %s:' % repr(self.index))
        if self.message:
