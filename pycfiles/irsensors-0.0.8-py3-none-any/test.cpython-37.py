...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ISEN\AppData\Local\Temp\pip-install-whg15vss\serial\serial\test.py
...
def _object_discrepancies(a, b):
    ...
    for property_ in a_properties | b_properties:
        ...
        if a_value != b_value:
            discrepancies[property_] = (
             a_value, b_value)

    return discrepancies


def json(model_instance, raise_validation_errors=True):
    model(model_instance=model_instance,
      format_='json',
      raise_validation_errors=raise_validation_errors)


def yaml(model_instance, raise_validation_errors=True):
    model(model_instance=model_instance,
      format_='yaml',
      raise_validation_errors=raise_validation_errors)


def xml(model_instance, raise_validation_errors=True):
    raise NotImplementedError()


def model(model_instance, format_, raise_validation_errors=True):
    """
    ...
    if not isinstance(model_instance, Model):
        ...
        raise TypeError('`%s` requires an instance of `%s` for the parameter `model_instance`, not%s' % (
         calling_function_qualified_name(),
         qualified_name(Model),
         (':\n%s' if '\n' in value_representation else ' `%s`') % value_representation))
    else:
        ...
        if isinstance(model_instance, serial.model.Object):
            ...
            try:
                ...
                try:
                    ...
                    message = [
                     'Discrepancies were found between the instance of `%s` provided and ' % qualified_model_name + 'a serialized/deserialized clone:']
                    if sa != sb:
                        message.append('\n    :\n\n        %s\n        !=\n        %s' % (
                         sa,
                         sb))
                    ...
                    e.args = tuple(chain((
                     e.args[0] + '\n' + '\n'.join(message) if e.args else '\n'.join(message),), e.args[1:] if e.args else tuple()))
                    raise e
                finally:
                    e = None
                    del e
            ...
            if format_ == 'json':
                reloaded_marshalled_data = _json.loads(string,
                ...
                if format_ == 'yaml':
                    ...
                    if format_ == 'xml':
                        ...
                        raise ValueError('Valid serialization types for parameter `format_` are "json", "yaml", or "xml, not' + (':\n%s' if '\n' in format_representation else ' %s.') % format_representation)
            keys = set()
            for property_name, property in serial.meta.read(model_instance).properties.items():
                ...
                if (isinstance(property_value, Model) or hasattr)(property_value, '__iter__'):
                    isinstance(property_value, (str, native_str, bytes)) or model(property_value, format_=format_, raise_validation_errors=raise_validation_errors)

            for k in reloaded_marshalled_data.keys():
                if k not in keys:
                    raise KeyError('"%s" not found in serialized/re-deserialized data: %s' % (
                     k,
                     string))

        else:
            if isinstance(model_instance, serial.model.Array):
                serial.marshal.validate(model_instance)
                for item in model_instance:
                    if (isinstance(item, Model) or hasattr)(item, '__iter__'):
                        isinstance(item, (str, native_str, bytes)) or model(item, format_=format_, raise_validation_errors=raise_validation_errors)

            else:
                if isinstance(model_instance, serial.model.Dictionary):
                    serial.marshal.validate(model_instance)
                    for key, value in model_instance.items():
                        if (isinstance(value, Model) or hasattr)(value, '__iter__'):
                            isinstance(value, (str, native_str, bytes)) or model(value, format_=format_, raise_validation_errors=raise_validation_errors)