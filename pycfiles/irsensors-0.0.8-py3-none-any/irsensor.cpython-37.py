...
# [GCC 8.4.0]
# Embedded file name: D:\Bureau\educat_irsensor\irsensors\irsensors\irsensor.py
# Compiled at: 2019-08-26 20:10:20
# Size of source mod 2**32: 8504 bytes
...
def mean(float_list):
    ...
    return float_list is None or float_list or None
    if len(float_list) == 1:
        return float_list[0]
    sum = 0
...
class IRSensor:
    ...
    def __setattr__(self, attribute_name, attribute_value):
        ...
        raise TypeError("'IRSensor' object does not support attribute assignment (attributeName -> " + attribute_name + ', attributeValue -> ' + str(attribute_value) + ')')

    def __delattr__(self, attribute_name):
        """Raise an TypeError when the user tries to delete an attribute."""
        raise TypeError("'IRSensor' object does not support attribute deletion (attributeName -> " + attribute_name + ')')
    ...
    def __setitem__(self, item_name, item_value):
        ...
        raise TypeError("'IRSensor' object does not support item assignment (attributeName -> " + item_name + ', attributeValue -> ' + str(item_value) + ')')

    def __delitem__(self, item_name):
        """Raise an TypeError when the user tries to delete an attribute as items."""
        raise TypeError("'IRSensor' object does not support item deletion (attributeName -> " + item_name + ')')
    ...
    def __repr__(self):
        ...
        return '<Class = IRSensor, ID = ' + str(self['ID']) + ', History = ' + str(getattr(self, '_history')) + ', Distance = ' + str(self['distance']) + '>'
