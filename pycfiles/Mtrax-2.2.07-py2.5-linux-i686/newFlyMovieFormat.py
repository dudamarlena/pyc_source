...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mtrax/newFlyMovieFormat.py
# Compiled at: 2007-11-12 10:52:09
import sys, struct, warnings, os.path, numpy as nx
...
class FlyMovie():
    ...
    def init_sbfmf(self):
        ...
        print 'nframes = ' + str(self.n_frames)
        print 'difference_mode = ' + str(difference_mode)
        print 'indexloc = ' + str(self.indexloc)
        self.bgcenter = nx.fromstring(self.file.read(struct.calcsize('<d') * nr * nc))
        self.bgstd = nx.fromstring(self.file.read(struct.calcsize('<d') * nr * nc))
        ff = self.file.tell()
        self.file.seek(self.indexloc, 0)
        self.framelocs = nx.fromstring(self.file.read(self.n_frames * 8), nx.uint64)
        print 'framelocs = ' + str(self.framelocs)
        if version == '0.1':
            self.format = 'MONO8'
            self.bits_per_pixel = 8
    ...
    def _read_next_frame(self):
        if self.issbfmf:
            ...
            idx = nx.fromstring(self.file.read(npixels * 4), nx.uint32)
            v = nx.fromstring(self.file.read(npixels * 1), nx.uint8)
    ...
    def _read_next_timestamp(self):
        ...
        self.file.seek(self.bytes_per_chunk - read_len, 1)
        if timestamp_buf == '':
            raise NoMoreFramesException('EOF')
        (timestamp,) = struct.unpack(TIMESTAMP_FMT, timestamp_buf)
        return timestamp
...
class FlyMovieSaver():
    ...
    def __init__(self, filename, version=1, seek_ok=True, compressor=None, comp_level=1, format=None, bits_per_pixel=None):
    ...
    def _add_frame_v1(self, origframe, timestamp=nan):
        ...
        elif self.framesize != frame.shape:
