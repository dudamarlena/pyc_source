...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mtrax/annfiles.py
# Compiled at: 2008-08-14 18:43:17
import numpy as num, scipy.io.mio, wx
...
class AnnotationFile():
    ...
    def CheckAnnHeader(self):
        ...
        if line == 'mtrax header\n':
            ...
            raise InvalidFileFormatException("Annotation file does not start with 'mtrax header'")
        i = 0
        while True:
            line = self.file.readline()
            if line == '':
            ...
            if len(value) == 0:
                continue
            if parameter == 'background median' or parameter == 'background mean' or parameter == 'background mad' or parameter == 'background std' or parameter == 'hfnorm':
                sz = int(value)
                self.file.seek(sz, 1)
    ...
    def ReadAnnHeader(self, background):
        """Read header from an annotation file."""
        if self.file.readline() == 'mtrax header\n':
            pass
        else:
            raise InvalidFileFormatException("Annotation file does not start with 'mtrax header'")
        while True:
