...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mtrax/setarena.py
# Compiled at: 2008-08-04 11:01:48
import wx
from params import params
from wx import xrc
import motmot.wxvideo.wxvideo as wxvideo, imagesk, scipy.signal as signal, numpy as num, houghcircles
from houghcircles import *
import scipy.misc.pilutil as pilutil, pkg_resources, colormapk, motmot.wxvalidatedtext.wxvalidatedtext as wxvt
...
class SetArena():
    ...
    def InitializeValues(self):
        ...
        self.max_radius = max(params.movie.get_width() * self.nc_resize_ratio, params.movie.get_height() * self.nr_resize_ratio) / 2.0
        self.max_x = (params.movie.get_width() - 1.0) * self.nc_resize_ratio
        self.max_y = (params.movie.get_height() - 1.0) * self.nr_resize_ratio
...
def detectarena(edgemag, approxa=None, approxb=None, approxr=None):
    ...
    if isguesseda:
        ...
        maxa = approxa + 0.1 * nc
    if isguessedb:
        minb = approxb - 0.025 * nr
        maxb = approxb + 0.025 * nr
    else:
        minb = approxb - 0.1 * nr
        maxb = approxb + 0.1 * nr
    if isguessedr:
        minr = approxr - 0.025 * min(nc, nr)
        maxr = approxr + 0.025 * min(nc, nr)
    else:
        minr = approxr - 0.125 * min(nc, nr)
        maxr = approxr + 0.125 * min(nc, nr)
    nbinsa = 20
    nbinsb = 20
    nbinsr = 20
