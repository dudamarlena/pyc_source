...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mtrax/mtrax.py
# Compiled at: 2008-08-14 20:06:52
import matplotlib
...
class MtraxApp(algorithm.MtraxAlgorithm):

    def OnInit(self):
        """
        Start up the Mtrax GUI
        """
    ...
    def run_noninteractive(self):
        """
        Run Mtrax in non-interactive mode.
        """
        self.frame = None
        if not hasattr(self, 'filename') or self.filename is None:
    ...
    def CheckAnnotation(self):
        ...
        if params.interactive and not params.batch_executing and wx.MessageBox('Read old annotation data?', 'Read?', wx.YES_NO) == wx.YES:
    ...
    def ReadAnnotationData(self):
        ...
        if params.interactive:
            ...
            self.status.SetStatusText('reading annotation data from file', params.status_box)
            wx.BeginBusyCursor()
            wx.Yield()
    ...
    def OpenMovie(self):
        ...
        try:
            ...
            self.n_frames = 0
            if params.interactive:
                wx.MessageBox('Could not open the movie ' + self.filename, 'Error', wx.ICON_ERROR)
            else:
            ...
            if params.interactive:
                ...
                print 'about to update img_wind'
                sys.stdout.flush()
                self.img_wind.update_image_and_drawings('mtraxmain', img, format='MONO8')
                print 'succeeded in updating img_wind'
                sys.stdout.flush()
                self.img_wind_child = self.img_wind.get_child_canvas('mtraxmain')
    ...
    def EnableControls(self):
        ...
        if movieready:
            ...
            issbfmf = False
        annready = movieready and hasattr(self, 'ann_data') and self.ann_data is not None and len(self.ann_data) > 0
        ...
        self.menu.Enable(xrc.XRCID('menu_track_writesbfmf'), movieready and not issbfmf and not istracking)
        self.menu.Enable(xrc.XRCID('menu_track_resume'), movieready and not istracking and not isplaying and annready)
        self.menu.Enable(xrc.XRCID('menu_track_resume_here'), movieready and not istracking and not isplaying and annready)
    ...
    def UpdateStatusMovie(self):
        ...
        try:
            if len(self.filename) == 0:
                ...
                self.status.SetStatusText('.../' + self.file, params.file_box)
    ...
    def ShowCurrentFrame(self, framenumber=None):
        ...
        except (IndexError, movies.NoMoreFramesException):
            return
        ...
        if self.ann_data is not None and self.ann_file is not None and framenumber >= 0:
            ...
            if draw_frame >= len(self.ann_data):
                draw_frame = -1
        else:
            draw_frame = -1
        ...
        if self.menu.IsChecked(xrc.XRCID('menu_playback_show_ann')) and draw_frame >= 0:
            ...
            linesegs = draw.annotate_image(ellipses, old_pts, self.ellipse_thickness)
            (linesegs, linecolors) = imagesk.separate_linesegs_colors(linesegs)
            self.img_wind.update_image_and_drawings('mtraxmain', frame8, format='MONO8', linesegs=linesegs, lineseg_colors=linecolors)
            self.num_flies_text.SetValue('N. Flies: %02d' % len(ellipses))
        else:
            self.num_flies_text.SetValue('')
            self.img_wind.update_image_and_drawings('mtraxmain', frame8, format='MONO8')
    ...
    def OnResize(self, evt):
        ...
        try:
            self.ShowCurrentFrame()
            new_size = wx.Size(self.img_wind.GetRect().GetWidth() - 30, self.slider.GetRect().GetHeight())
    ...
    def OnWriteSBFMF(self, evt):
        ...
        self.dowritesbfmf = True
        print 'set dowritesbfmf to True'
    ...
    def OnBatchExecute(self, evt):
        ...
        for filename in start_list:
            ...
            self.OpenMovie()
            self.UpdateStatusMovie()
            print 'opened movie ' + self.filename
            if self.menu.IsEnabled(xrc.XRCID('menu_track_start')):
                self.DoAll()
            if not params.batch_executing:
                break
    ...
    def OnStats(self, evt):
        ...
        wx.BeginBusyCursor()
        wx.Yield()
        if self.batch is not None and self.menu.IsChecked(xrc.XRCID('menu_stats_batch')) and True:
            batch_stats = True
            self.status.SetStatusText('reading annotation data from files', params.status_box)
            self.batch_data = []
        ...
        if evt.GetId() == xrc.XRCID('menu_stats_vel'):
            ...
            if batch_stats:
                panel = draw.VelPlotPanel(frame, self.batch_data, 'velocity (%d files)' % len(self.batch.file_list), params.max_jump)
            else:
                panel = draw.VelPlotPanel(frame, self.ann_data, 'velocity (%s)' % self.file, params.max_jump)
        elif evt.GetId() == xrc.XRCID('menu_stats_orn'):
            frame = wx.Frame(None, -1, 'orientation (%d bins)' % draw.const.orn_bins)
            if batch_stats:
                panel = draw.OrnPlotPanel(frame, self.batch_data, 'orientation (%d files)' % len(self.batch.file_list))
            else:
                panel = draw.OrnPlotPanel(frame, self.ann_data, 'orientation (%s)' % self.file)
        elif evt.GetId() == xrc.XRCID('menu_stats_space'):
            frame = wx.Frame(None, -1, 'space (%d bins)' % draw.const.space_bins)
            if batch_stats:
                panel = draw.SpacePlotPanel(frame, self.batch_data, 'space (%d files)' % len(self.batch.file_list))
            else:
                panel = draw.SpacePlotPanel(frame, self.ann_data, 'space (%s)' % self.file)
        elif evt.GetId() == xrc.XRCID('menu_stats_pos'):
            frame = wx.Frame(None, -1, 'position (bins %d pixels)' % draw.const.pos_binsize)
            if batch_stats:
                panel = draw.PosPlotPanel(frame, self.batch_data, 'position (%d files)' % len(self.batch.file_list), self.movie.get_width(), self.movie.get_height())
            else:
                panel = draw.PosPlotPanel(frame, self.ann_data, 'position (%s)' % self.file, self.movie.get_width(), self.movie.get_height())
...
def main():
    args = (0, )
    kw = {}
    if int(os.environ.get('MTRAX_NO_REDIRECT', '0')):
        args = (0, )
        kw = {}
        ...
        args = ()
        kw = dict(redirect=True, filename='mtrax.log')
    app = MtraxApp(*args, **kw)
    app.MainLoop()
