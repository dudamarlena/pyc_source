...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mtrax/newestconncomps.py
# Compiled at: 2007-11-14 19:39:24
import numpy as num
from ellipsesk import *
from pylab import *
import scipy.ndimage as meas, kcluster
...
def trysplit(ellipses, i, isdone, L, dfore):
    ...
    isforebox0 = Lbox == i + 1
    dforebox[Lbox != i + 1] = 0
    for currthresh in num.linspace(params.n_bg_std_thresh, min(3 * params.n_bg_std_thresh, num.max(dforebox)), 10):
    ...
    if ncomponents == 1:
        ...
        ncomponents = 2
        while True:
            (mu, S, priors, gamma, negloglik) = kcluster.gmm(x, ncomponents, weights=w, kmeansthresh=0.1, emthresh=0.1)
            err = 0
