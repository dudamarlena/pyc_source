...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\test\master_webhmi.py
# Compiled at: 2020-01-19 11:46:57
"""
...
from bottle import route, run, template, TEMPLATES, redirect, request
import modbus_tk.modbus_tcp as modbus_tcp, modbus_tk.modbus_rtu as modbus_rtu, modbus_tk.modbus as modbus, sqlite3, webbrowser
SERIAL = True
if SERIAL:
    SERIAL_PORTS = {}
    try:
...
DEBUG = False if SERIAL else True
...
def show_results(master_id, slave_id, function_code, start_address, length):
    ...
    try:
        ...
        return template('templates/master_results', results=results, start=int(start_address), lines=lines, name=name, url=url, master=master, friendly_name=friendly_name)
    except modbus.ModbusError as msg:
...
def show_results_all_hr(master_id, slave_id):
    ...
    try:
        ...
        return template('templates/master_results_all_hr', results=all_results, url=url, name=name, master=master, friendly_name=friendly_name)
    except modbus.ModbusError as msg:
        return template('templates/modbus_error', msg=msg, name=name, url=url, master=master, friendly_name=friendly_name)
