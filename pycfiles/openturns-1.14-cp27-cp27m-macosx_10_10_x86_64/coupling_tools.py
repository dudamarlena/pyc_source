...
# [GCC 8.4.0]
# Embedded file name: lib/python2.7/site-packages/openturns/coupling_tools.py
...
"""
import re, shutil, os, shlex, subprocess, sys
debug = False
...
def check_param(obj, obj_type):
    ...
    try:
        ...
        raise AssertionError('error: parameter (' + str(type(obj)) + ': ' + str(obj) + ') must be of ' + str(obj_type) + '!')
...
def execute(cmd, workdir=None, is_shell=False, shell_exe=None, hide_win=True, check_exit_code=True, get_stdout=False, get_stderr=False, timeout=None):
    """
    ...
    proc = subprocess.Popen(process_args, shell=is_shell, cwd=workdir, executable=shell_exe, stdout=stdout, stderr=stderr, startupinfo=startupinfo)
    if sys.version_info >= (3, 3):
        try:
            ...
            raise RuntimeError('Command "' + cmd + '" times out after ' + str(timeout) + 's')

    else:
        stdout_data, stderr_data = proc.communicate()
    ret = proc.poll()
    if check_exit_code and ret != 0:
        raise RuntimeError('Command "' + cmd + '" returned exit code ' + str(ret))
...
def get_regex(filename, patterns, encoding=default_encoding):
    ...
    for pattern in patterns:
        pattern = pattern.replace('\\R', '[+-]? *(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:[eE][+-]?\\d+)?')
...
def get_real_from_line(line):
    ...
    if result is None:
        raise EOFError('error: real not found at the beginning of this line: (' + line + ')!')
    return result
...
def get_line_col(filename, skip_line=0, skip_col=0, seek=0, encoding=default_encoding):
    ...
    if seek > 0:
        handle.seek(seek)
    if debug:
        sys.stderr.write('get_line_col(skip_line=' + str(skip_line) + ', skip_col=' + str(skip_col) + ', seek=' + str(seek) + ')\n')
    if skip_line < 0:
        lines_cache = []
        ...
        if len(lines_cache) < lines_cache_size:
            err_msg = 'error: the file has less than ' + str(lines_cache_size) + ' lines!'
            handle.close()
...
def get_value(filename, token=None, skip_token=0, skip_line=0, skip_col=0, encoding=default_encoding):
    ...
    """
    if debug:
        sys.stderr.write('get_value(token=' + str(token) + ', skip_token=' + str(skip_token) + ', skip_line=' + str(skip_line) + ', skip_col=' + str(skip_col) + ')\n')
    check_param(filename, str)
    if token is not None:
        check_param(token, str)
    else:
        if skip_token != 0:
            raise AssertionError('error: skip_token parameter needs token parameter to be set!')
        ...
        if skip_line == 0 and skip_col == 0:
            ...
            handle.close()
            result = get_line_col(filename, skip_line, skip_col, seek_pos, encoding=encoding)
    return result
...
def get(filename, tokens=None, skip_tokens=None, skip_lines=None, skip_cols=None, encoding=default_encoding):
    ...
    for n in range(nb_values):
        results[n] = get_value(filename, tokens[n], skip_tokens[n], skip_lines[n], skip_cols[n], encoding=encoding)
