...
# [GCC 8.4.0]
# Embedded file name: lib/python2.7/site-packages/openturns/dist_bundle2.py
# Compiled at: 2019-11-13 10:35:34
...
class Histogram(openturns.model_copula.ContinuousDistribution):
    ...
    width : positive sequence of float
        :math:`w_i` is the width of the class :math:`i`.
    height : positive sequence of float
        :math:`h_i` is the density of the class :math:`i`.
    ticks : increasing sequence of float
        :math:`t_0` is the lower bound of the first class and :math:`t_i` is the upper bound of the class :math:`i`.
    frequencies : positive sequence of float
        :math:`f_i` is the probability of the class :math:`i`.
    ...
    .. math::
        ...
        f_X(x) = \sum_{i=1}^{n}h_i\;\boldsymbol{1}_{[t_{i-1},x_i]}(x)

    The values :math:`h_i` are normalized such that :math:`\sum_i^n h_iw_i=1`.
    ...
    .. math::
        ...
        \begin{eqnarray*}
            \Expect{X} & = & \frac{1}{2 \sum_{i=1}^n h_i\,w_i}
                             \sum_{i=1}^n h_i\,w_i (x_{i+1} + x_i)\\
            \Var{X} & = & \frac{1}{3 \sum_{i=1}^n h_i\,w_i}
...
class HistogramFactory(openturns.model_copula.DistributionFactoryImplementation):
    ...
    def computeSilvermanBandwidth(self, sample, useQuantile=True):
        return _dist_bundle2.HistogramFactory_computeSilvermanBandwidth(self, sample, useQuantile)
...
class InverseNormal(openturns.model_copula.ContinuousDistribution):
    ...
    def setLambdaMu(self, _lambda, mu):
        return _dist_bundle2.InverseNormal_setLambdaMu(self, _lambda, mu)
...
class Logistic(openturns.model_copula.ContinuousDistribution):
    ...
    def setAlpha(self, alpha):
        return _dist_bundle2.Logistic_setAlpha(self, alpha)
...
class LogNormalFactory(openturns.model_copula.DistributionFactoryImplementation):
    ...
    **Least squares method estimator:**
    ...
    When :math:`\gamma` is known and the x_i follow a Log-Normal distribution then
    we use linear least-squares to solve the relation:

    .. math::
      :label: least_squares_estimator
...
class MeixnerDistribution(openturns.model_copula.ContinuousDistribution):
    ...
    def setMu(self, mu):
        return _dist_bundle2.MeixnerDistribution_setMu(self, mu)
...
class MethodOfMomentsFactory(openturns.model_copula.DistributionFactoryImplementation):
    ...
    def setOptimizationProblem(self, problem):
        """
        ...
        problem : :class:`~openturns.OptimizationProblem`
            The problem used for numerical optimization of the moments.
        """
        return _dist_bundle2.MethodOfMomentsFactory_setOptimizationProblem(self, problem)
    ...
    def getOptimizationProblem(self):
        ...
        problem : :class:`~openturns.OptimizationProblem`
            The problem used for numerical optimization of the moments.
        """
        return _dist_bundle2.MethodOfMomentsFactory_getOptimizationProblem(self)
...
class MultinomialFactory(openturns.model_copula.DistributionFactoryImplementation):
    ...
    Maximum likelihood estimators are used in the n-d case,
    else in 1-d we use the following equations:
    ...
    The resolution is done using Brents method.
