...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/checks/pkgdir.py
...
class MismatchedPN(results.PackageResult, results.Error):
    __doc__ = 'Ebuilds that have different names than their parent directory.'
...
class InvalidPN(results.PackageResult, results.Error):
    __doc__ = 'Ebuilds that have invalid package names.'
...
class DuplicateFiles(results.PackageResult, results.Warning):
    __doc__ = 'Two or more identical files in FILESDIR.'
...
class EmptyFile(results.PackageResult, results.Warning):
    __doc__ = 'File in FILESDIR is empty.'
...
class ExecutableFile(results.PackageResult, results.Warning):
    __doc__ = "File has executable bit, but doesn't need it."
...
class SizeViolation(results.PackageResult, results.Warning):
    __doc__ = 'File in $FILESDIR is too large (current limit is 20k).'
...
class InvalidUTF8(results.PackageResult, results.Error):
    __doc__ = "File isn't UTF-8 compliant."
...
class PkgDirCheck(Check):
    __doc__ = 'Scan ebuild directory for various file-related issues.'
    ...
    def feed(self, pkgset):
        ...
        for filename in listdir(pkg_path):
            ...
            if filename.endswith(ebuild_ext):
                ...
                if self.options.gentoo_repo and filename not in ('Manifest', 'metadata.xml',
                                                                 'files'):
            ...
            for filename in files:
                ...
                if stat.S_ISREG(file_stat.st_mode):
                    ...
                    if file_stat.st_size > 20480:
                        yield SizeViolation((pjoin(base_dir, filename)),
                          (file_stat.st_size), pkg=pkg)
                    banned_chars = set(filename) - allowed_filename_chars_set
                    if banned_chars:
                        yield BannedCharacter((pjoin(base_dir, filename)),
                          (sorted(banned_chars)), pkg=pkg)
...
class EqualVersionsCheck(Check):
    __doc__ = 'Scan package ebuilds for semantically equal versions.'
    scope = base.package_scope
