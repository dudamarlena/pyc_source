...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/checks/profiles.py
# Compiled at: 2020-02-09 15:46:32
# Size of source mod 2**32: 17182 bytes
import os
...
class UnknownProfilePackages(results.ProfilesResult, results.Warning):
    __doc__ = "Profile files include package entries that don't exist in the repo."
...
class UnknownProfilePackageUse(results.ProfilesResult, results.Warning):
    __doc__ = "Profile files include entries with USE flags that aren't used on any matching packages."
...
class UnknownProfileUse(results.ProfilesResult, results.Warning):
    __doc__ = "Profile files include USE flags that don't exist."
...
class UnknownProfilePackageKeywords(results.ProfilesResult, results.Warning):
    __doc__ = "Profile files include package keywords that don't exist."
...
class ProfileWarning(results.ProfilesResult, results.LogWarning):
    __doc__ = 'Badly formatted data in various profile files.'


class ProfileError(results.ProfilesResult, results.LogError):
    __doc__ = 'Erroneously formatted data in various profile files.'


class _ProfileNode(profiles_mod.ProfileNode):
    __doc__ = 'Re-inherited to disable instance caching.'


class ProfilesCheck(Check):
    __doc__ = 'Scan repo profiles for unknown flags/packages.'
    required_addons = (
     addons.UseAddon,)
    ...
    def __init__(self, *args, use_addon):
        ...
        self.non_profile_dirs = frozenset(pjoin(self.profiles_dir, x) for x in addons.ProfileAddon.non_profile_dirs)
        special_keywords = {
         '-*'}
    ...
    def available_iuse(self):
        ...
        return frozenset(local_iuse | self.iuse_handler.global_iuse | self.iuse_handler.global_iuse_expand | self.iuse_handler.global_iuse_implicit)
    ...
    def finish(self):
        ...
        def _use(filename, profile, vals):
            ...
            for _, entries in d.items():
                for _, disabled, enabled in entries:
                    ...
                    if unknown_disabled:
                        unknown_use[profile.path][filename].extend('-' + u for u in unknown_disabled)
                    if unknown_enabled:
                        unknown_use[profile.path][filename].extend(unknown_enabled)
        ...
        def _deprecated(filename, profile, vals):
            if vals is not None:
                ...
                try:
                    ...
                    yield ProfileError(f"nonexistent replacement {replacement!r} for deprecated profile: {profile.name!r}")

        file_parse_map = {'packages':(
          'packages', _pkg_atoms), 
         'package.mask':(
          'masks', _pkg_atoms), 
         'package.unmask':(
          'unmasks', _pkg_atoms), 
         'package.deprecated':(
          'pkg_deprecated', _pkg_atoms), 
         'package.use':(
          'pkg_use', _pkg_use), 
         'package.use.force':(
          'pkg_use_force', _pkg_use), 
         'package.use.stable.force':(
          'pkg_use_stable_force', _pkg_use), 
         'package.use.mask':(
          'pkg_use_mask', _pkg_use), 
         'package.use.stable.mask':(
          'pkg_use_stable_mask', _pkg_use), 
         'use.force':(
          'use_force', _use), 
         'use.stable.force':(
          'use_stable_force', _use), 
         'use.mask':(
          'use_mask', _use), 
         'use.stable.mask':(
          'use_stable_mask', _use), 
         'parent':(
          'parents', lambda *args: None), 
         'deprecated':(
          'deprecated', _deprecated), 
         'package.keywords':(
          'keywords', _pkg_keywords), 
         'package.accept_keywords':(
          'accept_keywords', _pkg_keywords)}
        profile_reports = []
            ...
            if root not in self.non_profile_dirs:
                ...
                for f in set(files).intersection(file_parse_map.keys()):
                    ...
                    with patch('pkgcore.log.logger.error', report_profile_errors):
                        with patch('pkgcore.log.logger.warning', report_profile_warnings):
        ...
        for path, filenames in sorted(unknown_pkgs.items()):
            for filename, vals in filenames.items():
                pkgs = map(str, vals)
                yield UnknownProfilePackages(pjoin(path[len(self.profiles_dir):].lstrip('/'), filename), pkgs)

        for path, filenames in sorted(unknown_pkg_use.items()):
            for filename, vals in filenames.items():
                for pkg, flags in vals:
                    yield UnknownProfilePackageUse(pjoin(path[len(self.profiles_dir):].lstrip('/'), filename), str(pkg), flags)

        for path, filenames in sorted(unknown_use.items()):
            for filename, vals in filenames.items():
                yield UnknownProfileUse(pjoin(path[len(self.profiles_dir):].lstrip('/'), filename), vals)

        for path, filenames in sorted(unknown_keywords.items()):
            for filename, vals in filenames.items():
                for pkg, keywords in vals:
                    yield UnknownProfilePackageKeywords(pjoin(path[len(self.profiles_dir):].lstrip('/'), filename), str(pkg), keywords)


class UnusedProfileDirs(results.ProfilesResult, results.Warning):
    __doc__ = 'Unused profile directories detected.'
...
class ArchesWithoutProfiles(results.ProfilesResult, results.Warning):
    __doc__ = 'Arches without corresponding profile listings.'
...
class NonexistentProfilePath(results.ProfilesResult, results.Error):
    __doc__ = "Specified profile path in profiles.desc doesn't exist."
...
class LaggingProfileEapi(results.ProfilesResult, results.Warning):
    __doc__ = 'Profile has an EAPI that is older than one of its parents.'
    ...
    @property
    def desc(self):
        return f"{self.profile!r} profile has EAPI {self.eapi}, {self.parent!r} parent has EAPI {self.parent_eapi}"


class UnknownCategories(results.ProfilesResult, results.Warning):
    __doc__ = "Category directories that aren't listed in a repo's categories.\n\n    Or the categories of the repo's masters as well.\n    "
    ...
    def finish(self):
        ...
        with patch('pkgcore.log.logger.error', report_profile_errors):
            with patch('pkgcore.log.logger.warning', report_profile_warnings):
                profiles = Profiles.parse((self.profiles_dir),
                  (self.repo.repo_id), known_status=known_profile_statuses,
        ...
        for p in profiles:
            ...
            for parent in profile.stack:
                seen_profile_dirs.update(dir_parents(parent.path[len(self.profiles_dir):]))
