...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/base.py
...
class Scope:
    __doc__ = 'Generic scope for scans, checks, and results.'
...
scopes = ImmutableDict({'git':commit_scope, 
...
class _CheckSet:
    __doc__ = 'Run only listed checks.'
...
class Whitelist(_CheckSet):
    __doc__ = 'Only run checks matching one of the provided patterns.'

    def filter(self, checks):
        return list(c for c in set(checks) if any(p(f"{c.__module__}.{c.__name__}") for p in self.patterns))


class Blacklist(_CheckSet):
    __doc__ = 'Only run checks not matching any of the provided patterns.'

    def filter(self, checks):
        return list(c for c in set(checks) if not any(p(f"{c.__module__}.{c.__name__}") for p in self.patterns))
