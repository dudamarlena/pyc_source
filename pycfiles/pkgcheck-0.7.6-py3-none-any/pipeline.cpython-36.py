...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/pipeline.py
...
class Pipeline:
    __doc__ = 'Check-running pipeline leveraging scope-based parallelism.'
    ...
    def __init__(self, options, scan_scope, pipes, restrict):
        ...
        self.pkg_scan = scan_scope in (base.version_scope, base.package_scope) and isinstance(restrict, boolean.AndRestriction)
        ...
        try:
            for scope in sorted((scoped_pipes['sync']), reverse=True):
                ...
                if scope is base.version_scope:
                    ...
                    if scope is base.package_scope:
        ...
        try:
            ...
            for pipe_mapping in self.pipes:
                for (source, exec_type), checks in pipe_mapping.items():
                    if exec_type == 'async':
                        runner = AsyncCheckRunner((self.options),
                          source, checks, results_q=results_q)
                    else:
...
class CheckRunner:
    ...
    def _metadata_error_cb(self, e):
        ...
        process_callback = cls is MetadataError or cls in getattr(self._running_check, 'known_results', self._known_results)
        if process_callback:
    ...
    def __eq__(self, other):
        return self.__class__ is other.__class__ and frozenset(self.checks) == frozenset(other.checks)
