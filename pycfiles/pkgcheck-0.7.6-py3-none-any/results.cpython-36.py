# uncompyle6 version 3.7.4
# Python bytecode 3.6 (3379)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/results.py
# Compiled at: 2020-02-09 15:46:32
# Size of source mod 2**32: 8116 bytes
"""Base classes for check results."""
from pkgcore.ebuild import cpv
from snakeoil import klass
from . import base
from .packages import FilteredPkg, RawCPV

class _ResultAttrs(type):
    __doc__ = 'Metaclass for setting attributes on base class objects.'

    @property
    def name(cls):
        if cls._name is not None:
            return cls._name
        else:
            return cls.__name__


class Result(metaclass=_ResultAttrs):
    __doc__ = 'Generic report result returned from a check.'
    _filtered = False
    scope = base.repo_scope
    level = None
    color = None
    _name = None

    @property
    def name(self):
        if self._name is not None:
            return self._name
        else:
            return self.__class__.__name__

    def __str__(self):
        return self.desc

    @property
    def desc(self):
        """Result description."""
        pass

    @property
    def _attrs(self):
        """Return all public result attributes."""
        return {k:v for k, v in self.__dict__.items() if not k.startswith('_') if not k.startswith('_')}

    @staticmethod
    def attrs_to_pkg(d):
        """Reconstruct a package object from split attributes."""
        category = d.pop('category', None)
        package = d.pop('package', None)
        version = d.pop('version', None)
        if any((category, package, version)):
            pkg = RawCPV(category, package, version)
            d['pkg'] = pkg
        return d

    def __eq__(self, other):
        return self._attrs == other._attrs

    def __hash__(self):
        return hash(tuple(sorted(self._attrs)))

    def __lt__(self, other):
        try:
            if self.scope is other.scope:
                if self.name == other.name:
                    return self.desc < other.desc
                else:
                    return self.name < other.name
            return self.scope < other.scope
        except AttributeError:
            pass

        return False


class Error(Result):
    __doc__ = 'Result with an error priority level.'
    level = 'error'
    color = 'red'


class Warning(Result):
    __doc__ = 'Result with a warning priority level.'
    level = 'warning'
    color = 'yellow'


class Info(Result):
    __doc__ = 'Result with an info priority level.'
    level = 'info'
    color = 'green'


class CommitResult(Result):
    __doc__ = 'Result related to a specific git commit.'
    scope = base.commit_scope

    def __init__(self, commit, **kwargs):
        (super().__init__)(**kwargs)
        self.commit = commit
        self._attr = 'commit'

    def __lt__(self, other):
        try:
            if self.commit == other.commit:
                if self.name == other.name:
                    return self.desc < other.desc
                else:
                    return self.name < other.name
        except AttributeError:
            pass

        return False


class ProfilesResult(Result):
    __doc__ = 'Result related to profiles.'
    scope = base.profiles_scope


class EclassResult(Result):
    __doc__ = 'Result related to a specific eclass.'
    scope = base.eclass_scope

    def __init__(self, eclass, **kwargs):
        (super().__init__)(**kwargs)
        self.eclass = str(eclass)
        self._attr = 'eclass'

    def __lt__(self, other):
        try:
            if self.eclass == other.eclass:
                if self.name == other.name:
                    return self.desc < other.desc
                else:
                    return self.name < other.name
            return self.eclass < other.eclass
        except AttributeError:
            pass

        return False


class CategoryResult(Result):
    __doc__ = 'Result related to a specific category.'
    scope = base.category_scope

    def __init__(self, pkg, **kwargs):
        (super().__init__)(**kwargs)
        self.category = pkg.category
        self._attr = 'category'

    def __lt__(self, other):
        try:
            if self.category != other.category:
                return self.category < other.category
        except AttributeError:
            pass

        return super().__lt__(other)


class PackageResult(CategoryResult):
    __doc__ = 'Result related to a specific package.'
    scope = base.package_scope

    def __init__(self, pkg, **kwargs):
        (super().__init__)(pkg, **kwargs)
        self.package = pkg.package
        self._attr = 'package'

    def __lt__(self, other):
        try:
            if self.package != other.package:
                return self.package < other.package
        except AttributeError:
            pass

        return super().__lt__(other)


class VersionResult(PackageResult):
    __doc__ = 'Result related to a specific version of a package.'
    scope = base.version_scope

    def __init__(self, pkg, **kwargs):
        (super().__init__)(pkg, **kwargs)
        self.version = pkg.fullver
        self._attr = 'version'

    @klass.jit_attr
    def ver_rev(self):
        version, _, revision = self.version.partition('-r')
        revision = cpv._Revision(revision)
        return (version, revision)

    def __lt__(self, other, cmp=None):
        try:
            if cmp is None:
                cmp = (cpv.ver_cmp)(*self.ver_rev + other.ver_rev)
            else:
                if cmp < 0:
                    return True
                if cmp > 0:
                    return False
        except AttributeError:
            pass

        return super().__lt__(other)


class LineResult(VersionResult):
    __doc__ = 'Result related to a specific line of an ebuild.'

    def __init__(self, line, lineno, **kwargs):
        (super().__init__)(**kwargs)
        self.line = line
        self.lineno = lineno

    def __lt__(self, other):
        cmp = None
        try:
            cmp = (cpv.ver_cmp)(*self.ver_rev + other.ver_rev)
            if cmp == 0:
                if self.lineno < other.lineno:
                    return True
                if self.lineno > other.lineno:
                    return False
        except AttributeError:
            pass

        return super().__lt__(other, cmp=cmp)


class FilteredVersionResult(VersionResult):
    __doc__ = 'Result that will be optionally filtered for old packages by default.'

    def __init__(self, pkg, **kwargs):
        if isinstance(pkg, FilteredPkg):
            self._filtered = True
            pkg = pkg._pkg
        (super().__init__)(pkg, **kwargs)


class _LogResult(Result):
    __doc__ = 'Message caught from a logger instance.'

    def __init__(self, msg):
        super().__init__()
        self.msg = msg

    @property
    def desc(self):
        return self.msg


class LogWarning(_LogResult, Warning):
    __doc__ = 'Warning caught from a logger instance.'


class LogError(_LogResult, Error):
    __doc__ = 'Error caught from a logger instance.'


class _RegisterMetadataErrors(_ResultAttrs):
    __doc__ = 'Metaclass for registering known metadata results.'

    def __new__(cls, name, bases, class_dict):
        new_cls = type.__new__(cls, name, bases, class_dict)
        attr = new_cls._attr
        if attr is not None:
            new_cls.results.add(new_cls)
            setting = new_cls.result_mapping.setdefault(attr, new_cls)
            if setting != new_cls:
                raise ValueError(f"metadata attribute {attr!r} already registered: {setting!r}")
        else:
            if new_cls.__name__ != 'MetadataError':
                raise ValueError(f"class missing metadata attributes: {new_cls!r}")
        return new_cls


class MetadataError(VersionResult, Error, metaclass=_RegisterMetadataErrors):
    __doc__ = "Problem detected with a package's metadata."
    _attr = None
    result_mapping = {}
    results = set()

    def __init__(self, attr, msg, **kwargs):
        (super().__init__)(**kwargs)
        self.attr = attr
        self.msg = str(msg)

    @property
    def desc(self):
        if self._attr is not None:
            return self.msg
        else:
            return f"attr({self.attr}): {self.msg}"