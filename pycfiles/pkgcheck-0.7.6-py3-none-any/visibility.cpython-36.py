...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/checks/visibility.py
...
class FakeConfigurable:
    __doc__ = 'Package wrapper binding profile data.'
    configurable = True
    __slots__ = ('use', 'iuse', '_forced_use', '_masked_use', '_pkg_use', '_raw_pkg',
                 '_profile')
    ...
    def __init__(self, pkg, profile):
        ...
        use_defaults = {x[1:] for x in pkg.iuse if x[0] == '+' if x[0] == '+'}
        enabled_use = (use_defaults | profile.use | self._pkg_use | self._forced_use) - self._masked_use
        object.__setattr__(self, 'use', frozenset(enabled_use & (profile.iuse_effective | pkg.iuse_effective)))
...
class VisibleVcsPkg(results.VersionResult, results.Error):
    __doc__ = 'Package is VCS-based, but visible.'
    ...
    def desc(self):
        ...
        return f'VCS version visible for KEYWORDS="{self.arch}", profile {self.profile}{num_profiles}'


class NonexistentDeps(results.VersionResult, results.Warning):
    __doc__ = 'No matches exist for a package dependency.'
...
class UncheckableDep(results.VersionResult, results.Warning):
    __doc__ = 'Given dependency cannot be checked due to the number of transitive use deps in it.'
...
class _NonsolvableDeps(results.VersionResult):
    __doc__ = 'No potential solution for a depset attribute.'

    def __init__(self, attr, keyword, profile, deps, profile_status, profile_deprecated, num_profiles=None, **kwargs):
        (super().__init__)(**kwargs)
        self.attr = attr
...
class VisibilityCheck(feeds.EvaluateDepSet, feeds.QueryCache, Check):
    ...
    def __init__(self, *args, profile_addon):
        ...
        self.report_cls_map = {'stable':NonsolvableDepsInStable, 
                    ...
                    if self.options.verbosity > 0:
                        for failures, profiles in profile_failures.items():
                            for profile_status, cls in self.report_cls_map.items():
                                for profile in sorted((profiles.get(profile_status, ())),
                                  key=(attrgetter('key', 'name'))):
                                    yield cls(attr,
                                      (profile.key), (profile.name), failures, profile_status,
                                      (profile.deprecated), pkg=pkg)
                        ...
                        for failures, profiles in profile_failures.items():
                            for profile_status, cls in self.report_cls_map.items():
                                ...
                                if status_profiles:
                                    ...
                                    yield cls(attr,
                                      (profile.key), (profile.name), failures,
                                      profile_status, (profile.deprecated),
                                      (len(status_profiles)), pkg=pkg)
