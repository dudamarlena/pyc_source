...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/checks/metadata.py
# Compiled at: 2020-02-09 15:46:32
# Size of source mod 2**32: 50521 bytes
import os, re
...
class MissingLicenseFile(results.VersionResult, results.Error):
    __doc__ = 'Used license(s) have no matching license file(s).'
...
class MissingLicense(results.VersionResult, results.Error):
    __doc__ = 'Package has no LICENSE defined.'
    desc = 'no license defined'


class InvalidLicense(results.MetadataError):
    __doc__ = "Package's LICENSE is invalid."
    _attr = 'license'


class MissingLicenseRestricts(results.VersionResult, results.Error):
    __doc__ = 'Restrictive license used without matching RESTRICT.'
...
class UnnecessaryLicense(results.VersionResult, results.Warning):
    __doc__ = 'LICENSE defined for package that is license-less.'
...
class LicenseCheck(Check):
    __doc__ = 'LICENSE validity checks.'
    ...
    def _required_licenses(self, license_group, nodes, restricts=None):
        ...
        for node in nodes:
            ...
            if isinstance(node, str):
                if node not in license_group:
                    continue
                else:
                    if isinstance(node, boolean.AndRestriction):
    ...
    def feed(self, pkg):
        if self.eula is not None:
            for license, restrictions in self._required_licenses(self.eula, pkg.license):
                restricts = frozenset(chain.from_iterable(x.vals for x in restrictions if not x.negate))
                ...
                if not self.mirror_restricts.intersection(license_restrictions):
                    ...
                    if missing_restricts:
                        yield MissingLicenseRestricts('EULA',
                          license, missing_restricts, pkg=pkg)
...
class _UseFlagsResult(results.VersionResult, results.Error):
    __doc__ = 'Generic USE flags result.'
    _type = None
...
class InvalidUseFlags(_UseFlagsResult):
    __doc__ = 'Package IUSE contains invalid USE flags.'
    _type = 'invalid'


class UnknownUseFlags(_UseFlagsResult):
    __doc__ = 'Package IUSE contains unknown USE flags.'
    _type = 'unknown'


class IuseCheck(Check):
    __doc__ = 'IUSE validity checks.'
    required_addons = (
     addons.UseAddon,)
...
class DeprecatedEapi(_EapiResult, results.Warning):
    __doc__ = "Package's EAPI is deprecated according to repo metadata."
    _type = 'deprecated'


class BannedEapi(_EapiResult, results.Error):
    __doc__ = "Package's EAPI is banned according to repo metadata."
    _type = 'banned'


class EapiCheck(Check):
    __doc__ = 'Scan for packages with banned or deprecated EAPIs.'
...
class InvalidEapi(results.MetadataError):
    __doc__ = "Package's EAPI is invalid."
    _attr = 'eapi'


class InvalidSlot(results.MetadataError):
    __doc__ = "Package's SLOT is invalid."
    _attr = 'slot'


class SourcingError(results.MetadataError):
    __doc__ = 'Failed sourcing ebuild.'
    _attr = 'data'


class SourcingCheck(Check):
    __doc__ = 'Scan for packages with sourcing errors or invalid, sourced metadata variables.'
...
class RequiredUseDefaults(results.VersionResult, results.Warning):
    ...
    @property
    def desc(self):
        if self.use or self.num_profiles is not None:
            if self.num_profiles > 1:
                num_profiles = f" ({self.num_profiles} total)"
            else:
                num_profiles = ''
            return f"profile: {self.profile!r}{num_profiles} failed REQUIRED_USE: {self.required_use}"
        else:
            return f"keyword: {self.keyword}, profile: {self.profile!r}, default USE: [{', '.join(self.use)}] -- failed REQUIRED_USE: {self.required_use}"


class InvalidRequiredUse(results.MetadataError):
    __doc__ = "Package's REQUIRED_USE is invalid."
    _attr = 'required_use'


class RequiredUseCheck(Check):
    __doc__ = 'REQUIRED_USE validity checks.'
    _source = (
     sources.RestrictionRepoSource,
     (
      packages.PackageRestriction('eapi', values.GetAttrRestriction('options.has_required_use', values.FunctionRestriction(bool))),))
        ...
        if self.options.verbosity > 0:
            for node, profile_info in failures.items():
                for use, keyword, profile in profile_info:
                    yield RequiredUseDefaults((str(node)),
                      (sorted(use)), keyword, profile, pkg=pkg)
            ...
            for node, profile_info in failures.items():
                ...
                yield RequiredUseDefaults((str(node)),
                  profile=profile, num_profiles=num_profiles, pkg=pkg)


class UnusedLocalUse(results.PackageResult, results.Warning):
    __doc__ = 'Unused local USE flag(s).'
...
class MatchingGlobalUse(results.PackageResult, results.Error):
    __doc__ = 'Local USE flag description matches a global USE flag.'
...
class ProbableGlobalUse(results.PackageResult, results.Warning):
    __doc__ = 'Local USE flag description closely matches a global USE flag.'
...
class LocalUseCheck(Check):
    __doc__ = 'Check local USE flags in metadata.xml for various issues.'
    ...
    def __init__(self, *args, use_addon):
        (super().__init__)(*args)
        self.iuse_handler = use_addon
        self.global_use = {flag:desc for matcher, (flag, desc) in self.options.target_repo.config.use_desc}
        self.use_expand_groups = dict()
        for key in self.options.target_repo.config.use_expand_desc.keys():
            self.use_expand_groups[key] = {flag for flag, desc in self.options.target_repo.config.use_expand_desc[key]}
...
class MissingSlotDepCheck(Check):
    __doc__ = 'Check for missing slot dependencies.'
    _source = (
     sources.RestrictionRepoSource,
     (
      packages.PackageRestriction('eapi', values.GetAttrRestriction('options.sub_slotting', values.FunctionRestriction(bool))),))
    required_addons = (addons.UseAddon,)
    ...
    def feed(self, pkg):
        ...
        for dep in (x for x in set(rdepend).intersection(depend) if not (x.blocks or x.slot is not None or x.slot_operator is not None)):
...
class MissingUseDepDefault(results.VersionResult, results.Warning):
    __doc__ = 'Package dependencies with USE dependencies missing defaults.'
    ...
    def desc(self):
        ...
        pkgs = ', '.join(self.pkgs)
        return f'{self.attr}="{self.atom}": USE flag {self.flag!r} missing from package{s}: [ {pkgs} ]'


class OutdatedBlocker(results.VersionResult, results.Info):
    __doc__ = 'Blocker dependency removed more than two years ago from the tree.\n\n    Note that this ignores slot/subslot deps and USE deps in blocker atoms.\n    '
...
class DeprecatedPkg(results.VersionResult, results.Warning):
    __doc__ = 'Package dependency is flagged in profiles/package.deprecated.'
...
class BadDependency(results.VersionResult, results.Error):
    __doc__ = 'Package dependency is bad for some reason.'
...
class InvalidDepend(results.MetadataError):
    __doc__ = 'Package has invalid DEPEND.'
    _attr = 'depend'


class InvalidRdepend(results.MetadataError):
    __doc__ = 'Package has invalid RDEPEND.'
    _attr = 'rdepend'


class InvalidPdepend(results.MetadataError):
    __doc__ = 'Package has invalid PDEPEND.'
    _attr = 'pdepend'


class InvalidBdepend(results.MetadataError):
    __doc__ = 'Package has invalid BDEPEND.'
    _attr = 'bdepend'


class DependencyCheck(Check):
    __doc__ = 'Check BDEPEND, DEPEND, RDEPEND, and PDEPEND.'
    required_addons = (
     addons.UseAddon, git.GitAddon)
    known_results = frozenset([
    ...
    def feed(self, pkg):
        for attr in sorted(x.lower() for x in pkg.eapi.dep_keys):
            ...
            nodes, unstated = self.iuse_filter((
             atom_cls, boolean.OrRestriction),
              pkg, deps, attr=attr)
            yield from unstated
                ...
                for atom in iflatten_instance(node, (atom_cls,)):
                    ...
                    if in_or_restriction:
                        if atom.slot_operator == '=':
                            yield BadDependency(attr,
                              atom, '= slot operator used inside || block', pkg=pkg)
                    if pkg.eapi.options.has_use_dep_defaults:
                        if atom.use is not None:
                            ...
                            if atom.blocks:
                                if atom.match(pkg):
                                    ...
                                    if atom.slot_operator == '=':
                                        yield BadDependency(attr,
                                          atom, '= slot operator used in blocker', pkg=pkg)
                            ...
                            matches = self.options.search_repo.match(unblocked) or self.existence_repo.match(unblocked)
                            if matches:
...
class OverlappingKeywords(results.VersionResult, results.Warning):
    __doc__ = 'Packages having overlapping arch and ~arch KEYWORDS.'
...
class DuplicateKeywords(results.VersionResult, results.Warning):
    __doc__ = 'Packages having duplicate KEYWORDS.'
...
class MissingVirtualKeywords(results.VersionResult, results.Warning):
    __doc__ = 'Virtual packages with keywords missing from their dependencies.'
...
class KeywordsCheck(Check):
    __doc__ = 'Check package keywords for sanity; empty keywords, and -* are flagged.'
    required_addons = (
     addons.UseAddon,)
    known_results = frozenset([
    ...
    def __init__(self, *args, use_addon):
        ...
        self.valid_keywords = special_keywords | stable_keywords | unstable_keywords | disabled_keywords
        self.portage_keywords = {
         '*', '~*'}

    def feed(self, pkg):
        if len(pkg.keywords) == 1:
            if pkg.keywords[0] == '-*':
                ...
                unstable = {x[1:] for x in pkg.keywords if x[0] == '~' if x[0] == '~'}
                stable = {x for x in pkg.keywords if x[0] != '~' if x[0] != '~'}
                overlapping = unstable & stable
                if overlapping:
                    keywords = ', '.join(map(str, sorted(zip(overlapping, ('~' + x for x in overlapping)))))
            ...
            if pkg.sorted_keywords != pkg.keywords:
                if self.options.verbosity < 1:
                    ...
                    yield UnsortedKeywords((pkg.keywords),
                      sorted_keywords=(pkg.sorted_keywords), pkg=pkg)
        if pkg.category == 'virtual':
            keywords = set()
            rdepend, _ = self.iuse_filter((atom_cls,), pkg, pkg.rdepend)
            for x in set(rdepend):
                for p in self.options.search_repo.match(x.no_usedeps):
                    keywords.update(x for x in p.keywords if x.lstrip('~') in self.valid_arches)
...
class MissingUri(results.VersionResult, results.Warning):
    __doc__ = "RESTRICT=fetch isn't set, yet no full URI exists."
...
class UnknownMirror(results.VersionResult, results.Error):
    __doc__ = 'URI uses an unknown mirror.'
...
class RedundantUriRename(results.VersionResult, results.Warning):
    __doc__ = "URI uses a redundant rename that doesn't change the filename."
...
class SrcUriCheck(Check):
    ...
    def __init__(self, *args, use_addon):
        ...
        self.zip_to_tar_re = re.compile('https?://(github\\.com/.*?/.*?/archive/.+\\.zip|gitlab\\.com/.*?/.*?/-/archive/.+\\.zip)')
    ...
    def feed(self, pkg):
        ...
        for f_inst, restrictions in fetchables.items():
            if f_inst.filename in seen:
                ...
                unknown_mirrors = [(m, sub_uri) for m, sub_uri in mirrors if isinstance(m, unknown_mirror)]
                ...
                bad_filenames_re = f"^({PN}|v?{PV}|[0-9a-f]{{40}}){exts}$"
            ...
            if not f_inst.uri and 'fetch' not in pkg.restrict.evaluate_depset(restricts):
                ...
                for uri in f_inst.uri:
                    ...
                    if i == -1:
                        ...
                        if uri[:i] not in self.valid_protos:
...
class BadDescription(results.VersionResult, results.Warning):
    __doc__ = "Package's description is bad for some reason."
    ...
    def feed(self, pkg):
        if not pkg.homepage:
            ...
            if pkg.category in self.missing_categories:
                yield BadHomepage(f"HOMEPAGE should be undefined for {pkg.category!r} packages",
                  pkg=pkg)
            else:
                for homepage in pkg.homepage:
                    if homepage.rstrip('/') in self.generic_sites:
                        ...
                        if i == -1:
                            ...
                            if homepage[:i] not in SrcUriCheck.valid_protos:
                                yield BadHomepage(f"HOMEPAGE={homepage!r} uses unsupported protocol {homepage[:i]!r}",
                                  pkg=pkg)


class UnknownRestrict(results.VersionResult, results.Warning):
    __doc__ = "Package's RESTRICT metadata has unknown entries."
...
class UnknownProperties(results.VersionResult, results.Warning):
    __doc__ = "Package's PROPERTIES metadata has unknown entries."
...
class InvalidRestrict(results.MetadataError):
    __doc__ = "Package's RESTRICT is invalid."
    _attr = 'restrict'


class InvalidProperties(results.MetadataError):
    __doc__ = "Package's PROPERTIES is invalid."
    _attr = 'properties'


class _RestrictPropertiesCheck(Check):
    __doc__ = 'Generic check for RESTRICT and PROPERTIES.'
    _attr = None
...
class RestrictCheck(_RestrictPropertiesCheck):
    __doc__ = 'RESTRICT related checks.'
...
class PropertiesCheck(_RestrictPropertiesCheck):
    __doc__ = 'PROPERTIES related checks.'
...
class RestrictTestCheck(Check):
    __doc__ = 'Check whether packages specify RESTRICT="!test? ( test )".'
...
class MissingUnpackerDepCheck(Check):
    __doc__ = 'Check whether package is missing unpacker dependencies.'
    known_results = frozenset([MissingUnpackerDep])
    required_addons = (addons.UseAddon,)
    non_system_unpackers = ImmutableDict({'.zip':frozenset(['app-arch/unzip']), 
    ...
    def feed(self, pkg):
        ...
        for unpackers, filenames in missing_unpackers.items():
            yield MissingUnpackerDep((str(pkg.eapi)),
              (sorted(filenames)), (sorted(unpackers)), pkg=pkg)