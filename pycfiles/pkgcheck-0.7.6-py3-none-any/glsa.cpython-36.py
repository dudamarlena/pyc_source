...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/checks/glsa.py
...
class VulnerablePackage(results.VersionResult, results.Error):
    __doc__ = 'Packages marked as vulnerable by GLSAs.'
...
class GlsaCheck(GentooRepoCheck):
    __doc__ = 'Scan for vulnerable ebuilds in the tree.\n\n    Requires a GLSA directory for vulnerability info.\n    '
    known_results = frozenset([VulnerablePackage])

    @staticmethod
    def mangle_argparser(parser):
        parser.plugin.add_argument('--glsa-dir',
          dest='glsa_location', type=existent_dir, help='source directory for glsas; tries to autodetermine it, may be required if no glsa dirs are known')
    ...
    def check_args(cls, parser, namespace):
        ...
        if glsa_loc is None:
            ...
            if len(glsa_dirs) > 1:
                ...
                parser.error(f"--glsa-dir needs to be specified to select one of multiple glsa sources: {glsa_dirs}")
            try:
                glsa_loc = glsa_dirs[0]
            except IndexError:
                selected_checks = namespace.selected_checks
                if selected_checks is not None:
                    if cls.__name__ in selected_checks[1]:
                        parser.error('no available glsa source, --glsa-dir must be specified')
                namespace.glsa_enabled = False
                if namespace.verbosity > 1:
                    logger.warning('disabling GLSA checks due to no glsa source being found, and the check not being explicitly enabled')
                return
            ...
            if vuln.match(pkg):
                ...
                for v in collect_package_restrictions(vuln, ['keywords']):
                    if isinstance(v.restriction, values.ContainmentMatch2):
                        ...
                        raise Exception(f"unexpected restriction sequence- {v.restriction} in {vuln}")

                keys = {x.lstrip('~') for x in pkg.keywords if not x.startswith('-') if not x.startswith('-')}
                if arches:
