...
# [GCC 8.4.0]
# Embedded file name: /home/runner/work/pkgcheck/pkgcheck/build/lib/pkgcheck/git.py
# Compiled at: 2020-02-09 15:46:32
# Size of source mod 2**32: 21097 bytes
"""Git specific support and addon."""
import argparse, os, pickle, shlex, subprocess, sys
...
demand_compile_regexp('ebuild_ADM_regex', f"^(?P<status>[ADM])\\t{_ebuild_path_regex}$")
demand_compile_regexp('ebuild_R_regex', f"^(?P<status>R)\\d+\\t{_ebuild_path_regex_raw}\\t{_ebuild_path_regex}$")
demand_compile_regexp('eclass_regex', '^eclass/(?P<eclass>\\S+)\\.eclass$')

class GitCommit:
    __doc__ = 'Git commit objects.'
...
class GitPkgChange:
    __doc__ = 'Git package change objects.'
...
class ParsedGitRepo(UserDict, caches.Cache):
    __doc__ = 'Parse repository git logs.'
    ...
    def _parse_file_line(line):
        ...
        if match:
            ...
            try:
                return (
                 atom_cls(f"={category}/{pkg}"), status)
    ...
    @classmethod
    def parse_git_log(cls, repo_path, git_cmd=None, commit=None, pkgs=False, debug=False):
    ...
    def _pkg_changes(self, local=False, **kwargs):
        ...
        for pkg in (self.parse_git_log)(self.location, cmd, pkgs=True, **kwargs):
            ...
            if key not in seen:
                seen.add(key)
                self.data.setdefault(atom.category, {}).setdefault(atom.package, {})[(atom.fullver, pkg.status)] = {'date':pkg.commit.commit_date, 
                 'status':pkg.status, 
                 'commit':pkg.commit.hash if not local else pkg.commit}


class _GitCommitPkg(cpv.VersionedCPV):
    __doc__ = 'Fake packages encapsulating commits parsed from git log.'
...
class _HistoricalRepo(SimpleTree):
    __doc__ = 'Repository encapsulating historical git data.'
    _status_filter = None
    ...
    def _internal_gen_candidates(self, candidates, sorter, raw_pkg_cls, **kwargs):
        for cp in sorter(candidates):
            yield from sorter(raw_pkg_cls((cp[0]), (cp[1]), ver, **data) for ver, data in self.versions.get(cp, ()))
...
class GitAddon(base.Addon, caches.CachedAddon):
    ...
    def mangle_argparser(cls, parser):
        ...
        group.add_argument('--commits',
          action=_ScanCommits, nargs='?', metavar='COMMIT',
          const='origin',
          default=None,
        ...
        if namespace.commits:
            if namespace.targets:
                ...
                if p.returncode != 0:
                    ...
                    if not p.stdout:
                        parser.exit()
                pkgs, eclasses = partition((p.stdout.splitlines()),
    ...
    def get_commit_hash(repo_location, commit='origin/HEAD'):
        ...
        ret, out = spawn_get_output([
         'git', 'rev-parse', commit],
          cwd=repo_location)
        if ret != 0:
            raise ValueError(f"failed retrieving {commit} commit hash for git repo: {repo_location}")
        return out[0].strip()
        ...
        if self.options.cache['git']:
            for repo in repos:
                ...
                if git_repo is not None:
                    if repo.location == getattr(git_repo, 'location', None):
                        if commit != git_repo.commit:
                            with output_lock:
                                old, new = git_repo.commit[:13], commit[:13]
                                print(f"updating {repo} git repo cache: {old} -> {new}",
                                  file=(sys.stderr))
                        ...
                        with output_lock:
                            print(f"creating {repo} git repo cache: {commit[:13]}",
                              file=(sys.stderr))
