...
# Size of source mod 2**32: 11764 bytes
"""
...
import solstice.core
from solstice.core import defines
logging.config.fileConfig((solstice.core.get_logging_config()), disable_existing_loggers=False)
logger = logging.getLogger(__name__)
logger.setLevel(solstice.core.get_logging_level())
...
class SolsticeAsset(artella_asset.ArtellaAsset, object):
    ...
    def import_file_by_extension(self, extension=None):
        ...
        if extension == defines.SOLSTICE_ALEMBIC_EXTENSION:
            ...
            if extension == defines.SOLSTICE_STANDIN_EXTENSION:
                self.import_standin_file()
            else:
                if extension == defines.SOLSTICE_RIG_EXTENSION:
    ...
    def import_rig_file(self):
        ...
        """
        self.import_file(file_type=(defines.SOLSTICE_RIG_ASSET_TYPE),
    ...
    def reference_rig_file(self):
        ...
        """
        self.reference_file(file_type=(defines.SOLSTICE_RIG_ASSET_TYPE),
          status=(artella_defines.ARTELLA_SYNC_PUBLISHED_ASSET_STATUS))
    ...
    def import_standin_file(self):
        ...
        if not latest_published_local_versions:
            ...
            return
        standin_file_type = self.get_file_type((defines.SOLSTICE_MODEL_ASSET_TYPE),
          extension=(defines.SOLSTICE_STANDIN_EXTENSION))
    ...
    def import_alembic_file(self, parent_name=None, fix_path=True):
        ...
        if not latest_published_local_versions:
            logger.warning('Asset {} has not model files synced!'.format(self.get_name()))
            return
        alembic_file_type = self.get_file_type((defines.SOLSTICE_MODEL_ASSET_TYPE),
          extension=(defines.SOLSTICE_ALEMBIC_EXTENSION))
        ...
        if not self._node or not tp.Dcc.object_exists(self._node):
            ...
            if self._tag_info_dict:
                ...
                if type(shaders_dict) != dict:
                    logger.error('Impossible to get dictionary from shaders info. Maybe shaders are not set up properly. Please contact TD!')
                else:
                    return shaders_dict
            elif not tp.Dcc.attribute_exists(node=(self._node), attribute_name='shaders'):
                return
            else:
                ...
                if type(shaders_dict) != dict:
                    logger.error('Impossible to get dictionary from shaders attribute. Maybe shaders are not set up properly. Please contact TD!')
                else:
                    return shaders_dict
            return shaders_attr