...
# Size of source mod 2**32: 7950 bytes
"""
...
class AlembicAssetFile(assetfile.ArtellaAssetType, object):
    ...
    def _import_file(self, path, fix_path=True, *args, **kwargs):
        ...
        alembicmanager.importer.import_alembic(project=(self.get_project()),
          alembic_path=path,
          parent=parent,
          fix_path=fix_path)
    ...
    def _reference_file(self, path, fix_path=True):
        ...
        """
        if tp.is_houdini():
            return alembicmanager.importer.import_alembic(project=(self.get_project()),
              alembic_path=path,
              fix_path=fix_path)
        else:
            return alembicmanager.importer.reference_alembic(project=(self.get_project()),
              alembic_path=path,
              fix_path=fix_path)
