...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/model.py
# Compiled at: 2019-11-04 07:23:49
# Size of source mod 2**32: 3660 bytes
...
class Model(object):
    ...
    def train_and_eval_then_save(self, train_inpf, eval_inpf, configure):
        evaluate_result, export_results, final_saved_model = train_model(train_inpf=train_inpf,
          eval_inpf=eval_inpf,
          config=configure,
    ...
    def get_default_config(self):
        ...
        params = {'dim':300, 
         ...
         'preds':{'train':utils.join_path(result_dir, '{}.txt'.format('preds_train')), 
          'test':utils.join_path(result_dir, '{}.txt'.format('preds_test'))}, 
         'optimizer_params':{},  'saved_model_dir':utils.join_path(result_dir, 'saved_model'), 
         'hook':{'stop_if_no_increase': {'min_steps':100, 
                                  'run_every_secs':60, 
         ...
         'use_tpu':False, 
         'tpu_config':{'tpu_name':None, 
          'zone':None,  'gcp_project':None}, 
         'save_checkpoints_secs':60, 
         'learning_rate':0.001, 
         'max_steps':None, 
         'max_steps_without_increase':1000, 
         'train_hook':{},  'shuffle_pool_size':30, 
         'embedding_dim':64}
        ...
        params['vocab_data'] = np.loadtxt(vocab_data_file,
          dtype=(np.unicode), comments=None, encoding=None).tolist()
