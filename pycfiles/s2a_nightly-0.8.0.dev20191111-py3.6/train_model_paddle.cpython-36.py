...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/trainer/train_model_paddle.py
# Compiled at: 2019-07-30 09:27:42
# Size of source mod 2**32: 3421 bytes
import os, time, paddle, paddle.fluid as fluid
...
def train_model(train_inpf, eval_inpf, config, *args, **kwargs):
    ...
    with fluid.program_guard(main_program=main_program, startup_program=startup_program):
        ...
        embed_char = fluid.layers.embedding(input=words,
          size=[
         config['embedding_vocabulary_size'], config['embedding_dim']],
          dtype='float32')
        place = fluid.CPUPlace()
        train_reader = paddle.batch(paddle.reader.shuffle(train_inpf, buf_size=500),
          batch_size=(config['batch_size']))
        forward_hidden_state, _ = fluid.layers.dynamic_lstm(input=embed_char,
          size=(config['embedding_dim']),
          candidate_activation='relu',
          gate_activation='sigmoid',
          cell_activation='sigmoid')
        backward_hidden_state, _ = fluid.layers.dynamic_lstm(input=embed_char,
          size=(config['embedding_dim']),
          candidate_activation='relu',
          gate_activation='sigmoid',
          cell_activation='sigmoid',
        ...
        crf_cost = fluid.layers.linear_chain_crf(input=score,
          label=tags,
          param_attr=fluid.ParamAttr(name='crfw'))
        avg_cost = fluid.layers.mean(crf_cost)
        crf_decode = fluid.layers.crf_decoding(input=score,
          param_attr=fluid.ParamAttr(name='crfw'))
        ...
        for pass_id in range(config['epochs']):
            ...
            for data in train_reader():
                ...
                avg_loss_value, = exe.run(main_program,
                  feed=feed, fetch_list=[avg_cost], return_numpy=True)
