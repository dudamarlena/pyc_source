...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/trainer/cli.py
...
def converter_for_request(request: Request) -> Any:
    return {'words':request.query, 
     'words_len':[len(list(filter(lambda x: x != 0.0, text))) for text in request.query]}
