...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/input.py
# Compiled at: 2019-11-07 21:17:24
# Size of source mod 2**32: 7112 bytes
import collections, functools, json, logging
...
class Lookuper(object):
    ...
    def __init__(self, index_table: Dict[(str, int)]):
        ...
        self.inverse_index_table = OrderedDict(sorted([(v, k) for k, v in self.index_table.items()], key=(lambda x: x[0])))
...
def read_assets():
    return {'vocab_filename':'data/unicode_char_list.txt', 
...
def parse_to_dataset(data_generator_func, config=None, shuffle_and_repeat=False):
    ...
    defaults = (('<pad>', 0), 'O')
    dataset = tf.data.Dataset.from_generator((functools.partial(generator_func, data_generator_func, config)),
      output_shapes=shapes,
      output_types=types)
    if shuffle_and_repeat:
        dataset = dataset.shuffle(config['shuffle_pool_size']).repeat(config['epochs'])
    dataset = dataset.padded_batch(config['batch_size'], shapes, defaults).prefetch(1)
    return dataset
