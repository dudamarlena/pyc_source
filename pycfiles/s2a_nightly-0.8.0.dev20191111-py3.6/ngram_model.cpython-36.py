...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/algorithms/ngram_model.py
# Compiled at: 2019-07-30 09:27:42
# Size of source mod 2**32: 7970 bytes
...
class NgramModel(object):
    ...
    def input_layer(self):
        ...
        vocab_words = tf.contrib.lookup.index_table_from_tensor(mapping_strings,
        ...
        return (
         indices, num_tags, word_ids, nwords)
    ...
    def input_lookup_layer(self):
        ...
        feature_column = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_vocabulary_list(key='lookup',
          vocabulary_list=[
         0, 1]))
        ...
        feature_shape = [
         raw_feature_shape[0], raw_feature_shape[1], 5]
    ...
    def embedding_layer(self, word_ids):
        ...
        embedding_array = np.vstack([glove, [[0.0] * self.params['dim']]])
    ...
    def loss_layer(self, preds, ground_true, nwords, crf_params):
        with tf.name_scope('CRF_log_likelihood'):
            log_likelihood, _ = tf.contrib.crf.crf_log_likelihood(preds, ground_true, nwords, crf_params)
    ...
    def __call__(self):
        ...
        metrics = self.compute_metrics(true_tag_ids, pred_ids, num_tags, indices, nwords)
