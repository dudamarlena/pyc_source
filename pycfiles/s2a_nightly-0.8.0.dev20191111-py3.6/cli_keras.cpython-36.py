...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/trainer/cli_keras.py
# Compiled at: 2019-11-05 03:13:47
...
def main():
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint((os.path.join(create_dir_if_needed(config['model_dir']), 'cp-{epoch:04d}.ckpt')),
    ...
    tf.keras.experimental.export_saved_model(model,
      (create_dir_if_needed(config['saved_model_dir'])), serving_only=True)
    export_as_deliverable_model((create_dir_if_needed(config['deliverable_model_dir'])),
      keras_saved_model=(config['saved_model_dir']),
      vocabulary_lookup_table=vocabulary_lookuper,
      tag_lookup_table=tag_lookuper,
      padding_parameter={'maxlen':MAX_SENTENCE_LEN, 
