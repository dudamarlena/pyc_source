...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/seq2annotation/algorithms/IDCNN_CRF_model.py
...
class IdcnnCrfModel(Model):
    ...
    def idcnn_layer(self, embeddings):
        with tf.variable_scope('idcnn'):
            ...
            shape = [
             1, self.params['filter_width'], self.params['dim'], self.params['num_filter']]
            print(shape)
            filter_weights = tf.get_variable('idcnn_filter',
              shape=shape)
            layerInput = tf.nn.conv2d(model_inputs, filter_weights,
              strides=[
             1, 1, 1, 1],
              padding='SAME',
              name='init_layer')
