...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/environment/dpisonline/src/kotti_alert/kotti_alert/views/view.py
# Compiled at: 2019-09-18 15:00:51
"""
...
@view_config(name='kotti-alert', renderer='kotti_alert:templates/message.pt')
    ...
    def __call__(self):
        ...
        if cookie_str:
            ...
            for cookie in cookies:
                try:
                    alert_ids.append(int(cookie.replace(' ', '')))
        ...
        alert = Alert.get_by_priority(user=user, excludes=alert_ids)
        return {'alert': alert}
...
class AlertControlPanel(BaseView):
    ...
    @view_config(name='all-alerts', permission='admin', root_only=True, renderer='kotti_alert:templates/list.pt')
    ...
    def default_view(self):
        if self.request.user:
            if self.request.has_permission('admin', self.context):
                return {}
            user = self.request.user
            if self.context.username_or_group != user.name and self.context.username_or_group not in user.groups and self.context.username_or_group != '':
                self.request.session.flash(('You do not have permission to view {} alert message').format(self.context.title), 'warning')
                raise HTTPForbidden()
        elif self.context.username_or_group != '':
            self.request.session.flash(('Please login to view {} alert message').format(self.context.title), 'warning')
            raise HTTPForbidden()
        return {}
