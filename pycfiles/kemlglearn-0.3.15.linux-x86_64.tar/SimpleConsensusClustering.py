...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/kemlglearn/cluster/consensus/SimpleConsensusClustering.py
# Compiled at: 2018-02-01 04:01:33
"""
...
class SimpleConsensusClustering(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def __init__(self, n_clusters, n_clusters_base=None, ncb_rand=False, base='kmeans', n_components=10, consensus='coincidence', consensus2='kmeans'):
    ...
    def _fit_process_coincidence(self, X):
        ...
        for i in range(self.n_components):
            ...
            if self.base == 'kmeans':
                ...
                km = SpectralClustering(n_clusters=ncl, assign_labels='discretize', affinity='nearest_neighbors', n_neighbors=30)
            km.fit(X)
        ...
        if self.consensus2 == 'kmeans':
            ...
            kmc.fit(coin_matrix)
            return (
             kmc.cluster_centers_, kmc.labels_)
        else:
            if self.consensus2 == 'spectral':
                kmc = SpectralClustering(n_clusters=self.n_clusters, assign_labels='discretize', affinity='nearest_neighbors', n_neighbors=40)
                kmc.fit(coin_matrix)
                return (
                 None, kmc.labels_)
            return
