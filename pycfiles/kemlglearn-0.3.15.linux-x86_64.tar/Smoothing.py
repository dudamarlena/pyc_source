...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/kemlglearn/time_series/smoothing/Smoothing.py
...
def tvdip(y, lambdas, display=1, stoptol=0.001, maxiter=60):
    ...
    for idx, l in enumerate(lambdas):
        ...
        for iters in range(maxiter):
            ...
            gap = pobj - dobj
            if display:
                print ('{:5d} {:7.2e} {:7.2e} {:7.2e}').format(iters, pobj[(0, 0)], dobj[(0,
                                                                                          0)], gap[(0,
                                                                                                    0)])
            ...
            step = 1
            if np.any(negIdx1):
                step = np.minimum(step, 0.99 * (-mu1[negIdx1] / dmu1[negIdx1]).min(0))
            if np.any(negIdx2):
                step = np.minimum(step, 0.99 * (-mu2[negIdx2] / dmu2[negIdx2]).min(0))
            for _ in range(MAXLSITER):
                ...
                if np.maximum(newf1.max(0), newf2.max(0)) < 0 and scp.linalg.norm(newResidual) <= (1 - ALPHA * step) * scp.linalg.norm(residual):
                    break
                step = BETA * step
