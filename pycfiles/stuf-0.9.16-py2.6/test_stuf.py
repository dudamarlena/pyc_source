...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/tests/test_stuf.py
# Compiled at: 2014-12-21 02:16:00
"""test stuf"""
...
class TestDefaultStuf(Base, unittest.TestCase):
    ...
    @property
    def _makeone(self):
        return self._impone(list, test1='test1', test2='test2', test3=dict(e=1))
...
class TestFrozenStuf(Base, unittest.TestCase):
    ...
    def test__setattr__(self):
        ...
        self.assertRaises(AttributeError, setattr(self.stuf, 'test1', 'test1again'))
        self.assertRaises(AttributeError, setattr(self.stuf.test3, 'e', 5))

    def test__setitem__(self):
        self.assertRaises(AttributeError, lambda : self.stuf.__setitem__('max', 3))
        self.assertRaises(AttributeError, lambda : self.stuf.__setitem__('test2', 'test2again'))
        self.assertRaises(AttributeError, lambda : self.stuf.test3.__setitem__('e', 5))
    ...
    def test_setdefault(self):
        self.assertRaises(AttributeError, lambda : self.stuf.test3.setdefault('e', 8))
        self.assertRaises(AttributeError, lambda : self.stuf.test3.setdefault('r', 8))
        self.assertRaises(AttributeError, lambda : self.stuf.setdefault('test1', 8))
        self.assertRaises(AttributeError, lambda : self.stuf.setdefault('pow', 8))
...
class TestChainStuf(Base, unittest.TestCase):
    ...
    @property
    def _makeone(self):
        return self._impone(dict(test1='test1'), dict(test2='test2'), dict(test3=dict(e=1)))
    ...
    def test_parents(self):
        ...
        self.assertEquals(stuffed.maps, [dict(test2='test2'), dict(test3=dict(e=1))])

    def test_new_child(self):
        from stuf.core import stuf
        stuffed = self.stuf.new_child()
        self.assertEquals(stuffed.maps, [
...
class TestCounter(unittest.TestCase):
    ...
    def test_multiset_operations(self):
        ...
        for _ in lrange(1000):
            p = self._impone(dict((elem, randrange(-2, 4)) for elem in elements))
            p.update(e=1, f=-1, g=0)
            q = self._impone(dict((elem, randrange(-2, 4)) for elem in elements))
        ...
        elements = 'abcdef'
        for _ in lrange(100):
            p = self._impone(dict((elem, randrange(0, 2)) for elem in elements))
            q = self._impone(dict((elem, randrange(0, 2)) for elem in elements))
    ...
    def test_subtract(self):
        ...
        self.assertEqual(c, self._impone(a=-6, b=-2, c=8, d=0, e=-5, f=-30, g=40, h=50))
        c = self._impone(a=-5, b=0, c=5, d=10, e=15, g=40)
