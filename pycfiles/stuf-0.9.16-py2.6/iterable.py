...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/stuf/iterable.py
...
def exhaustmap(call, mapping, filter=None, exception=StopIteration, _n=next):
    ...
    iterable = starmap(call, items(mapping) if filter is None else filter(filter, items(mapping)))
    try:
        while 1:
            _n(iterable)
...
xpartitems = partial(lambda x, p, c, i, f=None, *a, **k: x(p(c, *a, **k), i, f), exhaustmap, partial)