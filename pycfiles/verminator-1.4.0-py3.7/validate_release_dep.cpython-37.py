...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/verminator/validate_release_dep.py
...
class ReleaseInfo(object):
    _ReleaseInfo__instance_releases = dict()
    ...
    def add_release(cls, instance_name, release_desc, instance_version):
        ...
        if instance_name not in cls._ReleaseInfo__instance_releases:
            cls._ReleaseInfo__instance_releases[instance_name] = dict()
        elif release.release_version not in cls._ReleaseInfo__instance_releases[instance_name]:
            cls._ReleaseInfo__instance_releases[instance_name][release.release_version] = dict()
        else:
            raise ValueError('Duplicated release version {} for {}'.format(release.release_version, instance_name))
        cls._ReleaseInfo__instance_releases[instance_name][release.release_version] = release
        return release

    @classmethod
    def all_instance_releases(cls):
        return cls._ReleaseInfo__instance_releases
...
def validate_versioned_image(images, instance_name, instance_version):
    ...
    for rv in releases:
        ...
        assert found, 'Release version {} of {} {} not in a valid hot-fix range'.format(rv, instance_name, instance_version)
    ...
    for release_info in releases.values():
        for dep in release_info.dependencies:
            ...
            assert res <= 0, 'Invalid min-max range [min: {}, max: {}] for version {} of {} {}'.format(dep.min_version, dep.max_version, release_info.release_version, instance_name, instance_version)
    ...
    for instance_name, versions in all_instance_releases.items():
        for instance_version, release_info in versions.items():
            ...
            for dep in dependencies:
                ...
                assert dep_type in all_instance_releases, 'No dependence found {} for version {} of {}'.format(dep_type, instance_version, instance_name)
                found = _find_a_ranged_version(dep_type, minv=minv, maxv=maxv)
                assert found, 'No valid version defined for {} with max: {}, min: {}'.format(dep_type, maxv, minv)