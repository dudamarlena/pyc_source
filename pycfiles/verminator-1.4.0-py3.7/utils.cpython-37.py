...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/verminator/utils.py
...
FlexVersion.ordered_suffix = [
 'rc', 'final', None]
VersionMeta._v_regex = '(?P<prefix>.*\\-)?(?P<major>\\d+)(?P<minor>\\.\\d+)?(?P<maintenance>\\.\\d+)?(?P<build>\\.\\d+)?(?P<suffix_raw>\\-.*)?'
...
def parse_version(version, major_versioned=False):
    if version is None:
        return
    elif not isinstance(version, VersionMeta):
...
def is_major_version(version):
    version = parse_version(version)
    return version.maintenance is None and version.build is None and version.suffix is None and version.suffix_version is None
...
def concatenate_vranges(vranges, hard_merging=False):
    ...
    concatenated = [_concatenate_vranges_with_same_prefix(vranges, hard_merging) for vranges in prefixes.values()]
...
def _concatenate_vranges_with_same_prefix(vranges, hard_merging=False):
    ...
    :return: a list of concatenated version ranges, i.e. [(minv, maxv), ...]
    """
    sorted_vranges = sorted(vranges, key=(cmp_to_key(lambda x, y: FlexVersion.compares(x[0], y[0]))))
    res = [
     sorted_vranges[0]]
        ...
        if not hard_merging:
            if cmin.suffix is not None and pmin.suffix is not None:
                if cmin == pmax.add(VersionDelta(sver=1)):
                    ...
                    overlapped = pmin.in_range(cmin, cmax) or pmax.in_range(cmin, cmax) or cmin.in_range(pmin, pmax) or cmax.in_range(pmin, pmax)
                    if overlapped:
                        minv = cmin if cmin < pmin else pmin
                        maxv = cmax if cmax > pmax else pmax
                        res[-1] = (minv, maxv)
                        continue
                    elif pmax.substitute(cmin, ignore_suffix=True) == VersionDelta.zero:
                        if pmax.suffix == 'rc' and cmin.suffix == 'final':
                            res[-1] = (
                             pmin, cmax)
                            continue
                    delta = cmin.substitute(pmax, ignore_suffix=True)
                    if delta >= VersionDelta.zero:
                        if pmax.suffix == 'final':
                            res[-1] = (
                             pmin, cmax)
                            continue
                    res.append(vrange)
