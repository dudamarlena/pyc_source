...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/rdfextras/utils/termutils.py
# Compiled at: 2012-10-05 05:42:21
...
__all__ = [
 'SUBJECT', 'PREDICATE', 'OBJECT', 'CONTEXT', 'TERM_COMBINATIONS',
...
TERM_INSTANTIATION_DICT = {'U': URIRef, 
   'B': BNode, 
   'V': Variable, 
   'L': Literal}
GRAPH_TERM_DICT = {'F': (
...
def normalizeGraph(graph):
    ...
    if isinstance(graph, QuotedGraph):
        ...
        return (
         graph.identifier, term2Letter(graph.identifier))
...
def term2Letter(term):
    ...
    * :class:`~rdflib.graph.QuotedGraph`
    ...
    if isinstance(term, URIRef):
        ...
        if term is None:
            return 'L'
        raise Exception(('The given term (%s) is not an instance of any ' + 'of the known types (URIRef, BNode, Literal, QuotedGraph, ' + 'or Variable).  It is a %s') % (
         term, type(term)))
        return
...
def type2TermCombination(member, klass, context):
    ...
    try:
        ...
        raise Exception('Unable to persist' + 'classification triple: %s %s %s %s' % (
