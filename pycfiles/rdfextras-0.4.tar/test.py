...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/test/test_sparql/DAWG/test.py
# Compiled at: 2012-10-24 03:44:02
...
test = [
 ...
 'data/TypePromotion/tP-unsignedByte-short.rqdata/examples/ex11.2.3.1_0.rq',
...
tests2Skip = {'data-r2/bnode-coreference/query.rq': 'Query results must maintain bnode co-references in the dataset', 
...
graphtests = {'data-r2/construct/query-construct-optional.rq': 'data-r2/construct/result-construct-optional.ttl', 
...
manifestNS = {'rdfs': Namespace('http://www.w3.org/2000/01/rdf-schema#'), 
...
def generictest(testFile):
    func_name = __name__ = __doc__ = id = 'test_sparql.' + os.path.splitext(testFile)[0][8:].translate(maketrans('-/', '__'))
    ...
    if not len(store) == 0:
        ...
        assert os.path.exists(manifestPath) or os.path.exists(manifestPath2)
        manifestPath = manifestPath2
    manifestG.default_context.parse(open(manifestPath), publicID=TEST_BASE, format='n3')
    manifestData = manifestG.query(MANIFEST_QUERY, processor='sparql', initBindings={'query': TEST_BASE[queryFileName]}, initNs=manifestNS, DEBUG=False)
    store.rollback()
    store.close()
    for source, testCaseName, testCaseComment, expectedRT in manifestData:
        ...
        if expectedRT:
            ...
            if not trialAndErrorRTParse(resultG, expectedRT, DEBUG):
                log.debug('Unexpected result format (for %s), skipping' % expectedRT)
                store.rollback()
                store.close()
                continue
            log.debug('## Done .. ##')
            ...
            for solutionNode in resultG.objects(resultSetNode, RESULT_NS.solution):
                ...
                if len(rbinds) > 1 and (isinstance(rbinds, list) or isinstance(rbinds, tuple)):
                    bindings.append(frozenset(rbinds))
                elif len(rbinds) == 1 and (isinstance(rbinds, list) or isinstance(rbinds, tuple)):
        ...
        if EVALUATE and source:
            ...
            try:
                ...
                log.debug('Unexpected data format (for %s), skipping' % source)
                store.rollback()
                store.close()
                continue
            ...
            if expectedRT:
                ...
                if isinstance(result, Graph):
                    ...
                    assert set(bindings).difference(results) == set([]) or set(bindings) == results, '### Test Failed: ###\n\nB:\n%s\n\nR:\n%s\n\n' % (
...
def test_cases():
    ...
    for idx, testFile in enumerate(glob('data-r2/*/*.rq')):
        g = generictest
        g.__name__ = g.__doc__ = g.func_name = g.id = 'test.test_sparql.' + testFile[8:-3].translate(maketrans('-/', '__'))
        yield (g, testFile)
