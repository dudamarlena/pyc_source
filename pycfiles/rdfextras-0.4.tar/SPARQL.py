...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/rdfextras/store/SPARQL.py
# Compiled at: 2012-06-04 05:54:41
"""
...
"""
__version__ = '1.02'
__authors__ = 'Ivan Herman, Sergio Fernández, Carlos Tejo Alonso, Gunnar Aastrand Grimnes'
...
if getattr(sys, 'pypy_version_info', None) is not None or sys.platform.startswith('java') or sys.version_info[:2] < (2,
                                                                                                                     6):
...
def TraverseSPARQLResultDOM(doc, asDictionary=False):
    ...
    """
    vars = [ Variable(v.attrib['name']) for v in doc.findall('./{http://www.w3.org/2005/sparql-results#}head/{http://www.w3.org/2005/sparql-results#}variable')
           ]
...
def CastToTerm(node):
    ...
    if node.tag == '{%s}uri' % SPARQL_NS:
        return URIRef(node.text)
    if node.tag == '{%s}literal' % SPARQL_NS:
...
class SPARQLStore(SPARQLWrapper, Store):
    ...
    def query(self, graph, queryStringOrObj, initNs={}, initBindings={}, DEBUG=False):
        self.debug = DEBUG
...
class SPARQLUpdateStore(SPARQLStore):
    ...
    def __init__(self, queryEndpoint=None, updateEndpoint=None, bNodeAsURI=False):
        ...
        self.headers = {'Content-type': 'application/sparql-update', 'Connection': 'Keep-alive'}
