...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/test/test_sparql/test_parser.py
...
match_definitions = [
 ...
 (
  ...
  [
   (
    'PREFIX s: <s:ex>',
    components.PrefixDeclaration('s:', 's:ex'))]),
 (
  parser.Var,
  [
   (
 ...
 (
  ...
  [
   ...
   (
    "'bar'^^pre:type",
    components.ParsedDatatypedLiteral('bar', components.QName('pre:type'))),
   (
    "'bar'^^<s:type>",
    components.ParsedDatatypedLiteral('bar', URIRef('s:type')))]),
 (
  ...
  [
   ...
   (
    '+901', Literal(901)),
   (
    '-9.8e-2', Literal(-0.098, datatype=XSD.double))]),
 (
 ...
 (
  ...
  [
   (
    ...
    [
     components.PropertyValue(components.QName('foaf:nick'), [
...
def strict_Literal_eq(l0, l1):
    try:
        return l0._cmp_value == l1._cmp_value and type(l0._cmp_value) is type(l1._cmp_value) and l0.language == l1.language and l0.datatype == l1.datatype
    except:
        return False
...
def test_components_should_match():
    for def_index, definition in enumerate(match_definitions):
        ...
        for test_index, test in enumerate(tests):
            before, after = test
            should_match.description = should_match.__name__ + '.%d.%d' % (def_index, test_index)
