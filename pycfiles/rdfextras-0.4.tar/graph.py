...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/rdfextras/sparql/graph.py
...
__all__ = [
 'SPARQLGraph', 'GraphPattern', 'BasicGraphPattern']
...
class SPARQLGraph(object):
    ...
    SPARQL_DATASET = 0
    NAMED_GRAPH = 1
    __slots__ = ('graphVariable', 'DAWG_DATASET_COMPLIANCE', 'identifier', 'graphKind',
                 'graph')
...
class BasicGraphPattern(GraphPattern):
    ...
    def __repr__(self):
        if self.constraints:
            return 'Filter(.. a filter ..,BGP(%s))' % (',').join([ (',').join([self.canonicalTerm(pat[0]), self.canonicalTerm(pat[1]), self.canonicalTerm(pat[2])]) for pat in self.patterns
                                                                 ])
        else:
            return 'BGP(%s)' % (',').join([ '(' + (',').join([self.canonicalTerm(s), self.canonicalTerm(p), self.canonicalTerm(o)]) + ')' for s, p, o, f in self.patterns
                                          ])
