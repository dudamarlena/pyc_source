...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/rdfextras/sparql/results/xmlresults.py
# Compiled at: 2012-10-11 10:35:41
...
def parseTerm(element):
    ...
    if tag == RESULTS_NS_ET + 'literal':
        ...
        if tag == RESULTS_NS_ET + 'uri':
            return URIRef(text)
        if tag == RESULTS_NS_ET + 'bnode':
            return BNode(text)
...
class SPARQLXMLWriter:
    ...
    def __init__(self, output, encoding='utf-8'):
        ...
        writer.startPrefixMapping('sparql', SPARQL_XML_NAMESPACE)
    ...
    def write_header(self, allvarsL):
        ...
        for i in xrange(0, len(allvarsL)):
            attr_vals = {(None, 'name'): unicode(allvarsL[i])}
            attr_qnames = {(None, 'name'): 'name'}
            self.writer.startElementNS((
             SPARQL_XML_NAMESPACE, 'variable'), 'variable', AttributesNSImpl(attr_vals, attr_qnames))
    ...
    def write_ask(self, val):
        ...
        self.writer.endElementNS((
         SPARQL_XML_NAMESPACE, 'boolean'), 'boolean')
    ...
    def write_end_result(self):
        ...
        self.writer.endElementNS((
         SPARQL_XML_NAMESPACE, 'result'), 'result')
        self._resultStarted = False

    def write_binding(self, name, val):
        assert self._resultStarted
        attr_vals = {(None, 'name'): unicode(name)}
        attr_qnames = {(None, 'name'): 'name'}
        self.writer.startElementNS((
         SPARQL_XML_NAMESPACE, 'binding'), 'binding', AttributesNSImpl(attr_vals, attr_qnames))
        if isinstance(val, URIRef):
            ...
            self.writer.startElementNS((
             SPARQL_XML_NAMESPACE, 'literal'), 'literal', AttributesNSImpl(attr_vals, attr_qnames))
            self.writer.characters(val)
            self.writer.endElementNS((
    ...
    def close(self):
        if self._results:
            self.writer.endElementNS((
             SPARQL_XML_NAMESPACE, 'results'), 'results')
        self.writer.endElementNS((
         SPARQL_XML_NAMESPACE, 'sparql'), 'sparql')
