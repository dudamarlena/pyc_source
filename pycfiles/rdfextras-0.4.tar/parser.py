...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/rdfextras/sparql/parser.py
# Compiled at: 2012-04-24 09:28:08
import sys
from pyparsing import Regex, Suppress, Combine, Optional, CaselessKeyword, ZeroOrMore, OneOrMore, removeQuotes, quotedString, Empty, Literal, NoMatch, Group, oneOf, Forward, Keyword, ParseExpression, ParseElementEnhance, ParseException, col, lineno, restOfLine
import rdflib
...
if DEBUG:
    ...
    class ParseTracer(object):
        ...
        def start_action(self, instring, loc, expr):
            self.writer.startElement('attempt', attributes={'class': unicode(expr.__class__.__name__), 
               'loc': unicode(repr(loc)), 
               'expr': unicode(repr(expr)), 'lineno': unicode(lineno(loc, instring)), 
               'col': unicode(col(loc, instring))})
        ...
        def exception_action(self, instring, tokensStart, expr, err):
            self.writer.simpleElement('fail', attributes={'err': unicode(repr(err))})
            self.writer.endElement('attempt')
        ...
        def parse(self, parser, input, stream):
            ...
            try:
                ...
                self.writer.simpleElement('fail', attributes={'err': unicode(repr(e))})
    ...
    def data_dir(thing):
        return [ member for member in dir(thing) if not member.startswith('__') and not callable(getattr(thing, member))
               ]
        ...
        def __apply2(results):
            ...
            if projection is not None:
                if len(results) < len(projection):
                    raise ProjectionMismatchException('Expected at least %d results to make %s, got %d.' % (
...
COMMA = Suppress(',')
PERIOD = Suppress('.')
IRI = Regex('[^<>"{}|^`\\\\%s]*' % ('').join('\\x%02X' % i for i in range(33)))
IRI_REF = LT + IRI + GT
if DEBUG:
...
PN_PREFIX_re = PN_CHARS_BASE_re + '(?:(?:' + PN_CHARS_re + '|\\.)*' + '(?:' + PN_CHARS_re + '))?'
PN_PREFIX = Regex(PN_PREFIX_re)
PNAME_NS = Combine(Optional(PN_PREFIX, '') + COLON)
PN_LOCAL = Regex(regex_group(PN_CHARS_U_re + '|[0-9]') + regex_group(regex_group(PN_CHARS_re + '|\\.') + '*' + regex_group(PN_CHARS_re)) + '?')
PNAME_LN = Combine(PNAME_NS + PN_LOCAL)
WS_re = '[ \\t\\r\\n]*'
NIL = Group(Suppress(Regex('\\(' + WS_re + '\\)')))
BASE = Suppress(CaselessKeyword('BASE'))
BaseDecl = (BASE + IRI_REF).setParseAction(refer_component(components.BaseDeclaration))
if DEBUG:
    BaseDecl.setName('BaseDecl')
PREFIX = Suppress(CaselessKeyword('PREFIX'))
PrefixDecl = (PREFIX + PNAME_NS + IRI_REF).setParseAction(refer_component(components.PrefixDeclaration))
if DEBUG:
    PrefixDecl.setName('PrefixDecl')
Prologue = (Optional(BaseDecl, None) + Group(ZeroOrMore(PrefixDecl))).setParseAction(refer_component(components.Prolog))
if DEBUG:
    Prologue.setName('Prologue')
QM = Suppress('?')
USD = Suppress('$')
VARNAME = Regex(regex_group(PN_CHARS_U_re + '|[0-9]') + regex_group(PN_CHARS_U_re + '|[0-9]') + '*')
Var = ((QM | USD) + VARNAME).setParseAction(refer_component(rdflib.term.Variable))
if DEBUG:
    Var.setName('Var')
PrefixedName = PNAME_LN | PNAME_NS
if DEBUG:
    PrefixedName.setName('PrefixedName')
IRIref = IRI_REF.setParseAction(refer_component(components.IRIRef)) | PrefixedName.setParseAction(refer_component(components.QName))
if DEBUG:
    IRIref.setName('IRIref')
FROM = Suppress(CaselessKeyword('FROM'))
NAMED = Suppress(CaselessKeyword('NAMED'))
DatasetClause = FROM + (IRIref.copy().setParseAction(refer_component(components.RemoteGraph)) | NAMED + IRIref.copy().setParseAction(refer_component(components.NamedGraph)))
if DEBUG:
...
RDFLiteral = (String + DOUBLE_HAT + IRIref).setParseAction(refer_component(components.ParsedDatatypedLiteral)) | (String + Optional(LANGTAG, None)).setParseAction(refer_component(unescaped_literal))
if DEBUG:
...
INTEGER = Regex('[0-9]+').setParseAction(composition([
 refer_component(int), rdflib.term.Literal]))
DECIMAL_re = '[+-]?(?:[0-9]+\\.[0-9]*|\\.[0-9]+)'
FLOAT_re = '[+-]?(?:[0-9]+\\.[0-9]*%s|\\.[0-9]+%s|[0-9]+%s)' % ((
 EXPONENT_re,) * 3)
DECIMAL = Regex(DECIMAL_re).setParseAction(composition([
 refer_component(float),
 lambda x: rdflib.term.Literal(x, datatype=XSD.decimal)]))
FLOAT = Regex(FLOAT_re).setParseAction(composition([
 refer_component(float),
 lambda x: rdflib.term.Literal(x, datatype=XSD.double)]))
NumericLiteral = FLOAT | DECIMAL | INT
if DEBUG:
    NumericLiteral.setName('NumericLiteral')
BooleanLiteral = (Keyword('true') | Keyword('false')).setParseAction(refer_component(rdflib.term.Literal, datatype=XSD.boolean))
if DEBUG:
    BooleanLiteral.setName('BooleanLiteral')
ANON = Regex('\\[' + WS_re + '\\]').setParseAction(refer_component(rdflib.term.BNode, None, []))
BLANK_NODE_LABEL = (Suppress('_:') + PN_LOCAL).setParseAction(refer_component(rdflib.term.BNode))
...
RegexExpression = (REGEX + LP + Expression + COMMA + Expression + Optional(COMMA + Expression) + RP).setParseAction(refer_component(components.ParsedREGEXInvocation))
if DEBUG:
    RegexExpression.setName('RegexExpression')
BuiltInCall = (STR + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.STR])) | (LANG + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.LANG])) | (LANGMATCHES + LP + Expression + COMMA + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.LANGMATCHES])) | (DATATYPE + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.DATATYPE])) | (BOUND + LP + Var + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.BOUND])) | (sameTerm + LP + Expression + COMMA + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.sameTERM])) | (isIRI + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.isIRI])) | (isURI + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.isURI])) | (isBLANK + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.isBLANK])) | (isLITERAL + LP + Expression + RP).setParseAction(refer_component(components.BuiltinFunctionCall, [components.isLITERAL])) | RegexExpression
if DEBUG:
    BuiltInCall.setName('BuiltInCall')
ArgList = NIL | Group(LP + Expression + ZeroOrMore(COMMA + Expression) + RP)
if DEBUG:
    ArgList.setName('ArgList')
FunctionCall = (IRIref + ArgList).setParseAction(refer_component(components.FunctionCall))
if DEBUG:
    FunctionCall.setName('FunctionCall')
BrackettedExpression = LP + Expression + RP
if DEBUG:
    BrackettedExpression.setName('BrackettedExpression')
PrimaryExpression = BrackettedExpression | BuiltInCall | FunctionCall | IRIref | RDFLiteral | NumericLiteral | BooleanLiteral | Var
if DEBUG:
    PrimaryExpression.setName('PrimaryExpression')
UnaryExpression = (Suppress('!') + PrimaryExpression).setParseAction(refer_component(components.LogicalNegation)) | (Suppress('+') + PrimaryExpression).setParseAction(refer_component(components.NumericPositive)) | (Suppress('-') + PrimaryExpression).setParseAction(refer_component(components.NumericNegative)) | PrimaryExpression
if DEBUG:
    UnaryExpression.setName('UnaryExpression')
MultiplicativeExpression = Group(UnaryExpression + ZeroOrMore((Literal('*') | Literal('/')) + UnaryExpression)).setParseAction(refer_component(components.ParsedMultiplicativeExpressionList))
if DEBUG:
    MultiplicativeExpression.setName('MultiplicativeExpression')
AdditiveExpression = Group(MultiplicativeExpression + ZeroOrMore((Literal('+') | Literal('-')) + MultiplicativeExpression)).setParseAction(refer_component(components.ParsedAdditiveExpressionList))
if DEBUG:
    AdditiveExpression.setName('AdditiveExpression')
NumericExpression = AdditiveExpression
RelationalExpression = (NumericExpression + Suppress('=') + NumericExpression).setParseAction(refer_component(components.EqualityOperator)) | (NumericExpression + Suppress('!=') + NumericExpression).setParseAction(refer_component(components.NotEqualOperator)) | (NumericExpression + Suppress('<') + NumericExpression).setParseAction(refer_component(components.LessThanOperator)) | (NumericExpression + Suppress('>') + NumericExpression).setParseAction(refer_component(components.GreaterThanOperator)) | (NumericExpression + Suppress('<=') + NumericExpression).setParseAction(refer_component(components.LessThanOrEqualOperator)) | (NumericExpression + Suppress('>=') + NumericExpression).setParseAction(refer_component(components.GreaterThanOrEqualOperator)) | NumericExpression
if DEBUG:
    RelationalExpression.setName('RelationalExpression')
ValueLogical = RelationalExpression
ConditionalAndExpression = Group(ValueLogical + ZeroOrMore(Suppress('&&') + ValueLogical)).setParseAction(refer_component(components.ParsedRelationalExpressionList))
if DEBUG:
    ConditionalAndExpression.setName('ConditionalAndExpression')
ConditionalOrExpression = Group(ConditionalAndExpression + ZeroOrMore(Suppress('||') + ConditionalAndExpression)).setParseAction(refer_component(components.ParsedConditionalAndExpressionList))
if DEBUG:
    ConditionalOrExpression.setName('ConditionalOrExpression')
Expression << ConditionalOrExpression
Constraint = BrackettedExpression.copy().setParseAction(refer_component(components.ParsedExpressionFilter)) | (BuiltInCall | FunctionCall).setParseAction(refer_component(components.ParsedFunctionFilter))
if DEBUG:
...
Collection = (LP + Group(OneOrMore(GraphNode)) + RP).setParseAction(refer_component(components.ParsedCollection))
if DEBUG:
    Collection.setName('Collection')
ObjectList = Group(GraphNode + ZeroOrMore(COMMA + GraphNode))
if DEBUG:
    ObjectList.setName('ObjectList')
PropertyListItem = (Verb + ObjectList).setParseAction(refer_component(components.PropertyValue))
if DEBUG:
    PropertyListItem.setName('PropertyListItem')
PropertyListNotEmpty = Group(PropertyListItem + ZeroOrMore(SEMICOLON + Optional(PropertyListItem)))
if DEBUG:
    PropertyListNotEmpty.setName('PropertyListNotEmpty')
TriplesNode = Collection | (LB + PropertyListNotEmpty + RB).setParseAction(refer_component(components.Resource, [None]))
if DEBUG:
    TriplesNode.setName('TriplesNode')
GraphNode << (VarOrTerm | TriplesNode)
TriplesSameSubject = (VarOrTerm + PropertyListNotEmpty).setParseAction(refer_component(components.Resource)) | (LB + PropertyListNotEmpty + RB + Optional(PropertyListNotEmpty, [])).setParseAction(refer_component(components.TwiceReferencedBlankNode)) | (Collection + Optional(PropertyListNotEmpty, [])).setParseAction(setPropertyValueList)
if DEBUG:
    TriplesSameSubject.setName('TriplesSameSubject')
TriplesBlock = Forward()
TriplesBlock << TriplesSameSubject + Optional(PERIOD + Optional(TriplesBlock))
if DEBUG:
    TriplesBlock.setName('TriplesBlock')
GroupGraphPattern = Forward()
OPTIONAL = Suppress(CaselessKeyword('OPTIONAL'))
OptionalGraphPattern = (OPTIONAL + GroupGraphPattern).setParseAction(refer_component(components.ParsedOptionalGraphPattern))
if DEBUG:
    OptionalGraphPattern.setName('OptionalGraphPattern')
UNION = Suppress(CaselessKeyword('UNION'))
UnionGraphPattern = Group(GroupGraphPattern + OneOrMore(UNION + GroupGraphPattern)).setParseAction(refer_component(components.ParsedAlternativeGraphPattern))
if DEBUG:
    UnionGraphPattern.setName('UnionGraphPattern')
GRAPH = Suppress(CaselessKeyword('GRAPH'))
GraphGraphPattern = (GRAPH + VarOrIRIref + GroupGraphPattern).setParseAction(refer_component(components.ParsedGraphGraphPattern))
if DEBUG:
    GraphGraphPattern.setName('GraphGraphPattern')
GraphPatternNotTriples = OptionalGraphPattern | UnionGraphPattern | GraphGraphPattern | GroupGraphPattern
if DEBUG:
    GraphPatternNotTriples.setName('GraphPatternNotTriples')
GraphPattern = (Filter + Optional(PERIOD) + Optional(Group(TriplesBlock))).setParseAction(refer_component(components.GraphPattern, [None])) | (GraphPatternNotTriples + Optional(PERIOD) + Optional(Group(TriplesBlock), None)).setParseAction(refer_component(components.GraphPattern, [None], [1, 0, 2]))
if DEBUG:
    GraphPattern.setName('GraphPattern')
GroupGraphPattern << (LC + Optional(Group(TriplesBlock), None) + Group(ZeroOrMore(GraphPattern)) + RC).setParseAction(refer_component(components.ParsedGroupGraphPattern))
if DEBUG:
    GroupGraphPattern.setName('GroupGraphPattern')
WHERE = Suppress(Optional(CaselessKeyword('WHERE')))
WhereClause = (WHERE + Optional(GroupGraphPattern, None)).setParseAction(refer_component(components.WhereClause))
if DEBUG:
    WhereClause.setName('WhereClause')
RECUR = Suppress(CaselessKeyword('RECUR'))
TO = Suppress(CaselessKeyword('TO'))
RecurClause = (RECUR + Group(OneOrMore(Group(Var + TO + Var))) + Optional(GroupGraphPattern, None)).setParseAction(refer_component(components.RecurClause))
if DEBUG:
    RecurClause.setName('RecurClause')
ASC = Suppress(Optional(CaselessKeyword('ASC')))
DESC = Suppress(Optional(CaselessKeyword('DESC')))
OrderCondition = (ASC + BrackettedExpression).setParseAction(refer_component(components.ParsedOrderConditionExpression, [components.ASCENDING_ORDER], [1, 0])) | (DESC + BrackettedExpression).setParseAction(refer_component(components.ParsedOrderConditionExpression, [components.DESCENDING_ORDER], [1, 0])) | BrackettedExpression.copy().setParseAction(refer_component(components.ParsedOrderConditionExpression, [components.UNSPECIFIED_ORDER], [1, 0])) | BuiltInCall | FunctionCall | Var
if DEBUG:
...
SelectQuery = (SELECT + DISTINCT + (Group(OneOrMore(Var)) | Literal('*').setParseAction(as_empty)) + Group(ZeroOrMore(DatasetClause)) + WhereClause + Optional(RecurClause, None) + SolutionModifier).setParseAction(refer_component(components.SelectQuery, projection=[
 1, 2, 3, 4, 5, 0]))
if DEBUG:
...
ConstructQuery = (CONSTRUCT + ConstructTemplate + Group(ZeroOrMore(DatasetClause)) + WhereClause + SolutionModifier).setParseAction(refer_component(components.ConstructQuery))
if DEBUG:
    ConstructQuery.setName('ConstructQuery')
DESCRIBE = Suppress(CaselessKeyword('DESCRIBE'))
DescribeQuery = (DESCRIBE + (Group(OneOrMore(VarOrIRIref)) | Literal('*').setParseAction(as_empty)) + Group(ZeroOrMore(DatasetClause)) + Optional(WhereClause, None) + SolutionModifier).setParseAction(refer_component(components.DescribeQuery))
if DEBUG:
    DescribeQuery.setName('DescribeQuery')
ASK = Suppress(CaselessKeyword('ASK'))
AskQuery = (ASK + Group(ZeroOrMore(DatasetClause)) + WhereClause).setParseAction(refer_component(components.AskQuery))
if DEBUG:
    AskQuery.setName('AskQuery')
Query = (Prologue + (SelectQuery | ConstructQuery | DescribeQuery | AskQuery)).setParseAction(refer_component(components.Query))
