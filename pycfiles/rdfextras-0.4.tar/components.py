...
# [GCC 8.4.0]
# Embedded file name: /home/grimnes/projects/rdflib/rdfextras/rdfextras/sparql/components.py
...
class PrefixDeclaration(object):
    ...
    def __eq__(self, other):
        return self.namespaceMapping == other.namespaceMapping and self.qName == other.qName
...
class FunctionCall(object):
    ...
    def __repr__(self):
        return '%s(%s)' % (
         self.name,
         (',').join([ isinstance(i, ListRedirect) and i.reduce() or i for i in self.arguments
           ]))
...
class BuiltinFunctionCall(FunctionCall):
    ...
    def __repr__(self):
        return '%s(%s)' % (
         ...
         (',').join([ isinstance(i, ListRedirect) and str(i.reduce()) or repr(i) for i in self.arguments
           ]))
...
class ParsedGroupGraphPattern(object):
    ...
    def __len__(self):
        return len([ g for g in self.graphPatterns if isinstance(g, GraphPattern) and (g.triples or g.nonTripleGraphPattern) is not None
                   ])
    ...
    * Patterns on Named Graphs, where patterns are matched against named graphs
    ...
    def __repr__(self):
        if not self.triples and self.nonTripleGraphPattern is None:
            ...
            if self.triples and not self.nonTripleGraphPattern and not self.filter:
...
class SelectQuery(object):
    ...
    def __init__(self, variables, dataSetList, whereClause, recurClause, solutionModifier, distinct=None):
...
class Resource(RDFTerm):
    ...
    def extractPatterns(self):
        for prop, objs in self.propVals:
            for obj in objs:
                yield (
                 self.identifier, prop, obj)

    def __eq__(self, other):
        return self.identifier == other.identifier and self.propVals == other.propVals
...
DESCENDING_ORDER = 2
UNSPECIFIED_ORDER = 3
ORDER_VALUE_MAPPING = {ASCENDING_ORDER: 'Ascending', 
...
class PropertyValue(object):
    ...
    def __eq__(self, other):
        return self.property == other.property and self.objects == other.objects
