...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\odddML\resnet\resnet_layers.py
# Compiled at: 2019-12-20 04:29:12
...
class ResidualBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filter_num, stride=1):
        ...
        self.downsample.add(tf.keras.layers.Conv2D(filters=filter_num, kernel_size=(1,
                                                                                    1), strides=stride))
...
class ConvoBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filter_num, stride=1):
        ...
        self.conv2 = tf.keras.layers.Conv2D(filters=filter_num, kernel_size=(3, 3), strides=stride, padding='same')
        self.bn2 = tf.keras.layers.BatchNormalization()
        self.conv3 = tf.keras.layers.Conv2D(filters=(filter_num * 4), kernel_size=(1,
                                                                                   1), strides=1, padding='same')
        self.bn3 = tf.keras.layers.BatchNormalization()
        self.downsample = tf.keras.Sequential()
        self.downsample.add(tf.keras.layers.Conv2D(filters=(filter_num * 4), kernel_size=(1,
                                                                                          1), strides=stride))
        self.downsample.add(tf.keras.layers.BatchNormalization())
