...
# [GCC 8.4.0]
# Embedded file name: /home/c24b/projets/crawtext/newspaper/cleaners.py
...
class DocumentCleaner(object):
    ...
    def __init__(self, config):
        ...
        self.remove_nodes_re = '^side$|combx|retweet|mediaarticlerelated|menucontainer|navbar|storytopbar-bucket|utility-bar|inline-share-tools|comment|PopularQuestions|contact|foot|footer|Footer|footnote|cnn_strycaptiontxt|cnn_html_slideshow|cnn_strylftcntnt|links|meta$|shoutbox|sponsor|tags|socialnetworking|socialNetworking|cnnStryHghLght|cnn_stryspcvbx|^inset$|pagetools|post-attributes|welcome_form|contentTools2|the_answers|communitypromo|runaroundLeft|subscribe|vcard|articleheadings|date|^print$|popup|author-dropdown|tools|socialtools|byline|konafilter|KonaFilter|breadcrumbs|^fn$|wp-caption-text|legende|ajoutVideo|timestamp|js_replies'
        ...
        self.div_to_p_re = '<(a|blockquote|dl|div|img|ol|p|pre|table|ul)'
    ...
    def replace_walk_left_right(self, kid, kid_text, replacement_text, nodes_to_remove):
        ...
        if len(replace_text) > 1:
            ...
            while prev_node is not None and self.parser.getTag(prev_node) == 'a' and self.parser.getAttribute(prev_node, 'grv-usedalready') != 'yes':
            ...
            while next_node is not None and self.parser.getTag(next_node) == 'a' and self.parser.getAttribute(next_node, 'grv-usedalready') != 'yes':
    ...
    def get_replacement_nodes(self, doc, div):
        ...
        for kid in kids:
            if self.parser.getTag(kid) == 'p' and len(replacement_text) > 0:
                ...
                self.replace_walk_left_right(kid, kid_text, replacement_text, nodes_to_remove)
            else:
                nodes_to_return.append(kid)
