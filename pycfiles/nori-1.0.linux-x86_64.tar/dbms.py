...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nori/dbms/dbms.py
# Compiled at: 2013-12-09 18:04:28
"""
...
import sys, getpass, atexit
from .. import core
from ..ssh import SSH
core.exitvals['dbms_connect'] = dict(num=30, descr='\nError connecting to or disconnecting from a database;\nalso used for cursors.\n')
core.exitvals['dbms_execute'] = dict(num=31, descr='\nError executing a database query/command/function.\n')
core.supported_features['dbms'] = 'generic database support; subfeatures for each DBMS also required'
...
class DBMS(object):
    ...
    def __init__(self, prefix, delim='_', err_use_logger=True, err_warn_only=False, err_no_exit=False, warn_use_logger=True, warn_warn_only=True, warn_no_exit=False):
        """
        Populate instance variables.
    ...
    def create_settings(self, heading='', extra_text='', ignore=None, extra_requires=[], tunnel=True if 'ssh' in core.available_features else False):
        ...
        if tunnel:
            core.config_settings[pd + 'use_ssh_tunnel'] = dict(descr=('\nUse an SSH tunnel for the {0} connection (True/False)?\n\nIf True, specify the host in {1}ssh_host and the port in\n{1}remote_port instead of {1}host and\n{1}port.\n').format(self.DBMS_NAME, pd), default=False, cl_coercer=core.str_to_bool, requires=[
             'ssh'])
            ssh_extra_text = ("Ignored if cfg['{0}'] is False.").format(pd + 'use_ssh_tunnel')
            if extra_text:
                ssh_extra_text += '\n\n' + extra_text
            self.ssh = SSH(self._prefix, self._delim)
            self.ssh.create_settings(extra_text=ssh_extra_text, ignore=self._ignore_ssh_settings, extra_requires=self.REQUIRES + extra_requires, tunnel=True, default_local_port=self.DEFAULT_LOCAL_PORT, default_remote_port=self.DEFAULT_REMOTE_PORT)
        core.config_settings[pd + 'protocol'] = dict(descr=("\nProtocol to use for the {0} connection.\n\nCan be:\n    * 'tcp': use {1}host/port\n    * 'socket': use {1}socket_file\n").format(self.DBMS_NAME, pd) + (('\nIgnored if {0}use_ssh_tunnel is True.').format(pd) if tunnel else ''), default='tcp', cl_coercer=str)
        core.config_settings[pd + 'host'] = dict(descr=('\nRemote hostname for the {0} connection.\n').format(self.DBMS_NAME, pd) + (("\nIgnored if {0}use_ssh_tunnel is True or if\n{0}protocol is not 'tcp'.\n").format(pd) if tunnel else ("\nIgnored if {0}protocol is not 'tcp'.\n").format(pd)), default='localhost', cl_coercer=str)
        core.config_settings[pd + 'port'] = dict(descr=('\nRemote port number for the {0} connection.\n').format(self.DBMS_NAME, pd) + (("\nIgnored if {0}use_ssh_tunnel is True or if\n{0}protocol is not 'tcp'.\n").format(pd) if tunnel else ("\nIgnored if {0}protocol is not 'tcp'.\n").format(pd)), cl_coercer=int)
        core.config_settings[pd + 'socket_file'] = dict(descr=('\nPath to the socket file for the {0} connection.\n').format(self.DBMS_NAME, pd) + (("\nIgnored if {0}use_ssh_tunnel is True or if\n{0}protocol is not 'socket'.\n").format(pd) if tunnel else ("\nIgnored if {0}protocol is not 'socket'.\n").format(pd)), cl_coercer=str)
        core.config_settings[pd + 'user'] = dict(descr=('\nUsername for the {0} connection.\n').format(self.DBMS_NAME), cl_coercer=str)
        try:
        ...
        for s_name in setting_list:
            if 'requires' in core.config_settings[(pd + s_name)]:
                core.config_settings[(pd + s_name)]['requires'] += self.REQUIRES + extra_requires
            else:
                core.config_settings[(pd + s_name)]['requires'] = self.REQUIRES + extra_requires
    ...
    def validate_config(self):
        ...
        if not self._tunnel_config or not core.cfg[(pd + 'use_ssh_tunnel')]:
            if pd + 'protocol' not in core.config_settings or pd + 'protocol' in core.cfg and core.cfg[(pd + 'protocol')] == 'tcp':
                if pd + 'host' in core.cfg:
                    core.setting_check_not_blank(pd + 'host')
                if pd + 'port' in core.cfg:
                    core.setting_check_num(pd + 'port', 1, 65535)
            if pd + 'protocol' not in core.config_settings or pd + 'protocol' in core.cfg and core.cfg[(pd + 'protocol')] == 'socket':
    ...
    def populate_conn_args(self):
        ...
        if self._tunnel_config and core.cfg[(pd + 'use_ssh_tunnel')]:
            ...
            if pd + 'protocol' not in core.config_settings or pd + 'protocol' in core.cfg and core.cfg[(pd + 'protocol')] == 'socket':
                if pd + 'socket_file' in core.cfg:
                    self._conn_args['unix_socket'] = core.fix_path(core.cfg[(pd + 'socket_file')])
        if pd + 'user' in core.cfg:
    ...
    def read_password_file(self):
        ...
        try:
            ...
            core.file_error_handler(e, 'read', 'password file', cfg[(pd + 'pw_file')], must_exist=True, use_logger=True, warn_only=False, exit_val=core.exitvals['startup']['num'])
    ...
    def error_handler(self, e, err_verb, warn_verb, exit_val):
        ...
        if warn_only:
            msg = ('problem {0} {1} DBMS (config prefix/delim {2})').format(warn_verb, self.DBMS_NAME, core.pps(pd))
        else:
            msg = ('could not {0} {1} DBMS (config prefix/delim {2})').format(err_verb, self.DBMS_NAME, core.pps(pd))
        return core.generic_error_handler(e, msg, self.render_exception, use_logger, warn_only, exit_val)
    ...
    def check_supports_method(cls, method_name):
        ...
        if not cls.supports(method_name):
            core.email_logger.error(("Internal Error: {0}() was called on a {1} object,\nwhich doesn't support it; exiting.").format(method_name, cls.DBMS_NAME))
            sys.exit(core.exitvals['internal']['num'])

    def wrap_call(self, func, err_verb='call function on', warn_verb='calling function on', *args, **kwargs):
        """
        Wrap a DBMS function call in error handling.
        ...
        """
        core.status_logger.debug(('Calling DBMS function {0} with args:\n{1}\nand kwargs:\n{2}').format(func, args, kwargs))
        err = False
        ret = None
        try:
            ret = func(*args, **kwargs)
        except (self.MODULE.Warning, self.MODULE.Error) as e:
            self.error_handler(e, err_verb, warn_verb, core.exitvals['dbms_execute']['num'])
    ...
    def connect(self):
        ...
        if self._tunnel_config and core.cfg[(pd + 'use_ssh_tunnel')]:
            if self.err_no_exit:
                self.ssh.open_tunnel(self.DBMS_NAME + ' connection', True, self.err_use_logger, self.err_warn_only, None)
            else:
                self.ssh.open_tunnel(self.DBMS_NAME + ' connection', True, self.err_use_logger, self.err_warn_only)
        core.status_logger.info(('Connecting to {0} DBMS (config prefix/delim {1})...').format(self.DBMS_NAME, core.pps(pd)))
        try:
            self.conn = self.MODULE.connect(**self._conn_args)
        except (self.MODULE.Warning, self.MODULE.Error) as e:
            self.error_handler(e, 'connect to', 'connecting to', core.exitvals['dbms_connect']['num'])
            if isinstance(e, self.MODULE.Error):
                self.conn = None
                if self._tunnel_config and core.cfg[(pd + 'use_ssh_tunnel')]:
        ...
        core.status_logger.info(('{0} connection established.').format(self.DBMS_NAME))
        return True
    ...
    def close(self, force_no_exit=True):
        ...
        if self.conn is None:
            core.status_logger.info(('{0} connection (config prefix/delim {1})\nwas already closed.').format(self.DBMS_NAME, core.pps(pd)))
            return True
        else:
            ...
            try:
                ...
                self.error_handler(e, 'close connection to', 'closing connection to', core.exitvals['dbms_connect']['num'])
                if force_no_exit:
            ...
            self.conn = None
            if not err:
                core.status_logger.info(('{0} connection (config prefix/delim {1})\nhas been closed.').format(self.DBMS_NAME, core.pps(pd)))
    ...
    def cursor(self, main=True):
        ...
        try:
            ...
            self.error_handler(e, 'get cursor for', 'getting cursor for', core.exitvals['dbms_connect']['num'])
            if isinstance(e, self.MODULE.Error):
                cur = None
        ...
        if cur:
            if main:
                if (
                 self, None) not in DBMS._open_cursors:
                    DBMS._open_cursors.append((self, None))
            elif (
             self, cur) not in DBMS._open_cursors:
            ...
            core.status_logger.debug(('Got {0}{1} cursor.').format('main ' if main else '', self.DBMS_NAME))
        return cur
    ...
    def close_cursor(self, cur=None, force_no_exit=True):
        ...
        if cur is None and self.cur is None:
            core.status_logger.debug(('Main {0} cursor (config prefix/delim {1})\nwas already closed.').format(self.DBMS_NAME, core.pps(pd)))
            return True
        else:
            ...
            try:
                ...
                if force_no_exit:
                    ...
                    self.err_no_exit = True
                    self.warn_no_exit = True
                self.error_handler(e, ('close {0}cursor for').format(main_str), ('closing {0}cursor for').format(main_str), core.exitvals['dbms_connect']['num'])
                if force_no_exit:
                    self.restore_err_warn()
                if isinstance(e, self.MODULE.Error) and not self.err_warn_only:
                    err = True
            ...
            if cur is None:
                self.cur = None
            if not err:
                core.status_logger.debug(('{0}{1} cursor (config prefix/delim {2})\nhas been closed.').format(main_str.capitalize(), self.DBMS_NAME, core.pps(pd)))
            if cur is None:
                if (
                 self, None) in DBMS._open_cursors:
                    DBMS._open_cursors.remove((self, None))
            elif (
             self, cur) in DBMS._open_cursors:
    ...
    def callproc(self, cur, procname, param=None, has_results=False):
        ...
        if param is None:
            ret = self.wrap_call(cur.callproc, 'call stored procedure on', 'calling stored procedure on', procname)[0]
        else:
            ret = self.wrap_call(cur.callproc, 'call stored procedure on', 'calling stored procedure on', procname, param)[0]
        if not has_results:
    ...
    def execute(self, cur, query, param=None, has_results=False):
        ...
        cur = cur if cur else self.cur
        if param is None:
            ret = self.wrap_call(cur.execute, 'execute query on', 'executing query on', query)[0]
        else:
            ret = self.wrap_call(cur.execute, 'execute query on', 'executing query on', query, param)[0]
        if not has_results:
            self.auto_close_cursor()
            if self._fake_autocommit:
                if not self.commit():
    ...
    def executemany(self, cur, query, param_seq, has_results=False):
        ...
        self.auto_cursor()
        cur = cur if cur else self.cur
        ret = self.wrap_call(cur.executemany, 'execute queries on', 'executing queries on', query, param_seq)[0]
        if not has_results:
            self.auto_close_cursor()
            if self._fake_autocommit:
                if not self.commit():
    ...
    def fetchone(self, cur):
        ...
        cur = cur if cur else self.cur
        ret = self.wrap_call(cur.fetchone, 'retrieve data from', 'retrieving data from')
        if ret[1] is None:
            self.auto_close_cursor()
            if self._fake_autocommit:
                if not self.commit():
    ...
    def fetchmany(self, cur, size=None):
        ...
        cur = cur if cur else self.cur
        if size is None:
            ret = self.wrap_call(cur.fetchmany, 'retrieve data from', 'retrieving data from')
        else:
            ret = self.wrap_call(cur.fetchmany, 'retrieve data from', 'retrieving data from', size)
        if not ret[1]:
            self.auto_close_cursor()
            if self._fake_autocommit:
                if not self.commit():
    ...
    def fetchall(self, cur):
        ...
        cur = cur if cur else self.cur
        ret = self.wrap_call(cur.fetchall, 'retrieve data from', 'retrieving data from')
        self.auto_close_cursor()
        if self._fake_autocommit:
            if not self.commit():
                ret[0] = False
    ...
    def nextset(self, cur):
        ...
        cur = cur if cur else self.cur
        ret = self.wrap_call(cur.nextset, 'skip to the next set on', 'skipping to the next set on')
        if ret[1] is None:
            return
        else:
            return ret[0]
    ...
    def setinputsizes(self, cur, sizes):
        ...
        cur = cur if cur else self.cur
        return self.wrap_call(cur.setinputsizes, 'set input sizes on', 'setting input sizes on', sizes)[0]
    ...
    def setoutputsize(self, cur, size, column=None):
        ...
        cur = cur if cur else self.cur
        if column is None:
            return self.wrap_call(cur.setoutputsize, 'set output size on', 'setting output size on', size)[0]
        else:
            return self.wrap_call(cur.setoutputsize, 'set output size on', 'setting output size on', size, column)[0]
            return
    ...
    def commit(self):
        ...
        return self.wrap_call(self.conn.commit, 'commit transaction on', 'committing transaction on')[0]
    ...
    def rollback(self):
        ...
        return self.wrap_call(self.conn.rollback, 'roll back transaction on', 'rolling back transaction on')[0]
