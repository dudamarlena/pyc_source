...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/sphinx-prompt/__init__.py
# Compiled at: 2020-05-11 07:59:12
        ...
        if styles:
            ...
            if language == 'bash':
                ...
                if 'auto' in modifiers:
                    ...
                    if language in ('bash', 'python'):
                        for line in self.content:
                            statement.append(line)
                            if len(line) == 0 or not line[(-1)] == '\\':
                                html += '<span class="{0!s}">{1!s}</span>\n'.format(cache.get_prompt_class(prompt), highlight('\n'.join(statement), Lexer(), HtmlFormatter(nowrap=True)).strip('\r\n'))
                                if prompt is not None:
                        ...
                        for line in self.content:
                            html += '<span class="{0!s}">{1!s}</span>\n'.format(cache.get_prompt_class(prompt), highlight(line, Lexer(), HtmlFormatter(nowrap=True)).strip('\r\n'))
                            if prompt is not None:
