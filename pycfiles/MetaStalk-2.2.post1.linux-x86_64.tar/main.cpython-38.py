...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.8/site-packages/MetaStalk/main.py
# Compiled at: 2020-05-08 18:09:31
# Size of source mod 2**32: 5220 bytes
...
import os, logging, timeit, exifread
...
class MetaStalk:
    ...
    def run(self, args):
        ...
        self.plots = {'Stats':modules.stats(self.valid, self.invalid), 
    ...
    def file_search(self, parse_file: str):
        ...
        """
        if parse_file.lower().endswith(('heic', 'heif')):
            if self.heic:
...
def start():
    ...
    """
    parser = argparse.ArgumentParser(prog='MetaStalk', description='Tool to graph image metadata.')
    parser.add_argument('files', nargs='*', default=None, help='Path of photos to check.')
    parser.add_argument('-a', '--alphabetic', help='Sorts charts in alphabetical order rather than the default order', default=False,
      action='store_true')
    parser.add_argument('-d', '--debug', help='Sets logging level to DEBUG.', action='store_const',
      dest='loglevel',
      const=(logging.DEBUG),
    ...
    parser.add_argument('--no-open', help='Will only start the server and not open the browser to view it', default=False,
      action='store_true')
    parser.add_argument('-o', '--output', default='metastalk_exports', help='The name of the directory to output exports to. Will be created if it does not exist. Defaults to metastalk_exports.')
    parser.add_argument('-t', '--test', default=False, action='store_true', help='Does not show the graphs at the end.')
    parser.add_argument('-v', '--verbose', help='Sets logging level to INFO', action='store_const',
      dest='loglevel',
      const=(logging.INFO))
