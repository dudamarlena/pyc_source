...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.8/site-packages/MetaStalk/utils/export.py
...
class export:
    ...
    def __init__(self, choice: str, output_dir: str, plots: dict):
        ...
        if choice in ('html', 'html_offline'):
            ...
            if choice in ('pdf', 'svg', 'webp', 'jpeg', 'png'):
                self.image_export()
    ...
    def html_export(self):
        ...
        if self.choice == 'html_offline':
            ...
            offline = 'cdn'
        for name, chart in self.plots.items():
            chart.write_html(f"{self.output_dir}/{name}.html", include_plotlyjs=offline)