...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sunflower/hmm.py
# Compiled at: 2009-01-12 20:20:50
...
def load(filename, resource=False):
    if resource:
        species_info_resource = resource_stream('sunflower.data', SPECIES_RESOURCENAME)
...
def _make_connections(a, num_states, all_states):
    ...
    return [ _arrayize_connection_list(connection_list, num_states) for connection_list in [
     connection_list_f, connection_list_b]
           ]
...
def optimize(model):
    ...
    model.update(all_states=all_states, emitting_states=emitting_states, num_states=num_states, connections_f=connections[0], connections_b=connections[1])
