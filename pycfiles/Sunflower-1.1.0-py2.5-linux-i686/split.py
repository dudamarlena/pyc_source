...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sunflower/split.py
...
def split_file(inwalker, outfiles, measurements):
    for src_node in inwalker:
        ...
        for (outfile_index, outfile) in enumerate(outfiles):
            ...
            dst_node = outfile.createArray(group_pathname, name, dst_array, title, byteorder, createparents=True)
...
def split(infilename, out_template, inroot_pathname='/'):
    ...
    with h5open(infilename) as (infile):
        ...
        outfile_managers = (h5open(out_template % measurement, 'a') for measurement in measurements)
...
def parse_options(args):
    ...
    parser.add_option('-n', '--node', metavar='NODE', help='only produce output for children of NODE')
