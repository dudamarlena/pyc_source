...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sunflower/_optparse.py
# Compiled at: 2009-01-12 20:14:59
from __future__ import division
__version__ = '$Revision: 275 $'
from collections import defaultdict
from optparse import Option as _Option, OptionError, OptionGroup as _OptionGroup, OptionParser as _OptionParser, OptionValueError
import sys
...
class Option(_Option):
    ...
    ACTIONS = _Option.ACTIONS + ('load', 'update', 'update_const', 'update_true', 'update_false')
    STORE_ACTIONS = _Option.STORE_ACTIONS + ('load', 'update', 'update_const', 'update_true',
                                             'update_false', 'callback')
    ...
    def take_action(self, action, dest, opt, value, values, parser):
        if action == 'load':
            infile = open(value)
            add_values = (self.convert_value(opt, line.rstrip()) for line in infile)
...
class OptionParser(_OptionParser):
    ...
    def __init__(self, usage=None, option_list=None, option_class=Option, *args, **kwargs):
        _OptionParser.__init__(self, usage, option_list, option_class, *args, **kwargs)
