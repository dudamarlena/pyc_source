...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sunflower/report.py
...
import sys
from numpy import NINF, PINF, array, errstate, finfo, histogram, linspace, log1p, minimum, maximum, ndindex, newaxis, rollaxis, square, sqrt as asqrt, zeros
import tabdelim
from ._utils import correct_range, enum, progress
from .hdf5 import h5open, walk_included_nodes, walk_nodes
from .factorizers import AlignedOnlyAlignmentFactorizer, ByNucleotideAlignmentFactorizer, ByPosAlignmentFactorizer, LABEL_ALIGNED_ONLY_ALIGNED, LABEL_ALIGNED_ONLY_OBS, SubsOnlyAlignmentFactorizer, choose_factorizer, fmt_float, calc_total_datacol
FIELDNAMES = [
 'seqname', 'factor', 'datacol', 'pos', 'measurement',
...
def distance_jc69(p):
    ...
    return -0.75 * log1p(-1.3333333333333333 * p)
...
def histogram_arrays(factorized_arrays, ranges, total_datacol, num_bins, bin_range, measurement_min):
    ...
    for (seqname, transcript_array) in factorized_arrays:
        if start is None:
            ...
            res_shape = (
             len_factor, len_ranges, len_datacol_out, num_bins)
...
def aggregate_arrays(factorized_arrays, ranges, total_datacol, calc_d_T, measurement_min):
    sums = 0.0
    square_sums = 0.0
    divisors = 0
    mins = PINF
    ...
    for (seqname, transcript_array) in factorized_arrays:
        ...
        transcript_array[transcript_array_is_zero] = 0.0
    ...
    if calc_d_T:
        ...
        res = [
         means, sds, mins, maxs, divisors]
    return (
     array(res), start)


def process_arrays(factorized_arrays, ranges, total_datacol, calc_d_T, num_bins, bin_range, measurement_min):
    if num_bins:
        return histogram_arrays(factorized_arrays, ranges, total_datacol, num_bins, bin_range, measurement_min)
    else:
        return aggregate_arrays(factorized_arrays, ranges, total_datacol, calc_d_T, measurement_min)
...
def calc_stats_aggregate(factorized_arrays, *args, **kwargs):
    ...
    return (
     seqnames, [start], array([stats_mean]))
...
def aggregate_filenames(h5filenames, factorizer, aggregate_func, include_names, ranges, total_datacol, calc_d_T, num_bins, hist_max, measurement_min):
    ...
    with h5open(h5filename) as (h5file):
        ...
        (seqnames, starts, stats) = aggregate_func(checked_factorized_arrays, ranges, total_datacol, calc_d_T, num_bins, bin_range, measurement_min)
...
def report(h5filenames, factorizer, aggregate_func, start_override=None, include_names=None, ranges=[], all_columns=False, total_datacol=False, calc_d_T=False, num_bins=None, hist_max=None, measurement_min=0.0):
    assert not (calc_d_T and num_bins)
    (stats, measurement, datacolnames, seqnames, starts, bin_range) = aggregate_filenames(h5filenames, factorizer, aggregate_func, include_names, ranges, total_datacol, calc_d_T, num_bins, hist_max, measurement_min)
    if ranges:
    ...
    stats_transposed = stats.transpose(AXIS_SEQ, AXIS_FACTOR, AXIS_DATACOL, AXIS_POS, AXIS_STAT)
...
def parse_options(args):
    ...
    with OptionGroup(parser, 'Output control') as (group):
        group.add_option('-p', '--start', type=int, metavar='POS', help='[DEPRECATED] call the first position POS')
        group.add_option('-A', '--aggregate', action='store_const', dest='aggregate_func', default=calc_stats_sequential, const=calc_stats_aggregate, help='average results from different sequences (sum in histogram mode)')
        group.add_option('-T', '--total', action='store_true', help='total mutation or state')
        group.add_option('--no-total', action='store_false', dest='total', help='report mutation or state individually')
        group.add_option('-C', '--all-columns', action='store_true', help='include all columns in output, even if they are uninformative')
        group.add_option('-H', '--hist-bins', type=int, metavar='BINS', help='output histograms of BINS bins')
    with OptionGroup(parser, 'Specify a subset') as (group):
        group.add_option('-i', '--include', action='append', dest='include', metavar='ARRAY', help='report only on ARRAY')
        group.add_option('--include-from', action='load', dest='include', metavar='FILE', help='report only on arrays specified in FILE')
        group.add_option('-r', '--range', type=slice, default=[], action='append', metavar='RANGE', help='report only on average results for positions RANGE in the form of "-50:50"')
        group.add_option('--hist-max', type=float, metavar='LIMIT', help='calculate histogram bins in the range 0:LIMIT')
        group.add_option('--measurement-min', type=float, metavar='MIN', help='only include data where measurement > MIN', default=0.0)
    with OptionGroup(parser, 'Stratification factor control') as (group):
        group.add_option('-t', '--tab-delim', metavar='TABFILE', help='stratify on identifiers in TABFILE')
        group.add_option('-a', '--alignment', metavar='FASTAFILE', help='stratify on align slices in FASTAFILE')
        group.add_option('-d', '--distances', action='store_true', help='calculate evolutionary distances')
        group.add_option('-P', '--by-position', action='store_const', dest='alignment_factorizer', const=ByPosAlignmentFactorizer, default=ByNucleotideAlignmentFactorizer, help='stratify by position; disregard nucleotide')
        group.add_option('-s', '--only-substitutions', action='store_const', dest='alignment_factorizer', const=SubsOnlyAlignmentFactorizer, help='only include substitution positions')
...
def main(args=sys.argv[1:]):
    ...
    if total_datacol is None and ranges:
        total_datacol = True
    return report(args, factorizer, aggregate_func, start_override, include_names, ranges, all_columns, total_datacol, calc_d_T, num_bins, hist_max, measurement_min)
