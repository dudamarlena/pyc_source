...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sunflower/factorizers.py
# Compiled at: 2010-06-04 18:58:21
from __future__ import division, with_statement
__version__ = '$Revision: 457 $'
import csv, sys
from numpy import arange, array, empty, identity, isfinite, isnan, isneginf, newaxis, ones, zeros
import tabdelim
...
GAPPED_ALPHABET2LABELS_ALN = [
 ('N', 'ambiguous'), ('-', 'insertion'),
 ('.', 'unaligned'), ('!', 'error')]
...
def load_seqs(filename):
    ...
    with open(filename) as (seqfile):
        ...
        res = dict(((defline.partition(' ')[0], seq) for (defline, seq) in iterator))
        progress('  done')
        return res
...
class TabdelimFactorizer(Factorizer):
    ...
    def __init__(self, filename):
        ...
        self.labels = labels
        self.name2factor = dict(((name, factor_enum[label]) for (name, label) in name2label.iteritems()))
...
class AlignedOnlyAlignmentFactorizer(AlignmentFactorizer):
    ...
    def __init__(self, *args, **kwargs):
        AlignmentFactorizer.__init__(self, *args, **kwargs)
        shape = (
         LEN_GAPPED_ALPHABET, LEN_LABELS_ALIGNED_ONLY, LEN_ALPHABET)
...
class SubsOnlyAlignmentFactorizer(AlignmentFactorizer):
    ...
    def __init__(self, *args, **kwargs):
        AlignmentFactorizer.__init__(self, *args, **kwargs)
        shape = (
         LEN_GAPPED_ALPHABET, LEN_GAPPED_ALPHABET,
...
def add_total(addend_array, methodname='sum'):
    ...
    res[(Ellipsis, -1)] = getattr(addend_array, methodname)(-1)
    return res
