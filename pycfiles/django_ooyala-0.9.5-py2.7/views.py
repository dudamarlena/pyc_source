...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ooyala/views.py
...
def home(request):
    ...
    context = {'video_page': page, 
       'BASE_TEMPLATE': BASE_TEMPLATE}
    return list_detail.object_list(request, latest_videos, paginate_by=8, template_name='ooyala/video_index.html', extra_context=context)
...
def channel(request, object_id):
    ...
    context = {'video_page': video_homepage, 
       'video': video, 
       'BASE_TEMPLATE': BASE_TEMPLATE}
    return render_to_response('ooyala/video_item.html', context, context_instance=RequestContext(request))
...
def search(request):
    ...
    if 'search_query' in request.POST and request.POST['search_query'] != '':
        ...
        context = {'items': items, 
           'BASE_TEMPLATE': BASE_TEMPLATE}
        return list_detail.object_list(request, items, template_name='ooyala/search_results.html', extra_context={'search_query': search_query, 
