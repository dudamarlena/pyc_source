...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ComunioScore/sofascoredb.py
...
class SofascoreDB(DBHandler, Thread):
    __doc__ = ' class SofascoreDB to insert sofascore data into database\n\n    USAGE:\n            sofascoredb = SofascoreDB(update_frequence=21600, **dbparams)\n            sofascoredb.start()\n\n    '
    ...
    def __init__(self, season_date, update_season_frequence=21600, query_match_data_frequence=7200, **dbparams):
        ...
        self.update_season_frequence = update_season_frequence
        self.query_match_data_frequence = query_match_data_frequence
    ...
    def insert_season(self):
        ...
        sql = 'insert into {}.{} (match_day, match_type, match_id, start_timestamp, start_datetime, homeTeam, awayTeam, homeScore, awayScore, season) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'.format(self.comunioscore_schema, self.comunioscore_table_season)
    ...
    def update_season(self):
        ...
        self.season_data = self.bundesliga.season_data()
        sql = 'update {}.{} set match_type = %s, homeScore = %s, awayScore = %s where match_id = %s'.format(self.comunioscore_schema, self.comunioscore_table_season)
        try:
            for matchday in self.season_data:
    ...
    def get_last_match_day(self):
        ...
        """
        last_match_day_sql = "select match_day from {}.{} where match_type='finished' order by match_day desc limit 1".format(self.comunioscore_schema, self.comunioscore_table_season)
    ...
    def insert_points(self):
        ...
        points_sql = 'insert into {}.{} (userid, username, match_id, match_day, hometeam, awayteam) values (%s, %s, %s, %s, %s, %s)'.format(self.comunioscore_schema, self.comunioscore_table_points)
