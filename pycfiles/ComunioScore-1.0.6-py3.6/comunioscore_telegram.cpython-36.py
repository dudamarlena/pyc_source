...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ComunioScore/messenger/comunioscore_telegram.py
# Compiled at: 2020-05-01 18:27:16
# Size of source mod 2**32: 11356 bytes
...
class ComunioScoreTelegram:
    __doc__ = " class ComunioScoreTelegram to send updates to the Telegram group\n\n    USAGE:\n            cstelegram = ComunioScoreTelegram(token='')\n            cstelegram.new_msg()\n\n    "
    ...
    def __init__(self, token):
        ...
        self.msg_rate_conv_handler = ConversationHandler(entry_points=[CommandHandler('updates', self.update_msg_rate)], states={RATE: [MessageHandler(Filters.text, self.rate)]},
          fallbacks=[
         CommandHandler('stop', self.stop)])
        self.notify_conv_handler = ConversationHandler(entry_points=[CommandHandler('notify', self.notification)], states={NOTIFY: [MessageHandler(Filters.text, self.notify_on_off)]},
          fallbacks=[
         CommandHandler('stop', self.stop)])
        ...
        self.ranking_replies = [
         '*{user}*, don´t request the player ranking to often!!',
        ...
        self.notification_replies = [
         '*{user}*, you don´t have the permission to turn on/off the notifications!',
         '*{user}*, wtf are you doing here!!',
         '*{user}*, come on dude, are you kidding me?',
    ...
    def update_msg_rate(self, update, context):
        ...
        update.message.reply_markdown(('*{}*, please select the desired update rate! (Rates are in minutes)'.format(user.username)), reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
        return RATE
    ...
    def rate(self, update, context):
        ...
        if self.rate_event_handler:
            ...
            update.message.reply_markdown(('Thank you *{}*! I will change the update rate to *{}* minutes'.format(user.username, update.message.text)), reply_markup=(ReplyKeyboardRemove()))
            self.rate_event_handler(rate=rate)
        else:
            update.message.reply_markdown(('Oh sry *{}*! Errors have occured, so i can not change the update rate. Please try it again later'.format(user.username)), reply_markup=(ReplyKeyboardRemove()))
    ...
    def notification(self, update, context):
        ...
        update.message.reply_markdown(('*{}*, do you want to turn on or off the notifications?'.format(user.username)), reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
        return NOTIFY
