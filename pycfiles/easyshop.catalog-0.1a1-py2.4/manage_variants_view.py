...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/catalog/browser/manage_variants_view.py
...
class ManageVariantsView(BrowserView):
    ...
    def getProperties(self):
        ...
        for property in pm.getProperties():
            ...
            if len(options) == 0:
                continue
            result.append({'id': property.getId(), 'name': 'property_' + property.getId(), 'title': property.Title(), 'options': options})
    ...
    def getVariants(self):
        ...
        for variant in pvm.getVariants():
            ...
            result.append({'id': variant.getId(), 'path': ('/').join(variant.getPhysicalPath()), 'article_id': variant.getArticleId(), 'title': variant.Title(), 'url': variant.absolute_url(), 'properties': properties, 'properties_ids': properties_ids, 'price': variant.getPrice()})
