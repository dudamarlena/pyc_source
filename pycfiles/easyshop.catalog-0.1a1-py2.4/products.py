...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/catalog/viewlets/products.py
...
class ProductsViewlet(ViewletBase):
    ...
    def getInfo(self):
        ...
        batch_infos = {'parent_url': parent_url, 'previous_url': self._getPreviousUrl(batch), 'previous': batch.previous, 'next_url': self._getNextUrl(batch), 'next': batch.next, 'last_url': self._getLastUrl(batch), 'navigation_list': batch.navlist, 'number_of_pages': batch.numpages, 'page_number': batch.pagenumber}
