...
# [GCC 8.4.0]
# Embedded file name: build/bdist.freebsd-12.1-RELEASE-amd64/egg/bitcoinacceptor/__init__.py
# Compiled at: 2020-05-05 15:43:51
# Size of source mod 2**32: 13726 bytes
"""
...
import bit, bitcash, bitsv, requests
...
def _monero_unspents(unique, piconero_to_try, txids, host, port, user, password):
    ...
    incoming_tx = w.incoming(local_address=unique_address, min_height=minimum_height,
      confirmed=True,
      unconfirmed=False)
        ...
        if tx.transaction.hash not in txids:
            for piconero in piconero_to_try:
                if from_atomic(piconero) == tx.amount:
                    return (
                     return_address, tx.transaction.hash)

    return (
     return_address, False)


def _unspents(address, satoshis_to_try, unique, currency='btc', txids=[], monero_rpc=None, min_confirmations=MIN_CONFIRMATIONS):
    """
    ...
    if isinstance(satoshis_to_try, int):
        satoshis_to_try = [
         satoshis_to_try]
    elif 'get_unspents' in dir(our_bit.network.NetworkAPI):
    ...
    for unspent in unspents:
        ...
        for satoshis in satoshis_to_try:
            ...
            if unspent.amount == paid_satoshis and unspent.txid not in txids:
                return (
                 unspent.txid, unspent.amount)
...
def payment(address, satoshis_to_try, unique, currency='btc', txids=[], monero_rpc=None, min_confirmations=MIN_CONFIRMATIONS):
    """
    Accepts a payment.
    ...
    **These comments are be very inaccurate and out of date.**
    ...
    if currency == 'xmr':
        ...
        if not isinstance(monero_rpc, dict):
            ...
            raise ValueError(msg)
        address, txid = _monero_unspents(unique=unique, piconero_to_try=satoshis_to_try,
          txids=txids,
        ...
        txid, satoshis = _unspents(address, satoshis_to_try,
          unique,
          currency,
          txids,
          monero_rpc,
...
def fiat_payment(address, cents, unique, currency='btc', first_price=None, second_price=None, txids=[], monero_rpc=None, min_confirmations=MIN_CONFIRMATIONS):
    """
    ...
    if currency == 'xmr':
        first_satoshis = first_satoshis // 1000 * 1000
        second_satoshis = second_satoshis // 1000 * 1000
    else:
        ...
        if first_satoshis == second_satoshis:
            ...
            satoshis_to_try = [
             first_satoshis, second_satoshis]
    return payment(address, satoshis_to_try, unique, currency, txids, monero_rpc, min_confirmations)