...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/access/raam/raam.py
# Compiled at: 2020-02-24 17:09:30
# Size of source mod 2**32: 7430 bytes
import numpy as np, pandas as pd

def iterate_raam(demand, supply, travel, max_cycles=151, initial_step=0.2, min_step=0.005, half_life=50, limit_initial=20, verbose=False):
    ...
    for i in range(max_cycles):
        ...
        drlmin_new = slmin * slmax / (slmin + slmax) * (trlmax - trlmin + (dr + drotherlmax) / slmax - drotherlmin / slmin)
        ...
        if i < limit_initial:
            ...
            naive_assignment = delta_mat.sum(axis=0) / supply
...
def raam(demand_df, supply_df, cost_df, demand_index=True, demand_name='demand', supply_index=True, supply_name='supply', cost_origin='origin', cost_dest='dest', cost_name='cost', tau=60, rho=None, max_cycles=150, initial_step=0.2, min_step=0.005, half_life=50, verbose=False):
    r"""Calculate the rational agent access model's total cost --
    ...
    raam_cost = iterate_raam(demand_np, supply_np, travel_np, verbose=verbose, max_cycles=max_cycles,
