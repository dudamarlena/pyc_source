...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/access/__init__.py
# Compiled at: 2020-02-24 17:09:57
# Size of source mod 2**32: 72363 bytes
__version__ = '1.0.1'
import pandas as pd, requests, warnings, logging
from . import fca
from . import raam
from . import weights
from . import helpers
...
class access:
    ...
    def __init__(self, demand_df, demand_value, supply_df, supply_value=False, demand_index=True, supply_index=True, cost_df=None, cost_origin=None, cost_dest=None, cost_name=None, neighbor_cost_df=None, neighbor_cost_origin=None, neighbor_cost_dest=None, neighbor_cost_name=None):
        """
        ...
        if supply_index is not True:
            if supply_index not in supply_df.columns:
        ...
        if cost_df is not None:
            ...
            elif cost_dest not in cost_df.columns:
        ...
        if neighbor_cost_df is not None:
            ...
            elif neighbor_cost_dest not in neighbor_cost_df.columns:
    ...
    def weighted_catchment(self, name='catchment', supply_cost=None, supply_values=None, weight_fn=None, max_cost=None, normalize=False):
        """
        ...
        for s in supply_values:
            series = fca.weighted_catchment(loc_df=(self.supply_df), loc_index=True, loc_value=s, cost_df=(self.cost_df),
              cost_source=(self.cost_dest),
              cost_dest=(self.cost_origin),
              cost_cost=(self.default_cost),
              weight_fn=weight_fn,
              max_cost=max_cost)
    ...
    def fca_ratio(self, name='fca', demand_cost=None, supply_cost=None, supply_values=None, max_cost=None, normalize=False, noise='quiet'):
        """
        ...
        supply_values = helpers.sanitize_supplies(self, supply_values)
        for s in supply_values:
            series = fca.fca_ratio(demand_df=(self.demand_df), demand_index=(self.demand_df.index.name),
    ...
    def raam(self, name='raam', cost=None, supply_values=None, normalize=False, tau=60, rho=None, max_cycles=150, initial_step=0.2, half_life=50, min_step=0.005, verbose=False):
        ...
        supply_values = helpers.sanitize_supplies(self, supply_values)
        for s in supply_values:
            raam_costs = raam.raam(demand_df=(self.demand_df), supply_df=(self.supply_df), cost_df=(self.cost_df), demand_name=(self.demand_value),
              supply_name=s,
    ...
    def two_stage_fca(self, name='2sfca', cost=None, max_cost=None, supply_values=None, weight_fn=None, normalize=False):
        ...
        """
        if not self.supply_value_provided == True:
            raise AssertionError('You must provide a supply value in order to use this functionality.')
        else:
            if cost is None:
        ...
        for s in supply_values:
            series = fca.two_stage_fca(demand_df=(self.demand_df), demand_index=(self.demand_df.index.name),
              demand_name=(self.demand_value),
              supply_df=(self.supply_df),
              supply_index=(self.supply_df.index.name),
              supply_name=s,
    ...
    def enhanced_two_stage_fca(self, name='e2sfca', cost=None, supply_values=None, max_cost=None, weight_fn=None, normalize=False):
    ...
    def three_stage_fca(self, name='3sfca', cost=None, supply_values=None, max_cost=None, weight_fn=None, normalize=False):
        ...
        supply_values = helpers.sanitize_supplies(self, supply_values)
        for s in supply_values:
            series = fca.three_stage_fca(demand_df=(self.demand_df), demand_index=(self.demand_df.index.name),
              demand_name=(self.demand_value),
              supply_df=(self.supply_df),
              supply_index=(self.supply_df.index.name),
              supply_name=s,
    ...
    def user_cost(self, new_cost_df, origin, destination, name):
        ...
        """
        self.cost_df = self.cost_df.merge((new_cost_df[[origin, destination, name]]), how='outer',
          left_on=[
         self.cost_origin,
         self.cost_dest],
          right_on=[
    ...
    def user_cost_neighbors(self, new_cost_df, origin, destination, name):
        ...
        4  17093890101  17031010400  84.97          63268.514352
        """
        self.neighbor_cost_df = self.neighbor_cost_df.merge((new_cost_df[[origin, destination, name]]), how='outer',
          left_on=[
         self.neighbor_cost_origin,
