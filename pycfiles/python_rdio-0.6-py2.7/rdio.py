...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/rdio/rdio.py
# Compiled at: 2012-05-28 13:47:44
"""A library that provides a Python interface to Rdio's API."""
__author__ = 'benjaminkreeger@gmail.com'
__version__ = '0.4'
from urlparse import parse_qsl
import inspect, oauth2 as oauth, json, urllib, re
...
rdio_genders = {'m': (
   ...
   'f': (
       'female', 'her')}
rdio_activity_types = {0: (
     'track added to collection', '%s added some music to %s collection.'), 
   1: (
     'track added to playlist', '%s added some music to a playlist.'), 
   3: (
     'friend added', '%s added a friend.'), 
   5: (
     'user joined', '%s joined Rdio.'), 
   6: (
     'comment added to track', '%s commented on a track.'), 
   7: (
     'comment added to album', '%s commented on an album.'), 
   8: (
     'comment added to artist', '%s commented on an artist.'), 
   9: (
     'comment added to playlist', '%s commented on a playlist.'), 
   10: (
      'track added via match collection',
      '%s matched music to %s collection.'), 
   11: (
      'user subscribed to Rdio', '%s subscribed to Rdio.'), 
   12: (
      'track synced to mobile', '%s synced some music to %s mobile app.')}
methods = {'add_friend': 'addFriend', 
...
class RdioArtist(RdioObject):
    ...
    def __init__(self, data):
        ...
        if 'users' in data:
            self.users = parse_result_list(data['users'])
        if 'topSongsKey' in data:
            self.top_songs_key = data['topSongsKey']
        if 'count' in data:
            self.collection_track_count = data['count']
        if 'radioKey' in data:
            self.radio_key = data['radioKey']
        return
...
class RdioAlbum(RdioMusicObject):
    ...
    def __init__(self, data):
        ...
        if 'users' in data:
            self.users = parse_result_list(data['users'])
        if 'isCompilation' in data:
            self.is_compilation = data['isCompilation']
        return
...
class RdioTrack(RdioMusicObject):
    ...
    def __init__(self, data):
        ...
        if 'trackNum' in data:
            self.track_number = data['trackNum']
...
class RdioPlaylist(RdioObject):
    ...
    def __init__(self, data):
        ...
        self.last_updated = datetime.fromtimestamp(int(data['lastUpdated']))
        self.short_url = data['shortUrl']
        self.embed_url = data['embedUrl']
        self.track_keys = []
...
class RdioUser(RdioObject):
    ...
    def __init__(self, data):
        ...
        if 'lastSongPlayTime' in data:
            self.last_song_play_time = datetime.strptime(data['lastSongPlayTime'], TIME_FORMAT).replace(tzinfo=UTC)
        if 'isTrial' in data:
            self.is_trial = data['isTrial']
        if 'isSubscriber' in data:
            self.is_subscriber = data['isSubscriber']
        if 'isUnlimited' in data:
            self.is_unlimited = data['isUnlimited']
        ...
        if 'collectionKey' in data:
            self.collection_key = data['collectionKey']
        if 'followingUrl' in data:
            self.following_url = data['followingUrl']
        if 'collectionUrl' in data:
            self.collection_url = data['collectionUrl']
        if 'playlistsUrl' in data:
            self.playlists_url = data['playlistsUrl']
        if 'followersUrl' in data:
            self.followers_url = data['followersUrl']
        return
...
class RdioActivityItem(JSONBasedObject):
    ...
    def __init__(self, data):
        ...
        self.date = datetime.strptime(data['date'], TIME_FORMAT).replace(tzinfo=UTC)
        ...
        if 'reviewed_item' in data:
            self.reviewed_item = derive_rdio_type_from_data(data['reviewed_item'])
...
class Api(object):
    ...
    def __init__(self, consumer_key=None, consumer_secret=None, access_token_key=None, access_token_secret=None):
        ...
        """
        self.set_credentials(consumer_key=consumer_key, consumer_secret=consumer_secret, access_token_key=access_token_key, access_token_secret=access_token_secret)

    def set_credentials(self, consumer_key=None, consumer_secret=None, access_token_key=None, access_token_secret=None):
        ...
        if access_token_key and access_token_secret:
            self._oauth_access_token = oauth.Token(key=access_token_key, secret=access_token_secret)
            self._oauth_client = oauth.Client(self._oauth_consumer, self._oauth_access_token)
    ...
    def get_token_and_login_url(self, oauth_callback='oob'):
        ...
        try:
            response, content = self._oauth_client.request(OAUTH_TOKEN_URL, HTTP_METHOD, data)
    ...
    def authorize_with_verifier(self, oauth_verifier, request_token):
        ...
        try:
            ...
            self._oauth_request_token = oauth.Token(key=request_token['oauth_token'], secret=request_token['oauth_token_secret'])
            self._oauth_request_token.set_verifier(oauth_verifier)
            self._oauth_client = oauth.Client(self._oauth_consumer, self._oauth_request_token)
            response, content = self._oauth_client.request(OAUTH_ACCESS_URL, HTTP_METHOD)
            parsed_content = dict(parse_qsl(content))
            token = parsed_content['oauth_token']
            token_secret = parsed_content['oauth_token_secret']
            self.set_credentials(access_token_key=token, access_token_secret=token_secret)
    ...
    def call_api(self, data):
        ...
        response, content = self._oauth_client.request(ROOT_URL, HTTP_METHOD, data)
    ...
    def add_to_playlist(self, playlist, tracks):
        ...
        """
        data = {'method': methods['add_to_playlist'], 
           'playlist': playlist, 
           'tracks': (',').join(tracks)}
        print data
        return self.call_api_authenticated(data)
    ...
    def create_playlist(self, name, description, tracks, extras=[]):
        ...
        """
        data = {'method': methods['create_playlist'], 
           'name': name, 
    ...
    def find_user(self, email=None, vanity_name=None):
        ...
        if email:
            if validate_email(email):
                ...
                raise RdioInvalidParameterException('Invalid email address: %s.' % email)
        if vanity_name:
    ...
    def get(self, keys, extras=[]):
        ...
        if results:
            return parse_result_dictionary(results)
        else:
            return
    ...
    def get_activity_stream(self, user, scope, last_id=None):
        ...
        if scope:
            if scope in ('user', 'friends', 'everyone'):
                ...
                raise RdioInvalidParameterException(scope, 'scope', 'get_activity_stream')
        else:
        ...
        results = self.call_api(data)
        if results:
            return RdioActivityStream(results)
        else:
            return

    def get_albums_for_artist(self, artist, featuring=False, extras=[], start=None, count=None):
    ...
    def get_albums_for_artist_in_collection(self, artist, user=None):
        ...
        """
        data = {'method': methods['get_albums_for_artist_in_collection'], 
           'artist': artist}
        if user:
            data['user'] = user
        if user:
    ...
    def get_albums_in_collection(self, user=None, start=None, count=None, sort=None, query=None):
        ...
        if sort:
            if sort in ('dateAdded', 'playCount', 'artist', 'name'):
                ...
                raise RdioInvalidParameterException(sort, 'sort', 'get_albums_in_collection')
        if query:
            data['query'] = query
        if user:
            results = self.call_api(data)
    ...
    def get_artists_in_collection(self, user=None, start=None, count=None, sort=None, query=None):
        """Returns the albums in a user's collection.
        ...
        if sort:
            if sort in ('name', ):
                ...
                raise RdioInvalidParameterException(sort, 'sort', 'get_artists_in_collection')
        if query:
            data['query'] = query
        if user:
            results = self.call_api(data)
    ...
    def get_heavy_rotation(self, user=None, object_type=None, friends=False, limit=None):
       ...
       """
        ...
        if object_type:
            if object_type in ('artists', 'albums'):
                ...
                raise RdioInvalidParameterException(object_type, 'type', 'get_heavy_rotation')
        if friends:
    ...
    def get_new_releases(self, time=None, start=None, count=False, extras=[]):
        ...
        if time:
            if time in ('thisweek', 'lastweek', 'twoweeks'):
                ...
                raise RdioInvalidParameterException(time, 'time', 'get_new_releases')
        if start:
            data['start'] = start
        if count:
            data['count'] = count
    ...
    def get_object_from_short_code(self, short_code):
        ...
        """
        data = {'method': methods['get_object_from_short_code'], 
           'short_code': short_code}
        result = self.call_api_authenticated(data)
        if result:
            return derive_rdio_type_from_data(result)
        else:
            return
    ...
    def get_playlists(self, extras=[]):
        ...
        results = self.call_api_authenticated(data)
        if results:
            return RdioPlaylistSet(results)
        else:
            return
    ...
    def get_top_charts(self, result_type, start=None, count=None, extras=[]):
        ...
        if result_type in ('Artist', 'Album', 'Track', 'Playlist'):
            ...
            raise RdioInvalidParameterException(result_type, 'result_type', 'get_top_charts')
        if start:
            data['start'] = start
        if count:
            data['count'] = count
    ...
    def get_tracks_for_album_in_collection(self, album, user=None, extras=[]):
        ...
        """
        data = {'method': methods['get_tracks_for_album_in_collection'], 
           'album': album}
        if user:
            data['user'] = user
        if extras:
    ...
    def get_tracks_for_artist(self, artist, appears_on=None, extras=[], start=None, count=None):
    ...
    def get_tracks_for_artist_in_collection(self, artist, user=None, extras=[]):
        ...
        """
        data = {'method': methods['get_tracks_for_artist_in_collection'], 
           'artist': artist}
        if user:
            data['user'] = user
        if extras:
    ...
    def get_tracks_in_collection(self, user=None, start=None, count=None, sort=None, query=None):
        ...
        if sort:
            if sort in ('dateAdded', 'playCount', 'artist', 'album', 'name'):
                ...
                raise RdioInvalidParameterException(sort, 'sort', 'get_tracks_in_collection')
        if query:
            data['query'] = query
        results = self.call_api(data)
        if results:
    ...
    def remove_from_collection(self, keys):
        ...
        """
        data = {'method': methods['remove_from_collection'], 
           'keys': (',').join(keys)}
        return self.call_api_authenticated(data)
    ...
    def remove_from_playlist(self, playlist, tracks, index=None, count=None):
        ...
        """
        data = {'method': methods['remove_from_playlist'], 
           'playlist': playlist, 
    ...
    def search(self, query, types, never_or=None, extras=[], start=None, count=None):
        ...
        """
        data = {'method': methods['search'], 
           'query': query, 
           'types': (',').join(types)}
        if never_or:
            data['never_or'] = never_or
        if extras:
            data['extras'] = (',').join(extras)
        if start:
            data['start'] = start
    ...
    def set_playlist_collaborating(self, playlist, collaborating):
        ...
        """
        data = {'method': methods['set_playlist_collaborating'], 
           'playlist': playlist, 
    ...
    def set_playlist_collaboration_mode(self, playlist, mode):
        ...
        """
        data = {'method': methods['set_playlist_collaboration_mode'], 
           'playlist': playlist, 
           'mode': mode}
        return self.call_api_authenticated(data)
    ...
    def set_playlist_fields(self, playlist, name, description):
        ...
        """
        data = {'method': methods['set_playlist_fields'], 
           'playlist': playlist, 
           'name': name, 
           'description': description}
        return self.call_api_authenticated(data)
    ...
    def set_playlist_order(self, playlist, tracks):
        ...
        """
        data = {'method': methods['set_playlist_order'], 
           'playlist': playlist, 
           'tracks': (',').join(tracks)}
        return self.call_api_authenticated(data)
