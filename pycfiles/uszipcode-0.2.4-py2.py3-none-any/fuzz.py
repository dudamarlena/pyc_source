...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/uszipcode-project/uszipcode/pkg/fuzzywuzzy/fuzz.py
# Compiled at: 2018-09-29 23:45:55
from __future__ import unicode_literals
import platform, warnings
try:
    from .string_matcher import StringMatcher as SequenceMatcher
except ImportError:
...
def _process_and_sort(s, force_ascii, full_process=True):
    ...
    tokens = ts.split()
    sorted_string = (b' ').join(sorted(tokens))
...
def _token_set(s1, s2, partial=True, force_ascii=True, full_process=True):
    ...
    sorted_sect = (b' ').join(sorted(intersection))
    sorted_1to2 = (b' ').join(sorted(diff1to2))
    sorted_2to1 = (b' ').join(sorted(diff2to1))
    combined_1to2 = sorted_sect + b' ' + sorted_1to2
    combined_2to1 = sorted_sect + b' ' + sorted_2to1
