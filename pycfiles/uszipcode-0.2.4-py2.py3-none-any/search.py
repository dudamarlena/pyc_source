...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/uszipcode-project/uszipcode/search.py
# Compiled at: 2019-10-16 17:06:25
"""
This module allows developer to query zipcode with super clean API.
"""
import sys, math, heapq, sqlalchemy as sa
from collections import OrderedDict
from sqlalchemy.orm import sessionmaker
from six import integer_types, string_types
from pathlib_mate import Path
from .db import is_simple_db_file_exists, is_db_file_exists, connect_to_simple_zipcode_db, connect_to_zipcode_db, download_simple_db_file, download_db_file
...
class SearchEngine(object):
    ...
    def _get_cache_data(self):
            ...
            if major_city is not None:
                ...
                if state is not None:
                    ...
                    try:
                        ...
                        self._state_to_city_mapper[state] = [
                         major_city]

                    try:
                        self._city_to_state_mapper[major_city].append(state)
                    except:
                        self._city_to_state_mapper[major_city] = [
                         state]
        ...
        self._state_to_city_mapper = OrderedDict(sorted(self._state_to_city_mapper.items(), key=lambda x: x[0]))
        for v in self._state_to_city_mapper.values():
            v.sort()

        self._city_to_state_mapper = OrderedDict(sorted(self._city_to_state_mapper.items(), key=lambda x: x[0]))
    ...
    def find_state(self, state, best_match=True, min_similarity=70):
        ...
        if state.upper() in STATE_ABBR_SHORT_TO_LONG:
            ...
            if confidence >= min_similarity:
                result_state_short_list.append(STATE_ABBR_LONG_TO_SHORT[state_long])
        else:
        ...
        if len(result_state_short_list) == 0:
            message = "'%s' is not a valid state name, use 2 letter short name or correct full name please."
    ...
    def by_zipcode(self, zipcode, zipcode_type=None, zero_padding=True):
        ...
        if zero_padding:
            ...
            zipcode = str(zipcode)
        res = self.query(zipcode=zipcode, sort_by=None, returns=1, zipcode_type=zipcode_type)
        if len(res):
            return res[0]
        else:
    ...
    def by_prefix(self, prefix, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.zipcode.name, ascending=True, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by first N digits.

        Returns multiple results.
        """
        return self.query(prefix=prefix, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)
    ...
    def by_pattern(self, pattern, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.zipcode.name, ascending=True, returns=DEFAULT_LIMIT):
        ...
        Returns multiple results.
        """
        return self.query(pattern=pattern, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_city(self, city, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.zipcode.name, ascending=True, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by fuzzy City name.

        My engine use fuzzy match and guess what is the city you want.
        """
        return self.query(city=city, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_state(self, state, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.zipcode.name, ascending=True, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by fuzzy State name.

        My engine use fuzzy match and guess what is the state you want.
        """
        return self.query(state=state, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_city_and_state(self, city, state, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.zipcode.name, ascending=True, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by fuzzy city and state name.

        My engine use fuzzy match and guess what is the state you want.
        """
        return self.query(city=city, state=state, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_coordinates(self, lat, lng, radius=25.0, zipcode_type=ZipcodeType.Standard, sort_by=SORT_BY_DIST, ascending=True, returns=DEFAULT_LIMIT):
        u"""
        ...
        **中文文档**
        ...
        3. 对这些Zipcode计算出他们的距离, 然后按照距离远近排序。距离超过我们
          限定的半径的直接丢弃.
        """
        return self.query(lat=lat, lng=lng, radius=radius, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_population(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.population.name, ascending=False, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by population range.
        """
        return self.query(population_lower=lower, population_upper=upper, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_population_density(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.population_density.name, ascending=False, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by population density range.

        `population density` is `population per square miles on land`
        """
        return self.query(population_density_lower=lower, population_density_upper=upper, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_land_area_in_sqmi(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.land_area_in_sqmi.name, ascending=False, returns=DEFAULT_LIMIT):
        """
    ...
    def by_water_area_in_sqmi(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.water_area_in_sqmi.name, ascending=False, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by water area / sq miles range.
        """
        return self.query(water_area_in_sqmi_lower=lower, water_area_in_sqmi_upper=upper, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_housing_units(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.housing_units.name, ascending=False, returns=DEFAULT_LIMIT):
        """
    ...
    def by_occupied_housing_units(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.occupied_housing_units.name, ascending=False, returns=DEFAULT_LIMIT):
        """
    ...
    def by_median_home_value(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.median_home_value.name, ascending=False, returns=DEFAULT_LIMIT):
        """
        Search zipcode information by median home value.
        """
        return self.query(median_home_value_lower=lower, median_home_value_upper=upper, sort_by=sort_by, zipcode_type=zipcode_type, ascending=ascending, returns=returns)

    def by_median_household_income(self, lower=-1, upper=2147483648, zipcode_type=ZipcodeType.Standard, sort_by=SimpleZipcode.median_household_income.name, ascending=False, returns=DEFAULT_LIMIT):
        """
    ...
    def inspect_raw_data(self, zipcode):
        sql = ("SELECT * FROM {} WHERE zipcode = '{}'").format(self.zip_klass.__tablename__, zipcode)
        stmt = sa.text(sql)
