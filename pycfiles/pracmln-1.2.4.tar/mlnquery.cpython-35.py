...
# [GCC 8.4.0]
# Embedded file name: /home/nyga/work/code/pracmln/python3/pracmln/mlnquery.py
# Compiled at: 2018-04-24 04:48:32
# Size of source mod 2**32: 40357 bytes
import argparse, os, sys, ntpath, traceback
from tkinter import Frame, BOTH, Label, Button, OptionMenu, IntVar, Checkbutton, W, E, Entry, messagebox, END, DISABLED, NORMAL, Tk
...
import pstats, io, logging
logger = logs.getlogger(__name__)
GUI_SETTINGS = [
 'window_loc', 'db', 'method', 'use_emln', 'save',
...
class MLNQuery(object):
    ...
    def run(self):
        ...
        if isinstance(self.db, Database):
            ...
            if isinstance(self.db, list) and len(self.db) == 1:
                db = self.db[0]
            else:
                if isinstance(self.db, list) and len(self.db) == 0:
                    db = Database(mln)
                else:
                ...
                if self.verbose:
                    print(tabulate(sorted(list(params.items()), key=lambda k_v: str(k_v[0])), headers=('Parameter:',
                                                                                                       'Value:')))
...
class MLNQueryGUI(object):
    ...
    def __init__(self, master, gconf, directory=None):
        ...
        self.mln_container = FileEditBar(self.frame, directory=self.dir, filesettings={'extension': '.mln', 'ftypes': [('MLN files', '.mln')]}, defaultname='*unknown{}', importhook=self.import_mln, deletehook=self.delete_mln, projecthook=self.save_proj, filecontenthook=self.mlnfilecontent, fileslisthook=self.mlnfiles, updatehook=self.update_mln, onchangehook=self.project_setdirty)
        ...
        self.db_container = FileEditBar(self.frame, directory=self.dir, filesettings={'extension': '.db', 'ftypes': [('Database files', '.db')]}, defaultname='*unknown{}', importhook=self.import_db, deletehook=self.delete_db, projecthook=self.save_proj, filecontenthook=self.dbfilecontent, fileslisthook=self.dbfiles, updatehook=self.update_db, onchangehook=self.project_setdirty)
    ...
    def _got_focus(self, *_):
        if self.master.focus_get() == self.mln_container.editor:
            ...
            if self.master.focus_get() == self.db_container.editor:
                if not self.project.dbs and not self.db_container.file_buffer:
                    self.db_container.new_file()
            elif self.master.focus_get() == self.emln_container.editor and not self.project.emlns and not self.emln_container.file_buffer:
    ...
    def ask_save_project(self):
        fullfilename = asksaveasfilename(initialdir=self.project_dir, confirmoverwrite=True, filetypes=[
         ('PRACMLN project files', '.pracmln')], defaultextension='.pracmln')
    ...
    def update_mln(self, old=None, new=None, content=None, askoverwrite=True):
        ...
        if old == new and askoverwrite:
            ...
            if old == new and not askoverwrite:
                self.project.mlns[old] = content
            else:
