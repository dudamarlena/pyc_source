...
# [GCC 8.4.0]
# Embedded file name: /data/code/pracmln/python3/pracmln/utils/visualization.py
...
def plot_fscores(labels, series):
    ...
    for i, (l, s) in enumerate(zip(labels, series)):
        c = CVALUE[COLORS[i]]
        plt.plot((list(range(len(s)))), s, '-', marker=(MARKERS[i]), color=c, linewidth=2.5,
          markersize=12,
          fillstyle='full',
          label=l)
...
def plot_KLDiv_with_logscale(series):
    ...
    c = CVALUE[COLORS[0]]
    m = MARKERS[0]
    plt.plot((list(range(len(series)))), series, '-', marker=m, color=c, linewidth=2.5, markersize=12,
      fillstyle='full',
      label='Label')
    c = CVALUE[COLORS[1]]
    m = MARKERS[1]
