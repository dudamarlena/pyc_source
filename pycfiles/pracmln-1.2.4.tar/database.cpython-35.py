...
# [GCC 8.4.0]
# Embedded file name: /home/nyga/work/code/pracmln/python3/pracmln/mln/database.py
...
class Database(object):
    ...
    def _atomstr(self, gndatom):
        ...
        if type(gndatom) is str:
            ...
            if isinstance(gndatom, Logic.GroundAtom):
                atomstr = str(gndatom)
            else:
                if isinstance(gndatom, Logic.GroundLit):
    ...
    def add(self, gndlit, truth=1):
        ...
        if isinstance(gndlit, str):
            ...
            if isinstance(gndlit, Logic.GroundLit):
        ...
        for atom, _ in self:
            ...
            for arg, arg_, dom, dom_ in zip(args, args_, doms, doms_):
                if arg == arg_ and dom == dom_:
                    dontremove.add((dom, arg))

        for dom, arg in zip(doms, args):
            if (
             dom, arg) not in dontremove:
    ...
    def isempty(self):
        ...
        """
        return not any([x >= 0 and x <= 1 for x in list(self._evidence.values())]) and len(self.domains) == 0
    ...
    for line, l in enumerate(content.split('\n')):
        ...
        if l == '':
            ...
            if l == '---' and not db.isempty():
                ...
                if '{' in l:
                    ...
                    if l.startswith('#include'):
                        ...
                        dbs.extend(parse_db(content=mlnpath(includefilename).content, ignore_unknown_preds=ignore_unknown_preds, dirs=[p.resolve_path()] + dirs, projectpath=ifnone(p.project, projectpath, lambda x: '/'.join(p.path + [x])), mln=mln))
                        continue
                    else:
                        if l[0] in '0123456789':
                            ...
                            try:
                                _, predname, constants = mln.logic.parse_literal(gndatom)
                            ...
                            raise Exception('Unknown literals not supported (%s)' % l)
            try:
            ...
            if mln.predicate(predname) is None and ignore_unknown_preds:
                ...
                if mln.predicate(predname) is None:
