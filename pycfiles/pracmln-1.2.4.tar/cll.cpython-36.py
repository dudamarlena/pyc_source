...
# [GCC 8.4.0]
# Embedded file name: /data/code/pracmln/python3/pracmln/mln/learning/cll.py
        ...
        if len(literals) == 0:
            ...
            for gndlit in gndliterals:
                if not isinstance(gndlit, Logic.Equality):
                    if hasattr(self, 'qpreds'):
                        if gndlit.gndatom.predname not in self.qpreds:
                            continue
        ...
        for assign in Logic.iter_eq_varassignments(gndlit, formula, self.mrf) if isinstance(gndlit, Logic.Equality) else gndlit.itervargroundings(self.mrf):
            ...
            if isinstance(gndlit_, Logic.Equality):
                ...
                if atom.idx in processed:
                    ...
                    if isconj:
                        if truth == 0:
                            if f_gndlit_parts is not None:
                                if atom not in f_gndlit_parts:
                                    continue
                            else:
                                if isevidence:
                                    continue
