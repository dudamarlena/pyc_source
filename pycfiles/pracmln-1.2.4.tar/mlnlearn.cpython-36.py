...
# [GCC 8.4.0]
# Embedded file name: /data/code/pracmln/python3/pracmln/mlnlearn.py
# Compiled at: 2019-02-27 05:10:32
# Size of source mod 2**32: 48186 bytes
import argparse, fnmatch, io, pstats, tkinter.messagebox, traceback
...
class MLNLearn(object):
    ...
    def __init__(self, config=None, **params):
        ...
        if config is None:
            ...
            if isinstance(config, PRACMLNConfig):
    ...
    def run(self):
        ...
        if type(self.db) is list:
            if all([isinstance(e, Database) for e in self.db]):
                dbs = self.db
        else:
            if isinstance(self.db, Database):
                ...
                if isinstance(self.db, str):
                    ...
                    dbpaths = [
                     os.path.join(self.directory, 'db', db)]
                    dbs = []
                    for p in dbpaths:
        ...
        if self.verbose:
            ...
            if self.verbose:
                ...
                print(tabulate(sorted((list(confg.items())), key=(lambda key_v: str(key_v[0]))),
                  headers=('Parameter:', 'Value:')))
            params = dict([(k, getattr(self, k)) for k in ('multicore', 'verbose',
                                                           'profile', 'ignore_zero_weight_formulas')])
            if issubclass(self.method, DiscriminativeLearner):
                if self.discr_preds == QUERY_PREDS:
                    params['qpreds'] = self.qpreds
                elif self.discr_preds == EVIDENCE_PREDS:
...
class MLNLearnGUI:
    ...
    def __init__(self, master, gconf, directory=None):
        ...
        self.btn_saveproj = Button(project_container, text='Save Project as...',
        ...
        self.mln_container = FileEditBar((self.frame), directory=(self.dir), filesettings={'extension':'.mln', 
        ...
        self.cb_use_initial_weights = Checkbutton(frame, text='use initial weights',
        ...
        self.rbQueryPreds = Radiobutton(frame, text='Query preds:', variable=(self.discrPredicates),
          value=QUERY_PREDS)
        ...
        self.rbEvidencePreds = Radiobutton(frame, text='Evidence preds', variable=(self.discrPredicates),
        ...
        self.db_container = FileEditBar((self.frame), directory=(self.dir), filesettings={'extension':'.db', 
    ...
    def _got_focus(self, *_):
        if self.master.focus_get() == self.mln_container.editor:
            if not self.project.mlns:
                if not self.mln_container.file_buffer:
                    self.mln_container.new_file()
        elif self.master.focus_get() == self.db_container.editor:
            if not self.project.dbs:
                if not self.db_container.file_buffer:
    ...
    def noask_save_project(self):
        if self.project.name:
            if not self.project.name == DEFAULTNAME.format('.pracmln'):
                self.save_project(os.path.join(self.project_dir, self.project.name))
        else:
            self.ask_save_project()

    def ask_save_project(self):
        fullfilename = asksaveasfilename(initialdir=(self.project_dir), confirmoverwrite=True,
          filetypes=[
    ...
    def get_training_db_paths(self, pattern):
        ...
        if pattern is not None and pattern.strip():
            ...
            raise dbs or Exception('No training data given; A training database must be selected or a pattern must be specified')
        else:
            return (
             local, dbs)
    ...
    def learn(self, savegeometry=True, options=None, *_):
        ...
        try:
            ...
            if options.get('mlnarg') is not None:
                mlnobj = MLN(mlnfile=(os.path.abspath(options.get('mlnarg'))), logic=(self.config.get('logic', 'FirstOrderLogic')),
                  grammar=(self.config.get('grammar', 'PRACGrammar')))
            else:
                mlnobj = parse_mln(mln_content, searchpaths=[self.project_dir], projectpath=(os.path.join(self.project_dir, self.project.name)),
