...
# [GCC 8.4.0]
# Embedded file name: /home/nyga/work/code/pracmln/python3/pracmln/mlnlearn.py
    ...
    def run(self):
        ...
        if type(self.db) is list and all([isinstance(e, Database) for e in self.db]):
            ...
            if self.verbose:
                ...
                print(tabulate(sorted(list(confg.items()), key=lambda key_v: str(key_v[0])), headers=('Parameter:',
                                                                                                      'Value:')))
...
class MLNLearnGUI:
    ...
    def __init__(self, master, gconf, directory=None):
        ...
        self.mln_container = FileEditBar(self.frame, directory=self.dir, filesettings={'extension': '.mln', 'ftypes': [('MLN files', '.mln')]}, defaultname='*unknown{}', importhook=self.import_mln, deletehook=self.delete_mln, projecthook=self.save_proj, filecontenthook=self.mlnfilecontent, fileslisthook=self.mlnfiles, selectfilehook=self.set_outputfilename, updatehook=self.update_mln, onchangehook=self.project_setdirty)
        ...
        self.db_container = FileEditBar(self.frame, directory=self.dir, filesettings={'extension': '.db', 'ftypes': [('Database files', '.db')]}, defaultname='*unknown{}', importhook=self.import_db, deletehook=self.delete_db, projecthook=self.save_proj, filecontenthook=self.dbfilecontent, fileslisthook=self.dbfiles, selectfilehook=self.set_outputfilename, updatehook=self.update_db, onchangehook=self.project_setdirty)
    ...
    def _got_focus(self, *_):
        ...
        elif self.master.focus_get() == self.db_container.editor and not self.project.dbs and not self.db_container.file_buffer:
