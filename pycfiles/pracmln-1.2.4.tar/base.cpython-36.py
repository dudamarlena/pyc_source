...
# [GCC 8.4.0]
# Embedded file name: /data/code/pracmln/python3/pracmln/mln/base.py
# Compiled at: 2019-02-27 05:10:32
# Size of source mod 2**32: 34788 bytes
import pyparsing
...
from .mlnpreds import Predicate, FuzzyPredicate, SoftFunctionalPredicate, FunctionalPredicate
...
class MLN(object):
    ...
    def weights(self, wts):
        if len(wts) != len(self._formulas):
            ...
            wts = map(--- This code section failed: ---

 L. 117         0  LOAD_GLOBAL              type
                2  LOAD_FAST                'w'
 ...
 L. 118        18  LOAD_GLOBAL              HARD
               ...
               40  LOAD_GLOBAL              float
               42  LOAD_STR                 '%-10.6f'
               44  LOAD_GLOBAL              float
               46  LOAD_GLOBAL              eval
               48  LOAD_GLOBAL              str
               50  LOAD_FAST                'w'
               ...
               62  RETURN_END_IF_LAMBDA
             64_0  COME_FROM            38  '38'
             64_1  COME_FROM            14  '14'
               64  LOAD_FAST                'w'
               66  RETURN_VALUE_LAMBDA
               -1  LAMBDA_MARKER    

Parse error at or near `COME_FROM' instruction at offset 64_1
, wts)
        self._weights = wts
    ...
    def predicate(self, predicate):
        ...
        if isinstance(predicate, Predicate):
            ...
            if isinstance(predicate, pyparsing.ParseResults):
    ...
    def formula(self, formula, weight=0.0, fixweight=False, unique_templvars=None):
        """
    ...
    def learn(self, databases, method=BPLL, **params):
        ...
        if len(self.probreqs) > 0:
            fittingParams = {'fittingMethod':self.probabilityFittingInferenceMethod,  'fittingSteps':self.probabilityFittingMaxSteps, 
    ...
    def write(self, stream=sys.stdout, color=None):
        ...
        if color is None:
            ...
            if self.domain_decls:
                stream.write(colorize('// domain declarations\n', comment_color, color))
            for d in self.domain_decls:
                stream.write('%s\n' % d)

            stream.write('\n')
            if self.vars:
                stream.write(colorize('// variable definitions\n', comment_color, color))
...
def parse_mln(text, searchpaths=[
 '.'], projectpath=None, logic='FirstOrderLogic', grammar='PRACGrammar', mln=None):
    """
    ...
    text = re.compile('//\\s*<group>\\s*$', re.MULTILINE).sub('#group', text)
    text = re.compile('//\\s*</group>\\s*$', re.MULTILINE).sub('#group.', text)
                        ...
                        if line.startswith('#fixweight'):
                            ...
                            if line.startswith('#fuzzy'):
                                ...
                                if line.startswith('#include'):
                                    ...
                                    m = re.match('"(?P<filename>.+)"', filename)
                                    if m is not None:
                                    ...
                                    parse_mln(text=(mlnpath(includefilename).content), searchpaths=([p.resolve_path()] + dirs), projectpath=(ifnone(p.project, projectpath, lambda x: '/'.join(p.path + [x]))),
                                      logic=logic,
                                      grammar=grammar,
                                      mln=mln)
                                    continue
                                    ...
                                    if line.startswith('#unique'):
                                        ...
                                        if line.startswith('#AdaptiveMLNDependency'):
                            ...
                            if isPredDecl:
                                ...
                                try:
                                    ...
                                    for variant in formula.expandgrouplits():
                                        mln.formula(variant, weight=weight, fixweight=fixweight,
