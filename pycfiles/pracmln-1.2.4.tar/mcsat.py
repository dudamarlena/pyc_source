...
# [GCC 8.4.0]
# Embedded file name: /home/nyga/work/code/pracmln/python2/pracmln/mln/inference/mcsat.py
...
from pracmln.mln.util import item
    ...
    def _initkb(self, verbose=False):
        ...
        for i_gf, gf in enumerate(self.gndformulas):
            if isinstance(gf, Logic.Conjunction):
                ...
                if not isinstance(gf, Logic.TrueFalse):
                    clauses = [
                     gf]
                else:
                    continue
...
class SampleSAT():
    ...
    class _Clause(object):
        ...
        def _isbottleneck(self, atomidx):
            ...
            if all(map(lambda x: x == fst, atomidx2lits[atomidx])):
                return False
            return True
