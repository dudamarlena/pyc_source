...
# [GCC 8.4.0]
# Embedded file name: /home/nyga/code/pracmln/python2/pracmln/mln/learning/multidb.py
# Compiled at: 2018-04-24 10:15:24
...
class MultipleDatabaseLearner(AbstractLearner):
    ...
    def _f(self, w):
        if False:
            ...
            try:
                try:
                    for i, (f_, d_) in enumerate(pool.imap(with_tracing(_methodcaller('_f', sideeffects=True)), map(lambda l: (
                     l, w), self.learners))):
    ...
    def _grad(self, w):
        ...
        if False:
            ...
            try:
                try:
                    for i, (grad_, d_) in enumerate(pool.imap(with_tracing(_methodcaller('_grad', sideeffects=True)), map(lambda l: (
                     l, w), self.learners))):
                        self.learners[i].__dict__ = d_
                        grad += grad_
    ...
    def _hessian(self, w):
        ...
        if self.multicore:
            ...
            try:
                try:
                    for h in pool.imap(with_tracing(_methodcaller('_hessian')), map(lambda l: (
                     l, w), self.learners)):
                        hessian += h
