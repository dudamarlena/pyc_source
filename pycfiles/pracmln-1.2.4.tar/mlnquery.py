...
# [GCC 8.4.0]
# Embedded file name: /home/nyga/work/code/pracmln/python2/pracmln/mlnquery.py
# Compiled at: 2018-04-24 04:48:32
import argparse
from Tkinter import *
from tkFileDialog import askopenfilename, asksaveasfilename
import os, ntpath, tkMessageBox, traceback
...
import pstats, StringIO, pracmln
...
class MLNQuery(object):
    ...
    def run(self):
        ...
        if self.verbose:
            print tabulate(sorted(list(params.viewitems()), key=lambda (k, v): str(k)), headers=('Parameter:',
                                                                                                 'Value:'))
...
class MLNQueryGUI(object):
    ...
    def update_emln(self, old=None, new=None, content=None, askoverwrite=True):
        ...
        if old == new and askoverwrite:
            ...
            self.project.emlns[old] = content
        elif new in self.project.emlns:
            if askoverwrite:
                savechanges = tkMessageBox.askyesno('Save changes', ("A file '{}' already exists. Overwrite?").format(new))
                if savechanges:
