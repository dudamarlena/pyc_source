...
# [GCC 8.4.0]
# Embedded file name: C:\repositorios\web\djmicrosip_apps\django-microsip-api\microsip_api\models_base\metadatos\report_builder.py
# Compiled at: 2019-11-20 19:35:39
...
class ReportBulilderFieldManager(models.Manager):
    ...
    def create_simple(self, **kwargs):
        ...
        if not existe:
            sql = "\n                INSERT INTO rb_field (table_name, field_name, field_alias, datatype, selectable, searchable, sortable, autosearch, mandatory)                         VALUES (%s, %s, %s, %s, 'T', 'T', 'T', 'F', 'F')\n                "
            values_data = [
             table_name, field_name, field_alias, datatype]
            curs = connections[using].cursor()
...
class ReportBulilderTableManager(models.Manager):
    ...
    def create_simple(self, **kwargs):
        ...
        existe = curs.fetchall()[0][0] > 0
        if not existe:
            sql = '\n                INSERT INTO rb_table (table_name, table_alias)                         VALUES (%s, %s)\n                '
            values_data = [
...
class ReportBulilderJoinManager(models.Manager):
    ...
    def create_simple(self, **kwargs):
        ...
        existe = curs.fetchall()[0][0] > 0
        if not existe:
            sql = '\n                INSERT INTO rb_join (table_name1, table_name2, join_type, field_names1, operators, field_names2)                         VALUES (%s, %s, %s, %s, %s, %s)\n                '
