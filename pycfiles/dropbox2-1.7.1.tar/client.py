...
# [GCC 8.4.0]
# Embedded file name: /Users/rick/workspace/dropbox/dropbox/client.py
# Compiled at: 2013-09-20 14:28:47
from __future__ import absolute_import
import base64, re, os, sys, urllib
...
class DropboxClient(object):
    ...
    def __init__(self, oauth2_access_token, locale=None, rest_client=None):
        ...
        if isinstance(oauth2_access_token, basestring):
            ...
            if locale is not None:
                raise ValueError("The 'locale' parameter to DropboxClient is only useful when also passing in an OAuth 2 access token")
    ...
    class ChunkedUploader(object):
        ...
        def upload_chunked(self, chunk_size=4194304):
        ...
        def finish(self, path, overwrite=False, parent_rev=None):
            ...
            params = dict(overwrite=bool(overwrite), upload_id=self.upload_id)
    ...
    def put_file(self, full_path, file_obj, overwrite=False, parent_rev=None):
        ...
        return metadata.MetadataDict(self.rest_client.PUT(url, file_obj, headers))
    ...
    def delta(self, cursor=None):
        ...
        return metadata.MetadataDict(self.rest_client.POST(url, params, headers))
    ...
    def add_copy_ref(self, copy_ref, to_path):
        ...
        Returns:
         ...
         """
        path = '/fileops/copy'
        params = {'from_copy_ref': copy_ref, 'to_path': format_path(to_path), 
    ...
    def file_copy(self, from_path, to_path):
        ...
        """
        params = {'root': self.session.root, 'from_path': format_path(from_path), 
    ...
    def metadata(self, path, list=True, file_limit=25000, hash=None, rev=None, include_deleted=False):
        ...
        params = {'file_limit': file_limit, 'list': 'true', 
...
class DropboxOAuth2Flow(DropboxOAuth2FlowBase):
    ...
    def __init__(self, consumer_key, consumer_secret, redirect_uri, session, csrf_token_session_key, locale=None, rest_client=None):
        if rest_client is None:
            rest_client = RESTClient
    ...
    def finish(self, query_params):
        ...
        if error is not None and code is not None:
            raise self.BadRequestException("Query parameters 'code' and 'error' are both set;  only one must be set.")
        ...
        if not _safe_equals(csrf_token_from_session, given_csrf_token):
            raise self.CsrfException('expected %r, got %r' % (
