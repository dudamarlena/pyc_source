...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/simplesite/controllers/section.py
# Compiled at: 2008-11-08 10:03:44
import logging
...
class UniqueSectionPath(formencode.validators.FancyValidator):
    ...
    def _to_python(self, values, state):
        ...
        query = nav_q.filter_by(section=values['section'], type='section', path=values['path'])
        if request.urlvars['action'] == 'save':
            query = query.filter(model.Nav.id != int(request.urlvars['id']))
        existing = query.first()
        if existing is not None:
            raise formencode.Invalid('There is already a section in this section with this path', values, state)
        return values
        ...
        if values.get('type', 'section') == 'section':
            ...
            while section:
                if section.section == current_section.id:
                    raise formencode.Invalid('You cannot move a section to one of its subsections', values, state)
...
class SectionController(BaseController):
    ...
    def edit(self, id=None):
        ...
        if section is None:
            abort(404)
        values = {'name': section.name, 'path': section.path, 
    ...
    def save(self, id=None):
        ...
        if section is None:
            abort(404)
        if not (section.section == self.form_result['section'] and section.before == self.form_result['before']):
    ...
    def list(self):
        section_q = meta.Session.query(model.Section)
        c.paginator = paginate.Page(section_q, page=int(request.params.get('page', 1)), items_per_page=2, controller='section', action='list')
