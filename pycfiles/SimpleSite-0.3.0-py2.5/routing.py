...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/simplesite/config/routing.py
# Compiled at: 2008-11-08 11:13:12
...
def navigation_from_path(path_info):
    ...
    for path in path_parts[:-1]:
        ...
        elif path_info.endswith('/index') and len(result['breadcrumbs']) == len(path_info.split('/')) - 2:
    ...
    result['page'] = nav_q.filter_by(type='page', section=result['breadcrumbs'][(-1)].id, path=path_parts[(-1)]).first()
    ...
    result['menu'] = menu(nav_q, result['breadcrumbs'][(-2)].id, result['breadcrumbs'][(-2)].path_info)
    result['tabs'] = menu(nav_q, result['breadcrumbs'][0].id, result['breadcrumbs'][0].path_info)
    return result


def menu(nav_q, sectionid, path_info):
    last = None
    navs = [ nav for nav in nav_q.filter_by(section=sectionid).order_by(model.nav_table.c.before.desc()).all()
           ]
    for nav in navs:
    ...
    while len(menu) < len(navs):
        ...
        if not menu_dict.has_key(id):
            raise Exception("This section doesn't have an item %s to go before %r id %s" % (
             id, menu[0].name, menu[0].id))
...
def make_map():
    ...
    map.connect('/page/{pageid}/{controller}/{action}', requirements=dict(pageid='\\d+'))
    map.connect('/page/{pageid}/{controller}/{action}/{id}', requirements=dict(pageid='\\d+', id='\\d+'))
