...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/simplesite/controllers/page.py
# Compiled at: 2008-11-08 10:04:14
import logging
...
class UniquePagePath(formencode.validators.FancyValidator):
    ...
    def _to_python(self, values, state):
        ...
        query = nav_q.filter_by(section=values['section'], type='page', path=values['path'])
        if request.urlvars['action'] == 'save':
            query = query.filter(model.Nav.id != int(request.urlvars['id']))
        existing = query.first()
        if existing is not None:
            raise formencode.Invalid('There is already a page in this section with this path', values, state)
        return values
...
class NewPageForm(NewNavForm):
    ...
    content = formencode.validators.String(not_empty=True, messages={'empty': 'Please enter some content for the page. '})
    ...
    def _to_python(self, values, state):
        ...
        for tag_id in values['tags']:
            if tag_id not in all_tag_ids:
                raise formencode.Invalid('One or more selected tags could not be found in the database', values, state)
...
class PageController(BaseController):
    ...
    def new(self):
        ...
        if values.has_key('before') and values['before'] == 'None':
            values['before'] = ''
    ...
    def create(self):
        ...
        model.Nav.add_navigation_node(page, self.form_result['section'], self.form_result['before'])
        meta.Session.commit()
        return redirect_to('path', id=page.id)
    ...
    def edit(self, id=None):
        ...
        if page is None:
            abort(404)
        values = {'name': page.name, 'path': page.path, 
    ...
    def save(self, id=None):
        ...
        if page is None:
            abort(404)
        if not (page.section == self.form_result['section'] and page.before == self.form_result['before']):
    ...
    def list(self):
        page_q = meta.Session.query(model.Page)
        c.paginator = paginate.Page(page_q, page=int(request.params.get('page', 1)), items_per_page=2, controller='page', action='list')
