...
# [GCC 8.4.0]
# Embedded file name: /root/youkudownloader/youkudownloader/w56.py
# Compiled at: 2013-02-19 21:05:22
__all__ = [
 'w56_download', 'w56_download_by_id']
from common import *
import json
...
def w56_download_by_id(id, title=None, output_dir='.', merge=True):
    ...
    hd = info['hd']
    assert hd in (0, 1, 2)
    type = ['normal', 'clear', 'super', 'qvga', 'qqvga'][hd]
...
def w56_download(url, merge=True):
    id = r1('http://www.56.com/[uw]\\d+/v_(\\w+).html', url)
    if not id:
        id = r1('http://www.56.com/[uw]\\d+/play_album-aid-\\d+_vid-(\\w+).html', url)
