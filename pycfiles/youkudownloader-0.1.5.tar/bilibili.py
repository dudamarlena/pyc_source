...
# [GCC 8.4.0]
# Embedded file name: /root/youkudownloader/youkudownloader/bilibili.py
...
def parse_srt_p(p):
    ...
    return (
     pool, mode, font_size, font_color)


def parse_srt_xml(xml):
    d = re.findall('<d p="([^"]+)">(.*)</d>', xml)
    for x, y in d:
        p = parse_srt_p(x)
...
def bilibili_download(url, merge=True):
    assert re.match('http://(www.bilibili.tv|bilibili.kankanews.com|bilibili.smgbb.cn)/video/av(\\d+)', url)
    html = get_html(url)
    title = r1('<h2>([^<>]+)</h2>', html).decode('utf-8')
    title = unescape_html(title)
    title = escape_file_path(title)
    flashvars = r1_of(['flashvars="([^"]+)"', '"https://secure.bilibili.tv/secure,(cid=\\d+)"'], html)
    assert flashvars
