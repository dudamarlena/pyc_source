...
# [GCC 8.4.0]
# Embedded file name: ./mp4_join.py
...
def read_mvhd(stream, size, left, type):
    ...
    assert stream.read(10) == '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
...
def read_tkhd(stream, size, left, type):
    ...
    duration = read_uint(stream)
    left -= 20
    assert stream.read(8) == '\x00\x00\x00\x00\x00\x00\x00\x00'
...
def read_stsd(stream, size, left, type):
    ...
    class stsd_atom(Atom):
        ...
        def calsize(self):
            oldsize = self.size
            self.size = 16 + sum([ atom.calsize() for atom in self.body[1] ])
...
def read_stts(stream, size, left, type):
    ...
    class stts_atom(Atom):
        ...
        def calsize(self):
            oldsize = self.size
            self.size = 16 + len(self.body[1]) * 8
            assert oldsize == self.size, '%s: %d, %d' % (self.type, oldsize, self.size)
            return self.size
...
def read_stss(stream, size, left, type):
    ...
    class stss_atom(Atom):
        ...
        def calsize(self):
            self.size = 16 + len(self.body[1]) * 4
            return self.size
...
def read_stsc(stream, size, left, type):
    ...
    class stsc_atom(Atom):
        ...
        def calsize(self):
            self.size = 16 + len(self.body[1]) * 12
            return self.size
...
def read_stsz(stream, size, left, type):
    ...
    class stsz_atom(Atom):
        ...
        def calsize(self):
            self.size = 20 + len(self.body[3]) * 4
            return self.size
...
def read_mp4a(stream, size, left, type):
    body, stream = read_body_stream(stream, left)
    assert stream.read(6) == '\x00\x00\x00\x00\x00\x00'
    data_reference_index = read_ushort(stream)
    assert stream.read(8) == '\x00\x00\x00\x00\x00\x00\x00\x00'
...
def concat_mp4s(mp4s, output=None):
    if not mp4s:
        raise AssertionError('no mp4 file found')
        import os.path
        output = output or guess_output(mp4s)
