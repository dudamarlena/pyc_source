...
# [GCC 8.4.0]
# Embedded file name: /root/youkudownloader/youkudownloader/letv.py
# Compiled at: 2013-02-19 21:05:22
__all__ = [
 'letv_download', 'letv_download_playlist', 'letv_download_by_vid']
from common import *
...
def letv_download_by_vid(vid, title, merge=True):
    ...
    url1 = r1('url\\s*"\\s*[:=]\\s*"([^"]+)"', xml).replace('\\', '')
    xml = get_html(url1)
    url = r1('location\\s*"\\s*[:=]\\s*"([^"]+)"', xml).replace('\\', '')
    assert 'flv' in url
...
def letv_download(url, merge=True):
    ...
    vid = r1('[xv]id\\s*[:=]\\s*(\\d+)', html)
    assert vid
    title = r1('title\\s*[:=]\\s*[\'"]([^"]+)[\'"]\\s*,', html)
    if r1('html" target="_blank"\\>\\[([^"]+)\\]', html):
        title = r1('html" target="_blank"\\>\\[([^"]+)\\]', html)
    assert title
