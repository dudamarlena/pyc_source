...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/fedservice/meta_api.py
# Compiled at: 2019-05-29 15:00:52
# Size of source mod 2**32: 4186 bytes
import cherrypy, json, logging, os
...
class MetaAPI(object):
    ...
    def index(self):
        response = [
         ...
         '<link rel="stylesheet" type="text/css" href="/static/theme.css"></head><body><h1>Welcome to my metadata API endpoint</h1>',
         '</body></html>']
        return '\n'.join(response)
...
class MetaAPIDb(MetaAPI):
    ...
    def __init__(self, authn_info, db_uri, key_jar, issuer, static_dir='static', lifetime=14400, sign_alg='ES256'):
        MetaAPI.__init__(self, static_dir)
    ...
    def entity_statement(self, **kwargs):
        if 'iss' in kwargs:
            if kwargs['iss'] != self.issuer:
                raise cherrypy.HTTPError(403, 'Not prepared to sign as "{}"'.format(kwargs['iss']))
        else:
            kwargs['iss'] = self.issuer
        jws = db_make_entity_statement(self.db_uri, key_jar=self.key_jar, lifetime=self.lifetime, 
