...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/fedservice/entity_statement/collect.py
# Compiled at: 2019-12-09 14:40:04
...
def construct_entity_statement_query(api_endpoint, issuer, subject):
    return '{}?{}'.format(api_endpoint, urlencode({'iss':issuer, 
     'sub':subject}))
...
class Collector(object):
    ...
    def __init__(self, trust_anchors, http_cli=None, insecure=False, allowed_delta=300):
    ...
    def get_configuration_information(self, entity_id):
        ...
        response = (self.http_cli)('GET', 
         (construct_well_known_url(entity_id, 'openid-federation')), **kwargs)
        if response.status_code == 200:
    ...
    def collect_intermediate(self, entity_id, intermediate, seen=None, max_superiors=10):
        ...
        if entity_id == intermediate:
            if entity_id in self.trusted_anchors:
                return
        elif seen is None:
            _seen = []
        else:
        ...
        if entity_statement is None:
            ...
            entity_statement = self.get_entity_statement(fed_api_endpoint, intermediate, entity_id)
        ...
        if entity_statement:
            ...
            return (entity_statement,
             self.collect_superiors(intermediate, intermediate_statement,
               seen=_seen,
