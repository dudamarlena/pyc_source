...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/fedservice/rp/provider_info_discovery.py
# Compiled at: 2019-12-08 20:12:09
# Size of source mod 2**32: 5857 bytes
import logging
...
class FedProviderInfoDiscovery(ProviderInfoDiscovery):
    ...
    def __init__(self, service_context, state_db, conf=None, client_authn_factory=None, **kwargs):
        ProviderInfoDiscovery.__init__(self,
          service_context, state_db, conf=conf, client_authn_factory=client_authn_factory)
    ...
    def get_request_parameters(self, method='GET', **kwargs):
        ...
        p = urlparse(_iss)
        _qurl = '{}://{}/.well-known/openid-federation?{}'.format(p.scheme, p.netloc, urlencode(qpart))
        return {'url': _qurl}
    ...
    def update_service_context(self, statements, **kwargs):
        ...
        _fe.proposed_authority_hints = create_authority_hints(_fe.authority_hints, statements)
        if len(possible) == 1:
            for s in statements:
                if s.fo == possible[0]:
                    claims = s.metadata
                    ...
                    _sc.behaviour = map_configuration_to_preference(_sc.provider_info, _sc.client_preferences)

        else:
            claims = statements[0].metadata
            _pinfo = (self.response_cls)(**claims)
            _sc.behaviour = map_configuration_to_preference(_pinfo, _sc.client_preferences)
