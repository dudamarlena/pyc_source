...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/fedservice/__init__.py
# Compiled at: 2020-01-25 15:03:17
...
class FederationEntity(object):
    ...
    def __init__(self, entity_id, trusted_roots, authority_hints=None, key_jar=None, default_lifetime=86400, httpd=None, priority=None, entity_type='', opponent_entity_type='', registration_type=''):
    ...
    def create_entity_statement(self, iss, sub, key_jar=None, metadata=None, metadata_policy=None, authority_hints=None, lifetime=0, **kwargs):
        if not key_jar:
        ...
        return create_entity_statement(iss, sub, key_jar=key_jar, metadata=metadata, metadata_policy=metadata_policy, 
         authority_hints=authority_hints, 
         lifetime=lifetime, **kwargs)
...
def create_federation_entity(entity_id, **kwargs):
    ...
    if 'signing_keys' in kwargs:
        args['key_jar'] = init_key_jar(**kwargs['signing_keys'], **{'owner': entity_id})
    for param in ('entity_type', 'priority', 'opponent_entity_type', 'registration_type'):
        try:
