...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/errol/xmpp.py
# Compiled at: 2020-05-10 07:38:48
# Size of source mod 2**32: 16380 bytes
import os, asyncio, logging, slixmpp
...
class XmppManager(slixmpp.ClientXMPP):
    ...
    def __init__(self, jid=None, password=None, filename=None, server=None, action_str=None, full_filename=None, node=None, path=None, nick='nick', presence_file='/tmp/test.txt', room=None, pubsub_enable=False, muc_enable=False, errol_receiver=None, nick_reveiver=None, nick_sender=None):
    ...
    def send_msg_receiver(self, msg):
        self.send_message(mto=(self.errol_receiver), mbody=msg,
          mtype='chat')

    def send_muc(self, msg):
        if self.muc_enable:
            self.send_message(mto=(self.room), mbody=msg,
              mtype='groupchat')
            return True
        return False
    ...
    def muc_online(self, presence):
        ...
        if self.nick not in presence['muc']['nick']:
            self.send_message(mto=(self.room), mbody=('Bonjour, %s %s' % (presence['muc']['role'],
             presence['muc']['nick'])),
              mtype='groupchat')
    ...
    def muc_message(self, msg):
        ...
        if msg['mucnick'] != self.nick:
            if self.nick in msg['body']:
                self.send_message(mto=(msg['from'].bare), mbody=('I heard that, %s.' % msg['mucnick']),
                  mtype='groupchat')
        if self.nick_sender in msg['mucnick']:
            self.get_filename_from_message(msg['body'])
    ...
    async def send_file(self):
        if os.path.isdir(self.full_filename):
            logger.error('Filename is a directory')
            return None
    ...
    def _purge(self, msg):
        ...
        print('Purged all items from %s' % msg['pubsub_event']['purge']['node'])
    ...
    async def publish(self, action_str):
        if self.pubsub_enable:
            ...
            string_entry = string_entry.replace('DATE', datetime_str).replace('*FILE*', filename).replace('ACTION', action_str)
...
class XmppHandler:
    ...
    def prepare(self, path=None, filename=None, action=None, forever=False, debug=False, xmpp_conf=None):
        ...
        self.errol_receiver = xmpp_conf['receiver']
        ...
        if not self.jid is None:
            if self.password is None or self.errol_receiver is None:
    ...
    async def update_xmpp_instance(self):
        self.xmpp_instance = XmppManager(jid=(self.jid), password=(self.password), filename=(self.filename),
          server=(self.server),
          ...
          room=(self.room),
          pubsub_enable=(self.pubsub_enable),
          muc_enable=(self.muc_enable),
          errol_receiver=(self.errol_receiver),
          nick_reveiver=(self.nick_receiver),
          nick_sender=(self.nick_sender))
        self.xmpp_instance.register_plugin('xep_0030')
        self.xmpp_instance.register_plugin('xep_0065', {'auto_accept': True})
        self.xmpp_instance.register_plugin('xep_0030')
        ...
        self.xmpp_instance.register_plugin('xep_0199')
