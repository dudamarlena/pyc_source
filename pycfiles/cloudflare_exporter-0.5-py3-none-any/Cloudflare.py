...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/cloudflare_cli/Cloudflare.py
# Compiled at: 2017-08-27 08:18:16
import CloudFlare, json, string, re
...
class Cloudflare:
    ...
    def __init__(self):
        ...
        self.cf = CloudFlare.CloudFlare(email=os.environ['CLOUDFLARE_EMAIL'], token=os.environ['CLOUDFLARE_API'], raw=True)
        ...
        if record is None:
            ...
            try:
                if self.is_valid_ip(ip):
                    raw_dns_results = self.cf.zones.dns_records.post(self.zone_id, data={'type': 'A', 'name': host, 'content': ip})
                    print json.dumps(raw_dns_results, indent=5)
                else:
                    raw_dns_results = self.cf.zones.dns_records.post(self.zone_id, data={'type': 'CNAME', 'name': host, 'content': ip})
                    print json.dumps(raw_dns_results, indent=5)
            except CloudFlare.exceptions.CloudFlareAPIError as e:
            ...
            try:
                if self.is_valid_ip(ip):
                    ...
                    raw_dns_results = self.cf.zones.dns_records.put(record['zone_id'], record['id'], data={'id': record['id'], 'type': 'A', 'name': host, 'content': ip})
                    print json.dumps(raw_dns_results, indent=5)
                else:
                    print 'updating cname %s -> %s' % (ip, host)
                    raw_dns_results = self.cf.zones.dns_records.put(record['zone_id'], record['id'], data={'id': record['id'], 'type': 'CNAME', 'name': host, 'content': ip})
                    print json.dumps(raw_dns_results, indent=5)
            except CloudFlare.exceptions.CloudFlareAPIError as e:
...
def main():
    parser = OptionParser()
    parser.add_option('-i', '--ip', dest='ip', help='The IP address of the instance to use')
