...
# Compiled at: 2019-01-01 15:59:39
# Size of source mod 2**32: 14330 bytes
"""Tests for `trainline` package."""
import pytest, trainline
...
def test_class_Passenger():
    ...
    print()
    print(p1)
    if not p1.birthdate == '01/01/1980':
        ...
        if not p1.cards == []:
            ...
            id_p1 = p1.id
            assert len(id_p1) == 36
            assert p1.get_dict() == {'id':id_p1, 
             'age':age_p1, 
             'cards':[],  'label':id_p1}
            p2 = Passenger(birthdate='01/01/2006', cards=[
             trainline.JEUNE, trainline.WEEK_END])
            print(p2)
            ...
            id_p2 = p2.id
            assert len(id_p2) == 36
        assert p2.get_dict() == {'id':id_p2, 
         'age':age_p2, 
         'cards':[
          {'reference': trainline.JEUNE},
...
def test_get_station_id():
    ...
    if not station_id == TOULOUSE_STATION_ID:
        raise AssertionError
    else:
...
def test_search_3_passengers_and_bicyles():
    ...
    departure_station = 'Toulouse Matabiau'
    arrival_station = 'Bordeaux St-Jean'
    results = trainline.search(passengers=[
     Pierre, Sophie, Enzo],
    ...
    if not len(results) > 0:
        ...
        assert csv_header == 'departure_date;arrival_date;duration;number_of_segments;price;currency;transportation_mean;bicycle_reservation'
