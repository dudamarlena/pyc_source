...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nats/client.py
# Compiled at: 2014-08-31 03:53:57
"""Class: NatsClient
atributes:
...
from nats.error import NatsException, NatsClientException
...
class NatsClient(object):
    ...
    def publish(self, subject, msg=Protocol.EMPTY_MSG, opt_reply='', blk=None):
        ...
        if not subject:
            ...
            self.conn.send_command(('PUB {} {} {}{}{}{}').format(subject, opt_reply, len(msg), Protocol.CR_LF, msg, Protocol.CR_LF))
            if blk:
    ...
    def subscribe(self, subject, callback=None, **opts):
        ...
        if not subject:
            ...
            if 'max' in opts:
                sub['max'] = opts['max']
            self.conn.send_command(('SUB {} {} {}{}').format(subject, queue_str, sid, Protocol.CR_LF))
            self.subs[sid] = sub
            if 'max' in opts:
    ...
    def unsubscribe(self, sid, opt_max=None):
        ...
        self.conn.send_command(('UNSUB {}{}{}').format(sid, opt_max_str, Protocol.CR_LF))
        if sid not in self.subs:
            return
        else:
            sub = self.subs[sid]
