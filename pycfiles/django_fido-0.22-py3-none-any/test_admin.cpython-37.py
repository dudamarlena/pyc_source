...
# [GCC 8.4.0]
# Embedded file name: /home/jmusilek/github/django-fido/django_fido/tests/test_admin.py
# Compiled at: 2020-03-13 04:13:59
...
class TestFido2RegistrationRequestAdminView(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(USERNAME,
          first_name=USER_FIRST_NAME, last_name=USER_LAST_NAME, is_staff=True)
    ...
    def test_get(self):
        ...
        self.assertEqual(response.json()['publicKey']['user'], {'displayName':'Kryten 2X4B-523P', 
         'id':'kryten', 
         'name':'kryten'})
        self.assertEqual(response.json()['publicKey']['authenticatorSelection'], {'userVerification': 'discouraged'})
...
class TestAuthenticatorAddView(TestCase):
    ...
    def test_post(self, mock):
        self.client.force_login(self.superuser)
        response = self.client.post((self.url), data={'user':self.user.pk, 
        ...
        self.assertQuerysetEqual((Authenticator.objects.values_list('pk', 'user__username', 'attestation_data', 'label')),
          [
         (
