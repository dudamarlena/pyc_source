...
# [GCC 8.4.0]
# Embedded file name: /home/jmusilek/github/django-fido/django_fido/tests/test_views.py
# Compiled at: 2020-03-02 04:03:58
# Size of source mod 2**32: 16701 bytes
"""Test `django_fido.views` module."""
from __future__ import unicode_literals
import base64, json, fido2
...
from .data import ATTESTATION_OBJECT, AUTHENTICATION_CHALLENGE, AUTHENTICATION_CLIENT_DATA, AUTHENTICATOR_DATA, CREDENTIAL_ID, HOSTNAME, PASSWORD, REGISTRATION_CHALLENGE, REGISTRATION_CLIENT_DATA, SIGNATURE, USER_FIRST_NAME, USER_FULL_NAME, USER_LAST_NAME, USERNAME
from .utils import TEMPLATES
try:
    from fido2.webauthn import UserVerificationRequirement
except ImportError:
    import fido2.server as UserVerificationRequirement

User = get_user_model()

@override_settings(ROOT_URLCONF='django_fido.tests.urls')
class TestFido2RegistrationRequestView(TestCase):
    __doc__ = 'Test `Fido2RegistrationRequestView` class.'
    ...
    def _get_fido2_request(self, challenge, credentials):
        credential_params = [
         {'alg':-7, 
          'type':'public-key'}, {'alg':-8,  'type':'public-key'},
         {'alg':-37, 
          'type':'public-key'}, {'alg':-257,  'type':'public-key'}]
        rp_data = {'id':HOSTNAME, 
         'name':HOSTNAME}
        fido2_request = {'publicKey': {'rp':rp_data, 
                       'user':{'displayName':USER_FULL_NAME, 
...
class TestFido2RegistrationView(TestCase):
    __doc__ = 'Test `Fido2RegistrationView` class.'
    ...
    def test_post(self):
        ...
        session.save()
        response = self.client.post(self.url, {'client_data':REGISTRATION_CLIENT_DATA, 
...
class TestFido2AuthenticationRequestView(TestCase):
    __doc__ = 'Test `Fido2AuthenticationRequestView` class.'
    ...
    def test_get(self):
        ...
        state = self.client.session[FIDO2_REQUEST_SESSION_KEY]
        challenge = websafe_decode(state['challenge'])
        fido2_request = {'publicKey': {'rpId':'testserver',  'challenge':base64.b64encode(challenge).decode('utf-8'), 
                       'allowCredentials':[
                        {'id':CREDENTIAL_ID, 
                         'type':'public-key'}], 
                       'timeout':30000}}
...
@override_settings(ROOT_URLCONF='django_fido.tests.urls', TEMPLATES=TEMPLATES, AUTHENTICATION_BACKENDS=[
 'django_fido.backends.Fido2AuthenticationBackend'])
class TestFido2AuthenticationView(TestCase):
    __doc__ = 'Test `Fido2AuthenticationView` class.'
    ...
    def setUp(self):
        ...
        self.device = Authenticator.objects.create(user=(self.user), credential_id_data=CREDENTIAL_ID, attestation_data=ATTESTATION_OBJECT)
    ...
    def test_post(self):
        ...
        session[FIDO2_REQUEST_SESSION_KEY] = self.state
        session.save()
        post = {'client_data':AUTHENTICATION_CLIENT_DATA, 
         'credential_id':CREDENTIAL_ID,  'authenticator_data':AUTHENTICATOR_DATA, 
         'signature':SIGNATURE}
        ...
        self.assertQuerysetEqual((Authenticator.objects.values_list('user', 'counter')), [(self.user.pk, 152)], transform=tuple)
        self.assertNotIn(FIDO2_REQUEST_SESSION_KEY, self.client.session)
    ...
    def test_post_one_step(self):
        ...
        post = {'username':USERNAME, 
         'password':PASSWORD,  'client_data':AUTHENTICATION_CLIENT_DATA,  'credential_id':CREDENTIAL_ID, 
    ...
    def test_post_one_step_error(self):
        ...
        session.save()
        post = {'username':USERNAME, 
         'password':'wrong_password',  'client_data':AUTHENTICATION_CLIENT_DATA,  'credential_id':CREDENTIAL_ID, 
         'authenticator_data':AUTHENTICATOR_DATA,  'signature':SIGNATURE}
        response = self.client.post(self.url, post)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['form'].errors, {'__all__': ['Please enter a correct username and password and use valid FIDO2 security key.']})
        self.assertNotIn(FIDO2_REQUEST_SESSION_KEY, self.client.session)
        self.assertNotIn(AUTHENTICATION_USER_SESSION_KEY, self.client.session)
    ...
    def test_post_one_step_no_request(self):
        ...
        response = self.client.post(self.url, post)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['form'].errors, {'__all__': ['Authentication request not found.']})
        self.assertNotIn(FIDO2_REQUEST_SESSION_KEY, self.client.session)
        self.assertNotIn(AUTHENTICATION_USER_SESSION_KEY, self.client.session)