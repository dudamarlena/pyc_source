...
# [GCC 8.4.0]
# Embedded file name: /home/jmusilek/github/django-fido/django_fido/backends.py
...
class Fido2AuthenticationBackend(object):
    ...
    def authenticate(self, request: HttpRequest, user: AbstractBaseUser, fido2_server: Fido2Server, fido2_state: Dict[(str, bytes)], fido2_response: Dict[(str, Any)]) -> Optional[AbstractBaseUser]:
        """Authenticate using FIDO 2."""
        credentials = [a.credential for a in user.authenticators.all()]
        try:
            credential = fido2_server.authenticate_complete(fido2_state, credentials, fido2_response['credential_id'], fido2_response['client_data'], fido2_response['authenticator_data'], fido2_response['signature'])
...
class Fido2GeneralAuthenticationBackend(ModelBackend):
    __doc__ = 'Authenticated user using any username-password backend and Fido2AuthenticationBackend.'

    def __init__(self, *args, **kwargs):
        (super().__init__)(*args, **kwargs)
        self.fido_backend = Fido2AuthenticationBackend()

    def authenticate(self, request: HttpRequest, username: str, password: str, fido2_server: Fido2Server, fido2_state: Dict[(str, bytes)], fido2_response: Dict[(str, Any)], **kwargs) -> Optional[AbstractBaseUser]:
