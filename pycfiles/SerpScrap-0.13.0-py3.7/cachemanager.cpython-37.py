...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\scrapcore\cachemanager.py
...
class CacheManager:
    ...
    def clean_cache(self):
        ...
        if os.path.exists(cachedir):
            for file_name in os.listdir(cachedir):
                ...
                max_cache_time = 3600 * cache_time
    ...
    def cached_file_name(self, keyword, search_engine, scrape_mode, page_number):
        ...
        unique = [
         keyword, search_engine, scrape_mode, page_number]
        sha = hashlib.sha256()
        sha.update((b'').join((str(s).encode() for s in unique)))
        return '{file_name}.{extension}'.format(file_name=(sha.hexdigest()),
          extension='cache')
    ...
    def cache_results(self, parser, query, search_engine, scrape_mode, page_number, db_lock=None):
        ...
        if self.config.get('do_caching', False):
            if db_lock:
                db_lock.acquire()
            elif self.config.get('minimize_caching_files', False):
    ...
    def filter_scrape_jobs(self, scrape_jobs, session, scraper_search):
        ...
        for job in scrape_jobs:
            cache_name = self.cached_file_name(job['query'], job['search_engine'], job['scrape_method'], job['page_number'])
        ...
        self.logger.info('{} cache files found in {}'.format(len(files), self.config.get('cachedir')))
        self.logger.info('{}/{} objects have been read from the cache.\n        {} remain to get scraped.'.format(num_cached, num_total, num_total - num_cached))
    ...
    def get_serp_from_database(self, session, query, search_engine, scrape_method, page_number):
        try:
            serp = session.query(SearchEngineResultsPage).filter(SearchEngineResultsPage.query == query, SearchEngineResultsPage.search_engine_name == search_engine, SearchEngineResultsPage.scrape_method == scrape_method, SearchEngineResultsPage.page_number == page_number).first()
            return serp
        except Exception:
            return False