...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\scrapcore\parser\google_parser.py
# Compiled at: 2018-10-10 13:33:05
# Size of source mod 2**32: 6539 bytes
...
class GoogleParser(Parser):
    __doc__ = 'Parses SERP pages of the Google search engine.'
    ...
    page_number_selectors = [
     'div#foot div#navcnt td.cur::text']
    normal_search_selectors = {'results':{'items': {'container':'#center_col', 
     ...
     'videos':{'video_items': {'container':'g-inner-card', 
                      ...
                      'sitelinks':'div.osl::text'}}, 
     'news':{'news_items': {'container':'g-scrolling-carousel', 
                     ...
                     'rating':'div.osl a:first-of-type::text', 
                     'sitelinks':'div.osl::text'}}, 
     'shopping':{'shopping_items_main':{'container':'div.top-pla-group-inner', 
      ...
      'shopping_items_side':{'container':'div.cu-container', 
       'result_container':'div.mnr-c.pla-unit', 
       'link':'div.mnr-c.pla-unit a.pla-unit-title-link::attr(href)', 
       'snippet':'div.mnr-c.pla-unit::text', 
       'title':'div.mnr-c.pla-unit a.pla-unit-title-link > span::text', 
       ...
       'rating':'xxxx', 
       'sitelinks':'a.FfKHB > span::text'}}, 
     'ads_main':{'ads_item': {'container':'#center_col', 
     ...
     'related_keywords':{'related_items': {'container':'div.card-section', 
                        'result_container':'p.nVcaUb', 
                        'keyword':'a::text'}}}
    image_search_selectors = {'image': {'de_ip': {'container':'#isr_mc div.rg_di', 
        ...
        if self.searchtype == 'normal':
            ...
            if self.no_results is True:
                for key, i in self.iter_serp_items():
                    if 'snippet' in self.search_results[key][i] and self.query and self.query.replace('"', '') in self.search_results[key][i]['snippet']:
                        self.no_results = False
        ...
        if self.searchtype == 'image':
            for key, i in self.iter_serp_items():
                if self.search_results[key][i]:
                    ...
                    self.search_results[key][i] = {'link':meta_dict['ou'], 
                     'snippet':meta_dict['s'], 
                     'title':meta_dict['pt'], 
