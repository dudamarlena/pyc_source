...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\scrapcore\database.py
# Compiled at: 2017-12-08 06:46:26
# Size of source mod 2**32: 10079 bytes
"""
The database schema .
...
scraper_searches_serps = Table('scraper_searches_serps', Base.metadata, Column('scraper_search_id', Integer, ForeignKey('scraper_search.id')), Column('serp_id', Integer, ForeignKey('serp.id')))
...
class ScraperSearch(Base):
    ...
    serps = relationship('SearchEngineResultsPage',
        ...
        for key, value in parser.search_results.items():
            if isinstance(value, list):
                for link in value:
                    ...
                    [link.update({key: None}) for key in ('snippet', 'title', 'visible_link',
                                                          'rating', 'sitelinks') if key not in link]
                    Link(link=(link['link']),
        ...
        for key, value in parser.related_keywords.items():
            if isinstance(value, list) and len(value) > 0:
                for keyword in value:
                    ...
                    RelatedKeyword(keyword=(keyword['keyword']),
                      rank=(keyword['rank']),
                      serp=self)
...
class Link(Base):
    ...
    serp = relationship(SearchEngineResultsPage,
...
class SearchEngineProxyStatus(Base):
    __doc__ = 'Stores last proxy status for the given search engine.\n    A proxy can either work on a search engine or not.\n    '
...
def fixtures(config, session):
    ...
    for se in config.get('supported_search_engines', []):
        if se:
            ...
            search_engine or session.add(SearchEngine(name=se))
