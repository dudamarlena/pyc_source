...
# [GCC 8.4.0]
# Embedded file name: /Users/bilbo/Projects/envelopes/envelopes/tests/test_envelope.py
...
class Test_Envelope(BaseTestCase):
    ...
    def test_addrs_to_header(self):
        ...
        ok_header = 'test1@example.com,Test2 <test2@example.com>,Test3 <test3@example.com>'
    ...
    def test_raise(self):
        try:
            Envelope()._raise(RuntimeError, 'ęóąśłżźćń')
        except RuntimeError as exc:
            assert exc.args[0] == encoded('ęóąśłżźćń', 'utf-8')
        except:
            raise
        else:
    ...
    def test_to_mime_message_with_data(self):
        ...
        cc_header = 'cc1@example.com,Example CC2 <cc2@example.com>,Example CC3 <cc3@example.com>'
    ...
    def test_to_mime_message_with_many_to_addresses(self):
        ...
        mime_msg = envelope.to_mime_message()
        assert mime_msg is not None
        to_header = 'to1@example.com,Example To2 <to2@example.com>,Example To3 <to3@example.com>'
    ...
    def test_to_mime_message_unicode(self):
        msg = {'to_addr': ('to@example.com', 'ęóąśłżźćń'), 
           'from_addr': ('from@example.com', 'ęóąśłżźćń'), 
           'subject': 'ęóąśłżźćń', 
           'html_body': 'ęóąśłżźćń', 
           'text_body': 'ęóąśłżźćń', 
           'cc_addr': [
                     ('cc@example.com', 'ęóąśłżźćń')], 
           'bcc_addr': [
                      'ęóąśłżźćń <bcc@example.com>'], 
           'headers': {'X-Test': 'ęóąśłżźćń'}, 
           'charset': 'utf-8'}
        envelope = Envelope(**msg)
        ...
        assert mime_msg['To'] == enc_addr_header('ęóąśłżźćń', '<to@example.com>')
        assert mime_msg['From'] == enc_addr_header('ęóąśłżźćń', '<from@example.com>')
        assert mime_msg['CC'] == enc_addr_header('ęóąśłżźćń', '<cc@example.com>')
        assert 'BCC' not in mime_msg
    ...
    def test_send(self):
        envelope = Envelope(from_addr='spam@example.com', to_addr='eggs@example.com', subject='Testing envelopes!', text_body='Just a testy test.')
    ...
    def test_add_attachment(self):
        ...
        _something = self._tempfile(suffix='.something', prefix='ęóąśłżźćń')
        ...
        assert envelope._parts[5][1]['Content-Disposition'] == 'attachment; filename="%s"' % os.path.basename(encoded(_something, 'utf-8'))
    ...
    def test_repr(self):
        msg = self._dummy_message()
        envelope = Envelope(**msg)
        assert envelope.__repr__() == '<Envelope from="Example From <from@example.com>" to="Example To <to@example.com>" subject="I\'m a helicopter!">'