...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/prestapyt/prestapyt.py
# Compiled at: 2017-09-28 08:13:32
...
import urllib, warnings, requests, mimetypes, xml2dict, dict2xml
...
class PrestaShopWebService(object):
    ...
    def _check_status_code(self, status_code, content):
        ...
        """
        message_by_code = {204: 'No content', 400: 'Bad Request', 
           401: 'Unauthorized', 
           404: 'Not Found', 
        ...
        if status_code in (200, 201):
            return True
        if status_code == 401:
            raise PrestaShopAuthenticationError(message_by_code[status_code], status_code)
        elif status_code in message_by_code:
            ps_error_code, ps_error_msg = self._parse_error(content)
            raise PrestaShopWebServiceError(message_by_code[status_code], status_code, ps_error_msg=ps_error_msg, ps_error_code=ps_error_code)
        else:
            ps_error_code, ps_error_msg = self._parse_error(content)
            raise PrestaShopWebServiceError('Unknown error', status_code, ps_error_msg=ps_error_msg, ps_error_code=ps_error_code)
    ...
    def _check_version(self, version):
        ...
        """
        if version:
            if not LooseVersion(self.MIN_COMPATIBLE_VERSION) <= LooseVersion(version) <= LooseVersion(self.MAX_COMPATIBLE_VERSION):
                warnings.warn('This library may not be compatible with this version of PrestaShop (%s). Please upgrade/downgrade this library' % (
                 version,))
        return True
    ...
    def _execute(self, url, method, data=None, add_headers=None):
        ...
        try:
            response = self.client.request(method, url, data=data, headers=request_headers)
        finally:
            if self.verbose:
    ...
    def _parse(self, content):
        ...
        try:
            ...
            raise PrestaShopWebServiceError('HTTP XML response is not parsable : %s. %s' % (
             e, content[:512]))
    ...
    def _validate_query_options(self, options):
        ...
        unsupported = set([ param.split('[')[0] for param in options
                          ]).difference(supported)
        if unsupported:
            raise PrestaShopWebServiceError('Unsupported parameters: %s' % ((', ').join(tuple(unsupported)),))
        return True
...
class PrestaShopWebServiceDict(PrestaShopWebService):
    ...
    def search(self, resource, options=None):
        ...
        response = super(PrestaShopWebServiceDict, self).search(resource, options=options)
...
if __name__ == '__main__':
    ...
    address_data['address'].update({'address1': '1 Infinite Loop', 'address2': '', 
       'alias': 'manufacturer', 
       'city': 'Cupertino', 
       'company': '', 
       'deleted': '0', 
