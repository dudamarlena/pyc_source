...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/bdgenomics/workflows/spark.py
# Compiled at: 2017-09-18 01:42:06
import logging, multiprocessing, os, subprocess, time
from toil.job import Job
from toil.lib.docker import STOP, dockerCheckOutput
_log = logging.getLogger(__name__)
_SPARK_MASTER_PORT = '7077'

def spawn_spark_cluster(job, numWorkers, cores=None, memory=None, disk=None, overrideLeaderIP=None):
    """
    :param numWorkers: The number of worker nodes to have in the cluster.     Must be greater than or equal to 1.
    :param cores: Optional parameter to set the number of cores per node.     If not provided, we use the number of cores on the node that launches     the service.
    ...
    leaderService = SparkService(cores=cores, memory=memory, disk=disk, overrideLeaderIP=overrideLeaderIP)
    leaderIP = job.addService(leaderService)
    for i in range(numWorkers):
        job.addService(WorkerService(leaderIP, cores=cores, disk=disk, memory=memory), parentService=leaderService)

    return leaderIP


def _checkContainerStatus(sparkContainerID, hdfsContainerID, sparkNoun='leader', hdfsNoun='namenode'):
    containers = subprocess.check_output(['docker', 'ps', '-q'])
    shortSpark = sparkContainerID[0:11]
    shortHdfs = hdfsContainerID[0:11]
    if sparkContainerID not in containers and shortSpark not in containers or hdfsContainerID not in containers and shortHdfs not in containers:
...
class SparkService(Job.Service):
    ...
    def __init__(self, memory=None, disk=None, cores=None, overrideLeaderIP=None):
        """
    ...
    def start(self, job):
        ...
        self.sparkContainerID = dockerCheckOutput(job=job, defer=STOP, workDir=os.getcwd(), tool='quay.io/ucsc_cgl/apache-spark-master:2.1.1--acd08bd3e5670502636fb6842b777eb2aff6def7', dockerParameters=[
         '--net=host',
         '-d',
         '-v', '/mnt/ephemeral/:/ephemeral/:rw',
         '-e', 'SPARK_MASTER_IP=' + self.hostname,
         '-e', 'SPARK_LOCAL_DIRS=/ephemeral/spark/local',
         '-e', 'SPARK_WORKER_DIR=/ephemeral/spark/work'], parameters=[
         self.hostname])[:-1]
        _log.info('Started HDFS Datanode.')
        self.hdfsContainerID = dockerCheckOutput(job=job, defer=STOP, workDir=os.getcwd(), tool='quay.io/ucsc_cgl/apache-hadoop-master:2.7.4--9a9cdee76b65a50e63ad2ca8f9db6da2bd11965c', dockerParameters=[
         '--net=host',
         '-d'], parameters=[
         self.hostname])[:-1]
...
class WorkerService(Job.Service):
    ...
    def start(self, job):
        ...
        :param job: The underlying job.
        """
        self.sparkContainerID = dockerCheckOutput(job=job, defer=STOP, workDir=os.getcwd(), tool='quay.io/ucsc_cgl/apache-spark-worker:2.1.1--acd08bd3e5670502636fb6842b777eb2aff6def7', dockerParameters=[
         '--net=host',
         '-d',
         '-v', '/mnt/ephemeral/:/ephemeral/:rw',
         '-e',
         ...
         '-e', 'SPARK_LOCAL_DIRS=/ephemeral/spark/local',
         '-e', 'SPARK_WORKER_DIR=/ephemeral/spark/work'], parameters=[
         self.masterIP + ':' + _SPARK_MASTER_PORT])[:-1]
    ...
    def __start_datanode(self, job):
        ...
        :param job: The underlying job.
        """
        self.hdfsContainerID = dockerCheckOutput(job=job, defer=STOP, workDir=os.getcwd(), tool='quay.io/ucsc_cgl/apache-hadoop-worker:2.7.4--9a9cdee76b65a50e63ad2ca8f9db6da2bd11965c', dockerParameters=[
         '--net=host',
         '-d',
         '-v', '/mnt/ephemeral/:/ephemeral/:rw'], parameters=[
         self.masterIP])[:-1]
