...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/bdgenomics/workflows/tools/aligners.py
...
def run_star(job, r1_id, r2_id, star_index_url, wiggle=False, sort=True):
    ...
    assert sort and (
     os.stat(aligned_bam_path).st_size > 0, 'Aligned bam failed to sort. Ensure sufficient memory is free.')
    ...
    if wiggle:
        ...
        return (
         transcriptome_id, aligned_id, wiggle_id, log_id, sj_id)
    else:
        return (
         transcriptome_id, aligned_id, log_id, sj_id)
...
def run_bwakit(job, config, sort=True, trim=False, mark_secondary=False):
    ...
    work_dir = job.fileStore.getLocalTempDir()
    rg = None
    inputs = {'ref.fa': config.ref, 'ref.fa.fai': config.fai, 
    ...
    if getattr(config, 'rg_line', None):
        ...
        if all(getattr(config, elem, None) for elem in ['library', 'platform', 'program_unit', 'uuid']):
            rg = ('@RG\\tID:{0}').format(config.uuid)
    ...
    dockerCall(job=job, tool='quay.io/ucsc_cgl/bwakit:0.7.12--c85ccff267d5021b75bb1c9ccf5f4b79f91835cc', parameters=parameters, workDir=work_dir)
