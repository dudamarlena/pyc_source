...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/bdgenomics/workflows/tools/spark_tools.py
...
def _make_parameters(master_ip, default_parameters, memory, arguments, override_parameters):
    ...
    """
    require((override_parameters is not None or memory is not None) and (override_parameters is None or memory is None), 'Either the memory setting must be defined or you must provide Spark configuration parameters.')
    parameters = []
...
def call_conductor(job, master_ip, src, dst, memory=None, override_parameters=None):
    ...
    arguments = [
     '-C', src, dst]
    docker_parameters = [
     '--log-driver', 'none', master_ip.docker_parameters(['--net=host'])]
    dockerCall(job=job, tool='quay.io/ucsc_cgl/conductor', parameters=_make_parameters(master_ip, [], memory, arguments, override_parameters), dockerParameters=docker_parameters)
...
def call_adam(job, master_ip, arguments, memory=None, override_parameters=None, run_local=False, native_adam_path=None):
    ...
    if native_adam_path is None:
        ...
        check_call([
         os.path.join(native_adam_path, 'bin/adam-submit')] + default_params + arguments)
    return


def call_deca(job, master_ip, arguments, memory=None, override_parameters=None, work_dir=None, run_local=False, aws_access_key_id=None, aws_secret_access_key=None):
    """
    ...
    if aws_access_key_id:
        require(aws_secret_access_key, 'If AWS access key is passed, secret key must be defined')
    ...
    dockerCall(job=job, tool='quay.io/ucsc_cgl/deca:0.1.0--7d13833a1220001481c4de0489e893c93ee3310f', dockerParameters=docker_parameters, parameters=_make_parameters(master_ip, default_params, memory, arguments, override_parameters))