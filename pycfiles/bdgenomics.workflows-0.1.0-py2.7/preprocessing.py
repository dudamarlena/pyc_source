...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/bdgenomics/workflows/tools/preprocessing.py
...
def run_cutadapt(job, r1_id, r2_id, fwd_3pr_adapter, rev_3pr_adapter):
    ...
    dockerCall(job=job, tool='quay.io/ucsc_cgl/cutadapt:1.9--6bd44edd2b8f8f17e25c5a268fedaab65fa851d2', workDir=work_dir, parameters=parameters)
    if r1_id and r2_id:
...
def run_samtools_faidx(job, ref_id):
    ...
    dockerCall(job=job, workDir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e')
...
def run_samtools_index(job, bam):
    ...
    dockerCall(job=job, workDir=work_dir, parameters=parameters, tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e')
...
def _log_runtime(job, start, end, cmd):
    ...
    minutes = int(elapsed_time - 3600 * hours) / 60
    seconds = int(elapsed_time - 3600 * hours - 60 * minutes) % 60
...
def run_samtools_sort(job, bam):
    ...
    dockerCall(job=job, workDir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/samtools:1.3--256539928ea162949d8a65ca5c79a72ef557ce7c')
    end_time = time.time()
...
def run_samblaster(job, sam):
    ...
    command = ['/usr/local/bin/samblaster',
     ...
     '--ignoreUnmated']
    start_time = time.time()
    dockerCall(job=job, workDir=work_dir, parameters=command, tool='quay.io/biocontainers/samblaster:0.1.24--0')
    end_time = time.time()
...
def run_picard_create_sequence_dictionary(job, ref_id):
    ...
    dockerCall(job=job, workDir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e')
...
def run_gatk_preprocessing(job, bam, bai, ref, ref_dict, fai, g1k, mills, dbsnp, realign=False, unsafe=False):
    ...
    base_recal_disk = PromisedRequirement(lambda bam_, bai_, ref_size: bam_.size + bai_.size + 2 * ref_size, bqsr_input_bam, bqsr_input_bai, bqsr_ref_size)
    base_recal = job.wrapJobFn(run_base_recalibration, bqsr_input_bam, bqsr_input_bai, ref, ref_dict, fai, dbsnp, mills, unsafe=unsafe, cores=job.cores, disk=base_recal_disk, memory=job.memory)
    recalibrate_reads_disk = PromisedRequirement(lambda bam_, bai_, recal, ref_size: 2 * (bam_.size + bai_.size) + recal.size + ref_size, bqsr_input_bam, bqsr_input_bai, base_recal.rv(), genome_ref_size)
    recalibrate_reads = job.wrapJobFn(apply_bqsr_recalibration, base_recal.rv(), bqsr_input_bam, bqsr_input_bai, ref, ref_dict, fai, unsafe=unsafe, cores=job.cores, disk=recalibrate_reads_disk, memory=job.memory)
    job.addChild(mdups)
