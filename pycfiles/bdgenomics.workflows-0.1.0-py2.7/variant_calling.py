...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/bdgenomics/workflows/avocado_pipeline/variant_calling.py
...
from bdgenomics.workflows.tools.spark_tools import call_adam, call_avocado, call_conductor, MasterAddress, HDFS_MASTER_PORT, SPARK_MASTER_PORT
...
def download_run_and_upload(job, master_ip, inputs, spark_on_toil):
    ...
    try:
        ...
        if not inputs.run_local:
            ...
            call_conductor(job, master_ip, inputs.sample, hdfs_bam, container='fnothaft/conductor', memory=inputs.memory)
        else:
        ...
        call_adam(job, master_ip, [
         'transform', hdfs_bam, adam_input], memory=inputs.memory, run_local=inputs.run_local, container='fnothaft/adam')
        avocado_output = hdfs_prefix + '.gt.adam'
        _log.info('Calling variants with avocado.')
        call_avocado(job, master_ip, [
         'biallelicGenotyper', '-is_not_grc', adam_input, avocado_output], memory=inputs.memory, container='fnothaft/avocado')
        ...
        call_adam(job, master_ip, [
         ...
         '-single',
         '-sort_on_save',
         '-stringency', 'LENIENT'], memory=inputs.memory, run_local=inputs.run_local, container='fnothaft/adam')
        out_file = inputs.output_dir + '/' + sample_name + inputs.suffix + '.bam'
        if not inputs.run_local:
            _log.info('Uploading output VCF %s to %s.', output_vcf, out_file)
            call_conductor(job, master_ip, output_vcf, out_file, memory=inputs.memory, container='fnothaft/conductor')
...
def static_avocado_dag(job, inputs, sample, output_dir, suffix=''):
    ...
    if inputs.master_ip is not None or inputs.run_local:
        ...
        master_ip = spawn_spark_cluster(job, inputs.num_nodes - 1, cores=cores, memory=inputs.memory, sparkMasterContainer='fnothaft/apache-spark-master', sparkWorkerContainer='fnothaft/apache-spark-worker')
...
def main():
    ...
    parser_run.add_argument('--config', default='avocado.config', type=str, help='Path to the (filled in) config file, generated with "generate-config". \nDefault value: "%(default)s"')
    ...
    if args.command == 'generate-config':
        ...
        Job.Runner.startToil(Job.wrapJobFn(static_avocado_dag, inputs, args.sample, args.output_dir), args)
    return
