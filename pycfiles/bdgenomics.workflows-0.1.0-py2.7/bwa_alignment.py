...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/bdgenomics/workflows/cannoli_pipeline/bwa_alignment.py
...
from bdgenomics.workflows.tools.spark_tools import call_adam, call_cannoli, call_conductor, MasterAddress, HDFS_MASTER_PORT, SPARK_MASTER_PORT
...
def download_run_and_upload(job, master_ip, inputs, spark_on_toil):
    ...
    try:
        ...
        if not inputs.run_local:
            ...
            call_conductor(job, master_ip, inputs.sample, hdfs_reads, container='fnothaft/conductor', memory=inputs.memory)
            index_exts = [
            ...
            for ext in index_exts:
                ...
                call_conductor(job, master_ip, index_path, hdfs_index_ext, container='fnothaft/conductor', memory=inputs.memory)
            ...
            call_conductor(job, master_ip, sd_path, hdfs_sd, container='fnothaft/conductor', memory=inputs.memory)
        else:
        ...
        call_cannoli(job, master_ip, [
         ...
         '-index', hdfs_index,
         '-add_indices',
         '-sequence_dictionary', hdfs_sd], memory=inputs.memory, container='fnothaft/cannoli:1508-1509')
        out_file = inputs.output_dir + '/' + sample_name + inputs.suffix + '.bam'
        if not inputs.run_local:
            _log.info('Uploading output BAM %s to %s.', aligned_output, out_file)
            call_conductor(job, master_ip, aligned_output, out_file, memory=inputs.memory, container='fnothaft/conductor')
...
def static_cannoli_dag(job, inputs, sample, sample_id, output_dir, suffix=''):
    ...
    if inputs.master_ip is not None or inputs.run_local:
        spark_on_toil = False
        spark_work = job.wrapJobFn(download_run_and_upload, inputs.master_ip, inputs, spark_on_toil)
        job.addChild(spark_work)
    else:
        spark_on_toil = True
        cores = multiprocessing.cpu_count()
        master_ip = spawn_spark_cluster(job, inputs.num_nodes - 1, cores=cores, memory=inputs.memory, sparkMasterContainer='fnothaft/apache-spark-master:2.1.0--74e45e9a58550e14db0e1ad48624c839ebd5e8f8', sparkWorkerContainer='fnothaft/apache-spark-worker:2.1.0--74e45e9a58550e14db0e1ad48624c839ebd5e8f8')
        spark_work = job.wrapJobFn(download_run_and_upload, master_ip, inputs, spark_on_toil)
        job.addChild(spark_work)
    return
...
def main():
    ...
    parser_run.add_argument('--config', default='cannoli.config', type=str, help='Path to the (filled in) config file, generated with "generate-config". \nDefault value: "%(default)s"')
    parser_run.add_argument('--sample-id', help='The sample ID.')
    parser_run.add_argument('--sample', help='The S3 URL or local path to the input SAM or BAM file.NOTE: unlike other pipelines, we do not support ftp://, gnos://, etc. schemes.')
    parser_run.add_argument('--output-dir', required=True, default=None, help='full path where final results will be output')
    parser_run.add_argument('-s', '--suffix', default='', help='Additional suffix to add to the names of the output files')
    ...
    if args.command == 'generate-config':
        ...
        require(not (inputs.master_ip and inputs.num_nodes > 0), 'Only one of master_ip and num_nodes can be provided.')
        if not hasattr(inputs, 'master_ip'):
            require(inputs.num_nodes > 1, 'num_nodes allocates one Spark/HDFS master and n-1 workers, and thus must be greater than 1. %d was passed.' % inputs.num_nodes)
        for arg in [inputs.memory]:
            require(arg, ('Required argument {} missing from config').format(arg))

        Job.Runner.startToil(Job.wrapJobFn(static_cannoli_dag, inputs, args.sample, args.sample_id, args.output_dir), args)
    return
