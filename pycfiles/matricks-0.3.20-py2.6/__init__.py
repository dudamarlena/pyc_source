...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/matricks/__init__.py
# Compiled at: 2012-07-23 01:35:23
r"""
...
We'll use a small, representative dataset that is nothing more than a list of lists.  
 ...
 >>> test_raw_data = [ ['probe_id','ABC(12)','ABC(13)','DEFC0N(1)','CDDB4(5)','Jovi(1)','F774Lin(3)'],
...
TSV, instance creation would look like this:
 ...
 >>> test_raw_data_csv = ''' probe_id,ABC(12),ABC(13),DEFC0N(1),CDDB4(5),Jovi(1),F774Lin(3)
 ... ELMT_3401602,7.14727114229,1.682159,6.6022379846,6.6318799406,6.63021747852,6.57620493019
 ... ELMT_3401603,6.6469632681,1.682159,6.63635120703,6.70026291599,6.67341553263,6.66361340118
 ... ELMT_3401605,9.33488740366,1.682159,9.7365656865,8.88887581915,8.70271863949,9.39432724993
 ... ELMT_3401607,6.65137398038,1.682159,6.75639196465,6.70203184527,7.05207191931,6.96818993978
...
Suppose we take a small subste of our test data::
 ...
 >>> test_raw_data_2 = [ ['probe_id','ABC(12)','ABC(14)'],
 ... ['ELMT_3401602','7.14727114229','1.682159'],
 ... ['ELMT_3401603','6.6469632681','1.682159'],
 ... ]
...
"""
    _distFuncs = {'c': distance.pearson, 'a': lambda a, b: distance.pearson(a, b, absolute=True), 
    ...
    def __init__(self, *args, **kwargs):
        ...
        def parse_from_seq(rawlist, cvt):
            ...
            def apply_cvt(s, cnvrzn):
                if s == null or isinstance(s, (str, unicode)) and len(s) < 1:
                    return
                else:
                    try:
                        v = cnvrzn(s)
            ...
            return rawlist[:self._skiprows] + [ list(l[:self._skipcols]) + map(lambda c: apply_cvt(c, cvt), l[self._skipcols:])
             for l in rawlist[self._skiprows:]
                                              ]
        ...
        if len(args) == 0:
            ...
            if isinstance(args[0], Matricks):
                ...
                elif (rawdata.startswith(b'\x80\x02c') or rawdata.startswith('ccopy_reg')) and rawdata.split()[1] == self.__class__.__name__:
    ...
    @staticmethod
    def _trim(emx):
        r"""Strip leading rows that are empty and leading (left) columns that are empty. 
...
"""
    ...
    @property
    def data(self):
        return [ row[self._skipcols:] for row in self._data[self._skiprows:] ]
...
"""
        ...
        for s in lvec:
            if isinstance(s, int):
                result.append(self._labels[s])
            elif isinstance(s, (str, unicode)):
                if kwargs.get('re', False) and self._re_pat.search(s):
                    pat = re.compile(re.sub('([^\\\\])\\)', '\\1\\)', re.sub('([^\\\\])\\(', '\\1\\(', s)))
                    sublist = list()
                ...
                elif folded(s) in folded_labels:
                    if complement:
...
"""
        if isinstance(sl, slice):
            if isinstance(sl.start, (float, int)):
            ...
            if isinstance(rawkeys, str):
                rawkeys = [
                 rawkeys]
            elif isinstance(rawkeys, (float, int)):
                rawkeys = list(self._labels)
        ...
        keys = self.getLabels(flat=True, cmpl=kwargs.get('cmpl', False), re=kwargs.get('re', False), *rawkeys)
        try:
        ...
        if not isinstance(sl, slice):
            selected = map(lambda d: [ d[ki] for ki in key_ndx ], self._data)
            if data_only:
                return selected[self._skiprows:]
            return self.__class__(selected, purge=kwargs.get('purge', False), skipcols=new_skipcols, skiprows=self._skiprows, keycol=new_keycol)
        else:
            ...
            if data_only:
                return filtered
            return self.__class__(filtered, purge=kwargs.get('purge', False), skipcols=new_skipcols, skiprows=self._skiprows, keycol=new_keycol)
...
"""
    ...
    def __str__(self):
        ...
        col_list = [
         set(range(len(self.labels[:3]))), set(range(len(self.labels) - 3, len(self.labels)))]
        if col_list[0] & col_list[1]:
            col_list = list(col_list[0] | col_list[1])
        else:
            if len(self._labels) == 0:
                return '[[ ... ]]'
            col_list = list(col_list[0]) + [None] + list(col_list[1])
...
"""
    ...
    def scored(self, sc_cls, thresh=None, agg=None, label='score', null=None, sort=True, reverse=True, modal=False, cleanup=True, exclude=None):
...
"""
        if not isinstance(sc_cls, Scorer):
            return
        else:
            if agg is None or not isinstance(agg, FunctionType):
                agg = lambda x: x
...
"""
    ...
    def correlated(self, *rowRef, **kwargs):
        r"""Returns a `Matricks` instance in which the rows have been rearranged according to 
...
reference first, followed by the rest in ascending distance order.
 ...
 >>> distTestData = ''' ... pid\tC1\TC2\TC3\tC4\tC5
 ... A\t1.\t2.\t3.\t4.\t5.
 ... B\t5.\t4.\t3.\t2.\t1.
 ... C\t6.\t8.\t10.\t12.\t14.
 ... D\t.04\t.05\t.06\t.07\t.08
...
"""
        ...
        for r in self._data[self._skiprows:]:
            ...
            if dist is not None:
                result.append(r + [round(dist, digs)])

        r2 = sorted(result, key=lambda x: x[(-1)], reverse=kwargs.get('anti', False))
        if kwargs.get('clean', True):
            return self.__class__(self._data[:self._skiprows] + [ x[:-1] for x in r2 ], skipcols=self._skipcols, skiprows=self._skiprows)
        else:
            return self.__class__([ x + ['distance'] for x in self._data[:self._skiprows] ] + r2, skipcols=self._skipcols, skiprows=self._skiprows)
            return
...
"""
        if isinstance(k, (str, unicode)):
            ...
            if isinstance(k, int) and k >= 0 and k < self.__len__():
...
"""
        ...
        while len(biglist) > 1:
            ...
            ppmc_list = sorted([ (ppmo(filter(None, x[self._skipcols:])), x) for x in biglist[1:] if nullmap_tbl[x[0]] == ref_nullmap ], key=lambda x: x[0])
...
"""
        ...
        for r in self._data[self._skiprows:]:
            try:
                new_row = r[:self._skipcols] + [ f[1](r) for f in item_map ]
...
"""
        ...
        result = self.__class__([self.getLabels()] + sorted(self._data[self._skipcols:], cmp=cmp, key=lambda v: v[skey], reverse=reverse), skipcols=self._skipcols)
        return result
...
"""
        ...
        if junc in ('con', '&'):
            ...
            RH_sc = [ i for i in range(other._skipcols) if i != other._keycol if other._labels[i] in int_labels ]
            RH_nsc = [ i for i in range(other._skipcols, len(other._labels)) if i != other._keycol if other._labels[i] in int_labels ]
        elif junc in ('mod', '%'):
            ...
            RH_sc = [ i for i in range(other._skipcols) if i != other._keycol if other._labels[i] in mod_labels ]
            RH_nsc = [ i for i in range(other._skipcols, len(other._labels)) if i != other._keycol if other._labels[i] in mod_labels ]
            if self._keycol not in LH_sc:
                LH_sc.insert(0, self._keycol)
        if self._keycol not in LH_sc:
            LH_sc.insert(0, self._keycol)
        result = list()
        result.append([ self._labels[i] for i in LH_sc ] + [ other._labels[j] for j in RH_sc ] + [ self._labels[k] for k in LH_nsc ] + [ other._labels[t] for t in RH_nsc ])
        ...
        for r in self._data[self._skiprows:]:
            ...
            if not other._keyindex.has_key(key):
                ...
                oth_r_set = [
                 [
                  key] + [outer] * (len(other.labels) - 1)]
            else:
                oth_r_set = [ other._data[i] for i in other._keyindex[key] ]
            last_row = list(nones)
            for oth in oth_r_set:
                new_row = [ r[i] for i in LH_sc ] + [ oth[j] for j in RH_sc ] + [ r[k] for k in LH_nsc ] + [ oth[t] for t in RH_nsc ]
                if new_row != last_row:
...
"""
        result = {'aaData': self._data[offset:offset + limit] if limit is not None else self._data[offset:], 'aoColumns': [ {'sTitle': x, 'sName': x} for x in self._labels ], 'sColumns': self.getLabels(), 
...
"""
        ...
        for r in rows:
            if isinstance(r, (list, tuple)):
                clump = self.extractRows(rcrs=True, keycol=keycol, *r)
            elif isinstance(r, (str, unicode)):
                ndx = keyindex.get(r.upper(), [])
                clump = [ self._data[i] for i in ndx ]
            elif isinstance(r, slice):
                r = slice(r.start + self._skiprows if r.start is not None else self._skiprows, r.stop + self._skiprows if r.stop is not None else len(self._data), r.step)
                clump = self._data[r]
            elif isinstance(r, int):
                r = r + self._skiprows
                clump = self._data[r:r + 1]
            elif kwargs.get('exc', True) is True:
        ...
        if rcrs:
            ...
            return self.__class__(self._data[:self._skiprows] + result, skipcols=self._skipcols, skiprows=self._skiprows)
            return
