...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/matricks/__main__.py
# Compiled at: 2011-05-16 04:07:30
import logging
log = logging.getLogger('sample')
import re, sys, optparse, cfgparse, time
sys.path.extend(('..', '.'))
import matricks
default_cfg = '[DEFAULT]\n\n'
...
converters = {'float': float, 
   'int': int, 
   'integer': int, 'upper': lambda x: x.upper(), 
   'lower': lambda x: x.lower()}
formatters = {'s': lambda m: repr(m), 
...
def main():
    ...
    if opts.input is None or len(opts.input) < 1:
        ...
        if timing:
            log.info('matricks source inhaled in %6.4f s' % (time.time() - t0))
    if timing:
        t0 = time.time()
    conv = converters[opts.convert.lower()] if opts.convert is not None and opts.convert.lower() in converters.keys() else None
    inMx = matricks.Matricks(opts.input, fsep=opts.fsep, rsep=opts.rsep, skipcols=opts.skipcols, skiprows=opts.skiprows, cvt=conv)
    if timing:
        log.info('matricks created in %6.4f s' % (time.time() - t0))
    if timing:
        t0 = time.time()
    ...
    if timing:
        log.info('matricks spewed in %6.4f s' % (time.time() - t0))
    return 0
