...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/widget/form.py
# Compiled at: 2010-10-22 05:18:39
"""
...
__licence__ = 'BSD'
__all__ = [
 'FieldHolder', 'BaseForm', 'Form', 'ListForm', 'TableForm',
...
class AttrFieldsMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        new_class = super(AttrFieldsMetaclass, cls).__new__(cls, name, bases, attrs)
        return new_class
...
class FieldHolder(object):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, int):
            ...
            if len(fls) <= key:
                raise IndexError('This instance has %s field(s). given value(%s) is greater than that' % (
                 len(fls), key))
            return fls[key]
...
class BaseForm(FieldHolder):
    ...
    confirm_js = 'if(confirm("%s") == true) {location.replace(%s); return false;}else{return false;}'
    ...
    def __init__(self, action='', method='', submit='', clear='', id='', multipart=False, clas='', enginename='', form_title='', fields=[]):
        """
        A initialize method.
        """
    ...
    def get_attr_string(self):
        ...
        attrs = ''
        if self.method:
            attrs += 'method = "%s" ' % self.method
        if self.action:
            attrs += 'action = "%s" ' % self.action
        if self.clas:
            attrs += 'class = "%s" ' % self.clas
        if self.id:
            attrs += 'id = "%s" ' % self.id
        if self.multipart:
    ...
    def render_field(self, field, values={}, errors={}, translate=unicode):
        ...
        if isinstance(field, FieldHolder):
            ...
            fieldbody = uc(field.render_body(values.get(field.get_name(), None), translate=translate))
            if not field.RENDER_WRAPPER:
                return fieldbody
            c = {}
            ...
            if title and field.USE_FIELD_TITLE:
                ...
                if getattr(field, 'collapsable', False):
                    c['label'] += "<img src = '/style/img/collapsed.gif'class = 'toggle-button'/>"
                c['label'] += '</label>'
            c['field'] = fieldbody
    ...
    def render(self, values={}, errors={}, translate=unicode):
        ...
        context = {'attrs': attrs, 'fields': fields, 'buttons': buttons, 'form_title': '', 
           '_': translate}
        if self.form_title:
...
class Fieldset(BaseForm, BaseField):
    ...
    def render_body(self, values={}, errors={}, translate=unicode):
        ...
        c = {}
        if self.clas:
            c['attrs'] = "class = '%s'" % self.clas
        c['fields'] = fields
        c['title'] = self.title
        c['attrs'] = ''
        c['clas'] = self.clas
        c['_'] = translate
        return uc(th.render(c, self.enginename, tid=self.FSID), 'utf-8', 'replace')
...
class ULFieldset(Fieldset):
    ...
    def __init__(self, title='', name='', clas='flat', enginename='', fields=[]):
        """
        A initialize method.
        """
        self.title = title
    ...
    def render_field(self, field, values={}, errors={}, translate=unicode):
        ...
        if isinstance(field, FieldHolder):
            ...
            context['field'] = uc(field.render_body(values.get(field.get_name(), None), translate=translate))
            e = errors.get(field.get_name(), None)
            if e:
...
class SelectiveFieldset(Fieldset):
    ...
    def __init__(self, title='', name='', clas='flat', enginename='', fields=[], innerfields=[]):
        """
        A initialize method.
        """
        self.required = False
    ...
    def render_body(self, values={}, errors={}, translate=unicode):
        ...
        if self.get_name() in values:
            try:
                ...
                v = [ x for x in self.innerfields if x[0] == sel
                    ][0]
                sub_v = {}
                for k in d:
                    if k != '_selection':
...
class Form(BaseForm):
    ...
    def render(self, values={}, errors={}, translate=unicode):
        ...
        attrs = self.get_attr_string()
        buttons = self.get_button_string(translate)
        context = {'title': translate(self.button_title), 'name': '', 'error': '', 'description': ''}
        context['field'] = buttons
        context['cls'] = 'controlset'
        buttons = '<div class = "submit">%s</div>' % buttons
        context2 = {'attrs': attrs, 'fields': fields, 'buttons': buttons, 'form_title': '', 
           '_': translate}
        if self.form_title:
