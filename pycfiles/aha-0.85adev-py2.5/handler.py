...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/widget/handler.py
# Compiled at: 2010-10-22 05:18:24
"""
handler.py
...
import os
__all__ = ('get_tag', 'MediaHandler', 'TemplateHandler', 'TemplateEngine', 'templatehandler')
TEMPLATES = {'text/css': '<link rel = "stylesheet" type = "text/css" href = "%s" />', 
   'text/javascript': '<script type = "text/javascript" src = "%s"></script>'}
...
class TemplateHandler(object):
    ...
    def add_engine(self, engine):
        ...
        if not isinstance(engine, TemplateEngine):
            raise TypeError("The argument 'engine' must be a subclass of TemplateEngine")
...
class TemplateEngine(object):
    ...
    def get_template(self, path='', string='', tid=''):
        ...
        """
        if path and string:
            raise ValueError("Arguments 'path', 'string' must not be set at the same time")
...
class MakoTemplateEngine(TemplateEngine):
    ...
    def __init__(self, extension=DEFAULT_EXTENSION, dirs=[], charset='utf-8'):
        ...
        if self._charset.lower() != 'utf-8':
            du = True
        self.tlookup = TemplateLookup(directories=self.templdirs, disable_unicode=du, input_encoding=self._charset, output_encoding=self._charset)
        self.ext = extension
    ...
    def get_template(self, path='', string='', tid=''):
        ...
        if path:
            ...
            t = Template(string, input_encoding=self._charset, output_encoding=self._charset)
