...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/model/lazyloadingmodel.py
# Compiled at: 2010-10-22 05:16:15
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for n in keys:
            if isinstance(attrs[n], LazyPropertyBase):
                attrs[KEY_PREFIX + n] = db.StringProperty(required=False, default='')
                lazyprops.append(n)

        attrs['lazy_properties'] = lazyprops
        new_class = super(LazyLoadingMetaclass, cls).__new__(cls, name, bases, attrs)
        return new_class
