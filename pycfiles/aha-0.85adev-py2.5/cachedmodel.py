...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/model/cachedmodel.py
# Compiled at: 2010-10-22 05:16:19
...
class CachedModelBase(db.Model):
    ...
    def cache(self, dont_override_add_props=False):
        ...
        memcache.set(str(key), od, self.EXPIRE, namespace=CM_NAMESPACE)
        return
    ...
    def put(self, cache=True):
        ...
        if hasattr(self, '_entity'):
            ...
            if cache:
                self.cache()
        elif hasattr(self, '_internal_key'):
...
class CachedQuery(object):
    ...
    def fetch(self, limit, offset=0, force=False):
        ...
        if not done:
            ...
            if str_f not in kl:
                kl.append(str_f)
                kl = memcache.set('keys', kl, self.cls.EXPIRE, namespace=CQ_NAMESPACE)
            for i in r:
