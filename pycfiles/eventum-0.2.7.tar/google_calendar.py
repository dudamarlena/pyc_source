...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/lib/google_calendar.py
...
NO_CREDENTIALS = 'Failed to find the Google Calendar credentials file at `{}`, please create it by running:\n\n    $ python manage.py --authorize\nThe environment variable EVENTUM_GOOGLE_AUTH_ENABLED is currently set to `{}`.  If set to FALSE, Google Calendar calls will fail silently.'
...
class GoogleCalendarAPIClient:
    ...
    def __init__(self, app):
        ...
        try:
            ...
            if self.app.config['EVENTUM_GOOGLE_AUTH_ENABLED']:
                ...
                gae_environ = 'FALSE'
            print >> stderr, NO_CREDENTIALS.format(self.app.config['EVENTUM_INSTALLED_APP_CREDENTIALS_PATH'], gae_environ)
            exit(1)
    ...
    def before_request(self):
        ...
        """
        self.service = self._get_service()
        self.private_calendar_id = self.app.config['EVENTUM_PRIVATE_CALENDAR_ID']
        self.public_calendar_id = self.app.config['EVENTUM_PUBLIC_CALENDAR_ID']
    ...
    def _get_service(self):
        ...
        """
        storage = Storage(self.app.config['EVENTUM_INSTALLED_APP_CREDENTIALS_PATH'])
    ...
    def create_event(self, event):
        ...
        request = self.service.events().insert(calendarId=calendar_id, body=resource)
    ...
    def update_event(self, stale_event, as_exception=False):
        ...
        request = self.service.events().update(calendarId=calendar_id, eventId=event_id_for_update, body=resource)
        try:
    ...
    def publish_event(self, stale_event):
        ...
        return self.move_event(event, from_id=self.private_calendar_id, to_id=self.public_calendar_id)
    ...
    def unpublish_event(self, stale_event):
        ...
        return self.move_event(event, from_id=self.public_calendar_id, to_id=self.private_calendar_id)
    ...
    def move_event(self, event, from_id, to_id):
        ...
        request = self.service.events().move(calendarId=from_id, eventId=event.gcal_id, destination=to_id)
        try:
    ...
    def delete_event(self, event, as_exception=False):
        ...
        if as_exception:
            ...
            request = self.service.events().update(calendarId=calendar_id, eventId=instance['id'], body=instance)
        else:
            current_app.logger.info('[GOOGLE_CALENDAR]: Delete Event')
            request = self.service.events().delete(calendarId=calendar_id, eventId=event.gcal_id)
        try:
            return self._execute_request(request)
        except EventumError.GCalAPI.NotFound as e:
    ...
    def _instance_resource_for_event_in_series(self, event):
        ...
        page_token = None
        while True:
            request = self.service.events().instances(calendarId=calendar_id, eventId=event.gcal_id, pageToken=page_token)
    ...
    def _execute_request(self, request):
        ...
        try:
            ...
            raise EventumError.GCalAPI.BadStatusLine(message=e.message, line=e.line)
