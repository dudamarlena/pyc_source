...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/models/User.py
...
from mongoengine import Document, StringField, ReferenceField, EmailField, ListField, URLField, DictField, DateTimeField
from eventum.lib.regex import Regex
from eventum.models import BaseEventumDocument
now = datetime.now
USER_TYPES = {'fake_user': {'edit': False, 
                 'publish': False, 
                 'admin': False}, 
   'editor': {'edit': True, 
              'publish': False, 
...
class User(Document, BaseEventumDocument):
    ...
    slug = StringField(required=True, max_length=510, unique=True, regex=Regex.SLUG_REGEX)
    ...
    def get_profile_picture(self, size=50):
        ...
        if not self.image_url:
            return url_for('eventum.static', filename=current_app.config['EVENTUM_DEFAULT_PROFILE_PICTURE'])
