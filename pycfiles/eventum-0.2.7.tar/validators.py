...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/forms/validators.py
...
def image_with_same_name(form, field):
    ...
    if Image.objects(filename=field.data).count() != 1:
        return ValidationError(message=("Can't find image '{}' in the database").format(field.data))
...
class UniqueEmail(object):
    ...
    def __call__(self, form, field):
        ...
        :raises: :class:`wtforms.validators.ValidationError`
        """
        if form.user_type.data != 'fake_user' and Whitelist.objects(email=field.data).count():
            raise ValidationError(self.message)