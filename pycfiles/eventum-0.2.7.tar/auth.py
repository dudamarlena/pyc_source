...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/routes/auth.py
...
from oauth2client.client import FlowExchangeError, flow_from_clientsecrets, AccessTokenCredentials
...
def login():
    ...
    **Methods:** ``GET``
    ...
    if g.user is not None and 'gplus_id' in session:
        ...
        return render_template('eventum_auth/login.html', client_id=client_id, state=session['state'], next=next)
...
def store_token():
    ...
    **Methods:** ``POST``
    ...
    code = request.data
    try:
        oauth_flow = flow_from_clientsecrets(current_app.config['EVENTUM_CLIENT_SECRETS_PATH'], scope='')
        oauth_flow.redirect_uri = 'postmessage'
        credentials = oauth_flow.step2_exchange(code)
    except FlowExchangeError:
        return json_error_message('Failed to upgrade the authorization code.', 401)
    ...
    if User.objects(gplus_id=gplus_id).count() == 0:
        ...
        people_document = gplus_service.people().get(userId='me').execute(http=http)
        email = people_document['emails'][0]['value']
        if Whitelist.objects(email=email).count() != 1:
            return json_error_message('User has not been whitelisted.', 401, {'whitelisted': False, 'email': email})
        return json_success({'redirect_url': url_for('.create_profile', next=request.args.get('next'), name=people_document['displayName'], email=email, image_url=people_document['image']['url'])})
...
def create_profile():
    ...
    **Methods:** ``GET, POST``
    ...
    if g.user is not None and 'gplus_id' in session:
        ...
        form = CreateProfileForm(request.form, name=request.args['name'], email=request.args['email'], next=request.args['next'])
        if form.validate_on_submit():
            if User.objects(email=form.email.data).count() != 0:
                user = User.objects.get(email=form.email.data)
                user.openid = session['openid']
                user.name = form.name.data
                flash('Account with this email already exists.  Overridden.', MESSAGE_FLASH)
                user.register_login()
                user.save()
            else:
                ...
                wl.redeemed = True
                wl.save()
                user = User(email=form.email.data, name=form.name.data, gplus_id=session['gplus_id'], user_type=user_type, image_url=request.args.get('image_url'))
        ...
        return render_template('eventum_auth/create_profile.html', image_url=request.args.get('image_url'), form=form)
...
def disconnect():
    ...
    **Methods:** ``GET, POST``
    """
    credentials = AccessTokenCredentials(session.get('credentials'), request.headers.get('User-Agent'))
    if credentials is None:
        return json_error_message('Current user not connected.', 401)
    else:
        access_token = credentials.access_token
        url = ('https://accounts.google.com/o/oauth2/revoke?token={}').format(str(access_token))
        h = httplib2.Http()
