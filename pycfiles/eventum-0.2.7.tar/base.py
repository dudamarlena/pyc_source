...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/routes/base.py
...
"""
import sys
from flask import g, session, render_template, request, redirect, flash, url_for, current_app
...
def lookup_current_user():
    ...
    if not current_app.config.get('EVENTUM_GOOGLE_AUTH_ENABLED'):
        ...
        try:
            ...
            user = User(name='Super User', gplus_id=SUPER_USER_GPLUS_ID, user_type='admin', email='email@email.com')
            user.save()
...
def register_error_handlers(blueprint):
    ...
    def exception_handler(error):
        ...
        if not current_app.config['DEBUG'] and request.path.startswith('/admin'):
            flash(('An uncaught error occured: {}').format(error.strerror), ERROR_FLASH)
            return redirect(url_for('admin.index'))
        current_app.handle_exception(error)

    @blueprint.errorhandler(400)
    def bad_request(error):
        """Handle 400 errors."""
        return (
         render_template('eventum_error/400.html'), 400)

    @blueprint.errorhandler(401)
    def not_authorized(error):
        """Handle 401 errors."""
        return (
         render_template('eventum_error/401.html'), 401)

    @blueprint.errorhandler(403)
    def forbidden(error):
        """Handle 403 errors."""
        return (
         render_template('eventum_error/403.html'), 403)

    @blueprint.errorhandler(404)
    def not_found(error):
        """Handle 404 errors."""
        return (
         render_template('eventum_error/404.html'), 404)

    @blueprint.errorhandler(405)
    def method_not_allowed(error):
        """Handle 405 errors."""
        return (
         render_template('eventum_error/405.html', method=request.method), 405)
