...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/routes/users.py
...
from flask import Blueprint, render_template, request, flash, session, g, redirect, url_for, abort
...
def index():
    ...
    **Methods:** ``GET``
    ...
    return render_template('eventum_users/users.html', whitelist_form=whitelist_form, upload_form=upload_form, whitelist=Whitelist.objects(redeemed=False), users=User.objects(), images=Image.objects(), current_user=g.user)
...
def user(slug):
    ...
    **Methods:** ``GET, POST``
    ...
    form = EditUserForm(request.form, name=user.name, email=user.email, user_type=user.user_type)
    ...
    return render_template('eventum_users/user.html', user=user, form=form, current_user=g.user)
...
def become(level=0):
    ...
    **Methods:** ``GET``
    ...
    """
    level = int(level)
    admin_privileges = {'edit': level > 0, 
       'publish': level > 1, 
       'admin': level > 2}
    db_dict = dict((('set__privileges__{}').format(k), v) for k, v in admin_privileges.iteritems())
