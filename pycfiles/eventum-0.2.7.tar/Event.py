...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/models/Event.py
# Compiled at: 2016-04-19 10:40:04
"""
.. module:: Event
...
from mongoengine import Document, DateTimeField, StringField, ReferenceField, BooleanField, IntField, ValidationError
...
class Event(Document, BaseEventumDocument):
    ...
    def clean(self):
        ...
        if self.start_date and self.end_date and self.start_date > self.end_date:
            raise ValidationError('Start date should always come before end date. Got (%r,%r)' % (
             self.start_date,
             self.end_date))
        if self.start_date == self.start_time and self.start_time is not None and self.end_time is not None and self.start_time > self.end_time:
            raise ValidationError('Start time should always come before end time. Got (%r,%r)' % (
             self.start_time,
             self.end_time))
        return
    ...
    def image_url(self):
        ...
        return url_for('eventum.static', filename=current_app.config['EVENTUM_DEFAULT_EVENT_IMAGE'])
    ...
    def human_readable_time(self):
        ...
        :rtype: str
        """
        return ('{}-{}').format(self._human_readable_start_time(), self._human_readable_end_time())
    ...
    def human_readable_datetime(self):
        ...
        """
        if self.start_date:
            start_date = self.start_date.strftime('%A, %B %d ').replace(' 0', ' ')
        else:
            start_date = '???, ??/?? '
        ...
        if self.end_date:
            if not self.start_date or self.start_date != self.end_date:
                end_date = self.end_date.strftime('%A, %B %d ').replace(' 0', ' ')
            else:
                end_date = ''
        else:
            end_date = '???, ??/?? '
        ...
        if not end_date:
            separator = '-'
        return ('{}{}{}{}{}').format(start_date, start_time, separator, end_date, end_time)
    ...
    def _start_and_end_time_share_am_or_pm(self):
        ...
        :rtype: bool
        """
        return self.start_time is not None and self.end_time is not None and not self.is_multiday() and self.start_time.strftime('%p') == self.end_time.strftime('%p')
    ...
    def to_jsonifiable(self):
        ...
        """
        attrs = [
         'date_created', 'date_modified', 'title', 'location', 'slug',
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return 'Event(title=%r, location=%r, creator=%r, start=%r, end=%r, published=%r)' % (
         self.title, self.location, self.creator,
