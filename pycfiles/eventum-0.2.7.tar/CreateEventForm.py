...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/forms/CreateEventForm.py
...
from wtforms import StringField, DateField, TextAreaField, BooleanField, SelectField, IntegerField, RadioField
from wtforms.validators import Required, ValidationError, Optional, NumberRange, Regexp, URL
from eventum.forms.validators import UniqueEvent
from eventum.lib.regex import Regex
SHORT_DESCRIPTION_PLACEHOLDER = 'Short Description.  This should be **one to two** sentences long.'
LONG_DESCRIPTION_PLACEHOLDER = 'Long Description.  This should be **four to five** sentences.  Feel free to include [links](http://adicu.com).'
...
class CreateEventForm(Form):
    ...
    frequency = SelectField('Repeats', choices=[('weekly', 'Weekly')], default='weekly')
    every = IntegerField('Every', [NumberRange(min=1, max=30)], default=1)
    ends = RadioField('Ends', choices=[('after', 'After'), ('on', 'On')], default='after')
    num_occurrences = IntegerField('Every', [NumberRange(min=1)], default=1)
    recurrence_end_date = DateField('Repeat End Date', [Optional()], format=DATE_FORMAT)
    recurrence_summary = StringField('Summary')
    short_description = TextAreaField('Short description', default=SHORT_DESCRIPTION_PLACEHOLDER)
    long_description = TextAreaField('Long description', default=LONG_DESCRIPTION_PLACEHOLDER)
    ...
    def post_validate(self, validation_stopped):
        ...
        if not validation_stopped:
            ...
            if all([start_date, start_time, end_date, end_time]) and start_time > end_time:
