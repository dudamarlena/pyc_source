...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/routes/posts.py
...
"""
from flask import Blueprint, render_template, request, send_from_directory, abort, redirect, url_for, g, flash, current_app
...
def new():
    ...
    **Methods:** ``POST``
    ...
    form.author.choices = [ (str(u.id), u.name + ' (You)' if u == g.user else u.name) for u in User.objects()
                          ]
    ...
    if form.validate_on_submit():
        ...
        post = BlogPost(title=form.title.data, slug=form.slug.data, images=images, markdown_content=form.body.data, author=author, posted_by=g.user, post_tags=tags)
        post.save()
    ...
    return render_template('eventum_posts/edit.html', user=g.user, form=form, images=images, upload_form=upload_form)
...
def edit(post_id):
    ...
    **Methods:** ``GET, POST``
    ...
    form = CreateBlogPostForm(request.form, title=post.title, slug=post.slug, published=post.published, body=post.markdown_content, images=[ image.filename for image in post.images ], author=str(post.author.id), featured_image=feat_img, tags=post.post_tags)
    form.author.choices = [ (str(u.id), u.name + ' (You)' if u == g.user else u.name) for u in User.objects()
                          ]
    form.author.default = str(g.user.id)
    images = [ image for image in Image.objects() if image not in post.images ]
    return render_template('eventum_posts/edit.html', user=g.user, form=form, post=post, images=images, upload_form=upload_form)
...
def fetch_epiceditor_themes(folder, path):
    ...
    **Methods:** ``GET``
    ...
    """
    return send_from_directory(current_app.config['EVENTUM_STATIC_FOLDER'], ('css/lib/epiceditor/{folder}/{path}').format(folder=folder, path=path))