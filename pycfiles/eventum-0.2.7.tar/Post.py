...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/models/Post.py
...
from mongoengine import Document, DateTimeField, ReferenceField, StringField, BooleanField, ListField
...
class Post(Document, BaseEventumDocument):
    ...
    categories = ListField(StringField(db_field='category', max_length=255), default=list)
        ...
        if self.images:
            for image in self.images:
                if image.filename in self.html_content:
                    self.html_content = self.html_content.replace('src="' + image.filename + '"', 'src="' + image.url() + '"')
