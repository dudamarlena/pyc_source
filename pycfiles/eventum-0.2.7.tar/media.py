...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/routes/media.py
...
"""
import os
from flask import Blueprint, request, redirect, url_for, render_template, send_from_directory, g, flash, jsonify, current_app
...
def index():
    ...
    **Methods:** ``GET``
    ...
    return render_template('eventum_media/media.html', images=images, form=form)
...
def allowed_file(filename):
    ...
    :rtype: bool
    """
    return '.' in filename and os.path.splitext(filename)[1] in current_app.config['EVENTUM_ALLOWED_UPLOAD_EXTENSIONS']
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        if f:
            ...
            default_path = os.path.join(current_app.config['EVENTUM_UPLOAD_FOLDER'], filename)
            image = Image(filename=filename, default_path=default_path, creator=g.user)
            image.save()
...
def file(filename):
    ...
    **Methods:** ``GET``
    ...
    """
    return send_from_directory(current_app.config['EVENTUM_UPLOAD_FOLDER'], filename)
