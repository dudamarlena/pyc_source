...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/models/BlogPost.py
...
class BlogPost(Post):
    ...
    def get_related_posts(self):
        ...
        for tag in self.post_tags:
            for obj in BlogPost.objects(post_tags=tag, published=True, id__ne=self.id, featured_image__ne=None):
                if obj not in related_posts:
                    related_posts[obj] = 1
                else:
                    related_posts[obj] += 1

        sorted_posts = [ x[0] for x in sorted(related_posts.items(), key=itemgetter(1), reverse=True) ]
        return sorted_posts
