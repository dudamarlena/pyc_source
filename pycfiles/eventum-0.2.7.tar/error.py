...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/lib/error.py
...
_ERROR_DATA = {'GCalAPI': (
             ...
             {'MissingID': (
                            ...
                            {'FellBackToCreate': (
                                                  ...
                                                  611, HTTP_OK, {})}), 
                'BadStatusLine': (
                                'Encountered a BadStatusLine error with the Google Calendar API.',
                ...
                'NotFound': (
                           ...
                           {'UpdateFellBackToCreate': (
                                                       "Couldn't find an event to update. Successfully fell back to create.",
                                                       641, HTTP_OK, {}), 
                              'MoveFellBackToCreate': (
                                                     "Couldn't find an event to move. Successfully fell back to create.",
                                                     642, HTTP_OK, {})}), 
                'PublishFailed': (
                                'Failed to Publish Event',
                                650, HTTP_INTERNAL_SERVER_ERROR,
                                {'PublishedFalse': (
                                                    'Event must have the `published` set to `True`  before publishing.',
                                                    651, HTTP_INTERNAL_SERVER_ERROR, {}), 
                                   'PublishedTrue': (
                                                   'Event must have the `published` set to `True`  before unpublishing.',
                                                   652, HTTP_INTERNAL_SERVER_ERROR, {})}), 
                'EventMustEndOnOrAfter': (
                                        'A series should either end on a specific date, or after a specific number of occurences.  Series.ends_on and ends_after are both false.',
...
class EventumError(Exception):
    ...
    def log_error(self):
        ...
        if self.data:
            current_app.logger.error(('[{}][DATA]: {}').format(self.error_type, self.data))
    ...
    def _form_message(self, message, subs):
        ...
        if n_subs > len(subs):
            current_app.logger.warning(('[{}._form_message]: {}').format(self.error_type, 'Not enough subs provided'))
            subs = subs + ('', ) * (n_subs - len(subs))
        if len(subs) > n_subs:
            current_app.logger.warning(('[{}._form_message]: {}').format(self.error_type, 'Too many subs provided'))
            subs = subs[:n_subs]
        if subs:
