...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/working/eventum/eventum/routes/events.py
...
from eventum.forms import CreateEventForm, EditEventForm, DeleteEventForm, UploadImageForm
...
def index():
    ...
    **Methods:** ``GET``
    ...
    past_events, this_week, next_week, future_events = _get_events_for_template(past, future)
    return render_template('eventum_events/events.html', past_events=past_events, this_week=this_week, next_week=next_week, future_events=future_events)
...
def _get_events_for_template(past, future):
    ...
    this_week = Event.objects(start_date__gte=last_sunday, start_date__lt=next_sunday).order_by('start_date')
    next_week = Event.objects(start_date__gte=next_sunday, start_date__lt=following_sunday).order_by('start_date')
    past_events = []
    future_events = []
    for week_no in range(past):
        ...
        events = Event.objects(start_date__gte=starting_sunday, start_date__lt=ending_sunday)
        past_events.insert(0, {'week_name': week_name, 
           'events': events})
    ...
    for week_no in range(future):
        ...
        week_name = _format_for_display(starting_sunday)
        events = Event.objects(start_date__gte=starting_sunday, start_date__lt=ending_sunday)
        future_events.append({'week_name': week_name, 
           'events': events})
...
def create():
    ...
    **Methods:** ``GET, POST``
    ...
    return render_template('eventum_events/create.html', form=form, delete_form=delete_form, upload_form=upload_form, images=images)
...
def edit(event_id):
    ...
    **Methods:** ``GET, POST``
    ...
    form = EditEventForm(event, request.form) if request.method == 'POST' else EventsHelper.create_form(event, request)
    if form.validate_on_submit():
        try:
    ...
    upload_form = UploadImageForm()
    images = Image.objects()
    return render_template('eventum_events/edit.html', form=form, event=event, delete_form=delete_form, upload_form=upload_form, images=images)
...
def set_published_status(event_id, status):
    ...
    if Event.objects(id=object_id).count() == 1:
        ...
        if status != event.published:
            ...
            flash('The event had not been published.  No changes made.', MESSAGE_FLASH)
    else:
        flash('Invalid event id', ERROR_FLASH)
    return redirect(url_for('.index'))
