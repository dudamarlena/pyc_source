...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlalchemy_hawq/ddl.py
# Compiled at: 2019-10-07 13:43:57
# Size of source mod 2**32: 4820 bytes
"""
    ...
    if table_opts['appendonly'] is not None:
        ...
        if table_opts['orientation'] is not None:
            if str(table_opts['orientation']).upper() not in {'ROW', 'PARQUET'}:
                raise ValueError('orientation ({}) must be one of {}'.format(table_opts['orientation'], {'ROW', 'PARQUET'}))
        ...
        if table_opts['compresstype'] is not None:
            if table_opts['compresstype'] not in compresstype_values:
                raise ValueError('compresstype ({}) must be one of {}'.format(table_opts['compresstype'], compresstype_values))
        ...
        if table_opts['compresslevel'] is not None:
            if table_opts['compresslevel'] < 0 or table_opts['compresslevel'] > 9:
                raise ValueError('compresslevel ({}) must be an integer between 0-9'.format(table_opts['compresslevel']))
    ...
    def create_table_constraints(self, table, _include_foreign_key_constraints=None):
        ...
        for constraint in constraints:
            if constraint._create_rule is None or constraint._create_rule(self):
                if not (self.dialect.supports_alter and getattr(constraint, 'use_alter', False)):
