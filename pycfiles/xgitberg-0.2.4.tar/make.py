...
# Compiled at: 2016-02-09 11:21:13
"""
...
class LocalRepo:
    ...
    def commit(self, message):
        with CdContext(self.book.local_path):
            try:
                sh.git('commit', '-m', ('"{message}"').format(message=message))
...
class NewFilesHandler:
    ...
    def template_readme(self):
        ...
        readme_text = template.render(title=self.book.meta.title, author=self.book.meta.author, book_id=self.book.book_id)
        readme_path = ('{0}/{1}').format(self.book.local_path, self.README_FILENAME)
