...
# [GCC 8.4.0]
# Embedded file name: /Users/ivan.afonichkin/Programming/mlflow/mlflow/models/container/__init__.py
# Compiled at: 2020-02-17 11:56:18
...
import multiprocessing, os, signal, shutil
from subprocess import check_call, Popen
import sys
from pkg_resources import resource_filename
import mlflow, mlflow.version
...
def _init(cmd):
    ...
    if cmd == 'serve':
        ...
        if cmd == 'train':
            ...
            raise Exception('Unrecognized command {cmd}, full args = {args}'.format(cmd=cmd, args=(str(sys.argv))))
...
def _install_pyfunc_deps(model_path=None, install_mlflow=False):
    ...
    if has_env:
        if install_mlflow:
            install_mlflow_cmd = [
             'pip install /opt/mlflow/.' if _container_includes_mlflow_source() else 'pip install mlflow=={}'.format(MLFLOW_VERSION)]
...
def _serve_pyfunc(model):
    ...
    cmd = 'gunicorn -w {cpu_count} '.format(cpu_count=cpu_count) + '${GUNICORN_CMD_ARGS} mlflow.models.container.scoring_server.wsgi:app'
...
def _serve_mleap():
    serve_cmd = [
     'java', '-cp', '"/opt/java/jars/*"', 'org.mlflow.sagemaker.ScoringServer',
