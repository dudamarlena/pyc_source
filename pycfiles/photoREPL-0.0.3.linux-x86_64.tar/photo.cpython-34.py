...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/photorepl/photo.py
# Compiled at: 2015-07-12 11:01:45
...
class Photo(Raw):
    ...
    def __setattr__(self, name, value):
        if name == 'options' and type(value) is Options:
            self.__dict__['options'] = AutoUpdatingOptions(attrs=dict(zip(value.keys(), value.values())), photo=self)
            try:
                self.update()
            except AttributeError:
                pass
    ...
    def show(self):
        ...
        """
        Gdk.threads_enter()
        self.preview = self.ui_thread.open_window(self.tempfile, rawfile=self.filename)
        Gdk.threads_leave()
