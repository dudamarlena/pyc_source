...
# [GCC 8.4.0]
# Embedded file name: /Users/zapix/projects/django-confirmaction/build/lib/confirmaction/main.py
# Compiled at: 2014-07-08 11:57:28
import json
...
def _import_func(func_addr):
    if '.' not in func_addr:
        raise ValueError("'%s' is not a func address" % func_addr)
    try:
        module = import_module(('.').join(func_addr.split('.')[:-1]))
    except ImportError:
...
def get_action_func(func_addr):
    ...
    if not hasattr(func, 'confirm_action') or not func.confirm_action:
        raise exceptions.NotActionException('%s not an action' % func_addr)
    return func


def set_action(user_contact, func_addr, func_kwargs=None, message_template=None, template_context=None, live_time=None, generate_code_func=None, send_code_func=None):
    """
    ...
    message_template = message_template or app_settings.ACTIVATION_MESSAGE_TEMPLATE
    template_context = template_context or {}
    template_context.update({'code': code})
    try:
        send_code_func(user_contact, render_to_string(message_template, template_context))
    except Exception as e:
        raise exceptions.DidNotSendMessage(str(e))

    action = models.Action.objects.create(user_contact=user_contact, action_func=func_addr, confirm_code=SHA256.new(settings.SECRET_KEY + code).hexdigest(), live_time=live_time or app_settings.CODE_LIVE_TIME)
    models.ActionArg.objects.bulk_create([ models.ActionArg(action=action, name=str(key), value=str(value)) for key, value in func_kwargs.iteritems()
                                         ])
    return action
...
def apply_action(action_pk, code):
    ...
    if not action.is_actual():
        raise exceptions.OutOfDate('Action out of date')
    if not action.confirm_code == code:
        raise exceptions.WrongCode('Wrong code for action')
    if action.action_status != models.Action.NOT_CONFIRMED:
        raise exceptions.UsedAction('This action has been used')
