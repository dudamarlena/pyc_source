...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/knyfe/perm_test.py
# Compiled at: 2012-05-22 08:52:32
    ...
    class Progress:
        ...
        def set(self, value):
            ...
            if value >= self.max:
                ...
                sys.stdout.write('\r' + '|' * p50 + '\x1b[30m' + '·' * (50 - p50) + '\x1b[0m %02d%% %s' % (p50 * 2, self.label))
                sys.stdout.flush()
...
def two_sample(A, B, limit=10000, progress_bar=None):
    ...
    n_comb = np.multiply.reduce(np.array(range(len(data) - len(A) + 1, len(data) + 1))) / np.multiply.reduce(np.array(range(1, len(A) + 1)))
    n_comb = limit or total_perms
    counts[2] = min(2 * counts[0:2].min(), n_comb)
