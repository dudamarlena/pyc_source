...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/knyfe/knyfe.py
# Compiled at: 2012-06-13 14:22:27
"""
    knyfe
...
import logging, random, copy, glob, tablib, csv
__title__ = 'knyfe'
__version__ = '0.4.3'
...
class Data:
    ...
    def _combine_labels(self, other, template='{self} + {other}'):
        ...
        if hasattr(other, 'label'):
            if self.label() == 'Unnamed Dataset':
                ...
                if other.label() == 'Unnamed Dataset':
                    return self.label()
    ...
    def __sub__(self, other):
        ...
        if type(other) not in (list, tuple):
            other = other.data
        diff = [ sample for sample in self.data if sample not in other ]
        new_data.data = diff
        return new_data
    ...
    def __isub__(self, other):
        if type(other) not in (list, tuple):
            other = other.data
        self.data = [ sample for sample in self.data if sample not in other ]
        return self
    ...
    def filter(self, _func=__default, **kwargs):
        ...
        if _func is not None and callable(_func):
            ...
            if _func is not None and type(_func) is str:
                return self.filter((lambda trial: trial.has_key(_func) and trial[_func]), **kwargs)
            if _func is not None and type(_func) is np.ndarray and _func.dtype is np.dtype('bool'):
        ...
        def _guess_scale(value):
            if type(value) is float:
                ...
                if type(value) is int:
                    return Data.ORDINAL
                if type(value) in (str, unicode, bool):
                    return Data.NOMINAL
