...
# [GCC 8.4.0]
# Embedded file name: /Users/samba/Projects/Personal/briesemeister.me/virtenv/lib/python2.7/site-packages/tackle/wsgi.py
# Compiled at: 2016-02-11 18:03:12
...
class WSGIRoute(object):
    RE_PARSE_PATH = re.compile('<([a-zA-Z_]+)?(?::([^>]+))?>')
    ...
    @cached_property
    def template(self):
        base = self.RE_PARSE_PATH.sub('{\\1}', self.path)
...
class WSGIService(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if request_host in self.hostmap:
            ...
            return [
             'The requested site (%s) is not currently configured.' % request_host]
