...
# [GCC 8.4.0]
# Embedded file name: /Users/samba/Projects/Personal/briesemeister.me/virtenv/lib/python2.7/site-packages/tackle/middleware.py
    ...
    def run_before(self, environ, start_response):
        ...
        for pattern, target, permanent in self._map:
            ...
            if match is not None:
                ...
                if self.retain_path:
                    parts[2] = info.path
                kwargs.update({'hostname': info.hostname, 
                   'path': info.path, 
                   'query': info.query, 
...
class StaticFileMiddleware(Middleware):
    ...
    match_content_types = [
     (
      '\\.js$', 'text/javascript', default_cache_life),
     (
      '\\.json$', 'application/json', default_cache_life),
     ('\\.txt$', 'text/plain', 600),
     (
      '\\.css$', 'text/css', default_cache_life)]
