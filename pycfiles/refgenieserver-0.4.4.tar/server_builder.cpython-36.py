...
# [GCC 8.4.0]
# Embedded file name: /Users/mstolarczyk/Uczelnia/UVA/code/refgenieserver/refgenieserver/server_builder.py
# Compiled at: 2020-01-15 13:05:10
...
def archive(rgc, registry_paths, force, remove, cfg_path, genomes_desc):
    ...
    if float(rgc[CFG_VERSION_KEY]) < float(REQ_CFG_VERSION):
        raise ConfigNotCompliantError('You need to update the genome config to v{} in order to use the archiver. The required version can be generated with refgenie >= {}'.format(REQ_CFG_VERSION, REFGENIE_BY_CFG[REQ_CFG_VERSION]))
    else:
        cfg_archive_folder_key = CFG_ARCHIVE_KEY if CFG_ARCHIVE_KEY in rgc else CFG_ARCHIVE_KEY_OLD
        if CFG_ARCHIVE_CONFIG_KEY in rgc:
            ...
            try:
                ...
                raise GenomeConfigFormatError("The config '{}' is missing a {} entry. Can't determine the desired archive.".format(cfg_path, ' or '.join([CFG_ARCHIVE_KEY, CFG_ARCHIVE_KEY_OLD])))

            if os.path.isfile(server_rgc_path):
            ...
            if os.path.exists(server_rgc_path):
                ...
                genomes or _LOGGER.error('No genomes found')
                exit(1)
            else:
    ...
    for genome in genomes:
        ...
        genome_desc = rgc[CFG_GENOMES_KEY][genome].setdefault(CFG_GENOME_DESC_KEY, DESC_PLACEHOLDER) if genomes_desc is None or genome not in descs else descs[genome]
        ...
        for asset_name in assets if isinstance(assets, list) else [assets]:
            asset_desc = rgc[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset_name].setdefault(CFG_ASSET_DESC_KEY, DESC_PLACEHOLDER)
            ...
            for tag_name in tags if isinstance(tags, list) else [tags]:
                if not rgc.is_asset_complete(genome, asset_name, tag_name):
                    ...
                    parents = rgc[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset_name][CFG_ASSET_TAGS_KEY][tag_name].setdefault(CFG_ASSET_PARENTS_KEY, [])
                    children = rgc[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset_name][CFG_ASSET_TAGS_KEY][tag_name].setdefault(CFG_ASSET_CHILDREN_KEY, [])
                    seek_keys = rgc[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset_name][CFG_ASSET_TAGS_KEY][tag_name].setdefault(CFG_SEEK_KEYS_KEY, {})
                    asset_digest = rgc[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset_name][CFG_ASSET_TAGS_KEY][tag_name].setdefault(CFG_ASSET_CHECKSUM_KEY, None)
                        ...
                        try:
                            ...
                            with rgc_server as (r):
                                for parent in parents:
                                    _LOGGER.debug('updating {} children list with {}'.format(parent, '{}/{}:{}'.format(genome, asset_name, tag_name)))
                                    ...
                                    try:
                                        ...
                                        _LOGGER.warning("'{}/{}:{}'s parent '{}' does not exist, skipping relationship updates".format(genome, asset_name, tag_name, parent))
                                        continue
...
def _copy_log(input_dir, target_dir, asset_name, tag_name):
    ...
    if log_path:
        if os.path.exists(log_path):
            run(('cp ' + log_path + ' ' + os.path.join(target_dir, TEMPLATE_LOG.format(asset_name, tag_name))),
              shell=True)
