...
# [GCC 8.4.0]
# Embedded file name: /Users/mstolarczyk/Uczelnia/UVA/code/refgenieserver/refgenieserver/helpers.py
# Compiled at: 2020-01-16 09:24:15
...
def build_parser():
    ...
    for cmd, desc in msg_by_cmd.items():
        ...
        sps[cmd].add_argument('-c',
          '--config', required=False, dest='config', help=("A path to the refgenie config file (YAML). If not provided, the first available environment variable among: '{}' will be used if set. Currently: {}".format(', '.join(CFG_ENV_VARS), env_var_val)))
    ...
    sps['archive'].add_argument('--genomes-desc',
      dest='genomes_desc',
      type=str,
      default=None,
      help='Path to a CSV file with genomes descriptions. Format: genome_name, genome description')
    sps['archive'].add_argument('-f',
      '--force', action='store_true',
      dest='force',
      help='whether the server file tree should be rebuilt even if exists')
    sps['archive'].add_argument('-r',
      '--remove', action='store_true',
      dest='remove',
      help='Remove selected genome, genome/asset or genome/asset:tag')
    sps['archive'].add_argument('asset_registry_paths',
      metavar='asset-registry-paths', type=str, nargs='*', help='One or more registry path strings that identify assets, e.g. hg38/fasta:tag')
    return parser
