...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tei_transformer/tags.py
# Compiled at: 2016-02-15 04:16:19
# Size of source mod 2**32: 15058 bytes
import calendar
...
class Head(TEITag):
    ...
    def get_replacement(self):
        ...
        if not self.tail or not self.tail.strip():
            ...
            if next_sibling is not None and next_sibling.localname == 'p' and next_sibling.get('rend') is None:
            ...
            if div_type == 'title':
                ...
                if div_type == 'diaryentry':
                    return self.process_head_level_three(identifier)
                if div_type == 'diaryentrysection':
...
class Div(TEITag):
    ...
    def process_month(self):
        ...
        return '\n\\addcontentsline{toc}{chapter}                {%s %s}' % (self.attrib['n'], year)
...
class Fmt:
    ...
    def __new__(cls, rend, name):
        ...
        fmt_keys = [
         'emph', 'single', 'double', 'superscript', 'smcp']
...
class FmtTag(TEITag):
    __doc__ = 'A tag dependent on common formatting rules.'
    targets = [
     'soCalled', 'supplied', 'bibl', 'hi', 'q']
...
class GenericTag(TEITag):
    __doc__ = '"Tags to be handled in a generic way.'
    deletes = [
     'lb']
    ...
    def get_replacement(self):
        ...
        if name in self.text_replaces:
            return self.text
        if name in self.unwraps:
            return self.unwrap()
        if name in self.deletes:
            return self.delete()


class FilterTag(TEITag):
    __doc__ = 'Tag handled with its children'
    targets = [
     'app', 'choice']
...
class PersName(TEITag):
    ...
    def _known(self, ref):
        ...
        def _index():
            t = (
             person.indexname + '|innote', self.text)
            return '\\indexperson{%s}{%s}' % t

        def _person():
            t = (
             ref, person.indexname, person.description, self.text)
