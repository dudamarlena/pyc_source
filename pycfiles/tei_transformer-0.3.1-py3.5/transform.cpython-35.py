...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tei_transformer/transform.py
# Compiled at: 2016-02-15 02:50:59
# Size of source mod 2**32: 10243 bytes
...
class ParserMethods:
    __doc__ = 'Methods for parsing and transforming XML.'
...
class Transformer(ParserMethods):
    __doc__ = 'Transform inputname, latexify the text produced, and make a pdf'

    def __init__(self, inputname, outputname=None, force=False, standalone=False):
    ...
    class Resources:
        ...
        def __new__(cls, *args, **kwargs):
            inputpath, outputname, resource_maker, basename_maker = cls._get_args(*args, **kwargs)
            classifications = config['resource_classifications']
            resources = {k:[resource_maker(r) for r in v] for k, v in classifications.items()}

            def groups(target):
                return iter(resources[target])

            parsepaths = (
             inputpath, next(groups('parsepath')))
        ...
        def _get_args(cls, inputpath, outname=None, standalone=False):
            ...
            if not work_dir.exists():
                work_dir.mkdir()
            args = [
             work_dir, resource_dir, standalone,
        ...
        @staticmethod
        def _process_resource(work_dir, resource_dir, standalone, basename, resources, resource_name):
...
def main():
    ...
    parser.add_argument('inputname', help='TEI file to transform')
    parser.add_argument('-o', '--outputname', help='Filename of the transformed file.', default=None)
    parser.add_argument('-f', '--force', help='Force recompilation even if unchanged.', action='store_true')
    parser.add_argument('-s', '--standalone', help='Do not include introduction or appendices', action='store_true')
