...
# Embedded file name: /Users/alanjds/src/git/adwords-client/adwords_client/client.py
# Compiled at: 2017-07-12 10:46:26
# Size of source mod 2**32: 30269 bytes
import csv, datetime, inspect, io, logging, time
from io import StringIO
import googleads.adwords, pandas as pd
...
class AdWords:
    ...
    def get_report(self, report_type, customer_id, target_name, create_table=False, exclude_fields=[], exclude_terms=[
 'Significance'], exclude_behavior=['Segment'], include_fields=[], *args, **kwargs):
        ...
        to_remove = set([name for name, item in report_csv.items() if item['Behavior'] in exclude_behavior or any(term in name for term in exclude_terms) or only_fields is not None and name not in only_fields])
        to_remove = to_remove.union(exclude_fields)
        fields = [field for field in report_csv if field not in to_remove]
        fields += include_fields
        fields.sort()
        casts = {'Money':'REAL',  'Long':'BIGINT', 
         'Double':'REAL', 
         'Integer':'BIGINT', 
         'Bid':'REAL'}
        ...
        if not simple_download:
            ...
            data = pd.read_csv((io.BytesIO(report_stream)), compression='gzip',
              header=None,
              names=fields,
              encoding='utf-8',
            ...
            data.to_sql(target_name, (self.engine),
              index=False,
 ...
 [
  'Segment'], [], *args, **kwargs)
    ...
    def log_batchjob(self, table_name, batchjob_service, comment=''):
        ...
        data = {'creation_time':datetime.datetime.now().isoformat(),  'client_id':client_id, 
    ...
    def set_batchjob_status(self, jobs_table, old_jobs, jobs):
        ...
        with self.engine.begin() as (conn):
            for client_id in jobs:
                data = [{'status':job['status'],  'result_url':job['downloadUrl'].url if 'downloadUrl' in job else '',  'client_id':client_id,  'batchjob_id':job['id']} for job in jobs[client_id] if job['status'] != old_jobs[client_id][job['id']][0]['status']]
                if data:
        ...
        def build_bid_change_operation(internal_operation):
            if utils.cents_as_money(internal_operation.new_bid) != utils.cents_as_money(internal_operation.old_bid):
                if internal_operation.keyword_id > -1:
                    yield operations.add_keyword_cpc_bid_adjustment_operation(int(internal_operation.adgroup_id), int(internal_operation.keyword_id), utils.cents_as_money(internal_operation.new_bid))
                else:
                    yield operations.add_adgroup_operation(int(internal_operation.campaign_id), int(internal_operation.adgroup_id), utils.cents_as_money(internal_operation.new_bid))
            else:
                yield
    ...
    def modify_keywords_text(self, table_name, batchlog_table='batchlog_table'):
        ...
        def build_new_keyword_operation(internal_operation):
            if internal_operation.keyword_id > -1:
                yield operations.add_biddable_adgroup_criterion_operation((int(internal_operation.adgroup_id)),
                  'SET',
                  'Keyword',
                  (int(internal_operation.keyword_id)),
                  userStatus='PAUSED')
                yield operations.add_new_keyword_operation(int(internal_operation.adgroup_id), internal_operation.new_text, internal_operation.keyword_match_type.upper(), internal_operation.status.upper(), int(internal_operation.cpc_bid))
            else:
                yield
    ...
    def modify_budgets(self, operations_table_name, batchlog_table='batchlog_table'):
        ...
        def build_budget_operation(internal_operation):
            yield from operations.add_adgroup_label_operation(adgroup_id=('%.0f' % internal_operation.adgroup_id),
    ...
    def add_adgroups_labels(self, operations_table_name, batchlog_table='batchlog_table'):
        ...
        def build_adgroup_label_operation(internal_operation):
            yield from operations.apply_new_budget(campaign_id=('%.0f' % internal_operation.campaign_id),
    ...
    def modify_adgroups_names(self, operations_table_name, batchlog_table='batchlog_table'):
        ...
        def build_adgroup_name_operation(internal_operation):
            yield from operations.set_adgroup_name_operation(adgroup_id=('%.0f' % internal_operation.adgroup_id),
    ...
    def create_labels(self, table_name):
        ...
        for client_id, lines in df.groupby('client_id'):
            ...
            for label_op in lines.itertuples():
                operations_list.extend(operations.add_label_operation(label_op.label))
