...
# Embedded file name: /Users/alanjds/src/git/adwords-client/adwords_client/adwordsapi/operations.py
...
def add_adgroup_label_operation(adgroup_id, label_id):
    yield {'xsi_type':'AdGroupLabelOperation', 
     'operator':'ADD', 
     'operand':{'xsi_type':'AdGroupLabel', 
      'adGroupId':adgroup_id, 
      'labelId':label_id}}


def set_adgroup_name_operation(adgroup_id, name):
    yield {'xsi_type':'AdGroupOperation', 
     'operator':'SET', 
     'operand':{'xsi_type':'AdGroup', 
      'id':adgroup_id, 
      'name':name}}
...
def add_adgroup(self, campaign_id, adgroup_id, operator):
    operation = {'xsi_type':'AdGroupOperation', 
     'operand':{'xsi_type':'AdGroup', 
...
def set_campaign_budget(budget_id, campaign_id):
    return {'xsi_type':'CampaignOperation', 
     'operator':'SET', 
     'operand':{'id':int(campaign_id), 
      'budget':{'xsi_type':'Budget', 
...
def add_budget(amount, budget_id, delivery='ACCELERATED', shared=False):
    ...
    if delivery:
        ...
        if shared:
            operation['operand'].update({'isExplicitlyShared':True, 
...
def build_money(money):
    return {'xsi_type':'Money', 
     'microAmount':cents_as_money(money)}


def add_biddable_adgroup_criterion_operation(adgroup_id, operator, xsi_type, criteria_id=None, criterion_params={}, **kwargs):
    ...
    operation = {'xsi_type':'AdGroupCriterionOperation', 
     'operand':operand, 
     'operator':operator}
    return operation
...
def build_new_bidding_strategy_configuration():
    return {'xsi_type':'BiddingStrategyConfiguration', 
     'bids':[]}


def add_keyword_cpc_bid_adjustment_operation(adgroup_id, criteria_id, value):
    bid_operation = add_biddable_adgroup_criterion_operation(adgroup_id, 'SET', 'Keyword', criteria_id)
...
def add_new_keyword_operation(adgroup_id, text, match_type, status, value):
    new_keyword_operation = add_biddable_adgroup_criterion_operation(adgroup_id,
      'ADD',
      'Keyword',
      criterion_params={'text':text, 
...
def add_adgroup_cpc_bid_adjustment_operation(campaign_id, adgroup_id, value):
    bid_operation = add_adgroup_operation(campaign_id, adgroup_id, 'SET')
    bidding_strategy = build_new_bidding_strategy_configuration()
    bid_operation['operand']['biddingStrategyConfiguration'] = bidding_strategy
    bid_type = build_new_bid_type('CpcBid', value)
    bid_operation['operand']['biddingStrategyConfiguration']['bids'].append(bid_type)
