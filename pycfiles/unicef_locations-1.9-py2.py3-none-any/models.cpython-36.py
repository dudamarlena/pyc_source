...
# [GCC 8.4.0]
# Embedded file name: /home/jojo/workspace/locations/src/unicef_locations/models.py
# Compiled at: 2019-04-19 21:07:20
# Size of source mod 2**32: 6108 bytes
import logging
...
class LocationsManager(TreeManager):
    ...
    def get_queryset(self):
        return super(LocationsManager, self).get_queryset().filter(is_active=True).order_by('name').select_related('gateway')

    def archived_locations(self):
        return super(LocationsManager, self).get_queryset().filter(is_active=False).order_by('name').select_related('gateway')

    def all_locations(self):
        return super(LocationsManager, self).get_queryset().order_by('name').select_related('gateway')
...
class Location(TimeStampedModel, MPTTModel):
    ...
    gateway = models.ForeignKey(GatewayType,
      verbose_name=(_('Location Type')), on_delete=(models.CASCADE))
    ...
    longitude = models.FloatField(verbose_name=(_('Longitude')),
      null=True,
      blank=True)
    p_code = models.CharField(verbose_name=(_('P Code')),
      max_length=32,
      blank=True,
      default='')
    ...
    geom = models.MultiPolygonField(verbose_name=(_('Geo Point')),
      null=True,
      blank=True)
    ...
    def __str__(self):
        return '{} ({} {}: {})'.format(self.name, self.gateway.name, 'CERD' if self.gateway.name == 'School' else 'PCode', self.p_code if self.p_code else '')
...
class LocationRemapHistory(TimeStampedModel):
    ...
    old_location = models.ForeignKey(Location,
      ...
      on_delete=(models.CASCADE),
      related_name='+')
    new_location = models.ForeignKey(Location,
...
class CartoDBTable(TimeStampedModel, MPTTModel):
    ...
    parent = TreeForeignKey('self',
      null=True, blank=True, related_name='children', db_index=True, verbose_name=(_('Parent')),
      on_delete=(models.CASCADE))
