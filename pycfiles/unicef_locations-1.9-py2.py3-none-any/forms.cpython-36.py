...
# [GCC 8.4.0]
# Embedded file name: /home/jojo/workspace/locations/src/unicef_locations/forms.py
...
class CartoDBTableForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        try:
            ...
            if parent_code_col:
                if parent_code_col not in row:
                    raise ValidationError('The Parent Code column ({}) is not in table: {}'.format(parent_code_col, table_name))
            if remap_table_name:
                try:
                    ...
                    if 'old_pcode' not in row.keys():
                        raise ValidationError('The Old PCode column ({}) is not in table: {}'.format('old_pcode', remap_table_name))
                    if 'new_pcode' not in row.keys():
                        raise ValidationError('The New PCode column ({}) is not in table: {}'.format('new_pcode', remap_table_name))
