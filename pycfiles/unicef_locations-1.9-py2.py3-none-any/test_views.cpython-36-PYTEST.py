...
# [GCC 8.4.0]
# Embedded file name: /home/jojo/workspace/locations/src/unicef_locations/tests/test_views.py
...
class TestLocationViews(BaseTenantTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.heavy_detail_expected_keys = sorted(('id', 'name', 'p_code', 'location_type',
                                                 'location_type_admin_level', 'parent',
                                                 'geo_point'))
    ...
    def test_location_delete_etag(self):
        ...
        @py_assert1 = etag_before != etag_after
        if not @py_assert1:
            @py_format3 = @pytest_ar._call_reprcompare(('!=', ), (@py_assert1,), ('%(py0)s != %(py2)s', ), (etag_before, etag_after)) % {'py0':@pytest_ar._saferepr(etag_before) if 'etag_before' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(etag_before) else 'etag_before',  'py2':@pytest_ar._saferepr(etag_after) if 'etag_after' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(etag_after) else 'etag_after'}
            @py_format5 = 'assert %(py4)s' % {'py4': @py_format3}
            raise AssertionError(@pytest_ar._format_explanation(@py_format5))
        @py_assert1 = None

    def test_api_location_autocomplete(self):
        response = self.forced_auth_req('get', (reverse('locations:locations_autocomplete')), user=(self.unicef_staff),
          data={'q': 'Loc'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 5)
        self.assertEqual(sorted(response.data[0].keys()), ['id', 'name', 'p_code'])
