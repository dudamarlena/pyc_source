...
# [GCC 8.4.0]
# Embedded file name: /home/jojo/workspace/locations/src/unicef_locations/tasks.py
# Compiled at: 2019-04-19 21:07:17
# Size of source mod 2**32: 16580 bytes
import time, celery
...
def update_sites_from_cartodb(carto_table_pk):
    ...
    try:
        ...
        try:
            ...
            remap_table_valid, remapped_pcode_pairs, remap_old_pcodes, remap_new_pcodes = validate_remap_table(database_pcodes, new_carto_pcodes, carto_table, sql_client)
            ...
            with transaction.atomic():
                ...
                with Location.objects.disable_mptt_updates():
                    for row in rows:
                        ...
                        if carto_table.parent_code_col:
                        ...
                        if carto_table.remap_table_name:
                            if len(remapped_pcode_pairs) > 0:
                                for remap_row in remapped_pcode_pairs:
                                    if carto_pcode == remap_row['new_pcode']:
                                        remapped_old_pcodes.add(remap_row['old_pcode'])

                        succ, sites_not_added, sites_created, sites_updated, sites_remapped, partial_results = create_location(carto_pcode, carto_table, parent, parent_instance, remapped_old_pcodes, site_name, row, sites_not_added, sites_created, sites_updated, sites_remapped)
        ...
        logger.warning('Table name {}: {} sites created, {} sites updated, {} sites remapped, {} sites skipped'.format(carto_table.table_name, sites_created, sites_updated, sites_remapped, sites_not_added))
        return results


def create_location(pcode, carto_table, parent, parent_instance, remapped_old_pcodes, site_name, row, sites_not_added, sites_created, sites_updated, sites_remapped):
    results = None
    try:
        location = None
        ...
        logger.warning('Multiple locations found for: {}, {} ({})'.format(carto_table.location_type, site_name, pcode))
        sites_not_added += 1
        return (False, sites_not_added, sites_created, sites_updated, sites_remapped, results)
    except Location.DoesNotExist:
        pass

    create_args = location or {'p_code':pcode, 
    ...
    try:
        ...
        logger.info('{}: {} ({})'.format('Added', location.name, carto_table.location_type.name))
        results = []
        if remapped_locations:
            for remapped_location in remapped_locations:
                ...
                sites_remapped += 1
                logger.info('{}: {} ({})'.format('Remapped', remapped_location.name, carto_table.location_type.name))
    ...
    if not row['the_geom']:
        return (
         False, sites_not_added, sites_created, sites_updated, sites_remapped, results)
    else:
...
def get_cartodb_locations(sql_client, carto_table):
    ...
    try:
        ...
        logger.exception('Cannot fetch pagination prequisites from CartoDB for table {}'.format(carto_table.table_name))
        return (
         False, [])
    else:
        offset = 0
        ...
        elif carto_table.parent_code_col and carto_table.parent:
            qry = 'select st_AsGeoJSON(the_geom) as the_geom, {}, {}, {} from {}'.format(carto_table.name_col, carto_table.pcode_col, carto_table.parent_code_col, carto_table.table_name)
        else:
            qry = 'select st_AsGeoJSON(the_geom) as the_geom, {}, {} from {}'.format(carto_table.name_col, carto_table.pcode_col, carto_table.table_name)
        while offset <= max_id:
            paged_qry = qry + ' WHERE {} > {} AND {} <= {}'.format(cartodb_id_col, offset, cartodb_id_col, offset + limit)
            logger.info('Requesting rows between {} and {} for {}'.format(offset, offset + limit, carto_table.table_name))
            time.sleep(1)
        ...
        try:
            ...
            for remap_row in remapped_pcode_pairs:
                if 'old_pcode' not in remap_row or 'new_pcode' not in remap_row:
                    return (
                     False, remapped_pcode_pairs, remap_old_pcodes, remap_new_pcodes)
            ...
            if len(bad_old_pcodes) > 0:
                logger.exception('Invalid old_pcode found in the remap table: {}'.format(','.join(bad_old_pcodes)))
                remap_table_valid = False
            if len(bad_new_pcodes) > 0:
                logger.exception('Invalid new_pcode found in the remap table: {}'.format(','.join(bad_new_pcodes)))
                remap_table_valid = False
    return (
...
def duplicate_pcodes_exist(database_pcodes, new_carto_pcodes, remap_old_pcodes):
    ...
    if duplicate_database_pcodes:
        logger.exception('Duplicates found in the existing database pcodes: {}'.format(','.join(duplicate_database_pcodes)))
    ...
    if duplicate_carto_pcodes:
        logger.exception('Duplicates found in the new CartoDB pcodes: {}'.format(','.join(duplicate_database_pcodes)))
        duplicates_found = True
    temp = {}
    ...
    if duplicate_remap_old_pcodes:
        logger.exception('Duplicates found in the remap table `old pcode` column: {}'.format(','.join(duplicate_remap_old_pcodes)))
        duplicates_found = True
