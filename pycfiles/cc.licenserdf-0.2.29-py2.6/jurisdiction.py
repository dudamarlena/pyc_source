...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/cc/licenserdf/tools/jurisdiction.py
...
def makeOpts():
    ...
    info_subparser = subparsers.add_parser('info', help='Display jurisdiction information.')
    launch_subparser = subparsers.add_parser('launch', help='Mark the jurisdiction as launched.')
    add_subparser = subparsers.add_parser('add', help='Add the jurisdiction.')

    def add_common_args(subparser):
        subparser.add_argument('-f', '--file', dest='rdf_file', action='store', help='Location of the jurisdictions RDF file; defaults to ./rdf/jurisdictions.rdf')
        subparser.add_argument('jurisdiction', nargs=1, help='Jurisdiction to operate on.')

    add_common_args(info_subparser)
    add_common_args(launch_subparser)
    add_common_args(add_subparser)
    add_subparser.add_argument('-i', '--i18n-dir', dest='i18n_dir', action='store', help='Location containing .po files; defaults to cc.i18n/cc/i18n/i18n/')
    add_subparser.add_argument('--lang', dest='langs', help='Comma delimited list of languages for the specified jurisdiction')
    add_subparser.add_argument('--uri', dest='juris_uri', help='The URI of the jurisdiction specific web page.')
    parser.set_defaults(rdf_file=pkg_resources.resource_filename('cc.licenserdf', 'rdf/jurisdictions.rdf'))
    add_subparser.set_defaults(juris_uri=None, i18n_dir=pkg_resources.resource_filename('cc.i18n', 'i18n/'))
    return parser
...
def info(opts, printer=_printer):
    ...
    if (
     j_ref, NS_RDF.type, NS_CC.Jurisdiction) not in j_graph:
...
def add(opts, __save_graph=save_graph):
    ...
    j_graph.add((
     j_ref, NS_DC['title'],
     Literal('${%s}' % mappers.COUNTRY_MAP[jurisdiction[:-1]], lang='i18n')))
