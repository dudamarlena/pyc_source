...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htmlpetrol/render.py
# Compiled at: 2010-09-01 11:47:56
try:
...
__tag_behaviour__ = [
 {'tag': 'input', 
    'type': [
           'text', 'hidden', 'password', 'button', 'submit'], 
    'property': 'value', 
    'value': '__literal__'},
 {'tag': 'input', 
    'type': [
        ...
        for node in dom.getiterator(tag['tag']):
            ...
            if 'type' in tag:
                if node.attrib.get('type') in tag['type']:
                    apply_to(node, tag, defaults[node.attrib.get('name')])
            else:
                apply_to(node, tag, defaults[node.attrib.get('name')])

    html = '%s%s%s' % (html_pre, ElementTree.tostring(dom), html_post)
        ...
        if node.attrib.get('for') in errors:
            ...
            for child in node.getiterator():
                if string == None:
                    string = ''
                    continue
                string += ElementTree.tostring(child)
