...
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/pypi-release/pypi_up/__init__.py
# Compiled at: 2016-02-11 02:32:49
...
import ConfigParser, argparse, os, re, sh
...
def main():
    try:
        ...
        desc += ' - pypi-up a simple command line tool to increase version number of packageand release on Pypi. Also Git Tag/Push the release'
        parser = argparse.ArgumentParser(prog=prog, description=desc)
        parser.add_argument('--setup', help='Setup PYPI-REL', action='store_true')
        parser.add_argument('-v', '--version', help='Show the current version', action='store_true')
        parser.add_argument('-p', '--patch', help='Increment PATCH version', action='store_true')
        parser.add_argument('-m', '--minor', help='Increment MINOR version and reset patch', action='store_true')
        parser.add_argument('-j', '--major', help='Increment MAJOR version and reset minor and patch', action='store_true')
        parser.add_argument('-e', '--edit', help='Manually enter the version number to bump', action='store')
        parser.add_argument('--dry', help='DRY RUN. To test the release, but it will not make any changes', action='store_true')
        parser.add_argument('-x', '--skip-prompt', help='Skip prompt', action='store_true')
        ...
        with sh.pushd(CWD):
            ...
            def test_auto_inc(auto_inc_):
                return auto_inc == auto_inc_ and not arg.patch and not arg.minor and not arg.major and not arg.version and not arg.edit
            ...
            print ''
            if not arg.skip_prompt and raw_input('Continue with the release? (y | n) ').strip() == 'n':
                print '** Release Aborted'
                print '-' * 80
                exit()
            skip_tag = not arg.skip_prompt and raw_input('Git Tag/Push release version? (y | n) ').strip().lower() == 'n'
            skip_pypi = not arg.skip_prompt and raw_input('Release to PYPI? (y | n) ').strip().lower() == 'n'
            print ''
            if not arg.dry:
                rvnup.write()
