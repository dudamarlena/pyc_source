...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jupyterlab_twitter/handlers.py
# Compiled at: 2018-07-06 03:04:10
# Size of source mod 2**32: 19963 bytes
"""
...
import logging, os, sys, tornado.httpserver, tornado.ioloop, tornado.options, tornado.web, tornado.gen, tornado.auth
...
class TwitterPostHandler(BaseHandler, tornado.auth.TwitterMixin):
    ...
    def post(self):
        ...
        try:
            ...
            new_entry = yield self.twitter_request('/statuses/update',
...
class Twitter_API_handler(Twitter_handler):
    ...
    def post(self):
        ...
        if showtoplevel['code'] != 0:
            ...
            result = {'code':showtoplevel['code'], 
             'data':{'showtoplevel':showtoplevel, 
              'branch':branch, 
              'log':log, 
              'status':status}}
...
class Twitter_status_handler(Twitter_handler):
    ...
    def get(self):
        ...
        status.
        """
        self.finish(json.dumps({'add_all':'check', 
         'filename':'filename', 
         'top_repo_path':'path'}))
    ...
    def post(self):
        ...
        if my_data['checkout_branch']:
            if my_data['new_check']:
                ...
                my_output = self.twitter.checkout_new_branch(my_data['branchname'], top_repo_path)
            else:
                print('switch to an old branch')
                my_output = self.twitter.checkout_branch(my_data['branchname'], top_repo_path)
        else:
...
def setup_handlers(web_app):
    ...
    twitter_handlers = [
     ...
     (
      '/twitter/info', TwitterInfoHandler),
     (
      '/twitter/auth', TwitterAuthHandler),
     (
      '/twitter/auth/popup', TwitterAuthPopupHandler),
     (
      '/services/twitter/auth/popup', TwitterAuthPopupHandler),
     (
      '/twitter/signout', TwitterSignoutHandler),
     (
      '/twitter/post', TwitterPostHandler)]
    base_url = web_app.settings['base_url']
    twitter_handlers = [(ujoin(base_url, x[0]), x[1]) for x in twitter_handlers]
