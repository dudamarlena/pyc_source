...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/lsqfitgp/_kernels.py
# Compiled at: 2020-04-23 16:30:17
...
extend.defvjp(_maternp, lambda ans, x, p: lambda g: g * _maternp_deriv(x, p))
...
extend.defvjp(_matern32, lambda ans, x: lambda g: g * -x * np.exp(-x))
...
extend.defvjp(_matern52, lambda ans, x: lambda g: g * -x / 3 * _matern32(x))
...
def FracBrownian(x, y, H=0.5):
    ...
    assert np.all(y >= 0)
    H2 = 2 * H
    return 0.5 * (x ** H2 + y ** H2 - np.abs(x - y) ** H2)
...
def PPKernel(r, q=0, D=1):
    ...
    if q == 0:
        ...
        if q == 1:
            ...
            if q == 2:
                poly = 1 + r * (j + 2 + r * ((0.3333333333333333 * j + 1.3333333333333333) * j + 1))
            else:
                if q == 3:
                    poly = 1 + r * (j + 3 + r * ((0.4 * j + 2.4) * j + 3 + r * (((0.06666666666666667 * j + 0.6) * j + 1.5333333333333334) * j + 1)))
