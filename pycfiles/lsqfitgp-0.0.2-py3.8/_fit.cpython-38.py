...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/lsqfitgp/_fit.py
# Compiled at: 2020-04-26 15:53:36
# Size of source mod 2**32: 2934 bytes
import gvar
...
def _flat(x):
    if isinstance(x, np.ndarray):
        return x.reshape(-1)
    if isinstance(x, gvar.BufferDict):
        return x.buf
...
def _unflat(x, original):
    ...
    if isinstance(original, gvar.BufferDict):
...
def empbayes_fit(hyperprior, gpfactory, data):
    ...
    def fun(p):
        ...
        return -gp.marginal_likelihood(data) + 0.5 * np.sum(diagres ** 2)
