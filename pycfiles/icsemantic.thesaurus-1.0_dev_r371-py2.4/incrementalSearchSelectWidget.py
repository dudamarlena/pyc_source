...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/icsemantic/thesaurus/browser/incrementalSearchSelectWidget.py
...
class IncrementalSearchSelectWidget(SimpleInputWidget):
    ...
    def __call__(self):
        ...
        html += '<div id="div_%s"></div>' % safe_name(self.name)
        return html
...
class IncrementalSearchSelectAzax(KSSView):
    ...
    def html_conceptSelectedSearchExpression(self):
        ...
        if current_input == '':
            ...
            if results['concepts_count'] > 0:
                ...
                newSearchExpression = (', ').join(search_words)
                if self.modifyingConcept() == len(search_words) - 1:
                    newSearchExpression += ', '
            elif self.allow_new_terms():
                if self.modifyingConcept() == len(search_words) - 1:
                    newSearchExpression = self.searchExpression() + ', '
                else:
                    newSearchExpression = self.searchExpression()
                ...
                selectedConcept = ''
        return (
         safe_unicode(newSearchExpression), selectedConcept)
    ...
    def html_word_item(self, word, selected_word, current_input):
        if word == selected_word:
            return '            <i>%s</i><br>' % self.html_emphasize_input(word, current_input)
        else:
            return '            %s<br>' % self.html_emphasize_input(word, current_input)

    def html_emphasize_input(self, word, current_input):
        if current_input != '':
            return safe_unicode(('<b>' + current_input + '</b>').join(word.split(current_input)))
        else:
            return safe_unicode(word)

    def query(self, search_expression):
        return thesaurus_utility().query(search_expression=search_expression, narrowerthan=None, broaderthan=None, contexts=[], languages=supportedLanguages(), inbranch=None, hidden=None, max_results=self.max_results(), first_result=self.first_result())
    ...
    def concepts(self):
        return (',').join([ self.to_string(cid) for cid in self.cids() ])
