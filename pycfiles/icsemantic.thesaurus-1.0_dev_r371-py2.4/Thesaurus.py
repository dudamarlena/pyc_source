...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/icsemantic/thesaurus/Thesaurus.py
...
import transaction, StringIO
from zope.app.component.hooks import getSite
from persistent import Persistent, list, dict
_format = {'SKOSCore': ioSKOSCore, 'Ding': ioDing}
...
class Thesaurus(SimpleItem):
    ...
    def rdf_term_concepts(self, term, context=None, encoding='utf-8'):
        temp = StringIO.StringIO()
        ioSKOSCoreModule.write_concepts(temp, self.term_concepts(term, context))
        return temp.getvalue()

    def rdf_concepts_search(self, search_expression, context=None):
        temp = StringIO.StringIO()
        ioSKOSCoreModule.write_concepts(temp, self.concepts_search_objects(search_expression, context))
        return temp.getvalue()

    def commiting_methods(self):
        return [
         'set_prefered', 'append_term', 'append_concept', 'replace_concept_at']
...
def wrap(anObject, attribute_name):
    ...
    """
    if not hasattr(anObject.__class__, attribute_name + '_wrapped') or not getattr(anObject, attribute_name + '_wrapped'):
