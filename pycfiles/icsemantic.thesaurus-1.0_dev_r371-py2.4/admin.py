...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/icsemantic/thesaurus/browser/admin.py
...
from icsemantic.thesaurus import update_keywords_incrementalsearchwidget_property
from icsemantic.thesaurus.replaceKeywordsWidget import replace_keywords_widget_at_metadata_page, replace_keywords_widget_to_original_at_metadata_page
...
class ThesaurusUpload(BaseSettingsForm):
    ...
    def handle_edit_action(self, action, data):
        if form.applyChanges(self.context, self.form_fields, data, self.adapters):
            self.load_thesaurus(data['thesaurus_file'], data['default_language'], contexts=[data['thesaurus_context']], format=data['thesaurus_format'], encoding=data['encoding'], new=data['new'])
    ...
    def load_thesaurus(self, file_content, default_language, contexts=[], format='SKOSCore', encoding='utf-8', new=False):
        thesaurus_utility().load_string(file_content, default_language, contexts=contexts, format=format, encoding=encoding, new=new)
...
class VerticalSelectTest(BaseSettingsForm):
    ...
    """
    __module__ = __name__
    form_name = _('Vertical Select Test Form')
    ...
    def handle_edit_action(self, action, data):
        if form.applyChanges(self.context, self.form_fields, data, self.adapters):
            zope.event.notify(ObjectModifiedEvent(self.context))
            self.status = _('Updated on ${date_time}.\nConcepts selected: ${selected_concepts}', mapping={'date_time': str(datetime.utcnow()), 'selected_concepts': str(data['vertical_select'])})
        else:
            self.status = _('No changes')


class icSemanticThesaurusConfigurationForm(BaseSettingsForm):
    """ It is a configlet that allow to change thesaurus configuration.
        """
    __module__ = __name__
    form_name = _('Thesaurus Configuration')
    form_fields = form.Fields(interfaces.IicSemanticThesaurusConfiguration)

    @form.action(_('Apply'), condition=form.haveInputWidgets)
    def handle_edit_action(self, action, data):
        if form.applyChanges(self.context, self.form_fields, data, self.adapters):
            app = self.context.restrictedTraverse('/')
            update_keywords_incrementalsearchwidget_property(app, data['keywords_incrementalsearchwidget'])
            if data['keywords_incrementalsearchwidget']:
                replace_keywords_widget_at_metadata_page(StringIO())
            else:
                replace_keywords_widget_to_original_at_metadata_page(StringIO())
            zope.event.notify(ObjectModifiedEvent(self.context))
            self.status = _('Updated on ${date_time}', mapping={'date_time': str(datetime.utcnow())})
        else:
            self.status = _('No changes')
