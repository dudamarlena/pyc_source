...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/microcsound/handlers.py
# Compiled at: 2017-11-27 09:06:21
...
def handle_instrument_parameter(event):
    ...
    try:
        ...
        print('Instrument parameters must be declared as a string before they can be altered! Problem statement is: %s' % event)
        exit()
...
def handle_length(event):
    ...
    z = re.search('(?:\\[L:)?([0-9]{1,4})[/]([0-9]{1,4})(?:\\])?', event)
...
def handle_pedal(event):
    ...
    if 'PD' in event:
        ...
        ev = re.match('PD(?P<arrival>\\d{1,3})', event)
        state_obj.arrival = int(ev.group('arrival')) * state_obj.length + state_obj.grid_time
    else:
...
def handle_symbolic_notation(event):
    ...
    mylist = re.search("(?P<articul>[.\\(]?)(?P<pitch>(?:\\^/2|_/2|[_^=/\\\\<>!?]|\\xc2\\xa1|\\xc2\\xbf)*[a-g](?:\\*)*[',]*)(?P<len>[0-9]{0,2})(?P<tie>[-]?)(?P<legato_end>[)]?)", event)
...
def handle_numeric_notation(event):
    ...
    mylist = re.search('(?P<articul>[.\\(]?)(?:(?P<oct>[0-9]+)[.])?(?P<deg>[-]?[0-9]+)(?P<legato_end>[\\)]?)(?P<tie>[| t]*)', event)
    articul = mylist.group('articul')
    try:
    ...
    if deg:
        degree_raw = int(deg)
        if state_obj.div > 0:
            degree = (state_obj.octave - constants.MIDDLE_C_OCTAVE) * state_obj.div + degree_raw
            pitch = helpers.degree2hz(degree, state_obj.div)
        else:
            pitch = degree_raw
    length_factor = 1
...
def handle_JI_notation(event):
    ...
    mylist = re.search('(?P<articul>[.\\(]?)(?P<ratio>[0-9]+[:][0-9]+)(?P<legato_end>[\\)]?)(?P<tie>[| t]*)', event)
    articul = mylist.group('articul')
    try:
