...
# [GCC 8.4.0]
# Embedded file name: /home/hansek/projects/django-translation-manager/translation_manager/admin.py
...
class TranslationEntryAdmin(admin.ModelAdmin):
    ...
    save_on_top = True
    default_fields = [
     'original', 'language', 'get_hint', 'translation', 'occurrences', 'locale_parent_dir', 'domain']
    ...
    if not list_fields:
        ...
        list_filter = [
         'language', 'locale_parent_dir', 'domain', TranslationStateFilter]
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        ...
        if db_field.name == 'translation':
            add_styles(formfield.widget, 'height: 26px;')
        return formfield
    ...
    def get_urls(self):
        ...
        urls = [
         url('^make/$', (wrap(self.make_translations_view)), name=('%s_%s_make' % info)),
         url('^compile/$', (wrap(self.compile_translations_view)), name=('%s_%s_compile' % info)),
         url('^load_from_po/$', (wrap(self.load_from_po_view)), name=('%s_%s_load' % info)),
         url('^get_make_translations_status/$', (wrap(self.get_make_translations_status)), name=('%s_%s_status' % info)),
         url('^sync/$', (wrap(self.sync_translations)), name=('%s_%s_sync' % info))]
        ...
        if not cache.get(get_settings('TRANSLATIONS_PROCESSING_STATE_CACHE_KEY')):
            ...
            try:
                if translation_mode == 'sync':
                    ...
                    if translation_mode == 'async_django_rq':
    ...
    def sync_translations(self, request):
        if not request.user.has_perm('translation_manager.sync'):
            ...
            url = '{}?token={}'.format(get_settings('TRANSLATIONS_SYNC_REMOTE_URL'), get_settings('TRANSLATIONS_SYNC_REMOTE_TOKEN'))
            ...
            for language, domains in data.items():
                for domain, translation_entries in domains.items():
                    for original, translation_entry in translation_entries.items():
                        ...
                        RemoteTranslationEntry.objects.create(translation=(translation_entry['translation']),
...
class ProxyTranslationEntryAdmin(TranslationEntryAdmin):
    fields = [
     'original', 'language', 'get_hint', 'changed', 'translation', 'use_remote', 'remote_translation', 'remote_changed', 'domain']
    list_display = fields
    ...
    def use_remote(self, obj):
        ...
        if obj.remote_translation_entry:
            return mark_safe('<input type="button" data-id="{id}" class="btn btn-info btn-use-remote" value="{name}"/><xmp id="{id}" style="display: none">{remote_translation_value}</xmp>'.format(name=(_('admin-translation_manager-use_remote-value')),
