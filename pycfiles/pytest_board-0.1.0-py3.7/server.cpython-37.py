...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pytest_board/server.py
    ...
    def handle_latest(self):
        ...
        for test in self.latest_result['tests']:
            ...
            if dirname not in packages:
                packages[dirname] = dict(PackageName=dirname,
                  Coverage=0.0,
                  Elapsed=0.0,
                  Outcome='passed',
                  BuildOutput='',
            ...
            test_results = dict(File=filename,
              Line=(test['lineno']),
        ...
        return json.dumps(dict(Packages=packages,
          Paused=False,
          Revision=revision))
