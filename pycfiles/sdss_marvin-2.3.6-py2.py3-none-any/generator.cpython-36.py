...
# [GCC 8.4.0]
# Embedded file name: /Users/Brian/Work/github_projects/marvin_pypi/python/marvin/extern/wtforms-alchemy/wtforms_alchemy/generator.py
# Compiled at: 2018-01-12 14:08:15
# Size of source mod 2**32: 20077 bytes
try:
...
import six, sqlalchemy as sa
from sqlalchemy.dialects import postgresql
from sqlalchemy.orm.properties import ColumnProperty
from sqlalchemy_utils import types
from wtforms import BooleanField, Field, FloatField, PasswordField, TextAreaField
from wtforms.widgets import CheckboxInput, TextArea
from wtforms_components import ColorField, DateField, DateIntervalField, DateTimeField, DateTimeIntervalField, DateTimeLocalField, DecimalField, DecimalIntervalField, EmailField, IntegerField, IntIntervalField, SelectField, StringField, TimeField
from wtforms_components.widgets import ColorInput, DateInput, DateTimeInput, DateTimeLocalInput, EmailInput, NumberInput, TextInput, TimeInput
from .exc import AttributeTypeException, InvalidAttributeException, UnknownTypeException
from .fields import CountryField, PhoneNumberField, WeekDaysField
from .utils import choice_type_coerce_factory, ClassMap, flatten, is_date_column, is_number, is_number_range, is_scalar, null_or_unicode, strip_string, translated_attributes
...
class FormGenerator(object):
    ...
    def skip_column(self, column):
        ...
        """
        if not self.meta.include_foreign_keys:
            if column.foreign_keys:
                return True
            elif not self.meta.include_primary_keys:
                ...
                if isinstance(column.type, types.TSVectorType):
                    return True
                if self.meta.only_indexed_fields:
                    if not self.has_index(column):
                        return True
            return isinstance(column, sa.Column) or True
        else:
            return False
    ...
    def filters(self, column):
        ...
        if isinstance(column.type, sa.types.String) and self.meta.strip_string_fields and should_trim is None or should_trim is True:
    ...
    def date_format(self, column):
        ...
        :param column: SQLAlchemy Column object
        """
        if isinstance(column.type, sa.types.DateTime) or isinstance(column.type, types.ArrowType):
    ...
    def type_specific_parameters(self, column):
        ...
        """
        kwargs = {}
        if hasattr(column.type, 'enums') or column.info.get('choices') or isinstance(column.type, types.ChoiceType):
        ...
        if step is not None:
            ...
            if isinstance(column.type, sa.types.Numeric):
                if column.type.scale is not None:
                    if not column.info.get('choices'):
    ...
    def select_field_kwargs(self, column):
        ...
        elif 'choices' in column.info:
            if column.info['choices']:
    ...
    def coerce(self, column):
        ...
        if 'coerce' in column.info:
            ...
            if column.nullable:
                if issubclass(python_type, six.string_types):
                    return null_or_unicode
            return python_type
    ...
    def required_validator(self, column):
        ...
        :param column: SQLAlchemy Column object
        """
        if not self.meta.all_fields_optional:
    ...
    def additional_validators(self, key, column):
        ...
        if 'validators' in column.info:
            if column.info['validators']:
                try:
    ...
    def unique_validator(self, key, column):
        ...
        """
        if column.unique:
            return self.get_validator('unique',
    ...
    def range_validator(self, column):
        ...
        if min_ is not None or max_ is not None:
            if is_number(column.type) or is_number_range(column.type):
                ...
                if isinstance(column.type, sa.types.Time):
    ...
    def get_field_class(self, column):
        ...
        if 'form_field_class' in column.info:
            if column.info['form_field_class']:
                return column.info['form_field_class']
            else:
                if 'choices' in column.info:
                    if column.info['choices']:
                        return SelectField
                if column.type not in self.TYPE_MAP and isinstance(column.type, sa.types.TypeDecorator):
            ...
            try:
                ...
                if inspect.isclass(column_type):
                    if issubclass(column_type, Field):
                        return column_type
