...
def isModel(model):
    ...
    if isinstance(model, DeclarativeMeta) and hasattr(model, '__table__') and model.__table__.name:
        return True
    False
            ...
            if nexus and isModel(nexus):
                ...
                if not nexus:
                    ...
                    for tableB in tables:
                        newJoins = self._getShortestPath(nexus, tableB, format_out=format_out, removeA=True)
                        self._joinList(joins, newJoins)
