...
# [GCC 8.4.0]
# Embedded file name: /Users/Brian/Work/github_projects/marvin_pypi/python/marvin/extern/marvin_brain/python/brain/utils/general/general.py
# Compiled at: 2018-08-13 18:12:44
# Size of source mod 2**32: 5373 bytes
import os, decimal, datetime, numpy as np, json
from brain import bconfig
from brain.core.exceptions import BrainError, BrainWarning
__all__ = [
 'getDbMachine', 'merge', 'convertIvarToErr', 'compress_data', 'uncompress_data']
...
def _compress_msgpack(data, uncompress=None):
    ...
    try:
        ...
        compress_with = 'json'
        raise BrainWarning('Must have Python packages msgpack and msgpack_numpy installed to use msgpack compression.  Defaulting to json')
    else:
        m.patch()
    try:
        if uncompress:
...
def compress_data(data, compress_with=None, uncompress=None):
    ...
    elif not compress_with in bconfig._compression_types:
        raise AssertionError('compress_with must be one of {0}'.format(bconfig._compression_types))
    else:
