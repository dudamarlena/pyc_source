...
# [GCC 8.4.0]
# Embedded file name: /Users/Brian/Work/github_projects/marvin_pypi/python/marvin/extern/wtforms-alchemy/wtforms_alchemy/fields.py
# Compiled at: 2018-01-12 14:08:15
# Size of source mod 2**32: 17346 bytes
...
class ModelFieldList(FieldList):
    ...
    def _get_bound_field_for_entry(self, formdata, data, index):
        if not not self.max_entries:
            if not len(self.entries) < self.max_entries:
                raise AssertionError('You cannot have more than max_entries entries in this FieldList')
        new_index = self.last_index = index or self.last_index + 1
        name = '%s-%d' % (self.short_name, new_index)
        id = '%s-%d' % (self.id, new_index)
        if hasattr(self, 'meta'):
            return self.unbound_field.bind(form=None,
              name=name,
              prefix=(self._prefix),
              id=id,
              _meta=(self.meta))
    ...
    def _add_entry(self, formdata=None, data=_unset_value, index=None):
        field = self._get_bound_field_for_entry(formdata=formdata,
          data=data,
          index=index)
        if data != _unset_value and data:
            if formdata:
...
class CountryField(SelectField):
    ...
    def _get_choices(self):
        territories = [(code, name) for code, name in six.iteritems(i18n.get_locale().territories) if len(code) == 2 if code not in ('QO',
                                                                                                                                     'QU',
                                                                                                                                     'ZZ')]
...
class QuerySelectField(SelectFieldBase):
    ...
    def __init__(self, label=None, validators=None, query_factory=None, get_pk=None, get_label=None, allow_blank=False, blank_text='', **kwargs):
        ...
        if get_label is None:
            ...
            if isinstance(get_label, string_types):
...
class QuerySelectMultipleField(QuerySelectField):
 ...
 label,
 validators, default=default, **kwargs)
...
class GroupedQuerySelectField(SelectField):
    ...
    def __init__(self, label=None, validators=None, query_factory=None, get_pk=None, get_label=None, get_group=None, allow_blank=False, blank_text='', blank_value='__None', **kwargs):
        (super(GroupedQuerySelectField, self).__init__)(
 label,
 validators, coerce=lambda x: x, **kwargs)
        self.query = None
        self.query_factory = query_factory
        if get_pk is None:
            self.get_pk = get_pk_from_identity
    ...
    def _pre_process_object_list(self, object_list):
        return sorted(object_list,
          key=(lambda x: (
         x[1] or '', self.get_label(x[2]) or '')))

    @property
    def choices(self):
        if not self._choices:
            object_list = map(lambda x: (
             x[0], self.get_group(x[1]), x[1]), self._get_object_list())
    ...
    def process_formdata(self, valuelist):
        if valuelist:
            if self.allow_blank:
                if valuelist[0] == self.blank_value:
                    self.data = None
            else:
                self._data = None
                self._formdata = valuelist[0]
...
class WeekDaysField(SelectMultipleField):
    ...
    def _get_choices(self):
        ...
        for day in days:
            yield (
             day.index, day.get_name(context='stand-alone'))
...
class PhoneNumberField(StringField):
    ...
    def __init__(self, label=None, validators=None, region='US', display_format='national', **kwargs):
