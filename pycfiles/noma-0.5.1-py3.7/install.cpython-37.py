...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noma/install.py
# Compiled at: 2019-09-25 06:48:41
# Size of source mod 2**32: 14453 bytes
"""
...
def enable_swap():
    ...
    if add_boot.returncode == 0:
        if add_default.returncode == 0:
            return True
    return False
...
def check_for_destruction(device, path):
    ...
    if destroy:
        ...
        if unmounted:
            usb.is_mounted(device) or print('Going to format {d} with ext4 now'.format(d=device))
            call(['mkfs.ext4', '-F', '/dev/' + device])
            if mnt_ext4(device, path) == 0:
...
def setup_fstab(device, mount):
    ...
    if ext4_mounted:
        with open('/etc/fstab', 'a') as (file):
            fstab = '\nUUID={u} {m} ext4 defaults,noatime 0 0'.format(u=(usb.get_uuid(device)),
              m=mount)
            file.write(fstab)
    else:
...
def enable_compose():
    ...
    check_to_fetch('/etc/init.d/docker-compose', 'https://raw.githubusercontent.com/lncm/pi-factory/b12c6f43d11be58dac03a2513cfd2abbb16f6526/etc/init.d/docker-compose')
...
def install_box():
    ...
    install_tor()
    rc_add('tor', 'default')
    check_to_fetch('/home/lncm/public_html/pos/index.html', 'https://raw.githubusercontent.com/lncm/invoicer-ui/master/dist/index.html')
