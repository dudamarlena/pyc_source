...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noma/bitcoind.py
# Compiled at: 2019-09-25 06:48:41
# Size of source mod 2**32: 10023 bytes
"""
    ...
    def set_permissions(working_path):
        ...
        for path, dirs, files in os.walk(working_path):
            ...
            for directory in dirs:
                ...
                os.chmod(os.path.join(path, directory), 493)

            for file in files:
                os.chown(os.path.join(path, file), lncm_uid, lncm_gid)
                os.chmod(os.path.join(path, file), 484)
    ...
    if bitcoind_dir_exists:
        ...
        if snapshot_path.is_file():
            ...
            if pathlib.Path(bitcoind_dir / 'blocks').is_dir():
                ...
                return
            elif pathlib.Path(bitcoind_dir / 'chainstate').is_dir():
...
def generate_rpcauth(username, password=''):
    ...
    try:
        with open('/media/important/important/rpc.txt', 'a') as (file):
            file.write('rpcauth={r}\nusername={u}\npassword={p}'.format(r=auth_value,
              u=username,
              p=password))
...
def get_kv(key, config_path):
    ...
    :return: value of key
    """
    import itertools, configparser
    parser = configparser.ConfigParser(strict=False)
    with open(config_path) as (lines):
        lines = itertools.chain(('[main]', ), lines)
