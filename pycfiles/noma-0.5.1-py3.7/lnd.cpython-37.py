...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noma/lnd.py
...
def encodemacaroons(macaroonfile=cfg.MACAROON_PATH, tlsfile=cfg.TLS_CERT_PATH):
    ...
    if path.exists(str(macaroonfile)):
        if path.exists(str(tlsfile)):
...
def autounlock():
    ...
    try:
        response = post((cfg.URL_UNLOCKWALLET),
          verify=(cfg.TLS_CERT_PATH), data=(dumps(data)))
    except Exception:
        pass
    else:
        try:
...
def create_wallet():
    ...
    if data:
        return_data = post((cfg.URL_INITWALLET),
          verify=(cfg.TLS_CERT_PATH), data=(dumps(data)))
        if return_data.status_code == 200:
