...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.6/dist-packages/nosyd/nosyd.py
# Compiled at: 2009-10-09 10:50:53
import os, stat, time, os.path, logging, re, sys
from xunit import *
from utils import *
import cache
version = '0.0.5'
...
class Nosyd:
    ...
    def _update_projects_checksums(self):
        ...
        for monitored_pn in self.projects.keys():
            if monitored_pn not in project_names:
...
class PyNotifier(Notifier):
    ...
    def notify(self, msg1, msg2, urgency=Notifier.URGENCY_LOW):
        ...
        import pynotify
        pyurgencies = {self.URGENCY_LOW: pynotify.URGENCY_LOW, 
...
class GrowlNotifier(Notifier):
    ...
    def notify(self, msg1, msg2, urgency=Notifier.URGENCY_LOW):
        ...
        growlpriorities = {self.URGENCY_LOW: -1, 
           self.URGENCY_NORMAL: 0, 
           self.URGENCY_CRITICAL: 1}
        sticky = False
...
def main():
    ...
    parser.add_option('-a', '--add', default=None, action='store_true', help='Start monitoring the specified or current directory')
    parser.add_option('-r', '--remove', action='store_true', help='Stop monitoring the specified or current directory')
    parser.add_option('-l', '--list', dest='list', action='store_true', default=False, help='List the monitored projects')
    parser.add_option('-c', '--clean', dest='clean', action='store_true', default=False, help="Clean the projects nosyd can't track anymore (links point to nowhere)")
    parser.add_option('-1', '--local', dest='local', action='store_true', default=False, help='Run the standalone nosyd on the specified or current directory')
    parser.add_option('-s', '--stop', dest='stop', action='store_true', default=False, help='Stops the running server, if any')
