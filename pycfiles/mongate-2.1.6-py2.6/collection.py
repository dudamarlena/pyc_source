...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tests/collection.py
# Compiled at: 2011-01-07 23:33:51
import unittest
...
class TestCollection(unittest.TestCase):
    ...
    def test_collection_insert_with_invalid_characters(self):
        oid = self.collection.insert({'name': 'Benjamin & Company=bar', 
           'profession': 'Software Developer?'})
        self.assertTrue(oid)

    def _insert_data(self):
        self.collection.insert({'name': 'Benjamin', 
           'age': 27, 
           'profession': 'Software Developer'})
        return self.collection.insert({'apple': 'tasty', 
           'banana': 'phallic'})
    ...
    def test_collection_find_with_invalid_characters(self):
        oid = self.collection.insert({'name': 'Benjamin & Company', 
           'profession': 'Software Developer?'})
    ...
    def test_update_with_invalid_characters(self):
        self._insert_data()
        self.collection.update({'name': 'Benjamin', 
           'invalid': '&&&'}, {'$inc': {'age': 1}})
        retrieved_collection = self.collection.find({'name': 'Benjamin'})
