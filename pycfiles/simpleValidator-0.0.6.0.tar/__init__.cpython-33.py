    ...
    def validate(self, what, how):
        ...
        for rule in rulelist:
            if ':' in rule:
                ...
                callback = self._Validator__call_rule(rule=rulevalue[0], value=what, constraint=rulevalue[1])
            else:
                rulevalue = [
                 rule]
                callback = self._Validator__call_rule(rule=rule, value=what)
            if not callback:
                if len(rulevalue) == 2:
    ...
    def __validate(self):
        ...
        for field in self.rules:
            ...
            for rule in rulelist:
                if ':' in rule:
                    rulevalue = rule.split(':')
                    callback = self._Validator__call_rule(rule=rulevalue[0], value=self.fields[field], constraint=rulevalue[1])
                else:
                    rulevalue = [
                     rule]
                    callback = self._Validator__call_rule(rule=rule, value=self.fields[field])
                if not callback:
                    if len(rulevalue) == 2:
