...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neox/commons/menu_buttons.py
...
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QScroller, QVBoxLayout, QPushButton, QGridLayout, QScrollArea, QLabel
...
class GridButtons(QWidget):
    ...
    def set_items(self, rows):
        ...
        for row in rows:
            ...
            item_button = CustomButton(parent=self,
              id=(row[0]),
              title=(row[2]),
              desc=(str(row[3])),
...
class MenuDash(QVBoxLayout):
    ...
    def create_categories(self):
        ...
        for value in self.values:
            ...
            button = CustomButton(parent=self,
              id=name_button,
              icon=(value['icon']),
              desc=(value['name']),
            ...
            grid_buttons = GridButtons((self.parent), (value['items']), cols, action=(self.method_on_selected))
    ...
    def action_back(self):
        self.setState({'layout_invisible': False})

    def selected_method(self, args):
        self.setState({'layout_invisible':True, 
         'button':args})