...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neox/commons/pyson.py
# Compiled at: 2020-02-26 23:29:02
# Size of source mod 2**32: 19412 bytes
try:
...
__all__ = [
 'PYSONEncoder', 'PYSONDecoder', 'Eval', 'Not', 'Bool', 'And', 'Or',
...
class PYSON(object):
    ...
    def __and__(self, other):
        if isinstance(other, PYSON):
            if other.types() != set([bool]):
                other = Bool(other)
        if isinstance(self, And):
            if not isinstance(self, Or):
    ...
    def default(self, obj):
        ...
        if isinstance(obj, datetime.date):
            if isinstance(obj, datetime.datetime):
                return DateTime(obj.year, obj.month, obj.day, obj.hour, obj.minute, obj.second, obj.microsecond).pyson()
...
class PYSONDecoder(json.JSONDecoder):
    ...
    def __init__(self, context=None, noeval=False):
        self._PYSONDecoder__context = context or {}
        self.noeval = noeval
    ...
    def _object_hook(self, dct):
        if '__class__' in dct:
            ...
            if klass:
                if not self.noeval:
                    return klass.eval(dct, self._PYSONDecoder__context)
                dct = dct.copy()
                del dct['__class__']
                return klass(**dct)
        return dct
...
class Not(PYSON):
    ...
    def __init__(self, v):
        ...
        if isinstance(v, PYSON) and not v.types() == set([bool]):
    ...
    def __init__(self, *statements, **kwargs):
        ...
        for statement in statements:
            if isinstance(statement, PYSON):
                if not statement.types() == set([bool]):
                    raise AssertionError('statement must be boolean')
            else:
                assert isinstance(statement, bool), 'statement must be boolean'
    ...
    def __init__(self, s1, s2, e=False):
        ...
        for i in (statement1, statement2):
            if isinstance(i, PYSON):
                if not i.types().issubset(set([int, int, float])):
                    raise AssertionError('statement must be an integer or a float')
            else:
                assert isinstance(i, (int, float)), 'statement must be an integer or a float'

        if isinstance(equal, PYSON) and not equal.types() == set([bool]):
...
class If(PYSON):
    ...
    def __init__(self, c, t, e=None):
        ...
        if isinstance(condition, PYSON) and not condition.types() == set([bool]):
            raise AssertionError('condition must be boolean')
        else:
            ...
            assert then_types == else_types, 'then and else statements must be the same type'
...
class Get(PYSON):
    ...
    def __init__(self, v, k, d=''):
        ...
        if isinstance(obj, PYSON) and not obj.types() == set([dict]):
            raise AssertionError('obj must be a dict')
        else:
            if not isinstance(obj, dict):
                raise AssertionError('obj must be a dict')
            else:
                self._obj = obj
                if isinstance(key, PYSON) and not key.types() == set([str]):
...
class In(PYSON):
    ...
    def __init__(self, k, v):
        ...
        if isinstance(key, PYSON) and not key.types().issubset(set([str, int])):
            raise AssertionError('key must be a string or an integer or a long')
        else:
            if not isinstance(key, (str, int)):
                raise AssertionError('key must be a string or an integer or a long')
            elif isinstance(obj, PYSON):
                assert obj.types().issubset(set([dict, list])), 'obj must be a dict or a list'
                if obj.types() == set([dict]) and not isinstance(key, str):
                    raise AssertionError('key must be a string')
            else:
...
class Date(PYSON):
    ...
    def __init__(self, year=None, month=None, day=None, delta_years=0, delta_months=0, delta_days=0, **kwargs):
        ...
        for i in (year, month, day, delta_years, delta_months, delta_days):
            if isinstance(i, PYSON):
                if not i.types().issubset(set([int, int, type(None)])):
                    raise AssertionError('%s must be an integer or None' % (i,))
            else:
                assert isinstance(i, (int, type(None))), '%s must be an integer or None' % (i,)
    ...
    @staticmethod
    def eval(dct, context):
        return datetime.date.today() + relativedelta(year=(dct['y']),
          month=(dct['M']),
          day=(dct['d']),
          years=(dct['dy']),
          months=(dct['dM']),
...
class DateTime(Date):
    ...
    def __init__(self, year=None, month=None, day=None, hour=None, minute=None, second=None, microsecond=None, delta_years=0, delta_months=0, delta_days=0, delta_hours=0, delta_minutes=0, delta_seconds=0, delta_microseconds=0, **kwargs):
        ...
        (super(DateTime, self).__init__)(year=year, month=month, day=day, delta_years=delta_years, 
         delta_months=delta_months, delta_days=delta_days, **kwargs)
        for i in (hour, minute, second, microsecond,
         delta_hours, delta_minutes, delta_seconds, delta_microseconds):
            if isinstance(i, PYSON):
                if not i.types() == set([int, type(None)]):
                    raise AssertionError('%s must be an integer or None' % (i,))
            else:
                assert isinstance(i, (int, type(None))), '%s must be an integer or None' % (i,)
    ...
    def __repr_params__(self):
        ...
        return date_params[:3] + (self._hour, self._minute, self._second, self._microsecond) + date_params[3:] + (self._delta_hours, self._delta_minutes, self._delta_seconds,
    ...
    @staticmethod
    def eval(dct, context):
        return datetime.datetime.now() + relativedelta(year=(dct['y']),
          month=(dct['M']),
          day=(dct['d']),
          hour=(dct['h']),
          minute=(dct['m']),
...
class Len(PYSON):
    ...
    def __init__(self, v):
        ...
        if isinstance(v, PYSON) and not v.types().issubset(set([dict, list, str])):
            raise AssertionError('value must be a dict or a list or a string')
        else:
            assert isinstance(v, (dict, list, str)), 'value must be a dict or list or a string'
        self._value = v
