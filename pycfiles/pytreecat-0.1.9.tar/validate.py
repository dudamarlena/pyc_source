...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/validate.py
# Compiled at: 2017-08-14 23:03:39
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import multiprocessing, os, sys, numpy as np
...
def process_eval_task(task):
    ...
    model_path = os.path.join(models_dir, ('model.{}.pkz').format(serialize_config(config)))
    try:
...
def eval(dataset_path, param_csv_path, models_dir, result_path, **options):
    ...
    lines = [ [ row['config'][param] for param in header ] + [row['logprob'], row['l1_loss']] for row in result if 'logprob' in row and 'l1_loss' in row
            ]
    lines.sort()
    for line in lines:
