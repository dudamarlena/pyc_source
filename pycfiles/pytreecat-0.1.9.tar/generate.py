...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/generate.py
...
def generate_dataset(num_rows, num_cols, num_cats=4, rate=1.0):
    ...
    dataset = {'schema': {'ragged_index': ragged_index, 'tree_prior': np.zeros(K, np.float32)}, 
       'data': data}
    return dataset
...
def generate_dataset_file(num_rows, num_cols, num_cats=4, rate=1.0):
    ...
    """
    path = os.path.join(DATA, ('{}-{}-{}-{:0.1f}.dataset.pkz').format(num_rows, num_cols, num_cats, rate))
...
def generate_clean_dataset(tree, num_rows, num_cats):
    ...
    model = {'config': config, 'tree': tree, 
       'edge_logits': np.zeros(K, np.float32), 
       'suffstats': {'ragged_index': ragged_index, 
                     'vert_ss': vert_ss, 
                     'edge_ss': edge_ss, 
                     'feat_ss': feat_ss, 
                     'meas_ss': meas_ss}}
    ...
    dataset = {'schema': {'ragged_index': ragged_index}, 
       'data': data}
    return dataset


def generate_fake_model(num_rows, num_cols, num_cats, num_components, dataset=None):
...
def generate_fake_ensemble(num_rows, num_cols, num_cats, num_components):
    ...
    for sub_seed in range(3):
        ...
        model = generate_fake_model(num_rows, num_cols, num_cats, num_components, dataset)
...
def generate_model_file(num_rows, num_cols, num_cats=4, rate=1.0):
    ...
    """
    path = os.path.join(DATA, ('{}-{}-{}-{:0.1f}.model.pkz').format(num_rows, num_cols, num_cats, rate))
    V = num_cols
    K = V * (V - 1) // 2
    if os.path.exists(path):
        return path
