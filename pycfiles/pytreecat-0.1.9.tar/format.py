...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/format.py
# Compiled at: 2017-08-14 23:03:39
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import csv, gzip, hashlib, io, logging, os, re, sys
from collections import Counter
from collections import defaultdict
from contextlib import contextmanager
import jsonpickle, jsonpickle.ext.numpy, numpy as np, pandas as pd
...
def pickle_dump(data, filename):
    ...
    if filename.endswith('.pkz'):
        ...
        raise ValueError(('Cannot determine format: {}').format(os.path.basename(filename)))
...
@parsable
def join_csvs(column, csvs_in, csv_out, encoding_in='utf-8', encoding_out='utf-8'):
...
@parsable
def guess_schema(data_csvs_in, types_csv_out, values_csv_out, encoding='utf-8'):
    ...
    for name in feature_names:
        ...
        values[name].sort(key=lambda vc: (-vc[1], vc[0]))

    feature_types = [ guess_feature_type(totals[f], values[f]) for f in feature_names ]
    print(('Found {} features: {} categoricals + {} ordinals').format(len(feature_names), sum(t == CATEGORICAL for t in feature_types), sum(t == ORDINAL for t in feature_types)))
...
def load_schema(types_csv_in, values_csv_in, groups_csv_in, encoding='utf-8'):
    print(('Loading schema from {}, {}, {}').format(types_csv_in, values_csv_in, groups_csv_in))
    feature_names = []
    feature_types = {}
    with csv_reader(types_csv_in, encoding) as (reader):
        ...
        for row in reader:
            ...
            if typename not in VALID_TYPES:
                raise ValueError(('Invalid type: {}\n  expected one of: {}').format(typename, (', ').join(VALID_TYPES)))
...
def load_data(schema, data_csv_in, encoding='utf-8'):
    ...
    with csv_reader(data_csv_in, encoding) as (reader):
        ...
        for i, name in enumerate(header):
            if name in feature_types:
                metas[i] = (name, feature_types[name],
    ...
    print(('Loaded {} cells in {} rows, {:0.1f}% observed').format(cells, len(rows), 100.0 * cells / len(rows) / len(feature_types)))
...
@parsable
def import_data(data_csvs_in, types_csv_in, values_csv_in, groups_csv_in, dataset_out, encoding='utf-8'):
    ...
    data = np.concatenate([ load_data(schema, data_csv_in, groups_csv_in, encoding) for data_csv_in in data_csvs_in.split(',')
                          ])
