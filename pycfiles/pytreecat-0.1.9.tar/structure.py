...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/structure.py
...
class TreeStructure(object):
    ...
    def __eq__(self, other):
        return self._num_vertices == other._num_vertices and (self._tree_grid == other._tree_grid).all()
...
def make_propagation_program(grid, root=None):
    ...
    for v, parent, children in reversed(nested_program):
        ...
        for child in children:
            program[pos, :] = [
             OP_IN, v, child, edge_dict[(v, child)]]
            pos += 1
    ...
    for v, parent, children in nested_program:
        if parent is not None:
            program[pos, :] = [
             OP_OUT, v, parent, edge_dict[(v, parent)]]
            pos += 1
        program[pos, :] = [
         OP_DOWN, v, 0, 0]
        pos += 1
...
def sample_tree(grid, edge_logits, edges):
    ...
    accepted = 0
    log2_choices = []
    for e in np.random.permutation(E):
...
def order_vertices(edges):
    ...
    while stack:
        ...
        lhs = []
        rhs = []
        parts = [ (v2, orders[v2]) for v2 in neighbors[v] if orders[v2] is not None
                ]
