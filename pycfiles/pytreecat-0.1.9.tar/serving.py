...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/serving.py
...
def correlation(probs):
    ...
    mutual_information = entropy(probs.sum(0)) + entropy(probs.sum(1)) - entropy(probs.flatten())
...
class ServerBase(object):
    ...
    def median(self, counts, data):
        ...
        for v in range(V):
            ...
            result[:, beg:end] = quantize_from_probs2(marginals[:, beg:end], counts[v])
...
class TreeCatServer(ServerBase):
    ...
    def __init__(self, model):
        ...
        self._estimated_tree = tuple(estimate_tree(self._tree.complete_grid, self._edge_logits))
        self._tree.gc()
    ...
    def latent_perplexity(self):
        ...
        """
        V, E, M, R = self._VEMR
        latent_entropy = np.array([ entropy(self._vert_probs[v, :]) for v in range(V) ], dtype=np.float32)
        ...
        for root in range(V):
            ...
            for op, v, v2, e in program:
                if op == OP_ROOT:
                    ...
                    if v > v2:
                        trans = trans.T
                    messages[v, :, :] = np.dot(trans / vert_probs[v2, np.newaxis, :], messages[v2, :, :])
...
class EnsembleServer(ServerBase):
    ...
    def sample(self, N, counts, data=None):
        ...
        samples = np.concatenate([ server.sample(sub_N, counts, data) for server, sub_N in zip(self._ensemble, sub_Ns)
                                 ])
        np.random.shuffle(samples)
    ...
    def logprob(self, data):
        logprobs = np.stack([ server.logprob(data) for server in self._ensemble ])
...
class DataServer(object):
    ...
    def logprob(self, rows, evidence=None):
        ...
        if evidence is None:
            ...
            return self._server.logprob(data + ragged_evidence) - self._server.logprob(data + evidence)
            return
