...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/plotting.py
...
import numpy as np, scipy.cluster, scipy.linalg, scipy.spatial
...
def plot_chord(begin, end, spacing, color, alpha=None):
    ...
    xy[[1, 2], :] *= 1 - 0.6666666666666666 * dist + 0.16666666666666666 * dist ** 2 - spacing
    path = Path(xy, codes)
    patch = PathPatch(path, facecolor='none', edgecolor=color, lw=1, alpha=alpha)
    pyplot.gca().add_patch(patch)


def plot_circular(server, tree_samples=0, fontsize=8, color='#4488aa', contract=0.08):
    ...
    edges = server.estimate_tree()
    order, order_inv = order_vertices(edges)
    edges = np.array([ [order[v1], order[v2]] for v1, v2 in edges ], dtype=np.int32)
    ...
    for v, name in enumerate(feature_names):
        ...
        if y > 0:
            ...
            props['va'] = 'top'
            y += adjust
        pyplot.text(R_text * x, R_text * y, name, props, rotation=rot, fontsize=fontsize, alpha=alphas[v])
    ...
    pyplot.plot(np.stack([R_obs * X, X]), np.stack([R_obs * Y, Y]), color=color, lw=0.75)
    for v1, v2 in edges:
...
def add_panning_to_svg(source, destin=None):
    ...
    for line in source_lines:
        ...
        if line.startswith('<svg '):
            add('<svg height="100%" width="100%" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\n')
