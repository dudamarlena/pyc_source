...
# [GCC 8.4.0]
# Embedded file name: /home/fritz/github/posterior/treecat/treecat/training.py
# Compiled at: 2017-08-14 23:03:39
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import itertools, logging, numpy as np
...
def make_annealing_schedule(num_rows, epochs, sample_tree_rate):
    ...
    while num_assigned < num_rows:
        if state >= 0.0:
            yield (
             'add_row', next(row_to_add))
            state -= remove_rate
            num_assigned += 1
            next_batch -= sample_tree_rate
        else:
            yield (
             'remove_row', next(row_to_remove))
            state += add_rate
            num_assigned -= 1
...
@profile
@jit(nopython=True, cache=True)
def jit_add_row(ragged_index, data_row, tree_grid, program, assignments, vert_ss, edge_ss, feat_ss, meas_ss, vert_probs, edge_probs, feat_probs, meas_probs):
...
@profile
@jit(nopython=True, cache=True)
def jit_remove_row(ragged_index, data_row, tree_grid, assignments, vert_ss, edge_ss, feat_ss, meas_ss, edge_probs):
    for v, m in enumerate(assignments):
        vert_ss[(v, m)] -= 1
...
class TreeCatTrainer(object):
    ...
    def __init__(self, ragged_index, data, tree_prior, config):
        ...
        """
        logger.info('TreeCatTrainer of %d x %d data', data[0].shape[0], len(data))
        ...
        E = V - 1
        K = V * (V - 1) // 2
        M = self._config['model_num_clusters']
        assert M <= 128, ('Invalid model_num_clusters > 128: {}').format(M)
        self._VEKM = (V, E, K, M)
        self._vert_prior = 0.5
        self._edge_prior = 0.5 / M
        self._feat_prior = 0.5 / M
        self._meas_prior = self._feat_prior * np.array([ ragged_index[(v + 1)] - ragged_index[v] for v in range(V) ], dtype=np.float32).reshape((V, 1))
        self._gammaln_table = gammaln(np.arange(1 + N, dtype=np.float32) + self._edge_prior)
    ...
    def add_row(self, row_id):
        ...
        jit_add_row(self._ragged_index, self._data[row_id, :], self._tree.tree_grid, self._program, self._assignments[row_id, :], self._vert_ss, self._edge_ss, self._feat_ss, self._meas_ss, self._vert_probs, self._edge_probs, self._feat_probs, self._meas_probs)
    ...
    def remove_row(self, row_id):
        ...
        jit_remove_row(self._ragged_index, self._data[row_id, :], self._tree.tree_grid, self._assignments[row_id, :], self._vert_ss, self._edge_ss, self._feat_ss, self._meas_ss, self._edge_probs)
    ...
    def get_edge_logits(self):
        ...
        result = compute_edge_logits(M, self._tree.complete_grid, assignments, self._gammaln_table, vert_logits)
    ...
    def sample_tree(self):
        ...
        """
        logger.info('TreeCatTrainer.sample_tree given %d rows', len(self._assigned_rows))
    ...
    def estimate_tree(self):
        ...
        Returns:
            A pair (edges, edge_logits), where:
                ...
                edge_logits: A [K]-shaped numpy array of edge logits.
        """
        logger.info('TreeCatTrainer.estimate_tree given %d rows', len(self._assigned_rows))
        complete_grid = self._tree.complete_grid
        edge_logits = self.get_edge_logits()
    ...
    def logprob(self):
        ...
        for e, v1, v2 in self._tree.tree_grid.T:
            logprob += logprob_dc(self._edge_ss[e, :, :], self._edge_prior) - vertex_logits[v1] - vertex_logits[v2]
    ...
    def train(self):
        ...
        return {'config': self._config, 
           ...
           'assignments': self._assignments, 
           'suffstats': {'ragged_index': self._ragged_index, 
