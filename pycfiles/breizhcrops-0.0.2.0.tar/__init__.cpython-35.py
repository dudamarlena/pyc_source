...
# Embedded file name: build/bdist.macosx-10.6-intel/egg/breinify/__init__.py
# Compiled at: 2017-04-18 17:02:27
# Size of source mod 2**32: 10835 bytes
import base64, copy, hashlib, hmac, json, logging, requests, time
...
class Breinify:
    ...
    def send_activity(self, user, activity_type, category=None, description=None, tags=None, url=None, referrer=None, activity_time=None):
        """
    ...
    def lookup(self, user, dimensions):
        ...
        for dim in dimensions:
            if dim not in self.getSupportedLookupDimensions():
                raise brein_exception.invalidArgumentException(dim, self.getSupportedLookupDimensions())

        if self.secret is not None:
            self._Breinify__signLookup(to_push)
        response = requests.post(self.service_end_point + 'lookup', data=json.dumps(to_push))
        if response.status_code != 200:
            raise brein_exception.BreinAPIConnectionError(response)
        result = json.loads(response.text)
        return result

    def temporal_data(self, user=None, unixtime=None, ip=None, location_free_text=None, location_city=None, location_state=None, location_country=None, location_latitude=None, location_longitude=None, location_shapes=None):
        """
        ...
        if self.secret is not None:
            self._Breinify__signTemporal(to_push)
    ...
    def __pushActivity__(self, to_push):
        try:
            if self.secret is not None:
                self._Breinify__signActivity(to_push)
            res = requests.post(self.service_end_point + 'activity', data=json.dumps(to_push))
            return res
        except Exception as e:
            logging.getLogger('breinify').error(e)

    def __hashSig(self, to_push, message):
        signature = base64.b64encode(hmac.new(self.secret.encode('UTF-8'), str(message).encode('UTF-8'), digestmod=hashlib.sha256).digest()).decode('UTF-8')
    ...
    def __signActivity(self, to_push):
        ...
        self._Breinify__hashSig(to_push, message)

    def __signLookup(self, to_push):
        if self.secret is None:
            raise brein_exception.noSecretKeyException()
        message = to_push['lookup']['dimensions'][0] + str(to_push['unixTimestamp']) + str(len(to_push['lookup']['dimensions']))
        self._Breinify__hashSig(to_push, message)

    def __signTemporal(self, to_push):
        message = str(to_push['unixTimestamp']) + '-'
        if 'user' in to_push and 'additional' in to_push['user'] and 'localDateTime' in to_push['user']['additional']:
            message = message + str(to_push['user']['additional']['localDateTime'])
        message = message + '-'
        if 'user' in to_push and 'additional' in to_push['user'] and 'timeZone' in to_push['user']['additional']:
