...
# [GCC 8.4.0]
# Embedded file name: /home/doakey/Sync/Programming/reflowrst/reflowrst/tools/data2simplerst.py
...
def get_current_column(column_widths, string):
    total = 0
    current_column = 0
    while total < len(string):
        total += column_widths[current_column] + 2
        current_column += 1
...
def data2simplerst(table, spans, use_headers=True, headers_row=0):
    ...
    for row in range(len(table)):
        for column in range(len(table[row])):
            if not in_spans(row, column, spans):
            ...
            if includes_spans(table, row, spans):
                ...
                while column < len(table[row]):
                    ...
                    if table[row][column] == '':
                        ...
                        current_column = get_current_column(column_widths, new_underline)
                        new_underline += space_fill(column_widths[current_column], '-') + '  '
                    column += 1
