...
# [GCC 8.4.0]
# Embedded file name: /home/doakey/Sync/Programming/reflowrst/reflowrst/tools/data2rst.py
...
class TableTypeError(Exception):
    __doc__ = 'Raised if a Table is not a List of Lists'


class TableContentError(Exception):
    __doc__ = 'Raised if a Table is empty or has inconsistent column counts'
...
class Cell:
    __doc__ = 'Holds the text and data for an rst text cell'
        ...
        if self_right == other_left:
            if self_top == other_top:
                if self_bottom == other_bottom:
                    if self.right_sections >= other.left_sections:
                        return 'RIGHT'
        if self_left == other_left:
            if self_right == other_right:
                if self_top == other_bottom:
                    if self.top_sections >= other.bottom_sections:
                        return 'TOP'
        if self_left == other_left:
            if self_right == other_right:
                if self_bottom == other_top:
                    if self.bottom_sections >= other.top_sections:
                        return 'BOTTOM'
        if self_left == other_right:
            if self_top == other_top:
                if self_bottom == other_bottom:
                    if self.left_sections >= other.right_sections:
                        return 'LEFT'
        return 'NONE'
    ...
    def merge(self, other):
        ...
        if self.mergeableDirection(other) == 'RIGHT':
            ...
            return True
        if self.mergeableDirection(other) == 'TOP':
            ...
            return True
        if self.mergeableDirection(other) == 'BOTTOM':
            ...
            if self.mergeableDirection(other) == 'LEFT':
                for i in range(len(self_lines)):
    ...
    while len(span) > 0:
        ...
        for i in range(len(checked)):
            if row == checked[i][0] and abs(col - checked[i][1]) == 1:
                ...
                if abs(row - checked[i][0]) == 1 and col == checked[i][1]:
                    matched = True
...
def data2rst(table, spans=[
 [
  [
   0, 0]]], use_headers=True, center_cells=False, center_headers=False):
    checkTable(table)
    for row in range(len(table)):
        for column in range(len(table[row])):
