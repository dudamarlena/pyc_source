...
# [GCC 8.4.0]
# Embedded file name: /home/doakey/Sync/Programming/reflowrst/reflowrst/reflow.py
# Compiled at: 2018-01-31 21:15:07
# Size of source mod 2**32: 30723 bytes
...
def get_last_enumerator(lines, i):
    ...
    if i == 0:
        ...
        if i > 1:
            if lines[(i - 1)] == '':
                if lines[(i - 2)].replace(lines[(i - 2)].lstrip(), '') == this_leading_space:
                    return this_enumerator
...
def renumerate_enumerators(lines):
    ...
    for i in range(len(lines)):
        if is_enumerated_list_item(lines, i):
            ...
            if not is_bullet_list_item(lines, i) and not lines[i].rstrip() == '':
                first_enumerator = True
        ...
        if lines[index] == '':
            ...
            for x in range(index, len(lines)):
                if lines[x] == '' or lines[x].startswith(last_leading_space):
                    if is_enumerated_list_item(lines, x):
                        ...
                        if not lines[x] == '' and not lines[x].startswith(last_leading_space + '  '):
                            break
                else:
                    break
            ...
            for x in reversed(range(0, len(reflowed))):
                if reflowed[x] == '' or reflowed[x].startswith(last_leading_space):
                    if is_enumerated_list_item(reflowed, x):
                        ...
                        if not reflowed[x] == '' and not reflowed[x].startswith(last_leading_space + '  '):
                            break
                else:
                    break

            flat, index = collect_enumerated_list_item(lines, index, max_ewidth)
            if space:
            ...
            for i in range(index, len(lines)):
                if is_field(lines, i):
                    ...
                    if not lines[i].startswith(leading_space + '    '):
                        if not lines[i] == '':
                            break
                if lines[i] == '':
                    if i < len(lines) - 1:
                        if not lines[(i + 1)].startswith(leading_space + ' '):
                            if not lines[(i + 1)] == '':
                                break
...
def get_column_combo(height, col_height_width_dicts):
    ...
    for col in col_height_width_dicts:
        ...
        if type(addition).__name__ == 'int':
            ...
            if type(addition).__name__ == 'tuple':
                combo.extend(addition)

    return combo


def reevaluate_max_col_widths(max_col_widths, min_col_widths, space):
    """
...
def make_cell_width_dict(table, row, column, spans, max_col_widths, min_col_widths):
    ...
    breakpoints = get_breakpoints(table[row][column], min_col_widths[column], max_col_widths[column])
...
def get_key_column_widths(table, spans, max_col_widths, min_col_widths):
    """
    ...
    for row in range(len(table)):
        ...
        for column in range(len(table[row])):
            cell_width_dicts[(-1)].append(make_cell_width_dict(table, row, column, spans, max_col_widths, min_col_widths))
...
def make_spanned_column_height_width_dict(table, min_column_widths, column, long_row, spans, max_column_widths, cell_width_dicts, text_space):
    """
    ...
    for row in range(len(cell_width_dicts)):
        for col in range(len(cell_width_dicts[row])):
            if col in included_columns:
                in_spans(row, col, spans) or key_column_widths[col].extend(list(cell_width_dicts[row][col].keys()))
    ...
    for row in range(len(table)):
        ...
        for col in range(len(table[row])):
            ...
            for span in spans:
                if [
                 row, col] in span and span not in adjusted_spans:
    ...
    return (
     height_width_dict, column + span_col_count)
...
def get_cell_widths(table, spans, space):
    ...
    if sum(max_col_widths) <= text_space or space == 0:
        ...
        if sum(min_col_widths) > text_space:
            new_column_widths = min_col_widths
        else:
            max_col_widths = reevaluate_max_col_widths(max_col_widths, min_col_widths, space)
            cell_width_dicts, key_column_widths = get_key_column_widths(table, spans, max_col_widths, min_col_widths)
            col_height_width_dicts = []
            column = 0
            while column < column_count:
                long_row = has_column_span(table, column, spans)
                if not long_row:
                    col_height_width_dicts.append(make_column_height_width_dict(column, cell_width_dicts))
                    column += 1
                else:
                    height_width_dict, column = make_spanned_column_height_width_dict(table, min_col_widths, column, long_row, spans, max_col_widths, cell_width_dicts, text_space)
