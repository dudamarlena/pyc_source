...
# [GCC 8.4.0]
# Embedded file name: /Users/luoli/PycharmProjects/iqiyi/venv/lib/python2.7/site-packages/iqiyiop/AuthModule.py
# Compiled at: 2018-11-15 21:58:20
import os, base64, urllib, hashlib, hmac, json, logging, datetime, requests, binascii, random, string, urllib2
IAM_NOHEADERS = 400
...
IAM_SIGNINGKEY = 460
IAM_SIGNATURE = 480
errmessage = {400: '(IAM) missing required headers', 401: '(IAM) headers lack host', 402: '(IAM) header to singed no exist', 
   420: '(IAM) missing required method', 440: '(IAM) uri is empty', 460: '(IAM) signing key error', 
...
class ErrorInfo(object):
    ...
    def tostring(self):
        ...
        """
        info = '%s [Error %d]' % (self._message, self._code)
        if self._info:
            info += ' %s' % self._info
        return info
...
class BceSigner(object):
    ...
    def _calc_signingkey(self, auth):
        """ Get a a signing key """
        string_to_sign = ('/').join((auth['version'], auth['access'], auth['timestamp'], auth['period']))
        try:
    ...
    def _calc_signature(self, key, request, signedheaders):
        ...
        cr = ('\n').join((request['method'].upper(), self.normalized_uri(request['uri']), self.canonical_qs(params),
