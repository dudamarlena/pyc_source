...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dress_blog/views.py
...
class PostListView(ListView):
    ...
    def get_queryset(self):
        posts = Post.objects.for_app_models('dress_blog.story', 'dress_blog.quote').order_by('-pub_date')
        return posts
...
class TagDetailView(ListView):
    ...
    def get_queryset(self):
        return TaggedItem.objects.filter(tag__name__iexact=self.kwargs.get('slug', '')).order_by('-id')
...
class DiaryDetailView(DateDetailView):
    ...
    def get_object(self, queryset=None):
        ...
        day = self.get_day()
        self.date = _date_from_string(year, self.get_year_format(), month, self.get_month_format(), day, self.get_day_format())
        qs = queryset or self.get_queryset()
        if not self.get_allow_future() and self.date > datetime.date.today():
            raise Http404(_('Future %(verbose_name_plural)s not available because %(class_name)s.allow_future is False.') % {'verbose_name_plural': qs.model._meta.verbose_name_plural, 
