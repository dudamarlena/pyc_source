...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dress_blog/models.py
...
UI_COVER_CHOICES = (
 (
  '3col', _('3 columns')), ('4col', _('4 columns')))
THEMES_PATH = getattr(settings, 'DRESS_BLOG_THEMES_PATH', os.path.join(settings.STATIC_ROOT, 'dress_blog/themes'))
...
class Config(models.Model):
    ...
    title = models.CharField(max_length=100, help_text=_("Blog's name or title"))
    theme = models.CharField(max_length=50, default='initial', help_text=_('Design for the blog'), choices=themes(THEMES_PATH))
    posts_in_index = models.IntegerField(default=1, help_text=_('Visible posts in 3-columns layout front page.'))
    stories_in_index = models.IntegerField(default=5, help_text=_('Visible stories in 4-columns layout front page.'))
    quotes_in_index = models.IntegerField(default=5, help_text=_('Visible quotes in 4-columns layout front page.'))
    diary_entries_in_index = models.IntegerField(default=1, help_text=_('Visible days in diary in front page.'))
    comments_in_index = models.IntegerField(default=5, help_text=_('Visible comments in front page.'))
    email_subscribe_url = models.URLField(_('subscribe via email url'), blank=True, null=True)
    show_author = models.BooleanField(default=False, help_text=_("Show author's full name along in posts"))
    ping_google = models.BooleanField(default=False, help_text=_('Notify Google on new submissions'))
    excerpt_length = models.IntegerField(default=500, help_text=_('The character length of the post body field displayed in RSS and preview templates.'))
    meta_author = models.CharField(max_length=255, help_text=_("List of authors or company/organization's name"))
    meta_keywords = models.CharField(max_length=255, help_text=_('List of keywords to help improve quality of search results'))
    meta_description = models.TextField(blank=True, help_text=_('What the blog is about, topics, editorial line...'))
...
class BasePost(models.Model):
    ...
    def get_content_object_url(self):
        ...
        return urlresolvers.reverse('post-url-redirect', args=(
         self.content_type_id, self.object_pk))
...
class PostManager(models.Manager):
    ...
    def published(self):
        return self.get_query_set().filter(status__gte=2, pub_date__lte=datetime.datetime.now())

    def draft(self, author=None):
        if not author:
            return self.get_query_set().filter(status=1).order_by('-mod_date')
        else:
            return self.get_query_set().filter(status=1, author=author).order_by('-mod_date')
    ...
    def for_content_types(self, content_types):
        return self.get_query_set().filter(content_type__in=content_types, status__gte=2, pub_date__lte=datetime.datetime.now())
...
class Story(Post):
    ...
    def save(self, *args, **kwargs):
        ...
        self.abstract_markup = mark_safe(formatter(self.abstract, filter_name=self.markup))
...
class Quote(Post):
    ...
    body = TextFieldWithInlines()
    body_markup = models.TextField(editable=True, blank=True, null=True)
    quote_author = models.CharField(blank=False, null=False, max_length=255, help_text=_("quote's author"))
...
class BlogRoll(models.Model):
    ...
    authors = models.CharField(blank=False, null=False, max_length=1024, help_text=_('Comma separated list of authors'))
