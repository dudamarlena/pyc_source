...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dress_blog/feeds.py
...
class PostsByTag(Feed):
    ...
    def title(self, obj):
        return "%s posts tagged as '%s' feed" % (self.config.title, obj.name)

    def description(self):
        return "%s latest posts tagged as '%s' feed." % (self.config.title, obj.name)
