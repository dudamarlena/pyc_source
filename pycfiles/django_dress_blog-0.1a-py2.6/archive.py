...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dress_blog/templatetags/archive.py
...
class StoryArchive(template.Node):
    ...
    def render(self, context):
        dates = Story.objects.published().dates('pub_date', 'month', order='DESC')
        if dates:
...
class QuoteArchive(template.Node):
    ...
    def render(self, context):
        dates = Quote.objects.published().dates('pub_date', 'month', order='DESC')
        if dates:
            context[self.var_name] = dates
        return ''
...
def _get_content_types(tagname, tokens):
    ...
    for token in tokens:
        try:
            ...
            raise TemplateSyntaxError("Argument %s in %r must be in the format 'app.model'" % (
             token, tagname))
        except ContentType.DoesNotExist:
            raise TemplateSyntaxError("%r tag has non-existant content-type: '%s.%s'" % (
             tagname, app, model))
