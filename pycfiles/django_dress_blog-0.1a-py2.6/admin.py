...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dress_blog/admin.py
...
class ConfigAdmin(admin.ModelAdmin):
    fieldsets = (
     ...
     (
      'META',
      {'fields': ('meta_author', 'meta_description', 'meta_keywords')}))
...
class StoryAdmin(AdminTextFieldWithInlinesMixin, PostAdmin):
    ...
    fieldsets = (
     (None,
      {'fields': ('title', 'slug', 'markup', 'abstract', 'body')}),
     (
      'Post data',
     ...
     (
      'Converted markup',
      {'classes': ('collapse', ), 'fields': ('abstract_markup', 'body_markup')}))
...
class DiaryDetailInline(AdminTextFieldWithInlinesMixin, admin.StackedInline):
    ...
    fieldsets = (
     (None,
      {'fields': (
                  ('status', 'author', 'markup', 'tags', 'allow_comments'),
                  ('body', 'pub_date'))}),
     (
      'Converted markup',
      {'classes': ('collapse', ), 'fields': ('body_markup', )}))
    extra = 0
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        field = super(DiaryDetailInline, self).formfield_for_dbfield(db_field, **kwargs)
        from django import forms
        from django.contrib.auth import models
        if db_field.name == 'author':
            if kwargs.get('request', False):
                ...
                queryset = models.User.objects.all()
                field = forms.ModelChoiceField(queryset=queryset, initial=current_user.id)
