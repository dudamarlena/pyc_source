...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vxstreamlib.py
# Compiled at: 2018-08-09 16:21:55
# Size of source mod 2**32: 22966 bytes
import gzip, hashlib, io, json, logging, os.path, re, shutil, smtplib, sys, tempfile, threading, time, traceback, warnings, zipfile
...
class VxStreamSubmission(object):
    __doc__ = 'Represents a sample that was submitted to VxStream.'
...
class VxStreamSubmissionManager(object):
    __doc__ = 'Utility class that manages multiple submissions with notification callbacks.'

    def __init__(self, server, submissions, delay=1):
        if not isinstance(server, VxStreamServer):
    ...
    def execute(self):
        ...
        for submission in self.submissions.values():
            ...
            if result == VXSTREAM_STATUS_ERROR:
                ...
                if result == VXSTREAM_STATUS_SUCCESS:
...
class VxStreamServer(object):
    ...
    def get_status(self, sha256, environment_id):
        status_url = '{}/api/state/{}'.format(self.url, self.get_sample_url(sha256, environment_id))
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            result = requests.get(status_url, verify=False, headers=VXSTREAM_HEADERS, proxies=(self.proxies))
        if result.status_code != 200:
            logging.error('got result {} from vxstream: {}'.format(result.status_code, result.reason))
            return
        else:
            json_result = json.loads(result.text)
            logging.debug('got response_code {} state {} for sha256 {} environment_id {}'.format(json_result['response_code'], json_result['response']['state'], sha256, environment_id))
            return json_result['response']['state']

    def get_sample_url(self, sha256, environment_id):
        return '{}/?apikey={}&secret={}&environmentId={}'.format(sha256, self.api_key, self.secret, environment_id)
    ...
    def reanalyze(self, file_path, environment_id):
        ...
        result = requests.post(url, data=data, verify=False, headers=VXSTREAM_HEADERS, proxies=(self.proxies))
    ...
    def submit(self, file_path, environment_id):
        ...
        if status != VXSTREAM_STATUS_UNKNOWN:
            ...
            with open(file_path, 'rb') as (fp_binary):
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter('ignore')
                    result = requests.post(submit_url, data=data, files=files, verify=False, headers=VXSTREAM_HEADERS, proxies=(self.proxies))
                if result.status_code != 200:
    ...
    def download(self, sha256, environment_id, _type, path):
        download_url = '{}/api/result/{}&type={}'.format(self.url, self.get_sample_url(sha256, environment_id), _type)
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            logging.info('downloading {} for {} env {}'.format(_type, sha256, environment_id))
            result = requests.get(download_url, verify=False, headers=VXSTREAM_HEADERS, proxies=(self.proxies))
        if result.status_code != 200:
            logging.error('got result {} from vxstream: {}'.format(result.status_code, result.reason))
            return
        else:
    ...
    def download_dropped_files(self, sha256, environment_id, target_dir):
        ...
        download_url = '{}/api/sample-dropped-files/{}?environmentId={}&apikey={}&secret={}'.format(self.url, sha256, environment_id, self.api_key, self.secret)
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
        ...
        try:
            ...
            for compressed_path in file_list:
                if '.DROPPED.' not in compressed_path:
                    ...
                    DROPPED_FILE_REGEX = re.compile('^(.+?)\\.[0-9]+\\.DROPPED\\.gz')
...
def main():
    ...
    parser.add_argument('--from-stdin', required=False, default=False, action='store_true', dest='from_stdin', help='Read the files to scan from standard input.')
    parser.add_argument('--sha256', required=False, default=None, dest='sha256', help='Download the results of the given sha256 hash.')
    parser.add_argument('--environment_id', required=False, default='3', dest='environment_id', help='Select the environment ID to use.')
    parser.add_argument('-u', '--url', required=False, default='https://vxstream.local', dest='url', help='The URI to submit the files to.')
    parser.add_argument('-d', '--dir', required=False, default='vxstream.out', dest='output_dir', help='The output directory to place the downloaded results into.')
    parser.add_argument('-l', '--log-level', required=False, default='WARNING', dest='log_level', help='The logging level to use for log events.')
    parser.add_argument('-e', '--email', required=False, action='append', default=[], dest='email_addresses', help='Send an email when completed.  Can specify more than one of these options.')
    parser.add_argument('--smtp-server', required=False, default='ashsmtp.asco.ashland.com', dest='smtp_server', help='Specify an alternate SMTP server to use.')
    ...
    parser.add_argument('--delay', required=False, default=1, type=int, dest='delay', help='The number of seconds to delay in between each attempt to query status of submitted files.')
    parser.add_argument('--split-memory-dump', required=False, default=False, action='store_true', dest='split_memory', help='Download the memory dump as separate files instead of an individual file.')
