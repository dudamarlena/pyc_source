...
# Embedded file name: /Users/ryanh/src/pynsot/tests/test_models.py
# Compiled at: 2019-10-16 17:52:59
# Size of source mod 2**32: 9733 bytes
__doc__ = '\nTest the Models\n'
...
def test_payload_not_none_raw_and_not(client, site):
    ...
    assert n.payload
    assert n.ensure()
    n2 = Network(raw=(get_result(client.sites(site['id']).networks('8.8.8.0/24').get())))
    assert n2.payload
    assert len(n2)
    assert n == n2
    assert n2.keys()
