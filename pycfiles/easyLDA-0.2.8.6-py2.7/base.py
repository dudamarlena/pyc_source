...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/easyLDA/base.py
# Compiled at: 2018-02-12 16:45:08
import sys
try:
    reload(sys)
...
class PipelineLDA(object):
    ...
    def __init__(self, path, topics, grams, passes=1):
        ...
        logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO, filename=self.folder + self.name + ('_log.log').format(self.name), filemode='w')
    ...
    def train(self):
        ...
        self.ldamodel = Lda(self.doc_term_matrix, num_topics=num_topics, id2word=self.dictionary, passes=self.passes)
        print ('used: {:.2f}s').format(time() - start)
