...
# [GCC 8.4.0]
# Embedded file name: /home/hedley/django/instances/mote/mote/models.py
...
RESERVED_IDS = ('mote', 'json', 'metadata.json', 'projects', 'aspects', 'patterns',
                'elements', 'variations')
...
class Base(object):
    ...
    def _get_children(self, klass, subdirectory=''):
        ...
        if os.path.exists(pth):
            li = [klass(id, self) for id in os.listdir(pth) if not id.startswith('.') and id not in RESERVED_IDS]
...
class Variation(Base):
    __doc__ = 'A variation *is* an element but the subclassing breaks down'
...
class Aspect(Base):
    __doc__ = 'Examples of aspects are website, email templates.'
...
class Project(Base):
    ...
    def metadata(self):
        ...
        try:
            ...
            raise RuntimeError('Project %s needs a metadata.json file' % self.id)
