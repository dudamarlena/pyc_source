...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/corenlp/__init__.py
# Compiled at: 2014-04-18 18:09:35
import xml.etree.cElementTree as ET, nltk
from collections import defaultdict

class Document:

    def __init__(self, xmlfile, pos=True, lemma=True, ner=True, parse=True, coref=True, basic_deps=False, coll_deps=False, coll_ccp_deps=True, verbose=False):
        sents, coref_chains = _parse_source(xmlfile, use_pos=pos, use_lemma=lemma, use_ner=ner, use_parse=parse, use_coref=coref, use_basic_deps=basic_deps, use_coll_deps=coll_deps, use_coll_ccp_deps=coll_ccp_deps, verbose=verbose)
        self.sents = sents
...
class Sentence:
    ...
    def __init__(self, tokens, parse, basic_deps, collapsed_deps, collapsed_ccproc_deps, idx, sentiment, sentiment_value):
    ...
    def __repr__(self):
        return ('Sentence ({}) {}').format(self.idx, unicode(self))
...
class Token:
    ...
    def __init__(self, surface, lem, pos, ner, char_offset_begin, char_offset_end, idx):
...
class TypedDependency:
    ...
    def __repr__(self):
        return ('(Dep:{} {} :: Type: {} :: Gov: {} {})').format(self.dep.idx, self.dep, self.type, self.gov.idx, self.gov)
...
class Mention:
    ...
    def __str__(self):
        return ('Mention(sent={}, start={}, head={}, end={})').format(self.sent, self.start, self.head, self.end)
...
def _parse_source(source, use_pos=True, use_lemma=True, use_ner=True, use_parse=True, use_coref=True, use_basic_deps=False, use_coll_deps=False, use_coll_ccp_deps=True, verbose=False):
    _word = None
    _lemma = None
    _pos = None
    _ner = None
        ...
        if event == 'start':
            ...
            if _word == '``' or _word == "''":
                if _char_offset_end - _char_offset_begin == 1:
                    _word = '"'
            _tokens.append(Token(_word, _lemma, _pos, _ner, _char_offset_begin, _char_offset_end, _token_idx))
            _word = None
            _lemma = None
            _pos = None
            _ner = None
            ...
            if idx > -1:
                ...
                _governor = Token('ROOT', 'root', 'ROOT', None, None, None, -1)
            ...
            _mentions.append(Mention(_coref_start, _coref_end, _coref_head, _coref_sentence))
            ...
            if _not_in_coref:
                ...
                sents.append(Sentence(_tokens, _parse, _basic_deps, _collapsed_deps, _collapsed_ccproc_deps, _sent_idx, sentiment, sentiment_val))
                _tokens = []
                _parse = None
                _basic_deps = None
                _collapsed_deps = None
