...
class JSONAPIMetadata(SimpleMetadata):
    ...
    try:
        ...
        """
            ...
            return OrderedDict([(
             format_value(field_name), self.get_field_info(field)) for field_name, field in serializer.fields.items()])
                        ...
                        if not field_info.get('relationship_resource'):
                            if hasattr(field, 'choices'):
                                field_info['choices'] = [{'value':choice_value, 
                                 'display_name':force_str(choice_name, strings_only=True)} for choice_value, choice_name in field.choices.items()]
