...
# [GCC 8.4.0]
# Embedded file name: /home/sao/src/github/django-json-api/django-rest-framework-json-api/rest_framework_json_api/metadata.py
# Compiled at: 2019-12-13 14:54:46
...
class JSONAPIMetadata(SimpleMetadata):
    ...
    type_lookup = ClassLookupDict({serializers.Field: 'GenericField', 
    ...
    try:
        relation_type_lookup = ClassLookupDict({related.ManyToManyDescriptor: 'ManyToMany', 
         related.ReverseManyToOneDescriptor: 'OneToMany', 
         related.ForwardManyToOneDescriptor: 'ManyToOne'})
    except AttributeError:
        relation_type_lookup = ClassLookupDict({related.ManyRelatedObjectsDescriptor: 'ManyToMany', 
    ...
    def get_serializer_info(self, serializer):
        ...
        return OrderedDict([(format_value(field_name), self.get_field_info(field)) for field_name, field in serializer.fields.items()])
    ...
    def get_field_info(self, field):
        ...
        if not field_info.get('read_only') and not field_info.get('relationship_resource') and hasattr(field, 'choices'):
            field_info['choices'] = [{'value': choice_value, 'display_name': force_str(choice_name, strings_only=True)} for choice_value, choice_name in field.choices.items()]
