...
# Compiled at: 2020-02-07 13:31:53
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if expand_polymorphic_types:
                    if issubclass(serializer, PolymorphicModelSerializer):
...
def get_related_resource_type(relation):
    ...
    if hasattr(relation, '_meta'):
        ...
        if hasattr(relation, 'model'):
            relation_model = relation.model
        else:
            if hasattr(relation, 'get_queryset'):
                if relation.get_queryset() is not None:
                    relation_model = relation.get_queryset().model
    if getattr(relation, 'many', False) and hasattr(relation.child, 'Meta') and hasattr(relation.child.Meta, 'model'):
        relation_model = relation.child.Meta.model
    else:
        parent_serializer = relation.parent
        parent_model = None
        if parent_serializer.instance is not None:
            parent_model = parent_serializer.instance._meta.model
        else:
            if hasattr(parent_serializer, 'Meta'):
                ...
                if hasattr(parent_serializer, 'parent'):
                    if hasattr(parent_serializer.parent, 'Meta'):
        ...
        if parent_model is not None:
            ...
            if parent_model_relation_type is ReverseManyToOneDescriptor:
                ...
                if parent_model_relation_type is ManyToManyDescriptor:
                    ...
                    if parent_model_relation_type is ReverseGenericManyToOneDescriptor:
