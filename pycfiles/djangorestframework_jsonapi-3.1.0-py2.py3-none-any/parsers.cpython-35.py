...
class JSONParser(parsers.JSONParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        if isinstance(view, RelationshipView):
            if isinstance(data, list):
                for resource_identifier_object in data:
                    if not (resource_identifier_object.get('id') and resource_identifier_object.get('type')):
                        raise ParseError('Received data contains one or more malformed JSONAPI Resource Identifier Object(s)')

            else:
                if not (data.get('id') and data.get('type')):
            ...
            if request.method in ('PUT', 'POST', 'PATCH'):
                ...
                if data.get('type') != resource_name:
                    raise exceptions.Conflict("The resource object's type ({data_type}) is not the type that constitute the collection represented by the endpoint ({resource_type}).".format(data_type=data.get('type'), resource_type=resource_name))
        elif data.get('type') not in resource_name:
            raise exceptions.Conflict("The resource object's type ({data_type}) is not the type that constitute the collection represented by the endpoint (one of [{resource_types}]).".format(data_type=data.get('type'), resource_types=', '.join(resource_name)))
        ...
        if serializer_class is not None and issubclass(serializer_class, serializers.PolymorphicModelSerializer):
