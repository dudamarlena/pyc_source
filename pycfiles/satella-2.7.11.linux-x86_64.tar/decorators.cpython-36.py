...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.6.7/lib/python3.6/site-packages/satella/coding/decorators.py
# Compiled at: 2020-04-22 08:35:40
# Size of source mod 2**32: 6591 bytes
import itertools, typing as tp
from ..exceptions import PreconditionError
__all__ = [
 'precondition', 'for_argument', 'PreconditionError', 'short_none', 'has_keys',
...
def precondition(*t_ops: tp.Union[(tp.Callable[([T], bool)], Expression)], **kw_opts: tp.Union[(tp.Callable[([T], bool)], Expression)]):
    """
    ...
    def outer(fun):
        ...
        def inner(*args, **kwargs):
            ...
            for kwarg in kwargs:
                if kwarg in kw_ops:
                    raise kw_ops[kwarg](kwargs[kwarg]) or PreconditionError('Argument %s failed precondition check' % (kwarg,))
...
def for_argument(*t_ops: tp.Callable[([T], U)], **t_kwops: tp.Callable[([T], U)]):
    ...
    def outer(fun):
        ...
        def inner(*args, **kwargs):
            ...
            a = fun(*(_NOP if op is None else op(arg) for arg, op in itertools.zip_longest(args, t_ops, fillvalue=None)), **)
            return returns(a)
