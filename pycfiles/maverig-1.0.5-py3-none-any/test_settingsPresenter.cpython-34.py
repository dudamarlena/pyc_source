...
# [GCC 8.4.0]
# Embedded file name: C:\Users\Sascha\Documents\PycharmProjects\maverig\maverig\tests\test_settingsPresenter.py
# Compiled at: 2015-02-17 04:36:04
# Size of source mod 2**32: 4947 bytes
...
class TestSettingsPresenter(TestCase):
    ...
    def test_install_language(self):
        """check default language and select a different language for testing"""
        if self.settings_presenter.model.language == 'de_DE':
            language = 'en_EN'
        else:
            if self.settings_presenter.model.language == 'en_EN':
                language = 'de_DE'
            else:
                if self.settings_presenter.model.language == 'fr_FR':
                    language = 'de_DE'
                else:
                    language = 'de_DE'
            self.settings_presenter.install_language(True)
            if not self.settings_presenter.model.language != language:
