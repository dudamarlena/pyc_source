...
# [GCC 8.4.0]
# Embedded file name: kafkaka/client.py
# Compiled at: 2015-01-09 02:15:27
import itertools
from kafkaka.define import DEFAULT_SOCKET_TIMEOUT_SECONDS, DEFAULT_KAFKA_PORT, DEFAULT_RETRY_TIMES
from kafkaka.define import KafkaError, ConnectionError, LeaderNotAvailable, UnknownTopicOrPartition, check_and_raise_error
from kafkaka.conn import Connection as KafkaConnection
from kafkaka.protocol import MetaStruct, MetaResponseStruct, ProduceStruct, ProduceResponseStruct
import logging
...
class KafkaClient(object):
    ...
    def __init__(self, hosts, client_id=CLIENT_ID, timeout=DEFAULT_SOCKET_TIMEOUT_SECONDS, retry_times=DEFAULT_RETRY_TIMES, topic_names=[]):
    ...
    def _pack_boot_metadata(self, expected_topics):
        ...
        request_bytes = MetaStruct(head=dict(correlation_id=correlation_id), topic_length=len(expected_topics), topics=_topics).pack2bin()
        return (
         correlation_id, _topics, request_bytes)
    ...
    def _pack_send_message(self, topic_name, *msg):
        ...
        correlation_id = self._get_next_correlation_id()
        request_bytes = ProduceStruct(head=dict(correlation_id=correlation_id), payloads=[
         dict(topic_name=topic_name, topic_payloads=[
          dict(partition=partition_id, message_set=[ dict(message=dict(message=dict(value=v))) for v in msg ])])]).pack2bin()
