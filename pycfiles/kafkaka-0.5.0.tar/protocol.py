...
# [GCC 8.4.0]
# Embedded file name: kafkaka/protocol.py
# Compiled at: 2015-01-12 05:14:22
from kafkaka.bstruct import Struct, ShortField, IntField, CharField, UnsignedCharField, LongLongField, UnsignedIntField, Crc32Field
...
if __name__ == '__main__':
    s = HeadStruct(KafkaProtocol.METADATA_KEY, 0, 111)
    print s.pack2bin()
    s2 = HeadStruct(KafkaProtocol.METADATA_KEY, 0, 112, client_id='ok')
    print s2.pack2bin()
    m1 = MetaStruct(head=dict(correlation_id=111))
    print m1.dump2nametuple()
    print m1.pack2bin()
    print m1._values
    m2 = MetaStruct(head=dict(correlation_id=222))
    print m2.pack2bin()
    binary = m1.pack2bin()
    m3 = MetaStruct()
    print 'unpack'
    ...
    msg = MessageStruct(value='test')
    print (
     msg.dump2nametuple(), msg.pack2bin())
    crc_msg = Crc32MessageStruct(message=dict(value='test'))
    assert crc_msg.pack2bin() == b'Y*G\x87\x00\x00\xff\xff\xff\xff\x00\x00\x00\x04test'
    crc_msg_with_length = Crc32MessageStructWithLength(message=dict(message=dict(value='test')))
    assert crc_msg_with_length.pack2bin() == b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x12Y*G\x87\x00\x00\xff\xff\xff\xff\x00\x00\x00\x04test'
    msg_set = MessageSetStruct(partition=1, message_set=[
     dict(message=dict(message=dict(value='test')))])
    msg_set.pack2bin()
    produce_msg = ProduceStruct(head=dict(correlation_id=1), payloads=[
     dict(topic_name='im-msg', topic_payloads=[
      dict(partition=0, message_set=[
       dict(message=dict(message=dict(value='test')))])])])
    assert produce_msg.pack2bin() == b'\x00\x00\x00\x00\x00\x00\x00\x01\x00\x07kafkaka\x00\x01\x00\x00\x03\xe8\x00\x00\x00\x01\x00\x06im-msg\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x1e\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x12Y*G\x87\x00\x00\xff\xff\xff\xff\x00\x00\x00\x04test'
    out = ProduceStruct()
    ...
    print [p]
    print [
     ProduceStruct(head=dict(correlation_id=1), payloads=[
      dict(topic_name='im-msg', topic_payloads=[
       dict(partition=0, message_set=[
        dict(message=dict(message=dict(value=p)))])])]).pack2bin()]