...
# [GCC 8.4.0]
# Embedded file name: /home/alex/mgxrace/django-warmama/warmama/views.py
# Compiled at: 2015-05-16 23:08:12
# Size of source mod 2**32: 20357 bytes
import logging, random, requests, six, uuid
...
class ServerLogin(APIView):
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        server = models.Server.objects.get(_ip_flt(ipv4, ipv6), login=POST['authkey'], banned=False)
        server.hostname = POST['hostname']
        server.demos_baseurl = POST['demos_baseurl']
        server.save()
        session, created = models.ServerSession.objects.get_or_create(user=server, ip=ipv4, ipv6=ipv6, defaults={'port': POST['port']})
        if not created:
...
class ServerLogout(APIView):
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        ...
        """
        session = models.ServerSession.objects.get(_ip_flt(ipv4, ipv6), pk=POST['ssession'])
...
class ServerClientConnect(APIView):
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        server_session = models.ServerSession.objects.get(_ip_flt(ipv4, ipv6), pk=POST['ssession'])
        caddr, _ = POST['cip']
        cipv4, cipv6 = _ipv4_ipv6(caddr)
        logger.info('SCC pk %d, tid %d, ts %d, now %s', POST['csession'], POST['cticket'], server_session.user_id, timezone.now())
        client_session = models.PlayerSession.objects.select_related('user').get(_ip_flt(cipv4, cipv6), pk=POST['csession'], ticket_id=POST['cticket'], ticket_server=server_session, ticket_expiration__gt=timezone.now())
        logger.info('Attaching client session %s to server session %s', client_session, server_session)
        client_session.ticket_id = None
        client_session.ticket_server = None
        client_session.ticket_expiration = None
        client_session.server_session = server_session
        client_session.save()
        stats = models.PlayerStat.objects.select_related('gametype').filter(player_id=client_session.user_id).order_by('gametype__name')
        data = {'id': client_session.pk, 
         'login': client_session.user.login, 
         'ratings': [{'gametype': stat.gametype.name,  'rating': float(stat.rating),  'deviation': float(stat.deviation)} for stat in stats]}
        return JsonResponse(data)


class ServerClientDisconnect(APIView):
    __doc__ = 'Server client disconnect'
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        server_session = models.ServerSession.objects.get(_ip_flt(ipv4, ipv6), pk=POST['ssession'])
        client_session = models.PlayerSession.objects.get(pk=POST['csession'], server_session=server_session)
        if POST['gameon']:
            logger.info('Adding session %d to purgables', client_session.pk)
            models.PurgePlayer.objects.create(player_id=client_session.user_id, session=client_session, server_session=server_session)
        client_session.ticket_id = None
        client_session.ticket_server = None
        client_session.ticket_expiration = None
        client_session.server_session = None
        client_session.save()
        logger.info('ServerClientDisconnect %d %d ok', client_session.pk, server_session.pk)
        return JsonResponse({'status': 1})


class ServerHeartbeat(APIView):
    __doc__ = 'Server heartbeat'
...
class ServerMatchReport(APIView):
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        ...
        ratings = {'gametype': matchresult.gametype.name, 
         'ratings': [[sessionid, float(matchplayer.newrating)] for sessionid, matchplayer in matchplayers.items()]}
        models.PlayerSession.objects.filter(server_session=server_session, purgable=True).delete()
        return JsonResponse({'status': 1,  'ratings': ratings})


class ServerMatchUUID(APIView):
    __doc__ = 'Server Match UUID'
    exc_response = {'uuid': ''}
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        ...
        server_session.save()
        logger.info('MatchUUID request from %s, returning %s', ipv4 or ipv6, server_session.next_match_uuid)
        return JsonResponse({'uuid': server_session.next_match_uuid})


class ClientLogin(APIView):
    __doc__ = 'Client Login'
    ...
    def _step_one(self, request, POST=None, ipv4='', ipv6=''):
        ...
        logger.info('Clientlogin 1 login: %s, ip: %s', POST['login'], ipv4 or ipv6)
    ...
    def _step_two(self, request, POST=None, ipv4='', ipv6=''):
        ...
        logger.info('Clientlogin 2 handle: %d, ip: %s', POST['handle'], ipv4 or ipv6)
        login_player = models.LoginPlayer.objects.get(pk=POST['handle'])
        if not login_player.ready:
            logger.info('Clientlogin 2 not ready. handle: %d, ip: %s', POST['handle'], ipv4 or ipv6)
        ...
        if not created:
            ...
            if player_session.ip != ipv4:
                if player_session.ipv6 != ipv6:
        ...
        player_session.save()
        stats = models.PlayerStat.objects.select_related('gametype').filter(player=player).order_by('gametype__name')
...
class ClientLogout(APIView):
    __doc__ = 'Client Logout'

    class PostForm(forms.Form):
        csession = forms.IntegerField(min_value=1)

    def post(self, request, POST=None, ipv4='', ipv6=''):
        client_session = models.PlayerSession.objects.get(_ip_flt(ipv4, ipv6), pk=POST['csession'])
...
class ClientConnect(APIView):
    ...
    def post(self, request, POST=None, ipv4='', ipv6=''):
        sipv4, sipv6 = _ipv4_ipv6(POST['saddr'][0])
        port = POST['saddr'][1] or settings.WARMAMA_DEFAULT_SERVER_PORT
        server_session = models.ServerSession.objects.get(_ip_flt(sipv4, sipv6), port=port)
        client_session = models.PlayerSession.objects.get(_ip_flt(ipv4, ipv6), pk=POST['csession'])
        logger.info('Found server %d for client %d (addr %s)', server_session.pk, client_session.pk, sipv4 or sipv6)
        client_session.ticket_id = random.randint(1, 268435455)
