...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mtcnn/mtcnn.py
# Compiled at: 2019-11-14 12:16:36
# Size of source mod 2**32: 17537 bytes
import cv2, numpy as np, pkg_resources
...
class MTCNN(object):
    ...
    def detect_faces(self, img) -> list:
        ...
        """
        if not (img is None or hasattr(img, 'shape')):
            raise InvalidImage('Image not valid.')
        height, width, _ = img.shape
        stage_status = StageStatus(width=width, height=height)
        m = 12 / self._min_face_size
        min_layer = np.amin([height, width]) * m
        scales = self._MTCNN__compute_scale_pyramid(m, min_layer)
        stages = [
         self._MTCNN__stage1, self._MTCNN__stage2, self._MTCNN__stage3]
        ...
        for bounding_box, keypoints in zip(total_boxes, points.T):
            bounding_boxes.append({'box':[
             ...
             'keypoints':{'left_eye':(
               int(keypoints[0]), int(keypoints[5])), 
              'right_eye':(
               int(keypoints[1]), int(keypoints[6])), 
              'nose':(
               int(keypoints[2]), int(keypoints[7])), 
              'mouth_left':(
               int(keypoints[3]), int(keypoints[8])), 
              'mouth_right':(
               int(keypoints[4]), int(keypoints[9]))}})
    ...
    def __stage1(self, image, scales: list, stage_status: StageStatus):
        ...
        status = stage_status
        for scale in scales:
            scaled_image = self._MTCNN__scale_image(image, scale)
            img_x = np.expand_dims(scaled_image, 0)
            img_y = np.transpose(img_x, (0, 2, 1, 3))
            out = self._pnet.predict(img_y)
            out0 = np.transpose(out[0], (0, 2, 1, 3))
            out1 = np.transpose(out[1], (0, 2, 1, 3))
            boxes, _ = self._MTCNN__generate_bounding_box(out1[0, :, :, 1].copy(), out0[0, :, :, :].copy(), scale, self._steps_threshold[0])
            pick = self._MTCNN__nms(boxes.copy(), 0.5, 'Union')
        ...
        if numboxes > 0:
            pick = self._MTCNN__nms(total_boxes.copy(), 0.7, 'Union')
            ...
            total_boxes = self._MTCNN__rerec(total_boxes.copy())
            total_boxes[:, 0:4] = np.fix(total_boxes[:, 0:4]).astype(np.int32)
            status = StageStatus((self._MTCNN__pad(total_boxes.copy(), stage_status.width, stage_status.height)), width=(stage_status.width),
    ...
    def __stage2(self, img, total_boxes, stage_status: StageStatus):
        ...
        for k in range(0, num_boxes):
            ...
            if not (tmp.shape[0] > 0 and tmp.shape[1] > 0):
                if tmp.shape[0] == 0:
                    if tmp.shape[1] == 0:
        ...
        if total_boxes.shape[0] > 0:
            pick = self._MTCNN__nms(total_boxes, 0.7, 'Union')
            total_boxes = total_boxes[pick, :]
            total_boxes = self._MTCNN__bbreg(total_boxes.copy(), np.transpose(mv[:, pick]))
            total_boxes = self._MTCNN__rerec(total_boxes.copy())
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        points[0:5, :] = np.tile(w, (5, 1)) * points[0:5, :] + np.tile(total_boxes[:, 0], (5,
                                                                                           1)) - 1
        points[5:10, :] = np.tile(h, (5, 1)) * points[5:10, :] + np.tile(total_boxes[:, 1], (5,
                                                                                             1)) - 1
        if total_boxes.shape[0] > 0:
            total_boxes = self._MTCNN__bbreg(total_boxes.copy(), np.transpose(mv))
            pick = self._MTCNN__nms(total_boxes.copy(), 0.7, 'Min')
            total_boxes = total_boxes[pick, :]
            points = points[:, pick]
