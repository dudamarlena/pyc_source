...
# [GCC 8.4.0]
# Embedded file name: /Users/hugo/.virtualenvs/django-trello-webhooks/lib/python2.7/site-packages/trello_webhooks/admin.py
...
class CallbackEventAdmin(admin.ModelAdmin):
    list_display = ('timestamp', 'webhook_', 'event_type', 'has_template', 'member_name',
                    'board_', 'list_', 'card_')
    list_filter = ('timestamp', 'event_type')
    fields = ('timestamp', 'webhook', 'event_type', 'rendered', 'payload_')
    readonly_fields = ('timestamp', 'webhook', 'event_type', 'rendered', 'payload_')
    ...
    def payload_(self, instance):
        ...
        """
        pretty = json.dumps(instance.event_payload, sort_keys=True, indent=4, separators=(',',
                                                                                          ': '))
...
class CallbackEventInline(admin.StackedInline):
    model = CallbackEvent
    extra = 0
    fields = ('timestamp_', 'action_taken_by', 'event_type', 'rendered')
    readonly_fields = ('timestamp_', 'event_type', 'action_taken_by', 'board_name',
                       'list_name', 'card_name', 'rendered')
    ordering = ('-id', )
    ...
    def timestamp_(self, instance):
        url = reverse('admin:%s_%s_change' % (
         ...
         instance.id,))
        return format_html('<a href="{}">{}</a>', url, date_format(instance.timestamp, settings.DATETIME_FORMAT))
...
class WebhookAdmin(admin.ModelAdmin):
    list_display = ('auth_token_', 'description', 'trello_model_id', 'trello_id', 'is_active',
                    'created_at', 'last_updated_at')
    readonly_fields = ('trello_id', 'created_at', 'last_updated_at')
    form = WebhookForm
    ...
    def sync(self, request, queryset):
        ...
        logger.info('%s synced %i Webhooks from the admin site.', request.user, count)
