...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/h2o/config.py
...
class Config(ConfigParser):
    ...
    def getboolean(self, section, option, default=False):
        """
        transforms the result to an boolean.

        default may be either True/False or a string value
        """
    ...
    def getlist(self, section, option, default=[]):
        """
        trasform result p1,p2,...,pn into a list of strings
        """
        return [ x.strip() for x in self.get(section, option, (',').join(default)).split(',') ]
    ...
    def setDefaults(self):
        """
        subclasses can set here the defaults
        """
        pass
