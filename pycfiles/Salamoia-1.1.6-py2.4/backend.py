...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/nacl/backend.py
...
class NACLBackend(Backend):
    ...
    def scriptPath(self):
        cfg = Config()
        return cfg.get('general', 'scriptsPath', os.path.join(self.installationPath(), 'scripts'))
    ...
    def configFilenames(self):
        return [
         '/usr/share/salamoia/nacl/site.conf', '/etc/salamoia/nacl.conf', '~/.naclrc']
...
class BackendService(Service):
    ...
    def _accessCheck(self, object, action):
        ...
        authIds = [
         authToken.id] + authToken.principal.groups
        for i in authIds:
