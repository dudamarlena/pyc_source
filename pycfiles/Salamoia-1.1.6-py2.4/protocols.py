...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/h2o/protocols.py
...
def multiAdapt(objects, protocol, default=_marker, debug=False):
    ...
    """
    if debug:
        Ione.warning('multiAdapt debug: multi adapting', protocol, _multiAdapterRepository.get(protocol), _multiAdapterRepository)
    for (pr, adapter) in _multiAdapterRepository.get(protocol) or ():
        if debug:
            Ione.warning('multiAdapt debug: len(objects) (%s) == len(pr) (%s) -> %s', len(objects), len(pr), len(objects) == len(pr))
