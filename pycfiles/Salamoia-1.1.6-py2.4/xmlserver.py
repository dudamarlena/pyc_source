...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/h2o/xmlserver.py
...
import string, threading, sys, traceback
...
class Marshaller(xmlrpclib.Marshaller):
    ...
    def __dump(self, value, write):
        ...
        try:
            ...
            Ione.exception('adaptation error')
...
class Server(ThreadingMixIn, SimpleXMLRPCServer):
    ...
    def __init__(self, port, loglevel, control, bindAddress=None):
        """
        ...
        """
        if not bindAddress or bindAddress == '*':
            bindAddress = '0.0.0.0'
        SimpleXMLRPCServer.__init__(self, (bindAddress, port), requestHandler=SalamoiaXMLRPCRequestHandler, logRequests=loglevel)
    ...
    def _dispatch(self, method, params):
        ...
        try:
            ...
            if Config.defaultConfig().getboolean('debug', 'showXMLRPCException', 'yes'):
...
class SalamoiaXMLRPCRequestHandler(SimpleXMLRPCRequestHandler):
    ...
    def do_POST(self):
        ...
        if authType == 'Basic':
            ...
            principal = Principal(user, password, service)
        elif authType == 'Token':
            principal = Principal.fromToken(credentials, service)
        else:
            Ione.error('unknown authentication type', authType)
            return self.unauthenticated()
        if principal:
            principal.authInfo['authMethod'] = authType
        if principal and self.server.instance._authenticate(principal):
            service._authenticated(principal)
    ...
    def base_do_POST(self):
        try:
            ...
            gzipped = False
            if self.headers.get('Accept-Encoding') == 'deflate':
                gzipped = True
            if gzipped:
                response = self.zlibCompressBuf(response)
