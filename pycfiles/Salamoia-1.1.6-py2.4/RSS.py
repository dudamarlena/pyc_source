...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/utility/RSS.py
...
"""
import UserDict, sys, codecs, sha, types, signal, xml.sax as sax, xml.sax.saxutils as saxutils, cPickle as pickle, cStringIO as StringIO
__version__ = '0.45'
...
ns = _NamespaceMap()
rssNamespaces = [
 ns.rss09, ns.rss10, None]
rssSections = [
 (
  ns.rss10, 'channel'), (ns.rss10, 'image'), (ns.rss10, 'textarea')]
rssElements = [
 'rss', 'channel', 'image', 'textarea', 'item', 'items', 'title', 'link', 'description', 'url']
rdfResources = [
 (
  ns.rss10, 'image'), (ns.rss10, 'textarea'), (ns.admin, 'errorReportsTo'), (ns.admin, 'generatorAgent'), (ns.annotate, 'reference'), (ns.cp, 'server')]
...
class ChannelBase:
    ...
    def output(self, items):
        ...
        assert type(items) is types.ListType, 'items must be a list (%s)' % type(items)
        ...
        namespaces = {ns.rdf: 1}
        namespaces.update(dict(channelMD.keys() + imageMD.keys() + textinputMD.keys()))
        ...
        o.startElementNS((ns.rss10, 'channel'), None, {(ns.rdf, 'about'): channelMD[(ns.rss10, 'link')]})
        o.ignorableWhitespace('\n')
        for (name, data) in channelMD.items():
            if name in [(ns.rss10, 'items'), (ns.rss10, 'image'), (ns.rss10, 'textinput')]:
                continue
            o.ignorableWhitespace('  ')
            if name in rdfResources:
                ...
                if '<' in data:
                    o.startElementNS(name, None, {(ns.rdf, 'parseType'): 'Literal'})
                else:
        ...
        for id in items:
            o.ignorableWhitespace('   ')
            o.startElementNS((ns.rdf, 'li'), None, {(ns.rdf, 'resource'): self.getItem(id).get((ns.rss10, 'link'), _make_hash(self.getItem(id)))})
        ...
        if imageMD.has_key((ns.rss10, 'url')):
            o.startElementNS((ns.rss10, 'image'), None, {(ns.rdf, 'about'): imageMD[(ns.rss10, 'url')]})
            o.endElementNS((ns.rss10, 'image'), None)
            o.ignorableWhitespace('\n')
        if textinputMD.has_key((ns.rss10, 'link')):
            o.startElementNS((ns.rss10, 'textinput'), None, {(ns.rdf, 'about'): textinputMD[(ns.rss10, 'link')]})
        ...
        for id in items:
            ...
            o.startElementNS((ns.rss10, 'item'), None, {(ns.rdf, 'about'): item.get((ns.rss10, 'link'), _make_hash(item))})
            o.ignorableWhitespace('\n')
...
class TrackingChannel(ChannelBase, UserDict.UserDict):
    ...
    def addItem(self, item, index=0):
        if index == -1:
            index = len(self.data[(ns.rss10, 'items')])
        uri = item.get((ns.rss10, 'link'), _make_hash(item))
...
class RSSParser(sax.handler.ContentHandler):
    ...
    def __init__(self, channel, encoding='utf-8'):
        ...
        self._context = []
        self._tmp_item = {}
        self._tmp_md = {(ns.rss10, 'channel'): {}, (ns.rss10, 'image'): {}, (ns.rss10, 'textinput'): {}}
        self._tmp_buf = ''
        self.version = None
        return
    ...
    def startElementNS(self, name, qname, attrs):
        ...
        if name == (ns.rss10, 'item'):
            self._tmp_item = {}
            self._tmp_buf = ''
        elif len(self._context) > 1 and self._context[(-2)] == (ns.rss10, 'item') and name in rdfResources:
    ...
    def endElementNS(self, name, qname):
        if name[0] in rssNamespaces and name[1] in rssElements:
            ...
            name = (
             ns.rss091, name[1])
        if (
         ns.rss10, 'item') in self._context:
            if self._context[(-1)] == (ns.rss10, 'item'):
                self.channel.addItem(self._tmp_item, len(self.channel))
                self._tmp_item = {}
            elif self._context[(-2)] == (ns.rss10, 'item'):
