...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/h2o/generics.py
# Compiled at: 2007-12-02 16:26:58
from salamoia.h2o.functional import partial, rpartial
__all__ = ['Any', 'GenericFunction', 'Each', 'EachHaving']
...
class EachAttr(object):
    """
    EachAttr(list).attr is equivalent to [x.attr for x in list]
    ...
    def __getattr__(self, name):
        return map(rpartial(getattr, name), self._sequence)
...
class HavingAttr(object):
    ...
    It's useful mostly in combination with Each:

    >>> Each(HavingAttr([1, None, ' test']).strip).strip()
    ['test']

    To avoid duplicating the method name you can use the EachHaving sugar
    """
    __module__ = __name__
...
class EachHaving(HavingAttr):
    """
    Sytactical sugar for of Each(HavingAttr(list).method).method(..)

    >>> EachHaving([1, None, ' test']).strip()
    ['test']
    ...
    def __getattr__(self, name):
        return getattr(Each(super(EachHaving, self).__getattr__(name)), name)
