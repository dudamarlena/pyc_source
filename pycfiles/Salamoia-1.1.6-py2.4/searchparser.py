...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/h2o/searchparser.py
...
class SearchParser(salamoia.utility.tpg.Parser):
    ...
    START/e -> absolute/e | one/e | query/e;

    absolute/e -> '&' dn/b $e=BaseSpec(AnySpec(), 'base', b)$;

    one/e -> '\^' dn/b $e=BaseSpec(AnySpec(), 'one', b)$;

    query/e -> $e=AnySpec()$ (Expr/e|'\*')? $b=None$ $s=None$ (('->' | '@') (str/b | dn/b) )? (';' scope/s)? 
               $e=BaseSpec(e,s,b)$;
    ...
    Fact/s -> 'max' '\(' string/v '\)' $s=MaxSpec(v)$ |
              ...
              $k=None$ 'type' '=' string/v ( 'with' 'key' '=' string/k)?  $s=TypeSpec(v, k)$ |
              'type' '!=' string/v $s=NotSpec([TypeSpec(v)]) $ |
