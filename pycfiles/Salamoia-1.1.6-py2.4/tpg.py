...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/utility/tpg.py
...
import parser, re, sre_parse, sys
try:
    enumerate
except NameError:
...
class LexerOptions:
    ...
    def word_bounded(self, expr):
        r""" add word boundaries (\b) to expr if it looks like an identifier
        """
...
class NamedGroupLexer(LexerOptions):
    """ NamedGroupLexer(word_bounded, compile_options)
...
class Lexer(NamedGroupLexer):
    """ Lexer(word_bounded, compile_options)
...
class CacheNamedGroupLexer(NamedGroupLexer):
    """ CacheNamedGroupLexer(word_bounded, compile_options)
...
class CacheLexer(Lexer):
    """ CacheLexer(word_bounded, compile_options)
...
class ContextSensitiveLexer(LexerOptions):
    """ ContextSensitiveLexer(word_bounded, compile_options)
    ...
    def def_token(self, name, expr, value=_id):
        ...
        if name not in self.tokens and name not in self.separators:
            self.tokens[name] = (
             self.re_compile(self.word_bounded(expr)), value)
        else:
            raise SemanticError('Duplicate token definition (%s)' % name)
...
class TPGParser(tpg.Parser):
    ...
    def init_lexer(self):
        ...
        lexer.def_token('_tok_1', 'set')
        lexer.def_token('_tok_2', '=')
        lexer.def_token('_tok_3', 'separator')
        lexer.def_token('_tok_4', 'token')
        lexer.def_token('_tok_5', ':')
        lexer.def_token('_tok_6', ';')
        lexer.def_token('_tok_7', '->')
        lexer.def_token('_tok_8', '\\|')
        lexer.def_token('_tok_9', '\\(')
        lexer.def_token('_tok_10', '\\)')
        lexer.def_token('_tok_11', 'check')
        lexer.def_token('_tok_12', 'error')
        lexer.def_token('_tok_13', '@')
        lexer.def_token('_tok_14', '\\+')
        lexer.def_token('_tok_15', '\\?')
        lexer.def_token('_tok_16', ',')
        lexer.def_token('_tok_17', '<')
        lexer.def_token('_tok_18', '>')
        lexer.def_token('_tok_19', '/')
        lexer.def_separator('spaces', '\\s+')
        lexer.def_separator('comment', '#.*')
        lexer.def_token('string', '\\"(\\\\.|[^\\"\\\\]+)*\\"|\'(\\\\.|[^\'\\\\]+)*\'')
        lexer.def_token('code', '\\{\\{(\\}?[^\\}]+)*\\}\\}|\\$[^\\$\\n]*\\$|\\$.*\\n([ \\t]*\\$.*\\n)*', self.Code)
        lexer.def_token('ident', '\\w+')
        lexer.def_token('lcbra', '\\{')
        lexer.def_token('rcbra', '\\}')
        lexer.def_token('star2', '\\*\\*')
        lexer.def_token('star', '\\*')
        return lexer

    def START(self):
        """ START -> OPTIONS TOKENS RULES """
    ...
    def TOKEN(self):
        """ TOKEN -> ('separator' | 'token') ident ':'? string (PY_EXPR ';'? | ';') """
        _p1 = self.lexer.token()
        try:
            self.eat('_tok_3')
    ...
    def PY_EXPR(self):
        """ PY_EXPR -> ident | string | code | ARGS """
        _p1 = self.lexer.token()
        try:
            try:
                name = self.eat('ident')
    ...
    class Options:
        __module__ = __name__
        option_dict = {'lexer': ({'NamedGroupLexer': NamedGroupLexer, 'Lexer': Lexer, 'CacheNamedGroupLexer': CacheNamedGroupLexer, 'CacheLexer': CacheLexer, 'ContextSensitiveLexer': ContextSensitiveLexer}, 'NamedGroupLexer'), 'word_boundary': ({'True': True, 'False': False}, 'True'), 'lexer_ignorecase': ({'True': 'IGNORECASE', 'False': False}, 'False'), 'lexer_locale': ({'True': 'LOCALE', 'False': False}, 'False'), 'lexer_multiline': ({'True': 'MULTILINE', 'False': False}, 'False'), 'lexer_dotall': ({'True': 'DOTALL', 'False': False}, 'False'), 'lexer_verbose': ({'True': 'VERBOSE', 'False': False}, 'False'), 'lexer_unicode': ({'True': 'UNICODE', 'False': False}, 'False')}
        ...
        def lexer_compile_options(self):
            options = [self.lexer_ignorecase, self.lexer_locale, self.lexer_multiline, self.lexer_dotall, self.lexer_verbose, self.lexer_unicode]
            return ('+').join([ 'salamoia.utility.tpg.re.%s' % opt for opt in options if opt ]) or 0
    ...
    class Rule:
        ...
        def gen_code(self):
            ...
            return (self.head.name, [self.head.gen_def(), tab + 'r""" %s -> %s """' % (self.head.gen_doc(self), self.body.gen_doc(self)), self.body.gen_code(tab, counters, None), self.head.gen_ret(tab)])
    ...
    class And(list):
        ...
        def gen_code(self, indent, counters, pos):
            return self and [self[0].gen_code(indent, counters, pos), [ a.gen_code(indent, counters, None) for a in self[1:] ]]
    ...
    class Or(NotEmpty):
        ...
        def gen_code(self, indent, counters, pos):
            p = pos or counters('p')
            return [pos is None and indent + '%s = self.lexer.token()' % p or (), indent + 'try:', self.a.gen_code(indent + tab, counters, p), indent + 'except salamoia.utility.tpg.WrongToken:', indent + tab + 'self.lexer.back(%s)' % p, self.b.gen_code(indent + tab, counters, p)]
        ...
        def gen_code(self, indent, counters, pos):
            if (
             self.min, self.max) == (0, 1):
                p = pos or counters('p')
                return [pos is None and indent + '%s = self.lexer.token()' % p or (), indent + 'try:', self.a.gen_code(indent + tab, counters, p), indent + 'except salamoia.utility.tpg.WrongToken:', indent + tab + 'self.lexer.back(%s)' % p]
            elif (
             self.min, self.max) == (0, None):
                p = pos or counters('p')
                return [indent + 'while True:', indent + tab + '%s = self.lexer.token()' % p, indent + tab + 'try:', self.a.gen_code(indent + tab + tab, counters, p), indent + tab + 'except salamoia.utility.tpg.WrongToken:', indent + tab + tab + 'self.lexer.back(%s)' % p, indent + tab + tab + 'break']
            elif (
             self.min, self.max) == (1, None):
                p = pos or counters('p')
                n = counters('n')
                return [indent + '%s = 0' % n, indent + 'while True:', indent + tab + '%s = self.lexer.token()' % p, indent + tab + 'try:', self.a.gen_code(indent + tab + tab, counters, p), indent + tab + tab + '%s += 1' % n, indent + tab + 'except salamoia.utility.tpg.WrongToken:', indent + tab + tab + 'if %s < 1: raise' % n, indent + tab + tab + 'self.lexer.back(%s)' % p, indent + tab + tab + 'break']
            else:
                p = pos or counters('p')
                n = counters('n')
                min = self.min.gen_code()
                max = self.max.gen_code()
                return [indent + '%s = 0' % n, indent + 'while %s:' % (max == 'None' and 'True' or '%s < %s' % (n, max)), indent + tab + '%s = self.lexer.token()' % p, indent + tab + 'try:', self.a.gen_code(indent + tab + tab, counters, p), indent + tab + tab + '%s += 1' % n, indent + tab + 'except salamoia.utility.tpg.WrongToken:', indent + tab + tab + 'if %s < %s: raise' % (n, min), indent + tab + tab + 'self.lexer.back(%s)' % p, indent + tab + tab + 'break']
            return
    ...
    def gen(self, options, tokens, rules):
        ...
        yield self.make_code('init_lexer', 'def init_lexer(self):', lexer is ContextSensitiveLexer and [tab + 'self.eat = self.eatCSL'] or (), tab + 'lexer = salamoia.utility.tpg.%s(%s, %s)' % (lexer.__name__, word_bounded, lexer_options), [ tab + tok.gen_def() for tok in inline_tokens ], [ tab + tok.gen_def() for tok in tokens ], tab + 'return lexer')
        tokens_from_name = {}
