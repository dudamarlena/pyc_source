...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/tales/tales.py
# Compiled at: 2007-12-02 16:26:58
"""TALES

An implementation of a TAL expression engine

$Id: tales.py 39752 2005-10-30 20:16:09Z srichter $
"""
__docformat__ = 'reStructuredText'
import re
try:
    from salamoia import tal
except ImportError:
    tal = None

if tal:
    from salamoia.tal.interfaces import ITALExpressionEngine
    from salamoia.tal.interfaces import ITALExpressionCompiler
    from salamoia.tal.interfaces import ITALExpressionErrorInfo
    from salamoia.tales.interfaces import ITALESIterator
NAME_RE = '[a-zA-Z][a-zA-Z0-9_]*'
_parse_expr = re.compile('(%s):' % NAME_RE).match
...
class ExpressionEngine(object):
    ...
    def registerType(self, name, handler):
        ...
        types = self.types
        if name in types:
            raise RegistrationError('Multiple registrations for Expression type "%s".' % name)
        types[name] = handler
    ...
    def registerBaseName(self, name, object):
        ...
        if name in base_names:
            raise RegistrationError('Multiple registrations for base name "%s".' % name)
...
class TALESTracebackSupplement(object):
    """Implementation of salamoia.exceptions.ITracebackSupplement"""
    __module__ = __name__
