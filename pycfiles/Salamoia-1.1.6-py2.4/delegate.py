...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/salamoia/h2o/delegate.py
...
def Delegate(delegateName):
    ...
    class DelegateBase(DelegateHacks):
        __module__ = __name__

        def __repr__(self):
            subject = getattr(self, delegateName)
            return '<Delegate on %s: %r>' % (delegateName, subject)
    ...
    def __getattr__(self, name):
        ...
        if hasattr(type(subject), name):
            ...
            if hasattr(attr, '__get__'):
                func = attr.__get__(subject, type(subject))
...
class DelegateHacks(object):
    """
    This class is the base class for all base delegates. It implements wrappers
    for basic python special methods that are not automatically looked up with __getattr__.

    The wrappers are implemented with the @lazy decorator so that the attribute lookup is done only
    once and then cached in the instance dictionary.

    When the attribute is not found it tries to mimic the same exceptions raised 
    """
    __module__ = __name__
    ...
    def __len__(self):
        try:
            ...
            raise ValueError, 'len() of unsized object'
    ...
    def __getitem__(self):
        try:
            ...
            raise ValueError, 'unsubscriptable object'
    ...
    def __delitem__(self):
        try:
            ...
            raise ValueError, 'object does not support item deletion'
