...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_rnaseq/tools/preprocessing.py
...
def download_and_process_tar(job, config):
    ...
    process = job.wrapJobFn(process_sample, config, input_tar=download.rv(), disk=PromisedRequirement(lambda x: x.size * 10, download.rv()))
...
def download_and_process_fastqs(job, config):
    ...
    process = job.wrapJobFn(process_sample, config, fastq_ids=download.rv(), disk=PromisedRequirement(lambda xs: sum(x.size for x in xs) * 5, download.rv()))
    job.addChild(download)
    download.addChild(process)
    return process.rv()
...
def download_and_process_bam(job, config):
    ...
    if config.cutadapt:
        ...
        return job.addChildJobFn(run_cutadapt, r1, r2, config.fwd_3pr_adapter, config.rev_3pr_adapter, disk=disk).rv()
    return (
     r1, r2)
