...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_rnaseq/input_generation.py
...
def star_index(job, args):
    ...
    dockerCall(job, tool='quay.io/ucsc_cgl/star:2.4.2a--bcbd5122b69ff6ac4ef61958e47bde94001cfe80', workDir=job.tempDir, parameters=parameters)
...
def rsem_index(job, args):
    ...
    dockerCall(job, tool='quay.io/ucsc_cgl/rsem:1.2.25--d4275175cc8df36967db460b06337a14f40d2f21', parameters=parameters, dockerParameters=docker_parameters)
...
def kallisto_index(job, args):
    ...
    parameters = [
     'index', 'transcriptome.fa', '-i', ('/data/{}.index').format(args.kallisto_name)]
    dockerCall(job, tool='quay.io/ucsc_cgl/kallisto:0.43.1--355c19b1fb6fbb85f7f8293e95fb8a1e9d0da163', workDir=job.tempDir, parameters=parameters)
...
def _create_transcriptome(job, args, work_dir):
    ...
    parameters = [
     'gtf_to_fasta', '/data/annotation.gtf', '/data/ref.fa', '/data/transcriptome.fa']
...
def hera_index(job, args):
    ...
    dockerCall(job, tool='jvivian/hera', workDir=job.tempDir, parameters=parameters, dockerParameters=docker_parameters)
...
def main():
    ...
    parser.add_argument('--ref', type=str, default=None, help='Path to reference fasta needed to generate RSEM and STAR input')
    parser.add_argument('--gtf', type=str, default=None, help='Path to GTF annotation file needed to generate RSEM and STAR input')
    parser.add_argument('--transcriptome', type=str, default=None, help='Path to transcriptome needed to generate Kallisto input. If this file is not provided,\n the reference and GTF will be used to create a transcriptome. \nIf only this file is provided, just Kallisto will be run.')
