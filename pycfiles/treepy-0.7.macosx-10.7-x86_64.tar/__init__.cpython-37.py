...
# [GCC 8.4.0]
# Embedded file name: /Users/kourtneykroll/anaconda3/lib/python3.7/site-packages/treepy/__init__.py
...
def tabulate(*args, **kwargs):
    ...
    def _align_column(strings, alignment, minwidth=0, has_invisible=True, enable_widechars=False, is_multiline=False):
        ...
        if is_multiline:
            ...
            lines_pad = [[(s, maxwidth + len(s) - width_fn(s)) for s in group] for group in lines]
            padded_strings = ['\n'.join([padfn(w, s) for s, w in group]) for group in lines_pad]
        else:
            if not enable_widechars:
                padded_strings = has_invisible or [padfn(maxwidth, s) for s in strings]
            else:
