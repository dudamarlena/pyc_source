...
# [GCC 8.4.0]
# Embedded file name: /Users/kourtneykroll/anaconda3/lib/python3.7/site-packages/treepy/mls_generation.py
# Compiled at: 2019-08-10 02:25:16
# Size of source mod 2**32: 4552 bytes
import treepy, os, math, itertools
...
class MultiLS(object):
    ...
    def process(self):
        ...
        for depth, parent in enumerate(self.get_parents(self.root)):
            children = sorted((child for child in parent.iterdir() if self.criteria(child)),
              key=_getatime, reverse=True)
    ...
    def display_children(self, children, depth):
        ...
        if self.args['r']:
            ...
            dotdot_prefix = ''
        max_child_len = min([truncate_len,
         max([len(child.name) + (1 if child.is_dir() else 0) + len(dotdot_prefix) for child in children])])
