...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/vbp/__init__.py
# Compiled at: 2019-04-28 23:48:42
# Size of source mod 2**32: 41093 bytes
"""
...
python -m vbp.run
"""
import os, abc, sys, enum, math, numpy, pandas, random, shutil, logging, pathlib, argparse, datetime, warnings, fbprophet, traceback, matplotlib, collections, matplotlib.offsetbox, statsmodels.tools, statsmodels.formula.api
VERSION = '0.3.6'
numpy.seterr('raise')
...
class DataSource(abc.ABC):
    __doc__ = 'The DataSource class is the base abstract class for VBP.'
    ...
    def ensure_options(self, args):
        parser = hasattr(self, 'options') or create_parser()
        ...
        parser.set_defaults(show_graphs=False,
        ...
        if self.options.output_dir == self.default_output_dir:
            clean = True
        if not self.options.do_not_clean:
            if clean:
    ...
    def run_modeled_value_based_prioritization(self):
        ...
        if self.options.manual_scales is not None:
            ...
            elif manual_scales_file.endswith('xlsx'):
                manual_scales = pandas.read_excel(manual_scales_file)
            else:
                if manual_scales_file.endswith('csv'):
        ...
        if calculated_scales is not None:
            if calculated_scales.empty is False:
    ...
    def create_output_name(self, name):
        name = name.replace(' ', '_').replace('(', '').replace(')', '').replace('[', '').replace(']', '').replace('{', '').replace('}', '').replace('/', '_').replace(':', '_').replace('\\', '_').replace('<', '_').replace('>', '_').replace('!', '_').replace('"', '_').replace("'", '_').replace('|', '_').replace('?', '_').replace(',', '').replace('*', '_')
        if len(name) > 4:
            if name[(len(name) - 4)] == '.':
                if name.find('.') < len(name) - 4:
                    name = name[:name.rfind('.')].replace('.', '_') + name[name.rfind('.'):]
        outputdir = self.options.output_dir
        if self.options.data_type is not None:
            if self.options.data_type_subdir:
    ...
    def write_spreadsheet(self, df, name, use_digits_grouping_number_format=False):
        if self.options.write_spreadsheets:
            ...
            for idx, col in enumerate(df2):
                ...
                worksheet.set_column(idx, idx, max_len, digits_grouping_format)
    ...
    def find_best_fitting_models(self, df):
        ...
        result[self.predict_column_name] = result[self.predict_column_name].apply(lambda x:         if x < 0:
0 # Avoid dead code: x)
        return result
    ...
    def create_model_prophet(self, action, i, count):
        ...
        prophet.fit(df, algorithm='Newton')
        ...
        index = (
         self.get_obfuscated_name(action), 'Prophet')
...
class TimeSeriesDataSource(DataSource):
    ...
    def run_predict(self):
        ...
        if model_results is not None:
            if len(model_results) > 0:
                ...
                extra_columns = [
                 self.options.base_column, self.options.derivative_column]
    ...
    def create_model_ets(self, action, predict, i, count):
        ...
        for title, result in results.items():
            index = (
             self.get_obfuscated_name(action), title)
    ...
    def create_model_ols(self, action, degree, predict, i, count):
        ...
        df = df.reset_index()
        model_map = self.get_model_map_base()
        model_map.update({'AdjustedR2':{},  'ProbFStatistic':{},  'LogLikelihood':{},  'BreuschPagan':{},  'Formula':{}})
        ...
        index = (
         self.get_obfuscated_name(action), degree)
        ...
        matplotlib.pyplot.plot([end], [predicted], 'cD')
    ...
    def recursive_list_process(self, node, leaves_only, accumulator):
        ...
        if self == node:
            if self.value is not None:
                if leaves_only:
                    if not leaves_only or len(node) == 0:
