...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/vbp/ucod/world.py
# Compiled at: 2019-03-31 18:13:54
# Size of source mod 2**32: 12442 bytes
import os, vbp, sys, enum, numpy, pandas, zipfile, datetime, matplotlib, vbp.ucod.icd, urllib.request
...
class UCODWorld(vbp.ucod.icd.ICDDataSource):
    ...
    def load_who_populations(self):
        populations = pandas.read_csv((self.options.data_world_who_population),
          usecols=[
         'Country', 'Year', 'Pop1'])
    ...
    def load_country_codes(self):
        country_codes = pandas.read_csv((self.options.data_world_who_country_codes),
          index_col=0,
          squeeze=True)
    ...
    def run_load(self):
        ...
        if self.options.data_type == DataType.WORLD_ICD10_MINIMALLY_GROUPED:
            ...
            if self.options.data_type == DataType.WORLD_ICD10_CHAPTERS_ALL:
                deaths = self.process_grouping(deaths, deaths_per_year, False, False)
            else:
                if self.options.data_type == DataType.WORLD_ICD10_SUB_CHAPTERS:
                    deaths = self.process_grouping(deaths, deaths_per_year, False, True)
                else:
                    if self.options.data_type == DataType.WORLD_ICD10_CHAPTER_ROOTS:
    ...
    def process_grouping(self, deaths, deaths_per_year, roots_only, leaves_only):
        ...
        deaths = df[(['Date'] + [col for col in df if col != 'Date' if col != 'Query'])]
        return deaths
    ...
    def read_unpopdata(self, sheet_name):
        unpopdata = pandas.read_excel((self.options.data_world_un_population),
          skiprows=(range(0, 16)),
          index_col=2,
    ...
    def map_un_country_to_who_country(self, c):
        return c.strip().replace('Réunion', 'Reunion').replace('Bolivia (Plurinational State of)', 'Bolivia').replace('Caribbean Netherlands', 'Netherlands Antilles').replace('Saint Vincent and the Grenadines', 'Saint Vincent and Grenadines').replace('United States Virgin Islands', 'Virgin Islands (USA)').replace('Venezuela (Bolivarian Republic of)', 'Venezuela').replace('Cabo Verde', 'Cape Verde').replace('State of Palestine', 'Occupied Palestinian Territory')

    def find_population(self, country_code, year, populations, unpopdata, country_codes):
        if (
         country_code, year) in populations.index:
    ...
    def read_icd10(self, file):
        df = pandas.read_csv(file,
          usecols=[
         'Country', 'Year', 'List', 'Cause', 'Deaths1'],
          dtype={'List':str, 
         'Cause':str, 
