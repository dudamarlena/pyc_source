...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/vbp/run.py
# Compiled at: 2019-03-31 19:49:22
# Size of source mod 2**32: 11787 bytes
import io, os, vbp, sys, numpy, pandas, inspect, argparse, traceback, matplotlib, matplotlib.pyplot, vbp.example, vbp.ucod.united_states, vbp.ucod.world
            ...
            if options.command_name == 'predict':
                ...
                if options.command_name == 'list':
                    ...
                    if options.command_name == 'count':
                        ...
                        if options.command_name == 'manual_scale_functions':
                            ...
                            for i, data_type in enumerate(data_types):
                                ...
                                if not options.prefix is not None:
                                    if options.suffix is not None:
                                        df[ds.pretty_action_column_name] = df[ds.obfuscated_column_name]
                                        df = df.reindex(columns=([ds.pretty_action_column_name] + list([x for x in df.columns if x != ds.pretty_action_column_name if x != ds.obfuscated_column_name] + [ds.obfuscated_column_name])))
                            ...
                            if options.command_name == 'action_data':
                                ds = create_data_source(data_source_classes, options)
                                ds.load(options.args)
                                print(ds.get_action_data(options.action))
                                print_finished_ds(ds.options)
                            else:
                                if options.command_name == 'prepare_data':
                                    ds = create_data_source(data_source_classes, options)
                                    ds.ensure_options(options.args)
                                    ds.prepare_data()
                                    print_finished_ds(ds.options)
                                else:
                                    if options.command_name == 'test':
                                        ds = create_data_source(data_source_classes, options)
                                        ds.ensure_options(options.args)
                                        ds.test()
                                        print_finished_ds(ds.options)
