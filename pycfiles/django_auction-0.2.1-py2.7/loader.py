...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/auction/utils/loader.py
# Compiled at: 2013-04-22 04:27:13
"""
These utils are ripped directly from Django-Shop.
"""
from django.conf import settings
from django.core import exceptions
from django.utils.importlib import import_module
CLASS_PATH_ERROR = "django-auction is unable to interpret settings value for %s. %s should be in the form of a tupple: ('path.to.models.Class', 'app_label')."
...
def load_class(class_path, setting_name=None):
    ...
    if not isinstance(class_path, basestring):
        try:
            ...
            if setting_name:
                ...
                raise exceptions.ImproperlyConfigured(CLASS_PATH_ERROR % ('this setting',
                                                                          'It'))
    ...
    try:
        ...
        if setting_name:
            txt = 'Backend module "%s" does not define a "%s" class. Check your %s setting' % (
...
def get_model_string(model_name):
    ...
    """
    setting_name = 'AUCTION_%s_MODEL' % model_name.upper().replace('_', '')
