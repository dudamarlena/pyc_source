...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/auction/tests/test_models.py
# Compiled at: 2013-05-03 04:41:15
import auction.models.bases
from django.test import TestCase
from django.template.defaultfilters import slugify
import datetime, auction.utils.generic
...
class BidBasketModelTests(TestCase, TestBaseClassMixin):
    ...
    def setUp(self):
        ...
        self.auction, created = auction.models.Auction.objects.get_or_create(name=auction_name, slug=slugify(auction_name), start_date=now, end_date=now + datetime.timedelta(365), active=True, total_bids=0)
        self.auction2, created = auction.models.Auction.objects.get_or_create(name=auction_name2, slug=slugify(auction_name2), start_date=now, end_date=now + datetime.timedelta(365), active=True, total_bids=0)
        lot_name = 'Foo bar'
        lot_name2 = 'some item'
        lot_name3 = 'cool item'
        ct = ContentType.objects.get_for_model(self.auction)
        self.lot, created = auction.models.Lot.objects.get_or_create(name=lot_name, slug=slugify(lot_name), active=True, content_type=ct, object_id=self.auction.pk)
        self.lot2, created = auction.models.Lot.objects.get_or_create(name=lot_name2, slug=slugify(lot_name2), active=True, content_type=ct, object_id=self.auction.pk)
        self.lot3, created = auction.models.Lot.objects.get_or_create(name=lot_name3, slug=slugify(lot_name3), active=True, content_type=ct, object_id=self.auction2.pk)
        self.request = Mock()
    ...
    def test_update_bid_with_multiple_bid_items(self):
        ...
        """
        self.lot.is_biddable = True
        self.lot2.is_biddable = True
        self.lot3.is_biddable = True
        self.lot.save()
        self.lot2.save()
        self.lot3.save()
        bidbasket = auction.utils.generic.get_or_create_bidbasket(self.request)
        ...
        biditem_1 = auction.models.BidItem.objects.get(bid_basket=bidbasket, lot_id=self.lot.pk)
        biditem_2 = auction.models.BidItem.objects.get(bid_basket=bidbasket, lot_id=self.lot2.pk)
        biditem_3 = auction.models.BidItem.objects.get(bid_basket=bidbasket, lot_id=self.lot3.pk)
    ...
    def test_delete_bid_with_multiple_bid_items(self):
        ...
        biditem = auction.models.BidItem.objects.get(bid_basket=bidbasket, lot_id=self.lot2.pk)
        bidbasket.delete_bid(biditem.pk)
        biditems = auction.models.BidItem.objects.filter(bid_basket=bidbasket)
    ...
    def test_add_bid_to_inactive_lot(self):
        ...
        a, created = auction.models.Auction.objects.get_or_create(name=auction_name, slug=slugify(auction_name), start_date=now, end_date=now + datetime.timedelta(365), active=True, total_bids=0)
        lot_name = 'whatever'
        ct = ContentType.objects.get_for_model(a)
        lot, created = auction.models.Lot.objects.get_or_create(name=lot_name, slug=slugify(lot_name), active=False, content_type=ct, object_id=a.pk)
        bidbasket = auction.utils.generic.get_or_create_bidbasket(self.request)
    ...
    def test_update_bid_locked_biditem(self):
        ...
        lot_name = 'whatever'
        ct = ContentType.objects.get_for_model(a)
        lot, created = auction.models.Lot.objects.get_or_create(name=lot_name, slug=slugify(lot_name), active=True, is_biddable=True, content_type=ct, object_id=a.pk)
        bidbasket = auction.utils.generic.get_or_create_bidbasket(self.request)
