...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/auction/models/bases.py
# Compiled at: 2013-05-07 22:17:46
...
class BaseBidBasket(models.Model):
    ...
    def add_bid(self, lot, amount):
        ...
        item, created = BidItem.objects.get_or_create(bid_basket=self, content_type=ContentType.objects.get_for_model(Lot), lot_id=lot.pk)
        if item:
            item.amount = amount
            item.save()
        return item
...
class BaseAuctionLot(PolymorphicModel):
    ...
    content_type = models.ForeignKey(ContentType, related_name='%(app_label)s_%(class)s_lots', verbose_name=_('Content type'))
