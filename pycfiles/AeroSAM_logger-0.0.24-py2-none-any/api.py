...
# Embedded file name: D:\Lukas\Nextcloud\university\AE All\aeropy\aeropy\airfoils\executables\api.py
# Compiled at: 2017-02-21 17:40:54
...
import subprocess as subp, numpy as np, tempfile, os, re, shutil
...
class Executable:
    ...
    def __init__(self, executable, cwd=None):
        self.inst = subp.Popen(os.path.join(os.path.dirname(os.path.realpath(__file__)), executable), stdin=subp.PIPE, stdout=subp.PIPE, stderr=subp.PIPE, cwd=cwd)
...
class XFOIL(Executable):
    ...
    def execute(self, alphas):
        ...
        for i, alpha in enumerate(alphas):
            self.config.append('ALFA %.4f' % (alpha * 180.0 / np.pi))
    ...
    def output_polar(self):
        ...
        data_array = np.array([ clean_split(dataline) for dataline in datalines ], dtype='float')
        return (
...
class SU2EDU(Executable):
    ...
    def output_surface(self):
        self.cmdlogcheck()
        data_header = np.char.strip(np.genfromtxt(os.path.join(self.cwd, 'surface_flow.csv'), dtype='str', max_rows=1, delimiter=',', autostrip=True), '"')
        data_array = np.loadtxt(os.path.join(self.cwd, 'surface_flow.csv'), skiprows=1, delimiter=',')
        return (
         data_array, data_header)

    def output_history(self):
        self.cmdlogcheck()
        data_header = np.char.strip(np.genfromtxt(os.path.join(self.cwd, 'history.csv'), dtype='str', max_rows=1, delimiter=',', autostrip=True), '"')
