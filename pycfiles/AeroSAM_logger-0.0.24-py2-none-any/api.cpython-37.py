...
# Compiled at: 2019-02-12 14:23:10
...
class Executable:
    ...
    def __init__(self, executable, cwd=None):
        self.inst = subp.Popen((os.path.join(os.path.dirname(os.path.realpath(__file__)), executable)), stdin=(subp.PIPE), stdout=(subp.PIPE),
          stderr=(subp.PIPE),
          cwd=cwd)
...
class NonBlockingStreamReader:
    ...
    def readline(self, timeout=None):
        try:
            return self._q.get(block=(timeout is not None), timeout=timeout).decode()
        except Empty:
            return
