...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/flint/src/flinter/struct_analysis.py
# Compiled at: 2020-05-13 15:51:37
# Size of source mod 2**32: 6447 bytes
""" main source for Flint "Because our fortran code stinks"""
import sys
__all__ = [
 'struct_analysis', 'print_errors', 'print_score']

class LangFtn:
    __doc__ = ' class with all info and function specific to a fortran statement\n    '
...
def get_variables(statement_lst):
    ...
    for statement in statement_lst:
        for typename in types_lst:
            if statement.startswith(typename) and '::' in statement:
...
def test_blockst(statement_lst):
    ...
    out = {'statements':clean_statement_lst, 
...
def ifdoerrors(maxdepth):
    out = list()
    if maxdepth > 5:
        out.append('too-many-levels : ' + str(maxdepth) + '/5 nested IF and DO blocks')
    return out
