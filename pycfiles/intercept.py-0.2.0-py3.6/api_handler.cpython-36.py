...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/intercept/api_handler.py
# Compiled at: 2018-12-27 16:24:21
# Size of source mod 2**32: 4953 bytes
import inspect
from typing import T, Any, Dict, List, Type
import anyio
from intercept.data_format import DataFormat
from intercept.events import Event, AuthEvent, ChatEvent, InfoEvent, CommandEvent, ConnectEvent, BroadcastEvent, ConnectedEvent, TraceStartEvent, TraceCompleteEvent, ConfigEvent, ErrorEvent
...
class APIHandler:
    ...
    def __init__(self, client, fmt: DataFormat=DataFormat.CLEAN, bot=True, bufsize: int=16777216):
        self.client = client
        self._bot = bot
    ...
    async def wait_for(self, event=None, command=None, type_: T=Event) -> T:
        if event is not None:
            f = lambda x: x['event'] == event or event == 'command' and 'error' in x and x['error'] == 'Invalid command'
        else:
