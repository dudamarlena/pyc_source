...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\empywiz\expandtemplate.py
...
__license__ = 'BSD'
import re, sys, os, operator, shutil, tempfile, time, em, wizdefaults, wizprompters, empywizcfg as conf
...
def setClipText(aString):
    import win32clipboard as w, win32con
    w.OpenClipboard()
    w.EmptyClipboard()
...
def file_to_clipboard(template):
    ...
    while 1:
        ...
        questions = 0
        normalvars = dict([ (k, vars[k]) for k in vars.keys() if 'once' not in vars[k]
                          ])
...
def expand_tree(path):
    ...
    for (dpath, dnames, fnames) in os.walk(path):
        templates += [ os.path.join(dpath, f) for f in fnames if f.lower().endswith('.em')
                     ]
    ...
    if os.path.isfile(renamefile):
        renames = [ l.strip().split(',') for l in open(renamefile) if l.find(',') != -1 ]
...
def discover_template(template):
    if not os.path.isdir(template):
        projects = [ d for d in os.listdir(conf.project_templates_dir) if os.path.isdir(os.path.join(conf.project_templates_dir, d)) ]
        print projects
...
def process_existing_files(roots, templates):
    ...
    def expanded_lines(lines):
        for l in lines:
            tp = re.search('templatepoint:\\s*(\\w+)', l)
            if not tp:
                yield l
                continue
            expname = tp.group(1)
...
def main():
    global delete_emfiles
    global force_overwrite
    global options
    global preserve_emfiles
    global tempdir
    ...
    parser = OptionParser()
    parser.add_option('-p', '--preserve-templates', action='store_true', help='Preserve source .em files after directory expansion', dest='preserve_emfiles', default=False)
    parser.add_option('-d', '--delete-templates', action='store_true', help='Delete source .em files after directory expansion', dest='delete_emfiles', default=False)
    parser.add_option('-f', '--force', action='store_true', help='Force overwrite of target files if any exist', dest='force_overwrite', default=False)
    parser.add_option('-e', '--expandhere', action='store', help='Expand a template here', dest='expandhere', default=None)
    parser.add_option('-m', '--manipulate', action='append', help='Manipulate files', dest='manipulationguide', default=None)
