...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pylshvec/lshvec.py
# Compiled at: 2019-10-31 12:30:10
# Size of source mod 2**32: 7821 bytes
"""
...
import jnius_config, re
logger = utils.get_logger('pylshvec')
sub_re = re.compile('[^ACGT]')
...
class LSHVec(object):
    ...
    def __init__(self, model_file, hash_file, threshold=0.005, max_results=500, batch_size=1024, num_thread=1, only_show_main_tax=False, without_uncult=True):
        """
        ...
        jlshvec = PyLSHVec(self._LSHVec__to_java_stirng(model_file), self._LSHVec__to_java_stirng(hash_file))
    ...
    def embedding_single_thread(self, seq_or_seqs):
        ...
        if is_str:
            seq_or_seqs = [
             seq_or_seqs]
        seq_or_seqs = self._LSHVec__sub_seqs(seq_or_seqs)
        ret = [self.model.getEmbedding(self._LSHVec__to_java_stirng(u)) for u in seq_or_seqs]
        if is_str:
            return ret[0]
        else:
            return ret
    ...
    def predict_single_thread(self, seq_or_seqs):
        ...
        if is_str:
            seq_or_seqs = [
             seq_or_seqs]
        seq_or_seqs = self._LSHVec__sub_seqs(seq_or_seqs)
        ret = [self.model.predict(self._LSHVec__to_java_stirng(u)) for u in seq_or_seqs]
    ...
    def embedding(self, seq_or_seqs):
        ...
        if is_str:
            seq_or_seqs = [
             seq_or_seqs]
        seq_or_seqs = self._LSHVec__sub_seqs(seq_or_seqs)
        seq_or_seqs = [self._LSHVec__to_java_stirng(u) for u in seq_or_seqs]
