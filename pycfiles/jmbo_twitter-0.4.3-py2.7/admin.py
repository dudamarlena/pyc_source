...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jmbo_twitter/admin.py
...
class FeedAdmin(ModelBaseAdmin):
    inlines = []
    list_display = ('title', '_image', 'subtitle', 'publish_on', 'retract_on', '_get_absolute_url',
    ...
    def _actions(self, obj):
        ...
        try:
            url = (
             reverse('feed-fetch-force', args=[obj.id]),)
        ...
        try:
            url = (
             reverse('feed-tweets', args=[obj.id]),)
...
class SearchAdmin(ModelBaseAdmin):
    inlines = []
    list_display = ('title', 'subtitle', 'criteria', 'publish_on', 'retract_on', '_get_absolute_url',
                    'owner', 'created', '_actions')
    ...
    def _actions(self, obj):
        ...
        try:
            url = (
             reverse('search-fetch-force', args=[obj.id]),)
            one = '<a href="%s">Fetch tweets</a>' % url
        except NoReverseMatch:
            one = "Fetch tweets - add jmbo_twitter admin_urls to settings, eg. <code>(r'^admin/', include('jmbo_twitter.admin_urls'))</code>"
        ...
        try:
            url = (
             reverse('search-tweets', args=[obj.id]),)
            two = '<a href="%s" target="_blank">View tweets</a>' % url
        except NoReverseMatch:
            two = "View tweets - add jmbo_twitter admin_urls to settings, eg. <code>(r'^admin/', include('jmbo_twitter.admin_urls'))</code>"
