...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/papydvd/views.py
# Compiled at: 2011-04-25 11:02:25
...
class MainView(object):
    ...
    def __call__(self):
        ...
        return {'last_movie': last_movie, 'directors_count': directors_count, 
    ...
    def all_genres(self):
        ...
        return [ {'name': x.name, 'id': x.genre_id, 'description': x.description} for x in genres
               ]
...
class LoadMoviesView(MainView):
    ...
    def __call__(self):
        ...
        return Response(body=json.dumps({'movies': [ {'id': x.movie_id, 'primary_number': x.primary_number, 'secondary_number': x.secondary_number, 'title': x.title, 'genre': x.genre and x.genre.name or None, 'year': x.year, 'director': x.director and x.director.name or None} for x in movies
                   ], 
           'canGoLeft': b_start > 0, 
           'canGoRight': allMoviesCount > moviesCount}), content_type='application/json')
...
class AddMovieView(object):
    ...
    def __call__(self):
        ...
        movie = Movie(title=form['title'], genre_id=form['genre_id'], director_id=form['director_id'], year=form['year'], story='', cover=cover)
        dbsession.add(movie)
        dbsession.flush()
...
class AddGenreView(MainView):
    ...
    def __call__(self):
        ...
        dbsession.add(genre)
        dbsession.flush()
        return Response(body=json.dumps([ {'name': x['name'], 'id': x['id'], 'selected': x['name'] == form['name']} for x in self.all_genres
...
class AddDirectorView(MainView):
    ...
    def __call__(self):
        ...
        dbsession.add(genre)
        dbsession.flush()
        return Response(body=json.dumps([ {'name': x['name'], 'id': x['id'], 'selected': x['name'] == form['name']} for x in self.all_directors
                                        ]), content_type='application/json')
...
class LoadMovieView(MainView):
    ...
    def __call__(self):
        ...
        return Response(body=json.dumps({'title': movie.title, 'primary_number': movie.primary_number, 
           'secondary_number': movie.secondary_number or 0, 
           'genre_id': movie.genre and movie.genre.genre_id or '', 
           'year': movie.year, 
           'director_id': movie.director and movie.director.director_id or ''}), content_type='application/json')
...
class LoadDirectorView(MainView):
    ...
    def __call__(self):
        ...
        return Response(body=json.dumps({'name': movie.name}), content_type='application/json')
...
class DeleteGenreView(MainView):
    ...
    def __call__(self):
        ...
        self.error_message = 'Non posso rimuovere il genere perché è usato ancora in %s film.' % movies_using_genre
...
class ImageView(object):
    ...
    def __call__(self):
        ...
        return Response(body=image.data, content_type='image/jpg', headerlist=[
...
class DumpMoviesView(object):
    ...
    def __call__(self):
        ...
        return Response(body=out.getvalue(), content_type='text/csv', headerlist=[
