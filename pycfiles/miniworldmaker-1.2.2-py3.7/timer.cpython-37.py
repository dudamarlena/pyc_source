...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\miniworldmaker\tools\timer.py
# Compiled at: 2020-02-16 08:47:08
    ...
    def _call_method(self):
        ...
        if type(self.parameters) == list:
            if len(sig.parameters) == len(self.parameters):
                ...
                raise Exception('Wrong number of arguments for ' + str(self.method) + ' in , got ' + str(len(self.parameters)) + ' but should be ' + str(len(sig.parameters)) + 'Additional Information: File:' + str(info.filename), '; Method: ' + str(method))
