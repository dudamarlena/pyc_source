...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\miniworldmaker\tokens\token.py
# Compiled at: 2020-02-16 10:03:22
# Size of source mod 2**32: 31958 bytes
import math
from typing import Union
import pygame
import miniworldmaker.app as app
import miniworldmaker.appearances as appearances
from miniworldmaker.appearances import costume
from miniworldmaker.board_positions import board_position
import miniworldmaker.physics as ph
...
class Meta(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if hasattr(instance, 'setup'):
            instance.setup()
        if hasattr(instance, 'is_static'):
            if instance.is_static is True:
...
class Token(pygame.sprite.DirtySprite, metaclass=Meta):
    ...
    def from_center(cls, center_position):
        ...
        """
        obj = cls(position=(0, 0))
        obj.center = center_position
        return obj
    ...
    def all_subclasses(cls):
        ...
        def rec_all_subs(cls):
            if cls.subclasses is None:
                return set(cls.__subclasses__()).union([s for c in cls.__subclasses__() for s in rec_all_subs(c)])
    ...
    def add_costume(self, source: Union[(str, tuple)]=(255, 255, 255, 0)) -> costume.Costume:
        ...
        if type(source) == str:
            ...
            self.__class__.class_image = self.__class__.class_image or source
        else:
    ...
    def bounce_from_border(self, borders):
        ...
        if not 'top' in borders or self.direction <= 0 and self.direction > -90 or self.direction <= 90 and self.direction >= 0:
            self.point_in_direction(0)
            incidence = self.direction - angle
            self.turn_left(180 - incidence)
        else:
            pass
        if not 'bottom' in borders or self.direction < -90 and self.direction >= -180 or self.direction > 90 and self.direction <= 180:
            self.point_in_direction(180)
            incidence = self.direction - angle
            self.turn_left(180 - incidence)
        else:
            if 'left' in borders and self.direction <= 0:
                self.point_in_direction(-90)
                incidence = self.direction - angle
                self.turn_left(180 - incidence)
            else:
                if 'right' in borders and self.direction >= 0:
                    self.point_in_direction(90)
                    incidence = self.direction - angle
                    self.turn_left(180 - incidence)
                else:
    ...
    def check_for_deprecated_methods(cls):
        ...
        if 'get_event' in members:
            print("Deprecated method 'get_event' found in " + str(cls) + ', use specific methods (on_key_down, on_key_pressed...) instead')
        ...
        if [member for member in members if member.startswith('on_sensing_collision_with')]:
            print("Deprecated method 'on_sensing_collision_with_[token_class]' found in " + str(cls) + ", use 'on_touching_[token_class]' instead")
        if [member for member in members if member.startswith('on_sensing_separation_with')]:
            print("Deprecated method 'on_sensing_separation_with_[token_class]' found in " + str(cls) + ", use 'on_separation_with_[token_class]' instead")
