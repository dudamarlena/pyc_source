...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\miniworldmaker\containers\toolbar_widgets.py
# Compiled at: 2020-02-02 04:46:00
    ...
    def get_event(self, event, data):
        if event == 'mouse_left':
            if self.file is None:
                Tk().withdraw()
                try:
                    self.file = filedialog.asksaveasfilename(initialdir='./', title='Select file', filetypes=(('db files', '*.db'),
                                                                                                              ('all files', '*.*')))
...
class LoadButton(ToolbarWidget):
    ...
    def get_event(self, event, data):
        if event == 'mouse_left':
            Tk().withdraw()
            if self.file is None:
                self.file = filedialog.askopenfilename(initialdir='./', title='Select file', filetypes=(('db files', '*.db'),
                                                                                                        ('all files', '*.*')))
