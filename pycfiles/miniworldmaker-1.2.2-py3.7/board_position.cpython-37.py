...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\miniworldmaker\board_positions\board_position.py
# Compiled at: 2020-02-12 00:39:13
# Size of source mod 2**32: 7461 bytes
...
class BoardPosition(collections.namedtuple('Point', ['x', 'y'])):
    ...
    def near(self, other, distance):
        ...
        if isinstance(other, tuple):
            ...
            if self.x <= other.x + distance and self.x >= other.x - distance and self.y <= other.y + distance and self.y >= other.y - distance:
                return True
        else:
            return False
    ...
    def is_on_board(self):
        ...
        if self.x >= 0:
            if self.y >= 0:
                if self.x < board.columns:
                    if self.y < board.rows:
                        return True
        return False
