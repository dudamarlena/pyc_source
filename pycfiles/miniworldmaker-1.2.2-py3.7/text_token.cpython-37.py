...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\miniworldmaker\tokens\text_token.py
# Compiled at: 2020-02-14 04:59:01
...
class TextToken(token.Token):
    ...
    def set_auto_size(self):
        if self.auto_size:
            self.size = (
             self.costume.font_size * len(self.costume.text), self.costume.font_size)
        else:
