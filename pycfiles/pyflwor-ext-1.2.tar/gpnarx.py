...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gpnarx/gpnarx.py
# Compiled at: 2018-02-01 11:59:15
import sys
if sys.version_info < (3, ):
    range = xrange
import numpy as np, pandas as pd, scipy.linalg as la, scipy.sparse as sp, scipy.stats as ss
...
class GPNARX(tsm.TSM):
    ...
    **** GAUSSIAN PROCESS NONLINEAR AUTOREGRESSIVE (GP-NARX) MODELS ****
    ...
    def __init__(self, data, ar, kernel, integ=0, target=None):
        ...
        self.data = np.array(self.data_full[self.max_lag:self.data_full.shape[0]])
    ...
    def _construct_predict(self, beta, h):
        ...
        return (
         predictions, variances, predictions - 1.98 * np.power(variances, 0.5), predictions + 1.98 * np.power(variances, 0.5))
    ...
    def predict_is(self, h=5, fit_once=True):
        ...
        predictions = []
        for t in range(0, h):
            x = GPNARX(ar=self.ar, kernel=self.kernel, integ=self.integ, data=self.data_original[:-h + t])
