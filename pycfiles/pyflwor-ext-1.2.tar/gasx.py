...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gas/gasx.py
...
class GASX(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE SCORE E(X)OGENOUS VARIABLE (GASX) MODELS ****
    ...
    def _uncythonized_model(self, beta):
        ...
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta = np.matmul(self.X[self.integ + self.max_lag:], parm[self.sc + self.ar:self.sc + self.ar + len(self.X_names)])
        theta, self.model_scores = gasx_recursion(parm, theta, self.model_scores, self.model_Y, self.ar, self.sc, self.model_Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
         np.array(theta), self.model_Y, self.model_scores)
    ...
    def _uncythonized_mb_model(self, beta, mini_batch):
        ...
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta = np.matmul(X[self.integ + self.max_lag:], parm[self.sc + self.ar:self.sc + self.ar + len(self.X_names)])
        theta, self.model_scores = gasx_recursion(parm, theta, self.model_scores, Y, self.ar, self.sc, Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
         np.array(theta), Y, self.model_scores)
