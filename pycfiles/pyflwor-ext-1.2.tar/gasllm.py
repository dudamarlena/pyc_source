...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gas/gasllm.py
...
class GASLLEV(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE SCORE LOCAL LEVEL MODELS ****
    ...
    def __init__(self, data, family, integ=0, target=None):
        ...
        if self.cythonized is True:
            ...
            if self.family.gradient_only is True:
                self.recursion = self.family.gradientllev_recursion()
            else:
                self.recursion = self.family.newtonllev_recursion()
        else:
            self._model = self._uncythonized_model
            self._mb_model = self._uncythonized_mb_model
        self.model_name = self.model_name2 + ' GAS LLM'
    ...
    def _create_latent_variables(self):
        ...
        None (changes model attributes)
        """
        self.latent_variables.add_z('SC(1)', fam.Normal(0, 0.5, transform=None), fam.Normal(0, 3))
        return
    ...
    def _cythonized_model(self, beta):
        ...
        return (
         np.array(theta), self.model_Y, np.array(self.model_scores))
    ...
    def _cythonized_mb_model(self, beta, mini_batch):
        ...
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores = self.recursion(parm, theta, self.model_scores, Y, Y.shape[0], model_scale, model_shape, model_skewness, self.max_lag)
        return (
         np.array(theta), Y, np.array(self.model_scores))
    ...
    def _uncythonized_model(self, beta):
        ...
        return (
         theta, self.model_Y, self.model_scores)

    def _uncythonized_mb_model(self, beta):
        """ Creates the structure of the model
        ...
        theta = np.zeros(Y.shape[0])
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores = gas_llev_recursion(parm, theta, self.model_scores, Y, Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
         theta, Y, self.model_scores)
    ...
    def _sim_prediction(self, theta, Y, scores, h, t_params, simulations):
        ...
        for n in range(0, simulations):
            ...
            for t in range(0, h):
                ...
                theta_exp = np.append(theta_exp, [new_value])
                scores_exp = np.append(scores_exp, scores[np.random.randint(scores.shape[0])])
    ...
    def _sim_prediction_bayes(self, h, simulations):
        ...
        for n in range(0, simulations):
            ...
            theta, Y, scores = self._model(t_z)
            ...
            scores_exp = scores.copy()
            for t in range(0, h):
                new_value = theta_exp[(-1)] + t_z[0] * scores_exp[(-1)]
                if self.model_name2 == 'Exponential':
                    rnd_value = self.family.draw_variable(1.0 / self.link(new_value), model_scale, model_shape, model_skewness, 1)[0]
                else:
                    rnd_value = self.family.draw_variable(self.link(new_value), model_scale, model_shape, model_skewness, 1)[0]
    ...
    def _sim_predicted_mean(self, theta, Y, scores, h, t_params, simulations):
    ...
    def plot_predict(self, h=5, past_values=20, intervals=True, **kwargs):
        ...
        if self.latent_variables.estimated is False:
            ...
            if self.latent_variables.estimation_method in ('M-H', ):
                ...
                mean_values = self._mean_prediction(theta, Y, scores, h, t_z)
                ...
                if intervals is True:
                    sim_values = self._sim_prediction(theta, Y, scores, h, t_z, 15000)
                else:
                    sim_values = self._sim_prediction(theta, Y, scores, h, t_z, 2)
