...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gas/gas.py
...
class GAS(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE SCORE (GAS) MODELS ****
    ...
    def __init__(self, data, ar, sc, family, integ=0, target=None):
        ...
        if self.cythonized is True:
            ...
            if self.family.gradient_only is True:
                self.recursion = self.family.gradient_recursion()
            else:
                self.recursion = self.family.newton_recursion()
        else:
    ...
    def _uncythonized_model(self, beta):
        ...
        theta = np.ones(self.model_Y.shape[0]) * parm[0]
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores = gas_recursion(parm, theta, self.model_scores, self.model_Y, self.ar, self.sc, self.model_Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
    ...
    def _uncythonized_mb_model(self, beta, mini_batch):
        ...
        theta = np.ones(Y.shape[0]) * parm[0]
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores = gas_recursion(parm, theta, self.model_scores, Y, self.ar, self.sc, Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
