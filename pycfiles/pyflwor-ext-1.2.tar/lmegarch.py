...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/lmegarch.py
    ...
    **** LONG MEMORY BETA-t-EGARCH MODELS ****
    ...
    def _create_latent_variables(self):
        ...
        for component in range(2):
            ...
            for p_term in range(self.p):
                ...
                if p_term == 0:
                    self.latent_variables.z_list[(1 + p_term + component * (self.p + self.q))].start = 3.0
                else:
                    self.latent_variables.z_list[(1 + p_term + component * (self.p + self.q))].start = 2.0
            ...
            for q_term in range(self.q):
                ...
                if p_term == 0 and component == 0:
                    self.latent_variables.z_list[(1 + self.p + q_term + component * (self.p + self.q))].start = -4.0
                elif p_term == 0 and component == 1:
                    self.latent_variables.z_list[(1 + self.p + q_term + component * (self.p + self.q))].start = -3.0
    ...
    def _mean_prediction(self, lmda, lmda_c, Y, scores, h, t_params):
        ...
        for t in range(0, h):
            ...
            lmda_exp = np.append(lmda_exp, [new_value_comp.sum() + t_params[0]])
    ...
    def _sim_prediction(self, lmda, lmda_c, Y, scores, h, t_params, simulations):
        ...
        for n in range(0, simulations):
            ...
            for t in range(0, h):
                ...
                scores_exp = np.append(scores_exp, scores[rnd_no])
    ...
    def _sim_prediction_bayes(self, h, simulations):
        ...
        for n in range(0, simulations):
            ...
            for t in range(0, h):
                ...
                lmda_exp = np.append(lmda_exp, [new_value_comp.sum() + t_z[0]])
                lmda_c_exp[0] = np.append(lmda_c_exp[0], new_value_comp[0])
                lmda_c_exp[1] = np.append(lmda_c_exp[1], new_value_comp[1])
                rnd_no = np.random.randint(scores.shape[0])
                scores_exp = np.append(scores_exp, scores[rnd_no])
    ...
    def _sim_predicted_mean(self, lmda, lmda_c, Y, scores, h, t_params, simulations):
        """ Simulates a h-step ahead mean prediction
    ...
    def _summarize_simulations(self, lmda, sim_vector, date_index, h, past_values):
        ...
        return (
         error_bars, forecasted_values, plot_values, plot_index)
    ...
    def neg_loglik(self, beta):
        ...
        return -np.sum(ss.t.logpdf(x=Y, df=self.latent_variables.z_list[(-2)].prior.transform(beta[(-2)]), loc=np.ones(lmda.shape[0]) * self.latent_variables.z_list[(-1)].prior.transform(beta[(-1)]), scale=np.exp(lmda / 2.0)))
    ...
    def mb_neg_loglik(self, beta, mini_batch):
        ...
        The negative logliklihood of the model
        """
        lmda, _, Y, ___ = self._mb_model(beta, mini_batch)
        return -np.sum(ss.t.logpdf(x=Y, df=self.latent_variables.z_list[(-2)].prior.transform(beta[(-2)]), loc=np.ones(lmda.shape[0]) * self.latent_variables.z_list[(-1)].prior.transform(beta[(-1)]), scale=np.exp(lmda / 2.0)))
    ...
    def plot_fit(self, **kwargs):
        ...
        if self.latent_variables.estimated is False:
            ...
            sigma2, lmda_c, Y, ___ = self._model(self.latent_variables.get_z_values())
    ...
    def plot_predict(self, h=5, past_values=20, intervals=True, **kwargs):
        ...
        if self.latent_variables.estimated is False:
            ...
            if self.latent_variables.estimation_method in ('M-H', ):
                ...
                sim_values = self._sim_prediction(lmda, lmda_c, Y, scores, h, t_z, 15000)
    ...
    def predict_is(self, h=5, fit_once=True, fit_method='MLE', intervals=False, **kwargs):
        ...
        if intervals is True:
            predictions.rename(columns={0: self.data_name, 1: '1% Prediction Interval', 2: '5% Prediction Interval', 
               3: '95% Prediction Interval', 4: '99% Prediction Interval'}, inplace=True)
        else:
        ...
        if self.latent_variables.estimated is False:
            ...
            if self.latent_variables.estimation_method in ('M-H', ):
                ...
                if intervals is True:
                    ...
                    sim_values = self._sim_prediction(lmda, lmda_c, Y, scores, h, t_z, 2)
                mean_values = self._sim_predicted_mean(lmda, lmda_c, Y, scores, h, t_z, 15000)
            ...
            if intervals is False:
                ...
                result.rename(columns={0: self.data_name, 1: '1% Prediction Interval', 2: '5% Prediction Interval', 
                   3: '95% Prediction Interval', 4: '99% Prediction Interval'}, inplace=True)
    ...
    def sample(self, nsims=1000):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            data_draws = np.array([ ss.t.rvs(loc=self.latent_variables.z_list[(-1)].prior.transform(lv_draws[(-1, i)]), df=self.latent_variables.z_list[(-2)].prior.transform(lv_draws[(-2, i)]), scale=np.exp(sigmas[i] / 2.0)) for i in range(nsims)
                                  ])
            return data_draws
    ...
    def plot_sample(self, nsims=10, plot_data=True, **kwargs):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            plt.figure(figsize=figsize)
            date_index = self.index[max(self.p, self.q):]
            lmda, _, Y, _ = self._model(self.latent_variables.get_z_values())
