...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/arma/nnar.py
...
class NNAR(tsm.TSM):
    ...
    **** NEURAL NETWORK AUTOREGRESSIVE INTEGRATED (NNAR) MODELS ****
    ...
    def __init__(self, data, ar, units, layers, integ=0, target=None, family=fam.Normal(), activation=np.tanh, initialize_random=True):
    ...
    def predict_is(self, h=5, fit_once=True, fit_method='MLE', intervals=False, **kwargs):
        ...
        predictions = []
        for t in range(0, h):
            x = NNAR(ar=self.ar, integ=self.integ, units=self.units, layers=self.layers, target=self.target, data=self.data_original[:-h + t], family=self.family)
