...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/garch.py
...
class GARCH(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE CONDITIONAL HETEROSCEDASTICITY (GARCH) MODELS ****
    ...
    def _create_latent_variables(self):
        ...
        self.latent_variables.z_list[0].start = -7.0
    ...
    def _mean_prediction(self, sigma2, Y, scores, h, t_params):
        ...
        for t in range(0, h):
            ...
            sigma2_exp = np.append(sigma2_exp, [new_value])
    ...
    def sample(self, nsims=1000):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            data_draws = np.array([ ss.norm.rvs(loc=self.latent_variables.z_list[(-1)].prior.transform(lv_draws[(-1, i)]), scale=np.sqrt(sigmas[i])) for i in range(nsims)
                                  ])
            return data_draws
