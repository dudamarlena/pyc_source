...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/tsm.py
...
import numdifftools as nd, pandas as pd
...
class TSM(object):
    ...
    def _categorize_model_output(self, z):
        if self.model_type in ('GAS', 'GASX', 'GASLLEV', 'GARCH', 'EGARCH', 'EGARCHM'):
            ...
            X_names = None
        return (
         theta, Y, scores, states, states_var, X_names)

    def _bbvi_fit(self, posterior, optimizer='RMSProp', iterations=1000, map_start=True, batch_size=12, mini_batch=None, learning_rate=0.001, record_elbo=False, quiet_progress=False, **kwargs):
        ...
        if self.model_type not in ('GPNARX', 'GPR', 'GP', 'GASRank') and map_start is True and mini_batch is None:
            p = optimize.minimize(posterior, phi, method='L-BFGS-B')
    ...
    def _ols_fit(self):
        ...
        return MLEResults(data_name=self.data_name, X_names=X_names, model_name=self.model_name, model_type=self.model_type, latent_variables=latent_variables_store, results=None, data=Y, index=self.index, multivariate_model=self.multivariate_model, objective_object=self.neg_loglik, method=method, ihessian=ihessian, signal=theta, scores=scores, z_hide=self._z_hide, max_lag=self.max_lag, states=states, states_var=states_var)
    ...
    def _optimize_fit(self, obj_type=None, **kwargs):
        ...
        """
        preopt_search = kwargs.get('preopt_search', True)
        ...
        if preopt_search is True:
            ...
            phi = self.latent_variables.get_z_starting_values()
        phi = kwargs.get('start', phi).copy()
        p = optimize.minimize(obj_type, phi, method='L-BFGS-B', options={'gtol': 1e-08})
        if preoptimized is True:
            p2 = optimize.minimize(obj_type, self.latent_variables.get_z_starting_values(), method='L-BFGS-B', options={'gtol': 1e-08})
    ...
    def fit(self, method=None, **kwargs):
        ...
        if method == 'MLE':
            ...
            if method == 'M-H':
                return self._mcmc_fit(nsims=nsims, method=method, cov_matrix=cov_matrix, map_start=map_start, quiet_progress=quiet_progress)
            if method == 'Laplace':
            ...
            if method == 'BBVI':
                ...
                return self._bbvi_fit(posterior, optimizer=optimizer, iterations=iterations, batch_size=batch_size, mini_batch=mini_batch, map_start=map_start, learning_rate=learning_rate, record_elbo=record_elbo, quiet_progress=quiet_progress)
            if method == 'OLS':
    ...
    def draw_latent_variables(self, nsims=5000):
        ...
        if self.latent_variables.estimation_method is None:
            ...
            if self.latent_variables.estimation_method == 'BBVI':
                return np.array([ i.q.draw_variable_local(size=nsims) for i in self.latent_variables.z_list ])
            if self.latent_variables.estimation_method == 'M-H':
