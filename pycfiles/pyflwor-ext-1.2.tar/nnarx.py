...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/arma/nnarx.py
...
class NNARX(tsm.TSM):
    ...
    **** NEURAL NETWORK AUTOREGRESSIVE (NNAR) MODELS ****
    ...
    def _model(self, beta):
        ...
        return (
         neural_network_tanh(Y, self.X, z, self.units, self.layers, self.ar + len(self.X_names)), Y)
    ...
    def _mb_model(self, beta, mini_batch):
        ...
        z = np.array([ self.latent_variables.z_list[k].prior.transform(beta[k]) for k in range(beta.shape[0]) ])
        return (
         neural_network_tanh_mb(Y, X, z, self.units, self.layers, self.ar + len(self.X_names)), Y)
    ...
    def predict_new(self, X, z):
        ...
        for layer in range(1, self.layers):
            for unit in range(self.units):
                if layer == 1:
                    hidden_layer_output[(unit, layer - 1)] = self.activation(np.matmul(first_layer_output, z[params_used + unit * self.units + (layer - 1) * self.units ** 2:params_used + (unit + 1) * self.units + (layer - 1) * self.units ** 2]))
                else:
                    hidden_layer_output[(unit, layer - 1)] = self.activation(np.matmul(hidden_layer_output[:, layer - 1], z[params_used + unit * self.units + (layer - 1) * self.units ** 2:params_used + (unit + 1) * self.units + (layer - 1) * self.units ** 2]))
    ...
    def predict_is(self, h=5, fit_once=True, fit_method='MLE', intervals=False, **kwargs):
        ...
        predictions = []
        for t in range(0, h):
            x = NNAR(ar=self.ar, units=self.units, layers=self.layers, data=self.data_original[:-h + t], family=self.family)
