...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/arma/tests/test_arimax_normal.py
...
def test_no_terms():
    ...
    """
    model = ARIMAX(formula='y ~ x1', data=data, ar=0, ma=0)
    x = model.fit()
...
def test_couple_terms_integ():
    ...
    values are not nan
    """
    model = ARIMAX(formula='y ~ x1', data=data, ar=1, ma=1, integ=1)
    x = model.fit()
    assert len(model.latent_variables.z_list) == 5
    lvs = np.array([ i.value for i in model.latent_variables.z_list ])
    assert len(lvs[np.isnan(lvs)]) == 0
...
def test_2_no_terms():
    ...
    are not nan
    """
    model = ARIMAX(formula='y ~ x1 + x2', data=data, ar=0, ma=0)
    x = model.fit()
