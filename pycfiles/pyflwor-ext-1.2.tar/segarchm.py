...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/segarchm.py
    ...
    **** skew BETA-t-EGARCH IN MEAN MODELS ****
    ...
    def _create_latent_variables(self):
        ...
        for p_term in range(self.p):
            ...
            if p_term == 0:
                self.latent_variables.z_list[(-1)].start = 3.0
            else:
                self.latent_variables.z_list[(-1)].start = -4.0

        for q_term in range(self.q):
            self.latent_variables.add_z('q(' + str(q_term + 1) + ')', fam.Normal(0, 0.5, transform='logit'), fam.Normal(0, 3))
            if q_term == 0:
                self.latent_variables.z_list[(-1)].start = -1.5
            else:
                self.latent_variables.z_list[(-1)].start = -4.0
    ...
    def _mean_prediction(self, lmda, Y, scores, h, t_params):
        ...
        for t in range(0, h):
            ...
            lmda_exp = np.append(lmda_exp, [new_value])
    ...
    def _sim_prediction(self, lmda, Y, scores, h, t_params, simulations):
        ...
        for n in range(0, simulations):
            ...
            for t in range(0, h):
                ...
                Y_exp = np.append(Y_exp, Y[np.random.randint(Y.shape[0])])
    ...
    def _sim_prediction_bayes(self, h, simulations):
        ...
        for n in range(0, simulations):
            ...
            scores_exp = scores.copy()
            Y_exp = Y.copy()
            temp_theta = t_z[(-2)] + (t_z[(-4)] - 1.0 / t_z[(-4)]) * np.exp(lmda_exp[(-1)] / 2.0) * m1
            for t in range(0, h):
                new_value = t_z[0]
                if self.p != 0:
                    for j in range(1, self.p + 1):
                ...
                if self.leverage is True:
                    m1 = np.sqrt(t_z[(-3)]) * sp.gamma((t_z[(-3)] - 1.0) / 2.0) / (np.sqrt(np.pi) * sp.gamma(t_z[(-3)] / 2.0))
                    new_value += t_z[(1 + self.p + self.q)] * np.sign(-(Y_exp[(-1)] - temp_theta)) * (scores_exp[(-1)] + 1)
                temp_theta = t_z[(-2)] + (t_z[(-4)] - 1.0 / t_z[(-4)]) * np.exp(new_value / 2.0) * m1 + t_z[(-1)] * np.exp(new_value / 2.0)
                lmda_exp = np.append(lmda_exp, [new_value])
                scores_exp = np.append(scores_exp, scores[np.random.randint(scores.shape[0])])
                Y_exp = np.append(Y_exp, Y[np.random.randint(Y.shape[0])])
    ...
    def sample(self, nsims=1000):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            data_draws = np.array([ fam.Skewt.draw_variable(loc=self.latent_variables.z_list[(-2)].prior.transform(lv_draws[(-2, i)]), shape=self.latent_variables.z_list[(-3)].prior.transform(lv_draws[(-3, i)]), skewness=self.latent_variables.z_list[(-4)].prior.transform(lv_draws[(-4, i)]), nsims=len(sigmas[i]), scale=np.exp(sigmas[i] / 2.0)) for i in range(nsims)
                                  ])
            return data_draws
