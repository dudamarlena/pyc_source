...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gas/gasllt.py
...
class GASLLT(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE SCORE LOCAL LINEAR TREND MODELS ****
    ...
    def __init__(self, data, family, integ=0, target=None):
        ...
        if self.cythonized is True:
            ...
            if self.family.gradient_only is True:
                self.recursion = self.family.gradientllt_recursion()
            else:
                self.recursion = self.family.newtonllt_recursion()
        else:
    ...
    def _cythonized_model(self, beta):
        ...
        return (
         np.array(theta), theta_t, self.model_Y, np.array(self.model_scores))
    ...
    def _cythonized_mb_model(self, beta, mini_batch):
        ...
        return (
         np.array(theta), theta_t, Y, np.array(self.model_scores))
    ...
    def _uncythonized_model(self, beta):
        ...
        theta_t = np.zeros(self.model_Y.shape[0])
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores = gas_llt_recursion(parm, theta, theta_t, self.model_scores, self.model_Y, self.model_Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
         theta, theta_t, self.model_Y, self.model_scores)
    ...
    def _uncythonized_mb_model(self, beta, mini_batch):
        ...
        theta_t = np.zeros(Y.shape[0])
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores = gas_llt_recursion(parm, theta, theta_t, self.model_scores, Y, Y.shape[0], self.family.score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
         theta, theta_t, Y, self.model_scores)
