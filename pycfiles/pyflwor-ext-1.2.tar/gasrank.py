...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gas/gasrank.py
# Compiled at: 2018-02-01 11:59:15
import sys
if sys.version_info < (3, ):
    range = xrange
import numpy as np, pandas as pd, scipy.stats as ss, scipy.special as sp
...
class GASRank(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE SCORE (GAS) RANK MODELS ****
    ...
    def _model_two_components(self, beta):
        ...
        return (
         theta, self.data, state_vectors_1)
    ...
    def _model_abilities_two_components(self, beta):
        ...
        return (
         state_vectors_store_1, state_vectors_store_2)
            ...
            if type(team_ids) == type([]):
                if type(team_ids[0]) == str:
                    for team_id in team_ids:
                        plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values()).T[self.team_dict[team_id]], trim='b'), label=self.team_strings[self.team_dict[team_id]])

                else:
                    for team_id in team_ids:
                        plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values()).T[team_id], trim='b'), label=self.team_strings[team_id])

            elif type(team_ids) == str:
                plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values()).T[self.team_dict[team_ids]], trim='b'), label=self.team_strings[self.team_dict[team_ids]])
            else:
                plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values()).T[team_ids], trim='b'), label=self.team_strings[team_ids])
            plt.legend()
            plt.ylabel('Power')
            plt.xlabel('Games')
            plt.show()
    ...
    def plot_abilities_two_components(self, team_ids, component_id=0, **kwargs):
        ...
        if self.latent_variables.estimated is False:
            ...
            if type(team_ids) == type([]):
                if type(team_ids[0]) == str:
                    for team_id in team_ids:
                        plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values())[component_id].T[name_dict[team_id]], trim='b'), label=name_strings[name_dict[team_id]])

                else:
                    for team_id in team_ids:
                        plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values())[component_id].T[team_id], trim='b'), label=name_strings[team_id])

            elif type(team_ids) == str:
                plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values())[component_id].T[name_dict[team_ids]], trim='b'), label=name_strings[name_dict[team_ids]])
            else:
                plt.plot(np.trim_zeros(self._model_abilities(self.latent_variables.get_z_values())[component_id].T[team_ids], trim='b'), label=name_strings[team_ids])
            plt.legend()
            plt.ylabel('Power')
            plt.xlabel('Games')
            plt.show()
