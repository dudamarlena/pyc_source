...
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/var/var.py
# Compiled at: 2018-02-01 11:59:15
import sys
if sys.version_info < (3, ):
    range = xrange
import numpy as np, pandas as pd, scipy.stats as ss, datetime
...
class VAR(tsm.TSM):
    ...
    **** VECTOR AUTOREGRESSION (VAR) MODELS ****
    ...
    def __init__(self, data, lags, target=None, integ=0, use_ols_covariance=False):
        ...
        self._z_hide = np.power(self.data.shape[0], 2) - (np.power(self.data.shape[0], 2) - self.data.shape[0]) / 2
    ...
    def _forecast_mean(self, h, t_params, Y, shock_type=None, shock_index=0, shock_value=None, shock_dir='positive', irf_intervals=False):
        ...
        for t in range(0, h):
            ...
            for variable in range(0, self.ylen):
                ...
                new_values[variable] = t_params[index_ref]
    ...
    def construct_wishart(self, v, X):
        ...
        """
        self.adjust_prior(list(range(int(len(self.latent_variables.z_list) - self.ylen - (self.ylen ** 2 - self.ylen) / 2), int(len(self.latent_variables.z_list)))), fam.InverseWishart(v, X))