...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/epitome/functions.py
# Compiled at: 2020-01-22 09:58:54
# Size of source mod 2**32: 21407 bytes
"""
================
Helper functions
================
...
import pandas as pd, collections, numpy as np, os
from collections import Counter
from itertools import groupby
from scipy.io import loadmat
from .constants import *
import scipy.sparse, pyranges as pr
from operator import itemgetter
import urllib, sys, requests, urllib, tqdm
from zipfile import ZipFile
import gzip, multiprocessing

class Region:
    __doc__ = 'Genomic Region'
    ...
    if not os.path.exists(final_dst):
        ...
        if first_byte < file_size:
            ...
            pbar = tqdm.tqdm(total=file_size,
              ...
              unit='B',
              unit_scale=True,
              desc=('Dataset not found. Downloading Epitome data to %s...' % dst))
...
def get_assays_from_feature_file(feature_name_file=None, eligible_assays=None, eligible_cells=None, min_cells_per_assay=3, min_assays_per_cell=2):
    ...
    indexed_assays = {k:v for k, v in indexed_assays.items() if len(v) >= min_assays_per_cell}
...
def bedtools_intersect(file_triple):
    ...
    if file_triple[2]:
        ...
        t3 = t2.tolist()
        return (
         list(map(lambda x: Region(x[0], x[1], x[2]), t3)), overlap_vector)
    else:
        return (
         None, overlap_vector)
...
def bedFile2Vector(bed_file, allpos_bed_file):
    ...
    bed_files = [
     (
      allpos_bed_file, bed_file, False), (bed_file, allpos_bed_file, True)]
...
def calculate_epitome_regions(all_regions_file):
    ...
    EPITOME_TEST_REGIONS = [chr8_start, chr9_end]
    EPITOME_TRAIN_REGIONS = [
     [
      0, EPITOME_VALID_REGIONS[0]],
     [
      EPITOME_TEST_REGIONS[1], contig_ranges['chr21'][1]]]
    return (
     EPITOME_TRAIN_REGIONS, EPITOME_VALID_REGIONS, EPITOME_TEST_REGIONS)
...
def concatenate_all_data(data, region_file):
    ...
    return np.concatenate([data[Dataset.TRAIN][:, 0:chr6_end],
     data[Dataset.VALID],
     data[Dataset.TEST],
     data[Dataset.TRAIN][:, chr6_end:]],
      axis=1)