...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/epitome/viz.py
# Compiled at: 2020-01-20 03:36:57
...
def joint_plot(dict_model1, dict_model2, metric='AUC', model1_name='model1', model2_name='model2', outlier_filter=None):
    """
    ...
    def ann(row):
        ind = row[0]
        r = row[1]
        plt.gca().annotate(ind, xy=(r[model1_name], r[model2_name]), xytext=(2, 2),
...
def plot_uncertainty(preds_mean, preds_std, truth, title):
    ...
    plt.fill_between(xdata, (preds_mean - preds_std), (preds_mean + preds_std), color='gray',
      alpha=0.2)
    plt.title(title)
    plt.show()
...
import matplotlib.pyplot as plt, matplotlib.lines as mlines, matplotlib.transforms as mtransforms
...
def calibration_plot(truth, preds, assay_dict, list_assaymap):
    ...
    for i in range(truth.shape[1]):
        ...
        if not np.isnan(assay_dict[list_assaymap[(i + 1)]]['AUC']):
