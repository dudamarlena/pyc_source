...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/pyipv8/ipv8/taskmanager.py
...
class TaskManager(object):
    ...
    def cancel_all_pending_tasks(self):
        ...
        """
        with self._task_lock:
            assert all([ isinstance(task, (Deferred, DelayedCall, LoopingCall, tuple)) for task in self._pending_tasks.values()
                       ]), self._pending_tasks
    ...
    def is_pending_task_active(self, name):
        ...
        with self._task_lock:
            task = self._pending_tasks.get(name, None)
            if task:
                return self._get_isactive_stopper(task)[0]
            return False
        return
    ...
    def _get_isactive_stopper(self, task):
        ...
        with self._task_lock:
            if isinstance(task, Deferred):
                return (
                 not task.called, getattr(task, 'cancel', None))
            if isinstance(task, DelayedCall):
                return (task.active(), task.cancel)
            if isinstance(task, LoopingCall):
