...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/core/transaction.py
# Compiled at: 2019-06-08 08:28:02
...
class Transaction(object):
    ...
    def __init__(self, transaction_id, assets, order_id, partner_order_id, timestamp):
        ...
        self._assets = assets
        self._transferred_assets = AssetPair(AssetAmount(0, assets.first.asset_id), AssetAmount(0, assets.second.asset_id))
    ...
    def from_database(cls, data, payments):
        ...
        """
        trader_id, transaction_id, order_number, partner_trader_id, partner_order_number, asset1_amount, asset1_type, asset1_transferred, asset2_amount, asset2_type, asset2_transferred, transaction_timestamp, sent_wallet_info, received_wallet_info, incoming_address, outgoing_address, partner_incoming_address, partner_outgoing_address = data
        transaction_id = TransactionId(bytes(transaction_id))
        transaction = cls(transaction_id, AssetPair(AssetAmount(asset1_amount, str(asset1_type)), AssetAmount(asset2_amount, str(asset2_type))), OrderId(TraderId(bytes(trader_id)), OrderNumber(order_number)), OrderId(TraderId(bytes(partner_trader_id)), OrderNumber(partner_order_number)), Timestamp(transaction_timestamp))
        transaction._transferred_assets = AssetPair(AssetAmount(asset1_transferred, str(asset1_type)), AssetAmount(asset2_transferred, str(asset2_type)))
    ...
    def to_database(self):
        ...
        """
        return (
         database_blob(bytes(self.order_id.trader_id)), database_blob(bytes(self.transaction_id)),
    ...
    def from_accepted_trade(cls, accepted_trade, transaction_id):
        ...
        :rtype: Transaction
        """
        return cls(transaction_id, accepted_trade.assets, accepted_trade.recipient_order_id, accepted_trade.order_id, Timestamp.now())
    ...
    def from_tx_init_block(cls, tx_init_block):
        ...
        order_id = OrderId(TraderId(unhexlify(tx_dict['partner_trader_id'])), OrderNumber(tx_dict['partner_order_number']))
        partner_order_id = OrderId(TraderId(unhexlify(tx_dict['trader_id'])), OrderNumber(tx_dict['order_number']))
        return cls(TransactionId(tx_init_block.hash), AssetPair.from_dictionary(tx_dict['assets']), order_id, partner_order_id, Timestamp.now())
    ...
    def add_payment(self, payment):
        ...
        """
        self._logger.debug('Adding transferred assets %s to transaction %s', payment.transferred_assets, self.transaction_id.as_hex())
    ...
    def is_payment_complete(self):
        return self.transferred_assets.first >= self.assets.first and self.transferred_assets.second >= self.assets.second

    def to_block_dictionary(self):
        """
        Return a dictionary with a representation of this transaction (to add to a tx_done block).
        """
        return {'trader_id': self.order_id.trader_id.as_hex(), 
