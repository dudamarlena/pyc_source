...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/core/tick.py
# Compiled at: 2019-06-08 08:25:28
...
class Tick(object):
    ...
    def from_database(cls, data):
        trader_id, order_number, asset1_amount, asset1_type, asset2_amount, asset2_type, timeout, timestamp, is_ask, traded, block_hash = data
        tick_cls = Ask if is_ask else Bid
        order_id = OrderId(TraderId(trader_id), OrderNumber(order_number))
        return tick_cls(order_id, AssetPair(AssetAmount(asset1_amount, str(asset1_type)), AssetAmount(asset2_amount, str(asset2_type))), Timeout(timeout), Timestamp(timestamp), traded=traded, block_hash=bytes(block_hash))
    ...
    def is_valid(self):
        ...
        :rtype: bool
        """
        return not self._timeout.is_timed_out(self._timestamp) and int(old_round(time.time() * 1000)) >= int(self.timestamp) - self.TIME_TOLERANCE and int(self._timeout) <= MAX_ORDER_TIMEOUT
...
class Ask(Tick):
    ...
    def from_block(cls, block):
        ...
        return cls(OrderId(TraderId(unhexlify(tx_dict['trader_id'])), OrderNumber(tx_dict['order_number'])), AssetPair.from_dictionary(tx_dict['assets']), Timeout(tx_dict['timeout']), Timestamp(tx_dict['timestamp']), traded=tx_dict['traded'], block_hash=block.hash)
