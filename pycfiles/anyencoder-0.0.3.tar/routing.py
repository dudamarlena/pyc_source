...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/pyipv8/ipv8/dht/routing.py
...
def calc_node_id(ip, mid):
    ip_bin = inet_aton(ip)
    ip_mask = '\x03\x0f?\x00xff'
    ip_masked = ('').join([ chr(ord(ip_bin[i:i + 1]) & ord(ip_mask[i:i + 1])) for i in range(4) ])
    crc32_unsigned = binascii.crc32(cast_to_bin(ip_masked)) % 4294967296
...
class Node(Peer):
    ...
    def status(self):
        ...
        if now - self.last_response < 900 or self.last_response > 0 and now - self.last_query < 900:
...
class RoutingTable(object):
    ...
    def get_bucket(self, node_id):
        node_id_binary = id_to_binary_string(node_id)
        return self.trie.longest_prefix_value(cast_to_unicode(node_id_binary), default=None) or self.trie['']
        ...
        with self.lock:
            ...
            for i in reversed(range(len(prefix) + 1)):
                for suffix in self.trie.suffixes(prefix[:i]):
                    ...
                    nodes |= {node for node in list(bucket.nodes.values()) if node.status != NODE_STATUS_BAD and (exclude_node is None or node.id != exclude_node.id)}
