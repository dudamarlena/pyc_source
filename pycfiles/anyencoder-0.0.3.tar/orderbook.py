...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/core/orderbook.py
...
class OrderBook(TaskManager):
    ...
    def update_ticks(self, ask_order_dict, bid_order_dict, traded_quantity):
        ...
        """
        ask_order_id = OrderId(TraderId(unhexlify(ask_order_dict['trader_id'])), OrderNumber(ask_order_dict['order_number']))
        bid_order_id = OrderId(TraderId(unhexlify(bid_order_dict['trader_id'])), OrderNumber(bid_order_dict['order_number']))
        self._logger.debug('Updating ticks in order book: %s and %s (traded quantity: %s)', str(ask_order_id), str(bid_order_id), str(traded_quantity))
        ...
        elif not ask_exists and ask_order_dict['traded'] < ask_order_dict['assets']['first']['amount'] and ask_order_id not in self.completed_orders:
            new_pair = AssetPair.from_dictionary(ask_order_dict['assets'])
            ask = Ask(ask_order_id, new_pair, Timeout(ask_order_dict['timeout']), Timestamp(ask_order_dict['timestamp']), traded=ask_order_dict['traded'])
            self.insert_ask(ask)
        ...
        if bid_exists and bid_order_dict['traded'] >= self.get_tick(bid_order_id).traded:
            ...
            if tick.traded >= tick.assets.first.amount:
                self.remove_tick(tick.order_id)
                self.completed_orders.add(tick.order_id)
        elif not bid_exists and bid_order_dict['traded'] < bid_order_dict['assets']['first']['amount'] and bid_order_id not in self.completed_orders:
            new_pair = AssetPair.from_dictionary(bid_order_dict['assets'])
            bid = Bid(bid_order_id, new_pair, Timeout(bid_order_dict['timeout']), Timestamp(bid_order_dict['timestamp']), traded=bid_order_dict['traded'])
            self.insert_bid(bid)
    ...
    def get_bid_ask_spread(self, price_wallet_id, quantity_wallet_id):
        ...
        :rtype: Price
        """
        spread = self.get_ask_price(price_wallet_id, quantity_wallet_id).frac - self.get_bid_price(price_wallet_id, quantity_wallet_id).frac
