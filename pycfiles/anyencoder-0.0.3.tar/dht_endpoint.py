...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/../pyipv8/ipv8/REST/dht_endpoint.py
...
class DHTBlockEndpoint(BaseEndpoint, BlockListener):
    ...
    def __init__(self, dht, trustchain):
        ...
        self.block_maintenance_task = self.trustchain.register_task('block_maintenance', LoopingCall(self.publish_latest_block))
            ...
            try:
                ...
                if public_key.verify(package.signature, pre_signed_content):
                    ...
                    if package.version not in new_blocks:
                        new_blocks[package.version] = [
                         ''] * package.block_count
                    new_blocks[package.version][package.block_position] = package.payload
            except PackError:
                logging.error('PackError: Found a clandestine entry in the DHT when reconstructing TC blocks: %s', entry)

        for version in new_blocks:
            new_blocks[version] = ('').join(new_blocks[version])
    ...
    def publish_block(self, block, republish=False):
        ...
        if block:
            ...
            actual_version = max((republish or self).block_version if 1 else self.block_version - 1, 0)
            ...
            def publish_chunk(_, slice_pointer, chunk_idx, chunk_attempt=1):
                ...
                if chunk_attempt > self.ATTEMPT_LIMIT:
                    logging.error('Publishing latest block failed after %d attempts on chunk %d', self.ATTEMPT_LIMIT, chunk_idx)
                    return
                ...
                blob_chunk = self.serializer.pack_multiple(DHTBlockPayload(signature, actual_version, chunk_idx, total_chunks, chunk).to_pack_list())
    ...
    def render_GET(self, request):
        ...
        def on_success(block_chunks):
            if not block_chunks:
                request.setResponseCode(http.NOT_FOUND)
                request.write(self.twisted_dumps({'error': {'handled': True, 
        ...
        raw_public_key = b64decode(request.args['public_key'][0])
...
class DHTStatisticsEndpoint(BaseEndpoint):
    ...
    def render_GET(self, request):
        ...
        stats = {'node_id': hexlify(self.dht.my_node_id), 'peer_id': hexlify(self.dht.my_peer.mid), 
        ...
        if isinstance(self.dht, DHTDiscoveryCommunity):
            stats.update({'num_peers_in_store': {hexlify(key):len(peers) for key, peers in self.dht.store.items()}, 'num_store_for_me': {hexlify(key):len(peers) for key, peers in self.dht.store_for_me.items()}})
...
class SpecificDHTPeerEndpoint(BaseEndpoint):
    ...
    def render_GET(self, request):
        ...
        def on_success(nodes):
            node_dicts = []
            for node in nodes:
                node_dicts.append({'public_key': b64encode(node.public_key.key_to_bin()).decode('utf-8'), 
        ...
        if not self.dht:
            ...
            for key, raw_values in self.dht.storage.items.items():
                ...
                for value in values:
                    ...
                    dicts.append({'public_key': b64encode(public_key).decode('utf-8') if public_key else None, 
                       'value': hexlify(data)})
...
class SpecificDHTValueEndpoint(BaseEndpoint):
    ...
    def render_PUT(self, request):
        ...
        self.dht.store_value(self.key, unhexlify(parameters['value'][0]), sign=True).addCallbacks(on_success, on_failure)
        return NOT_DONE_YET