...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/wallet/tc_wallet.py
...
class TrustchainWallet(Wallet, BlockListener):
    ...
    def get_balance(self):
        return succeed({'available': int(self.get_bandwidth_tokens() / MEGA_DIV), 
           'pending': 0, 
    ...
    def create_transfer_block(self, peer, quantity):
        ...
        deferred = self.trustchain.sign_block(peer, peer.public_key.key_to_bin(), block_type='tribler_bandwidth', transaction=transaction)
        addCallback(deferred, lambda _: None)
        latest_block = self.trustchain.persistence.get_latest(self.trustchain.my_peer.public_key.key_to_bin(), block_type='tribler_bandwidth')
        txid = '%s.%s.%d.%d' % (hexlify(latest_block.public_key).decode('utf-8'),
         latest_block.sequence_number, 0, int(quantity * MEGA_DIV))
        self.transaction_history.append({'id': txid, 
           'outgoing': True, 
    ...
    def get_num_unique_interactors(self, public_key):
        ...
        for block in self.trustchain.persistence.get_latest_blocks(public_key, limit=-1, block_type='tribler_bandwidth'):
        ...
        return (
         len(peers_you_helped), len(peers_helped_you))
    ...
    def bootstrap_new_identity(self, amount):
        ...
        """
        crypto = ECCrypto()
        tmp_peer = Peer(crypto.generate_key('curve25519'))
        transaction = {'up': 0, 
           'down': amount, 
           'type': 'tribler_bandwidth'}
        local_half_block = TriblerBandwidthBlock.create('tribler_bandwidth', transaction, self.trustchain.persistence, self.trustchain.my_peer.public_key.key_to_bin(), link_pk=tmp_peer.public_key.key_to_bin())
        local_half_block.sign(self.trustchain.my_peer.key)
        tmp_half_block = TriblerBandwidthBlock.create('tribler_bandwidth', transaction, self.trustchain.persistence, tmp_peer.public_key.key_to_bin(), link=local_half_block, link_pk=self.trustchain.my_peer.public_key.key_to_bin())
        ...
        block = {'block_hash': b64encode(tmp_half_block.hash), 'sequence_number': tmp_half_block.sequence_number}
        result = {'private_key': b64encode(tmp_peer.key.key_to_bin()), 'transaction': {'up': amount, 'down': 0}, 'block': block}
        return result
