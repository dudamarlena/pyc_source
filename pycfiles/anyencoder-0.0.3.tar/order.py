...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/core/order.py
...
class OrderId(object):
    ...
    def __bytes__(self):
        return '%s.%d' % (self.trader_id.as_hex().encode('utf-8'), self.order_number)
...
class Order(object):
    ...
    def from_database(cls, data, reserved_ticks):
        ...
        """
        trader_id, order_number, asset1_amount, asset1_type, asset2_amount, asset2_type, traded_quantity, received_quantity, timeout, order_timestamp, completed_timestamp, is_ask, cancelled, verified = data
        order_id = OrderId(TraderId(bytes(trader_id)), OrderNumber(order_number))
        order = cls(order_id, AssetPair(AssetAmount(asset1_amount, str(asset1_type)), AssetAmount(asset2_amount, str(asset2_type))), Timeout(timeout), Timestamp(order_timestamp), bool(is_ask))
    ...
    def to_database(self):
        ...
        return (
         database_blob(bytes(self.order_id.trader_id)), text_type(self.order_id.order_number),
    ...
    def status(self):
        ...
        if self.is_complete():
            return 'completed'
        if self._timeout.is_timed_out(self._timestamp):
            return 'expired'
        return 'open'
    ...
    def reserve_quantity_for_tick(self, order_id, quantity):
        ...
        self._logger.debug('Reserved %d quantity for order id %s (own order id: %s),total quantity: %d, traded quantity: %d, reserved quantity: %d', quantity, str(order_id), str(self.order_id), self.total_quantity, self.traded_quantity, self.reserved_quantity)
    ...
    def release_quantity_for_tick(self, order_id, quantity):
        ...
        if self._reserved_quantity >= quantity:
            ...
            if self._reserved_ticks[order_id] <= 0:
                del self._reserved_ticks[order_id]
        else:
            raise ValueError('Not enough reserved quantity for order id %s' % order_id)
        self._logger.debug('Released quantity for order id %s (own order id: %s),total quantity: %d, traded quantity: %d, reserved quantity: %d', str(order_id), str(self.order_id), self.total_quantity, self.traded_quantity, self.reserved_quantity)
    ...
    def add_trade(self, other_order_id, transferred_assets):
        self._logger.debug('Adding trade for order %s with quantity %s (other id: %s)', str(self.order_id), transferred_assets, str(other_order_id))
