...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vm_automation/esxiVm.py
...
import datetime, json, paramiko, requests, ssl, socket, time
...
class esxiServer():
    ...
    def connect(self):
        ...
        if hasattr(ssl, '_create_unverified_context'):
            ...
            try:
                self.connection = SmartConnect(host=self.hostname, user=self.username, pwd=self.password, port=int(self.port), sslContext=context)
    ...
    def cloneToServer(self, srcVm, destServer, destDatastore, destVm, timeout=1800):
        if type(srcVm) != str or type(destVm) != str:
            ...
            try:
                stdin, stdout, stderr = session.exec_command('/tmp/ovftool/ovftool -dm=thin -ds=' + destDatastore + ' --name=' + destVm + ' vi://' + srcServer + '/' + srcVm + ' vi://' + destServer)
                output = stdout.read()
    ...
    def clone(self, srcVm, destVm, thinProvision=True):
        if type(srcVm) != str or type(destVm) != str:
            ...
            if result:
                try:
                    session.exec_command('find "' + path + '/' + srcVm + '" -maxdepth 1 -type f | grep -v ".vmdk"' + ' | while read file; do cp "$file" "' + path + '/' + destVm + '"; done')
            ...
            if result:
                try:
                    ...
                    if thinProvision:
                        stdin, stdout, stderr = session.exec_command('vmkfstools -i "' + path + '/' + srcVmdk + '" -d thin "' + path + '/' + destVm + '/' + destVmdk + '"')
                    else:
                        stdin, stdout, stderr = session.exec_command('vmkfstools -i "' + path + '/' + srcVmdk + '" -d zeroedthick "' + path + '/' + destVm + '/' + destVmdk + '"')
                    output = stdout.read()
            ...
            if result:
                ...
                try:
                    session.exec_command('vim-cmd solo/registervm ' + path + '/' + destVm + '/' + srcVm + '.vmx ' + destVm)
    ...
    def __toggleFirewallRules(self, session, enabled=False):
        ...
        for service in ['sshClient', 'httpClient']:
            try:
                session.exec_command('esxcli network firewall ruleset set -e ' + str(enabled).lower() + ' -r ' + service)
            except paramiko.SSHException:
