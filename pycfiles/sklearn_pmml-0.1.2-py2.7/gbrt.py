...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/sklearn_pmml/convert/gbrt.py
# Compiled at: 2015-10-12 11:06:40
from copy import copy
from sklearn.ensemble import GradientBoostingClassifier
from sklearn.ensemble.gradient_boosting import LogOddsEstimator
from sklearn_pmml.convert.features import *
from sklearn_pmml.convert.model import EstimatorConverter, ClassifierConverter, ModelMode, RegressionConverter, Schema, TransformationContext
...
class GradientBoostingConverter(ClassifierConverter):
    ...
    """
    SCHEMAS_IN_MINING_MODEL = {
     Schema.INPUT}

    def __init__(self, estimator, context):
        super(GradientBoostingConverter, self).__init__(estimator, context)
        assert isinstance(estimator, GradientBoostingClassifier), 'This converter can only process GradientBoostingClassifier instances'
        ...
        assert context.schemas[Schema.OUTPUT][0].optype == FeatureOpType.CATEGORICAL, 'Classification output must be categorical'
    ...
    def output_transformation(self):
        ...
        output_field = pmml.OutputField(dataType=positive_category.data_type.value, feature='transformedValue', name=Schema.CATEGORIES.extract_feature_name(positive_category), optype=positive_category.optype.value)
        neg = pmml.Apply(function='*')
        neg.append(pmml.FieldRef(field=Schema.INTERNAL.extract_feature_name(positive_category.namespace)))
        neg.append(pmml.Constant(-(1 + self.estimator.n_estimators * self.estimator.learning_rate), dataType=FeatureType.DOUBLE.value))
        ...
        output_field = pmml.OutputField(dataType=negative_category.data_type.value, feature='transformedValue', name=Schema.CATEGORIES.extract_feature_name(negative_category), optype=negative_category.optype.value)
        ...
        output_field = pmml.OutputField(feature='transformedValue', name=Schema.OUTPUT.extract_feature_name(label), optype=label.optype.value, dataType=label.data_type.value)
    ...
    def segmentation(self):
        ...
        regression_context.schemas[Schema.OUTPUT] = [
         RealNumericFeature(name=self.context.schemas[Schema.OUTPUT][0].name, namespace=Schema.NUMERIC.namespace)]
