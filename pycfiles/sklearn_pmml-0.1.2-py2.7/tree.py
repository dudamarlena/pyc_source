...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/sklearn_pmml/convert/tree.py
# Compiled at: 2015-10-12 11:40:13
...
class DecisionTreeConverter(EstimatorConverter):
    ...
    def __init__(self, estimator, context, mode):
        ...
        if mode == ModelMode.CLASSIFICATION:
            ...
            assert isinstance(self.estimator, ClassifierMixin), 'Only a classifier can be serialized in classification mode'
        if mode == ModelMode.REGRESSION:
            assert isinstance(self.context.schemas[Schema.OUTPUT][0], NumericFeature), 'Only a numeric feature can be an output of regression'
            assert isinstance(self.estimator, RegressorMixin), 'Only a regressor can be serialized in regression mode'
        assert estimator.tree_.value.shape[1] == len(self.context.schemas[Schema.OUTPUT]), ('Tree outputs {} results while the schema specifies {} output fields').format(estimator.tree_.value.shape[1], len(self.context.schemas[Schema.OUTPUT]))
    ...
    def _model(self):
        assert Schema.NUMERIC in self.context.schemas, ('Either build transformation dictionary or provide {} schema in context').format(Schema.NUMERIC)
        ...
        tm.Node = self._transform_node(self.estimator.tree_, self.NODE_ROOT, self.context.schemas[Schema.NUMERIC], self.context.schemas[Schema.OUTPUT][0])
        return tm
    ...
    def _transform_node(self, tree, index, input_schema, output_feature, enter_condition=None):
        ...
        if tree.children_left[index] != TREE_LEAF:
            ...
            left_child = self._transform_node(tree, tree.children_left[index], input_schema, output_feature, enter_condition=pmml.SimplePredicate(field=feature.full_name, operator=DecisionTreeConverter.OPERATOR_LE, value_=tree.threshold[index]))
            ...
            if self.model_function == ModelMode.CLASSIFICATION:
                ...
                for i in range(len(tree.value[index][0])):
                    ...
                    node.append(pmml.ScoreDistribution(recordCount=left_score.recordCount + right_score.recordCount, value_=left_score.value_, confidence=prob))
                    if score_prob < prob:
            ...
            if self.model_function == ModelMode.CLASSIFICATION:
                ...
                for i in range(len(probs)):
                    node.append(pmml.ScoreDistribution(confidence=probs[i], recordCount=node_value[i], value_=output_feature.from_number(i)))
    ...
    def output(self):
        ...
        for feature in self.context.schemas[Schema.OUTPUT]:
            output_field = pmml.OutputField(name=Schema.OUTPUT.extract_feature_name(feature), feature='predictedValue', optype=feature.optype.value, dataType=feature.data_type.value)
            output.append(output_field)

        for feature in self.context.schemas[Schema.CATEGORIES]:
            output_field = pmml.OutputField(name=Schema.CATEGORIES.extract_feature_name(feature), optype=feature.optype.value, dataType=feature.data_type.value, feature='probability', targetField=Schema.INTERNAL.extract_feature_name(feature.namespace), value_=feature.name)
            output.append(output_field)

        return output


estimator_to_converter[DecisionTreeClassifier] = partial(DecisionTreeConverter, mode=ModelMode.CLASSIFICATION)
estimator_to_converter[DecisionTreeRegressor] = partial(DecisionTreeConverter, mode=ModelMode.REGRESSION)