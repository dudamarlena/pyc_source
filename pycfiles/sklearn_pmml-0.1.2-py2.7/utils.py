...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/sklearn_pmml/convert/utils.py
# Compiled at: 2015-10-07 12:28:00
...
class DerivedFeatureTransformations(object):
    ...
    def field_in_list(field, values):
        ...
        mv.append(it)
        return {DerivedFeatureTransformations.TRANSFORMATION: mv, 
    ...
    def map_values(field, value_map, default_value):
        ...
        mv.append(it)
        return {DerivedFeatureTransformations.TRANSFORMATION: mv, 
           DerivedFeatureTransformations.FUNCTION: lambda df: np.vectorize(partial(value_map.get, default_value))(df[field])}
    ...
    def arithmetics(tree):
        ...
        def greedy_evaluation(node):
            if isinstance(node, str):
                return (
                 lambda df: df[node], pmml.FieldRef(field=node))
            else:
                if isinstance(node, (tuple, list)):
                    args = map(greedy_evaluation, node[1:])
                    functions = {'*': lambda df: np.multiply(*[ _[0](df) for _ in args ]), 
                    ...
                    expr = {'*': partial(basic_function, '*'), 
                       '-': partial(basic_function, '-'), 
                       '+': partial(basic_function, '+'), 
                       '/': partial(basic_function, '/'), 
                       '%': mod_function}.get(node[0])([ a[1] for a in args ])
                    func = functions[node[0]]
                    return (
                     func, expr)
                return (
                 lambda df: node, pmml.Constant(node, dataType='double'))

        function, transformation = greedy_evaluation(tree)
        return {DerivedFeatureTransformations.TRANSFORMATION: transformation, 
...
def assert_equal(feature, expected, actual):
    ...
    if feature.data_type == FeatureType.STRING:
        assert all(actual == expected), ('Some passed values of "{}" don\'t match the evaluated results').format(feature.full_name)
    else:
        np.testing.assert_almost_equal(actual, expected, err_msg=('Some passed values of "{}" don\'t match the evaluated results').format(feature.full_name))