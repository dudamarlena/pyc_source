...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/sklearn_pmml/convert/model.py
# Compiled at: 2015-10-12 11:51:41
...
class EstimatorConverter(object):
    ...
    def __init__(self, estimator, context, mode):
        ...
        assert not any(isinstance(_, DerivedFeature) for _ in context.schemas[Schema.INPUT]), 'Input schema represents the input fields only'
        assert all(isinstance(_, DerivedFeature) for _ in context.schemas[Schema.DERIVED]), 'Derived schema represents the set of automatically generated fields'
        assert not any(isinstance(_, DerivedFeature) for _ in context.schemas[Schema.OUTPUT]), 'Only regular features allowed in output schema; use Output transformation if you want to transform values'
        categories = []
        for feature in context.schemas[Schema.OUTPUT]:
            if isinstance(feature, CategoricalFeature):
                for value in feature.value_list:
                    categories.append(RealNumericFeature(name=value, namespace=feature.name))
    ...
    def data_dictionary(self):
        ...
        for schema, fields in sorted(self.context.schemas.items(), key=lambda x: x[0].name):
            ...
            if schema.eligible_for_data_dictionary:
                for f in fields:
                    data_field = pmml.DataField(dataType=f.data_type.value, name=schema.extract_feature_name(f), optype=f.optype.value)
    ...
    def output(self):
        ...
        for feature in self.context.schemas[Schema.OUTPUT]:
            output_field = pmml.OutputField(name=Schema.OUTPUT.extract_feature_name(feature), feature='predictedValue')
    ...
    def transformation_dictionary(self):
        ...
        for f in self.context.schemas[Schema.DERIVED]:
            ...
            df = pmml.DerivedField(name=ef.full_name, optype=ef.optype.value, dataType=ef.data_type.value)
    ...
    def model_verification(self, verification_data):
        ...
        for key in self.context.schemas[Schema.MODEL]:
            assert Schema.MODEL.extract_feature_name(key) in verification_model_input.columns, ('Unknown feature "{}" in the MODEL schema').format(key.full_name)
        ...
        for s in [Schema.OUTPUT, Schema.INTERNAL]:
            if s in self.SCHEMAS_IN_MINING_MODEL:
                for f in self.context.schemas.get(s, []):
                    ms.append(pmml.MiningField(name=s.extract_feature_name(f), usageType='predicted'))
