...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.8/site-packages/salling_group_holidays/api.py
# Compiled at: 2020-04-21 07:05:44
...
class v1:
    ...
    def _make_request(self, params, path='/'):
        ...
        response = requests.get(('{}{}'.format(self._url, path)), headers=headers,
          params=params)
        if response.status_code != 200:
            if 'error' in response.json():
                raise SallingGroupHolidaysException(response.json()['error'])
            else:
                if 'developerMessage' in response.json():
    ...
    def holidays(self, start_date, end_date):
        ...
        response = self._make_request({'startDate':start_date.isoformat(),  'endDate':end_date.isoformat()})
        result = {{'name':val['name'], 
         'holiday':val['nationalHoliday']}:datetime.datetime.strptime(val['date'], '%Y-%m-%d').date() for val in response.json()}
        return result