...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/updatorr/gtkui.py
# Compiled at: 2012-02-23 07:23:23
import os, logging, locale, pkg_resources
from datetime import datetime
import gtk, deluge.component as component, deluge.common
...
class GtkUI(GtkPluginBase):
    ...
    def update_status_bar(self):
        ...
        if self.status_bar_item is None:
            self.status_bar_item = component.get('StatusBar').add_item(stock=gtk.STOCK_REFRESH, text='', tooltip=_('Updatorr autoupdate status information'))
    ...
    def get_status(self, status_data):
        ...
        if status_data[0] == 0:
            ...
            next_update = datetime.fromtimestamp(int(status_data[0]) + int(status_data[1] * 3600)).strftime(self.DATE_FORMAT)
    ...
    def init_tv_trackers(self):
        ...
        if self.trackers_data_model is None:
            ...
            self.glade.signal_autoconnect({'on_btn_test_clicked': self.on_btn_test_clicked, 
    ...
    def on_btn_test_clicked(self, widget):
        ...
        data = [
         self.tv_current_domain, self.ed_login.get_text(), self.ed_password.get_text()]
    ...
    CONTEXT_UPDATE_CHOICES = {True: _('Enable autoupdates'), 
       False: _('Disable autoupdates')}
    CONTEXT_RUN_LABELS = {True: _('Check updates for selected'), 
       False: _('Torrent updates in progress...')}
    CONTEXT_RUN_ALL_LABELS = {True: _('Check updates for scheduled'), 
    ...
    def on_apply_prefs(self):
        ...
        config = {'walk_period': self.glade.get_widget('walk_period').get_text(), 
