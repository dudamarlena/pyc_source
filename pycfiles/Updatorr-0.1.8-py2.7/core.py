...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/updatorr/core.py
# Compiled at: 2013-04-01 09:07:21
import os, logging, threading, base64, pkgutil, time, re, deluge.configmanager, deluge.component as component
...
DEFAULT_PREFS = {'last_walk': 0, 
   'walk_period': 24, 
   'trackers_settings': {}, 'torrents_to_update': []}
RE_LINK = re.compile('(?P<url>https?://[^\\s]+)')
...
class Core(CorePluginBase):
    ...
    def update_trackers_settings(self):
        ...
        for domain, handler in get_registered_handlers().items():
            if domain not in self.trackers_settings:
                domain_dict = {'login_required': handler.login_required, 'login': '', 
                   'password': '', 
                   'cookies': None}
    ...
    def get_status(self):
        ...
        return (
         self.last_walk, self.walk_period, self.walking)
