...
# Compiled at: 2017-12-04 15:26:00
# Size of source mod 2**32: 128428 bytes
from __future__ import unicode_literals
import json, redis, pytz
...
class OrgTest(DashTest):
    ...
    def test_build_boundaries(self):
        boundaries = dict()
        boundaries['geojson:%d' % self.org.pk] = dict(type='FeatureCollection',
          features=[
         dict(type='Feature', geometry=dict(type='MultiPolygon', coordinates=[[1, 2], [3, 4]]),
           properties=dict(name='Burundi', id='R195269', level=1))])
        boundaries['geojson:%d:%s' % (self.org.pk, 'R195269')] = dict(type='FeatureCollection',
          features=[
         dict(type='Feature', geometry=dict(type='MultiPolygon', coordinates=[[5, 6], [7, 8]]),
        ...
        with patch('dash.orgs.models.datetime_to_ms') as (mock_datetime_to_ms):
            ...
            with patch('django.core.cache.cache.set') as (cache_set_mock):
                ...
                with patch('dash.orgs.models.TembaClient1.get_boundaries') as (mock_client):
                    ...
                    level_1_boundary = Boundary.create(boundary='R195269', name='Burundi', level=1, parent='', geometry=geometry1)
                    level_2_boundary = Boundary.create(boundary='R195270', name='Bujumbura', level=2, parent='R195269', geometry=geometry2)
                    mock_client.return_value = [
                     level_1_boundary, level_2_boundary]
                    self.assertEqual(self.org.build_boundaries(), boundaries)
                    cache_set_mock.assert_called_with('org:%d:boundaries' % self.org.pk, dict(time=500, results=boundaries), 2592000)
        ...
        with patch('dash.orgs.models.Org.get_boundaries') as (mock_get_boundaries):
            ...
            self.assertEqual(self.org.get_country_geojson(), dict(type='FeatureCollection', features=[
             dict(type='Feature', geometry=dict(type='MultiPolygon', coordinates=[[1, 2], [3, 4]]),
               properties=dict(name='Burundi', id='R195269', level=1))]))
            self.assertEqual(self.org.get_state_geojson('R195269'), dict(type='FeatureCollection', features=[
             dict(type='Feature', geometry=dict(type='MultiPolygon', coordinates=[[5, 6], [7, 8]]),
               properties=dict(name='Bujumbura', id='R195270', level=2))]))
    ...
    def test_org_edit(self):
        with patch('dash.orgs.models.Org.get_country_geojson') as (mock):
            mock.return_value = dict(type='FeatureCollection', features=[
             dict(type='Feature', properties=dict(id='R3713501', level=1,
               name='Abia'),
            ...
            self.assertTrue('featured_state' not in response.context['form'].fields)
            ...
            post_data = dict(name='Rwanda', shortcode='224433',
            ...
            self.assertFalse(org.get_config('featured_state'))
            self.assertEquals(response.request['PATH_INFO'], reverse('orgs.org_home'))
            response = self.client.get(edit_url, SERVER_NAME='uganda.ureport.io')
            self.assertEquals(response.status_code, 200)
...
class StoryTest(DashTest):
    ...
    def test_story_model(self):
        self.story = Story.objects.create(title='Story 1', content=('content ' * 20),
          org=(self.uganda),
          created_by=(self.admin),
          modified_by=(self.admin))
