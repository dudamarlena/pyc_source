...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/enlp/visualisation/freq_distribution.py
...
def wordcloud_plot(token_list, colormap='rainbow', ax=None):
    ...
    if ax is None:
        ...
        fig = None
    wordcloud = WordCloud(width=1500, height=1200, margin=0, colormap=colormap,
...
def dist_plot(word_counts, log=False, shade_singles=True, shade_top25=True, ax=None):
    ...
    ax.grid(True)
    if shade_singles:
        ax.axvspan((min(np.argwhere(counts == 1.0))), (max(np.argwhere(counts == 1.0))), alpha=0.5, color='lightblue',
        ...
        if shade_top25:
            ...
            top25 = num_words / 4.0
            ax.axvspan((min(np.argwhere(words_cum_sum <= top25))), (max(np.argwhere(words_cum_sum <= top25))), alpha=0.5,
              color='red',
              label='25% of occurances')
            ax.legend()
        if log:
