...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/ptysh_base.py
...
class BasicCommand(Singleton):
    ...
    def __init__(self):
        self._basic_command = [
         [
          'enable', 'enable mode', self.cmd_enable, False, True],
         [
    ...
    def run_command(self, in_cmd):
        if in_cmd[COMMAND_LIST_CMD_IDX].strip() == 'show':
            ...
            if in_cmd[COMMAND_LIST_CMD_IDX].strip() == 'configure':
                if len(in_cmd) == 1:
                    return False
    ...
    def run_command(self, in_cmd):
        ...
        for cmd in cmd_list:
            ...
            if len(cmd) == 2:
                ...
                Status().set_current_node(cmd[0])
                self._subnode_modules_command = cmd[1]
            else:
                cmd_function = cmd[COMMAND_LIST_FUNC_IDX]
                try:
                    cmd_function(*in_cmd[1:])
                except:
