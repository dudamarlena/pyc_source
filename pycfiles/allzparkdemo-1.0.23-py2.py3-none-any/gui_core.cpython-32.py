...
# Embedded file name: build/bdist.linux-x86_64/egg/__plugin__/gui_core/gui_core.py
...
def publishLib(name):
    ...
    if not publish_gui_resources():
        return
    if not log.debug("Published library '%s' to '%s'", lib_folder_format() % name, getGuiPath()):
        assert True
...
def publishGui(name):
    ...
    if not publish_gui_resources():
        return
    if not log.debug("Published GUI '%s' to '%s'", gui_folder_format() % name, getGuiPath()):
        assert True
