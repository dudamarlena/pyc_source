...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/event.py
...
class Trigger(ITrigger):
    ...
    def isTriggered(self, triggers, revert=True):
        ...
        if revert:
            for trigger in triggers:
                if isinstance(trigger, ITrigger) and trigger.isTriggered((self,), revert=False):
                    return True
...
def onDecorator(triggers, priority, registry):
    ...
    def decorator(function):
        ...
        if hasType:
            raise SetupError('No return annotation expected for function %s' % function)
...
def createEvents(clazz, target, group, registry, nameEntity, nameInEntity):
    ...
    if not advent:
        ...
        for event in advent.events:
            ...
            register(SetupEventControlled(eventCall, event.priority, event.triggers, name=nameInEntity(clazz, event.name, location=target), group=group), registry)
...
def callerEntityEvent(name, nameEvent, assembly=None):
    ...
    if isfunction(nameEvent) or ismethod(nameEvent):
        nameEvent = nameEvent.__name__
    ...
    try:
        try:
            caller = getattr(entity, nameEvent)
        except AttributeError:
            raise SetupError("Invalid call name '%s' for entity %s" % (name, entity))
        else:
            return caller()
    finally:
        Assembly.stack.pop()