...
# Embedded file name: build/bdist.linux-x86_64/egg/package_extender.py
...
class PackageExtender:
    ...
    def __init__(self):
        ...
        """
        self._PackageExtender__loading = set()
        self._PackageExtender__unittest = False
        self._PackageExtender__unextended = set()
    ...
    def setForUnitTest(self, unittest):
        ...
        self._PackageExtender__unittest = unittest
    ...
    def addFreezedPackage(self, name):
        ...
        self._PackageExtender__unextended.add(name)

    def find_module(self, name, paths=None):
        """
        @see: http://www.python.org/dev/peps/pep-0302/
        """
        if is_builtin(name) == 0 and name not in self._PackageExtender__loading and name not in sys.modules:
            self._PackageExtender__loading.add(name)
            loader = find_loader(name)
            self._PackageExtender__loading.remove(name)
            if self._isUnextended(name):
                ...
                elif self._PackageExtender__unittest:
    ...
    def _isUnextended(self, name):
        ...
        """
        for unname in self._PackageExtender__unextended:
...
class PackageLoader:
    ...
    def __init__(self, loader):
        ...
        self._PackageLoader__loader = loader

    def load_module(self, name):
        """
        @see: http://www.python.org/dev/peps/pep-0302/
        """
        return _extendPackagePaths(self._PackageLoader__loader.load_module(name))

    def __getattr__(self, name):
        return getattr(self._PackageLoader__loader, name)
...
class ModuleLoader:
    ...
    def __init__(self, loader):
        ...
        """
        assert loader, 'A loader is required'
        self._ModuleLoader__loader = loader
    ...
    def load_module(self, name):
        ...
        """
        try:
            module = self._ModuleLoader__loader.load_module(name)
        except:
            print('-' * 150, file=sys.stderr)
            print('Problem occurred while loading module %r from loader %s' % (name, self._ModuleLoader__loader), file=sys.stderr)
            traceback.print_exc()
            print('-' * 150, file=sys.stderr)
            raise
    ...
    def __getattr__(self, name):
        return getattr(self._ModuleLoader__loader, name)
