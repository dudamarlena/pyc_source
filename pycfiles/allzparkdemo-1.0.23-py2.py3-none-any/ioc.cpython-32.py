...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/ioc.py
...
from ._impl._setup import SetupEntity, SetupSource, SetupConfig, SetupFunction, SetupEvent, SetupEventReplace, SetupSourceReplace, SetupStart, SetupEventCancel, register, SetupConfigReplace, setupsOf
...
def doc(setup, doc):
    ...
    if isinstance(setup, SetupConfigReplace):
        setup = setup.target
    if setup.documentation is not None:
        setup.documentation += '\n%s' % cleandoc(doc)
    return
...
def before(*setups, auto=True):
    ...
    """
    for setup in setups:
        if not isinstance(setup, SetupFunction):
            raise AssertionError('Invalid setup function %s' % setup)
    ...
    def decorator(function):
        hasType, type = process(function)
        if hasType:
            raise SetupError('No return type expected for function %s' % function)
        return update_wrapper(register(SetupEvent(function, tuple(setup.name for setup in setups), SetupEvent.BEFORE, auto), callerLocals()), function)
...
def after(*setups, auto=True):
    ...
    def decorator(function):
        ...
        if hasType:
            raise SetupError('No return type expected for function %s' % function)
        return update_wrapper(register(SetupEvent(function, tuple(setup.name for setup in setups), SetupEvent.AFTER, auto), callerLocals()), function)
...
def start(*args, priority=PRIORITY_NORMAL):
    ...
    """
    if not args:
        return partial(start, priority=priority)
    assert len(args) == 1, 'Expected only one argument that is the decorator function, got %s arguments' % len(args)
...
def entityOf(identifier, module=None):
    ...
    if isinstance(identifier, str):
        ...
        if not identifier.startswith('%s.' % group):
            identifier = '%s.%s' % (group, identifier)
        for setup in setups:
            assert isinstance(setup, SetupSource)
            if isinstance(identifier, str):
                if setup.name == identifier:
                    found.append(setup)
                else:
                    continue
        ...
        for setup in setups:
            assert isinstance(setup, SetupSource)
            if setup.isOf(identifier):
                found.append(setup)
                continue

    if not found:
        raise SetupError("No setup entity as found for '%s'" % identifier)
    if len(found) > 1:
        raise SetupError('To many setup entities found:\n%s\nfor: %s' % (
...
def process(function):
    ...
    if not isfunction(function):
        raise SetupError('Expected a function as the argument, got %s' % function)
    if function.__name__ == '<lambda>':
        raise SetupError('Lambda functions cannot be used %s' % function)
    ...
    return (
     'return' in fnArgs.annotations, fnArgs.annotations.get('return'))
...
def processWithOneArg(function):
    ...
    return (
     len(fnArgs.args) > 0, 'return' in fnArgs.annotations, fnArgs.annotations.get('return'))