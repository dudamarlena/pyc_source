...
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/bind.py
                ...
                for handler in processHandlers(handlers):
                    if methods:
                        for method in methods:
                            registerProxyHandler(handler, getattr(proxy, method))
...
class BinderSetup:
    ...
    def __call__(self, proxy):
        ...
        if not self._binders:
            ...
            if isinstance(binders, (list, tuple)):
                self._binders = binders
            else:
                if callable(binders):
                    self._binders = (binders,)
                else:
                    raise SetupError('Invalid binders provided from %s' % self._source)
            for binder in self._binders:
                if not callable(binder):
                    raise AssertionError('Invalid binder call %s' % binder)
...
def processBinders(binders):
    ...
    processed = []
    if isinstance(binders, (list, tuple)):
        processed.extend(binders)
    elif binders:
        processed.append(binders)
    for k in range(len(processed)):
        if isinstance(processed[k], SetupSource):
            processed[k] = BinderSetup(processed[k])
            continue
...
def processHandlers(handlers):
    ...
    if isinstance(handlers, Iterable):
        for handler in handlers:
            if callable(handler):
                processed.extend(processHandlers(handler()))
            else:
