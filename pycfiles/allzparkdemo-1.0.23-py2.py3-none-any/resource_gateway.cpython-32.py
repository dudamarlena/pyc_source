...
# Embedded file name: build/bdist.linux-x86_64/egg/acl/core/impl/processor/resource_gateway.py
...
from ally.http.spec.server import HTTP_GET, HTTP_DELETE, HTTP_POST, HTTP_PUT, IEncoderPath
from ally.support.core.util_resources import findNodesFor, propertyTypesOf, ReplacerWithMarkers, pathForNode
...
class GatewaysFromPermissions(HandlerProcessorProceed):
    ...
    def processFilters(self, types, filters, provider, encoder):
        ...
        for rfilter in filters:
            ...
            if occurence > 1:
                log.error("Ambiguous resource filter type '%s', has to many occurrences in path types: %s", rfilter.resource, ','.join(str(typ) for typ in types) if types else 'no types')
                continue
            elif occurence == 0:
                if not log.debug("Invalid resource filter type '%s', is not present in the path types: %s", rfilter.resource, ','.join(str(typ) for typ in types) if types else 'no types'):
                    if not True:
