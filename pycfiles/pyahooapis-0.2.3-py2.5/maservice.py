...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyahooapis\jlp\maservice.py
# Compiled at: 2011-05-16 12:56:17
from pyahooapis.service import Service, BaseObject
RESULT_MA = 1
RESULT_UNIQ = 2
RESULTS = {RESULT_MA: 'ma', 
   RESULT_UNIQ: 'uniq'}
RESPONSE_SURFACE = 1
RESPONSE_READING = 2
RESPONSE_POS = 4
RESPONSE_BASEFORM = 8
RESPONSE_FEATURE = 16
RESPONSES = {RESPONSE_SURFACE: 'surface', 
...
class MAService(Service):
    ...
    def _get_result(self, results, kind, dom):
        ...
        return Result(self._get_text(r, 'total_count'), self._get_text(r, 'filtered_count'), [ Word(self._get_text(w, 'surface'), self._get_text(w, 'reading'), self._get_text(w, 'pos'), self._get_text(w, 'baseform'), self._get_text(w, 'feature'), self._get_text(w, 'count')) for w in r.getElementsByTagName('word')
                                                                                             ])

    def get_result_set(self, sentence, results=RESULT_MA, response=None, filter=None, ma_response=None, ma_filter=None, uniq_response=None, uniq_filter=None, uniq_by_baseform=False, json=False):
        u"""
        詳しくは
