...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/xwot1/model2Python.py
# Compiled at: 2015-10-27 03:39:37
import os, shutil, string, logging, logging.config, sys, xml.dom.minidom, argparse, traceback, re
...
class Model2Python():
    ...
    def createPythonService(self, source_node, path):
        ...
        shutil.copytree(resource_filename(Requirement.parse('XWoT_Model_Translator'), 'xwot:/REST-Server-Skeleton'), project_name)
    ...
    def createNodeManagerService(self, source_node, path):
        ...
        self.__log.info('Creating Server: ' + project_name)
        shutil.copytree(resource_filename(Requirement.parse('XWoT_Model_Translator'), 'xwot:/NM_REST-Server-Skeleton'), project_name)
        self.addResourceDefinitions(source_node, project_name, '')
        filein = open(project_name + '/rest-server.py')
        src = string.Template(filein.read())
        r = {'imports': '', 'pathdef': ''}
    ...
    def addResourceDefinitions(self, node, project_path, parent_filename):
        ...
        resource = {'name': node.getAttribute('name').replace(' ', ''), 'type': node.getAttribute('xsi:type'), 'uri': parent_filename + '/' + node.getAttribute('uri')}
        ...
        if len(self.getResourceNodes(node)) > 0:
            childSubstitute = 'else:\n' + '            return ' + node.getAttribute('name').replace(' ', '') + 'API' + "(self.datagen, name, self.__port, '')" + '\n'
        else:
            childSubstitute = 'return ' + node.getAttribute('name').replace(' ', '') + 'API' + "(self.datagen, name, self.__port, '')" + '\n'
    ...
    def createResourceFile(self, node, project_path, parent_filename):
        ...
        if parent_filename == '':
            pathdef = classnameClass + '=' + classname + "(data, '', self.__port, '')" + '\n        ' + "root.putChild('" + node.getAttribute('uri').replace('{', '').replace('}', '') + "',  " + classnameClass + ')' + '\n        $pathdef'
    ...
    def createPublisherClient(self, project_path, classname, parent_uri):
        ...
        resource = {'name': classname, 'type': 'publisher', 'uri': parent_uri + '/pub/{id}'}
    ...
    def addHTMLInfoTableBody(self, htmlfile, resource):
        tablebody = '    <div id="' + resource['name'] + '" class="method">' + '\n' + '        <tr>' + '\n' + '            <td>' + '\n' + '                <div id="' + resource['name'] + 'Address" class="address"><a href="' + resource['uri'] + '">http://localhost:9000' + resource['uri'] + '</a></div></td>' + '\n' + '            <td>' + '\n' + '                <div id="' + resource['name'] + 'Label">' + resource['name'] + ' - ' + resource['type'] + '</div>' + '\n' + '            </td>' + '\n' + '            <div id="' + resource['name'] + 'Operation" class="operation">' + '\n' + '                <td>' + '\n' + '                    <div id="' + resource['name'] + 'Input" class="input">-</div>' + '\n' + '                    /' + '\n' + '                    <div id="' + resource['name'] + 'Output" class="output">JSON-XML-HTML</div>' + '\n' + '                </td>' + '\n' + '                <td>' + '\n' + '                    <div class="label">' + resource['method'] + '</div>' + '\n' + '                </td>' + '\n' + '            </div>' + '\n' + '            <td></td>' + '\n' + '        </tr>' + '\n' + '    </div>' + '\n' + '$tablebody'
