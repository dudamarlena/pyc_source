...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\srllib\signal.py
...
class _MethodProxy(object):
    ...
    def __call__(self, *args, **kwds):
        ...
        try:
            mthd(instance, *args, **kwds)
        except TypeError as err:
            if not sys.exc_info()[(-1)].tb_next:
                raise CallFailure("Calling slot %s.%s resulted in TypeError, check your arguments; the original exception was: `%s'" % (
...
class Signal(object):
    ...
    def __connect(self, slot, defArgs, defKwds):
        e = (
         slot, defArgs, defKwds)
        if self.__find_slot(slot) is None:
            self._slots.append(e)
        if slot.object not in self.__obj2slots:
