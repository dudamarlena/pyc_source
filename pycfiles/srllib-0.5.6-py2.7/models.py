...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\srllib\qtgui\models.py
...
class _SetDataCommand(QtGui.QUndoCommand):
    ...
    def __init__(self, undo_model, model, index, role2value, parent=None, clear=False):
        assert not isinstance(model, int)
        QtGui.QUndoCommand.__init__(self, 'set item data')
        self.__undo_model, self.__model, self.__row, self.__col, self.__role2value, self.__parent, self.__clear = (
         undo_model,
...
class UndoItemModel(QtGui.QSortFilterProxyModel):
    ...
    def __init__(self, undo_stack=None, hor_headers=None, ver_headers=None, parent=None):
        """ Constructor.
    ...
    def append_row(self, values, undo_text=None):
        ...
        self.undo_stack.push(_AppendRowCommand(self.__model, items, text=undo_text))

    def setData(self, index, value, role=Qt.EditRole):
        role2value = {role: value}
        self.undo_stack.push(_SetDataCommand(self, self.__model, self.__model.index(index.row(), index.column()), role2value))
        return True

    def setItemData(self, index, roles, clear=False):
        self.undo_stack.push(_SetDataCommand(self, self.__model, self.__model.index(index.row(), index.column()), roles, clear=clear))
        return True
    ...
    def removeRows(self, row, count, parent=QtCore.QModelIndex(), undo_text=None):
