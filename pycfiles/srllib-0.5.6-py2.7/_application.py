...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\srllib\qtgui\_application.py
# Compiled at: 2012-05-11 12:09:02
""" Functionality on top of QApplication.
"""
import sys, traceback, Queue
from PyQt4.QtGui import *
import warnings, srllib.threading
...
class Application(QApplication):
    ...
    def __slot_timed_out(self):
        ...
        while True:
            try:
                mthd, args, kwds, optimize = self.__deferred_queue.get_nowait()
            except Queue.Empty:
                break
    ...
    def __thrdexc_hook(self, exc):
        self.__exchook(exc.exc_type, exc.exc_value, exc.exc_traceback, in_thread=exc.name)
    ...
    def __exchook(self, exc, value, tb, in_thread=None):
        ...
        if exc is not KeyboardInterrupt:
            ...
            message_critical('Fatal Error', 'An unexpected exception was encountered%s, the application will have to be shut down.' % (
             thrdSpecific,), detailed_text=msg, informative_text='The detailed text provides full technical information of how the error happened, so developers may resolve the problem. This information should also be visible in the application log.')
