...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\srllib\util.py
# Compiled at: 2012-06-25 07:50:43
...
"""
import stat, shutil, os.path, imp, fnmatch, sys, errno, codecs
try:
    import hashlib
except ImportError:
    import sha
...
def replace_root(path, new_root, orig_root=None):
    ...
    if os.path.sep not in path:
        ...
        if orig_root is None:
            ...
            raise (path.startswith(orig_root) or ValueError)("Path '%s' doesn't start with specified original root '%s'" % (
             path, orig_root))
...
def remove_dir(path, ignore_errors=False, force=False, recurse=True):
    ...
    """
    logger.debug("Removing directory '%s' (ignore_errors: %s, force: %s, recurse: %s)" % (
     path, ignore_errors, force, recurse))
    ...
    if recurse:
        for dpath, dnames, fnames in walkdir(path, topdown=False, errorfunc=handle_err):
...
def move_file(src, dest, force=False):
    ...
    """
    if get_os_name() == Os_Windows and os.path.isfile(dest):
        if force:
...
@_raise_permissions
def walkdir(path, errorfunc=None, topdown=True, ignore=None, sort=False):
    ...
    for d in dnames:
        if os.path.islink(os.path.join(path, d)):
            continue
        for x in walkdir(os.path.join(path, d), errorfunc=errorfunc, topdown=topdown, ignore=ignore, sort=sort):
            yield x
...
@_raise_permissions
def copy_dir(sourcedir, destdir, callback=no_op, ignore=[], mode=CopyDir_New, copyfile=None, fs_mode=None):
...
def create_tempfile(suffix='', prefix='tmp', close=True, content=None, encoding=None, dir=None):
...
def _sig(st):
    return (
     stat.S_IFMT(st.st_mode), st.st_size, stat.S_IMODE(st.st_mode),
     st.st_uid, st.st_gid)
...
def compare_dirs(dir0, dir1, shallow=True, ignore=[], filecheck_func=None):
    ...
    def checkfiles(path0, path1):
        chksum0, chksum1 = get_checksum(path0, format=Checksum_Binary), get_checksum(path1, format=Checksum_Binary)
        r = chksum0 == chksum1
        if not r:
    ...
    if len(os.listdir(dir0)) == 0:
        ...
        for dpath, dnames, fnames in walkdir(dir0, handle_err):
            ...
            contents1 = [ e for e in os.listdir(os.path.join(dir1, reldir)) if e not in ignore
                        ]
...
def resolve_path(executable):
    ...
    if get_os_name() == Os_Windows:
        try:
            ...
            try:
                exepath = win32api.FindExecutable(executable)[1]
