...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/userschema/schema.py
# Compiled at: 2007-01-31 11:58:07
""" userschema.schema
...
_TYPES = {'Bool': (Bool, _from_bool), 'Float': (Float, float), 'Int': (Int, int), 'Text': (Text, unicode), 'TextLine': (TextLine, unicode)}
...
def fromCSV(csv_text, name, module_name=None, bases=None, dialect='excel'):
    ...
    for cols in csv.reader(csv_text, dialect):
        ...
        rest = cols[3:]
        if type_name == 'Choice':
            attrs[field_name] = Choice(__name__=field_name, default=unicode(default), values=rest)
        elif type_name == 'ChoiceSet':
            field = Choice(__name__=field_name, values=rest)
            default = sets.Set(unicode(default).split(','))
            attrs[field_name] = ChoiceSet(value_type=field, default=default)
        else:
            (field_type, converter) = _TYPES[type_name]
            attrs[field_name] = field_type(__name__=field_name, default=converter(default))

    return InterfaceClass(name=name, bases=tuple(bases), attrs=attrs, __doc__='Generated from CSV', __module__=module_name)


_INPUT_TYPES = {'text': TextLine, 'checkbox': Bool, 'radio': Choice}
_MARSHALLED_TYPES = {'int': (Int, int), 'float': (Float, float)}

def fromHTMLForm(html_text, name=None, form_name=None, module_name=None, bases=None, encoding='UTF-8'):
    ...
    for input in form.findall('.//input'):
        ...
        if widget_type == 'text':
            ...
            info = {'value': unicode(input.attrib.get('value')), 'is_default': checked.lower() in ('checked', 'true'), 'title': title}
            checkboxes.setdefault(field_name, []).append(info)
            continue
        elif widget_type == 'radio':
            checked = input.attrib.get('checked', '')
            info = {'value': unicode(input.attrib['value']), 'is_default': checked.lower() in ('checked', 'true'), 'title': title}
        ...
        attrs[field_name] = field_type(__name__=field_name, title=title, default=default)
    ...
    for (field_name, infos) in radios.items():
        ...
        attrs[field_name] = Choice(__name__=field_name, values=values, default=default, title=info['title'])

    for (field_name, infos) in checkboxes.items():
        if len(infos) == 1:
            attrs[field_name] = Bool(__name__=field_name, default=infos[0]['is_default'], title=info['title'])
            continue
        default = []
        values = []
        for info in infos:
        ...
        field = Choice(__name__=field_name, values=values)
        attrs[field_name] = ChoiceSet(value_type=field, default=default, title=info['title'])
    ...
    for select in form.findall('.//select'):
        ...
        for option in select.findall('.//option'):
            ...
            info = {'token': unicode(token), 'value': unicode(value), 'is_default': selected.lower() in ('selected', 'true')}
            options.append(info)
        ...
        if multiple:
            default = sets.Set(default)
            field = Choice(__name__=field_name, vocabulary=vocabulary)
            attrs[field_name] = ChoiceSet(value_type=field, default=default, title=title)
        else:
            attrs[field_name] = Choice(__name__=field_name, vocabulary=vocabulary, default=default, title=title)
    ...
    for textarea in form.findall('.//textarea'):
        ...
        attrs[field_name] = Text(__name__=field_name, default=default, title=title)

    return InterfaceClass(name=name, bases=tuple(bases), attrs=attrs, __doc__='Generated from HTML Form', __module__=module_name)