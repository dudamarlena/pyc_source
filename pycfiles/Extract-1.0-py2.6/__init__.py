...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/extract/__init__.py
# Compiled at: 2011-10-03 23:02:44
import bisect, operator, numpy as np, numpy.ma as ma
...
WEIGHT = 1
__all__ = [
 'extract_point', 'MOM3', 'MOM4p1', 'PIRATA_Profile', 'ARGO_Profile',
...
def extract_point(model, ctd):
    ...
    for z in -ctd.depth[:-1]:
        ...
        for (i, j, k) in ((i, j, k) for i in (0, 1) for j in (0, 1)):
            distance = np.sqrt((x - lon[(i0 + i)]) ** 2 + (y - lat[(j0 + j)]) ** 2 + (z - depth[k]) ** 2)
        ...
        if w:
            v0 /= w
            v1 /= w
        v = v0 + (v1 - v0) * seconds(t - time[l0]) / seconds(time[(l0 + 1)] - time[l0])
        values.append(v)
    ...
    for (i, measure) in enumerate(data):
        ...
        for (modelfield, datafield) in fields:
            ...
            try:
                ...
                dv = ma.masked_equal(np.array(measure.data[:-1]), measure.field.missing_value)
...
class MOM4p1(Model):
    ...
    def get_data(self, l0, j0, i0):
        return ma.masked_equal(self.field.array[l0:l0 + 2, :, j0:j0 + 2, i0:i0 + 2], self.field.missing_value)
...
class MOM3(Model):
    ...
    def time(self):
        (TIME,) = self.field.dimensions[(-4)]
        return [ (v, self.data[TIME].units) for v in np.cumsum(self.data[TIME][:])
               ]
...
class ARGO_Profile(Measure):
    ...
    @property
    def depth(self):
        return -(np.array(list(self.depthdata.pressure)) / 10.0)
...
def select_profiles(indexurl, begin=None, end=None, urcrnrlat=None, urcrnrlon=None, llcrnrlat=None, llcrnrlon=None):
    ...
    if begin:
        ...
        conds.append(c)
    if urcrnrlat:
        c = (index.sequence.latitude > llcrnrlat) & (index.sequence.latitude < urcrnrlat) & (index.sequence.longitude > llcrnrlon) & (index.sequence.longitude < urcrnrlon)
        conds.append(c)
...
def build_exp(*args, **kwargs):
    ...
    exp['model_unit_conversion'] = kwargs.get('model_unit_conversion', lambda x: x)
    return exp
...
def load_data(model, cruise, exps, cache=None):
    if cache is None:
        cache = {}
    if not cache.keys():
        (lons, lats, values_model, values_ctd, values_depth, values_time) = get_data(model, cruise, exps)
        cache['lons'] = lons
        cache['lats'] = lats
