...
# Embedded file name: build/bdist.linux-x86_64/egg/pylinkirc/plugins/antispam.py
# Compiled at: 2020-04-11 03:31:40
# Size of source mod 2**32: 19682 bytes
...
_UNICODE_CHARMAP = {'A':'AΑАᎪᗅᴀ𝐀𝐴𝑨𝒜𝓐𝔄𝔸𝕬𝖠𝗔𝘈𝘼𝙰𝚨𝛢𝜜𝝖𝞐', 
        ...
        if irc.is_oper(target):
            ...
            if channel:
                ...
                if exempt_level not in EXEMPT_OPTIONS:
                    log.error('(%s) Antispam exempt %r is not a valid setting, falling back to defaults; accepted settings include: %s', irc.name, exempt_level, ', '.join(EXEMPT_OPTIONS))
                    exempt_level = DEFAULT_EXEMPT_OPTION
                if exempt_level == 'voice':
                    if c.is_voice_plus(target):
                        log.debug('(%s) antispam: refusing to punish voiced and above %s/%s', irc.name, target, target_nick)
                        return False
                if exempt_level == 'halfop':
                    if c.is_halfop_plus(target):
            ...
            bans = set()
            log.debug('(%s) antispam: got %r as punishment for %s/%s', irc.name, punishment, target, irc.get_friendly_name(target))
            ...
            def _kick():
                ...
                irc.call_hooks([my_uid, 'ANTISPAM_KICK',
                 {'channel':channel,  'text':reason,  'target':target,  'parse_as':'KICK'}])

            def _kill():
                if target not in irc.users:
                    log.debug('(%s) antispam: not killing %s/%s; they already left', irc.name, target, irc.get_friendly_name(target))
                    return
                userdata = irc.users[target]
                irc.kill(my_uid, target, reason)
                irc.call_hooks([my_uid, 'ANTISPAM_KILL',
                 {'target':target,  'text':reason,  'userdata':userdata, 
                  'parse_as':'KILL'}])
            ...
            for action in set(punishment.split('+')):
                if action not in PUNISH_OPTIONS:
                    log.error('(%s) Antispam punishment %r is not a valid setting; accepted settings include: %s OR any combination of these joined together with a "+".', irc.name, punishment, ', '.join(PUNISH_OPTIONS))
                    return
                if action == 'block':
                        ...
                        if action == 'kick':
                            if channel:
                                try:
                                    ...
                                    log.warning('(%s) antispam: Kicks are not supported on this network, skipping; target was %s/%s', irc.name, target_nick, channel)
                                else:
                                    successful_punishments += 1
                        if action == 'ban':
                            if channel:
                                try:
                                    ...
                                    log.warning('(%s) antispam: Bans are not supported on this network, skipping; target was %s/%s', irc.name, target_nick, channel)
                                else:
                                    successful_punishments += 1
                        if action == 'quiet':
                            pass
                    if channel:
                        try:
                            ...
                            log.warning('(%s) antispam: Quiet is not supported on this network, skipping; target was %s/%s', irc.name, target_nick, channel)
                        else:
                            successful_punishments += 1
            ...
            if bans:
                ...
                irc.call_hooks([my_uid, 'ANTISPAM_BAN',
                 {'target':channel, 
                  'modes':bans,  'parse_as':'MODE'}])
            if kill:
                try:
                    _kill()
                except NotImplementedError:
                    log.warning('(%s) antispam: Kills are not supported on this network, skipping; target was %s/%s', irc.name, target_nick, channel)
                else:
                    successful_punishments += 1
            if not successful_punishments:
                log.warning('(%s) antispam: Failed to punish %s with %r, target was %s', irc.name, target_nick, punishment, channel or 'a PM')
        return bool(successful_punishments)


MASSHIGHLIGHT_DEFAULTS = {'min_length':50, 
 'min_nicks':5, 
...
def handle_masshighlight(irc, source, command, args):
    ...
    channel = args['target']
    text = args['text']
    mhl_settings = irc.get_service_option('antispam', 'masshighlight', MASSHIGHLIGHT_DEFAULTS)
    ...
    if len(text) < mhl_settings.get('min_length', MASSHIGHLIGHT_DEFAULTS['min_length']):
        ...
        for word in words:
            ...
            if len(nicks_caught) >= min_nicks:
                ...
                log.info('(%s) antispam: punishing %s => %s for mass highlight spam', irc.name, irc.get_friendly_name(source), channel)
                punished = _punish(irc, source, channel, punishment, reason)
                break

        log.debug('(%s) antispam.masshighlight: got %s/%s nicks on message to %r', irc.name, len(nicks_caught), min_nicks, channel)
        return not punished


utils.add_hook(handle_masshighlight, 'PRIVMSG', priority=1000)
utils.add_hook(handle_masshighlight, 'NOTICE', priority=1000)
TEXTFILTER_DEFAULTS = {'reason':'Spam is prohibited', 
...
def handle_textfilter(irc, source, command, args):
    ...
    target = args['target']
    text = args['text']
    txf_settings = irc.get_service_option('antispam', 'textfilter', TEXTFILTER_DEFAULTS)
    ...
    if irc.is_channel(target):
        ...
        if watch_pms == 'services':
            irc.get_service_bot(target) or log.debug("(%s) antispam.textfilters: skipping processing; %r is not a service bot (watch_pms='services')", irc.name, target)
            return
        else:
            if watch_pms == 'all':
                ...
                return
            txf_globs = set(conf.conf.get('antispam', {}).get('textfilter_globs', [])) | set(irc.serverdata.get('antispam_textfilter_globs', []))
            ...
            for filterglob in txf_globs:
                if utils.match_text(filterglob, text):
                    log.info('(%s) antispam: punishing %s => %s for text filter %r', irc.name, irc.get_friendly_name(source), irc.get_friendly_name(target), filterglob)
...
PARTQUIT_DEFAULTS = {'watch_quits':True, 
 'watch_parts':True, 
...
def handle_partquit(irc, source, command, args):
    ...
    text = args.get('text')
    pq_settings = irc.get_service_option('antispam', 'partquit', PARTQUIT_DEFAULTS)
    if not text:
        return
    if command == 'QUIT':
        if not pq_settings.get('watch_quits', True):
            return
    if command == 'PART':
        if not pq_settings.get('watch_parts', True):
            return
    pq_globs = set(conf.conf.get('antispam', {}).get('partquit_globs', [])) | set(irc.serverdata.get('antispam_partquit_globs', []))
    if not pq_globs:
        return
    for filterglob in pq_globs:
        if utils.match_text(filterglob, text):
            if command == 'PART':
                ...
                log.info('(%s) antispam: filtered part message from %s on %s due to part/quit filter glob %s', irc.name, irc.get_hostmask(source), ','.join(args['channels']), filterglob)
            else:
                filtered_message = pq_settings.get('quit_filter_message', PARTQUIT_DEFAULTS['quit_filter_message'])
                log.info('(%s) antispam: filtered quit message from %s due to part/quit filter glob %s', irc.name, args['userdata'].nick, filterglob)
