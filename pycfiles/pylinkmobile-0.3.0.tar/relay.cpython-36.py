...
# Embedded file name: build/bdist.linux-x86_64/egg/pylinkirc/plugins/relay.py
# Compiled at: 2020-04-11 03:31:40
# Size of source mod 2**32: 136368 bytes
import base64, inspect, string, threading, time
...
def initialize_all(irc):
    ...
    t = threading.Thread(target=_initialize_all, daemon=True, name=('relay initialize_all thread from network %r' % irc.name))
    t.start()
...
def main(irc=None):
    ...
    if 'relay' in conf.conf:
        if conf.conf['relay'].get('allow_free_oper_links', True):
    ...
    if irc is not None:
        for netname, ircobj in world.networkobjects.items():
            ...
            if 'relay_no_ips' in ircobj.serverdata:
                log.warning('(%s) The "relay_no_ips" option is deprecated as of 2.0-beta1. Consider migrating to "ip_share_pools", which provides more fine-grained control over which networks see which networks\' IPs.', netname)

    if 'relay' in conf.conf:
        if 'show_ips' in conf.conf['relay']:
            log.warning('The "relay::show_ips" option is deprecated as of 2.0-beta1. Consider migrating to "ip_share_pools", which provides more fine-grained control over which networks see which networks\' IPs.')
...
def die(irc=None):
    ...
    datastore.die()
    try:
        world.services['pylink'].clear_persistent_channels(None, 'relay', part_reason=RELAY_UNLOADED_MSG)
    except KeyError:
...
def normalize_nick(irc, netname, nick, times_tagged=0, uid=''):
    ...
    if irc.has_cap('freeform-nicks'):
        ...
        if USE_UNIDECODE:
            if not is_unicode_capable:
                ...
                if decoded_nick:
                    ...
                    nick = base64.b64encode((nick.encode(irc.encoding, 'replace')), altchars='[]')
                    nick = nick.decode()
        nick = _replace_special(nick)
        netname = _replace_special(netname)
        separator = irc.serverdata.get('separator') or conf.conf.get('relay', {}).get('separator') or '/'
        if times_tagged == 0:
            ...
            protocol_allows_slashes = irc.has_cap('slash-in-nicks') or irc.serverdata.get('relay_force_slashes')
            ...
            nick = is_unicode_capable or _sanitize(nick)
        else:
            for char in '!+%@&~#$:\'"?*,.':
        ...
        while irc.nick_to_uid(nick) not in (None, uid):
            times_tagged += 1
            log.debug('(%s) relay.normalize_nick: nick %r is in use; incrementing times tagged to %s.', irc.name, nick, times_tagged)
            nick = normalize_nick(irc, netname, orig_nick, times_tagged=times_tagged, uid=uid)

        finalLength = len(nick)
        assert finalLength <= maxnicklen, 'Normalized nick %r went over max nick length (got: %s, allowed: %s!)' % (
         nick, finalLength, maxnicklen)
        return nick
...
def spawn_relay_server(irc, remoteirc):
    ...
    if irc.connected.is_set():
        try:
            ...
            sid = irc.spawn_server(('%s.%s' % (remoteirc.name, suffix)), desc=('PyLink Relay network - %s' % remoteirc.get_full_network_name()))
            if needs_delayed_eob:
                irc._endburst_delay = old_eob_delay
        except (RuntimeError, ValueError):
            log.exception('(%s) Failed to spawn server for %r (possible jupe?):', irc.name, remoteirc.name)
            irc.disconnect()
            return
        else:
            ...
            return sid
    log.debug('(%s) skipping spawn_relay_server(%s, %s); the local server (%s) is not ready yet', irc.name, irc.name, remoteirc.name, irc.name)
    log.debug('(%s) spawn_relay_server: current thread is %s', irc.name, threading.current_thread().name)
...
def get_relay_server_sid(irc, remoteirc, spawn_if_missing=True):
    ...
    """
    log.debug('(%s) Grabbing spawnlocks_servers[%s] from thread %r in function %r', irc.name, irc.name, threading.current_thread().name, inspect.currentframe().f_code.co_name)
...
def _has_common_pool(sourcenet, targetnet, namespace):
    ...
    if 'relay' not in conf.conf:
        ...
        for pool in conf.conf['relay'].get(namespace) or []:
            if sourcenet in pool:
                if targetnet in pool:
                    log.debug('relay._has_common_pool: found networks %r and %r in %s pool %r', sourcenet, targetnet, namespace, pool)
                    return True
...
def spawn_relay_user(irc, remoteirc, user, times_tagged=0, reuse_sid=None):
    ...
    if ('o', None) in userobj.modes:
        if hasattr(userobj, 'opertype'):
            log.debug('(%s) spawn_relay_user: setting OPERTYPE of client for %r to %s', irc.name, user, userobj.opertype)
        ...
        if reuse_sid:
            ...
            rsid or log.debug('(%s) spawn_relay_user: aborting user spawn for %s/%s @ %s (failed to retrieve a working SID).', irc.name, user, nick, remoteirc.name)
        return
    else:
        try:
            showRealIP = conf.conf['relay']['show_ips'] and not irc.serverdata.get('relay_no_ips') and not remoteirc.serverdata.get('relay_no_ips')
        except KeyError:
            showRealIP = False
        ...
        if showRealIP:
            ...
            realhost = None
            ip = '0.0.0.0'
        u = remoteirc.spawn_client(nick, ident=ident, host=host, realname=realname, modes=modes, opertype=opertype,
          server=rsid,
          ip=ip,
          realhost=realhost).uid
        try:
            remoteirc.users[u].remote = (
             irc.name, user)
...
def get_remote_user(irc, remoteirc, user, spawn_if_missing=True, times_tagged=0, reuse_sid=None):
    ...
    if irc.connected.is_set():
        ...
        if user in irc.users:
            ...
            if hide:
                log.debug('(%s) get_remote_user: ignoring user %s since they are marked invisible', irc.name, user)
                return
            log.debug('(%s) Grabbing spawnlocks[%s] from thread %r in function %r', irc.name, irc.name, threading.current_thread().name, inspect.currentframe().f_code.co_name)
            ...
            log.debug('(%s) skipping spawn_relay_user(%s, %s, %s, ...); the local server (%s) is not ready yet', irc.name, irc.name, remoteirc.name, user, irc.name)
            log.debug('(%s) get_remote_user: current thread is %s', irc.name, threading.current_thread().name)
...
def get_orig_user(irc, user, targetirc=None):
    ...
    log.debug('(%s) relay.get_orig_user: remoteuser set to %r (looking up %s/%s).', irc.name, remoteuser, user, irc.name)
    if remoteuser:
        sourceobj = world.networkobjects.get(remoteuser[0])
        if targetirc and sourceobj:
            if remoteuser[0] == targetirc.name:
                return remoteuser[1]
            res = get_remote_user(sourceobj, targetirc, (remoteuser[1]), spawn_if_missing=False)
            log.debug('(%s) relay.get_orig_user: targetirc found as %s, getting %r as remoteuser for %r (looking up %s/%s).', irc.name, targetirc.name, res, remoteuser[1], user, irc.name)
            return res
        else:
            return remoteuser
...
def get_relay(irc, channel):
    ...
    pair, if one exists."""
    chanpair = (
     irc.name, irc.to_lower(str(channel)))
    if chanpair in db:
        return chanpair
...
def _claim_should_bounce(irc, channel):
    ...
    with claim_bounce_cache_lock:
        if irc.name not in claim_bounce_cache:
            ...
            if not success:
                log.warning('(%s) relay: %s received more than %s claim bounces in %s seconds - your channel may be desynced!', irc.name, channel, limit, ttl)
            return success
...
def check_claim(irc, channel, sender, chanobj=None):
    ...
    log.debug('(%s) relay.check_claim: sender modes (%s/%s) are %s (mlist=%s)', irc.name, sender, channel, sender_modes, mlist)
    success = not relay or irc.name == relay[0] or not db[relay]['claim'] or irc.name in db[relay]['claim'] or any([mode in sender_modes for mode in {'a', 'o', 'q', 'h', 'y'}]) and not irc.is_privileged_service(sender) or irc.is_internal_client(sender) or irc.is_internal_server(sender)
    if not success:
        with claim_bounce_cache_lock:
            if irc.name not in claim_bounce_cache:
...
def get_supported_umodes(irc, remoteirc, modes):
    ...
    for modepair in modes:
        ...
        for name, m in irc.umodes.items():
            if name.startswith('*'):
                ...
                if modechar == m:
                    if name not in WHITELISTED_UMODES:
                        log.debug("(%s) relay.get_supported_umodes: skipping mode (%r, %r) because it isn't a whitelisted (safe) mode for relay.", irc.name, modechar, arg)
                        break
            ...
            log.debug("(%s) relay.get_supported_umodes: skipping mode (%r, %r) because the remote network (%s)'s IRCd (%s) doesn't support it.", irc.name, modechar, arg, remoteirc.name, remoteirc.protoname)
...
def relay_joins(irc, channel, users, ts, targetirc=None, **kwargs):
    ...
    """
    log.debug('(%s) relay.relay_joins: called on %r with users %r, targetirc=%s', irc.name, channel, ['%s/%s' % (user, irc.get_friendly_name(user)) for user in users], targetirc)
    if ts < 750000:
            ...
            if queued_users:
                if burst or len(queued_users) > 1 or queued_users[0][0]:
                    if claim_passed:
                        ...
                        if modedelta_modes:
                            adding = (
                             remoteirc.name, remotechan) in relay['links']
                            for mode in modedelta_modes:
                                ...
                                if modechar:
                                    if modechar in remoteirc.cmodes['*A'] or modechar in remoteirc.prefixmodes:
                                        log.warning('(%s) Refusing to set modedelta mode %r on %s because it is a list or prefix mode', irc.name, modechar, channel)
                                        continue
                                    else:
                                        if not remoteirc.has_cap('can-spawn-clients'):
                                            log.debug('(%s) relay.handle_mode: Not enforcing modedelta modes on bot-only network %s', irc.name, remoteirc.name)
                                            continue
                                        modedelta_mode = ('+%s' % modechar, mode[1])
                                        if adding:
                                            log.debug('(%s) relay.relay_joins: adding %r on %s/%s (modedelta)', irc.name, str(modedelta_mode), remoteirc.name, remotechan)
                                            modes.append(modedelta_mode)
                                        elif modedelta_mode in modes:
                                            log.debug('(%s) relay.relay_joins: removing %r on %s/%s (modedelta)', irc.name, str(modedelta_mode), remoteirc.name, remotechan)
...
def relay_part(irc, *args, **kwargs):
    ...
    def _relay_part_loop(irc, remoteirc, channel, user):
        ...
        if is_relay_client(remoteirc, remoteuser):
    ...
    if not remotechan:
        ...
        if remoteirc.protoname == 'clientbot' or irc.protoname == 'clientbot':
            ...
            if modesync not in CLIENTBOT_MODESYNC_OPTIONS:
                modesync = 'none'
                log.warning('relay: Bad clientbot_modesync option %s: valid values are %s', modesync, CLIENTBOT_MODESYNC_OPTIONS)
            if modesync == 'none':
                return []
            if modesync == 'half':
            ...
            for name, m in irc.cmodes.items():
                ...
                if modechar == m:
                    if name not in whitelist:
                        log.debug("(%s) relay.get_supported_cmodes: skipping mode (%r, %r) because it isn't a whitelisted (safe) mode for relay.", irc.name, modechar, arg)
                        break
                    ...
                    if name in remoteirc.extbans_acting:
                        ...
                        pending_extban_prefixes.append(name)
                        log.debug('(%s) relay.get_supported_cmodes: folding mode %s%s %s to %s%s %s%s for %s', irc.name, prefix, modechar, old_arg, prefix, supported_char, remoteirc.extbans_acting[name], arg, remoteirc.name)
                    else:
                        if supported_char is None:
                            continue
                    if modechar in irc.prefixmodes:
                        log.debug('(%s) relay.get_supported_cmodes: coersing argument of (%r, %r) for network %r.', irc.name, modechar, arg, remoteirc.name)
                        if not irc.has_cap('can-spawn-clients'):
                            if irc.pseudoclient:
                                if arg == irc.pseudoclient.uid:
                                    log.debug('(%s) relay.get_supported_cmodes: filtering prefix change (%r, %r) on Clientbot relayer', irc.name, name, arg)
                                    break
                        arg = get_orig_user(irc, arg, targetirc=remoteirc) or get_remote_user(irc, remoteirc, arg, spawn_if_missing=False)
                        if arg is None:
                            break
                        log.debug('(%s) relay.get_supported_cmodes: argument found as (%r, %r) for network %r.', irc.name, modechar, arg, remoteirc.name)
                        oplist = []
                        if remotechan in remoteirc.channels:
                            oplist = remoteirc.channels[remotechan].prefixmodes[name]
                        log.debug('(%s) relay.get_supported_cmodes: list of %ss on %r is: %s', irc.name, name, remotechan, oplist)
                        if prefix == '+' and arg in oplist:
                            log.debug('(%s) relay.get_supported_cmodes: skipping setting %s on %s/%s because it appears to be already set.', irc.name, name, arg, remoteirc.name)
                            break
                    else:
                        if arg:
                            for extban_name, extban_prefix in irc.extbans_acting.items():
                                if name in {'ban', 'banexception'}:
                                    if arg.startswith(extban_prefix):
                                        ...
                                        if extban_name in remoteirc.cmodes:
                                            ...
                                            log.debug('(%s) relay.get_supported_cmodes: expanding acting extban %s%s %s to %s%s %s for %s', irc.name, prefix, orig_supported_char, old_arg, prefix, supported_char, arg, remoteirc.name)
                                            name = extban_name
                                        else:
                                            if extban_name in remoteirc.extbans_acting:
                                                pending_extban_prefixes.append(extban_name)
                                                arg = arg[len(extban_prefix):]
                                                log.debug('(%s) relay.get_supported_cmodes: expanding acting extban %s%s %s to %s%s %s%s for %s', irc.name, prefix, orig_supported_char, old_arg, prefix, supported_char, remoteirc.extbans_acting[extban_name], arg, remoteirc.name)
                                            else:
                                                log.debug("(%s) relay.get_supported_cmodes: blocking acting extban %s%s %s as target %s doesn't support it", irc.name, prefix, supported_char, arg, remoteirc.name)
                            ...
                            for extban_name, extban_prefix in irc.extbans_matching.items():
                                ...
                                if arg == extban_prefix:
                                    if extban_name in remoteirc.extbans_matching:
                                        ...
                                        log.debug('(%s) relay.get_supported_cmodes: mangling static matching extban %s => %s for %s', irc.name, old_arg, arg, remoteirc.name)
                                        break
                                    else:
                                        log.debug("(%s) relay.get_supported_cmodes: setting mode_parse_aborted as (%r, %r) (name=%r; extban_name=%r) doesn't match any (static) extban on %s", irc.name, supported_char, arg, name, extban_name, remoteirc.name)
                                        mode_parse_aborted = True
                                else:
                                    if extban_prefix.endswith(':'):
                                        if arg.startswith(extban_prefix):
                                            if extban_name in remoteirc.extbans_matching:
                                                arg = arg[len(extban_prefix):]
                                                arg = remoteirc.extbans_matching[extban_name] + arg
                                                log.debug('(%s) relay.get_supported_cmodes: mangling matching extban arg %s => %s for %s', irc.name, old_arg, arg, remoteirc.name)
                                                break
                                            else:
                                                log.debug("(%s) relay.get_supported_cmodes: setting mode_parse_aborted as (%r, %r) (name=%r; extban_name=%r) doesn't match any (dynamic) extban on %s", irc.name, supported_char, arg, name, extban_name, remoteirc.name)
                                                mode_parse_aborted = True
                            else:
                                if name in ('ban', 'banexception', 'invex', 'quiet'):
                                    if not remoteirc.is_hostmask(arg):
                                        log.debug("(%s) relay.get_supported_cmodes: skipping unsupported extban/mode (%r, %r) because it doesn't match nick!user@host. (name=%r)", irc.name, supported_char, arg, name)
                                        break

                            while pending_extban_prefixes:
                                next_prefix = pending_extban_prefixes.pop()
                                log.debug('(%s) relay.get_supported_cmodes: readding extban prefix %r (%r) to (%r, %r) for %s', irc.name, next_prefix, remoteirc.extbans_acting[next_prefix], supported_char, arg, remoteirc.name)
                                arg = remoteirc.extbans_acting[next_prefix] + arg

                        else:
                            if mode_parse_aborted:
                                log.debug('(%s) relay.get_supported_cmodes: blocking unsupported extban/mode (%r, %r) for %s (mode_parse_aborted)', irc.name, supported_char, arg, remoteirc.name)
                                break
                            final_modepair = (
                             prefix + supported_char, arg)
                            if prefix == '+':
                                if remotechan not in remoteirc.channels or final_modepair in remoteirc.channels[remotechan].modes:
                                    log.debug('(%s) relay.get_supported_cmodes: skipping setting mode (%r, %r) on %s%s because it appears to be already set.', irc.name, supported_char, arg, remoteirc.name, remotechan)
                                    break
                        supported_modes.append(final_modepair)
                        log.debug('(%s) relay.get_supported_cmodes: added modepair (%r, %r) for %s%s', irc.name, supported_char, arg, remoteirc.name, remotechan)
                        break
...
def handle_relay_whois(irc, source, command, args):
    ...
    if origuser:
        ...
        wreply(320, ':is a remote user connected via PyLink Relay. Home network: %s; Home nick: %s' % (
         netname, realuser.nick))
        if _check_send_key('whois_show_accounts'):
            if realuser.services_account:
                wreply(330, '%s :is logged in (on %s) as' % (realuser.services_account, netname))
        if _check_send_key('whois_show_server'):
            if realirc.has_cap('can-track-servers'):
...
def handle_operup(irc, numeric, command, args):
    ...
    def _handle_operup_func(irc, remoteirc, user):
        log.debug('(%s) relay.handle_opertype: setting OPERTYPE of %s/%s to %s', irc.name, user, remoteirc.name, newtype)
...
def handle_join(irc, numeric, command, args):
    ...
    if chandata and not claim_passed:
        ...
        for user in users:
            ...
            log.debug('(%s) relay.handle_join: mode diff for %s on %s: %s oldmodes=%s newmodes=%s', irc.name, user, channel, modediff, oldmodes, newmodes)
        ...
        for user in users:
            ...
            if show_splits:
                ...
                try:
                    ...
                    log.warning('(%s) relay.handle_squit: Failed to get server name for %s', irc.name, uplink)
            ...
            for channel in [c for c in channels if get_relay(irc, c)]:
                for user in irc.channels[channel].users.copy():
                    if not irc.is_internal_client(user) and not is_relay_client(irc, user):
                        irc.call_hooks([irc.sid, 'CLIENTBOT_SERVICE_KICKED',
                         {'channel':channel,  'target':user,  'text':'Clientbot was force parted (%s)' % text or 'None', 
                          'parse_as':'KICK'}])
...
def handle_messages(irc, numeric, command, args):
    ...
    if numeric in irc.servers:
        if not notice:
            log.debug('(%s) relay.handle_messages: dropping PM from server %s to %s', irc.name, numeric, target)
            return
    ...
    log.debug('(%s) relay.handle_messages: splitting target %r into prefixes=%r, target=%r', irc.name, args['target'], prefixes, target)
    if irc.is_channel(target):

        def _handle_messages_loop(irc, remoteirc, numeric, command, args, notice, target, text, msgprefixes):
            ...
            if not real_target or not irc.connected.is_set():
                ...
                log.debug('(%s) relay: filtering message prefixes for %s%s from %s to %s', irc.name, remoteirc.name, real_target, orig_msgprefixes, msgprefixes)
                if orig_msgprefixes and not msgprefixes:
                    if 'h' in orig_msgprefixes:
                        msgprefixes.append('o')
                    else:
                        log.debug('(%s) relay: dropping message for %s%s, orig_prefixes=%r since prefixes were empty after filtering.', irc.name, remoteirc.name, real_target, orig_msgprefixes)
                        return
                else:
                    ...
                    user or irc.serverdata.get('relay_weird_senders', conf.conf.get('relay', {}).get('accept_weird_senders', True)) or log.debug('(%s) Dropping message for %s from user-less sender %s', irc.name, real_target, numeric)
                    return
                if numeric not in irc.servers:
                    if not irc.get_service_bot(numeric):
                        displayedname = irc.get_friendly_name(numeric)
                        real_text = '<%s/%s> %s' % (displayedname, irc.name, text)
                real_text = text
            try:
                user = get_relay_server_sid(remoteirc, irc, spawn_if_missing=False) if notice else remoteirc.pseudoclient.uid
                if not user:
                    return
            except AttributeError:
                return
                ...
                if remoteirc.pseudoclient.uid not in remoteirc.users:
                    ...
                    real_text = text
                log.debug('(%s) relay.handle_messages: sending message to %s from %s on behalf of %s', irc.name, real_target, user, numeric)
                if real_target.startswith(tuple(irc.prefixmodes.values())):
                    if not remoteirc.has_cap('has-statusmsg'):
                        log.debug('(%s) Not sending message destined to %s/%s because the remote does not support STATUSMSG.', irc.name, remoteirc.name, real_target)
                        return
                try:
                    if notice:
                    ...
                    log.warning('(%s) relay: Relay client %s on %s was killed while trying to send a message through it!', irc.name, remoteirc.name, user)
                    return

        iterate_all(irc, _handle_messages_loop, extra_args=(
         numeric, command, args, notice, target, text, prefixes))
    else:
        origuser = get_orig_user(irc, target)
        if origuser is None:
            return
        ...
        if homenet not in remoteusers.keys():
            if not irc.is_privileged_service(numeric):
                irc.msg(numeric, ('You must be in a common channel with %r in order to send messages.' % irc.users[target].nick),
                  notice=True)
            return
        ...
        if not remoteirc.has_cap('can-spawn-clients'):
            if not conf.conf.get('relay', {}).get('allow_clientbot_pms'):
                if not irc.is_privileged_service(numeric):
                    irc.msg(numeric, 'Private messages to users connected via Clientbot have been administratively disabled.', notice=True)
                return
        user = get_remote_user(irc, remoteirc, numeric, spawn_if_missing=False)
    try:
        if notice:
...
def handle_kick(irc, source, command, args):
            ...
            if target == irc.pseudoclient.uid:
                for user in irc.channels[channel].users:
                    if not irc.is_internal_client(user) and not is_relay_client(irc, user):
                        ...
                        irc.call_hooks([irc.sid, 'CLIENTBOT_SERVICE_KICKED',
                         {'channel':channel,  'target':user,  'text':reason, 
                          'parse_as':'KICK'}])
    ...
    def _handle_kick_loop(irc, remoteirc, source, command, args):
        ...
        if remoteirc != irc:
            if not remoteirc.users[real_target].channels:
                if not origuser:
    ...
    if is_relay_client(irc, target) and not check_claim(irc, channel, kicker):
        if _claim_should_bounce(irc, channel):
            ...
            if kicker in irc.users:
                log.info('(%s) relay: Blocked KICK (reason %r) from %s/%s to %s/%s on %s.', irc.name, args['text'], irc.users[source].nick, irc.name, homenick, homenet, channel)
                irc.msg(kicker, ('This channel is claimed; your kick to %s has been blocked because you are not (half)opped.' % channel),
                  notice=True)
            else:
                log.info('(%s) relay: Blocked KICK (reason %r) from server %s to %s/%s on %s.', irc.name, args['text'], irc.servers[source].name, homenick, homenet, channel)
        return
    iterate_all(irc, _handle_kick_loop, extra_args=(source, command, args))
    if origuser:
        if not irc.users[target].channels:
...
def handle_chgclient(irc, source, command, args):
    ...
    if args.get('newhost'):
        ...
        if args.get('newident'):
            field = 'IDENT'
            text = args['newident']
        else:
            if args.get('newgecos'):
                field = 'GECOS'
                text = args['newgecos']
    if field:
        ...
        def _handle_chgclient_loop(irc, remoteirc, user):
            try:
                ...
                log.debug("(%s) relay.handle_chgclient: Ignoring changing field %r of %s on %s (for %s/%s); remote IRCd doesn't support it", irc.name, field, user, remoteirc.name, target, irc.name)
                return
...
def handle_mode(irc, numeric, command, args):
    ...
    def _handle_mode_loop(irc, remoteirc, numeric, command, target, modes):
        if irc.is_channel(target):
            ...
            remotesender = get_remote_user(irc, remoteirc, numeric, spawn_if_missing=False) or get_relay_server_sid(remoteirc, irc) or remoteirc.sid
            if not remoteirc.has_cap('can-spawn-clients'):
                if numeric in irc.servers and not irc.servers[numeric].has_eob:
                    log.debug('(%s) Not relaying modes from server %s/%s to %s as it has not finished bursting', irc.name, numeric, irc.get_friendly_name(numeric), remoteirc.name)
                else:
                    friendly_modes = []
                    for modepair in modes:
                        ...
                        if modechar in irc.prefixmodes:
                            ...
                            if orig_user:
                                if orig_user[0] == remoteirc.name:
                                    continue
                            modepair = (
                             modepair[0], irc.get_friendly_name(modepair[1]))
                        else:
                            if modechar in irc.cmodes['*A']:
                                if irc.is_hostmask(modepair[1]):
                                    if conf.conf.get('relay', {}).get('clientbot_modesync', 'none').lower() != 'none':
                                        continue
            ...
            if use_hideoper:
                if ('-%s' % hideoper_mode, None) in modes:
    ...
    if irc.is_channel(target):
        ...
        if not relay_entry:
            ...
            modedelta_modes = list(filter(None, [irc.cmodes.get(named_modepair[0]) for named_modepair in modedelta_modes]))
        if not check_claim(irc, target, numeric, chanobj=oldchan):
            ...
            if irc.is_privileged_service(numeric):
                reversed_modes = [modepair for modepair in reversed_modes if modepair[0][(-1)] in irc.prefixmodes and not irc.is_privileged_service(modepair[1]) or modepair[0][(-1)] in irc.cmodes['*A']]
            modes.clear()
        for modepair in modes.copy():
            log.debug('(%s) relay.handle_mode: checking if modepair %s is in %s', irc.name, str(modepair), str(modedelta_modes))
            modechar = modepair[0][(-1)]
            if modechar in modedelta_modes:
                if modechar in irc.cmodes['*A'] or modechar in irc.prefixmodes:
                    log.debug('(%s) relay.handle_mode: Not enforcing invalid modedelta mode %s on %s (list or prefix mode)', irc.name, str(modepair), target)
                    continue
                else:
                    if not irc.has_cap('can-spawn-clients'):
                        log.debug('(%s) relay.handle_mode: Not enforcing modedelta modes on bot-only network', irc.name)
                        continue
                modes.remove(modepair)
                if relay_entry[0] != irc.name:
                    reversed_modes += irc.reverse_modes(target, [modepair], oldobj=oldchan)
                    log.debug('(%s) relay.handle_mode: Reverting change of modedelta mode %s on %s', irc.name, str(modepair), target)
                else:
                    log.debug('(%s) relay.handle_mode: Not propagating change of modedelta mode %s on %s', irc.name, str(modepair), target)

    if reversed_modes:
        if _claim_should_bounce(irc, target):
            log.debug('(%s) relay.handle_mode: Reversing mode changes %r on %s with %r.', irc.name, args['modes'], target, reversed_modes)
            irc.mode(irc.sid, target, reversed_modes)
        else:
            log.debug('(%s) relay.handle_mode: Fake reversing mode changes %r on %s with %r.', irc.name, args['modes'], target, reversed_modes)
...
def handle_topic(irc, numeric, command, args):
    ...
    if check_claim(irc, channel, numeric):
        ...
        def _handle_topic_loop(irc, remoteirc, numeric, command, args):
            ...
            topic = args['text']
            remotechan = get_remote_channel(irc, remoteirc, channel)
            if remotechan is None or remotechan not in remoteirc.channels or topic == remoteirc.channels[remotechan].topic:
                return
            else:
        ...
        if _has_common_pool(irc.name, realuser[0], 'kill_share_pools'):
            if numeric in irc.users:
                ...
                def _relay_kill_loop(irc, remoteirc):
                    ...
                    if rtarget:
                        rsender = get_relay_server_sid(remoteirc, irc, spawn_if_missing=False) or remoteirc.sid
                    ...
                    if numeric in irc.users:
                        ...
                        def _relay_kill_to_kick(origirc, remoteirc, rtarget):
                            ...
                            if not remotechan:
                                return
                            rsender = get_relay_server_sid(remoteirc, irc, spawn_if_missing=False) or remoteirc.sid
                            log.debug('(%s) relay.handle_kill: forwarding kill to %s/%s@%s as kick to %s/%s@%s on %s', irc.name, realuser[1], target_nick, realuser[0], rtarget, remoteirc.get_friendly_name(rtarget), remoteirc.name, remotechan)
                            remoteirc.kick(rsender, remotechan, rtarget, fwd_reason)

                        iterate_all_present(origirc, realuser[1], _relay_kill_to_kick)
                        hsender = get_relay_server_sid(origirc, irc, spawn_if_missing=False) or homeirc.sid
                        log.debug('(%s) relay.handle_kill: forwarding kill to %s/%s@%s as kick on %s', irc.name, realuser[1], target_nick, realuser[0], homechan)
...
def handle_away(irc, numeric, command, args):
    iterate_all_present(irc, numeric, lambda irc, remoteirc, user: remoteirc.away(user, args['text']))
...
def handle_invite(irc, source, command, args):
    ...
    if is_relay_client(irc, target):
        ...
        if remotesource is None:
            irc.msg(source, ('You must be in a common channel with %s to invite them to channels.' % irc.users[target].nick),
              notice=True)
        else:
            if remotechan is None:
                irc.msg(source, 'You cannot invite someone to a channel not on their network!', notice=True)
            else:
                remoteirc.invite(remotesource, remoteuser, remotechan)
...
def handle_services_login(irc, numeric, command, args):
    ...
    """
    iterate_all_present(irc, numeric, lambda irc, remoteirc, user: remoteirc.call_hooks([user, 'PYLINK_RELAY_SERVICES_LOGIN', args]))
        ...
        for chanpair, entrydata in db.items():
            if chanpair[0] == irc.name:
                for leaf in entrydata['links']:
                    ...
                    if remoteirc:
                        if remoteirc.connected.is_set():
                            if announcement:
                                log.debug('(%s) relay: Announcing disconnect to %s%s', irc.name, leaf[0], leaf[1])
                                text = string.Template(announcement).safe_substitute({'homenetwork':irc.name, 
                                 'homechannel':chanpair[1],  'network':remoteirc.name, 
                                 'channel':leaf[1]})
                        ...
                        for sname, sbot in world.services.items():
                            if sname == 'pylink':
                                ...
                                try:
                                    sbot.remove_persistent_channel(remoteirc, 'relay', (leaf[1]), part_reason=CHANNEL_DELINKED_MSG)
                                except KeyError:
                                    continue
                                else:
                                    log.debug('(%s) Removed service %r from %s%s as the home network disconnected', irc.name, sname, leaf[0], leaf[1])
...
def forcetag_nick(irc, target):
    ...
    try:
        ...
        log.debug('(%s) relay.forcetag_nick: Fixing nick of relay client %r (%s) to %s', irc.name, target, nick, newnick)
        if nick == newnick:
            log.debug('(%s) relay.forcetag_nick: New nick %s for %r matches old nick %s', irc.name, newnick, target, nick)
            return False
        else:
...
def handle_knock(irc, source, command, args):
    ...
    def _handle_knock_loop(irc, remoteirc, source, command, args):
        ...
        if use_fallback:
            ...
            log.debug('(%s) relay: using fallback KNOCK routine for %s on %s/%s', irc.name, nick, remoteirc.name, remotechan)
            prefix = '%' if 'h' in remoteirc.prefixmodes else '@'
            remoteirc.notice(remoteirc.pseudoclient.uid, prefix + remotechan, 'Knock from %s@%s (*not* invitable from this network): %s' % (
             nick, irc.name, text))
...
def create(irc, source, args):
    ...
    try:
        ...
        db[(irc.name, str(channel))] = {'links':set(), 
         ...
         'allowed_nets':set(), 
         'claim':[irc.name] if irc.get_service_option('relay', 'enable_default_claim', True) else []}
...
def destroy(irc, source, args):
    ...
    if not irc.is_channel(channel):
        ...
        if entry in db:
            stop_relay(entry)
            del db[entry]
            log.info('(%s) relay: Channel %s destroyed by %s.', irc.name, channel, irc.get_hostmask(source))
            irc.reply('Done.')
        else:
            irc.error("No such channel %r exists. If you're trying to delink a channel from another network, use the DESTROY command." % channel)
        return
...
def link(irc, source, args):
    ...
    if localchan not in irc.channels or source not in irc.channels[localchan].users:
        ...
        if irc.protoname == 'clientbot':
            if not irc.channels[localchan].is_op_plus(source):
                if irc.pseudoclient:
    ...
    try:
        ...
        if not whitelist_mode and irc.name in entry['blocked_nets'] or whitelist_mode and irc.name not in entry.get('allowed_nets', set()):
            irc.error('Access denied (target channel is not open to links).')
            log.warning('(%s) relay: Blocking link request %s%s -> %s%s from %s due to LINKACL (whitelist_mode=%s)', irc.name, irc.name, localchan, remotenet, channel, irc.get_hostmask(source), whitelist_mode)
            return
        for link in entry['links']:
            if link[0] == irc.name:
                irc.error("Remote channel '%s%s' is already linked here as %r." % (
                 remotenet, args.channel, link[1]))
                return

        if args.force_ts:
            permissions.check_permissions(irc, source, ['relay.link.force_ts', 'relay.link.force'])
            log.info('(%s) relay: Forcing link %s%s -> %s%s', irc.name, irc.name, localchan, remotenet, args.channel)
        else:
            if not world.networkobjects[remotenet].connected.is_set():
                log.debug('(%s) relay: Blocking link request %s%s -> %s%s because the target network is down', irc.name, irc.name, localchan, remotenet, args.channel)
                irc.error('The target network %s is not connected; refusing to link (you may be able to override this with the --force option).' % remotenet)
                return
        ...
        if our_ts < their_ts:
            if irc.has_cap('has-ts'):
                log.debug('(%s) relay: Blocking link request %s%s -> %s%s due to bad TS (%s < %s)', irc.name, irc.name, localchan, remotenet, channel, our_ts, their_ts)
                irc.error("The channel creation date (TS) on %s (%s) is lower than the target channel's (%s); refusing to link. You should clear the local channel %s first before linking, or use a different local channel (you may be able to override this with the --force option)." % (
                 localchan, our_ts, their_ts, localchan))
                return
        entry['links'].add((irc.name, localchan))
        log.info('(%s) relay: Channel %s linked to %s%s by %s.', irc.name, localchan, remotenet, args.channel, irc.get_hostmask(source))
...
def delink(irc, source, args):
    ...
    try:
        ...
        if entry:
            if entry[0] == irc.name:
                if not remotenet:
                    irc.error("You must select a network to delink, or use the 'destroy' command to remove this relay entirely (it was created on the current network).")
                    return
                for link in db[entry]['links'].copy():
                    if link[0] == remotenet:
            ...
            irc.reply('Done.')
            log.info('(%s) relay: Channel %s delinked from %s%s by %s.', irc.name, channel, entry[0], entry[1], irc.get_hostmask(source))
        else:
...
def linked(irc, source, args):
    ...
    remote_networks = [netname for netname, ircobj in world.networkobjects.copy().items() if ircobj.connected.is_set()]
    ...
    for k, v in sorted(db.items()):
        if net:
            if not (net == k[0] or net in [link[0] for link in v['links']]):
                continue
            else:
                s = '\x02%s%s\x02 ' % k
                ...
                if remoteirc and channel in remoteirc.channels:
                    ...
                    if ('s', None) in c.modes or ('p', None) in c.modes:
                        ...
                        if irc.is_oper(source) or localchan and source in irc.channels[localchan].users:
                            ...
                            continue
                    if v['links']:
                        s += ' '.join([''.join(link) for link in sorted(v['links']) if link[0] in world.networkobjects if world.networkobjects[link[0]].connected.is_set()])
                    else:
                        s += '(no relays yet)'
                irc.reply(s, private=True)
...
def claim(irc, source, args):
    ...
    try:
        ...
        relay = (
         irc.name, channel)
        if relay not in db:
            irc.error('No relay %r exists on this network (this command must be run on the network the channel was created on).' % channel)
            return
...
def modedelta(irc, source, args):
    ...
    try:
        ...
        modedelta_diff = [('-%s' % modepair[0], modepair[1]) for modepair in old_modes if modepair not in target_modes]
                ...
                for modepair in target_modes:
                    ...
                    if mchar:
                        if mchar in remoteirc.cmodes['*A'] or mchar in remoteirc.prefixmodes:
                            log.warning('(%s) Refusing to set modedelta mode %r on %s because it is a list or prefix mode', irc.name, mchar, remotechan)
                            continue
                        else:
                            if not remoteirc.has_cap('can-spawn-clients'):
                                log.debug('(%s) relay.handle_mode: Not enforcing modedelta modes on bot-only network %s', irc.name, remoteirc.name)
    ...
    try:
        ...
        if len(args) >= 2:
            if args[1].strip() == '-':
                ...
                log.info('(%s) %s set the description for %s to: %s', irc.name, irc.get_hostmask(source), channel, newdesc)
