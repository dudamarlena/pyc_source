...
# Size of source mod 2**32: 15898 bytes
...
def index(request, conn=None, **kwargs):
    ...
    if len(init['initially_select']) > 0:
        ...
        if first_obj not in ('project', 'screen') and first_sel is not None:
    ...
    if first_sel is not None:
        ...
        if user_id not in set([x.id for x in leaders]) | set([x.id for x in members]):
            if user_id != -1:
                user_id = None
        if user_id is None:
