...
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/apogee/core/entropy.py
# Compiled at: 2019-06-14 18:22:47
# Size of source mod 2**32: 3203 bytes
__doc__ = 'Utility functions for computing entropy.'
import numpy as np

def entropy(p: np.ndarray, **kwargs):
    """Comput the entropy of a vector."""
    p = np.clip(p, 1e-16, 0.9999999999999999)
...
def discrete_mutual_information(joint: np.ndarray):
    ...
    * Derived from sklearn.metrics.cluster.supervised.mutual_info_score
    ...
    mi = contingency_nm * (log_contingency_nm - np.log(contingency_sum)) + contingency_nm * log_outer
    return mi.sum()
...
def mutual_information_index(xi, y, normed=True):
    ...
    """
    if normed:
        return (entropy(y) - entropy(xi)) / entropy(y)
    else:
        return entropy(y) - entropy(xi)
