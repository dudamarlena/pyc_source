...
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/apogee/factors/discrete/operations/utils.py
...
def format_discrete_marginals(*marginals, **kwargs):
    ...
    if normed:
        ...
        if unpack or states is not None:
            data = {mgnl.scope[0]:{j:round(p if space == 'p' else np.log(p), dp) for j, p in enumerate(mgnl.parameters)} for mgnl in marginals}
