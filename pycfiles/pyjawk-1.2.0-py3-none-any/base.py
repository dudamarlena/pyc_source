...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/pyjaw/base.py
# Compiled at: 2009-02-10 14:43:42
...
class RejawClient(object):
    ...
    def __init__(self, api_key, hostname='localhost:3000'):
    ...
    def create_session(self, username=None, password=None):
        ...
        result = self._post('session/create', {'api_key': self.api_key, 'username': username, 
    ...
    def get_people(self, p_type, username=None, page=1, per_page=10, detail=None):
        """Relationship lists: get people"""
        return self._get('user/get_people', {'type': p_type, 'username': username, 'page': page, 
           'per_page': 10, 'detail': detail})

    def get_inbox(self, inbox_type='shouts_all', page=1, per_page=10, detail=None, include_replies=None):
        """Relationship lists: get inbox"""
        return self._get('user/get_inbox', {'type': inbox_type, 'page': page, 'per_page': per_page, 'detail': detail, 
    ...
    def block(self, username):
        ...
        return self._post('user/block', {'username': username})

    def unblock(self, username):
        """Unblock the specified username"""
        return self._post('user/unblock', {'username': username})
    ...
    def shout(self, text, room=None):
        return self._post('conversation/shout', {'text': text, 'room': room})
    ...
    def favorite_conversation(self, conversation_id):
        return self._post('conversation/favorite', {'cid': conversation_id})

    def unfavorite_conversation(self, conversation_id):
        return self._post('conversation/favorite', {'cid': conversation_id})
    ...
    def search_users(self, query, page=1, per_page=None, detail=None):
        """Search Rejaw users by keywords"""
        return self._get('user/search', {'q': query, 'page': page, 'per_page': per_page, 'detail': detail})

    def search_conversations(self, query):
        """Search shouts, replies, and, possibly, whispers, by keywords. If
        an authenticated session id is provided, results may contain whispers
        (and replies in whispers) that the authenticated user is a member of.
        Otherwise, results will only contain public shouts and replies in those shouts."""
        return self._get('conversation/search', {'q': query})
    ...
    def get_room_info(self, room):
        self._get('room/get_info', {'room': room})

    def get_room_members(self, room):
        self._get('room/get_members', {'room': room})

    def add_room_user(self, room, username):
        self._post('room/add_user', {'room': room, 'username': username})

    def remove_room_user(self, room, username):
        self._post('room/remove_user', {'room': room, 'username': username})

    def grant_room_owner(self, room, username):
        self._post('room/grant_owner', {'room': room, 'username': username})

    def revoke_room_owner(self, room, username):
        self._post('room/revoke_owner', {'room': room, 'username': username})
