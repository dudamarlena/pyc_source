...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flame/stats/base_model.py
...
import numpy as np, os, copy, time, glob
...
class BaseEstimator:
    ...
    def __init__(self, X, Y, parameters):
        ...
        if self.parameters['imbalance'] is not None:
    ...
    def CF_quantitative_validation(self):
        ...
        for i in range(len(seeds)):
            X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.25, random_state=i,
              shuffle=False)
            conformal_pred = AggregatedCp(IcpRegressor(RegressorNc(RegressorAdapter(self.estimator))), BootstrapSampler())
    ...
    def CF_qualitative_validation(self):
        ...
        for i in range(len(seeds)):
            X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.25, random_state=i,
              shuffle=True)
            conformal_pred = AggregatedCp(IcpClassifier(ClassifierNc(ClassifierAdapter(self.estimator), MarginErrFunc())), BootstrapSampler())
            conformal_pred.fit(X_train, Y_train)
            prediction = conformal_pred.predict(X_test, self.conformalSignificance)
            c0_correct = 0
            c1_correct = 0
    ...
    def quantitativeValidation(self):
        ...
        y_pred = cross_val_predict((copy.copy(self.estimator)), (copy.copy(X)), (copy.copy(Y)),
          cv=(self.cv), n_jobs=1)
    ...
    def qualitativeValidation(self):
        ...
        if len(Yp) != len(Y):
            ...
            results = []
            self.TNpred, self.FPpred, self.FNpred, self.TPpred = confusion_matrix(Y,
              Yp, labels=[0, 1]).ravel()
            ...
            self.TN, self.FP, self.FN, self.TP = confusion_matrix(Y,
    ...
    def optimize(self, X, Y, estimator, tune_parameters):
        ...
        tclf = GridSearchCV(estimator, tune_parameters, scoring=metric,
          cv=(self.cv),
          n_jobs=(-1))
        tclf.fit(X, Y)
    ...
    def conformalProject(self, Xb, results):
        ...
        prediction = self.conformal_pred.predict(Xb,
          significance=(self.conformalSignificance))
        if self.quantitative:
