...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flame/idata.py
# Compiled at: 2018-06-21 09:52:41
# Size of source mod 2**32: 31214 bytes
import os, sys, pickle, shutil, json, tempfile, multiprocessing as mp, pathlib, numpy as np
from rdkit import Chem
from standardiser import standardise
import flame.chem.sdfileutils as sdfu, flame.chem.compute_md as computeMD, flame.chem.convert_3d as convert3D
...
class Idata:
    ...
    def __init__(self, parameters, input_source):
        ...
        self.dest_path = '.'
        self.results = {'manifest':[],  'meta':{'main':[],  'endpoint':self.parameters['endpoint'], 
          'version':self.parameters['version']}}
        if 'ext_input' in parameters:
            if parameters['ext_input']:
            ...
            for mol in suppl:
                if mol is None:
                    ...
                    name = sdfu.getName(mol,
                      count=obj_num, field=(self.parameters['SDFile_name']), suppl=suppl)
                    activity_num = None
                    exp = None
    ...
    def computeMD(self, ifile, method):
        ...
        registered_methods = [
         (
          'RDKit_properties', computeMD._RDKit_properties),
         (
            ...
            if imethod[0] in method:
                ...
                if not success:
                    return (
                     success, results)
                if is_empty:
                    combined_md = results[0]
                    combined_nm = results[1]
                    combined_sc = results[2]
        ...
        return (
         True, (combined_md, combined_nm, combined_sc))
    ...
    def consolidate(self, results, nobj):
        ...
        return (
         True, (xmatrix, var_nam, success_list))
    ...
    def save(self):
        ...
        md5_input = utils.md5sum(self.ifile)
        try:
            ...
            for i, ifile in enumerate(file_list):
                if not success_list[i]:
                    ...
                    if not success:
                        ...
                        first_mol = False
                    elif len(results[0]) != num_var:
                        print('ERROR: (@workflow_objects) MD length for molecule #', str(i + 1), 'in file ' + input_file + 'does not match the MD length of the first molecule')
