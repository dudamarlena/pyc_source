...
# [GCC 8.4.0]
# Embedded file name: /Users/nino/dev/marx/tests/workflow/test_step.py
...
from marx.workflow.step import Step, LogicUnit, ArgSpec, ResultSpec, ResultObject
import nose.tools
...
class Test(unittest.TestCase):
    ...
    def test_result_mapper_list(self):
        ...
        with patch('inspect.getargspec') as (argspec):
            argspec.return_value = [[]]
            Step(m, result_map={'baz': ('returned', 'bar'), 'boz': ('returned', 'boz')})(ctx)
        assert m.called
        print ctx.__dict__
        assert ctx.baz == 1
        assert ctx.boz == 2
    ...
    def test_auto_map_bad_field(self):
        ...
        nose.tools.assert_raises(AttributeError, Step(Unit(), arg_map=Unit.AutoMap()), Ctx())
...
class TestLogicUnit(unittest.TestCase):
    ...
    def test_args(self):
        ...
        nose.tools.assert_raises(TypeError, u, an_arg=1, astr=1)
    ...
    def test_composition(self):
        ...
        s = Step(IsSameDomainUser(), arg_map={IsSameDomainUser.USER: 'actor', IsSameDomainUser.DOMAIN: 'domain'})
    ...
    def test_default_value(self):
        ...
        nose.tools.assert_raises(ValueError, HasADefault())
        nose.tools.assert_raises(ValueError, HasADefault(), meow=1)
        nose.tools.assert_raises(IndexError, HasADefault(), meow=10)
        return
    ...
    def test_nullable(self):
        ...
        nose.tools.assert_raises(ValueError, Nullable())
        nose.tools.assert_raises(ValueError, Nullable(), meow=None)
