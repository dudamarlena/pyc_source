...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gearbox\transmition\gears.py
# Compiled at: 2015-04-24 15:20:02
...
class Material(object):
    ...
    def __init__(self, sh_limit, sf_limit, brinell, classification, name='', e=206000.0, poisson=0.3, density=7.83e-06):
...
class Gear(object):
    ...
    def __init__(self, profile, material, z, beta, b, bs, alpha=20, m=1, x=0.0, sr=0, rz=0, precision_grade=6, shaft_diameter=0, schema=0, l=0, s=0, backlash=0, gear_crown=1, gear_condition=1, helix_modification=1, favorable_contact=True):
        ...
        f_pt = 0.3 * (mc + 0.4 * sqrt(dc)) + 4.0
        f_p = 0.3 * mc + 1.25 * sqrt(dc) + 7.0
        ...
        f_h_beta = 0.07 * sqrt(dc) + 0.45 * sqrt(bc) + 3.0
...
class Transmition(object):
    ...
    def __calculate(self, gear_one, gear_two, rpm_in, rpm_out):
        ...
        inv = involute(gear_one.alpha_t) + 2 * (gear_one.x + gear_two.x) / (gear_one.z + gear_two.z) * tan(radians(gear_one.alpha))
        ...
        self.epsilon_alpha = (0.5 * (sqrt(gear_one.da ** 2 - gear_one.db ** 2) + sqrt(gear_two.da ** 2 - gear_two.db ** 2)) - self.a * sin(radians(self.alpha_wt))) / (pi * gear_one.m * cos(radians(gear_one.alpha_t)) / cos(radians(gear_one.beta)))
        ...
        self.ft = 1000.0 * self.n * 60000 / (pi * gear_one.d * rpm_in)
