...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gearbox\export\export.py
# Compiled at: 2015-05-03 05:54:20
import os
...
class ExportGear(object):
    ...
    def __init__(self, gear):
        ...
        c = gear.profile.c
        b = gear.b
        gear_data = {'m_n': m, 'z': z, 'beta': beta, 'alpha_n': alpha, 'x': x, 'rho_f': rho_f, 'd_s': d_s, 'c': c, 'b': b}
    ...
    def matlab_comsol(self, output_folder='', model_name='model', type='2D'):
        ...
        output_from_parsed_template = template.render(gear=self.gear.export_data.gear, model_name=model_name, model_path=output_folder)
    ...
    def ansys(self, output_folder='', model_name='model', type='2D'):
        ...
        self.gear.export_data.gear.shaftcoords[0] = [
         -self.gear.export_data.gear.shaftcoords[1][0],
...
class ExportPair(object):
    ...
    def __init__(self, pair):
        ...
        shaft = [
         [
          0, self.wheel.data['d_s'] / 2],
         rotate([[0, self.wheel.data['d_s'] / 2]], 0.5 * (self.wheel.data['d_s'] / self.wheel.data['z'] * 360 / self.wheel.data['d_s']))[0]]
    ...
    def __aw(self):
        inv = involute(self.pinion.data['alpha_t']) + 2 * (self.pinion.data['x'] + self.wheel.data['x']) / (self.pinion.data['z'] + self.wheel.data['z']) * tan(radians(self.pinion.data['alpha_n']))
        alpha_wt = arcinvolute(inv)
        a = (self.pinion.data['z'] + self.wheel.data['z']) * self.pinion.data['m_n'] / (2 * cos(radians(self.pinion.data['beta'])))
