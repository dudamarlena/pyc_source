...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gearbox\standards\agma.py
# Compiled at: 2015-04-24 15:23:55
...
class Pitting(object):
    ...
    def calculate(self):
        ...
        SigmaH = Cp * sqrt(Ft * Ka * Kv * Ks * (Kh / (d * b)) * Cf / I)
        return {'sigmaH': SigmaH, 
           'Cp': Cp, 
           'Wt': Ft, 
           'Ka': Ka, 
           'Kv': Kv, 
...
class Bending(object):
    ...
    def calculate(self):
        ...
        sigmaFOne = Ft * Ka * Kv * Ks * (1.0 / (b * mt)) * (Kh * Kb / J[0])
        sigmaFTwo = Ft * Ka * Kv * Ks * (1.0 / (b * mt)) * (Kh * Kb / J[1])
...
def __GeometryFactor__(pair):
    ...
    if psi == 0.0:
        ...
        if mf > 1:
            na = mf % 1
            if 1 - nr < na:
                Lmin = (mp * F - (1.0 - na) * (1.0 - nr) * Px) / cos(psi_b)
            else:
    ...
    if mf <= 1:
        ...
        C_psi = 1
    if n1 > 0:
        I = cos(fi_r) * C_psi ** 2.0 / ((1.0 / rho_1 + 1.0 / rho_2) * d * mN)
    else:
        I = cos(fi_r) * C_psi ** 2.0 / ((1.0 / rho_1 - 1.0 / rho_2) * d * mN)
    ...
    def _J(n1, mG, Ro1, R1, R2, Rb1, C4, x, delta_sn, nc, hao, xo, rho_ao, delta_ao):
        ...
        while 1:
            ...
            y_ = 2 * hf / cos(beta_n) ** 2 - kf * sin(beta_n) + no / n * (rno_2 * sin(alpha_n) / (r_s_no * sin(alpha_n + mi_no)) - 1) * (2 * xi_nf * tan(beta_n) - eta_nf - 2 * hf / cos(beta_n) ** 2) - rno_2 * (cos(alpha_n) - sin(alpha_n) / tan(alpha_n + mi_no)) * ((1 + sin(beta_n) ** 2.0) / cos(beta_n))
    ...
    J2 = _J(n2, 1.0 / mG, Ro2, R2, R1, Rb2, C6 - C2, x2, delta_sn2, nc2, hao2, xo2, rho_ao2, delta_ao2)
    return {'I': I, 
       'J': [
           J1, J2]}
...
def __LoadDistribution__(pair):
    ...
    if pair.gear_one.bs <= 25:
        ...
        Cpf = pair.gear_one.bs / (10 * pair.gear_one.d) - 0.0375 + 0.000492 * pair.gear_one.bs
    else:
        Cpf = pair.gear_one.bs / (10 * pair.gear_one.d) - 0.1109 + 0.000815 * pair.gear_one.bs - 3.53e-07 * pair.gear_one.bs ** 2
    if pair.gear_one.s / pair.gear_one.l < 0.175:
        Cpm = 1
    else:
        Cpm = 1.1
    A = [0.247, 0.127, 0.0675, 0.038]
    B = [0.000657, 0.000622, 0.000504, 0.000402]
    C = [-1.186e-07, -1.69e-07, -1.44e-07, -1.27e-07]
    Cma = A[(pair.gear_box_type - 1)] + B[(pair.gear_box_type - 1)] * pair.gear_one.bs + (C[(pair.gear_box_type - 1)] * pair.gear_one.bs) ** 2
    return 1.0 + Cmc[(pair.gear_one.gear_crown - 1)] * (Cpf * Cpm + Cma * Ce[(pair.gear_one.gear_condition - 1)])
