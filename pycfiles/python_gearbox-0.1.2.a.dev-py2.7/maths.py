...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gearbox\libs\maths.py
# Compiled at: 2015-05-03 05:55:03
...
def rotate(coords, angle, center=(0, 0)):
    ...
    :return:
    """
    rotated = [ [cos(radians(angle)) * (coord[0] - center[0]) - sin(radians(angle)) * (coord[1] - center[1]) + center[0], sin(radians(angle)) * (coord[0] - center[0]) + cos(radians(angle)) * (coord[1] - center[1]) + center[1]] for coord in coords
              ]
    return rotated