...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gearbox\standards\iso.py
# Compiled at: 2015-04-25 03:11:45
...
def __c__(pair):
    x = [
     0, 0.04723, 0.15551, 0.25791, -0.00635, -0.11654, -0.00193, -0.24188, 0.00529, 0.00182]
    qp = x[1] + x[2] / pair.gear_one.zn + x[3] / pair.gear_two.zn + x[4] * pair.gear_one.x + x[5] * pair.gear_one.x / pair.gear_one.zn + x[6] * pair.gear_two.x + x[7] * pair.gear_two.x / pair.gear_two.zn + x[8] * pair.gear_one.x ** 2 + x[9] * pair.gear_two.x ** 2
...
def __yb__(gear, pair, fbx):
    ...
    if material == 'V' or material == 'St' or material == 'GGG(perl)' or material == 'GTS':
        yb = 320.0 / sigmahlimit * fbx
        if 5 < v <= 10 and yb > 25600.0 / sigmahlimit:
            yb = 25600.0 / sigmahlimit
        elif v > 10 and yb > 12800.0 / sigmahlimit:
            yb = 12800.0 / sigmahlimit
...
def __kv__(pair):
    ...
    if sr_two == 0:
        ...
        a_two = 1 / (1 - q_two ** 4)
    m_red = pi / 8 * (dm_one / db_one) ** 2 * (dm_one ** 2 / (1 / (a_one * rho_one) + 1 / (a_two * rho_two * u ** 2)))
    ...
    if 1 < epsilon_gama <= 1.5:
        ...
        cv7 = 1
    cay_one = 1.0 / 8 * (pair.gear_one.material.sh_limit / 97.0 - 18.45) ** 2 + 1.5
    cay_two = 1.0 / 8 * (pair.gear_two.material.sh_limit / 97.0 - 18.45) ** 2 + 1.5
    ...
    if n <= ns:
        ...
        kv = cv5 * bp + cv6 * bf + cv7 + (cv1 * bp + cv2 * bf + cv4 * bk + 1 - (cv5 * bp + cv6 * bf + cv7)) / 0.35 * (1.5 - n)
    return kv
...
def __khb__(pair):
    ...
    if fby * cgammabeta / (2 * fm_b) >= 1:
        khb = sqrt(2.0 * fby * cgammabeta / fm_b)
    else:
...
class Pitting(object):
    ...
    def __zh(pair):
        ...
        return sqrt(2.0 * cos(beta_b) * cos(alpha_wt) / (cos(alpha_t) ** 2.0 * sin(alpha_wt)))
    ...
    def __zb(pair):
        ...
        zb = 0
        zd = 0
        m1 = tan(alpha_wt) / sqrt((sqrt(da_one ** 2 / db_one ** 2 - 1) - 2 * pi / z_one) * (sqrt(da_two ** 2 / db_two ** 2 - 1) - (epsilon_alpha - 1) * (2 * pi) / z_two))
        m2 = tan(alpha_wt) / sqrt((sqrt(da_two ** 2 / db_two ** 2 - 1) - 2 * pi / z_two) * (sqrt(da_one ** 2 / db_one ** 2 - 1) - (epsilon_alpha - 1) * (2 * pi) / z_one))
        if beta is 0:
            if epsilon_alpha > 1:
                if m1 <= 1:
                    zb = 1
            ...
            if z_one / z_two > 1.5:
                zd = 1
        elif epsilon_alpha > 1 and epsilon_beta >= 1:
            zb = 1
            zd = zb
    ...
    def __znt(self, material, rpm):
        ...
        if material == 'NV(nitrocar)':
            y = [
             1.1, 1.1, 1.02, 1, 0.97, 0.93, 0.89, 0.85]
        elif material == 'GG' or material == 'GGG(ferr)' or material == 'NT' or material == 'NV(nitr)':
            y = [
             1.3, 1.3, 1.07, 1, 0.97, 0.93, 0.89, 0.85]
        else:
            y = [
             1.6, 1.6, 1.36, 1.14, 1, 0.98, 0.915, 0.85]
        x = [
         10000.0, 100000.0, 1000000.0, 2000000.0, 10000000.0, 100000000.0, 1000000000.0, 10000000000.0]
    ...
    def __zr(self, pair):
        ...
        if sh_limit_1 < sh_limit_2:
            ...
            sh_min = sh_limit_2
        rz = (rz_one + rz_two) / 2.0
        rz10 = rz * (10.0 / self.__r_red(pair)) ** (1.0 / 3.0)
        if 850 <= sh_min <= 1200:
...
class Bending(object):
    ...
    def __yrel(gear):
        ...
        if rz < 1:
            ...
            if material == 'St':
                return 1.07
            if material == 'GG' or material == 'GGG(ferr)' or material == 'NT' or material == 'NV(nitrocar)' or material == 'NV(nitr)':
                return 1.025
        else:
            if material == 'V' or material == 'GGG(perl)' or material == 'Eh' or material == 'IF' or material == 'GTS':
    ...
    def __ynt(pair):
        ...
        for material, nl in ([materialone, nlone], [materialtwo, nltwo]):
            if material == 'V' or material == 'GGG(perl)' or material == 'GTS' or material == 'St':
                y = [
                 2.5, 2.5, 2.5, 1.79, 1.2, 1, 0.98, 0.94, 0.895, 0.85]
            elif material == 'Eh' or material == 'IF':
                y = [
                 2.5, 2.5, 1.97, 1.53, 1.15, 1, 0.98, 0.94, 0.895, 0.85]
            elif material == 'GG' or material == 'GGG(ferr)' or material == 'NT' or material == 'NV(nitr)':
                y = [
                 1.6, 1.6, 1.4, 1.21, 1.07, 1, 0.98, 0.94, 0.895, 0.85]
            elif material == 'NV(nitrocar)':
                y = [
                 1.1, 1.1, 1.07, 1.05, 1.01, 1, 0.98, 0.94, 0.895, 0.85]
            x = [100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 3000000.0, 10000000.0, 100000000.0, 1000000000.0, 10000000000.0]
    ...
    def __ydelta(self, pair):
        ...
        xpone = 1 / 5.0 * (1 + 2 * qsone)
        xptwo = 1 / 5.0 * (1 + 2 * qstwo)
        xt = 1 / 5.0 * (1 + 5.0)
    ...
    def __ydt(pair):
        ...
        if epsilonalphan <= 2.05 or epsilonalphan > 2.05 and gprecision > 4:
            return 1
        if 2.05 < epsilonalphan <= 2.5 and gprecision <= 4:
            return -0.666 * epsilonalphan + 2.366
        if epsilonalphan > 2.5 and gprecision <= 4:
            return 0.9
    ...
    def __aux(pair):
        ...
        daone = m * (zone / cos(beta) + 2.0 * (hap + xone))
        datwo = m * (ztwo / cos(beta) + 2.0 * (hap + xtwo))
        epsilonalphan = epsilonalpha / cos(betab) ** 2
        ...
        denone = 2.0 * sqrt((sqrt((danone / 2.0) ** 2 - (dbnone / 2.0) ** 2) - pi * pair.gear_one.d * cos(beta) * cos(alpha) / zone * (epsilonalphan - 1)) ** 2 + (dbnone / 2) ** 2)
        dentwo = 2.0 * sqrt((sqrt((dantwo / 2.0) ** 2 - (dbntwo / 2.0) ** 2) - pi * dtwo * cos(beta) * cos(alpha) / ztwo * (epsilonalphan - 1)) ** 2 + (dbntwo / 2) ** 2)
        alphaenone = acos(dbnone / denone)
        alphaentwo = acos(dbntwo / dentwo)
        gamaeone = (0.5 * pi + 2.0 * tan(alpha) * xone) / znone + degrees(involute(alpha)) - degrees(involute(alphaenone))
        gamaetwo = (0.5 * pi + 2.0 * tan(alpha) * xtwo) / zntwo + degrees(involute(alpha)) - degrees(involute(alphaentwo))
        ...
        rhofone = m * (rhofp / m + 2 * gone ** 2 / (cos(thetaone) * (znone * cos(thetaone) ** 2 - 2 * gone)))
        rhoftwo = m * (rhofp / m + 2 * gtwo ** 2 / (cos(thetatwo) * (zntwo * cos(thetatwo) ** 2 - 2 * gtwo)))
        hfeone = 0.5 * m * ((cos(gamaeone) - sin(gamaeone) * tan(alphafenone)) * denone / m - znone * cos(pi / 3 - thetaone) - (gone / cos(thetaone) - rhofp / m))
        hfetwo = 0.5 * m * ((cos(gamaetwo) - sin(gamaetwo) * tan(alphafentwo)) * dentwo / m - zntwo * cos(pi / 3 - thetatwo) - (gtwo / cos(thetatwo) - rhofp / m))
        return (
         alphafenone, alphafentwo, sfnone, sfntwo, rhofone, rhoftwo, hfeone, hfetwo)
