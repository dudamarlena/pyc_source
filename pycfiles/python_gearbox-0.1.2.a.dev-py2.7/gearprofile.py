...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gearbox\libs\gearprofile.py
# Compiled at: 2015-04-26 02:19:15
"""
...
class CylindricalGearWheel(GearWheel):
    ...
    def _tooth_thickness(self, d_y):
        ...
        s_yt = d_y * ((pi + 4 * self.data.get('x_E') * tan(radians(self.data.get('alpha_n')))) / 2 / self.data.get('z') + inv(self.data.get('alpha_t')) - inv(alpha_yt))
        s_y = d_y * sin(s_yt / d_y)
        d_yc = d_y * cos(s_yt / d_y)
        return (
         s_y, d_yc)
    ...
    def _analyze_formcoords(self):
        ...
        d_f = 2 * min(half_tooth_polar[:, 0])
        d_a = 2 * max(half_tooth_polar[:, 0])
        tau = 2 * (max(half_tooth_polar[:, 1]) - min(half_tooth_polar[:, 1]))
        z = int(round(2 * pi / tau))
    ...
    def __init__(self, geardata, flankmods=None, formcoords=None):
        ...
        self.data.update({'alpha_t': degrees(atan(tan(radians(self.data.get('alpha_n'))) / cos(radians(self.data.get('beta')))))})
        self.data.update({'s_p': pi * self.data['m_n'] / 2 + 2 * self.data['m_n'] * self.data['x'] * tan(radians(self.data['alpha_n']))})
        ...
        elif not self.data.get('A_s') <= 0:
            raise ValueError('tooth thickness allowance positive')
        self.data.update({'x_E': self.data.get('x') + self.data.get('A_s') / 2 / tan(radians(self.data.get('alpha_n'))) / self.data.get('m_n')})
        if 'd_w' in self.data and 'alpha_wt' not in self.data:
            ...
            self.data.update({'alpha_wt': degrees(acos(self.data.get('d') / self.data.get('d_w') * cos(radians(self.data.get('alpha_t')))))})
        if 'alpha_wt' in self.data and 'd_w' not in self.data:
            self.data.update({'d_w': self.data.get('d') * cos(radians(self.data.get('alpha_t'))) / cos(radians(self.data.get('alpha_wt')))})
        ...
        if not formcoords:
            ...
            if 'a' in self.data and 'k' not in self.data:
                ...
                self.data.update({'k': (self.data.get('a') - self.data.get('a_d')) / self.data.get('m_n') - (self.data.get('x') + self.data.get('x_2'))})
            else:
            ...
            if 'd_f' in self.data:
                ...
                self.data.update({'d_f': self.data.get('d') + 2 * self.data.get('x_E') * self.data.get('m_n') - 2 * (self.data.get('m_n') + self.data.get('c'))})
            if 'd_a' in self.data:
                if not sign(self.data.get('d_a')) == isexternal:
                    raise ValueError('sign of tip diameter')
            else:
                self.data.update({'d_a': self.data.get('d') + 2 * self.data.get('x') * self.data.get('m_n') + 2 * self.data.get('m_n') + 2 * self.data.get('k') * self.data.get('m_n')})
            if 'h_k' in self.data:
    ...
    def _make_form_coords(self):
        ...
        inv_extension = False
        lower_index = 0
        start_rootcirc_index = lower_index + 1
        ...
        start_involute_index = end_fillet_index + 1
        ...
        start_tipcirc_index = end_chamfer_index + 1
        ...
        if isexternal > 0:
            if 'd_Ff' not in self.data:
                if self.data.get('d_f') ** 2 + 4 * self.data.get('rho_f') * self.data.get('d_f') >= self.data.get('d_b') ** 2:
                    self.data.update({'d_Ff': isexternal * sqrt((sqrt((self.data.get('d_f') + 2 * self.data.get('rho_f')) ** 2 - self.data.get('d_b') ** 2) - 2 * self.data.get('rho_f')) ** 2 + self.data.get('d_b') ** 2)})
                    ...
                    s_yt, d_yc = self._tooth_thickness(self.data.get('d_b'))
                    nu = atan(s_yt / d_yc)
                    fil_end_point = np.array([-d_tangent / 2 * sin(nu),
                     d_tangent / 2 * cos(nu)])
                ...
                s_yt, d_yc = self._tooth_thickness(self.data.get('d_Ff'))
                if abs(self.data.get('d_Ff')) >= abs(self.data.get('d_b')):
                    fil_end_point = np.array([-s_yt / 2, d_yc / 2])
                else:
                    nu = atan(s_yt / d_yc)
            ...
            def root_circle_center_func(phi):
                return fil_end_point + self.data.get('rho_f') * np.array([sin(phi[0]), cos(phi[0])]) - (self.data.get('d_f') / 2 + self.data.get('rho_f')) * np.array([sin(phi[1]), cos(phi[1])])
            ...
            phi_start = -asin(2 * rootcirc_start_point[0] / self.data.get('d_f'))
            if abs(phi_start - acos(2 * rootcirc_start_point[1] / self.data.get('d_f'))) > tol:
                phi_start = pi - phi_start
            phi_end = -asin(2 * fil_start_point[0] / self.data.get('d_f'))
            if abs(phi_end - acos(2 * fil_start_point[1] / self.data.get('d_f'))) > tol:
                phi_end = pi - phi_end
            if abs(phi_start - phi_end) > tol:
            ...
            if abs(phi_start - acos(-(fil_start_point[1] - fil_center_point[1]) / self.data.get('rho_f'))) > tol:
                phi_start = pi - phi_start
            phi_end = asin((fil_end_point[0] - fil_center_point[0]) / self.data.get('rho_f'))
            ...
            if (inv_start_point - fil_end_point).any():
                print 'involute was extended'
            d_start = isexternal * norm(inv_start_point, 2) * 2
            d_end = isexternal * norm(inv_end_point, 2) * 2
        ...
        phi_start = -asin(2 * tipcirc_start_point[0] / self.data.get('d_a'))
        if abs(phi_start - acos(2 * tipcirc_start_point[1] / self.data.get('d_a'))) > tol:
            phi_start = pi - phi_start
        phi_end = -asin(2 * tipcirc_end_point[0] / self.data.get('d_a'))
        if abs(phi_end - acos(2 * tipcirc_end_point[1] / self.data.get('d_a'))) > tol:
            phi_end = pi - phi_end
        if isexternal < 0:
            phi_end = phi_end + pi
        if abs(phi_start - phi_end) > tol:
...
class GearExport(object):
    ...
    def __init__(self, pairdata):
        ...
        gear = {'z': self.data['z'], 'x': self.data['x'], 'alpha_n': self.data['alpha_n'], 'beta': self.data['beta'], 'm_n': self.data['m_n'], 
           'rho_f': self.data['rho_f'], 'd_s': self.data['d_s'], 'c': self.data['c'], 'b': self.data['b']}
    ...
    def __set_gear(geardata):
        ...
        shaft = [[0, gear.data['d_s'] / 2],
         rotate([[0, gear.data['d_s'] / 2]], 0.5 * (gear.data['d_s'] / gear.data['z'] * 360 / gear.data['d_s']))[0]]
