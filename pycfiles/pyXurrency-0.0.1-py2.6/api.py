...
# [GCC 8.4.0]
# Embedded file name: _build\bdist.win32\egg\xurrency\api.py
# Compiled at: 2011-02-28 21:19:30
import urllib2, json, datetime, xurrency.cache
        ...
        if self._is_cached(self._create_key(base, target)):
            ...
            if jsondata['status'] == 'fail':
                ...
                if response_code == self.LIMIT_REACHED:
                    if self._create_key(base, target) not in self._cache._cache:
                ...
                elif response_code == self.API_KEY_INVALID or response_code == self.API_KEY_EXPIRED:
                    raise XurrencyAPIInvalidKeyError(message)
                else:
                    raise XurrencyError(message)
            else:
                result = jsondata['result']
                self._set_cache(self._create_key(base, target), result, datetime.datetime.strptime(result['updated_at'], '%Y-%m-%dT%H:%M:%SZ'))
        return result
