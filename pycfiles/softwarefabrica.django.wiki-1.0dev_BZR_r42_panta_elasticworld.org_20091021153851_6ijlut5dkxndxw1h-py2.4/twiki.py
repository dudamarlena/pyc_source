...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/wiki/importer/twiki.py
...
"""
import sys, os, getopt, glob, re, string, StringIO, datetime, optparse, signal as unixsignal, logging
...
SKIP_TWIKI_FILES = ('NobodyGroup.txt', 'PatternSkinUserViewTemplate.txt', 'PublishForm.txt',
                    'TWikiAdminGroup.txt', 'TWikiContributor.txt', 'TWikiGroups.txt',
                    'TWikiGuestLeftBar.txt', 'TWikiPreferences.txt', 'TWikiUsers.txt',
                    'TWikiContributor.txt', 'TWikiGroupTemplate.txt', 'TWikiGuest.txt',
                    'TWikiRegistrationAgent.txt', 'UnknownUser.txt', 'UserForm.txt',
                    'UserHomepageHeader.txt', 'UserListByDateJoined.txt', 'UserListByLocation.txt',
                    'UserListByPhotograph.txt', 'UserListHeader.txt', 'UserList.txt',
                    'UserViewTemplate.txt', 'WebAtom.txt', 'WebSearchAdvanced.txt',
                    'WebSearch.txt', 'WebTopicCreator.txt', 'WebChanges.txt', 'WebIndex.txt',
                    'WebLeftBar.txt', 'WebNotify.txt', 'WebPreferences.txt', 'WebRss.txt',
                    'WebStatistics.txt', 'WebTopicList.txt')
VERBOSITY_QUIET = 0
VERBOSITY_NORMAL = 1
VERBOSITY_VERBOSE = 2
VERBOSITY_DEBUG = 3
        ...
        while remaining:
            ...
            if m:
                ...
                if ol_el:
                    ...
                    remaining = c_io.remaining()
                    continue
            m = re.match('(?P<leading>.*?)' + '(?: ' + '(?P<nl> \\s*\\r?\\n )' + ' | ' + '(?P<meta> \\s*%META:[^\\n\\r]*%\\r?\\n )' + ' | ' + '(?P<include> \\s*%INCLUDE{[^\\n\\r]*}%\\r?\\n )' + ' | ' + '(?P<verbatim> \\s*\\<verbatim\\>[ ]*\\r?\\n.*?^\\<\\/verbatim\\>[ ]*\\r?\\n )' + ' | ' + '(?P<attachlink> \\s*\\[\\[%ATTACHURL%\\/[^\\]]*\\](?:\\[[^\\]]*\\])?\\] )' + ' | ' + '(?P<wikilink> \\s*\\[\\[.*?\\](?:\\[.*?\\])?\\] )' + ' | ' + '(?P<url> \\s*https?://\\S+ )' + ' | ' + '(?P<bold> (?=\\s?)\\*(?=\\S)[^,.;\\t\\n\\r]*?(?=\\S)\\*(?=\\s?))' + ' | ' + '(?P<boldit> (?=\\s?)\\_\\_(?=\\S)[^,.;\\t\\n\\r]*?(?=\\S)\\_\\_(?=\\s?))' + ' | ' + '(?P<it> (?=\\s?)\\_(?=\\S)[^,.;\\t\\n\\r]*?(?=\\S)\\_(?=\\s?))' + ' | ' + '(?P<boldcode> (?=\\s?)\\=\\=(?=\\S)[^,.;\\t\\n\\r]*?(?=\\S)\\=\\=(?=\\s?))' + ' | ' + '(?P<code> (?=\\s?)\\=(?=\\S)[^,.;\\t\\n\\r]*?(?=\\S)\\=(?=\\s?))' + ' | ' + '(?P<br> \\%BR\\%)' + ' | ' + '(?P<h1> ^\\s*---\\+\\s+[^\\n\\r]+\\r?\\n )' + ' | ' + '(?P<h2> ^\\s*---\\+\\+\\s+[^\\n\\r]+\\r?\\n )' + ' | ' + '(?P<h3> ^\\s*---\\+\\+\\+\\s+[^\\n\\r]+\\r?\\n )' + ' | ' + '(?P<h4> ^\\s*---\\+\\+\\+\\+\\s+[^\\n\\r]+\\r?\\n )' + ' | ' + '(?P<hr> ^\\s*---+\\s*\\r?\\n ))' + '(?P<remaining>.*)', remaining, re.DOTALL | re.MULTILINE | re.VERBOSE)
            if m:
                leading = m.group('leading')
                remaining = m.group('remaining')
                if leading and leading != '' and leading != '':
                    if isinstance(self.rootdom, Document):
                ...
                if m.group('nl'):
                    ...
                    contents = re.match('\\s*\\<verbatim\\>[ ]*\\r?\\n(.*)^\\<\\/verbatim\\>[ ]*\\r?\\n', m.group('verbatim'), re.DOTALL | re.MULTILINE).group(1)
                    contents_el = CodeBlock(text=contents)
                elif m.group('url'):
                    contents = re.match('\\s*(https?://\\S+)', m.group('url')).group(1)
                    contents_el = Link(contents)
                elif m.group('bold'):
                    contents = re.match('\\*(.*?)\\*', m.group('bold')).group(1)
                    contents_el = Strong()
                    TWikiConverter(contents_el, contents)
                elif m.group('it'):
                    contents = re.match('\\_(.*?)\\_', m.group('it')).group(1)
                    contents_el = Emphasis()
                    TWikiConverter(contents_el, contents)
                elif m.group('boldit'):
                    contents = re.match('\\_\\_(.*?)\\_\\_', m.group('boldit')).group(1)
                    contents_el = EmphasisStrong()
                    TWikiConverter(contents_el, contents)
                elif m.group('boldcode'):
                    contents = re.match('\\=\\=(.*?)\\=\\=', m.group('boldcode')).group(1)
                    inner_el = CodeSpan()
                    contents_el = Strong(inner_el)
                    TWikiConverter(inner_el, contents)
                elif m.group('code'):
                    contents = re.match('\\=(.*?)\\=', m.group('code')).group(1)
                    ...
                    m2 = re.match('\\s*\\[\\[%ATTACHURL%\\/(.*?)\\]\\[(.*?)\\]\\]', m.group('attachlink'))
                    if m2:
                    ...
                    m2 = re.match('\\s*\\[\\[(.*?)\\]\\[(.*?)\\]\\]', m.group('wikilink'))
                    if m2:
                        (target, desc) = m2.groups()
                    else:
                        m2 = re.match('\\s*\\[\\[(.*?)\\]\\]', m.group('wikilink'))
                ...
                if close_paragraph:
                    if target_el and isinstance(target_el, Paragraph):
                        if target_el.children is None or len(target_el.children) == 0:
                            ...
                            if isinstance(cld, Text) and (cld.text == '' or cld.text == ''):
                                p = target_el.parent
                                p.removeChild(target_el)
                                in_paragraph = False
                                close_paragraph = False
...
def main():
    ...
    Program entry point
    """
    parser = optparse.OptionParser(version='softwarefabrica TWiki wiki import tool (%%prog) version %s (lib. %s)' % (__version__, version.get_version()), usage='%prog [options]\n\nImport a TWiki wiki into a softwarefabrica wiki.\n')
    parser.add_option('-s', '--settings', help='specify SETTINGS as the django settings module', action='store', dest='django_settings', default='', metavar='SETTINGS')
    parser.add_option('-i', '--input-dir', help='specify INPUTDIR as the input directory (default: %s)' % repr(DEFAULT_INPUT_DIR), action='store', dest='inputdir', default=DEFAULT_INPUT_DIR, metavar='INPUTDIR')
    parser.add_option('-a', '--attachment-dir', help='specify ATTACHMENTDIR as the input attachment directory (default: determined)', action='store', dest='attachmentdir', default=None, metavar='ATTACHMENTDIR')
    parser.add_option('-t', '--target', '--target-wiki', help='specify WIKI as the destination wiki (default: %s)' % repr(DEFAULT_TARGET_WIKI), action='store', dest='targetwiki', default=DEFAULT_TARGET_WIKI, metavar='WIKI')
    parser.add_option('-o', '--overwrite', help='force overwrite of existing pages', action='store_true', dest='overwrite', default=False)
    ...
    parser.add_option('--test', help='perform internal testing.', action='store_true', dest='test', default=False)
