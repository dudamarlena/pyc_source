...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/wiki/models.py
...
import datetime
PARENT_SEPARATOR = '|'
URL_PARENT_SEPARATOR = '|'
WIKI_PAGE_SEPARATOR = '.'
URL_WIKI_PAGE_SEPARATOR = '.'
...
class Wiki(models.Model):
    ...
    def get_absolute_url(self):
        return (
         'wiki-page-list', (), {'wikiname': self.fullname_url})
    ...
    def get_full_name(self, separator=PARENT_SEPARATOR):
        ...
        if self.parent is not None:
            return '%s%s%s' % (self.parent.get_full_name(separator=separator), separator, self.name)
        return self.name
...
class Page(models.Model):
    ...
    def get_absolute_url(self):
        return (
         'wiki-page-detail', (), {'wikiname': self.wiki.fullname_url, 'pagename': self.name})
    ...
    def get_absolute_name(self, wiki_separator=PARENT_SEPARATOR, page_separator=PARENT_SEPARATOR, separator=WIKI_PAGE_SEPARATOR):
        ...
        """
        return '%s%s%s' % (self.wiki.get_full_name(separator=wiki_separator), separator, self.get_full_name(separator=page_separator))
...
class PageContent(models.Model):
    ...
    def __unicode__(self):
        return '%s - r%s' % (self.page, self.rev)
...
class Attachment(models.Model):
    ...
    name = models.CharField(_('name'), max_length=40, blank=False, db_index=True)
    desc = models.CharField(_('description'), max_length=200, blank=True)
    attachment = models.FileField(_('attachment'), upload_to=page_attachment_upload_to, max_length=200)
