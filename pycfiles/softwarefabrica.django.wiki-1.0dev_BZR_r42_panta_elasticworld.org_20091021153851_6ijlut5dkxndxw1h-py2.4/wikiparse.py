...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/wiki/wikiparse.py
...
class WikiParser(object):
    ...
    def __init__(self, pagecontent, src_text, do_html_sanitization=False, mdwn_extensions=['footnotes', 'tables']):
    ...
    def markup_parse(self, text):
        ...
        if hasattr(markdown, 'version'):
            ...
            if getattr(markdown, 'version_info', None) < (1, 7):
                ...
                md = markdown.Markdown(force_unicode(text), extensions=extensions, safe_mode=safe_mode)
    ...
    def parse_wiki_tags(self, text):
        ...
        remaining = text
        while remaining:
            m = re.match('(?P<leading>.*?) (?:' + '(?P<mdwn_link> \\[[A-Za-z0-9_\\+ \t]+\\]\\([A-Za-z0-9:\\/-_%&\\@\\?.=]+\\))' + ' | ' + '(?P<mdwn_img> \\!\\[[A-Za-z0-9_\\+ \t]+\\]\\([A-Za-z0-9:\\/-_%&\\@\\?.= \t"]+\\))' + ' | ' + '(?P<mdwn_img_ref> \\!\\[[A-Za-z0-9_\\+ \t]+\\]\\[[A-Za-z0-9:\\/-_%&\\@\\?.= \t"]+\\])' + ' | ' + '(?P<wikiword> [A-Z][a-z]+[A-Z][a-zA-Z0-9_]+)' + ' | ' + '(?P<attachmentlink> \\[\\[attachment:[^\\]\\|]+(?:\\|[^\\]]+)?\\]\\])' + ' | ' + '(?P<wikilink> \\[\\[[^\\]\\|]+(?:\\|[^\\]]+)?\\]\\])' + ' ) (?P<remaining>.*)', remaining, re.MULTILINE | re.VERBOSE | re.DOTALL)
            if m:
                ...
                if m.group('mdwn_link'):
                    ...
                    try:
                        ...
                        link = '<a href="%s"%s>%s</a>' % (url, a_class_str, wikiword)
                        if link:
                            parsed += link
                elif m.group('attachmentlink'):
                    attlink = m.group('attachmentlink')
                    (attname, linktitle) = re.match('\\[\\[attachment:(?P<attname>[^\\]\\|]+)(?:\\|(?P<atttitle>[^\\]]+))?\\]\\]', attlink).groups()
                    if attname and re.match('".*"', attname):
                        attname = attname[1:-1]
                    if linktitle and re.match('".*"', linktitle):
                        linktitle = linktitle[1:-1]
                    if linktitle:
                        linktitle = sanitize_html(linktitle, valid_tags='', valid_attrs='')
                    if linktitle:
                        linktitle = re.sub('\\`', '', linktitle, 0)
                    l_wikiname = wikiname
                    ...
                    try:
                        ...
                        if a_class:
                            a_class_str = ' class="%s"' % a_class
                        if attachment:
                            link = '<a href="%s"%s%s>%s</a></span>' % (url, a_class_str, a_title_str, linktitle)
                        else:
                            link = '<a href="%s"%s%s>%s</a></span>' % (url, a_class_str, a_title_str, l_attname)
                        if link:
                            parsed += link
                elif m.group('wikilink'):
                    wlink = m.group('wikilink')
                    (wikiword, linktitle) = re.match('\\[\\[(?P<wikiword>[^\\]\\|]+)(?:\\|(?P<wikititle>[^\\]]+))?\\]\\]', wlink).groups()
                    if linktitle and re.match('".*"', linktitle):
                        linktitle = linktitle[1:-1]
                    if linktitle:
                        linktitle = sanitize_html(linktitle, valid_tags='', valid_attrs='')
                    ...
                    l_pagename = wikiword
                    l_external = False
                    if wikiword.startswith('http://') or wikiword.startswith('https://') or wikiword.startswith('ftp://') or wikiword.startswith('ftps://') or wikiword.startswith('svn://') or wikiword.startswith('bzr://') or wikiword.startswith('sftp://') or wikiword.startswith('bzr+sftp://'):
                        l_external = True
                    ...
                    try:
                        ...
                        if a_class:
                            a_class_str = ' class="%s"' % a_class
                        link = '<a href="%s"%s%s>%s</a>' % (url, a_class_str, a_title_str, linktext)
                        if link:
                            parsed += link
            else:
                break
