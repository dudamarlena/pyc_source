...
# Embedded file name: build/bdist.macosx-10.8-intel/egg/borda/client.py
# Compiled at: 2013-08-05 13:27:40
__doc__ = 'This module sends requests to a Borda server in order to create an election,\nregister candidates and voters, and issue votes on behalf of the voters.'
import argparse, json, sys, logging, requests
...
def add_candidate(args):
    ...
    try:
        candidate_request = requests.put(resource('election'), data=candidate)
...
def add_voter(args):
    ...
    try:
        voter_request = requests.post(resource('vote'), data=voter)
...
def voter_votes(args):
    ...
    voter = {'name': args.name, 
       'votes': args.votes}
    try:
        voter_votes_request = requests.put(resource('vote'), data=voter)
...
def run():
    ...
    parse_vote.add_argument('-v', '--votes', required=True, type=comma_separated_strings)
