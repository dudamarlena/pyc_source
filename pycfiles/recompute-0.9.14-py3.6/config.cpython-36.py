...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/recompute/config.py
# Compiled at: 2019-03-26 00:42:28
...
class ConfigManager(object):
    __doc__ = 'ConfigManager encapsulates the global config file.'
    ...
    """
        if self.config_exists() and not force and not configfile:
            ...
            config.set('general', 'instance', '0')
            config.set('general', 'remote_home', 'projects/')
            configfile = configfile if configfile else self.configfile
    ...
    """
        return [self.config[section] for section in self.config.sections() if 'instance ' in section]
    ...
    """
        idx = len([sec for sec in self.config.sections() if 'instance ' in sec])
        self.config['instance {}'.format(idx)] = {'username':instance.username, 
         'host':instance.host, 
