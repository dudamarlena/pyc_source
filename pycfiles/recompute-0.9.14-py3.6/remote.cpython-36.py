...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/recompute/remote.py
# Compiled at: 2019-03-25 07:23:03
# Size of source mod 2**32: 19211 bytes
"""remote.py
...
class Remote(object):
    __doc__ = 'Remote models the remote machine'
    ...
    """
        ...
        if not cache:
            self.make_dirs()
        self.logfile = os.path.join(self.remote_dir, '{}.log'.format(self.bundle.name))
        self.local_logfile = os.path.join(self.bundle.path, self.logfile.split('/')[(-1)])
    ...
    """
        ...
        void_as_dict = {'instance':self.instance, 
         'bundle':self.bundle, 
    ...
    """
        ...
        _body = cmd.SSH_MAKE_DIR.format(username=(self.instance.username),
    ...
    """
        _header = cmd.SSH_HEADER.format(password=(self.instance.password))
        _body = cmd.RSYNC.format(deps_file=(self.bundle.db),
    ...
    """
        ...
        self.copy_file_to_remote(os.path.join(self.bundle.path, runner), self.remote_dir)
    ...
    def execute_command(self, cmdstr, run_async=False, log=False, logfile=None, bypass_subprocess=True):
    ...
    """
        ...
        if run_async:
            ...
            return process.remote_execute(cmdstr, (self.instance), bypass_subprocess=bypass_subprocess)
    ...
    """
        ...
        _header = cmd.SSH_HEADER.format(password=(self.instance.password))
        _body = cmd.SCP_TO_REMOTE.format(username=(self.instance.username),
          host=(self.instance.host),
    ...
    """
        ...
        _header = cmd.SSH_HEADER.format(password=(self.instance.password))
        _body = cmd.SCP_FROM_REMOTE.format(username=(self.instance.username),
          host=(self.instance.host),
          remotepath=remotepath,
          localpath=localpath)
        copy_cmd = ' '.join([_header, _body])
    ...
    """
        if len(packages) == 0:
            ...
            return
        pip_install_cmd = 'python3 -m pip install --user {}'.format(' '.join(packages))
    ...
    def get_session(self):
        ...
        os.system(cmd.SSH_INTO_REMOTE_DIR.format(username=(self.instance.username),
    ...
    """
        ...
        commands = [
         cmd.JUPYTER_SERVER.format(port_num=server_port_num)]
        notebook_server_pid, output = self.async_execute(commands, logfile='/dev/null',
          name=notebook_server_name)
        logger.info('\tStarted notebook server in remote machine :{}'.format(server_port_num))
        print('{username}@{host}:{port}'.format(username=(self.instance.username),
          host=(self.instance.host),
          port=server_port_num))
        client_port_num = utils.rand_client_port()
        cmd_local = cmd.JUPYTER_CLIENT.format(username=(self.instance.username),
          password=(self.instance.password),
          host=(self.instance.host),
        ...
        if run_async:
            print('Async Execution not implemented yet!')
        pid, output = process.execute(cmd_local)
        if not pid and not output:
    ...
    """
        if force:
            command = ' '.join([
             ...
             cmd.SSH_EXEC.format(username=(self.instance.username), host=(self.instance.host), cmd=cmd.PROCESS_LIST_LINUX.format(username=(self.instance.username)))])
            ...
            self.processes.extend([('zombie/spawn', pid) for pid in pids if pid not in known_pids])
            self.cache_()
    ...
    """
        ...
        if len(processes) > 0:
            ...
            if len(procs_to_kill) > 0:
                process.kill_remote_process([p[(-1)] for p in procs_to_kill],
