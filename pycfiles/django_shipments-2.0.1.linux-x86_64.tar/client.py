...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/ups/client.py
# Compiled at: 2014-08-02 20:36:07
import urllib, urlparse, os, suds, logging
...
class UPSClient(object):
    ...
    def _create_shipment(self, client, packages, shipper_address, recipient_address, box_shape, namespace='ns3', create_reference_number=True, can_add_delivery_confirmation=True):
        ...
        shipment.Shipper.Address.PostalCode = shipper_address.zipCode
        ...
        shipment.ShipTo.Address.PostalCode = recipient_address.zipCode
    ...
    def rate(self, packages, shipper, recipient, packaging_type):
        ...
        try:
            response = client.service.ProcessRate(Request=request, PickupType=pickup, CustomerClassification=classification, Shipment=shipment)
            ...
            for r in response.RatedShipment:
                ...
                info.append({'service': service_lookup.get(r.Service.Code, unknown_service), 
                   'package': '', 
                   'delivery_day': '', 
                   'cost': cost, 
                   'currency': currency})
