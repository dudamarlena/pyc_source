...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/helpdesk/views/public.py
...
def homepage(request):
    if not request.user.is_authenticated:
        if helpdesk_settings.HELPDESK_REDIRECT_TO_LOGIN_BY_DEFAULT:
    ...
    return render_to_resp(request=request, template_name='helpdesk/public_homepage.html', context={'form':form, 
            ...
            if ticket:
                ...
                if 'close' in request.GET:
                    if ticket.status == Ticket.RESOLVED_STATUS:
                        ...
                        request.POST = {'new_status':Ticket.CLOSED_STATUS,  'public':1, 
                         'title':ticket.title, 
                ...
                if helpdesk_settings.HELPDESK_NAVIGATION_ENABLED:
                    redirect_url = reverse('helpdesk_view', args=[ticket_id])
                return render_to_resp(request=request, template_name='helpdesk/public_view_ticket.html', context={'ticket':ticket, 
                 'helpdesk_settings':helpdesk_settings, 
                 'next':redirect_url})
    return render_to_resp(request=request, template_name='helpdesk/public_view_form.html', context={'ticket':ticket, 
     'email':email, 
...
def change_language(request):
    ...
    return render_to_resp(request=request, template_name='helpdesk/public_change_language.html', context={'next': return_to})