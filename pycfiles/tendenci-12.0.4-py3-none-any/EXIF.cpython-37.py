...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/photos/utils/EXIF.py
# Compiled at: 2020-03-30 17:48:04
# Size of source mod 2**32: 60745 bytes
...
FIELD_TYPES = ((0, 'X', 'Proprietary'), (1, 'B', 'Byte'), (1, 'A', 'ASCII'), (2, 'S', 'Short'),
               (4, 'L', 'Long'), (8, 'R', 'Ratio'), (1, 'SB', 'Signed Byte'), (1, 'U', 'Undefined'),
               (2, 'SS', 'Signed Short'), (4, 'SL', 'Signed Long'), (8, 'SR', 'Signed Ratio'))
...
class EXIF_header:
    ...
    def dump_IFD(self, ifd, ifd_name, dict=EXIF_TAGS, relative=0, stop_tag='UNDEF'):
        ...
        for i in range(entries):
            ...
            field_type = detailed or tag in IGNORE_TAGS or self.s2n(entry + 2, 2)
    ...
    def decode_maker_note(self):
        ...
        if 'NIKON' in make:
            if note.values[0:7] == [78, 105, 107, 111, 110, 0, 1]:
                ...
                if note.values[0:7] == [78, 105, 107, 111, 110, 0, 2]:
                    if self.debug:
                        print('Looks like a labeled type 2 Nikon MakerNote')
                    if note.values[12:14] != [0, 42]:
                        if note.values[12:14] != [42, 0]:
                            raise ValueError("Missing marker tag '42' in MakerNote.")
                    self.dump_IFD((note.field_offset + 10 + 8), 'MakerNote', dict=MAKERNOTE_NIKON_NEWER_TAGS,
                      relative=1)
                else:
                    if self.debug:
...
def process_file(f, stop_tag='UNDEF', details=True, strict=False, debug=False):
    ...
    if data[0:4] in ('II*\x00', 'MM\x00*'):
        ...
        if data[0:2] == 'ÿØ':
            while data[2] == 'ÿ' and data[6:10] in ('JFIF', 'JFXX', 'OLYM', 'Phot'):
                length = ord(data[4]) * 256 + ord(data[5])
                f.read(length - 8)
                data = 'ÿ\x00' + f.read(10)
                fake_exif = 1
            ...
            if debug:
                ...
                if thumb:
                    if thumb.printable == 'Uncompressed TIFF':
                ...
                if 'EXIF MakerNote' in hdr.tags:
                    if 'Image Make' in hdr.tags and detailed:
