...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/forums/templatetags/pybb_tags.py
# Compiled at: 2020-03-30 17:48:04
# Size of source mod 2**32: 9293 bytes
import inspect, math, time, warnings
...
import django.utils.timezone as tznow
try:
    import pytils
    pytils_enabled = True
except ImportError:
...
def pybb_topic_moderated_by(topic, user):
    ...
    """
    warnings.warn('pybb_topic_moderated_by filter is deprecated and will be removed in later releases. Use pybb_may_moderate_topic(user, topic) filter instead', DeprecationWarning)
...
def pybb_editable_by(post, user):
    ...
    """
    warnings.warn('pybb_editable_by filter is deprecated and will be removed in later releases. Use pybb_may_edit_post(user, post) filter instead', DeprecationWarning)
...
def pybb_is_topic_unread(topic, user):
    ...
    unread = not ForumReadTracker.objects.filter(forum=(topic.forum),
      user=(user.id),
      time_stamp__gte=last_topic_update).exists()
    unread &= not TopicReadTracker.objects.filter(topic=topic,
      user=(user.id),
      time_stamp__gte=last_topic_update).exists()
    return unread
...
def pybb_topic_unread(topics, user):
    ...
    if user.is_authenticated:
        ...
        forum_marks = dict([(m.forum_id, m.time_stamp) for m in ForumReadTracker.objects.filter(user=user, forum__in=forums_ids)])
        if len(forum_marks):
            for topic in topic_list:
...
def pybb_forum_unread(forums, user):
    ...
    if user.is_authenticated:
        ...
        forum_marks = ForumReadTracker.objects.filter(user=user,
          forum__in=forum_list).select_related('forum')
        ...
        for mark in forum_marks:
            ...
            if curr_forum.updated is None or curr_forum.updated <= mark.time_stamp:
                forum_dict[mark.forum.id].unread = any((f.unread for f in pybb_forum_unread(curr_forum.child_forums.all(), user))) or False
...
def load_perms_filters():
    ...
    for method in inspect.getmembers(perms):
        if not inspect.ismethod(method[1]) or inspect.getfullargspec(method[1]).args[0] == 'self' and (method[0].startswith('may') or method[0].startswith('filter')):
            if len(inspect.getfullargspec(method[1]).args) == 3:
                register.filter('%s%s' % ('pybb_', method[0]), partial(method[0], perms))
            elif len(inspect.getfullargspec(method[1]).args) == 2:
