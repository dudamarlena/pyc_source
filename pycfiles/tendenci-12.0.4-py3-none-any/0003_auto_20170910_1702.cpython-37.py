...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/jobs/migrations/0003_auto_20170910_1702.py
# Compiled at: 2020-03-30 17:48:04
# Size of source mod 2**32: 9616 bytes
import os
...
def migrate_customized_jobs_templates():
   ...
   {% if request.user.is_superuser %}
    ...
    for file_path in files_list:
        if os.path.isfile(file_path):
            with open(file_path, 'r') as (f):
                content = f.read()
                p = '{0}([\\d\\D\\s\\S\\w\\W]*?){1}([\\d\\D\\s\\S\\w\\W]*?){2}'.format(re.escape('<fieldset class="boxy-grey" >'), re.escape('{{ categoryform|styled_form }}'), re.escape('</fieldset>'))
                content = re.sub(p, '', content)
                p = '{0}\\s*{1}'.format(re.escape('<script type="text/javascript" src="{% static \'js/email-verification.js\' %}">'), re.escape('</script>'))
                content = re.sub(p, '{0}\n{1}'.format('<script type="text/javascript" src="{% static \'js/email-verification.js\' %}"> </script>', '<script type="text/javascript">{% include \'jobs/include/get_subcategories.js\' %} </script>)'), content)
    ...
    if os.path.isfile(file_path):
        find_replace_list = [
         ('{% with job.category_set as job_cat %}', '{% with job.cat as job_cat %}'),
        ...
        with open(file_path, 'r') as (f):
            ...
            p = '{0}\\s+{1}\\s+{2}'.format(re.escape('<li>'), re.escape('<a href="{% url \'category.update\' job.opt_app_label job.opt_module_name job.pk %}">{% trans "Edit Categories" %}</a>'), re.escape('</li>'))
            content = re.sub(p, '', content)
        with open(file_path, 'w') as (f):
            f.write(content)
    ...
    if os.path.isfile(file_path):
        with open(file_path, 'r') as (f):
            content = f.read()
            content = content.replace("var $catAndSubcatSelect = $('#id_categories, #id_subcategories')", "var $catAndSubcatSelect = $('#id_cat, #id_sub_cat')")
        with open(file_path, 'w') as (f):
            f.write(content)
    ...
    if os.path.isfile(file_path):
        with open(file_path, 'r') as (f):
            content = f.read()
            p = '{0}\\s+{1}\\s+{2}\\s+{3}\\s+{4}'.format(re.escape('<li class="content-item">'), re.escape('<span class="app-name">'), re.escape('<a href="{% url \'category.update\' app_object.opt_app_label job.opt_module_name app_object.pk %}">{% trans "Edit Categories" %}</a>'), re.escape('</span>'), re.escape('</li>'))
            manage_cat = '\n            {% if request.user.is_superuser %}\n            <li class="content-item">\n                <span class="app-name">\n                    <a href="{% url \'admin:jobs_category_changelist\' %}">{% trans "Manage Categories" %}</a>\n                </span>\n            </li>\n            {% endif %}\n            '
...
def migrate_categories_data(apps, schema_editor):
    ...
    import tendenci.apps.jobs.models as JobCategory
    for job in Job.objects.all():
        ...
        for cat_item in cat_items:
            if cat_item.category:
                ...
                if cat_name:
                    job_cat, = JobCategory.objects.filter(name=cat_name, parent__isnull=True)[:1] or [None]
                    if not job_cat:
                        ...
                        if sub_cat_name:
                            job_sub_cat, = JobCategory.objects.filter(name=sub_cat_name, parent=job_cat)[:1] or [None]
                            job_sub_cat = job_sub_cat or JobCategory(name=sub_cat_name, slug=sub_cat_name)
