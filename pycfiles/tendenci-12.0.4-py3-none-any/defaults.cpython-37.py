...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/forums/defaults.py
# Compiled at: 2020-03-30 17:48:04
# Size of source mod 2**32: 8080 bytes
...
PYBB_MAX_AVATAR_SIZE = getattr(settings, 'PYBB_MAX_AVATAR_SIZE', 51200)
...
PYBB_ATTACHMENT_SIZE_LIMIT = getattr(settings, 'PYBB_ATTACHMENT_SIZE_LIMIT', 1048576)
...
PYBB_DEFAULT_AVATAR_URL = getattr(settings, 'PYBB_DEFAULT_AVATAR_URL', static('pybb/img/default_avatar.jpg'))
...
PYBB_SMILES = getattr(settings, 'PYBB_SMILES', {'&gt;_&lt;':'angry.png', 
 ':.(':'cry.png', 
 'o_O':'eyes.png', 
 '[]_[]':'geek.png', 
 '8)':'glasses.png', 
...
callable_warning = '%(setting_name)s should not be a callable anymore but a path to the parser classes.ex : myproject.markup.CustomBBCodeParser. It will stop working in next pybbm release.'
wrong_setting_warning = '%s setting will be removed in next pybbm version. Place your custom quote functions in markup class and override PYBB_MARKUP_ENGINES_PATHS/PYBB_MARKUP settings'
...
def getsetting_with_deprecation_check(all_settings, setting_name):
    ...
    for value in values:
        if isinstance(value, string_types):
            continue
        warnings.warn(callable_warning % {'setting_name': setting_name}, DeprecationWarning)
...
if PYBB_MARKUP:
    if PYBB_MARKUP not in PYBB_MARKUP_ENGINES:
        if not PYBB_MARKUP_ENGINES:
            warnings.warn('There is no markup engines defined in your settings. Default pybb.base.BaseParser will be used.Please set correct PYBB_MARKUP_ENGINES_PATHS and PYBB_MARKUP settings.', DeprecationWarning)
            PYBB_MARKUP = None
        else:
            if 'bbcode' in PYBB_MARKUP_ENGINES:
                PYBB_MARKUP = 'bbcode'
            else:
    ...
    if not hasattr(settings, 'PYBB_BODY_CLEANERS'):
        PYBB_BODY_CLEANERS = [
         'tendenci.apps.forums.markup.base.rstrip_str', 'tendenci.apps.forums.markup.base.filter_blanks']
    else:
...
def bbcode(s):
    warnings.warn(bad_function_warning % {'bad':'tendenci.apps.forums.defaults.bbcode', 
     'good':'pytendenci.apps.forums.markup.bbcode.BBCodeParser'}, DeprecationWarning)
    from markup.bbcode import BBCodeParser
    return BBCodeParser().format(s)


def markdown(s):
    warnings.warn(bad_function_warning % {'bad':'tendenci.apps.forums.defaults.markdown', 
     'good':'tendenci.apps.forums.markup.markdown.MarkdownParser'}, DeprecationWarning)
...
def _render_quote(name, value, options, parent, context):
    warnings.warn('pybb.defaults._render_quote function is deprecated. This function is internal of new pybb.markup.bbcode.BBCodeParser class.', DeprecationWarning)
    from markup.bbcode import BBCodeParser
    return BBCodeParser()._render_quote(name, value, options, parent, context)


def smile_it(s):
    warnings.warn(bad_function_warning % {'bad':'tendenci.apps.forums.defaults.smile_it', 
     'good':'tendenci.apps.forums.markup.base.smile_it'}, DeprecationWarning)
    import markup.base as real_smile_it
