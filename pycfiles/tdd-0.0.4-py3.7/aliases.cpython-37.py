...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tdd/aliases.py
# Compiled at: 2019-06-08 09:42:53
...
__all__ = [
 'aliasDictionary', 'mapSeq2Standard', 'standard_aliases']
...
def aliasDictionary(sequence, aliases):
    ...
    inverse_dict = dict()
    _ = [inverse_dict.update({alias: aliastuple[0]}) for aliastuple in aliases.items() for alias in aliastuple[1]]
    testDict = dict(((s.lower(), s) for s in sequence))
    aliasedSet = set(inverse_dict.keys()) & set(testDict.keys())
    aliasDict = dict(((testDict[key], inverse_dict[key]) for key in aliasedSet if testDict[key] != inverse_dict[key]))
    return aliasDict
