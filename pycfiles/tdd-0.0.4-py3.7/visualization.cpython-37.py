...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tdd/visualization.py
# Compiled at: 2019-04-15 09:04:07
# Size of source mod 2**32: 30716 bytes
"""
...
import pyproj, healpy as hp
from .trig import pixelsForAng, convertToCelestialCoordinates
from .healpix import healpix_boundaries
from matplotlib.legend_handler import HandlerPatch
import matplotlib.patches as mpatches
__all__ = [
 'plot_south_steradian_view', 'HPTileVis', 'split_PolygonSegments',
...
class MilkyWayExtension(object):
    ...
    def mw_boundaries(self):
        ...
        """
        phi = np.arange(0.0, 2.0 * np.pi, 0.1)
        theta_l = np.ones_like(phi) * 110 * np.pi / 180.0
        theta_h = np.ones_like(phi) * 70 * np.pi / 180.0
    ...
    def mw_polygon(self):
        ...
        x, y = np.concatenate((x_l, x_h[::-1])), np.concatenate((y_l, y_h[::-1]))
        p = Polygon((zip(x, y)), color=(self.color),
          fill=(self.fill),
          alpha=(self.alpha),
          lw=(self.lw),
...
class ObsVisualization(with_metaclass(abc.ABCMeta, object)):
    __doc__ = 'Abstract base class for describing simulations'
...
class AllSkySNVisualization(ObsVisualization):
    __doc__ = ' Class implementing simplest ObsVisualization)'

    def __init__(self, bandColorDict, radius_deg=4.0, showMW=True, colorCodeRedshifts=True, showVisibleFields=False, showVarScatter=False):
    ...
    def generate_mw_polygons(self, m, fill=True, color='y', alpha=0.1, lw=0.0, edgecolor='y', zorder=15, min_lon=-90.0, ax=None):
        """obtain polygons representing the extent of the MW"""
        mwext = MilkyWayExtension(m=m, fill=fill, color=color, alpha=alpha, lw=lw,
          edgecolor=edgecolor,
          ax=ax,
          zorder=zorder,
          min_lon=min_lon)
    ...
    def _hack_legend(self, ax, colors, labels, bbox=(1, 1), loc='best'):
        """ hack legend """
        cs, texts = [], []
        for colors, text in zip(('g', 'r', 'y'), ('ZTF g band', 'ZTF r band', 'ZTF i band')):
            c = mpatches.Circle((0.5, 0.5), 0.25, facecolor='w', edgecolor=colors,
              linewidth=1)
            cs.append(c)
            texts.append(text)

        l = ax.legend(cs, texts, handler_map={mpatches.Circle: HandlerEllipse()},
          loc=loc,
          bbox_to_anchor=bbox)
        return l
    ...
    def generate_image_bg(self, projection='moll', drawmapboundary=True, bg_color='k', mwcolor='w', mwfill=False, mw_alpha=1.0, mw_edgecolor='w', mw_lw=1.0, figsize=(12, 6), **kwargs):
        """Generate a figure axis, and a Basemap child instance"""
        fig, ax = plt.subplots(figsize=figsize)
        m = AllSkyMap(projection=projection, lon_0=0.0, lat_0=0.0, ax=ax,
          celestial=True)
        _ = m.drawparallels(np.arange(-91.0, 91.0, 60.0))
        _ = m.drawmeridians(np.arange(-180.0, 181.0, 60.0))
        ...
        if self.show_mw:
            if mwfill:
                ...
                min_lon = -90
            polygons, bounds = self.generate_mw_polygons(m, color=mwcolor, alpha=mw_alpha,
              fill=mwfill,
              lw=mw_lw,
              min_lon=min_lon,
            ...
            if mwfill:
                ...
                _ = ax.plot((np.roll(x_h, break_h)), (np.roll(y_h, break_h)), ls='dashed',
                  color=mw_edgecolor,
                  lw=mw_lw,
                  label='MW')
                _ = ax.plot((np.roll(x_l, break_l)), (np.roll(y_l, break_l)), ls='dashed',
                  color=mw_edgecolor,
                  lw=mw_lw)
        return (
         fig, ax, m)

    def generate_camera(self, lon_0, lat_0, m, ax, band='g', radius_deg=4.0, default_color='k'):
        ...
        camera_polygons = (m.tissot)(lon_0=lon_0, lat_0=lat_0, radius_deg=radius_deg, npts=100, 
         ax=ax, add_patch=True, **dict(fill=False, edgecolor=c, lw=2))
    ...
    def generate_image(self, ra, dec, radius_deg, mjd=None, npts=100, band='g', projection='moll', drawmapboundary=True, mwColor='w', mwAlpha=1.0, mwEdgeColor='w', mwLw=1.0, mwFill=False, bg_color='k', alpha=0.5, vfcolor='k', cmap=plt.cm.Reds, sndf=None, zlow=0.0, zhigh=0.2, surveystart=None, bbox=(1, 1), loc=None, **kwargs):
        """
        Use methods above to create an image of the sky and optionally save
        it.
        """
        fig, ax, m = (self.generate_image_bg)(projection=projection, drawmapboundary=drawmapboundary, 
         mwcolor=mwColor, 
         mw_alpha=mwAlpha, mw_edgecolor=mwEdgeColor, 
         mwfill=mwFill, 
         mw_lw=mwLw, 
         bg_color=bg_color, **kwargs)
        if self.show_visible_fields:
            visible_polygons = (self.get_visible_field_polygons)(mjd, m, facecolor=vfcolor, 
             alpha=alpha, **kwargs)
            for poly in visible_polygons:
                _ = ax.add_patch(poly)

        else:
            camera_polygons = self.generate_camera(lon_0=ra, lat_0=dec, m=m, ax=ax, band=band)
            xx = None
            if self.show_var_scatter:
                ...
                if self.colorCodeRedshifts:
                    ...
                    xx = ax.scatter(x, y, lw=0.0, s=s, c=z,
                      zorder=10,
                      norm=normalize,
                      cmap=cmap)
                    cb = (plt.colorbar)(xx, ax=ax, norm=normalize, **dict(orientation='horizontal'))
                    cb.set_label('redshift')
                else:
                    ax.scatter(x, y, s=(s.values), c='w', edgecolors='w', zorder=10)
        label = self.label_time_image(mjd, surveystart)
        ax.set_title(label)
        cvals = (
         mwColor, vfcolor)
        names = ('MW Region', 'Desired SN fields')
        legend = self._hack_legend(ax, colors=cvals, labels=names, bbox=bbox, loc=loc)
        return (fig, ax, m, xx)
...
def split_PolygonSegments(lon, lat, lon_split=180.0, epsilon=1e-06):
...
class AllSkyMap(Basemap):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lon_split = 180.0
        self.epsilon = 1e-10

    def tissot(self, lon_0, lat_0, radius_deg, npts, ax=None, epsilon=1e-10, add_patch=True, **kwargs):
        r"""
        ...
        ra = ra % 360.0
        lon_0 = lon_0 % 360.0
        polyseg_list = split_PolygonSegments(ra, dec, lon_split=(self.lon_split))
        ...
        for radec in polyseg_list:
            if len(radec) > 0:
                ...
                ra[mask] -= 360.0
                points_on_split = np.logical_or(ra > 180.0 - epsilon, ra < -180.0 + epsilon)
        ...
        for poly_segs in split_poly_normed:
            lon, lat = poly_segs
            if len(lon) > 1:
                pt = (self.polygonize)(lon, lat, zorder=20.0, **kwargs)
                if add_patch:
                    ax.add_patch(pt)
                pts.append(pt)
    ...
    def polygonize(self, lon, lat, zorder=1, **kwargs):
        ...
        x = np.asarray(x)
        y = np.asarray(y)
        mask = np.logical_and(x < 1e+20, y < 1e+20)
        xy = zip(x[mask], y[mask])
        pt = Polygon(xy, zorder=zorder, **kwargs)
        return pt


def plot_south_steradian_view(ra, dec, numPoints=100, radius=1.75, boundary=20.0, ax=None, show_frame=True):
    """
    """
    if ax is None:
        fig, ax = plt.subplots()
    m = Basemap(projection='spstere', boundinglat=boundary, lon_0=0.0, ax=ax)
    if show_frame:
...
class HPTileVis(object):
    ...
    def tileCenter(self, ipix):
        ...
        ra, dec = convertToCelestialCoordinates(theta, phi, input_unit='radians', output_unit='degrees')
        return (ra, dec)

    def pointingSummary(self, tileID=None, ra=None, dec=None, angularUnits='degrees', allPointings=None):
        """
        ...
        if tileID is None:
            ...
            tileID = self.tileIDfromCelestialCoordinates(ra, dec,
              (self.nside),
              units=angularUnits)
        obsHistIDs = self.hpTile.pointingSequenceForTile(tileID=tileID, allPointings=allPointings)
        s = self.opsout.summary.ix[obsHistIDs]
        return s

    def pointingCenters(self, tileID, raCol='ditheredRA', decCol='ditheredDec', query=None, opsimAngularUnits='radians', outputAngularUnits='degrees', groupby=None):
        """
        tileID : int
            index for healpix tile
        ...
        """
        summary = self.pointingSummary(tileID)
    ...
    def plotTilePointings(self, tileID, raCol='ditheredRA', decCol='ditheredDec', radius=1.75, paddingFactors=1, query=None, ax=None, projection='cyl', tile_centers=None, corners=None, drawPointings=True, **kwargs):
        """
        ...
        lon, lat = healpix_boundaries(tileID, nside=(self.nside), units='degrees', convention='celestial',
          step=10,
          nest=True)
        x, y = m(lon, lat)
        xy = zip(x, y)
        healpixels = Polygon(xy, facecolor='w', fill=False, alpha=1.0, edgecolor='k',
          lw=2)
        if drawPointings:
            ra, dec = self.pointingCenters(tileID,
              raCol=raCol, decCol=decCol, query=query)
