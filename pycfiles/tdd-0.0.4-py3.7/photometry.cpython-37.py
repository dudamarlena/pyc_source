...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tdd/photometry.py
# Compiled at: 2019-03-27 17:17:32
# Size of source mod 2**32: 11604 bytes
"""
...
__all__ = [
 'PhotTables']
import abc, numpy as np, pandas as pd
...
class PhotTables(object):
    ...
    def coaddedTable(self, timeOffset=0.0, timeStep=1.0, avg_cols=('mjd', 'flux', 'fluxerr', 'zp'), additionalAvgCols=None, additionalColsKept=('tileID', 'fieldID', 'zpsys'), additionalAggFuncs=('first', 'first', 'first'), prepend_colNames='coadd_'):
        """
        ...
        lcs = LightCurve.add_weightedColumns(lcs, avg_cols=avg_cols,
        ...
        lcs = LightCurve.coaddpreprocessed(lcs, include_snid=include_snid, cols=weightedcols,
          additionalColsKept=additionalColsKept,
          additionalAggFuncs='first',
          keepAll=False,
          keepCounts=True)
        if prepend_colNames is not None:
            coldict = dict(((col, prepend_colNames + col) for col in lcs.columns if col not in ('snid',
                                                                                                'band')))
            lcs.rename(columns=coldict, inplace=True)
        return lcs

    def summary(self, coadd=True, coaddTimeStep=1.0, coaddTimeOffset=0.0, paramsdf=None):
        """
        ...
        if coadd:
            tmp = self.coaddedTable(timeStep=coaddTimeStep, timeOffset=coaddTimeOffset,
              prepend_colNames='')
            nightlySummary = LightCurve.summarize(tmp, summary_prefix='coadd_')
...
class BasePhotometry(with_metaclass(abc.ABCMeta, object)):
    ...
    @staticmethod
    def statTable(PropTuple, callableTuple, grouped=None, dataframe=None, groupTuple=None):
        """
        Parameters
        ----------
        PropTuple :
...
class Photometry(BasePhotometry):
    ...
    def __init__(self, lcs, maxObsHistID=10000000, singleLCProps=None):
        """
        """
