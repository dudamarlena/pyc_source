...
# [GCC 8.4.0]
# Embedded file name: /Users/qt/work/pyside/pyside-setup/pyside2_install/py2.7-qt5.14.2-64bit-release/lib/python2.7/site-packages/shiboken2/files.dir/shibokensupport/typing27.py
...
import collections, functools, re as stdlib_re, sys, types, copy
try:
...
def _type_check(arg, msg):
    ...
    if arg is None:
        ...
        if type(arg).__name__ in ('_Union', '_Optional') and not getattr(arg, b'__origin__', None) or isinstance(arg, TypingMeta) and arg._gorg in (Generic, Protocol):
...
class _Final(_FinalTypingBase):
    ...
    def __getitem__(self, item):
        cls = type(self)
        if self.__type__ is None:
            return cls(_type_check(item, (b'{} accepts only single type.').format(cls.__name__[1:])), _root=True)
        else:
            raise TypeError((b'{} cannot be further subscripted').format(cls.__name__[1:]))
            return
...
class _Literal(_FinalTypingBase):
    ...
    def __getitem__(self, item):
        ...
        if self.__values__ is None:
            if not isinstance(item, tuple):
                item = (
                 item,)
            return cls(values=item, _root=True)
        else:
            raise TypeError((b'{} cannot be further subscripted').format(cls.__name__[1:]))
            return
    ...
    def __repr__(self):
        ...
        if self.__values__ is not None:
            r += (b'[{}]').format((b', ').join(map(_type_repr, self.__values__)))
        return r
...
class GenericMeta(TypingMeta, abc.ABCMeta):
    ...
    * if Generic[...] is present, then it should list all type variables
    ...
    def __new__(cls, name, bases, namespace, tvars=None, args=None, origin=None, extra=None, orig_bases=None):
        ...
        if tvars is not None:
            ...
            for base in bases:
                ...
                if isinstance(base, GenericMeta) and base.__origin__ in (Generic, Protocol):
                    if gvars is not None:
                        raise TypeError(b'Cannot inherit from Generic[...] or Protocol[...] multiple times.')
            ...
            if gvars is None:
                ...
                if not tvarset <= gvarset:
                    raise TypeError(b'Some type variables (%s) are not listed in %s[%s]' % (
                     (b', ').join(str(t) for t in tvars if t not in gvarset),
                     b'Generic' if any(b.__origin__ is Generic for b in bases) else b'Protocol',
        ...
        super(GenericMeta, self).__setattr__(b'_gorg', self if not origin else origin._gorg)
    ...
    def __getitem__(self, params):
        ...
        if self in (Generic, Protocol):
            if not all(isinstance(p, TypeVar) for p in params):
                raise TypeError(b'Parameters to %s[...] must all be type variables' % self.__name__)
            if len(set(params)) != len(params):
                raise TypeError(b'Parameters to %s[...] must all be unique' % self.__name__)
            tvars = params
            args = params
    ...
    def __instancecheck__(self, instance):
        if hasattr(instance, b'__class__'):
...
_PROTO_WHITELIST = [
 b'Callable', b'Iterable', b'Iterator',
 b'Hashable', b'Sized', b'Container', b'Collection',
 b'Reversible', b'ContextManager']
...
class _ProtocolMeta(GenericMeta):
    ...
    def __init__(cls, *args, **kwargs):
        ...
        if not cls.__dict__.get(b'_is_protocol', None):
        ...
        if cls._is_protocol:
            for base in cls.__mro__[1:]:
                if not (base in (object, Generic) or base.__module__ == b'_abcoll' and base.__name__ in _PROTO_WHITELIST or isinstance(base, TypingMeta) and base._is_protocol or isinstance(base, GenericMeta) and base.__origin__ is Generic):
                    raise TypeError(b'Protocols can only inherit from other protocols, got %r' % base)

            cls._callable_members_only = all(callable(getattr(cls, attr)) for attr in cls._get_protocol_attrs())

            def _no_init(self, *args, **kwargs):
                if type(self)._is_protocol:
                    raise TypeError(b'Protocols cannot be instantiated')
        ...
        def _proto_hook(cls, other):
            if not cls.__dict__.get(b'_is_protocol', None):
                ...
                if not isinstance(other, type):
                    raise TypeError(b'issubclass() arg 1 must be a new-style class')
        ...
        if b'__subclasshook__' not in cls.__dict__:
    ...
    def __instancecheck__(self, instance):
        if isinstance(instance, type):
            return False
        if (not getattr(self, b'_is_protocol', False) or self._callable_members_only) and issubclass(instance.__class__, self):
            return True
        if self._is_protocol:
            if all(hasattr(instance, attr) and (not callable(getattr(self, attr)) or getattr(instance, attr) is not None) for attr in self._get_protocol_attrs()):
                return True
        return super(GenericMeta, self).__instancecheck__(instance)

    def __subclasscheck__(self, cls):
        if self.__dict__.get(b'_is_protocol', None) and not self.__dict__.get(b'_is_runtime_protocol', None):
            if sys._getframe(1).f_globals[b'__name__'] in ('abc', 'functools') or sys._getframe(2).f_globals[b'__name__'] == b'typing':
                return False
            raise TypeError(b'Instance and class checks can only be used with @runtime_checkable protocols')
        if self.__dict__.get(b'_is_runtime_protocol', None) and not self._callable_members_only:
            if sys._getframe(1).f_globals[b'__name__'] in ('abc', 'functools'):
                return super(GenericMeta, self).__subclasscheck__(cls)
            raise TypeError(b"Protocols with non-method members don't support issubclass()")
    ...
    def _get_protocol_attrs(self):
        ...
        for base in self.__mro__[:-1]:
            if base.__name__ in ('Protocol', 'Generic'):
                continue
            annotations = getattr(base, b'__annotations__', {})
            for attr in list(base.__dict__.keys()) + list(annotations.keys()):
                if not attr.startswith(b'_abc_') and attr not in ('__abstractmethods__',
                                                                  '__annotations__',
                                                                  '__weakref__',
                                                                  '_is_protocol',
                                                                  '_is_runtime_protocol',
                                                                  '__dict__', '__args__',
                                                                  '__slots__', '_get_protocol_attrs',
                                                                  '__next_in_mro__',
                                                                  '__parameters__',
...
class Protocol(object):
    ...
    def __new__(cls, *args, **kwds):
        if cls._gorg is Protocol:
            raise TypeError(b'Type Protocol cannot be instantiated; it can be used only as a base class')
        return _generic_new(cls.__next_in_mro__, cls, *args, **kwds)
...
def runtime_checkable(cls):
    ...
    if not isinstance(cls, _ProtocolMeta) or not cls._is_protocol:
        raise TypeError(b'@runtime_checkable can be only applied to protocol classes, got %r' % cls)
...
def _check_fails(cls, other):
    try:
        if sys._getframe(1).f_globals[b'__name__'] not in ('abc', 'functools', 'typing'):
            raise TypeError(b'TypedDict does not support instance and class checks')
    except (AttributeError, ValueError):
        pass
...
def _typeddict_new(cls, _typename, _fields=None, **kwargs):
    total = kwargs.pop(b'total', True)
    if _fields is None:
        _fields = kwargs
    else:
        if kwargs:
            raise TypeError(b'TypedDict takes either a dict or keyword arguments, but not both')
        ns = {b'__annotations__': dict(_fields), b'__total__': total}
        try:
            ns[b'__module__'] = sys._getframe(1).f_globals.get(b'__name__', b'__main__')
        except (AttributeError, ValueError):
            pass
...
class _TypedDictMeta(type):
    ...
    def __new__(cls, name, bases, ns, total=True):
        ns[b'__new__'] = _typeddict_new if name == b'TypedDict' else _dict_new
        tp_dict = super(_TypedDictMeta, cls).__new__(cls, name, (dict,), ns)
        anns = ns.get(b'__annotations__', {})
        msg = b"TypedDict('Name', {f0: t0, f1: t1, ...}); each t must be a type"
        anns = {n:_type_check(tp, msg) for n, tp in anns.items()}
        for base in bases:
            anns.update(base.__dict__.get(b'__annotations__', {}))

        tp_dict.__annotations__ = anns
        if not hasattr(tp_dict, b'__total__'):
