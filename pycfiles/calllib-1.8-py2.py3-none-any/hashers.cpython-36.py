...
# Embedded file name: /Users/Ari/src/callisto-core/callisto_core/delivery/hashers.py
# Compiled at: 2019-08-07 16:42:06
# Size of source mod 2**32: 7512 bytes
import base64, argon2
...
def get_hasher(algorithm='default'):
    ...
    try:
        ...
        raise ValueError('Unknown key hashing algorithm {0}.Did you specify it in the KEY_HASHERS setting?'.format(algorithm))
...
def make_key(encode_prefix, key, salt):
    ...
    if hasher.algorithm == 'pbkdf2_sha256':
        if hasher.must_update(encode_prefix):
...
class PBKDF2KeyHasher(PBKDF2PasswordHasher):
    ...
    def split_encoded(self, encoded):
        ...
        return (
         prefix, force_bytes(stretched_key))
...
class Argon2KeyHasher(BasePasswordHasher):
    ...
    def encode(self, key, salt, **kwargs):
        if not key is not None:
            ...
            raise AssertionError
        data = argon2.low_level.hash_secret((force_bytes(key)),
          (force_bytes(salt)),
    ...
    def verify(self, key, encoded):
        ...
        try:
            return argon2.low_level.verify_secret((force_bytes('$' + rest)),
              (force_bytes(key)), type=(argon2.low_level.Type.I))
