...
# Embedded file name: /Users/Ari/src/callisto-core/callisto_core/delivery/utils.py
...
class RecordDataUtil(object):
    ...
    def _add_question_answer(self, question: dict):
        pk = question.get('id')
        if pk:
            self.new_data[self.answer_key].update({f"question_{pk}": question.get('answer', '')})
    ...
    def _add_form_fields(self, question: dict, extra={}) -> dict:
        new_form = {**{'section':question.get('section', 1), 
         'type':question.get('type'), 
         'id':question.get('id'), 
         'question_text':'<p>{}</p>'.format(question.get('question_text', '')), 
         'field_id':'question_{}'.format(question.get('id'))}, **extra}
