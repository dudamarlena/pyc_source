...
# [GCC 8.4.0]
# Embedded file name: /Users/prabhu/src/git/VTKPythonPackage/VTK-osx_2.7/Wrapping/Python/vtk/gtk/GtkGLExtVTKRenderWindow.py
...
class GtkGLExtVTKRenderWindow(GtkGLExtVTKRenderWindowBase):
    ...
    def OnKeyPress(self, wid, event=None):
        if event.keyval == gdk.keyval_from_name('r') or event.keyval == gdk.keyval_from_name('R'):
            ...
            if event.keyval == gdk.keyval_from_name('w') or event.keyval == gdk.keyval_from_name('W'):
                self.Wireframe()
                return gtk.TRUE
            if event.keyval == gdk.keyval_from_name('s') or event.keyval == gdk.keyval_from_name('S'):
                self.Surface()
                return gtk.TRUE
            if event.keyval == gdk.keyval_from_name('p') or event.keyval == gdk.keyval_from_name('P'):
                m = self.get_pointer()
