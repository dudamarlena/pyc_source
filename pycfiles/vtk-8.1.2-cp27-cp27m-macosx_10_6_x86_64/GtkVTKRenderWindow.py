...
# [GCC 8.4.0]
# Embedded file name: /Users/prabhu/src/git/VTKPythonPackage/VTK-osx_2.7/Wrapping/Python/vtk/gtk/GtkVTKRenderWindow.py
...
class GtkVTKRenderWindow(GtkVTKRenderWindowBase):
    ...
    def OnMouseMove(self, wid, event=None):
        if event.state & GDK.BUTTON1_MASK == GDK.BUTTON1_MASK:
            ...
            if event.state & GDK.BUTTON2_MASK == GDK.BUTTON2_MASK:
                m = self.get_pointer()
                self.Pan(m[0], m[1])
                return gtk.TRUE
            else:
                if event.state & GDK.BUTTON3_MASK == GDK.BUTTON3_MASK:
                    m = self.get_pointer()
                    self.Zoom(m[0], m[1])
                    return gtk.TRUE
                return gtk.FALSE
    ...
    def OnKeyPress(self, wid, event=None):
        if event.keyval == GDK.r or event.keyval == GDK.R:
            ...
            if event.keyval == GDK.w or event.keyval == GDK.W:
                self.Wireframe()
                return gtk.TRUE
            if event.keyval == GDK.s or event.keyval == GDK.S:
                self.Surface()
                return gtk.TRUE
            if event.keyval == GDK.p or event.keyval == GDK.P:
                m = self.get_pointer()
