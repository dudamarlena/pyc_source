...
# [GCC 8.4.0]
# Embedded file name: /Users/prabhu/src/git/VTKPythonPackage/VTK-osx_2.7/Wrapping/Python/vtk/tk/vtkTkRenderWidget.py
...
class vtkTkRenderWidget(tkinter.Widget):
    ...
    def BindTkRenderWidget(self):
        ...
        """
        self.bind('<ButtonPress>', lambda e, s=self: s.StartMotion(e.x, e.y))
        self.bind('<ButtonRelease>', lambda e, s=self: s.EndMotion(e.x, e.y))
        self.bind('<B1-Motion>', lambda e, s=self: s.Rotate(e.x, e.y))
        self.bind('<B2-Motion>', lambda e, s=self: s.Pan(e.x, e.y))
        self.bind('<B3-Motion>', lambda e, s=self: s.Zoom(e.x, e.y))
        self.bind('<Shift-B1-Motion>', lambda e, s=self: s.Pan(e.x, e.y))
        self.bind('<KeyPress-r>', lambda e, s=self: s.Reset(e.x, e.y))
        self.bind('<KeyPress-u>', lambda e, s=self: s.deiconify())
        self.bind('<KeyPress-w>', lambda e, s=self: s.Wireframe())
        self.bind('<KeyPress-s>', lambda e, s=self: s.Surface())
        self.bind('<KeyPress-p>', lambda e, s=self: s.PickActor(e.x, e.y))
        if self._FocusOnEnter:
            self.bind('<Enter>', lambda e, s=self: s.Enter(e.x, e.y))
            self.bind('<Leave>', lambda e, s=self: s.Leave(e.x, e.y))
        else:
            self.bind('<ButtonPress>', lambda e, s=self: s.Enter(e.x, e.y))
    ...
    def Pan(self, x, y):
        if self._CurrentRenderer:
            ...
            if camera.GetParallelProjection():
                ...
                camera.SetFocalPoint(fPoint0 - rPoint0 + fPoint0, fPoint1 - rPoint1 + fPoint1, fPoint2 - rPoint2 + fPoint2)
                camera.SetPosition(fPoint0 - rPoint0 + pPoint0, fPoint1 - rPoint1 + pPoint1, fPoint2 - rPoint2 + pPoint2)
            self._LastX = x
            self._LastY = y
            self.Render()
    ...
    def PickActor(self, x, y):
        if self._CurrentRenderer:
            ...
            if self._PickedAssembly != None and self._PrePickedProperty != None:
