...
# [GCC 8.4.0]
# Embedded file name: /Users/prabhu/src/git/VTKPythonPackage/VTK-osx_2.7/Wrapping/Python/vtk/numpy_interface/internal_algorithms.py
...
def _cell_derivatives(narray, dataset, attribute_type, filter):
    ...
    if dsa.ArrayAssociation.POINT == narray.Association:
        ...
        if dsa.ArrayAssociation.CELL == narray.Association:
            filter.Update()
...
def _cell_quality(dataset, quality):
    ...
    filter.SetInputData(ds)
    if 'area' == quality:
        filter.SetQualityMeasureToArea()
    elif 'aspect' == quality:
        filter.SetQualityMeasureToAspectRatio()
    elif 'aspect_gamma' == quality:
        filter.SetQualityMeasureToAspectGamma()
    elif 'condition' == quality:
        filter.SetQualityMeasureToCondition()
    elif 'diagonal' == quality:
        filter.SetQualityMeasureToDiagonal()
    elif 'jacobian' == quality:
        filter.SetQualityMeasureToJacobian()
    elif 'max_angle' == quality:
        filter.SetQualityMeasureToMaxAngle()
    elif 'shear' == quality:
        filter.SetQualityMeasureToShear()
    elif 'skew' == quality:
        filter.SetQualityMeasureToSkew()
    elif 'min_angle' == quality:
        filter.SetQualityMeasureToMinAngle()
    elif 'volume' == quality:
        filter.SetQualityMeasureToVolume()
    else:
        raise RuntimeError('Unknown cell quality [' + quality + '].')
    filter.Update()
...
def _matrix_math_filter(narray, operation):
    if operation not in ('Determinant', 'Inverse', 'Eigenvalue', 'Eigenvector'):
        raise RuntimeError('Unknown quality measure [' + operation + ']' + ' Supported are [Determinant, Inverse, Eigenvalue, Eigenvector]')
    if narray.ndim != 3:
        raise RuntimeError(operation + ' only works for an array of matrices(3D array).' + ' Input shape ' + str(narray.shape))
    elif narray.shape[1] != narray.shape[2]:
        raise RuntimeError(operation + ' requires an array of 2D square matrices.' + ' Input shape ' + str(narray.shape))
    if not narray.flags.contiguous:
        narray = narray.copy()
    ...
    if operation == 'Determinant':
        filter.SetOperationToDeterminant()
    elif operation == 'Inverse':
        filter.SetOperationToInverse()
    elif operation == 'Eigenvalue':
        filter.SetOperationToEigenvalue()
    elif operation == 'Eigenvector':
        filter.SetOperationToEigenvector()
    filter.SetInputData(ds)
    filter.Update()
...
def cross(x, y):
    ...
    if x.ndim != y.ndim or x.shape != y.shape:
        raise RuntimeError('Both operands must have same dimension and shape.' + ' Input shapes ' + str(x.shape) + ' and ' + str(y.shape))
    if x.ndim != 1 and x.ndim != 2:
        raise RuntimeError('Cross only works for 3D vectors or an array of 3D vectors.' + ' Input shapes ' + str(x.shape) + ' and ' + str(y.shape))
    if x.ndim == 1 and x.shape[0] != 3:
        raise RuntimeError('Cross only works for 3D vectors.' + ' Input shapes ' + str(x.shape) + ' and ' + str(y.shape))
    if x.ndim == 2 and x.shape[1] != 3:
        raise RuntimeError('Cross only works for an array of 3D vectors.' + 'Input shapes ' + str(x.shape) + ' and ' + str(y.shape))
...
def curl(narray, dataset=None):
    ...
    if narray.ndim != 2 or narray.shape[1] != 3:
        raise RuntimeError('Curl only works with an array of 3D vectors.' + 'Input shape ' + str(narray.shape))
...
def divergence(narray, dataset=None):
    ...
    if narray.ndim != 2 or narray.shape[1] != 3:
        raise RuntimeError('Divergence only works with an array of 3D vectors.' + ' Input shape ' + str(narray.shape))
    g = gradient(narray, dataset)
    g = g.reshape(g.shape[0], 3, 3)
    a = dsa.VTKArray(numpy.add.reduce(g.diagonal(axis1=1, axis2=2), 1), dataset=g.DataSet)
    try:
...
def dot(a1, a2):
    ...
    if a1.shape[1] != a2.shape[1]:
        raise RuntimeError('Dot product only works with vectors of same dimension.' + ' Input shapes ' + str(a1.shape) + ' and ' + str(a2.shape))
    m = a1 * a2
...
def gradient(narray, dataset=None):
    ...
    if ncomp != 1 and ncomp != 3:
        raise RuntimeError('Gradient only works with scalars (1 component) and vectors (3 component)' + ' Input shape ' + str(narray.shape))
    cd = vtk.vtkCellDerivatives()
    if ncomp == 1:
