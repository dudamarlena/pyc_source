...
# [GCC 8.4.0]
# Embedded file name: /Users/prabhu/src/git/VTKPythonPackage/VTK-osx_2.7/Wrapping/Python/vtk/tk/vtkTkImageViewerWidget.py
...
class vtkTkImageViewerWidget(tkinter.Widget):
    ...
    def __init__(self, master, cnf={}, **kw):
        ...
        tkinter.Widget.__init__(self, master, 'vtkTkImageViewerWidget', cnf, kw)
        if doubleBuffer:
    ...
    def BindTkImageViewer(self):
        ...
        self._OldFocus = 0
        self._InExpose = 0
        self.bind('<ButtonPress-1>', lambda e, s=self: s.StartWindowLevelInteraction(e.x, e.y))
        self.bind('<B1-Motion>', lambda e, s=self: s.UpdateWindowLevelInteraction(e.x, e.y))
        self.bind('<ButtonRelease-1>', lambda e, s=self: s.EndWindowLevelInteraction())
        self.bind('<ButtonPress-3>', lambda e, s=self: s.StartQueryInteraction(e.x, e.y))
        self.bind('<B3-Motion>', lambda e, s=self: s.UpdateQueryInteraction(e.x, e.y))
        self.bind('<ButtonRelease-3>', lambda e, s=self: s.EndQueryInteraction())
        self.bind('<Expose>', lambda e, s=self: s.ExposeTkImageViewer())
        self.bind('<Enter>', lambda e, s=self: s.EnterTkViewer())
        self.bind('<Leave>', lambda e, s=self: s.LeaveTkViewer())
        self.bind('<KeyPress-e>', lambda e, s=self: s.quit())
        self.bind('<KeyPress-r>', lambda e, s=self: s.ResetTkImageViewer())
    ...
    def ExposeTkImageViewer(self):
        if self._InExpose == 0:
            self._InExpose = 1
            if not self._ImageViewer.GetRenderWindow().IsA('vtkCocoaRenderWindow'):
                self.update()
            self._ImageViewer.Render()
            self._InExpose = 0
