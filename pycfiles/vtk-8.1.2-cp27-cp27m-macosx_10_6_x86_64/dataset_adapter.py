...
# [GCC 8.4.0]
# Embedded file name: /Users/prabhu/src/git/VTKPythonPackage/VTK-osx_2.7/Wrapping/Python/vtk/numpy_interface/dataset_adapter.py
...
try:
    ...
    raise RuntimeError('This module depends on the numpy module. Please makesure that it is installed properly.')
...
def reshape_append_ones(a1, a2):
    ...
    if isinstance(a1, numpy.ndarray) and isinstance(a2, numpy.ndarray):
        len1 = len(a1.shape)
        len2 = len(a2.shape)
        if len1 == len2 or len1 == 0 or len2 == 0 or a1.shape[0] != a2.shape[0]:
            return l
        if len1 < len2:
            d = len1
            maxLength = len2
...
class VTKCompositeDataArray(object):
    ...
    def __init__(self, arrays=[], dataset=None, name=None, association=None):
    ...
    def _numeric_op(self, other, op):
        ...
        return VTKCompositeDataArray(res, dataset=self.DataSet, association=self.Association)
...
class DataSetAttributes(VTKObjectWrapper):
    ...
    def append(self, narray, name):
        ...
        elif not isinstance(narray, numpy.ndarray):
            arrLength = 1
        else:
        ...
        if not isinstance(narray, numpy.ndarray) or numpy.ndim(narray) == 0:
            ...
            if len(shape) == 3:
                ...
                if narray.strides[1] / size == 3 and narray.strides[2] / size == 1 or narray.strides[1] / size == 1 and narray.strides[2] / size == 3 and not narray.flags.contiguous:
...
class CompositeDataSetAttributes:
    ...
    def append(self, narray, name):
        ...
        if narray is NoneArray:
            ...
            added = False
            if not isinstance(narray, VTKCompositeDataArray):
                for ds in self.DataSet:
    ...
    def GetArray(self, idx):
        ...
        if arrayname not in self.ArrayNames:
            ...
            if arrayname not in self.Arrays or self.Arrays[arrayname]() is None:
                array = VTKCompositeDataArray(dataset=self.DataSet, name=arrayname, association=self.Association)
...
class PolyData(PointSet):
    ...
    def GetPolygons(self):
        ...
        if not self.VTKObject.GetPolys():
            ...
            return vtkDataArrayToVTKArray(self.VTKObject.GetPolys().GetData(), self)
...
class UnstructuredGrid(PointSet):
    ...
    def GetCells(self):
        ...
        if not self.VTKObject.GetCells():
            ...
            return vtkDataArrayToVTKArray(self.VTKObject.GetCells().GetData(), self)
