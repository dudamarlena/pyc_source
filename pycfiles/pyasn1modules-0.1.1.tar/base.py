...
# Embedded file name: build/bdist.linux-x86_64/egg/pyasn1/type/base.py
...
__all__ = [
 'Asn1Item', 'Asn1Type', 'SimpleAsn1Type', 'ConstructedAsn1Type']
    ...
    *PyAsn1Error* exception.
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            op_names = [ name for typ in (str, int, list, dict) for name in dir(typ) if name not in cls.skipMethods if name.startswith('__') if name.endswith('__') if calling.callable(getattr(typ, name)) ]
...
class SimpleAsn1Type(Asn1Type):
    ...
    def __repr__(self):
        representation = '%s %s object' % (self.__class__.__name__, self.isValue and 'value' or 'schema')
