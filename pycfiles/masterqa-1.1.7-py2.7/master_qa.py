...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/masterqa/master_qa.py
# Compiled at: 2019-04-02 16:07:06
...
class __MasterQATestCase__(BaseCase):
    ...
    def clear_out_old_logs(self, archive_past_runs=True, get_log_folder=False):
        ...
        if archive_past_runs:
            ...
            filelist = [ f for f in os.listdir('./%s' % LATEST_REPORT_DIR) if f.startswith('failed_') or f == RESULTS_PAGE or f.startswith('automation_failure') or f == BAD_PAGE_LOG
                       ]
            for f in filelist:
    ...
    def jq_confirm_dialog(self, question):
        ...
        title_content = '<center><font color="#7700bb">Manual Check #%s:</font></center><hr><font color="#0066ff">%s</font>' % (
         count, question)
    ...
    def manual_page_check(self, *args):
            ...
            if instructions and '?' not in instructions:
                ...
                if use_jqc:
                    ...
                    try:
                        ...
                        status = 'Failure!'
                        pre_status = self.execute_script("return jconfirm.lastClicked.hasClass('btn-green')")
                        if pre_status:
                            status = 'Success!'
        ...
        if 'Success!' in str(status):
            ...
            self.page_results_list.append('"%s","%s","%s","%s","%s","%s","%s","%s"' % (
