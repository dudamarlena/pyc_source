...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mathml/pmathml/element.py
# Compiled at: 2005-03-06 08:59:29
...
class Element(events.EventSource):
    ...
    class Attribute:
        _length_rx = re.compile('^(?P<number>-?(?:\\d+(?:\\.\\d+)?|\\.\\d+)(?:[eE][+-]?\\d+)?)(?P<unit>[a-zA-Z]{1,2})?$')
        ...
        element = property(__get_element, None, None, 'The element this attribute belongs to')
        ...
        def asLength(self, default_unit_scale=1.0):
            if isinstance(self.__value, float):
                return self.__value
            assert isinstance(self.__value, str) or isinstance(self.__value, unicode)
    ...
    all_children = property(getAllChildren, None, None, 'recursively expanded list, depth-first, of child elements')
    ...
    def getAttribute(self, name, recursive=1, default=None):
        ...
        assert isinstance(name, str) or isinstance(name, unicode), 'type(name) == %s (value=%s) != str' % (str(type(name)), str(name))
        try:
    ...
    def getScriptLevel(self):
        ...
        if scriptlevel is None:
            ...
            if scriptlevel[0] == '+' or scriptlevel[0] == '-':
                self._script_level = self.parent.getScriptLevel() + int(scriptlevel)
            else:
    ...
    def getFontSize(self):
        ...
        size = fontsize * math.pow(scriptsizemultiplier, scriptlevel - fontsize_scriptlevel)
        if minsize is None:
            return size
        else:
    ...
    font_size = property(getFontSize, None, __delFontSize, 'Font size (max text height) for this element')
    ...
    embellished_container = property(getEmbellishedContainer, None, None, 'If this element is part of an embellished operator group, the toplevel container of that group')
    embellished_core = property(getEmbellishedCore, None, None, 'If this element is part of an embellished operator group, the innermost operator element of that group')
