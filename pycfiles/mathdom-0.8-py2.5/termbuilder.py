...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mathml/termbuilder.py
# Compiled at: 2006-02-06 02:06:08
"""
Implementation of the standard AST output converters.
"""
__all__ = ('TermBuilder', 'LiteralTermBuilder', 'InfixTermBuilder', 'PrefixTermBuilder',
           'PostfixTermBuilder', 'tree_converters')
try:
...
from mathml.termparser import ConverterRegistry, TERM_OPERATOR_ORDER, BOOL_CMP_OPERATORS

class TermBuilder(object):
    """Abstract superclass for term builders."""
    OPERATOR_ORDER = list((op for ops in (TERM_OPERATOR_ORDER, '| in',
     BOOL_CMP_OPERATORS, 'and xor or') for op in ops.split()))
    ...
    def _build_children(self, operator, children, status):
        if operator == 'name' or operator[:6] == 'const:':
            return children
        return [ (' ').join(operand) for operand in imap(self._recursive_build, children, repeat(status)) ]
    ...
    def _recursive_build(self, tree, status):
        ...
        dispatch_name = operator.replace(':', '_')
...
class LiteralTermBuilder(TermBuilder):
    ...
    def _handle_const(self, operator, operands, status):
        return [
         unicode(str(operands[0]).lower(), 'ascii')]
...
class InfixTermBuilder(LiteralTermBuilder):
    ...
    def _handleOP(self, operator, operands, affin_status):
        ...
        if my_affin >= parent_affin:
            ...
            return chain((operands[0],), chain(*zip(chain(repeat(output_operator)), islice(operands, 1, None))))
        return

    def _handle(self, operator, operands, affin_status):
        return [
         self._map_operator(operator), '(', (', ').join(operands), ')']
