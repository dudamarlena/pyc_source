...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mathml/lmathdom.py
# Compiled at: 2008-03-11 14:16:35
__all__ = [
 'MathDOM', 'Apply', 'Constant', 'Identifier', 'Name',
...
TYPE_MAP = {'real': Decimal, 
   'complex': Complex, 
   'rational': Rational, 
   'e-notation': ENotation}
STYLESHEET_MAPPING = {'mathmlc2p': ('mathml', 'pmathml'), 
...
for (xsl_name, (input_type, output_type)) in STYLESHEET_MAPPING.iteritems():
    try:
        STYLESHEETS[output_type] = (
         input_type, UTILS_STYLESHEETS[xsl_name])
    except KeyError:
        pass
    except Exception, e:
...
class MathElement(_etree.ElementBase):
    ...
    def xpath(self, expression, other_namespaces=None):
        ...
        evaluator = _etree.XPathElementEvaluator(self, namespaces=other_namespaces)
        evaluator.registerNamespace('math', MATHML_NAMESPACE_URI)
        return super(MathElement, self).xpath(expression, namespaces=other_namespaces)

    def _xpath(self, xpath):
        try:
            evaluate = self.__xpath_evaluator
        except AttributeError:
            evaluate = self.__xpath_evaluator = _etree.XPathElementEvaluator(self, namespaces=_MATH_NS_DICT).evaluate
...
class math_cn(SerializableMathElement):
    ...
    def set_complex(self, value):
        ...
        try:
            tuple_value = (
             value.real_str, value.imag_str)
        except AttributeError:
            tuple_value = (
             unicode(value.real), unicode(value.imag))
...
class MathDOM(object):
    ...
    def createApply(self, name, *args):
        ...
        function_tag = SubElement(apply_tag, '{%s}%s' % (MATHML_NAMESPACE_URI, name))
        if args:
...
def Apply(parent, name, *args):
    ...
    if isinstance(parent, MathDOM):
        parent = parent.getroot()
    apply_tag = SubElement(parent, '{%s}apply' % MATHML_NAMESPACE_URI)
    function_tag = SubElement(apply_tag, '{%s}%s' % (MATHML_NAMESPACE_URI, name))
    if args:
        if len(args) == 1 and isinstance(args[0], (list, tuple)):
            args = args[0]
...
def _prepare_mathml_classes():
    classes = [ (cls.IMPLEMENTS.split(), cls) for cls in _all_names if isinstance(cls, type) if issubclass(cls, MathElement) if hasattr(cls, 'IMPLEMENTS')
              ]
