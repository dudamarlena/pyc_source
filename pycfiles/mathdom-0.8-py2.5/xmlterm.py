...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mathml/xmlterm.py
# Compiled at: 2005-11-28 10:54:57
"""
...
_ELEMENT_CONSTANT_MAP = {'true': 'true', 
   'false': 'false', 
   'pi': 'pi', 
   'i': 'imaginaryi', 
   'e': 'exponentiale'}
...
def dom_to_tree(doc_or_element):
    ...
    def _recursive_dom_to_tree(element):
        ...
        if constant:
            ...
            return [
             'const:%s' % element.valuetype().replace('-', ''), element.value()]
        elif mtype == 'apply':
...
class SaxTerm(XMLReader):
    ...
    def parse_term(self, expression, input_type):
        if hasattr(expression, 'getCharacterStream'):
    ...
    def _recursive_tree_to_sax(self, tree):
        ...
        if mapped_operator:
            ...
            if operator == 'const:bool':
                ...
                self._write_element('cn', mkstr(tree[1]), self._attributes(type=operator[6:]))
        elif operator == 'case':
