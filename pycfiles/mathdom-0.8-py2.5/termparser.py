...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mathml/termparser.py
# Compiled at: 2007-04-08 16:09:46
"""
...
"""
__all__ = ('term_parsers', 'ParseException')
try:
...
class InfixTermParser(TermParserBase):
    ...
    def p_case(self, p_arithmetic_exp, p_bool_expression):
        p_case = Suppress(CaselessKeyword('CASE') + Optional(CaselessKeyword('WHEN'))) + p_bool_expression + Suppress(CaselessKeyword('THEN')) + p_arithmetic_exp + Suppress(CaselessKeyword('ELSE')) + p_arithmetic_exp + Suppress(CaselessKeyword('END'))
...
class InfixBoolExpressionParser(BoolParserBase):
    ...
    @cached
    def p_cmp_exp(self):
        p_exp = self.p_str_cmp() | self.p_list_cmp() | self.p_arithmetic_cmp() | self.p_factor_cmp() | self.p_bool_cmp()
        p_exp.setParseAction(self._build_expression_tree)
        return p_exp
