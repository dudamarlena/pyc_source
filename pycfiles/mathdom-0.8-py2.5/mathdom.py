...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mathml/mathdom.py
# Compiled at: 2005-11-18 13:59:15
"""
...
def method_elements(element_names='', defined_in=''):
    if defined_in:
        ...
        elements = frozenset(chain(element_names.split(), (name for s in defined_in.split() for name in global_sources[s].split())))
    else:
    ...
    def register_function(function):
        ...
        for element_name in elements:
            try:
                ...
                METHODS_BY_ELEMENT_NAME[element_name] = [
                 new_function]
...
class MathDOM(object):
    ...
    def __augmentElements(self, node):
        ...
        if node.nodeType == node.ELEMENT_NODE and node.namespaceURI == MATHML_NAMESPACE_URI:
            ...
            node.__dict__.update(((method_name, build_instancemethod(method, node, node_class)) for (method_name, method) in chain(self.__common_methods, element_methods)))
            node.dom = self
