...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/ppmc/__init__.py
# Compiled at: 2011-09-28 07:42:08
"""
...
"""
    ...
    if N < 2 or N != len(b):
        ...
        if numerator != 0.0:
            return sum(map(lambda x, y: x * y, a_diff, b_diff)) / numerator
        return -1.0
...
"""
    ...
    def __call__(self, b):
        if self._N is None or len(b) != self._N:
            return
        else:
            ...
            if numerator != 0:
                return sum(map(lambda x, y: x * y, self._a_diff, b_diff)) / numerator
            return -1.0
