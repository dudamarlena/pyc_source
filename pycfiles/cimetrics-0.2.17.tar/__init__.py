...
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/cimese_net/__init__.py
# Compiled at: 2018-07-24 13:50:57
import pkg_resources, gzip, math
DATA_PATH = pkg_resources.resource_filename('cimese_net', 'data/')
import keras
from keras import applications
import numpy as np, cv2, pickle as dill
...
def similarity_score(full_medians, clip_medians, length):
    ...
    for i in range(len(extended)):
        sim = math.sqrt(sum(math.pow(a - b, 2) for a, b in zip(extended[i].reshape(-1, 1), np.array(template).reshape(-1, 1))))
        all_corr.append(sim)
