...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/GPSPhoto/gpsphoto.py
# Compiled at: 2019-06-11 17:40:47
import sys, webbrowser, exifread, json
...
class GPSInfo(object):
    ...
    def __validateCoord(self, coord):
        ...
        if not (isinstance(coord, tuple) or isinstance(coord, list)):
            ...
            raise InvalidLatitude(str('Invalid latitude %f, valid range ' + '(-89.999999-89.999999)') % coord[0])
        elif coord[1] > 179.999999 or coord[1] < -179.999999:
            raise InvalidLongitude(str('Invalid longitude %f, valid range ' + '(-179.999999-179.999999)') % coord[1])
        self.__coord = coord
    ...
    def __validateTimeStamp(self, timeStamp):
        """Validate timeStamp"""
        if not (timeStamp is None or isinstance(timeStamp, datetime) or isinstance(timeStamp, str)):
            ...
            try:
                ...
                raise ValueError(str("timeStamp '%s' invalid format, valid " + "format 'YYYY:MM:DD hh:mm:ss' " + "i.e.'1970:01:01 09:05:05'") % timeStamp)
    ...
    def getGPSFormattedDate(self):
        """Returns GPS Formatted Date in str form
            i.e. '1970:05:01'"""
        return '%s' % str.encode(self.timeStamp.split(' ')[0])
    ...
    def getElevation(self):
        ...
        """
        if self.__googleApiKey is not None:
            coords = self.__coord
            url = str('https://maps.googleapis.com/maps/api/elevation/json?' + 'locations=' + str(coords[0]) + ',' + str(coords[1]) + str('&key=%s' % self.__googleApiKey))
            website = urlopen(url)
...
class GPSPhoto(object):
    ...
    def __validateCoordAndQuadArguments(self, coord, quad):
        ...
        if not str(quad).upper()[0] == 'N' and not str(quad).upper()[0] == 'S' and not str(quad).upper()[0] == 'E' and not str(quad).upper()[0] == 'W':
        ...
        if len(coord) >= 2:
            ...
            if not (isinstance(coord[1], int) or isinstance(coord[1], float)):
                raise ValueError('coord minute value must be of types int or ' + 'float')
            if str(quad).upper()[0] == 'N' or str(quad).upper()[0] == 'S':
                if coord[0] < 0 or coord[0] > 89:
                    raise InvalidLatitude(str('Invalid latitude degrees %d, ' + 'valid range (0-89)') % coord[0])
            elif coord[0] < 0 or coord[0] > 179:
                raise InvalidLongitude(str('Invalid longitude degrees %d, ' + 'valid range (0-179)') % coord[0])
            if coord[1] < 0 or coord[1] > 59:
                raise InvalidMinutes('Invalid minutes %d, valid range (0-59)' % coord[1])
        if len(coord) == 3:
            ...
            if coord[2] < 0 or coord[2] > 59.999999:
                raise InvalidSeconds(str('Invalid seconds %f, valid range ' + '(0-59.999999)') % coord[2])
    ...
    def __validateLoad(self):
        ...
        if self.__gpsRawDict is None:
            if not self.__filename == '':
                raise IOError('%s file did not load, corrupt or invalid file' % self.__filename)
            else:
    ...
    def decimal2Degree(self, coord):
        ...
        secLat = int(round((lat - int(lat) - float(minLat) / 60) * 36000000))
        secLon = int(round((lon - int(lon) - float(minLon) / 60) * 36000000))
        gpsDict[2] = (
         (
          degLat, 1), (minLat, 1), (secLat, 10000))
    ...
    def __getGPSData(self):
        ...
        self.__gpsDict = None
        if self.__gpsRawDict is not None:
            tags = self.__gpsRawDict
            self.__gpsDict = {}
            for tag in list(tags.keys()):
                if tag == 'GPS GPSTimeStamp':
                    t = [ x.strip() for x in str(tags[tag]).strip('[').strip(']').split(',') ]
                    m = t[2] if '/' not in t[2] else str(int(t[2].split('/')[0]) / int(t[2].split('/')[1]))
                    self.__gpsDict['UTC-Time'] = t[0] + ':' + t[1] + ':' + m
                elif tag == 'GPS GPSDate':
                    d = str(tags[tag]).split(':')
                    self.__gpsDict['Date'] = str(str(d[1]) + '/' + str(d[2]) + '/' + str(d[0]))
                elif tag == 'GPS GPSLatitude':
                    itude = str(tags[tag]).strip('[').strip(']').split(',')
                    var = parseItude(itude)
                    if len(var) > 1:
                        lat = self.coord2decimal(var, str(tags['GPS GPSLatitudeRef']))
                    else:
                        lat = float(var[0])
                    ...
                    var = parseItude(itude)
                    if len(var) > 1:
                        lng = self.coord2decimal(var, str(tags['GPS GPSLongitudeRef']))
                    else:
                        lng = float(var[0])
    ...
    def modGPSData(self, gpsInfo, newFileName):
        ...
        def getBaseExif(timeStamp):
            ...
            exif = {}
            exif['Exif'] = {}
            exif['Exif'][36867] = '%s' % str.encode(timeStamp)
            exif['0th'] = {}
            exif['0th'][34665] = 58
            exif['0th'][306] = '%s' % str.encode(timeStamp)
            exif['Interop'] = {}
            exif['1st'] = {}
            exif['1st'][513] = 126
            exif['1st'][514] = 0
        ...
        gpsDict[5] = 1 if gpsInfo.alt < 0 else 0
        gpsDict[6] = (abs(gpsInfo.alt) * 1000, 1000)
        gpsDict[27] = 'ASCII\x00\x00\x00GPS'
        gpsDict[7] = gpsInfo.getGPSFormattedTime()
        gpsDict[29] = '%s' % gpsInfo.getGPSFormattedDate()
        for key in gpsDict:
            ...
            try:
                if sys.argv[1].upper()[1] == 'H':
                    ...
                    try:
                        ...
                        sys.stderr.write(str('%s does not contain GPS ' + 'Data\n') % sys.argv[2])
                        sys.exit(1)

                elif sys.argv[1].upper()[1] == 'E':
                    if len(sys.argv) < 5:
                        notEnoughArguments()
                    alt = getElevation((float(sys.argv[2]), float(sys.argv[3])), sys.argv[4])
                ...
                sys.exit(1)
            except (InvalidLatitude, InvalidLongitude, OSError, ValueError, IOError) as e:
                sys.stderr.write('\n' + str(e) + '\n')
                sys.exit(1)
