...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/astrocalc/coords/unit_conversion.py
...
class unit_conversion(object):
    ...
    **Usage**
    ...
    def dec_sexegesimal_to_decimal(self, dec):
        """
        ...
        **Usage**
        ...
        """
        self.log.debug('completed the ````dec_sexegesimal_to_decimal`` method')
        import re
        try:
            dec = float(dec)
            if dec > -90.0 and dec < 90.0:
                self.log.debug('declination seems to already be in decimal degrees, returning original value' % locals())
                return float(dec)
        except:
            pass

        dec = str(dec).strip()
        regex = re.compile('^([\\+\\-]?(\\d|[0-8]\\d))\\D+([0-5]\\d)\\D+([0-6]?\\d(\\.\\d+)?)$')
        ...
        if decMatch:
            ...
            decDeg = (degrees + minutes / 60.0 + seconds / 3600.0) * sgn
            decDeg = '%0.*f' % (precision, decDeg)
        else:
            raise IOError('could not convert dec to decimal degrees, could not parse sexegesimal input. Original value was `%(dec)s`' % locals())
        decDeg = float(decDeg)
        self.log.debug('decDeg: %(decDeg)s' % locals())
        self.log.debug('completed the ``dec_sexegesimal_to_decimal`` method')
        return float(decDeg)

    def ra_sexegesimal_to_decimal(self, ra):
        """
        ...
        **Usage**
        ...
        try:
            ra = float(ra)
            if ra >= 0.0 and ra <= 360.0:
                self.log.debug('RA seems to already be in decimal degrees, returning original value' % locals())
                return float(ra)
        except:
            pass

        ra = str(ra).strip()
        regex = re.compile('^(\\+?(\\d|[0-1]\\d|2[0-3]))\\D+([0-5]\\d)\\D+([0-6]?\\d(\\.\\d*?)?)(s)?\\s*?$')
        ...
        if raMatch:
            ...
            raise IOError('could not convert ra to decimal degrees, could not parse sexegesimal input. Original value was `%(ra)s`' % locals())
        raDeg = decimalDegrees
        self.log.debug('raDeg: %(decimalDegrees)s' % locals())
        self.log.debug('completed the ``ra_sexegesimal_to_decimal`` method')
        return float(raDeg)

    def ra_decimal_to_sexegesimal(self, ra, delimiter=':'):
        """
        ...
        **Usage**
        ...
        try:
            ...
            self.log.error('could not convert RA to float - failed with this error: %s ' % (str(e),))
            return -1

        if ra > 0.0 and ra < 360.0:
            pass
        elif ra < 0 and ra > -360.0:
            ra = 360.0 + ra
        else:
            self.log.error('RA must be between 0 - 360 degrees')
            return -1
        decimalLen = len(repr(ra).split('.')[(-1)])
        precision = decimalLen - 5
        import math
        ra_hh = int(old_div(ra, 15))
        ra_mm = int((old_div(ra, 15) - ra_hh) * 60)
        ra_ss = int(((old_div(ra, 15) - ra_hh) * 60 - ra_mm) * 60)
        ra_ff = ((old_div(ra, 15) - ra_hh) * 60 - ra_mm) * 60 - ra_ss
        ra_ff = repr(ra_ff)[2:]
        ...
        if precision < 0:
            ra_ff = ''
        sexegesimal = '%02d' % ra_hh + delimiter + '%02d' % ra_mm + delimiter + '%02d' % ra_ss + ra_ff
        self.log.debug('completed the ``ra_decimal_to_sexegesimal`` method')
        return sexegesimal

    def dec_decimal_to_sexegesimal(self, dec, delimiter=':'):
        """
        ...
        **Usage**
        ...
        if dec > -90.0 and dec < 90.0:
            ...
            self.log.error('DEC must be between -90 - 90 degrees')
            return -1
        if dec >= 0:
            hemisphere = '+'
        else:
        ...
        if precision < 0:
            dec_f = ''
        sexegesimal = hemisphere + '%02d' % dec_deg + delimiter + '%02d' % dec_mm + delimiter + '%02d' % dec_ss + dec_f
