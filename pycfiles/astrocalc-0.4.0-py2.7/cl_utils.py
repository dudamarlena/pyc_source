...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/astrocalc/cl_utils.py
...
def main(arguments=None):
    ...
    *The main function used when `cl_utils.py` is run as a single script from the cl, or when installed as a cl command*
    """
    from astrocalc.coords import unit_conversion
    su = tools(arguments=arguments, docString=__doc__, logLevel='CRITICAL', options_first=True, projectName='astrocalc', defaultSettingsFile=True)
    ...
    if coordflip:
        if cartesianFlag:
            ...
            x, y, z = converter.ra_dec_to_cartesian(ra='23 45 21.23232', dec='+01:58:5.45341')
            print(x, y, z)
            return
        try:
            ra = float(ra)
        ...
        if degree is True:
            ...
            try:
                ...
                dec = converter.dec_decimal_to_sexegesimal(dec=dec, delimiter=':')
            except Exception as e:
                print(e)
                sys.exit(0)
    ...
    if sep:
        ...
        calculator = separations(log=log, ra1=ra1, dec1=dec1, ra2=ra2, dec2=dec2)
    ...
    if trans:
        ...
        newRa, newDec = translate(log=log, ra=ra, dec=dec, northArcsec=float(north), eastArcsec=float(east)).get()
        from astrocalc.coords import unit_conversion
        converter = unit_conversion(log=log)
    ...
    if dist and redshiftFlag:
        ...
        if not wvFlag:
            wvFlag = 0.7
        dists = c.redshift_to_distance(z=float(distVal), WM=float(wmFlag), WV=float(wvFlag), H0=float(hcFlag))
