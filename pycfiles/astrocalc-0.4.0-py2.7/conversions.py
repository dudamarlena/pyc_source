...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/astrocalc/times/conversions.py
...
class conversions(object):
    ...
    **Usage**
    ...
    def ut_datetime_to_mjd(self, utDatetime):
        """*ut datetime to mjd*
        ...
        **Usage**
        ...
        matchObject = re.match('^(?P<year>\\d{4})\\D?(?P<month>(0\\d|1[0-2]))\\D?(?P<day>([0-2]\\d|3[0-1])(\\.\\d+)?)(\\D?(?P<hours>([0-1]\\d|2[0-3]))\\D?(?P<minutes>\\d{2})(\\D?(?P<seconds>\\d{2}(\\.\\d*?)?))?)?s?$', utDatetime)
        if not matchObject:
            self.log.error('UT Datetime is not in a recognised format. Input value was `%(utDatetime)s`' % locals())
            raise IOError('UT Datetime is not in a recognised format. Input value was `%(utDatetime)s`' % locals())
        ...
        extraTime = 0.0
        if '.' in repr(seconds):
            extraTime = old_div(float('.' + repr(seconds).split('.')[(-1)]), 86400.0)
        t = (
    ...
    def decimal_day_to_day_hour_min_sec(self, daysFloat):
        ...
        **Usage**
        ...
        """
        self.log.debug('completed the ````decimal_day_to_day_hour_min_sec`` method')
