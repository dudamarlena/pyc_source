...
# Embedded file name: build/bdist.linux-x86_64/egg/pypoly/component/plugin/session_file.py
# Compiled at: 2011-11-24 07:12:27
__doc__ = '\nStore session data in files.\n'
import os, time, pickle, types, pypoly
...
class FileSessionHandler(SessionPlugin):
    ...
    def __init__(self, session_id):
        ...
        if self.session_id == None:
            ...
            if filename == None or not os.path.exists(filename) or not os.path.isfile(filename):
                self.create_session()
        self._lock()
        self._load()
        return
    ...
    def _get_session_filename(self):
        ...
        filename = os.path.abspath(os.path.join(session_path, self.session_id))
    ...
    def _lock(self):
        ...
        while True:
            try:
                fp_lock = os.open(self._get_lock_filename(), os.O_CREAT | os.O_WRONLY | os.O_EXCL)
                os.close(fp_lock)
                pypoly.log.debug('Waited %f seconds to lock session' % (time.time() - start))
                break
            except OSError, msg:
                time.sleep(0.1)
                if time.time() - start > 20:
                    pypoly.log.warning('Waited over %f seconds to lock session. Stopping!' % (time.time() - start))
    ...
    def set(self, name, value):
        if self.mode == pypoly.session.MODE_READONLY:
            pypoly.log.warning("Can't set session value. In read only mode.")
            return False
