...
# Embedded file name: build/bdist.linux-x86_64/egg/pypoly/_hook.py
# Compiled at: 2011-11-05 07:06:25
import pypoly
...
class HookHandler(dict):
    ...
    def __init__(self):
        ...
        Init the Hooksystem
        """
        self._hooks = {'content.web.webpage.head': {}, 'http.request.serve.pre': {}}
    ...
    def get_wrapped(self, *hooks):
        ...
        for hook in hooks:
            if hook[0] in self._hooks:
                wrapper.register(hook[0], self.get_list(hook[0]), *hook[1], **hook[2])
    ...
    def register(self, namespace, name, function):
        ...
        if namespace in self._hooks:
            ...
            pypoly.log.warning('Unknown namespace: %(namesapce)s' % dict(namesapce=namespace))
...
class HookWrapper(object):
    ...
    def get(self, namespace):
        if namespace not in self._hooks:
            pypoly.log.warning('Unknown namespace: %(namesapce)s' % dict(namesapce=namespace))
            return []
        results = []
        for hook in self._hooks[namespace][0]:
            hook_res = hook(*self._hooks[namespace][1], **self._hooks[namespace][2])
