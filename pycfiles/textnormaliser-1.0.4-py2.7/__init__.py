...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/textnormaliser/__init__.py
# Compiled at: 2018-12-19 07:42:26
import argparse, codecs, multiprocessing
from multiprocessing.dummy import Pool as ThreadPool
import operator, os, pickle, re, string, sys, threading
from langdetect import detect_langs
from langdetect import DetectorFactory
import nltk, tqdm, spacy, langdetect
...
def _normalise_text(text, nlp):
    ...
    for entity in doc.ents:
        ...
        new_text = re.sub('[^\\w\\s]', '_', new_text)
    ...
    for pos_tag in pos_tags:
        if not _is_anything(pos_tag[0]) or pos_tag[0] in replaced_text:
            ...
            continue
        pos_tokens.append(('{}_{}').format(pos_tag[1], pos_tag[0]))

    lower_text = (' ').join(pos_tokens).lower()
    if label:
        lower_text = label + ' ' + lower_text
    if lower_text[(-1)] != '\n':
        lower_text += '\n'
    return lower_text
