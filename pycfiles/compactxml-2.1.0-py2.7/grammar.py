...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/compactxml/grammar.py
# Compiled at: 2011-02-06 01:33:31
...
def create_grammar():
    ...
    def continuation(delimiter):
        return pyparsing.LineEnd().suppress() + pyparsing.FollowedBy(delimiter) + addons.checkIndent(aIndentations) + pyparsing.Literal(delimiter)
    ...
    def start_command(command):
        return pyparsing.Literal('?') + command + pyparsing.Empty()

    internalComment = start_command('comment') - pyparsing.Optional(unquotedLiteral)
    ...
    simple = statement(comment | processing | namespace | attributeExpansion | attribute | text | internalComment | restartIndentation | resumeIndentation | default | variable | contents)
    ...
    commandStatements = pyparsing.Group(statement(internalComment | load | encoding | default | variable) | attributeDefinitionStatement | elementDefinitionStatement)
    doctypeStatements = pyparsing.Group(statement(doctype))
    rootStatements = pyparsing.Group(statement(comment | processing | internalComment) | elementStatement)
    grammar = pyparsing.Optional(endStatement) + pyparsing.ZeroOrMore(addons.checkIndent(aIndentations) + commandStatements) + pyparsing.Optional(addons.checkIndent(aIndentations) + doctypeStatements) + pyparsing.OneOrMore(addons.checkIndent(aIndentations) + rootStatements) + pyparsing.StringEnd()
