...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/ims_lti_py/tool_config.py
...
class ToolConfig:
        ...
        for child in root.getchildren():
            ...
            if 'custom' in child.tag:
                for custom_child in child.getchildren():
                    self.custom_params[custom_child.attrib['name']] = custom_child.text
                ...
                for ext_child in child.getchildren():
                    if 'property' in ext_child.tag:
                        ...
                        for option_child in ext_child.getchildren():
                            options[option_child.attrib['name']] = option_child.text
    ...
    def to_xml(self, opts=defaultdict(lambda : None)):
        ...
        if any('vendor_' + key for key in vendor_keys) or self.vendor_contact_email:
            ...
            for key in vendor_keys:
                if getattr(self, 'vendor_' + key) != None:
                    v_node = etree.SubElement(vendor_node, '{%s}%s' % (NSMAP['lticp'], key))
                    v_node.text = getattr(self, 'vendor_' + key)

            if getattr(self, 'vendor_contact_email'):
                v_node = etree.SubElement(vendor_node, '{%s}%s' % (NSMAP['lticp'], 'contact'))
                c_name = etree.SubElement(v_node, '{%s}%s' % (NSMAP['lticp'], 'name'))
                c_name.text = self.vendor_contact_name
                c_email = etree.SubElement(v_node, '{%s}%s' % (NSMAP['lticp'], 'email'))
            ...
            for key, params in sorted(self.extensions.items()):
                ...
                for key, val in params.iteritems():
                    if isinstance(val, dict):
                        options_node = etree.SubElement(extension_node, '{%s}%s' % (NSMAP['lticm'], 'options'), name=key)
                        for key, val in val.iteritems():
                            property_node = etree.SubElement(options_node, '{%s}%s' % (NSMAP['lticm'], 'property'), name=key)
        ...
        if getattr(self, 'cartridge_icon'):
            identifierref = etree.SubElement(root, 'cartridge_icon', identifierref=self.cartridge_icon)
