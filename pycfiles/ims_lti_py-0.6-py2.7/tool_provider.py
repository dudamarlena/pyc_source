...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/ims_lti_py/tool_provider.py
# Compiled at: 2013-02-01 11:39:40
from launch_params import LaunchParamsMixin
from request_validator import RequestValidatorMixin, FlaskRequestValidatorMixin, DjangoRequestValidatorMixin
...
class ToolProvider(LaunchParamsMixin, RequestValidatorMixin, object):
    ...
    def has_role(self, role):
        ...
        """
        return self.roles and any([ re.search(role, our_role, re.I) for our_role in self.roles
                                  ])
    ...
    def build_return_url(self):
        ...
        if not self.launch_presentation_return_url:
            ...
            lti_message_fields = [
             'lti_errormsg', 'lti_errorlog',
             'lti_msg', 'lti_log']
            messages = dict([ (key, getattr(self, key)) for key in lti_message_fields if getattr(self, key, None)
                            ])
    ...
    def new_request(self):
        ...
        opts = {'consumer_key': self.consumer_key, 
