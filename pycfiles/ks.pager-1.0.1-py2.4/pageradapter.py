...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ks/pager/pageradapter/pageradapter.py
# Compiled at: 2007-11-08 12:36:37
...
import math, re, urlparse, urllib, cgi
...
class PagerAdapter(object):
    ...
    def chunkUrlList(self):
        ...
        return [ Chunk(title=unicode(x + 1), selected=x == currentChunk, url=self.getChunkUrl(False, chunkSize * (x - 1))) for x in xrange(start, stop) ]
    ...
    def chunk(self):
        ...
        if self.context is None or not self.count:
            return []
        return self.context.getChunk(self.start, self.chunkSize, *self.kv, **self.kw)
    ...
    def getChunkUrl(self, previous, start):
        ...
        params = cgi.parse_qsl(self.request['QUERY_STRING'])
