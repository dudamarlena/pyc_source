...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/conf/config_object.py
# Compiled at: 2016-08-01 11:57:45
import logging
logger = logging.getLogger(__name__)
logger.debug('%s loaded', __name__)
import os, ConfigParser, doorpi
...
class ConfigObject:
    ...
    def load_config(configfile, search_for_defaults=True):
        ...
        if search_for_defaults:
            configfile_name = ConfigObject.find_config(configfile)
        else:
    ...
    def set_value(self, section, key, value, log=True, password=False):
        ...
        if key not in self.__sections[section].keys():
            ...
            logger.debug("overwrite key %s in section %s from '%s' to '%s'", key, section, self.__sections[section][key], password_friendly_value)
    ...
    def delete_section(self, section, delete_empty_only=True, log=True):
        ...
        try:
            ...
            if log:
                logger.warning('delete section %s failed: %s', section, exp)
            return False

    def delete_key(self, section, key, log=True):
        try:
            if log:
                logger.info('delete key %s from section %s', key, section)
    ...
    def get_string(self, section, key, default='', log=True, password=False, store_if_not_exists=True):
        ...
        if value is None:
            value = default
            if store_if_not_exists:
                self.set_value(section, key, default, log, password)
    ...
    def get_sections(self, filter='', log=True):
        ...
        if log:
            logger.trace('get_sections returns %s', return_list)
        return return_list
