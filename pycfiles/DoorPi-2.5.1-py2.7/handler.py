...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/action/handler.py
...
class EventLog(object):
    ...
    def __init__(self, file_name):
        ...
        try:
            ...
            self._db = sqlite3.connect(database=file_name, timeout=1, check_same_thread=False)
    ...
    def get_event_log_entries(self, max_count=100, filter=''):
        ...
        for single_row in self.execute_sql(sql_statement):
            return_object.append({'event_id': single_row[0], 
    ...
    def insert_event_log(self, event_id, fired_by, event_name, start_time, additional_infos):
        sql_statement = ('\n        INSERT INTO event_log VALUES (\n            "{event_id}","{fired_by}","{event_name}",{start_time},"{additional_infos}"\n        );\n        ').format(event_id=event_id, fired_by=fired_by.replace('"', "'"), event_name=event_name.replace('"', "'"), start_time=start_time, additional_infos=str(additional_infos).replace('"', "'"))
        self.execute_sql(sql_statement)

    def insert_action_log(self, event_id, action_name, start_time, action_result):
        sql_statement = ('\n        INSERT INTO action_log VALUES (\n            "{event_id}","{action_name}",{start_time},"{action_result}"\n        );\n        ').format(event_id=event_id, action_name=action_name.replace('"', "'"), start_time=start_time, action_result=str(action_result).replace('"', "'"))
        self.execute_sql(sql_statement)
...
class EventHandler:
    ...
    def event_history(self):
        return self.db.get_event_log_entries()
    ...
    def additional_informations(self):
        return self.__additional_informations
    ...
    def register_event(self, event_name, event_source):
        ...
        if event_name not in self.__Events:
            self.__Events[event_name] = [
             event_source]
            if not silent:
                logger.trace('added event_name %s and registered source %s', event_name, event_source)
            ...
            logger.trace('nothing to do - event %s from source %s is already known', event_name, event_source)
    ...
    def fire_event_asynchron(self, event_name, event_source, kwargs=None):
        ...
        if self.__destroy and not silent:
            return False
        if not silent:
            logger.trace('fire Event %s from %s asyncron', event_name, event_source)
        return threading.Thread(target=self.fire_event_synchron, args=(
         event_name, event_source, kwargs), name='%s from %s' % (event_name, event_source)).start()
    ...
    def fire_event_asynchron_daemon(self, event_name, event_source, kwargs=None):
        ...
        t = threading.Thread(target=self.fire_event_synchron, args=(
         event_name, event_source, kwargs), name='daemon %s from %s' % (event_name, event_source))
        t.daemon = True
        t.start()
    ...
    def fire_event_synchron(self, event_name, event_source, kwargs=None):
        ...
        if self.__destroy and not silent:
            ...
            if not silent:
                self.db.insert_event_log(event_fire_id, event_source, event_name, start_time, kwargs)
            if event_source not in self.__Sources:
            ...
            if kwargs is None:
                kwargs = {}
            kwargs.update({'last_fired': str(start_time), 
            ...
            if not silent:
                logger.debug('[%s] fire for event %s this actions %s ', event_fire_id, event_name, self.__Actions[event_name])
            for action in self.__Actions[event_name]:
                if not silent:
                    logger.trace('[%s] try to fire action %s', event_fire_id, action)
                try:
                    result = action.run(silent)
                    if not silent:
                        self.db.insert_action_log(event_fire_id, action.name, start_time, result)
