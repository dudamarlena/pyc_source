...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/doorpi.py
# Compiled at: 2016-08-01 11:57:45
import logging
logger = logging.getLogger(__name__)
logger.debug('%s loaded', __name__)
import sys, argparse, time, os, datetime, cgi, tempfile, metadata
...
class DoorPiShutdownAction(SingleAction):
    pass


class DoorPiNotExistsException(Exception):
    pass


class DoorPiEventHandlerNotExistsException(Exception):
    pass


class DoorPiRestartException(Exception):
    pass
...
class DoorPi(object):
    ...
    @property
    def name(self):
        return str(metadata.package)

    @property
    def name_and_version(self):
        return str(metadata.package) + ' - version: ' + metadata.version
        ...
        if 'KeyboardHandler' not in self.keyboard.name:
            ...
            for keyboard_name in self.keyboard.loaded_keyboards:
                ...
                for input_pin in self.config.get_keys(section_name, log=False):
                    self.event_handler.register_action('OnKeyPressed_' + keyboard_name + '.' + input_pin, self.config.get(section_name, input_pin))
    ...
    def destroy(self):
        ...
        if not self.event_handler or self.event_handler.threads == None:
            ...
            while timeout > 0 and self.modules_destroyed is not True:
                logger.debug('wait %s seconds for threads %s and %s event', timeout, len(self.event_handler.threads[1:]), len(self.event_handler.sources))
    ...
    def parse_string(self, input_string):
        ...
        for key in self.additional_informations.keys():
            ...
            infos_as_html += '</td><td>'
            infos_as_html += '<i>' + cgi.escape(str(self.additional_informations.get(key)).replace('\r\n', '<br />')) + '</i>'
            infos_as_html += '</td></tr>'

        infos_as_html += '</table>'
        mapping_table = {'INFOS_PLAIN': str(self.additional_informations), 
           'INFOS': infos_as_html, 
        ...
        if self.config:
            mapping_table.update({'LAST_SNAPSHOT': str(self.config.get_string('DoorPi', 'last_snapshot', log=False))})
        if self.keyboard and 'KeyboardHandler' not in self.keyboard.name:
            ...
            if self.keyboard and 'KeyboardHandler' in self.keyboard.name:
            ...
            for key in mapping_table.keys():
                parsed_string = parsed_string.replace('!' + key + '!', mapping_table[key])

            for key in self.additional_informations.keys():
                parsed_string = parsed_string.replace('!' + key + '!', str(self.additional_informations[key]))
