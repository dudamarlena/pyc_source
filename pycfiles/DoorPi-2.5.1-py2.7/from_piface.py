...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/keyboard/from_piface.py
...
def get(**kwargs):
    return PiFace(**kwargs)


class PiFace(KeyboardAbstractBaseClass):

    def __init__(self, input_pins, output_pins, keyboard_name, bouncetime, polarity=0, pressed_on_key_down=True, *args, **kwargs):
        logger.debug('__init__(input_pins = %s, output_pins = %s, polarity = %s)', input_pins, output_pins, polarity)
        ...
        for input_pin in self._InputPins:
            self.__listener.register(pin_num=input_pin, direction=p.IODIR_BOTH, callback=self.event_detect, settle_time=bouncetime / 1000)
