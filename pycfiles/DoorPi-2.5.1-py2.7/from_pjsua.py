...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/sipphone/from_pjsua.py
...
class Pjsua(SipphoneAbstractBaseClass):
    ...
    def sound_codecs(self):
        try:
            ...
            for codec in self.lib.enum_codecs():
                all_codecs.append({'name': codec.name, 
    ...
    def current_call_dump(self):
        try:
            return {'direction': 'incoming' if self.current_call.info().role == 0 else 'outgoing', 'remote_uri': self.current_call.info().remote_uri, 
    ...
    def thread_register(self, name):
        return self.lib.thread_register(name)
    ...
    def start(self):
        ...
        self.__Lib.init(ua_cfg=doorpi.sipphone.pjsua_lib.Config.create_UAConfig(), media_cfg=doorpi.sipphone.pjsua_lib.Config.create_MediaConfig(), log_cfg=doorpi.sipphone.pjsua_lib.Config.create_LogConfig())
        logger.debug('init transport')
        transport = self.__Lib.create_transport(type=pj.TransportType.UDP, cfg=doorpi.sipphone.pjsua_lib.Config.create_TransportConfig())
        logger.debug('Listening on: %s', str(transport.info().host))
        logger.debug('Port: %s', str(transport.info().port))
        logger.debug('Lib.start()')
        self.lib.start(0)
        DoorPi().event_handler.register_action(event_name='OnTimeTick', action_object='pjsip_handle_events:50')
        logger.debug('init Acc')
        self.current_account_callback = SipPhoneAccountCallBack()
        self.__account = self.__Lib.create_account(acc_config=doorpi.sipphone.pjsua_lib.Config.create_AccountConfig(), set_default=True, cb=self.current_account_callback)
    ...
    def self_check(self, *args, **kwargs):
        ...
        if self.current_call is not None:
            ...
            try:
                if self.current_call.info().call_time == 0 and self.current_call.info().total_time > self.call_timeout:
