...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/sipphone/from_linphone.py
...
class LinPhone(SipphoneAbstractBaseClass):
    ...
    def _create_payload_enum(self, payloads):
        try:
            ...
            for codec in payloads:
                all_codecs.append({'name': codec.mime_type, 
    ...
    def current_call_dump(self):
        try:
            return {'direction': 'incoming' if self.current_call.dir == 0 else 'outgoing', 'remote_uri': self.current_call.remote_address_as_string, 
                ...
                if firewall_policy == 'PolicyUseStun':
                    ...
                    if firewall_policy == 'PolicyUseIce':
                        ...
                        if len(self.core.sound_devices) == 0:
                            ...
                            logger.debug('------------------------------------')
                            for sound_device in self.core.sound_devices:
                                logger.debug('| %s | %s  | %s', 'X' if self.core.sound_device_can_capture(sound_device) else 'O', 'X' if self.core.sound_device_can_playback(sound_device) else 'O', sound_device)
    ...
    def self_check(self, *args, **kwargs):
        ...
        elif int(self.current_call_duration) > self.core.in_call_timeout - 0.5:
    ...
    def call(self, number):
        ...
        if not self.current_call:
            ...
            if self.current_call_duration <= 2:
                logger.debug('same call %s again while call is running since %s seconds? -> skip', self.core.current_call.remote_address.as_string_uri_only(), self.current_call_duration)
            else:
