...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/action/SingleActions/time_tick.py
# Compiled at: 2016-08-01 11:57:45
import logging
logger = logging.getLogger(__name__)
logger.debug('%s loaded', __name__)
from doorpi.action.base import SingleAction
import time, datetime, doorpi
    ...
    if datetime_now.hour != datetime_past.hour:
        ...
        for hour in HOUR_RANGE:
            if hour is datetime_now.hour:
                doorpi.DoorPi().event_handler('OnTimeHour%s' % hour, __name__)
    ...
    if datetime_now.minute != datetime_past.minute:
        ...
        for minute in MINUTE_RANGE:
            if minute is datetime_now.minute:
                doorpi.DoorPi().event_handler('OnTimeMinute%s' % minute, __name__)
