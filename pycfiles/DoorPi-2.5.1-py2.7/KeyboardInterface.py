...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/keyboard/KeyboardInterface.py
...
class KeyboardImportError(ImportError):
    pass


class UnknownOutputPin(Exception):
    pass
...
def load_single_keyboard(keyboard_name):
    ...
    bouncetime = doorpi.DoorPi().config.get_float(section_name, 'bouncetime', 2000, store_if_not_exists=store_if_not_exists)
    ...
    try:
        keyboard = importlib.import_module('doorpi.keyboard.from_' + keyboard_type).get(input_pins=input_pins, output_pins=output_pins, bouncetime=bouncetime, polarity=polarity, keyboard_name=keyboard_name, keyboard_type=keyboard_type, conf_pre=conf_pre, conf_post=conf_post, pressed_on_key_down=pressed_on_key_down)
        ...
        for keyboard_name in config_keyboards:
            ...
            for output_pin in output_pins:
                ...
                if output_pin_name in self.__OutputMappingTable:
                    logger.warning('overwriting existing name of outputpin "%s" (exists in %s and %s)', output_pin_name, self.__OutputMappingTable[output_pin_name], keyboard_name)
