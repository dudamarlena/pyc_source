...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/keyboard/from_gpio.py
...
class GPIO(KeyboardAbstractBaseClass):
    ...
    def __init__(self, input_pins, output_pins, conf_pre, conf_post, keyboard_name, bouncetime=200, polarity=0, pressed_on_key_down=True, *args, **kwargs):
        logger.debug('__init__(input_pins = %s, output_pins = %s, bouncetime = %s, polarity = %s)', input_pins, output_pins, bouncetime, polarity)
        ...
        if pull_up_down == 'PUD_DOWN':
            ...
            for input_pin in self._InputPins:
                RPiGPIO.add_event_detect(input_pin, RPiGPIO.BOTH, callback=self.event_detect, bouncetime=int(bouncetime))
