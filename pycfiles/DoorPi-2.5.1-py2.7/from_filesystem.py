...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/keyboard/from_filesystem.py
...
class FileSystem(KeyboardAbstractBaseClass, FileSystemEventHandler):
    ...
    def __init__(self, input_pins, output_pins, conf_pre, conf_post, keyboard_name, polarity=0, *args, **kwargs):
        logger.debug('FileSystem.__init__(input_pins = %s, output_pins = %s, polarity = %s)', input_pins, output_pins, polarity)
        ...
        if self.__base_path_input == '':
            raise MissingMandatoryParameter('base_path_input in %s ' % section_name)
        if self.__base_path_output == '':
            raise MissingMandatoryParameter('base_path_output in %s ' % section_name)
