...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/doorpi/status/webserver_lib/request_handler.py
...
class WebServerRequestHandlerShutdownAction(SingleAction):
    pass
...
class DoorPiWebRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_control(self, control_order, para):
        ...
        try:
            ...
            if control_order == 'trigger_event':
                ...
                result_object['message'] = 'config_value_set %s' % ('success' if result_object['success'] else 'failed')
            elif control_order == 'config_value_delete':
                result_object['success'] = control_config_delete_key(**para)
                result_object['message'] = 'config_value_delete %s' % ('success' if result_object['success'] else 'failed')
            elif control_order == 'config_save':
                result_object['success'] = control_config_save(**para)
                result_object['message'] = 'config_save %s' % ('success' if result_object['success'] else 'failed')
    ...
    def clear_parameters(self, raw_parameters):
        if 'module' not in raw_parameters.keys():
            raw_parameters['module'] = []
        if 'name' not in raw_parameters.keys():
            raw_parameters['name'] = []
        if 'value' not in raw_parameters.keys():
    ...
    def create_virtual_resource(self, path, raw_parameters):
        ...
        try:
            if path.path == '/mirror':
                ...
                return_object = doorpi.DoorPi().get_status(modules=raw_parameters['module'], name=raw_parameters['name'], value=raw_parameters['value']).dictionary
                ...
                return_object = self.parse_content(return_object, MODULE_AREA_NAME=raw_parameters['module'][0] or '', MODULE_NAME=raw_parameters['name'][0] or '')
                raw_parameters['output'] = 'html'
        except Exception as exp:
            return_object = dict(error_message=str(exp))

        if 'output' not in raw_parameters.keys():
    ...
    def return_virtual_resource(self, prepared_object, return_type='json'):
        if isinstance(return_type, list) and len(return_type) > 0:
        ...
        if return_type in ('json_beautified_parsed', 'json.beautified.parsed', 'beautified.json.parsed',
                           ''):
    ...
    def return_file_content(self, path):
        ...
        return self.return_message(content, mime)
    ...
    def login_form(self):
        ...
        self.return_message(message=self.parse_content(login_form_content), content_type='text/html; charset=utf-8', http_code=401)
        return True
        ...
        try:
            try:
                ...
                for match in matches:
                    if match not in mapping_table.keys():
                        continue
