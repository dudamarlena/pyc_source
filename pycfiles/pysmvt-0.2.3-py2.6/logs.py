...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysmvt/logs.py
...
def _create_handlers_from_settings(settings):
    ...
    if settings.logs.errors.enabled:
        ...
        error_handler = RotatingFileHandler(path.join(settings.dirs.logs, 'errors.log'), maxBytes=settings.logs.max_bytes, backupCount=settings.logs.backup_count)
    ...
    if settings.logs.application.enabled:
        ...
        formatter = logging.Formatter(format_str)
        app_handler = RotatingFileHandler(path.join(settings.dirs.logs, 'application.log'), maxBytes=settings.logs.max_bytes, backupCount=settings.logs.backup_count)
