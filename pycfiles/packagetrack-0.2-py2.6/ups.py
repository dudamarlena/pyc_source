...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/packagetrack/ups.py
# Compiled at: 2010-06-21 16:37:57
import urllib
...
class UPSInterface(object):
    ...
    def build_access_request(self):
        ...
        d = {'AccessRequest': {'AccessLicenseNumber': config.get('UPS', 'license_number'), 'UserId': config.get('UPS', 'user_id'), 
    ...
    def build_track_request(self, tracking_number):
        ...
        d = {'TrackRequest': {'Request': req, 'TrackingNumber': tracking_number}}
        return dict_to_xml(d)

    def build_request(self, tracking_number):
        return self.build_access_request() + self.build_track_request(tracking_number)
    ...
    def parse_response(self, raw):
        ...
        est_delivery_date = datetime.strptime(root['Shipment']['ScheduledDeliveryDate'], '%Y%m%d')
        ...
        return TrackingInfo(last_update=last_update, delivery_date=est_delivery_date, status=status)
    ...
    def url(self, tracking_number):
        ...
        return 'http://wwwapps.ups.com/WebTracking/processInputRequest?TypeOfInquiryNumber=T&InquiryNumber1=%s' % tracking_number