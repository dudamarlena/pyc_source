...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/storyboard.py
# Compiled at: 2018-11-27 05:09:47
# Size of source mod 2**32: 61788 bytes
...
try:
    import numpy.random as _poisson
    import numpy.random as _exponential
    logger.debug('Using numpy for faster and better random variables.')
except ImportError:
    import rumba.recpoisson as _poisson
...
def _execute_command(node, cmd, callback=None, e_callback=None, as_root=False):
    if e_callback is None:

        def e_callback(e):
            logger.warning('Could not execute command "%s" on node "%s". Error: %s.', cmd, node.name, e)
    ...
    POOL.apply_async((node.execute_command),
      args=(
     cmd,),
...
class Client(_SBEntity):
    ...
    def __init__(self, ap, nodes=None, options=None, shutdown='kill <pid>', c_id=None, as_root=False):
        """
    ...
    def process(self, duration, node=None, proc_id=None):
        ...
        return ClientProcess(proc_id, self.id, self.startup, duration, node, self.shutdown, self.as_root)


class ClientProcess(_SBEntity):
    __doc__ = 'Class representing a running client application process on a node'

    def __init__(self, proc_id, ap_id, startup, duration, node, shutdown='kill <pid>', as_root=False):
        """
    ...
    def run(self):
        """Starts this process"""
        logger.debug('Starting client app %s on node %s with duration %s.', self.ap_id, self.node.name, self.duration)
        cmd = self._make_run_cmd()
        self.running = True
        try:
            self.pid = self.node.execute_command(cmd, as_root=(self.as_root))
        except ssh_support.SSHException:
            logger.warning('Could not start client %s on node %s.', self.ap_id, self.node.name)

        logger.debug('Client app %s on node %s got pid %s.', self.ap_id, self.node.name, self.pid)
    ...
    def run_async(self):
        ...
        def e_callback(e):
            logger.warning('Could not start client %s on node %s. Error: %s.', self.ap_id, self.node.name, e)

        logger.debug('Starting client app %s on node %s with duration %s (async).', self.ap_id, self.node.name, self.duration)
        cmd = self._make_run_cmd()
        self.running = True
    ...
    def stop(self):
        """Stops this process"""
        if self.shutdown != '':
            logger.debug('Killing client %s on node %s.', self.ap_id, self.node.name)

            def callback(_):
                logger.debug('Client %s on node %s has terminated.', self.ap_id, self.node.name)
...
class Server(_SBEntity):
    __doc__ = 'Class representing a server app running in the experiment'
    current_id = -1
    ...
    def __init__(self, ap, arrival_rate, mean_duration, options=None, max_clients=float('inf'), clients=None, nodes=None, min_duration=2, s_id=None, as_root=False, difs=None):
        """
    ...
    def make_client_process(self, duration=None, node=None, proc_id=None, client=None):
        ...
        return c.process(duration=(float('%.2f' % (d,))),
          node=n,
          proc_id=p)

    def _make_run_cmd(self, node):
        run_cmd = self.ap + (' ' + self.options if self.options is not None else '')
    ...
    def run(self):
        ...
        for node in self.nodes:
            ...
            logger.debug('Starting server %s on node %s.', self.id, node.name)
    ...
    def stop(self):
        ...
        for node, pid in self.pids.items():
            logger.debug('Killing server %s on node %s.', self.id, node.name)
            try:
                _execute_command(node, ('kill %s' % (pid,)), as_root=(self.as_root))
            except ssh_support.SSHException:
                logger.warning('Could not kill server %s on node %s.', self.id, node.name)


class StoryBoard(_SBEntity):
    __doc__ = 'Class representing the storyboard of an experiment'
    ...
    def _validate_and_add_server(self, s, n=None):
        if self.experiment is None:
            raise ValueError('Cannot add a server before setting the experiment.')
        else:
        ...
        for node in s.nodes:
            if node not in self.experiment.nodes:
                raise ValueError('Cannot run server on node %s, not in experiment.' % (
                 node.name,))
    ...
    def schedule_action(self, call, args=None, kwargs=None, c_time=None, trigger=None, ev_id=None):
        """
    ...
    def schedule_command(self, t, node, command):
        ...
        if node not in self.experiment.nodes:
            raise ValueError('Cannot run command on node %s, not in experiment.' % (
             node.name,))
    ...
    def run_client_of(self, server, duration=None, node=None, proc_id=None, callback=None):
        """
    ...
    def run_client(self, client, duration, node=None, proc_id=None, callback=None, asyn=True):
        """
        ...
        if isinstance(client, str):
            ...
            if len(client.nodes) == 0:
                raise ValueError('No nodes registered for client %s', client.id)
            node = random.choice(client.nodes)
        else:
            if isinstance(node, str):
                node = self.node_map[node]
        if callback is None:
            ...
            if not hasattr(callback, '__len__'):
                callback = [
                 callback]
            else:
    ...
    def generate_script(self, clean=True):
        ...
        while t < self.duration:
            ...
            for server in self.server_apps.values():
                ...
                for d, n, p, c in c_l:
                    if d > self.duration - t:
                        continue
                    start = self._make_process_events(t, d, n, p, c)
                    self._script.add_event(start)

            t += self.SCRIPT_RESOLUTION

    def _make_process_events(self, t, d, n, p, c):
        start_action = functools.partial(self.run_client, c, d, n, p)
    ...
    def start(self):
        ...
        logger.debug('Command list is: %s.', {x:[(y.name, z) for y, z in t] for x, t in self.commands_list.items()})
        self.start_time = time.time()
        script = 'logname="$1"; shift; nohup "${@}" > /tmp/${logname}.rumba.log 2>&1  & echo "$!"'
        logger.debug('Writing utility startup script on nodes.')
        for node in self.node_map.values():
            _execute_command(node, "echo '#!/usr/bin/env bash' > startup.sh; echo '%s' >> startup.sh && chmod a+x startup.sh" % (
             script,))
    ...
    def fetch_logs(self, local_dir=None):
        ...
        if not os.path.isdir(local_dir):
            raise Exception('Destination "%s" is not a directory. Cannot fetch logs.' % local_dir)
        for node in self.node_map.values():
            dst_dir = os.path.join(local_dir, node.name)
            if not os.path.isdir(dst_dir):
                os.mkdir(dst_dir)
            logs_list = node.execute_command('ls /tmp/*.rumba.log || echo ""')
    ...
    def schedule_export_dif_bandwidth(self, t, filename, dif):
        ...
        if self.experiment is None:
            raise ValueError('An experiment is needed to schedule commands.')
        action = functools.partial(self.experiment.export_dif_bandwidth, filename, dif)
    ...
    def capture_traffic(self, start, end, node, dif):
        ...
        for ipcp in dif.ipcps:
            ...
            if not node.has_tcpdump:
                ssh_support.aptitude_install(self.experiment.testbed, node, ['tcpdump'])
                node.has_tcpdump = True
            pcap_file = node.name + '_' + dif.name + '_' + str(uuid.uuid4())[0:4] + '.pcap'
            tcpd_client = Client(ap='tcpdump', options=('-i %s -w %s' % (
             ipcp.ifname, pcap_file))).process(end - start, node, 'tcpdump_proc')
            self.schedule_action((tcpd_client.run), c_time=start)
            end_event = self.schedule_action((tcpd_client.stop), c_time=end)
            self.schedule_action((node.fetch_file),
              args=[
             pcap_file, self.experiment.log_dir],
              kwargs={'sudo': True},
              trigger=end_event)


class Event(object):
    __doc__ = 'Class representing an event in a `.StoryBoard`'
    cur_id = -1
    ...
    def __init__(self, action, ev_id=None, ev_time=None, trigger=None):
        ...
        if ev_time is None:
            if trigger is None:
                raise ValueError('No condition specified for event %s.' % self.id)
        self.action = action
        self.time = ev_time
    ...
    def _action_repr(self):
        if isinstance(self.action, functools.partial):
            ...
            arg_list += [k + '=' + self._action_arg_repr(v) for k, v in self.action.keywords.items()]
            args = ' '.join(arg_list)
        else:
    ...
    def _start(self):
        logger.debug('Running event %s, action %s', self.id, self._action_repr())
        self.pre_exec()
        if self.done:
 ...
 L.1423         0  LOAD_FAST                'self'
                2  LOAD_ATTR                time
                4  LOAD_CONST               None
                6  COMPARE_OP               is
                8  POP_JUMP_IF_TRUE     20  'to 20'
               10  LOAD_FAST                'cur_time'
               12  LOAD_FAST                'self'
               14  LOAD_ATTR                time
               16  COMPARE_OP               >
               18  JUMP_IF_FALSE_OR_POP    36  'to 36'
             20_0  COME_FROM             8  '8'

 L.1424        20  LOAD_FAST                'self'
               22  LOAD_ATTR                _trigger
               24  LOAD_CONST               None
               26  COMPARE_OP               is
               28  JUMP_IF_TRUE_OR_POP    36  'to 36'
               30  LOAD_FAST                'self'
               32  LOAD_ATTR                _trigger
               34  LOAD_ATTR                done
...
Parse error at or near `RETURN_VALUE' instruction at offset 36

    def _done(self):
        self.done = True
        if self.exception is not None:
            logger.warning('Event %s failed. %s: %s.', self.id, type(self.exception).__name__, str(self.exception))
        self.post_exec()
    ...
    def __repr__(self):
        if self._repr is None:
            e_name = self._Event__recover_e_name()
            if e_name is None:
                logger.warning('Event %s has no valid entity name, cannot serialize correctly.' % (
                 self.id,))
            self._repr = '%(prefix)s &%(label)s | %(entity)s %(method)s' % {'prefix':self._prefix_repr(), 
             'label':self.id, 
             'entity':e_name, 
...
class _Script(object):
    ...
    def _parse_entities(self):
        ...
        self._entities = {'sb':self._storyboard, 
    ...
    def _nested_iterator(self, d):
        for t in ((x, self.events_by_id[x]) for e_lst in d.values() for x in e_lst):
            yield t
    ...
    def _pop_non_str_arg(self, args_l):
        ...
        try:
            ...
            raise ValueError('Syntax error: %s is not a valid value. If it is supposed to be a string, enclose it in single quotes.' % (
             arg_s,))
    ...
    def write(self, buffer):
        ...
        ev_list.sort(key=(lambda x:         if x.time is not None:
x.time # Avoid dead code: float('+inf')))
    ...
    def _parse_conditions(self, *conditions):
        ...
        for cond in conditions:
            ...
            raise ValueError('Syntax error: cannot parse condition {}. Either the condition is malformed, or there are multiple triggers of the same type.')
