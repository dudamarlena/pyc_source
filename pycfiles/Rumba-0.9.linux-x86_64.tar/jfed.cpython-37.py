...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/testbeds/jfed.py
# Compiled at: 2018-08-31 05:12:48
# Size of source mod 2**32: 14850 bytes
import subprocess, getpass
import xml.dom.minidom as xml
import os.path, time, tarfile, sys
...
class Testbed(mod.Testbed):
    ...
    def __init__(self, exp_name, username, cert_file, exp_hours='2', proj_name='rumba', authority='wall2.ilabt.iminds.be', image=None, image_custom=False, image_owner=None, use_physical_machines=None):
        """
        ...
        self.jfed_jar = os.path.join(mod.cache_dir, 'jfed_cli/experimenter-cli.jar')
        ...
        if 'exogeni' in authority:
            ...
            if 'wall' in authority:
                ...
                if 'cloudlab' or 'geniracks' in authority:
                    self.authority = 'urn:publicid:IDN+' + authority + '+authority+cm'
                else:
        ...
        if use_physical_machines is None:
            ...
            self.image = 'urn:publicid:IDN+' + authority + '+image+' + image_owner + ':' + image
        else:
            self.image = None
        if not os.path.exists(self.jfed_jar):
            ...
            tar.extractall()
            tar.close()
            os.rename(os.path.join(os.getcwd(), 'jfed_cli'), os.path.join(mod.cache_dir, 'jfed_cli'))
            os.remove(tarball)
    ...
    def _create_rspec(self, experiment):
        ...
        top_el.setAttribute('xmlns:emulab', 'http://www.protogeni.net/resources/rspec/ext/emulab/1')
        top_el.setAttribute('xmlns:jfedBonfire', 'http://jfed.iminds.be/rspec/ext/jfed-bonfire/1')
        top_el.setAttribute('xmlns:delay', 'http://www.protogeni.net/resources/rspec/ext/delay/1')
        top_el.setAttribute('xmlns:jfed-command', 'http://jfed.iminds.be/rspec/ext/jfed-command/1')
        top_el.setAttribute('xmlns:client', 'http://www.protogeni.net/resources/rspec/ext/client/1')
        top_el.setAttribute('xmlns:jfed-ssh-keys', 'http://jfed.iminds.be/rspec/ext/jfed-ssh-keys/1')
        top_el.setAttribute('xmlns:jfed', 'http://jfed.iminds.be/rspec/ext/jfed/1')
        top_el.setAttribute('xmlns:sharedvlan', 'http://www.protogeni.net/resources/rspec/ext/shared-vlan/1')
        top_el.setAttribute('xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance')
        top_el.setAttribute('xsi:schemaLocation', 'http://www.geni.net/resources/rspec/3 http://www.geni.net/resources/rspec/3/request.xsd')
    ...
    def _swap_in(self, experiment):
        ...
        for xml_node in xml_nodes:
            ...
            if 'wall' in self.auth_name:
                ssh_support.set_http_proxy(self, node_n, 'https://proxy.atlantis.ugent.be:8080')
            ssh_support.execute_command(self, node_n.ssh_config, 'sudo touch /var/lib/cloud/instance/locale-check.skip || true')
            for intf in intfs:
                aux_mac_address = intf.getAttribute('mac_address')
                if 'wall' in self.auth_name:
                    mac = ':'.join([aux_mac_address[i:i + 2] for i in range(0, 12, 2)])
                else:
                    mac = aux_mac_address
                command = 'echo \'#!/usr/bin/env bash\' > mac2ifname.sh; echo \'for i in $(ls /sys/class/net/); do addr="$(cat /sys/class/net/$i/address)"; if [[ "$addr" == "$1" ]]; then echo "$i"; fi; done\' >> mac2ifname.sh'
                ssh_support.execute_command(self, node_n.ssh_config, command)
                ssh_support.execute_command(self, node_n.ssh_config, 'cd ~ && chmod a+x mac2ifname.sh')
                ifname = ssh_support.execute_command(self, node_n.ssh_config, './mac2ifname.sh ' + mac)
                i_name = intf.getAttribute('client_id')
                for ipcp in node_n.ipcps:
                    if isinstance(ipcp, mod.ShimEthIPCP) and self.if_id[ipcp] == i_name:
                        ipcp.ifname = ifname
                        if ifname is None or ifname == '':
                            raise Exception('Could not determine name of node %s interface %s' % (
                             node_n.name, mac))
                        else:
