...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/testbeds/emulab.py
...
class Testbed(mod.Testbed):
    ...
    def __init__(self, exp_name, username, proj_name, password='', url='wall2.ilabt.iminds.be', image=None):
        """
    ...
    def _full_name(self, node_name):
        ...
        """
        return node_name + '.' + self.exp_name + '.' + self.proj_name + '.' + self.url
    ...
    def _swap_exp_in(self):
        ...
        """
        cmd = '/usr/testbed/bin/sslxmlrpc_client.py swapexp proj=' + self.proj_name + ' exp=' + self.exp_name + ' direction=in'
        try:
            ssh.execute_command(self, self.ops_ssh_config, cmd)
        except ssh.SSHException as e:
            try:
                line = re.findall('not swapped out', str(e))
                if line:
    ...
    def _create_experiment(self, experiment):
        ...
        dest_file_name = '/users/' + self.username + '/temp_ns_file.%s.ns' % os.getpid()
        ssh.write_text_to_file(self, self.ops_ssh_config, ns, dest_file_name)
        cmd = '/usr/testbed/bin/sslxmlrpc_client.py startexp batch=false wait=true proj="' + proj_name + '" exp="' + exp_name + '" noswapin=true ' + 'nsfilepath="' + dest_file_name + '"'
        try:
            try:
    ...
    def _generate_ns_script(self, experiment):
        ...
        for dif in experiment.dif_ordering:
            if isinstance(dif, mod.ShimEthDIF):
                if len(dif.ipcps) != 2:
                    continue
                ns2_script += 'set ' + dif.name + ' [$ns duplex-link $' + dif.members[0].name + ' $' + dif.members[1].name + ' 1000Mb 0ms DropTail]\n'
    ...
    def _wait_until_nodes_up(self):
        ...
        cmd = '/usr/testbed/bin/script_wrapper.py expinfo -e' + self.proj_name + ',' + self.exp_name + ' -a | grep State | cut -f2,2 -d " "'
    ...
    def _complete_experiment_graph(self, experiment):
        ...
        for node in experiment.nodes:
            ...
            for item in output:
                item = item.split()
                for ipcp in node.ipcps:
                    if isinstance(ipcp, mod.ShimEthIPCP) and self.ip[ipcp] == item[1]:
                        ipcp.ifname = item[0]
    ...
    def _swap_out(self, experiment):
        ...
        :param experiment: The experiment.
        """
        cmd = '/usr/testbed/bin/sslxmlrpc_client.py swapexp proj=' + self.proj_name + ' exp=' + self.exp_name + ' direction=out'
        ssh.execute_command(self, self.ops_ssh_config, cmd)