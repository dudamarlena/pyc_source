...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/prototypes/irati.py
# Compiled at: 2018-07-03 03:25:34
# Size of source mod 2**32: 18792 bytes
...
class Experiment(mod.Experiment):
    ...
    def __init__(self, testbed, nodes=None, git_repo='https://github.com/IRATI/stack', git_branch='arcfire', installpath=None, varpath=None, app_mappings=None, enrollment_strategy='minimal'):
        """
        ...
        """
        mod.Experiment.__init__(self, testbed,
          nodes,
          git_repo,
          git_branch,
          prototype_logs=[
    ...
    def install(self):
        packages = [
         'g++', 'gcc', 'libtool', 'linux-headers-$(uname -r)',
        ...
        cmds = [
         ...
         'cd ~/stack && ' + self.sudo('./configure && ') + self.sudo('make install')]
        names = []
        executors = []
        args = []
    ...
    def process_node(self, node):
        ...
        ssh.copy_files_to_testbed(self.testbed, node.ssh_config, gen_files, '')
        cmds = [
         self.sudo('hostname %(name)s' % format_args),
        ...
        cmds += [self.sudo('modprobe rina-default-plugin'),
         ...
         self.sudo('ipcm -a "%(ipcmcomps)s" -c /etc/%(name)s.ipcm.conf -l %(verb)s > /tmp/ipcmstart.log 2>&1 &' % format_args)]
    ...
    def enroll_nodes(self):
        ...
        for enrollment_list in self.enrollments:
            for e in enrollment_list:
                logger.info('Enrolling %s to DIF %s against neighbor %s, through lower DIF %s.', e['enrollee'].name, e['dif'].name, e['enroller'].name, e['lower_dif'].name)
                time.sleep(1)
                ...
                cmd = self.sudo('enroll.py --lower-dif %(ldif)s --dif %(dif)s --ipcm-conf /etc/%(nname)s.ipcm.conf --enrollee-name %(iname)s.IPCP --enroller-name %(o_iname)s.IPCP' % e_args)
                ssh.execute_command(self.testbed, e['enrollee'].node.ssh_config, cmd)
    ...
    def write_conf(self):
        ...
        if len(app_mappings) == 0:
            ...
            for app, dif in app_mappings:
                irati_templates.da_map_base['applicationToDIFMappings'].append({'encodedAppName':app, 
                 'difName':dif})

        if self.manager:
            irati_templates.get_ipcmconf_base()['addons'] = {'mad': {'managerAppName':'', 
                     'NMSDIFs':[
                      {'DIF': '%s' % mgmt_dif_name}], 
                     'managerConnections':[
        ...
        for node in self.nodes:
            ...
            for ipcp in node.ipcps:
                if isinstance(ipcp, mod.ShimEthIPCP):
                    shim = ipcp2shim_map[ipcp.name]
                    shim_name = self.dif_name(shim)
                    ipcmconf['ipcProcessesToCreate'].append({'apName':'eth.%s.IPCP' % ipcp.name, 
                     'apInstance':'1', 
                     'difName':shim_name})
                    template_file_name = self.conf_dir('shimeth.%s.%s.dif' % (
                     node.name, shim_name))
                    ipcmconf['difConfigurations'].append({'name':shim_name, 
                     'template':os.path.basename(template_file_name)})
                    fout = open(template_file_name, 'w')
                    fout.write(json.dumps({'difType':'shim-eth-vlan', 
                     'configParameters':{'interface-name': ipcp.ifname}},
                      indent=4,
                      sort_keys=True))
                    fout.close()
                    conf_files.setdefault(node, []).append('shimeth.%s.%s.dif' % (
                     node.name, shim_name))
        ...
        for dif in self.dif_ordering:
            ...
            for node in dif.members:
                ...
                for lower_dif in node.dif_registrations[dif]:
                    normal_ipcp['difsToRegisterAt'].append(self.dif_name(lower_dif))

                ipcmconf['ipcProcessesToCreate'].append(normal_ipcp)
                ipcmconf['difConfigurations'].append({'name':'%s' % (dif.name,), 
                 'template':'normal.%s.%s.dif' % (node_name, dif.name)})
                for other_node in dif.members:
                    difconfs[dif.name][other_node.name]['knownIPCProcessAddresses'].append({'apName':'%s.%s.IPCP' % (dif.name, node_name), 
                     'apInstance':'1', 
                ...
                for component in policy_dict:
                    for policy_name in policy_dict[component]:
                        ...
                        irati_templates.translate_policy(difconfs[dif.name][node_name], component, policy_name, params)

        with open(self.conf_dir('da.map'), 'w') as (da_map_file):
            json.dump((irati_templates.da_map_base), da_map_file,
              indent=4,
              sort_keys=True)
        for node in self.nodes:
            with open(self.conf_dir('%s.ipcm.conf' % (
             node.name,)), 'w') as (node_file):
                json.dump((ipcmconfs[node.name]), node_file,
                  indent=4,
                  sort_keys=True)
            conf_files.setdefault(node, []).append('%s.ipcm.conf' % (node.name,))
            ...
            if dif_conf:
                for node in dif.members:
                    with open(self.conf_dir('normal.%s.%s.dif' % (
                     node.name, dif.name)), 'w') as (dif_conf_file):
                        json.dump((dif_conf[node.name]), dif_conf_file,
                          indent=4,
                          sort_keys=True)
                    conf_files.setdefault(node, []).append('normal.%s.%s.dif' % (node.name, dif.name))
