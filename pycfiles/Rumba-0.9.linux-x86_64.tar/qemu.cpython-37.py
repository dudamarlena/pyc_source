...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/testbeds/qemu.py
# Compiled at: 2018-08-31 05:12:48
# Size of source mod 2**32: 17620 bytes
import multiprocessing as old_m, os, subprocess, sys, time
...
class Testbed(mod.Testbed):
    ...
    def __init__(self, exp_name='foo', bzimage_path=None, initramfs_path=None, proj_name='rumba', password='root', username='root', public_key_path=os.path.join(USER_HOME, '.ssh', 'id_rsa.pub'), use_vhost=True):
        """
        ...
        .. note:: In case no bzimage or initramfs is provided, Rumba
                  ...
                  from a repository.
        """
        mod.Testbed.__init__(self, exp_name, username,
          password, proj_name,
        ...
        for node in experiment.nodes:
            for ipcp in node.ipcps:
                if isinstance(ipcp, mod.ShimEthIPCP):
                    ...
                    port_set = [x for x in self.vms[node_name]['ports'] if x['shim'].name == shim_name]
                    port = port_set[0]
                    port_id = port['port_id']
                    vm_id = self.vms[node_name]['id']
                    mac = '00:0a:0a:0a:%02x:%02x' % (vm_id, port_id)
                    logger.info('Recovering ifname for port: %s.', port['tap_id'])
                    output = node.execute_command('mac2ifname ' + mac)
                    ipcp.ifname = output
                    node.execute_command('ip link set dev %(ifname)s up' % {'ifname': ipcp.ifname})
    ...
    def _swap_in(self, experiment):
        ...
        if os.geteuid() != 0:
            try:
                ...
                if self.vhost:
                    if os.access('/dev/vhost-net', os.R_OK):
                        if os.access('/dev/vhost-net', os.W_OK):
                            if not (os.access('/dev/kvm', os.R_OK) and os.access('/dev/kvm', os.W_OK)):
                                raise Exception('Cannot open vhost device. Make sure it is available and you have rw permissions on /dev/vhost-net')
            ...
            if not self.bzimage_path:
                ...
                if not os.path.exists(self.bzimage_path):
                    ...
                    urlretrieve((url_prefix + bzimage), filename=(self.bzimage_path))
                    print('\n')
        elif not self.initramfs_path:
            ...
            if not os.path.exists(self.initramfs_path):
                ...
                urlretrieve((url_prefix + initramfs), filename=(self.initramfs_path))
                print('\n')
            ...
            for node in shim.members:
                ...
                if len(ipcp_set) > 1:
                    raise Exception('Error: more than one ipcp in common between shim dif %s and node %s' % (
                     shim.name, node.name))
                ipcp = ipcp_set[0]
                assert ipcp.name == '%s.%s' % (shim.name, node.name), 'Incorrect Shim Ipcp found: expected %s.%s, found %s' % (
        ...
        logger.info('Interfaces setup complete. Building VMs (this might take a while).')
        ...
        for node in experiment.nodes:
            ...
            vars_dict = {'fwdp':fwdp, 
             'id':vmid,  'mac':mac,  'bzimage':self.bzimage_path, 
             'initramfs':self.initramfs_path, 
             'fwdc':fwdc, 
             'memory':vm_memory, 
             'frontend':vm_frontend,  'vmname':name, 
             'log_file':log_file}
            host_fwd_str = 'hostfwd=tcp::%(fwdp)s-:22' % vars_dict
            vars_dict['hostfwdstr'] = host_fwd_str
            command = 'qemu-system-x86_64 '
            command += '-kernel %(bzimage)s -append "console=ttyS0" -initrd %(initramfs)s ' % vars_dict
            if os.path.exists('/dev/kvm'):
                command += '--enable-kvm '
            command += '-vga std -display none -smp 1 -m %(memory)sM -device %(frontend)s,mac=%(mac)s,netdev=mgmt -netdev user,id=mgmt,%(hostfwdstr)s -serial file:%(log_file)s ' % vars_dict
            del vars_dict
            for port in vm['ports']:
                tap_id = port['tap_id']
                mac = '00:0a:0a:0a:%02x:%02x' % (vmid, port['port_id'])
                port['mac'] = mac
                command += '-device %(frontend)s,mac=%(mac)s,netdev=data%(idx)s -netdev tap,ifname=%(tap)s,id=data%(idx)s,script=no,downscript=no%(vhost)s ' % {'mac':mac, 
                 'tap':tap_id,  'idx':port['port_id'],  'frontend':vm_frontend, 
                 'vhost':',vhost=on' if self.vhost else ''}

            booting_budget -= 1
            if booting_budget <= 0:
                logger.debug('Sleeping %s secs waiting for the VMs to boot', boot_backoff)
                time.sleep(boot_backoff)
                booting_budget = boot_batch_size
            with open('%s/qemu_out_%s' % (mod.tmp_dir, vmid), 'w') as (out_file):
                logger.debug('executing >> %s', command)
                self.boot_processes.append(subprocess.Popen((command.split()), stdout=out_file))
            vmid += 1

        if booting_budget < boot_batch_size:
            tsleep = boot_backoff * (boot_batch_size - booting_budget) / boot_batch_size
            logger.debug('Sleeping %s secs waiting for the last VMs to boot', tsleep)
            time.sleep(tsleep)
        ...
        if self.key_path is not None:
            for node in experiment.nodes:
                try:
                    ...
                    node.execute_command('cd /root/.ssh; mv authorized_keys old.authorized_keys; cat old.authorized_keys id_rsa.pub > authorized_keys')
                except SSHException as e:
                    try:
                        logger.warning('Could not install ssh key into node %s. %s.', node.name, str(e))
