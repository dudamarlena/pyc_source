...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/elements/topology.py
# Compiled at: 2018-07-25 06:08:30
# Size of source mod 2**32: 29228 bytes
...
class ShimEthDIF(DIF):
    ...
    def link_quality(self, _link_quality):
        if not _link_quality:
            raise ValueError('Cannot set link_quality to None, use del link_quality to reset')
    ...
    def set_delay(self, delay=0, jitter=None, correlation=None, distribution=None):
        """
    ...
    def set_loss(self, loss=0, correlation=None):
        """
...
class NormalDIF(DIF):
    ...
    def __init__(self, name, members=None, policy=None, add_default_qos_cubes=True):
        """
...
class Delay(object):
    ...
    def __init__(self, delay=0, jitter=None, correlation=None, distribution=None):
        """
        ...
        if delay < 0:
            ...
            if not correlation or correlation < 0 or correlation > 100:
...
class LinkQuality(object):
    ...
    def build_commands(self, ipcp):
        ...
        if self.rate:
            cmds.append('tc qdisc add dev %s root handle 1: htb default 1' % ipcp.ifname)
    ...
    def apply(self, shim):
        if not self.delay:
            self.loss or self.rate or self.deactivate(shim)
        else:
            for ipcp in shim.ipcps:
                if not ipcp.ifname:
                    logger.error('Could not apply LinkQuality to IPCP because the interface name is None')
                    continue
                ipcp.node.execute_commands((self.build_commands(ipcp)), as_root=True)
    ...
    def deactivate(self, shim):
        for ipcp in shim.ipcps:
            if ipcp not in LinkQuality._active:
                continue
            if not ipcp.ifname:
                logger.error('Could not remove LinkQuality from IPCP because the interface name is None')
                continue
            ipcp.node.execute_command(('tc qdisc del dev %s root' % ipcp.ifname),
              as_root=True)
...
class Node(object):
    ...
    def __init__(self, name, difs=None, dif_registrations=None, policies=None, machine_type=None):
        """
    ...
    def _undeclared_dif(self, dif):
        if dif not in self.difs:
            raise Exception('Invalid registration: node %s is not declared to be part of DIF %s' % (
             self.name, dif.name))
        ...
        for dif in self.dif_registrations:
            if isinstance(dif, NormalDIF):
                ...
                for reg in self.dif_registrations[dif]:
                    ...
                    buffer.write('  - %sdifid: %s, diftype: %s, diftype_number: 1%s\n' % (
                     '{', nm, tp, '}'))
    ...
    def execute_commands(self, commands, as_root=False, time_out=3, use_proxy=False):
        """
    ...
    def execute_command(self, command, as_root=False, time_out=3, use_proxy=False):
        """
    ...
    def set_link_state(self, dif, state):
        ...
        self.execute_command(('ip link set dev ' + ipcp.ifname + ' ' + state), as_root=True)
...
class IPCP(object):
    ...
    def __eq__(self, other):
        return other is not None and self.name == other.name and self.dif == other.dif
...
class Policy(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, Policy):
            return False
        return other.dif == self.dif and other.node == self.node and other._dict == self._dict

    def __str__(self):
        node_str = ' Node: ' + self.node if self.node is not None else ''
        return 'Policy[Dif: %(dif)s,%(node_str)s Dict: %(dict)s]' % {'dif':self.dif, 
         'node_str':node_str,  'dict':self._dict}

    def __repr__(self):
        node_str = ' Node: ' + self.node if self.node is not None else ''
        s = 'Policy[ Dif: %(dif)s,%(node_str)s' % {'dif':self.dif, 
         'node_str':node_str}
        comps = []
