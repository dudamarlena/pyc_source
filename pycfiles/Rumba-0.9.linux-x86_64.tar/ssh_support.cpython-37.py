...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/rumba/ssh_support.py
# Compiled at: 2018-07-25 06:09:26
# Size of source mod 2**32: 14799 bytes
import os, paramiko, re, time
    ...
    while retry < max_retries:
        ...
        try:
            ...
            if proxy_server is not None:
                ...
                proxy_client.connect(proxy_server, 22, username, password, look_for_keys=True,
                  timeout=time_out)
            ...
            ssh_client.connect(hostname, port, username, password, look_for_keys=True,
              timeout=time_out,
              sock=proxy)
            return (ssh_client, proxy_client)
        except paramiko.ssh_exception.BadHostKeyException:
            retry += 1
            logger.error(hostname + ' has a mismatching entry in ' + '~/.ssh/known_hosts')
            ...
            retry += 1
            logger.error('Failed to connect to host, retrying: ' + str(retry) + '/' + str(max_retries) + ' retries')
...
def ssh_connect_check(ssh_config, testbed, time_out, force_reconnect=False):
    if not (ssh_config.client is None or ssh_config.client.get_transport()).is_active or force_reconnect:
        client, proxy_client = ssh_connect(ssh_config.hostname, ssh_config.port, testbed.username, testbed.password, time_out, ssh_config.proxy_server)
...
def ssh_chan(ssh_config, testbed, time_out):
    ...
    while retry < max_retries:
        try:
            ...
            try:
                if str(e) == 'No existing session' or str(e) == 'SSH session not active':
                    logger.debug('Failed to open transport: ' + str(e))
                    retry += 1
                    logger.debug('Trying to reconnect: ' + str(retry) + '/' + str(max_retries) + ' retries.')
                    ssh_connect_check(ssh_config, testbed, time_out, force_reconnect=True)
                else:
                    raise e
            finally:
                e = None
...
def copy_files_from_testbed(testbed, ssh_config, paths, destination, sudo=False):
    """
    Copies local files to a remote node.
...
def copy_file_from_testbed(testbed, ssh_config, path, destination, sudo=False):
    """
    Copies a local file to a remote node.
...
def setup_vlans(testbed, node, vlans):
    ...
    cmds = [sudo('for file in $(ls -d /etc/udev/rules.d/*); do rm $file; ln -s /dev/null $file; done')]
    for item in vlans:
        args = {'ifname':str(item[0]), 
         'vlan':str(item[1])}
        cmds += [
         sudo('ip link add link %(ifname)s name %(ifname)s.%(vlan)s type vlan id %(vlan)s' % args),
...
def aptitude_install(testbed, node, packages):
    ...
    package_install += '--yes'
    cmds = [
     sudo('sh -c \'while fuser /var/lib/dpkg/lock > /dev/null 2>&1; do sleep 1; echo "Waiting for dpkg..."; done\''),
