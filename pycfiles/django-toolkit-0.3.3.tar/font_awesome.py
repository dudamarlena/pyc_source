...
# [GCC 8.4.0]
# Embedded file name: /home/ahayes/.virtualenvs/roicrm-django1.7/local/lib/python2.7/site-packages/django_toolkit/font_awesome.py
# Compiled at: 2014-04-10 23:48:02
...
class Stack(BaseCollection):
    ...
    def render(self):
        ...
        """
        return '<span class="icon-stack">%s</span>' % ('').join([ item.render(['icon-stack-base'] if i == 0 else []) for i, item in enumerate(self.items) ])
...
class ButtonGroup(BaseCollection):
    ...
    def render(self):
        ...
        """
        return '<div class="btn-group">%s</div>' % ('').join([ item.render() for i, item in enumerate(self.items) ])


class Button:

    def __init__(self, inner=None, data_tip=None, view=None, view_kwargs=[], view_args=[], next=None, href=None, title=None, attrs={}, target=False, modal=False, submodal=False, data_target=True, css=[]):
        self.inner = inner
        self.href = href
        self.view = view
