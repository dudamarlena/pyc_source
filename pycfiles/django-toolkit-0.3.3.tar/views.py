...
# [GCC 8.4.0]
# Embedded file name: /home/ahayes/.virtualenvs/roicrm-django1.7/local/lib/python2.7/site-packages/django_toolkit/views.py
# Compiled at: 2015-04-23 20:05:41
from __future__ import absolute_import
import json, os, magic
...
from django.views.generic.dates import YearMixin, MonthMixin, DayMixin, _date_from_string
import collections
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.breadcrumbs:
            ...
            for breadcrumb in self.breadcrumbs:
                ...
                context['breadcrumbs'].append({'href': reverse(breadcrumb['viewname']), 
                   'title': breadcrumb['title']})

            context['breadcrumbs'].append({'href': self.object.get_absolute_url(), 
               'title': self.object})
            context['breadcrumbs'].append({'href': self.request.path, 
               'title': 'Update'})
        return context


@class_login_required
class LoginRequiredDetailView(DetailView):
    pass


@class_login_required
class LoginRequiredCreateView(GenericCreateView):
    pass


@class_login_required
class LoginRequiredUpdateView(GenericUpdateView):
    pass


@class_login_required
class LoginRequiredDeleteView(GenericDeleteView):
    pass
...
class ModelCallMethodsView(SingleObjectMixin, RedirectNextOrBackView):
    ...
    def call(self, obj, method, request, *args, **kwargs):
        return self.get_callable(obj, method)(*self.callable_args(request, *args, **kwargs), **self.callable_kwargs(request, *args, **kwargs))
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if isinstance(e, ValidationError):
                ...
                return RedirectNextOrBackView.get(self, request, *args, **kwargs)
            if len([ expect for expect in self.expect if isinstance(e, expect) ]) > 0:
...
@class_login_required
class LoginRequiredModelCallMethodsView(ModelCallMethodsView):
    pass
...
class BaseDayView(YearMixin, MonthMixin, DayMixin):
    ...
    def get_date(self):
        ...
        day = self.get_day()
        return _date_from_string(year, self.get_year_format(), month, self.get_month_format(), day, self.get_day_format())
...
class ModelExtraContextDetailView(ModelExtraContextMixin, DetailView):
    pass


class ModelExtraContextUpdateView(ModelExtraContextMixin, GenericUpdateView):
    pass
...
class XEditableGenericUpdateView(GenericUpdateView, AjaxMixin):
    ...
    def form_invalid(self, form):
        if self.is_ajax():
            errors = form.errors
            context = dict(success=False, msg=(',').join([ v.as_text() for k, v in errors.items() ]))
