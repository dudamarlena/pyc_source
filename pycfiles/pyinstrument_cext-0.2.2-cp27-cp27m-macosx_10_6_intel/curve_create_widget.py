...
# Embedded file name: build\bdist.win32\egg\pyinstruments\curvestore\curve_create_widget.py
# Compiled at: 2013-12-15 08:37:09
...
class CurveCreateWidget(QtGui.QWidget, object):
    ...
    def __init__(self, default_name='some_curve', default_window='default', comment='', tags=[], parent=None):
...
class CurveTagWidgetDummy(QtGui.QWidget, object):
    ...
    def get_selected_tags(self):
        tags = [ self._get_tag_from_item(item) for item in self.tree_widget.selectedItems()
               ]
        return tags
    ...
    def _setup_ui(self):
        ...
        self.tree_widget.headerItem().setText(0, QtGui.QApplication.translate('MainWindow', 'tags', None, QtGui.QApplication.UnicodeUTF8))
    ...
    def _contextMenu(self, point):
        ...
        def add_tag():
            ...
            if name_clicked != '':
                ...
                proposition = name_clicked
            tag, confirm = dialog.getText(QtGui.QWidget(), 'new tag', 'enter tag name', 0, proposition)
            tag = str(tag)
            if tag.endswith('/'):
