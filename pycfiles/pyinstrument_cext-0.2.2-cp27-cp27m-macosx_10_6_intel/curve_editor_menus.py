...
# Embedded file name: build\bdist.win32\egg\pyinstruments\curvefinder\gui\curve_editor_menus.py
# Compiled at: 2013-12-18 05:30:26
...
class MenuDB(QtGui.QMenu):
    ...
    def __init__(self, parent, widget):
        ...
        self.backup_all_files.triggered.connect(self._backup_all_files)
                ...
                for filename in files:
                    ...
                    if os.path.isfile(fname):
                        if fname.endswith('.h5'):
                            ...
                            message = 'A curve with the id ' + str(id) + ' already exists in your database.' + '\n What should we do with curve ' + cur.params['name'] + ' ?'
                            if allanswers <= -1:
...
class CurveEditorToolBar(QtGui.QToolBar, object):
    popup_unread_activated = QtCore.pyqtSignal(name='popup_unread_activated')
    popup_unread_deactivated = QtCore.pyqtSignal(name='popup_unread_deactivated')

    def __init__(self, parent):
        super(CurveEditorToolBar, self).__init__(parent)
        self._checkbox_popup_unread = NamedCheckBox(self, 'popup unread curves')
        self.addWidget(self._checkbox_popup_unread)
        self._checkbox_popup_unread.checked.connect(self.popup_unread_activated)
        self._checkbox_popup_unread.unchecked.connect(self.popup_unread_deactivated)
        self._checkbox_plot_popups = NamedCheckBox(self, 'plot popups')
