...
# Embedded file name: build\bdist.win32\egg\pyivi\ivicom\agna.py
# Compiled at: 2013-10-09 11:09:06
from pyivi.ivifactory import register_wrapper
from pyivi.ivicom import IviComWrapper
from pyivi.ivicom.ivicomwrapper import FieldsClass, pick_from_session
from pyivi import choices
from pyivi.common import add_sc_fields_enum, ShortCut, add_sc_fields, Enum
...
class Measurement(object):
    ...
    def fetch_complex(self):
        ...
        return array(list(real_)) + complex(0.0, 1.0) * array(list(imag_))
...
class ShortCutNA(ShortCut):
    ...
    def __init__(self, parent):
        ...
        self.measurement_idxs = Enum(['select measurement'] + self.active_channel.measurements.keys())
    ...
    def input_port(self, val):
        self.active_measurement.set_s_parameter(val, self.output_port)
        return val
    ...
    def output_port(self, val):
        self.active_measurement.set_s_parameter(self.input_port, val)
...
add_sc_fields(ShortCutNA, ShortCutNA._channel_related_fields, 'sc_active_channel')
add_sc_fields(ShortCutNA, ShortCutNA._measurement_related_fields, 'sc_active_measurement')
...
class AgNA(IviComWrapper):
    ...
    def sc_active_measurement(self):
        return self.channels[self.sc.channel_name].measurements[self.sc.measurement_name]
