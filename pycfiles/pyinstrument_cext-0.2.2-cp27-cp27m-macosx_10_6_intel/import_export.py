...
# Embedded file name: build\bdist.win32\egg\pyinstruments\curvestore\import_export.py
...
def update_all_files():
    res = QtGui.QMessageBox.question(QtGui.QWidget(), 'are you sure', "It's strongly advised to backup the directory " + MEDIA_ROOT + ' before updating the files. Proceed anyway ?', 'Cancel', 'OK')
    if res == 0:
        return
...
class CustomMessageBox(QtGui.QMessageBox):
    ...
    def ask(self):
        ...
        return (
         self.checkbox.checkState() == 2, dic[res])
    ...
    while askagain:
        ...
        if inplace:
            askagain = os.path.commonprefix([os.path.abspath(dirname),
             os.path.abspath(MEDIA_ROOT)]) != os.path.abspath(MEDIA_ROOT)
            if askagain:
        ...
        if 'parent_id' in c.params:
            if not c.params['parent_id'] == 0:
                ...
                try:
                    ...
                    print 'Parent id ' + str(c.params['parent_id']) + ' of curve with id ' + str(id) + ' does not exist. Orphan created!'
                else:
                    parent.add_child(c)
...
def clear_db():
    res = QtGui.QMessageBox.question(QtGui.QWidget(), 'are you sure', "It's strongly advised to backup the file " + DATABASE_FILE + ' before resetting the database. Careful, this command leaves all files in place, if you plan on using import files in place to recover the database, please, make sure they have been updated first.', 'Cancel', 'OK')
    if res:
...
def cleanup_directory():
    ...
    print to_remove
    if QtGui.QMessageBox.question(QtGui.QWidget(), 'Are you sure?', 'Found ' + str(len(to_remove)) + ' h5 files in the directory ' + MEDIA_ROOT + ' without entry in the database. Are you sure you want to delete them ?', 'Cancel', 'OK'):
        for f in to_remove:
            os.remove(f)
...
def forget_db_location():
    ...
    QtGui.QMessageBox.information(QtGui.QWidget(), 'change-database', 'change will take effect at the next startup')