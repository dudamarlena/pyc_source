...
# Embedded file name: build\bdist.win32\egg\pyinstruments\datalogger\models.py
# Compiled at: 2013-11-15 07:15:45
from django.db import models
from django.core.exceptions import ObjectDoesNotExist
from pyinstruments.datastore.settings import MEDIA_ROOT
import time
from datetime import datetime
import pandas, h5py, os.path, numpy as np
NaN = float('NaN')
...
class MeasurementPoint(models.Model):
    ...
    sensor = models.ForeignKey(Sensor, related_name='measurement_points', blank=True)
...
class SensingDevice(object):
    ...
    def log(self, value, mtime=-1.0):
        if mtime == -1.0:
            mtime = self.now()
        self.lastpoint = MeasurementPoint(sensor=self.sensorlog, value=value, time=mtime)
        if value >= self.MINVAL and value <= self.MAXVAL:
            self.lastpoint.save()
        else:
            print 'Measurement Error: value ' + str(value) + ' of sensor ' + self.sensorlog.name + ' lies out of defined range. \n'
        return self.lastpoint

    def create_point(self, value, mtime):
        try:
            v = MeasurementPoint.objects.get(sensor=self.sensorlog, time=mtime)
        except ObjectDoesNotExist:
            self.log(value, mtime)
            return True

        if not v.value == value:
            raise ValueError('Trying to overvrite an existing datapoint with a different value for ' + self.name + ' at time ' + str(mtime) + '!')
        return False

    def getlastpoint(self):
        val = MeasurementPoint.objects.filter(sensor=self.sensorlog).latest()
        return {'value': val.value, 'time': val.time, 
    ...
    def getallpoints(self):
        return self.toSeries(MeasurementPoint.objects.filter(sensor=self.sensorlog))

    def getallpoints_raw(self):
        return self.toSeries_raw(MeasurementPoint.objects.filter(sensor=self.sensorlog))
    ...
    def getallpointssince(self, sincetime):
        ...
        if sincetime < 0:
            sincetime += self.now()
        return self.toSeries(MeasurementPoint.objects.filter(sensor=self.sensorlog, time__gt=sincetime))
    ...
    def getallpointsrange(self, starttime, stoptime=NaN):
        ...
        if starttime < 0:
            starttime += self.now()
            stoptime += self.now()
        return self.toSeries(MeasurementPoint.objects.filter(sensor=self.sensorlog, time__range=(
         starttime, stoptime)))
    ...
    def toSeries(self, res):
        if self.raw:
            ...
            return pandas.Series(data=[ i.value for i in res ], index=[ datetime.fromtimestamp(k.time) for k in res ], name=self.sensorlog.name)

    def toSeries_raw(self, res):
        return pandas.Series(data=[ i.value for i in res ], index=[ k.time for k in res ], name=self.sensorlog.name)

    def plot(self, lastseconds=-1.0):
        if lastseconds == -1:
            since = 0
        else:
        ...
        with pandas.get_store(filename, 'r') as (store):
            for sensorname in metadata:
                ...
                for i, v in data.iteritems():
                    mp = MeasurementPoint(sensor=sens, value=v, time=i)
                    written += 1
