...
# Embedded file name: build\bdist.win32\egg\pyhardware\drivers\serial\serial_driver.py
...
class SerialDriver(Driver):
    ...
    def __init__(self, logical_name, address, simulate, baudrate=9600, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=1, dsrdtr=False, xonxoff=False, rtscts=False):
        ...
        self.rtscts = rtscts
        self.serial = self.get_driver(port=self.address, baudrate=self.baudrate, bytesize=self.bytesize, parity=self.parity, stopbits=self.stopbits, timeout=self.timeout, dsrdtr=self.dsrdtr, xonxoff=self.xonxoff, rtscts=self.rtscts)
