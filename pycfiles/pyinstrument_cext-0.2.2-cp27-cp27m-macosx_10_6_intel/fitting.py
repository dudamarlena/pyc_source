...
import pandas, scipy.optimize, numpy, numpy as np, collections, math
...
class FitFunctions(object):
    ...
    def _guessringdown(self):
        ...
        for sweep in range(sweeps):
            ...
            low = self.data[(sweep + 0.8) * ringtime + delta:(sweep + 0.95) * ringtime - delta]
            for i in high.index:
                highsum += self.data[i]
                highs += 1
        ...
        tempfit = Fit(data=firstlows, func='linear', autoguessfunction='', fixed_params={}, manualguess_params={}, verbosemode=False, maxiter=10)
        ...
        fit_params = dict(ringspersweeptime=ringspersweeptime, gamma=gamma, y0=y0, scale=scale, overshoot=overshoot)
    ...
    def _guessringdown_linear(self):
        ...
        overshoot = tempfitparams['y0'] + tempfitparams['slope'] * min(firstlows.index) - scale - y0
        fit_params = dict(ringspersweeptime=ringspersweeptime, gamma=gamma, y0=y0, scale=scale, overshoot=overshoot)
        self.fixed_params = dict(ringspersweeptime=ringspersweeptime, y0=y0, scale=scale, overshoot=overshoot)
        return fit_params
    ...
    def _guessringdown_old(self):
        ...
        gamma = slope / (20 * math.pi * math.log10(math.e))
        fit_params = dict(ringspersweeptime=ringspersweeptime, gamma=gamma, y0=y0, scale=scale, overshoot=overshoot)
        return fit_params
    ...
    def lorentz_complex(self, bandwidth, scale_re, scale_im, x0, y0_re, y0_im):
        x = self.x()
        return (scale_re + scale_im * complex(0.0, 1.0)) / (1 + complex(0.0, 1.0) * (x - x0) / bandwidth) + y0_re + y0_im * complex(0.0, 1.0)
    ...
    def _guesslorentz_complex(self):
        ...
        max = self.data[x0]
        bw = magdata.sum() / magmax * (self.x().max() - self.x().min()) / length
        fit_params = dict(bandwidth=bw, scale_re=real(max), scale_im=imag(max), x0=x0, y0_re=real(bg), y0_im=imag(bg))
    ...
    def _guesslorentzSB(self):
        ...
        return {'scale': lor.params['scale'], 'x0': lor.params['x0'], 
           'y0': lor.params['y0'], 
           'bandwidth': lor.params['bandwidth'], 
           'SBwidth': (x_up - x_lo) / 2, 
           'SBscale': 1.0 / lor.params['scale'] * (scale_up + scale_lo) / 2}
...
class Fit(FitFunctions):
    ...
    def __init__(self, data, func, fixed_params={}, manualguess_params={}, autoguessfunction='', verbosemode=True, maxiter=1000, errfn='error_vector', autofit=True, graphicalfit=False):
        self._x_npy = None
        self._y_npy = None
    ...
    def fit(self):
        res = scipy.optimize.leastsq(func=self.fn_error, x0=self.fit_params.values(), xtol=0.0001, ftol=0.0001, gtol=0.0001)
        self.sqerror = self.getsqerror()
        self.comment('Fit completed with sqerror = ' + str(self.sqerror))
        self.comment('Obtained parameter values: ')
        self.comment(dict(self.getparams()))
    ...
    def fit_old(self):
        res = scipy.optimize.minimize(fun=self.fn_error, x0=self.fit_params.values(), args=(), method='BFGS', jac=False, hess=None, hessp=None, bounds=None, constraints=(), tol=None, callback=self.printstatus, options={'maxiter': self.maxiter, 'disp': self.verbosemode, 'gtol': 0.0001})
        self.sqerror = self.getsqerror()
        self.comment('Fit completed with sqerror = ' + str(self.sqerror))
        self.comment('Obtained parameter values: ')
        self.comment(dict(self.getparams()))
