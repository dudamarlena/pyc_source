...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/djsld/tests/djsld-test/tests.py
# Compiled at: 2012-10-05 18:48:39
"""
...
class ClassificationTest(unittest.TestCase):
    ...
    def test_colorbrewer_name(self):
        ...
        """
        sld = generator.as_quantiles(Hydrant.objects.filter(pressure=2), 'number', 5, geofield='location', colorbrewername='Greys')
        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)
        expected = [
    ...
    def test_invert_gradient(self):
        ...
        expected = [
         (255, 255, 255), (204, 204, 204), (153, 153, 153), (102, 102, 102), (51, 51, 51)]
        for i, n in enumerate(shades):
            exp_shade = '#%02x%02x%02x' % expected[i]
        ...
        self.assertEquals(len(shades), 5)
        expected = [
         (0, 0, 0), (51, 51, 51), (102, 102, 102), (153, 153, 153), (204, 204, 204)]
        for i, n in enumerate(shades):
            exp_shade = '#%02x%02x%02x' % expected[i]
            self.assertEqual(n.text, exp_shade, 'Shade %d is not correct.' % i)
    ...
    def test_inverted_colorbrewer(self):
        ...
        """
        sld = generator.as_quantiles(Hydrant.objects.filter(pressure=2), 'number', 5, geofield='location', colorbrewername='Greys', invertgradient=True)
        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)
        expected = [
         (37, 37, 37), (99, 99, 99), (150, 150, 150), (204, 204, 204), (247, 247, 247)]
        shades = sld._node.xpath('//sld:Fill/sld:CssParameter[@name="fill"]', namespaces=sld._nsmap)
        self.assertEqual(len(shades), 5)
        for i, n in enumerate(shades):
            exp_shade = '#%02x%02x%02x' % expected[i]
