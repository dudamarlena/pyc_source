...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/Aituglo/Desktop/Onyx/onyx/skills/core.py
...
def load_skill(skill_descriptor):
    try:
        ...
        if hasattr(skill_module, 'create') and callable(skill_module.create):
...
def get_skills(skills_folder):
    ...
    for i in possible_skills:
        ...
        if isdir(location) and MainModule + '.py' not in os.listdir(location):
            for j in os.listdir(location):
                name = join(location, j)
                if not isdir(name) or MainModule + '.py' not in os.listdir(name):
                    continue
                skills.append(create_skill_descriptor(name))

        if not isdir(location) or MainModule + '.py' not in os.listdir(location):
            continue
