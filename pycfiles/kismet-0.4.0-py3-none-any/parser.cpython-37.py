...
# [GCC 8.4.0]
# Embedded file name: /Users/mara_kim/Documents/code/github/kismet-py/kismet/parser.py
# Compiled at: 2019-02-04 20:05:04
...
class Expr:
    ...
    def __init__(self, expr: any, args: Tuple['Expr']=()):
        self.expr = expr if callable(expr) else (lambda : (expr, pretty(expr)))
        self.args = args
        self.value = None
        self.repr = None
...
class KismetTransformer(Transformer):
    ...
    def add(self, args):
        ...
        def f(x, y):
            return (
             torch.add(x.value, y.value), x.repr + ' + ' + y.repr)
    ...
    def sub(self, args):
        ...
        def f(x, y):
            return (
             torch.add(x.value, torch.mul(y.value, -1)), x.repr + ' - ' + y.repr)
    ...
    def mul(self, args):
        ...
        def f(x, y):
            return (
             torch.mul(x.value, y.value), x.repr + ' * ' + y.repr)
    ...
    def div(self, args):
        ...
        def f(x, y):
            return (
             torch.div(x.value, y.value), x.repr + ' / ' + y.repr)
    ...
    def array(self, args):
        return Expr(lambda *args: (
         list(values(args)), '[' + ', '.join(reprs(args)) + ']'), args)

    def tuple(self, args):
        return Expr(lambda *args: (
         tuple(values(args)), '(' + ', '.join(reprs(args)) + ')'), args)
