...
# [GCC 8.4.0]
# Embedded file name: /Users/jsick/.virtualenvs/paperweight/lib/python2.7/site-packages/paperweight/document.py
# Compiled at: 2015-01-12 18:45:11
"""
...
__all__ = [
 'FilesystemTexDocument', 'GitTexDocument', 'TexDocument']
...
class TexDocument(object):
    ...
    def find_input_documents(self):
        ...
        """
        paths = []
        itr = chain(texutils.input_pattern.finditer(self.text), texutils.input_ifexists_pattern.finditer(self.text))
        for match in itr:
            fname = match.group(1)
    ...
    def bib_path(self):
        ...
        osx_path = os.path.expanduser(('~/Library/texmf/bibtex/bib/{0}').format(bib_name))
        ...
        for match in texutils.cite_pattern.finditer(self.text):
            ...
            for section_pos, section_name in self._sections:
                if section_pos < numwordsbefore:
                    containing_section = (
                     section_pos, section_name)

            citebody = match.groups()
            keys = citebody[(-1)].replace(' ', '').split(',')
            for key in keys:
                cite_instance = {'position': numwordsbefore, 'wordsbefore': (' ').join(wordsbefore[-n_words:]), 
    ...
    def bibitems(self):
        ...
        for i, line in enumerate(lines):
            if line.lstrip().startswith('\\bibitem'):
                ...
                while True:
                    try:
                        if lines[(i + j)].startswith('\\bibitem') is False and lines[(i + j)] != '\n':
                            line += lines[(i + j)]
    ...
    def __init__(self, path, recursive=True):
        ...
        if recursive:
            ...
            for path in child_paths:
                self._children[path] = FilesystemTexDocument(path, recursive=True)
