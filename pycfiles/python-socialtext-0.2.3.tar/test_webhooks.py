...
# [GCC 8.4.0]
# Embedded file name: /Users/justin/Projects/python-socialtext/tests/test_webhooks/test_webhooks.py
...
def test_create_page_webhook():
    ...
    hook = st.webhooks.create_page_webhook(action, url, workspace_id=workspace_id, page_id=page_id)
    assert_isinstance(hook, Webhook)
...
def test_create_signal_webhook():
    ...
    user_id = 123
    tag = 'abc123'
    hook = st.webhooks.create_signal_webhook(action, url, account_id=account_id, user_id=user_id, tag=tag)
    assert_isinstance(hook, Webhook)
    assert_equal(getattr(hook, 'class'), action)
    assert_equal(hook.url, url)
