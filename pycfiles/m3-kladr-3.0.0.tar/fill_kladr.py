...
# [GCC 8.4.0]
# Embedded file name: /home/pirogov/.virtualenvs/paid/local/lib/python2.7/site-packages/kladr/fill_kladr.py
...
def fill_kladr(region_only=None, dbf_path='', clear_before=True, update=False):
    ...
    region_only - первые два символа региона,
    для ограничения загрузки только этого региона
    """
    COMMIT_PER = 5000
    if update:
    ...
    if clear_before:
        ...
        for mod in [models.KladrStreet, models.KladrGeo]:
            sql = 'TRUNCATE TABLE %s CASCADE;' % connection.ops.quote_name(mod._meta.db_table)
            cursor.execute(sql)

        transaction.commit()
        print 'Старые записи КЛАДРа удалены.'
    sub_list = {}
    region_list = {}
    city_list = {}
    geo_places_list = {}
    ...
    for sub_value in sub_list.itervalues():
        new_kladr_geo_sub = make_obj(models.KladrGeo, sub_value['code'])
    ...
    for region_key in region_list.itervalues():
        ...
        if not region_key['flag']:
            sub = sub_list[code]
            new_kladr_geo_region = make_obj(models.KladrGeo, region_key['code'])
            ...
            if transaction_commit_counter % COMMIT_PER == 0:
                transaction.commit()
            if i % 1000 == 1:
                print '     ' + str(i) + '/' + str(list_count) + ' ' + new_kladr_geo_region.name
            i = i + 1
    ...
    for city_key in city_list.itervalues():
        ...
        if code[2:] == '000':
            if not city_key['flag']:
                code = code[:2]
                sub = sub_list[code]
                new_kladr_geo_city = make_obj(models.KladrGeo, city_key['code'])
                ...
                if transaction_commit_counter % COMMIT_PER == 0:
                    transaction.commit()
                if i % 1000 == 1:
                    print '        ' + str(i) + '/' + str(list_count) + ' ' + new_kladr_geo_city.name
                i = i + 1
        elif code[2:] != '000':
            if not city_key['flag']:
        ...
        if code[2:5] == '000' and code[5:8] != '000' or code[2:5] != '000' and code[5:8] != '000':
            if not place_value['flag']:
                ...
                try:
                    ...
                    new_kladr_geo_place = make_obj(models.KladrGeo, place_value['code'])
                    ...
                    if transaction_commit_counter % COMMIT_PER == 0:
                        transaction.commit()
                    if i % 1000 == 1:
                        print '             ' + str(i) + '/' + str(list_count) + ' ' + new_kladr_geo_place.name
                    i = i + 1
        ...
        if code[15:] == '00':
            if code[2:11] == '000000000':
                ...
                try:
                    ...
                    street_sub = make_obj(models.KladrStreet, street[2].decode('866'))
                    ...
                    if transaction_commit_counter % COMMIT_PER == 0:
                        transaction.commit()
                    if i % 1000 == 1:
                        print '                    ' + str(i) + '/' + str(list_count) + ' ' + street_sub.name
                    i = i + 1
                ...
                try:
                    ...
                    street_city = make_obj(models.KladrStreet, street[2].decode('866'))
                    ...
                    if transaction_commit_counter % COMMIT_PER == 0:
                        transaction.commit()
                    if i % 1000 == 1:
                        print '                    ' + str(i) + '/' + str(list_count) + ' ' + street_city.name
                    i = i + 1
                ...
                try:
                    ...
                    street_place = make_obj(models.KladrStreet, street[2].decode('866'))
                    ...
                    if transaction_commit_counter % COMMIT_PER == 0:
                        transaction.commit()
                    if i % 1000 == 1:
                        print '                    ' + str(i) + '/' + str(list_count) + ' ' + street_place.name
                    i = i + 1

    transaction.commit()
    print 'Загрузка КЛАДР в систему завершена'
    return