...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/column_builders.py
# Compiled at: 2020-05-03 13:27:02
import random, string, time, numpy as np, pandas as pd, dtale.global_state as global_state
...
class NumericColumnBuilder(object):
    ...
    def build_code(self):
        ...
        return ("df.loc[:, '{name}'] = {left} {operation} {right}").format(name=self.name, operation=operations.get(operation), left=("df['{}']").format(left['col']) if 'col' in left else left['val'], right=("df['{}']").format(right['col']) if 'col' in right else right['val'])
...
class DatetimeColumnBuilder(object):
    ...
    def build_code(self):
        ...
        freq, how = conversion_key.split('_')
        freq = FREQ_MAPPING[freq]
        return ("{name}_data = data[['{col}']].set_index('{col}').index.to_period('{freq}')'.to_timestamp(how='{how}').normalize()\ndf.loc[:, '{name}'] = pd.Series({name}_data, index=df.index, name='{name}')").format(name=self.name, col=self.cfg['col'], freq=freq, how=how)
...
class BinsColumnBuilder(object):
    ...
    def build_code(self):
        ...
        bins_code = []
        if operation == 'cut':
            bins_code.append(("{name}_data = pd.cut(df['{col}'], bins={bins})").format(name=self.name, col=col, bins=bins))
        else:
            bins_code.append(("{name}_data = pd.qcut(df['{col}'], bins={bins})").format(name=self.name, col=col, bins=bins))
        if labels:
            ...
            bins_code.append('{name}_cats = {idx: str(cat) for idx, cat in enumerate({name}_data.cat.categories)}')
...
class RandomColumnBuilder(object):
    ...
    def build_column(self, data):
        ...
        if 'string' == rand_type:
            ...
            return pd.Series([ id_generator(**kwargs) for _ in range(len(data)) ], index=data.index, name=self.name)
        if 'int' == rand_type:
            low = self.cfg.get('low', 0)
            high = self.cfg.get('high', 100)
            return pd.Series(np.random.randint(low, high=high, size=len(data)), index=data.index, name=self.name)
        if 'date' == rand_type:
            ...
            if timestamps:
                ...
                def pp(start, end, n):
                    start_u = start.value // 1000000000
                    end_u = end.value // 1000000000
                    return pd.DatetimeIndex((1000000000 * np.random.randint(start_u, end_u, n)).view('M8[ns]'))
    ...
    def build_code(self):
        ...
        if 'string' == rand_type:
            ...
            kwargs = (', ').join(kwargs)
            return ("import number\nimport random\n\ndef id_generator(size=1500, chars=string.ascii_uppercase + string.digits):\n\treturn ''.join(random.choice(chars) for _ in range(size))\n\ndf.loc[:, '{name}'] = pd.Series([id_generator({kwargs}) for _ in range(len(df)], index=df.index)").format(kwargs=kwargs, name=self.name)
            return ("df.loc[:, '{name}'] = df['{col}'].dt.{property}").format(name=self.name, **self.cfg)
        if 'bool' == rand_type:
            return ("df.loc[:, '{name}'] = pd.Series(np.random.choice([True, False], len(df)), index=data.index").format(name=self.name)
        if 'date' == rand_type:
            start = pd.Timestamp(self.cfg.get('start') or '19000101')
            end = pd.Timestamp(self.cfg.get('end') or '21991231')
            business_days = self.cfg.get('businessDay') is True
            timestamps = self.cfg.get('timestamps') is True
            if timestamps:
                code = ("def pp(start, end, n):\n\tstart_u = start.value // 10 ** 9\n\tend_u = end.value // 10 ** 9\n\treturn pd.DatetimeIndex(\n\t\t(10 ** 9 * np.random.randint(start_u, end_u, n, dtype=np.int64)).view('M8[ns]')\n)\n\ndf.loc[:, '{name}'] = pd.Series(\n\tpp(pd.Timestamp('{start}'), pd.Timestamp('{end}'), len(df)), index=df.index\n)").format(name=self.name, start=start.strftime('%Y%m%d'), end=end.strftime('%Y%m%d'))
            else:
                freq = ", freq='B'" if business_days else ''
                code = ("dates = pd.date_range('{start}', '{end}'{freq})\ndates = [dates[i] for i in np.random.randint(0, len(dates) - 1, size=len(data))]\ndf.loc[:, '{name}'] = pd.Series(dates, index=data.index)").format(name=self.name, start=start.strftime('%Y%m%d'), end=end.strftime('%Y%m%d'), freq=freq)
            return code
        if 'choice' == rand_type:
            choices = self.cfg.get('choices') or 'a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z'
            choices = choices.split(',')
            return ("df.loc[:, '{name}'] = pd.Series(np.random.choice({choices}, len(df)), index=df.index)").format(choices=("', '").join(choices), name=self.name)
        if 'int' == rand_type:
            low = self.cfg.get('low', 0)
            high = self.cfg.get('high', 100)
            return ("import numpy as np\n\ndf.loc[:, '{name}'] = pd.Series(np.random.randint({low}, high={high}, size=len(df)), index=df.index)").format(name=self.name, low=low, high=high)
        low = self.cfg.get('low', 0)
        high = self.cfg.get('high', 1)
        return ("import numpy as np\n\ndf.loc[:, '{name}'] = pd.Series(np.random.uniform({low}, {high}, len(df)), index=df.index)").format(low=low, high=high, name=self.name)
    ...
    def build_inner_code(self):
        ...
        if classifier == 'S':
            if to_type == 'date':
                ...
                if to_type == 'int':
                    return ("pd.Series({s}.astype('float').astype('int'), name='{name}', index={s}.index)").format(s=s, name=self.name)
                return ("pd.Series({s}.astype({to_type}), name='{name}', index={s}.index)").format(s=s, to_type=to_type, name=self.name)
            ...
            if classifier == 'I':
                if to_type == 'date':
                    unit = self.cfg.get('unit') or 'D'
                    if unit == 'YYYYMMDD':
                        return ("pd.Series({s}.astype(str).apply(pd.Timestamp), name='{name}', index={s}.index)").format(s=s, name=self.name)
                    return ("pd.Series(pd.to_datetime({s}, unit='{unit}'), name='{name}', index={s}.index)").format(s=s, name=self.name, unit=unit)
                return ("pd.Series({s}.astype('{to_type}'), name='{name}', index={s}.index)").format(s=s, to_type=to_type, name=self.name)
            if classifier == 'F':
                return ("pd.Series(s.astype('{to_type}'), name='{name}', index={s}.index)").format(s=s, to_type=to_type, name=self.name)
            if classifier == 'D':
                if to_type == 'int':
                    unit = self.cfg.get('unit') or 'D'
                    if unit == 'YYYYMMDD':
                        return ("pd.Series({s}.dt.strftime('%Y%m%d').astype(int), name='{name}', index={s}.index)").format(s=s, name=self.name)
                    return ("pd.Series(\n\t{s}.apply(lambda x: time.mktime(x.timetuple())).astype(int), \nname='{name}', index={s}.index\n)").format(s=s, name=self.name)
                return ("pd.Series({s}.dt.strftime('{fmt}'), name='{name}', index={s}.index)").format(fmt=self.cfg.get('fmt') or '%Y%m%d', s=s, name=self.name)
            if classifier == 'B':
                return ("pd.Series(s.astype('{to_type}'), name='{name}', index={s}.index)").format(s=s, to_type=to_type, name=self.name)
        raise NotImplementedError(('data type conversion not supported for dtype: {}').format(from_type))
