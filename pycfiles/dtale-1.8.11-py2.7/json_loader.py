...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/cli/loaders/json_loader.py
...
def loader_func(**kwargs):
    ...
    if normalize:
        ...
        return pd.read_json(path, **{k:v for k, v in kwargs.items() if k in loader_prop_keys(LOADER_PROPS) if k in loader_prop_keys(LOADER_PROPS)})
...
def find_loader(kwargs):
    ...
    if len([ f for f in json_opts.values() if f ]):
        ...
        def _json_loader():
            json_arg_parsers = {'parse_dates': lambda v: v.split(',') if v else None}
