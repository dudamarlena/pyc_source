...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/app.py
# Compiled at: 2020-05-03 13:27:02
from __future__ import absolute_import, print_function
import os, random, socket, sys, time, traceback
...
from dtale.utils import DuplicateDataError, build_shutdown_url, build_url, dict_merge, get_host, running_with_flask_debug
...
class DtaleFlaskTesting(FlaskClient):
    ...
    def get(self, *args, **kwargs):
        ...
        """
        return super(DtaleFlaskTesting, self).get(base_url=('http://{host}:{port}').format(host=self.host, port=self.port), *args, **kwargs)
...
def build_app(url, host=None, reaper_on=True, hide_shutdown=False, github_fork=False):
    ...
    def page_not_found(e=None):
        ...
        """
        return (
         render_template('dtale/errors/404.html', page='', error=e, stacktrace=str(traceback.format_exc())), 404)
    ...
    def internal_server_error(e=None):
        ...
        """
        return (
         render_template('dtale/errors/500.html', page='', error=e, stacktrace=str(traceback.format_exc())), 500)
        ...
        if curr_base != new_base:
            if is_up(new_base):
                try:
                    ...
                    raise IOError(('Could not kill process at {}, possibly something else is running at port {}. Please try another port.').format(new_base, port))
...
def find_free_port():
    ...
    while is_port_in_use(base):
        base += 1
        if base > max_port:
            msg = ('D-Tale could not find an open port from {} to {}, please increase your range by altering the environment variables DTALE_MIN_PORT & DTALE_MAX_PORT.').format(min_port, max_port)
            raise IOError(msg)

    return base


def show(data=None, host=None, port=None, name=None, debug=False, subprocess=True, data_loader=None, reaper_on=True, open_browser=False, notebook=False, force=False, context_vars=None, ignore_duplicate=False, **kwargs):
    """
    ...
    try:
        ...
        instance = startup(url, data=data, data_loader=data_loader, name=name, context_vars=context_vars, ignore_duplicate=ignore_duplicate)
        ...
        print(('It looks like this data may have already been loaded to D-Tale based on shape and column names. Here is URL of the data that seems to match it:\n\n{}\n\nIf you still want to load this data please use the following command:\n\ndtale.show(df, ignore_duplicate=True)').format(DtaleData(ex.data_id, build_url(ACTIVE_PORT, ACTIVE_HOST)).main_url()))
...
def offline_chart(df, chart_type=None, query=None, x=None, y=None, z=None, group=None, agg=None, window=None, rolling_comp=None, barmode=None, barsort=None, yaxis=None, filepath=None, **kwargs):
    """
    ...
    output = instance.offline_chart(chart_type=chart_type, query=query, x=x, y=y, z=z, group=group, agg=agg, window=window, rolling_comp=rolling_comp, barmode=barmode, barsort=barsort, yaxis=yaxis, filepath=filepath, **kwargs)
    global_state.cleanup()
    return output