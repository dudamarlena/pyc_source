...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/data_reshapers.py
# Compiled at: 2020-05-03 13:27:02
import pandas as pd, dtale.global_state as global_state
from dtale.utils import make_list, run_query

def flatten_columns(df, columns=None):
    if columns is not None:
        return [ (' ').join([ ('{}-{}').format(c1, str(c2)) for c1, c2 in zip(make_list(columns), make_list(col_val)) ]).strip() for col_val in df.columns.values
               ]
    else:
...
class DataReshaper(object):
    ...
    def reshape(self):
        data = run_query(global_state.get_data(self.data_id), (global_state.get_settings(self.data_id) or {}).get('query'), global_state.get_context_variables(self.data_id))
...
class PivotBuilder(object):
    ...
    def build_code(self):
        ...
        code = []
        if aggfunc is not None or len(values) > 1:
            code.append(("df = pd.pivot_table(df, index='{}', columns='{}', values=['{}'], aggfunc='{}')").format(index, columns, ("', '").join(values), aggfunc))
            if len(values) > 1:
                code.append("df.columns = [' '.join([str(c) for c in col]).strip() for col in df.columns.values]")
            elif len(values) == 1:
                code.append('df.columns = df.columns.droplevel(0)')
        else:
            code.append(("df = df.pivot(index='{index}', columns='{columns}', values='{values}')").format(index=index, columns=columns, values=values[0]))
...
class AggregateBuilder(object):
    ...
    def build_code(self):
        ...
        if agg_type == 'func':
            if cols is not None:
                return ("df = df.groupby(['{index}'])['{columns}'].{agg}()").format(index=index, columns=("', '").join(cols), agg=agg)
