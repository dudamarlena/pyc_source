...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/views.py
# Compiled at: 2020-05-03 13:27:02
from __future__ import absolute_import, division
import os, time, traceback, webbrowser
from builtins import map, range, str, zip
from logging import getLogger
from flask import json, make_response, redirect, render_template, request, url_for
...
from dtale.dash_application.charts import build_raw_chart, chart_url_params, chart_url_querystring, export_chart, export_chart_data, url_encode_func
from dtale.data_reshapers import DataReshaper
from dtale.utils import DuplicateDataError, build_code_export, build_query, build_shutdown_url, classify_type, dict_merge, divide_chunks, export_to_csv_buffer, find_dtype, find_dtype_formatter, find_selected_column, get_bool_arg, get_dtypes, get_int_arg, get_json_arg, get_str_arg, grid_columns, grid_formatter, json_date, json_float, json_int, json_timestamp, jsonify, jsonify_error, make_list, retrieve_grid_params, run_query, running_with_flask_debug, running_with_pytest, sort_df_for_grid
...
class DtaleData(object):
    ...
    def notebook(self, route='/dtale/iframe/', params=None, width='100%', height=475):
        ...
        self._notebook_handle = display(self._build_iframe(route=route, params=params, width=width, height=height), display_id=True)
    ...
    def notebook_charts(self, chart_type='line', query=None, x=None, y=None, z=None, group=None, agg=None, window=None, rolling_comp=None, barmode=None, barsort=None, width='100%', height=800):
        """
        ...
        :return: :class:`ipython:IPython.display.IFrame`
        """
        params = dict(chart_type=chart_type, query=query, x=x, y=make_list(y), z=z, group=make_list(group), agg=agg, window=window, rolling_comp=rolling_comp, barmode=barmode, barsort=barsort)
        self.notebook(route='/charts/', params=chart_url_querystring(params), width=width, height=height)

    def offline_chart(self, chart_type=None, query=None, x=None, y=None, z=None, group=None, agg=None, window=None, rolling_comp=None, barmode=None, barsort=None, yaxis=None, filepath=None, **kwargs):
        """
        ...
        """
        params = dict(chart_type=chart_type, query=query, x=x, y=make_list(y), z=z, group=make_list(group), agg=agg, window=window, rolling_comp=rolling_comp, barmode=barmode, barsort=barsort, yaxis=yaxis)
...
def dtype_formatter(data, dtypes, data_ranges, prev_dtypes=None):
    ...
    def _formatter(col_index, col):
        ...
        s = data[col]
        dtype_data = dict(name=col, dtype=dtype, index=col_index, visible=visible, hasMissing=int(s.isnull().sum()), hasOutliers=0)
        classification = classify_type(dtype)
        if classification in ('F', 'I') and not data[col].isnull().all() and col in data_ranges:
...
def startup(url, data=None, data_loader=None, name=None, data_id=None, context_vars=None, ignore_duplicate=False):
    ...
    if data is not None:
        if not isinstance(data, (pd.DataFrame, pd.Series, pd.DatetimeIndex, pd.MultiIndex)):
            raise Exception('data loaded must be one of the following types: pandas.DataFrame, pandas.Series, pandas.DatetimeIndex')
...
def base_render_template(template, data_id, **kwargs):
    ...
    return render_template(template, data_id=data_id, settings=json.dumps(curr_settings), version=str(version), processes=len(global_state.get_data()), **kwargs)
...
def get_processes():
    ...
    def _load_process(data_id):
        ...
        return dict(data_id=data_id, rows=len(data), columns=len(dtypes), names=(',').join([ c['name'] for c in dtypes ]), start=json_date(mdata['start']), ts=json_timestamp(mdata['start']), name=mdata['name'])
...
def refresh_col_indexes(data_id):
    ...
    global_state.set_dtypes(data_id, [ dict_merge(curr_dtypes[c], dict(index=idx)) for idx, c in enumerate(curr_data.columns) ])
...
def update_column_position(data_id):
    ...
    try:
        ...
        if action == 'front':
            curr_cols = [
             col] + [ c for c in curr_cols if c != col ]
        elif action == 'back':
...
def update_locked(data_id):
    ...
    try:
        ...
        final_cols = curr_settings['locked'] + [ c for c in curr_data.columns if c not in curr_settings['locked'] ]
...
def test_filter(data_id):
    ...
    try:
        ...
        run_query(global_state.get_data(data_id), build_query(data_id, query), global_state.get_context_variables(data_id))
        if get_str_arg(request, 'save'):
            curr_settings = global_state.get_settings(data_id) or {}
            if query is not None:
                curr_settings = dict_merge(curr_settings, dict(query=query))
            else:
                curr_settings = {k:v for k, v in curr_settings.items() if k != 'query' if k != 'query'}
            global_state.set_settings(data_id, curr_settings)
        return jsonify(dict(success=True))
    except BaseException as e:
        return jsonify_error(e)
    ...
    if additional_aggs:
        for agg in additional_aggs:
            if agg == 'mode':
                ...
                code.append(("# mode\nmode = df['{col}'].mode().values\nstats['mode'] = np.nan if len(mode) > 1 else mode[0]").format(col=column_series.name))
                continue
            desc[agg] = getattr(column_series, agg)()
            code.append(("# {agg}\nstats['{agg}'] = df['{col}'].{agg}()").format(col=column_series.name, agg=agg))

    desc_f_overrides = {'I': lambda f, i, c: f.add_int(i, c, as_string=True), 
...
def describe(data_id, column):
    ...
    try:
        ...
        if len(uniq_vals) <= 100:
            ...
            return_data['uniques'] = dict(data=[ uniq_f(u) for u in uniq_vals ], top=False)
        else:
            uniq_vals = data[column].value_counts().sort_values(ascending=False).head(100).index.values
            return_data['uniques'] = dict(data=[ uniq_f(u) for u in uniq_vals ], top=True)
...
def outliers(data_id, column):
    try:
        ...
        code = ("s = df['{column}']\nq1 = s.quantile(0.25)\nq3 = s.quantile(0.75)\niqr = q3 - q1\niqr_lower = q1 - 1.5 * iqr\niqr_upper = q3 + 1.5 * iqr\noutliers = dict(s[(s < iqr_lower) | (s > iqr_upper)])").format(column=column)
...
def edit_cell(data_id, column):
    try:
        ...
        updated_str = updated
        curr_settings = global_state.get_settings(data_id)
        data = run_query(global_state.get_data(data_id), build_query(data_id, curr_settings.get('query')), global_state.get_context_variables(data_id), ignore_empty=True)
        ...
        if updated in ('nan', 'inf'):
            ...
            code.append(("df.loc[{row_index}, '{column}'] = {updated}").format(row_index=row_index, column=column, updated=updated_str))
        else:
            classification = classify_type(dtype)
            if classification == 'B':
                ...
                if classification == 'I':
                    ...
                    if dtype.startswith('category') and updated not in data[column].unique():
                        ...
                        code.append(("data['{column}'].cat.add_categories('{updated}', inplace=True)").format(column=column, updated=updated))
                    updated_str = ("'{}'").format(updated)
                data.at[(row_index, column)] = updated
                code.append(("df.at[{row_index}, '{column}'] = {updated}").format(row_index=row_index, column=column, updated=updated_str))
            curr_history = global_state.get_history(data_id) or []
            curr_history += code
            global_state.set_history(data_id, curr_history)
            data = global_state.get_data(data_id)
...
def get_data(data_id):
    ...
    try:
        ...
        if params.get('sort') is not None:
            ...
            curr_settings = {k:v for k, v in curr_settings.items() if k != 'sort' if k != 'sort'}
        data = run_query(global_state.get_data(data_id), build_query(data_id, curr_settings.get('query')), global_state.get_context_variables(data_id), ignore_empty=True)
        global_state.set_settings(data_id, curr_settings)
        total = len(data)
        results = {}
        ...
        columns = [
         dict(name=IDX_COL, dtype='int64', visible=True)] + global_state.get_dtypes(data_id)
...
def get_column_analysis(data_id):
    ...
    def handle_top(df, top):
        if top is not None:
            top = int(top)
            return (
             df[:top] if top > 0 else df[top:], top)
        else:
            if len(df) > 100:
                top = 100
                return (
        ...
        if data_type == 'value_counts':
            ...
            if ordinal_col is not None:
                if ordinal_agg == 'pctsum':
                    ...
                    code.append(("ordinal_data = df.groupby('{col}')[['{ordinal}']].sum()\nordinal_data = ordinal_data / ordinal_data.sum()").format(col=selected_col, ordinal=ordinal_col))
                else:
                    ordinal_data = getattr(data.groupby(selected_col)[[ordinal_col]], ordinal_agg)()
                    code.append(("ordinal_data = df.groupby('{col}')[['{ordinal}']].{agg}()").format(col=selected_col, ordinal=ordinal_col, agg=ordinal_agg))
                hist['ordinal'] = ordinal_data
                hist = hist.sort_values('ordinal')
                code.append(("chart['ordinal'] = ordinal_data\nchart = chart.sort_values('ordinal')").format(col=selected_col, ordinal=ordinal_col, agg=ordinal_agg))
            ...
            aggs = [
             'count', 'sum' if category_agg == 'pctsum' else category_agg]
            hist = data.groupby(category_col)[[selected_col]].agg(aggs)
            hist.columns = hist.columns.droplevel(0)
            hist.columns = ['count', 'data']
            code.append(("chart = data.groupby('{cat}')[['{col}']].agg(['{aggs}'])").format(cat=category_col, col=selected_col, aggs=("', '").join(aggs)))
            ...
            code.append(("chart = np.histogram(df[~pd.isnull(df['{col}'])][['{col}']], bins={bins})").format(col=selected_col, bins=bins))
...
def get_correlations(data_id):
    ...
    try:
        curr_settings = global_state.get_settings(data_id) or {}
        data = run_query(global_state.get_data(data_id), build_query(data_id, curr_settings.get('query')), global_state.get_context_variables(data_id))
        valid_corr_cols = []
        valid_date_cols = []
...
def get_chart_data(data_id):
    ...
    """
    try:
        data = run_query(global_state.get_data(data_id), build_query(data_id, get_str_arg(request, 'query')), global_state.get_context_variables(data_id))
        ...
        data, code = build_base_chart(data, x, y, group_col=group_col, agg=agg, allow_duplicates=allow_duplicates, rolling_win=window, rolling_comp=comp)
        data['success'] = True
        return jsonify(data)
    except BaseException as e:
        return jsonify_error(e)
...
def get_correlations_ts(data_id):
    ...
    try:
        ...
        if rolling_window:
            ...
            code.append(("corr_ts = df[['{date_col}', '{col1}', '{col2}']].set_index('{date_col}')\ncorr_ts = corr_ts[['{col1}', '{col2}']].rolling({rolling_window}).corr().reset_index()\ncorr_ts = corr_ts.dropna()\ncorr_ts = corr_ts[corr_ts['level_1'] == '{col1}'][['{date_col}', '{col2}']]").format(col1=col1, col2=col2, date_col=date_col, rolling_window=rolling_window))
        else:
...
def get_scatter(data_id):
    ...
    try:
        ...
        if rolling:
            ...
            code.append(("idx = min(df[df['{date_col}'] == '{date}'].index) + 1\nscatter_data = scatter_data.iloc[max(idx - {window}, 0):idx]\nscatter_data = scatter_data['{cols}'].dropna(how='any')").format(date_col=date_col, date=date, window=window, cols=("', '").join(sorted(list(set(cols)) + [date_col]))))
        else:
            data = data[(data[date_col] == date)] if date else data
            data = data[cols].dropna(how='any')
            code.append(("scatter_data = df[df['{date_col}'] == '{date}']" if date else 'scatter_data = df').format(date_col=date_col, date=date))
            code.append(("scatter_data = scatter_data['{cols}'].dropna(how='any')").format(cols=("', '").join(cols)))
        ...
        stats = dict(pearson='N/A' if pd.isnull(pearson) else pearson, spearman='N/A' if pd.isnull(spearman) else spearman, correlated=len(data), only_in_s0=len(data[data[col1].isnull()]), only_in_s1=len(data[data[col2].isnull()]))
        code.append(("scatter_data['{idx_col}'] = scatter_data.index\ns0 = scatter_data['{col1}']\ns1 = scatter_data['{col2}']\npearson = s0.corr(s1, method='pearson')\nspearman = s0.corr(s1, method='spearman')\nonly_in_s0 = len(scatter_data[scatter_data['{col1}'].isnull()])\nonly_in_s1 = len(scatter_data[scatter_data['{col2}'].isnull()])").format(col1=col1, col2=col2, idx_col=idx_col))
        if len(data) > 15000:
            return jsonify(stats=stats, code=('\n').join(code), error='Dataset exceeds 15,000 records, cannot render scatter. Please apply filter...')
