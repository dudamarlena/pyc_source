...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/cli/clickutils.py
...
LOG_LEVELS = dict(debug=logging.DEBUG, info=logging.INFO, warning=logging.WARNING, error=logging.ERROR, critical=logging.CRITICAL)
...
def get_loader_options(key, options):
    ...
    return dict((_build_key(k), v) for k, v in options.items() if k.startswith(key) and v is not None)
