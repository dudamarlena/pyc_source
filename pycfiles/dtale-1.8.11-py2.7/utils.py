...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/utils.py
# Compiled at: 2020-05-03 13:27:02
from __future__ import division
import decimal, json, os, socket, sys, time, traceback
...
class JSONFormatter(object):
    ...
    def format_lists(self, df):
        return {name:[ f(v, nan_display=self.nan_display) for v in df[name].values ] for _idx, name, f in self.fmts if name in df.columns}

    def format_df(self, df):
        formatters = {col:f for _idx, col, f in self.fmts}
        cols = [ col for col in df.columns if col in formatters ]
        return pd.concat([ df[col].apply(lambda v: formatters[col](v, nan_display=self.nan_display))
...
DF_MAPPINGS = {'I': lambda f, i, c: f.add_int(i, c), 
...
def format_grid(df):
    ...
    return {'results': f.format_dicts(df.itertuples()), 
       'columns': col_types}
