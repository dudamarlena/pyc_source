...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/global_state.py
...
def use_store(store_class, create_store):
    ...
    assert all(hasattr(store_class, a) for a in ('get', 'clear', '__setitem__', '__delitem__',
                                                 '__len__', '__contains__')), 'Missing required methods'
    assert issubclass(store_class, MutableMapping) or hasattr(store_class, 'to_dict'), 'Must subclass MutableMapping or implement "to_dict"'
