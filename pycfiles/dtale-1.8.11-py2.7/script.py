...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/cli/script.py
...
@click.option('--no-reaper', is_flag=True, help='flag to turn off auto-reaping (process cleanup after period of inactivity)')
...
@click.option('--log-level', help='Set the logging level', type=click.Choice(list(LOG_LEVELS.keys())), default='info', show_default=True)
...
def main(host=None, port=None, debug=False, no_reaper=False, open_browser=False, name=None, **kwargs):
    ...
    show(host=host, port=int(port or find_free_port()), debug=debug, subprocess=False, data_loader=data_loader, reaper_on=(not no_reaper), open_browser=open_browser, name=name, **kwargs)
