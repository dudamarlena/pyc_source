...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/cli/loaders/csv_loader.py
...
def loader_func(**kwargs):
    ...
    return pd.read_csv(path, **{k:v for k, v in kwargs.items() if k in loader_prop_keys(LOADER_PROPS) if k in loader_prop_keys(LOADER_PROPS)})
...
def find_loader(kwargs):
    ...
    if len([ f for f in csv_opts.values() if f ]):
        ...
        def _csv_loader():
            csv_arg_parsers = {'parse_dates': lambda v: v.split(',') if v else None}
