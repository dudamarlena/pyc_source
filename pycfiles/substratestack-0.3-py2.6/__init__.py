...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/substratestack/__init__.py
# Compiled at: 2011-06-20 09:30:54
...
class SubstrateStack:
    ...
    def add_via(self, via, metal1_name, metal2_name):
        ...
        if self.get_interface_position(metal1_interface) > self.get_interface_position(metal2_interface):
    ...
    def get_interface_position(self, interface):
        ...
        position = -self.bulk_layer.thickness
    ...
    def get_via_height(self, via):
        ...
        if via.bottom_metal.extend_direction == UP:
            top_of_bottom_metal = self.get_interface_position(via.bottom_metal.bottom_interface)
            top_of_bottom_metal += via.bottom_metal.thickness
        else:
            top_of_bottom_metal = self.get_interface_position(via.bottom_metal.top_interface)
        if via.top_metal.extend_direction == DOWN:
            bottom_of_top_metal = self.get_interface_position(via.top_metal.top_interface)
            bottom_of_top_metal -= via.top_metal.thickness
        else:
            bottom_of_top_metal = self.get_interface_position(via.top_metal.bottom_interface)
    ...
    def split_oxide_layer(self, position):
        ...
        for (i, oxide_layer) in enumerate(self.oxide_layers):
            ...
            oxide_bottom = self.get_interface_position(oxide_layer.bottom_interface)
            if oxide_top > position and position > oxide_bottom:
                oxide_layer.thickness = position - oxide_bottom
                new_oxide_layer = OxideLayer(oxide_top - position, oxide_layer.epsilon_rel, oxide_layer.loss_tangent)
                new_interface = Interface(oxide_layer, new_oxide_layer)
                self.interfaces.insert(self.get_interface_number(oxide_layer.top_interface), new_interface)
    ...
    def get_interface_by_position(self, position):
        ...
        for interface in self.interfaces:
            if abs(self.get_interface_position(interface) - position) < float_threshold:
                return interface
        ...
        * all metals extend up
        ...
        for metal_layer in self.metal_layers:
            if metal_layer.top_interface and not metal_layer.bottom_interface:
                top_position = self.get_interface_position(metal_layer.top_interface)
                bottom_position = top_position - metal_layer.thickness
                bottom_interface = self.get_interface_by_position(bottom_position)
                if bottom_interface:
                    metal_layer.bottom_interface = bottom_interface
                else:
                    metal_layer.bottom_interface = self.split_oxide_layer(bottom_position)
            elif metal_layer.bottom_interface and not metal_layer.top_interface:
                bottom_position = self.get_interface_position(metal_layer.bottom_interface)
                ...
                if top_interface:
                    ...
                    metal_layer.top_interface = self.split_oxide_layer(top_position)
    ...
    def merge_oxide_layers(self, oxide_layers):
        ...
        for (i, oxide_layer) in enumerate(oxide_layers[1:]):
            assert oxide_layer.bottom_interface == oxide_layers[i].top_interface
        ...
        merged_oxide_layer = OxideLayer(total_thickness, total_thickness / total_epsilon_rel, total_loss_tangent / total_thickness)
    ...
    def simplify(self):
        ...
        if not self.is_standard():
            self.standardize()
        bottom_oxide_layer_index = self.oxide_layers.index(self.bulk_layer.top_interface.top_layer)
        for metal_layer in self.metal_layers:
            if isinstance(metal_layer.bottom_interface.bottom_layer, BulkLayer):
                continue
            top_oxide_layer_index = self.oxide_layers.index(metal_layer.bottom_interface.bottom_layer)
            self.merge_oxide_layers(self.oxide_layers[bottom_oxide_layer_index:top_oxide_layer_index + 1])
            bottom_oxide_layer_index = self.oxide_layers.index(metal_layer.bottom_interface.top_layer)

        top_oxide_layer_index = self.oxide_layers.index(self.interfaces[(-1)].bottom_layer)
        self.merge_oxide_layers(self.oxide_layers[bottom_oxide_layer_index:top_oxide_layer_index + 1])
    ...
    def simplify2(self):
        ...
        for metal_layer in self.metal_layers:
            ...
            print self.interfaces.index(metal_layer.bottom_interface),
            print '<<', self.interfaces.index(metal_layer.top_interface)
            print bottom_oxide_layer_index, '<', top_oxide_layer_index
            self.merge_oxide_layers(self.oxide_layers[bottom_oxide_layer_index:top_oxide_layer_index + 1])
            bottom_oxide_layer_index = self.oxide_layers.index(metal_layer.bottom_interface.top_layer)
            top_oxide_layer_index = self.oxide_layers.index(metal_layer.top_interface.bottom_layer)
            print bottom_oxide_layer_index, '<', top_oxide_layer_index
            self.merge_oxide_layers(self.oxide_layers[bottom_oxide_layer_index:top_oxide_layer_index + 1])
            bottom_oxide_layer_index = self.oxide_layers.index(metal_layer.top_interface.top_layer)
    ...
    def write_momentum_substrate(self, filename, infinite_ground_plane=False):
        ...
        for (i, oxide_layer) in enumerate(self.oxide_layers):
            ...
            if metal:
                ...
                metal_text.append('MET%s %s %s 1 2 3 %s 0 Siemens/m Siemens/m 1 %s um' % (
                 str(metal_number).ljust(3), metal.name.ljust(10),
                 str(y - (oxide_layer.thickness - metal.thickness)).ljust(12),
                 str(sigma).ljust(16),
                ...
                if via:
                    ...
                    metal_text.append('MET%s %s %s 0 4 3 %s 0 Siemens/m Siemens/m 0 %s um' % (
        ...
        if not infinite_ground_plane:
            text.append('SUB%d AIR 1 1 0 0 1 0 -1 0 0 1 0 3' % (len(self.oxide_layers) + 2))
        text += metal_text
        f.write(('\n').join(text))
        f.close()
        self.oxide_layers.reverse()
    ...
    def write_sonnet_technology(self, filename):
        ...
        text.append('BOX %d 4064 4064 32 32 20 0' % (len(self.oxide_layers) + 1))
        text.append('      %g %g 1 %g 0 %g 0 "%s"' % (500, 1.0, 0.0, 0.0, 'air'))
        for (i, oxide_layer) in enumerate(self.oxide_layers):
    ...
    def draw(self, filename, pages=3, single_page=True):
        ...
        def draw_oxide(oxide_layer, y):
            ...
            c.rect(x1 * units.mm, y * units.mm, (x2 - x1) * units.mm, oxide_thickness * units.mm, stroke=1, fill=1)
            c.setFillGray(0.0)
            c.drawString(x_thickness * units.mm, (y + oxide_thickness / 2) * units.mm - fontsize / 2, 'd = %g μm (%g kA)' % (oxide_layer.thickness / um,
             oxide_layer.thickness / kA))
            c.drawString(x_eps * units.mm, (y + oxide_thickness / 2) * units.mm - fontsize / 2, 'εr = %g' % (oxide_layer.epsilon_rel,))
            return oxide_thickness

        def draw_interface(interface, y):
            number = self.get_interface_number(interface)
            c.drawRightString((x1 - x_interface_number - x_space) * units.mm, y * units.mm - fontsize / 2, '%d' % (number,))
            c.drawRightString((x1 - x_interface_number2 - x_space) * units.mm, y * units.mm - fontsize / 2, '%d' % (total_interfaces - number,))
            c.line((x1 - x_interface_number) * units.mm, y * units.mm, x1 * units.mm, y * units.mm)
            c.line(x2 * units.mm, y * units.mm, (x2 + x_interface_number / 2) * units.mm, y * units.mm)
            c.drawString((x2 + x_interface_number / 2 + x_space) * units.mm, y * units.mm - fontsize / 2, '%g μm' % (self.get_interface_position(interface) / um,))
        ...
        def draw_metal(metal_layer):
            ...
            c.rect((x1 + x_metal_offset) * units.mm, y * units.mm, x_metal_width * units.mm, extend_direction * metal_thickness * units.mm, stroke=1, fill=1)
            center = (x1 + x_metal_offset + x_metal_width / 2.0,
             y + extend_direction * metal_thickness / 2.0)
            text = (metal_layer.name,
             'd = %g μm (%g kA)' % (metal_layer.thickness / um,
              metal_layer.thickness / kA),
             'σ = %.3g S/m' % metal_layer.get_conductivity(),
             'Rsheet = %g mΩ/□' % (metal_layer.sheet_resistance / mOhm_sq))
            c.setFillGray(0.0)
        ...
        def draw_via(via, metal, y):
            ...
            c.rect(x * units.mm, y_top_of_metal * units.mm, x_via_width * units.mm, via_height * units.mm, stroke=1, fill=1)
            center = (x1 + x_metal_offset + x_metal_width / 2.0,
             y + via_height / 2.0)
            if extend_direction == UP:
                center = (
                 center[0], center[1] + metal_thickness)
            text = (
             via.name,
             'h = %g μm (%g kA)' % (via.get_height() / um,
              via.get_height() / kA),
             'σeq = %.3g S/m' % via.get_conductivity(),
             'R = %g Ω' % via.resistance,
