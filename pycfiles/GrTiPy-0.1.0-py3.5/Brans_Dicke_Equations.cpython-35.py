...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\GrTiPy\Brans_Dicke_Equations.py
# Compiled at: 2018-10-26 14:59:22
...
def Brans_Dicke_Equations(d, x, g, ginverse, psi, T):
    ...
    for a in range(d):
        for b in range(d):
            summ = Ricci[a][b] - 0.5 * R * g[a][b] - Lambda * g[a][b] + 8 * pi / phi * T[a][b] * g[a][b] + omega / phi ** 2 * (diff(phi, x[a]) * diff(phi, x[b]) - 0.5 * g[a][b] * f) + 1 / phi * (diff(phi, x[a], x[b]) - g[a][b] * fin)
            G[a][b] = summ
