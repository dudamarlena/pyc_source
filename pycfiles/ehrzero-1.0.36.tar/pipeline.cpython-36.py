...
# [GCC 8.4.0]
# Embedded file name: /project2/ishanu/DMYTRO/ICD9_PIPELINE/CMD_TRIAL/ehrzero/pipeline.py
# Compiled at: 2019-04-12 12:28:39
# Size of source mod 2**32: 37658 bytes
...
def run_saved_pipeline(PIPELINE):
    ...
    for KEY in PIPELINE['CONDITIONS']:
        ...
        Z = Z3Classifier(llk_path='bin/llk', use_own_pfsa=True,
          posmod=(PIPELINE['PFSA_POS'] % KEY),
          negmod=(PIPELINE['PFSA_NEG'] % KEY))
        llk_set['record'] = [' '.join(A.split()[:PIPELINE['LENGTH']]) for A in llk_set['record']]
...
def predict(patients_file, out_file, disease_groups, phenotypes, gender, model_path, pfsa_path, separator, delimiter, n_first_weeks=9999, save_features=False, procdb='bin/procdb', dx_input='dx_input.dat', work_dir='STANDALONE', T=1, N=2, z=1, a=20):
    if work_dir not in os.listdir():
        ...
        with open(patients_file, 'r') as (input_file):
            ...
            with open(os.path.join(work_dir, dx_input), 'w') as (dx_file):
                for i, line in enumerate(patients):
                    print(i)
                    dx_file.write(convert_to_dx(line, separator, delimiter, i))
        ...
        predictions = run_saved_pipeline({'LENGTH':n_first_weeks, 
         ...
         'Scaler':MINMAX, 
         'Imputer':IMPUTER, 
         'PFSA_POS':pfsa_path + '/POS.pfsa', 
         'PFSA_NEG':pfsa_path + '/NEG.pfsa'})
        if not save_features:
            predictions[['patient_id', 'predicted_risk']].to_csv(out_file, index=False)
        else:
