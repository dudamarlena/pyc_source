...
# [GCC 8.4.0]
# Embedded file name: /home/ishanu/.local/lib/python3.6/site-packages/ehrzero/tools.py
# Compiled at: 2019-04-24 21:51:55
...
def reduce_memory(df):
    for col in df.columns:
        if df[col].dtype != object:
            ...
            elif mn > np.iinfo(np.int16).min:
                if mx < np.iinfo(np.int16).max:
                    df[col] = df[col].astype(np.int16)
            else:
                if mn > np.iinfo(np.int32).min:
                    if mx < np.iinfo(np.int32).max:
                        df[col] = df[col].astype(np.int32)
                    if mn > np.iinfo(np.int64).min and mx < np.iinfo(np.int64).max:
