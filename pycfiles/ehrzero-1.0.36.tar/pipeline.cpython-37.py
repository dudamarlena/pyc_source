...
# [GCC 8.4.0]
# Embedded file name: /home/ishanu/ZED/Research/XG3_/pycode/predictor/ehrzero/pipeline.py
# Compiled at: 2019-04-11 14:51:28
# Size of source mod 2**32: 39019 bytes
...
def run_saved_pipeline(PIPELINE):
    dfs = []
    ...
    df = combine_disease_groups(dfs, prediction=True)
    ...
    XXX = X.copy()
    XXX['patient_id'] = IDSS
    XXX['predicted_risk'] = lgbm_predict[:, 1]
    return XXX


def predict(patients_file, out_file, disease_groups, phenotypes, model_path, pfsa_path, separator, delimiter, n_first_weeks=9999, save_features=False, procdb='bin/procdb', verbose=0, optimal_thresholds={'M':0.0331428, 
 'F':0.0194285}, tprs={'M':0.138947, 
 'F':0.093377}, fprs={'M':0.007715841, 
 'F':0.002057727}, dx_input='dx_input.dat', work_dir='STANDALONE', T=1, N=2, z=1, a=5):
    if work_dir not in os.listdir():
        ...
        for KEY in disease_groups:
            ...
            ENCODINGS[KEY] = optimize_memory(retrieve_raw_records((encoding_path % KEY), n_first_weeks, full=True))
            try:
        ...
        predictions['p0'] = pd.Series([p0(row[2], row[3], ROC) for row in predictions[['patient_id', 'predicted_risk', 'diagnosis']].itertuples()]).values
        if not save_features:
            OUT = predictions[['patient_id', 'predicted_risk', 'diagnosis', 'p0']]
        else:
            OUT = predictions
...
def p0(risk, diagnosis, ROC):
    tpr, fpr = get_nearest_fpr_tpr(ROC, risk)
    if not diagnosis:
        return tpr
    return 1 - fpr