...
# [GCC 8.4.0]
# Embedded file name: /home/jinli11/Desktop/ehrzero/ehrzero/predictor/ehrzero_/pipeline_zero.py
# Compiled at: 2019-04-24 17:50:38
# Size of source mod 2**32: 33921 bytes
import os, numpy as np, itertools, pickle
...
from ehrzero_.feature_engineering import get_max_streak_length, get_dynamics
from ehrzero_.tools import combine_disease_groups, bash
from ehrzero_.z3 import Z3Classifier
from ehrzero_.preprocessing import convert_to_dx, optimize_memory, retrieve_raw_records
import warnings
...
def load_sequence_data(GENDER, DISEASE_GROUPS, short_path, full_path):
    short_seqs = {}
    full_seqs = {}
    ...
    return (
     short_seqs, full_seqs)


def combine_disease_groups(dfs, ID='patient_id', prediction=False):
    df = reduce(lambda left, right: pd.merge(left, right, on=ID, how='outer').drop_duplicates(), dfs)
    if prediction:
        return df
    else:
...
def run_pipeline(RUNS, GENDER, DISEASE_GROUPS, SHORT_SEQUENCES, FULL_SEQUENCES, RESULT_DIR='SUBMISSION/PFSA', HYPERPARAMETERS='NEXTONE_TUNING.csv', PFSA_PATH='SUBMISSION/PFSA/%s/%s', LLK_PATH='bin/llk', MODEL_PATH='', APPEND_PREDICTIONS_TO='', FIPS='00000', TUNING_LABEL='SAMPLE', POS_EPSILON=0.24, NEG_EPSILON=0.2, PFSA_SPLIT=0.42, TEST_SPLIT=0.4, VALID_SPLIT=0.15, LLK_SEQUENCE_LENGTH=150, downsample=0, verbose=False, save_preds=False):
    start = True
    patient_data = []
    ...
    for x in range(RUNS):
        ...
        for KEY in DISEASE_GROUPS:
            ...
            Z.fit(fit_set, peps=POS_EPSILON, neps=NEG_EPSILON)
            preds['patient_id'] = list(llk_set.patient_id)
            LL = Z.predict_loglike(llk_set)
        ...
        params = list(df.drop(['patient_id', 'target'], 1).columns) + ['MEAN', 'STD', 'RANGE'] + ['AUC', 'AUC_lgbm', 'AUC_nbayes']
        if len(IMP) == 0:
        ...
        TUNED_PARAMS = pd.read_csv(HYPERPARAMETERS)
        ...
        LGBM.fit(Xtt, ytt, eval_set=[
         (
          Xtt, ytt), (Xtv, ytv)], eval_metric='auc', early_stopping_rounds=100, verbose=False)
...
def train_models(GENDER, DISEASE_GROUPS, LLK_SEQUENCE_LENGTH, PFSA_SET, LLK_SET, PFSA_PATH, MODEL_PATH, HYPERPARAMETERS, POS_EPSILON=0.24, NEG_EPSILON=0.2):
    ...
    for KEY in DISEASE_GROUPS:
        ...
        LL = Z.predict_loglike(LLK_SET[KEY])
...
def run_saved_pipeline(PIPELINE, evaluate=False, verbose=False, work_dir='./'):
    ...
    for KEY in PIPELINE['CONDITIONS']:
        ...
        Z = Z3Classifier(use_own_pfsa=True, posmod=PIPELINE['PFSA_POS'] % KEY, negmod=PIPELINE['PFSA_NEG'] % KEY)
        llk_set.to_csv(work_dir + '/SEQ_' + KEY + '.csv', index=False)
        llk_set['record'] = [' '.join(A.split()[1:PIPELINE['LENGTH'] + 1]) for A in llk_set['record']]
        LL = Z.predict_loglike(llk_set[['record']])
        preds[KEY] = np.array(list(LL[1])) - np.array(list(LL[0]))
        preds['patient_id'] = list(llk_set.patient_id)
        VALUES = np.array([[int(int(i) == 1) for i in rec.split()] for rec in llk_set.record])
        RAW_VAL = np.array([[int(i) for i in rec.split()] for rec in llk_set.record])
...
def predict(patients_file, out_file, disease_groups, phenotypes, model_path, pfsa_path, separator, delimiter, n_first_weeks=9999, save_features=False, procdb='bin/procdb', verbose=0, optimal_thresholds={'M': 0.017379, 'F': 0.00617}, dx_input='dx_input.dat', work_dir='STANDALONE', T=1, N=2, z=1, a=5):
    ...
    predictions = run_saved_pipeline({'LENGTH': n_first_weeks, 
     ...
     'PFSA_NEG': pfsa_path % gender + '/%s' + '/NEG.pfsa'}, evaluate=True, verbose=verbose, work_dir=work_dir)
    threshold = optimal_thresholds[gender]
    predictions['diagnosis'] = [int(pred > threshold) for pred in predictions.predicted_risk]
    predictions['confidence'] = pd.Series([p0(row[2], row[3], ROC) for row in predictions[['patient_id',
     'predicted_risk',
...
def predict_with_confidence(patients_file, out_file, disease_groups, phenotypes, model_path, pfsa_path, separator, delimiter, n_first_weeks=[
 9999], procdb='bin/procdb', verbose=0, optimal_thresholds={'M': 0.017379, 'F': 0.00617}, dx_input='dx_input.dat', work_dir='tmp', T=1, N=2, z=1, a=5):
    ...
    for seq_length in n_first_weeks:
        ...
        MINMAX = pickle.load(open(model_path + '/%s/NB_SCALER.pickle' % gender, 'rb'))
        IMPUTER = pickle.load(open(model_path + '/%s/IMPUTER.pickle' % gender, 'rb'))
        predictions = run_saved_pipeline({'LENGTH': seq_length, 
         'CONDITIONS': disease_groups, 
         'DATA': ENCODINGS, 
         'LGBM': LGBM, 
         'NB': NB, 
         ...
         'Imputer': IMPUTER, 
         'PFSA_POS': pfsa_path % gender + '/%s' + '/POS.pfsa', 
         'PFSA_NEG': pfsa_path % gender + '/%s' + '/NEG.pfsa'}, evaluate=False, verbose=verbose, work_dir=work_dir)[[
         'patient_id', 'risk']]
        ROC_WEEKS = [
        ...
        predictions['confidence'] = pd.Series([confidence(row[2], row[3], seq_length, ROCS) for row in predictions[['patient_id',
         'risk',
         'diagnosis']].itertuples()]).values
