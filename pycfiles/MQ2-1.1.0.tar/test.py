...
# [GCC 8.4.0]
# Embedded file name: /home/pierrey/repos/gitrepo/MQ2/test/test.py
# Compiled at: 2013-06-11 11:15:55
"""
...
TEST_INPUT_FAILED = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'invalid', 'multi_plugin.zip')
...
class MQ2tests(unittest.TestCase):
    ...
    def test_mq2_fake_zip(self):
        u""" Test MQ² with a fake zip file """
    ...
    def test_mq2_get_matrix_dimensions(self):
        u""" Test MQ² get_matrix_dimensions function """
        length, width = MQ2.get_matrix_dimensions(os.path.join(TEST_FOLDER, 'mapqtl', 'qtls.exp'))
        self.assertEqual(length, 5)
        self.assertEqual(width, 14)

    def test_mq2_read_input_file(self):
        u""" Test MQ² read_input_file function """
        matrix = MQ2.read_input_file(os.path.join(TEST_FOLDER, 'mapqtl', 'qtls.exp'), sep=',')
        self.assertEqual(len(matrix), 5)
        self.assertEqual(len(matrix[0]), 14)

    def test_MQ2_write_matrix(self):
        u""" Test MQ² write_matrix function """
        data = [
         [
          1, 2, 3], [1, 2, 3, 4], 'test']
    ...
    def test_get_plugin_and_folder_too_many_inputs(self):
        ...
        """
        self.assertRaises(MQ2.MQ2Exception, mq2.get_plugin_and_folder, inputzip=TEST_INPUT_PASSED, inputfile=TEST_INPUT_FILE)

    def test_get_plugin_and_folder_no_input(self):
        """ Test the get_plugin_and_folder function with no input
        """
        self.assertRaises(MQ2.MQ2Exception, mq2.get_plugin_and_folder)
