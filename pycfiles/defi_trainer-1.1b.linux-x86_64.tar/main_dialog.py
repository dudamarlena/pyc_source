...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.6/dist-packages/defi_trainer/ui/dialogs/main_dialog.py
# Compiled at: 2009-05-31 14:37:54
"""
...
class DefiDialog(QtGui.QDialog, Ui_MainDlg):
    ...
    def __init__(self, defis, parent=None):
        ...
        self.connect(self.show_defi_button, QtCore.SIGNAL('clicked()'), self.show_defi)
        self.connect(self.correct_defi_button, QtCore.SIGNAL('clicked()'), self.action_defi_correct, QtCore.SLOT('trigger()'))
        self.connect(self.incorrect_defi_button, QtCore.SIGNAL('clicked()'), self.action_defi_incorrect, QtCore.SLOT('trigger()'))
        self.connect(self.action_defi_correct, QtCore.SIGNAL('triggered()'), self.correct_defi)
        self.connect(self.action_defi_incorrect, QtCore.SIGNAL('triggered()'), self.next_defi)
        ...
        self.connect(self.correct_shortcut, QtCore.SIGNAL('activated()'), self.action_defi_correct, QtCore.SLOT('trigger()'))
        self.connect(self.incorrect_shortcut, QtCore.SIGNAL('activated()'), self.action_defi_incorrect, QtCore.SLOT('trigger()'))
        self.defis_right = 0
        self.update_stats()
        ...
        for (section, element) in defis.iteritems():
            for item in element:
                ...
                if section != '__STANDARD__':
                    new_element['name'] += ' (%s)' % section
                new_element['defi'] = ''
                for string in item[1]:
                    new_element['defi'] += unicode(string) + ' '
    ...
    def correct_defi(self):
        ...
        del self.defis[0]
        if len(self.defis) == 0:
            QtGui.QMessageBox.information(self, self.tr('All definitions correct!'), self.tr("All definitions correct. Let's start again!"))
