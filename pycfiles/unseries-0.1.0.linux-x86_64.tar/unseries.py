...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/unseries.py
# Compiled at: 2017-06-19 11:07:51
...
class Series:
    ...
    def __init__(self, n, d={0: 0}, name='g', analytic=False):
        ...
        for k, v in d.items():
            if isinstance(v, AffineScalarFunc):
                ...
                print ('Series constructor warning: Type(v)={}').format(type(v))
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Series):
            ...
            if other == 0 or sum(map(lambda v: v == 0, self.gSeries.values())) == len(self.gSeries):
                return 0
    ...
    def __invert__(self):
        ...
        if self.gSeries[0] == 1:
            c = 1.0
            normed_series = self + Series(self.n, {0: -1}, self.name, analytic=self.analytic)
        else:
            if self.gSeries[0] != 0:
                c = 1.0 / self.gSeries[0]
                normed_series = self / self.gSeries[0] + Series(self.n, {0: -1}, self.name, analytic=self.analytic)
            else:
    ...
    def __div__(self, other):
        ...
        if isinstance(other, (int, float, Variable, AffineScalarFunc)):
...
if __name__ == '__main__':
    Z1 = Series(1)
    Z2 = Series(2, {0: ufloat(-4, 0.3), 1: ufloat(2, 0.002)})
    print ('Z1 = {}').format(Z1)
    print ('Z2 = {}').format(Z2)
