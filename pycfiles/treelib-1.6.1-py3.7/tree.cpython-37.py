...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treelib/tree.py
# Compiled at: 2020-02-27 08:49:21
# Size of source mod 2**32: 37485 bytes
"""
...
class Tree(object):
    __doc__ = 'Tree objects are made of Node(s) stored in _nodes dictionary.'
    ...
    def __print_backend(self, nid=None, level=ROOT, idhidden=True, filter=None, key=None, reverse=False, line_type='ascii-ex', data_property=None, func=print):
        """
    ...
    def __get(self, nid, level, filter_, key, reverse, line_type):
        ...
        dt = {'ascii':('|', '|-- ', '+-- '), 
         ...
         'ascii-emv':('║', '╟── ', '╙── '), 
         'ascii-emh':('│', '╞══ ', '╘══ ')}[line_type]
        return self._Tree__get_iter(nid, level, filter_, key, reverse, dt, [])
    ...
    def __get_iter(self, nid, level, filter_, key, reverse, dt, is_last):
        ...
        if not self.contains(nid):
            ...
            if level == self.ROOT:
                ...
                leading = ''.join(map(lambda x:                 if not x:
dt_vline + '   ' # Avoid dead code: '    ', is_last[0:-1]))
            ...
            if filter_(node) and node.expanded:
                ...
                for idx, child in enumerate(children):
                    ...
                    for item in self._Tree__get_iter(child.identifier, level, filter_, key, reverse, dt, is_last):
                        yield item
    ...
    def add_node(self, node, parent=None):
        ...
        self._Tree__update_fpointer(pid, node.identifier, self.node_class.ADD)
        self._Tree__update_bpointer(node.identifier, pid)
    ...
    def ancestor(self, nid, level=None):
        ...
        if nid == self.root:
            return self[nid]
        if level >= self.level(descendant.identifier):
            raise InvalidLevelNumber("Descendant level (level %s) must be greater                                       than its ancestor's level (level %s)" % (str(self.level(descendant.identifier)), level))
    ...
    def expand_tree(self, nid=None, mode=DEPTH, filter=None, key=None, reverse=False, sorting=True):
        """
                ...
                if mode in [self.DEPTH, self.WIDTH]:
                    ...
                    if mode is self.ZIGZAG:
                        ...
                        while stack:
                            ...
                            direction = stack or not direction
    ...
    def get_node(self, nid):
        ...
        """
        return nid is None or self.contains(nid) or None
    ...
    def move_node(self, source, destination):
        ...
        self._Tree__update_fpointer(parent, source, self.node_class.DELETE)
        self._Tree__update_fpointer(destination, source, self.node_class.ADD)
        self._Tree__update_bpointer(source, destination)
    ...
    def parent(self, nid):
        ...
        if not self.contains(nid):
            ...
            return pid is None or self.contains(pid) or None
        return self[pid]
    ...
    def paste(self, nid, new_tree, deep=False):
        ...
        if not isinstance(new_tree, Tree):
            ...
            assert self.contains(nid), "Node '%s' is not in the tree" % nid
        ...
        self._Tree__update_bpointer(new_tree.root, nid)
        self._Tree__update_fpointer(nid, new_tree.root, self.node_class.ADD)
    ...
    def remove_node(self, identifier):
        ...
        for id_ in removed:
            if id_ == self.root:
                self.root = None
            self._Tree__update_bpointer(id_, None)
            for cid in self[id_].successors(self._identifier) or []:
                self._Tree__update_fpointer(id_, cid, self.node_class.DELETE)

        self._Tree__update_fpointer(parent, identifier, self.node_class.DELETE)
        self._Tree__update_bpointer(identifier, None)
        for id_ in removed:
            self.nodes.pop(id_)
    ...
    def remove_subtree(self, nid, identifier=None):
        ...
        self._Tree__update_fpointer(parent, nid, self.node_class.DELETE)
        return st
    ...
    def save2file(self, filename, nid=None, level=ROOT, idhidden=True, filter=None, key=None, reverse=False, line_type='ascii-ex', data_property=None):
        """
        ...
        self._Tree__print_backend(nid, level, idhidden, filter, key,
          reverse, line_type, data_property, func=handler)

    def show(self, nid=None, level=ROOT, idhidden=True, filter=None, key=None, reverse=False, line_type='ascii-ex', data_property=None, stdout=True):
        """
        ...
        * Version >= 1.2.7a*: you can also specify the ``line_type`` parameter, such as 'ascii' (default), 'ascii-ex', 'ascii-exr', 'ascii-em', 'ascii-emv', 'ascii-emh') to the change graphical form.
        ...
        try:
            self._Tree__print_backend(nid, level, idhidden, filter, key,
              reverse, line_type, data_property, func=write)
        ...
        for attr, val in attrs.items():
            if attr == 'identifier':
                ...
                if cn.predecessor(self._identifier) is not None:
                    self[cn.predecessor(self._identifier)].update_successors(nid,
                      mode=(self.node_class.REPLACE), replace=val, tree_id=(self._identifier))
    ...
    def to_dict(self, nid=None, key=None, sort=True, reverse=False, with_data=False):
        ...
        if self[nid].expanded:
            ...
            if sort:
                queue.sort(key=key, reverse=reverse)
            for elem in queue:
                tree_dict[ntag]['children'].append(self.to_dict((elem.identifier), with_data=with_data, sort=sort, reverse=reverse))
