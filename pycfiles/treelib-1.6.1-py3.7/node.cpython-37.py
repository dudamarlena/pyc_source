...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/treelib/node.py
...
class Node(object):
    ...
    def set_successors(self, value, tree_id=None):
        ...
        setter_lookup = {'NoneType':lambda x: list(), 
         'list':lambda x: x, 
    ...
    def update_successors(self, nid, mode=ADD, replace=None, tree_id=None):
        ...
        def _manipulator_replace():
            if replace is None:
                raise NodePropertyError('Argument "repalce" should be provided when mode is {}'.format(mode))
            ind = self.successors(tree_id).index(nid)
            self.successors(tree_id)[ind] = replace

        manipulator_lookup = {self.ADD: '_manipulator_append', 
