...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/tests/test_crawler.py
# Compiled at: 2013-07-03 00:22:08
import pytest
from os import path
...
def test_crawl_blacklist(webapp):
    ...
    assert set(result['urls']) == set([ urljoin(webapp.server.base, x) for x in ('foo/',
                                                                                 'asdf/') ])
    assert not result['errors']
...
def test_crawl_max_depth(webapp):
    ...
    assert not result['errors']
    assert sorted(result['urls']) == list(x.format(webapp.server.base) for x in ('{0:s}/asdf/',
                                                                                 '{0:s}/foo/'))
