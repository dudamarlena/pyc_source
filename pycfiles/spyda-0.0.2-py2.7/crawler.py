...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/spyda/crawler.py
# Compiled at: 2013-10-07 22:34:19
"""Crawler"""
import sys
...
def crawl(root_url, allowed_urls=None, blacklist=None, max_depth=0, patterns=None, verbose=False, whitelist=None):
    ...
    while queue:
        try:
            ...
            verbose and log(' {0:d} {1:s} {2:s} {3:s} {4:d} {5:s}', response.status, response.reason, response['content-type'], response.get('content-length', ''), len(links), current_url.utf8())
            for link in links:
...
def parse_options():
    ...
    parser.add_option('-a', '--allowed_url', action='append', default=None, dest='allowed_urls', help='(@deprecated) Allowed url to traverse (multiple allowed).')
    parser.add_option('-b', '--blacklist', action='append', default=None, dest='blacklist', help='Blacklisted URL to not traverse (multiple allowed).')
    parser.add_option('-d', '--max_depth', action='store', type=int, default=0, dest='max_depth', help='Maximum depth to follow (0 for unlimited)')
    parser.add_option('-p', '--pattern', action='append', default=None, dest='patterns', help='URL pattern to match (multiple allowed).')
    parser.add_option('-v', '--verbose', action='store_true', default=False, dest='verbose', help='Enable verbose logging')
    parser.add_option('-w', '--whitelist', action='append', default=None, dest='whitelist', help='Whitelisted URL to traverse (multiple allowed).')
...
def main():
    ...
    if opts.verbose:
        ...
        print ('{0:d} urls in {1:0.2f}s ({2:d}/s) CPU: {3:0.2f}s').format(urls, duration, urls_per_second, cputime)
