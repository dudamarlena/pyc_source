...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htmllist/htmllist_demo.py
...
def get_options_parser():
    ...
    global MAX_AVG_WORDS
    global MIN_AVG_WORDS
    global MIN_FACTOR
    parser = OptionParser(description=(' ').join(main.__doc__.split()))
    parser.add_option('-m', '--monitor', action='store', type='string', default='./temp', dest='monitor', help='Directory to monitor [default: %default]')
    parser.add_option('-o', '--outfile', action='store', type='string', default='./temp', dest='outfile', help='The name of the output file\n[default: %default]')
    parser.add_option('-f', '--format', action='store', type='string', dest='format', default='html', help='Output format, also the type of\nthe output file (html|xml)')
    parser.add_option('-i', '--infile', action='store', type='string', dest='infile', help='Optional input file. Mutual exclusive to\n--monitor, --std, --test')
    parser.add_option('-s', '--std', action='store_true', dest='std', default=False, help='Get the input HTML from the standard input and write the output\nto the standard output, mutual exclusive to --monitor, --output and --test\n[default: %default]')
    parser.add_option('-b', '--browser', action='store_false', dest='browser', default=True, help='Open output file in a web-browser [default: %default]')
    parser.add_option('-v', '--verbose', action='store', type='int', default='0', dest='verbose', help='Verbose mode (level 0-5) [default: %default]')
    group = OptionGroup(parser, 'Script Configuration', 'Pass custom configuration to this script.')
    group.add_option('--algo_dict', action='store', type='string', dest='algo_dict', help='Path to a file that defines a dictionary to\nupdate the algorithms configuration (see "algo_dict.py" documentation)')
    group.add_option('--max_words', action='store', type='int', dest='max_words', help='Maximum average number of words in an HTML\nsection [default: %s]' % MAX_AVG_WORDS)
    group.add_option('--min_words', action='store', type='int', dest='min_words', help='Minimum average number of words in an HTML\nsection [default: %s]' % MIN_AVG_WORDS)
    group.add_option('--min_factor', action='store', type='float', dest='min_factor', help='Minimum pattern factor [default: %s]' % MIN_FACTOR)
    parser.add_option_group(group)
    group = OptionGroup(parser, 'HTML List Configuration', 'Pass custom configuration to the htmllist package (See documentation\nfor help). For updating specific algorithm configuration use the "algo_dict"\noption.')
    group.add_option('--include_tags', action='store', type='string', default=INCLUDE_TAGS, dest='include_tags', help='[default: %default]')
    group.add_option('--exclude_tags', action='store', type='string', default=EXCLUDE_TAGS, dest='exclude_tags', help='[default: %default]')
    group.add_option('--min_len', action='store', type='int', dest='min_len', help='[default: %s]' % MIN_LEN)
    group.add_option('--max_len', action='store', type='int', dest='max_len', help='[default: %s]' % MAX_LEN)
    group.add_option('--min_repeat', action='store', type='int', dest='min_repeat', help='[default: %s]' % MIN_REPEAT)
    group.add_option('--max_repeat', action='store', type='int', dest='max_repeat', help='[default: %s]' % MAX_REPEAT)
    group.add_option('--max_stdv', action='store', type='float', dest='max_stdv', help='[default: %s]' % MAX_STDV)
    group.add_option('--min_weight', action='store', type='float', dest='min_weight', help='[default: %s]' % MIN_WEIGHT)
    group.add_option('--min_coverage', action='store', type='float', dest='min_coverage', help='[default: %s]' % MIN_COVERAGE)
    group.add_option('--min_comp', action='store', type='float', dest='min_comp', help='[default: %s]' % MIN_COMPACTNESS)
    group.add_option('--max_patterns', action='store', type='int', dest='max_patterns', help='[default: %s]' % MAX_PATTERNS)
    parser.add_option_group(group)
    group = OptionGroup(parser, 'For Regression Tests', 'Normal user should not use these options (See documentation for help)')
    group.add_option('--test', action='store_true', dest='test', default=False, help='Create a regression test [default:%default]')
    parser.add_option_group(group)
    return parser
...
def orderd_htmllists(hls):
    ...
    lst = [ TmpData(hl, i, algo_dict[hl.pattern_cls.__class__], hl.factor, pattern_quality(hl.pattern_cls, hl.break_cls), hl.avrg_words_in_section()) for hl in hls for i in hl
          ]
    lst = filter(lambda x: True and x.quality >= x.data.min_quality and x.avrg_words < MAX_AVG_WORDS and x.avrg_words > MIN_AVG_WORDS and x.factor > MIN_FACTOR, lst)
...
def print_html(hls, fl):
    ...
    fl.write('<HTML><HEAD><TITLE>Html-List Breakdown</TITLE></HEAD><BODY><DIV>\n\n')
    for hl in orderd_htmllists(hls):
        lst = hl.get_html_list()
        if not lst or not hl.is_list_valid:
            continue
