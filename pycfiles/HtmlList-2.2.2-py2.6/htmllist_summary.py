...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htmllist/htmllist_summary.py
...
class HtmlListSummary(HtmlList):
    ...
    def __init__(self):
        ...
        self.remove_tags = ('br', 'link', 'nobr', 'em', 'table', 'tr', 'strong', 'cite',
                            'b', 'u', 'i', 'wbr')
    ...
    def handle_sub_html(self, lst, next):
        ...
        lst = [ mtch for mtch in self._title_re.finditer(sub_html) if strip_tags(mtch.group(2)).strip()
              ]
        if lst:
            title = unquote_html(strip_tags(lst[0].group(2).strip('"\'\t\r\n ')))
        else:
            title = None
        if lst:
            link = self._link_re.search(lst[0].group(0))
            if link:
                link = unquote_html(link.group(2).strip('"\'\t\r\n '))
        else:
            link = None
    ...
    def get_html_list_url(self, url, validate=True):
        ...
        if not self.text:
            ...
            if lst:
                if validate and not self._validate(lst):
                    return
                return lst
            return
...
if __name__ == '__main__':
    ...
    for query in argv[1:]:
        ...
        if not lst:
            print 'ERROR:', query, hl.err_lst
        else:
            hl._print_list(query, lst)
