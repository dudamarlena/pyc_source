...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htmllist/htmllist_base.py
...
class HtmlList(Iterable):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ('min_len', 'max_len', 'min_repeat', 'max_repeat', 'max_stdv', 'min_weight',
                    'min_coverage', 'min_comp', 'max_patterns', 'debug_level', 'pattern_num'):
    ...
    def __getattr__(self, name):
        ...
        if name in ('factor', 'pattern', 'indices_lst', 'repeats', 'relevant_items'):
            return getattr(self._rp, name)
        if name in ('exclude_tags', 'include_tags'):
