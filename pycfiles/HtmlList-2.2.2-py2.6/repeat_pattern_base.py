...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htmllist/repeat_pattern_base.py
# Compiled at: 2010-10-16 07:02:00
"""
This is a base class for all algorithms of the repeat-pattern classes.
...
method. In addition I am using "weight" (see the method doc).
...
import math
from tag_tools import CONTAINER_TAGS, Tag
CONTAINER_PENALTY = 0.5
...
class RepeatPatternBase(Iterable):
    ...
    def __init__(self, min_len=2, max_len=0, min_repeat=2, max_repeat=0, max_stdv=1, min_weight=0, min_coverage=0, min_comp=0, max_patterns=1, debug_level=0):
    ...
    def _derivative(self, lst, order=1):
        ...
        while order:
            if len(lst) < 2:
                return []
            new_lst = [ abs(lst[(i + 1)] - lst[i]) for i in range(len(new_lst) - 1) ]
            order -= 1
    ...
    def _nstdv(self, lst):
        """ This method calculates a normalized standard deviation on a list of
                ...
                by the maximum of the list, the result should be equal or greater than
        ...
        stdv = math.sqrt(sqr_sum / length - avg * avg)
        return stdv / max(lst)

    def _weight(self, indices):
        """ The weight of a pattern is the distance between the middle index of
                the input-list and the average index of the pattern's span.
        ...
        avg_index = float(sum((end + start) / 2.0 for (start, end) in indices)) / len(indices)
        weight = 1 - abs(half_len - avg_index) / half_len
        return weight

    def _coverage(self, indices):
        """ Measures the volume of content in a pattern, that is how much of the
    ...
    def _compactness(self, pattern, indices):
        ...
        if len(indices) < 2:
            return 0
        return float(len(indices) * len(pattern)) / (indices[(-1)][1] - indices[0][0])
    ...
    def num_patterns(self, value):
        ...
        if value > self._num_patterns:
            self._matrix.extend([None] * self.NUM_FIELDS for i in range(value - self._num_patterns))
    ...
    def process_by_pattern(self, input_lst):
        ...
        n = len(input_lst)
        d = len(self.pattern)
        pattern_set = set(self.pattern)
        self.indices_lst = []
        self.repeats = 0
        i = j = 0
        while i < n:
            if input_lst[i] == self.pattern[j]:
                if j == 0:
                    start = i
                j += 1
            elif input_lst[i] in pattern_set:
    ...
    def sort(self, sort_by=FACTOR):
        ...
        if sort_by == self.PATTERN:
            ...
            raise TypeError('sort_by must be one of RepeatPatternBase fields constants')
    ...
    def get_factor(self, pattern, indices, print_data=False):
        ...
        if repeats < self.min_repeat:
            ...
            if self.max_repeat and repeats > self.max_repeat:
                return 0
            if ptrn_len < self.min_len:
                return 0
            if self.max_len and ptrn_len > self.max_len:
                return 0
            regularity = self._nstdv(self._derivative([ index[0] for index in indices ]))
            if regularity > self.max_stdv:
                return 0
            if self.LEN_ONE_MAGIC is not None and len(pattern) == 1 and regularity > self.LEN_ONE_MAGIC:
                return 0
            weight = self._weight(indices)
            if self.min_weight and weight < self.min_weight:
                return 0
            coverage = self._coverage(indices)
            if self.min_coverage and coverage < self.min_coverage:
                return 0
            compactness = self._compactness(pattern, indices)
            if self.min_comp and compactness < self.min_comp:
                return 0
            container = 1 if not isinstance(pattern[0], Tag) or pattern[0] in CONTAINER_TAGS else CONTAINER_PENALTY
            factor = (1 - regularity) * weight * coverage * compactness * container
            if print_data:
                print 'Factor: %s\n\tRegularity: %s\n\tWeight: %s\n\tCoverage: %s\n\tCompactness: %s' % (
    ...
    def clean_indices(self, indices):
        ...
        def _overlap(index):
            for ind in self.indices_lst:
                if ind[1] > index[0] and ind[0] < index[1]:
                    return True
    ...
    def break_indices_lst(self, indices, two_tuple=True):
        """ Break a list of indices to a list of lists of indices, so all indices
                ...
                A side effect of this method is the removal of indices-lists shorter than
                the minimum.
        ...
        if stdv < self.max_stdv:
            return [indices]
        max_gap = max(der_lst)
        for (index, val) in enumerate(der_lst):
            if max_gap == val:
                return self.break_indices_lst(indices[:index + 1], two_tuple=two_tuple) + self.break_indices_lst(indices[index + 1:], two_tuple=two_tuple)
