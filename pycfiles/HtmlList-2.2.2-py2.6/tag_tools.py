...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htmllist/tag_tools.py
...
"""
FORMAT_ATTRIBUTES = ('class', 'style', 'type', 'headers')
...
class Tag(object):
    ...
    def __init__(self, name, attrs=None, other_info=None):
        ...
        if attrs:
            self._attrs = tuple((name, val) for (name, val) in attrs.items() if name in FORMAT_ATTRIBUTES)
        else:
            self._attrs = None
    ...
    def __repr__(self):
        """ Print for debugging """
        if self._attrs:
            attrs = ' ' + (',').join('%s="%s"' % (name, val) for (name, val) in self._attrs)
        else:
            attrs = ''
        if self._info:
            info = ' (%s)' % self._info
