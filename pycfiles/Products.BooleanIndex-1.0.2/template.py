...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/util/template.py
    ...
    def _interpret_for(self, vars, expr, content, ns, out):
        ...
        for item in expr:
            if len(vars) == 1:
                ...
                raise ValueError('Need %i items to unpack (got %i items)' % (
                 len(vars), len(item)))
...
def trim_lex(tokens):
    ...
    for i in range(len(tokens)):
        ...
        if isinstance(tokens[i], basestring):
            continue
        item = current[0]
        if not statement_re.search(item):
            if item not in single_statements:
                continue
            prev = i or ''
        else:
            prev = tokens[(i - 1)]
...
def parse_one_cond(tokens, name, context):
    ...
    if first.startswith('if '):
        part = (
         'if', pos, first[3:].lstrip(), content)
    elif first.startswith('elif '):
        part = (
         'elif', pos, first[5:].lstrip(), content)
    elif first == 'else':
        part = (
         'else', pos, None, content)
    else:
