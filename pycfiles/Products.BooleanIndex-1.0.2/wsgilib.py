...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/wsgilib.py
...
import warnings
__all__ = [
 'add_close', 'add_start_close', 'capture_output', 'catch_errors',
...
def raw_interactive(application, path='', raise_on_wsgi_error=False, **environ):
    ...
    try:
        try:
            ...
            e.args = (
             e.args[0] + ' iterable: %r' % app_iter,) + e.args[1:]
            raise
    ...
    return (
     data['status'], data['headers'], ('').join(output),
     errors.getvalue())
...
def _warn_deprecated(new_func):
    ...
    def replacement(*args, **kw):
        warnings.warn('The function wsgilib.%s has been moved to %s' % (
         new_name, new_path), DeprecationWarning, 2)
