...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/debug/watchthreads.py
...
class WatchThreads(object):
    ...
    def __call__(self, environ, start_response):
        if 'paste.httpserver.thread_pool' not in environ:
            ...
            return [
             'You must use the threaded Paste HTTP server to use this application']
        else:
    ...
    def kill(self, environ, start_response):
        ...
        exc = httpexceptions.HTTPFound(headers=[
         (
          'Location', script_name + '?kill=%s' % thread_id)])
        return exc(environ, start_response)
