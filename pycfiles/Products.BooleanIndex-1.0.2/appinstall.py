...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/appinstall.py
...
class AbstractInstallCommand(Command):
    ...
    if os.environ.get('HOME'):
        default_sysconfigs.insert(0, (False,
         os.path.join(os.environ['HOME'], '.paste', 'config', 'sysconfig.py')))
...
class SetupCommand(AbstractInstallCommand):
    ...
    parser = AbstractInstallCommand.standard_parser(simulate=True, quiet=True, interactive=True)
    parser.add_option('--name', action='store', dest='section_name', default=None, help='The name of the section to set up (default: app:main)')
    ...
    def command(self):
        ...
        if dist is None:
            raise BadCommand('The section %r is not the application (probably a filter).  You should add #section_name, where section_name is the section that configures your application' % plain_section)
...
class Installer(object):
    ...
    def template_renderer(self, content, vars, filename=None):
        ...
        if self.use_cheetah:
            ...
            tmpl = Cheetah.Template.Template(content, searchList=[
             vars])
...
# global Cheetah ## Warning: Unused global