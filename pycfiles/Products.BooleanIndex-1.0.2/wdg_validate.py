...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/debug/wdg_validate.py
...
class WDGValidateMiddleware(object):
    ...
    def add_error(self, html_page, html_errors):
        ...
        if match:
            return [
             html_page[:match.start()] + add_text + html_page[match.start():]]
        else:
            return [
             html_page + add_text]
