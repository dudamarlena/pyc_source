...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/debug/fsdiff.py
...
import operator, re
__all__ = [
 'Diff', 'Snapshot', 'File', 'Dir', 'report_expected_diffs',
 'show_diff']
...
class Diff(object):
    ...
    def report(self, header=True, dates=False):
        ...
        for (name, files, show_size) in [('created', self.created, True),
         (
...
class Snapshot(IterableUserDict):
    ...
    def __init__(self, base_path, files=None, ignore_wildcards=(), ignore_paths=(), ignore_hidden=True):
    ...
    def clone(self):
        return self.__class__(base_path=self.base_path, ignore_wildcards=self.ignore_wildcards, ignore_paths=self.ignore_paths, ignore_hidden=self.ignore_hidden)
...
def show_diff(actual_content, expected_content):
    actual_lines = [ l.strip() for l in actual_content.splitlines() if l.strip()
                   ]
