...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/copydir.py
...
def copy_dir(source, dest, vars, verbosity, simulate, indent=0, use_cheetah=False, sub_vars=True, interactive=False, svn_add=True, overwrite=True, template_renderer=None):
    """
    ...
    if not os.path.exists(dest):
        ...
        if not simulate:
            svn_makedirs(dest, svn_add=svn_add, verbosity=verbosity, pad=pad)
    elif verbosity >= 2:
    ...
    for name in names:
        ...
        if use_pkg_resources:
            if pkg_resources.resource_isdir(source[0], full):
                ...
                copy_dir((source[0], full), dest_full, vars, verbosity, simulate, indent=indent + 1, use_cheetah=use_cheetah, sub_vars=sub_vars, interactive=interactive, svn_add=svn_add, template_renderer=template_renderer)
                continue
            elif not use_pkg_resources and os.path.isdir(full):
                if verbosity:
                    print '%sRecursing into %s' % (pad, os.path.basename(full))
                copy_dir(full, dest_full, vars, verbosity, simulate, indent=indent + 1, use_cheetah=use_cheetah, sub_vars=sub_vars, interactive=interactive, svn_add=svn_add, template_renderer=template_renderer)
                continue
            ...
            if sub_file:
                try:
                    content = substitute_content(content, vars, filename=full, use_cheetah=use_cheetah, template_renderer=template_renderer)
                except SkipTemplate:
                    continue
                else:
                    if content is None:
            ...
            if already_exists:
                ...
                if old_content == content:
                    ...
                    continue
                if interactive:
                    if not query_interactive(full, dest_full, content, old_content, simulate=simulate):
                        continue
                elif not overwrite:
                    continue
            ...
            if svn_add and not already_exists:
                if os.path.exists(os.path.join(os.path.dirname(os.path.abspath(dest_full)), '.svn')) or verbosity > 1:
                ...
                if not simulate:
                    if subprocess is None:
                        raise RuntimeError('copydir failed, environment does not support subprocess module')
...
def query_interactive(src_fn, dest_fn, src_content, dest_content, simulate):
    ...
    added = len([ l for l in u_diff if l.startswith('+') if not l.startswith('+++')
                ])
    removed = len([ l for l in u_diff if l.startswith('-') if not l.startswith('---')
                  ])
    if added > removed:
...
def substitute_content(content, vars, filename='<string>', use_cheetah=False, template_renderer=None):
    ...
    if template_renderer is not None:
        ...
        tmpl = Cheetah.Template.Template(source=content, searchList=[
         vars])
