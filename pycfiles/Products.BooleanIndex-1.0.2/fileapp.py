...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/fileapp.py
...
BLOCK_SIZE = 65536
__all__ = [
 'DataApp', 'FileApp', 'DirectoryApp', 'ArchiveStore']
...
class DataApp(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if method not in self.allowed_methods:
            exc = HTTPMethodNotAllowed('You cannot %s a file' % method, headers=[
             (
...
class ArchiveStore(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if application:
            ...
            app.set_content(self.archive.read(path), time.mktime(info.date_time + (0,
                                                                                   0,
                                                                                   0)))
            self.cache[path] = app
