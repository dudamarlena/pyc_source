...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/filemaker.py
...
class FileOp(object):
    ...
    def __init__(self, simulate=False, verbose=True, interactive=True, source_dir=None, template_vars=None):
        """
    ...
    def copy_file(self, template, dest, filename=None, add_py=True, package=True, template_renderer=None):
        """
        ...
        content = self.load_content(base_package, cdir, filename, template, template_renderer=template_renderer)
        if add_py:
    ...
    def find_dir(self, dirname, package=False):
        ...
        packages = [ l.strip() for l in f.readlines() if l.strip() if not l.strip().startswith('#')
                   ]
        f.close()
        if not len(packages):
    ...
    def ensure_dir(self, dir, svn_add=True, package=False):
        ...
        if not dir:
            return
            os.path.exists(dir) or self.ensure_dir(os.path.dirname(dir), svn_add=svn_add, package=package)
            if self.verbose:
