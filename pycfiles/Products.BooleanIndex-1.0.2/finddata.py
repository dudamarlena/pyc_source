...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/util/finddata.py
            ...
            if os.path.isdir(fn):
                ...
                for pattern in exclude_directories:
                    if fnmatchcase(name, pattern) or fn.lower() == pattern.lower():
                        bad_name = True
                        if show_ignored:
                            print >> sys.stderr, 'Directory %s ignored by pattern %s' % (
                             fn, pattern)
                        break
                ...
                for pattern in exclude:
                    if fnmatchcase(name, pattern) or fn.lower() == pattern.lower():
                        bad_name = True
                        if show_ignored:
                            print >> sys.stderr, 'File %s ignored by pattern %s' % (
                             fn, pattern)
                        break
