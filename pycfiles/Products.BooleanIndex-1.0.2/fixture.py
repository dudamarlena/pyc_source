...
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/fixture.py
...
import sys, random, urllib, urlparse, mimetypes, time, cgi, os, shutil, smtplib, shlex
...
class TestResponse(object):
    ...
    def _parse_forms(self):
        ...
        assert not started, 'Danging form: %r' % self.body[started:]
    ...
    def header(self, name, default=NoDefault):
        ...
        for (cur_name, value) in self.headers:
            if cur_name.lower() == name.lower():
                assert not found, 'Ambiguous header: %s matches %r and %r' % (
                 name, found, value)
                found = value

        if found is None:
            if default is NoDefault:
                raise KeyError('No header found: %r (from %s)' % (
    ...
    def follow(self, **kw):
        ...
        """
        assert self.status >= 300 and self.status < 400, 'You can only follow redirect responses (not %s)' % self.full_status
...
class Form(object):
        ...
        for match in self._tag_re.finditer(self.text):
            ...
            if tag == 'select':
                if end:
                    ...
                    raise in_select or AssertionError, '%r without starting select' % match.group(0)
                in_select = None
                continue
            if tag == 'textarea':
                if end:
                ...
                if tag == 'input':
                    if attrs.get('type') == 'radio':
                        ...
                        field = field or Radio(self, tag, name, match.start(), **attrs)
                ...
                assert tag == 'textarea' and not in_textarea, 'Nested textareas: %r and %r' % (
                 in_textarea, match.group(0))
                in_textarea = (field, match.end())
            elif tag == 'select':
                assert not in_select, 'Nested selects: %r and %r' % (
    ...
    def __setitem__(self, name, value):
        ...
        assert fields is not None, 'No field by the name %r found (fields: %s)' % (
    ...
    def __getitem__(self, name):
        ...
        """
        fields = self.fields.get(name)
        assert fields is not None, 'No field by the name %r found' % name
        assert len(fields) == 1, 'Multiple fields match %r: %s' % (
         name, (', ').join(map(repr, fields)))
        return fields[0]
    ...
    def set(self, name, value, index=None):
        ...
        if index is None:
            ...
            fields = self.fields.get(name)
            assert fields is not None, 'No fields found matching %r' % name
            field = fields[index]
            field.value = value
        return
...
class Select(Field):
    ...
    def __init__(self, *args, **attrs):
        ...
        assert not self.multiple, '<select multiple> not yet supported'
...
class TestFileEnvironment(object):
    ...
    def run(self, script, *args, **kw):
        ...
        if ' ' in script:
            assert not args, 'You cannot give a multi-argument script (%r) and arguments (%s)' % (
             script, args)
    ...
    def _find_exe(self, script_name):
        if self.script_path is None:
            ...
            raise OSError('Script %s could not be found in %s' % (
...
class ProcResult(object):
    ...
    def __str__(self):
        ...
        for (name, files, show_size) in [('created', self.files_created, True),
         (
