...
# Embedded file name: build/bdist.linux-i686/egg/pydo/base.py
...
from pydo.utils import _tupleize, _setize, formatTexp, moduleize, _strip_tablename, every, string_to_obj
from itertools import izip
import re
from inspect import isfunction
_group_pat = re.compile('\\s*group ', re.I)
    ...
    def __init__(cls, cl_name, bases, namespace):
        ...
        if namespace.get('guess_columns', False):
            ...
            if 'fields' in namespace or 'unique' in namespace:
                raise ValueError, 'incompatible declarations: guess_columns with explicit declaration of fields and/or unique'
        ...
        updatefields = (x in cls._fields and x in simplefields or (x, y) for (x, y) in fielddict.iteritems())
...
class PyDO(dict):
    ...
    def project(cls, *fields, **kwargs):
        ...
        if cls._projections.has_key(t):
            ...
            kls = type(klsname, (cls,), dict(fields=fields, mutable=mutable, table=cls.getTable(False), _is_projection=True))
    ...
    def _update_raw(self, adict):
        ...
        sql = 'UPDATE %s SET %s WHERE %s' % (self.getTable(), (', ').join(sqlbuff), where)
    ...
    def updateSome(cls, adict, *args, **fieldData):
        ...
        conn = cls.getDBI()
        converter = conn.getConverter()
        sqlbuff = ['UPDATE ', cls.getTable(), ' SET ', (', ').join([ '%s = %s' % (x, converter(y)) for (x, y) in adict.iteritems() ])]
    ...
    def _validateFields(cls, adict):
        ...
        for k in adict:
            if not cls._fields.has_key(k):
                raise KeyError, 'object %s has no field %s' % (cls, k)
    ...
    def _new(cls, fieldData, refetch):
        ...
        sql = 'INSERT INTO %s (%s) VALUES  (%s)' % (cls.getTable(), (', ').join(cols), (', ').join(converted))
    ...
    def _uniqueWhere(cls, conn, kw, converter=None):
        ...
        if not unique:
            raise ValueError, 'No way to get unique row! %s %s' % (str(kw), unique)
        for u in unique:
    ...
    def _baseSelect(cls, qualified=False):
        ...
        return 'SELECT %s FROM %s' % ((', ').join(cls.getColumns(qualified)), cls.getTable())
    ...
    @classmethod
    def getSome(cls, *args, **fieldData):
    ...
    def joinTable(self, thisAttrNames, pivotTable, thisSideColumns, thatSideColumns, thatObject, thatAttrNames, *whereArgs, **extra):
        ...
        (sql, vals) = self._joinTableSQL(conn, thisAttrNames, pivotTable, thisSideColumns, thatSideColumns, thatObject, thatAttrNames, extraTables, wheresql, converter, order, limit, offset)
        results = conn.execute(sql, vals)
        if results and isinstance(results, (list, tuple)):
            return map(thatObject, results)
        return []

    def _joinTableSQL(self, conn, thisAttrNames, pivotTable, thisSideColumns, thatSideColumns, thatObject, thatAttrNames, extraTables, wheresql, converter, order, limit, offset):
        ...
        if len(thisSideColumns) != len(thisAttrNames):
            raise ValueError, 'thisSideColumns and thisAttrNames must contain the same number of elements'
        if len(thatSideColumns) != len(thatAttrNames):
            raise ValueError, 'thatSideColumns and thatAttrNames must contain the same number of elements'
        sql = [
         thatObject._baseSelect(True), ', ', (', ').join([pivotTable] + extraTables), ' WHERE ']
        joins = []
        ...
        joins.extend([ '%s.%s = %s.%s' % (pivotTable, col, thatObject.getTable(), attr) for (attr, col) in zip(thatAttrNames, thatSideColumns)
                     ])
...
class ForeignKey(object):
    ...
    def __init__(self, this_side, that_side, kls):
        ...
        if len(this_side) != len(that_side):
            raise ValueError, 'lengths of keys do not match: %d (length of %s) != %d (length of %s)' % (len(this_side), str(this_side), len(that_side), str(that_side))
    ...
    def __set__(self, obj, value):
        ...
        elif not isinstance(value, self.kls):
            raise ValueError, 'value passed is a %s, not an instance of %s' % (type(value), self.kls.__name__)
        else:
            obj.update(dict(((this, value[that]) for (this, that) in zip(self.this_side, self.that_side))))
        return
...
def ManyToMany(this_side, pivot_table, this_pivot_side, that_pivot_side, kls, that_side):
    ...
    def getMany(self, *args, **kwargs):
        if isinstance(kls, basestring):
            ...
            realkls = kls
        return self.joinTable(this_side, pivot_table, this_pivot_side, that_pivot_side, realkls, that_side, *args, **kwargs)

    return getMany


__all__ = [
 'PyDO', 'autoschema', 'ForeignKey', 'OneToMany', 'ManyToMany']