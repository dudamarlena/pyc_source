...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dimer/nnet/config_spec.py
...
class CfgFactory(object):
    ...
    def _from_settings(cls, path, new=tuple.__new__, len=len):
        ...
        cfg = _open_cfg(path)
        iterable = map(lambda (k, t): t(cfg.get(cls._section, k)), zip(cls._fields, cls._types))
...
class MtrainSpec(namedtuple('MtrainSpec', 'batch_size l1_rate l2_rate lr tau momentum_mult nepochs minepochs patience'), CfgFactory):
    __slots__ = ()
    _types = (
     int, float, float, float, int, float, int, int, int)
    _section = 'modtrain'

    def _check_consistency(self):
        if self.minepochs > self.nepochs:
            raise ValueError('minepochs (%d) > nepochs (%d)', self.minepochs, self.nepochs)
...
class DataSpec(namedtuple('DataSpec', 'dataname tracks width train_s valid_s labels track_names label_names batch_size train_batches valid_batches')):
    ...
    def batches_from_data(self, tot_size, batch_s, valid_s, valid_idx, rng):
        ...
        if batch_s > tot_size * min(valid_s, 1 - valid_s):
            raise ValueError('batch size (%d) too big > min(valid_s=%d, train_s=%d)', batch_s, tot_size * valid_s, tot_size * (1 - valid_s))
    ...
    def _from_archive(cls, path, batch_s, rng, valid_s, valid_idx):
        ...
        objX = archive.load_object(archive.archname(path), archive.basename(path) + '/X')
        objT = archive.load_object(archive.archname(path), archive.basename(path) + '/T')
        ...
        train_batches, valid_batches = cls.batches_from_data(len(gene_names), batch_s, valid_s, valid_idx, rng)
