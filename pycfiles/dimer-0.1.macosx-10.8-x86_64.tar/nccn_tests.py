...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dimer/nnet/nccn_tests.py
...
class TestLRLayer(base_test_classes.CNNLayerTester):
    ...
    def get_input(self):
        return np.asarray(self.rng.uniform(low=-1, high=1, size=(
         self.bs, self.nin)), dtype=np.float64)

    def get_layer(self):
        l = LogisticReg(T.matrix('X'), self.nin, self.nout, self.rng, self.get_input().dtype)
        l._weights_[0].set_value(np.asarray(rng.rand(self.nin, self.nout), dtype=l.get_weights()[0].dtype))
        return l
    ...
    def test_activation(self):
        ...
        i = self.get_input()
        self.assertFalse(np.all(out_f(i) == 1.0 / self.nout))
        layer.set_weights(self.zero_weights())
        self.assertTrue(np.all(out_f(self.get_input()) == 1.0 / self.nout))
        layer._weights_[0].set_value(np.ones(self.zero_weights()[0].shape, self.zero_weights()[0].dtype))
        print out_f(i) - 1.0 / self.nout
        self.assertAlmostEqual(np.max(np.abs(out_f(i) - 1.0 / self.nout)), 0)
...
class TestHLayer(base_test_classes.CNNLayerTester):
    ...
    def get_layer(self):
        return HiddenLayer(T.matrix('X'), self.nin, self.nout, self.rng, self.get_input().dtype)
...
class TestCPLayer(base_test_classes.CNNLayerTester):
    ...
    def get_layer(self):
        return ConvPoolLayer(T.dtensor4('X'), self.fsh, self.imsh, self.rng, (2, 2), self.get_input().dtype)

    def get_input(self):
        return np.asarray(self.rng.uniform(low=-1, high=1, size=self.imsh), dtype=np.float64)
...
class TestCnnModel(base_test_classes.ModelTester, base_test_classes.NpyTester):
    ...
    def setUp(self):
        ...
        self.G = 2
        self.CW_sh = (
         self.arch[0][0], 1, self.arch[1][0][0], self.arch[1][0][1])
    ...
    def get_model(self):
        return CnnModel(self.arch, self.lrs, self.imsh, self.G, rng, self.get_input().dtype, 'int32')
    ...
    def test_cost(self):
        M = self.get_model()
        l1 = rng.randint(0, 10) / 10.0
        l2 = rng.randint(0, 10) / 10.0
        y = self.get_output()
        x = self.get_input()
    ...
    def test_activation_vs_i(self):
        ...
        model = self.get_model()
        out_f = theano.function(inputs=[model[0].input], outputs=model[0].activation())
        x = self.get_input()
    ...
    def test_update(self):
        ...
        grad_f = theano.function(inputs=[idx], outputs=T.grad(model.cost(0, 0), wrt=params), givens={model.X: x[idx:self.bs * (idx + 1)], model.Y: y[idx:self.bs * (idx + 1)]})
