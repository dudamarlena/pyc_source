...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dimer/nnet/autoencoder.py
...
class AutoEncoder(Model):
    ...
    def __init__(self, ins, hs, rng, theano_rng, wdtype, corruption_level):
        ...
        super(AutoEncoder, self).__init__([
         AELayer(X, ins, hs, rng, theano_rng, wdtype, corruption_level)])
...
class AEStack(Model):
    r"""a stack of denoising autoencoders. Each layer is a denoising autoencoder.
    A denoising autoencoders tries to reconstruct the input from a corrupted
    version of it by projecting it first in a latent space and reprojecting
    it afterwards back in the input space. Please refer to Vincent et al.,2008
    for more details. If x is the input then equation (1) computes a partially
    ...
    def __init__(self, ins, hs_lst, rng, theano_rng, wdtype, corruption_level):
        ...
        layers = []
        for hs in hs_lst:
            ael = AELayer(X, ins, hs, rng, theano_rng, wdtype, corruption_level)
            layers.append(ael)
            X = layers[(-1)].encoder
            ins = hs
