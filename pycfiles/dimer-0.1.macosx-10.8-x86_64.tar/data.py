...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dimer/data.py
# Compiled at: 2013-07-30 19:18:47
"""
...
"""
import logging, numpy as np, pandas as pd, theano, archive, ops
...
class aaHDFArchive(object):
    ...
    def dump(self, path):
        ...
        if X is not None:
            ...
            archive.save_object(arch, '%s/X' % key, pd.Panel(normX.reshape(nsamp, ntrack, width), items=X.items, major_axis=X.major_axis, minor_axis=X.minor_axis))
            archive.save_object(arch, '%s/meanX' % key, pd.DataFrame(meanX, index=X.major_axis, columns=X.minor_axis))
            archive.save_object(arch, '%s/sdX' % key, pd.DataFrame(sdX, index=X.major_axis, columns=X.minor_axis))
        if Y is not None:
...
class aAnchorDataset(object):
    ...
    def __init__(self, X, Y, T, batch_size, valid_s=None, valid_idx=None, rng=None):
        ...
        nb = self.X.shape[0] / self.batch_size
        self.train_batches, self.valid_batches = self.__batches(self.X.shape[0], self.batch_size, valid_s or 0.25, valid_idx or nb - nb / 4, rng)
    ...
    def __sh_anon(self, what, shape=None, borrow=True):
        ...
        if getattr(self, '_AnchorDataset__sh%s' % what) is None:
            ...
            setattr(self, '_AnchorDataset__sh%s' % what, theano.shared(init_val, borrow=borrow))
    ...
    def __batches(tot_size, batch_s, valid_s, valid_idx, rng):
        ...
        try:
            ...
            raise ValueError('valid_idx (%d) should be between 0 and %d', valid_idx, len(all_batches) - 1)
    ...
    def normalize_features(x):
        ...
        pX = pd.Panel(normX, items=x.items, major_axis=x.major_axis, minor_axis=x.minor_axis)
    ...
    def fit_features(x):
        ...
        return pd.Panel(fitX.reshape(x.shape), items=x.items, major_axis=x.major_axis, minor_axis=x.minor_axis)
    ...
    def __init__(self, X, Y, T):
        ...
        if T is not None and X.shape[0] != T.shape[0]:
            raise ValueError('|X| (%d) != |T| (%d)' % (X.shape[0],
             T.shape[0]))
        log.info('allocated dataset. X of shape %s, Y %s, T %s', str(self.X.shape), self.Y is None and 'missing' or str(self.Y.shape), self.T is None and 'missing' or str(self.T.shape))
        return
...
class TheanoShare(object):
    ...
    def __sh_anon(self, what, shape=None, borrow=True):
        ...
        if getattr(self, '_TheanoShare__sh%s' % what) is None:
            ...
            if shape:
                init_val = init_val.reshape(shape)
            setattr(self, '_TheanoShare__sh%s' % what, theano.shared(init_val, borrow=borrow))
...
class TrainDataset(object):
    ...
    def __init__(self, batch_s, tot_s=None, valid_s=None, valid_idx=None, rng=None):
        ...
        if self.X.shape[0] < tot_s:
            log.warning('total size (%d) > dataset size (%d). adjusting ...', tot_s, self.X.shape[0])
...
class AnchorAnnotation(object):
    ...
    def __init__(self, anchors, tracks, width, labels):
        self.pX = pd.Panel(self.X, items=anchors, major_axis=tracks, minor_axis=width)
        if self.Y is not None:
            self.sY = pd.Series(self.Y, index=anchors)
        self.label_names = None
        if self.T is not None:
            coden = map(lambda v: labels[v], self.T)
            self.dfT = pd.DataFrame({'label_code': self.T, 'label_name': coden})
...
class AnchorDataset(Dataset, AnchorAnnotation, TheanoShare):
    ...
    def dump(self, path):
        ...
        if X is not None:
            ...
            normX, meanX, sdX = self.normalize_features(X.values.reshape(nsamp, -1))
            archive.save_object(arch, '%s/X' % key, pd.Panel(normX.reshape(nsamp, ntrack, width), items=X.items, major_axis=X.major_axis, minor_axis=X.minor_axis))
            archive.save_object(arch, '%s/meanX' % key, pd.DataFrame(meanX.reshape(ntrack, width), index=X.major_axis, columns=X.minor_axis))
            archive.save_object(arch, '%s/sdX' % key, pd.DataFrame(sdX.reshape(ntrack, width), index=X.major_axis, columns=X.minor_axis))
        if Y is not None:
            archive.save_object(arch, '%s/Y' % key, Y)
        if T is not None:
            archive.save_object(arch, '%s/T' % key, T)
...
class TrainAnchorDataset(AnchorDataset, TrainDataset):
    ...
    def __init__(self, X, Y, T, batch_s, tot_s=None, valid_s=None, valid_idx=None, rng=None):
