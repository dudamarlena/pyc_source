...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dimer/genome/bedops_tests.py
# Compiled at: 2013-08-03 19:56:07
import sys, os, random, unittest, tempfile
    ...
    def setUp(self):
        if not (hasattr(self, 'anchor_fn') and os.path.exists(self.anchor_fn)):
            with tempfile.NamedTemporaryFile(delete=False) as (rfd):
                rfd.write(('\n').join(('chr1\t10\t100\ta1\t1.3\t+', 'chr1\t200\t290\ta2\t22.2\t+',
                                       'chr4\t5\t95\ta3\t43.5\t-\n')))
        ...
        if not (hasattr(self, 't1') and os.path.exists(self.t1)):
            with tempfile.NamedTemporaryFile(delete=False) as (mfd):
                mfd.write(('\n').join(('chr1\t15\t30\tt11\t1', 'chr1\t25\t30\tmp2\t0.5',
                                       'chr3\t55\t80\tmp2\t1', 'chr4\t20\t56\tmp3\t23.3\n')))
                self.t1 = mfd.name
        self.anchor_len = 90
