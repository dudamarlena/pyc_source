...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/MEEGbuddy/TMS_EEG_GUI.py
# Compiled at: 2018-11-29 08:50:08
...
from tkinter import Tk, Canvas, Frame, Label, Button, Entry, Listbox, Scrollbar, OptionMenu, StringVar, IntVar, Checkbutton
...
class TMS_EEG_GUI(Frame):
    ...
    def save_setup(self):
        ...
        if suffix == 'fif':
            ...
            if suffix == 'vhdr':
                ...
                try:
                    raw = read_raw_brainvision(raw_fname, preload=(os.path.join(self.sub_dirs['tmp'], 'raw_tmp')))
                except Exception as e:
                    print(e)
                ...
                try:
                    ...
                    ref_name = [ch for ch in montage.dig_ch_pos if ch not in raw.info['ch_names']]
                    if len(ref_name) != 1:
                        ref_name = simpledialog.askstring('Reference Channel', 'What is the reference channel name?',
                          parent=(self.root))
                    else:
                        ref_name = ref_name[0]
                    ...
                    raw = raw.add_channels([ref])
                    for ch_type in ('eog', 'ecg', 'emg', 'stim'):
                        chs = [ch for ch in raw.info['ch_names'] if ch_type in ch.lower()]
                        for ch in chs:
                            ch_ix = raw.ch_names.index(ch)
                            raw._data[ch_ix, :] *= 1e-06
    ...
    def save_exp(self, raw):
        ...
        fname = '_'.join([str(self.data_entries[i].get()).replace('.', '_').replace('/', '_') for i in name_items])
        ...
        for stim_ch in stim_chs:
            ...
            info = create_info([str(t) for t in this_triggers], raw.info['sfreq'], ['stim' for _ in this_triggers])
    ...
    def getFrame(self, y0, y1, x0, x1):
        frame = Frame((self.root), height=((y1 - y0) * self.size - self.size / 100),
    ...
    def load_or_init_screen(self):
        self.data_entries = {}
        Label((self.getFrame(0, 0.1, 0, 0.5)), text='Create New', font=(self.large_font)).pack(fill='both', expand=1)
        Label((self.getFrame(0, 0.1, 0.5, 1)), text='Load', font=(self.large_font)).pack(fill='both', expand=1)
        Label((self.getFrame(0.1, 0.15, 0, 0.1)), text='Load new raw file', wraplength=(0.1 * self.size),
          font=(self.medium_font)).pack(fill='both',
          expand=1)
        ...
        Label((self.getFrame(0.15, 0.2, 0, 0.1)), text='Load new location file', wraplength=(0.1 * self.size),
          font=(self.medium_font)).pack(fill='both',
          expand=1)
        ...
        load_new_file_button = Button((self.getFrame(0.1, 0.15, 0.4, 0.5)), text='Browse', font=(self.medium_font))
        load_new_file_button.pack(fill='both', expand=1)
        load_new_file_button.configure(command=(lambda : self.show_file_browser(new_file, (('vhdr', '*.vhdr'), ('fif', '*.fif')))))
        load_loc_file_button = Button((self.getFrame(0.15, 0.2, 0.4, 0.5)), text='Browse', font=(self.medium_font))
        load_loc_file_button.pack(fill='both', expand=1)
        load_loc_file_button.configure(command=(lambda : self.show_file_browser(loc_file, (('bvct', '*.bvct'), ))))
        Label((self.getFrame(0.1, 0.2, 0.5, 0.6)), text='Load previous', wraplength=(0.1 * self.size), font=(self.medium_font)).pack(fill='both', expand=1)
        ...
        load_file_list = Listbox((self.getFrame(0.1, 0.18, 0.6, 0.88)), height=3, font=(self.large_font),
          yscrollcommand=(scrollbar_v.set),
          xscrollcommand=(scrollbar_h.set))
        load_file_list.pack(fill='both', expand=1)
        load_files = [f for f in os.listdir(self.sub_dirs['saved_exp']) if f.split('.')[(-1)] == 'json']
        ...
        load_previous_button = Button((self.getFrame(0.1, 0.2, 0.9, 1)), text='Load', font=(self.medium_font))
        load_previous_button.pack(fill='both', expand=1)
        load_previous_button.configure(command=(lambda : self.load_exp(load_file_list.get(load_file_list.curselection()))))

        def make_data_label(y0, y1, text):
            Label((self.getFrame(y0, y1, 0, 0.2)), text=text, font=(self.medium_font)).pack(fill='both', expand=1)
        ...
        save_button = Button((self.getFrame(0.85, 0.925, 0.45, 0.55)), text='Save', font=(self.medium_font))
        save_button.pack(fill='both', expand=1)
        save_button.configure(command=(self.save_setup))
        next_button = Button((self.getFrame(0.85, 0.925, 0.85, 0.95)), text='Next', font=(self.medium_font))
    ...
    def trials_rejection_screen(self):
        self.data_entries = {}
        Label((self.getFrame(0, 0.1, 0, 0.3)), text='Raw Data & Reject Trials', font=(self.medium_font)).pack(fill='both', expand=1)
        Label((self.getFrame(0.1, 0.15, 0.02, 0.08)), text='Trigger Number', wraplength=(0.05 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        trigger = self.make_option_menu(self.getFrame(0.1, 0.15, 0.08, 0.15), self.data['triggers'].split(','))
        Label((self.getFrame(0.15, 0.2, 0.02, 0.08)), text='Trigger Delay (ms)', wraplength=(0.05 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        ...
        Label((self.getFrame(0.2, 0.25, 0.02, 0.08)), text='EOG Threshold', wraplength=(0.05 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        EOG_threshold = self.make_option_menu(self.getFrame(0.2, 0.25, 0.08, 0.15), [
         '70'])
        self.data_entries['EOG Threshold'] = EOG_threshold
        Label((self.getFrame(0.1, 0.15, 0.16, 0.22)), text='Threshold', wraplength=(0.05 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        threshold = self.make_option_menu(self.getFrame(0.1, 0.15, 0.22, 0.3), [
         '1000'])
        self.data_entries['Threshold'] = threshold
        Label((self.getFrame(0.15, 0.17, 0.16, 0.22)), text='Baseline', wraplength=(0.06 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        Label((self.getFrame(0.15, 0.17, 0.22, 0.26)), text='min', wraplength=(0.04 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        Label((self.getFrame(0.15, 0.17, 0.26, 0.3)), text='max', wraplength=(0.04 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        ...
        bl_normalize = IntVar()
        bl_normalize.set(1)
        bl_normalize_radio = Checkbutton((self.getFrame(0.18, 0.2, 0.16, 0.22)), text='Norm',
          variable=bl_normalize,
          command=(self.changeNormalize))
        bl_normalize_radio.pack(fill='both', expand=1)
        self.data_entries['Baseline Normalize'] = bl_normalize
        Label((self.getFrame(0.2, 0.25, 0.16, 0.22)), text='Interval (ms)', wraplength=(0.05 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        Label((self.getFrame(0.2, 0.22, 0.22, 0.26)), text='min', wraplength=(0.04 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
        Label((self.getFrame(0.2, 0.22, 0.26, 0.3)), text='max', wraplength=(0.04 * self.size),
          font=(self.small_font)).pack(fill='both',
          expand=1)
    ...
    def changeNormalize(self):
        if self.data_entries['Baseline Normalize'].get():
            self.buddy.events['Baseline'] = [self.data['stim_chs'][0],
    ...
    def init_buddy(self):
        self.buddy = MEEGbuddy(subject=(self.data['Name']), fdata=(self.data['fdata']),
          behavior={'Condition': [self.data['Condition'] for i in range(self.data['n_epochs'])]},
          baseline=[
         self.data['triggers'][0],
