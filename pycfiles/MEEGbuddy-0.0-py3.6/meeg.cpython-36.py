...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/MEEGbuddy/meeg.py
# Compiled at: 2019-02-13 14:01:11
# Size of source mod 2**32: 155254 bytes
try:
    from mne.io import Raw, RawArray, set_eeg_reference, BaseRaw
    from mne.preprocessing import ICA, read_ica, create_eog_epochs, create_ecg_epochs, fix_stim_artifact, maxwell_filter
    from mne.epochs import concatenate_epochs
    from mne import compute_covariance, Epochs, EpochsArray, find_events, pick_types, read_source_estimate, compute_morph_matrix, set_log_level, read_trans, read_bem_solution, make_forward_solution, read_epochs, read_source_spaces, BaseEpochs, read_evokeds, EvokedArray, read_labels_from_annot, Label
    from mne.utils import set_config
    from mne.time_frequency import tfr_morlet, tfr_array_morlet, tfr_array_multitaper, AverageTFR, morlet
    from mne.minimum_norm import make_inverse_operator, apply_inverse_epochs, apply_inverse, read_inverse_operator, write_inverse_operator, source_induced_power, source_band_induced_power
...
try:
    ...
    raise ImportError('Unable to import core tools (pandas,glob,re,json,os,scipy,tqdm,joblib,warnings)... must install to continue')
...
try:
    import seaborn as sns, matplotlib.pyplot as plt, matplotlib.patches as patches, matplotlib.ticker as ticker
...
class MEEGbuddy:
    ...
    def __init__(self, subject=None, session=None, fdata=None, behavior=None, baseline=None, stimuli=None, eeg=False, meg=False, response=None, task=None, no_response=None, exclude_response=None, tbuffer=1, subjects_dir=None, epochs=None, event=None, fs_subjects_dir=None, bemf=None, srcf=None, transf=None, seed=551832, file=None):
        """
        ...
        if file is None:
            ...
            meta_data['Task'] = task
            processes = [
             'meta_data', 'raw', 'epochs', 'source_estimates', 'plots',
             'analyses', 'behavior']
            meta_data['Process Directories'] = {}
            for process in processes:
                meta_data['Process Directories'][process] = os.path.join(subjects_dir, process, subject)
            ...
            if not baseline or len(baseline) != 3:
                print('Baseline must contain a channel, start time and stop time. Okay to continue, use normalized=False when making epochs')
            meta_data['Baseline'] = baseline
            meta_data['Time Buffer'] = tbuffer
            meta_data['Seed'] = seed
            if fs_subjects_dir is None:
                print('Please provide the SUBJECTS_DIR specified to freesurfer if you want to do source estimation. These files are not copied over to save space and to preserve the original file identies for clarity and troubleshooting. Pass fs_subjects_dir=False to supress this warning')
            else:
    ...
    def _fname(self, process_dir, keyword, ftype, *tags):
        if process_dir == 'plots':
            fname = self.process_dirs[process_dir]
        else:
    ...
    def _save_raw(self, raw, keyword=None):
        ...
        raw.save((self._fname('raw', 'raw', 'fif', keyword)), verbose=False,
          overwrite=True)

    def _load_raw(self, keyword=None):
        if os.path.isfile(self._fname('raw', 'raw', 'fif', keyword)):
            raw = Raw((self._fname('raw', 'raw', 'fif', keyword)), verbose=False,
              preload=True)
            ...
            if keyword is None:
                ...
                raw = raw.pick_types(meg=(self.meg), eeg=(self.eeg), stim=True, eog=True,
                  ecg=True,
                  emg=True)
            else:
                raise ValueError('No raw data file found ' + 'for %s' % keyword * (keyword is not None))
        return raw

    def _has_raw(self, keyword=None):
        return os.path.isfile(self._fname('raw', 'raw', 'fif', keyword))

    def getTriggers(self, keyword=None):
    ...
    def _load_ICA(self, keyword=None):
        ...
        if os.path.isfile(fname):
            ...
            print('ICA loaded.')
            return ica
        print('No ICA data file found %s' % (keyword if keyword is not None else ''))
    ...
    def findICA(self, eogs=None, ecgs=None, event=None, keyword_in=None, keyword_out=None, n_components=None, l_freq=None, h_freq=40, detrend=1, component_optimization_n=3, tmin=None, tmax=None, vis_tmin=None, vis_tmax=None, seed=11, overwrite=False):
        data_types = [
         'grad', 'mag'] * self.meg + ['eeg'] * self.eeg
        if all([self._has_ICA(keyword=(dt if keyword_out is None else dt + '_' + keyword_out)) for dt in data_types]):
            if not overwrite:
                raise ValueError("ICA already calculated, use 'overwrite=True' to recalculate.")
        if event is None:
        ...
        if not all([ch in inst.ch_names for ch in eogs + ecgs]):
            ...
            for dt in data_types:
                print(dt)
                ica = ICA(method='fastica', n_components=n_components, random_state=seed)
                inst2 = inst.copy().pick_types(meg=(False if dt == 'eeg' else dt), eeg=(dt == 'eeg'))
                ica.fit(inst2)
                fig = ica.plot_components(picks=(np.arange(ica.n_components)), show=False)
                kw = dt if keyword_out is None else dt + '_' + keyword_out
                fig.savefig(self._fname('plots', 'components', 'jpg', kw))
                plt.close(fig)
                if isinstance(inst, BaseRaw):
                    raw = inst.copy().pick_types(meg=(False if dt == 'eeg' else dt), eeg=(dt == 'eeg'),
                      eog=True,
                      ecg=True)
                    all_scores = self._make_ICA_components(raw, ica, eogs, ecgs, detrend, l_freq, h_freq, kw, vis_tmin, vis_tmax)
                inst2 = ica.apply(inst2, exclude=(ica.exclude))
                self._save_ICA(ica, keyword=kw)
                ica_insts.append(inst2)

            ica_insts.append(inst.copy().pick_types(meg=False, eeg=False, eog=True, ecg=True,
              stim=True))
            inst = self._combine_insts(ica_insts)
            if isinstance(inst, BaseRaw):
                self._save_raw(inst, keyword=keyword_out)
            else:
                self._save_epochs(inst, event, keyword=(keyword_out if keyword_out is not None else 'ica'))
    ...
    def _make_ICA_components(self, raw, ica, eogs, ecgs, detrend, l_freq, h_freq, kw, vis_tmin, vis_tmax):
    ...
    def _exclude_ICA_components(self, ica, ch, indices, scores):
        ...
        print('Components removed for %s: ' % ch + ' '.join([str(i) for i in indices]))
        fig = ica.plot_scores(scores, exclude=indices, show=False)
        fig.savefig(self._fname('plots', 'source_scores', 'jpg', ch))
        plt.close(fig)

    def plotICA(self, eogs=None, ecgs=None, event=None, keyword_in=None, keyword_out=None, tmin=None, tmax=None, ylim=dict(eeg=[-40, 40], grad=[-400, 400], mag=[-1000, 1000]), show=True):
        if event is None:
            inst = self._load_raw(keyword=keyword_in)
        else:
            inst = self._load_epochs(event, keyword=keyword_in)
        ...
        for dt in data_types:
            ...
            if isinstance(inst, BaseRaw):
                ...
                fig = inst.average().plot(show=False, ylim=ylim, window_title='Before ICA')
                self._show_fig(fig, show)
                fig2 = inst2.average().plot(show=False, ylim=ylim, window_title='After ICA')
    ...
    def _save_epochs(self, epochs, event, keyword=None):
        print('Saving epochs for ' + event + ' %s' % keyword * (keyword is not None))
        epochs.save(self._fname('epochs', 'epo', 'fif', event, keyword))

    def _save_evoked(self, evoked, event, keyword=None):
        print('Saving evoked for ' + event + ' %s' % keyword * (keyword is not None))
        evoked.save(self._fname('evoked', 'ave', 'fif', event, keyword))

    def _load_epochs(self, event, keyword=None):
        fname = self._fname('epochs', 'epo', 'fif', event, keyword)
        if not os.path.isfile(fname):
            raise ValueError(event + ' epochs must be made first' + ' for %s' % keyword * (keyword is not None))
        epochs = read_epochs(fname, verbose=False, preload=True)
        print('%s epochs loaded' % event + ' for %s' % keyword * (keyword is not None))
        epochs._data = epochs._data.astype('float64')
        return epochs

    def _has_epochs(self, event, keyword=None):
        return os.path.isfile(self._fname('epochs', 'epo', 'fif', event, keyword))

    def _load_evoked(self, event, keyword=None):
        fname = self._fname('epochs', 'ave', 'fif', event, keyword)
        if not os.path.isfile(fname):
            raise ValueError(event + ' evoked must be made first' + ' for autoreject' * ar + ' for %s' % keyword * (keyword is not None))
        evoked = read_evokeds(fname, verbose=False)
        print('%s epochs loaded' % event + ' for autoreject' * ar + ' for %s' % keyword * (keyword is not None))
        return evoked[0]
    ...
    def _save_autoreject(self, event, ar, reject_log):
        np.savez_compressed((self._fname('epochs', 'ar', 'npz', event)), ar=ar, reject_log=reject_log)

    def _save_TFR(self, tfr, frequencies, n_cycles, event, condition, value, keyword, compressed=True):
        print('Saving TFR for %s %s %s' % (event, condition, value))
        if compressed:
            np.savez_compressed((self._fname('analyses', 'tfr', 'npz', event, condition, value, keyword)),
              tfr=tfr,
              frequencies=frequencies,
              n_cycles=n_cycles)
        else:
            np.save(self._fname('analyses', 'tfr', 'npy', event, condition, value, keyword), tfr)
            np.savez_compressed((self._fname('analyses', 'tfr_params', 'npz', event, condition, value, keyword)),
              frequencies=frequencies,
              n_cycles=n_cycles)

    def _load_TFR(self, event, condition, value, keyword=None):
        fname = self._fname('analyses', 'tfr', 'npy', event, condition, value, keyword)
        fname1b = self._fname('analyses', 'tfr_params', 'npz', event, condition, value, keyword)
        fname2 = self._fname('TFR', 'tfr', 'npz', event, condition, value, keyword)
        if os.path.isfile(fname):
            if os.path.isfile(fname1b):
                tfr = np.load(fname)
                f = np.load(fname1b)
    ...
    def _save_CPT(self, event, condition, value, clusters, cluster_p_values, times, frequencies=None, band=None):
        print('Saving CPT for %s %s %s' % (event, condition, value))
        if band:
            np.savez_compressed((self._fname('analyses', 'CPT', 'npz', event, condition, value, band)),
              clusters=clusters,
              cluster_p_values=cluster_p_values,
              band=band)
        else:
            if frequencies:
                np.savez_compressed((self._fname('analyses', 'CPT', 'npz', event, condition, value, 'tfr')),
                  clusters=clusters,
                  frequencies=frequencies,
                  cluster_p_values=cluster_p_values)
            else:
                np.savez_compressed((self._fname('analyses', 'CPT', 'npz', event, condition, value)),
                  clusters=clusters,
                  cluster_p_values=cluster_p_values)
        ...
        if band:
            ...
            if os.path.isfile(fname):
                ...
                if band:
                    ...
                    if tfr:
                        return (
                         f['clusters'], f['cluster_p_values'], f['frequencies'])
                    return (f['clusters'], f['cluster_p_values'])
            else:
                raise ValueError('Cluster permuation test not found for %s %s %s' % (
                 event, condition, value))

    def _save_inverse(self, inv, lambda2, method, pick_ori, event, condition, value, keyword=None):
        print('Saving inverse for %s %s %s' % (event, condition, value))
        write_inverse_operator((self._fname('source_estimates', 'inv', 'fif', keyword, event, condition, value)),
          inv,
          verbose=False)
        np.savez_compressed((self._fname('source_estimates', 'inverse_params', 'npz', keyword, event, condition, value)),
          lambda2=lambda2,
          method=method,
          pick_ori=pick_ori)

    def _load_inverse(self, event, condition, value, keyword=None):
        fname = self._fname('source_estimates', 'inv', 'fif', keyword, event, condition, value)
        fname2 = self._fname('source_estimates', 'inverse_params', 'npz', keyword, event, condition, value)
        if os.path.isfile(fname):
            if os.path.isfile(fname2):
                f = np.load(fname2)
                return (
                 read_inverse_operator(fname), f['lambda2'].item(),
    ...
    def _save_source(self, stc, event, condition, value, keyword=None, fs_av=False):
        if fs_av:
            print('Saving source fs average for %s %s %s' % (event, condition,
             value))
            stc.save((self._fname('source_estimates', 'source', None, keyword, event, condition, value, 'fs_av')),
              ftype='stc')
        else:
            print('Saving source for %s %s %s' % (event, condition, value))
            stc.save((self._fname('source_estimates', 'source', None, keyword, event, condition, value)),
              ftype='stc')

    def _load_source(self, event, condition, value, keyword=None, fs_av=False):
        fname = self._fname('sources', 'source-lh', '.stc', keyword, event, condition, value, 'fs_av' * fs_av)
        if os.path.isfile(fname):
            print('Fs average s' * fs_av + 'S' * (not fs_av) + 'ource loaded for ' + '%s %s %s' % (event, condition, value))
    ...
    def _save_PSD_image(self, image, keyword, ch, N, deltaN, fmin, fmax, NW):
        ...
        np.savez_compressed((self._fname('analyses', 'image', 'npz', ch, 'N_%i_dN_%.2f' % (N, deltaN), 'fmin_%.2f_fmax_%.2f_NW_%i' % (fmin, fmax, NW))),
          image=image)

    def _load_PSD_image(self, keyword, ch, N, deltaN, fmin, fmax, NW):
        fname = self._fname('analyses', 'image', 'npz', ch, 'N_%i_dN_%.2f' % (N, deltaN), 'fmin_%.2f_fmax_%.2f_NW_%i' % (fmin, fmax, NW))
        if os.path.isfile(fname):
            print('Loading image')
    ...
    def autoMarkBads(self, keyword_in=None, keyword_out=None, flat=dict(grad=1e-11, mag=5e-13,
  eeg=2e-05), reject=dict(grad=5e-10, mag=1e-11,
  eeg=0.0005), bad_seeds=0.25, seeds=1000, datalen=1000, overwrite=False):
        keyword_out = keyword_out if keyword_out is not None else keyword_in
        if os.path.isfile(self._fname('raw', 'raw', 'fif', keyword_out)):
            if not overwrite:
                print("Raw data already marked for bads, use 'overwrite=True' to recalculate.")
                return
        ...
        for dt in data_types:
            print(dt)
            raw2 = raw.copy().pick_types(meg=(dt if dt in ('grad', 'mag') else False), eeg=(dt == 'eeg'))
            for i in range(len(raw2.ch_names)):
                ...
                if flat_count > seeds * bad_seeds:
                    ...
                    if reject_count > seeds * bad_seeds:
                        raw.info['bads'].append(raw2.ch_names[i])
    ...
    def plotRaw(self, n_per_screen=20, scalings=None, keyword=None, l_freq=0.5, h_freq=40, overwrite=False):
        if os.path.isfile(self._fname('raw', 'raw', 'fif', keyword)):
            if not overwrite:
        ...
        raw2.plot(show=True, block=True, color=dict(eog='steelblue'), title=('%s Bad Channel Selection' % self.subject),
          order=order,
          scalings=scalings)
    ...
    def makeEpochs(self, keyword_in=None, keyword_out=None, detrend=0, normalized=True, overwrite=False):
        if all([self._has_epochs(event, keyword_out) for event in self.events]):
            ...
            if self.baseline:
                ...
                n_events = self._make_epochs(raw, 'Baseline', ch, tmin, tmax, detrend, keyword_out,
                  n_events=n_events)
        else:
            if normalized:
            ...
            for event in self.events:
                ...
                n_events = self._make_epochs(raw, event, ch, tmin, tmax, detrend, keyword_out,
                  n_events=n_events)
                if normalized:
                    epochs = self._load_epochs(event, keyword=keyword_out)
                    epochs_data = epochs.get_data()
                    epochs_demeaned_data = np.array([arr - baseline_arr.T for arr in epochs_data.T]).T
                    epochs = EpochsArray(epochs_demeaned_data, (epochs.info), events=(epochs.events),
                      verbose=False,
                      proj=False,
                      tmin=(tmin - self.tbuffer))

            if self.response:
                ch, tmin, tmax = self.response
                self._make_epochs(raw, 'Response', ch, tmin, tmax, detrend, keyword_out, n_events=n_events,
                  response=True)

    def _make_epochs(self, raw, event, ch, tmin, tmax, detrend, keyword_out, n_events=None, response=False):
        ...
        try:
            ...
            raise ValueError('%s channel not found in raw' % event + ', maybe you meant to use normalized=False' * (event == 'Baseline'))
        ...
        if response:
            ...
            if n_events is not None:
                if n_events2 + len(self.no_response) != n_events:
                    raise ValueError('%i events compared to ' % n_events + '%i responses + %i excluded responses ' % (n_events2, diff) + "doesn't add up")
            events[:, 2] = response_events
        else:
            exclude = self.exclude_response
            if n_events is not None:
                if n_events2 != n_events:
                    raise ValueError('%i events from previous stimuli, ' % n_events + '%i events from %s' % (n_events2, event))
            events[:, 2] = np.arange(n_events2)
        events = np.delete(events, exclude, axis=0)
        epochs = Epochs(raw, events, tmin=(tmin - self.tbuffer), tmax=(tmax + self.tbuffer),
          baseline=None,
          verbose=False,
          detrend=detrend,
          preload=True)
        if self.eeg:
            epochs = epochs.set_eeg_reference(ref_channels='average', projection=False)
        self._save_epochs(epochs, event, keyword=keyword_out)
        return n_events2

    def demeanEpochs(self, event, condition, values=None, keyword_in=None, keyword_out=None):
        ...
        for value in values:
            ...
            baseline_arr = baseline_data.mean(axis=0).mean(axis=1)
        ...
        epochs_demeaned = EpochsArray(epochs_data, (epochs.info), events=(epochs.events),
          verbose=False,
          proj=False,
          tmin=(epochs.tmin))
    ...
    def plotEpochs(self, event, n_epochs=20, n_channels=20, scalings=None, tmin=None, tmax=None, l_freq=None, h_freq=None, keyword_in=None, keyword_out=None):
        keyword_out = keyword_in if keyword_out is None else keyword_out
        epochs = self._load_epochs(event, keyword=keyword_in)
        tmin, tmax = self._default_t(event, tmin, tmax)
        ...
        if l_freq is not None or h_freq is not None:
            ...
            if len(epochs.event_id) != len(epochs):
                ...
                epochs_copy.plot(n_epochs=n_epochs, n_channels=n_channels, block=True, scalings=scalings)
    ...
    def plotTopo(self, event, condition=None, values=None, epochs=None, keyword=None, ylim={'eeg': [-30, 30]}, l_freq=None, h_freq=None, tmin=None, tmax=None, detrend=1, seed=11, downsample=True, show=True):
        ...
        fig, axs = plt.subplots((2 * self.meg + self.eeg), (len(values)), figsize=(
         5 * len(values), 5 * (2 * self.meg + self.eeg)))
        ...
        fname = (self._fname)('plots', 'evoked', 'jpg', keyword, event, condition, *value_indices.keys())
        fig.savefig(fname)
        self._show_fig(fig, show)

    def plotTopomapBands(self, event, condition, values=None, keyword=None, tfr_keyword=None, contrast=False, tmin=None, tmax=None, tfr=True, bands={'theta':(4, 8),  'alpha':(8, 15),  'beta':(15, 30)}, vmin=None, vmax=None, contours=6, time_points=5, show=True):
        for band in bands:
            self.plotTopomap(event, condition, values=values, keyword=keyword, contrast=contrast,
              tmin=tmin,
              tmax=tmax,
              tfr=True,
              tfr_keyword=tfr_keyword,
              band_struct=(
             band, bands[band][0], bands[band][1]),
              vmin=vmin,
              vmax=vmax,
              contours=contours,
    ...
    def plotTopomap(self, event, condition, values=None, keyword=None, tfr_keyword=None, contrast=False, tmin=None, tmax=None, tfr=False, band_struct=None, vmin=None, vmax=None, contours=6, time_points=5, show=True):
        ...
        if tfr:
            tind = np.array([i for i, t in enumerate(times) if t >= tmin if t <= tmax])
            values_dict, frequencies = self._get_tfr_data(event, condition, values, tfr_keyword, value_indices, tind,
              band=band_struct, mean_and_std=False, band_mean=False)
        else:
            values_dict = self._get_data(epochs, values, value_indices, tmin, tmax, mean_and_std=False)
            frequencies = None
        if contrast:
            ...
            if tfr:
                ...
                for i, t in enumerate(np.linspace(tmin, tmax, time_points)):
                    evo_con.plot_topomap(colorbar=(True if i == time_points - 1 else False), vmin=vmin,
                      vmax=vmax,
                      contours=contours,
                      axes=(axes[i]),
                      title=('time=%0.1f' % t),
                ...
                evo_con.plot_topomap(colorbar=True, vmin=vmin, vmax=vmax, contours=contours,
                  axes=axes,
                  show=False)
            fig.savefig(self._fname('plots', 'topo', 'jpg', event, condition, values[0], values[1], '' if band_struct is None else band_struct[0]))
            self._show_fig(fig, show)
        else:
            for i, value in enumerate(values):
                fig, axes = plt.subplots(1, time_points + (not tfr))
                ...
                if tfr:
                    ...
                    dt = (tmax - tmin) / time_points
                    for i, t in enumerate(np.linspace(tmin, tmax, time_points)):
                        evo.plot_topomap(colorbar=(True if i == time_points - 1 else False), vmin=vmin,
                          vmax=vmax,
                          contours=contours,
                          axes=(axes[i]),
                          title=('time=%0.1f' % t),
                    ...
                    evo.plot_topomap(colorbar=True, vmin=vmin, vmax=vmax, contours=contours,
                      axes=axes,
                      show=False)
                fig.savefig(self._fname('plots', 'topo', 'jpg', event, condition, value, '' if band_struct is None else band_struct[0]))
                self._show_fig(fig, show)

    def dropEpochsByBehaviorIndices(self, bad_indices, event, keyword_in=None, keyword_out=None):
        epochs = self._load_epochs(event, keyword=keyword_in)
    ...
    def markBadChannels(self, bad_channels, event=None, keyword_in=None, keyword_out=None):
        keyword_out = keyword_in if keyword_out is None else keyword_out
        if event is None:
            raw = self._load_raw(keyword=keyword_in)
            ...
            if event is 'all':
                for event in self.events:
                    self.markBadChannels(bad_channels, event=event, keyword_in=keyword_in,
    ...
    def alignBaselineEpochs(self, event, keyword=None):
        ...
        exclude = [i for i in range(len(bl_epochs)) if i not in epochs.selection]
        bl_epochs.drop(exclude)
        self._save_epochs(bl_epochs, 'Baseline', keyword=keyword)

    def plotEvoked(self, event, condition=None, values=None, epochs=None, keyword=None, image=True, ylim={'eeg': [-10, 20]}, l_freq=None, h_freq=None, tmin=None, tmax=None, detrend=1, seed=11, downsample=True, picks=None, show=True):
        epochs = self._prepare_epochs(event, epochs, keyword, tmin, tmax, l_freq, h_freq)
        if condition is None:
            values = [
             'all']
            ...
            if value not in value_indices:
                ...
                if value == 'all':
                    ...
                    if y_dim > 1:
                        ...
                        axs2 = axs
                if self.meg:
                    axs3 = [axs2[0], axs2[1], axs2[2]] if self.eeg else [axs2[0]] if self.eeg else [axs2[0], axs2[1]]
                    evoked.plot(axes=axs3, show=False, ylim=ylim, picks=picks)
                    axs2[0].set_title('%s %s %s %s' % (self.subject, event, condition, value) + ', %i trials used' % len(indices))
                    if image:
                        axs3 = [axs2[3], axs2[4], axs2[5]] if (self.meg and self.eeg) else ([axs2[1]] if self.eeg else [axs2[2], axs2[3]])
    ...
    def _prepare_epochs(self, event, epochs, keyword, tmin, tmax, l_freq, h_freq):
        tmin, tmax = self._default_t(event, tmin, tmax)
        if epochs is None:
            epochs = self._load_epochs(event, keyword=keyword)
        else:
    ...
    def _default_t(self, event, tmin, tmax):
        if tmin is None:
            ...
            if type(tmin) is str:
                condition = tmin
                if condition in self._get_conditions():
                    tmin = {}
                    for i, value in enumerate(self._get_behavior(condition)):
                        tmin[i] = -value
    ...
    def _get_n(self):
        return len(read_csv(self.behavior))

    def _get_behavior(self, condition):
        df = read_csv(self.behavior)
        return df[condition]

    def _get_conditions(self):
        return read_csv(self.behavior).columns

    def _behavior_to_epochs_indices(self, epochs, indices):
        return [self._behavior_to_epochs_index(epochs, i) for i in indices if self._behavior_to_epochs_index(epochs, i)]
    ...
    def _get_binned_indices(self, epochs, condition, bins):
        bin_indices = {}
        h, edges = np.histogram([cd for cd in self._get_behavior(condition) if not np.isnan(cd)],
          bins=bins)
        for j in range(1, len(edges)):
            indices = [i for i in range(self.n) if self.behavior[condition][i] >= edges[(j - 1)] if self.behavior[condition][i] <= edges[j]]
    ...
    def _get_indices(self, epochs, condition, values):
        value_indices = {}
        if len(values) > 4:
            if all([istype(val, int) or istype(val, float) for val in values]):
                binsize = float(value[1] - value[0])
        for value in values:
            if len(values) > 4:
                if all([istype(val, int) or istype(val, float) for val in values]):
                    indices = [i for i in range(self.n) if self.behavior[condition][i] >= value - binsize / 2 if value + binsize / 2 >= self.behavior[condition][i]]
                else:
                    indices = [i for i in range(self.n) if self.behavior[condition][i] == value]
    ...
    def channelPlot(self, event, condition, values=None, keyword=None, butterfly=False, contrast=False, aux=False, tmin=None, tmax=None, vmin=None, vmax=None, show=True):
        self._plotter_main(event, condition, values, butterfly=butterfly, contrast=contrast,
          aux=aux,
          keyword=keyword,
          tmin=tmin,
          tmax=tmax,
    ...
    def plotTFR(self, event, condition, values=None, keyword=None, tfr_keyword=None, contrast=False, butterfly=False, aux=False, bands={'theta':(4, 8), 
 'alpha':(8, 15),  'beta':(15, 30)}, tmin=None, tmax=None, vmin=None, vmax=None):
        if bands:
            for band in bands:
                print(band + ' band')
                fmin, fmax = bands[band]
                band_struct = (band, fmin, fmax)
                self._plotter_main(event, condition, values, contrast=contrast, aux=aux,
                  keyword=keyword,
                  butterfly=butterfly,
                  tfr=True,
                  band=band_struct,
            ...
            values = self._default_values(values, condition, contrast)
            for value in values:
                self._plotter_main(event, condition, [value], contrast=contrast, aux=aux,
                  keyword=keyword,
                  butterfly=butterfly,
                  tfr=True,
                  band=None,
    ...
    def _setup_plot(self, ch_dict, butterfly=False, contrast=False, values=None):
        if butterfly:
            ...
            fig.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1, wspace=0.05,
              hspace=0.05)
    ...
    def _default_values(self, values, condition, contrast=False):
        if values is None:
            values = np.unique([cd for cd in self.behavior[condition] if type(cd) is str or type(cd) is np.string_ or type(cd) is np.str_ or not np.isnan(cd)])
            if len(values) > 5:
                if all([istype(val, int) or istype(val, float) for val in values]):
    ...
    def _get_tfr_data(self, event, condition, values, keyword, value_indices, tind, band=None, mean_and_std=True, band_mean=True):
        values_dict = {}
        frequencies_old = None
        for value in values:
            epochs_data, frequencies, _ = self._load_TFR(event, condition, value, keyword)
            epochs_data = np.swapaxes(epochs_data, 2, 3)
            if frequencies_old is not None:
                if frequencies != frequencies_old:
                    raise ValueError('TFRs must be compared for the same frequencies')
            if band is not None:
                band_name, fmin, fmax = band
                band_indices = [index for index in range(len(frequencies)) if frequencies[index] >= fmin if frequencies[index] <= fmax]
                epochs_std = np.sqrt(epochs_data[:, :, :, band_indices].mean(axis=3) ** 2 + epochs_data[:, :, :, band_indices].std(axis=3) ** 2)
                epochs_data = epochs_data[:, :, :, band_indices]
                if band_mean:
                    epochs_data = epochs_data.mean(axis=3)
            if mean_and_std:
                if band is None:
                    epochs_std = np.sqrt(epochs_data.mean(axis=0) ** 2 + epochs_data.std(axis=0) ** 2)
                else:
                    epochs_std = np.sqrt(epochs_std.mean(axis=0) ** 2 + epochs_std.std(axis=0) ** 2)
        ...
        if band is not None:
            frequencies = [f for f in frequencies if f > band[1] if f < band[2]]
        return (
    ...
    def _plotter_main(self, event, condition, values, keyword=None, aux=False, butterfly=False, contrast=False, tfr=False, band=None, tfr_keyword=None, tmin=None, tmax=None, vmin=None, vmax=None, show=True):
        ...
        if tfr:
            tind = np.array([i for i, t in enumerate(times) if t >= tmin if t <= tmax])
            values_dict, frequencies = self._get_tfr_data(event, condition, values, tfr_keyword, value_indices,
              tind, band=band)
        else:
        ...
        if contrast:
            ...
            self._plot_decider(epochs_mean, epochs_std, times, axs, fig, butterfly, contrast, values, ch_dict, tfr, band, frequencies, vmin, vmax)
        else:
            for i, value in enumerate(values):
                epochs_mean, epochs_std = values_dict[value]
                if butterfly:
                    axs[i].set_title(value)
                    self._plot_decider(epochs_mean, epochs_std, times, axs[i], fig, butterfly, contrast, values, ch_dict, tfr, band, frequencies, vmin, vmax)
                else:
                    self._plot_decider(epochs_mean, epochs_std, times, axs, fig, butterfly, contrast, values, ch_dict, tfr, band, frequencies, vmin, vmax)
        ...
        self._prepare_fig(fig, event, condition, values, aux=aux, butterfly=butterfly, contrast=contrast,
          tfr=tfr,
          band=band,
          ar=ar,
          keyword=keyword,
    ...
    def _plot_decider(self, epochs_mean, epochs_std, times, axs, fig, butterfly, contrast, values, ch_dict, tfr, band, frequencies, vmin, vmax, clusters=None, cluster_p_values=None):
        vmin, vmax = self._default_vs(epochs_mean[ch_dict.keys()], epochs_std[ch_dict.keys()], vmin, vmax)
        if tfr:
            if band is not None:
                self._plot_band(epochs_mean, epochs_std, times, axs, ch_dict, butterfly,
                  vmin, vmax, clusters=clusters, cluster_p_values=cluster_p_values)
            else:
                self._plot_heatmap(epochs_mean, epochs_std, times, axs, fig, butterfly,
                  ch_dict, frequencies, vmin, vmax, clusters=clusters,
                  cluster_p_values=cluster_p_values)
        else:
            self._plot_voltage(epochs_mean, epochs_std, times, axs, butterfly, ch_dict,
              vmin, vmax, clusters=clusters, cluster_p_values=cluster_p_values)

    def _plot_voltage(self, epochs_mean, epochs_std, times, axs, butterfly, ch_dict, vmin, vmax, clusters=None, cluster_p_values=None):
        epochs_mean *= 1000000.0
        epochs_std *= 1000000.0
        vmin *= 1000000.0
        vmax *= 1000000.0
        for i, ch in enumerate(ch_dict):
            ...
            if not butterfly:
                ax.fill_between(times, (v - epochs_std[ch]), (v + epochs_std[ch]), color=(lines[0].get_color()),
                  alpha=0.5)
            if clusters and cluster_p_values:
                for i_c, c in enumerate(clusters):
                    c = c[0]
                    if cluster_p_values[i_c] <= 0.05:
                        h = ax.axvspan((time[c.start]), (time[(c.stop - 1)]), color='r', alpha=0.3)
                    else:
                        ax.axvspan((time[c.start]), (time[(c.stop - 1)]), color=(0.3,
                                                                                 0.3,
                                                                                 0.3),
                          alpha=0.3)

    def _plot_heatmap(self, epochs_mean, epochs_std, times, axs, fig, butterfly, ch_dict, frequencies, vmin, vmax, clusters=None, cluster_p_values=None):
        ...
        for i, ch in enumerate(ch_dict):
            ...
            if clusters:
                ...
                im = ax.imshow(current_data, aspect=aspect, extent=extent, cmap=cmap,
                  norm=norm)
    ...
    def _plot_band(self, epochs_mean, epochs_std, times, axs, ch_dict, butterfly, vmin, vmax, clusters=None, cluster_p_values=None):
        for i, ch in enumerate(ch_dict):
            if butterfly:
                ax = axs
            else:
            ...
            if not butterfly:
                ax.fill_between(times, (epochs_mean[ch] - epochs_std[ch]), (epochs_mean[ch] + epochs_std[ch]),
            ...
            if clusters and cluster_p_values:
                for i_c, c in enumerate(clusters):
                    ...
                    if cluster_p_values[i_c] <= 0.05:
                        ...
                        ax.axvspan((timse[c.start]), (times[(c.stop - 1)]), color=(0.3,
                                                                                   0.3,
                                                                                   0.3),
                          alpha=0.3)

    def _prepare_fig(self, fig, event, condition, values, aux=False, butterfly=False, contrast=False, tfr=False, band=None, keyword=None, show=True):
        if tfr:
            if band:
        ...
        title = event + ' ' + condition + ' ' + ' '.join([str(value) for value in values]) + ' contrast' * contrast
        if tfr:
            if band:
                bandname, _, _ = band
                title += ' ' + bandname + ' band'
            ...
            bandname = ''
        fig.suptitle(title)
        fig.savefig((self._fname)('plots', 'plot', 'jpg', contrast * 'contrast', 'tfr' * tfr, 'aux' * aux, 'butterfly' * butterfly, (bandname + '_band') * (band is not None), keyword, event, condition, *values))
        self._show_fig(fig, show)

    def makeWavelets(self, event, condition, values=None, keyword_in=None, keyword_out=None, fmin=3, fmax=35, nmin=3, nmax=10, steps=32, compressed=False, normalize=True, overwrite=False):
        ...
        value_indices = self._get_indices(epochs, condition, values)
        values_dict = self._get_data(epochs, values, value_indices, (tmin - self.tbuffer), (tmax + self.tbuffer),
          mean_and_std=False)
        if normalize:
            ...
            bl_value_indices = self._get_indices(bl_epochs, condition, values)
            bl_values_dict = self._get_data(bl_epochs, values, bl_value_indices, (bl_tmin - self.tbuffer),
              (bl_tmax + self.tbuffer),
              mean_and_std=False)
        for value in values:
            if overwrite or not (os.path.isfile(self._fname('TFR', 'tfr', 'npz', 'Baseline', condition, value)) and os.path.isfile(self._fname('TFR', 'tfr', 'npz', event, condition, value))):
                if normalize:
                    bl_tfr = tfr_array_morlet((bl_values_dict[value]), sfreq=(bl_epochs.info['sfreq']),
                      freqs=frequencies,
                      n_cycles=n_cycles,
                      output='power')
                    bl_tind = bl_epochs.time_as_index(bl_times)
                    bl_tfr = bl_tfr[:, :, :, bl_tind]
                    self._save_TFR(bl_tfr, frequencies, n_cycles, 'Baseline', condition, value,
                      keyword_out, compressed=compressed)
                    bl_power = bl_tfr.mean(axis=0).mean(axis=(-1))
                    bl_power = bl_power[np.newaxis, :, :, np.newaxis]
                current_data = values_dict[value]
                current_data -= current_data.mean(axis=0)
                tfr = tfr_array_morlet(current_data, sfreq=(epochs.info['sfreq']), freqs=frequencies,
                  n_cycles=n_cycles,
                  output='power')
                ...
                if normalize:
                    tile_shape = (
                     current_data.shape[0], 1, 1, len(times))
                    bl_power = np.tile(bl_power, tile_shape)
                    tfr /= bl_power
                self._save_TFR(tfr, frequencies, n_cycles, event, condition, value, keyword_out,
                  compressed=compressed)
                del tfr
                del bl_tfr
            else:
                print('TFR already calculated for %s,' % value + ' use keyword overwrite=True to recalculate')

    def psdMultitaper(self, keyword=None, ch='Oz', N=6, deltaN=0.25, NW=3.0, fmin=0.5, fmax=25, BW=1.0, assign_states=True, labels={'Wake':'red', 
 'Sleep':'white'}, overwrite=False, n_jobs=10, vmin=None, vmax=None, adaptive=False, jackknife=True, low_bias=True):
        raw = self._load_raw(keyword=keyword)
        ...
        if image is None:
            ...
            with Parallel(n_jobs=n_jobs) as (parallel):
                results = parallel(delayed(tsa.multi_taper_psd)((raw_data[int(round(i * deltaN * Fs)):int(round((i * deltaN + N) * Fs))]), Fs=Fs, NW=NW, BW=BW, adaptive=adaptive, jackknife=jackknife, low_bias=low_bias) for i in tqdm(range(n_windows)))
            ...
            f_inds = [i for i, freq in enumerate(f) if freq >= fmin if freq <= fmax]
            image = image[f_inds]
            ...
            if assign_states:
                ...
                for label in labels:
                    ...
                    buttons.append(ButtonClickProcessor(label_ax, label, labels[label], ax1, drs, image))
        ...
        fig2.savefig(self._fname('plots', 'psd_multitaper', 'jpg', 'N_%i_dN_%.2f' % (N, deltaN), 'fmin_%.2f_fmax_%.2f_NW_%i' % (fmin, fmax, NW)))
        plt.close(fig2)
        if assign_states:
            plt.show(fig)
    ...
    def assignConditionFromStateTimes(self, event, condition, state_times, no_state='Neither'):
        event_times = self.getEventTimes(event)
        states = np.tile(no_state, len(event_times))
        for i, t in enumerate(event_times):
            for state in state_times:
                if any([t <= tmax and t >= tmin for tmin, tmax in state_times[state]]):
                    states[i] = state

        self.behavior[condition] = states
        self._save_behavior()

    def CPTByBand(self, event, condition, values=None, keyword=None, tfr_keyword=None, tmin=None, tmax=None, threshold=6.0, aux=False, bands={'theta':(4, 8), 
 'alpha':(8, 15),  'beta':(15, 30)}, contrast=True):
        for band in bands:
            fmin, fmax = bands[band]
            band_struct = (band, fmin, fmax)
            self.CPT(event, condition, values, keyword=keyword, tfr_keyword=tfr_keyword,
              tmin=tmin,
              tmax=tmax,
              tfr=True,
              threshold=threshold,
    ...
    def sourceCPT(self, event, condition, values=None, keyword=None, tmin=None, tmax=None, threshold=4.0, contrast=False, aux=False, n_permutations=1000, n_jobs=10):
        alpha = 0.3
        values = self._default_values(values, condition, contrast)
        tmin, tmax = self._default_t(event, tmin, tmax)
        if contrast:
            ...
            clusters, cluster_p_values = self._CPT(source_data0, source_data1, threshold, ch_dict, n_permutations=n_permutations,
              n_jobs=n_jobs)
            self._save_CPT(event, condition, ('%s-%s' % (values[0], values[1])), clusters,
              cluster_p_values, times, frequencies=frequencies,
              band=band)
        else:
            for value in values:
                ...
                bl_source_data = self._equalize_baseline_length(source_data, bl_source_data)
                clusters, cluster_p_values = self._CPT(source_data, bl_source_data, threshold, n_permutations=n_permutations,
                  n_jobs=n_jobs)
                self._save_CPT(event, condition, value, clusters, cluster_p_values,
                  times, frequencies=frequencies,
                  band=band)

    def CPT(self, event, condition, values=None, keyword=None, tmin=None, tmax=None, threshold=4.0, tfr=False, band=None, tfr_keyword=None, contrast=False, n_permutations=1000, n_jobs=10):
        ...
        if not contrast:
            ...
            if tfr:
                ...
                if not contrast:
                    bl_values_dict, bl_frequencies = self._get_tfr_data('Baseline', condition, values, tfr_keyword, bl_value_indices,
                      t_ind, band=band)
            else:
                values_dict = self._get_data(epochs, values, value_indices, tmin, tmax)
                if not contrast:
                    bl_values_dict = self._get_data(bl_epochs, values, bl_value_indices, tmin, tmax)
                frequencies = None
            if contrast:
                epochs_data0 = values_dict[values[0]]
                epochs_data1 = values_dict[values[1]]
                clusters, cluster_p_values = self._CPT(epochs_data0, epochs_data1, threshold, n_permutations=n_permutations,
                  n_jobs=n_jobs)
                self._save_CPT(event, condition, ('%s-%s' % (values[0], values[1])), clusters,
                  cluster_p_values, times, frequencies=frequencies,
                  band=band)
                ...
                for value in values:
                    ...
                    clusters, cluster_p_values = self._CPT(evo_data, bl_evo_data, threshold, n_permutations=n_permutations,
                      n_jobs=n_jobs)
                    self._save_CPT(event, condition, value, clusters, cluster_p_values,
                      times, frequencies=frequencies,
                      band=band)
    ...
    def _equalize_baseline_length(value_data, bl_data):
        ...
        if bl_len < val_len:
            ...
            print('Using %.2f ' % (n_reps + float(remainder) / bl_len) + 'repetitions of the baseline period for permuation')
            baseline = np.tile(bl_data, n_reps)
            remainder_baseline = np.take(bl_data, (range(bl_len - remainder, bl_len)),
              axis=(-1))
    ...
    def _CPT(self, data0, data1, threshold, n_permutations=1000, n_jobs=10):
        T_obs, clusters, cluster_p_values, H0 = permutation_cluster_test([data0, data1], n_permutations=n_permutations, threshold=threshold,
          tail=0,
          n_jobs=n_jobs,
          buffer_size=None,
          verbose=False)
    ...
    def plotControlVariables(self, conditions=None):
        ...
        for param in conditions:
            ...
            if any([isinstance(self.behavior[param][index], str) for index in range(self.n)]):
                ...
                t = [i for i, v in enumerate(self.behavior[param]) if not np.isnan(v)]
        ...
        for i in range(len(conditions)):
            for j in range(i + 1, len(conditions)):
                fig, ax = plt.subplots()
                iscat1 = any([isinstance(self.behavior[params[i]][index], str) for index in range(self.n)]) or len(np.unique(self.behavior[params[i]])) > 5
                iscat2 = any([isinstance(self.behavior[params[j]][index], str) for index in range(self.n)]) or len(np.unique(self.behavior[params[j]])) > 5
                if iscat1:
                    if iscat2:
                        sns.countplot(x=(self.behavior[params[i]]), hue=(self.behavior[params[j]]),
                          ax=ax)
                if iscat1 or iscat2:
                    sns.stripplot(x=(self.behavior[params[i]]), y=(self.behavior[params[j]]),
                      jitter=True)
                    sns.violinplot(x=(self.behavior[params[i]]), y=(self.behavior[params[j]]))
                else:
                    indices = [index for index in range(self.n) if not (np.isnan(self.behavior[params[i]][index]) or np.isnan(self.behavior[params[j]][index]))]
                    column1 = [self.behavior[params[i]][index] for index in indices]
                    column2 = [self.behavior[params[j]][index] for index in indices]
                    heatmat = np.histogram2d(column1, column2, bins=9)
                    centers1 = heatmat[1][:-1] + float(heatmat[1][1] - heatmat[1][0]) / 2
                    centers1 = [round(c, 2) for c in centers1]
                    centers2 = heatmat[2][:-1] + float(heatmat[2][1] - heatmat[2][0]) / 2
                    centers2 = [round(c, 2) for c in centers2]
                    heatdf = DataFrame(columns=centers1, index=centers2, data=(heatmat[0]))
    ...
    def markAutoReject(self, event, keyword_in=None, keyword_out=None, bad_ar_threshold=0.5, n_jobs=10, n_interpolates=[
 1, 2, 3, 5, 7, 10, 20], random_state=89, consensus_percs=np.linspace(0, 1.0, 11), overwrite=False):
        if self._has_epochs(event, keyword_out):
            if not overwrite:
                print("Autoreject already calculated, use 'overwrite=True' to recalculate.")
                return
        epochs = self._load_epochs(event, keyword=keyword)
        picks = pick_types((epochs.info), meg=(self.meg), eeg=(self.eeg), stim=False, eog=False,
    ...
    def plotAutoReject(self, event, keyword_in=None, keyword_out=None, ylim=dict(eeg=(-30, 30)), show=True):
        ...
        if self.eeg:
            losses = {'eeg': ar.loss_['eeg'].mean(axis=(-1))}
        else:
        ...
        for modality in losses:
            ...
            fig, ax = plt.subplots()
            im = ax.matshow((loss.T * 1000000.0), cmap=(plt.get_cmap('viridis')))
            ...
            rect = patches.Rectangle((idx - 0.5, jdx - 0.5), 1, 1, linewidth=2, edgecolor='r',
              facecolor='none')
            ax.add_patch(rect)
            ax.xaxis.set_ticks_position('bottom')
            ax.set_xlabel('Consensus percentage $\\kappa$')
            ax.set_ylabel('Max sensors interpolated $\\rho$')
            ax.set_title('Mean cross validation error (x 1e6) %s' % modality)
            fig.colorbar(im)
            fig.savefig(self._fname('plots', 'ar', 'jpg', event, modality, 'consensus_v_interpolates'))
            self._show_fig(fig, show)

        fig = epochs_comparison.average().plot(ylim=ylim, spatial_colors=True, window_title='Before Autoreject',
          show=False)
        fig.savefig(self._fname('plots', 'ar', 'jpg', event, 'ar_before'))
        self._show_fig(fig, show)
        epochs_ar.average().plot(ylim=ylim, spatial_colors=True, window_title='After Autoreject',
          show=False)
        fig.savefig(self._fname('plots', 'ar', 'jpg', event, 'ar_after'))
        self._show_fig(fig, show)

    def plotRejectLog(self, event, keyword_in=None, scalings=dict(eeg=4e-05)):
        ar, reject_log = self._load_autoreject(event)
        epochs_comparison = self._load_epochs(event, keyword=keyword_in)
        reject_log.plot_epochs(epochs_comparison, scalings=scalings, title='Dropped and interpolated Epochs')

    def epochs2source(self, event, condition, values=None, snr=1.0, keyword_in=None, keyword_out=None, method='dSPM', pick_ori='normal', shared_baseline=False, overwrite=False):
        if not all([self.fs_dir, self.bemf, self.srcf, self.transf]):
            raise ValueError('Source estimation parameters not defined, either add to meta data or redefine MEEGbuddy')
        else:
            keyword_out = keyword_in if keyword_out is None else keyword_out
            values = self._default_values(values, condition)
            epochs = self._load_epochs(event, keyword=keyword_in)
            bl_epochs = self._load_epochs('Baseline', keyword=keyword_in)
            value_indices = self._get_indices(epochs, condition, values)
            if all([self._has_source(event, condition, value, keyword_out) for value in values]):
                if not overwrite:
                    print("Sources already computed, use 'overwrite=True' to recalculate")
                    return
            bem, source, coord_trans, lambda2, epochs, bl_epochs, fwd = self._source_setup(event, snr, epochs, bl_epochs)
            print('Making inverse for all...')
            inv = self._generate_inverse(epochs, fwd, bl_epochs, lambda2, method, pick_ori)
            self._save_inverse(inv, lambda2, method, pick_ori, event, condition, 'all', keyword_out)
            print('Applying inverse on baseline for all...')
            bl_evoked = bl_epochs.average()
            bl_stc = apply_inverse(bl_evoked, inv, lambda2=lambda2, method=method, pick_ori=pick_ori)
            self._save_source(bl_stc, 'Baseline', condition, 'all', keyword_out)
            bl_value_indices = shared_baseline or self._get_indices(bl_epochs, condition, values)
        for value in values:
            if not shared_baseline:
                if value not in bl_value_indices:
                    pass
                else:
                    bl_indices = bl_value_indices[value]
                    print('Making inverse for %s...' % value)
                    inv = self._generate_inverse(epochs, fwd, bl_epochs[bl_indices], lambda2, method, pick_ori)
                    self._save_inverse(inv, lambda2, method, pick_ori, event, condition, value, keyword_out)
                    ...
                    bl_stc = apply_inverse(bl_evoked, inv, lambda2=lambda2, method=method,
                      pick_ori=pick_ori)
                    self._save_source(bl_stc, 'Baseline', condition, value, keyword_out)
            print('Applying inverse for %s' % value)
            indices = value_indices[value]
            current_epochs = epochs[indices]
            evoked = current_epochs.average()
            stc = apply_inverse(evoked, inv, lambda2=lambda2, method=method, pick_ori=pick_ori)
    ...
    def _source_setup(self, event, snr, epochs, bl_epochs):
        ...
        epochs = epochs.crop(tmin=tmin, tmax=tmax)
        if self.eeg:
            epochs = epochs.set_eeg_reference(ref_channels='average', projection=True)
        ...
        if self.eeg:
            bl_epochs = bl_epochs.set_eeg_reference(ref_channels='average', projection=True)
        print('Making forward model...')
        fwd = make_forward_solution((epochs.info), coord_trans, source, bem, meg=(self.meg),
          eeg=(self.eeg),
          mindist=1.0)
    ...
    def fsaverageMorph(self, event, condition, values=None, keyword=None):
        ...
        for value in values:
            ...
            self._save_source(stc_fs, event, condition, value, fs_av=True, keyword=keyword)
    ...
    def sourceContrast(self, event, condition, values=None, keyword=None):
        ...
        if stc0:
            if stc1:
                stc_con = stc0 - stc1
                self._save_source(stc_con, event, condition, ('%s %s Contrast' % (values[0], values[1])),
                  keyword=keyword)

    def plotSourceSpace(self, event, condition, values=None, tmin=None, tmax=None, fs_av=False, keyword=None, downsample=False, seed=11, hemi='both', size=(800, 800), time_dilation=25, clim='auto', use_saved_stc=False, gif_combine=True, views=[
 'lat', 'med', 'cau', 'dor', 'ven', 'fro', 'par'], show=True):
        ...
        for value in values:
            if use_saved_stc:
                if not os.path.isfile(self._fname('sources', 'source-lh', 'stc', event, condition, value, 'fs_av' * fs_av)):
                    raise ValueError('The data must be converted to source space first.')
                stc = self._load_source(event, condition, value, fs_av, keyword=keyword)
                stc.crop(tmin=tmin, tmax=tmax)
            else:
                indices = value_indices[value]
                if downsample:
                    print('Subsampling %i/%i ' % (nTR, len(indices)) + 'for %s %s.' % (condition, value))
                    np.random.shuffle(indices)
                    indices = sorted(indices[:nTR])
                inv, lambda2, method, pick_ori = self._load_inverse(event, condition, value, keyword=keyword)
                stc_Evoked = epochs[indices].average()
                stc = apply_inverse(stc_Evoked, inv, lambda2=lambda2, method=method,
                  pick_ori=pick_ori,
                  verbose=False)
            gif_names = []
            for view in views:
                gif_name = self._fname('plots', 'source_plot', 'gif', event, condition, value, keyword, hemi, view)
            ...
            if gif_combine:
                ...
                anim = combine_gifs((self._fname)('plots', 'source_plot', 'gif', event, condition, value, keyword, hemi, *views), 60, *gif_names)
                if show:
                    plt.show()

    def interpolateArtifact(self, event, use_raw=True, keyword=None, mode='spline', npoint_art=1, offset=0, points=10, k=3):
        if mode not in ('spline', 'linear', 'window'):
            ...
            if mode in ('linear', 'window'):
                ...
                interp = fix_stim_artifact((inst.copy()), events=events, tmin=tmin, tmax=tmax,
                  mode=mode,
                  stim_channel=stim_ch)
            inst_data = inst.copy().get_data()
            if mode == 'spline':
                ch_ind = pick_types((inst.info), meg=(self.meg), eeg=(self.eeg), eog=True,
                  ecg=True,
                  stim=False)
            else:
                ch_ind = pick_types((inst.info), meg=(self.meg), eeg=(self.eeg), eog=True,
            ...
            if isinstance(inst, BaseEpochs):
                ...
                for i in range(inst_data.shape[0]):
                    ...
                    interp_data[i] = self._interpolate(epoch_data, ch_ind, [
                     event_ind], npoint_art, offset, points, k)

                interp_spline = EpochsArray(interp_data, (inst.info), events=(inst.events), tmin=(inst.tmin),
                  verbose=False)
            else:
                interp_data = self._interpolate(inst_data, ch_ind, events[:, 0], npoint_art, offset, points, k)
    ...
    def plotInterpolateArtifact(self, event, use_raw=True, keyword=None, mode='linear', npoint_art=1, offset=0, points=10, k=3, show=True, ylim=[
 -0.0005, 0.0005], tmin=-0.03, tmax=0.03, baseline=(-1.1, -0.1)):
        if mode not in ('spline', 'linear', 'window'):
            raise ValueError('Mode must be spline/linear/window')
        else:
            interp2, inst, events = self.interpolateArtifact(event, use_raw=use_raw, keyword=keyword, mode=mode,
              npoint_art=npoint_art,
              offset=offset,
              points=points,
              k=k)
            if use_raw:
                epochs = Epochs(inst, events, preload=True, verbose=False, detrend=1, tmin=tmin,
                  tmax=tmax,
                  baseline=None)
                interp = Epochs(interp2, events, preload=True, verbose=False, detrend=1, tmin=tmin,
                  tmax=tmax,
                  baseline=None)
            else:
                epochs = inst.copy()
        ...
        for ax, ch_ind in zip([ax1, ax2], [this_ind, aux_ind]):
            for ch, interp_ch in zip(evoked_data[ch_ind], interp_data[ch_ind]):
                ...
                ax.plot((times[interp_ind]), (interp_ch[interp_ind] - interp_ch_mean),
                  color='g')
        ...
        fig.savefig(self._fname('plots', 'interp', 'jpg', event, keyword, 'raw' * use_raw, 'points_%i' % points, 'offset_%i' % offset, 'npoint_art_%i' % npoint_art))
        self._show_fig(fig, show)
        return (interp2, inst)
    ...
    def filterEpochs(self, event, keyword_in=None, keyword_out=None, h_freq=None, l_freq=None):
        epochs = self._load_epochs(event, keyword=keyword_in)
        epochs = epochs.copy().filter(h_freq=h_freq, l_freq=l_freq)
        self._save_epochs(epochs, event, keyword=keyword_out)

    def filterRaw(self, keyword_in=None, keyword_out=None, l_freq=None, h_freq=None, maxwell=False, overwrite=False):
        keyword_out = keyword_out if keyword_out is not None else keyword_in
        fname = self._fname('raw', 'raw', 'fif', keyword_out)
        if self._has_raw(keyword=keyword_out):
            if not overwrite:
                raise ValueError('Raw file already exists ' + 'for %s ' % keyword_out * (keyword_out is not None), "use 'overwrite=True'")
        else:
            raw = self._load_raw(keyword=keyword_in)
            if maxwell:
    ...
    def sourceBootstrap(self, event, keyword_in=None, keyword_out=None, snr=1.0, method='dSPM', pick_ori='normal', tfr=True, itc=True, fmin=7, fmax=35, nmin=3, nmax=10, steps=7, bands={'alpha':(7, 15), 
 'beta':(15, 35)}, Nboot=1000, Nave=50, seed=13, n_jobs=10, use_fft=True, mode='same', batch=10, overwrite=False):
        ...
        band_inds = {band:[i for i, f in enumerate(freqs) if f >= bands[band][0] if f <= bands[band][1]] for band in bands if [i for i, f in enumerate(freqs) if f >= bands[band][0] if f <= bands[band][1]]}
        n_cycles = np.logspace(np.log10(nmin), np.log10(nmax), steps)
        keyword_out = keyword_in if keyword_out is None else keyword_out
        ...
        if os.path.isfile(fname):
            ...
            for j, i in enumerate(epochs.events[:, 2]):
                if i not in bl_epochs.events[:, 2]:
            ...
            stcs = np.memmap(('sb_%s_%s_workfile' % (event, keyword_out)), dtype='float64',
              mode='w+',
              shape=(
             batch, fwd['nsource'], len(epochs.times)))
            if tfr:
                Ws = mne.time_frequency.morlet((epochs.info['sfreq']), freqs,
                  n_cycles=n_cycles, zero_mean=False)
                powers = {band:np.memmap(('sb_tfr_%s_%s_workfile' % (event, keyword_out)), dtype='float64', mode='w+', shape=(batch, fwd['nsource'], len(epochs.times))) for band in bands}
                if itc:
                    itcs = {band:np.memmap(('sb_tfr_%s_%s_workfile' % (event, keyword_out)), dtype='float64', mode='w+', shape=(batch, fwd['nsource'], len(epochs.times))) for band in bands}
                else:
                    itcs = None
            else:
                powers = itcs = Ws = None
        ...
        for i_min, i_max in zip(mins, maxs):
            ...
            fname2 = self._fname('source_estimates', 'bootstrap', 'npz', '%i-%i' % (i_min, i_max), event, keyword_out)
            if os.path.isfile(fname2):
                if not overwrite:
                    continue
                for i, k in enumerate(tqdm(range(i_min, i_max))):
                    indices = bootstrap_indices[k]
                    bl_indices = [np.where(bl_events == j)[0][0] for j in events[indices]]
                    inv = self._generate_inverse(epochs, fwd, bl_epochs[bl_indices], lambda2, method, pick_ori)
                    evoked = epochs[indices].average()
                    stc = apply_inverse(evoked, inv, lambda2=lambda2, method=method, pick_ori=pick_ori)
                    stcs[i] = stc.data[:]
                    if tfr:
                        this_tfr = mne.time_frequency.tfr.cwt((stc.data.copy()), Ws, use_fft=use_fft,
                          mode=mode)
                ...
                if tfr:
                    for band in bands:
                        fname3 = self._fname('source_estimates', 'bootstrap_power_%s' % band, 'npz', '%i-%i' % (i_min, i_max), event, keyword_out)
                        np.savez_compressed(fname3, powers=(powers[band]))
                        if itc:
                            fname4 = self._fname('source_estimates', 'bootstrap_itc_%s' % band, 'npz', '%i-%i' % (i_min, i_max), event, keyword_out)
                            np.savez_compressed(fname4, itcs=(itcs[band]))

        inv = self._generate_inverse(epochs, fwd, bl_epochs, lambda2, method, pick_ori)
        evoked = epochs.average()
        stc = apply_inverse(evoked, inv, lambda2=lambda2, method=method, pick_ori=pick_ori)
        np.savez_compressed(fname, events=events, batch=batch, tfr=tfr, itc=itc, freqs=freqs,
          n_cycles=n_cycles,
          bands=bands,
          bootstrap_indices=bootstrap_indices)
        self._save_source(stc, event, 'Bootstrap', 'base', keyword=keyword_out)

    def sourceCorrelation(self, event, condition, keyword_in=None, keyword_out=None, baseline=(-0.5, -0.1), n_permutations=1000, overwrite=False):
        ...
        keyword_out = keyword_in if keyword_out is None else keyword_out
        fname_in = self._fname('source_estimates', 'bootstrap', 'npz', event, keyword_in)
        fname_out = self._fname('source_estimates', 'correlation', 'npz', event, keyword_in)
        if not os.path.isfile(fname_in):
            raise ValueError('Bootstraps must be computed first(check that keywords match)')
        ...
        for i in range(Nboot):
            bootstrap_conditions.append(np.nanmean(np.array([self.behavior[condition][j] for j in bootstrap_indices[i]])))
        ...
        bl_indices = np.where((stc.times >= baseline[0]) & (stc.times < baseline[1]))[0]
        ...
        if tfr:
            powers = {band:np.memmap(('sb_power_%s_%s_%s_workfile' % (event, keyword_out, band)), dtype='float64', mode='w+', shape=(Nboot, nSRC, nTIMES)) for band in bands}
            power_result = {band:stc_copy.copy() for band in bands}
            power_bl_dist = {band:np.zeros((stc.data.shape[0], n_permutations)) for band in bands}
            if itc:
                itcs = {band:np.memmap(('sb_itc_%s_%s_%s_workfile' % (event, keyword_out, band)), dtype='float64', mode='w+', shape=(Nboot, nSRC, nTIMES)) for band in bands}
                itc_result = {band:stc_copy.copy() for band in bands}
                itc_bl_dist = {band:np.zeros((stc.data.shape[0], n_permutations)) for band in bands}
        mins = range(0, Nboot - batch + 1, batch)
        maxs = range(batch, Nboot + 1, batch)
        for i_min, i_max in zip(mins, maxs):
            ...
            if tfr:
                for band in bands:
                    ...
                    if itc:
                        ...
                        fname4 = self._fname('sources', 'bootstrap_itc_%s' % band, 'npz', '%i-%i' % (i_min, i_max), event, keyword_out)
        ...
        for s_ind in tqdm(range(nSRC)):
            ...
            for p_ind in range(n_permutations):
                ...
                _, _, r, _, _ = linregress(dist[permutation_indices[p_ind]], bootstrap_conditions)
                bl_dist[(s_ind, p_ind)] = r
                if tfr:
                    for band in bands:
                        power_dist = power_s_data[band][:, bl_indices].mean(axis=1)
                        _, _, r, _, _ = linregress(power_dist[permutation_indices[p_ind]], bootstrap_conditions)
                        power_bl_dist[band][(s_ind, p_ind)] = r
                        if itc:
                            itc_dist = itc_s_data[band][:, bl_indices].mean(axis=1)
                            _, _, r, _, _ = linregress(itc_dist[permutation_indices[p_ind]], bootstrap_conditions)
        ...
        for s_ind in tqdm(range(nSRC)):
            ...
            for t_ind in range(nTIMES):
                ...
                stc_result.data[(s_ind, t_ind)] = 1.0 / p * np.sign(r) if p > 0 else 1.0 / n_permutations * np.sign(r)
                if tfr:
                    for band in bands:
                        ...
                        power_result[band].data[(s_ind, t_ind)] = 1.0 / p * np.sign(r) if p > 0 else 1.0 / n_permutations * np.sign(r)
                        if itc:
                            ...
                            itc_result[band].data[(s_ind, t_ind)] = 1.0 / p * np.sign(r) if p > 0 else 1.0 / n_permutations * np.sign(r)

        self._save_source(stc_result, event, condition, 'correlation', keyword=keyword_out)
        if tfr:
            for band in bands:
                self._save_source((power_result[band]), event, condition, ('power_correlation_%s' % band),
                  keyword=keyword_out)
                if itc:
                    self._save_source((itc_result[band]), event, condition, ('itc_correlation_%s' % band),
                      keyword=keyword_out)

    def noreunPhi(self, event, condition, values=None, keyword_in=None, keyword_out=None, tmin=None, tmax=None, npoint_art=0, Nboot=480, alpha=0.01, downsample=True, seed=11, shared_baseline=False, fs_av=False, bl_tmin=-0.5, bl_tmax=-0.1, recalculate_baseline=False, recalculate_PCI=False):
        ...
        values = self._default_values(values, condition)
        value_indices = self._get_indices(epochs, condition, values)
        bl_tind = np.intersect1d(np.where(bl_tmin <= epochs.times), np.where(epochs.times <= bl_tmax))
        nTR = min([len(value_indices[value]) for value in value_indices])

        def preprocess(epochs, indices, bl_tind, event, condition, value, nTR, keyword_in):
            Y = epochs[indices].get_data()
            inv, lambda2, method, pick_ori = self._load_inverse(event, condition, value, keyword=keyword_in)
            nSRC = inv['nsource']
            nTIME = len(epochs.times)
            J = apply_inverse((epochs[indices].average()), inv, method=method,
              lambda2=lambda2,
              pick_ori=pick_ori,
              verbose=False).data
        ...
        def baseline_bootstrap(Y, J, bl_tind, Norm, NUM, DEN, Nboot, alpha, info, inv, lambda2, method, pick_ori):
            ...
            for per in tqdm(range(Nboot)):
                ...
                for j in range(nTR):
                    randonsampT = np.random.choice(bl_tind, N0, replace=True)
                    YT[j] = Y[randontrialsT[j]][:, randonsampT]

                YTE = EpochsArray(YT, info, verbose=False)
                ET = apply_inverse((YTE.average()), inv, method=method, lambda2=lambda2,
                  pick_ori=pick_ori,
                  verbose=False).data
                ET = (ET - NUM) / DEN
                Bootstraps[per, :] = np.max((np.abs(ET)), axis=0)

            Bootstraps = np.sort(np.reshape(Bootstraps, Nboot * N0))
            calpha = 1 - alpha
            calpha_index = int(np.floor(calpha * Nboot * N0))
            TT = Norm * Bootstraps[calpha_index]
            Threshold = np.kron(np.ones((1, nTIME)), TT.reshape((nSRC, 1)))
            return Threshold

        def gettind(epochs, tmin, tmax, npoint_art):
            tind = np.intersect1d(np.where(tmin <= epochs.times), np.where(epochs.times <= tmax))
        ...
        for value in ['all'] if shared_baseline else values:
            if os.path.isfile(self._fname('analyses', 'phi_threshold', 'npz', keyword_out, event, condition, value)):
                if not recalculate_baseline:
                ...
                Y, J, inv, lambda2, method, pick_ori, NUM, DEN, Norm = preprocess(epochs, indices, bl_tind, event, condition, value, nTR, ar, keyword_in)
                Threshold = baseline_bootstrap(Y, J, bl_tind, Norm, NUM, DEN, Nboot, alpha, info, inv, lambda2, method, pick_ori)
                self._save_noreun_baseline(Y, J, Threshold, epochs[indices].events[:, 2], bl_tmin, bl_tmax, Nboot, alpha, event, condition, value, keyword_out)

        for value in values:
            if os.path.isfile(self._fname('analyses', 'pci', 'npz', keyword_out, event, condition, value)):
                if not recalculate_PCI:
                    print('PCI already computed')
            else:
                Y, J, Threshold, events, bl_tmin, bl_tmax, Nboot, alpha = self._load_noreun_baseline(event, condition, 'all' if shared_baseline else value, keyword_out)
                ...
                binJ = binJrank[:, tind]
                ct = computePCI(binJ)
                self._save_noreun_PCI(ct, binJ, tmin, tmax, npoint_art, event, condition, value, keyword_out)

    def _load_noreun_baseline(self, event, condition, value, keyword=None):
        fname = self._fname('noreun_phi', 'Threshold', 'npz', keyword, event, condition, value)
        if os.path.isfile(fname):
            f = np.load(fname)
            try:
                return (
                 f['Y'], f['J'], f['Threshold'], f['events'], f['bl_tmin'].item(),
                 f['bl_tmax'].item(), f['Nboot'].item(), f['alpha'].item())
            except:
                return (
                 f['Y'], f['J'], f['Threshold'], f['bl_tmin'].item(),
                 f['bl_tmax'].item(), f['Nboot'].item(), f['alpha'].item())
    ...
    def _save_noreun_baseline(self, Y, J, Threshold, events, bl_tmin, bl_tmax, Nboot, alpha, event, condition, value, keyword=None):
        fname = self._fname('analyses', 'phi_threshold', 'npz', keyword, event, condition, value)
        np.savez_compressed(fname, Y=Y, J=J, Threshold=Threshold, bl_tmin=bl_tmin, bl_tmax=bl_tmax,
          events=events,
          Nboot=Nboot,
          alpha=alpha)

    def _load_noreun_PCI(self, event, condition, value, keyword=None):
        fname = self._fname('noreun_phi', 'pci', 'npz', keyword, event, condition, value)
        if os.path.isfile(fname):
            ...
            raise ValueError('%s %s %s %s %s PCI not calculated' % (
             ...
             keyword if keyword is not None else ''))
        return (
         f['ct'], f['binJ'], f['tmin'].item(), f['tmax'].item(), f['npoint_art'].item())

    def _save_noreun_PCI(self, ct, binJ, tmin, tmax, npoint_art, event, condition, value, keyword=None):
        print('Saving noreun PCI for %s %s %s' % (event, condition, value))
        fname = self._fname('analyses', 'pci', 'npz', keyword, event, condition, value)
        np.savez_compressed(fname, ct=ct, binJ=binJ, tmin=tmin, tmax=tmax, npoint_art=npoint_art)

    def plotNoreunPCI(self, event, condition, values=None, keyword=None, ssm=True, pci=True, downsampled=True, shared_baseline=False, fontsize=24, wspace=0.4, linewidth=4, show=True):
        values = self._default_values(values, condition)
        if len(values) > 1:
        ...
        yMAX = 0
        for i, value in enumerate(values):
            ct, binJ, tmin, tmax, npoint_art = self._load_noreun_PCI(event, condition, value, ar, keyword)
            Y, J, Threshold, events, bl_tmin, bl_tmax, Nboot, alpha = self._load_noreun_baseline(event, condition, 'all' if shared_baseline else value, keyword)
            ...
            if ssm:
                ...
                ax.imshow(binJ, extent=[0, nTIME, 0, nSRC], aspect='auto',
                  cmap='Greys')
            ...
            if pci:
                ...
                ax.plot((range(ct.shape[0])), ct, 'y-', linewidth=linewidth,
                  alpha=1,
                  zorder=1)
                ax.plot((range(ct.shape[0])), ct, 'b-', linewidth=(linewidth / 2),
                  alpha=1,
                  zorder=2)
                ax.set_ylabel('PCI', fontsize=fontsize)
            title = '%s' % value + ', PCI=%2.2g' % ct[(-1)] * pci + ', %i Trials' % Y.shape[0]
        ...
        if pci:
            [ax.set_ylim(ymax=(yMAX * 1.05)) for ax in axs]
        title = '%s %s' % (event, condition) + ' Significant Sources' * ssm + ' and' * (pci and ssm) + ' PCI' * pci + ' %s' % keyword * (keyword is not None)
        fig.suptitle(title, fontsize=fontsize)
        fig.savefig((self._fname)('plots', 'noreun_phi_plot', 'jpg', keyword, event, condition, *values))
        self._show_fig(fig, show)
    ...
    def waveletConnectivity(self, event, condition, values=None, keyword_in=None, keyword_out=None, downsample=True, seed=13, threshold=0.001, min_dist=0.05, tmin=None, tmax=None, fmin=None, fmax=None, method='pli', bandwidth=1, cwt_freqs='default', cwt_n_cyles='default', tube_radius=0.001, fig_width=600, fig_height=600, n_jobs=5, stc=True, fps=60, bands={'theta':(4, 8), 
 'alpha':(8, 15),  'beta':(15, 30)}):
        if downsample:
            np.random.seed(seed)
        else:
            keyword_out = keyword_in if keyword_out is None else keyword_out
        ...
        for dt in ['grad', 'mag'] * self.meg + ['eeg'] * self.eeg:
            this_epochs = epochs.copy().pick_types(meg=(False if dt == 'eeg' else dt), eeg=(dt == 'eeg'))
            for band_name in bands:
                fmin, fmax = bands[band_name]
                freqs = np.arange(fmin, fmax, bandwidth) if cwt_freqs == 'default' else cwt_freqs
                n_cycles = np.arange(fmin, fmax, bandwidth) if cwt_n_cycles == 'default' else cwt_n_cycles
                for value in values:
                    indices = value_indices[value]
                    if downsample:
                        ...
                        np.random.shuffle(indices)
                        indices = indices[:nTR]
                    con, freqs, times, n_epochs, n_tapers = spectral_connectivity((this_epochs[indices]), method=method, mode='cwt_morlet',
                      sfreq=sfreq,
                      fmin=fmin,
                      fmax=fmax,
                      faverage=True,
                      ...
                      n_jobs=n_jobs)
                    con = con.squeeze()
                    fig = mlab.figure(size=(fig_width, fig_height), bgcolor=(0.5, 0.5,
                                                                             0.5))
                    view = (-88.7, 40.8, 0.76, np.array([-0.00039, -0.0085, -0.01]))
                    (mlab.view)(*view)
                    mlab.scalarbar(title='Phase Lag Index (PLI)', nb_labels=4)
                    sens_loc = np.array([ch['loc'][:3] for ch in this_epochs.info['chs']])
                    pts = mlab.points3d((sens_loc[:, 0]), (sens_loc[:, 1]), (sens_loc[:, 2]), color=(1,
                                                                                                     1,
                                                                                                     1),
                      opacity=1,
                      scale_factor=0.005)
                    ...
                    for node0, node1 in con_nodes[0]:
                        ...
                        points = mlab.plot3d([x1, x2], [y1, y2], [z1, z2], [val, val], vmin=(con_threshold * 0.01),
                          vmax=(con_max * 0.01),
                    ...
                    anim = animation.FuncAnimation(fig, animate, frames=(len(epochs.times)),
                      interval=10,
                      blit=True)
                    anim.save((self._fname('plots', 'con_plot', 'gif', event, condition, value)),
                      fps=fps,
                      writer='imagemagick',
                      savefig_kwargs={'facecolor': 'black'})


def create_demi_events(raw_fname, window_size, shift, epoches_nun=0, fname_out=None, overwrite=False):
    if fname_out is None:
        if not overwrite:
            raise ValueError("No out file name specified, use 'overwrite=True'to overwrite.")
    ...
    for win_ind in range(epoches_nun):
        demi_events[win_ind] = [
         win_ind * windows_shift, win_ind * windows_shift + windows_length, 0]
...
def loadMEEGbuddies(subjects_dir, eeg=None, meg=None, task=None):
    files = []
    for mbf in os.listdir(os.path.join(subjects_dir, 'meta_data')):
        test = True
        for kw, use in {'eeg':eeg,  'meg':meg}.items():
            if use is not None and (use and kw not in mbf or not use and kw in mbf):
                test = False
...
class Comparator:
    __doc__ = ' Compares MEEG data generated by M/EEGbuddy for subject level comparisons.'
