...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/MEEGbuddy/bv2fif.py
...
def bv2fif(dataf, corf, new_sfreq=1000.0, use_event_ch=False, event_ch='STI TMS', ch_order=None, threshold=0.2, ch_name='Fz', padding=5, min_event_dist=1.8, max_event_dist=2.5, eogs=('VEOG', 'HEOG'), ecg='ECG', preload='default'):
    if preload == 'default':
        ...
        if new_sfreq < raw.info['sfreq']:
            if use_event_ch:
                events = find_events(raw, stim_channel=event_ch, output='onset', min_duration=(1 / raw.info['sfreq']))
                events = events[:, 0]
            else:
                ch = abs(raw._data[raw.ch_names.index(ch_name)])
                sfreq = raw.info['sfreq']
                events = list()
                for i in tqdm(range(100 + int(padding * sfreq), len(ch) - 100, int(min_event_dist * sfreq))):
                ...
                events = [events[i] for i in range(len(events)) if i == 0 or abs(events[i] - events[(i - 1)]) > sfreq * min_event_dist and abs(events[i] - events[(i - 1)]) < sfreq * max_event_dist]
    ...
    fname = os.getcwd() + '/' + os.path.splitext(os.path.basename(dataf))[0] + '-raw.fif'
