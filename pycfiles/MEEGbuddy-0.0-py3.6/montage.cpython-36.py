...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/MEEGbuddy/montage.py
# Compiled at: 2019-02-12 17:24:47
# Size of source mod 2**32: 38156 bytes
from collections import Iterable
import os, os.path as op, numpy as np, xml.etree.ElementTree as ElementTree
from ..viz import plot_montage
from .channels import _contains_ch_type
from ..transforms import apply_trans, get_ras_to_neuromag_trans, _sph_to_cart, _topo_to_sph, _str_to_frame, _frame_to_str
from ..io.meas_info import _make_dig_points, _read_dig_points, _read_dig_fif, write_dig
from ..io.pick import pick_types
from ..io.open import fiff_open
from ..io.constants import FIFF
from ..utils import _check_fname, warn, copy_function_doc_to_method_doc, _clean_names
from ..externals.six import string_types
from ..externals.six.moves import map
from .layout import _pol_to_cart, _cart_to_sph

class Montage(object):
    __doc__ = "Montage for standard EEG electrode locations.\n\n    .. warning:: Montages should typically be loaded from a file using\n                 :func:`mne.channels.read_montage` instead of\n                 instantiating this class directly.\n\n    Parameters\n    ----------\n    pos : array, shape (n_channels, 3)\n        The positions of the channels in 3d.\n    ch_names : list\n        The channel names.\n    kind : str\n        The type of montage (e.g. 'standard_1005').\n    selection : array of int\n        The indices of the selected channels in the montage file.\n\n    See Also\n    --------\n    DigMontage\n    read_montage\n    read_dig_montage\n\n    Notes\n    -----\n    .. versionadded:: 0.9.0\n    "

    def __init__(self, pos, ch_names, kind, selection, nasion=None, lpa=None, rpa=None):
        self.pos = pos
    ...
    def plot(self, scale_factor=20, show_names=True, kind='topomap', show=True):
        return plot_montage(self, scale_factor=scale_factor, show_names=show_names,
          kind=kind,
          show=show)
...
def get_builtin_montages():
    ...
    supported = ('.elc', '.txt', '.csd', '.sfp', '.elp', '.hpts', '.loc', '.locs',
                 '.eloc', '.bvef')
...
def read_montage(kind, ch_names=None, path=None, unit='m', transform=False):
    ...
    if path is None:
        ...
        if not op.isabs(kind):
            ...
            montages = [m for m in montages if m[1] in supported if kind == m[0]]
            if len(montages) != 1:
                    ...
                    if ext == '.csd':
                        ...
                        if ext == '.elp':
                            ...
                            if ext == '.hpts':
                                ...
                                if ext in ('.loc', '.locs', '.eloc'):
                                    ch_names_ = np.loadtxt(fname, dtype='S4', usecols=[
                                     3]).astype(str).tolist()
...
class DigMontage(object):
    ...
    def __init__(self, hsp=None, hpi=None, elp=None, point_names=None, nasion=None, lpa=None, rpa=None, dev_head_t=None, dig_ch_pos=None, coord_frame='unknown'):
        ...
        if not isinstance(coord_frame, string_types) or coord_frame not in _str_to_frame:
    ...
    def __repr__(self):
        """Return string representation."""
        s = '<DigMontage | %d extras (headshape), %d HPIs, %d fiducials, %d channels>' % (
        ...
        if self.coord_frame == 'head':
            ...
            if any(x is None for x in (nasion, rpa, lpa)):
                if self.elp is None or self.point_names is None:
                    raise ValueError('ELP points and names must be specified for transformation.')
                names = [name.lower() for name in self.point_names]
                kinds = ('nasion', 'lpa', 'rpa')
                missing = [name for name in kinds if name not in names]
                if len(missing) > 0:
                    raise ValueError('The points %s are missing, but are needed to transform the points to the MNE coordinate system. Either add the points, or read the montage with transform=False.' % str(missing))
                ...
                self.point_names = [p for pi, p in enumerate(self.point_names) if mask[pi]]
            native_head_t = get_ras_to_neuromag_trans(nasion, lpa, rpa)
            self.nasion, self.lpa, self.rpa = apply_trans(native_head_t, np.array([nasion, lpa, rpa]))
    ...
    def compute_dev_head_t(self):
        ...
        if self.elp is None or self.hpi is None:
            raise RuntimeError('must have both elp and hpi to compute the device to head transform')
        self.dev_head_t = fit_matched_points(tgt_pts=(self.elp), src_pts=(self.hpi),
          out='trans')

    def _get_dig(self):
        """Get the digitization list."""
        return _make_dig_points(nasion=(self.nasion),
          lpa=(self.lpa),
          rpa=(self.rpa),
          hpi=(self.elp),
          extra_points=(self.hsp),
    ...
    def save(self, fname):
        ...
        """
        if self.coord_frame != 'head':
            raise RuntimeError('Can only write out digitization points in head coordinates.')
        write_dig(fname, self._get_dig())


_cardinal_ident_mapping = {FIFF.FIFFV_POINT_NASION: 'nasion', 
    ...
    if isinstance(montage, Montage):
        if update_ch_names:
            ...
            for ii, ch_name in enumerate(montage.ch_names):
                ch_info = {'cal':1.0, 
                 'logno':ii + 1,  'scanno':ii + 1,  'range':1.0, 
                 'unit_mul':0,  'ch_name':ch_name,  'unit':FIFF.FIFF_UNIT_V, 
                 'kind':FIFF.FIFFV_EEG_CH,  'coord_frame':FIFF.FIFFV_COORD_HEAD, 
            ...
            if not _contains_ch_type(info, 'eeg'):
                ...
                if len(set(montage_lower)) == len(montage_lower):
                    if len(set(info_lower)) == len(info_lower):
                ...
                montage_ch_names = _clean_names(montage_ch_names, remove_whitespace=True)
                dig = dict()
                for pos, ch_name in zip(montage.pos, montage_ch_names):
                    if ch_name not in info_ch_names:
                        continue
                    ch_idx = info_ch_names.index(ch_name)
                    info['chs'][ch_idx]['loc'] = np.r_[(pos, [0.0] * 9)]
                    sensors_found.append(ch_idx)
                    dig[ch_name] = pos

                if set_dig:
                    info['dig'] = _make_dig_points(nasion=(montage.nasion),
                      lpa=(montage.lpa),
                      rpa=(montage.rpa),
                      dig_ch_pos=dig)
            if len(sensors_found) == 0:
                raise ValueError('None of the sensors defined in the montage were found in the info structure. Check the channel names.')
        eeg_sensors = pick_types(info, meg=False, ref_meg=False, eeg=True, exclude=[])
        not_found = np.setdiff1d(eeg_sensors, sensors_found)
        if len(not_found) > 0:
            not_found_names = [info['ch_names'][ch] for ch in not_found]
            warn('The following EEG sensors did not have a position specified in the selected montage: ' + str(not_found_names) + '. Their position has been left untouched.')
    else:
        if isinstance(montage, DigMontage):
            ...
            if montage.dig_ch_pos is not None:
                ...
                for ch_name, ch_pos in montage.dig_ch_pos.items():
                    if ch_name == 'EEG000':
                        ...
                        if ch_name not in info['ch_names']:
                            raise RuntimeError('Montage channel %s not found in info' % ch_name)
                        idx = info['ch_names'].index(ch_name)
                        did_set[idx] = True
                        this_loc = np.concatenate((ch_pos, eeg_ref_pos))
                        info['chs'][idx]['loc'][:6] = this_loc

                did_not_set = [info['chs'][ii]['ch_name'] for ii in np.where(is_eeg & ~did_set)[0]]
                if len(did_not_set) > 0:
                    warn('Did not set %s channel positions:\n%s' % (
                     len(did_not_set), ', '.join(did_not_set)))
        else:
            raise TypeError("Montage must be a 'Montage' or 'DigMontage' instead of '%s'." % type(montage))