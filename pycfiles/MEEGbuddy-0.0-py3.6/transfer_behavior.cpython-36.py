...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/MEEGbuddy/transfer_behavior.py
...
def transfer_behavior(task):
    ...
    subjectfiles = [f for f in subjectfiles if 'params' not in f]
    rex_subject = re.compile('[a-z]{2}\\d{3}')
    rex_block = re.compile('block_\\d{1}')
    rex_date = re.compile('20[0-9]{2}_\\d{2}_\\d{2}')
                ...
                if subject in dfs:
                    if modality in dfs[subject]:
                        if date in dfs[subject][modality]:
                            dfs[subject][modality][date] = dfs[subject][modality][date].append(d).sort_values('Trial')
                        else:
