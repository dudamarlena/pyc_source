...
# [GCC 8.4.0]
# Embedded file name: /home/jmcfarlane/dev/personal/Notable/scripts/../notable/bottle.py
# Compiled at: 2013-03-08 02:41:32
"""
...
class Router(object):
    ...
    def __init__(self, strict=False):
        ...
        self.filters = {'re': self.re_filter,  'int': self.int_filter,  'float': self.float_filter, 
         'path': self.path_filter}

    def re_filter(self, conf):
        return (
         conf or self.default_pattern, None, None)
...
class Route(object):
    ...
    def all_plugins(self):
        ...
        for p in reversed(self.app.plugins + self.plugins):
            ...
            if name:
                if name in self.skiplist or name in unique:
                    continue
                if not p in self.skiplist:
                    if type(p) in self.skiplist:
                        continue
                    if name:
                        unique.add(name)
                    yield p
...
class TemplatePlugin(object):
    ...
    def apply(self, callback, route):
        ...
        if isinstance(conf, (tuple, list)) and len(conf) == 2:
            ...
            if isinstance(conf, str) and 'template_opts' in route.config:
...
def path_shift(script_name, path_info, shift=1):
    ...
    if shift > 0 and shift <= len(pathlist):
        ...
        if shift < 0 and shift >= -len(scriptlist):
...
class SimpleTemplate(BaseTemplate):
    ...
    def execute(self, _stdout, *args, **kwargs):
        ...
        env.update({'_stdout': _stdout,  '_printlist': _stdout.extend,  '_include': self.subtemplate, 
         ...
         'get': env.get,  'setdefault': env.setdefault, 
