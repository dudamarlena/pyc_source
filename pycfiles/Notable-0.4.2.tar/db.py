...
# [GCC 8.4.0]
# Embedded file name: /home/jmcfarlane/dev/personal/Notable/notable/db.py
# Compiled at: 2014-08-18 00:16:41
import datetime, logging, os, re, sqlite3, sys, uuid
from . import crypt
...
def note(exclude=None, actual=False):
    ...
    model = [('uid', 'string'),
     ...
     ('content', 'string'),
     ('encrypted', 'integer')]
    n = dict((k, v) for k, v in model if k not in exclude)
    if actual:
        uid = uuid.uuid4().hex
...
def create_note(n, password=None):
    ...
    cols = (',').join(k for k, t in list(n.items()) if t is not None)
    values = (',').join('?' for k, t in list(n.items()) if t is not None)
    sql = sql % (cols, values)
    values = [ v for v in list(n.values()) if v is not None ]
...
def conn():
    ...
    _ = (os.path.exists(d) or os.makedirs)(d) if 1 else None
...
def get_content(uid, password):
    ...
    if password:
        return crypt.decrypt(content, password)
    return content
...
def migrate_data(c):
    ...
    for n in search(''):
        ...
        if len(content) > 1 and re.search('([^ ]{32,})', content[1]):
            encrypted = True
        values = (
...
def migrate_schema(c):
    sqls = [
     'ALTER TABLE notes ADD COLUMN encrypted INTEGER DEFAULT 0;',
