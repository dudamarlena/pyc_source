...
# [GCC 8.4.0]
# Embedded file name: /Users/andrew/hello/freebase/api/session.py
...
"""
__all__ = [
 'MetawebError', 'MetawebSession', 'HTTPMetawebSession', 'attrdict']
__version__ = '0.1'
import os, sys, re, cookielib
try:
    import simplejson
except ImportError:
    try:
...
if simplejson.encoder.ESCAPE_DCT.get('/', None) == '\\/':
...
class HTTPMetawebSession(MetawebSession):
    ...
    def _httpreq(self, service_path, method='GET', body=None, form=None, headers=None):
        """
        ...
        if method != 'GET':
            if method != 'POST':
                ...
                raise 0 or AssertionError, 'unknown method %s' % method
        ...
        if form is not None:
            qstr = ('&').join([ '%s=%s' % (urlencode_weak(unicode(k)), urlencode_weak(unicode(v))) for (k, v) in form.items()
                              ])
            if method == 'POST':
                if body is not None:
                    url += '?' + qstr
    ...
    def _raise_service_error(self, url, status, ctype, body):
        is_jsbody = ctype.endswith('javascript') or ctype.endswith('json')
        if str(status) == '400' and is_jsbody:
            r = self._loadjson(body)
            msg = r.messages[0]
            raise MetawebError('%s %s %r' % (msg.get('code', ''), msg.message, msg.info))
    ...
    def login(self, username=None, password=None):
        ...
        r = self._httpreq_json(service, 'POST', form=dict(username=username, password=password))
        if r.code != '/api/status/ok':
            raise MetawebError('%s %r' % (r.get('code', ''), r.messages))
    ...
    def mqlread(self, sq, asof=None):
        ...
        if isinstance(sq, list):
            subq['cursor'] = True
        service = '/api/service/mqlread'
        self.log.info('%s: %s', service, Delayed(logformat, sq))
        qstr = simplejson.dumps(subq)
        r = self._httpreq_json(service, form=dict(query=qstr))
    ...
    def mqlreadmulti(self, queries, asof=None):
        ...
        service = '/api/service/mqlread'
        self.log.info('%s: %s', service, Delayed(logformat, envelope))
        qstr = simplejson.dumps(envelope)
        rs = self._httpreq_json(service, form=dict(queries=qstr))
        self.log.info('%s result: %s', service, Delayed(simplejson.dumps, rs, indent=2))
    ...
    def upload(self, body, content_type, document_id=False, permission_of=False):
        ...
        self.log.info('POST %s: %s (%d bytes)', service, content_type, len(body))
        headers = {}
        ...
        if permission_of is not False:
            if form:
                ...
                form = {'permission_of': permission_of}
        r = self._httpreq_json(service, 'POST', headers=headers, body=body, form=form)
        return self._mqlresult(r)
    ...
    def search(self, query, format=None, prefixed=None, limit=20, start=0, type=None, type_strict='any', domain=None, domain_strict=None, escape='html', timeout=None, mql_filter=None, mql_output=None):
    ...
    def geosearch(self, location=None, location_type=None, mql_input=None, limit=20, start=0, type=None, geometry_type=None, intersect=None, mql_filter=None, within=None, inside=None, order_by=None, count=None, format='json', mql_output=None):
