...
# Size of source mod 2**32: 9110 bytes
...
class Header:
    ...
    def raw(self) -> bytearray:
        return bytearray(struct.pack(PATTERN, self.indicator, self.version, self.flags.type, self.flags.encrypted, self.flags.cluster, self.flags.size, self.command.ljust(32, b'\x00'), self.checksum))
...
class Packet:
    ...
    data: bytearray
    encrypted_checksum = field(default=None)
    encrypted_checksum: bytes
    ...
    def as_dict(self) -> dict:
        return {'header':self.header.as_dict(), 
         'data':self.data, 
...
class Protocol:
        ...
        if not key:
            ...
            if not key.can_encrypt():
                ...
                if encrypt:
                    ...
                    chunk = 65535 if (key.size_in_bytes() if key else self.key.size_in_bytes()) - 66 > 65535 else (key.size_in_bytes() if key else self.key.size_in_bytes()) - 66
                    encrypted = bytearray()
                    for i in (data[i:i + chunk] for i in range(0, data.__len__(), chunk)):
                        encrypted.extend(cipher.encrypt(i))

            return Packet(Header(Flags(type_, encrypt, key.size_in_bytes() if key else self.key.size_in_bytes() if encrypt else 0, len(data)), command, b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'), encrypted if encrypt else data)
    ...
    def unpack_header(raw: Union[(bytes, bytearray)]) -> Header:
        if isinstance(raw, (bytes, bytearray)):
            ...
            try:
                flags = Flags(*struct.unpack(PATTERN, raw[:76])[2:6])
            except struct.error:
    ...
    def unpack(self, raw: Union[(bytes, bytearray)], key: RSA.RsaKey=None) -> Packet:
        if isinstance(raw, (bytes, bytearray)):
            if isinstance(raw, bytes):
                ...
                header = self.unpack_header(raw[:HEADER_SIZE])
            ...
            return Packet(header, decrypted if header.flags.encrypted else raw[HEADER_SIZE:], bytes(raw[44:76]) if header.flags.encrypted else None)
        raise TypeError('Raw must be bytes or bytearray')