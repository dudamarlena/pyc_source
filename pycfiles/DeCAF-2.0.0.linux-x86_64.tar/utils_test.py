...
# [GCC 8.4.0]
# Embedded file name: /home/martas/miniconda/envs/test/lib/python2.7/site-packages/tests/utils_test.py
# Compiled at: 2016-07-15 04:56:39
"""
...
class UtilsTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.phars = [ Pharmacophore.read(fname) for fname in sorted(glob('tests/*.phar'))
                     ]
    ...
    def testCompareNodes(self):
        ...
        for n1 in range(self.phars[0].numnodes):
            for n2 in range(self.phars[0].numnodes):
                s, t = compare_nodes(self.phars[0].nodes[n1], self.phars[0].nodes[n2])
                if n1 == n2:
                    self.assertEqual(s, max_sim)
                else:
                    min_length = max(len(self.phars[0].nodes[n1]['type']), len(self.phars[0].nodes[n2]['type']))
                    self.assertGreaterEqual(len(t), min_length)
                    if len(t) == len(self.phars[0].nodes[n1]['type']) == len(self.phars[0].nodes[n2]['type']):
                        self.assertEqual(s, max_sim)
        ...
        for d in [0.0, 1.0]:
            for i in range(len(self.phars)):
                for j in range(len(self.phars)):
                    s, c, m = map_pharmacophores(self.phars[i], self.phars[j], dist_tol=d, coarse_grained=False)
                    scores[i][j] = s
                    costs[i][j] = c
                    self.assertEqual(len(m[0]), len(m[1]))
                    best_mapped[i][j] = len(m[0])
                    s2, c2, m2 = map_pharmacophores(self.phars[i], self.phars[j], dist_tol=d, coarse_grained=False, add_neighbours=True)
            ...
            for i in range(len(self.phars)):
                ...
                self.assertEqual(scores[i][i], 2.0 * best_mapped[i][i])
                ...
                for j in range(i):
                    ...
                    self.assertLessEqual(scores[i][j], 2.0 * min(self.phars[i].numnodes, self.phars[j].numnodes))
    ...
    def testCombine(self):
        ...
        for i in range(len(self.phars)):
            for j in range(len(self.phars)):
                _, _, m = map_pharmacophores(self.phars[i], self.phars[j], coarse_grained=False)
    ...
    def testFilter(self):
        ...
        model2 = filter_nodes(model1, freq_range=(cutoff, 1.0))
        freq = model1.molecules
        for node in model2.nodes:
            self.assertGreaterEqual(node['freq'], freq * cutoff)

    def testValidation(self):
        from decaf.utils import compare_nodes, distances, dfs, filter_nodes, map_pharmacophores as mp, similarity, split_components, combine_pharmacophores as cp
        ...
        for f in [mp, similarity]:
            ...
            self.assertRaises(TypeError, f, p, p, coarse_grained=False, add_neighbours='1')
