...
# [GCC 8.4.0]
# Embedded file name: /home/martas/miniconda/envs/test/lib/python2.7/site-packages/decaf/toolkits/ob.py
# Compiled at: 2016-07-15 01:17:16
"""OpenBabel toolkit for DeCAF"""
from decaf import PHARS, Pharmacophore
import pybel, openbabel as ob, numpy as np
...
def phar_from_mol(ligand):
    ...
    if not isinstance(ligand, pybel.Molecule):
        raise TypeError('Invalid ligand! Expected pybel.Molecule object, got %s instead' % type(ligand).__name__)
    matches = {}
    ...
    for phar, atoms in matches.items():
        for atom in atoms:
            if atom in points:
                ...
                nodes.append({'label': atom, 'type': {phar: 1.0}, 'freq': 1.0})
                points[atom] = idx
                idx += 1
    ...
    for i in range(len(keys)):
        for j in range(i):
            dist = float(__count_bonds(ligand.atoms[(keys[i] - 1)].OBAtom, ligand.atoms[(keys[j] - 1)].OBAtom, [ keys[k] for k in range(len(keys)) if k not in [i, j]
                                                                                                               ]))
            if dist > -1:
...
def layout(p):
    ...
    if not isinstance(p, Pharmacophore):
        raise TypeError('Expected Pharmacophore object, got %s instead' % type(p).__name__)
