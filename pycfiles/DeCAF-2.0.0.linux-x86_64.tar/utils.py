...
# [GCC 8.4.0]
# Embedded file name: /home/martas/miniconda/envs/test/lib/python2.7/site-packages/decaf/utils.py
# Compiled at: 2016-07-15 02:22:01
"""
functions for DeCAF
...
def compare_nodes(n1, n2):
    ...
    if not isinstance(n1, dict):
        raise TypeError('Invalid n1! Expected dict, got %s instead' % type(n1).__name__)
    if not isinstance(n2, dict):
        raise TypeError('Invalid n2! Expected dict, got %s instead' % type(n2).__name__)
...
def get_rings(phar):
    ...
    if not isinstance(phar, Pharmacophore):
        raise TypeError('Expected Pharmacophore, got %s instead' % type(phar).__name__)
        ...
        for v in tmp:
            if v in np.where(p.edges[n] > 0.0)[0]:
                if v not in visited:
                    ...
                    to_check.remove(v)
                    spanning_tree[v] = n
                    cycles += dfs_backedge(p, v, to_check, visited, spanning_tree)
...
def distances(p):
    ...
    if not isinstance(p, Pharmacophore):
        raise TypeError('Expected Pharmacophore, got %s instead' % type(p).__name__)
...
def dfs(p, n, to_check=None, visited=None):
    ...
    if visited is None:
        ...
        if not isinstance(visited, list):
            raise TypeError('Expected list, got %s instead' % type(visited).__name__)
        else:
            for i in visited:
                if not isinstance(i, int):
                    raise TypeError('Invalid visited list! Node %s is not int!' % i)
                elif i < 0 or i >= p.numnodes:
                    raise ValueError('Invalid visited list! Node index out of range: %s' % i)
        ...
        if to_check is None:
            ...
            for i in to_check:
                if not isinstance(i, int):
                    ...
                    raise ValueError('Invalid to_check list! Node index out ofrange: %s' % i)

    if not isinstance(n, int):
        raise TypeError('Starting node is not int!')
    else:
        if n < 0 or n >= p.numnodes:
...
def split_components(p, nodes=None):
    ...
    if nodes is None:
        ...
        if not isinstance(nodes, list):
            raise TypeError('Expected nodes list, got %s instead' % type(nodes).__name__)
        else:
            for n in nodes:
                if not isinstance(n, int):
...
def __modular_product(p1, p2, dist1=None, dist2=None, dist_tol=0):
    ...
    for i in range(len(rings1)):
        for j in range(len(rings2)):
            ...
            if weighted_freq > 0.0:
                nodes.append({'n1': p1.numnodes + i, 'n2': p2.numnodes + j, 'members': [
                             members1[i], members2[j]]})
                scores.append(weighted_freq)
    ...
    for i in range(n):
        for j in range(i):
            if nodes[i]['n1'] == nodes[j]['n1'] or nodes[i]['n2'] == nodes[j]['n2']:
                continue
            if nodes[i]['n1'] >= p1.numnodes:
            ...
            if nodes[j]['n1'] >= p1.numnodes:
                ...
                idxj1 = [w]
                idxj2 = [s]
            if len(set(idxi1) & set(idxj1)) > 0 or len(set(idxi2) & set(idxj2)) > 0:
                continue
            is_connected = False
            d1 = float('inf')
            for p in idxi1:
    ...
    return (
     nodes, scores, edges, costs)


def __BronKerbosch(edges, P=None, X=None, R=None, degrees=None, neigh=None):
    u"""Bron-Kerbosch algorithm for finding all maximal cliques in a graph
    ...
    if len(P) == 0 and len(X) == 0:
        ...
        for v in P - neigh[pivot]:
            cliques += __BronKerbosch(edges, degrees=degrees, R=R | set([v]), P=P & neigh[v], X=X & neigh[v], neigh=neigh)
            P = P - set([v])
            X = X | set([v])
...
def __align_rings(p1, p2, n1, n2, idx1, idx2, mapping=None, dist1=None, dist2=None, dist_tol=0):
    ...
    for i in range(len(n1)):
        ...
        if len(idx1) > 0:
            ...
            compatible = np.where(np.all(np.abs(d1 - d2) <= dist_tol, axis=1))[0]
        else:
    ...
    return (
     score, cost, [idx1, idx2])


def __add_neighbours(p1, p2, n1, n2, idx1, idx2, mapping=None, dist1=None, dist2=None, dist_tol=0, pairs=None):
    ...
    def is_compatible(pair1, pair2):
        if pair1[0] == pair2[0] or pair1[1] == pair1[1]:
            return False
        if p1.edges[(pair1[0], pair2[0])] != 0 and p2.edges[(pair1[1], pair2[1])] != 0:
            dist_diff = math.fabs(p1.edges[(pair1[0], pair2[0])] - p2.edges[(pair1[1], pair2[1])])
        ...
        for neigh1 in neighbours1:
            for neigh2 in neighbours2:
                if mapping[(neigh1, neigh2)] > 0:
                    dist_diff = np.abs(dist1[(idx1, neigh1)] - dist2[(idx2, neigh2)])
                    connected = np.where(p1.edges[(idx1, neigh1)] + p2.edges[(idx2, neigh2)])
    ...
    assert len(connected[0]) > 0 and np.max(dist_diff[connected]) <= dist_tol, 'cost too high'
    cost = np.sum(dist_diff[connected]) / 2.0
    aln = [
     idx1[:], idx2[:]]
    for i, pair in enumerate(pairs):
        compatible = [ p for p in pairs[i + 1:] if is_compatible(pair, p) ]
        s, c, (aln1, aln2) = __add_neighbours(p1, p2, n1[:], n2[:], idx1 + [pair[0]], idx2 + [pair[1]], mapping, dist1, dist2, dist_tol, compatible)
...
def map_pharmacophores(p1, p2, dist_tol=0.0, coarse_grained=True, add_neighbours=False):
    ...
    if not isinstance(p1, Pharmacophore):
        raise TypeError('Expected Pharmacophore, got %s instead' % type(p1).__name__)
    if not isinstance(p2, Pharmacophore):
        raise TypeError('Expected Pharmacophore, got %s instead' % type(p2).__name__)
    ...
    if coarse_grained and add_neighbours:
        warnings.warn('Neighbours cannot be added to coarse-grained alignment.If you want to add neighbours, use coarse_grained=False')
    mapping = np.zeros((p1.numnodes, p2.numnodes))
    for i in range(p1.numnodes):
        for j in range(p2.numnodes):
            weighted_freq, _ = compare_nodes(p1.nodes[i], p2.nodes[j])
    ...
    cost = 0.0
    scorecost = float('-inf')
    nodes, scores, edges, costs = __modular_product(p1, p2, dist1, dist2, dist_tol)
    ...
    if not coarse_grained:
        ...
        for i in range(len(idx1)):
            s, c, (tmp1, tmp2) = __align_rings(p1, p2, n1[i], n2[i], idx1[i], idx2[i], mapping, dist1, dist2, dist_tol)
            if s - c > scorecost or s - c == scorecost and s > score:
                score = s
                cost = c
                scorecost = s - c
                ...
                aln2 = tmp2[:]
            if add_neighbours:
                remaining1 = [ node for node in range(p1.numnodes) if node not in aln1 ]
                remaining2 = [ node for node in range(p2.numnodes) if node not in aln2
                             ]
                score, cost, (aln1, aln2) = __add_neighbours(p1, p2, remaining1, remaining2, aln1[:], aln2[:], mapping, dist1, dist2, dist_tol)
...
def similarity(p1, p2, dist_tol=0.0, coarse_grained=True, add_neighbours=False):
    ...
    score, cost, _ = map_pharmacophores(p1, p2, dist_tol, coarse_grained, add_neighbours)
    a1 = 0.0
    a2 = 0.0
    for n in p1.nodes:
        a1 += n['freq']
...
def combine_pharmacophores(p1, p2, dist_tol=0.0, freq_cutoff=0.0, add_neighbours=False):
    ...
    if not isinstance(add_neighbours, bool):
        raise TypeError('add_neighbours must be bool!')
    _, _, mapped_nodes = map_pharmacophores(p1, p2, dist_tol, coarse_grained=False, add_neighbours=add_neighbours)
    dist1 = distances(p1)
    dist1[p1.edges > 0] = p1.edges[(p1.edges > 0)]
    dist2 = distances(p2)
    dist2[p2.edges > 0] = p2.edges[(p2.edges > 0)]
    ...
    for i in range(len(mapped_nodes[0])):
        ...
        nodes.append({'label': idx, 'type': types, 'freq': u['freq'] + v['freq']})
        for j in [0, 1]:
    ...
    new_p = Pharmacophore(nodes=nodes, edges=edges, molecules=molecules, title=title)
    warnings.simplefilter('always', UserWarning)
    if new_p.numnodes == 0:
        warnings.warn('Empty common part!')
    freq_cutoff = molecules * freq_cutoff
    to_add = [
     [ i for i in range(p1.numnodes) if i not in mapped_nodes[0] and p1.nodes[i]['freq'] >= freq_cutoff
     ],
     [ i for i in range(p2.numnodes) if i not in mapped_nodes[1] and p2.nodes[i]['freq'] >= freq_cutoff
     ]]
    ...
    if comp_nr > 1:
                ...
                for n1 in components[i]:
                    for n2 in components[j]:
                        if n1 in added[0] and n2 in added[0]:
                            ...
                            freq1 = p1.nodes[added[0][n1]]['freq'] + p1.nodes[added[0][n2]]['freq']
                        else:
                            d1 = 0
                            freq1 = 0
                        if n1 in added[1] and n2 in added[1]:
                            d2 = dist2[(added[1][n1], added[1][n2])]
                            freq2 = p2.nodes[added[1][n1]]['freq'] + p2.nodes[added[1][n2]]['freq']
                        else:
                            d2 = 0
                            freq2 = 0
                        if freq1 + freq2 == 0:
        ...
        sorted_connections = np.unravel_index(comp_dist.argsort(axis=None), comp_dist.shape)
...
def inclusive_similarity(p1, p2, dist_tol=0.0, coarse_grained=True, add_neighbours=False):
    ...
    return (
     score * (m1 / m) / a1, score * (m2 / m) / a2, cost)
...
def filter_nodes(p, freq_range=(0.0, 1.0), rm_outside=True):
    ...
    if not isinstance(freq_range, tuple):
        ...
        if not (len(freq_range) == 2 and (isinstance(freq_range[0], float) or isinstance(freq_range[0], int)) and (isinstance(freq_range[1], float) or isinstance(freq_range[1], int)) and freq_range[0] <= freq_range[1]):
        ...
        if rm_outside:
            for n in range(p.numnodes - 1, -1, -1):
                if new_p.nodes[n]['freq'] < freq_range[0] or new_p.nodes[n]['freq'] > freq_range[1]:
                    new_p.remove_node(n)
...
def spring_layout(p, c0=0.2, c1=1.0):
    ...
    if not isinstance(p, Pharmacophore):
        raise TypeError('Expected Pharmacophore, got %s instead' % type(p).__name__)
    if not ((isinstance(c0, float) or isinstance(c0, int)) and (isinstance(c1, float) or isinstance(c1, int))):
...
def draw(p, layout='rd'):
    ...
    if not isinstance(layout, str):
        raise TypeError('Invalid layout! Expected str, got %s instead.' % type(layout).__name__)
    if p.numnodes == 0:
        raise ValueError('Pharmacophore is empty!')
    if layout == 'rd':
        try:
        ...
        if layout == 'ob':
            ...
            raise ValueError("Wrong layout specified! Use 'rd', 'ob' or 'spring'instead.")
        ax_coeff = 1.0
        ...
        axis = (
         ...
         np.max(pos[:, 1]) + 1)
        plt.axis(axis)
        ax_coeff = 12.0 / max(axis[1] - axis[0], axis[3] - axis[2])
        for i in range(p.numnodes):
            for j in range(i):
                if p.edges[(i, j)] > 0:
            ...
            for t in p.nodes[i]['type']:
                ...
                ax.add_artist(w)
                ax.text(pos[i][0], pos[i][1], str(p.nodes[i]['label']), color='#000000', ha='center', va='center', size=fsize)
                theta1 = theta2
