...
# [GCC 8.4.0]
# Embedded file name: /home/martas/miniconda/envs/test/lib/python2.7/site-packages/decaf/toolkits/rd.py
# Compiled at: 2016-07-15 01:18:44
...
def phar_from_mol(ligand):
    ...
    if not isinstance(ligand, Chem.Mol):
        raise TypeError('Invalid ligand! Expected RDKit.Chem.Mol object, got %s instead' % type(ligand).__name__)
    matches = {}
    ...
    for i in range(len(keys)):
        for j in range(i):
            dist = float(__count_bonds(ligand, keys[i], keys[j], [ keys[k] for k in range(len(keys)) if k not in [i, j]
                                                                 ]))
            if dist > -1:
    ...
    if not ligand.HasProp('_Name'):
        ...
        return Pharmacophore(nodes, edges, molecules=1.0, title=ligand.GetProp('_Name'))
