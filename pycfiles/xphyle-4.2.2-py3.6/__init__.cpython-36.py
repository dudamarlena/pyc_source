...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/xphyle/__init__.py
# Compiled at: 2019-12-13 16:28:50
# Size of source mod 2**32: 47762 bytes
...
import sys, warnings
from typing import Callable, Container, Iterable, Iterator, Union, Sequence, List, Tuple, Dict, Any, Generic, TypeVar, Generator, IO, Optional, Type, cast
import pkg_resources
from xphyle.formats import FORMATS, THREADS, CompressionFormat
from xphyle.paths import STDIN, STDOUT, STDERR, EXECUTABLE_CACHE, check_readable_file, check_writable_file, safe_check_readable_file, deprecated_str_to_path, convert_std_placeholder
from xphyle.progress import ITERABLE_PROGRESS, PROCESS_PROGRESS
from xphyle.types import FileType, FileLikeInterface, FileLike, FileMode, ModeArg, ModeAccess, ModeCoding, CompressionArg, EventType, EventTypeArg, PathOrFile, FileLikeBase, AnyChar
...
class EventListener(Generic[E], metaclass=ABCMeta):
    __doc__ = 'Base class for listener events that can be registered on a\n    FileLikeWrapper.\n\n    Args:\n        kwargs: keyword arguments to pass through to ``execute``\n    '
...
class EventManager:
    __doc__ = 'Mixin type for classes that allow registering event listners.\n    '
...
class FileLikeWrapper(EventManager, FileLikeBase):
    ...
    def readline(self, size: int=-1) -> AnyChar:
        return self._fileobj.readline(size)

    def readlines(self, hint: int=-1) -> List[AnyChar]:
...
class FileWrapper(FileLikeWrapper):
    ...
    def __init__(self, source, mode='w', compression=False, name=None, close_fileobj=True, **kwargs):
        if isinstance(source, Path):
            ...
            if name is None:
                if hasattr(source_fileobj, 'name'):
                    name = str(getattr(source_fileobj, 'name'))
                self._path = Path(name) if name else None
            else:
                super().__init__(source_fileobj,
                  compression=compression, close_fileobj=close_fileobj)
                self._name = str(name)
                if mode is None:
...
class StdWrapper(FileLikeWrapper):
    __doc__ = 'Wrapper around stdin/stdout/stderr.\n\n    Args:\n        stream: The stream to wrap.\n        compression: Compression type.\n    '

    def __init__(self, stream, compression=False):
 ...
 self, args, stdin=stdin, stdout=stdout, stderr=stderr, **kwargs)
    ...
    def mode(self) -> str:
        if self.writable():
            ...
            if self.readable():
                if ('b' in mode) == ('b' in self.get_reader().mode):
                    mode += 'r'
            return mode
        if self.readable():
    ...
    def __iter__(self) -> Iterator[AnyChar]:
        ...
        if not self._iterator:
            self._iterator = iter(ITERABLE_PROGRESS.wrap((cast(Iterable[AnyChar], self.get_reader())),
              desc=(str(self))))
        return self._iterator
    ...
    def close1(self, timeout: float=None, raise_on_error: bool=False, record_output: bool=False, terminate: bool=False) -> Optional[int]:
...
DEFAULTS = dict(xopen_context_wrapper=False)
DEFAULTS: Dict[(str, Any)]

def configure(default_xopen_context_wrapper: Optional[bool]=None, progress: Optional[bool]=None, progress_wrapper: Optional[Callable[(..., Iterable)]]=None, system_progress: Optional[bool]=None, system_progress_wrapper: Optional[Union[(str, Sequence[str])]]=None, threads: Optional[Union[(int, bool)]]=None, executable_path: Optional[Union[(PurePath, Sequence[PurePath])]]=None) -> None:
    """Conifgure xphyle.
...
@contextmanager
def open_(target: OpenArg, mode: ModeArg=None, errors: bool=True, wrap_fileobj: bool=True, **kwargs) -> Generator[(FileLike, None, None)]:
    ...
    if target is None:
        ...
        is_fileobj = not (isinstance(target, str) or isinstance(target, PurePath) or target in (str, bytes))
        if not wrap_fileobj:
            if is_fileobj:
                yield target
            else:
                raise ValueError("'wrap_fileobj must be True if 'path' is not file-like")
        else:
...
def xopen(target: OpenArg, mode: ModeArg=None, compression: CompressionArg=None, use_system: bool=True, allow_subprocesses: bool=True, context_wrapper: bool=None, file_type: FileType=None, validate: bool=True, overwrite: bool=True, close_fileobj: bool=True, **kwargs) -> FileLike:
    """
            ...
            if is_path:
                ...
                if is_std:
                    ...
                    if is_buffer:
                        ...
                        file_type = is_str or FileType.FILELIKE
        elif target.startswith('|'):
            file_type = FileType.PROCESS
    else:
        if file_type == FileType.BUFFER:
            if is_str or is_path or isinstance(target, bytes):
                if not mode:
                    mode = FileMode(access='r', coding=('t' if is_str else 'b'))
                is_buffer = True
            else:
                if (is_str or is_path or is_buffer) == (file_type is FileType.FILELIKE) or is_std != (file_type is FileType.STDIO) or is_buffer != (file_type is FileType.BUFFER):
            ...
            if file_type in (FileType.URL, None):
                ...
                file_type = file_type or (FileType.URL if url_parts else FileType.LOCAL)
            elif not url_parts:
                raise ValueError(f"{target} is not a valid URL")
            elif not mode:
                mode = is_buffer or FileMode()
            else:
                if target == str:
                    mode = FileMode('wt')
                else:
            ...
            if isinstance(mode, str):
                if 'U' in mode:
                    if 'newline' in kwargs:
                        if kwargs['newline'] is not None:
                            raise ValueError("newline={} not compatible with universal newlines ('U') mode".format(kwargs['newline']))
                    mode = FileMode(mode)
                elif context_wrapper is None:
                    ...
                    if not allow_subprocesses:
                        ...
                        if mode.writable:
                            if compression is True:
                                raise ValueError('Can determine compression automatically when writing to process stdin')
                            else:
                            ...
                            outstream = 'stdout'
                    popen_args[outstream] = dict(mode=mode,
                ...
                if file_type is FileType.BUFFER:
                    if target == str:
                        ...
                        if target == bytes:
                            ...
                            if is_str or isinstance(target, bytes):
                                if not mode.readable:
                                    raise ValueError("'mode' must be readable when 'file_type' == BUFFER and 'target' is string or bytes.")
                                if is_str:
        ...
        if file_type is FileType.STDIO:
            ...
            if hasattr(fileobj, 'mode'):
                ...
                if hasattr(fileobj, 'readable'):
                    ...
                    fileobj_mode = FileMode(access=access,
                      coding=('t' if hasattr(fileobj, 'encoding') else 'b'))
                else:
                    raise ValueError('Cannot determine file mode')
            if not (mode.readable and fileobj_mode.readable or mode.writable and fileobj_mode.writable):
                raise ValueError('mode {} and file mode {} are not compatible'.format(mode, fileobj_mode))
            is_bin = fileobj_mode.binary
            if is_bin or compression:
                raise ValueError('Cannot compress to/decompress from a text-mode file/buffer')
            else:
                guess_format = False
            ...
            if mode.readable:
                ...
                raise ValueError('Could not guess compression format from {}'.format(target))
        else:
            if file_type is FileType.URL:
                if not mode.readable:
                ...
                if file_type is FileType.LOCAL:
                    if is_str:
                        target = Path(target)
                    if mode.readable:
            ...
            if validate:
                if guess != compression:
                    if not mode.readable:
                        if FORMATS.has_compatible_extension(compression, guess):
                            pass
                    else:
                        raise ValueError('Acutal compression format {} is not compatible with expected format {}'.format(guess, compression))
            if guess:
                compression = guess
            else:
                if compression is True:
 ...
 fileobj or target, mode, use_system=use_system, **kwargs)
                    ...
                    if not fileobj:
                        ...
                        if is_bin:
                            if is_std or file_type is FileType.FILELIKE:
                ...
                if is_std:
                    ...
                    if file_type == FileType.BUFFER:
                        fileobj = BufferWrapper(fileobj,
                          buffer, compression=compression, close_fileobj=close_fileobj)
                    else:
...
def guess_file_format(path: PurePath) -> str:
    ...
    if fmt is None:
        if safe_check_readable_file(path):
...
def get_compressor(name_or_path: Union[(str, PurePath)]) -> Optional[CompressionFormat]:
    ...
    if fmt is None:
        if isinstance(name_or_path, PurePath):
            if safe_check_readable_file(cast(PurePath, name_or_path)):
...
def popen(args: Union[(str, Iterable)], stdin: PopenStdParamsArg=None, stdout: PopenStdParamsArg=None, stderr: PopenStdParamsArg=None, shell: bool=False, **kwargs) -> Process:
    ...
    std_args = {}
    for name, arg, default_mode in zip(('stdin', 'stdout', 'stderr'), (stdin, stdout, stderr), ('rb',
                                                                                                'wb',
                                                                                                'wb')):
        if arg is None:
            kwargs[name] = None
