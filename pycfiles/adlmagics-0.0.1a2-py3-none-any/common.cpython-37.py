...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/utils/common.py
...
def calculate_correct_percentages(orig_labels, attack_labels, instances):
    ...
    return (
     orig_precent_correct, attack_precent_correct, difference)
...
def fuzz_matrix(matrix: np.ndarray):
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            m[i][j] += abs(np.random.normal(0, 1e-05))
...
def report(result, name=''):
    ...
    after_learner_percent_correct = (len(true_labels) - after_learner_incorrect) * 100 / len(true_labels)
    print('\n###################################################################')
