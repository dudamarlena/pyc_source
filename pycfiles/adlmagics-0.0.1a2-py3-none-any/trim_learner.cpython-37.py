...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/learners/trim_learner.py
# Compiled at: 2018-07-20 16:00:09
# Size of source mod 2**32: 7280 bytes
...
class TRIMLearner(Learner):
    ...
    def __init__(self, training_instances: List[Instance], n: int, lda=0.1, max_iter=50, verbose=False):
        """
    ...
    def train(self):
        ...
        if len(self.training_instances) < 2:
            ...
            if self.redo_problem_on_train:
                ...
                self.temp_tuple = (
                 w, b, tau, irl_selection_param, prob)
            else:
                w, b, tau, irl_selection_param, prob = self.temp_tuple
        irl_selection_param.value = self.irl_selection
        tau.value = self.tau
        prob.solve(solver=(cvx.ECOS), verbose=(self.verbose), parallel=True, warm_start=True,
          ignore_dcp=True)
    ...
    def predict(self, instances):
        ...
        labels = list(map(lambda x:         if x >= 0:
1 # Avoid dead code: -1, labels))
        return labels
