...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/effective_sql/__init__.py
# Compiled at: 2019-04-03 22:51:40
"""
    ...
    Version: 0.1.10
"""
import sys, gevent, pymysql, pymysql.cursors
...
CURSORS = {'Cursor': pymysql.cursors.Cursor, 
...
class MySQL:
    ...
    def __init__(self, kwargs=None):
        ...
        :param kwargs:
            ...
            potational:
                ...
                maxcached=3,   # 启动时，最大缓存连接数
                maxconnections=8,  # 最大允许连接数
        ...
        self.maxcached = kwargs.get('maxcached', 2)
        self.maxconnections = kwargs.get('maxconnections', 2)
        self.maxshared = kwargs.get('maxshared', 0)
        self.maxusage = kwargs.get('maxusage', 0)
        ...
        self._pool_init()
        logging.basicConfig(format='%(asctime)s [line:%(lineno)3d] %(levelname)7s: %(message)s', level=getattr(logging, self.loglevel))
        self.logger = logging
    ...
    def _pool_init(self):
        self.pool = PooledDB(pymysql, host=self.host, user=self.user, passwd=self.passwd, db=self.db, port=self.port, charset=self.charset, mincached=self.mincached, maxcached=self.maxcached, maxconnections=self.maxconnections, maxshared=self.maxshared, maxusage=self.maxusage, reset=self.reset, blocking=self.blocking, setsession=self.setsession, failures=self.failures, ping=self.ping)
        ...
        while retry_cnt < self.execute_retries:
            try:
                try:
                    ...
                    conn.commit()
                    self.logger.debug(records + ' %s time success .' % (retry_cnt + 1))
                    break
                except Exception as e:
                    conn.rollback()
                    result = 0
                    retry_cnt += 1
                    self.logger.error(records + ' %s time fail for %s. Sleep for %s seconds then retry.' % (
                     retry_cnt, traceback.format_exc(0), self.fail_sleep))
    ...
    def _batch_action(self, sql, params, batch, pojo, optimized=True):
        ...
        for start, end in idxes:
            ...
            if not optimized:
                res, cursor = self._atomic_action(sql, params[start:end], detail)
                futures.append(res)
            else:
                futures.append(gevent.spawn(self._atomic_action, sql, params[start:end], detail))
                gevent.joinall(futures)

        result = [] if is_select else None
        if is_select:
            ...
            result = sum([ future.get() if optimized else future for future in futures ])
        return result

    def fly(self, sql, params=None, pojo=None, fields=None, batch=1024, propagation=True, showargs=True):
        u"""
        :param sql:
        ...
        """
        sqlinfo = ('%s\n%s' % (sql, params[:1] if len(params) > 20 else params if showargs else '')).decode('utf-8')
