...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\sdo.py
# Compiled at: 2009-06-01 06:45:34
import sets, operator
    ...
    def _construct_geo_interface(self):
        ...
        for g in quadruplets:
            ...
            if g[1] == 1:
                ...
                result_coordinates.append([coords])
            elif g[1] == 2003 and result_types[(-1)] == 'Polygon':
                for j in range(int(g[0]) - 1, int(g[3]) - 1, 2):
                    coords.append([self.SDO_ORDINATES[j], self.SDO_ORDINATES[(j + 1)]])
        ...
        if len(result_types) == 0:
            ...
            for i in range(0, len(result_types)):
                geometries.append({'type': result_types[i], 'coordinates': result_coordinates[i]})
