...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\testsdo.py
# Compiled at: 2009-06-01 05:00:05
import cx_Oracle, simplejson
from shapely.geometry import asShape
from sdo import Geometry as SdoGeometry
from sdo import Geometry
import sdo, re, sys
...
for sel in test_data:
    ...
    p = SdoGeometry(sdo_geometry)
    print 'dimensions:' + str(p.dimensions())
    print 'SDO_ORDINATES:' + str(p.SDO_ORDINATES)
    print 'particular:' + str(p.get_quadruplets())
    print 'to_geo: ' + simplejson.dumps(p.__geo_interface__)
    print 'wktfrom shapely: ' + asShape(p).wkt
    print 'bounds shapely: ' + str(asShape(p).bounds)
    print 'this wkt:------' + str(p.wkt())
    cur.execute('select SDO_UTIL.TO_WKTGEOMETRY(' + sel + ') FROM DUAL')
