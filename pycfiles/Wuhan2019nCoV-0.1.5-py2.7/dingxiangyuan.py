...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/wuhanncov/dingxiangyuan.py
# Compiled at: 2020-01-29 05:32:30
"""
...
class Event:
    ...
    def is_same(self, event):
        return self.get_title().__eq__(event.get_title()) or self.summary == event.summary and self.timestamp_ms == event.timestamp_ms
...
class Summary:
    ...
    def __init__(self, summary_json=None):
        ...
        self.deleted = False
        self.content = '确诊 %d 例，疑似 %d 例，死亡 %d 例，治愈 %d 例' % (
    ...
    def print_desc(self, last_summary=None):
        if last_summary is None or last_summary.content != self.content and self.confirm_count >= last_summary.confirm_count and self.survive_count >= last_summary.survive_count:
...
class DingXiangYuan:
    ...
    def fetch():
        ...
        summary_re = re.compile('try *\\{ *window\\.getStatisticsService *=(.*)\\}catch\\(e\\)')
        detail_re = re.compile('try *\\{ *window\\.getTimelineService *=(.*)\\}catch\\(e\\)')
        is_in_body = False
