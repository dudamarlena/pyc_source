...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/wuhanncov/__init__.py
# Compiled at: 2020-01-29 05:34:12
"""
...
def main():
    ...
    parser.add_argument('dimension', nargs='*', help=('output dimensions: {}').format((', ').join(dimensions)), default=[
     'terminal', 'mac'])
    parser.add_argument('-lark-url', '--lark-url', dest='lark_url', help='the webhook or lark url for lark dimension for lark notify', default='')
    parser.add_argument('--hide-terminal-process', dest='hide_terminal_process', action='store_true', help='whether need to hide process waiting output')
    parser.add_argument('--ignore-first-two-note', dest='ignore_first_two_note', action='store_true', help='whether need to ignore first two output')
    ...
    if not exists(cache_path):
        mkdir(cache_path)
    CheckLoop(cache_path, [DingXiangYuan(), WangYi(), YanShiXinWen(), FengHuang(), TouTiao()]).start(hide_terminal_process, ignore_first_two_note)