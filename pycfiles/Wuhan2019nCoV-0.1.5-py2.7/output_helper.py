...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/wuhanncov/output_helper.py
# Compiled at: 2020-01-28 11:41:15
...
def notify_event(event):
    ...
    if OutputHelper.is_terminal_output:
        ...
        desc = '--------------                                               \n' + colorized
        print (' ').join([desc]).encode('utf-8').strip()
...
def notify_summary(summary, increase_confirm_count, increase_dead_count, increase_survive_count):
    ...
    if increase_survive_count > 0:
        title = '%s 新增痊愈 %d 例' % (title, increase_survive_count)
    if OutputHelper.is_terminal_output:
        print '======================================================='
        terminal_info = colorize(summary.content, fg=GREEN)
        if len(title) > 0:
            terminal_info = '[' + title + '] ' + terminal_info
        print (' ').join([terminal_info]).encode('utf-8').strip()
        print '======================================================='
    if OutputHelper.is_lark_output:
    ...
    if not OutputHelper.is_mac_output:
        ...
        if notify_title is None:
            if len(notify_message_list) > 0:
                ...
                notify_title = None
        elif len(notify_message_list) > 0:
            OutputHelper.last_mac_title = notify_message_list[0].get_title()
        else:
            OutputHelper.last_mac_title = '无新闻只有总数变化'
        if notify_title is not None:
            try:
                message = '最后一条: %s' % OutputHelper.last_mac_title
            except UnicodeDecodeError:
                message = '最后一条: <未知>'

            subtitle = '包含更新%d条' % len(notify_message_list)
            notify_mac(title=notify_title, subtitle=subtitle, message=message)
        return
