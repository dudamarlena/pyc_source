...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/sblibs/display/animation.py
...
'.join(space_wave(x) for x in range(100))
    ...
    """
    wave = cycle(char)
    return (b'').join(next(wave) for x in range(int((amplitude + 1) * abs(sin(frequency * phase)))))
...
def _spinner(control):
    ...
    template = b'{padding} {start} {message} {end}'
    NBRAILY = (b'').join(x * 5 for x in asciiart.BRAILY)
    ...
    for i, (start, end) in enumerate(iterator):
        ...
        message = b'\r' + template.format(message=colorize_no_reset(control.message, b'red'), padding=colorize_no_reset(padding, b'blue'), start=colorize_no_reset(start, b'cyan'), end=color.colorize(end, b'cyan'))
...
class AnimatedDecorator(decorator.Decorator):
    ...
    def start(self, autopush=True):
        ...
        self.spinner.message = (b' - ').join(self.animation.messages)
...
animated = AnimatedDecorator(b'loading')
