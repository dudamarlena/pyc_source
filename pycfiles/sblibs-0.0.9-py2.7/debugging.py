...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/sblibs/display/debugging.py
...
def debug(function):
    ...
    def _wrapper(*args, **kwargs):
        ...
        for key, value in kwargs.items():
            args += tuple([(b'{}={!r}').format(key, value)])

        if len(args) == 1:
            args = (b'({})').format(args[0])
        print (b'@{0}{1} -> {2}').format(function.__name__, args, result)
...
def counter(function):
    ...
    def _wrapper(*args, **kwargs):
        ...
        print (b'{} has been used: {}x').format(funcname, count)
        return res
...
def count_time(function):
    ...
    def _wrapper(*args, **kwargs):
        ...
        diff = time() - before
        funcname = function.__name__
        print (b'{!r} func leave it {:.2f} ms to finish').format(funcname, diff)
        _wrapper.time = diff
        return result
