...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pydosh/models/importModel.py
...
class CsvRecordItem(TreeItem):
    ...
    def _dataFuncProcessed(self, column, role):
        if role == QtCore.Qt.ForegroundRole:
            ...
            if column == enum.kImportColumn_Date:
                return self._date
            if column == enum.kImportColumn_Credit:
                if self._credit:
                    return '%.02f' % self._credit
                return
            if column == enum.kImportColumn_Debit:
                if self._debit:
                    return '%.02f' % abs(self._debit)
                return
            if column == enum.kImportColumn_Description:
                return self._desc
