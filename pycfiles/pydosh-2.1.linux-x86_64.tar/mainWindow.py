...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pydosh/mainWindow.py
...
import currency, utils, models, dialogs, stylesheet, enum, pydosh_rc

class PydoshWindow(Ui_pydosh, QtGui.QMainWindow):
    __operatorMap = {'=': operator.eq, 
       '>': operator.gt, 
       '<': operator.lt, 
       '>=': operator.ge, 
       '<=': operator.le}
    ...
    def showAbout(self):
        """ Show 'about' info
                """
        QtGui.QMessageBox.about(self, 'About pydosh', '<html><p><h2>pydosh</h2></p><p>version %s</p><p>by Will Hall <a href="mailto:will@innerhippy.com">will@innerhippy.com</a></p><p>Copywrite (c) 2013.</p><p>Written using Qt %s, PySide %s</p><p><a href="http://www.innerhippy.com">www.innerhippy.com</a></p>enjoy!</html>' % (
         __version__, QtCore.__version__, pyside_version))
    ...
    def deleteRecords(self):
        ...
        selectionModel = self.tableView.selectionModel()
        proxyModel = self.tableView.model()
        if QtGui.QMessageBox.question(self, 'Delete Records', 'Are you sure you want to delete %d rows?' % len(selectionModel.selectedRows()), QtGui.QMessageBox.Yes | QtGui.QMessageBox.No) != QtGui.QMessageBox.Yes:
            return
        with utils.showWaitCursor():
            ...
            if not proxyModel.sourceModel().deleteRecords(indexes):
                QtGui.QMessageBox.critical(self, 'Database Error', proxyModel.sourceModel().lastError().text(), QtGui.QMessageBox.Ok)
            self.populateAccounts()
            self.populateDates()
            self.setDateRange()
    ...
    def removeTag(self):
        ...
        for proxyIndex in self.tagView.selectionModel().selectedRows():
            ...
            if assignedRecords:
                if QtGui.QMessageBox.question(self, 'Delete Tags', 'There are %d records assigned to this tag\nSure you want to delete it?' % len(assignedRecords), QtGui.QMessageBox.Yes | QtGui.QMessageBox.No) != QtGui.QMessageBox.Yes:
                    continue
            with utils.showWaitCursor():
