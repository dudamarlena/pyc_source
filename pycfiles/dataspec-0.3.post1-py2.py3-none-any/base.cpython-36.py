...
# Compiled at: 2020-05-05 20:04:21
# Size of source mod 2**32: 51400 bytes
import functools, inspect, re, sys
from abc import ABC, abstractmethod
from collections import defaultdict, namedtuple
from enum import EnumMeta
from itertools import chain
from typing import Any, Callable, FrozenSet, Generic, Hashable, Iterable, Iterator, List, Mapping, MutableMapping, NamedTuple, Optional, Sequence, Set, Tuple, Type, TypeVar, Union, cast
import attr
...
SpecPredicate = Union[(
 ...
 Mapping[(ObjectSpecKey, 'SpecPredicate')],
    ...
    def as_map(self) -> Mapping[(str, Union[(str, List[str])])]:
        ...
        if isinstance(self.pred, Spec):
            ...
            if callable(self.pred):
                ...
                pred = str(self.pred)
        return {'message':self.message,  'pred':pred, 
...
class Spec(ABC):
    ...
    def validate_all(self, v: Any) -> List[ErrorDetails]:
        """
    ...
    def compose_conformer(self, conformer: Conformer) -> 'Spec':
        ...
        if existing_conformer is None:
            ...
            def conform_spec(v):
                ...
                return conformer(existing_conformer(v))
...
def tag_maybe(maybe_tag: Union[(Tag, T)], *args: T) -> Tuple[(Optional[Tag], Tuple[(T, ...)])]:
...
class ValidatorSpec(Spec):
    ...
    @classmethod
    def from_validators(cls, tag: Tag, *preds: ValidatorFn, conformer: Optional[Conformer]=None) -> Spec:
...
class CollSpec(Spec):
    ...
    _validate_coll = None
    _validate_coll: Optional[Spec]

    @classmethod
    def from_val(cls, tag: Optional[Tag], sequence: Sequence[Union[(SpecPredicate, CollSpecKwargs)]], conformer: Conformer=None) -> Spec:
        ...
        def conform_coll(v):
            return out_type or type(v)(spec.conform(e) for e in v)
...
class DictSpec(Spec):
    ...
    _reqkeyspecs: Mapping[(Hashable, Spec)]
    _optkeyspecs = attr.ib(factory=dict)
    _optkeyspecs: Mapping[(Hashable, Spec)]
    conformer = None
    conformer: Optional[Conformer]

    @classmethod
    def from_val(cls, tag: Optional[Tag], kvspec: Mapping[(Hashable, SpecPredicate)], conformer: Optional[Conformer]=None) -> Spec:
        reqkeys = {}
        optkeys = {}
        for k, v in kvspec.items():
            if isinstance(k, OptionalKey):
                if k.key in reqkeys:
                    raise KeyError(f"Optional key '{k.key}' duplicates key already defined in required keys")
                optkeys[k.key] = make_spec(v)
            else:
                if k in optkeys:
                    raise KeyError(f"Required key '{k}' duplicates key already defined in optional keys")
                reqkeys[k] = make_spec(v)

        def conform_mapping(d):
            conformed_d = {}
    ...
    @classmethod
    def merge(cls, tag: Optional[Tag], *specs: 'DictSpec', conformer: Optional[Conformer]=None) -> Spec:
        assert len(specs) >= 2, 'Must merge at least two Specs'
        map_pred = defaultdict(list)
        for spec in specs:
        ...
        return cls.from_val((tag or f"merge-of-{'-'.join(spec.tag for spec in specs)}"),
          {k:all_spec(str(k), *v) for k, v in map_pred.items()}, conformer=conformer)


def kv_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conform_keys: bool=False, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    return ValidatorSpec(tag,
      _kv_valid, conformer=(compose_conformers(conform_mapping, conformer)))


@attr.s(auto_attribs=True, frozen=True, slots=True)
class ObjectSpec(Spec):
    tag: Tag
    _reqattrspecs = attr.ib(factory=dict)
    _reqattrspecs: Mapping[(str, Spec)]
    _optattrspecs = attr.ib(factory=dict)
    _optattrspecs: Mapping[(str, Spec)]
    conformer = None
    conformer: Optional[Conformer]

    @classmethod
    def from_val(cls, tag: Optional[Tag], kvspec: Mapping[(ObjectSpecKey, SpecPredicate)], conformer: Optional[Conformer]=None) -> Spec:
        reqattrs = {}
        optattrs = {}
        for k, v in kvspec.items():
            if isinstance(k, OptionalKey):
                if k.key in reqattrs:
                    raise KeyError(f"Optional attribute '{k.key}' duplicates key already defined in required attributes")
                optattrs[k.key] = make_spec(v)
            else:
                if k in optattrs:
                    raise KeyError(f"Required attribute '{k}' duplicates key already defined in optional attributes")
                reqattrs[k] = make_spec(v)

        return cls((tag or 'obj'),
...
class SetSpec(Spec):
    ...
    @classmethod
    def from_enum(cls, tag: Optional[Tag], pred: EnumMeta, conformer: Optional[Conformer]=None):
        return cls((tag or pred.__name__),
...
class TupleSpec(Spec):
    ...
    def from_val(cls, tag: Optional[Tag], pred: Tuple[(SpecPredicate, ...)], conformer: Conformer=None):
        ...
        def conform_tuple(v):
            return namedtuple_type and namedtuple_type._make or tuple(spec.conform(v) for spec, v in zip(specs, v))
...
def all_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    if len(preds) == 1:
        return make_spec(filter(None, (tag,)), *(
...
def any_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, tag_conformed: bool=False, conformer: Optional[Conformer]=None) -> Spec:
    """
...
def merge_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None) -> Spec:
    """
...
def pred_to_validator(message: str, complement: bool=False, convert_value: Callable[([Any], Any)]=_identity, **fmtkwargs) -> Callable[([PredicateFn], ValidatorFn)]:
    ...
    def to_validator(pred):
        ...
        validator.is_validator_fn = True
        return validator
...
def make_spec(tag_or_pred: Union[(Tag, SpecPredicate)], *preds: SpecPredicate, conformer: Optional[Conformer]=None) -> Spec:
    """
    ...
    if callable(pred):
        ...
        if sig is not None and sig.return_annotation is Iterator[ErrorDetails] or getattr(pred, 'is_validator_fn', False):
