...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/sparse/pkt_build.py
# Compiled at: 2015-06-16 13:16:13
from __future__ import absolute_import
import numpy as np, pycuda.gpuarray as gpuarray
from six.moves import range

def build_pkt_data_structure(spmv, packet_nr_to_dofs, max_thread_costs, old2new_fetch_indices, csr_mat, thread_count, thread_assignments, local_row_costs):
    packet_start = 0
    base_dof_nr = 0
    index_array = np.zeros(max_thread_costs * thread_count, dtype=spmv.packed_index_dtype)
    data_array = np.zeros(max_thread_costs * thread_count, dtype=spmv.dtype)
    thread_starts = np.zeros(thread_count, dtype=spmv.index_dtype)
    thread_ends = np.zeros(thread_count, dtype=spmv.index_dtype)
    for packet_nr, packet_dofs in enumerate(packet_nr_to_dofs):
        ...
        for thread_offset in range(spmv.threads_per_packet):
            ...
            max_packet_items = max(max_packet_items, thread_items)
