...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/gpuarray.py
# Compiled at: 2015-07-29 09:56:12
from __future__ import division, absolute_import
import numpy as np, pycuda.elementwise as elementwise
from pytools import memoize, memoize_method
import pycuda.driver as drv
from pycuda.compyte.array import as_strided as _as_strided, f_contiguous_strides as _f_contiguous_strides, c_contiguous_strides as _c_contiguous_strides, ArrayFlags as _ArrayFlags, get_common_dtype as _get_common_dtype_base
     ...
     (
      'double', np.float64, [1, 2])]:
        for count in counts:
            ...
            setattr(vec, 'make_' + name, staticmethod(eval('lambda %s: array((%s), dtype=my_dtype)' % (
             my_field_names, my_field_names), dict(array=np.array, my_dtype=dtype))))
...
def _splay_backend(n, dev):
    ...
    max_threads = 128
    max_blocks = 4 * devdata.thread_blocks_per_mp * dev.get_attribute(drv.device_attribute.MULTIPROCESSOR_COUNT)
    if n < min_threads:
        block_count = 1
    ...
    return (
     (
      block_count, 1), (threads_per_block, 1, 1))
    ...
    def func(self, other):
        ...
        if isinstance(other, GPUArray):
            if not self.shape == other.shape:
                ...
                raise (other.flags.forc or RuntimeError)('only contiguous arrays may be used as arguments to this operation')
            result = self._new_like_me()
            func = elementwise.get_binary_op_kernel(self.dtype, other.dtype, result.dtype, operator)
            func.prepared_async_call(self._grid, self._block, None, self.gpudata, other.gpudata, result.gpudata, self.mem_size)
            return result
        else:
            result = self._new_like_me()
            func = elementwise.get_scalar_op_kernel(self.dtype, result.dtype, operator)
            func.prepared_async_call(self._grid, self._block, None, self.gpudata, other, result.gpudata, self.mem_size)
            return result
            return
...
class GPUArray(object):
    ...
    def __init__(self, shape, dtype, allocator=drv.mem_alloc, base=None, gpudata=None, strides=None, order='C'):
    ...
    def get(self, ary=None, pagelocked=False, async=False, stream=None):
        if ary is None:
            ...
            if self.shape != ary.shape:
                from warnings import warn
                warn('get() between arrays of different shape is deprecated and will be removed in PyCUDA 2017.x', DeprecationWarning, stacklevel=2)
                ary = ary.reshape(self.shape)
    ...
    def _axpbyz(self, selffac, other, otherfac, out, add_timer=None, stream=None):
        ...
        if add_timer is not None:
            add_timer(3 * self.size, func.prepared_timed_call(self._grid, selffac, self.gpudata, otherfac, other.gpudata, out.gpudata, self.mem_size))
        else:
            func.prepared_async_call(self._grid, self._block, stream, selffac, self.gpudata, otherfac, other.gpudata, out.gpudata, self.mem_size)
        return out
    ...
    def _axpbz(self, selffac, other, out, stream=None):
        ...
        func.prepared_async_call(self._grid, self._block, stream, selffac, self.gpudata, other, out.gpudata, self.mem_size)
        return out
    ...
    def _elwise_multiply(self, other, out, stream=None):
        ...
        func.prepared_async_call(self._grid, self._block, stream, self.gpudata, other.gpudata, out.gpudata, self.mem_size)
        return out
    ...
    def _new_like_me(self, dtype=None):
        ...
        return self.__class__(self.shape, dtype, allocator=self.allocator, strides=strides)
    ...
    def fill(self, value, stream=None):
        ...
        func.prepared_async_call(self._grid, self._block, stream, value, self.gpudata, self.mem_size)
        return self

    def bind_to_texref(self, texref, allow_offset=False):
        return texref.set_address(self.gpudata, self.nbytes, allow_offset=allow_offset) / self.dtype.itemsize

    def bind_to_texref_ext(self, texref, channels=1, allow_double_hack=False, allow_complex_hack=False, allow_offset=False):
        if not self.flags.forc:
            raise RuntimeError('only contiguous arrays may be used as arguments to this operation')
        if self.dtype == np.float64 and allow_double_hack:
            ...
            if channels != 1:
                raise ValueError("'fake' complex64 textures can only have one channel")
            channels = 2
            fmt = drv.array_format.UNSIGNED_INT32
            read_as_int = True
        elif self.dtype == np.complex128 and allow_complex_hack:
            if channels != 1:
                raise ValueError("'fake' complex128 textures can only have one channel")
            channels = 4
            fmt = drv.array_format.SIGNED_INT32
            read_as_int = True
        else:
    ...
    def reshape(self, *shape):
        ...
        return GPUArray(shape=shape, dtype=self.dtype, allocator=self.allocator, base=self, gpudata=int(self.gpudata))
    ...
    def view(self, dtype=None):
        ...
        min_stride_axis = argmin2((axis, abs(stride)) for axis, stride in enumerate(self.strides))
        if self.shape[min_stride_axis] * old_itemsize % itemsize != 0:
            raise ValueError('new type not compatible with array')
        new_shape = self.shape[:min_stride_axis] + (self.shape[min_stride_axis] * old_itemsize // itemsize,) + self.shape[min_stride_axis + 1:]
        new_strides = self.strides[:min_stride_axis] + (self.strides[min_stride_axis] * itemsize // old_itemsize,) + self.strides[min_stride_axis + 1:]
        return GPUArray(shape=new_shape, dtype=dtype, allocator=self.allocator, strides=new_strides, base=self, gpudata=int(self.gpudata))
    ...
    def transpose(self, axes=None):
        ...
        return GPUArray(shape=tuple(new_shape), dtype=self.dtype, allocator=self.allocator, base=self.base or self, gpudata=self.gpudata, strides=tuple(new_strides))
    ...
    def __getitem__(self, index):
        ...
        return GPUArray(shape=tuple(new_shape), dtype=self.dtype, allocator=self.allocator, base=self, gpudata=int(self.gpudata) + new_offset, strides=tuple(new_strides))
...
def empty_like(other_ary):
    result = GPUArray(other_ary.shape, other_ary.dtype, other_ary.allocator)
    return result
...
def arange(*args, **kwargs):
    ...
    if not explicit_dtype and dtype != np.float32:
        from warnings import warn
        warn('behavior change: arange guessed dtype other than float32. suggest specifying explicit dtype.')
    from math import ceil
    size = int(ceil((stop - start) / step))
    result = GPUArray((size,), dtype)
    func = elementwise.get_arange_kernel(dtype)
    func.prepared_async_call(result._grid, result._block, kwargs.get('stream'), result.gpudata, start, step, size)
    return result
...
def _memcpy_discontig(dst, src, async=False, stream=None):
    ...
    if src.flags.forc and dst.flags.forc:
        ...
        while i < len(shape):
            ...
            if src_strides[(i - 1)] * shape[(i - 1)] == src_strides[i] and dst_strides[(i - 1)] * shape[(i - 1)] == dst_strides[i]:
                shape[(i - 1):(i + 1)] = [
                 shape[(i - 1)] * shape[i]]
                del src_strides[i]
                del dst_strides[i]
...
six.moves.copyreg.pickle(GPUArray, lambda data: (
 to_gpu, (data.get(),)), to_gpu)
    ...
    if not len(arrays):
        ...
        if out is None:
            ...
            for start_i in range(0, len(arrays), chunk_size):
                ...
                func.prepared_async_call(indices._grid, indices._block, stream, indices.gpudata, *([ o.gpudata for o in out[chunk_slice] ] + [
                 indices.size]))

        return out


def multi_take_put(arrays, dest_indices, src_indices, dest_shape=None, out=None, stream=None, src_offsets=None):
    if not len(arrays):
        return []
    else:
        from pytools import single_valued
        ...
        if out is None:
            ...
            def make_func_for_chunk_size(chunk_size):
                return elementwise.get_take_put_kernel(a_dtype, src_indices.dtype, with_offsets=src_offsets is not None, vec_count=chunk_size)
            ...
            for start_i in range(0, len(arrays), chunk_size):
                ...
                func.prepared_async_call(src_indices._grid, src_indices._block, stream, dest_indices.gpudata, src_indices.gpudata, *([ o.gpudata for o in out[chunk_slice] ] + src_offsets_list[chunk_slice] + [src_indices.size]))
...
def multi_put(arrays, dest_indices, dest_shape=None, out=None, stream=None):
    if not len(arrays):
        ...
        if out is None:
            ...
            def make_func_for_chunk_size(chunk_size):
                return elementwise.get_put_kernel(a_dtype, dest_indices.dtype, vec_count=chunk_size)
            ...
            for start_i in range(0, len(arrays), chunk_size):
                ...
                func.prepared_async_call(dest_indices._grid, dest_indices._block, stream, dest_indices.gpudata, *([ o.gpudata for o in out[chunk_slice] ] + [ i.gpudata for i in arrays[chunk_slice] ] + [dest_indices.size]))
...
def if_positive(criterion, then_, else_, out=None, stream=None):
    ...
    func.prepared_async_call(criterion._grid, criterion._block, stream, criterion.gpudata, then_.gpudata, else_.gpudata, out.gpudata, criterion.size)
    return out
...
def _make_binary_minmax_func(which):
    ...
    def f(a, b, out=None, stream=None):
        if isinstance(a, GPUArray) and isinstance(b, GPUArray):
            if out is None:
                out = empty_like(a)
            func = elementwise.get_binary_minmax_kernel(which, a.dtype, b.dtype, out.dtype, use_scalar=False)
            func.prepared_async_call(a._grid, a._block, stream, a.gpudata, b.gpudata, out.gpudata, a.size)
        elif isinstance(a, GPUArray):
            if out is None:
                out = empty_like(a)
            func = elementwise.get_binary_minmax_kernel(which, a.dtype, a.dtype, out.dtype, use_scalar=True)
            func.prepared_async_call(a._grid, a._block, stream, a.gpudata, b, out.gpudata, a.size)
        else:
            if out is None:
                out = empty_like(b)
            func = elementwise.get_binary_minmax_kernel(which, b.dtype, b.dtype, out.dtype, use_scalar=True)
            func.prepared_async_call(b._grid, b._block, stream, b.gpudata, a, out.gpudata, b.size)
        return out
