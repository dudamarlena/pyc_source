...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/compiler.py
# Compiled at: 2015-06-19 12:43:01
...
def preprocess_source(source, options, nvcc):
    ...
    os.close(handle)
    cmdline = [
     nvcc, '--preprocess'] + options + [source_path]
    ...
    if result != 0:
        ...
        raise CompileError('nvcc preprocessing of %s failed' % source_path, cmdline, stderr=stderr)
    if len(stdout) < 0.5 * len(source):
        from pycuda.driver import CompileError
        raise CompileError('nvcc preprocessing of %s failed with ridiculously small code output - likely unsupported compiler.' % source_path, cmdline, stderr=stderr.decode('utf-8', 'replace'))
    unlink(source_path)
    return stdout.decode('utf-8', 'replace')
...
def compile_plain(source, options, keep, nvcc, cache_dir, target='cubin'):
    ...
    if result != 0 or no_output and (stdout or stderr):
        if result == 0:
            from warnings import warn
            warn('PyCUDA: nvcc exited with status 0, but appears to have encountered an error')
        from pycuda.driver import CompileError
        raise CompileError('nvcc compilation of %s failed' % cu_file_path, cmdline, stdout=stdout.decode('utf-8', 'replace'), stderr=stderr.decode('utf-8', 'replace'))
    if stdout or stderr:
        lcase_err_text = (stdout + stderr).decode('utf-8', 'replace').lower()
        from warnings import warn
        if 'demoted' in lcase_err_text or 'demoting' in lcase_err_text:
            warn('nvcc said it demoted types in source code it compiled--this is likely not what you want.', stacklevel=4)
...
import os
DEFAULT_NVCC_FLAGS = [ _flag.strip() for _flag in os.environ.get('PYCUDA_DEFAULT_NVCC_FLAGS', '').split() if _flag.strip()
                     ]

def compile(source, nvcc='nvcc', options=None, keep=False, no_extern_c=False, arch=None, code=None, cache_dir=None, include_dirs=[], target='cubin'):
    if not target in ('cubin', 'ptx', 'fatbin'):
        raise AssertionError
        source = no_extern_c or 'extern "C" {\n%s\n}\n' % source
    if options is None:
    ...
    if cache_dir is None:
        from os.path import join
        import appdirs
        cache_dir = os.path.join(appdirs.user_cache_dir('pycuda', 'pycuda'), 'compiler-cache-v1')
    ...
    if 'darwin' in sys.platform and sys.maxint == 9223372036854775807:
        ...
        if 'win32' in sys.platform and sys.maxsize == 9223372036854775807:
            options.append('-m64')
...
class SourceModule(object):
    ...
    def __init__(self, source, nvcc='nvcc', options=None, keep=False, no_extern_c=False, arch=None, code=None, cache_dir=None, include_dirs=[]):
        self._check_arch(arch)
        cubin = compile(source, nvcc, options, keep, no_extern_c, arch, code, cache_dir, include_dirs)
        ...
        if arch is None:
            ...
            try:
                ...
                if tuple(map(int, tuple(arch.split('_')[1]))) > capability:
                    from warnings import warn
                    warn('trying to compile for a compute capability higher than selected GPU')
            except:
                pass
