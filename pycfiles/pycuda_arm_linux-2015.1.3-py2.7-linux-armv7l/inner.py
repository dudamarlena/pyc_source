...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/sparse/inner.py
    ...
    def get_host_result(self):
        if not self.gpu_finished:
            if self.gpu_finished_evt.query():
                ...
                self.host_dest = self.pagelocked_allocator(self.gpu_result.shape, self.gpu_result.dtype, self.copy_stream)
                drv.memcpy_dtoh_async(self.host_dest, self.gpu_result.gpudata, self.copy_stream)
                self.copy_finished_evt = drv.Event()
                self.copy_finished_evt.record()
        elif self.copy_finished_evt.query():
