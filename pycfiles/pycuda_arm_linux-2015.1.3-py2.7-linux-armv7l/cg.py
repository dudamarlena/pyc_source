...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/sparse/cg.py
# Compiled at: 2015-06-16 13:16:13
from __future__ import division
from __future__ import absolute_import
from pycuda.sparse.inner import AsyncInnerProduct
from pytools import memoize_method
import pycuda.gpuarray as gpuarray, numpy as np
...
class CGStateContainer:
    ...
    def guarded_div_kernel(self, dtype_x, dtype_y, dtype_z):
        ...
        return get_elwise_kernel('%(tp_x)s *x, %(tp_y)s *y, %(tp_z)s *z' % {'tp_x': dtype_to_ctype(dtype_x), 
           ...
           'tp_z': dtype_to_ctype(dtype_z)}, 'z[i] = y[i] == 0 ? 0 : (x[i] / y[i])', 'divide')
    ...
    def guarded_div(self, a, b):
        ...
        func.prepared_async_call(a._grid, a._block, None, a.gpudata, b.gpudata, result.gpudata, a.mem_size)
        return result
    ...
    def run(self, max_iterations=None, tol=1e-07, debug_callback=None):
        check_interval = 20
        ...
        while iterations < max_iterations:
            ...
            if debug_callback is not None:
                if compute_real_residual:
                    ...
                    what = 'it'
                debug_callback(what, iterations, self.x, self.residual, self.d, self.delta)
                    ...
                    while i < len(rdq):
                        ...
                        if delta is not None:
                            if abs(delta) < tol * tol * abs(delta_0):
                                if debug_callback is not None:
                                    debug_callback('end', iterations, self.x, self.residual, self.d, self.delta)
                                return self.x
                            rdq.pop(i)
                        else:
                            i += 1
...
def solve_pkt_with_cg(pkt_spmv, b, precon=None, x=None, tol=1e-07, max_iterations=None, debug=False, pagelocked_allocator=None):
    if x is None:
        x = gpuarray.zeros(pkt_spmv.shape[0], dtype=pkt_spmv.dtype, allocator=b.allocator)
    else:
    ...
    return (
     pkt_spmv.unpermute(result), it_count[0], res_count[0])