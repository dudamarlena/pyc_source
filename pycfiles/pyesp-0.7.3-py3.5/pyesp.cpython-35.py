...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pyesp/pyesp.py
# Compiled at: 2017-11-17 19:20:33
# Size of source mod 2**32: 8012 bytes
import os, threading
...
class SerialPort(QSerialPort):
    __doc__ = 'docstring for SerialPort'
    ...
    def write_data(self, data):
        if self.open() and self.write(bytearray(data, 'windows-1251')) != 0:
            self.workerbreak = False
            return
...
class ESP(QCoreApplication):
    __doc__ = 'docstring for ESP'
    _api_file = {'MPY': os.path.join(os.path.dirname(__file__), __api_mpy_path__), 
    ...
    def __init__(self, serialconfig, platform='USER', api_file=None, loglevel=0):
        ...
        try:
            sc = serialconfig.split(',')
            _serialconfig = {'portName': str(sc[0]), 
             'baudRate': int(sc[1]), 
             'dataBits': int(sc[2]), 
             'parity': int(sc[3]), 
             'stopBits': int(sc[4]), 
            ...
            raise BaseException('error in serial config parametrs - ' + str(e))
        ...
        if platform in ESP._api_file.keys():
            ...
            self.txfer = b''
            self.rxfer = b''
            self.serialport = SerialPort(_serialconfig)
            self.serialport.readline_signal.connect(self.read, no_receiver_check=True)
            self._response_callback = None

    def send(self, command, **kwargs):
        """ """
        self._response_callback = kwargs.get('callback', None)
        ...
        self.txfer += "\r\nprint('END')"
        self.txfer.replace('\\r', '\\\\r').replace('\\n', '\\\\n')
