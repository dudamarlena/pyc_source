...
# [GCC 8.4.0]
# Embedded file name: autoant/processors.py
# Compiled at: 2014-09-25 09:53:43
import re, os, pickle, shutil, logging
...
class ProcessorRename(BaseProcessor):
    ...
    def run(self, file_item):
        ...
        try:
            new_file_name = re.sub(('{0}').format(self.rule_origin), ('{0}').format(self.rule_destination), file_item.name)
            ...
            log.error(('{0}: Renamed file error file {1} :{2}').format(self.name, file_item, e))
            return False
...
class ProcessorMove(BaseProcessor):
    ...
    def run(self, file_item):
        ...
        try:
            ...
            log.error(('{0}: Moved file error file {1} :{2}').format(self.name, file_item, e))
            return False
...
class ProcessorCopy(BaseProcessor):
    ...
    def run(self, file_item):
        ...
        try:
            ...
            log.error(('{0}: Copy file error {1} :{2}').format(self.name, file_item, e))
            return False
...
class ProcessorSMB(BaseProcessorRemoteCP):
    ...
    def run(self, file_item):
        ...
        try:
            ...
            log.error(('{0}: SMB Put error to {1} file {2} :{3}').format(self.name, self.remote_host, file_item, e))
            return False
...
class ProcessorFTP(BaseProcessorRemoteCP):
    ...
    def run(self, file_item):
        ...
        try:
            ...
            log.error(('{0}: FTP Put error to {1} file {2} :{3}').format(self.name, self.remote_host, file_item, e))
            return False
...
class ProcessorSCP(BaseProcessorRemoteCP):
    ...
    def connect(self):
        try:
            ...
            self.ssh.connect(self.remote_host, username=self.username, password=self.password, key_filename=self.key_filename, timeout=self.timeout)
        except Exception as e:
            log.error(('{0}: Connect error to {1} {2}').format(self.name, self.remote_host, e))
            return False
    ...
    def disconnect(self):
        ...
        log.info(('{0}: SFTP Disconnected from {1} with {2}').format(self.name, self.remote_host, self.username))
    ...
    def run(self, file_item):
        ...
        try:
            ...
            log.error(('{0}: SFTP Put error to {1} file {2} :{3}').format(self.name, self.remote_host, file_item, e))
            return False
...
processor_providers = {'baseprocessor': BaseProcessor, 'scp': ProcessorSCP, 
   'ftp': ProcessorFTP}