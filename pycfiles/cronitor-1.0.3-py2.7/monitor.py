...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/cronitor/monitor.py
# Compiled at: 2018-07-12 03:25:55
...
class Monitor(object):
    ...
    def pause(self, code, hours):
        return self.__get(('{0}/{1}/pause/{2}').format(self.api_endpoint, code, hours))

    def clone(self, code, name=None):
        return requests.post(self.api_endpoint, auth=(
         self.api_key, ''), timeout=10, data=json.dumps({'code': code, name: name}), headers={'content-type': 'application/json'})

    def __ping(self, code, method, msg=''):
        params = dict(auth_key=self.auth_key, msg=msg)
        return requests.get(('{0}/{1}/{2}').format(self.ping_endpoint, code, method), params=params, timeout=10)

    def __get(self, url):
        return requests.get(url, timeout=10, auth=(
         self.api_key, ''), headers={'content-type': 'application/json'})

    def __create(self, payload):
        return requests.post(self.api_endpoint, auth=(
         self.api_key, ''), data=json.dumps(payload), headers={'content-type': 'application/json'}, timeout=10)

    def __update(self, payload=None, code=None):
        return requests.put(('{0}/{1}').format(self.api_endpoint, code), auth=(
         self.api_key, ''), data=json.dumps(payload), headers={'content-type': 'application/json'}, timeout=10)

    def __delete(self, code):
        return requests.delete(('{0}/{1}').format(self.api_endpoint, code), auth=(
         self.api_key, ''), headers={'content-type': 'application/json'}, timeout=10)

    @staticmethod
    def __prepare_notifications(notifications):
        if notifications:
            return {'emails': notifications.get('emails', []), 
    ...
    def __prepare_payload(self, tags, name, note, notifications, rules):
        return {'code': 'new_monitor', 'name': name, 
           'type': 'heartbeat', 
