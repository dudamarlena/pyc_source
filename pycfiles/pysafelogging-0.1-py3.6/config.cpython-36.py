...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysafelogging/config.py
# Compiled at: 2019-01-27 21:20:51
# Size of source mod 2**32: 36184 bytes
"""
...
"""
import errno, io, pysafelogging, pysafelogging.handlers, re, struct, sys, traceback
try:
...
class ConvertingMixin(object):
    __doc__ = "For ConvertingXXX's, this mixin class provides common functions"
...
class ConvertingDict(dict, ConvertingMixin):
    __doc__ = 'A converting dictionary wrapper.'
...
class ConvertingList(list, ConvertingMixin):
    __doc__ = 'A converting list wrapper.'
...
class ConvertingTuple(tuple, ConvertingMixin):
    __doc__ = 'A converting tuple wrapper.'
...
class DictConfigurator(BaseConfigurator):
    ...
    def configure_handler(self, config):
        ...
        if issubclass(klass, pysafelogging.handlers.MemoryHandler) and 'target' in config:
            try:
            ...
            if issubclass(klass, pysafelogging.handlers.SMTPHandler):
                if 'mailhost' in config:
                    config['mailhost'] = self.as_tuple(config['mailhost'])
                if issubclass(klass, pysafelogging.handlers.SysLogHandler):
