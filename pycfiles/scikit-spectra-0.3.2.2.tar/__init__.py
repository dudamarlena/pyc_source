...
# [GCC 8.4.0]
# Embedded file name: /home/glue/Desktop/scikit-spectra/skspec/data/__init__.py
# Compiled at: 2015-04-24 11:04:41
...
__all__ = [
 'aunps_glass', 'aunps_water', 'solvent_evap', 'trip_peaks']
...
def load_ts(filepath, *args, **kwargs):
    ...
    if ext == '.pickle':
        ...
        raise DataError('%s must have file extension .csv or .pickle, not %s' % (
         filepath, ext))
...
def trip_peaks(*args, **kwargs):
    ...
    trips = Spectra.from_csv(filepath, header=None, name='Three Peaks', index_col=0, skiprows=1)
    trips.reference = 0
    trips.index = SpecIndex(trips.index + 50, unit='ev')
    trips.varunit = Unit(short='pz', full='Polarization', symbol='\\phi')
    return trips
