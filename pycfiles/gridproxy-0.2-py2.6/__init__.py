...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/gridproxy/__init__.py
# Compiled at: 2012-07-16 12:56:42
"""Grid proxy certificate functions."""
import os, re, struct, tempfile, time
...
def generate_proxycert(new_pkey, cert, key, **kwargs):
    ...
    not_after.set_time(now + args.get('lifetime', 43200) + CLOCK_SKEW)
    ...
    proxy.set_serial_number(int(serial & 2147483647))
    ...
    if legacy:
        if full:
            proxy_subject.add_entry_by_txt('CN', ASN1.MBSTRING_ASC, 'proxy', -1, -1, 0)
        else:
            proxy_subject.add_entry_by_txt('CN', ASN1.MBSTRING_ASC, 'limited proxy', -1, -1, 0)
    else:
        proxy_subject.add_entry_by_txt('CN', ASN1.MBSTRING_ASC, str(serial), -1, -1, 0)
    ...
    if not legacy:
        if full:
            proxy.add_ext(X509.new_extension('proxyCertInfo', 'critical, language:Inherit all', 1))
        else:
            proxy.add_ext(X509.new_extension('proxyCertInfo', 'critical, language:1.3.6.1.4.1.3536.1.1.1.9', 1))
    sign_pkey = EVP.PKey()
