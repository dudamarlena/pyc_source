...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/library_simulator/generator.py
# Compiled at: 2019-06-03 11:42:55
# Size of source mod 2**32: 12927 bytes
__description__ = '\nMain class for generating new clones from the library.\n'
__author__ = 'Michael J. Harms'
__date__ = '2019-05-31'
from . import data
import pandas as pd, numpy as np, warnings, os, copy
...
class LibraryGenerator:
    ...
    def generate_clone(self, num_mutations):
        ...
        sites_to_mutate = np.random.choice((self._mut_sites), size=num_mutations,
          replace=False,
          p=(self._mut_probs))
        ...
        for i in sites_to_mutate:
            seq[i] = np.random.choice((self._mut_outcomes[i][0]), p=(self._mut_outcomes[i][1]))
        ...
        if has_indel or has_stop:
            ...
            aa_diffs = [(self._aa_seq[i], i, new_aa_seq[i]) for i in range(shortest_len) if self._aa_seq[i] != new_aa_seq[i]]
            num_diff = len(aa_diffs)
            ...
            if self._aa_seq[0] == 'M':
                if new_aa_seq[0] != 'M':
                    altered_start = True
        return (
         aa_diffs, base_diffs, num_diff, has_indel, has_stop, altered_start)
