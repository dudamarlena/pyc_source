...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/FBpyGIF/main.py
# Compiled at: 2017-09-05 07:56:56
# Size of source mod 2**32: 2855 bytes
if __name__ != '__main__':
    if __name__ != 'FBpyGIF.main':
...
def main(argv=None):
    ...
    if args.clear != -1:
        fb.fill_scr(args.clear >> 16, args.clear >> 8 & 255, args.clear & 255)
        exit(0)
    ...
    try:
        try:
            ...
            if args.color_test:
                ...
                fb.fill_scr_ani(event=e, delay=0.03333333333333333)
            else:
                ...
                if args.no_loop:
                    ...
                    for path in it:
                        ...
                        if what(path) == 'gif':
                            if args.animate_delay:
                                Timer(args.animate_delay, lambda e: e.set(), [e]).start()
