...
class Alias(models.Model):
    ...
    domain = type(_site_domain)(_('domain name'), max_length=_site_domain.max_length, unique=True, help_text=_('Either "domain" or "domain:port"'))
    site = models.ForeignKey(Site, related_name='aliases', on_delete=models.CASCADE)
    is_canonical = models.NullBooleanField(_('is canonical?'), default=None, editable=False, validators=[
     validate_true_or_none], help_text=_('Does this domain name match the one in site?'))
    redirect_to_canonical = models.BooleanField(_('redirect to canonical?'), default=True, help_text=_('Should this domain name redirect to the one in site?'))
    ...
    def sync(cls, site, force_insert=False):
        ...
        if force_insert:
            ...
            alias, created = cls.objects.get_or_create(site=site, is_canonical=True, defaults={'domain': domain})
