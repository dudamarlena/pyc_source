...
urlpatterns = [
 url('^domain/$', lambda request, *args, **kwargs: HttpResponse(str(Site.objects.get_current())))]
...
@pytest.mark.django_db
@skipUnless(Site._meta.installed, 'django.contrib.sites is not in settings.INSTALLED_APPS')
@override_settings(SITE_ID=SiteID(default=0),
  CACHE_MULTISITE_ALIAS='multisite',
...
class DynamicSiteMiddlewareFallbackTest(TestCase):
    ...
    def test_string_class(self):
        ...
        settings.MULTISITE_FALLBACK_KWARGS = {'url':'http://example.com/',  'permanent':False}
        request = self.factory.get('/')
        response = DynamicSiteMiddleware().process_request(request)
    ...
    def test_class_view(self):
        ...
        settings.MULTISITE_FALLBACK = RedirectView.as_view(url='http://example.com/',
          permanent=False)
        request = self.factory.get('/')
        response = DynamicSiteMiddleware().process_request(request)
        self.assertEqual(response.status_code, 302)
...
class SiteCacheTest(TestCase):
    ...
    @override_settings(CACHE_MULTISITE_ALIAS='multisite',
      CACHES={'multisite': {'BACKEND':'django.core.cache.backends.locmem.LocMemCache', 
...
class AliasTest(TestCase):
    ...
    def test_create(self):
        ...
        self.assertRaises(ValidationError,
          ...
          domain=(site1.domain), site=site1)
        self.assertRaises(ValidationError,
          (Alias.objects.create),
          domain=(site2.domain), site=site1)
        self.assertRaises(ValidationError,
          (Alias.objects.create),
          domain='1a.example', site=site1)
        self.assertRaises(ValidationError,
          (Alias.objects.create),
          domain='1A.EXAMPLE', site=site2)
        self.assertRaises(ValidationError,
          (Alias.objects.create),
          domain='2.EXAMPLE', site=site2)
    ...
    def test_sync(self):
        ...
        Alias.sync(site=site)
        self.assertEqual(Alias.objects.get(site=site).domain, site.domain)
        self.assertRaises(ValidationError, (Alias.sync),
          site=site, force_insert=True)
...
@pytest.mark.django_db
@override_settings(MULTISITE_COOKIE_DOMAIN_DEPTH=0,
...
if django.VERSION < (1, 8):
    TEMPLATE_SETTINGS = {'TEMPLATE_LOADERS':['multisite.template.loaders.filesystem.Loader'],  'TEMPLATE_DIRS':[
