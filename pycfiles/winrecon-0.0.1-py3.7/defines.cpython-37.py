...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\winrecon\cf\defines.py
# Compiled at: 2019-03-05 08:26:22
# Size of source mod 2**32: 23500 bytes
"""
Common definitions.
"""
...
if WIN32_VERBOSE_MODE:
    ...
    class WinFuncHook(object):
        ...
        def __init__(self, name):
            self._WinFuncHook__name = name

        def __getattr__(self, name):
            if name.startswith('_'):
                return object.__getattr__(self, name)
            return WinCallHook(self._WinFuncHook__name, name)


    class WinCallHook(object):

        def __init__(self, dllname, funcname):
            self._WinCallHook__dllname = dllname
            self._WinCallHook__funcname = funcname
            self._WinCallHook__func = getattr(getattr(ctypes.windll, dllname), funcname)

        def __copy_attribute(self, attribute):
            try:
                value = getattr(self, attribute)
                setattr(self._WinCallHook__func, attribute, value)
            except AttributeError:
                try:
                    delattr(self._WinCallHook__func, attribute)
                except AttributeError:
                    pass

        def __call__(self, *argv):
            self._WinCallHook__copy_attribute('argtypes')
            self._WinCallHook__copy_attribute('restype')
            self._WinCallHook__copy_attribute('errcheck')
            print('----------')
            print('%s ! %s %r' % (self._WinCallHook__dllname, self._WinCallHook__funcname, argv))
            retval = (self._WinCallHook__func)(*argv)
            print('== %r' % (retval,))
            return retval
...
try:
    ...
    SIZE_T = {1:BYTE, 
     2:WORD,  4:DWORD,  8:QWORD}[sizeof(LPVOID)]
