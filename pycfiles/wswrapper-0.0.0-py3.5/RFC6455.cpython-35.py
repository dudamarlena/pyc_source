...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wswrapper/RFC6455.py
# Compiled at: 2016-07-04 19:17:25
# Size of source mod 2**32: 10513 bytes
from Crypto.Random import get_random_bytes
import base64, hashlib
from .httpParser import *
__all__ = [
 'Frame', 'Handshake', 'RFCError']
OPCODES = {0: 'Continuation', 
 1: 'Text', 
 2: 'Binary', 
...
STATUS_CODES = {1000: 'Normal_close', 
...
class Frame:
    __doc__ = 'Contains methods to pack and unpack a websocket frame\n    '
    ...
    def __init__(self, fin=1, rsv=(0, 0, 0), opcode=None, mask=0, masking_key=None, payload_length=None, payload=b''):
        self.fin = fin
        self.rsv = rsv
        self.opcode = opcode
        self.mask = mask
    ...
    def unpack_header(self, header):
        ...
        """
        self.fin = header[0] >> 7
        self.rsv = (header[0] >> 6 & 1,
         header[0] >> 5 & 1,
         header[0] >> 4 & 1)
        self.opcode = header[0] & 15
        if not self.is_opcode(self.opcode):
            raise RFCError(RFCError.ERROR_INVALID_OPCODE, 'Invalid opcode ({})'.format(self.opcode))
        self.mask = header[1] >> 7
        self.payload_length = header[1] & 127
    ...
    def pack(self):
        if self.opcode is None:
            raise RFCError(RFCError.ERROR_INVALID_OPCODE, 'Opcode not set')
        else:
            if not self.is_opcode(self.opcode):
                raise RFCError(RFCError.ERROR_INVALID_OPCODE, 'Invalid opcode ({})'.format(self.opcode))
            ...
            if payload_length < 126:
                ...
                if payload_length < 65536:
                    ...
                    if payload_length < 18446744073709551616:
                        ...
                        raise RFCError(RFCError.ERROR_PAYLOAD_TOO_LARGE, 'Payload too large ({})'.format(payload_length))
    ...
    def __str__(self):
        ...
        if len(payload) < len(self.payload):
            payload += '...'
        return '<{}Frame opcode={}, payload_length={}, payload={}, mask={}, key={}>'.format(OPCODES[self.opcode], self.opcode, self.payload_length, payload, self.mask, self.masking_key)
    ...
    def close_frame(self, status_code, payload=b''):
        ...
        if not self.is_status_code(status_code):
            raise RFCError(RFCError.ERROR_INVALID_STATUS_CODE, 'Invalid status code ({})'.format(status_code))
...
class Handshake:
    ...
    BASE_SERVER_HANDSHAKE = HttpResponse(status_code=b'101 Switching Protocols', headers={b'Upgrade': b'websocket', 
     b'Connection': b'Upgrade'})
    BASE_CLIENT_HANDSHAKE = HttpRequest(method=b'GET', headers={b'Upgrade': b'websocket', 
     b'Connection': b'Upgrade', 
    ...
    def pack_server_handshake(self, client_handshake=None, client_key=None, additional_headers=None):
        if client_handshake:
            try:
                ...
                raise RFCError(RFCError.ERROR_INVALID_HANDSHAKE, 'Invalid client handshake ({})'.format(err))

            try:
                client_key = request.headers[self.CLIENT_KEY_HEADER]
            except KeyError:
                raise RFCError(RFCError.ERROR_INVALID_HANDSHAKE, 'Invalid client handshake, no key header found')
