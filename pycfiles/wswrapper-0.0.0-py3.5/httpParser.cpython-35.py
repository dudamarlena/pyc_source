...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wswrapper/httpParser.py
# Compiled at: 2016-07-04 19:03:30
...
' will make an entry in the header dict
        ...
        if len(key_value) != 2:
            raise HttpError(HttpError.ERROR_INVALID_HEADER_LINE, 'Invalid header line ({})'.format(header))
...
class HttpRequest(Http):
    ...
    def __init__(self, method=b'GET', uri=b'/', http_version=b'HTTP/1.1', headers=None, body=b''):
        self.method = method
        self.uri = uri
    ...
    def unpack(self, request):
        ...
        if len(lines) == 0:
            raise HttpError(HttpError.ERROR_EMPTY_HTTP, 'Request is emtpy')
        header = lines[0].split()
        if len(header) != 3:
            raise HttpError(HttpError.ERROR_INVALID_REQUEST, 'Invalid request line ({})'.format(lines[0]))
        if header[0] not in REQUEST_METHODS:
            raise HttpError(HttpError.ERROR_INVALID_REQUEST, 'Invalid request method ({})'.format(header[0]))
    ...
    def pack(self):
        if None in [self.method, self.uri, self.http_version]:
            raise HttpError(HttpError.ERROR_INVALID_REQUEST, "Can't pack while method, uri or http_version i still None type")
        request = (b' ').join([self.method, self.uri, self.http_version]) + b'\r\n'
        request += self.pack_headers_and_body()
        return request

    def __str__(self):
        return '<HttpRequest, method={}, uri={}, version={}>'.format(self.method, self.uri, self.http_version)


class HttpResponse(Http):

    def __init__(self, http_version=b'HTTP/1.1', status_code=b'200 OK', headers=None, body=b''):
        Http.__init__(self, headers, body)
    ...
    def unpack(self, response):
        ...
        if len(lines) == 0:
            raise HttpError(HttpError.ERROR_EMPTY_HTTP, 'Response is emtpy')
        header = lines[0].split()
        if len(header) < 2:
            raise HttpError(HttpError.ERROR_INVALID_RESPONSE, 'Response line is missing a status code')
    ...
    def pack(self):
        if None in [self.http_version, self.status_code]:
            raise HttpError(HttpError.ERROR_INVALID_RESPONSE, "Can't pack while http_version or status_code i still None type")
        request = (b' ').join([self.http_version, self.status_code]) + b'\r\n'
        request += self.pack_headers_and_body()
        return request

    def __str__(self):
        return '<HttpResponse, version={}, status={}>'.format(self.http_version, self.status_code)
