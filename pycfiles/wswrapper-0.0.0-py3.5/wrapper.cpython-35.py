...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wswrapper/wrapper.py
...
class Wrapper:
    ...
    STATE_CLOSING = 2
    STATE_CLOSED = 3
    STATES = {STATE_CONNECTING: 'Connecting', 
     STATE_OPEN: 'Open', 
    ...
    def do_handshake_as_client(self, uri=b'/', headers=None):
        ...
        request = Handshake().pack_client_handshake(client_key=key, uri=uri, additional_headers=headers)
        self.sock.send(request)
        response = self.sock.recv(1024)
        try:
            response = HttpResponse().unpack(response)
        except HttpError as err:
            raise WSError(WSError.ERROR_INVALID_HANDSHAKE_RESPONSE, 'Received invalid handshake response from server ({})'.format(err))

        try:
            server_key = response.headers[Handshake.SERVER_KEY_HEADER]
        except KeyError:
            raise WSError(WSError.ERROR_INVALID_HANDSHAKE_RESPONSE, 'Did not find server key header in handshake response')

        if server_key != Handshake.get_server_key(key):
            raise WSError(WSError.ERROR_INVALID_HANDSHAKE_RESPONSE, 'Received invalid server response key during WS handshake')
    ...
    def do_handshake_as_server(self, headers=None):
        ...
        try:
            ...
            raise WSError(WSError.ERROR_INVALID_HANDSHAKE_REQUEST, 'Received invalid handshake request from client ({})'.format(err))

        try:
            client_key = request.headers[Handshake.CLIENT_KEY_HEADER]
        except KeyError:
            raise WSError(WSError.ERROR_INVALID_HANDSHAKE_REQUEST, 'Did not find client key header in handshake request')

        try:
            response = Handshake().pack_server_handshake(client_key=client_key, additional_headers=headers)
        except RFCError as err:
            raise WSError(WSError.ERROR_INVALID_HANDSHAKE_REQUEST, 'Received invalid client handshake request ({})'.format(err))
    ...
    def recv_frame_info(self):
        ...
        if len(frame_header) != 2:
            raise WSError(WSError.ERROR_FRAME_INFO_FAILED, 'Failed to receive frame header')
        try:
            frame = Frame().unpack_header(frame_header)
        except RFCError as err:
            raise WSError(WSError.ERROR_FRAME_INFO_FAILED, 'Failed to process frame header ({})'.format(err.args[1]))
        ...
        if frame.extended_payload_byte_length != 0:
            ...
            if len(extended_payload_bytes) != frame.extended_payload_byte_length:
                raise WSError(WSError.ERROR_FRAME_INFO_FAILED, 'Failed to receive extended payload length')
            frame.set_payload_length(extended_payload_bytes)
        if frame.mask:
            frame.masking_key = self.sock.recv(4)
            if len(frame.masking_key) != 4:
                raise WSError(WSError.ERROR_FRAME_INFO_FAILED, 'Failed to receive masking key')
    ...
    def recv(self, buffersize, flags=0):
        ...
        if self.current_frame is None:
            try:
                ...
                if err.args[0] == errno.EWOULDBLOCK:
                    raise WSError(WSError.ERROR_FRAME_INFO_FAILED, 'Frame info missing in buffer')
                else:
                    raise
        ...
        if frame.opcode == Frame.OPCODE_CLOSE:
            ...
            return b''
        if frame.opcode in [Frame.OPCODE_TEXT, Frame.OPCODE_BINARY]:
            return self.recv_payload(buffersize)
        raise WSError(WSError.ERROR_FRAME_INFO_FAILED, 'Unimplemented opcode')

    def send(self, data, opcode=Frame.OPCODE_TEXT):
        if self.state not in [self.STATE_CLOSING, self.STATE_OPEN]:
            raise WSError(WSError.ERROR_STATE_ERROR, "Can't send websocket frame while websocket is in {} state".format(self.STATES[self.state]))
        frame = Frame(opcode=opcode, payload=data, mask=self.is_client)
        ...
        if self.state != self.STATE_CONNECTING:
            if not self.close_frame_sent:
                if self.close_payload:
                    close_frame = Frame(opcode=Frame.OPCODE_CLOSE, payload=self.close_payload).pack()
                else:
