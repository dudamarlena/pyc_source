...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/monitor/views/api/cl_item.py
# Compiled at: 2016-06-27 04:20:00
# Size of source mod 2**32: 5283 bytes
import aiozmq
from aiozmq import rpc
import asyncio, logging, msgpack
...
def cl_item_list(request):
    ...
    return get_list(Item, request.GET, sqla_session=get_session(request), record_wrapper=record_wrapper)
...
def cl_item_read(request):
    ...
    """
    res = _send_item_request(request, {'action': 'get_item_details', 
...
def cl_item_patch(request):
    ...
    if out_data:
        return _send_item_request(request, {'action': 'clear_item', 
         'item_id': request.context.record_id, 
         'values': out_data})
    return False
...
def _ensure_item_clearing_event_type(request):
    ...
    """
    session = DBSession()
    if session.query(EventType).filter(EventType.name == DATA_CLEARING_EVENT_TYPE).count() == 0:
        event_type = EventType()
        event_type.description = 'Event type to carry Xbus requests related to data clearing items.'
...
def _send_item_request_(front_url, login, password, data, loop):
    ...
    event_id = yield from client.call.start_event(token, envelope_id, DATA_CLEARING_EVENT_TYPE, 0)
    log.debug('Event created: %s', event_id)
    yield from client.call.send_item(token, envelope_id, event_id, msgpack.packb(data, use_bin_type=True))
