...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/monitor/views/api/event_type.py
# Compiled at: 2016-06-27 04:20:00
# Size of source mod 2**32: 6777 bytes
...
def event_type_rel_add(request):
    ...
    if rel is None or rel_list is None or not hasattr(rel_list, 'append'):
        raise HTTPBadRequest(json_body={'error': 'Relationship {} does not exist'.format(rel_name)})
    query = DBSession.query(rel.mapper)
    added_record = query.get(rid)
    if added_record is None:
        raise HTTPNotFound(json_body={'error': 'Event node ID {id} not found'.format(id=rid)})
    if added_record not in rel_list:
        rel_list.append(added_record)
    else:
        raise HTTPBadRequest(json_body={'error': 'Object is already in the relationship'})
...
def event_type_rel_delete(request):
    ...
    if removed_record in rel_list:
        ...
        raise HTTPBadRequest(json_body={'error': 'Object is not in the relationship'})
    return Response(status_int=204, json_body={})
...
@view_config(route_name='event_type_graph', renderer='json', http_cache=0)
def event_type_graph(request):
    ...
    while nodes:
        if i >= len(nodes):
            i = 0
        if loop >= len(nodes):
            raise HTTPInternalServerError(json_body={'error': 'Invalid event graph', 
             'partial_res': res, 
