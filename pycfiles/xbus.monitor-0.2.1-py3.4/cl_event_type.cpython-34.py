...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/monitor/views/api/cl_event_type.py
...
def cl_event_type_rel_add(request):
    ...
    if added_record is None:
        raise HTTPNotFound(json_body={'error': 'Event type ID {id} not found'.format(id=rid)})
...
def cl_event_type_rel_list(request):
    ...
    if rel is None or rel_list is None or not hasattr(rel_list, 'filter'):
        raise HTTPBadRequest(json_body={'error': 'Relationship {} does not exist'.format(rel_name)})
    return get_list(rel.mapper, request.GET, rel_list, sqla_session=get_session(request))
