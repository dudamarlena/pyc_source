...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyrrd\rrd.py
# Compiled at: 2014-01-12 21:44:33
import re
...
class RRD(mapper.RRDMapper):
    ...
    def __init__(self, filename=None, start=None, step=300, ds=None, rra=None, mode='w', backend=external):
    ...
    def fetch(self, cf='AVERAGE', resolution=None, start=None, end=None, returnStyle='ds', useBindings=False):
        """
    ...
    def last(self):
        ...
        This method is identical to self.lastupdate in most cases, but will use
        'rrdtool last' command if self.lastupdate is undefined.
        ...
        >>> rrd2 = RRD(rrdfile.name)

        # Compare 'lastudpate' attributes. This should fail because the
        # attribute is only defined after load() or buffferValue()
        >>> rrd2.lastupdate == rrd.lastupdate
        False

        # Compare last() methods. They should be equal because it forces a
        # datbase lookup.
        >>> rrd2.last() == rrd.last()
        True
        """
        if not self.lastupdate:
            self.lastupdate = float(self.backend.last(self.filename))
...
class DataSource(mapper.DSMapper):
    ...
    def __init__(self, dsName=None, dsType=None, heartbeat=None, minval='U', maxval='U', rpn=None):
...
class RRA(mapper.RRAMapper):
    ...
    def __init__(self, cf=None, xff=None, steps=None, rows=None, alpha=None, beta=None, seasonal_period=None, rra_num=None, gamma=None, threshold=None, window_length=None, cdpPrepObject=None, databaseObject=None):
