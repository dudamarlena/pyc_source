...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyrrd\graph.py
...
def validateImageFormat(format):
    ...
    if format in valid:
        return format
    valid = (' ').join(valid)
    raise ValueError, 'The image format must be one of the ' + 'following: %s' % valid
...
class DataDefinition(object):
    ...
    def __init__(self, vname='', rrdfile='', dsName='', cdef='AVERAGE', step=None, start=None, end=None, reduce=None):
    ...
    def __repr__(self):
        ...
        """
        if not (self.vname and self.rrdfile and self.dsName and self.cdef):
            msg = 'vname, rrdfile, dsName, and cdef ' + 'are all required attributes and cannot be None.'
            raise ValueError, msg
        main = 'DEF:%(vname)s=%(rrdfile)s:%(dsName)s:%(cdef)s' % self.__dict__
        tail = ''
        if self.step:
...
class VariableDefinition(object):
    ...
    use with RRDTool.
        ...
        """
        main = self.abbr + ':%(vname)s=%(rpn)s' % self.__dict__
        return main
...
class Line(object):
    ...
    def __init__(self, width=None, value=None, defObj=None, color=None, legend='', stack=False):
        """
        ...
        if value:
            if not (isinstance(value, str) or isinstance(value, int)):
                raise ValueError, "The parameter 'value' must be " + 'either a string or an integer.'
        elif not defObj:
            raise Exception, 'You must provide either a value ' + 'or a definition object.'
        else:
            value = defObj.vname
        self.vname = value
        self.abbr = 'LINE'
...
class Area(Line):
    ...
    def __init__(self, width=None, value=None, defObj=None, color=None, legend='', stack=False):
        ...
        be automatically extraced from the object and used.
        """
        super(Area, self).__init__(value=value, defObj=defObj, color=color, legend=legend, stack=stack)
        self.abbr = 'AREA'
    ...
    def __init__(self, defObj=None, color=None, fraction=None, legend=''):
        ...
        if fraction:
            if not (isinstance(fraction, float) or isinstance(fraction, int)):
                raise TypeError, "The parameter 'fraction' must" + 'be a float value between 0 and 1.'
            elif 0 <= fraction <= 1:
                value2 = fraction
            else:
                raise ValueError, "The parameter 'fraction' must" + 'be a value between 0 and 1.'
        if not color:
...
class ColorAttributes(object):
    ...
    def __init__(self, background=None, canvas=None, lefttop_border=None, rightbottom_border=None, major_grid=None, font=None, axis=None, frame=None, arrow=None):
        """
