...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/mocha/mocha/contrib/auth/models.py
# Compiled at: 2017-10-28 21:29:04
import datetime, arrow
from . import exceptions
from mocha import db, utils, send_mail, bcrypt, _
...
class AuthUser(db.Model):
    ...
    @classmethod
    def new(cls, username, password, email=None, first_name='', last_name='', login_method=None, role='MEMBER'):
        """
        Create a new user
        :param username: str
        :param password: str
...
class AuthUserFederation(db.Model):
    ...
    def get_user(cls, provider, federated_id):
        login = cls.query().filter(cls.provider == provider).filter(cls.federated_id == federated_id).first()
        if login:
            return login.user
        else:
            return
