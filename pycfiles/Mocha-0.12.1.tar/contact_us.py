...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/mocha/mocha/contrib/views/contact_us.py
# Compiled at: 2017-06-04 16:07:38
"""
Contact Page
"""
from mocha import Mocha, _, config, url_for, abort, request, utils, flash_success, flash_error, flash_data, get_flash_data, send_mail, recaptcha, page_attr, redirect, decorators as deco, db, render, exceptions
import mocha.contrib, mocha.contrib.app_data as app_data, logging
__version__ = '1.0.0'
__options__ = {}
...
class Main(Mocha):
    ...
    def _register(cls, app, **kwargs):
        ...
        data = {'recipients': __options__.get('recipients'), 
    ...
    def page(self):
        recipients = app_data.get(APP_DATA_KEY, 'recipients') or __options__.get('recipients') or config('CONTACT_EMAIL')
        if not recipients:
            abort(500, 'ContactPage missing email recipient')
        success_message = app_data.get(APP_DATA_KEY, 'success_message', __options__.get('success_message'))
...
class Admin(Mocha):
    ...
    def index(self):
        ...
        return {'recipients': adata.get('recipients'), 
    ...
    def post(self):
        ...
        if not recipients or not success_message:
            ...
            data = {'recipients': recipients, 'success_message': success_message}
