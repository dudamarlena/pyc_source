...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/mocha/mocha/core.py
...
from flask import Flask, g, render_template, flash, session, make_response, Response, request as f_request, abort, url_for as f_url_for, redirect as f_redirect
__all__ = [
 'Mocha',
 'Brew',
 'db',
...
def config(key, default=None):
    ...
    """
    if Mocha._app:
        return Mocha._app.config.get(key, default)
    return default
...
class Mocha(object):
    ...
    def _expose_models(cls):
        ...
        """
        if db._IS_OK_:
            register_models(**{m.__name__:m for m in db.Model.__subclasses__() if not hasattr(models, m.__name__) if not hasattr(models, m.__name__)})
