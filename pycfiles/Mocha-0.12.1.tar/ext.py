...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/mocha/mocha/ext.py
# Compiled at: 2017-06-17 17:08:09
"""
Extensions
"""
import re, six, logging
...
from . import Mocha, init_app, utils, exceptions, g, config, signals
...
def _session(app):
    ...
    if uri:
        ...
        if 'redis' in scheme:
            ...
            raise exceptions.MochaError("Invalid Session Store. '%s' provided" % scheme)
    if store:
    ...
    def send(self, to, subject=None, body=None, reply_to=None, template=None, **kwargs):
        ...
        if not isinstance(to, list):
            ...
            kwargs.update({'subject': subject, 
               'body': body, 
               'reply_to': reply_to})
            raise (self.validated or exceptions.MochaError)('Mail configuration error')
...
def download_file(filename, object_name=None, content=None, as_attachment=True, timeout=60):
    ...
    if content:
        ...
        buff.write(content)
        buff.seek(0)
        return send_file(buff, attachment_filename=filename, as_attachment=as_attachment)
