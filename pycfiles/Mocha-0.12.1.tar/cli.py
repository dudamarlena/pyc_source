...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/mocha/mocha/cli.py
# Compiled at: 2017-07-09 20:33:21
r"""
...
def add_view(name, no_template):
    ...
    if os.path.isfile(viewdest) or os.path.isfile(tpldest):
        ...
        with open(viewdest, 'r+') as (vd):
            content = vd.read().replace('%ROUTE%', name.lower()).replace('%NAV_TITLE%', name.capitalize())
            vd.seek(0)
            vd.write(content)
            vd.truncate()
        if not no_template:
