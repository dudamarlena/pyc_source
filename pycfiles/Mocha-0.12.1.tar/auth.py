...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/mocha/mocha/contrib/views/auth.py
# Compiled at: 2017-10-28 21:55:08
...
from mocha import Mocha, _, g, models, page_attr, request, redirect, flash_success, flash_error, flash_info, abort, recaptcha, decorators as h_deco, exceptions as mocha_exc, exceptions, utils, paginate, views, url_for, session, upload_file, delete_file, render
import mocha.contrib
from mocha.contrib.auth import ACTIONS, oauth, is_authenticated, not_authenticated, get_user_by_action_token, session_set_require_password_change, visible_to_managers, authenticate, with_federation, create_session, UserModel, create_user, exceptions, __options__ as auth_options, login_manager, decorators as deco
        ...
        if request.method == 'POST':
            try:
                ...
                if __options__.get('registration_username'):
                    ...
                    login_method = 'email'
                if not first_name:
                    raise mocha_exc.AppError(_('First Name or Name is required'))
                else:
                    ...
                    if with_oauth and oauth_provider and oauth_user_id:
                        ...
                        create_session(user)
                        return redirect(request.form.get('next') or views.auth.Account.account_settings)
    ...
    def _register_oauth_session_user(self, user):
        ...
        if oauth_session:
            if 'provider' in oauth_session and 'user_id' in oauth_session:
                user.add_federated_login(provider=oauth_session.get('provider'), federated_id=oauth_session.get('user_id'))
        ...
        **** This methods doesn't save the user token, it only retrieves the ID
        ...
        if action not in valid_actions or 'oauth' not in __options__.get('registration_methods') or not __options__.get('allow_registration') or not hasattr(oauth, provider):
            ...
            if action == 'authorized' and oauth_user_id:
                if is_authenticated():
                    try:
                        current_user.add_federated_login(provider=provider, federated_id=oauth_user_id)
    ...
    def update_info(self):
        ...
        try:
            ...
            if file:
                ...
                prefix += '/'
                profile_image = upload_file(None, file, name=utils.guid(), prefix=prefix, public=True, extensions=[
                 'jpg', 'jpeg',
                 'png', 'gif'])
        ...
        try:
            ...
            if action == 'info':
                ...
                if user.profile_image is not None:
                    ...
                    user.update_info(profile_image=None, _action=ACTIONS['PROFILE_IMAGE'])
            ...
            try:
                ...
                if name and level:
                    ...
                    if level in _levels or name in _names:
                        raise exceptions.AuthError("Can't modify PRIMARY Roles - name: %s, level: %s " % (
                         name, level))
                    elif id:
                        ...
                        if role:
                            if action == 'delete':
                                ...
                                if role.level != level and models.AuthUserRole.get_by_level(level):
                                    ...
                                    flash_success("Role '%s (%s)' updated successfully" % (
                                     name, level))
                        else:
                        ...
                        flash_success("New Role '%s (%s)' addedd successfully" % (
                         name, level))
            except exceptions.AuthError as ex:
