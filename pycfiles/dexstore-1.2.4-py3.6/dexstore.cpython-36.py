...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexstore/dexstore.py
# Compiled at: 2019-03-20 04:11:58
# Size of source mod 2**32: 57270 bytes
import logging
...
class DexStore(AbstractGrapheneChain):
    ...
    def transfer(self, to, amount, asset, memo='', account=None, **kwargs):
        ...
        op = (operations.Transfer)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         from=account['id'], 
         to=to['id'], 
 ...
 'asset_id':amount.asset['id']}, 
    ...
    def create_account(self, account_name, registrar=None, referrer='1.2.35641', referrer_percent=50, owner_key=None, active_key=None, memo_key=None, owner_account=None, active_account=None, password=None, additional_owner_keys=[], additional_active_keys=[], additional_owner_accounts=[], additional_active_accounts=[], proxy_account='proxy-to-self', storekeys=True, **kwargs):
        ...
        if not registrar:
            if self.config['default_account']:
                ...
                if not registrar:
                    raise ValueError('Not registrar account given. Define it with registrar=x, or set the default_account using uptick')
                if password:
            ...
            if password:
                ...
                owner_key_authority = [
                 [
                  format(owner_pubkey, self.prefix), 1]]
                ...
                raise ValueError('Call incomplete! Provide either a password, owner/active public keys or owner/active accounts + memo key!')
    ...
    def upgrade_account(self, account=None, **kwargs):
        ...
        if not account:
            raise ValueError('You need to provide an account')
        account = Account(account, blockchain_instance=self)
        op = (operations.Account_upgrade)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
    ...
    def allow(self, foreign, weight=None, permission='active', account=None, threshold=None, **kwargs):
        ...
        op = (operations.Account_update)(**{'fee': {'amount':0,  'asset_id':'1.3.0'}, 
         'account': account['id'], 
    ...
    def disallow(self, foreign, permission='active', account=None, threshold=None, **kwargs):
        ...
        if not account:
            if 'default_account' in self.config:
                ...
                raise affected_items or ValueError('Changes nothing!')
    ...
    def update_memo_key(self, key, account=None, **kwargs):
        ...
        op = (operations.Account_update)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         account=account['id'], 
    ...
    def approvewitness(self, witnesses, account=None, **kwargs):
        ...
        if not account:
            ...
            witnesses = isinstance(witnesses, (list, set, tuple)) or {
             witnesses}
        ...
        options['num_witness'] = len(list(filter(lambda x: float(x.split(':')[0]) == 1, options['votes'])))
 ...
 'asset_id':'1.3.0'}, 
    ...
    def approvecommittee(self, committees, account=None, **kwargs):
        ...
        if not account:
            ...
            account = Account(account, blockchain_instance=self)
            options = account['options']
            committees = isinstance(committees, (list, set, tuple)) or {
             committees}
 ...
 'asset_id':'1.3.0'}, 
    ...
    def disapproveproposal(self, proposal_ids, account=None, approver=None, **kwargs):
        ...
        for proposal_id in proposal_ids:
            proposal = Proposal(proposal_id, blockchain_instance=self)
            op.append((operations.Proposal_update)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
             fee_paying_account=account['id'], 
             proposal=proposal['id'], 
 ...
 approver['id']], 
    ...
    def approveworker(self, workers, account=None, **kwargs):
        ...
        if not account:
            ...
            account = Account(account, blockchain_instance=self)
            options = account['options']
            workers = isinstance(workers, (list, set, tuple)) or {
             workers}
 ...
 'asset_id':'1.3.0'}, 
    ...
    def cancel(self, orderNumbers, account=None, **kwargs):
        ...
        if not account:
            ...
            orderNumbers = isinstance(orderNumbers, (list, set, tuple)) or {
             orderNumbers}
        op = []
        for order in orderNumbers:
            op.append((operations.Limit_order_cancel)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
             fee_paying_account=account['id'], 
             order=order, 
             extensions=[], prefix=self.prefix))
    ...
    def vesting_balance_withdraw(self, vesting_id, amount=None, account=None, **kwargs):
        ...
        if not account:
            ...
            if not account:
                raise ValueError('You need to provide an account')
            account = Account(account, blockchain_instance=self)
            obj = amount or Vesting(vesting_id, blockchain_instance=self)
            amount = obj.claimable
        op = (operations.Vesting_balance_withdraw)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
 ...
 'asset_id':amount['asset']['id']}, 
    ...
    def publish_price_feed(self, symbol, settlement_price, cer=None, mssr=110, mcr=200, account=None):
        ...
        if not mcr > 100:
            ...
            if not isinstance(settlement_price, Price):
                raise AssertionError('settlement_price needs to be instance of `dexstore.price.Price`!')
            else:
                if not cer is None:
                    assert isinstance(cer, Price), 'cer needs to be instance of `dexstore.price.Price`!'
                    if not account:
                        if 'default_account' in self.config:
                            account = self.config['default_account']
                    if not account:
                    ...
                    if not asset['id'] == settlement_price['base']['asset']['id']:
                        if not asset['id'] == settlement_price['quote']['asset']['id']:
                            raise AssertionError("Price needs to contain the asset of the symbol you'd like to produce a feed for!")
                    assert asset.is_bitasset, 'Symbol needs to be a bitasset!'
                    if not settlement_price['base']['asset']['id'] == backing_asset:
                        assert settlement_price['quote']['asset']['id'] == backing_asset, 'The Price needs to be relative to the backing collateral!'
        ...
        op = (operations.Asset_publish_feed)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         publisher=account['id'], 
         asset_id=asset['id'], 
         feed={'settlement_price':settlement_price.as_base(symbol).json(), 
 ...
 'maintenance_collateral_ratio':int(mcr * 10)}, 
    ...
    def update_cer(self, symbol, cer, account=None):
        ...
        """
        if not isinstance(cer, Price):
            raise AssertionError('cer needs to be instance of `dexstore.price.Price`!')
        else:
            if not account:
                if 'default_account' in self.config:
                    account = self.config['default_account']
                ...
                account = Account(account, blockchain_instance=self)
                asset = Asset(symbol, blockchain_instance=self, full=True)
                if not asset['id'] == cer['base']['asset']['id']:
                    assert asset['id'] == cer['quote']['asset']['id'], "Price needs to contain the asset of the symbol you'd like to produce a feed for!"
            cer = cer.as_base(symbol)
            if cer['quote']['asset']['id'] != '1.3.0':
                raise ValueError("CER must be defined against core asset '1.3.0'")
        options = asset['options']
        options.update({'core_exchange_rate': cer.as_base(symbol).json()})
        op = (operations.Asset_update)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         issuer=account['id'], 
    ...
    def update_witness(self, witness_identifier, url=None, key=None, **kwargs):
        ...
        op = (operations.Witness_update)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         prefix=self.prefix, 
         witness=witness['id'], 
    ...
    def reserve(self, amount, account=None, **kwargs):
        ...
        if not isinstance(amount, Amount):
            ...
            if not account:
                if 'default_account' in self.config:
                    account = self.config['default_account']
            raise account or ValueError('You need to provide an account')
        account = Account(account, blockchain_instance=self)
        op = (operations.Asset_reserve)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         payer=account['id'], 
         amount_to_reserve={'amount':int(amount), 
 'asset_id':amount['asset']['id']}, 
         extensions=[])
        return (self.finalizeOp)(op, account, 'active', **kwargs)

    def create_worker(self, name, daily_pay, end, url='', begin=None, payment_type='vesting', pay_vesting_period_days=0, account=None, **kwargs):
        """ Create a worker
        ...
        if not daily_pay['asset']['id'] == '1.3.0':
            raise AssertionError
        elif not begin:
 ...
 'asset_id':'1.3.0'}, 
    ...
    def fund_fee_pool(self, symbol, amount, account=None, **kwargs):
        ...
        op = (operations.Asset_fund_fee_pool)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
    ...
    def create_committee_member(self, url='', account=None, **kwargs):
        ...
        if not account:
            raise ValueError('You need to provide an account')
        account = Account(account, blockchain_instance=self)
        op = (operations.Committee_member_create)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         committee_member_account=account['id'], 
         url=url)
        return (self.finalizeOp)(op, account, 'active', **kwargs)

    def account_whitelist(self, account_to_whitelist, lists=[
 'white'], account=None, **kwargs):
        ...
        op = (operations.Account_whitelist)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
    ...
    def bid_collateral(self, additional_collateral, debt_covered, account=None, **kwargs):
        if not account:
            if 'default_account' in self.config:
                account = self.config['default_account']
            if not account:
            ...
            raise isinstance(debt_covered, Amount) or ValueError('debt_covered must be of type Amount')
        op = (operations.Bid_collateral)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         bidder=account['id'], 
    ...
    def asset_settle(self, amount, account=None, **kwargs):
        if not account:
            ...
            if not account:
                raise ValueError('You need to provide an account')
            account = Account(account, blockchain_instance=self)
            raise isinstance(amount, Amount) or ValueError("'amount' must be of type Amount")
        op = (operations.Asset_settle)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         account=account['id'], 
         amount=amount.json(), 
         extensions=[])
    ...
    def htlc_create(self, amount, to, preimage, hash_type='ripemd160', account=None, expiration=3600, **kwargs):
        import hashlib
        ...
        if not account:
            ...
            if hash_type == 'ripemd160':
                preimage_type = 0
                preimage_hash = hexlify(ripemd160(hexlify(bytes(preimage, 'utf-8')))).decode('ascii')
            else:
                if hash_type == 'sha1':
                    preimage_type = 1
                    ...
                    if hash_type == 'sha256':
                        ...
                        raise ValueError("Unknown 'hash_type'. Must be 'sha1', 'sha256', or 'ripemd160'")
        op = (operations.Htlc_create)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         from=account['id'], 
         to=to['id'], 
         amount=amount.json(), 
         preimage_hash=[
 preimage_type, preimage_hash], 
    ...
    def htlc_redeem(self, htlc_id, preimage, account=None, **kwargs):
        ...
        if not account:
            account = htlc['to']
        account = Account(account, blockchain_instance=self)
        op = (operations.Htlc_redeem)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         redeemer=account['id'], 
