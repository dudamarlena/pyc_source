...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexstore/dex.py
...
class Dex(BlockchainInstance):
    ...
    def returnFees(self):
        ...
        for f in fees:
            ...
            fs = f[1]
            for _type in fs:
                fs[_type] = float(fs[_type]) * scale / 10000.0 / 10 ** base['precision']
    ...
    def list_debt_positions(self, account=None):
        ...
        for debt in account.get('call_orders'):
            ...
            r[quote['symbol']] = {'collateral':collateral_amount, 
             'debt':debt_amount, 
    ...
    def close_debt_position(self, symbol, account=None):
        ...
        op = (operations.Call_order_update)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         delta_debt={'amount':int(-float(debt['debt']) * 10 ** asset['precision']), 
 'asset_id':asset['id']}, 
         delta_collateral={'amount':int(-float(debt['collateral']) * 10 ** collateral_asset['precision']), 
 'asset_id':collateral_asset['id']}, 
    ...
    def adjust_debt(self, delta, new_collateral_ratio=None, account=None, target_collateral_ratio=None):
        ...
        if not account:
            ...
            settlement_price = Price((bitasset['current_feed']['settlement_price']),
              blockchain_instance=(self.blockchain))
            if symbol in current_dedst:
                amount_of_collateral = (float(current_dedst[symbol]['debt']) + float(delta['amount'])) * new_collateral_ratio / float(settlement_price)
            ...
            if fundsHave <= fundsNeeded:
                raise ValueError('Not enough funds available. Need %f %s, but only %f %s are available' % (
                 ...
                 fundsHave,
                 collateral_asset['symbol']))
            payload = {'fee':{'amount':0, 
              'asset_id':'1.3.0'}, 
             'delta_debt':{'amount':int(float(delta) * 10 ** asset['precision']), 
              'asset_id':asset['id']}, 
             'delta_collateral':{'amount':int(float(amount_of_collateral) * 10 ** collateral_asset['precision']), 
              'asset_id':collateral_asset['id']}, 
             'funding_account':account['id'], 
             'extensions':{}}
            if target_collateral_ratio:
                payload['extensions'].update(dict(target_collateral_ratio=(int(target_collateral_ratio * 100))))
    ...
    def adjust_collateral_ratio(self, symbol, new_collateral_ratio, account=None, target_collateral_ratio=None):
        ...
        return self.adjust_debt((Amount(0, symbol)),
    ...
    def borrow(self, amount, collateral_ratio=None, account=None, target_collateral_ratio=None):
        ...
        """
        return self.adjust_debt(amount,
          collateral_ratio,
          account,
          target_collateral_ratio=target_collateral_ratio)