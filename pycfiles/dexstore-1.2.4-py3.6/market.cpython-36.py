...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexstore/market.py
# Compiled at: 2019-03-20 14:33:32
# Size of source mod 2**32: 25453 bytes
...
class Market(dict):
    ...
    def ticker(self):
        ...
        if 'bitasset_data_id' in self['quote']:
            ...
            if backing_asset_id == self['base']['id']:
                ...
                data['quoteSettlement_price'] = Price(sp,
                  blockchain_instance=(self.blockchain))
                if sp['base']['asset_id'] == self['quote']['id']:
                    data['quoteSettlement_price'] = data['quoteSettlement_price'].invert()
        else:
            if 'bitasset_data_id' in self['base']:
                bitasset = self.blockchain.rpc.get_object(self['base']['bitasset_data_id'])
                backing_asset_id = bitasset['options']['short_backing_asset']
                if backing_asset_id == self['quote']['id']:
                    data['baseSettlement_price'] = Price((bitasset['current_feed']['settlement_price']),
                      blockchain_instance=(self.blockchain))
            ticker = self.blockchain.rpc.get_ticker(self['base']['id'], self['quote']['id'])
            data['baseVolume'] = Amount((ticker['base_volume']),
              (self['base']), blockchain_instance=(self.blockchain))
            data['quoteVolume'] = Amount((ticker['quote_volume']),
              (self['quote']), blockchain_instance=(self.blockchain))
            data['lowestAsk'] = Price((ticker['lowest_ask']),
              base=(self['base']),
              quote=(self['quote']),
              blockchain_instance=(self.blockchain))
            data['highestBid'] = Price((ticker['highest_bid']),
              base=(self['base']),
              quote=(self['quote']),
              blockchain_instance=(self.blockchain))
    ...
    def volume24h(self):
        ...
        """
        volume = self.blockchain.rpc.get_24_volume(self['base']['id'], self['quote']['id'])
        return {self['base']['symbol']: Amount((volume['base_volume']),
                                   (self['base']), blockchain_instance=(self.blockchain)), 
         
         self['quote']['symbol']: Amount((volume['quote_volume']),
                                    (self['quote']),
                                    blockchain_instance=(self.blockchain))}
    ...
    def orderbook(self, limit=25):
        ...
        """
        orders = self.blockchain.rpc.get_order_book(self['base']['id'], self['quote']['id'], limit)
        asks = list(map(lambda x: Order((float(x['price'])),
          quote=Amount((x['quote']),
          (self['quote']), blockchain_instance=(self.blockchain)),
          base=Amount((x['base']),
          (self['base']), blockchain_instance=(self.blockchain)),
          blockchain_instance=(self.blockchain)), orders['asks']))
        bids = list(map(lambda x: Order((float(x['price'])),
          quote=Amount((x['quote']),
          (self['quote']), blockchain_instance=(self.blockchain)),
          base=Amount((x['base']),
          (self['base']), blockchain_instance=(self.blockchain)),
          blockchain_instance=(self.blockchain)), orders['bids']))
        data = {'asks':asks, 
         'bids':bids}
        return data
    ...
    def trades(self, limit=25, start=None, stop=None):
        ...
        while True:
            ...
            if not sequence:
                orders = self.blockchain.rpc.get_trade_history(self['base']['symbol'], self['quote']['symbol'], formatTime(stop), formatTime(start), first_run)
            else:
                continuous_limit = min(limit - cnt, 100)
                orders = self.blockchain.rpc.get_trade_history_by_sequence(self['base']['symbol'], self['quote']['symbol'], sequence, formatTime(start), continuous_limit)
            if len(orders) == 0:
                return
            for order in orders:
                cnt += 1
                yield FilledOrder(order,
                  quote=Amount((order['amount']),
                  (self['quote']),
                  blockchain_instance=(self.blockchain)),
                  base=Amount((float(order['amount']) * float(order['price'])),
                  (self['base']),
                  blockchain_instance=(self.blockchain)),
                  blockchain_instance=(self.blockchain))
                if cnt >= limit:
    ...
    def accounttrades(self, account=None, limit=25):
        ...
        filled = self.blockchain.rpc.get_fill_order_history((self['base']['id']),
          (self['quote']['id']), (2 * limit), api='history')
        trades = []
        for f in filled:
            if f['op']['account_id'] == account['id']:
                trades.append(FilledOrder(f,
                  base=(self['base']),
                  quote=(self['quote']),
                  blockchain_instance=(self.blockchain)))
    ...
    def accountopenorders(self, account=None):
        ...
        for o in orders:
            if o['sell_price']['base']['asset_id'] == self['base']['id'] and o['sell_price']['quote']['asset_id'] == self['quote']['id'] or o['sell_price']['base']['asset_id'] == self['quote']['id'] and o['sell_price']['quote']['asset_id'] == self['base']['id']:
                r.append(Order(o, blockchain_instance=(self.blockchain)))

        return r

    def buy(self, price, amount, expiration=None, killfill=False, account=None, returnOrderId=False, **kwargs):
        ...
        """
        if not expiration:
            expiration = self.blockchain.config['order-expiration'] or 31536000
        else:
            if not account:
                if 'default_account' in self.blockchain.config:
                    account = self.blockchain.config['default_account']
                ...
                if isinstance(amount, Amount):
                    ...
                    amount = Amount(amount,
                      (self['quote']['symbol']), blockchain_instance=(self.blockchain))
            order = (operations.Limit_order_create)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
             seller=account['id'], 
             amount_to_sell={'amount':int(round(float(amount) * float(price) * 10 ** self['base']['precision'])), 
 'asset_id':self['base']['id']}, 
             min_to_receive={'amount':int(round(float(amount) * 10 ** self['quote']['precision'])), 
 'asset_id':self['quote']['id']}, 
    ...
    def sell(self, price, amount, expiration=None, killfill=False, account=None, returnOrderId=False, **kwargs):
 ...
 'asset_id':'1.3.0'}, 
             seller=account['id'], 
             amount_to_sell={'amount':int(round(float(amount) * 10 ** self['quote']['precision'])), 
 'asset_id':self['quote']['id']}, 
             min_to_receive={'amount':int(round(float(amount) * float(price) * 10 ** self['base']['precision'])), 
 'asset_id':self['base']['id']}, 
             expiration=formatTimeFromNow(expiration), 
             fill_or_kill=killfill)
            if returnOrderId:
