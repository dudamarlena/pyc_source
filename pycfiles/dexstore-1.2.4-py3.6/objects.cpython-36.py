...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexstorebase/objects.py
...
from graphenebase.types import Optional, PointInTime, Set, Signature, Static_variant, String, Uint8, Uint16, Uint32, Uint64, Varint32, Void, VoteId
...
class Operation(GrapheneOperation):
    __doc__ = ' Need to overwrite a few attributes to load proper operations from\n        dexstore\n    '
    module = 'dexstorebase.operations'
    operations = operations


class ObjectId(GPHObjectId):
    __doc__ = ' Need to overwrite a few attributes to load proper object_types from\n        dexstore\n    '
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if len(args) == 1:
                if len(kwargs) == 0:
                    kwargs = args[0]
            kwargs['key_auths'] = sorted((kwargs['key_auths']),
              key=(lambda x: PublicKey((x[0]), prefix=prefix)),
              reverse=False)
            accountAuths = Map([[ObjectId(e[0], 'account'), Uint16(e[1])] for e in kwargs['account_auths']])
