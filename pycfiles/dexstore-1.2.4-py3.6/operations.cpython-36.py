...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexstorebase/operations.py
# Compiled at: 2019-03-20 04:13:33
# Size of source mod 2**32: 34974 bytes
import json
from collections import OrderedDict
from binascii import hexlify, unhexlify
from graphenebase.types import Array, Bool, Bytes, Fixed_array, Id, Int16, Int64, Map, Optional, PointInTime, Set, Signature, Static_variant, String, Uint8, Uint16, Uint32, Uint64, Varint32, Void, VoteId, Ripemd160, Sha1, Sha256
from .account import PublicKey
from .objects import AccountCreateExtensions, AccountOptions, Asset, AssetOptions, BitAssetOptions, CallOrderExtension, GrapheneObject, Memo, ObjectId, Operation, Permission, Price, PriceFeed, SpecialAuthority, Worker_initializer, isArgsThisClass
...
def getOperationClassForId(op_id):
    ...
    """
    if op_id in class_idmap:
        return class_idmap[op_id]


def getOperationIdForClass(name):
    """ Convert an operation classname into the corresponding id
    """
    if name in class_namemap:
        return class_namemap[name]
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if 'new_issuer' in kwargs:
                raise ValueError('Cannot change asset_issuer with Asset_update anylonger! (BSIP29)')
            super().__init__(OrderedDict([
             (
              'fee', Asset(kwargs['fee'])),
             (
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if len(args) == 1:
                if len(kwargs) == 0:
                    kwargs = args[0]
            elif 'memo' in kwargs and kwargs['memo']:
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if len(args) == 1:
                if len(kwargs) == 0:
                    kwargs = args[0]
            for o in ('active_approvals_to_add', 'active_approvals_to_remove', 'owner_approvals_to_add',
                      'owner_approvals_to_remove', 'key_approvals_to_add', 'key_approvals_to_remove'):
                if o not in kwargs:
                    kwargs[o] = []
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if len(args) == 1:
                if len(kwargs) == 0:
                    kwargs = args[0]
                if 'new_url' in kwargs:
                    if kwargs['new_url']:
                        new_url = Optional(String(kwargs['new_url']))
                new_url = Optional(None)
            elif 'new_signing_key' in kwargs and kwargs['new_signing_key']:
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if len(args) == 1:
                if len(kwargs) == 0:
                    kwargs = args[0]
            kwargs['new_feed_producers'] = sorted((kwargs['new_feed_producers']),
