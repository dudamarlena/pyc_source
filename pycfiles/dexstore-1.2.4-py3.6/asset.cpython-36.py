...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexstore/asset.py
# Compiled at: 2019-03-20 04:11:58
# Size of source mod 2**32: 19094 bytes
import json
from dexstorebase import operations
from dexstorebase.asset_permissions import asset_permissions, force_flag, test_permissions, todict
...
class Asset(GrapheneAsset):
    ...
    def max_market_fee(self):
        ...
        return Amount({'amount':self['options']['max_market_fee'], 
         'asset_id':self['id']})
    ...
    def get_settle_orders(self, limit=100):
        ...
        for settle in ret[:limit]:
            r.append({'account':Account(settle['owner'],
               lazy=True, blockchain_instance=self.blockchain), 
    ...
    def halt(self):
        ...
        """
        from .account import Account
        nullaccount = Account('null-account',
          blockchain_instance=(self.blockchain))
        ...
        options.update({'flags':flags_int, 
         'whitelist_authorities':[
          nullaccount['id']], 
         'blacklist_authorities':[],  'whitelist_markets':[
          self['id']], 
         'blacklist_markets':[]})
 ...
 'asset_id':'1.3.0'}, 
    ...
    def release(self, whitelist_authorities=[], blacklist_authorities=[], whitelist_markets=[], blacklist_markets=[]):
        ...
        flags_int = force_flag(options['flags'], flags)
        options.update({'flags':flags_int, 
         'whitelist_authorities':[Account(a)['id'] for a in whitelist_authorities], 
         'blacklist_authorities':[Account(a)['id'] for a in blacklist_authorities], 
 ...
 'asset_id':'1.3.0'}, 
    ...
    def seize(self, from_account, to_account, amount):
        ...
        op = (operations.Override_transfer)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         issuer=self['issuer'], 
    ...
    def add_authorities(self, type, authorities=[]):
        ...
        """
        if not type in ('blacklist', 'whitelist'):
            raise AssertionError
        else:
            ...
            options = self['options']
            if type == 'whitelist':
                options['whitelist_authorities'].extend([Account(a)['id'] for a in authorities])
            if type == 'blacklist':
                options['blacklist_authorities'].extend([Account(a)['id'] for a in authorities])
        op = (operations.Asset_update)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         issuer=self['issuer'], 
         asset_to_update=self['id'], 
    ...
    def add_markets(self, type, authorities=[], force_enable=True):
        ...
        if not type in ('blacklist', 'whitelist'):
            ...
            if force_enable:
                ...
                assert test_permissions(options['flags'], ['white_list']), 'whitelist feature not enabled'
            if type == 'whitelist':
 ...
 'asset_id':'1.3.0'}, 
    ...
    def update_feed_producers(self, producers):
        ...
        from .account import Account
        op = (operations.Asset_update_feed_producers)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         issuer=self['issuer'], 
         asset_to_update=self['id'], 
    ...
    def change_issuer(self, new_issuer, **kwargs):
        ...
        op = (operations.Asset_update_issuer)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         issuer=self['issuer'], 
         asset_to_update=self['id'], 
    ...
    def issue(self, amount, to, memo=None, **kwargs):
        ...
        op = (operations.Asset_issue)(fee={'amount':0, 
 'asset_id':'1.3.0'}, 
         issuer=account['id'], 
         asset_to_issue={'amount':int(amount * 10 ** self['precision']), 
 'asset_id':self['id']}, 
