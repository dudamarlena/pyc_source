...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/isodatetime/main.py
# Compiled at: 2019-03-08 06:55:13
# Size of source mod 2**32: 11797 bytes
r"""Parse and format date-time points and durations.
...
def main():
    ...
    for o_args, o_kwargs in [
     ...
     [
      ...
      {'action':'store', 
       'choices':[
        '360day', '365day', '366day', 'gregorian'], 
    ...
    if args.version_mode:
        print(__version__)
        return
    date_time_oper = DateTimeOperator(parse_format=(args.parse_format),
      utc_mode=(args.utc_mode),
      calendar_mode=(args.calendar),
      ref_point_str=(args.ref_point_str))
    try:
        if len(args.items) >= 2:
            out = date_time_oper.diff_time_point_strs(args.items[0], args.items[1], args.offsets1, args.offsets2, args.print_format, args.duration_print_format)
        else:
            if args.items and args.items[0].startswith('R'):
                outs = []
                for item in date_time_oper.iter_recurrence_str(args.items[0], args.print_format):
                    outs.append(item)
                ...
                if args.items and args.duration_print_format:
                    out = date_time_oper.format_duration_str(args.items[0], args.duration_print_format)
                else:
                    time_point_str = None
                    if args.items:
                        time_point_str = args.items[0]
                    out = date_time_oper.process_time_point_str(time_point_str, args.offsets1, args.print_format)
