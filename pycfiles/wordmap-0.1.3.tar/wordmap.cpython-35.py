...
# [GCC 8.4.0]
# Embedded file name: ../../wordmap/wordmap.py
# Compiled at: 2019-08-28 12:13:19
# Size of source mod 2**32: 24202 bytes
...
import numpy as np, rasterfairy, webbrowser, traceback, itertools, calendar, argparse, warnings, sklearn, gensim, joblib, codecs, glob2, copy, time, json, six, os, re
try:
...
class EpochLogger(CallbackAny2Vec):
    __doc__ = 'Helper class that logs epoch number during Gensim training'
...
class Model:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.args.get('model', False):
            ...
            self.model = None
            self.cache_dir = os.path.join(kwargs.get('cache_dir', cache_dir), self.args['model_type'])
    ...
    def format_args(self):
        ...
        if self.args['texts']:
            if isinstance(self.args['texts'], six.string_types):
    ...
    def validate_args(self):
        ...
        if args.get('layouts', []) != ['obj'] and args['layouts'] != ['img']:
            ...
            if 'obj' in args.get('layouts', []) and 'obj_file' not in args:
    ...
    def set_model(self, input_data):
        ...
        if self.args['model_type'] == 'word2vec':
            self.model = gensim.models.Word2Vec(input_data, size=self.args['size'], window=self.args['window'], min_count=self.args['min_count'], workers=self.args['workers'], callbacks=[
             EpochLogger()], max_final_vocab=self.args.get('max_n', None), iter=self.args.get('iter', 20))
            print(' * created model with', len(self.model.wv.index2entity), 'words')
        else:
            if self.args['model_type'] == 'doc2vec':
                self.model = gensim.models.Doc2Vec(input_data, vector_size=self.args['size'], window=self.args['window'], min_count=self.args['min_count'], workers=self.args['workers'], callbacks=[
                 EpochLogger()], iter=self.args.get('iter', 20))
    ...
    def log_files(self):
        ...
        sep = '    \n    '
        print(' * preparing to parse {0} files:{1}'.format(len(self.texts), sep + sep.join(display_files)))
    ...
    """
        if os.path.exists(path):
            with codecs.open(path, 'r', self.args['encoding']) as (f):
                return re.sub('[^\\w\\s]', '', f.read().lower()).split()
        else:
    ...
    def create_manifest(self):
        ...
        if self.args.get('model_type', False) == 'word2vec':
            ...
            if self.args.get('model_type', False) == 'doc2vec':
...
class Manifest:
    ...
    def get_layout_params(self, layout):
        ...
        l = []
        keys = [i for i in self.args if i.startswith(layout) and self.args[i] and isinstance(self.args[i], list)]
    ...
    def get_manifest_json(self):
        ...
        d['params'] = self.args
        for i in self.layouts:
            d['layouts'][i.layout].append({'filename': i.filename, 
...
class Layout:
    ...
    def get_model(self):
        if self.layout == 'umap':
            return UMAP(n_components=self.params.get('n_components'), verbose=self.params.get('verbose'), n_neighbors=self.params.get('umap_n_neighbors'), min_dist=self.params.get('umap_min_dist'))
        if self.layout == 'tsne':
            if multicore_tsne:
                return MulticoreTSNE()
            return TSNE(n_components=self.params.get('n_components'), verbose=self.params.get('verbose'))
        if self.layout == 'ivis':
            return Ivis(model=self.params.get('ivis_model'), embedding_dims=self.params.get('n_components'), k=self.params.get('ivis_k'), verbose=self.params.get('verbose'))
        if self.layout == 'grid':
        ...
        if cache:
            ...
            try:
                ...
                if self.params['n_clusters']:
                    ...
                    data.update({'clusters': clusters.labels_.tolist(), 
                     'cluster_centers': self.round(clusters.cluster_centers_.tolist())})
                if self.params.get('n_components', None) == 2 and self.params.get('lloyd_iterations', None):
                    data.update({'jittered': self.jitter_positions(positions)})
                self.json = data
                self.write_to_cache()
...
def parse():
  ...
  """
    parser = argparse.ArgumentParser(description='Transform text data into a large WebGL Visualization', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
