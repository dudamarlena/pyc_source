...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Sourcery/utils.py
# Compiled at: 2016-04-02 05:05:07
import matplotlib
matplotlib.use('Agg')
import Tigger, pyfits, numpy, subprocess, tempfile, os, sys, logging, pylab
...
def image_data(data, prefix=None):
    ...
    ndim = len(data.shape)
    if ndim == 4:
        return data[(0, 0, Ellipsis)]
    if ndim == 3:
        return data[(0, Ellipsis)]
    if ndim == 2:
        return data[...]
    log.error(' FITS file has more than 4 axes. Aborting')
...
def thresh_mask(imagename, imagedata, output, hdr, thresh, noise=None, sigma=False, smooth=None, prefix=None, savemask=False):
    ...
    if smooth:
        ...
        scales = [0.1, 2.0, 5.0, 10.0, 20.0, 40.0]
        smooth = None
        for scale in scales:
            kk = scale * beam
...
def sources_extraction(image, output=None, sourcefinder_name='pybdsm', prefix=None, **kw):
    ...
    if sourcefinder_name.lower() == 'pybdsm':
        ...
        img.write_catalog(outfile=output, format='fits', catalog_type='gaul', clobber=True)
...
def plot_local_variance(modellsm, noise, prefix, threshold):
    ...
    local = [ src.l / 1e-06 for src in model.sources ]
    pylab.figure()
    pylab.plot([noise / 1e-06] * len(local))
...
def compute_psf_correlation(imagedata, psfdata, psfhdr, pos, step=None):
    ...
    norm_data = (data_region - data_region.min()) / (data_region.max() - data_region.min())
    return (
     norm_data, psf_region)


def plot(pos, neg, rel=None, labels=None, show=False, savefig=None, prefix=None):
    log = logger(level=0, prefix=prefix)
    ...
    if not savefig:
        ...
        for counter, (i, j, x, y) in enumerate(plots):
            ...
            cov = numpy.array([(kernel[0] ** 2, 0.0), (0.0, kernel[1] ** 2)]) * (4.0 / ((nplanes + 2) * nneg)) ** (1.0 / (nplanes + 4.0))
