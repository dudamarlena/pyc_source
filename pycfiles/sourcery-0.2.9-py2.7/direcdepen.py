...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Sourcery/direcdepen.py
# Compiled at: 2016-04-02 05:05:07
import matplotlib
matplotlib.use('Agg')
import Tigger
from Tigger.Coordinates import angular_dist_pos_angle as dist
import utils, numpy, math

class load(object):

    def __init__(self, imagename, pmodel, nmodel, header, psfname=None, noise=None, snr_thresh=40, local_thresh=0.4, high_corr_thresh=0.5, negdetec_region=10, negatives_thresh=5, phasecenter_excl_radius=None, prefix=None, loglevel=0):
        ...
        self.noise = noise
        if not self.noise:
            self.log.info(' No noise value provided. Setting it to 1e-6. Please provide the noise.')
            self.noise = 1e-06
        self.dd_tag = 'dE'
        ...
        self.phaserad = phasecenter_excl_radius
        self.negregion = negdetec_region
