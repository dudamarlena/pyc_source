...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dirstat\SizeColorProvider.py
# Compiled at: 2006-06-23 01:23:02
...
class SizeColorProvider(object):
    ...
    def __init__(self):
        ...
        self._data_provider = {'int:size': self.get_integer_size, 'int:blocks': self.get_integer_blocks, 'int:mtime': self.get_integer_mtime, 'int:mtimefromnow': self.get_integer_mtimefromnow, 'int:one': self.get_integer_one, 'int:onefile': self.get_integer_onefile, 'type': self.get_color_type}
    ...
    def get_color_type(self, fileinfo):
        ...
        if fileinfo:
            if fileinfo.isFile():
                ...
                while idx != -1:
                    ext = filename[idx + 1:]
                    if tabext.has_key(ext):
                        return colormatch.get(tabext[ext], defaultColor)
                    if tablowerext.has_key(ext.lower()):
                        return colormatch.get(tablowerext[ext.lower()], defaultColor)
                    idx = filename.find('.', idx + 1)

                for key in contain.keys():
                    if filename.find(key) != -1:
                        return colormatch.get(contain[key], defaultColor)

                for key in exactmatch.keys():
                    if filename == key:
                        return colormatch.get(exactmatch[key], defaultColor)
