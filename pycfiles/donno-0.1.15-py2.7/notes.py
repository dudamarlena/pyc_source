...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/donno/notes.py
# Compiled at: 2014-03-12 22:32:45
import sys, time, shutil, os, glob, datetime, subprocess
from settings import note_base, lst_file, notebooks, valid_nb, invalid_nb, repo, trash_path
...
class Notes:
    ...
    def listnotes(self):
        ...
        if len(all_mkd) == 0:
            return 'Nothing'
        list_res = subprocess.Popen(['ls', '-t'] + all_mkd, stdout=subprocess.PIPE)
        recent = subprocess.Popen(('head -' + str(self.listcnt)).split(), stdin=list_res.stdout, stdout=subprocess.PIPE)
        most_recent = recent.communicate()[0]
        return self.post_process(most_recent)

    def nonincr_search(self, targets):
        first_cmd = subprocess.Popen(['grep', '-i', '-l', targets.pop(0)] + glob.glob(repo + '*.mkd'), stdout=subprocess.PIPE)
        cmd_queue = [first_cmd]
        while len(targets) > 0:
            cmd = subprocess.Popen(['xargs', 'grep', '-i', '-l', targets.pop(0)], stdin=cmd_queue[(-1)].stdout, stdout=subprocess.PIPE)
    ...
    def simple_search(self, targets, incremental):
        ...
        if incremental and os.path.exists(cache_file):
            first_cmd = subprocess.Popen([
             'xargs', 'grep', '-i', '-l', targets.pop(0)], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            cmd_queue = [first_cmd]
            while len(targets) > 0:
                cmd = subprocess.Popen(['xargs', 'grep', '-i', '-l', targets.pop(0)], stdin=cmd_queue[(-1)].stdout, stdout=subprocess.PIPE)
                cmd_queue.append(cmd)
    ...
    def complex_search(self, titles, tags, contents, notebook):
        ...
        if len(targets) == 0:
            ...
            first_cmd = subprocess.Popen(['grep', '-i', '-l', targets.pop(0)] + glob.glob(repo + notebooklist), stdout=subprocess.PIPE)
            cmd_queue = [first_cmd]
            while len(targets) > 0:
                cmd = subprocess.Popen(['xargs', 'grep', '-i', '-l', targets.pop(0)], stdin=cmd_queue[(-1)].stdout, stdout=subprocess.PIPE)
                cmd_queue.append(cmd)
...
class Note:
    ...
    def listAttr(self):
        ...
        created = created_raw[0:2] + '-' + created_raw[2:4] + '-' + created_raw[4:6] + ' ' + created_raw[6:8] + ':' + created_raw[8:10]
        attr.append(created)
    ...
    def _edit_note(self, listno, viewmode=False):
        ...
        shutil.copyfile(notepath, old_backuped + notename + '.bak')
        if viewmode:
