...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/donno/filesync.py
# Compiled at: 2014-04-08 23:03:38
...
def backup(upload):
    ...
    with tarfile.open(fullpath, 'r:bz2') as (tar):
        ...
        print os.path.getsize(fullpath), 'file number:',
        print str(len(tar.getmembers()))
    if upload:
...
def restore():
    ...
    if ret == 0:
        rep = raw_input('There are files newer than archive, continue restore? [y/n] ')
        if rep != 'y':
