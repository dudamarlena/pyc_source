...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/donno/cloudsync.py
...
import notes
from settings import note_base, get_passport, repo, lst_file, trash_path
...
def _upload(notefiles):
    ...
    for note in notefiles:
        with open(note, 'r') as (f):
            response = client.put_file(remote_repo + os.path.basename(note), f, overwrite=True)
        ...
        if len(recs) > 0:
            ...
            table.insert(filename=fname, last_modified=lmt.strftime('%y-%m-%d %H:%M:%S'))
        counter = counter + 1
...
def _upload_new_notes():
    ...
    local_changes = os.popen('find ' + repo + ' -newermt "' + last_sync_time + '" -name "*.mkd"').read().split()
...
def _sync_with_server():
    ...
    local_changes = {}
    local_changes['add'] = os.popen('find ' + repo + ' -newermt "' + last_sync_time + '" -name "*.mkd"').read().split()
    local_changes['del'] = os.popen('find ' + trash_path + ' -newermt "' + last_sync_time + '" -name "*.mkd"').read().split()
