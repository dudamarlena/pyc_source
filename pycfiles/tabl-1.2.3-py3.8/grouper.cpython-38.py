...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/tabl/grouper.py
# Compiled at: 2020-03-12 15:31:38
# Size of source mod 2**32: 34177 bytes
"""
.. module:: tabl.tabl
...
try:
    ...
    def tabulate(tabl, columns=None, tablefmt=None):
        """Alt tabulate
        """
        outp = ''
        outp += ' '.join(columns) + '\n'
...
try:
        ...
        """
                if len(self) == 0:
                    ...
                    if hasattr(row, 'items'):
                        assert set(self.columns) == set(row), 'Not the same columns in Tbl: {} {}'.format(self.columns, row.keys())
                        ...
                        if len(row) == len(self.columns):
                            ...
                            raise ValueError('Number of elements in {row} not equal '.format(row=row), 'to number of columns in Tbl.')
                if not self.valid:
        ...
        """
                if len(self) == 0 and isinstance(tabl, Tbl):
                    ...
                    if len(self) == 0:
                        ...
                        if isinstance(tabl, Tbl):
                            assert set(self.columns) == set(tabl.columns), 'Not the same columns in Tbl: {} {}'.format(self.columns, tabl.columns)
                            for col, dta in zip(tabl.columns, tabl.data):
                                ci = self.columns.index(col)
                                self.data[ci] = np.concatenate([self.data[ci], dta])

                        else:
                            if isinstance(tabl, pd.DataFrame):
                                assert set(self.columns) == set(tabl), 'Not the same columns in Tbl: {} {}'.format(self.columns, tabl.keys())
    ...
    Examples:
        ...
        """
                ...
                try:
                    self[(r, c)]
                    NotImplementedError(('Setting Tbl slices with Tbl ', 'slice is not yet implemented.'))
                except (ValueError, TypeError, KeyError) as e:
                    try:
                        pass
                    finally:
        ...
        """
                tabl = tabulate([[c[r] for c in self.data] for r in range(min(len(self), self.max_repr_rows))],
                  (self.columns),
        ...
        """
                return Tbl({v.astype(dty):k for k, v, dty in zip(self.columns, self.data, dtypes)})
