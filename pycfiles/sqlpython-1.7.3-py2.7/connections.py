...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlpython/connections.py
# Compiled at: 2012-05-26 21:28:24
import re, os, getpass, gerald, gerald.utilities, gerald.utilities.dburi, time, optparse, doctest, pyparsing, dbapiext
gerald_classes = {}
try:
    import cx_Oracle
...
class ObjectDescriptor(object):
    ...
    def match_pattern(self, pattern, specific_owner=None):
        ...
        return right_owner and (compiled.match(self.type) or compiled.match(self.type + '/') or compiled.match(self.unqualified_name) or compiled.match(self.unqualified_path))
...
class DatabaseInstance(object):
    ...
    def __init__(self, arg, opts, default_rdbms='oracle'):
        ...
        if not self.parse_connect_uri(arg):
            ...
            if connectargs:
                for param in ('username', 'password', 'database', 'port', 'hostname',
                              'mode'):
    ...
    ls_parser = pyparsing.Optional(sqlname('owner') + '/') + pyparsing.Optional(typname('type') + '/') + pyparsing.Optional(sqlname('name')) + pyparsing.stringEnd | pyparsing.Optional(typname('type') + '/') + pyparsing.Optional(sqlname('owner') + '.') + pyparsing.Optional(sqlname('name')) + pyparsing.stringEnd
        ...
        for row in curs:
            ...
            for line_number, line in enumerate(code.splitlines()):
                if target.search(line):
                    yield (
                     row[0], row[1], row[2], line_number, line)
...
parser.add_option('-H', '--hostname', type='string', help='Machine where database is hosted')
parser.add_option('-p', '--port', type='int', help='Port to connect to')
parser.add_option('--password', type='string', help='Password')
parser.add_option('-d', '--database', type='string', help='Database name to connect to')
parser.add_option('-U', '--username', type='string', help='Database user name to connect as')
...
class MySQLInstance(DatabaseInstance):
    ...
    def connect(self):
        self.connection = MySQLdb.connect(host=self.hostname, user=self.username, passwd=self.password, db=self.database, port=self.port, sql_mode='ANSI')
    ...
    gerald_types = {'TABLE': gerald.mysql_schema.Table, 'VIEW': gerald.mysql_schema.View, 
...
class PostgresInstance(DatabaseInstance):
    ...
    def connect(self):
        self.connection = psycopg2.connect(host=self.hostname, user=self.username, password=self.password, database=self.database, port=self.port)
    ...
    gerald_types = {'BASE TABLE': gerald.postgres_schema.Table, 'VIEW': gerald.postgres_schema.View}
...
class OracleInstance(DatabaseInstance):
    ...
    default_port = 1521
    paramstyle = 'named'
    connection_parser = re.compile('(?P<username>[^/\\s@]*)(/(?P<password>[^/\\s@]*))?(@((?P<hostname>[^/\\s:]*)(:(?P<port>\\d{1,4}))?/)?(?P<database>[^/\\s:]*))?(\\s+as\\s+(?P<mode>sys(dba|oper)))?', re.IGNORECASE)
    ...
    def connect(self):
        self.connection = cx_Oracle.connect(user=self.username, password=self.password, dsn=self.dsn, mode=self.mode)
    ...
    gerald_types = {'TABLE': gerald.oracle_schema.Table, 'VIEW': gerald.oracle_schema.View}
