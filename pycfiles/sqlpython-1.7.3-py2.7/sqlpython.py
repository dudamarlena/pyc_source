...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlpython/sqlpython.py
# Compiled at: 2012-05-26 21:31:17
import cmd2, getpass, binascii, re, os, platform, pyparsing, connections
__version__ = '1.7.3'
try:
    import cx_Oracle
except ImportError:
...
class sqlpython(cmd2.Cmd):
    ...
    @cmd2.options([
     cmd2.make_option('-a', '--add', action='store_true', help='add connection (keep current connection)'),
     cmd2.make_option('-c', '--close', action='store_true', help='close connection {N} (or current)'),
     cmd2.make_option('-C', '--closeall', action='store_true', help='close all connections'),
     ...
     cmd2.make_option('-H', '--hostname', type='string', help='Machine where database is hosted'),
     cmd2.make_option('-p', '--port', type='int', help='Port to connect to'),
     cmd2.make_option('--password', type='string', help='Password'),
     cmd2.make_option('-d', '--database', type='string', help='Database name to connect to'),
     cmd2.make_option('-U', '--username', type='string', help='Database user name to connect as')])
    ...
    _availability_regex = re.compile('\\(\\s*Availab(.*)\\)', re.IGNORECASE)
    ...
    def _show_errors(self, all_users=False, limit=None, mintime=None, targets=[]):
        ...
        if targets:
            target = 'AND (%s)\n' % (' OR ').join("ae.type || '/' || ae.name LIKE '%s'" % t.upper().replace('*', '%') for t in targets)
        else:
            target = ''
...
\p   pie chart"""
    ...
    bindScanner = {'oracle': Parser(pyparsing.Literal(':') + pyparsing.Word(pyparsing.alphanums + '_$#')), 'postgres': Parser(pyparsing.Literal('%(') + legal_sql_word + ')s')}
    ...
    def pmatrix(self, rows, maxlen=30, heading=True, restructuredtext=False):
        ...
        for i in rrows:
            rowsi = map(str, rows[i])
            split = []
            mustsplit = 0
            for j in rcols:
                if str(self.coltypes[j]) == "<type 'cx_Oracle.BINARY'>":
                ...
                if maxen[j] <= maxlen:
                    ...
                    rowsi[j] = rowsi[j][0:maxlen - 1]

            toprint.append(rowsi)
            if mustsplit != 0:
        ...
        for i in rcols:
            maxcol = maxen[i]
            name = names[i]
            sepcols.append('-' * maxcol)
            names[i] = name + ' ' * (maxcol - len(name))
            rrows2 = range(len(toprint))
            for j in rrows2:
                val = toprint[j][i]
                if str(self.coltypes[i]) == "<type 'cx_Oracle.NUMBER'>":
