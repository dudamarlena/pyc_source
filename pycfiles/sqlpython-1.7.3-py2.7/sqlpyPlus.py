...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlpython/sqlpyPlus.py
...
"""
import sys, os, re, sqlpython, pyparsing, re, completion, datetime, pickle, binascii, subprocess, time, itertools
try:
...
softwareLists = {'diff/merge': [
                (
                 '/usr/bin/meld', '%s %s %s'),
                (
                 '/usr/bin/kdiff3', '%s %s %s'),
                (
                 'C:\\Program Files\\Araxis\\Araxis Merge v6.5\\Merge.exe', '"%s" %s %s'),
                (
                 'C:\\Program Files\\TortoiseSVN\\bin\\TortoiseMerge.exe', '"%s" /base:"%s" /mine:"%s"'),
                (
                 'FileMerge', '%s %s %s'),
                (
                 'kompare', '%s %s %s'),
                ...
                (
                 'diff', '%s %s %s'),
                (
                 'c:\\windows\\system32\\comp.exe', '%s %s %s')], 
   'text editor': [
                 (
                  'gedit', '%s %s'),
                 (
...
class Result(tuple):
    ...
    def __str__(self):
        return ('\n').join('%s: %s' % (colname, self[idx]) for idx, colname in enumerate(self.resultset.colnames))
...
class sqlpyPlus(sqlpython.sqlpython):
    ...
    abbrev = True
    sqlpython.sqlpython.shortcuts.update({':': 'setbind', '\\': 'psql', 
       '@': 'get'})
    ...
    def parseline(self, line):
        ...
        if cmd in ('select', 'sleect', 'insert', 'update', 'delete', 'describe', 'desc',
                   'comments', 'pull', 'refs', 'desc', 'triggers', 'find') and not hasattr(self, 'curs'):
    ...
    def postcmd(self, stop, line):
        ...
        if hasattr(self, 'rdbms') and self.rdbms == 'oracle' and self.serveroutput:
            self.dbms_output()
        return stop
    ...
    tableNameFinder = re.compile('from\\s+([\\w$#_"]+)', re.IGNORECASE | re.MULTILINE | re.DOTALL)
        ...
        if tblname:
            ...
            if outformat in output_templates:
                ...
                if outformat == '\\t':
                    ...
                    transpr = [ [ rows[y][x] for y in range(len(rows)) ] for x in range(len(rows[0])) ]
                    newdesc = [ ['ROW N.' + str(y), 10] for y in range(len(rows)) ]
                    for x in range(len(self.coltypes)):
                        if str(self.coltypes[x]) == "<type 'cx_Oracle.BINARY'>":
                            rname = transpr[x][0]
                    ...
                    plot.draw()
                    return ''
                result = self.pmatrix(self.rows, self.maxfetch, heading=self.heading, restructuredtext=outformat == '\\r')
        return result
    ...
    columnNameRegex = re.compile('select\\s+(.*)from', re.IGNORECASE | re.DOTALL | re.MULTILINE)
    ...
    def completedefault(self, text, line, begidx, endidx):
        ...
        if segment in ('select', 'where', 'having', 'set', 'order by', 'group by'):
            ...
            completions = [ n for n in self.get_names() if n.startswith('do_') ] + ['insert', 'update', 'delete', 'drop', 'alter', 'begin', 'declare', 'create']
    ...
    columnlistPattern = pyparsing.SkipTo(pyparsing.CaselessKeyword('from'))('columns') + pyparsing.SkipTo(pyparsing.stringEnd)('remainder')
    ...
    colNumber.ignore(pyparsing.cStyleComment).ignore(Parser.comment_def).ignore(pyparsing.sglQuotedString).ignore(pyparsing.dblQuotedString)
    wildSqlParser = colNumber ^ colName ^ wildColName
    wildSqlParser.ignore(pyparsing.cStyleComment).ignore(Parser.comment_def).ignore(pyparsing.sglQuotedString).ignore(pyparsing.dblQuotedString)
    ...
    def expandWildSql(self, arg):
        ...
        parseresults = [ p for p in parseresults if p[0].column_number or '*' in p[0].column_name or '%' in p[0].column_name or '?' in p[0].column_name or p[0].exclude
                       ]
        if not parseresults:
            return arg
    ...
    def do_select(self, arg, bindVarsIn=None, terminator=None):
        ...
        if cx_Oracle and cx_Oracle.BLOB in self.coltypes:
            self.rows = [ [ coltype == cx_Oracle.BLOB and BlobDisplayer(datum, rownum < self.bloblimit) or datum for datum, coltype in zip(row, self.coltypes) ] for rownum, row in enumerate(self.rows)
                        ]
    ...
    @options([make_option('-d', '--dump', action='store_true', help='dump results to files'),
     ...
     make_option('-w', '--width', type='int', default=5, help='# of lines before and after --lineNo'),
    ...
    def _with_line_numbers(self, txt):
        ...
        txt = ('\n').join(template % (n + 1, line) for n, line in enumerate(txt))
        return txt
    ...
    def do_show(self, arg):
        ...
        if not arg.strip():
            ...
            if argpieces[0][:3] == 'tab':
                return self.do_ls('table/*')
            if (
             argpieces[0], argpieces[1:2][:3]) == ('all', 'err'):
                return self._show_errors(all_users=False, limit=None, targets=argpieces[2:])
            if argpieces[0].startswith('variable') or argpieces[0].startswith('param'):
                argpieces.pop(0)
                if argpieces[:1] == ['like']:
                    argpieces.pop(0)
                if argpieces:
    ...
    all_users_option = make_option('-a', '--all', action='store_const', dest='scope', default={'col': '', 'view': 'user', 'schemas': 'user', 'firstcol': ''}, const={'col': ', owner', 'view': 'all', 'schemas': 'all', 'firstcol': 'owner, '})
    ...
    standard_options = [all_users_option,
        ...
        for owner, object_type, name in self.current_instance.objects(arg, opts):
            ...
            if hasattr(obj, 'columns'):
                ...
                if opts.long and hasattr(obj, 'constraints'):
                    ...
                    self.rows = [ (col['sequence'], col['name'], col['nullable'] and 'NULL' or 'NOT NULL', self._col_type_descriptor(col), col['name'] in primary_key_columns and 'P' or col['name'] in unique_key_columns and 'U' or '', col.get('default') or '', col.get('comment') or '') for col in cols
                                ]
                else:
                    self.colnames = ('Name Nullable Type').split()
                    self.rows = [ (col['name'], col['nullable'] and 'NULL' or 'NOT NULL', self._col_type_descriptor(col)) for col in cols
                                ]
                self.coltypes = [
                 str] * len(self.colnames)
                ...
                end_heading = re.compile('\\bDECLARE|BEGIN\\b', re.IGNORECASE)
    ...
    def do_psql(self, arg):
        r"""Shortcut commands emulating psql's backslash commands.
    ...
    table_type_name = {'postgres': 'base table', 'mysql': 'BASE TABLE', 'oracle': 'TABLE'}
    ...
    def do_head(self, arg):
        r"""Shortcut for SELECT * FROM <arg>;10
    ...
    def interpret_variable_assignment(self, arg):
        ...
        try:
            ...
            return (
             False, str(arg).split()[(-1)] or None, None)
    ...
    object_types = ('BASE TABLE', 'CLUSTER', 'CONSUMER GROUP', 'CONTEXT', 'DIRECTORY',
                    'EDITION', 'EVALUATION CONTEXT', 'FUNCTION', 'INDEX', 'INDEX PARTITION',
                    'INDEXTYPE', 'JAVA CLASS', 'JAVA DATA', 'JAVA RESOURCE', 'JOB',
                    'JOB CLASS', 'LIBRARY', 'MATERIALIZED VIEW', 'OPERATOR', 'PACKAGE',
                    'PACKAGE BODY', 'PROCEDURE', 'PROGRAM', 'RULE', 'RULE SET', 'SCHEDULE',
                    'SEQUENCE', 'SYNONYM', 'TABLE', 'TABLE PARTITION', 'TRIGGER',
                    'TYPE', 'TYPE BODY', 'VIEW', 'WINDOW', 'WINDOW GROUP', 'XML SCHEMA')
    ...
    def do_grep(self, arg, opts):
        ...
        re_pattern = re.compile(self._to_re_wildcards(pattern), opts.ignorecase and re.IGNORECASE or 0)
        for target in targets:
            for owner, object_type, name in self.current_instance.objects(target, opts):
                ...
                if hasattr(obj, 'columns'):
                    ...
                    sql = self.parsed(sql, terminator=arg.parsed.terminator or ';', suffix=arg.parsed.suffix)
                    self.do_select(sql)
                elif hasattr(obj, 'source'):
