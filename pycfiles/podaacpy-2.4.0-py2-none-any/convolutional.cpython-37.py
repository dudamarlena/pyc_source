...
# Embedded file name: build\bdist.win-amd64\egg\poda\layers\convolutional.py
# Compiled at: 2019-09-30 02:25:28
# Size of source mod 2**32: 17777 bytes
...
def avarage_pool_1d(input_tensor, kernel_sizes=3, stride_sizes=1, paddings='same', names=None):
    ...
    if names != None:
        ...
        names = 'avg_pool_1d'
    if not paddings == 'Valid':
        if paddings == 'valid' or paddings == 'VALID':
            paddings = 'VALID'
    elif not paddings == 'Same':
        if paddings == 'same' or paddings == 'SAME':
            paddings = 'SAME'
    else:
        paddings = 'SAME'
...
def batch_normalization(input_tensor, is_trainable=True, decay=0.999, epsilon=0.001):
    """[summary]
...
def convolution_2d(input_tensor, number_filters, kernel_sizes=(3, 3), stride_sizes=(1, 1), paddings='same', activations='relu', batch_normalizations=False, dropout_rates=None, names=None):
    ...
    layer = tf.nn.conv2d(input=input_tensor, filter=weight, strides=[stride_sizes[0], stride_sizes[1]], padding=paddings, use_cudnn_on_gpu=True, data_format='NHWC',
      dilations=[1, 1, 1, 1],
      name=('conv_2d_' + names))
    if activations != None:
