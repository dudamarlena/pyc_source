...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/amqpdeliver/hg.py
...
def send_bundle1(ui, repo, node):
    server = ui.config(CONFIG_SECTION, 'server', default=DEFAULT_SERVER)
    exchange = ui.config(CONFIG_SECTION, 'exchange')
    userid = ui.config(CONFIG_SECTION, 'userid', default=DEFAULT_USERID)
    password = ui.config(CONFIG_SECTION, 'userid', default=DEFAULT_PASSWORD)
    virtual_host = ui.config(CONFIG_SECTION, 'virtual-host', default=DEFAULT_VIRTUAL_HOST)
    ...
    send.send(server=server, exchange=exchange, body=body, userid=ui.config(CONFIG_SECTION, 'userid', default=DEFAULT_USERID), password=ui.config(CONFIG_SECTION, 'userid', default=DEFAULT_PASSWORD), virtual_host=ui.config(CONFIG_SECTION, 'virtual-host', default=DEFAULT_VIRTUAL_HOST), message_id=node)
