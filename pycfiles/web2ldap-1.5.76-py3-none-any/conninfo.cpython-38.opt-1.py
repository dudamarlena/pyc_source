...
# Compiled at: 2020-05-04 07:38:29
# Size of source mod 2**32: 12424 bytes
"""
            ...
            try:
                ...
                if app.ls.sasl_auth:
                    ...
                    sasl_auth_info = '<table>%s</table>' % '\n'.join(['<tr><td>%s</td><td>%s</td></tr>' % (
                     ...
                     app.form.utf2display(repr(val))) for key, val in app.ls.sasl_auth.cb_value_dict.items() if val])
                else:
                    sasl_mech = 'simple'
