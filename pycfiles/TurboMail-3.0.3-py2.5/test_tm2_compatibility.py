...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/tests/test_tm2_compatibility.py
# Compiled at: 2009-08-24 19:48:22
"""Test that most interfaces of TurboMail 2.x are working in TurboMail 3.x"""
import email, unittest, warnings, turbomail
...
class TestTurboMail2xCompatibility(TurboMailTestCase):
    ...
    def test_message_instantiation_with_keyword_parameters(self):
        message = Message(sender='from@example.com', recipient='to@example.com', subject='Test')
        message.plain = 'Hello world!'
        msg_string = str(message)
        self.failUnless('From: from@example.com' in msg_string)
        self.failUnless('To: to@example.com' in msg_string)
    ...
    def test_message_instantiation_with_tuples(self):
        message = Message(('Foo Bar', 'from@example.com'), ('To', 'to@example.com'), 'Test')
        message.plain = 'Hello world!'
        msg_string = str(message)
    ...
    def test_message_properties(self):
        ...
        message.plain = 'Hello world!'
        property_names = [
         'bcc', 'cc', 'date', 'disposition', 'encoding',
    ...
    def test_use_sender_for_author_if_no_author_given(self):
        message = Message(sender='sender@example.com', to='to@example.com', subject='Test')
    ...
    def test_can_send_pregenerated_messages_with_dict(self):
        ...
        fake_setuptools = {'immediate': ImmediateManager, 'debug': DebugTransportFactory}
        interface.start(config, extra_classes=fake_setuptools)
        msg_info = dict(sender='from@example.com', recipients='to@example.com', message=rfc822_msg)
