...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/tests/test_smtp_transport.py
...
class TestSMTPTransport(SMTPTestCase, TurboMailTestCase):
    ...
    def setUp(self):
        self.config = {'mail.on': True, 'mail.manager': 'immediate', 'mail.transport': 'smtp', 
           'mail.brand': False}
        self.fake_setuptools = {'immediate': ImmediateManager, 'smtp': SMTPTransportFactory()}
        super(TestSMTPTransport, self).setUp()
        self._ignore_warning('Configuration key ".+" is deprecated.*')
        self.msg = self.build_message()

    def build_message(self):
        return Message('foo@example.com', 'to@example.com', 'Test', plain='Plain text body')
    ...
    def test_send_messages_also_to_cc_recipients(self):
        ...
        self.assertEqual(1, queue.qsize())
        msg = queue.get()
        self.assertEqual(set(['to@example.com', 'cc@example.com']), set(msg.smtp_to))
