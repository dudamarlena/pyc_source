...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pywattbox\wattbox.py
# Compiled at: 2019-03-30 10:22:53
# Size of source mod 2**32: 6967 bytes
...
class WattBox(object):
    ...
    def __init__(self, ip, port=80, user='wattbox', password='wattbox'):
        ...
        result = requests.get(('{}/wattbox_info.xml'.format(self.base_host)),
    ...
    def send_command(self, outlet, command):
        _ = requests.get(('{}/control.cgi?outlet={}&command={}'.format(self.base_host, outlet, command)),
          auth=(HTTPBasicAuth(self.user, self.password)))

    def send_master_command(self, command):
        if command not in (Commands.ON, Commands.OFF):
            raise ValueError('Command ({}) can only be `Commands.ON` or `Commands.OFF`.'.format(command))
