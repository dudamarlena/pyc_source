...
# Embedded file name: build/bdist.linux-x86_64/egg/Products/FinisAfricae/content/Catalog.py
# Compiled at: 2008-07-28 18:54:32
...
class CatalogContainer(ATBTreeFolder):
    ...
    locallyAllowedTypes = immediatelyAddableTypes = allowed_content_types = ('CatalogObjectMods',
                                                                             'Large Plone Folder')
    ...
    def _create_slot(self, base, slot):
        fldid = base.invokeFactory(self.slot_folder_type, '%s' % slot, title='slot %s' % slot, description='Catalog slot container')
    ...
    def appendModsObject(self, modsData=None, oldData=None, updateMetadata=True):
        ...
        modsobjid = slot_obj.invokeFactory('CatalogObjectMods', '%s' % self.catalog_id_counter, title=None, description='Catalog object', xmlData=modsData, isisData=oldData)
        if updateMetadata:
