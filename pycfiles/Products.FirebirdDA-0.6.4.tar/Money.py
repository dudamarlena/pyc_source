...
# Embedded file name: build/bdist.linux-i686/egg/Products/FinanceFields/Money.py
# Compiled at: 2010-03-10 14:21:02
__doc__ = ' Money extends FixedPoint with currency information\n\n    NOTE: We do not allow any operations with floats\n'
RCS_id = '$Id: Money.py 189 2005-10-08 07:07:58Z roche $'
import re, string
from fixedpoint import FixedPoint
from Currency import Currency, CURRENCIES, SYMBOLS_MAP
from types import StringType, UnicodeType, StringTypes, FloatType, IntType
...
def thousands_commas(amount):
    temp = '%.2f' % amount
    profile = re.compile('(\\d)(\\d\\d\\d[.,])')
    while 1:
        (temp, count) = re.subn(profile, '\\1,\\2', temp)
        if not count:
            break
...
class Money:
    ...
    def __init__(self, amount, currency):
        ...
        assert isinstance(currency, Currency), (str(currency), type(currency))
        self.ccy = currency
    ...
    def __repr__(self):
        symbol = self.ccy.currency_symbol or self.ccy.int_currency_symbol
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Money):
            assert self.ccy == other.currency(), ('currency mismatch', `self`, `other`)
    ...
    def __divmod__(self, other):
        ...
        return (
         Money(q, self.ccy), Money(r, self.ccy))
