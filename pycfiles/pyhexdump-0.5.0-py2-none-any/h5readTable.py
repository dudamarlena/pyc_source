...
# Embedded file name: build\bdist.win32\egg\pyhexad\h5readTable.py
# Compiled at: 2014-12-08 16:26:48
import logging, h5py
...
def get_table(loc, path, columns=None, first=None, last=None, step=None):
    ...
    if not is_valid:
        ...
        if loc.get(path) is None or loc.get(path, getclass=True) != h5py.Dataset:
        ...
        if (idx[1] - idx[0]) / idx[2] >= Limits.EXCEL_MAX_ROWS:
            return (None, 'The requested number of rows exceeds the maximum number of rows Excel can display.')
        y = [
         list(col_names)]
...
@xl_func('string filename, string tablename, var columns, var first, var last, var step : string', category='HDF5', thread_safe=False, macro=True, disable_function_wizard_calc=True)
def h5readTable(filename, tablename, columns, first=-1, last=-1, step=-1):
    ...
    if not isinstance(filename, str):
        ...
        with h5py.File(filename, 'r') as (f):
            x, ret = get_table(f, tablename, columns, int(first) if first is not None else None, int(last) if last is not None else None, int(step) if step is not None else None)
            if x is None:
                return ret
            draw_table(x)
        return ret