...
# Embedded file name: build\bdist.win32\egg\pyhexad\h5readArray.py
    ...
    if not is_valid:
        ...
        if rk == 1:
            ...
            with dst.astype(mem_type):
                x = dst[(slc0, slc1)]
                return (
                 x, '%d x %d' % (x.shape[0], x.shape[1]))
        else:
            return (None, 'Unsupported HDF5 array rank.')
        return


@xl_func('string filename, string arrayname, var first, var last, var step : string', category='HDF5', thread_safe=False, macro=True, disable_function_wizard_calc=True)
