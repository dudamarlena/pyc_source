...
# [GCC 8.4.0]
# Embedded file name: /Users/wangwenpei/Codes/nextoa/cabric/cabric/cloud/qingcloud.py
...
class QingCloud(object):
    ...
    def connect(self, zone=None):
        ...
        self.logger.debug('api-auth with key: %s,%s', key, secret[0:2] + '**********' + secret[-2:])
        if None in [zone, key, secret]:
            raise ValueError("Please set `QINGCLOUD_ACCESS_KEY',`QINGCLOUD_ACCESS_SECRET',and make sure `zone' has set")
        self.connector = qingcloud.iaas.connect_to_zone(zone, key, secret)
        return
    ...
    def get_or_add_load_balancer_backends(self, loadbalancer_listener, backend, *args, **kwargs):
        try:
            ...
            self.connector.add_backends_to_listener(loadbalancer_listener, [
             backend], *args, **kwargs)
            backend = self.get_loadbalancer_backend(loadbalancer_listener, backend['loadbalancer_backend_name'])
