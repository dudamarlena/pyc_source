...
# [GCC 8.4.0]
# Embedded file name: /Users/wangwenpei/Codes/nextoa/cabric/cabric/components/deploy.py
# Compiled at: 2018-06-04 06:05:18
# Size of source mod 2**32: 23098 bytes
import json, os
...
from cabric.utils import get_roots, bind_hosts, execute, get_repo, put, run, get_home, get_git_host, known_host, cd
try:
    input = raw_input
except NameError:
    pass
...
class DeployComponent(Component):
    ...
    def create_github_key(self, private_key, github, remote_user, key_length=8192):
        ...
        for v in authorized_methods:
            temp_config = v()
            response = requests.get(('https://api.github.com/repos/{}/keys'.format(github)),
              auth=temp_config)
            if response.status_code != 200:
                ...
                if self.options.debug:
                    ...
                    self.warn('github-api status-code: %d' % response.status_code)
        ...
        for v in uploaded_keys:
            response = requests.delete(('https://api.github.com/repos/{}/keys/{}'.format(github, v.get('id'))),
              auth=auth_config)
        ...
        response = requests.post(('https://api.github.com/repos/{}/keys'.format(github)),
          data=(json.dumps({'title':key_title, 
    ...
    def get_project_python(self, user, project_name):
        ...
        with settings(warn_only=True):
            version = run('test -f {0}/.python-version && cat {0}/.python-version'.format(project_path))
    ...
    def install_requirements(self, user, project_name, pip='pip'):
        ...
        if python_version:
            ...
            python_version = run('test -e ~%(user)s/.python-version && cat ~%(user)s/.python-version || echo ""' % {'user': user})
            bin_dir = bin_home + 'versions/%s/bin' % python_version
        if not python_version:
            bin_dir = bin_home + 'shims'
        else:
            requirement_files = [os.path.join(project_path, 'requirements.txt'),
        ...
        for f in requirement_files:
            run("test -f {0} && {1}/{2} install -U  -r {0} || echo '{0} not exist,skip install...'".format(f, bin_dir, pip_tool))
        ...
        for f in requirement_files:
            run("test -f {0} && {1}/{2} install -r {0} || echo '{0} not exist,skip install...'".format(f, bin_dir, pip_tool))
    ...
    def migrate_db(self, user, project_name, django_settings='web.online'):
        ...
        with cd(project_path):
            run(('test -f %(project_path)s/manage.py && python %(project_path)s/manage.py migrate --settings=%(settings)s || echo "skip migrate database"' % {'project_path':project_path, 
             'settings':'web.online'}),
              user=user)
    ...
    def upload_resources(self, user, project_name, working_root=None, static_prefix=None):
        ...
        if not os.path.exists(django_manage):
            self.warn('local root is not a django project,skip upload resources')
            return
        with settings(warn_only=True):
        ...
        * finish
        ...
        if options.verbose == 3:
            ...
            if not options.skip_migrate:
                command_list.append(lambda : self.migrate_db(user, project_name, config.get('settings', 'web.online')))
            if not options.skip_upload_resources:
                resource_dir = os.path.expanduser(options.resource_dir)
                command_list.append(lambda : self.upload_resources(user, project_name, working_root=resource_dir,
                  static_prefix=(config.get('static-prefix'))))
                command_list.append(lambda : self.upload_javascripts(user, project_name, working_root=resource_dir))
    ...
    def add_arguments(cls):
        ...
        return [
         (
          ('commit', ),
          dict(nargs='?', help='set which commit to deploy,default is latest version')),
         (
          ('--parallel', '-P'),
         ...
         (
          ('--force-renew', ),
          dict(action='store_true', help='only works when user set github value')),
         (
          ('--skip-source-code', ),
         ...
         (
          ('--resource-dir', ),
          dict(nargs='?', default=(os.getcwd()), help='set resource upload directory'))]