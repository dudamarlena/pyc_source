...
# [GCC 8.4.0]
# Embedded file name: /Users/wangwenpei/Codes/nextoa/cabric/cabric/utils.py
# Compiled at: 2018-04-08 08:34:37
# Size of source mod 2**32: 11281 bytes
import os, sys
...
def get_roots(project):
    ...
    roots = (
     package_root, config_root, fabric_root)
...
def put(local_path, remote_path):
    ...
    if env.host_string:
        ...
        tpl = str('%(bin)s -e ssh%(port_str)s%(recursive_str)s %(local_path)s %(host)s:%(remote_path)s')
        port = int(env.host_string[env.host_string.find(':') + 1:]) if env.host_string.find(':') > -1 else 22
        host = env.host_string[0:env.host_string.find(':')] if env.host_string.find(':') > -1 else env.host_string
        port_str = ' %s' % port
        recursive_str = ''
...
def bind_hosts(fabric_root, select_env, parallel=False, machine_config=None):
    ...
    :return:
    """
    machine_config = machine_config or os.path.join(fabric_root, select_env + '.conf')
...
def run(cmd, user=None, capture=None, *args, **kwargs):
    ...
    if env.hosts:
        ...
        return fabric_local(cmd, *args, capture=capture, **kwargs)
    ...
    try:
        try:
            ...
            for name, args, kwargs, arg_hosts, arg_roles, arg_exclude_hosts in commands_to_run:
                results.append(fab_execute(name, *args, hosts=arg_hosts, roles=arg_roles, exclude_hosts=arg_exclude_hosts, **kwargs))
