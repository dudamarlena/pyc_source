...
# Compiled at: 2018-01-18 22:56:18
import json, os
...
class DeployComponent(Component):
    ...
    def create_github_key(self, private_key, github, remote_user, key_length=8192):
        ...
        if not os.path.exists(private_key_dir):
            os.makedirs(private_key_dir, 448)
        ...
        response = requests.post(('https://api.github.com/repos/{}/keys').format(github), data=json.dumps({'title': key_title, 
           'key': pubkey.exportKey('OpenSSH'), 
           'read_only': True}), auth=auth_config)
    ...
    def run(self, options):
        ...
        * finish
        ...
        if not options.skip_upload_resources:
            ...
            command_list.append(lambda : self.upload_resources(user, project_name, working_root=resource_dir, static_prefix=config.get('static-prefix')))
