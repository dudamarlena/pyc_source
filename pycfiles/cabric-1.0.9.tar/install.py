...
# [GCC 8.4.0]
# Embedded file name: /Users/wangwenpei/Codes/nextoa/cabric/cabric/components/install.py
# Compiled at: 2018-01-09 11:22:15
import json, os
from cliez.component import Component
from fabric.context_managers import settings, cd
from cabric.utils import get_roots, mirror_put, run, bind_hosts, execute, get_platform, run_block, exist_group, exist_user
try:
...
class InstallComponent(Component):
        ...
        def on_centos():
            ...
            * create group explicitly
            ...
            for user in users:
                ...
                run(('useradd {1} -d {2} {4} -s {3} {0}').format(username, group_str, home, shell, system_str))
                if perm:
                    run(('chmod {1} {0}').format(home, perm))
    ...
    def install_pyenv(self, versions=None, skip_pkg=False):
        ...
        if remote_os == 'centos':
            if not skip_pkg:
                ...
                run('yum install -y gcc gcc-c++ make autoconf certbot libffi-devel ncurses-devel expat-devel zlib-devel zlib libzip-devel bzip2 bzip2-devel bzip2-libs mariadb-devel mariadb-libs sqlite-devel libxml2 libxml2-devel libxslt libxslt-devel libcurl-devel pcre-devel pcre libmcrypt libmcrypt-devel openssl-devel openssl-libs libjpeg libjpeg-devel libpng libpng-devel freetype freetype-devel  libtiff-devel lcms2-devel libwebp-devel tcl-devel tk-devel')
            run('export PYENV_ROOT=/usr/local/var/pyenv && curl -L http://nextoa.com/bin/pyenv-install.sh  | bash')
    ...
    def install_pypi(self, pypi_configs, skip_pkg=False):
        ...
        def install(pypi_config):
            ...
            used_version = python_version if python_version else self.system_python_version
            for pkg in pypi_config.get('packages'):
                run('export PYENV_ROOT="/usr/local/var/pyenv/" && export PYENV_VERSION=%s && pip install %s' % (
                 used_version, pkg))
