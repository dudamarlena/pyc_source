...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/dpcluster/test.py
# Compiled at: 2013-06-21 16:43:47
import unittest
from dpcluster import *

def grad_check(f, x, eps=0.0001):
...
class Tests(unittest.TestCase):
    ...
    def test_gaussian(self):
        ...
        mu = mus[0, :]
        x = data[0, :]
        ll = -k * 0.5 * math.log(2 * np.pi) - 0.5 * np.linalg.slogdet(Sg)[1] - 0.5 * ((mu - x) * scipy.linalg.solve(Sg, mu - x)).sum()
    ...
    def test_niw(self):
        ...
        k = k[0]
        nu = nu[0]
        ll1 = -p * 0.5 * math.log(2 * np.pi) - 0.5 * np.linalg.slogdet(Sg / k)[1] - 0.5 * ((mu0 - mu) * scipy.linalg.solve(Sg / k, mu0 - mu)).sum()
        ll2 = 0.5 * nu * np.linalg.slogdet(Psi)[1] - 0.5 * nu * p * np.log(2) - scipy.special.multigammaln(0.5 * nu, p) - 0.5 * (nu + p + 1) * np.linalg.slogdet(Sg)[1] - 0.5 * np.sum(Psi * np.linalg.inv(Sg))
        self.assertAlmostEqual(ll1 + ll2, lls[(0, 0)])
        al = 1e-10
        nu1 = al * nus[1, :] - al * nus[0, :] + 0.5 * nus[0, :] + 0.5 * nus[1, :]
        nu2 = al * nus[0, :] - al * nus[1, :] + 0.5 * nus[0, :] + 0.5 * nus[1, :]
        diff = (d.log_partition(nu2[np.newaxis, :])[0] - d.log_partition(nu1[np.newaxis, :])[0])[0]
        jac = d.log_partition(0.5 * nus[0:1, :] + 0.5 * nus[1:2, :], (
         False, True, False))[1]
        ...
        grad_check(fmu, nus, eps=1e-05)
        grad_check(fps, nus, eps=1e-05)
    ...
    def test_gniw(self):
        ...
        al = 1e-10
        x1 = al * x[1, :] - al * x[0, :] + 0.5 * x[0, :] + 0.5 * x[1, :]
        x2 = al * x[0, :] - al * x[1, :] + 0.5 * x[0, :] + 0.5 * x[1, :]
        diff = d.posterior_ll(x2[np.newaxis, :], nus, (True, False, False), True)[0] - d.posterior_ll(x1[np.newaxis, :], nus, (True, False, False), True)[0]
        jac = d.posterior_ll(0.5 * x[0:1, :] + 0.5 * x[1:2, :], nus, (
    ...
    def gen_data(self, A, mu, n=10):
        ...
        ys = np.einsum('ij,nj->ni', A, xs) + np.random.multivariate_normal(np.zeros(A.shape[0]), np.eye(A.shape[0]), size=n)
    ...
    def test_batch_vdp(self):
        data = self.data
        n, d = data.shape
        prob = VDP(GaussianNIW(d), k=50, w=0.4)
    ...
    def test_ll(self):
        ...
        x1 = al * x[1, :] - al * x[0, :] + 0.5 * x[0, :] + 0.5 * x[1, :]
        x2 = al * x[0, :] - al * x[1, :] + 0.5 * x[0, :] + 0.5 * x[1, :]
        diff = prob.ll(x2[np.newaxis, :])[0] - prob.ll(x1[np.newaxis, :])[0]
        jac = prob.ll(0.5 * x[0:1, :] + 0.5 * x[1:2, :], (
         False, True, False))[1]
        diff_ = (jac * (x2 - x1)[np.newaxis, np.newaxis, :]).sum(2)
    ...
    def test_resp(self):
        ...
        np.testing.assert_equal(np.histogram(np.argmax(ps, 1), range(self.nc + 1))[0], n)
        al = 1e-05
        x1 = al * x[1, :] - al * x[0, :] + 0.5 * x[0, :] + 0.5 * x[1, :]
        x2 = al * x[0, :] - al * x[1, :] + 0.5 * x[0, :] + 0.5 * x[1, :]
        diff = prob.resp(x2[np.newaxis, :])[0] - prob.resp(x1[np.newaxis, :])[0]
        jac = prob.resp(0.5 * x[0:1, :] + 0.5 * x[1:2, :], (
         False, True, False))[1]
        diff_ = (jac * (x2 - x1)[np.newaxis, np.newaxis, :]).sum(2)
        r = diff / diff_
    ...
    def test_online_vdp(self):
        hvdp = OnlineVDP(GaussianNIW(3), w=0.01, k=20, tol=0.001, max_items=100)
        for t in range(10):
    ...
    def test_gniw_conditionals(self):
        ...
        nus = np.vstack([ distr.prior_param + distr.sufficient_stats(data).sum(0) for data in self.data.reshape(self.nc, -1, self.data.shape[1])
                        ])
        nus = np.vstack((nus, nus))
        nx = 200
        z = np.random.normal(size=distr.prior.dim * nx).reshape(nx, distr.prior.dim)
        iy = (0, 1)
        ix = (2, 3, 4)
        x = z[:, ix]
