...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/dpcluster/algorithms.py
# Compiled at: 2013-06-21 16:43:47
import math, numpy as np, scipy.linalg, scipy.special
...
class VDP(object):
    ...
    def __init__(self, distr, w=0.1, k=50, tol=1e-05, max_iters=10000):
    ...
    def conditional_expectation(self, x, iy, ix, ret_ll_gr_hs=(True, False, False)):
        ...
        ex, exg, trash = self.distr.conditional_expectation(x, self.tau, iy, ix, ret_ll_gr_hs)
        ef = np.einsum('nki,nk->ni', ex, ps)
        efg = np.einsum('nka,nki->nia', psg, ex) + np.einsum('nk,nkia->nia', ps, exg)
        return (
         ef, efg, None)

    def conditional_variance(self, x, iy, ix, ret_ll_gr_hs=(
 True, False, False)):
        ps, psg, trash = self.marginal(ix).resp(x, ret_ll_gr_hs)
        ex, exg, trash = self.distr.conditional_expectation(x, self.tau, iy, ix, ret_ll_gr_hs)
        vr, vrg, trash = self.distr.conditional_variance(x, self.tau, iy, ix, ret_ll_gr_hs)
    ...
    def var_cond_exp(self, x, iy, ix, ret_ll_gr_hs=(
 True, False, False), full_var=False):
        ps, psg, trash = self.marginal(ix).resp(x, ret_ll_gr_hs)
        vr, vrg, trash = self.distr.conditional_variance(x, self.tau, iy, ix, ret_ll_gr_hs, full_var)
        ps2 = ps * ps
...
class OnlineVDP():
    ...
    def __init__(self, distr, w=0.1, k=25, tol=0.001, max_items=100):
        self.distr = distr
        self.w = w
        self.wm = w
        self.k = k
...
class Predictor():
    ...
    def predict(self, x, lgh):
        ...
        if lgh[1]:
            ...
            ypg = mug[:, iy, :] + np.einsum('nij,nja->nia', p, dfg) + np.einsum('nija,nj->nia', pg, df)
        return (yp, ypg, None)

    def predict_old(self, z, lgh=(
 True, True, False), full_var=False):
        ix = self.ix
        iy = self.iy
        x = z[:, ix]
        y = z[:, iy]
...
class PredictorKL():
    ...
    def predict(self, x, lgh):
        ix = self.ix
        iy = self.iy
        dstr, tau, taug, tsh = self.model.distr.conditional(x, self.model.tau, iy, ix, lgh)
        ps, psg, trash = self.model.marginal(ix).resp(x, lgh)
        taun = np.einsum('nk,nki->ni', ps, tau)
        (mu, Psi, n, nu), (mug, Psig, ng, nug), trs = dstr.prior.nat2usual(taun, lgh)
        yp = mu
        if lgh[1]:
            taung = np.einsum('nkj,nki->nij', psg, tau) + np.einsum('nk,nkij->nij', ps, taug)
