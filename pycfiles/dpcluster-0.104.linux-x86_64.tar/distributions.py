...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/dpcluster/distributions.py
# Compiled at: 2013-06-21 16:43:47
import math, numpy as np, scipy.linalg, scipy.special, matplotlib.pyplot as plt, time
...
class ExponentialFamilyDistribution:
    ...
    def log_base_measure(self, x, ret_ll_gr_hs=(
 True, False, False)):
        """
    ...
    def log_partition(self, nu, ret_ll_gr_hs=(
 True, False, False)):
        """
    ...
    def ll(self, xs, nus, ret_ll_gr_hs=(
 True, False, False)):
        """
        ...
        """
        return (
         np.einsum('ci,di->dc', nus, xs) + self.log_base_measure(xs)[0] - self.log_partition(nus)[0][np.newaxis, :],)
...
class Gaussian(ExponentialFamilyDistribution):
    ...
    def usual2nat(self, mus, Sgs):
        ...
        nu = np.hstack((nu1, -0.5 * nu2.reshape(nu2.shape[0], -1)))
        return nu
    ...
    def log_partition(self, nus):
        ...
        t1 = -0.25 * np.einsum('ti,tij,tj->t', nu1, inv, nu1)
        t2 = -0.5 * np.array(map(self.slogdet, -2 * nu2))[:, 1]
        return (t1 + t2,)
...
class NIW(ExponentialFamilyDistribution):
 ...
 True, True, True)):
    ...
    def log_partition(self, nu, ret_ll_gr_hs=(
 True, False, False), no_k_grad=False):
        rt = ret_ll_gr_hs
        ll = None
        gr = None
        hs = None
        ...
        if rt[0]:
            if not nu.size == 1:
                lmg = scipy.special.multigammaln(0.5 * nu, d)
            else:
                lmg = scipy.special.multigammaln(0.5 * nu[0], d)
            al = -0.5 * d * np.log(l3) + 0.5 * nu * (d * np.log(2) - ld) + lmg
            ll = al
        if rt[1]:
            inv = np.array(map(np.linalg.inv, psi))
            g1 = (nu / l3)[:, np.newaxis] * np.einsum('nij,nj->ni', inv, l1)
            g2 = -0.5 * nu[:, np.newaxis] * inv.reshape(l2.shape[0], -1)
            g3 = (-0.5 * d / l3 - 0.5 / l3 * (g1 * l1).sum(1))[:, np.newaxis]
            if not no_k_grad:
                g4 = (+0.5 * d * np.log(2) - 0.5 * ld + 0.5 * self.multipsi(0.5 * nu, d))[:,
                 np.newaxis]
            else:
    ...
    def sufficient_stats(self, mus, Sgs):
        ...
        nu = np.hstack((nu1, -0.5 * Sgi.reshape(Sgi.shape[0], -1)))
        t1 = -0.5 * (Sgi * mus[:, :, np.newaxis] * mus[:, np.newaxis, :]).sum(1).sum(1)
        t2 = -0.5 * np.array(map(np.linalg.slogdet, Sgs))[:, 1]
...
class ConjugatePair:
    ...
    def posterior_ll(self, x, nu, ret_ll_gr_hs=(
 True, False, False), usual_x=False):
        ...
        llk, grk, hsk = self.prior.log_partition(nu_p.reshape((-1, d)), ret_ll_gr_hs)
        if ret_ll_gr_hs[0]:
...
class GaussianNIW(ConjugatePair):
    ...
    def __init__(self, d):
        ConjugatePair.__init__(self, Gaussian(d), NIW(d), np.concatenate([np.zeros(d * d + d), np.array([0, 2 * d + 1])]))
    ...
    def posterior_ll_cache(self, nu, rt):
        ...
        if rt:
            ll0 = scipy.special.gammaln(0.5 * nu0) - scipy.special.gammaln(0.5 * nu) - 0.5 * np.array(map(np.linalg.slogdet, psi))[:, 1] - 0.5 * p * np.log(nu) - 0.5 * p * np.log(np.pi)
        else:
            ll0 = None
    ...
    def posterior_ll(self, x, nu, ret_ll_gr_hs=(True, False, False), usual_x=False):
        if not usual_x or ret_ll_gr_hs[2]:
            return ConjugatePair.posterior_ll(self, x, nu, ret_ll_gr_hs=ret_ll_gr_hs, usual_x=usual_x)
        else:
            rt = ret_ll_gr_hs
    ...
    @cached
    def conditional_expectation(self, x, nu, iy, ix, ret_ll_gr_hs=(
 True, True, False)):
        mygx, mx, P, V1, V2, n = self.conditionals_cache(nu, iy, ix)
        m = mygx + np.einsum('kij,nj->nki', P, x)
        return (m, P[np.newaxis, :, :, :], None)

    def conditional_variance(self, x, nu, iy, ix, ret_ll_gr_hs=(
 True, True, False), full_var=True):
    ...
    def conditional(self, x, nu, iy, ix, lgh=(True, True, False)):
        ...
        if lgh[1]:
            ...
            l2g = mub[:, :, :, wx, wx] * mub[:, :, wx, :, wx] * l3g[:, :, wx, wx, :] + P[wx, :, :, wx, :] * mub[:, :, wx, :, wx] * l3[:, :, wx, wx, wx] + mub[:, :, :, wx, wx] * P[wx, :, wx, :, :] * l3[:, :, wx, wx, wx]
