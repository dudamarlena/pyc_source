...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gnucash_importer/ledger.py
...
class Ledger:
    __doc__ = 'Meant to be the main interaction with GnuCash.'
    _account = None
    _currency = None
    _dry_run = None
    _src_file = None
    ...
    def get_quantity_transactions(self):
        ...
        for counter in ET.parse(self.src_file).getroot()[1].iter('{http://www.gnucash.org/XML/gnc}count-data'):
            if counter.attrib['{http://www.gnucash.org/XML/cd}type'] == 'transaction':
