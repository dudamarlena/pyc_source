...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pydozeoff/__init__.py
# Compiled at: 2010-07-16 20:09:16
"""
...
def _get_cmdline_options():
    ...
    parser.add_option('--slideshow-path', '-s', default='.', help='path to the presentation root directory (the one with a slideshow.py file)')
    build = optparse.OptionGroup(parser, 'Starting/building presentations')
    parser.add_option_group(build)
    build.add_option('--create', '-c', dest='presentation_name', default='', help='start a new presentation at PRESENTATION_NAME directory')
    build.add_option('--build', '-b', dest='output_dir', default='', help='create offline version of a presentation at OUTPUT_DIR')
    serve = optparse.OptionGroup(parser, 'Serving presentations')
    parser.add_option_group(serve)
    serve.add_option('--server-port', '-p', dest='port', type='int', default='8080', help='serve the presentation via HTTP at the given PORT')
    serve.add_option('--debug', '-d', action='store_true', default=False, help='whether the HTTP server should start in debug mode')
    return (
     parser, parser.parse_args()[0])