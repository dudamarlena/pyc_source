...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qqqfome/daemon.py
# Compiled at: 2016-02-12 01:59:45
...
class DaemonProcess:
    ...
    def __init__(self, pidfile, stdin=os.devnull, stdout=os.devnull, stderr=os.devnull, workdir='.', umask='022'):
    ...
    def _make_me_daemon(self):
        ...
        sys.stdout.flush()
        sys.stderr.flush()
        si = open(self._stdin, 'r') if isinstance(self._stdin, str) else self._stdin
        so = open(self._stdout, 'a+') if isinstance(self._stdout, str) else self._stdout
        if self._stderr:
            se = open(self._stderr, 'a+') if isinstance(self._stderr, str) else self._stdout
        else:
            se = so
    ...
    def start_when_pid_file_exist(self):
        raise FileExistsError(s.daemon_pid_file_exist_error.format(self._pidfile))
    ...
    def stop_when_pid_file_not_exist(self):
        raise FileNotFoundError(s.daemon_pid_file_not_found_error.format(self._pidfile))
