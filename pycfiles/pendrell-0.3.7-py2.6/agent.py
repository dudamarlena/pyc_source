...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/agent.py
...
from twisted.internet import error as netErr, interfaces as netInterfaces, protocol, reactor
...
from pendrell.error import RedirectedResponse, TooManyConnections, UnauthorizedResponse, InsecureAuthentication
...
class Agent(object):
    """User agent."""
    maxRedirects = 5
    identifier = ('{pkg}/{ver} ({txPkg}/{txVer}; Python/{pyVer})').format(pkg=_PACKAGE.capitalize(), ver=_VERSION, txPkg=txVersion.package.capitalize(), txVer=txVersion.short(), pyVer=('{0}.{1}.{2}').format(*sys.version_info))
    ...
    @inlineCallbacks
    def open(self, request, authenticator=None, authenticators=None, followRedirect=None, proxy=None, _redirectCount=0, _unauthCount=0, **kw):
        ...
        try:
            ...
            if not followRedirect or _redirectCount == self.maxRedirects or rr.status == http.SEE_OTHER:
                raise
            log.debug('Redirecting to %r' % rr.location)
            response = yield self.open(request.redirect(rr.location), followRedirect=followRedirect, proxy=proxy, _redirectCount=(_redirectCount + 1), authenticator=authenticator, authenticators=authenticators, _unauthCount=_unauthCount, **kw)
            ...
            response = yield self.open(request, followRedirect=followRedirect, proxy=proxy, _redirectCount=_redirectCount, authenticators=authenticators, _unauthCount=(_unauthCount + 1), **kw)
        ...
        while authenticators and authorization is None:
            ...
            for (scheme, params) in unauth.challenges:
                if self._supportedAuthenticationScheme(scheme, authenticator):
                    try:
                        authorization = yield maybeDeferred(authenticator.authorize, scheme, **params)
                    except Exception, e:
                        log.debug(e)
    ...
    def buildRequest(self, request, **kw):
        ...
        if self.preferredTransferEncodings:
            request.headers.setdefault('TE', (',').join(self.preferredTransferEncodings))
    ...
    _requesterClasses = {'http': HTTPRequester, 
