...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/messages.py
# Compiled at: 2010-10-16 17:04:10
...
class Message(object):
    ...
    url = property(lambda self: self._url, lambda self, url: self.setURL(url))
...
class Response(Message):
    ...
    def fromResponse(klass, response, **kwArgs):
        ...
        kw = dict(request=response.request, url=str(response.url), method=response.method, headers=deepcopy(response.headers), version=response.version, status=response.status, message=response.message)
        kw.update(kwArgs)
    ...
    @property
    def hasStatus(self):
        return bool(self.version is not None and 0 < self.status < 1000 and self.message is not None)
...
class Request(Message, urllib2_Request):
    ...
    def __init__(self, url, method='GET', headers=None, data=None, downloadTo=None, closeConnection=False, proxy=None, redirectedFrom=None, unredirectedHeaders=None, **kw):
        """
        """
        headers = headers or dict()
        urllib2_Request.__init__(self, str(url), data=data, headers=headers, origin_req_host=kw.get('origin_req_host', redirectedFrom), unverifiable=kw.get('unverifiable', False))
    ...
    def fromRequest(klass, request, **kwArgs):
        from copy import deepcopy
        kw = dict(closeConnection=request.closeConnection, data=request.data, downloadTo=request.downloadTo, headers=deepcopy(request.headers), method=request.method, redirectedFrom=request.redirectedFrom, unredirectedHeaders=deepcopy(request.unredirectedHeaders), url=request.url)
        kw.update(kwArgs)
        url = kw.pop('url')
        return klass(url, **kw)
    ...
    def buildResponse(self):
        ...
        if isinstance(downloadTo, basestring):
            ...
            response = StreamResponse(self, self.url, self.method, stream=downloadTo)
        else:
    ...
    def set_proxy(self, proxy):
        assert proxy is None or isinstance(proxy, Proxy), 'Expected Proxy, got %s' % proxy.__class__.__name__
        self.proxy = proxy
        return
