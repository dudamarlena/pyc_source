...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/cases/test_auth.py
...
class BasicAuthTest(unittest.TestCase):
    ...
    def test_authorize(self):
        ...
        self.assertEquals('Basic ' + ('user:p4ss').encode('base64').strip(), authorization)
...
class DigestAuthTest(unittest.TestCase):
    ...
    def test_authorize(self):
        ...
        self.assertEquals({'realm': self.params['realm'], 
           'nonce': self.params['nonce'], 
           'response': self.auth.staticResponse, 
           'uri': self.params['uri'], 
           'username': self.creds[0]}, dict((k, v.strip('"\'')) for (k, v) in [ a.strip().split('=') for a in authorization.split(',') ]))