...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/cases/test_agent.py
...
from twisted.internet.defer import Deferred, gatherResults, inlineCallbacks, returnValue, setDebugging as setDeferredDebugging
...
class DownloadTest(AgentTest):
    ...
    def test_downloadToPath(self):
        d = self.agent.open('http://www.yahoo.com/', downloadTo=self.tempPath, headers={'Accept-encoding': 'gzip'})
        ...
        def cb_assertFileNotEmpty(response, path):
            ...
            log.msg('%s is %dB: %r' % (path, fileSize,
             open(path).read()))
            self.assertTrue(fileSize > 500, 'Expected more than %d bytes' % fileSize)
            return response
...
class TimeoutTest(PendrellTestMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server = reactor.listenTCP(self._port, _TimeoutServerFactory(), interface='127.0.0.1')
    ...
    def test_timeout(self):
        ...
        try:
            rsp = yield self.getPage('http://127.0.0.1:%d/' % self._port, timeout=self.requestTimeout)
...
class AuthorizationTest(unittest.TestCase):
    ...
    class StoopidAuthenticator(auth.UserPassAuthenticatorBase):
        ...
        def authorize(self, scheme, **params):
            return ('{scheme} realm="{params[realm]}",name="{name}",secret="{secret}"').format(scheme=scheme, params=params, name=self.name, secret=self.secret)
    ...
    def setUp(self):
        ...
        self.server = reactor.listenTCP(self._port, self.site, interface='127.0.0.1')
    ...
    def test_authorize_installed_journaled(self):
        try:
            ...
            self.assertEquals([
             (
              'Stoopid',
              {'realm': 'bocas@tacos', 'uri': '/', 'question': "What's the password?", 
                 'method': 'GET'})], j.journal)
