...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/cases/test_jigsaw.py
# Compiled at: 2010-09-13 20:49:22
...
class JigsawTestMixin(PendrellTestMixin):
    ...
    def test_200TEGzip(self):
        response = yield self.getPage('http://jigsaw.w3.org/HTTP/TE/foo.txt', headers={'TE': 'gzip'})
        self.assertEquals(200, response.status)
    ...
    def test_200TEDeflate(self):
        response = yield self.getPage('http://jigsaw.w3.org/HTTP/TE/foo.txt', headers={'TE': 'deflate'})
        self.assertEquals(200, response.status)
