...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/requester.py
# Compiled at: 2010-12-12 19:41:17
...
class Multiplexer(object):
    ...
    def buildRequester(self):
        return self.requesterClass(self.scheme, self.host, self.port, timeout=self.timeout)
    ...
    def _waitForAvailableRequester(self):
        ...
        elif self.maxConnections is None or len(self._requesters) < self.maxConnections:
            requester = self.buildRequester()
            self._requesters.append(requester)
        else:
            assert self.maxConnections is None or len(self._requesters) == self.maxConnections
            (requester, idx) = yield DeferredList([ r.waitForAvailability() for r in self._requesters ], fireOnOneCallback=True)
        returnValue(requester)
        return

    def loseConnection(self):
        return DeferredList([ r.loseConnection() for r in self._requesters ])
...
class RequesterBase(_ClientFactory):
    ...
    def disconnected(self):
        return bool(self._connector is None or self._connector.state == 'disconnected')
