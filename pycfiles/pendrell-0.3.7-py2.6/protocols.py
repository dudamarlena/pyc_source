...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/protocols.py
# Compiled at: 2010-12-26 02:29:02
...
from pendrell.error import IncompleteResponse, RedirectedResponse, ResponseTimeout, RetryResponse, UnauthorizedResponse, WebError, FailableMixin
...
class HTTPProtocol(basic.LineReceiver, policies.TimeoutMixin):
    ...
    def _currentResponseHasContent(self):
        return not (self._currentResponse.status in NO_BODY_CODES or self._currentResponse.request.method == 'HEAD' or self._contentLength == 0)
    ...
    def _processContent(self, raw):
        if self._chunkDecoder:
            ...
            if self._contentLength is not None:
                assert 0 <= self._contentSize <= self._contentLength, 'Content size (%d) is greater than content length (%d)' % (
...
class SOCKSv4ClientProtocol(protocol.Protocol):
    ...
    _CONNECT_CODE = 1
    _BIND_CODE = 2
    _REQUEST_GRANTED_CODE = 90
    _REQUEST_REJECTED_CODE = 91
    _REQUEST_REJECTED_IDENTD_CODE = 92
    _REQUEST_REJECTED_USER_CODE = 93
...
class SOCKSv4aClientProtocol(SOCKSv4ClientProtocol):
    ...
    def openConnection(self, host, port, user=''):
        ...
        domain = host
        packed = pack('!BBH', version, command, port) + server + user + '\x00' + host + '\x00'
        self.transport.write(packed)
...
class SOCKSIdentdRejected(SOCKSRejected):
    message = (
     'SOCKS request rejected because the server ' + 'cannot connect to identd on the client.',)


class SOCKSUserRejected(SOCKSRejected):
    message = 'SOCKS request because the client program and ' + 'identd report different user-ids.'
