...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/cases/test_functional.py
...
class PendrellDownloadTest(PendrellTestMixin, TestCase):
    ...
    def test_downloadRedirectedYahooToPath(self):
        ...
        page = yield self.getPage(url, downloadTo=self.tempPath, headers=gzipHeaders)
    ...
    def test_downloadYahooToPath(self):
        ...
        page = yield self.getPage(url, downloadTo=self.tempPath, headers={'Accept-encoding': 'gzip'})
        self.assertEquals(200, page.status)
        self.assertHeader(page, 'Content-encoding', 'gzip')
        contentLength = len(page)
        fileSize = os.stat(self.tempPath).st_size
...
class MD5Test(PendrellTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.md5Server = reactor.listenTCP(self.http_port, Site(MD5Root()), interface=self._lo0)
...
class RequestTest(PendrellTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.server = reactor.listenTCP(self.http_port, self.site, interface=self._lo0)
    ...
    def test_request_query(self):
        ...
        self.assertEquals([('GET', '/?whatup=notmuch', 'HTTP/1.1')], self.site.journal)