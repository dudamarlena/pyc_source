...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pendrell/error.py
# Compiled at: 2010-09-13 19:05:21
import re
...
filterwarnings('ignore', category=DeprecationWarning, message='BaseException\\.message has been deprecated as of Python 2\\.6')
...
class WebError(webErr.Error, FailableMixin):
    ...
    def __init__(self, response):
        webErr.Error.__init__(self, response.status or 0, message=response.message, response=response)
...
class RedirectedResponse(webErr.PageRedirect, WebError):
    ...
    def __init__(self, response):
        webErr.PageRedirect.__init__(self, response.status, location=response.headers['location'][0], response=response)
