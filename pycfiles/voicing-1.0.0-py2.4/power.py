...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/voicing/power.py
# Compiled at: 2008-03-22 09:08:15
import Num, math, hilbert_xform
...
def test1():
    ...
    for i in range(20):
        ...
        assert Num.alltrue(Num.equal(Num.less(x, 0.5), Num.less(xk, 0.5)))
        assert Num.alltrue(Num.equal(Num.less(x, 0.5), Num.less(xs0, 0.5)))
        assert Num.alltrue(Num.equal(Num.less(x, 0.5), Num.less(xs1, 0.5)))
        assert Num.alltrue(Num.equal(Num.less(x, 0.5), Num.less(xs2, 0.5)))
