...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/voicing/voice_misc.py
# Compiled at: 2008-02-23 19:39:35
import Num, math, ortho_poly, die
_wincache = {}
...
def window(n, k):
    ...
    w = (1 + Num.cos(opn.x * math.pi)) * opn.P(k)
    o = w / math.sqrt(Num.sum(w ** 2))
    if len(_wincache) > 20:
        _wincache.popitem()
    _wincache[cachekey] = o
    return o
...
def factor(n):
    """Factors numbers into a list of prime factors,
        in increasing order.
        """
    try:
...
def hipass_first_order(d, cutoff_freq):
    ...
    fr = complex(0.0, 1.0) * (f / cutoff_freq)
    ff = fr / (1 + fr)
    ff[0] = ff[0].real
    ff[-1] = ff[(-1)].real
...
def bandpass_first_order(d, cutoff_hp, cutoff_lp):
    ...
    dx = Num.FFT.rfft(d, nx)
    f = real_fft_freq(dx.shape[0])
    frlp = complex(0.0, 1.0) * (f / cutoff_lp)
    frhp = complex(0.0, 1.0) * (f / cutoff_hp)
