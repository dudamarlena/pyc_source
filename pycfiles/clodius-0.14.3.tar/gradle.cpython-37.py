...
# Embedded file name: /Users/MyKings/Documents/github/clocwalk/clocwalk/libs/analyzer/gradle.py
# Compiled at: 2020-02-27 01:00:32
...
def find_include_file(content):
    ...
    """
    result = None
    kw = re.compile('rootProject\\.file\\("(.+)?"\\)')
    conf_content = ''
    ...
    if find_list:
        result = [_ for _ in list(set(find_list)) if _.endswith('.gradle')]
    return result
...
def find_keyword_block(content, keyword='dependencies', l_bracket='{', r_bracket='}'):
    ...
    if content_list:
        ...
        for item in content_list:
            ...
            if is_found_keyword:
                if item.strip():
                    if item.strip() not in (r_bracket,):
...
def find_product_info(content, origin_file=None):
    ...
    for item in conf_content:
        ...
        if ' group ' in item and ' name ' in item and ' version ' in item:
            ...
            product = {'new_version':'', 
             'cve':'',  'parent_file':'',  'origin_file':origin_file}
            for b in line.split(','):
                if ':' in b:
                    key, value = b.split(':')
                    if 'group' in key:
            ...
            info_re = re.search('[\\"\']{1}(.+?)[\\"\']{1}', current_line)
        if info_re:
            info = info_re.group(1).split(':')
            if len(info) == 2:
                result.append({'vendor':info[0], 
                 'name':info[1], 
                 'version':'', 
                 'new_version':'', 
                 'cve':'', 
