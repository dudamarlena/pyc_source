...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\sc2common\containers.py
# Compiled at: 2018-12-24 11:38:03
# Size of source mod 2**32: 20857 bytes
...
class RestrictedType(object):
    __doc__ = 'USAGE: subclass this and redefine ALLOWED_TYPES to the limited type values\n    NOTE: if ALLOWED_TYPES is a dict, its values maps to sc2clientprotocol\n          internal values.'
    ALLOWED_TYPES = []

    def __init__(self, pType):
        if isinstance(pType, type(self)):
    ...
    def __setattr__(self, key, value):
        if key != 'type':
            ...
            if isinstance(allowed, dict):
                ...
                if len(result) == 0:
                    raise ValueError("given value '%s' %s is not a %s value.  Allowed:%s" % (
                     value, type(value), type(self), list(allowed)))
                elif len(result) > 1:
                    raise ValueError("given value '%s' %s defined too many matching values: %s" % (
    ...
    def __eq__(self, other):
        if isinstance(other, RestrictedType):
            if isinstance(self.type, MultiType):
                if isinstance(other.type, MultiType):
    ...
    def __lt__(self, other):
        ...
        if not isinstance(other, RestrictedType):
            ...
            if isinstance(other, MultiType):
                othrValue = other.type
            else:
...
class pySC2protocolObj(object):
    __doc__ = 'convert sc2clientprotocol object into py obj with all attributes defined'
    ...
    def __init__(self, protocolAttrNames, sc2protData=None):
        ...
        if isinstance(protocolAttrNames, pySC2protocolObj):
...
class MultiType(object):
    __doc__ = 'a simple class that allows comparison against both name and code value'
...
class MapPoint(object):
    __doc__ = 'define a very simple means to represent 2D/3D space'
...
class Cost(object):
    __doc__ = 'the amount of resources required to activate'
    ...
    def __eq__(self, otherCost):
        if not isinstance(otherCost, Cost):
            return False
        else:
            return self.mineral == otherCost.mineral and self.vespene == otherCost.vespene and self.supply == otherCost.supply and self.energy == otherCost.energy and self.time == otherCost.time and self.cooldown == otherCost.cooldown
    ...
    def __int__(self):
        ...
        return int(self.mineral or self.vespene or self.supply or self.energy or self.time or self.cooldown)
    ...
    def __sub__(self, otherCost):
        return Cost(mins=(self.mineral - otherCost.mineral),
