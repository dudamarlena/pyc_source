...
# Embedded file name: build/bdist.linux-i686/egg/pylocator/markers.py
# Compiled at: 2012-04-18 10:24:56
__doc__ = 'Define vtkActors for use in PyLocatorRenderWindow'
import math, vtk, uuid, numpy as n
...
class Marker(vtk.vtkActor):
    ...
    def contains(self, xyz):
        ...
        if xyz is None:
            ...
            d = math.sqrt(vtk.vtkMath.Distance2BetweenPoints(self.sphere.GetCenter(), xyz))
            if d < self.sphere.GetRadius():
                return 1
            return 0
            return
    ...
    def deep_copy(self):
        m = Marker(xyz=self.sphere.GetCenter(), radius=self.sphere.GetRadius(), rgb=self.GetProperty().GetColor(), uuid_=self.uuid)
