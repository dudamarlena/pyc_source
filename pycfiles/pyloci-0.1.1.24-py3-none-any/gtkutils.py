...
# Embedded file name: build/bdist.linux-i686/egg/pylocator/gtkutils.py
# Compiled at: 2012-04-18 08:50:56
import os, sys, StringIO, traceback, gobject, gtk
from gtk import gdk
...
def simple_msg(msg, parent=None, title=None):
    dialog = gtk.MessageDialog(parent=None, type=gtk.MESSAGE_INFO, buttons=gtk.BUTTONS_OK, message_format=msg)
    if parent is not None:
        dialog.set_transient_for(parent)
    if title is not None:
        dialog.set_title(title)
...
class Dialog_FileSelection(gtk.FileSelection):
    ...
    def __init__(self, defaultDir, okCallback, title='Select file', parent=None):
        ...
        self.cancel_button.connect('clicked', lambda *args: self.destroy())
        if parent is not None:
            self.set_transient_for(parent)
        self.show()
        return
...
class Dialog_DirSelection(gtk.FileSelection):
    ...
    def file_ok_sel(self, w):
        ...
        if not os.path.isdir(thisDir):
            simple_msg('You must select a directory\n%s is not a dir' % thisDir)
            return
...
def not_implemented(parent=None, *args):
    ...
    simple_msg('Not implemented yet; sorry!', title='Error: Feature Unimplemented', parent=parent)
...
def yes_or_no(msg, title, responseCallback, parent=None):
    ...
    def response(dialog, response):
        ...
        dialog.destroy()
    """
    dialog = gtk.MessageDialog(parent=parent, flags=gtk.DIALOG_DESTROY_WITH_PARENT, type=gtk.MESSAGE_INFO, buttons=gtk.BUTTONS_YES_NO, message_format=msg)
    dialog.set_title(title)
...
def get_num_range(minLabel='Min', maxLabel='Max', title='Enter range', parent=None, as_times=False):
...
def get_num_value(labelStr='Value', title='Enter value', parent=None, default=None):
    """Get a numeric value"""
    dlg = gtk.Dialog(title)
    if parent is not None:
        dlg.set_transient_for(parent)
...
def get_two_nums(label1Str='Min', label2Str='Max', title='Enter numbers', parent=None, tooltip1=None, tooltip2=None):
...
def get_three_nums(label1Str='Value 1', label2Str='Value 2', label3Str='Value 3', value1='', value2='', value3='', title='Enter numbers', parent=None, tooltip1=None, tooltip2=None, tooltip3=None):
...
class OpenSaveSaveAsHBox(gtk.HBox):
    ...
    def save_as(self, button):
        filename = self.fmanager.get_filename(title='Select filename to save to')
        if filename is None:
            return
        else:
    ...
    def save(self, button, **kwargs):
        ...
        if filename is None:
            filename = self.fmanager.get_filename(title='Save to filename')
        if filename is None:
            return
        else:
            try:
            ...
            if self.savehook is not None:
                ...
                if ok:
                    self.filename = filename
                    simple_msg('Saved markers to %s' % filename, parent=self.parentWin)
            return
...
gtk.main()
    ...
    def make_toolbar(self):
        ...
        button = toolbar.append_item('Save', 'Save as CSV', 'Private', iconw, self.save)
        return toolbar
...
class MyToolbar(gtk.Toolbar):
    ...
    def _init_toolbar2_2(self):
        for text, tooltip_text, stock, callback in self.toolitems:
            ...
            w = self.append_item(text, tooltip_text, 'Private', image, getattr(self, callback))
