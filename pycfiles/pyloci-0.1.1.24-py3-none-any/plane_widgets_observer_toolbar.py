...
# Embedded file name: build/bdist.linux-i686/egg/pylocator/plane_widgets_observer_toolbar.py
# Compiled at: 2012-04-18 08:59:54
import gtk
...
class ObserverToolbar(gtk.Toolbar):
    ...
    def __init__(self, pwo):
        ...
        self.append_item('Ortho', 'Restore orthogonality of plane widget', 'Private', iconw, ortho)
        ...
        self.append_item('Jump to', 'Move the planes to the point under the cursor or\n' + 'to the selected marker if only one marker is selected', 'Private', iconw, jumpto)
        ...
        def coplanar(self):
            ...
            planeO.SetCenter((p1[0] + p2[0] + p3[0]) / 3, (p1[1] + p2[1] + p3[1]) / 3, (p1[2] + p2[2] + p3[2]) / 3)
            planeO.Update()
            pwxyz = pwo.get_pwxyz()
        ...
        self.append_item('Set plane', 'Set the plane to be coplanar with 3 selected electrodes', 'Private', iconw, coplanar)