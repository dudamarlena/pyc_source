...
# Embedded file name: build/bdist.linux-i686/egg/pylocator/surf_renderer_props.py
# Compiled at: 2012-04-18 09:59:52
...
class SurfRendererProps(gtk.VBox):
    ...
    def _make_general_properties_expander(self):
        ...
        scrollbar.set_increments(0.05, 0.2)
    ...
    def __adjust_scrollbar_threshold_for_data(self):
        valid_increments = sorted([ 1.0 * 10 ** e for e in range(-2, 3) ] + [ 2.0 * 10 ** e for e in range(-2, 3) ] + [ 5.0 * 10 ** e for e in range(-2, 3) ])
        min_, median_, max_ = EventHandler().get_nifti_stats()
    ...
    def cb_edit_name(self, *args):
        ...
        if not new_name or new_name == param.label:
            return
