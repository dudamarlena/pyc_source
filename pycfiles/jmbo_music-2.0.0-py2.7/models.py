...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/music/models.py
...
class Credit(models.Model):
    contributor = models.ForeignKey('music.TrackContributor', related_name='credits')
    track = models.ForeignKey('music.Track', related_name='credits')
...
class TrackContributor(ModelBase):
    ...
    track = models.ManyToManyField('music.Track', through='music.Credit', related_name='contributors')
...
class Track(ModelBase):
    contributor = models.ManyToManyField('music.TrackContributor', through='music.Credit', related_name='tracks')
    album = models.ManyToManyField(Album, blank=True, null=True)
    audio_embed = models.TextField(blank=True, null=True, help_text='An audio embed script related to the track.')
    video_embed = models.TextField(blank=True, null=True, help_text='Embedding markup as supplied by Youtube.')
    last_played = models.DateTimeField(blank=True, null=True, db_index=True)
    length = models.IntegerField(blank=True, null=True, help_text='Length of track in seconds.')
    ...
    def get_primary_contributors(self, permitted=False):
        ...
        """
        primary_credits = []
        credits = self.credits.exclude(credit_option=None, credit_option__role_priority=None).order_by('credit_option__role_priority')
        if credits.exists():
        ...
        for credit in primary_credits:
            ...
            if permitted == False or permitted == True and contributor.is_permitted:
    ...
    def create_credit(self, contributor_title, role_type):
        ...
        if credit_options:
            ...
            credit_option = CreditOption.objects.create(role_type=role_type, role_name=role_type)
        credit, created = Credit.objects.get_or_create(contributor=contributor, track=self, credit_option=credit_option)
        return (
         credit, contributor)
    ...
    def youtube_id(self):
        ...
        if not self.video_embed:
            return ''
        m = re.search('/embed/([A-Za-z0-9\\-=_]*)', self.video_embed)
        if m:
            return m.group(1)
        return ''
...
class CreditOption(models.Model):
    ...
    role_type = models.CharField(max_length=16, default='artist', choices=(
     ('artist', 'artist'), ('composer', 'composer'), ('other', 'other')), help_text='An identifier used to do internal queries. Pick one that closest resembles the credit option.')
    role_name = models.CharField(max_length=256, blank=True, null=True, help_text='A friendly name for the credit option.')
    role_priority = models.IntegerField(blank=True, null=True, help_text='An artist is typically more important than the producer. Set role priorities accordingly.')
