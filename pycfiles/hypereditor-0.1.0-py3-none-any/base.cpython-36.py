...
# [GCC 8.4.0]
# Embedded file name: /Users/shimul/Projects/django-hyper-editor/hypereditor/blocks/base.py
# Compiled at: 2019-04-10 08:59:34
...
class CodeRenderer(object):
    __doc__ = 'An Utility Class for Collecting CSS and JS from all Blocks in Node Tree'
    css = []
    js = []
...
def _build_background_image(image):
    return {'background':'url("' + image['url'] + '")', 
     'background-repeat':'no-repeat', 
     'background-size':'cover'}


def _build_background_color(bgcolor):
    return {'background-color': 'rgba({r}, {g}, {b}, {a})'.format(r=(bgcolor.get('r')),
                           g=(bgcolor.get('g')),
                           b=(bgcolor.get('b')),
                           a=(bgcolor.get('a')))}
...
def _build_foreground_color(fgcolor):
    return {'color': 'rgba({r}, {g}, {b}, {a})'.format(r=(fgcolor.get('r')),
                g=(fgcolor.get('g')),
                b=(fgcolor.get('b')),
                a=(fgcolor.get('a')))}
...
class Block(object):
    ...
    def get_block_config(self):
        ...
        if not self.external:
            default_values = {'settings':self.initial if self.initial else {},  'general':self.general_initial if self.general_initial else {}}
            result = {'title':self.title, 
             'description':self.description, 
             'settings_schema':self.schema, 
             'default_values':default_values, 
             'config':{'styles': self.styles if self.styles else [{'id':'default',  'name':'Default'}]}}
            return result
        else:
            return