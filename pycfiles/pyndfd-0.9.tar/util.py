...
# Embedded file name: build/bdist.linux-i686/egg/pyndexter/util.py
# Compiled at: 2007-02-13 11:18:00
...
class URI(object):
    ...
    """
    __module__ = __name__
    _pattern = re.compile('(?:(?P<scheme>[^:]+)://)?(?:(?P<username>[^:@]*)(?::(?P<password>[^@]*))?@)?(?P<host>[^?/#:]*)(?::(P<port>[\\d+]+))?(?P<path>/[^#?]*)?(?:\\?(?P<query>[^#]*))?(?:#(?P<fragment>.*))?')
    __slots__ = ('scheme', 'username', 'password', 'host', 'port', '_path', 'query',
                 'fragment')

    def __init__(self, uri=None, scheme='', username='', password='', host='', port='', path='', query={}, fragment=''):
        self._path = ''
        if isinstance(uri, URI):
            from copy import copy
            (self.scheme, self.username, self.password, self.host, self.port, self.path, self.query, self.fragment) = (
             uri.scheme, uri.username, uri.password, uri.host, uri.port, uri.path, copy(uri.query), uri.fragment)
            ...
            groups = map(unquote, groups[0:6]) + [parse_qs(groups[6] or '')] + map(unquote, groups[7:])
            (self.scheme, self.username, self.password, self.host, self.port, self.path, self.query, self.fragment) = groups
        else:
            (self.scheme, self.username, self.password, self.host, self.port, self.path, self.query, self.fragment) = (scheme, username, password, host, port, path, query, fragment)
        return
    ...
    def __str__(self):
        uri = unicode(self.scheme and quote(self.scheme) + '://' or '')
        if self.username or self.password:
            if self.username:
                uri += quote(self.username)
            if self.password:
                uri += ':' + quote(self.password)
            uri += '@'
        uri += quote(self.host)
        if self.port:
            uri += ':%s' % port
        uri += quote(self.path)
        if self.query:
            uri += '?' + ('&').join([ ('&').join([ '%s=%s' % (k, quote(str(v))) for v in l ]) for (k, l) in sorted(self.query.items()) ])
        if self.fragment:
            uri += '#' + quote(self.fragment)
        return uri
