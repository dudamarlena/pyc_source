...
# Embedded file name: build/bdist.linux-i686/egg/pyndexter/__init__.py
# Compiled at: 2007-02-15 02:50:24
...
import re, os, pickle, gzip, inspect
...
class Document(object):
    ...
    def __init__(self, uri, content=None, source=None, changed=None, quality=1.0, **attributes):
    ...
    def __repr__(self):
        return '<%s %s>' % (self.__class__.__name__, (' ').join([ '%s=%s' % (k, repr(v)) for (k, v) in self.attributes.iteritems() ]))
    ...
    content = property(lambda self: self._get_content(), lambda self, value: self._set_content(value))
...
class Source(object):
    ...
    def marshal(self, file):
        ...
        gzip.GzipFile(filename='pyndexter source state', fileobj=file, mode='wb', compresslevel=1).write(state)
    ...
    def difference(self, file):
        ...
        try:
            ...
            raise InvalidState('Invalid state provided to document source. Exception was %s: %s' % (e.__class__.__name__, e))
...
class Query(QueryNode):
    ...
    """
    __module__ = __name__
    _tokenise_re = re.compile('(?P<ex>-)|(?P<or>or)|\\"(?P<dq>(?:\\\\.|[^\\"])*)\\"|\'(?P<sq>(?:\\\\.|[^\'])*)\'|(?P<te>(?:\\S)+)', re.I)
    _group_map = {'dq': QueryNode.TERM, 'sq': QueryNode.TERM, 'te': QueryNode.TERM, 'ex': QueryNode.NOT, 'or': QueryNode.OR}
    ...
    def terms(self, exclude_not=True):
        ...
        def _convert(node):
            ...
            if node.type == node.TERM:
                ...
                if node.type == node.NOT and exclude_not:
                    return
                else:
    ...
    def __call__(self, text):
        ...
        misc.set_filename('<%s compiled query>' % self.__class__.__name__, qast)
    ...
    def as_string(self, and_=' AND ', or_=' OR ', not_='NOT '):
        ...
        def _convert(node):
            ...
            if node.type == node.AND:
                return '%s%s%s' % (_convert(node.left), and_, _convert(node.right))
            elif node.type == node.OR:
                return '%s%s%s' % (_convert(node.left), or_, _convert(node.right))
            elif node.type == node.NOT:
    ...
    def _tokenise(self, phrase):
        ...
        [(2, '-'), (1, 'one')]
        """
        tokens = [ (self._group_map[token.lastgroup], token.group(token.lastindex)) for token in self._tokenise_re.finditer(phrase) ]
        return tokens
...
class Reducer(object):
    ...
    def __init__(self, words_re=re.compile('\\w+'), stemmer=lambda w: w, min_word_length=3, max_word_length=64, unique=False, split=False, lower=True):
...
class PluginFactory(object):
    ...
    def __init__(self, plugin, **arg_types):
        ...
        constructor."""
        self.plugin = plugin
        self.remapped = dict([ (k, v) for (k, v) in arg_types.iteritems() if isinstance(v, basestring) ])
        self.arg_types = arg_types
        (args, varargs, self.varkw, defaults) = inspect.getargspec(self.plugin.__init__)
    ...
    def __call__(self, uri=None, **kwargs):
        ...
        if uri is not None:
            ...
            uri_components = {'username': uri.username, 'password': uri.password, 'host': uri.host, 'path': uri.path, 'fragment': uri.fragment}
        ...
        for (k, v) in args.items():
            if v is not None:
                ...
                if isinstance(v, (tuple, list)) and not isinstance(type, self.List):
                    if len(v) != 1:
                ...
                if type is bool:
                    ...
                    try:
                        ...
                        raise ValueError('could not coerce argument "%s" with value "%s" to type "%s": %s' % (k, v, type, e))
...
class Framework(object):
    ...
    def __init__(self, indexer=None, mode=READWRITE, state_store=None, reduce=None, stemmer=None):
    ...
    def update(self, filter=None, context=None):
        ...
        if not self.state_store:
            raise IndexerError('Indexer not capable of storing source state, and store not provided to Framework - not capable of automatic updates.')
        if not filter:
        ...
        if self.state_store.exists():
            ...
            for (transition, uri) in filter(self, context, self.source.difference(store)):
    ...
    def _assert_rw(self):
        if self.mode != READWRITE:
            raise InvalidMode('%s must be in READWRITE mode for this operation' % self.__class__.__name__)
...
class Hit(object):
    ...
    def __repr__(self):
        return '<Hit %s>' % (' ').join([ '%s=%s' % (k, repr(v)) for (k, v) in self.attributes.iteritems() ])
