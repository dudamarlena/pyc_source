...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aspects.py
# Compiled at: 2008-08-10 13:17:10
"""
Aspects library allows wrapping methods and functions. Wraps are
functions which are called before the wrapped functions. A wrap may or
may not call the wrapped function at some point of their execution.

There are two kinds of wraps: old-style and new-style. New-style wraps
can be used in Python 2.5 and later.
...
2. Peeling off the topmost wrap:

   both styles: peeled_wrap = aspects.peel_around(func)

   peel_around returns the removed wrap. The peeled wrap can be used
   to wrap the same or any other method:
   aspects.with_wrap(peeled_wrap, func2)
...
4. Old-style wraps
   ...
   An old-style wrap is an ordinary function that is called with the
   arguments ment to the original method. The return value of the
   old-style wrap is received by the caller of the method.

   The original method is called from an old-style wrap as follows:

   orig_retval = self.__proceed(args_here)

   where orig_retval is the return value of the original
   method. Exceptions raised by the original method can be caught in
   the wrap.

5. New-style wraps

   New-style wraps are generators. The original function is called
   inside a wrap as follows:

   orig_retval = yield aspects.proceed

   to call with the same arguments as passed to the wrap, or

   orig_retval = yield aspects.proceed(args_here)

   to call with the specified arguments. Exceptions raised by the
   original method can be caught in the wrap if the yield statement is
   inside a try-except block.

   The wrap can return a value by yielding

   yield aspects.return_stop

   to return the same value as returned by the original function (if
   the original function was not called AspectsException is raised), or

   yield aspects.return_stop(value_here)

   to return the specified value (the original function needs not to
   ...
   time. However, the value can be also returned with return_cont:

   (args, kw_args) = yield aspects.return_cont

   returns the same value as returned by the original method, and

   (args, kw_args) = yield aspects.return_cont(value_here)

   returns the specified value. When the wrapped function is called,
   its call arguments are now passed to args and kw_args, and the
...
6. Pros and cons of new-style wraps compared to old-style

   + Allows wrapping functions and methods (old-style only methods)
   
   + Execution of a wrap can be continued in the middle (values of
   local variables are saved, performance is better)
   
   + New attributes are not added to the class or the module of the
   function when it is wrapped.

   - Requires at least Python 2.5.
   
"""
version_info = (1, 1)
import re, thread, types, inspect, sys
...
def wrap_around_re(aClass, wildcard, advice):
    ...
    for aMember in dir(aClass):
        ...
        if callable(realMember) and aMember[:6] != '__wrap' and aMember[:9] != '__proceed' and matcher.match(aMember):
...
def with_wrap(advice, *methods, **kwargs):
    ...
    if len(methods) == 1:
        return _with_wrap(advice, methods[0], instances=insts)
    retval = []
    for m in methods:
...
def _with_wrap(advice, method, instances=[]):
    ...
    try:
        ...
        orig_method = method
        new_method = _create_middleman(orig_method, advice)
        try:
            replace_method = getattr(method, '__aspects_rmf')
        except AttributeError:
            if hasattr(method, 'im_class'):
                replace_method = lambda orig, new: setattr(method.im_class, orig, new)
            elif hasattr(method, 'func_globals'):
                replace_method = lambda orig, new: method.func_globals.__setitem__(orig, new)
            else:
                ...
                replace_method = lambda orig, new: m.__dict__.__setitem__(orig, new)
        ...
        new_method.__aspects_cont = None
...
def _create_middleman(method, generator_advice):

    def generator_middleman(*args, **kwargs):
        if generator_middleman.__aspects_enabled == 0 or generator_middleman.__aspects_instances and id(args[0]) not in generator_middleman.__aspects_instances:
...
def wrap_around(method, advice, instances=[]):
    ...
    try:
        ...
        if len(instances) > 0:
            ...
            cond_execute_wrap = '(' + cond_execute_wrap + ')' + 'and (id(self) in self.' + instanceids_attr_name + ')'
...
def _peel_wrap(method, wrap):
    peeled_wraps = []
    peeled_wrap = aspects.peel_around(method)
    while method.__aspects_adv != wrap:
        peeled_wraps.append(peeled_wrap)
        method = getattr(method.im_class, method.__name__)
        peeled_wrap = aspects.peel_around(method)

    while peeled_wraps:
        method = getattr(method.im_class, method.__name__)
        aspects.with_wrap(peeled_wraps.pop(), method)
...
def peel_around(method):
    ...
    _permission_to_touch_wraps.acquire()
    try:
        if hasattr(method, '__aspects_enabled'):
...
def wrap_count(method):
    ...
    """
    return getattr(method, '__aspects_wrapid', -1) + 1
...
def _names_mce(method, wrap_number=None):
    ...
    if wrap_number != None:
        ...
        enabled_attr_name = None
    return (
     methods_name, methods_class, enabled_attr_name)
...
def _create_function(function_code, function_name, thread_lib=1):
    ...
    gl = {'_Proceed_stack_entry': _Proceed_stack_entry, '_remove_proceed_stack': _remove_proceed_stack}
    lo = {}
    exec (
     codeobj, gl, lo)
