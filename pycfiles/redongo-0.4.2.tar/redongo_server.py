...
# [GCC 8.4.0]
# Embedded file name: /home/mgalan/projects/redongo/redongo/redongo_server.py
# Compiled at: 2016-11-10 13:12:50
import datetime, logging, logging.handlers, os, pymongo, pytz, redis
from redis.sentinel import Sentinel
from redis.exceptions import RedisError
import server_exceptions, general_exceptions, signal, stoneredis, sys, time, traceback, utils, redongo_client, cipher_utils, serializer_utils, queue_utils
...
class RedongoServer(object):
    ...
    def __init__(self, mode, *args, **kwargs):
        ...
        self.max_expiration_time = 2592000
    ...
    def create_redis_connection(self):
        if self.mode == 'Redis':
            ...
            SENTINEL_POOL = Sentinel(options.sentinelServers, socket_timeout=0.1, max_connections=1000)
            self.redis = SENTINEL_POOL.master_for(options.sentinelName, redis_class=stoneredis.StoneRedis, socket_timeout=5, socket_connect_timeout=5)
                ...
                if object_found:
                    while self.objs:
                        try:
                            ...
                            except (server_exceptions.ObjectValidationError, general_exceptions.ApplicationSettingsError) as e:
                        ...
                        except (ValueError, TypeError, IndexError, ImportError, pickle.PickleError) as e:
    ...
    def handle_application_bulk(self, application_name):
        ...
        except (PyMongoError, InvalidDocument) as e:
    ...
    def create_add_query(self, obj, previous_field='', query=None):
        ...
        for field, value in obj.iteritems():
            if field == '_id':
                continue
            type_field = type(value)
            if type_field is int or type_field is long or type_field is float or type_field is bool:
...
def validateSentinelClient(parser, options):
    required_options = [
     'SENTINEL_SERVERS', 'SENTINEL_NAME']
    return validate(parser, options, required_options, silent=False)
