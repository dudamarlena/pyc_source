...
# [GCC 8.4.0]
# Embedded file name: /Users/jeremydw/git/pygrow/env/lib/python2.7/site-packages/jetway/jetway.py
# Compiled at: 2015-02-25 20:21:55
...
import base64, httplib2, logging, md5, mimetypes, os, progressbar, requests, threading
...
class Jetway(object):
    ...
    def __init__(self, project, name, host, secure=False, username='default', api='jetway', version='v0'):
        if '/' not in project:
    ...
    def get_service(self, username='default', reauth=False):
        ...
        return discovery.build(self._api, self._version, discoveryServiceUrl=self._url, http=http)
    ...
    def get_credentials(username, reauth=False):
        ...
        if credentials and not credentials.invalid:
            ...
            if credentials is None or reauth:
                ...
                flow = client.OAuth2WebServerFlow(CLIENT_ID, CLIENT_SECRET, OAUTH_SCOPES, redirect_uri=REDIRECT_URI)
...
class GoogleStorageSigner(object):
    ...
    def create_sign_requests_request(self, verb, fileset, paths_to_contents):
        ...
        return {'fileset': fileset, 'unsigned_requests': unsigned_requests}

    @staticmethod
    def execute_signed_request(signed_request, content=None):
        req = signed_request
        params = {'GoogleAccessId': req['params']['google_access_id'], 
           'Signature': req['params']['signature'], 
           'Expires': req['params']['expires']}
        if signed_request['verb'] == Verb.PUT:
            headers = {'Content-Type': req['headers']['content_type'], 'Content-MD5': req['headers']['content_md5'], 
