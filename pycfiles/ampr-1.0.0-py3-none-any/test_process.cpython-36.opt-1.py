...
# Embedded file name: /ampoule/test/test_process.py
# Compiled at: 2019-12-22 01:42:51
# Size of source mod 2**32: 30479 bytes
...
class TestAMPConnector(unittest.TestCase):
    ...
    def test_protocol(self):
        ...
        return ac.finished.addCallback(lambda _: self.assertEqual(sa.getvalue(), ''.join(str(x).encode('ascii') for x in range(99))))
    ...
    def test_startProcess(self):
        ...
        starter = main.ProcessStarter(bootstrap=BOOT, args=(
         STRING,),
    ...
    def test_env_setting(self):
        ...
        a = FakeAMP(s)
        STRING = 'ciao'
        BOOT = "import sys, io, os\ndef main():\n    with io.open(4, 'w' + ('b' if bytes is str else '')) as f:\n        f.write(os.environ['FOOBAR'])\nmain()\n"
        starter = main.ProcessStarter(bootstrap=BOOT, packages=('twisted', 'ampoule'),
          env={'FOOBAR': STRING})
    ...
    def test_startAMPProcess(self):
        ...
        c.callRemote((commands.Echo), data=STRING).addCallback(lambda response: self.assertEquals(response['response'], STRING)).addCallback(lambda _: c.callRemote(commands.Shutdown))
        return finished
    ...
    def test_BootstrapContextInstance(self):
        starter = main.ProcessStarter(packages=('twisted', 'ampoule'))
        c, finished = starter.startAMPProcess(TempDirChild, ampChildArgs=('foo', ))
        cwd = []
    ...
    def test_startAMPAndParentProtocol(self):
        ...
        subp.callRemote(Ping, data=DATA).addCallback(lambda response: self.assertEquals(response['response'], DATA + APPEND)).addCallback(lambda _: subp.callRemote(commands.Shutdown))
        return finished
...
class TestProcessPool(unittest.TestCase):
    ...
    def test_adjustPoolSize(self):
        ...
        return pp.start().addCallback(_resize1).addCallback(_resize2).addCallback(_resize3)
    ...
    def test_childRestart(self):
        ...
        def _checksAgain(_):
            return pp.doWork((commands.Echo), data=STRING).addCallback(lambda result: self.assertEquals(result['response'], STRING))
    ...
    def test_parentProtocolChange(self):
        ...
        def _checks(_):
            return pp.doWork(Ping, data=DATA).addCallback(lambda response: self.assertEquals(response['response'], DATA + APPEND))
    ...
    def test_recycling(self):
        ...
        def _checks2(pid):
            return pp.doWork(Pid).addCallback(lambda response: response['pid']).addCallback(self.assertNotEquals, pid)
    ...
    def test_disableProcessRecycling(self):
        ...
        def _checks2(pid):
            return pp.doWork(Pid).addCallback(lambda response: response['pid']).addCallback(self.assertEquals, pid).addCallback(lambda _: pid)

        def finish(reason):
            return pp.stop().addCallback(lambda _: reason)

        return pp.start().addCallback(_checks).addCallback(_checks2).addCallback(_checks2).addCallback(finish)
    ...
    def test_changeChildrenReactor(self):
        ...
        def checkDefault():
            pp = pool.ProcessPool(starter=main.ProcessStarter(childReactor=FIRST,
              packages=('twisted', 'ampoule')),
              ampChild=ReactorChild,
              min=MIN,
              max=MAX)
            pp.start()
            return pp.doWork(Reactor).addCallback(self.assertEquals, {'classname': 'SelectReactor'}).addCallback(lambda _: pp.stop())

        def checkPool(_):
            pp = pool.ProcessPool(starter=main.ProcessStarter(childReactor=SECOND,
              packages=('twisted', 'ampoule')),
              ampChild=ReactorChild,
              min=MIN,
              max=MAX)
            pp.start()
            return pp.doWork(Reactor).addCallback(self.assertEquals, {'classname': 'PollReactor'}).addCallback(lambda _: pp.stop())

        return checkDefault().addCallback(checkPool)
    ...
    def test_commandsWithoutResponse(self):
        ...
        def _check(_):
            return pp.doWork(GetResponse).addCallback(self.assertEquals, {'response': DATA})

        def _work(_):
            return pp.doWork(NoResponse, arg=DATA)

        return pp.start().addCallback(_work).addCallback(_check).addCallback(lambda _: pp.stop())
    ...
    def test_supplyChildArgs(self):
        ...
        def _check(result):
            return pp.doWork(Write).addCallback(self.assertEquals, {'response': 'body'})
    ...
    def test_processTimeoutSignal(self):
        ...
        time is correctly handled.
        """
        pp = pool.ProcessPool(WaitingChild, min=1, max=1, timeout_signal=SIGHUP)
    ...
    def test_processRestartAfterTimeout(self):
        ...
        def _work(_):
            ...
            d2 = pp.callRemote(Ping, data='hello')
            self.assertFailure(d1, error.ProcessTerminated)
            d2.addCallback(lambda result: self.assertEqual(result, {'response': 'hello'}))
