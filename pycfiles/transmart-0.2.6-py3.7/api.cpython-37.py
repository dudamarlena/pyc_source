...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/v2/api.py
...
if transmart.dependency_mode in ('FULL', 'BACKEND'):
    ...
    from .data_structures import ObservationSet, ObservationSetHD, TreeNodes, Patients, PatientSets, Studies, StudyList, RelationTypes
...
class Query:
    __doc__ = ' Utility to build queries for transmart v2 api. '
...
class TransmartV2:
    __doc__ = ' Connect to tranSMART v2 API using Python. '

    def __init__(self, host, offline_token=None, kc_url=None, kc_realm=None, client_id=None, print_urls=False, interactive=True, verify=None):
        """
    ...
    def observations(self, constraint=None, as_dataframe=False, **kwargs):
        ...
        """
        q = Query(handle='/v2/observations', method='POST',
          json={'type':'clinical', 
    ...
    def _observation_call_factory(self, handle, doc=None):
        ...
        def func(constraint=None, *args, **kwargs):
            q = Query(handle=('/v2/observations/' + handle), method='POST',
              json={'constraint': constraint_to_dict(constraint)})
            return self.query(q)
    ...
    def create_patient_set(self, name: str, constraint=None, **kwargs):
        ...
        :return: direct json
        """
        q = Query(handle='/v2/patient_sets', method='POST',
          params={'name': name},
    ...
    def tree_nodes(self, root=None, depth=0, counts=False, tags=True, hal=False):
        ...
        :return:
        """
        q = Query(handle='/v2/tree_nodes', params={'root':root, 
         'depth':depth, 
         'counts':counts, 
         'tags':tags},
          hal=hal)
    ...
    @default_constraint
    @add_to_queryable
    def get_hd_node_data(self, constraint=None, biomarker_constraint=None, biomarkers: list=None, biomarker_type='genes', projection='all_data', **kwargs):
        """
        :param constraint:
        ...
        if biomarker_constraint is None:
            biomarker_constraint = BiomarkerConstraint(biomarkers=biomarkers, biomarker_type=biomarker_type)
        q = Query(handle='/v2/observations', method='GET',
          params=dict(type='autodetect',
    ...
    def dimension_elements(self, dimension, constraint=None, **kwargs):
        q = Query(handle=('/v2/dimensions/{}/elements'.format(dimension)), method='GET',
          params=dict(constraint=(str(constraint))))
        return self.query(q)
    ...
    def new_constraint(self, *args, **kwargs):
        return ObservationConstraint(args, api=self, **kwargs)
