...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/v2/constraints/atomic.py
...
class BiomarkerConstraint:
    ...
    def __init__(self, biomarkers: list=None, biomarker_type='genes'):
        ...
        self._BiomarkerConstraint__biomarkers = None
        self.biomarkers = biomarkers

    @property
    def biomarkers(self):
        return self._BiomarkerConstraint__biomarkers

    @biomarkers.setter
    @input_check((list,))
    def biomarkers(self, value):
        self._BiomarkerConstraint__biomarkers = value

    def __str__(self):
        return json.dumps(self.json())

    def json(self):
        d_ = dict(type='biomarker', biomarkerType=(self.type_))
        if self.biomarkers:
...
class ValueListConstraint(ValueConstraint):
    ...
    def json(self):
        if len(self.value) == 1:
            return {'type':'value',  'valueType':self.value_type_, 
             'operator':self.operator, 
             'value':self.value[0]}
        return {'type':'or', 
         'args':[{'type':'value',  'valueType':self.value_type_,  'operator':self.operator,  'value':value} for value in self.value]}
...
class TrialVisitConstraint:
    ...
    def json(self):
        return {'type':'or', 
         'args':[{'type':'field',  'field':{'dimension':'trial visit',  'fieldName':'id',  'type':'NUMERIC'},  'operator':'=',  'value':value} for value in self.values]}
...
class StartTimeConstraint:
    ...
    def __init__(self, values):
        ...
        if len(self.values) != self.n_dates:
            raise ValueError('Expected {} dates, but got {}, for {!r}.'.format(self.n_dates, len(self.values), self.__class__))

    def json(self):
        return {'type':'time',  'field':{'dimension':'start time', 
          'fieldName':'startDate', 
          'type':'DATE'}, 
         'operator':self.operator, 
