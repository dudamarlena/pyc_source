...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/v2/concept_search.py
...
class ConceptSearcher:
    ...
    def search(self, query_string, limit=50, allowed_nodes: set=None):
        with self.ix.searcher() as (searcher):
            ...
            if allowed_nodes is not None:
                allowed_nodes = {doc_num for doc, doc_num in zip(searcher.documents(), searcher.document_numbers()) if doc.get('fullname') in allowed_nodes}
    ...
    def get_schema(self):
        ...
        if exists_in(schema_dir) and open_dir(schema_dir).doc_count() != 0:
            ...
            print('Existing index cache found. Loaded {} tree nodes. Hooray!'.format(self.ix.doc_count()))
        else:
            print('No valid cache found. Building indexes...')
            now = time.time()
            self._ConceptSearcher__build_whoosh_index(schema_dir)
    ...
    def __build_whoosh_index(self, schema_dir):
        ...
        with self.ix.writer(procs=2, multisegment=True, limitmb=512) as (writer):
            for key, value in self._tree_dict.items():
                writer.add_document(node=(key.replace('\\', ' ').replace('_', ' ')),
