...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/auth.py
# Compiled at: 2019-08-27 08:40:44
# Size of source mod 2**32: 3610 bytes
import abc, jwt, requests, time
...
class LegacyAuth(Authenticator):
    ...
    def get_token(self):
        user = input('Username: ')
        r = requests.post(('{}/oauth/token'.format(self.url)),
          params=dict(grant_type='password',
          client_id=(self.client_id),
          username=user),
          data=dict(password=(getpass('tranSMART password: '))))
        r.raise_for_status()
...
class KeyCloakAuth(Authenticator):
    ...
    def has_valid_token(self):
        ...
        if self.expiry is not None:
            if self.expiry > time.time() + 10:
                return True
        return False

    def get_token(self):
        offline_token = self.offline_token or input('Offline token: ')
        r = requests.post(url=(self.handle),
...
def get_auth(host, offline_token=None, kc_url=None, kc_realm=None, client_id=None) -> Authenticator:
    ...
    """
    if kc_url:
        return KeyCloakAuth(url=kc_url, realm=kc_realm,
