...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/v2/widgets/constraint_details.py
...
import arrow, ipywidgets as widgets
...
class ConstraintWidget:
    ...
    def __init__(self, observation_constraint):
        ...
        self.date_value_min = widgets.DatePicker(description='After:')
        self.date_value_max = widgets.DatePicker(description='before:',
        ...
        self.start_date_since = widgets.DatePicker(disabled=True,
          description='Start date:')
        self.start_date_before = widgets.DatePicker(description='through',
          disabled=True,
          style={'description_width': '50px'})
    ...
    def _build_numeric_range(self):
        ...
        numeric_range = widgets.FloatRangeSlider(value=(5.5, 7.5),
          min=0.0,
          max=10.0,
          step=0.1,
    ...
    def _build_list_selector(self, target, description):
        ...
        w = widgets.SelectMultiple(options=[], description=description)
    ...
    def update_trial_visits(self, visits):
        ...
        for tv in visits:
            ...
            if not tv.get('relTime'):
                if tv.get('relTimeUnit'):
    ...
    def update_obs_repr(self):
        self.obs_repr.value = '<div style="font-family:monospace;font-size:smaller;">{}</div>'.format(repr(self.constraint))
