...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/commons.py
...
def filter_tree(tree_dict, counts_per_study_and_concept):
    ...
    return {k for k, v in tree_dict.items() if v.get('studyId') in (*studies, *(None, ))}
        ...
        def wrapper(self, value):
            if value is not None:
                if type(value) not in types:
                    raise ValueError('Expected type {!r} for {!r}, but got {!r}'.format(types, func.__name__, type(value)))
