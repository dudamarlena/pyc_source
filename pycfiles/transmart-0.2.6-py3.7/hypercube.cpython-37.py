...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmart/api/v2/dashboard/hypercube.py
...
class Hypercube:
    ...
    def __init__(self):
        ...
        self._Hypercube__subjects_mask = None
    ...
    def subject_mask(self):
        ...
        """
        return self._Hypercube__subjects_mask

    @subject_mask.setter
    def subject_mask(self, values):
        self._Hypercube__subjects_mask = values
        if values is not None:
            self._subject_bool_mask = self.data[patient_id].isin(values)

    def add_variable(self, df):
        ns = {(v[0], v[1]) for v in df.loc[:, [concept_id, study_id]].drop_duplicates().values}
