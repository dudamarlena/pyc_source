...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/lightwriter/LightWriter.py
# Compiled at: 2014-09-02 02:58:40
import math, os, time, serial
...
class LightWriter(object):
    ...
    def __init__(self, port=None, baud=115200, timeout=1):
        if not port:
            possible_devices = [ dev for dev in os.listdir('/dev/') if ('usb' in dev or 'ACM' in dev) and dev.startswith('tty')
                               ]
    ...
    def glow(self, color='green', length=2, period=1, interval=0.05, brightness=100, **kwargs):
    ...
    def rotate(self, length=2, interval=0.05, **kwargs):
        steps = int(length / interval)
        for i in range(steps):
            self._frame.shift(1)
            self.write_frame()
    ...
    def back_and_forth(self, color='red', length=2, interval=0.1, brightness=100, background='black', **kwargs):
    ...
    def random(self, length=2, interval=0.1, brightness=100, **kwargs):
        steps = int(length / interval)
        for i in range(steps):
            self._frame.random()
            self._frame.set_brightness(brightness)
    ...
    def blink(self, color='white', length=2, interval=0.3, brightness=100, **kwargs):
    ...
    def rise_and_fall(self, color='red', height=10, length=2, interval=0.1, background='blue', brightness=100, **kwargs):
        stepper = 1
        height_counter = 1
        steps = int(length / interval)
        for i in range(steps):
