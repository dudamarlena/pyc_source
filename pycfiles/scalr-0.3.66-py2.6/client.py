...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/scalrtools/api/client.py
# Compiled at: 2015-05-22 07:43:26
"""
Created on Feb 21th, 2011

@author: Dmytro Korsakov
"""
import binascii, hashlib, logging, hmac, urlparse
try:
...
class ScalrConnection(object):
    ...
    def dm_create_source(self, type, url, auth_login=None, auth_password=None):
        ...
        if auth_password:
            params['AuthPassword'] = auth_password
    ...
    def dm_create_application(self, name, source_id, pre_deploy_script=None, post_deploy_script=None):
        ...
        if pre_deploy_script:
            params['PreDeployScript'] = pre_deploy_script
        if post_deploy_script:
            params['PostDeployScript'] = post_deploy_script
    ...
    def dm_get_deployment_task_log(self, task_id, start_from=None, records_limit=None):
        ...
        if records_limit:
            params['RecordsLimit'] = records_limit
    ...
    def terminate_server(self, server_id, decrease_min_instances_setting=None):
        ...
        params = {}
        params['ServerID'] = server_id
        if decrease_min_instances_setting:
            params['DecreaseMinInstancesSetting'] = decrease_min_instances_setting
    ...
    def execute_script(self, farm_id, script_id, timeout, async, farm_role_id=None, server_id=None, config_variables=None, revision=None):
        ...
        if server_id:
            params['ServerID'] = server_id
        if config_variables:
            params['ConfigVariables'] = config_variables
        if revision:
    ...
    def _read_list_servers_response(self, xml):
        return self._read_response(xml, node_name='ServerSet', cls=types.Server)
    ...
    def is_document_empty(self, response):
        if response and response.firstChild:
            return not response.firstChild.hasChildNodes()
        return False
