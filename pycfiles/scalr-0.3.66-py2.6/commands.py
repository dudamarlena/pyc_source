...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/scalrtools/commands.py
...
"""
import os, sys, time, json, xmlrpclib, collections, prettytable
...
class Command(object):
    ...
    @property
    def connection(self):
        return ScalrConnection(self.config.environment, logger=self.config.logger)
...
class FarmRoleUpdateParameterValue(Command):
    ...
    def run(self):
        args = (
         self.options.farm_role_id, self.options.param_name, self.options.param_value)
...
class StatisticsGetGraphURL(Command):
    ...
    @classmethod
    def inject_options(cls, parser):
        id_help = 'In case if object type is instance ObjectID shoudl be server id, in case if object type is role ObjectID should be role id \t\tand in case if object type is farm ObjectID should be farm id'
...
class ScriptExecute(Command):
    ...
    def run(self):
        args = (
         self.options.farm_id, self.options.script_id, self.options.timeout,
...
class FireCustomEvent(Command):
    ...
    def run(self):
        args = (
         self.options.server_id, self.options.event_name, self.parse_variables(self.options.variables))
...
class ApacheVhostCreate(Command):
    ...
    def run(self):
        ...
        if self.options.enable_ssl:
            if int(self.options.enable_ssl):
                self.require(self.options.pk_path, self.options.cert_path)
                print os.path.exists(self.options.cert_path) or '%s : file with certificate not found.' % self.options.cert_path
                sys.exit(1)
            else:
        ...
        args = (
         self.options.domain, self.options.farm_id, self.options.farm_role_id, self.options.document_root, self.options.enable_ssl, pk, cert)
...
class ConfigureEnv(Command):
    ...
    def run(self):
        e = Environment(url=self.options.api_url, key_id=self.options.key_id, key=self.options.key, env_id=self.options.env_id, api_version='2.3.0', auth_type='password')
...
class ConfigureLDAPEnv(Command):
    ...
    def run(self):
        e = Environment(url=self.options.api_url, ldap_login=self.options.login, env_id=self.options.env_id, api_version='2.3.0', auth_type='ldap')
        e.write(self.config.base_path)
        e = Environment.from_ini(self.config.base_path)
        print e
...
class GlobalVariablesList(Command):
    ...
    def run(self):
        args = (
         self.options.role_id, self.options.farm_id, self.options.farm_role_id, self.options.server_id)
...
class GlobalVariableSet(Command):
    ...
    def __init__(self, config, *args):
        ...
        self.require(self.options.param_name, self.options.param_value, self.options.farm_id or self.options.farm_role_id or self.options.role_id or self.options.server_id)
    ...
    def run(self):
        args = [
         self.options.param_name, self.options.param_value]
