...
# Embedded file name: build\bdist.win32\egg\pydicia.py
# Compiled at: 2008-03-05 11:48:03
import os, datetime
...
__all__ = [
 'Option', 'OptionConflict', 'Shipment', 'Batch', 'Status', 'add_to_package', 'iter_options', 'report_status', 'DAZzle', 'Services', 'Domestic', 'International', 'Customs', 'Insurance', 'DateAdvance', 'Today', 'Tomorrow', 'WeekendDelivery', 'HolidayDelivery', 'NoPostage', 'Postcard', 'Envelope', 'Flat', 'RectangularParcel', 'NonRectangularParcel', 'FlatRateEnvelope', 'FlatRateBox', 'FlatRateLargeBox', 'ToAddress', 'ReturnAddress', 'RubberStamp']
...
class Package:
    ...
    def __init__(self, batch, *data):
        ...
        self.data = data
        if data:
            add_to_package(data, self, False)
    ...
    def add_customs_item(self, item):
        ...
        n = str()
        add_to_package(NumberedOptions(self.total_items, CustomsWeight=item.weight * item.qty, CustomsDescription=item.desc, CustomsQuantity=item.qty, CustomsValue=item.value * item.qty, CustomsCountry=item.origin), self, False)
    ...
    def finish(self):
        ...
        if self.total_items:
            ...
            if self[('WeightOz', None)] is None:
                raise OptionConflict('Total package weight must be specified when Customs.Items are used')
            oz = Decimal(self[('WeightOz', None)])
            if oz < self.total_weight:
                raise OptionConflict('Total item weight is %s oz, but total package weight is only %s oz' % (self.total_weight, oz))
            if not self[('CustomsFormType', None)] or not self[('ContentsType', None)]:
                raise OptionConflict('Customs form + content type must be specified with items')
        return
...
class Batch:
    ...
    def cleanup_files(self):
        if self.filename:
            ...
            backupname = None
        for fname in [self.etree.attrib.get('OutputFile'), self.filename, backupname]:
            if fname:
                try:
                    os.unlink(fname)
                except os.error:
...
address_fields = ('ToAddress1 ToAddress2 ToAddress3 ToAddress4 ToAddress5 ToAddress6').split()
...
class Status(object):
    ...
    def __str__(self):
        return ('\n').join([ '%-20s: %r' % (k, getattr(self, k)) for k in self.__slots__ if getattr(self, k) is not None ])
...
inverses = dict(TRUE='FALSE', FALSE='TRUE', YES='NO', NO='YES', ON='OFF', OFF='ON')
...
class OptionBase(object):
    ...
    def set(self, package, isdefault=False):
        ...
        if old is not None and old != unicode(self.value):
            ...
            raise OptionConflict("Can't set '%s=%s' when '%s=%s' already set" % (name, self.value, name, old))
...
class DAZzle:
    ...
    Verify = Start('DAZ')
    exe_path = _get_registry_string(HKEY_CLASSES_ROOT, 'lytfile\\shell\\open\\command')

    def get_preference(prefname):
        return _get_registry_string(HKEY_CURRENT_USER, 'Software\\Envelope Manager\\dazzle\\Preferences', prefname)
    ...
    def run(args=(), sync=True):
        ...
        process = Popen([DAZzle.exe_path] + list(args), executable=DAZzle.exe_path)
        if sync:
            return process.wait()
        return process
...
class Customs:
    __module__ = __name__
    _make_symbols(locals(), 'value', ('NONE GEM CN22 CP72').split(), CustomsFormType)
    _make_symbols(locals(), 'value', ('Sample Gift Documents Other Merchandise ReturnedGoods').split(), lambda value: ContentsType(value.upper()))
    ...
    def Item(desc, weight, value, qty=1, origin='United States'):
        ...
        assert qty == int(qty)
        return (
         desc, Decimal(weight), Decimal(value), int(qty), origin)
...
def additional_tests():
    import doctest
    return doctest.DocFileSuite('README.txt', optionflags=doctest.ELLIPSIS | doctest.REPORT_ONLY_FIRST_FAILURE | doctest.NORMALIZE_WHITESPACE)