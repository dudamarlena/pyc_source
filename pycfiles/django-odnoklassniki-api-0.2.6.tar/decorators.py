...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-odnoklassniki-api/odnoklassniki_api/decorators.py
...
def fetch_all(func, return_all=None, always_all=False, pagination='anchor', has_more='has_more'):
    ...
    def wrapper(self, all=False, instances_all=None, *args, **kwargs):
        if len(args) > 0:
            raise ImproperlyConfigured("It's prohibited to use non-key arguments for method decorated with @fetch_all, method is %s.%s(), args=%s" % (
        ...
        if always_all or all:
            if isinstance(instances, QuerySet):
                ...
                if isinstance(instances, list):
                    ...
                    raise ValueError('Wrong type of response from func %s. It should be QuerySet or list, not a %s' % (
                     func, type(instances)))
...
def fetch_only_expired(func, timeout_days, expiration_fieldname='fetched', ids_argument='ids'):
    ...
    def wrapper(self, only_expired=False, *args, **kwargs):
        if len(args) > 0:
            raise ValueError("It's prohibited to use non-key arguments for method decorated with @fetch_all, method is %s.%s(), args=%s" % (
             self.__class__.__name__, func.__name__, args))
        if only_expired:
...
def renew_if_not_equal(model, instances, ids):
    if instances is not None and len(ids) == instances.count():
        return instances
    else:
