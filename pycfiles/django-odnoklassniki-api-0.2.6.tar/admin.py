...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-odnoklassniki-api/odnoklassniki_api/admin.py
# Compiled at: 2015-01-25 03:13:46
...
class OdnoklassnikiModelAdmin(admin.ModelAdmin):

    def ok_link(self, obj):
        return '<a href="%s" target="_blank">%s</a>' % (obj.get_url(), getattr(obj, 'slug', 'odnoklassniki.ru'))

    ok_link.short_description = 'odnoklassniki.ru'
    ok_link.allow_tags = True

    def get_readonly_fields(self, request, obj=None):
        if obj:
            return [ field.name for field in obj._meta.fields if field.name not in () ]
        return []