...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/faamtools/avaps.py
# Compiled at: 2016-02-08 12:51:54
# Size of source mod 2**32: 2620 bytes
"""
Functions to process AVAPS dropsondes data
"""
import datetime, netCDF4 as nc, numpy as np
...
def read_avaps_nc(fname, flds=None, time2datetime=False):
    ...
    """
    if flds == None:
        flds = dict(time='time', hgt='alt', lon='lon', lat='lat', u='u_wind', v='v_wind', wspd='wspd', wdir='wdir', pres='pres', tdry='tdry', thta='theta', dhgt='dz', tdew='dp', relh='rh', mixr='mr', thte='theta_e', thtv='theta_v')
    ...
    flds_list = [ii for ii in flds]
    fil_list = utils.filt_miss_row(*[getattr(dum, ii).raw for ii in flds_list])
    data = ObsData()
    for i, j in enumerate(fil_list):
        data(**{flds_list[i]: DsFld(raw=getattr(dum, flds_list[i]).raw, fil=j, units=getattr(dum, flds_list[i]).units, long_name=getattr(dum, flds_list[i]).long_name)})

    if time2datetime and 'time' in flds and hasattr(data.time, 'units'):
