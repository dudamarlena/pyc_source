...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/faamtools/utils.py
# Compiled at: 2016-03-08 08:48:22
...
def uv2wdir(u, v):
    return 180.0 + 180.0 / np.pi * np.arctan2(u, v)
...
def timestr2datetime(tref, fmt='%Y-%m-%d%H:%M:%S'):
    ...
    if spl[0].lower() == 'days':
        ...
        if spl[0].lower() == 'hours':
            nsec = 3600
        else:
            if spl[0].lower() == 'minutes':
                nsec = 60
            else:
                if spl[0].lower() == 'seconds':
                    nsec = 1
                else:
                    nsec = 0
