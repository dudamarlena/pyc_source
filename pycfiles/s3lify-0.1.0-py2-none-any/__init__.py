...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/s3-now/s3lify/__init__.py
# Compiled at: 2019-09-15 18:08:37
import re, yaml, time, boto3, botocore, json, os, threading, uuid, tempfile, mimetypes, tldextract
NAME = 'S3lify'
CWD = os.getcwd()
MANIFEST_FILE = '.s3lify.manifest'
MIMETYPE_MAP = {'.js': 'application/javascript', 
   '.mov': 'video/quicktime', 
   '.mp4': 'video/mp4', 
   '.m4v': 'video/x-m4v', 
   '.3gp': 'video/3gpp', 
...
S3_HOSTED_ZONE_IDS = {'us-east-1': 'Z3AQBSTGFYJSTF', 
...
class S3lify(object):
    ...
    def __init__(self, domain, region='us-east-1', aws_access_key_id=None, aws_secret_access_key=None, **kwargs):
        """
        ...
        """
        self.aws_params = {'aws_access_key_id': aws_access_key_id, 
    ...
    def route53_create_hosted_zone(self):
        ...
        if hosted_zone:
            return hosted_zone
        response = self._route53.create_hosted_zone(Name=self.tld_domain, CallerReference=caller_reference_uuid(), HostedZoneConfig={'Comment': 'HostedZone created by S3lify.py!', 
           'PrivateZone': False})
        ...
        record = {'Changes': [
                     {'Action': 'UPSERT', 
                        'ResourceRecordSet': {'Name': name, 
                                              ...
                                              'ResourceRecords': [
                                                                {'Value': value}]}}]}
        response = self._route53.change_resource_record_sets(HostedZoneId=hosted_zone['Id'], ChangeBatch=record)
        if response and 'ChangeInfo' in response:
            return True
        return False
    ...
    def route53domains_update_dns(self):
        ...
        try:
            ...
            if bool(set(nameservers2) & set(rdNS)) is False:
                ...
                response = self._route53domains.update_domain_nameservers(DomainName=self.tld_domain, Nameservers=Nameservers)
    ...
    def _route53_update_a_records(self, dns_name, zone_id=None):
        hosted_zone = self.route53_create_hosted_zone()
        change_batch_payload = {'Changes': [
                     {'Action': 'UPSERT', 
                        'ResourceRecordSet': {'Name': self.domain, 
                                              'Type': 'A', 
                                              'AliasTarget': {'HostedZoneId': zone_id or S3_HOSTED_ZONE_IDS[self.region], 
                                                              'DNSName': dns_name, 
                                                              'EvaluateTargetHealth': False}}}]}
        if self.set_www:
            change_batch_payload['Changes'].append({'Action': 'UPSERT', 
               'ResourceRecordSet': {'Name': self.www_domain, 
                                     'Type': 'A', 
                                     'AliasTarget': {'HostedZoneId': zone_id or S3_HOSTED_ZONE_IDS[self.region], 
                                                     'DNSName': dns_name, 
                                                     'EvaluateTargetHealth': False}}})
        response = self._route53.change_resource_record_sets(HostedZoneId=hosted_zone['Id'], ChangeBatch=change_batch_payload)
        if response and 'ChangeInfo' in response:
            return True
        return False
    ...
    def cloudfront_invalidate_objects(self):
        distribution_id = self.cloudfront_get_distribution_id()
        if distribution_id:
            response = self._cloudfront.create_invalidation(DistributionId=distribution_id, InvalidationBatch={'Paths': {'Quantity': 1, 
                         'Items': [
                                 '/*']}, 
    ...
    def acm_generate_certificate(self):
        ...
        arn = self._acm_get_certificate_arn()
        if not arn:
            resp = self._acm.request_certificate(DomainName=self.domain, ValidationMethod='DNS')
            time.sleep(2)
            if resp:
                return True
        ...
        if arn:
            ...
            if cert and cert['Certificate']:
                for domainValidations in cert['Certificate']['DomainValidationOptions']:
                    if domainValidations['ValidationStatus'] == 'PENDING_VALIDATION' and domainValidations['ValidationMethod'] == 'DNS':
                        cname_name = domainValidations['ResourceRecord']['Name']
                        cname_value = domainValidations['ResourceRecord']['Value']
                        return (
                         True, cname_name, cname_value)
    ...
    def s3_create_site(self, index_file='index.html', error_file='error.html'):
        ...
        if not exists:
            if error_code == '404':
                policy_payload = {'Version': '2012-10-17', 
                   'Statement': [
                ...
                website_payload = {'ErrorDocument': {'Key': error_file}, 
                ...
                if self.set_www:
                    redirect_payload = {'RedirectAllRequestsTo': {'HostName': self.domain, 
                                                 'Protocol': 'https'}}
                ...
                return True
            raise Exception("Can't create website's bucket '%s' on AWS S3. Error: %s" % (
             self.domain, error_message))
        return exists
    ...
    def s3_upload(self, build_dir):
        ...
        for root, dirs, files in os.walk(build_dir):
            for filename in files:
                ...
                kwargs = dict(aws_params=self.aws_params, bucket_name=self.domain, local_path=local_path, s3_path=s3_path, mimetype=mimetype)
                files_list.append(s3_path)
                threading.Thread(target=_s3_upload_file, kwargs=kwargs).start()
    ...
    def s3_purge_files(self, exclude_files=[
 'index.html', 'error.html']):
        ...
        for chunk in chunk_list(self._s3_get_manifest(), 1000):
            try:
                self._s3.delete_objects(Bucket=self.s3_bucket, Delete={'Objects': [ {'Key': f} for f in chunk if f not in exclude_files
                            ]})
    ...
    def _s3_update_manifest(self, files_list):
        ...
        if files_list:
            ...
            self._s3.put_object(Bucket=self.s3_bucket, Key=MANIFEST_FILE, Body=data, ACL='private')
...
def _s3_upload_file(aws_params, bucket_name, local_path, s3_path, mimetype):
    ...
    s3.upload_file(local_path, Bucket=bucket_name, Key=s3_path, ExtraArgs={'ContentType': mimetype})


def _make_cloudfront_config(domain_name, s3_domain, ssl_arn):
    id = 'S3-website-%s' % s3_domain
    return {'CallerReference': caller_reference_uuid(), 
       'Aliases': {'Quantity': 1, 'Items': [domain_name]}, 'Origins': {'Quantity': 1, 
                   'Items': [
                           {'Id': id, 
                              'DomainName': s3_domain, 
                              'OriginPath': '', 
                              'CustomHeaders': {'Quantity': 0, 'Items': []}, 'CustomOriginConfig': {'HTTPPort': 80, 
                                                     'HTTPSPort': 443, 
                                                     'OriginProtocolPolicy': 'http-only', 
                                                     'OriginSslProtocols': {'Quantity': 1, 'Items': ['TLSv1']}, 'OriginReadTimeout': 30, 
       ...
       'Logging': {'Enabled': False, 
                   ...
                   'Bucket': '', 
                   'Prefix': ''}, 
       'CacheBehaviors': {'Quantity': 0}, 'Restrictions': {'GeoRestriction': {'RestrictionType': 'none', 
                                           'Quantity': 0}}, 
       ...
       'DefaultCacheBehavior': {'TargetOriginId': id, 
                                'ForwardedValues': {'QueryString': False, 
                                                    'Cookies': {'Forward': 'none'}, 'Headers': {'Quantity': 0, 'Items': []}, 'QueryStringCacheKeys': {'Quantity': 0, 'Items': []}}, 
                                'TrustedSigners': {'Enabled': False, 'Quantity': 0, 'Items': []}, 'ViewerProtocolPolicy': 'redirect-to-https', 
                                'MinTTL': 0, 
       ...
       'CustomErrorResponses': {'Quantity': 0}, 'ViewerCertificate': {'ACMCertificateArn': ssl_arn, 
