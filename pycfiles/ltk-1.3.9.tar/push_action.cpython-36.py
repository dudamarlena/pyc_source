...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek36/lib/python3.6/site-packages/ltk/actions/push_action.py
    ...
    def _push_specific_files(self, patterns, **kwargs):
        ...
        for file in files:
            ...
            if not self.metadata_only and self.doc_manager.is_doc_new(file) and not self.doc_manager.is_translation(file, title, files, self):
                ...
                if len(self.metadata) > 0 or kwargs['due_date'] or kwargs['due_reason'] or self.doc_manager.is_doc_modified(file, self.path) and not self.metadata_only:
