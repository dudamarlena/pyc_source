...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek36/lib/python3.6/site-packages/ltk/actions/config_action.py
# Compiled at: 2020-04-15 14:10:55
# Size of source mod 2**32: 24836 bytes
...
class ConfigAction(Action):
            ...
            if 'locale' in kwargs:
                if kwargs['locale']:
                    ...
                    if 'workflow_id' in kwargs:
                        if kwargs['workflow_id']:
                            self.set_workflow_id(kwargs['workflow_id'])
                        else:
                            if 'download_folder' in kwargs:
                                if kwargs['download_folder']:
                                    self.set_download_folder(kwargs['download_folder'])
                                else:
                                    if 'latest_document' in kwargs:
                                        if kwargs['latest_document']:
                                            self.set_always_check_latest_doc(kwargs['latest_document'])
                                        else:
                                            if 'clone_option' in kwargs:
                                                if kwargs['clone_option']:
                                                    self.set_clone_option(kwargs['clone_option'])
                                                else:
                                                    if 'finalized_file' in kwargs:
                                                        if kwargs['finalized_file']:
                                                            self.set_finalized_file_option(kwargs['finalized_file'])
                                                    if 'unzip_file' in kwargs:
                                                        if kwargs['unzip_file']:
                                                            self.set_unzip_file_option(kwargs['unzip_file'])
                                                    if 'target_locales' in kwargs:
                                                        if kwargs['target_locales']:
                                                            self.set_target_locales(kwargs['target_locales'])
                                            else:
                                                if 'locale_folder' in kwargs:
                                                    if kwargs['locale_folder']:
                                                        self.set_locale_folder(kwargs['locale_folder'])
                                            if 'remove_locales' in kwargs:
                                                if kwargs['remove_locales']:
    ...
    def set_append_option(self, append_option):
        ...
        if append_option in frozenset({'none', 'full'}) or append_option[:append_option.find(':') + 1] in frozenset({'name:', 'number:'}):
            set_option = True
        ...
        if download_folder == '--none':
            if self.download_dir == '':
                ...
                if self.download_option != 'clone':
                    if self.watch_locales != None:
                        if len(self.locale_folders) != 0:
                            new_download_option = 'folder'
                    else:
                        new_download_option = 'same'
                    self.download_option = new_download_option
