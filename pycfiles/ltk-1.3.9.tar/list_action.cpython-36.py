...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek36/lib/python3.6/site-packages/ltk/actions/list_action.py
# Compiled at: 2019-11-20 16:41:05
# Size of source mod 2**32: 8847 bytes
    ...
    def list_action(self, **kwargs):
        if 'id_type' in kwargs:
            if kwargs['id_type']:
                ...
                if 'hide_docs' in kwargs:
                    if 'title' in kwargs and 'show_dests' in kwargs:
                        self.list_ids(kwargs['hide_docs'], kwargs['title'], kwargs['show_dests'])
        else:
            logger.error("Error on command line input for 'ltk list'")
    ...
    def list_ids(self, hide_docs, title=False, downloads=False):
        try:
            ...
            for i in range(len(ids)):
                ...
                if title:
                    headers = [
                     'Title', 'Lingotek ID', 'Locales']
                else:
                    headers = [
                     'Filename', 'Lingotek ID', 'Locales']
                if downloads:
