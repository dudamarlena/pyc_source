...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek/lib/python3.7/site-packages/ltk/actions/config_action.py
...
class ConfigAction(Action):
    ...
    def print_output(self):
        ...
        if self.print_config:
            ...
            if not (str(watch_locales) == '[]' or watch_locales):
                watch_locales = 'None'
            table = [
             [
              'Host', self.host],
    ...
    def set_append_option(self, append_option):
        ...
        if append_option in {'full', 'none'} or append_option[:append_option.find(':') + 1] in {'name:', 'number:'}:
            set_option = True
            if append_option[:3] == 'num':
                ...
                if append_option[:4] == 'name':
                    ...
                    self.conf_parser.has_option('main', 'append_option') or self.update_config_file('append_option', 'none', self.conf_parser, self.config_file_name, 'Update: Added optional file location appending (ltk config --help)')
                if self.print_config:
    ...
    def set_target_locales(self, target_locales):
        ...
        if not len(locales) > 0 or locales[0].lower() == 'none' or locales[0].lower() == '--none':
