...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek/lib/python3.7/site-packages/ltk/actions/download_action.py
# Compiled at: 2020-04-15 14:10:55
# Size of source mod 2**32: 15773 bytes
    ...
    def download_by_path(self, file_path, locale_codes, locale_ext, no_ext, auto_format, xliff):
        ...
        if docs:
            for entry in docs:
                ...
                if not 'clone' in self.download_option or locale_ext:
                    if no_ext:
                        if not locale_ext:
            ...
            if not entry:
                ...
                try:
                    ...
                    try:
                        ...
                        raise_error(doc_info.json(), 'Something went wrong trying to download document: {0}'.format(document_id), True)
                        return
                    finally:
                        e = None
                        del e
        ...
        try:
            ...
            if locale_code in locale_folders:
                ...
                if self.download_dir:
                    ...
                    incremental_path = ''
                    os.path.exists(self.download_root) or os.mkdir(self.download_root)
                if target_dirs:
