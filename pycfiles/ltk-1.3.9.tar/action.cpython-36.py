...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek36/lib/python3.6/site-packages/ltk/actions/action.py
...
class Action:
    ...
    def add_document(self, file_name, title, doc_metadata={}, **kwargs):
        ...
        if ltk.check_connection.check_for_connection() == False:
            ...
            if self.is_hidden_file(file_name):
                return
            try:
                if 'locale' not in kwargs or not kwargs['locale']:
                    locale = self.locale
                else:
    ...
    def locked_doc_response_manager(self, response, document_id, *args, **kwargs):
        if response.status_code == 423 and 'next_document_id' in response.json():
            ...
            return (
             (self.api.document_update)(response.json()['next_document_id'], *args, **kwargs), response.json()['next_document_id'])
        else:
            return (
        ...
        if all(field in self.default_metadata for field in fields):
            ...
            for field in fields:
                ...
                if field in self.default_metadata:
                    if self.default_metadata[field]:
                        if set_defaults:
        ...
        if os.path.exists(path):
            if os.path.isdir(path):
                for root, subdirs, files in os.walk(path):
                    for file in files:
                        if not ('desktop.ini' in file or 'Thumbs.db' in file or 'ehthumbs.db' in file):
