...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-qywz0dnj/ltk/ltk/actions/clone_action.py
...
class CloneAction(Action):
    ...
    def clone_folders(self, dest_path, folders_map, locale, copy_root=False):
        ...
        try:
            if not (folders_map and len(folders_map)):
        ...
        try:
            if not len(self.watch_locales) or self.watch_locales == set(['[]']):
                ...
                if not len(self.folder_manager.get_file_names()):
                    ...
                    return
                cloned_folders or logger.info('All locales have already been cloned.')
            return
