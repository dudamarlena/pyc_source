...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek/lib/python3.7/site-packages/ltk/commands.py
# Compiled at: 2020-04-15 14:10:55
# Size of source mod 2**32: 45012 bytes
""" Python Dependencies """
import click, ctypes, logging, os, subprocess, sys
...
@click.option('--host', default='myaccount.lingotek.com', help='Default environment: myaccount.lingotek.com')
...
@click.option('--path', type=click.Path(exists=True), help='The path to the project directory to be initialized; defaults to the current directory')
@click.option('-n', '--project_name', help='The preferred project name, defaults to the current directory name')
@click.option('-w', '--workflow_id', default='c675bd20-0688-11e2-892e-0800200c9a66', help='The id of the workflow to use for this project; defaults to machine translate only')
...
@click.option('-d', '--delete', flag_value=True, help='Delete the current project remotely and re-initialize')
...
@click.option('-c', '--clone_option', help="Toggle clone download option 'on' and 'off'. Turning clone 'on': Translations will be downloaded to a cloned folder structure, where the root folder for each locale is the locale folder specified in config or a locale folder inside of the default download folder. If a default download folder is not set, then translations will be downloaded to the directory where the project was initialized.Turning clone 'off': Downloaded translations will go into their locale folder (if specified) or default download folder, but not in a cloned folder structure. If no default download folder or locale folder is specified, downloaded translations will go to the same folder as their corresponding source files.")
@click.option('-ff', '--finalized_file', help="Toggle finalized file download option 'on' and 'off'. Turning finalized file on downloads the finalized file instead of the raw translation.                                                A finalized file is typically a file that has undergone some sort of post editing like Desktop Publishing after the translation has compeleted.")
...
@click.option('-d', '--download_folder', help='Specify a default folder for where downloaded translations should go. Use --none to remove the download folder. Using --none will cause downloaded translations to download to the same folder as their corresponding source files.')
@click.option('-t', '--target_locales', multiple=True, help='Specify target locales that documents in watch_folder should be assigned; may either specify with multiple -t flags (ex: -t locale -t locale) or give a list separated by commas and no spaces (ex: -t locale,locale)')
...
@click.option('-f', '--format', help="Format of file; if not specified, will use extension to detect; defaults to plaintext. Use ltk list -f to see possible formats. Files may not be added to Lingotek's system if not formatted correctly according to the specified format")
...
@click.argument('locales', required=False, nargs=(-1))
...
def rm(file_names, **kwargs):
    ...
    try:
        ...
        init_logger(action.path)
        if not file_names:
            if not ('all' in kwargs and kwargs['all']):
                'local' in kwargs and kwargs['local'] or logger.info('Usage: ltk rm [OPTIONS] FILE_NAMES...')
                return
        if len(file_names) > 0:
            file_names = remove_powershell_formatting(file_names)
...
@click.option('-t', '--timeout', type=(click.INT), default=60, help='The amount of time watch will sleep between polls, in seconds. Defaults to 1 minute')
...
def watch(ignore, delimiter, timeout, no_folders, force_poll):
    """
    ...
    try:
        ...
        init_logger(action.path)
        action.watch_action(ignore, delimiter, no_folders, force_poll)
    except (UninitializedError, RequestFailedError) as e:
        try:
            print_log(e)
            logger.error(e)
