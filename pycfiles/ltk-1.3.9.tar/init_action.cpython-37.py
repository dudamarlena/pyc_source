...
# [GCC 8.4.0]
# Embedded file name: /home/hectorlopez/.virtualenvs/lingotek/lib/python3.7/site-packages/ltk/actions/init_action.py
...
class InitAction:
    ...
    def init_action(self, host, access_token, client_id, project_path, folder_name, workflow_id, default_locale, browser, delete, reset):
        ...
        try:
            ...
            ran_oauth = False
            access_token = access_token or self.check_global(host)
    ...
    def reinit(self, host, project_path, delete, reset):
        if is_initialized(project_path):
            if not reset:
                ...
                if not delete:
                    return False
                option = yes_no_prompt('Are you sure you want to delete the current project? This will also delete the project in your community.', default_yes=True)
                if option:
    ...
    def get_user_append_option--- This code section failed: ---

 L. 731         0  LOAD_STR                 'not chosen'
                2  STORE_FAST               'option'

 L. 732         4  SETUP_LOOP           80  'to 80'
              6_0  COME_FROM            54  '54'
                6  LOAD_STR                 'none'
                8  LOAD_FAST                'option'
 ...
 L. 733        30  LOAD_STR                 'Input the append option you would like to use: '
               32  STORE_FAST               'prompt_message'

 L. 738        34  LOAD_GLOBAL              input
               36  LOAD_FAST                'prompt_message'
               38  CALL_FUNCTION_1       1  '1 positional argument'
               40  STORE_FAST               'option'

 L. 740        42  LOAD_FAST                'self'
               44  LOAD_METHOD              validate_append_option
               46  LOAD_FAST                'option'
               48  CALL_METHOD_1         1  '1 positional argument'
               50  LOAD_CONST               True
               52  COMPARE_OP               ==
               54  POP_JUMP_IF_FALSE     6  'to 6'

 L. 741        56  LOAD_GLOBAL              logger
               58  LOAD_METHOD              info
               60  LOAD_STR                 "Append option set to '{0}'\n"
...
Parse error at or near `POP_BLOCK' instruction at offset 78
    ...
    def validate_append_option(self, append_option):
        if append_option in {'none', 'full'} or append_option[:append_option.find(':') + 1] in {'name:', 'number:'}:
    ...
    def prompt_unzip_file_option(self):
        ...
        confirm = 'none'
        while confirm != 'on':
            if confirm != 'On' and confirm != 'ON' and confirm != 'off' and confirm != 'Off' and confirm != '':
