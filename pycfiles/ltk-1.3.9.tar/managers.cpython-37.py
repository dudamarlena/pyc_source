...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-qywz0dnj/ltk/ltk/managers.py
# Compiled at: 2019-11-20 16:41:05
# Size of source mod 2**32: 11990 bytes
...
class DocumentManager:
    ...
    def is_doc_new(self, file_name, root_path=None):
        if root_path != None:
            ...
            if file_name[:len(download_folder)] == download_folder:
                if download_folder is not '':
                    if len(file_name) < len(download_folder):
            ...
            for doc in content:
                ...
                if 'downloaded' in doc.keys():
                    ...
                    for locale in locales:
                        ...
                        if not possible_file_name == file_name:
                            if possible_no_clone_file_name == file_name:
                                pass
                            return False

        else:
            file_name_exists = self._db.search(where('file_name') == file_name)
            return file_name_exists or True
        return False
    ...
    def is_doc_modified(self, file_name, path):
        ...
        if entry:
            if entry['added'] < last_modified:
                if entry['last_mod'] < last_modified:
                    return True
        return False

    def add_document(self, title, create_date, doc_id, sys_mtime, last_mod, file_name, process_id, download_folder=''):
        entry = {'name':title, 
         'added':create_date,  'id':doc_id,  'sys_last_mod':sys_mtime, 
         'last_mod':last_mod,  'file_name':file_name,  'downloaded':[],  'download_folder':download_folder,  'process_id':process_id}
    ...
    def clear_prop(self, doc_id, prop):
        ...
        if isinstance(entry[prop], str):
            ...
            if isinstance(entry[prop], int):
                self.update_document(prop, 0, doc_id)
            else:
                if isinstance(entry[prop], list):
                    self.update_document(prop, [], doc_id)
                else:
                    if isinstance(entry[prop], dict):
                        self.update_document(prop, {}, doc_id)
