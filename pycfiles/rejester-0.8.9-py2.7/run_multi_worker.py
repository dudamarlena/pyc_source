...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rejester/run_multi_worker.py
...
import argparse, lockfile, logging, os, sys, time, traceback, daemon, dblogger, rejester
from rejester.exceptions import NoSuchWorkSpecError, NoSuchWorkUnitError
from rejester.run import absolute_path
from rejester.workers import run_worker, MultiWorker, SingleWorker, ForkWorker
import yakonfig
logger = logging.getLogger(__name__)

def args_run_worker(parser):
    parser.add_argument('--pidfile', metavar='FILE', type=absolute_path, help='file to hold process ID of worker')
    parser.add_argument('--logpath', metavar='FILE', type=absolute_path, help='file to receive local logs')
    parser.add_argument('--foreground', action='store_true', help='do not run the worker as a daemon')
...
def main():
    parser = argparse.ArgumentParser(description='run a rejester work-handling daemon')
