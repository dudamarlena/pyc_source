...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rejester/tests/test_cli.py
...
import errno, logging, json, os, signal, subprocess, sys, time, uuid
try:
    import regex as re
except:
    import re

import pytest, yaml, rejester
...
def test_config_from_env(redis_address, monkeypatch, namespace_string):
    ...
    with yakonfig.defaulted_config([
     rejester], config={'rejester': {'namespace': namespace_string}}):
        pass


@pytest.yield_fixture
def global_config(_rejester_config):
    with yakonfig.defaulted_config([rejester], config={'rejester': _rejester_config}):
...
def work_units(global_config):
    num_units = 11
    return dict([ (('key-{0}').format(x), {'sleep': 0.2, 'config': global_config}) for x in xrange(num_units)
                ])
...
def test_delete(manager, loaded, work_spec, namespace_string):
    ...
    assert manager.stdout.getvalue() == "deleting namespace '" + namespace_string + "'\n"
    spec = manager.task_master.get_work_spec(work_spec['name'])
    assert spec is None
    return
...
def test_status_by_name(manager, loaded, work_spec, work_units):
    ...
    ref = {'num_available': len(work_units), 
       'num_blocked': 0, 
       'num_failed': 0, 
       'num_finished': 0, 
       'num_pending': 0, 
...
def test_status_with_work(manager, worked, work_spec, work_units):
    manager.runcmd('status', ['-W', work_spec['name']])
    status = json.loads(manager.stdout.getvalue())
    ref = {'num_available': len(work_units) - 6, 
       'num_blocked': 0, 
       'num_failed': 2, 
       'num_finished': 3, 
       'num_pending': 1, 
...
def test_summary(manager, loaded, work_spec):
    ...
    assert manager.stdout.getvalue() == 'Work spec               Avail  Pending  Blocked   Failed Finished    Total\n==================== ======== ======== ======== ======== ======== ========\ntbundle                    11        0        0        0        0       11\n'


def test_summary_worked(manager, worked, work_spec):
    manager.runcmd('summary', [])
    assert manager.stdout.getvalue() == 'Work spec               Avail  Pending  Blocked   Failed Finished    Total\n==================== ======== ======== ======== ======== ======== ========\ntbundle                     5        1        0        2        3       11\n'
...
def test_work_units_strange_names(manager, work_spec):
    manager.task_master.update_bundle(work_spec, {'\x00': {'k': 'v'}, b'\xff': {'k': 'v'}, 'a\x00': {'k': 'v'}})
    manager.runcmd('work_units', ['-W', work_spec['name']])
    assert manager.stdout.getvalue() == b'\x00\na\x00\n\xff\n'


def test_work_units_details(manager, work_spec):
    manager.task_master.update_bundle(work_spec, {'u1': {'k1': 'v1'}, 'u2': {'k1': 'v2'}})
    manager.runcmd('work_units', ['-W', work_spec['name'], '--details'])
    assert manager.stdout.getvalue() == "'u1': {   'k1': 'v1'}\n'u2': {   'k1': 'v2'}\n" or manager.stdout.getvalue() == "'u2': {   'k1': 'v2'}\n'u1': {   'k1': 'v1'}\n"
...
def test_retry_not_loaded(manager, work_spec):
    ...
    assert manager.stdout.getvalue() == "Invalid work spec '" + work_spec['name'] + "'.\n"
...
def test_retry_fail_one_name(manager, worker, task_master, loaded, work_spec, work_units):
...
def test_retry_fail_one_all(manager, worker, task_master, loaded, work_spec, work_units):
    assert task_master.num_failed(work_spec['name']) == 0
    assert task_master.num_available(work_spec['name']) == len(work_units)
    unit = manager.task_master.get_work(worker.worker_id, available_gb=16)
    assert unit.work_spec_name == work_spec['name']
...
def test_clear_simple(manager, loaded, work_spec, work_units):
    assert manager.task_master.num_available(work_spec['name']) == len(work_units)
...
def test_clear_by_name(manager, loaded, work_spec, work_units):
    ...
    assert manager.task_master.num_available(work_spec['name']) == len(work_units) - 1
...
def test_clean_with_work(manager, worked, work_spec, work_units):
    manager.runcmd('clear', ['-W', work_spec['name']])
    assert manager.stdout.getvalue() == ('Removed {0} work units.\n').format(len(work_units))
    assert manager.task_master.status(work_spec['name']) == {'num_available': 0, 
       'num_pending': 0, 
       'num_blocked': 0, 
       'num_failed': 0, 
       'num_finished': 0, 
...
def test_clean_pending_with_work(manager, worked, work_spec, work_units):
    manager.runcmd('clear', ['-W', work_spec['name'], '-s', 'pending'])
    assert manager.stdout.getvalue() == 'Removed 1 work units.\n'
    assert manager.task_master.status(work_spec['name']) == {'num_available': len(work_units) - 6, 
       'num_pending': 0, 
       'num_blocked': 0, 
       'num_failed': 2, 
       'num_finished': 3, 
...
def test_run_worker_minimal(manager, tmpdir, global_config):
    ...
    cfgfile = str(tmpdir.join('config.yaml'))
    global_config['rejester']['fork_worker'] = {'heartbeat_interval': 1}
    with open(cfgfile, 'w') as (f):
        f.write(yaml.dump(global_config))
