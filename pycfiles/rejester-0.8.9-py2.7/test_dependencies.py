...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rejester/tests/test_dependencies.py
...
def test_two_simple_dependency(task_master):
    ...
    task_master.update_bundle(work_spec_a, work_units_aa, nice=0)
    task_master.update_bundle(work_spec_b, work_units_bb, nice=1)
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'bb', None))
    assert res == True
...
def test_two_hard_dependency(task_master):
    ...
    task_master.update_bundle(work_spec_a, work_units_aa, nice=0)
    task_master.update_bundle(work_spec_b, work_units_bb, nice=1)
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'bb', None), hard=True)
    assert res == True
...
def test_two_soft_dependency(task_master):
    ...
    task_master.update_bundle(work_spec_a, work_units_aa, nice=0)
    task_master.update_bundle(work_spec_b, work_units_bb, nice=1)
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'bb', None), hard=False)
    get_and_check_unit(task_master, 'b', 'bb', fail=True)
    get_and_check_unit(task_master, 'a', 'aa')
    get_and_check_done(task_master)
...
def test_missing_units(task_master):
    ...
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('a', 'bb', None))
    assert res is False
    res = task_master.add_dependent_work_units(('b', 'aa', None), ('b', 'bb', None))
    assert res is False
    with pytest.raises(NoSuchWorkSpecError):
        task_master.add_dependent_work_units(('a', 'aa', None), ('c', 'cc', None))
    with pytest.raises(NoSuchWorkSpecError):
        task_master.add_dependent_work_units(('c', 'cc', None), ('b', 'bb', None))
    return
...
def test_missing_units_definitions(task_master):
    ...
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('a', 'bb', None))
    assert res is True
    res = task_master.add_dependent_work_units(('b', 'aa', None), ('b', 'cc', {'x': 'x'}))
    assert res is False
    res = task_master.add_dependent_work_units(('b', 'bb', None), ('b', 'cc', None))
    assert res is True
    with pytest.raises(NoSuchWorkSpecError):
        task_master.add_dependent_work_units(('a', 'aa', None), ('c', 'cc', {'x': 'x'}))
    with pytest.raises(NoSuchWorkSpecError):
...
def test_three(task_master):
    ...
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'bb', None))
    assert res is True
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'cc', None))
    assert res is True
    get_and_check_unit(task_master, 'b', 'bb')
...
def test_three_soft_failure(task_master):
    ...
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'bb', None), hard=False)
    assert res is True
    res = task_master.add_dependent_work_units(('a', 'aa', None), ('b', 'cc', None), hard=False)
    assert res is True
    get_and_check_unit(task_master, 'b', 'bb', fail=True)
