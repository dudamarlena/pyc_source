...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rejester/_task_master.py
...
import abc, logging, os, pkg_resources, random, psutil, socket, time, traceback
from rejester._registry import Registry, nice_identifier
from rejester.exceptions import ProgrammerError, LockError, LostLease, EnvironmentError, NoSuchWorkSpecError, NoSuchWorkUnitError
...
FINISHED = 4
FAILED = 5
WORK_UNIT_STATUS_BY_NAME = {'AVAILABLE': AVAILABLE, 
   'BLOCKED': BLOCKED, 
   'PENDING': PENDING, 
   'FINISHED': FINISHED, 
   'FAILED': FAILED}
WORK_UNIT_STATUS_NAMES_BY_NUMBER = dict([ (v, k) for k, v in WORK_UNIT_STATUS_BY_NAME.iteritems() ])
...
class Worker(object):
    ...
    def environment(self):
        ...
        env = dict(worker_id=self.worker_id, parent=self.parent, hostname=hostname, aliases=tuple(aliases), ipaddrs=tuple(ipaddrs), fqdn=socket.getfqdn(), version=pkg_resources.get_distribution('rejester').version, working_set=[ (dist.key, dist.version) for dist in pkg_resources.WorkingSet() ], memory=psutil.virtual_memory(), pid=os.getpid())
        return env
    ...
    def heartbeat(self):
        ...
        self.task_master.worker_heartbeat(self.worker_id, mode, self.lifetime, self.environment(), parent=self.parent)
        return mode
...
class WorkUnit(object):
    ...
    def __init__(self, registry, work_spec_name, key, data, worker_id=None, expires=None, default_lifetime=900):
        ...
        if not isinstance(data, dict):
            raise ProgrammerError('WorkUnit must be given a dict() for data, otherwise exceptions will not get passed back up')
        self.data = data
        self.finished = False
        self.failed = False
    ...
    def __repr__(self):
        return ('WorkUnit(work_spec_name={0.work_spec_name}, key={0.key})').format(self)
    ...
    def module(self):
        ...
        if self._module_cache is None:
            ...
            if funclist:
                try:
                    self._module_cache = __import__(self.spec['module'], globals(), (), funclist, -1)
                except Exception:
                    logger.error('failed to load spec["module"] = %r', self.spec['module'], exc_info=True)
                    raise
    ...
    def terminate(self):
        ...
        if not terminate_function_name:
            ...
            if not terminate_function:
                ...
                return None
            logger.info(('calling terminate function for work unit {0}').format(self.key))
    ...
    def _refresh(self, session, stopping=False):
        ...
        """
        data = session.get(WORK_UNITS_ + self.work_spec_name + _FINISHED, self.key)
        if data is not None:
            self.finished = True
            self.data = data
            if not stopping:
            ...
            return
        self.finished = False
        data = session.get(WORK_UNITS_ + self.work_spec_name + _FAILED, self.key)
        if data is not None:
            self.failed = True
            self.data = data
            if not stopping:
            ...
            return
        self.failed = False
        data = session.get(WORK_UNITS_ + self.work_spec_name + _BLOCKED, self.key)
        if data is not None:
            self.data = data
            raise LostLease('work unit now blocked by others')
        worker_id = session.get(WORK_UNITS_ + self.work_spec_name + '_locks', self.key)
        if worker_id != self.worker_id:
            raise LostLease('work unit claimed by %r', worker_id)
        data = session.get(WORK_UNITS_ + self.work_spec_name, self.key)
        if data is None:
    ...
    def finish(self):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            ...
            if self.finished or self.failed:
                return
            session.move(WORK_UNITS_ + self.work_spec_name, WORK_UNITS_ + self.work_spec_name + _FINISHED, {self.key: self.data})
            session.popmany(WORK_UNITS_ + self.work_spec_name + '_locks', self.key, self.worker_id)
            blocks = session.get(WORK_UNITS_ + self.work_spec_name + _BLOCKS, self.key)
            if blocks is not None:
                for block in blocks:
                    ...
                    if len(depends) == 0:
                        ...
                        unitdef = session.get(WORK_UNITS_ + spec + _BLOCKED, unit)
                        session.move(WORK_UNITS_ + spec + _BLOCKED, WORK_UNITS_ + spec, {unit: unitdef})
                    else:
                        session.set(WORK_UNITS_ + spec + _DEPENDS, unit, depends)
    ...
    def fail(self, exc=None):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            ...
            session.move(WORK_UNITS_ + self.work_spec_name, WORK_UNITS_ + self.work_spec_name + _FAILED, {self.key: self.data})
            session.popmany(WORK_UNITS_ + self.work_spec_name + '_locks', self.key, self.worker_id)
            blocks = session.get(WORK_UNITS_ + self.work_spec_name + _BLOCKS, self.key)
            if blocks is not None:
                for block in blocks:
                    ...
                    if hard:
                        ...
                        if unitdef is not None:
                            session.move(WORK_UNITS_ + spec + _BLOCKED, WORK_UNITS_ + spec + _FAILED, {unit: unitdef})
                    else:
                        depends = session.get(WORK_UNITS_ + spec + _DEPENDS, unit)
                        if depends is None:
                            continue
...
class TaskMaster(object):
    ...
    *pending* if somebody is currently working on it.  When they
    ...
    def workers(self, alive=True):
        ...
        """
        return self.registry.filter(WORKER_OBSERVED_MODE, priority_min=alive and time.time() or '-inf')
    ...
    def dump(self):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            for work_spec_name in self.registry.pull(NICE_LEVELS).iterkeys():
                ...
                for key in scan(''):
                    logger.debug(('spec {0} unit {1} available or pending').format(work_spec_name, key))

                for key in scan(_BLOCKED):
                    blocked_on = session.get(WORK_UNITS_ + work_spec_name + _DEPENDS, key)
                    logger.debug(('spec {0} unit {1} blocked on {2!r}').format(work_spec_name, key, blocked_on))

                for key in scan(_FINISHED):
                    logger.debug(('spec {0} unit {1} finished').format(work_spec_name, key))

                for key in scan(_FAILED):
                    logger.debug(('spec {0} unit {1} failed').format(work_spec_name, key))
    ...
    def num_available(self, work_spec_name):
        ...
        """
        return self.registry.len(WORK_UNITS_ + work_spec_name, priority_max=time.time())
    ...
    def num_tasks(self, work_spec_name):
        ...
        return self.num_finished(work_spec_name) + self.num_failed(work_spec_name) + self.registry.len(WORK_UNITS_ + work_spec_name)
    ...
    def status(self, work_spec_name):
        ...
        """
        return dict(num_available=self.num_available(work_spec_name), num_pending=self.num_pending(work_spec_name), num_blocked=self.num_blocked(work_spec_name), num_finished=self.num_finished(work_spec_name), num_failed=self.num_failed(work_spec_name), num_tasks=self.num_tasks(work_spec_name))
    ...
    def list_work_specs(self, limit=None, start=None):
        ...
        """
        return (
         self.registry.pull(WORK_SPECS).items(), None)
    ...
    def _remove_some_work_units(self, work_spec_name, work_unit_names, suffix='', priority_min='-inf', priority_max='+inf'):
        ...
        if work_unit_names is None:
            ...
            while True:
                with self.registry.lock(identifier=self.worker_id) as (session):
                    names = session.filter(WORK_UNITS_ + work_spec_name + suffix, priority_min=priority_min, priority_max=priority_max, limit=1000)
                    if not names:
                        break
                    count += session.popmany((WORK_UNITS_ + work_spec_name + suffix), *names)

        else:
            with self.registry.lock(identifier=self.worker_id) as (session):
                count = session.popmany((WORK_UNITS_ + work_spec_name + suffix), *work_unit_names)
        return count

    def del_work_units(self, work_spec_name, work_unit_keys=None, state=None, all=False):
    ...
    def remove_available_work_units(self, work_spec_name, work_unit_names):
        ...
        """
        return self._remove_some_work_units(work_spec_name, work_unit_names, priority_max=time.time())
    ...
    def remove_pending_work_units(self, work_spec_name, work_unit_names):
        ...
        """
        return self._remove_some_work_units(work_spec_name, work_unit_names, priority_min=time.time())
    ...
    def remove_blocked_work_units(self, work_spec_name, work_unit_names):
        ...
        """
        return self._remove_some_work_units(work_spec_name, work_unit_names, suffix=_BLOCKED)
    ...
    def remove_failed_work_units(self, work_spec_name, work_unit_names):
        ...
        """
        return self._remove_some_work_units(work_spec_name, work_unit_names, suffix=_FAILED)
    ...
    def remove_finished_work_units(self, work_spec_name, work_unit_names):
        ...
        """
        return self._remove_some_work_units(work_spec_name, work_unit_names, suffix=_FINISHED)
    ...
    def inspect_work_unit(self, work_spec_name, work_unit_key):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            ...
            if not work_unit_data:
                work_unit_data = session.get(WORK_UNITS_ + work_spec_name + _BLOCKED, work_unit_key)
            if not work_unit_data:
                work_unit_data = session.get(WORK_UNITS_ + work_spec_name + _FINISHED, work_unit_key)
            if not work_unit_data:
                work_unit_data = session.get(WORK_UNITS_ + work_spec_name + _FAILED, work_unit_key)
    ...
    def reset_all(self, work_spec_name):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            session.move_all(WORK_UNITS_ + work_spec_name + _FINISHED, WORK_UNITS_ + work_spec_name)
    ...
    def update_bundle(self, work_spec, work_units, nice=0):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            ...
            for i in xrange(0, len(work_units), window_size):
                ...
                session.update(WORK_UNITS_ + work_spec_name, dict(work_units[i:i + window_size]))
    ...
    def add_work_units(self, work_spec_name, work_unit_key_vals):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            ...
            for i in xrange(0, len(work_unit_key_vals), window_size):
                self.registry.re_acquire_lock()
                session.update(WORK_UNITS_ + work_spec_name, dict(work_unit_key_vals[i:i + window_size]))
    ...
    def retry(self, work_spec_name, *work_unit_names):
        ...
        with self.registry.lock(identifier=self.worker_id) as (session):
            ...
            for work_unit_name in work_unit_names:
                unit = session.get(WORK_UNITS_ + work_spec_name + _FAILED, work_unit_name)
                if unit is None:
            ...
            session.move(WORK_UNITS_ + work_spec_name + _FAILED, WORK_UNITS_ + work_spec_name, units, priority=0)
        return
    ...
    def get_assigned_work_unit(self, worker_id, work_spec_name, work_unit_key):
        """get a specific WorkUnit that has already been assigned to a
        particular worker_id
        """
        with self.registry.lock(identifier=self.worker_id) as (session):
            assigned_work_unit_key = session.get(WORK_UNITS_ + work_spec_name + '_locks', worker_id)
            if not assigned_work_unit_key == work_unit_key:
                raise LostLease('assigned_work_unit_key=%r != %r' % (
                 assigned_work_unit_key, work_unit_key))
            work_unit_data = session.get(WORK_UNITS_ + work_spec_name, work_unit_key)
            return WorkUnit(self.registry, work_spec_name, work_unit_key, work_unit_data, worker_id=worker_id, default_lifetime=self.default_lifetime)
            ...
            for child in all_children.iterkeys():
                ...
                if work_spec_name:
                    ...
                    if data is None:
                        ...
                        result[child] = WorkUnit(self.registry, work_spec_name, work_unit_key, data, expires=expires, worker_id=assigned)
                else:
                    heartbeat = session.get(WORKER_OBSERVED_MODE, child)
                    if heartbeat:
                        result[child] = None
                    else:
                        session.popmany(WORKER_CHILDREN_ + worker_id, child)

        return result

    def worker_register(self, worker_id, mode=None, lifetime=6000, environment=None, parent=None):
        self.worker_heartbeat(worker_id, mode, lifetime, environment, parent)

    def worker_heartbeat(self, worker_id, mode=None, lifetime=6000, environment=None, parent=None):
        if environment is None:
            environment = {}
        with self.registry.lock(identifier=worker_id) as (session):
            session.set(WORKER_OBSERVED_MODE, worker_id, mode, priority=time.time() + lifetime)
            session.update(WORKER_STATE_ + worker_id, environment, expire=lifetime)
            if parent:
                session.update(WORKER_CHILDREN_ + parent, {worker_id: 1})
        return
