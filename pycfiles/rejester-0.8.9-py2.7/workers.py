...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rejester/workers.py
...
import errno, logging, multiprocessing
from operator import itemgetter
import os, psutil, random, Queue, signal, struct, sys, threading, time, traceback
from setproctitle import setproctitle
import dblogger, rejester
...
class HeadlessWorker(Worker):
    ...
    def __init__(self, config, worker_id, work_spec_name, work_unit_key):
        ...
        self.work_unit = self.task_master.get_assigned_work_unit(worker_id, work_spec_name, work_unit_key)
...
class MultiWorker(Worker):
    ...
    def available_gb(cls):
        if cls._available_gb is None:
            ...
            cls._available_gb = float(mem.available) / (multiprocessing.cpu_count() * 1073741824)
    ...
    def _poll_async_result(self, async_result, work_unit, do_update=True):
        if async_result is None:
            ...
            if not work_unit is not None:
                raise AssertionError
                if async_result.ready() or do_update:
    ...
    def _get_and_start_work(self):
        ...
        if work_unit is None:
            ...
            async_result = self.pool.apply_async(run_worker, (
             HeadlessWorker, self.task_master.registry.config,
             worker_id,
             work_unit.work_spec_name,
             work_unit.key), callback=self._finish_callback)
    ...
    def run(self):
        ...
        while True:
            mode = self.heartbeat()
            if mode != self._mode:
                logger.info(('worker {0} changed to mode {1}').format(self.worker_id, mode))
                self._mode = mode
            now = time.time()
...
class SingleWorker(Worker):
    ...
    def _run_unit(self, unit, set_title=False):
        try:
            if set_title:
                setproctitle(('rejester worker {0!r} {1!r}').format(unit.work_spec_name, unit.key))
            unit.run()
            unit.finish()
        except LostLease as e:
            pass
    ...
    def as_child(cls, global_config, parent=None):
        ...
        try:
            ...
            random.seed()
            yakonfig.set_default_config([yakonfig, dblogger, rejester], config=global_config)
...
class LoopWorker(SingleWorker):
    ...
    def run_loop(self, set_title=False):
        now = time.time()
        try:
            duration = yakonfig.get_global_config('rejester', 'fork_worker', 'child_lifetime')
        except KeyError:
            duration = 10
...
class ForkWorker(Worker):
    ...
    def __init__(self, config):
        ...
        self.heartbeat_deadline = time.time() - 1
    ...
    def set_signal_handlers(self):
        ...
        self.old_sigabrt = signal.signal(signal.SIGABRT, handler(self.scram))
        self.old_sigint = signal.signal(signal.SIGINT, handler(self.stop_gracefully))
        self.old_sigpipe = signal.signal(signal.SIGPIPE, handler(self.live_log_child))
        signal.siginterrupt(signal.SIGPIPE, False)
        self.old_sigterm = signal.signal(signal.SIGTERM, handler(self.stop_gracefully))
    ...
    def do_some_work(self, can_start_more):
        ...
        self.debug('loop', ('starting work loop, can_start_more={0!r}').format(can_start_more))
        while True:
            try:
            ...
            if pid == 0:
                ...
                self.debug('children', ('log child with pid {0} exited').format(pid))
                self.start_log_child()
            elif pid in self.children:
                self.children.remove(pid)
                if os.WIFEXITED(status):
                    code = os.WEXITSTATUS(status)
                    self.debug('children', ('worker {0} exited with code {1}').format(pid, code))
                    if code == SingleWorker.EXIT_SUCCESS:
                        any_happy_children = True
                    elif code == SingleWorker.EXIT_EXCEPTION:
                        self.log(logging.WARNING, ('child {0} reported failure').format(pid))
                        any_sad_children = True
                    elif code == SingleWorker.EXIT_BORED:
                        any_bored_children = True
                    else:
                        self.log(logging.WARNING, ('child {0} had odd exit code {1}').format(pid, code))
                elif os.WIFSIGNALED(status):
                    self.log(logging.WARNING, ('child {0} exited with signal {1}').format(pid, os.WTERMSIG(status)))
                    any_sad_children = True
                else:
                    self.log(logging.WARNING, ('child {0} went away with unknown status {1}').format(pid, status))
                    any_sad_children = True
            else:
                self.log(logging.WARNING, ("child {0} exited, but we don't recognize it").format(pid))
        ...
        if can_start_more and len(self.children) < self.num_workers:
            ...
            if pid == 0:
                ...
                if self.log_fd:
                    os.close(self.log_fd)
                LoopWorker.as_child(yakonfig.get_global_config(), parent=self.worker_id)
    ...
    def run(self):
        ...
        """
        setproctitle(('rejester fork_worker for namespace {0}').format(self.config.get('namespace', None)))
                ...
                while True:
                    ...
                    if time.time() >= self.heartbeat_deadline:
                        ...
                        if mode != self.last_mode:
                            self.log(logging.INFO, ('rejester global mode is {0!r}').format(mode))
                            self.last_mode = mode
                        self.heartbeat_deadline = time.time() + self.heartbeat_interval
                    ...
                    if len(self.children) == 0:
                        if mode == self.task_master.TERMINATE:
                            self.log(logging.INFO, 'stopping for rejester global shutdown')
                            break
                        if self.shutting_down:
                            self.log(logging.INFO, 'stopping in response to signal')
                            break
                    time.sleep(interval)

            except Exception:
                self.log(logging.CRITICAL, 'uncaught exception in worker: ' + traceback.format_exc())
