...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/snap/fundamental_polyhedron.py
...
class FundamentalPolyhedronEngine(McomplexEngine):
    ...
    def fromManifoldAndShapesMatchingSnapPea(manifold, shapes, normalize_matrices=False):
        ...
        f.unglue()
        f.visit_tetrahedra_to_compute_vertices(m.ChooseGenInitialTet, f.init_vertices_snappea())
    ...
    _VerticesInFace = {F:[ V for V in simplex.ZeroSubsimplices if t3m.is_subset(V, F) ] for F in simplex.TwoSubsimplices}
    ...
    def init_vertices_snappea(self):
        ...
        for perm in Perm4.A4():
            ...
            for sign in [one, minus_one]:
                candidates = {perm.image(simplex.V0): NumericField(0), 
                   perm.image(simplex.V1): Infinity, 
                   perm.image(simplex.V2): sign * sqrt_z, 
                   perm.image(simplex.V3): sign * sqrt_z_inv}
                if _are_vertices_close_to_snappea(candidates, tet.SnapPeaIdealVertices):
                    return candidates
...
_RemainingFace = {(V0, V1): V3, (V0, V2): V1, (V0, V3): V2, (V1, V0): V2, 
   (V1, V2): V3, (V1, V3): V0, (V2, V0): V3, 
   (V2, V1): V0, (V2, V3): V1, (V3, V0): V1, 
   (V3, V1): V2, (V3, V2): V0}
...
def _matrix_L1_distance_to_snappea(m, snappeaM):
    return sum([ abs(_diff_to_snappea(m[(i, j)], snappeaM[(i, j)])) for i in range(2) for j in range(2)
               ])
...
def _negate_matrices_to_match_snappea(matrices, G):
    ...
    """
    return [ _negate_matrix_to_match_snappea(m, matrix(G.SL2C(g))) for m, g in zip(matrices, G.generators())
           ]