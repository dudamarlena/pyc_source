...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/snap/interval_reps.py
...
def holonomy_from_shape_intervals(manifold, shape_intervals, fundamental_group_args=[], lift_to_SL2=True):
    """
    ...
    f = FundamentalPolyhedronEngine.fromManifoldAndShapesMatchingSnapPea(M, shape_intervals, normalize_matrices=True)
    mats = f.matrices_for_presentation(G, match_snappea=True)
    PG = polished_reps.ManifoldGroup(G.generators(), G.relators(), G.peripheral_curves(), mats)
    if lift_to_SL2:
        PG.lift_to_SL2C()
