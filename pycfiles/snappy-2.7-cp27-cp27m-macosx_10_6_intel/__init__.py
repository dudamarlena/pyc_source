...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/__init__.py
# Compiled at: 2019-07-15 23:56:54
import sys
from .SnapPy import AbelianGroup, HolonomyGroup, FundamentalGroup, DirichletDomain, CuspNeighborhood, SymmetryGroup, AlternatingKnotExteriors, NonalternatingKnotExteriors, pari
...
__all__ = [
 'Triangulation', 'Manifold', 'ManifoldHP', 'AbelianGroup',
...
def canonical_retriangulation(manifold, verified=False, interval_bits_precs=verify.default_interval_bits_precs, exact_bits_prec_and_degrees=verify.default_exact_bits_prec_and_degrees, verbose=False):
    """
    ...
    if verified:
        return verify.verified_canonical_retriangulation(manifold, interval_bits_precs=interval_bits_precs, exact_bits_prec_and_degrees=exact_bits_prec_and_degrees, verbose=verbose)
    else:
...
def isometry_signature(manifold, of_link=False, verified=False, interval_bits_precs=verify.default_interval_bits_precs, exact_bits_prec_and_degrees=verify.default_exact_bits_prec_and_degrees, verbose=False):
    """
    ...
    retrig = manifold.canonical_retriangulation(verified=verified, interval_bits_precs=interval_bits_precs, exact_bits_prec_and_degrees=exact_bits_prec_and_degrees, verbose=verbose)
    if not retrig:
        return None
    else:
        return retrig.triangulation_isosig(decorated=of_link, ignore_cusp_ordering=True, ignore_curve_orientations=True)


Manifold.isometry_signature = isometry_signature
ManifoldHP.isometry_signature = isometry_signature

def cusp_translations(manifold, areas=None, canonize=True, verified=False, bits_prec=None):
    """
    ...
    **Remark:** Since the code is (potentially) non-deterministic, this does not
    ...
    if canonize:
        ...
        manifold.canonize()
    return verify.cusp_translations_for_manifold(manifold, areas=areas, verified=verified, bits_prec=bits_prec)
...
def all_translations(self, verified=False, bits_prec=None):
    ...
    **Remark:** Since the code is (potentially) non-deterministic, the result of ::
    ...
    if verified or bits_prec:
        return verify.cusp_translations_for_neighborhood(self, verified=verified, bits_prec=bits_prec)
...
def _link_exterior(self, with_hyperbolic_structure=True, remove_finite_vertices=True):
    """
...
link_objects = []
from spherogram.links import Crossing, Strand, Link, Tangle, RationalTangle, ZeroTangle, InfinityTangle, IdentityBraid, random_link
