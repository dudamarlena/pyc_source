...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/processComponents.py
...
def process_component(py_eval, manifold_thunk, variables, component):
    params, body = processFileBase.extract_parameters_and_body_from_section(component)
    ...
    if dimension > 0 and 'FREE=VARIABLES' in params.keys():
        free_vars_str = processFileBase.remove_optional_outer_square_brackets(params['FREE=VARIABLES'])
        free_variables = [ remove_optional_quotes(v.strip()) for v in free_vars_str.split()
                         ]
    if dimension == 0:
        return process_solutions_provider(py_eval, manifold_thunk, body, 0, variables)
    else:
        witnesses = []
        witnesses_sections = processFileBase.find_section(body, 'WITNESSES')
        if witnesses_sections:
            assert len(witnesses_sections) == 1
            witnesses = process_witnesses(py_eval, manifold_thunk, witnesses_sections[0], dimension, variables)
        return NonZeroDimensionalComponent(witnesses=witnesses, dimension=dimension, free_variables=free_variables)
        return
