...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/verify/cuspCrossSection.py
...
class RealHoroTriangle():
    ...
    def __init__(self, tet, vertex, known_side, length_of_side):
        ...
        L = length_of_side
        self.lengths = {center_side: L, left_side: abs(z_left) * L, 
...
class ComplexHoroTriangle():
    ...
    def __init__(self, tet, vertex, known_side, length_of_side):
        left_side, center_side, right_side, z_left, z_right = HoroTriangleBase._sides_and_cross_ratios(tet, vertex, known_side)
        L = length_of_side
        self.lengths = {center_side: L, left_side: -z_left * L, 
...
class CuspCrossSectionBase(McomplexEngine):
    ...
    def _add_cusp_cross_sections(self):
        for T in self.mcomplex.Tetrahedra:
            T.horotriangles = {t3m.simplex.V0: None, t3m.simplex.V1: None, 
               t3m.simplex.V2: None, 
               t3m.simplex.V3: None}
    ...
    def _glued_to(tetrahedron, face):
        ...
        """
        return (
         tetrahedron.Neighbor[face], tetrahedron.Gluing[face].image(face))
    ...
    def check_logarithmic_edge_equations_and_positivity(self, NumericalField):
        ...
        for edge in self.mcomplex.Edges:
            ...
            twoPiI = NumericalField.pi() * NumericalField(complex(0.0, 2.0))
            if not abs(log_sum - twoPiI) < 1e-07:
    ...
    def _exp_distance_of_edges(edges):
        ...
        """
        return interval_aware_min([ ComplexCuspCrossSection._exp_distance_edge(edge) for edge in edges
                                  ])
