...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/fieldExtensions.py
...
def my_rnfequation(base_poly, extension_poly):
    ...
    base_var = base_vars[0]
    base_degree = base_poly.degree()
    ...
    extension_var = list(extra_vars)[0]
    extension_degree = extension_poly.degree(extension_var)
    ...
    for k in range(start_k, -100, -1):
        x = Polynomial.from_variable_name(extension_var) + Polynomial.constant_polynomial(k) * Polynomial.from_variable_name(base_var)
        power_of_x = Polynomial.constant_polynomial(1)
        mat = []
        for i in range(total_degree):
            mat.append(_poly_to_row(power_of_x, base_var, base_degree, extension_var, extension_degree))
            power_of_x = _reduced_polynomial(_reduced_polynomial(power_of_x * x, extension_poly, extension_var, extension_degree), base_poly, base_var, base_degree)

        row_largest_power = _poly_to_row(power_of_x, base_var, base_degree, extension_var, extension_degree)
        try:
            ...
            new_poly_1 = _row_to_poly(matrix.matrix_mult_vector(mat_inv_t, row_largest_power))
            new_poly_2 = Polynomial.from_variable_name('x') ** total_degree
            new_poly = new_poly_2 - new_poly_1
            old_var_in_new_var = _row_to_poly(matrix.matrix_mult_vector(mat_inv_t, [
             0, 1] + (total_degree - 2) * [0]))
            return (
             new_poly, old_var_in_new_var, k)
...
def _row_to_poly(row):
    ...
    return sum([ Polynomial.constant_polynomial(c) * x ** i for i, c in enumerate(row)
               ], zero)
