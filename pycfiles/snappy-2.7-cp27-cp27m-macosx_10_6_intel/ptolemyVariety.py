...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/ptolemyVariety.py
...
class PtolemyVariety(object):
    ...
    def __init__(self, manifold, N, obstruction_class, simplify, eliminate_fixed_ptolemys):
        ...
        if simplify:
            ...
            for sign, power, var1, var2 in self._identified_variables:
                self.canonical_representative[var1] = (
                 +1, 0, var1)
                if var2 != 1:
                    self.canonical_representative[var2] = (
                     +1, 0, var2)
                if order_of_u == 2:
            ...
            for var in self.variables:
                if var[0:2] == 'c_':
                    if var not in self.canonical_representative:
                        self.canonical_representative[var] = (
                         +1, 0, var)
    ...
    def to_magma(self, template_path='magma/default.magma_template'):
        ...
        return Template(template).safe_substitute(PREAMBLE=PREAMBLE, QUOTED_PREAMBLE=QUOTED_PREAMBLE, VARIABLES=(', ').join(self.variables), VARIABLES_QUOTED=(', ').join([ '"%s"' % v for v in self.variables ]), VARIABLE_NUMBER=len(self.variables), VARIABLES_WITH_NON_ZERO_CONDITION=(', ').join(self.variables_with_non_zero_condition), VARIABLES_WITH_NON_ZERO_CONDITION_QUOTED=(', ').join([ '"%s"' % v for v in self.variables_with_non_zero_condition
                                                                                                                                                                                                                                                                                                                                                                                                    ]), VARIABLE_WITH_NON_ZERO_CONDITION_NUMBER=len(self.variables_with_non_zero_condition), EQUATIONS=(',\n          ').join([ str(eqn) for eqn in self.equations
...
def _generate_ptolemy_relations(N, num_tet, has_obstruction_class):
    ...
    def generate_ptolemy_relation(tet, index):
        ...
        return generate_obstruction_variable(0) * generate_obstruction_variable(1) * generate_Ptolemy_coordinate((1,
                                                                                                                  1,
                                                                                                                  0,
                                                                                                                  0)) * generate_Ptolemy_coordinate((0,
                                                                                                                                                     0,
                                                                                                                                                     1,
                                                                                                                                                     1)) - generate_obstruction_variable(0) * generate_obstruction_variable(2) * generate_Ptolemy_coordinate((1,
                                                                                                                                                                                                                                                              0,
                                                                                                                                                                                                                                                              1,
                                                                                                                                                                                                                                                              0)) * generate_Ptolemy_coordinate((0,
                                                                                                                                                                                                                                                                                                 1,
                                                                                                                                                                                                                                                                                                 0,
                                                                                                                                                                                                                                                                                                 1)) + generate_obstruction_variable(0) * generate_obstruction_variable(3) * generate_Ptolemy_coordinate((1,
                                                                                                                                                                                                                                                                                                                                                                                                          0,
                                                                                                                                                                                                                                                                                                                                                                                                          0,
                                                                                                                                                                                                                                                                                                                                                                                                          1)) * generate_Ptolemy_coordinate((0,
                                                                                                                                                                                                                                                                                                                                                                                                                                             1,
                                                                                                                                                                                                                                                                                                                                                                                                                                             1,
                                                                                                                                                                                                                                                                                                                                                                                                                                             0))

    return [ generate_ptolemy_relation(tet, index) for tet in range(num_tet) for index in utilities.quadruples_with_fixed_sum_iterator(N - 2)
           ]
...
def _identified_variables_canonize(identified_variables):
    ...
    for sign, power, var1, var2 in identified_variables:
        if all_variables[var1] is not all_variables[var2]:
            new_dict = merge_two_dicts(sign, power, var1, var2, all_variables[var1], all_variables[var2])
    ...
    for variable, variable_dict in all_variables.items():
        if variable not in result:
            ...
            canonical_rep_sign, canonical_rep_power = variable_dict[canonical_rep]
...
def _retrieve_url(url):
    ...
    try:
        try:
            ...
            if url[:5] == 'http:':
                raise RuntimeError('Problem connecting to server while retrieving %s: %s' % (
                 url, e))
            else:
                raise PtolemyFileMissingError('The ptolemy variety has probably not been computed yet or the given data_url or environment variable PTOLEMY_DATA_URL is not configured correctly: %s. Also see %s' % (
                 e, overview_url))
    ...
    if url[:5] != 'http:':
        ...
        if code == 404:
            raise PtolemyFileMissingError('The ptolemy variety has probably not been computed yet, see %s (%s)' % (
             overview_url, httpErr))
