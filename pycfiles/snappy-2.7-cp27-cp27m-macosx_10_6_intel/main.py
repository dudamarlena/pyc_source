...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/twister/main.py
...
def _parse_surface(surface_contents):
    ...
    return (
     num_squares, curves, vertices, boundary_components)
...
class Surface:
    ...
    def info(self, verbose=False):
        ...
        if verbose:
            print('Inverse names: %s' % inv_annuli)
        print('Arcs: %s' % rectangles)
        if verbose:
            print('Inverse names: %s' % inv_rectangles)
        if verbose:
...
def LP_surface(genus, boundary, make_prefix_unique=True):
    ...
    contents = [
     '# A Twister surface file produced by LP_surface.']
...
def DT_drilling_surface(code, make_prefix_unique=True):
    ...
    for i in range(1, 2 * num_crossings + 1):
        if i % 2 == 1:
            ...
            if signs[k] == 1:
                cells = [
                 '-' + str(k * 5 + 4), '-' + str(k * 5 + 2), '+' + str(k * 5 + 0)]
            else:
                cells = [
                 '-' + str(k * 5 + 0), '-' + str(k * 5 + 2), '+' + str(k * 5 + 4)]
    ...
    for i in range(1, 2 * num_crossings + 1):
        ...
        if i % 2 == 1:
            ...
            if signs[k] == 1:
                ...
                cells = [
                 '+' + str(k * 5 + 0), '+' + str(l * 5 + 1)]
        name = 'a_' + str(i).zfill(padded_length)
        inverse_name = name.swapcase()
