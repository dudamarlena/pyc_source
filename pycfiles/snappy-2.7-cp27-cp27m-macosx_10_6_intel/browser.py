...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/browser.py
# Compiled at: 2019-07-15 23:56:54
from __future__ import unicode_literals
from builtins import range
import sys, os
if sys.version_info[0] < 3:
    import Tkinter as Tk_, ttk
...
if sys.version_info.major < 3 or sys.version_info.minor < 7:
    ...
    class Spinbox(ttk.Entry):
        ...
        def __init__(self, master=None, **kw):
            ttk.Entry.__init__(self, master, b'ttk::spinbox', **kw)
...
class SelectableText(ttk.Frame):
    ...
    def __init__(self, master, labeltext=b'', width=18, depth=2):
        ...
        self.value = value = Tk_.Entry(self, textvariable=self.var, state=b'readonly', borderwidth=0, readonlybackground=bg_color, highlightbackground=bg_color, highlightcolor=bg_color, highlightthickness=0, takefocus=False)
        if width:
...
class SelectableMessage(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        text.bind(b'<Button-1>', lambda *args: self.text.focus_set())
        text.bind(b'<FocusOut>', self.focus_out)
    ...
    def set(self, message):
        self.text.config(state=Tk_.NORMAL)
        self.text.delete(b'0.1', Tk_.END)
        self.text.selection_clear()
        self.text.insert(Tk_.INSERT, message)
        self.text.config(state=Tk_.DISABLED)

    def get(self):
        return self.text.get(b'0.1', Tk_.END)

    def copy(self, event):
        self.text.selection_get(selection=b'CLIPBOARD')


class DirichletTab(PolyhedronViewer):

    def __init__(self, facedicts, root, title=b'Polyhedron Tab', container=None):
        self.main_window = main_window
        self.style = style = SnapPyStyle()
        PolyhedronViewer.__init__(self, facedicts, root=root, title=title, container=container, bgcolor=style.groupBG)

    def update_menus(self, menubar):
        menubar.children[b'help'].activate([b'Polyhedron Viewer Help ...'])
...
class CuspNeighborhoodTab(HoroballViewer):
    ...
    def __init__(self, nbhd, root, title=b'Polyhedron Tab', container=None):
        self.main_window = main_window
        style = SnapPyStyle()
        if main_window:
            HoroballViewer.__init__(self, nbhd, root=root, title=title, container=container, bgcolor=style.groupBG, prefs=main_window.prefs)
        else:
            HoroballViewer.__init__(self, nbhd, root=root, title=title, container=container, bgcolor=style.groupBG)

    def update_menus(self, menubar):
        menubar.children[b'help'].activate([b'Horoball Viewer Help ...'])
...
class LinkTab(LinkViewer):
    ...
    def __init__(self, data, window):
        ...
        canvas.bind(b'<Configure>', lambda event: self.draw())
...
class Browser():
    ...
    def __init__(self, manifold, root=None):
        if manifold.num_tetrahedra() == 0:
            raise ValueError(b'The empty Manifold cannot be browsed.')
        ...
        self.dirichlet_viewer = DirichletTab(facedicts=[], root=window, container=dirichlet_tab)
        self.horoball_tab = horoball_tab = Tk_.Frame(window)
        self.horoball_viewer = CuspNeighborhoodTab(nbhd=None, root=window, container=horoball_tab)
        self.symmetry_tab = symmetry_tab = self.build_symmetry()
        self.link_tab = link_tab = self.build_link()
        notebook.add(invariants_tab, text=b'Invariants', padding=[0])
        notebook.add(dirichlet_tab, text=b'Dirichlet')
        notebook.add(horoball_tab, text=b'Cusp Nbhds')
        notebook.add(symmetry_tab, text=b'Symmetry', padding=[0])
        if link_tab:
            notebook.add(link_tab.canvas, text=b'Link')
        notebook.bind(b'<<NotebookTabChanged>>', self.update_current_tab)
        self.bottombar = bottombar = ttk.Frame(window, height=20)
        bg = self.style.ttk_style.lookup(b'TLable', b'background')
        fg = self.style.ttk_style.lookup(b'TLable', b'foreground')
        self.modeline = modeline = Tk_.Text(self.bottombar, height=1, relief=Tk_.FLAT, background=bg, foreground=fg, selectborderwidth=0, highlightbackground=bg, highlightcolor=bg, highlightthickness=0, takefocus=False, state=Tk_.DISABLED)
        self.modeline.tag_config(b'alert', foreground=b'red')
        self.build_menus()
    ...
    def build_side_panel(self):
        ...
        filling = ttk.Labelframe(side_panel, text=b'Filling Curves', padding=(10, 10))
        ...
        if num_cusps > 3:
            ...
            self.filling_canvas = canvas = Tk_.Canvas(filling, bd=0, highlightthickness=0, yscrollcommand=filling_scrollbar.set, scrollregion=(
             0, 0, cusp_box_width + 20,
        ...
        for n in range(num_cusps):
            R, G, B, A = GetColor(n)
            color = b'#%.3x%.3x%.3x' % (int(R * 4095), int(G * 4095), int(B * 4095))
            cusp = ttk.Labelframe(cusp_parent, text=b'Cusp %d' % n)
            mer_var = Tk_.StringVar(window, value=b'0')
            long_var = Tk_.StringVar(window, value=b'0')
            ...
            ttk.Label(cusp, text=b'Meridian:').grid(row=0, column=1, sticky=Tk_.E)
            ttk.Label(cusp, text=b'Longitude:').grid(row=1, column=1, sticky=Tk_.E)
            meridian = Spinbox(cusp, width=4, textvariable=mer_var, from_=-1000, to=1000, increment=1, name=b':%s:0' % n, validate=b'focusout', validatecommand=(
             window.register(self.validate_coeff), b'%P', b'%W'))
            meridian.bind(b'<Return>', self.do_filling)
            meridian.grid(row=0, column=2, sticky=Tk_.W, padx=0, pady=3)
            longitude = Spinbox(cusp, width=4, textvariable=long_var, from_=-1000, to=1000, increment=1, name=b':%s:1' % n, validate=b'focusout', validatecommand=(
             window.register(self.validate_coeff), b'%P', b'%W'))
            longitude.bind(b'<Return>', self.do_filling)
        ...
        modify = ttk.Labelframe(side_panel, text=b'Modify', padding=(10, 10))
        self.fill_button = ttk.Button(modify, text=b'Fill', command=self.do_filling).pack(padx=10, pady=5, expand=True, fill=Tk_.X)
        ttk.Button(modify, text=b'Retriangulate', command=self.retriangulate).pack(padx=10, pady=5, expand=True, fill=Tk_.X)
        modify.grid(row=1, column=0, padx=10, pady=10, sticky=Tk_.EW)
        create = ttk.Labelframe(side_panel, text=b'Create', padding=(10, 10))
        ttk.Button(create, text=b'Drill ...', command=self.drill).pack(padx=10, pady=5, expand=True, fill=Tk_.X)
        ttk.Button(create, text=b'Cover ...', command=self.cover).pack(padx=10, pady=5, expand=True, fill=Tk_.X)
    ...
    def build_invariants(self):
        ...
        self.basic = basic = ttk.LabelFrame(frame, text=b'Basic Invariants', padding=(10,
                                                                                      10))
        for n in range(4):
            basic.grid_rowconfigure(n, weight=1)

        self.volume = SelectableText(basic, labeltext=b'Volume:')
        self.volume.grid(row=0, column=0, padx=5, pady=0, sticky=Tk_.E)
        self.cs = SelectableText(basic, labeltext=b'Chern-Simons:')
        self.cs.grid(row=1, column=0, padx=5, pady=0, sticky=Tk_.E)
        self.homology = SelectableText(basic, labeltext=b'H₁:')
        self.homology.grid(row=2, column=0, padx=5, pady=0, sticky=Tk_.E)
        self.orientability = SelectableText(basic, labeltext=b'Orientable:')
        ...
        self.aka = ttk.LabelFrame(frame, text=b'Also Known As', padding=(10, 10))
        self.aka_viewer = aka_viewer = ttk.Treeview(self.aka, selectmode=b'none', height=4, columns=[
         b'manifold', b'as_link'], show=b'headings')
        aka_viewer.heading(b'manifold', text=b'Manifold')
        aka_viewer.column(b'manifold', stretch=True, width=100)
        aka_viewer.heading(b'as_link', text=b'Same Link')
        aka_viewer.column(b'as_link', stretch=True, width=100)
        aka_viewer.pack(expand=True, fill=Tk_.BOTH)
        self.aka.grid(row=1, column=0, padx=10, pady=10, sticky=Tk_.NSEW)
        self.fundamental = fundamental = ttk.LabelFrame(frame, text=b'Fundamental Group', padding=(10,
                                                                                                   10))
        ...
        self.simplify = ttk.Checkbutton(self.pi_one_options, variable=self.simplify_var, text=b'simplified presentation', command=self.compute_pi_one)
        self.simplify.grid(row=0, column=0, sticky=Tk_.W)
        self.minimize_var = Tk_.BooleanVar(fundamental, value=True)
        self.minimize = ttk.Checkbutton(self.pi_one_options, variable=self.minimize_var, text=b'minimal number of generators', command=self.compute_pi_one)
        self.minimize.grid(row=1, column=0, sticky=Tk_.W)
        self.gens_change_var = Tk_.BooleanVar(fundamental, value=True)
        self.gens_change = ttk.Checkbutton(self.pi_one_options, variable=self.gens_change_var, text=b'fillings may affect generators', command=self.compute_pi_one)
        self.gens_change.grid(row=2, column=0, sticky=Tk_.W)
        self.pi_one_options.grid(row=1, column=0, padx=10, pady=10, sticky=Tk_.W)
        fundamental.grid(row=0, column=1, rowspan=2, padx=10, pady=10, sticky=Tk_.W)
        self.length_spectrum_frame = ttk.LabelFrame(frame, text=b'Length Spectrum', padding=(10,
                                                                                             10))
        self.length_spectrum_frame.grid_columnconfigure(1, weight=1)
        ttk.Label(self.length_spectrum_frame, text=b'Cutoff:').grid(row=0, column=0, sticky=Tk_.E, pady=5)
        self.length_cutoff = 1.0
        self.cutoff_var = Tk_.StringVar(self.window, self.length_cutoff)
        self.cutoff_entry = cutoff_entry = ttk.Entry(self.length_spectrum_frame, takefocus=False, width=6, textvariable=self.cutoff_var, validate=b'focusout', validatecommand=(
         self.window.register(self.validate_cutoff), b'%P'))
        cutoff_entry.bind(b'<Return>', lambda event: self.window.focus_set())
        cutoff_entry.grid(row=0, column=1, sticky=Tk_.W, pady=5)
        self.geodesics = geodesics = ttk.Treeview(self.length_spectrum_frame, height=6, show=b'headings', selectmode=b'none', columns=[
         b'mult', b'length', b'topology', b'parity'])
        geodesics.heading(b'mult', text=b'Mult.')
        geodesics.column(b'mult', stretch=False, width=60)
        geodesics.heading(b'length', text=b'Complex Length')
        geodesics.column(b'length', stretch=True, minwidth=480)
        geodesics.heading(b'topology', text=b'Topology')
        geodesics.column(b'topology', stretch=True, width=100)
        geodesics.heading(b'parity', text=b'Parity')
        geodesics.column(b'parity', stretch=True, width=80)
        geodesics.grid(row=1, column=0, columnspan=2, sticky=Tk_.W, padx=10, pady=10)
        self.length_spectrum_frame.grid(row=4, columnspan=2, padx=10, pady=10, sticky=Tk_.NSEW)
        return frame
    ...
    def build_symmetry(self):
        ...
        self.symmetry = SelectableText(frame, labeltext=b'Symmetry Group:', width=30, depth=1)
        self.symmetry.grid(row=0, column=0, pady=20)
        message1 = ttk.Label(frame, text=b'Future releases of SnapPy will show more information here.')
        message2 = ttk.Label(frame, text=b'Type SymmetryGroup.<tab> in the command shell to see what is available.')
    ...
    def update_menus(self, menubar):
        ...
        menubar.children[b'help'].activate([])
    ...
    def update_modeline(self):
        ...
        if not self.manifold.is_orientable():
            modeline.insert(Tk_.END, b'Non-orientable; ')
        modeline.insert(Tk_.END, b'%s tetrahedra; %s' % (
         self.manifold.num_tetrahedra(), self.manifold.solution_type()))
        if len(self.dirichlet) == 0:
            modeline.insert(Tk_.END, b'  * failed to compute Dirichlet domain!', b'alert')
        modeline.config(state=Tk_.DISABLED)

    def update_current_tab(self, event=None):
        self.update_modeline()
        self.update_side_panel()
        tab_name = self.notebook.tab(self.notebook.select(), b'text')
        if tab_name == b'Invariants':
            self.update_menus(self.menubar)
            self.update_invariants()
        if tab_name == b'Cusp Nbhds':
    ...
    def update_side_panel(self):
        ...
        for n, coeffs in enumerate(current_fillings):
            for m in (0, 1):
                value = b'%g' % coeffs[m]
                value = b'0' if value == b'-0' else value
    ...
    def update_invariants(self):
        ...
        if not self.recompute_invariants:
            return
        self.orientability.set(b'Yes' if manifold.is_orientable() else b'No')
        try:
    ...
    def update_length_spectrum(self):
        OK = True
        try:
            self.length_spectrum = self.manifold.length_spectrum(self.length_cutoff)
        except RuntimeError:
            OK = False
            self.length_spectrum = []
        ...
        if OK:
            for geodesic in self.length_spectrum:
                parity = b'+' if geodesic[b'parity'].endswith(b'preserving') else b'-'
                self.geodesics.insert(b'', b'end', values=(
                 geodesic[b'multiplicity'],
                 Number(geodesic[b'length'], accuracy=25),
                 geodesic[b'topology'],
                 parity))

        else:
            self.geodesics.insert(b'', b'end', values=('?', 'Not computable without a Dirichlet domain',
                                                       '?', '?'))
    ...
    def update_aka(self):
        ...
        mflds = {}
        mflds[b'weak'] = [ N.name() for N in M.identify() ]
        mflds[b'strong'] = [ N.name() for N in M.identify(True) ]
        self._write_aka_info(mflds)

    def _aka_callback(self):
        self.window.after_cancel(self.aka_after_id)
        self.aka_after_id = None
        if self.identifier.state == b'finished':
    ...
    def _write_aka_info(self, mflds=None):
        ...
        if mflds is None:
            aka_viewer.insert(b'', b'end', values=('Working ...', ''))
        else:
            strong = set(mflds[b'strong'])
            weak = set(mflds[b'weak']) - strong
            for mfld in strong:
                aka_viewer.insert(b'', b'end', values=(mfld, b'Yes'))

            for mfld in weak:
                aka_viewer.insert(b'', b'end', values=(mfld, b'No'))
    ...
    def update_cusps(self):
        ...
        self.window.after(100, self.horoball_viewer.cutoff_entry.selection_clear)
        return
    ...
    def validate_coeff(self, P, W):
        ...
        try:
            ...
            if P == b'':
                ...
                value = b'%g' % self.manifold.cusp_info()[cusp].filling[curve]
                value = b'0' if value == b'-0' else value
                var.set(value)
            return False
    ...
    def validate_cutoff(self, P):
        try:
            ...
            self.window.after_idle(self.cutoff_var.set, str(self.length_cutoff))
            return False
    ...
    def do_filling(self, event=None):
        ...
        self.window.config(cursor=b'watch')
...
class Driller(SimpleDialog):
    ...
    def __init__(self, master, manifold):
        ...
        self.result = []
        style = SnapPyStyle()
        self.root = root = Tk_.Toplevel(master, class_=b'SnapPy', bg=style.windowBG)
        title = b'Drill'
        root.title(title)
        root.iconname(title)
        root.bind(b'<Return>', self.handle_return)
        top_frame = ttk.Frame(self.root)
        top_frame.grid_columnconfigure(0, weight=1)
        top_frame.grid_rowconfigure(2, weight=1)
        msg_font = Font(family=style.font_info[b'family'], weight=b'bold', size=int(style.font_info[b'size'] * 1.2))
        msg = ttk.Label(top_frame, font=msg_font, text=b'Choose which curves to drill out:')
        ...
        ttk.Label(segment_frame, text=b'Max segments: ').pack(side=Tk_.LEFT, padx=4)
        self.segment_entry = segment_entry = ttk.Entry(segment_frame, takefocus=False, width=2, textvariable=segment_var, validate=b'focusout', validatecommand=(
         root.register(self.validate_segments), b'%P'))
        segment_entry.pack(side=Tk_.LEFT)
        segment_frame.grid(row=1, column=0, pady=2)
        self.curves = curves = ttk.Treeview(top_frame, selectmode=b'extended', columns=[
         b'index', b'parity', b'length'], show=b'headings')
        curves.heading(b'index', text=b'#')
        curves.column(b'index', stretch=False, width=20)
        curves.heading(b'parity', text=b'Parity')
        curves.column(b'parity', stretch=False, width=80)
        curves.heading(b'length', text=b'Length')
        curves.column(b'length', stretch=True, width=460)
        curves.bind(b'<Double-Button-1>', self.drill)
        self.curves.grid(row=2, column=0, padx=6, pady=6, sticky=Tk_.NSEW)
        self.show_curves()
        top_frame.pack(fill=Tk_.BOTH, expand=1)
        button_frame = ttk.Frame(self.root)
        button = ttk.Button(button_frame, text=b'Drill', command=self.drill, default=b'active')
        button.pack(side=Tk_.LEFT, padx=6)
        button = ttk.Button(button_frame, text=b'Cancel', command=self.cancel)
        button.pack(side=Tk_.LEFT, padx=6)
        button_frame.pack(pady=6)
        self.root.protocol(b'WM_DELETE_WINDOW', self.wm_delete_window)
    ...
    def show_curves(self):
        ...
        for curve in self.manifold.dual_curves(max_segments=self.max_segments):
            n = curve[b'index']
            parity = b'+' if curve[b'parity'] == 1 else b'-'
            length = Number(curve[b'filled_length'], precision=25)
            self.curves.insert(b'', b'end', values=(n, parity, length))
...
class Coverer(SimpleDialog):
    ...
    def __init__(self, master, manifold):
        ...
        msg_font = Font(family=style.font_info[b'family'], weight=b'bold', size=int(style.font_info[b'size'] * 1.2))
        msg = ttk.Label(top_frame, font=msg_font, text=b'Choose covering spaces to browse:')
        ...
        degree_var.trace(b'w', self.show_covers)
        ttk.Label(degree_frame, text=b'Degree: ').grid(row=0, column=0, sticky=Tk_.E)
        ...
        cyclic_var.trace(b'w', self.show_covers)
        cyclic_or_not = ttk.Checkbutton(degree_frame, variable=cyclic_var, text=b'cyclic covers only')
        cyclic_or_not.grid(row=0, column=2, padx=6, sticky=Tk_.W)
        self.action = action = ttk.Button(degree_frame, text=b'Recompute', command=self.show_covers)
        action.grid(row=0, column=3, padx=8, sticky=Tk_.W)
        degree_frame.grid(row=1, column=0, pady=2, padx=6, sticky=Tk_.EW)
        self.covers = covers = ttk.Treeview(top_frame, selectmode=b'extended', columns=[
         b'index', b'cover_type', b'num_cusps', b'homology'], show=b'headings')
        covers.heading(b'index', text=b'')
        covers.column(b'index', stretch=False, width=40, minwidth=40)
        covers.heading(b'cover_type', text=b'Type')
        covers.column(b'cover_type', stretch=False, width=100)
        covers.heading(b'num_cusps', text=b'# Cusps')
        covers.column(b'num_cusps', stretch=False, width=100, anchor=Tk_.CENTER)
        covers.heading(b'homology', text=b'Homology')
        covers.column(b'homology', stretch=True, width=300)
        covers.bind(b'<Double-Button-1>', self.choose)
        self.covers.grid(row=2, column=0, columnspan=2, padx=6, pady=6, sticky=Tk_.NSEW)
        top_frame.pack(fill=Tk_.BOTH, expand=1)
        button_frame = ttk.Frame(self.root)
        button_frame.grid_columnconfigure(0, weight=1)
        button_frame.grid_columnconfigure(1, weight=1)
        self.browse = ttk.Button(button_frame, text=b'Browse', command=self.choose, default=b'active')
        ...
        self.root.protocol(b'WM_DELETE_WINDOW', self.cancel)
        self._set_transient(master)
        degree_var.set(b'2')
        cyclic_var.set(True)
        self.show_covers()
    ...
    def clear_list(self, *args):
        ...
        self.browse.config(default=b'normal')
        self.action.config(default=b'active')
        self.state = b'not ready'

    def show_covers(self, *args):
        self.state = b'ready'
        self.browse.config(default=b'active')
        self.action.config(default=b'normal')
        self.covers.delete(*self.covers.get_children())
        degree = int(self.degree_var.get())
        if self.cyclic_var.get():
            self.cover_list = self.manifold.covers(degree, cover_type=b'cyclic')
        else:
        ...
        for n, N in enumerate(self.cover_list):
            ...
            name = N.name()
            cover_type = N.cover_info()[b'type']
            self.covers.insert(b'', b'end', values=(
             n, cover_type, cusps, homology))

    def handle_return(self, event):
        if self.state == b'ready':
            self.choose()
        else:
            self.show_covers()
...
if __name__ == b'__main__':
    ...
    root = Tk_.Tk()
    root.withdraw()
    browser1 = Browser(Manifold(b'm125'), root)
    browser2 = Browser(Manifold(b'12n345'))
