...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/horoviewer.py
# Compiled at: 2019-07-15 23:56:54
from builtins import range
from .CyOpenGL import HoroballScene, OpenGLOrthoWidget, GetColor, GL_context
import os, sys
if sys.version_info[0] < 3:
    import Tkinter as Tk_, ttk
else:
    import tkinter as Tk_, tkinter.ttk as ttk

class HoroballViewer():

    def __init__(self, nbhd, which_cusp=0, cutoff=None, root=None, title='Horoball Viewer', prefs={'cusp_horoballs': True, 'cusp_triangulation': True, 
   'cusp_ford_domain': True, 
   'cusp_labels': True, 
   'cusp_parallelogram': True, 
   'cusp_cutoff': '0.1000'}, container=None, bgcolor=None):
        if bgcolor == None:
        ...
        if container:
            ...
            window.withdraw()
            window.title(title)
        self.pgram_var = pgram_var = Tk_.IntVar(window, value=prefs['cusp_parallelogram'])
        self.Ford_var = Ford_var = Tk_.IntVar(window, value=prefs['cusp_ford_domain'])
        self.tri_var = tri_var = Tk_.IntVar(window, value=prefs['cusp_triangulation'])
        self.horo_var = horo_var = Tk_.IntVar(window, value=prefs['cusp_horoballs'])
        self.label_var = label_var = Tk_.IntVar(window, value=prefs['cusp_labels'])
        ...
        view_menu.add_checkbutton(label='parallelogram', command=self.view_check, variable=self.pgram_var)
        view_menu.add_checkbutton(label='Ford edges', command=self.view_check, variable=self.Ford_var)
        view_menu.add_checkbutton(label='triangulation', command=self.view_check, variable=self.tri_var)
        view_menu.add_checkbutton(label='horoballs', command=self.view_check, variable=self.horo_var)
        view_menu.add_checkbutton(label='labels', command=self.view_check, variable=self.label_var)
        view_button.config(menu=view_menu)
        view_button.grid(row=0, column=0, columnspan=2, sticky=Tk_.W, padx=0, pady=0)
        flip_button = ttk.Checkbutton(option_frame, text='Flip', variable=self.flip_var, takefocus=False, command=self.flip)
        flip_button.grid(row=1, column=0, sticky=Tk_.W, padx=0, pady=0)
        self.cutoff_label = ttk.Label(option_frame, text='Cutoff: ')
        self.cutoff_var = cutoff_var = Tk_.StringVar(window, value='%.4f' % self.cutoff)
        self.cutoff_entry = ttk.Entry(option_frame, width=6, takefocus=False, textvariable=cutoff_var)
        ...
        ttk.Label(slider_frame, text='Cusp Position').grid(row=0, column=2, pady=0)
        ttk.Label(slider_frame, text='Volume').grid(row=0, column=3, pady=0, padx=0, sticky=Tk_.W)
        ...
        self.movie_id = 0
        self.build_menus()
        self.scene = HoroballScene(nbhd, pgram_var, Ford_var, tri_var, horo_var, label_var, flipped=self.flip_var.get(), cutoff=self.cutoff, which_cusp=self.which_cusp)
    ...
    def build_sliders(self):
        ...
        if nbhd is None:
            ...
            for n in range(num_cusps):
                ...
                slider = Tk_.Scale(self.slider_frames[n], showvalue=0, from_=0, to=100, width=11, length=200, orient=Tk_.HORIZONTAL, background=self.cusp_colors[n], troughcolor=self.bgcolor, borderwidth=1, relief=Tk_.FLAT, variable=Tk_.DoubleVar(self.window))
                slider.index = n
                slider.stamp = 0
    ...
    def new_scene(self, new_nbhd):
        ...
        self.build_sliders()
        self.scene.destroy()
        self.scene = HoroballScene(new_nbhd, self.pgram_var, self.Ford_var, self.tri_var, self.horo_var, self.label_var, flipped=self.flip_var.get(), cutoff=self.cutoff, which_cusp=self.which_cusp)
    ...
    def translate(self, event):
        ...
        self.scene.translate(X + Y * complex(0.0, 1.0))
        self.widget.tkTranslate(event)

    def add_help(self):
        help = Button(self.top_frame, text='Help', width=4, borderwidth=0, highlightthickness=0, background=self.bgcolor, command=self.widget.help)
