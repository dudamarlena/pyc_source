...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/snap/polished_reps.py
...
class MapToFreeAbelianization(Object):
    ...
    def __init__(self, fund_group):
        ...
        U, V, D = presentation.matsnf(flag=1)
        self.U = U
...
def clean_matrix(A, error, prec):
    return matrix([ [ clean_CC(A[x], error, prec) for x in ((i, 0), (i, 1)) ] for i in (0,
                                                                                        1)
                  ])
...
def is_essentially_Id2(M, error=0.001):
...
def polished_holonomy(manifold, bits_prec=100, fundamental_group_args=[], lift_to_SL2=True, ignore_solution_type=False, dec_prec=None):
    ...
    f = FundamentalPolyhedronEngine.fromManifoldAndShapesMatchingSnapPea(M, shapes, normalize_matrices=True)
