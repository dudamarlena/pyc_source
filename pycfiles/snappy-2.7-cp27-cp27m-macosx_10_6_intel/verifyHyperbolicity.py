...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/verify/verifyHyperbolicity.py
...
def check_logarithmic_gluing_equations_and_positively_oriented_tets(manifold, shape_intervals):
    ...
    LHSs = [ sum([ l * expo for l, expo in zip(equation, logs) ]) for equation in equations
           ]
    BaseField = shape_intervals[0].parent()
    TWO_PI_I = BaseField.pi() * BaseField(complex(0.0, 2.0))
    LHS_index = 0
    ...
    for cusp_index in range(n_cusps):
        num_LHSs, value = (2, 0) if manifold.cusp_info(cusp_index)['complete?'] else (
         1, TWO_PI_I)
...
@sage_method
def verify_hyperbolicity(manifold, verbose=False, bits_prec=53, holonomy=False, fundamental_group_args=[], lift_to_SL=True):
    """
    ...
    """
    try:
        shape_intervals = manifold.tetrahedra_shapes('rect', bits_prec=bits_prec, intervals=True)
    ...
    try:
        check_logarithmic_gluing_equations_and_positively_oriented_tets(manifold, shape_intervals)
    ...
    if holonomy:
        hol_rep = snap.interval_reps.holonomy_from_shape_intervals(manifold, shape_intervals, fundamental_group_args, lift_to_SL)
        hol_rep.shapes = shape_intervals
        return (
         True, hol_rep)
    else:
        return (
         True, shape_intervals)