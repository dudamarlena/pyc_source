...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/polynomial.py
...
class Monomial(object):
    ...
    def __str__(self):
        return self.to_string(print_coefficient_method=lambda x: default_print_coefficient_method(x), force_print_sign=False)
    ...
    def to_string(self, print_coefficient_method, force_print_sign):
        ...
        coefficient_sign, coefficient_str = print_coefficient_method(self._coefficient)
        if coefficient_str:
            v = [coefficient_str] + v
        if not v:
            v = ['1']
    ...
    def degree(self, var=None):
        ...
        return sum([ this_degree for this_var, this_degree in self._vars if var is None or this_var == var
                   ])

    def __mul__(self, other):
        """Multiply two monomials."""
        assert isinstance(other, Monomial)
        coefficient = _operator_type_policy(self._coefficient, other._coefficient, operator.mul)
        var_dict = _combine_dicts([
         dict(self._vars), dict(other._vars)], operator.add)
    ...
    def __eq__(self, other):
        ...
        assert isinstance(other, Monomial)
        return self._coefficient == other._coefficient and self._vars == other._vars
    ...
    def split_variable(self, variable):
        ...
        return (
         exponent, Monomial(self.get_coefficient(), remaining_terms))
...
class Polynomial(object):
    ...
    def __init__(self, monomials=()):
        ...
        list_of_vars_coeff_dicts = [ {monomial.get_vars(): monomial.get_coefficient()} for monomial in monomials
                                   ]
        combined_vars_coeff_dict = _combine_dicts(list_of_vars_coeff_dicts, _operator_type_policy)
        ordered_tuple_of_vars_coeff_pairs = _dict_to_ordered_tuple_of_pairs(combined_vars_coeff_dict)
        combined_monomials = [ Monomial(coefficient, vars) for vars, coefficient in ordered_tuple_of_vars_coeff_pairs if _coefficient_is_non_trivial(coefficient)
                             ]
    ...
    def __neg__(self):
        return Polynomial(tuple([ -monomial for monomial in self._monomials ]))
    ...
    def __mod__(self, other):
        ...
        other = other * Polynomial.constant_polynomial(Fraction(1, 1) / other.leading_coefficient())
        variable = other.variables()[0]
        assert not other.variables() or other.variables()[0] == variable
        rest = self
        while rest.degree() >= other.degree():
            degree_diff = rest.degree() - other.degree()
            leading_coeff = rest.leading_coefficient()
            rest = rest - Polynomial.constant_polynomial(leading_coeff) * Polynomial.from_variable_name(variable) ** degree_diff * other
    ...
    def to_string(self, print_coefficient_method):
        s = (' ').join([ monomial.to_string(print_coefficient_method, force_print_sign=True) for monomial in self._monomials
                       ])
        if s and s[0] == '+':
            return s[1:].lstrip()
        return s
    ...
    def substitute(self, d):
        ...
        def substitute_monomial(monomial):
            ...
            poly = Polynomial((
             Monomial(monomial.get_coefficient(), tuple(new_vars)),))
            for var, expo in vars:
                if var in d:
        ...
        return sum([ substitute_monomial(monomial) for monomial in self._monomials
                   ], Polynomial(()))
    ...
    def get_constant(self):
        ...
        constants = [ monomial.get_coefficient() for monomial in self._monomials if not monomial.get_vars()
                    ]
    ...
    def degree(self, var=None):
        ...
        return max([ monomial.degree(var) for monomial in self._monomials ] + [0])

    @classmethod
    def parse_string(cls, s, parse_coefficient_function=lambda x: parse_int_or_fraction(x)):
        """
    ...
    def curried_polynomial(self, variable):
        ...
        for monomial in self._monomials:
            ...
            poly = poly + (Polynomial.from_variable_name(variable) ** exponent).convert_coefficients(lambda x: Polynomial.constant_polynomial(x)) * Polynomial.constant_polynomial(Polynomial((remainder,)))
...
def _storage_type_policy(type_a, type_b):
    if not isinstance(type_a, type):
        ...
        if type_b in [int, long]:
            return type_a
        type_a == type_b or print(type_a, type_b)
...
def _parse_polynomial_from_string(s, parse_coefficient_function):
    ...
    if not not operator_stack:
        raise AssertionError
        return operand_stack or Polynomial(())
    else:
