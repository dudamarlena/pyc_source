...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/verify/interval_newton_shapes_engine.py
...
class IntervalNewtonShapesEngine:
    ...
    def mat_solve(m, v):
        ...
        for i in range(len(v)):
            max_index, max_val = max(enumerate(m1.column(i)[i:]), key=lambda x: x[1].abs().lower())
    ...
    def newton_iteration(equations, shape_intervals, point_in_intervals=None, interval_value_at_point=None):
        ...
        return point_in_intervals - IntervalNewtonShapesEngine.mat_solve(derivatives, interval_value_at_point)
    ...
    def certified_newton_iteration(equations, shape_intervals, point_in_intervals=None, interval_value_at_point=None):
        ...
        return (
         IntervalNewtonShapesEngine.interval_vector_is_contained_in(new_shapes, shape_intervals),
         new_shapes)
    ...
    def __init__(self, M, initial_shapes, bits_prec=None, dec_prec=None):
        ...
        self.initial_shapes = vector([ self.CIF(shape) for shape in initial_shapes ])
    ...
    def expand_until_certified(self, verbose=False):
        ...
        """
        interval_value_at_initial_shapes = IntervalNewtonShapesEngine.log_gluing_LHSs(self.equations, self.initial_shapes)
        shapes = self.initial_shapes
        num_iterations = 25 if self.prec > 53 else 11
        for i in range(num_iterations + 1):
            old_shapes = shapes
            try:
                is_certified, shapes = IntervalNewtonShapesEngine.certified_newton_iteration(self.equations, shapes, point_in_intervals=self.initial_shapes, interval_value_at_point=interval_value_at_initial_shapes)
            ...
            if is_certified:
                ...
                return True
            shapes = IntervalNewtonShapesEngine.interval_vector_union(shapes, old_shapes)
