...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/number.py
...
class Number(Number_baseclass):
    ...
    def __init__(self, data, accuracy=None, precision=None):
        ...
        if type not in ('t_INT', 't_FRAC', 't_REAL', 't_COMPLEX'):
            raise ValueError('Invalid initialization for a Number: %s has type %s!' % (self.gen, type))
    ...
    def _get_acc_prec(self, other):
        if is_exact(other):
            ...
            return (
             accuracy, min(self._precision, other_precision))
            return
    ...
    def as_string(self, full_precision=True):
        ...
        gen = self.gen
        if gen.imag() == 0:
            return self._real_string(gen.real(), self.accuracy, full_precision)
        else:
            real_part = self._real_string(gen.real(), self.accuracy, full_precision)
            imag_part = self._real_string(gen.imag(), self.accuracy, full_precision)
    ...
    def __reduce__(self):
        return (
         Number, (self.as_string(), self.accuracy, self._precision))
    ...
    def volume(self):
        ...
        if z == 0 or z == 1:
            ...
            volume = ((A * A).dilog(precision=bits).imag() + (B * B).dilog(precision=bits).imag() + (C * C).dilog(precision=bits).imag()) / 2
