...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/snap/nsagetools.py
...
if _within_sage:
    import sage
    from sage.all import ZZ, vector, matrix, block_matrix, gcd, prod, det, MatrixSpace, AbelianGroup, GroupAlgebra, SageObject, PolynomialRing, LaurentPolynomialRing
...
def search_for_low_rank_triangulation(M, trys=100, target_lower_bound=0):
    ...
    return (
     rank_upper_bound, rank_lower_bound, curr_best_tri)
...
def setup_fox_derivative(word, phi, var, involute=False):
    ...
    fox_ders = {}
    for g in gens:
        phi_ims[g] = (involute or phi)(g) if 1 else phi(g.swapcase())
        if g == g.lower():
...
def clean_laurent_to_poly(p, error=1e-08):
    R = p.parent()
    P = PolynomialRing(R.base_ring(), R.variable_names())
    t = P.gen()
...
def fast_determinant_of_laurent_poly_matrix(A):
    ...
    """
    R = A.base_ring()
    if not (hasattr(R, 'polynomial_ring') or False):
...
def compute_torsion(G, bits_prec, alpha=None, phi=None, phialpha=None, return_parts=False, return_as_poly=True, wada_conventions=False, symmetry_test=True):
    ...
    epsilon = (F.is_exact() or ZZ(2)) ** (-bits_prec // 3) if 1 else None
    big_epsilon = (F.is_exact() or ZZ(2)) ** (-bits_prec // 5) if 1 else None
    gens, rels = G.generators(), G.relators()
    k = len(gens)
    if phi == None:
        phi = MapToGroupRingOfFreeAbelianization(G, F)
    if not (len(rels) == len(gens) - 1 and len(phi.range().gens()) == 1):
        raise AssertionError
        ...
        if not wada_conventions:
            ...
            dsquared = d2 * d1
        raise (matrix_has_small_entries(dsquared, epsilon) or TorsionComputationError)('(boundary)^2 != 0')
