...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/matrix.py
...
def matrix_mult(m, n):
    ...
    return [ [ compute_entry(i, j) for j in range(num_cols_n) ] for i in range(num_rows_m)
           ]
...
def _get_only_non_zero_entry_in_col(m, col):
    entry = None
    for row in m:
        if not (row[col] == 0 or entry is None):
            raise AssertionError('more than one non-zero entry in column %d' % col)
            entry = row[col]
...
def _get_only_non_zero_entry_in_row(m, row):
    entry = None
    for i in m[row]:
        if not (i == 0 or entry is None):
            raise AssertionError('more than one non-zero entry in row %d' % row)
            entry = i
...
def _split_matrix_bottom_zero_rows(m):
    ...
    return (
     m[:number_top_rows], m[number_top_rows:])
...
def _bottom_row_stable_smith_normal_form(m):
    ...
    return (
     _expand_square_matrix(u_upleft, len(m_down)),
     v,
     d_up + m_down)


def _change_coordinates(u, v, m):
    return matrix_mult(matrix_mult(matrix_inverse(u), m), v)
