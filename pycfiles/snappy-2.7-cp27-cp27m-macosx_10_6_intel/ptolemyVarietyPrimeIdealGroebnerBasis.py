...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/ptolemyVarietyPrimeIdealGroebnerBasis.py
...
class PtolemyVarietyPrimeIdealGroebnerBasis:
    ...
    def __init__(self, polys, term_order, size, dimension, is_prime, free_variables, py_eval, manifold_thunk=lambda : None, witnesses=[]):
        self.polys = polys
    ...
    def _extensions_and_assignments(self):
        if not self._is_zero_dim_prime_and_lex():
            raise Exception('Need Groebner basis in lexicographic order of a zero-dimensional and prime ideal for finding solutions.')
        if not self._extensions_and_assignments_cache:
            self._extensions_and_assignments_cache = solutionsToPrimeIdealGroebnerBasis.extensions_and_assignments(self.polys)
    ...
    def solutions(self, numerical=False):
        if self.dimension > 0:
            return NonZeroDimensionalComponent([ witness.solutions(numerical=numerical) for witness in self.witnesses
                                               ], dimension=self.dimension, free_variables=self.free_variables)
        else:
            if numerical:
