...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/database.py
...
class ManifoldTable(object):
    ...
    def __init__(self, table='', db_path=None, mfld_hash=mfld_hash, **filter_args):
        self._table = table
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            ...
            if is_int_or_none(start) and is_int_or_none(stop):
                if start is None:
                    start = 0
                elif start < 0:
            ...
            raise IndexError('Use two ints or two floats for start and stop.')
        elif is_int(index):
            if index < 0:
            ...
            raise IndexError('%s is not a valid index type for manifolds.' % type(index))
        return matches[0]
    ...
    def siblings(self, mfld):
        ...
        initial_candidates = self.find("volume between %f and %f and cusps=%d and betti=%d and torsion='%s'" % (
    ...
    def identify(self, mfld, extends_to_link=False):
        ...
        if hasattr(mfld, 'volume'):
            bad_types = [
             'no solution found', 'not attempted']
            if mfld.solution_type() in bad_types:
                return False
            if mfld.volume() > self._max_volume + 0.1:
                return False
        if extends_to_link and True not in mfld.cusp_info('complete?'):
            return False
