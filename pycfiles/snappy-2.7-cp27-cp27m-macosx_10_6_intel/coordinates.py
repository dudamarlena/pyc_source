...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/coordinates.py
...
class PtolemyCoordinates(dict):
    ...
    def _init_matrix_and_inverse_cache(self):
        if self._matrix_cache and self._inverse_matrix_cache:
            return
        self._matrix_cache, self._inverse_matrix_cache = findLoops.images_of_original_generators(self, penalties=(0,
                                                                                                                  1,
                                                                                                                  1))
        ...
        * epsilon --- maximal allowed error when checking the relations, use
        ...
        for tet in range(num_tets):
            for index in utilities.quadruples_with_fixed_sum_iterator(N - 2):
                ...
                rel = s0 * s1 * get_ptolemy_coordinate((1, 1, 0, 0)) * get_ptolemy_coordinate((0,
                                                                                               0,
                                                                                               1,
                                                                                               1)) - s0 * s2 * get_ptolemy_coordinate((1,
                                                                                                                                       0,
                                                                                                                                       1,
                                                                                                                                       0)) * get_ptolemy_coordinate((0,
                                                                                                                                                                     1,
                                                                                                                                                                     0,
                                                                                                                                                                     1)) + s0 * s3 * get_ptolemy_coordinate((1,
                                                                                                                                                                                                             0,
                                                                                                                                                                                                             0,
                                                                                                                                                                                                             1)) * get_ptolemy_coordinate((0,
                                                                                                                                                                                                                                           1,
                                                                                                                                                                                                                                           1,
                                                                                                                                                                                                                                           0))
...
class Flattenings(dict):
    ...
    def from_tetrahedra_shapes_of_manifold(cls, M):
        ...
        Mcopy = M.copy()
        return Flattenings(dict([ (k, (log + PiI * p, z, p)) for k, log, z, p in zip(keys, log_all_cross_ratios, all_cross_ratios, flattenings)
...
class CrossRatios(dict):
    ...
    def induced_representation(self, N):
        ...
        d = dict([ key_value_pair(v, t, index) for v in [
         'z', 'zp', 'zpp'] for t in range(num_tetrahedra) for index in utilities.quadruples_with_fixed_sum_iterator(N - 2)
                 ])
...
def _ptolemy_to_cross_ratio(solution_dict, branch_factor=1, non_trivial_generalized_obstruction_class=False, as_flattenings=False):
    ...
    return (
     dict(sum([ compute_cross_ratios_and_flattenings(tet, index) for tet in range(num_tets) for index in utilities.quadruples_with_fixed_sum_iterator(N - 2)
         ], [])), evenN)


def _num_tetrahedra(solution_dict):
    return max([ int(key.split('_')[(-1)]) for key in solution_dict.keys()
               ]) + 1
