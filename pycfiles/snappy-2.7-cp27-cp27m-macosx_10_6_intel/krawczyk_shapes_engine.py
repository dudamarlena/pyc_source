...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/verify/krawczyk_shapes_engine.py
...
class KrawczykShapesEngine:
    ...
    def __init__(self, M, initial_shapes, bits_prec=None, dec_prec=None):
        ...
        approx_deriv = self.log_gluing_LHS_derivatives([ CDF(shape) for shape in initial_shapes ])
        ...
        self.first_term = self.initial_shapes - self.approx_inverse * value_at_initial_shapes
    ...
    def expand_until_certified(self, verbose=False):
        ...
        for i in range(num_iterations + 1):
            ...
            if KrawczykShapesEngine.interval_vector_is_contained_in(shapes, old_shapes):
                if verbose:
                    print 'Certified shapes after %d iterations' % (i + 1)
                self.certified_shapes = shapes
                return True
            shapes = KrawczykShapesEngine.interval_vector_union(shapes, old_shapes)
            if i == 0:
                shapes = KrawczykShapesEngine._expand_intervals_a_little(shapes)
