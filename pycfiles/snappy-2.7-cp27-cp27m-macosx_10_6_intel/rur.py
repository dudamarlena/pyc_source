...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/ptolemy/rur.py
# Compiled at: 2017-05-26 08:27:22
import operator, sys, functools
...
class RUR(object):
    ...
    def __init__(self, polymod_exponent_pairs):
        ...
        def add_to_list_of_pairs(polymod, exponent):
            ...
            for i, (p, e) in enumerate(reduced_polymod_exponent_pairs):
                if p - polymod == 0:
                    reduced_polymod_exponent_pairs[i] = (
                     p, e + exponent)
                    return
        ...
        reduced_polymod_exponent_pairs = [ (p, e % 2) if p + 1 == 0 else (p, e) for p, e in reduced_polymod_exponent_pairs
                                         ]
        self._polymod_exponent_pairs = [ (p, e) for p, e in reduced_polymod_exponent_pairs if not e == 0
                                       ]
    ...
    def __repr__(self):
        ...
        numerator = (' * ').join([ process_pair(pair) for pair in self._numerator_terms() ])
        denominator = (' * ').join([ process_pair(pair) for pair in self._denominator_terms() ])
        if not numerator:
            numerator = 1
        else:
    ...
    def evaluate_at_root(self, root):
        ...
        return _prod([ evaluate_poly(p) ** e for p, e in self._polymod_exponent_pairs ])
    ...
    def to_pari_fraction(self):
        ...
        """
        return pari(self._numerator()).lift() / pari(self._denominator()).lift()
    ...
    def __add__(self, other):
        ...
        new_numerator = self._numerator() * self_expand + other._numerator() * other_expand
    ...
    def __mul__(self, other):
        if isinstance(other, int):
            return self * RUR.from_int(other)
        return RUR(self._polymod_exponent_pairs + other._polymod_exponent_pairs)
