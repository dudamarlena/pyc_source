...
# [GCC 8.4.0]
# Embedded file name: /Users/dunfield/snappy/build/lib.macosx-10.6-intel-2.7/snappy/verify/exceptions.py
...
class CuspEquationLogLiftNumericalVerifyError(LogLiftNumericalVerifyError, CuspEquationType):
    """
    ...
    def __str__(self):
        return 'Numerical verification that logarthmic cusp equation has small error failed: %r == %r' % (
...
class CuspDevelopmentExactVerifyError(ExactVerifyError, CuspDevelopmentType):
    ...
    def __str__(self):
        return 'Inconsistency in the side lengths of the Euclidean Horotriangles for a cusp: %r = %r' % (
...
class ConsistencyWithSnapPeaNumericalVerifyError(NumericalVerifyError, ConsistencyWithSnapPeaType):
    ...
    def __str__(self):
        return 'Inconsistency between SnapPea kernel and verify: %r == %r' % (
         self.snappea_value, self.value)