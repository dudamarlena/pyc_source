...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nlrob/nlrob.py
# Compiled at: 2019-01-10 09:27:19
# Size of source mod 2**32: 40658 bytes
from .utils import *
...
class NlrobControl:
    ...
    def __init__(self, method, psi='bisquare', init='S', optimizer='JDEoptim', fnscale=None, tuning_chi_tau=None, tuning_chi_scale=None, tuning_chi=None, cutoff=2.5, *args, **kwargs):
...
def nlrob(formula, data, start=np.zeros(1), lower=np.array([-np.Inf]), upper=np.array([np.Inf]), weights=None, method='MM', psi=None, scale=None, control=None, test_vec='resid', maxit=20, tol=1e-06, algorithm='lm', doCov=False, trace=False):
    """
    ...
    if method != 'M':
        ...
        def fixAns(mod):
            ctrl = mod.get('ctrl')
            if isinstance(ctrl.psi, string_types):
                if isinstance(ctrl.tuning_psi_M, (int, float)):
        ...
        if method == 'MM':
            return fixAns(nlrob_MM(formula, data, lower=lower, upper=upper, tol=tol,
              ctrl=control))
        if method == 'CM':
            return fixAns(nlrob_CM(formula, data, lower=lower, upper=upper, tol=tol,
              ctrl=control))
        if method == 'tau':
            return fixAns(nlrob_tau(formula, data, lower=lower, upper=upper, tol=tol,
              ctrl=control))
        if method == 'mtl':
            return fixAns(nlrob_CM(formula, data, lower=lower, upper=upper, tol=tol,
              ctrl=control))
        ...
        if hasWgts:
            if np.any(weights < 0 or np.isnan(weights).any()):
        ...
        for iiter in range(maxit):
            ...
            if Scale == 0:
                ...
                out = nls(formula=formula, data=data, start=start, algorithm=algorithm, lower=lower,
                  upper=upper)
...
def nlrob_MM(formula, data, lower, upper, tol=1e-06, psi='bisquare', init='S', ctrl=NlrobControl('MM')):
    """
                    ...
                    if psi == 'hampel':
                        ...
                        def rho_inv(y):
                            ...
                            if a / C > y:
                                ...
                                if (2 * b - a) / C > y:
                                    return 0.5 * a + C / a * y
                                return r + np.sqrt(r ** 2 - ((r - b) * (2 * C / a * y + (b - a)) - b * r))
        ...
        if psi == 'bisquare':
            ...
            if psi == 'lqq':
                ...
                if psi == 'optimal':
                    ...
                    initial = JDEoptim((lower.values[0]), upper, objective_initial, tol=tol, fnscale=fnscale)
                    ...
                    M = minimize(fminfn, (initial.get('par')), jac=fmingr, args=sigma, method='L-BFGS-B', bounds=bounds,
                      tol=tol)
    ...
    vector = M.x
    fit = eval(right_hand_term)
    dictReturn = {'formula':formula,  'nobs':nobs, 
     'coefficients':coef, 
     'fitted_values':fit, 
     'residuals':y - fit, 
     'ctrl':ctrl, 
...
def nlrob_tau(formula, data, lower, upper, tol=1e-06, psi='bisquare', ctrl=NlrobControl('tau'), tuning_chi_scale=None, tuning_chi_tau=None):
    """
    ...
    if ctrl:
        ...
        rho2 = lambda t: Mchi(t, c2, psi)
        if psi == 'bisquare':
            rho_inv = lambda y: c1 * np.sqrt(1 - (1 - y) ** 0.3333333333333333)
        elif psi == 'optimal':
            rho_inv = lambda y: np.sqrt(y / 1.38) * c1 * 3
        else:
            ...
            def objective(vector):
                ...
                res = y - fit
                med_abs_res = np.median(np.abs(res))
                sigma = np.array(brentq(M_scale, (constant[0] * med_abs_res), (constant[1] * med_abs_res),
                  args=res))
        ...
        if npar > upper.size:
            upper = np.repeat(upper, npar)
    optRes = JDEoptim((lower.values[0]), upper, objective, tol=tol, fnscale=fnscale)
    it = optRes.get('iter')
...
def nlrob_CM(formula, data, lower, upper, tol=1e-06, psi='bisquare', ctrl=NlrobControl('CM')):
    """
    Compute an MM-estimator for nonlinear robust (constrained) regression.
    Returns a dictionary with all the variables of interest
    Parameters
...
def nlrob_mtl(formula, data, lower, upper, tol=1e-06, psi='bisquare', ctrl=NlrobControl('mtl')):
    """
...
def nls(formula, data, start, algorithm='lm', weights=None, lower=np.array([-np.Inf]), upper=np.array([np.Inf])):
    """
    ...
    func = get_func()
    par, cov = eval('curve_fit(func, %s, y, p0, %s)' % (x, other_params), globals(), locals())
    popt_str = ['par[%d]' % i for i, p in enumerate(par)]
    fit = eval('func(%s, %s)' % (x, ', '.join(popt_str)))
    res = y - fit
    dictReturn = {'coefficients':dict(zip(start.keys(), par)),  'residuals':res, 
     'cov':cov}
    return dictReturn