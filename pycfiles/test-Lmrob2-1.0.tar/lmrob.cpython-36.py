...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lmrob/lmrob.py
# Compiled at: 2019-01-10 09:27:19
# Size of source mod 2**32: 72474 bytes
from .utils import *
...
class LmrobControl:
    ...
    def __init__(self, setting=None, seed=None, nResample=500, tuning_chi=None, bb=0.5, tuning_psi=None, max_it=50, groups=5, n_group=400, k_fast_s=1, best_r_s=2, k_max=200, maxit_scale=200, k_m_s=20, refine_tol=1e-07, rel_tol=1e-07, scale_tol=1e-10, solve_tol=1e-07, trace_lev=0, mts=1000, subsampling='nonsingular', compute_rd=False, method='MM', psi=None, numpoints=10, cov=None, split_type='f', fast_s_large_n=2000, eps_outlier=None, eps_x=None, compute_outlier_stats=None, warn_limit_reject=0.5, warn_limit_meanrw=0.5):
        _Mchi_tuning_defaults = {'bisquare':np.array([1.54764]), 
        ...
        _Mpsi_tuning_defaults = {'huber':np.array([1.345]), 
         'bisquare':np.array([4.685061]), 
         'welsh':np.array([2.11]), 
         'ggw':np.array([-0.5, 1.5, 0.95, np.nan]), 
         'lqq':np.array([-0.5, 1.5, 0.95, np.nan]), 
...
def lmrob(formula='', data={}, subset=None, weights=np.array([]), na_action='drop', method='', model=True, return_x=True, return_y=True, singular_ok=True, contrasts={}, offset=np.array([]), control=None, init={}, **kwargs):
    """
    ...
    if control == None:
        ...
        if len(offset) != 0:
            ...
            z = {'coefficients':np.zeros((0, y.shape[1])) * np.nan if not np.isscalar(y) else np.array([]), 
             ...
             'weights':w, 
             'rank':0, 
             'df_residuals':np.sum(w[(w != 0)]) if w is not None else y.shape[0], 
             'converged':True, 
             'iter':0}
            ...
            if rankQR > 0:
                if singular_fit:
                    ...
                    x = x[:, p1]
                    mt['assign'] = assign[p1]
                elif inspect.isfunction(control.eps_x):
                    control.eps_x = lambda x: np.max(np.abs(x))
                else:
                    ini = init
                    if init:
                        if init == 'M-S':
                            ...
                            if ini != 'S':
                                if control.cov == '_vcov_avar1':
                                    control.cov = '_vcov_w'
                    z = lmrob_fit(x, y, control, init=init)
                    if ini.__class__ == str:
                        if not ini == control.method[:len(ini)]:
                ...
                z = {'coefficients':np.zeros([p, y.shape[1]]) * np.nan if y.shape[0] > 1 and y.shape[1] > 1 else np.zeros(p) * np.nan,  'residuals':y, 
                 'scale':np.nan, 
                 'fitted_values':0.0 * y, 
                 'cov':np.zeros([0, 0]) * np.nan, 
...
def lm_fit(x, y, tol_=1e-07):
    ...
    dqrls.argtypes = [ctypes.POINTER(ctypes.c_double),
     ...
     ctypes.POINTER(ctypes.c_double),
     ctypes.POINTER(ctypes.c_double)]
    dqrls(x_p, n_p, p_p, y_p, ny_p, tol_p, coefficients_p, residuals_p, effects_p, rank_p, pivot_p, qraux_p, work_p)
    pivoted = 0
    resultsDict = {'coefficients':coefficients, 
     'residuals':residuals, 
     'rank':rank, 
     'effects':effects, 
     'pivot':pivot, 
...
def lmrob_S(x_, y_, control, only_scale=False):
    ...
    R_lmrob_S.argtypes = [ctypes.POINTER(ctypes.c_double),
     ...
     ctypes.POINTER(ctypes.c_int),
     ctypes.POINTER(ctypes.c_int)]
    R_lmrob_S(xp, yp, ctypes.byref(n), ctypes.byref(p), ctypes.byref(nResample), scale_p, coefficients_p, c_chi_p, ctypes.byref(psi2ipsi), ctypes.byref(bb), ctypes.byref(best_r), ctypes.byref(groups), ctypes.byref(n_group), ctypes.byref(k_fast_s), ctypes.byref(k_iter), ctypes.byref(maxit_scale), ctypes.byref(refine_tol), ctypes.byref(inv_tol), ctypes.byref(scale_tol), converged_p, ctypes.byref(trace_lev), ctypes.byref(mts), ctypes.byref(ss), ctypes.byref(fast_s_large_n))
    if scale[0] < 0.0:
    ...
    if control.method == 'MM':
        ...
        if init:
            ...
            if init['control'] is None:
                ...
                if not len(M) or not M.__class__ == str:
                    est = 'l'
                else:
                    M1 = control.method[0]
                    ...
                    init = lmrob_S(x, y, control)
                    est = 'S'
                if 'float' not in init['coefficients'].dtype.name or not len(init['coefficients']) == x.shape[1] or 'float' not in init['scale'].dtype.name or not init['scale'] >= 0:
...
def lmrob__D__fit(obj, x=np.array([]), control=None, method=None):
    ...
    R_find_D_scale.argtypes = [
     ...
     ctypes.POINTER(ctypes.c_int),
     ctypes.POINTER(ctypes.c_int)]
    R_find_D_scale(r_p, kappa_p, tau_p, ctypes.byref(length), scale_1, c_p, ctypes.byref(ipsi), ctypes.byref(Type), ctypes.byref(rel_tol), ctypes.byref(k_max), ctypes.byref(converged))
...
def lmrob__M__fit(x=np.array([]), y=np.array([]), beta_initial=np.array([]), scale=None, control=None, method=None, obj=None):
    """
    ...
    R_lmrob_MM.argtypes = [
     ...
     ctypes.POINTER(ctypes.c_int),
     ctypes.POINTER(ctypes.c_int)]
    R_lmrob_MM(x_p, y_p, ctypes.byref(n), ctypes.byref(p), beta_initial_p, scale_p, coefficients_p, residuals_p, ctypes.byref(iterations), c_psi_p, ctypes.byref(ipsi), ctypes.byref(loss), ctypes.byref(rel_tol), ctypes.byref(converged), ctypes.byref(trace_lev), ctypes.byref(mts), ctypes.byref(ss))
    objM.update({'coefficients':coefficients,  'scale':scale_, 
     'residuals':residuals, 
     'loss':loss.value, 
    ...
    objM.update({'fitted_values':fitted_values,  'rweights':rweights.reshape((len(rweights), 1))})
    objM.update({'control': controlM})
    if not method_[(-1)] == 'M':
        method_ = method_ + 'M'
    qr = np.linalg.qr(x * np.sqrt(rweights).reshape(x.shape[0], 1))
    rank = np.linalg.matrix_rank(qr[1])
    if objM.get('cov') is not None:
        if method_ not in ('SM', 'MM'):
            if objM.get('control').cov == _vcov_avar1:
...
def lmrob_M_S(x, y, control, mf):
    ...
    if split.get('x1').shape[1] == 0:
        ...
        R_lmrob_M_S.argtypes = [ctypes.POINTER(ctypes.c_double),
         ...
         ctypes.POINTER(ctypes.c_int),
         ctypes.POINTER(ctypes.c_int)]
        R_lmrob_M_S(x1_p, x2_p, y_p, residuals_p, ctypes.byref(n), ctypes.byref(p1), ctypes.byref(p2), ctypes.byref(nResample), ctypes.byref(maxit_scale), scale_p, coefficients1_p, coefficients2_p, c_chi_p, ctypes.byref(ipsi), ctypes.byref(bb), ctypes.byref(k_m_s), ctypes.byref(max_k), ctypes.byref(rel_tol), ctypes.byref(inv_tol), ctypes.byref(scale_tol), converged_p, ctypes.byref(trace_lev), orthogonalize_p, subsample_p, descent_p, ctypes.byref(mts), ctypes.byref(ss))
...
def lmrob_lar(x, y, control=None):
    ...
    rllarsbi.argtypes = [ctypes.POINTER(ctypes.c_double),
     ...
     ctypes.POINTER(ctypes.c_double),
     ctypes.POINTER(ctypes.c_double)]
    rllarsbi(x_p, y_p, ctypes.byref(n), ctypes.byref(p), ctypes.byref(MDX), ctypes.byref(MDT), ctypes.byref(rel_tol), NIT_p, K_p, KODE_p, SIGMA_p, THETA_p, RS_p, SC1_p, SC2_p, SC3_p, SC4_p, ctypes.byref(bet0))
