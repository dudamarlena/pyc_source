...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lmrob/situation.py
...
def assert_python_version():
    if not (sys.version_info[0] >= 3 and sys.version_info[1] >= 3):
        raise RuntimeError('Python >=3.3 is required to run rpy2')
...
def r_home_from_registry():
    ...
    try:
        hkey = winreg.OpenKeyEx(winreg.HKEY_LOCAL_MACHINE, 'Software\\R-core\\R', 0, winreg.KEY_QUERY_VALUE)
