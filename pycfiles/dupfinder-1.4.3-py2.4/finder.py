...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dupfinder/finder.py
# Compiled at: 2010-01-12 03:33:36
"""Duplication finder utility.
Find duplication files in passed director[y/es]

%prog [options] <find_dir_1> [<find_dir_2> <find_dir_3>]
"""
import os, sys, time, csv, copy, operator, zlib
...
class DupFinder(object):
    ...
    def find(self):
        ...
        logger.info('There are %d files and directories in %s' % (len(dirsdata), self.dirs))
        resmap = []
    ...
    def getPathData(self, base_dir):
        ...
        for (dpath, dirs, files) in os.walk(base_dir, topdown=False):
            ...
            for f in files:
                ...
                hash_table[fp] = {'hash': filehash, 'size': fsize, 'type': 'F', 'mtime': fmtime}

            dmtime = getASCTime(os.path.getmtime(dpath))
            dirs_hashpart.sort()
            files_hashpart.sort()
            dirhash = hash((tuple(dirs_hashpart), tuple(files_hashpart)))
            hash_table[dpath] = {'hash': dirhash, 'size': dirsize, 'type': 'D', 'mtime': dmtime}
        ...
        for deep in deeps[:-1]:
            for data in deepdata[deep]:
                ...
                for lowdeep in deeps[deepindx + 1:]:
                    deepdata[lowdeep] = filter(lambda d: not d['path'].startswith(dpath), deepdata[lowdeep])
...
class DupOut(object):
    ...
    def out(self, dups, outf=file):
        ...
        logger.info('TOTAL duplications:%d [FILES:%d, DIRECTORIES:%d]' % (files + dirs, files, dirs))
    ...
    def csvFormatter(self, data, outfile):
        ...
        out = file(outfile, 'wb')
        fields = ['hash', 'size', 'type', 'ext', 'name', 'directory', 'modification', 'operation', 'operation_data']
        ...
        for (hsh, pdata) in data:
            ...
            if pdata['type'] == 'F':
                ...
                ext = ''
            writer.writerow({'hash': hsh, 'size': pdata['size'], 'type': pdata['type'], 'ext': ext, 'name': name, 'directory': dir, 'modification': pdata['mtime']})
            out.flush()
...
def getConsoleParams():
    ...
    parser.add_option('-o', '--output-file', dest='output_file', default=DupOut.file, help='Path to (name of) result file')
    parser.add_option('-m', '--minimal-size', dest='minSize', type='int', default=0, help="Set minimal size for duplications - smaler files will be filtered out, default='%default'")
    parser.add_option('-q', '--quick', dest='quick', action='store_true', default=False, help="Quick file comparison only by file-name and file-size. If False - file duplication calculated by content. Default='%default'")
    parser.add_option('-v', '--verbose', dest='verbose', action='store_true', default=False, help="Show duplications to console, default='%default'")
    ...
    return (dirs, options.minSize, options.output_file, output_format, options.quick, options.verbose)


def main(dirs=[], minSize=0, outf=DupOut.file, outformat=DupOut.formats[0], verbose=False):
    if not dirs:
