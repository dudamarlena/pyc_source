...
charts = Blueprint('jsondash',
  __name__,
  template_folder=TEMPLATE_DIR,
  static_url_path=STATIC_DIR,
  static_folder=STATIC_DIR)
default_config = dict(JSONDASH_FILTERUSERS=False,
...
def ctx():
    ...
    return dict(static_config=static,
      charts_config=config,
      page_title='dashboards',
      docs_url='https://github.com/christabor/flask_jsondash/blob/master/docs/',
...
def paginator(page=0, per_page=None, count=None):
    ...
    return Paginator(limit=per_page,
      per_page=per_page,
      curr_page=curr_page,
...
def dashboard():
    ...
    if setting('JSONDASH_FILTERUSERS'):
        ...
        if views:
            ...
            pagination = None
    kwargs = dict(views=views,
      view=None,
      paginator=pagination,
      creating=True,
      can_edit_global=auth(authtype='edit_global'),
      total_modules=(sum([len(view.get('modules', [])) for view in views if isinstance(view, dict)])))
