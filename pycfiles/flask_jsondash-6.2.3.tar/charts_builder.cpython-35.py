...
# [GCC 8.4.0]
# Embedded file name: /Users/taborc/git/spog/flask_jsondash/flask_jsondash/charts_builder.py
# Compiled at: 2017-06-13 14:37:35
# Size of source mod 2**32: 23028 bytes
"""
...
REQUIRED_STATIC_FAMILES = [
 'D3']
Paginator = namedtuple('Paginator', 'count per_page curr_page num_pages limit skip')
charts = Blueprint('jsondash', __name__, template_folder=TEMPLATE_DIR, static_url_path=STATIC_DIR, static_folder=STATIC_DIR)
default_config = dict(JSONDASH_FILTERUSERS=False, JSONDASH_GLOBALDASH=False, JSONDASH_GLOBAL_USER='global', JSONDASH_PERPAGE=25)
adapter = db.get_db_handler()

class InvalidSchemaError(ValueError):
    __doc__ = 'Wrapper exception for specific raising scenarios.'
...
def ctx():
    ...
    config = CHARTS_CONFIG if not static else local_static(CHARTS_CONFIG, static)
    return dict(static_config=static, charts_config=config, page_title='dashboards', docs_url='https://github.com/christabor/flask_jsondash/blob/master/docs/', demo_mode=request.args.get('jsondash_demo_mode', False), global_dashuser=setting('JSONDASH_GLOBAL_USER'), global_dashboards=setting('JSONDASH_GLOBALDASH'), username=metadata(key='username') if filter_user else None, filter_dashboards=filter_user)
        ...
        if family in families:
            if data['dependencies']:
                for dep in data['dependencies']:
                    assets['css'] += [css for css in CHARTS_CONFIG[dep]['css_url'] if css not in assets['css']]
                    assets['js'] += [js for js in CHARTS_CONFIG[dep]['js_url'] if js not in assets['js']]

            assets['css'] += [css for css in data['css_url'] if css not in assets['css']]
            assets['js'] += [js for js in data['js_url'] if js not in assets['js']]
...
def paginator(page=0, per_page=None, count=None):
    ...
    per_page = per_page if per_page > 2 else 2
    ...
    return Paginator(limit=per_page, per_page=per_page, curr_page=curr_page, skip=curr_page * per_page, num_pages=pages, count=count)
...
def dashboard():
    ...
    if setting('JSONDASH_FILTERUSERS'):
        ...
        if setting('JSONDASH_GLOBALDASH'):
            opts.update(filter=dict(created_by=setting('JSONDASH_GLOBAL_USER')))
            views += list(adapter.read(**opts))
    else:
        views = list(adapter.read(**opts))
    if views:
        ...
        pagination = None
    kwargs = dict(views=views, view=None, paginator=pagination, creating=True, can_edit_global=auth(authtype='edit_global'), total_modules=sum([len(view.get('modules', [])) for view in views if isinstance(view, dict)]))
...
def view(c_id):
    ...
    if 'modules' not in viewjson:
        ...
        return redirect(url_for('jsondash.dashboard'))
    active_charts = [v.get('family') for v in viewjson['modules'] if v.get('family') is not None]
    ...
    kwargs = dict(id=c_id, view=viewjson, num_rows=None if layout_type == 'freeform' else get_num_rows(viewjson), modules=sort_modules(viewjson), assets=get_active_assets(active_charts), can_edit=can_edit, can_edit_global=auth(authtype='edit_global'), is_global=is_global_dashboard(viewjson))
...
def validate_raw_json_grid(conf):
    ...
    for module in modules:
        try:
            ...
            raise InvalidSchemaError('Invalid row value for module "{}"'.format(module.get('name')))

    if not is_consecutive_rows(rows):
        raise InvalidSchemaError('Row order is not consecutive: "{}"!'.format(sorted(rows)))
    for module in modules:
        width = module.get('width')
        if width not in valid_cols:
            raise InvalidSchemaError('Invalid width for grid format: "{}"'.format(width))
        for field in fixed_only_required:
            if field not in module and layout == 'grid':
                ident = module.get('name', module)
                raise InvalidSchemaError('Invalid JSON. "{}" must be included in "{}" for fixed grid layouts'.format(field, ident))
    ...
    if layout == 'grid':
        ...
        for module in data.get('modules', []):
            ...
            try:
                ...
                raise InvalidSchemaError('Invalid value for width in `freeform` layout.')

            if module.get('row') is not None:
                raise InvalidSchemaError('Cannot mix `row` with `freeform` layout.')
...
def update(c_id):
    ...
    if edit_raw:
        ...
        if layout == 'grid' and modules and modules[0].get('row') is None:
            flash('Cannot use grid layout without specifying row(s)! Edit JSON manually to override this.', 'error')
            return redirect(view_url)
        data = dict(name=form_data['name'], layout=layout, modules=modules, id=c_id, date=now)
...
def create():
    ...
    d = dict(name=data['name'], modules=db.format_charts(data), date=str(dt.now()), id=new_id, layout=data.get('mode', 'grid'))
    d.update(**metadata())
...
def clone(c_id):
    ...
    data = dict(name=newname, modules=viewjson['modules'], date=str(dt.now()), id=str(uuid.uuid1()), layout=viewjson['layout'])
