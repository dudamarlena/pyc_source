...
# [GCC 8.4.0]
# Embedded file name: /Users/taborc/git/spog/flask_jsondash/flask_jsondash/model_factories.py
# Compiled at: 2017-06-02 14:35:57
"""
...
def make_fake_dashboard(name='Random chart', max_charts=10):
    ...
    """
    charts = ImmutableMultiDict([ make_fake_chart_data() for _ in range(max_charts) ])
    return dict(name=name, created_by='global', date=dt.now(), modules=db.format_charts(charts), id=str(uuid1()))
...
def make_fake_chart_data(**kwargs):
    ...
    config = dict(name=kwargs.get('name'), width=kwargs.get('width', randrange(200, 2000)), height=kwargs.get('height', randrange(200, 2000)), type=chart, family='C3', dataSource=kwargs.get('data_source', url), guid=_uuid)
    if will_use_inputs:
        config.update(inputs=dict(btn_classes=[
         'btn', 'btn-info', 'btn-sm'], submit_text='Submit', help_text='Change a value', options=[
...
@click.command()
@click.option('--records', default=10, help='Number of records to insert fake dashboard data into DB.')
@click.option('--max-charts', default=5, help='Number of charts per dashboard to create.')
def insert_dashboards(records, max_charts):
    """Insert a number of dashboard records into the database."""
    for i in range(records):
        data = make_fake_dashboard(name=('Test chart #{}').format(i), max_charts=max_charts)
