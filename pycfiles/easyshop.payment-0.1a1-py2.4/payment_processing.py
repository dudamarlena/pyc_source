...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/payment/adapters/payment_processing.py
    ...
    def process(self, order=None):
        ...
        for (i, item) in enumerate(IItemManagement(order).getItems()):
            if item.getProductTax() > 0:
                ...
                tax = 'N'
            line_items.append((str(i + 1), item.getProduct().Title(), str(item.getProductQuantity()), str(item.getProductPriceGross()), tax))

        amount = '%.2f' % IPrices(order).getPriceForCustomer()
        cc = EasyShopCcProcessor(server='test.authorize.net', login='39uaCH7r9K', key='9ME22bvLnu87P4FY')
        result = cc.authorizeAndCapture(amount=amount, card_num=card_num, exp_date=exp_date)
        if result.response == 'approved':
            return PaymentResult(PAYED, _('Your order has been payed.'))
        else:
...
class PayPalPaymentProcessor:
    ...
    def process(self, order):
        ...
        info = {'cmd': '_cart', 'upload': '1', 'business': 'heinz@haendler.de', 'currency_code': 'EUR', 'notify_url': '', 'return': '', 'last_name': shipping_address.getName(), 'address1': shipping_address.address_1, 'city': shipping_address.city, 'state': shipping_address.country, 'zip': shipping_address.zip_code, 'shipping_1': order.getShippingPriceNet(), 'tax_1': pc.getPriceGross() - pc.getPriceNet()}
