...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dobro/dobro.py
# Compiled at: 2018-03-02 08:32:24
from __future__ import print_function
import sys, os, subprocess, argparse, time, re, json, socket
from os import environ as env

class Dobro(object):
    VERSION = '0.4.0'
    PROGRAM_NAME = 'dobro'
    PROGRAM_DESCRIPTION = '%s - tag-centric digitalocean droplet manager' % PROGRAM_NAME
    NEW_MARKER = 'new'
    LOG_SEPARATOR = ': '
    SSH_DEFAULT_PORT = 22
    SSH_POLL_PERIOD = 3
    ...
    def __init__(self):
        ...
        self.SSH_PRIVATE_KEY_PATH = env.get('DOBRO_SSH_PRIVATE_KEY', env['HOME'] + '/.ssh/id_rsa')
        self.SSH_PUBLIC_KEY_PATH = env.get('DOBRO_SSH_PUBLIC_KEY', self.SSH_PRIVATE_KEY_PATH + '.pub')
        self.NEW_DROPLET_ARGS = env.get('DOBRO_NEW_DROPLET_ARGS', '--region lon1 --size 512mb --image ubuntu-16-04-x64').split(' ')
    ...
    def colour(self, s, code=False):
        if code:
            return '\x1b[%sm%s\x1b[39m' % (code, s)
        return s
    ...
    def log(self, level, *args):
        ...
        s = self.colour('[%s - %s] %s' % (
         self.PROGRAM_NAME,
         level_s,
         self.LOG_SEPARATOR.join(map(lambda x: str(x), args))), c)
    ...
    def doctl(self, *args, **kwargs):
        ...
        args_full = [
         'doctl', 'compute'] + list(args) + ['--output', kwargs.get('output', 'json')] + self.DOCTL_EXTRA_ARGS
        self.log(self.DEBUG, args_full)
        p = subprocess.Popen(args_full, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def sync_ssh_key(self):
        ...
        def push(method):
            ...
            output = self.doctl('ssh-key', 'create', do_key_name, '--public-key', current_key)
            return output[0]['id']
    ...
    def tag_droplet(self, droplet, **kwargs):
        ...
        for tag in tags:
            ...
            self.doctl('droplet', 'tag', str(droplet['id']), '--tag-name', tag)

    def untag_droplet(self, droplet, **kwargs):
        tags = list(self.cli_args.tag)
        for tag in tags:
            if tag not in droplet['tags']:
                continue
            self.log(self.INFO, 'droplet', 'untag', '%s => %s' % (self.q(droplet['name']), self.q(tag)))
            self.doctl('droplet', 'untag', str(droplet['id']), '--tag-name', tag)
    ...
    def cmd_create(self):
        ...
        params = [
         'droplet', 'create'] + names + ['--wait', '--ssh-keys', str(key_id)] + self.NEW_DROPLET_ARGS
        ...
        for droplet in droplets:
            ...
            if not self.cli_args.name:
                ...
                self.doctl('droplet-action', 'rename', str(droplet['id']), '--droplet-name', new_name, '--wait')
                subprocess.check_output(['ssh', '-t',
                 '-i', self.SSH_PRIVATE_KEY_PATH,
                 '-oStrictHostKeyChecking=no',
                 '-oIdentitiesOnly=yes',
                 'root@' + self.droplet_ip(droplet),
                 "                    hostname %s &&                     echo %s > /etc/hostname &&                     sed -i -e 's/^127\\.0\\.1\\.1.*$/127.0.1.1 %s/g' /etc/hosts" % (
                  self.q(new_name),
                  self.q(new_name),
                  new_name)])
        ...
        self.hook('post')
        for droplet in droplets:
            self.log(self.INFO, 'droplet', 'ip', '%s => %s' % (self.q(droplet['name']), self.q(self.droplet_ip(droplet))))
