...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/internal/_parameterized.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 15500 bytes
...
def _NonStringIterable(obj):
    return isinstance(obj, collections_abc.Iterable) and not isinstance(obj, six.string_types)
...
def _FormatParameterList(testcase_params):
    if isinstance(testcase_params, collections_abc.Mapping):
        ...
        if _NonStringIterable(testcase_params):
            return ', '.join(map(_CleanRepr, testcase_params))
        return _FormatParameterList((testcase_params,))


class _ParameterizedTestIter(object):
    __doc__ = 'Callable and iterable class for producing new test cases.'
    ...
    def __iter__(self):
        ...
        def MakeBoundParamTest(testcase_params):
            ...
            if naming_type is _FIRST_ARG:
                ...
                if naming_type is _ARGUMENT_REPR:
...
def _ModifyClass(class_object, testcases, naming_type):
    assert not getattr(class_object, '_id_suffix', None), 'Cannot add parameters to %s, which already has parameterized methods.' % (
     class_object,)
...
class TestCase(unittest.TestCase):
    __doc__ = 'Base class for test cases using the parameters decorator.'
...
def CoopTestCase(other_base_class):
  ...
  Returns:
    A new class object.
  """
    metaclass = type('CoopMetaclass', (
     other_base_class.__metaclass__,
     TestGeneratorMetaclass), {})
    return metaclass('CoopTestCase', (
