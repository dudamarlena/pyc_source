...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/grpcio/grpc/_utilities.py
# Compiled at: 2020-01-10 16:25:22
# Size of source mod 2**32: 5181 bytes
"""Internal utilities for gRPC Python."""
import collections, threading, time, logging, six, grpc
from grpc import _common
_LOGGER = logging.getLogger(__name__)
_DONE_CALLBACK_EXCEPTION_LOG_MESSAGE = 'Exception calling connectivity future "done" callback!'

class RpcMethodHandler(collections.namedtuple('_RpcMethodHandler', ('request_streaming', 'response_streaming',
...
class DictionaryGenericHandler(grpc.ServiceRpcHandler):
    ...
    def __init__(self, service, method_handlers):
        self._name = service
        self._method_handlers = {_common.fully_qualified_method(service, method):method_handler for method, method_handler in six.iteritems(method_handlers)}
    ...
    def _update(self, connectivity):
        with self._condition:
            if not self._cancelled:
                if connectivity is grpc.ChannelConnectivity.READY:
                    self._matured = True
