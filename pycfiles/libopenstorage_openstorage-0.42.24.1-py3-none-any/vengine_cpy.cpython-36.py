...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cffi/cffi/vengine_cpy.py
# Compiled at: 2020-01-10 16:25:38
# Size of source mod 2**32: 41470 bytes
import sys, imp
from . import model
...
class VCPythonEngine(object):
    ...
    def _do_collect_type(self, tp):
        if not isinstance(tp, model.PrimitiveType) or tp.name == 'long double':
    ...
    def _generate_struct_or_union_decl(self, tp, prefix, name):
        ...
        for fname, ftype, fbitsize, fqual in tp.enumfields():
            if fbitsize >= 0:
                ...
                if isinstance(ftype, model.ArrayType):
    ...
    def _generate_cpy_variable_decl(self, tp, name):
        if isinstance(tp, model.ArrayType):
            tp_ptr = model.PointerType(tp.item)
            self._generate_cpy_const(False, name, tp, vartp=tp_ptr, size_too=(tp.length == '...'))
        else:
            tp_ptr = model.PointerType(tp)
