...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/grpcio/grpc/beta/_client_adaptations.py
# Compiled at: 2020-01-10 16:25:22
# Size of source mod 2**32: 26290 bytes
...
_STATUS_CODE_TO_ABORTION_KIND_AND_ABORTION_ERROR_CLASS = {grpc.StatusCode.CANCELLED: (face.Abortion.Kind.CANCELLED,
...
def _credentials(grpc_call_options):
    if grpc_call_options is None:
        return
    else:
        return grpc_call_options.credentials
...
def _blocking_unary_unary(channel, group, method, timeout, with_call, protocol_options, metadata, metadata_transformer, request, request_serializer, response_deserializer):
    try:
        multi_callable = channel.unary_unary((_common.fully_qualified_method(group, method)),
        ...
        if with_call:
            ...
            return (
             response, _Rendezvous(None, None, call))
        else:
...
def _unary_stream(channel, group, method, timeout, protocol_options, metadata, metadata_transformer, request, request_serializer, response_deserializer):
    multi_callable = channel.unary_stream((_common.fully_qualified_method(group, method)),
      request_serializer=request_serializer,
      response_deserializer=response_deserializer)
    effective_metadata = _effective_metadata(metadata, metadata_transformer)
...
def _blocking_stream_unary(channel, group, method, timeout, with_call, protocol_options, metadata, metadata_transformer, request_iterator, request_serializer, response_deserializer):
    try:
        multi_callable = channel.stream_unary((_common.fully_qualified_method(group, method)),
          request_serializer=request_serializer,
          response_deserializer=response_deserializer)
        effective_metadata = _effective_metadata(metadata, metadata_transformer)
        if with_call:
            response, call = multi_callable.with_call(request_iterator,
              timeout=timeout,
              metadata=(_metadata.unbeta(effective_metadata)),
              credentials=(_credentials(protocol_options)))
            return (
...
def _future_stream_unary(channel, group, method, timeout, protocol_options, metadata, metadata_transformer, request_iterator, request_serializer, response_deserializer):
    multi_callable = channel.stream_unary((_common.fully_qualified_method(group, method)),
      ...
      response_deserializer=response_deserializer)
    effective_metadata = _effective_metadata(metadata, metadata_transformer)
    response_future = multi_callable.future(request_iterator,
      timeout=timeout,
      metadata=(_metadata.unbeta(effective_metadata)),
      credentials=(_credentials(protocol_options)))
    return _Rendezvous(response_future, None, response_future)
...
def _stream_stream(channel, group, method, timeout, protocol_options, metadata, metadata_transformer, request_iterator, request_serializer, response_deserializer):
    multi_callable = channel.stream_stream((_common.fully_qualified_method(group, method)),
      request_serializer=request_serializer,
      response_deserializer=response_deserializer)
    effective_metadata = _effective_metadata(metadata, metadata_transformer)
...
def generic_stub(channel, host, metadata_transformer, request_serializers, response_deserializers):
    return _GenericStub(channel, metadata_transformer, request_serializers, response_deserializers)


def dynamic_stub(channel, service, cardinalities, host, metadata_transformer, request_serializers, response_deserializers):
    return _DynamicStub(_GenericStub(channel, metadata_transformer, request_serializers, response_deserializers), service, cardinalities)