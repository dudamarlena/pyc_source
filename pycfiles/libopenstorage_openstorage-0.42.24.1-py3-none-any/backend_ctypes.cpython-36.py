...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cffi/cffi/backend_ctypes.py
# Compiled at: 2020-01-10 16:25:38
# Size of source mod 2**32: 42454 bytes
...
class CTypesData(object):
    ...
    def __iter__(self):
        raise TypeError('cdata %r does not support iteration' % self._get_c_name())
...
class CTypesGenericPtr(CTypesData):
    ...
    def _cast_from(cls, source):
        if source is None:
            ...
            if isinstance(source, CTypesData):
...
class CTypesBackend(object):
    PRIMITIVE_TYPES = {'char':ctypes.c_char, 
     ...
     '_Bool':ctypes.c_bool}
    for _name in ('unsigned long long', 'unsigned long', 'unsigned int', 'unsigned short',
                  'unsigned char'):
            ...
            if kind == 'float':
                ...
                def _cast_from(cls, source):
                    if isinstance(source, float):
                        ...
                        if isinstance(source, CTypesGenericPrimitive):
    ...
    def new_pointer_type(self, BItem):
        ...
        if BItem is getbtype(model.PrimitiveType('char')):
            ...
            if BItem in (getbtype(model.PrimitiveType('signed char')),
             getbtype(model.PrimitiveType('unsigned char'))):
                kind = 'bytep'
            else:
                if BItem is getbtype(model.void_type):
                    kind = 'voidp'
                else:
                    kind = 'generic'
        ...
        class CTypesPtr(CTypesGenericPtr):
            ...
            def __init__(self, init):
                ...
                if kind == 'charp':
                    self._CTypesPtr__as_strbuf = ctypes.create_string_buffer(ctypeobj.value + b'\x00')
                    self._as_ctype_ptr = ctypes.cast(self._CTypesPtr__as_strbuf, self._ctype)
                else:
    ...
    def complete_struct_or_union(self, CTypesStructOrUnion, fields, tp, totalsize=-1, totalalignment=-1, sflags=0, pack=0):
        ...
        for fname, BField, bitsize in fields:
            ...
            elif hasattr(CTypesStructOrUnion, fname):
    ...
    def new_function_type(self, BArgs, BResult, has_varargs):
        ...
        class CTypesFunctionPtr(CTypesGenericPtr):
            ...
            def __init__(self, init, error=None):
                import traceback
                if not not has_varargs:
                    raise AssertionError('varargs not supported for callbacks')
                else:
 ...
 ctypes.c_void_p, *[BArg._ctype for BArg in BArgs], **{'use_errno': True})
                    ...
                    for i, arg in enumerate(extraargs):
                        if arg is None:
                            ...
                            if not isinstance(arg, CTypesData):
                                raise TypeError('argument %d passed in the variadic part needs to be a cdata object (got %s)' % (
    ...
    def gcp(self, cdata, destructor, size=0):
        ...
        if destructor is None:
            try:
                ...
                raise TypeError('Can remove destructor only on a object previously returned by ffi.gc()')
    ...
    def typeoffsetof(self, BType, fieldname, num=0):
        if isinstance(fieldname, str):
            if num == 0:
                if issubclass(BType, CTypesGenericPtr):
                    BType = BType._BItem
            else:
            ...
            return (
             BField, BType._offsetof(fieldname))
