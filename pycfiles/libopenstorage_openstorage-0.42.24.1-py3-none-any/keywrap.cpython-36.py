...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cryptography/cryptography/hazmat/primitives/keywrap.py
...
def _unwrap_core(wrapping_key, a, r, backend):
    ...
    for j in reversed(range(6)):
        for i in reversed(range(n)):
            atr = struct.pack('>Q', struct.unpack('>Q', a)[0] ^ n * j + i + 1) + r[i]
...
def aes_key_wrap_with_padding(wrapping_key, key_to_wrap, backend):
    if len(wrapping_key) not in (16, 24, 32):
        raise ValueError('The wrapping key must be a valid AES key length')
    aiv = b'\xa6YY\xa6' + struct.pack('>i', len(key_to_wrap))
...
def aes_key_unwrap(wrapping_key, wrapped_key, backend):
    if len(wrapped_key) < 24:
        ...
        raise bytes_eq(a, aiv) or InvalidUnwrap()
    return (b'').join(r)
