...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/twine/twine/settings.py
# Compiled at: 2020-01-10 16:25:25
# Size of source mod 2**32: 11727 bytes
...
class Settings:
    ...
    def __init__(self, *, sign: bool=False, sign_with: Optional[str]='gpg', identity: Optional[str]=None, username: Optional[str]=None, password: Optional[str]=None, non_interactive: bool=False, comment: Optional[str]=None, config_file: str='~/.pypirc', skip_existing: bool=False, cacert: Optional[str]=None, client_cert: Optional[str]=None, repository_name: str='pypi', repository_url: Optional[str]=None, verbose: bool=False, disable_progress_bar: bool=False, **ignored_kwargs) -> None:
        ...
        self._handle_repository_options(repository_name=repository_name,
          repository_url=repository_url)
        self._handle_package_signing(sign=sign,
          sign_with=sign_with,
          identity=identity)
        self._handle_certificates(cacert, client_cert)
        self.auth = auth.Resolver.choose(not non_interactive)(self.repository_config, auth.CredentialInput(username, password))
    ...
    def register_argparse_arguments(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument('-r',
          '--repository', action=(utils.EnvironmentDefault),
          env='TWINE_REPOSITORY',
          default='pypi',
          help='The repository (package index) to upload the package to. Should be a section in the config file (default: %(default)s). (Can also be set via %(env)s environment variable.)')
        parser.add_argument('--repository-url',
          action=(utils.EnvironmentDefault),
          env='TWINE_REPOSITORY_URL',
          default=None,
          required=False,
          help='The repository (package index) URL to upload the package to. This overrides --repository. (Can also be set via %(env)s environment variable.)')
        parser.add_argument('-s',
          '--sign', action='store_true',
          default=False,
          help='Sign files to upload using GPG.')
        parser.add_argument('--sign-with',
          default='gpg',
          help='GPG program used to sign uploads (default: %(default)s).')
        parser.add_argument('-i',
          '--identity', help='GPG identity used to sign files.')
        parser.add_argument('-u',
          '--username', action=(utils.EnvironmentDefault),
          env='TWINE_USERNAME',
          required=False,
          help='The username to authenticate to the repository (package index) as. (Can also be set via %(env)s environment variable.)')
        parser.add_argument('-p',
          '--password', action=(utils.EnvironmentDefault),
          env='TWINE_PASSWORD',
          required=False,
          help='The password to authenticate to the repository (package index) with. (Can also be set via %(env)s environment variable.)')
        parser.add_argument('--non-interactive',
          action=(utils.EnvironmentFlag),
          env='TWINE_NON_INTERACTIVE',
          help='Do not interactively prompt for username/password if the required credentials are missing. (Can also be set via %(env)s environment variable.)')
        parser.add_argument('-c',
          '--comment', help='The comment to include with the distribution file.')
        ...
        parser.add_argument('--skip-existing',
          default=False,
          action='store_true',
          help='Continue uploading files if one already exists. (Only valid when uploading to PyPI. Other implementations may not support this.)')
        parser.add_argument('--cert',
          ...
          required=False,
          metavar='path',
          help='Path to alternate CA bundle (can also be set via %(env)s environment variable).')
    ...
    def _handle_package_signing(self, sign: bool, sign_with: Optional[str], identity: Optional[str]) -> None:
        if not sign:
            if identity:
                raise exceptions.InvalidSigningConfiguration('sign must be given along with identity')
        self.sign = sign
        self.sign_with = sign_with
        self.identity = identity

    def _handle_repository_options(self, repository_name: str, repository_url: Optional[str]) -> None:
        self.repository_config = utils.get_repository_from_config(self.config_file, repository_name, repository_url)
        self.repository_config['repository'] = utils.normalize_repository_url(cast(str, self.repository_config['repository']))

    def _handle_certificates(self, cacert: Optional[str], client_cert: Optional[str]) -> None:
    ...
    def check_repository_url(self) -> None:
        ...
        if repository_url.startswith((repository.LEGACY_PYPI,
         repository.LEGACY_TEST_PYPI)):
            raise exceptions.UploadToDeprecatedPyPIDetected.from_args(repository_url, utils.DEFAULT_REPOSITORY, utils.TEST_REPOSITORY)

    def create_repository(self) -> repository.Repository:
        """Create a new repository for uploading."""
        repo = repository.Repository(cast(str, self.repository_config['repository']), self.username, self.password, self.disable_progress_bar)
