...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/text_format.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 60350 bytes
...
import encodings.raw_unicode_escape, encodings.unicode_escape, io, re, six
if six.PY3:
    long = int
...
def MessageToString(message, as_utf8=False, as_one_line=False, use_short_repeated_primitives=False, pointy_brackets=False, use_index_order=False, float_format=None, double_format=None, use_field_number=False, descriptor_pool=None, indent=0, message_formatter=None, print_unknown_fields=False):
  ...
  """
    ...
    printer = _Printer(out, indent, as_utf8, as_one_line, use_short_repeated_primitives,
      pointy_brackets, use_index_order,
      float_format, double_format, use_field_number,
      descriptor_pool,
      message_formatter, print_unknown_fields=print_unknown_fields)
...
def PrintField(field, value, out, indent=0, as_utf8=False, as_one_line=False, use_short_repeated_primitives=False, pointy_brackets=False, use_index_order=False, float_format=None, double_format=None, message_formatter=None, print_unknown_fields=False):
    ...
    printer = _Printer(out, indent, as_utf8, as_one_line, use_short_repeated_primitives,
      pointy_brackets, use_index_order,
      float_format, double_format, message_formatter=message_formatter,
      print_unknown_fields=print_unknown_fields)
            ...
            if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_ENUM:
                ...
                if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_STRING:
                    ...
                    if isinstance(value, six.text_type):
                        if six.PY2 or not self.as_utf8:
                    ...
                    if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_BOOL:
                        ...
                        elif field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_DOUBLE:
                    ...
                    if field:
                        if field.type == descriptor.FieldDescriptor.TYPE_GROUP:
                            if field.message_type.name != name:
                                field = None
                if not field:
                    if not self.allow_unknown_field:
                ...
                if field.type in (descriptor.FieldDescriptor.TYPE_UINT32,
                 descriptor.FieldDescriptor.TYPE_FIXED32):
                    ...
                    if field.label == descriptor.FieldDescriptor.LABEL_REPEATED:
                        ...
                        if field.is_extension:
                            if not self._allow_multiple_scalars:
                                if not self._IsProto3Syntax(message):
                                    if message.HasExtension(field):
                                        raise tokenizer.ParseErrorPreviousToken('Message type "%s" should not have multiple "%s" extensions.' % (
                                         message.DESCRIPTOR.full_name, field.full_name))
                            else:
