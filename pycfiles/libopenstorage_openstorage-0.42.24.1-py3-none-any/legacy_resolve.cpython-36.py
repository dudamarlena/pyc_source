...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/pip/pip/_internal/legacy_resolve.py
# Compiled at: 2020-01-10 16:25:21
# Size of source mod 2**32: 17129 bytes
...
from pip._internal.utils.misc import dist_in_usersite, ensure_dir, normalize_version_info
...
def _check_dist_requires_python(dist, version_info, ignore_requires_python=False):
    """
    ...
    try:
        ...
        logger.warning('Package %r has an invalid Requires-Python: %s', dist.project_name, exc)
        return
    ...
    if ignore_requires_python:
        ...
        return
    raise UnsupportedPythonVersion('Package {!r} requires a different Python: {} not in {!r}'.format(dist.project_name, version, requires_python))


class Resolver(object):
    __doc__ = 'Resolves which packages need to be installed/uninstalled to perform     the requested operation without breaking the requirements of any package.\n    '
    _allowed_strategies = {
     'eager', 'only-if-needed', 'to-satisfy-only'}

    def __init__(self, preparer, session, finder, make_install_req, use_user_site, ignore_dependencies, ignore_installed, ignore_requires_python, force_reinstall, upgrade_strategy, py_version_info=None):
        super(Resolver, self).__init__()
        if not upgrade_strategy in self._allowed_strategies:
    ...
    def _is_upgrade_allowed(self, req):
        if self.upgrade_strategy == 'to-satisfy-only':
            ...
            if self.upgrade_strategy == 'eager':
                return True
            elif not self.upgrade_strategy == 'only-if-needed':
                raise AssertionError
    ...
    def _get_abstract_dist_for(self, req):
        ...
        if req.editable:
            ...
            if not req.satisfied_by is None:
                ...
                abstract_dist = self.preparer.prepare_linked_requirement(req, self.session, self.finder, self.require_hashes)
    ...
    def _resolve_one(self, requirement_set, req_to_install):
        ...
        if req_to_install.constraint or req_to_install.prepared:
            ...
            _check_dist_requires_python(dist,
              version_info=(self._py_version_info), ignore_requires_python=(self.ignore_requires_python))
            more_reqs = []

            def add_req(subreq, extras_requested):
                sub_install_req = self._make_install_req(str(subreq), req_to_install)
                parent_req_name = req_to_install.name
                to_scan_again, add_to_parent = requirement_set.add_requirement(sub_install_req,
