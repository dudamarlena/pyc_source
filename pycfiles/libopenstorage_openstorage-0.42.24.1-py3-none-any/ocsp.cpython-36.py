...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cryptography/cryptography/x509/ocsp.py
# Compiled at: 2020-01-10 16:25:37
# Size of source mod 2**32: 13310 bytes
...
from cryptography.x509.base import _EARLIEST_UTC_TIME, _convert_to_naive_utc_time, _reject_duplicate_extension
...
class _SingleResponse(object):
    ...
    def __init__(self, cert, issuer, algorithm, cert_status, this_update, next_update, revocation_time, revocation_reason):
        if not isinstance(cert, x509.Certificate) or not isinstance(issuer, x509.Certificate):
            raise TypeError('cert and issuer must be a Certificate')
        else:
            _verify_algorithm(algorithm)
            ...
            raise isinstance(cert_status, OCSPCertStatus) or TypeError('cert_status must be an item from the OCSPCertStatus enum')
        if cert_status is not OCSPCertStatus.REVOKED:
            ...
            if revocation_reason is not None:
                if not isinstance(revocation_reason, x509.ReasonFlags):
...
class OCSPResponseBuilder(object):
    ...
    def add_response(self, cert, issuer, algorithm, cert_status, this_update, next_update, revocation_time, revocation_reason):
    ...
    def responder_id(self, encoding, responder_cert):
        if self._responder_id is not None:
            ...
            raise isinstance(encoding, OCSPResponderEncoding) or TypeError('encoding must be an element from OCSPResponderEncoding')
    ...
    def certificates(self, certs):
        if self._certs is not None:
            ...
            raise all(isinstance(x, x509.Certificate) for x in certs) or TypeError('certs must be a list of Certificates')
        ...
        if isinstance(private_key, (
         ed25519.Ed25519PrivateKey, ed448.Ed448PrivateKey)):
            if algorithm is not None:
                raise ValueError('algorithm must be None when signing via ed25519 or ed448')
        else:
            if not isinstance(algorithm, hashes.HashAlgorithm):
                raise TypeError('Algorithm must be a registered hash algorithm.')
        return backend.create_ocsp_response(OCSPResponseStatus.SUCCESSFUL, self, private_key, algorithm)
