...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cffi/cffi/pkgconfig.py
...
def call(libname, flag, encoding=sys.getfilesystemencoding()):
    ...
    if sys.version_info >= (3, ) and not isinstance(bout, str):
        try:
            ...
            raise PkgConfigError('pkg-config %s %s returned bytes that cannot be decoded with encoding %r:\n%r' % (
             flag, libname, encoding, bout))

    if os.altsep != '\\':
        if '\\' in bout:
            raise PkgConfigError('pkg-config %s %s returned an unsupported backslash-escaped output:\n%r' % (
             flag, libname, bout))
    return bout
