...
# [GCC 8.4.0]
# Embedded file name: /home/lpabon/git/golang/porx/src/github.com/libopenstorage/openstorage-sdk-clients/sdk/python/build/lib/python3.6/site-packages/pycparser/ply/yacc.py
# Compiled at: 2018-07-25 08:51:15
# Size of source mod 2**32: 137323 bytes
...
class Grammar(object):
    ...
    def set_precedence(self, term, assoc, level):
        if not self.Productions == [None]:
            raise AssertionError('Must call set_precedence() before add_production()')
        else:
            if term in self.Precedence:
                raise GrammarError('Precedence already specified for terminal %r' % term)
            if assoc not in ('left', 'right', 'nonassoc'):
                raise GrammarError("Associativity must be one of 'left','right', or 'nonassoc'")
        self.Precedence[term] = (
         assoc, level)
...
class LRGeneratedTable(LRTable):
    ...
    def lr0_items(self):
        C = [
         self.lr0_closure([self.grammar.Productions[0].lr_next])]
        i = 0
        for I in C:
    ...
    def dr_relation(self, C, trans, nullable):
        ...
        if state == 0:
            if N == self.grammar.Productions[0].prod[0]:
                terms.append('$end')
        return terms
    ...
    def lr_parse_table(self):
        ...
        for I in C:
                                ...
                                if r > 0:
                                    ...
                                    if slevel < rlevel or slevel == rlevel and rprec == 'left':
                                        ...
                                        if not slevel:
                                            ...
                                            rlevel or log.info('  ! shift/reduce conflict for %s resolved as shift', a)
            ...
            not_used = 0
            for a, p, m in actlist:
                if a in st_action and p is not st_actionp[a] and (
    ...
    def validate_error_func(self):
        if self.error_func:
            if isinstance(self.error_func, types.FunctionType):
                ...
                if isinstance(self.error_func, types.MethodType):
                    ismethod = 1
                else:
            ...
            for level, p in enumerate(self.prec):
                if not isinstance(p, (list, tuple)):
                    ...
                    if len(p) < 2:
                        ...
                        assoc = p[0]
                        isinstance(assoc, string_types) or self.log.error('precedence associativity must be a string')
                        self.error = True
                        return
                for term in p[1:]:
...
def yacc(method='LALR', debug=yaccdebug, module=None, tabmodule=tab_module, start=None, check_recursion=True, optimize=False, write_tables=True, debugfile=debug_file, outputdir=None, debuglog=None, errorlog=None, picklefile=None):
    ...
    if tabmodule is None:
        ...
        if module:
            ...
            if debug:
                if lr.sr_conflicts or lr.rr_conflicts:
                    debuglog.warning('')
                    ...
                    for state, rule, rejected in lr.rr_conflicts:
                        if (
                         state, id(rule), id(rejected)) in already_reported:
                            pass
                        else:
