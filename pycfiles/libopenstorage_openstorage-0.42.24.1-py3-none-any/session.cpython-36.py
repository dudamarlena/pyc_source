...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/pip/pip/_internal/network/session.py
# Compiled at: 2020-01-10 16:25:21
# Size of source mod 2**32: 15842 bytes
    ...
    def __init__(self, *args, **kwargs):
        ...
        retries = urllib3.Retry(total=retries,
         ...
         500, 503, 520, 527],
          backoff_factor=0.25)
        if cache:
            if not check_path_owner(cache):
                logger.warning("The directory '%s' or its parent directory is not owned by the current user and the cache has been disabled. Please check the permissions and owner of that directory. If executing pip with sudo, you may want sudo's -H flag.", cache)
                cache = None
        else:
            if cache:
        ...
        for secure_origin in self.iter_secure_origins():
            ...
            if origin_protocol != secure_protocol:
                if secure_protocol != '*':
                    continue
            try:
                addr = ipaddress.ip_address(origin_host if isinstance(origin_host, six.text_type) or origin_host is None else origin_host.decode('utf8'))
                network = ipaddress.ip_network(secure_host if isinstance(secure_host, six.text_type) else secure_host.decode('utf8'))
            except ValueError:
                if origin_host:
