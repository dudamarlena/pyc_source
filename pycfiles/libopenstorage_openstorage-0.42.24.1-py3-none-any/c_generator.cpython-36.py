...
# [GCC 8.4.0]
# Embedded file name: /home/lpabon/git/golang/porx/src/github.com/libopenstorage/openstorage-sdk-clients/sdk/python/build/lib/python3.6/site-packages/pycparser/c_generator.py
# Compiled at: 2018-07-25 08:51:15
# Size of source mod 2**32: 14668 bytes
from . import c_ast

class CGenerator(object):
    __doc__ = ' Uses the same visitor pattern as c_ast.NodeVisitor, but modified to\n        return a value from each visit method, using string accumulation in\n        generic_visit.\n    '
    ...
    def visit_Enumerator(self, n):
        if not n.value:
            return '{indent}{name},\n'.format(indent=(self._make_indent()),
              name=(n.name))
        else:
    ...
    def _generate_type(self, n, modifiers=[]):
        ...
        if typ == c_ast.TypeDecl:
            ...
            for i, modifier in enumerate(modifiers):
                if isinstance(modifier, c_ast.ArrayDecl):
                    if i != 0:
                        if isinstance(modifiers[(i - 1)], c_ast.PtrDecl):
                            nstr = '(' + nstr + ')'
        ...
        if typ == c_ast.Typename:
            return self._generate_type(n.type)
        if typ == c_ast.IdentifierType:
            return ' '.join(n.names) + ' '
        else:
            if typ in (c_ast.ArrayDecl, c_ast.PtrDecl, c_ast.FuncDecl):
