...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/Pygments/pygments/lexers/textfmts.py
# Compiled at: 2020-01-10 16:25:35
# Size of source mod 2**32: 13778 bytes
"""
...
class NotmuchLexer(RegexLexer):
    ...
    tokens = {'root':[
      (
       '\\fmessage{\\s*', Keyword, ('message', 'message-attr'))], 
     'message-attr':[
      (
       '(\\s*id:\\s*)([^\\s]+)', bygroups(Name.Attribute, String)),
      (
       '(\\s*(?:depth|match|excluded):\\s*)(\\d+)',
     ...
     'body':[
      (
       '\\fpart{\\n', Keyword, 'part'),
      (
       '\\f(part|attachment){\\s*', Keyword, ('part', 'part-attr')),
      (
       '\\fbody}\\n', Keyword, '#pop')], 
     'part-attr':[
      (
       '(ID:\\s*)(\\d+)', bygroups(Name.Attribute, Number.Integer)),
      (
       '(,\\s*)((?:Filename|Content-id):\\s*)([^,]+)',
     ...
     'part':[
      (
       '\\f(?:part|attachment)}\\n', Keyword, '#pop'),
      (
       '\\f(?:part|attachment){\\s*', Keyword, ('#push', 'part-attr')),
      (
       '^Non-text part: .*\\n', Comment),
      (
       '(?s)(.*?(?=\\f(?:part|attachment)}\\n))', _highlight_code)]}
