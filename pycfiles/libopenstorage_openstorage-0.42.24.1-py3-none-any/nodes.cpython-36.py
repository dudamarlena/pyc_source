...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/docutils/docutils/nodes.py
# Compiled at: 2020-01-10 16:25:35
# Size of source mod 2**32: 78311 bytes
"""
...
class Element(Node):
    ...
    def __getitem__(self, key):
        if isinstance(key, str):
            ...
            if isinstance(key, int):
                ...
                assert isinstance(key, slice) and key.step in (None, 1), 'cannot handle slice with stride'
    ...
    def copy_attr_concatenate(self, attr, value, replace):
        ...
        """
        if self.get(attr) is not value:
            if isinstance(self.get(attr), list):
                if isinstance(value, list):
                    self.append_attr_list(attr, value)
            else:
                self.replace_attr(attr, value, replace)
