...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/internal/well_known_types_test.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 38845 bytes
...
class TimeUtilTest(TimeUtilTestBase):
    ...
    def testTimestampSerializeAndParse(self):
        ...
        self.assertEqual(28800, message.seconds)
        self.assertEqual(0, message.nanos)
        message.GetCurrentTime()
        self.assertNotEqual(28800, message.seconds)
    ...
    def testDatetimeConverison(self):
        ...
        self.assertEqual(datetime.datetime(1970, 1, 1, 0, 0, 1, 999000), message.ToDatetime())
    ...
    def testInvalidTimestamp(self):
        message = timestamp_pb2.Timestamp()
        self.assertRaisesRegexp(ValueError, 'Failed to parse timestamp: missing valid timezone offset.', message.FromJsonString, '')
        self.assertRaisesRegexp(ValueError, 'Failed to parse timestamp: invalid trailing data 1970-01-01T00:00:01Ztrail.', message.FromJsonString, '1970-01-01T00:00:01Ztrail')
        ...
        self.assertRaisesRegexp(ValueError, 'nanos 0123456789012 more than 9 fractional digits.', message.FromJsonString, '1970-01-01T00:00:00.0123456789012Z')
        self.assertRaisesRegexp(ValueError, 'Invalid timezone offset value: \\+08.', message.FromJsonString, '1972-01-01T01:00:00.01+08')
        self.assertRaisesRegexp(ValueError, 'year (0 )?is out of range', message.FromJsonString, '0000-01-01T00:00:00Z')
        message.seconds = 253402300800
        self.assertRaisesRegexp(OverflowError, 'date value out of range', message.ToJsonString)

    def testInvalidDuration(self):
        message = duration_pb2.Duration()
        self.assertRaisesRegexp(ValueError, 'Duration must end with letter "s": 1.', message.FromJsonString, '1')
        self.assertRaisesRegexp(ValueError, "Couldn't parse duration: 1...2s.", message.FromJsonString, '1...2s')
        text = '-315576000001.000000000s'
        self.assertRaisesRegexp(ValueError, 'Duration is not valid\\: Seconds -315576000001 must be in range \\[-315576000000\\, 315576000000\\].', message.FromJsonString, text)
        text = '315576000001.000000000s'
        self.assertRaisesRegexp(ValueError, 'Duration is not valid\\: Seconds 315576000001 must be in range \\[-315576000000\\, 315576000000\\].', message.FromJsonString, text)
        message.seconds = -315576000001
        message.nanos = 0
        self.assertRaisesRegexp(ValueError, 'Duration is not valid\\: Seconds -315576000001 must be in range \\[-315576000000\\, 315576000000\\].', message.ToJsonString)
        message.seconds = 0
        message.nanos = 1000000000
        self.assertRaisesRegexp(ValueError, 'Duration is not valid\\: Nanos 1000000000 must be in range \\[-999999999\\, 999999999\\].', message.ToJsonString)
        message.seconds = -1
        message.nanos = 1
        self.assertRaisesRegexp(ValueError, 'Duration is not valid\\: Sign mismatch.', message.ToJsonString)
...
class FieldMaskTest(unittest.TestCase):
    ...
    def testMergeErrors(self):
        ...
        with self.assertRaises(ValueError) as (e):
            mask.MergeMessage(src, dst)
        self.assertEqual('Error: Field optional_int32 in message protobuf_unittest.TestAllTypes is not a singular message field and cannot have sub-fields.', str(e.exception))
    ...
    def testSnakeCaseToCamelCase(self):
        ...
        self.assertRaisesRegexp(ValueError, 'Fail to print FieldMask to Json string: Path name Foo must not contain uppercase letters.', well_known_types._SnakeCaseToCamelCase, 'Foo')
        self.assertRaisesRegexp(ValueError, 'Fail to print FieldMask to Json string: The character after a "_" must be a lowercase letter in path name foo__bar.', well_known_types._SnakeCaseToCamelCase, 'foo__bar')
        self.assertRaisesRegexp(ValueError, 'Fail to print FieldMask to Json string: The character after a "_" must be a lowercase letter in path name foo_3bar.', well_known_types._SnakeCaseToCamelCase, 'foo_3bar')
        self.assertRaisesRegexp(ValueError, 'Fail to print FieldMask to Json string: Trailing "_" in path name foo_bar_.', well_known_types._SnakeCaseToCamelCase, 'foo_bar_')
    ...
    def testCamelCaseToSnakeCase(self):
        ...
        self.assertRaisesRegexp(ValueError, 'Fail to parse FieldMask: Path name foo_bar must not contain "_"s.', well_known_types._CamelCaseToSnakeCase, 'foo_bar')
...
class StructTest(unittest.TestCase):
    ...
    def testStruct(self):
        ...
        self.assertEqual([6, True, False, None, inner_struct], list(struct['key5'].items()))
    ...
    def testMergeFrom(self):
        ...
        dictionary = {'key1':5, 
         ...
         'key4':{'subkey': 11.0}, 
         'key5':[
          6, 'seven', True, False, None, {'subkey2': 9}], 
         'key6':[
          [
           'nested_list', True]], 
         'empty_struct':{},  'empty_list':[]}
...
class AnyTest(unittest.TestCase):
    ...
    def testPackDeterministic(self):
        ...
        golden = b'\n4type.googleapis.com/google.protobuf.internal.TestAny\x12F\x1a\x05\n\x010\x10\x00\x1a\x05\n\x011\x10\x02\x1a\x05\n\x012\x10\x04\x1a\x05\n\x013\x10\x06\x1a\x05\n\x014\x10\x08\x1a\x05\n\x015\x10\n\x1a\x05\n\x016\x10\x0c\x1a\x05\n\x017\x10\x0e\x1a\x05\n\x018\x10\x10\x1a\x05\n\x019\x10\x12'
