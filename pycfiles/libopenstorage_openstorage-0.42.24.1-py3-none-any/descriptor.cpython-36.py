...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/descriptor.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 41736 bytes
...
if api_implementation.Type() == 'cpp':
    ...
    class TypeTransformationError(Error):
        __doc__ = 'Error transforming between python proto type and corresponding C++ type.'
...
class _Lock(object):
    __doc__ = "Wrapper class of threading.Lock(), which is allowed by 'with'."
...
class _NestedDescriptorBase(DescriptorBase):
    __doc__ = 'Common class for descriptors that can be nested.'
    ...
    """
        if self.file is not None:
            if self._serialized_start is not None:
                if self._serialized_end is not None:
...
def MakeDescriptor(desc_proto, package='', build_file_if_cpp=True, syntax=None):
  ...
  """
    ...
    for nested_proto in desc_proto.nested_type:
        ...
        nested_desc = MakeDescriptor(nested_proto, package=('.'.join(full_message_name)),
    ...
    for field_proto in desc_proto.field:
        ...
        field = FieldDescriptor((field_proto.name),
          ...
          (field_proto.label),
          None, nested_desc, enum_desc, None, False, None, options=(_OptionsOrNone(field_proto)),
          has_default_value=False,
          json_name=json_name)
        fields.append(field)

    desc_name = '.'.join(full_message_name)
    return Descriptor((desc_proto.name), desc_name, None, None, fields, (list(nested_types.values())),
      (list(enum_types.values())), [], options=(_OptionsOrNone(desc_proto)))