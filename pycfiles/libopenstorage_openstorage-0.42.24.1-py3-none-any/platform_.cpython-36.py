...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/keyring/keyring/util/platform_.py
...
def _check_old_config_root():
    ...
    if os.path.isfile(config_file_old):
        if not os.path.isfile(config_file_new):
            msg = 'Keyring config exists only in the old location {config_file_old} and should be moved to {config_file_new} to work with this version of keyring.'
