...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cffi/cffi/recompiler.py
# Compiled at: 2020-01-10 16:25:38
# Size of source mod 2**32: 62755 bytes
import os, sys, io
...
class EnumExpr:
    ...
    def as_python_expr(self):
        prim_index = {(1, 0): PRIM_UINT8, (1, 1): PRIM_INT8, 
...
class Recompiler:
    ...
    def collect_step_tables(self):
        ...
        if not len(lst) == len(self._struct_unions):
    ...
    def _generate_cpy_function_decl(self, tp, name):
        if not not self.target_is_python:
            raise AssertionError
        else:
    ...
    def _generate_cpy_function_ctx(self, tp, name):
        ...
        self._lsts['global'].append(GlobalExpr(name, ('_cffi_f_%s' % name), (CffiOp(meth_kind, type_index)),
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None):
        ...
        if tp not in self.ffi._parser._included_declarations and (named_ptr is None or named_ptr not in self.ffi._parser._included_declarations):
            if tp.fldtypes is None:
                ...
                if tp.partial or any(tp.anonymous_struct_fields()):
                    pass
                else:
                    flags.append('_CFFI_F_CHECK_FIELDS')
                if tp.packed:
                    if tp.packed > 1:
                        raise NotImplementedError('%r is declared with \'pack=%r\'; only 0 or 1 are supported in API mode (try to use "...;", which does not require a \'pack\' declaration)' % (
                         tp, tp.packed))
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                if fbitsize >= 0:
                    ...
                    if cname is None or isinstance(fldtype, model.ArrayType) and fldtype.length is None:
                        size = '(size_t)-1'
                    else:
                        size = 'sizeof(((%s)0)->%s)' % (
            ...
            if tp not in self._seen_struct_unions:
                if tp.partial:
                    raise NotImplementedError('internal inconsistency: %r is partial but was not seen at this point' % (
                     tp,))
                elif tp.name.startswith('$'):
                    if tp.name[1:].isdigit():
