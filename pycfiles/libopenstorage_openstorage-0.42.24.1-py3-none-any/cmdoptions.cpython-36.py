...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/pip/pip/_internal/cli/cmdoptions.py
# Compiled at: 2020-01-10 16:25:21
# Size of source mod 2**32: 26839 bytes
"""
...
def no_binary():
    ...
    return Option('--no-binary',
      ...
      help='Do not use binary packages. Can be supplied multiple times, and each time adds to the existing value. Accepts either :all: to disable all binary packages, :none: to empty the set, or one or more package names with commas between them (no colons). Note that some packages are tricky to compile and may fail to install when this option is used on them.')
