...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/internal/json_format_test.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 45977 bytes
...
class JsonFormatTest(JsonFormatBase):
    ...
    def testPartialMessageToJson(self):
        message = json_format_proto3_pb2.TestMessage(string_value='test',
    ...
    def testExtensionErrors(self):
        self.CheckError('{"[extensionField]": {}}', 'Message type proto3.TestMessage does not have extensions')
    ...
    """
        ...
        message.message_set.Extensions[ext2].str = 'foo'
        message_dict = json_format.MessageToDict(message)
        golden_dict = {'messageSet': {'[protobuf_unittest.TestMessageSetExtension1.messageSetExtension]':{'i': 23}, 
                        '[protobuf_unittest.TestMessageSetExtension2.messageSetExtension]':{'str': 'foo'}}}
    ...
    """
        ...
        message_dict = json_format.MessageToDict(message)
        expected_dict = {'[protobuf_unittest.TestExtension.ext]': {'value': 'stuff'}}
    ...
    """
        ...
        message.message_set.Extensions[ext2].str = 'foo'
        message_text = json_format.MessageToJson(message)
        ext1_text = 'protobuf_unittest.TestMessageSetExtension1.messageSetExtension'
        ext2_text = 'protobuf_unittest.TestMessageSetExtension2.messageSetExtension'
        golden_text = '{"messageSet": {    "[%s]": {        "i": 23    },    "[%s]": {        "str": "foo"    }}}' % (
         ext1_text, ext2_text)
    ...
    def testMapFields(self):
        ...
        self.assertEqual(json.loads(json_format.MessageToJson(message, True)), json.loads('{"boolMap": {},"int32Map": {},"int64Map": {},"uint32Map": {},"uint64Map": {},"stringMap": {},"mapMap": {}}'))
        ...
        self.assertEqual(json.loads(json_format.MessageToJson(message, False)), json.loads('{"boolMap": {"false": 2, "true": 1},"int32Map": {"1": 2, "2": 3},"int64Map": {"1": 2, "2": 3},"uint32Map": {"1": 2, "2": 3},"uint64Map": {"1": 2, "2": 3},"stringMap": {"1": 2, "null": 3},"mapMap": {"1": {"boolMap": {"true": 3}}}}'))
    ...
    def testTimestampMessage(self):
        ...
        json_format.Parse(text, parsed_message)
        self.assertEqual(parsed_message.value.seconds, -28800)
        self.assertEqual(parsed_message.value.nanos, 10000000)
        self.assertEqual(parsed_message.repeated_value[0].seconds, -30600.0)
        self.assertEqual(parsed_message.repeated_value[1].seconds, 4980)
    ...
    def testStructMessage(self):
        ...
        self.assertEqual(json.loads(json_format.MessageToJson(message, False)), json.loads('{  "value": {    "address": {      "city": "SFO",       "house_number": 1024    },     "empty_struct": {},     "empty_list": [],     "age": 10,     "name": "Jim",     "attend": true,     "email": null,     "list": [6, "seven", true, false, null, {"subkey2": 9}]  },  "repeatedValue": [{"age": 11}, {}]}'))
    ...
    def testAnyMessage(self):
        ...
        self.CheckParseBack(message, parsed_message)
        test_message = json_format_proto3_pb2.TestMessage(bool_value=True,
          int32_value=20,
          int64_value=(-20),
          uint32_value=20,
          uint64_value=20,
    ...
    def testAnyMessageDescriptorPoolMissingType(self):
        ...
        self.assertEqual('Can not find message descriptor by type_url: type.googleapis.com/protobuf_unittest.OneString.', str(cm.exception))
    ...
    def testParseNull(self):
        ...
        self.CheckError('{"repeatedMessageValue":[null]}', 'Failed to parse repeatedMessageValue field: null is not allowed to be used as an element in a repeated field.')
    ...
    def testFloatPrecision(self):
        ...
        text = '{\n  "floatValue": 1.123457\n}'
        self.assertEqual(json_format.MessageToJson(message, float_precision=7), text)
    ...
    def testParseEnumValue(self):
        ...
        self.assertRaisesRegexp(json_format.ParseError, 'Failed to parse optionalNestedEnum field: Invalid enum value 12345 for enum type protobuf_unittest.TestAllTypes.NestedEnum.', json_format.Parse, '{"optionalNestedEnum": 12345}', message)
    ...
    def testInvalidRepeated(self):
        self.CheckError('{"repeatedInt32Value": 12345}', 'Failed to parse repeatedInt32Value field: repeated field repeatedInt32Value must be in \\[\\] which is 12345.')
    ...
    def testInvalidMap(self):
        ...
        text = '{"stringMap": 0}'
        self.assertRaisesRegexp(json_format.ParseError, 'Failed to parse stringMap field: Map field string_map must be in a dict which is 0.', json_format.Parse, text, message)

    def testInvalidTimestamp(self):
        message = json_format_proto3_pb2.TestTimestamp()
        text = '{"value": "10000-01-01T00:00:00.00Z"}'
        self.assertRaisesRegexp(json_format.ParseError, "Failed to parse value field: time data '10000-01-01T00:00:00' does not match format '%Y-%m-%dT%H:%M:%S'.", json_format.Parse, text, message)
        text = '{"value": "1970-01-01T00:00:00.0123456789012Z"}'
        self.assertRaisesRegexp(json_format.ParseError, 'nanos 0123456789012 more than 9 fractional digits.', json_format.Parse, text, message)
        text = '{"value": "1972-01-01T01:00:00.01+08"}'
        self.assertRaisesRegexp(json_format.ParseError, 'Invalid timezone offset value: \\+08.', json_format.Parse, text, message)
        text = '{"value": "0000-01-01T00:00:00Z"}'
        self.assertRaisesRegexp(json_format.ParseError, 'Failed to parse value field: year (0 )?is out of range.', json_format.Parse, text, message)
    ...
    def testParseDictAnyDescriptorPoolMissingType(self):
        js_dict = {'any_value': {'@type':'type.googleapis.com/proto3.MessageType', 
                       'value':1234}}
        ...
        self.assertEqual(str(cm.exception), 'Failed to parse any_value field: Can not find message descriptor by type_url: type.googleapis.com/proto3.MessageType..')
    ...
    def testSortKeys(self):
        message = json_format_proto3_pb2.TestMessage(bool_value=True, int32_value=1,
          int64_value=3,
          uint32_value=4,
          string_value='bla')
        self.assertEqual(json_format.MessageToJson(message, sort_keys=True), json.dumps({'boolValue':True,  'int32Value':1,  'int64Value':'3',  'uint32Value':4, 
         'stringValue':'bla'},
          indent=2,
          sort_keys=True))
