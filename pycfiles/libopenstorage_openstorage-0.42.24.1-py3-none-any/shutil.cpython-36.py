...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/pip/pip/_vendor/distlib/_backport/shutil.py
...
def copytree(src, dst, symlinks=False, ignore=None, copy_function=copy2, ignore_dangling_symlinks=False):
    ...
    try:
        ...
        if WindowsError is not None:
            if isinstance(why, WindowsError):
                pass
        else:
...
def _make_tarball(base_name, base_dir, compress='gzip', verbose=0, dry_run=0, owner=None, group=None, logger=None):
    ...
    if compress is not None:
        if compress not in compress_ext:
...
_ARCHIVE_FORMATS = {'gztar':(
 ...
 'bztar':(
  _make_tarball, [('compress', 'bzip2')], "bzip2'ed tar-file"), 
 'tar':(
  _make_tarball, [('compress', None)], 'uncompressed tar file'), 
 'zip':(
  _make_zipfile, [], 'ZIP file')}
if _BZ2_SUPPORTED:
...
def register_archive_format(name, function, extra_args=None, description=''):
    ...
    if extra_args is None:
        ...
        raise isinstance(extra_args, (tuple, list)) or TypeError('extra_args needs to be a sequence')
...
_UNPACK_FORMATS = {'gztar':(
  [
   '.tar.gz', '.tgz'], _unpack_tarfile, [], "gzip'ed tar-file"), 
 'tar':(
  [
