...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/grpcio/grpc/_channel.py
# Compiled at: 2020-01-10 16:25:22
# Size of source mod 2**32: 57534 bytes
"""Invocation-side implementation of gRPC Python."""
import copy, functools, logging, os, sys, threading, time, grpc, grpc.experimental
...
def _handle_event(event, state, response_deserializer):
    ...
    for batch_operation in event.batch_operations:
        ...
        if operation_type == cygrpc.OperationType.receive_initial_metadata:
            ...
            if operation_type == cygrpc.OperationType.receive_message:
                serialized_response = batch_operation.message()
                if serialized_response is not None:
                    response = _common.deserialize(serialized_response, response_deserializer)
                    if response is None:
                ...
                if operation_type == cygrpc.OperationType.receive_status_on_client:
                    state.trailing_metadata = batch_operation.trailing_metadata()
                    if state.code is None:
                        code = _common.CYGRPC_STATUS_CODE_TO_STATUS_CODE.get(batch_operation.code())
                        if code is None:
                            state.code = grpc.StatusCode.UNKNOWN
                            state.details = _unknown_code_details(code, batch_operation.details())
                        else:
                            state.code = code
    ...
    def handle_event(event):
        ...
        for callback in callbacks:
            try:
                ...
                logging.error('Exception in callback %s: %s', repr(callback.func), repr(e))
            ...
            with state.condition:
                if state.code is None and not state.cancelled:
                    ...
                    _common.wait((state.condition.wait),
                      _done,
                      spin_cb=(functools.partial(cygrpc.block_if_fork_in_progress, state)))
    ...
    def result(self, timeout=None):
        ...
        with self._state.condition:
            timed_out = _common.wait((self._state.condition.wait),
              (self._is_complete), timeout=timeout)
            if timed_out:
...
def _start_unary_request(request, timeout, request_serializer):
    ...
    if serialized_request is None:
        ...
        return (
         deadline, serialized_request, None)
...
def _stream_unary_invocation_operationses_and_tags(metadata, initial_metadata_flags):
    return tuple((operations, None) for operations in _stream_unary_invocation_operationses(metadata, initial_metadata_flags))
...
class _UnaryUnaryMultiCallable(grpc.UnaryUnaryMultiCallable):
    ...
    def _prepare(self, request, timeout, metadata, wait_for_ready, compression):
        ...
        if serialized_request is None:
            ...
            operations = (
             ...
             cygrpc.ReceiveStatusOnClientOperation(_EMPTY_FLAGS))
            return (
             state, operations, deadline, None)
    ...
    def _blocking(self, request, timeout, metadata, credentials, wait_for_ready, compression):
        ...
        if state is None:
            ...
            call = self._channel.segregated_call(cygrpc.PropagationConstants.GRPC_PROPAGATE_DEFAULTS, self._method, None, _determine_deadline(deadline), metadata, None if credentials is None else credentials._credentials, (
             (
              operations,
              None),), self._context)
    ...
    def future(self, request, timeout=None, metadata=None, credentials=None, wait_for_ready=None, compression=None):
        ...
        if state is None:
            ...
            call = self._managed_call(cygrpc.PropagationConstants.GRPC_PROPAGATE_DEFAULTS, self._method, None, deadline, metadata, None if credentials is None else credentials._credentials, (
    ...
    def __call__(self, request, timeout=None, metadata=None, credentials=None, wait_for_ready=None, compression=None):
        ...
        if serialized_request is None:
            ...
            operationses = (
             ...
             (
              cygrpc.ReceiveInitialMetadataOperation(_EMPTY_FLAGS),))
            call = self._managed_call(cygrpc.PropagationConstants.GRPC_PROPAGATE_DEFAULTS, self._method, None, _determine_deadline(deadline), metadata, None if credentials is None else credentials._credentials, operationses, _event_handler(state, self._response_deserializer), self._context)
            return _MultiThreadedRendezvous(state, call, self._response_deserializer, deadline)
...
class _StreamUnaryMultiCallable(grpc.StreamUnaryMultiCallable):
    ...
    def _blocking(self, request_iterator, timeout, metadata, credentials, wait_for_ready, compression):
        ...
        initial_metadata_flags = _InitialMetadataFlags().with_wait_for_ready(wait_for_ready)
        augmented_metadata = _compression.augment_metadata(metadata, compression)
        call = self._channel.segregated_call(cygrpc.PropagationConstants.GRPC_PROPAGATE_DEFAULTS, self._method, None, _determine_deadline(deadline), augmented_metadata, None if credentials is None else credentials._credentials, _stream_unary_invocation_operationses_and_tags(augmented_metadata, initial_metadata_flags), self._context)
    ...
    def future(self, request_iterator, timeout=None, metadata=None, credentials=None, wait_for_ready=None, compression=None):
        ...
        initial_metadata_flags = _InitialMetadataFlags().with_wait_for_ready(wait_for_ready)
        augmented_metadata = _compression.augment_metadata(metadata, compression)
        call = self._managed_call(cygrpc.PropagationConstants.GRPC_PROPAGATE_DEFAULTS, self._method, None, deadline, augmented_metadata, None if credentials is None else credentials._credentials, _stream_unary_invocation_operationses(metadata, initial_metadata_flags), event_handler, self._context)
    ...
    def __call__(self, request_iterator, timeout=None, metadata=None, credentials=None, wait_for_ready=None, compression=None):
        ...
        operationses = (
         ...
         (
          cygrpc.ReceiveInitialMetadataOperation(_EMPTY_FLAGS),))
        event_handler = _event_handler(state, self._response_deserializer)
        call = self._managed_call(cygrpc.PropagationConstants.GRPC_PROPAGATE_DEFAULTS, self._method, None, _determine_deadline(deadline), augmented_metadata, None if credentials is None else credentials._credentials, operationses, event_handler, self._context)
        _consume_request_iterator(request_iterator, state, call, self._request_serializer, event_handler)
        return _MultiThreadedRendezvous(state, call, self._response_deserializer, deadline)


class _InitialMetadataFlags(int):
    __doc__ = 'Stores immutable initial metadata flags'
    ...
    def with_wait_for_ready(self, wait_for_ready):
        if wait_for_ready is not None:
            ...
            return wait_for_ready or self.__class__(self & ~cygrpc.InitialMetadataFlags.wait_for_ready | cygrpc.InitialMetadataFlags.wait_for_ready_explicitly_set)
        else:
            return self
...
def _channel_managed_call_management(state):
    ...
    def create(flags, method, host, deadline, metadata, credentials, operationses, event_handler, context):
        operationses_and_tags = tuple((operations, event_handler) for operations in operationses)
        with state.lock:
            call = state.channel.integrated_call(flags, method, host, deadline, metadata, credentials, operationses_and_tags, context)
            if state.managed_calls == 0:
                state.managed_calls = 1
...
def _poll_connectivity(state, channel, initial_try_to_connect):
    ...
    with state.lock:
        ...
        callbacks = tuple(callback for callback, unused_but_known_to_be_none_connectivity in state.callbacks_and_connectivities)
        for callback_and_connectivity in state.callbacks_and_connectivities:
            callback_and_connectivity[1] = state.connectivity
    ...
    while 1:
        ...
        cygrpc.block_if_fork_in_progress(state)
        with state.lock:
            if not state.callbacks_and_connectivities:
                if not state.try_to_connect:
                    state.polling = False
    ...
    with state.lock:
        if not state.callbacks_and_connectivities:
            if not state.polling:
                polling_thread = cygrpc.ForkManagedThread(target=_poll_connectivity,
                  args=(
                 state, state.channel, bool(try_to_connect)))
                polling_thread.setDaemon(True)
                polling_thread.start()
                state.polling = True
                state.callbacks_and_connectivities.append([callback, None])
        elif not state.delivering:
...
def _unsubscribe(state, callback):
    with state.lock:
        for index, (subscribed_callback, unused_connectivity) in enumerate(state.callbacks_and_connectivities):
...
def _separate_channel_options(options):
    ...
    return (
     python_options, core_options)


class Channel(grpc.Channel):
    __doc__ = 'A cygrpc.Channel-backed implementation of grpc.Channel.'
    ...
    def __init__(self, target, options, credentials, compression):
        ...
        self._channel = cygrpc.Channel(_common.encode(target), _augment_options(core_options, compression), credentials)
    ...
    def unary_unary(self, method, request_serializer=None, response_deserializer=None):
        return _UnaryUnaryMultiCallable(self._channel, _channel_managed_call_management(self._call_state), _common.encode(method), request_serializer, response_deserializer)

    def unary_stream(self, method, request_serializer=None, response_deserializer=None):
        if self._single_threaded_unary_stream:
            return _SingleThreadedUnaryStreamMultiCallable(self._channel, _common.encode(method), request_serializer, response_deserializer)
        else:
            return _UnaryStreamMultiCallable(self._channel, _channel_managed_call_management(self._call_state), _common.encode(method), request_serializer, response_deserializer)

    def stream_unary(self, method, request_serializer=None, response_deserializer=None):
        return _StreamUnaryMultiCallable(self._channel, _channel_managed_call_management(self._call_state), _common.encode(method), request_serializer, response_deserializer)

    def stream_stream(self, method, request_serializer=None, response_deserializer=None):
        return _StreamStreamMultiCallable(self._channel, _channel_managed_call_management(self._call_state), _common.encode(method), request_serializer, response_deserializer)
