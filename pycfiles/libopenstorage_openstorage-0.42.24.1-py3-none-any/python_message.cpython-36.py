...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/internal/python_message.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 57549 bytes
...
def _AttachFieldHelpers(cls, field_descriptor):
    ...
    if not is_packable:
        ...
        if field_descriptor.containing_type.syntax == 'proto2':
    ...
    def AddDecoder(wiretype, is_packed):
        ...
        if decode_type == _FieldDescriptor.TYPE_ENUM:
            if type_checkers.SupportsOpenEnums(field_descriptor):
            ...
            if is_map_entry:
                ...
                if decode_type == _FieldDescriptor.TYPE_STRING:
                    is_strict_utf8_check = _IsStrictUtf8Check(field_descriptor)
                    field_decoder = decoder.StringDecoder(field_descriptor.number, is_repeated, is_packed, field_descriptor, field_descriptor._default_constructor, is_strict_utf8_check)
                else:
        ...
        cls._decoders_by_tag[tag_bytes] = (
         field_decoder, oneof_descriptor)
    ...
    if _IsMapField(field):
        ...
        if field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:
            ...
            def MakeSubMessageDefault(message):
                assert getattr(message_type, '_concrete_class', None), 'Uninitialized concrete class found for field %r (message type %r)' % (
...
def _AddPropertiesForField(field, cls):
  ...
  """
    if not _FieldDescriptor.MAX_CPPTYPE == 10:
...
def _AddSerializeToStringMethod(message_descriptor, cls):
    ...
    def SerializeToString(self, **kwargs):
        if not self.IsInitialized():
            raise message_mod.EncodeError('Message %s is missing required fields: %s' % (
...
def _AddMergeFromStringMethod(message_descriptor, cls):
    ...
    def MergeFromString(self, serialized):
        if isinstance(serialized, memoryview):
            if six.PY2:
                raise TypeError('memoryview not supported in Python 2 with the pure Python proto implementation: this is to maintain compatibility with the C++ implementation')
  ...
  protocol message class."""
    ...
    """
        ...
        for field, value in list(self._fields.items()):
            if field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:
                if field.label == _FieldDescriptor.LABEL_REPEATED:
                    if field.message_type.has_options:
...
def _AddMergeFromMethod(cls):
    ...
    def MergeFrom(self, msg):
        if not isinstance(msg, cls):
            raise TypeError('Parameter to MergeFrom() must be instance of same class: expected %s got %s.' % (
             cls.__name__, msg.__class__.__name__))
        else:
...
def _AddWhichOneofMethod(message_descriptor, cls):
    ...
    def WhichOneof(self, oneof_name):
        ...
        if nested_field is not None:
            if self.HasField(nested_field.name):
...
class _Listener(object):
    __doc__ = "MessageListener implementation that a parent message registers with its\n  child message.\n\n  In order to support semantics like:\n\n    foo.bar.baz.qux = 23\n    assert foo.HasField('bar')\n\n  ...child objects must have back references to their parents.\n  This helper class is at the heart of this support.\n  "
...
class _OneofListener(_Listener):
    __doc__ = 'Special listener implementation for setting composite oneof fields.'
