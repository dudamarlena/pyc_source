...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/protobuf/google/protobuf/internal/descriptor_pool_test.py
# Compiled at: 2020-01-10 16:25:26
# Size of source mod 2**32: 49650 bytes
...
class DescriptorPoolTestBase(object):
    ...
    def testFindFileContainingSymbol(self):
        ...
        file_desc5 = self.pool.FindFileContainingSymbol('protobuf_unittest.TestService')
        ...
        if not descriptor_pool.Default().FindFileContainingSymbol('google.protobuf.python.internal.Factory2Message.one_more_field'):
            raise AssertionError
        else:
            assert descriptor_pool.Default().FindFileContainingSymbol('google.protobuf.python.internal.another_field')
            assert descriptor_pool.Default().FindFileContainingSymbol('protobuf_unittest.TestService')
        file_desc6 = self.pool.FindFileContainingSymbol('google.protobuf.python.internal.Factory1Message.list_value')
        self.assertIsInstance(file_desc6, descriptor.FileDescriptor)
        self.assertEqual('google/protobuf/internal/factory_test1.proto', file_desc6.name)
        file_desc7 = self.pool.FindFileContainingSymbol('google.protobuf.python.internal.FACTORY_1_VALUE_0')
        self.assertIsInstance(file_desc7, descriptor.FileDescriptor)
        self.assertEqual('google/protobuf/internal/factory_test1.proto', file_desc7.name)
        file_desc8 = self.pool.FindFileContainingSymbol('protobuf_unittest.TestAllTypes.FOO')
        ...
        self.assertRaises(KeyError, self.pool.FindFileContainingSymbol, 'google.protobuf.python.internal.Factory1Message.none_field')
    ...
    def testFindOneofByName(self):
        oneof = self.pool.FindOneofByName('google.protobuf.python.internal.Factory2Message.oneof_field')
    ...
    def testConflictRegister(self):
        ...
        if api_implementation.Type() == 'cpp':
            ...
            pool.AddDescriptor(file_descriptor.message_types_by_name['TestAllTypes'])
            pool.AddEnumDescriptor(file_descriptor.enum_types_by_name['ForeignEnum'])
            pool.AddServiceDescriptor(file_descriptor.services_by_name['TestService'])
            pool.AddExtensionDescriptor(file_descriptor.extensions_by_name['optional_int32_extension'])
...
class DefaultDescriptorPoolTest(DescriptorPoolTestBase, unittest.TestCase):
    ...
    def testFindMethods(self):
        self.assertIs(self.pool.FindFileByName('google/protobuf/unittest.proto'), unittest_pb2.DESCRIPTOR)
        self.assertIs(self.pool.FindMessageTypeByName('protobuf_unittest.TestAllTypes'), unittest_pb2.TestAllTypes.DESCRIPTOR)
        self.assertIs(self.pool.FindFieldByName('protobuf_unittest.TestAllTypes.optional_int32'), unittest_pb2.TestAllTypes.DESCRIPTOR.fields_by_name['optional_int32'])
        self.assertIs(self.pool.FindEnumTypeByName('protobuf_unittest.ForeignEnum'), unittest_pb2.ForeignEnum.DESCRIPTOR)
        self.assertIs(self.pool.FindExtensionByName('protobuf_unittest.optional_int32_extension'), unittest_pb2.DESCRIPTOR.extensions_by_name['optional_int32_extension'])
        self.assertIs(self.pool.FindOneofByName('protobuf_unittest.TestAllTypes.oneof_field'), unittest_pb2.TestAllTypes.DESCRIPTOR.oneofs_by_name['oneof_field'])
        self.assertIs(self.pool.FindServiceByName('protobuf_unittest.TestService'), unittest_pb2.DESCRIPTOR.services_by_name['TestService'])
...
class CreateDescriptorPoolTest(DescriptorPoolTestBase, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pool.Add(descriptor_pb2.FileDescriptorProto.FromString(no_package_pb2.DESCRIPTOR.serialized_pb))


@testing_refleaks.TestCase
class SecondaryDescriptorFromDescriptorDB(DescriptorPoolTestBase, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db.Add(descriptor_pb2.FileDescriptorProto.FromString(unittest_import_public_pb2.DESCRIPTOR.serialized_pb))
        self.db.Add(descriptor_pb2.FileDescriptorProto.FromString(unittest_import_pb2.DESCRIPTOR.serialized_pb))
        self.db.Add(descriptor_pb2.FileDescriptorProto.FromString(unittest_pb2.DESCRIPTOR.serialized_pb))
        self.db.Add(descriptor_pb2.FileDescriptorProto.FromString(no_package_pb2.DESCRIPTOR.serialized_pb))
    ...
    def testErrorCollector(self):
        ...
        self.assertRaisesRegexp(KeyError, 'SubMessage', self.pool.FindMessageTypeByName, 'collector.ErrorMessage')
        self.assertRaisesRegexp(KeyError, 'SubMessage', self.pool.FindFileByName, 'error_file')
        with self.assertRaises(KeyError) as (exc):
            self.pool.FindFileByName('none_file')
        self.assertIn(str(exc.exception), ("'none_file'", '"Couldn\'t find file none_file"'))
        if api_implementation.Type() == 'cpp':
            ...
            self.assertEqual(str(exc.exception), "'Couldn\\'t build file for message collector.ErrorMessage\\n" + error_msg)
            with self.assertRaises(KeyError) as (exc):
                self.pool.FindFieldByName('collector.ErrorMessage.nested_message_field')
            self.assertEqual(str(exc.exception), "'Couldn\\'t build file for field collector.ErrorMessage.nested_message_field\\n" + error_msg)
            with self.assertRaises(KeyError) as (exc):
                self.pool.FindEnumTypeByName('collector.MyEnum')
            self.assertEqual(str(exc.exception), "'Couldn\\'t build file for enum collector.MyEnum\\n" + error_msg)
            with self.assertRaises(KeyError) as (exc):
                self.pool.FindFileContainingSymbol('collector.MyEnumValue')
            self.assertEqual(str(exc.exception), "'Couldn\\'t build file for symbol collector.MyEnumValue\\n" + error_msg)
            with self.assertRaises(KeyError) as (exc):
                self.pool.FindOneofByName('collector.ErrorMessage.MyOneof')
            self.assertEqual(str(exc.exception), "'Couldn\\'t build file for oneof collector.ErrorMessage.MyOneof\\n" + error_msg)
...
class AddDescriptorTest(unittest.TestCase):
    ...
    def testCustomDescriptorPool(self):
        ...
        file_proto = descriptor_pb2.FileDescriptorProto(name='some/filename/container.proto')
        ...
        pool = descriptor_pool.DescriptorPool()
        pool.Add(file_proto)
        self.assertEqual('TopMessage', pool.FindMessageTypeByName('TopMessage').name)
...
TEST1_FILE = ProtoFile('google/protobuf/internal/descriptor_pool_test1.proto', 'google.protobuf.python.internal', {'DescriptorPoolTest1':MessageType({'NestedEnum':EnumType([('ALPHA', 1), ('BETA', 2)]), 
  'NestedMessage':MessageType({'NestedEnum':EnumType([('EPSILON', 5), ('ZETA', 6)]), 
   'DeepNestedMessage':MessageType({'NestedEnum': EnumType([('ETA', 7), ('THETA', 8)])}, [
    (
  ...
  (
   ...
   is_extendable=True), 
 'DescriptorPoolTest2':MessageType({'NestedEnum':EnumType([('GAMMA', 3), ('DELTA', 4)]), 
  'NestedMessage':MessageType({'NestedEnum':EnumType([('IOTA', 9), ('KAPPA', 10)]), 
   'DeepNestedMessage':MessageType({'NestedEnum': EnumType([('LAMBDA', 11), ('MU', 12)])}, [
    (
  ...
  (
   'nested_message', MessageField(2, 'NestedMessage'))])})
TEST2_FILE = ProtoFile('google/protobuf/internal/descriptor_pool_test2.proto',
  'google.protobuf.python.internal',
  {'DescriptorPoolTest3': MessageType({'NestedEnum':EnumType([('NU', 13), ('XI', 14)]), 
                         'NestedMessage':MessageType({'NestedEnum':EnumType([('OMICRON', 15), ('PI', 16)]), 
                          'DeepNestedMessage':MessageType({'NestedEnum': EnumType([('RHO', 17), ('SIGMA', 18)])}, [
                           (
