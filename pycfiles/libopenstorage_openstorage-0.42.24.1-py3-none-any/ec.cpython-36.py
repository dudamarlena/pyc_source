...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cryptography/cryptography/hazmat/primitives/asymmetric/ec.py
...
class EllipticCurvePublicKey(object):
    ...
    def from_encoded_point(cls, curve, data):
        ...
        if six.indexbytes(data, 0) not in (2, 3, 4):
...
def derive_private_key(private_value, curve, backend):
    if not isinstance(private_value, six.integer_types):
        ...
        raise isinstance(curve, EllipticCurve) or TypeError('curve must provide the EllipticCurve interface.')
...
class EllipticCurvePublicNumbers(object):
    ...
    def encode_point(self):
        ...
        return b'\x04' + utils.int_to_bytes(self.x, byte_length) + utils.int_to_bytes(self.y, byte_length)
