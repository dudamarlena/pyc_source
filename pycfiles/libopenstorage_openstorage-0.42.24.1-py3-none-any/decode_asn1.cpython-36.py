...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cryptography/cryptography/hazmat/backends/openssl/decode_asn1.py
# Compiled at: 2020-01-10 16:25:38
# Size of source mod 2**32: 33303 bytes
...
def _decode_general_name(backend, gn):
    if gn.type == backend._lib.GEN_DNS:
        ...
        if gn.type == backend._lib.GEN_URI:
            ...
            if gn.type == backend._lib.GEN_RID:
...
class _X509ExtensionParser(object):
    ...
    def parse(self, backend, x509_obj):
        ...
        for i in range(self.ext_count(backend, x509_obj)):
            ...
            critical = crit == 1
            oid = x509.ObjectIdentifier(_obj2txt(backend, backend._lib.X509_EXTENSION_get_object(ext)))
            if oid in seen_oids:
                raise x509.DuplicateExtension('Duplicate {} extension found'.format(oid), oid)
            if oid == ExtensionOID.TLS_FEATURE:
                ...
                value = x509.TLSFeature([_TLS_FEATURE_TYPE_TO_ENUM[x] for x in parsed])
                extensions.append(x509.Extension(oid, critical, value))
                seen_oids.add(oid)
                continue
            else:
                if oid == ExtensionOID.PRECERT_POISON:
                    data = backend._lib.X509_EXTENSION_get_data(ext)
                    reader = DERReader(_asn1_string_to_bytes(backend, data))
                    reader.read_single_element(NULL).check_empty()
                    extensions.append(x509.Extension(oid, critical, x509.PrecertPoison()))
                    seen_oids.add(oid)
                    continue
                try:
                    ...
                    if ext_data == backend._ffi.NULL:
                        ...
                        raise ValueError("The {} extension is invalid and can't be parsed".format(oid))
