...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/twine/twine/repository.py
...
class Repository:
    ...
    def __init__(self, repository_url: str, username: Optional[str], password: Optional[str], disable_progress_bar: bool=False) -> None:
        self.url = repository_url
        self.session = requests.session()
        self.session.auth = (username or '', password or '') if username or password else None
    ...
    def _make_user_agent_string() -> str:
        ...
        return user_agent.UserAgentBuilder('twine', twine.__version__).include_extras(dependencies).include_implementation().build()

    def close(self):
        self.session.close()

    @staticmethod
    def _convert_data_to_list_of_tuples(data: Dict[(str, MetadataValue)]) -> List[Tuple[(str, MetadataValue)]]:
        data_to_send = []
    ...
    def _upload(self, package: PackageFile) -> Response:
        data = package.metadata_dictionary()
        data.update({':action':'file_upload', 
         'protocol_version':'1'})
        data_to_send = self._convert_data_to_list_of_tuples(data)
        ...
        with open(package.filename, 'rb') as (fp):
            ...
            encoder = MultipartEncoder(data_to_send)
            with ProgressBar(total=(encoder.len), unit='B',
              unit_scale=True,
              unit_divisor=1024,
              miniters=1,
              file=(sys.stdout),
    ...
    def upload(self, package: PackageFile, max_redirects: int=5) -> Response:
        ...
        while number_of_redirects < max_redirects:
            ...
            if 500 <= resp.status_code < 600:
                ...
                print('Received "{status_code}: {reason}" Package upload appears to have failed.  Retry {retry} of 5'.format(status_code=(resp.status_code),
                  reason=(resp.reason),
    ...
    def package_is_uploaded(self, package: PackageFile, bypass_cache: bool=False) -> bool:
    ...
    def release_urls(self, packages):
        if self.url.startswith(WAREHOUSE):
            ...
            if self.url.startswith(TEST_WAREHOUSE):
                ...
                return set()
        return {'{}project/{}/{}/'.format(url, package.safe_name, package.metadata.version) for package in packages}
