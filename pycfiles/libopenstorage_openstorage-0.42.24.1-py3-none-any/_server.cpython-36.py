...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/grpcio/grpc/_server.py
# Compiled at: 2020-01-10 16:25:22
# Size of source mod 2**32: 36603 bytes
"""Service-side implementation of gRPC Python."""
import collections, enum, logging, threading, time
...
def _application_code(code):
    ...
    if cygrpc_code is None:
        return cygrpc.StatusCode.unknown
    else:
        return cygrpc_code
    ...
    with state.condition:
        if state.compression_algorithm:
            compression_metadata = (
             _compression.compression_algorithm_to_metadata(state.compression_algorithm),)
            if metadata is None:
...
def _abort(state, call, code, details):
    if state.client is not _CANCELLED:
        ...
        if state.initial_metadata_allowed:
            operations = (_get_initial_metadata_operation(state, None),
             cygrpc.SendStatusFromServerOperation(state.trailing_metadata, effective_code, effective_details, _EMPTY_FLAGS))
            token = _SEND_INITIAL_METADATA_AND_SEND_STATUS_FROM_SERVER_TOKEN
        else:
            operations = (cygrpc.SendStatusFromServerOperation(state.trailing_metadata, effective_code, effective_details, _EMPTY_FLAGS),)
...
class _Context(grpc.ServicerContext):
    ...
    def peer_identity_key(self):
        ...
        if id_key is None:
            ...
            return _common.decode(id_key)

    def auth_context(self):
        return {_common.decode(key):value for key, value in six.iteritems(cygrpc.auth_context(self._rpc_event.call))}
...
class _RequestIterator(object):
    ...
    def _raise_or_start_receive_message(self):
        if self._state.client is _CANCELLED:
            ...
            if not _is_rpc_state_active(self._state):
                raise StopIteration()
            else:
                self._call.start_server_batch((
                 cygrpc.ReceiveMessageOperation(_EMPTY_FLAGS),), _receive_message(self._state, self._call, self._request_deserializer))
    ...
    def _look_for_request(self):
        if self._state.client is _CANCELLED:
            ...
            if _RECEIVE_MESSAGE_TOKEN not in self._state.due:
                raise StopIteration()
        else:
    ...
    def unary_request():
        with state.condition:
            ...
            rpc_event.call.start_server_batch((
             cygrpc.ReceiveMessageOperation(_EMPTY_FLAGS),), _receive_message(state, rpc_event.call, request_deserializer))
...
def _send_response(rpc_event, state, serialized_response):
    with state.condition:
        if not _is_rpc_state_active(state):
            ...
            if state.initial_metadata_allowed:
                operations = (_get_initial_metadata_operation(state, None),
                 cygrpc.SendMessageOperation(serialized_response, _get_send_message_op_flags_from_state(state)))
...
def _status(rpc_event, state, serialized_response):
    with state.condition:
        if state.client is not _CANCELLED:
            ...
            operations = [
             cygrpc.SendStatusFromServerOperation(state.trailing_metadata, code, details, _EMPTY_FLAGS)]
            if state.initial_metadata_allowed:
                operations.append(_get_initial_metadata_operation(state, None))
            if serialized_response is not None:
                operations.append(cygrpc.SendMessageOperation(serialized_response, _get_send_message_op_flags_from_state(state)))
            rpc_event.call.start_server_batch(operations, _send_status_from_server(state, _SEND_STATUS_FROM_SERVER_TOKEN))
            state.statused = True
            _reset_per_message_state(state)
...
def _select_thread_pool_for_behavior(behavior, default_thread_pool):
    if hasattr(behavior, 'experimental_thread_pool'):
        if isinstance(behavior.experimental_thread_pool, futures.ThreadPoolExecutor):
...
def _handle_unary_unary(rpc_event, state, method_handler, default_thread_pool):
    ...
    return thread_pool.submit(_unary_response_in_pool, rpc_event, state, method_handler.unary_unary, unary_request, method_handler.request_deserializer, method_handler.response_serializer)
...
def _handle_stream_unary(rpc_event, state, method_handler, default_thread_pool):
    ...
    thread_pool = _select_thread_pool_for_behavior(method_handler.stream_unary, default_thread_pool)
...
def _handle_with_method_handler(rpc_event, method_handler, thread_pool):
    ...
    with state.condition:
        rpc_event.call.start_server_batch((
         cygrpc.ReceiveCloseOnServerOperation(_EMPTY_FLAGS),), _receive_close_on_server(state))
    ...
    if not rpc_event.success:
        ...
        if rpc_event.call_details.method is not None:
            try:
                ...
                _LOGGER.exception(details)
                return (
                 _reject_rpc(rpc_event, cygrpc.StatusCode.unknown, b'Error in service handler!'), None)

            if method_handler is None:
                return (
                 _reject_rpc(rpc_event, cygrpc.StatusCode.unimplemented, b'Method not found!'), None)
            else:
                if concurrency_exceeded:
                    return (_reject_rpc(rpc_event, cygrpc.StatusCode.resource_exhausted, b'Concurrent RPC limit exceeded!'), None)
...
def _request_call(state):
    state.server.request_call(state.completion_queue, state.completion_queue, _REQUEST_CALL_TAG)
...
def _process_event_and_continue(state, event):
    ...
    if event.tag is _REQUEST_CALL_TAG:
        with state.lock:
...
def _validate_generic_rpc_handlers(generic_rpc_handlers):
    for generic_rpc_handler in generic_rpc_handlers:
        ...
        if service_attribute is None:
            raise AttributeError('"{}" must conform to grpc.GenericRpcHandler type but does not have "service" method!'.format(generic_rpc_handler))
...
class _Server(grpc.Server):
    ...
    def __init__(self, thread_pool, generic_handlers, interceptors, options, maximum_concurrent_rpcs, compression):
        ...
        self._state = _ServerState(completion_queue, server, generic_handlers, _interceptor.service_pipeline(interceptors), thread_pool, maximum_concurrent_rpcs)
...
def create_server(thread_pool, generic_rpc_handlers, interceptors, options, maximum_concurrent_rpcs, compression):
    _validate_generic_rpc_handlers(generic_rpc_handlers)
    return _Server(thread_pool, generic_rpc_handlers, interceptors, options, maximum_concurrent_rpcs, compression)