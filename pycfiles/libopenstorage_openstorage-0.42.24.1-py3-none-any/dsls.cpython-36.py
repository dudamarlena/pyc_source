...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/Pygments/pygments/lexers/dsls.py
# Compiled at: 2020-01-10 16:25:35
# Size of source mod 2**32: 35836 bytes
"""
...
__all__ = [
 'ProtoBufLexer', 'ZeekLexer', 'PuppetLexer', 'RslLexer',
...
class ProtoBufLexer(RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       '/(\\\\\\n)?\\*(.|\\n)*?\\*(\\\\\\n)?/', Comment.Multiline),
      (
       words(('import', 'option', 'optional', 'required', 'repeated', 'reserved', 'default',
       'packed', 'ctype', 'extensions', 'to', 'max', 'rpc', 'returns', 'oneof'),
         prefix='\\b', suffix='\\b'),
       Keyword),
      (
...
class ZeekLexer(RegexLexer):
    ...
    tokens = {'root':[
     ...
     'directives':[
      (
       '@(load-plugin|load-sigs|load|unload)\\b.*$', Comment.Preproc),
      (
       '@(DEBUG|DIR|FILENAME|deprecated|if|ifdef|ifndef|else|endif)\\b', Comment.Preproc),
      (
       '(@prefixes)\\s*(\\+?=).*$', Comment.Preproc)], 
     'attributes':[
      (
       words(('redef', 'priority', 'log', 'optional', 'default', 'add_func', 'delete_func',
       'expire_func', 'read_expire', 'write_expire', 'create_expire', 'synchronized',
       'persistent', 'rotate_interval', 'rotate_size', 'encrypt', 'raw_output', 'mergeable',
       'error_handler', 'type_column', 'deprecated'),
         prefix='&',
         suffix='\\b'),
       Keyword.Pseudo)], 
     'types':[
      (
       words(('any', 'enum', 'record', 'set', 'table', 'vector', 'function', 'hook', 'event',
       'addr', 'bool', 'count', 'double', 'file', 'int', 'interval', 'pattern', 'port',
       'string', 'subnet', 'time'),
         suffix='\\b'),
       Keyword.Type),
      (
       '(opaque)(\\s+)(of)(\\s+)((?:[A-Za-z_]\\w*)(?:::(?:[A-Za-z_]\\w*))*)\\b',
       bygroups(Keyword.Type, Text, Operator.Word, Text, Keyword.Type)),
      (
       '(type)(\\s+)((?:[A-Za-z_]\\w*)(?:::(?:[A-Za-z_]\\w*))*)(\\s*)(:)(\\s*)\\b(record|enum)\\b',
       bygroups(Keyword, Text, Name.Class, Text, Operator, Text, Keyword.Type)),
      (
       '(type)(\\s+)((?:[A-Za-z_]\\w*)(?:::(?:[A-Za-z_]\\w*))*)(\\s*)(:)',
       bygroups(Keyword, Text, Name, Text, Operator)),
      (
       '(redef)(\\s+)(record|enum)(\\s+)((?:[A-Za-z_]\\w*)(?:::(?:[A-Za-z_]\\w*))*)\\b',
       bygroups(Keyword, Text, Keyword.Type, Text, Name.Class))], 
     'keywords':[
      (
       words(('redef', 'export', 'if', 'else', 'for', 'while', 'return', 'break', 'next',
       'continue', 'fallthrough', 'switch', 'default', 'case', 'add', 'delete', 'when',
       'timeout', 'schedule'),
         suffix='\\b'),
       Keyword),
      (
       '(print)\\b', Keyword),
      (
       '(global|local|const|option)\\b', Keyword.Declaration),
      (
       '(module)(\\s+)(([A-Za-z_]\\w*)(?:::([A-Za-z_]\\w*))*)\\b',
     ...
     'literals':[
      ...
      (
       '\\d{1,5}/(udp|tcp|icmp|unknown)\\b', Number),
      (
       '(\\d{1,3}.){3}(\\d{1,3})\\b', Number),
      (
       '\\[([0-9a-fA-F]{0,4}:){2,7}([0-9a-fA-F]{0,4})?((\\d{1,3}.){3}(\\d{1,3}))?\\]', Number),
      (
       '0[xX]' + _hex + '+\\b', Number.Hex),
      (
       _float + '\\s*(day|hr|min|sec|msec|usec)s?\\b', Number.Float),
      (
       _float + '\\b', Number.Float),
      (
       '(\\d+)\\b', Number.Integer),
      (
       _h + '(\\.' + _h + ')+', String)], 
     ...
     'identifiers':[
      (
       '([a-zA-Z_]\\w*)(::)', bygroups(Name, Punctuation)),
      (
       '[a-zA-Z_]\\w*', Name)], 
     'string':[
      (
       '\\\\.', String.Escape),
      (
       '%-?[0-9]*(\\.[0-9]+)?[DTdxsefg]', String.Escape),
      (
       '"', String, '#pop'),
      (
       '.', String)], 
