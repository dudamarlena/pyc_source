...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cryptography/cryptography/hazmat/backends/openssl/encode_asn1.py
# Compiled at: 2020-01-10 16:25:38
# Size of source mod 2**32: 23593 bytes
...
def _encode_name_entry(backend, attribute):
    if attribute._type is _ASN1Type.BMPString:
        ...
        if attribute._type is _ASN1Type.UniversalString:
            value = attribute.value.encode('utf_32_be')
        else:
            value = attribute.value.encode('utf8')
    obj = _txt2obj_gc(backend, attribute.oid.dotted_string)
    name_entry = backend._lib.X509_NAME_ENTRY_create_by_OBJ(backend._ffi.NULL, obj, attribute._type.value, value, len(value))
    return name_entry
        ...
        if policy_info.policy_qualifiers:
            ...
            for qualifier in policy_info.policy_qualifiers:
                ...
                if isinstance(qualifier, six.text_type):
                    pqi.pqualid = _txt2obj(backend, x509.OID_CPS_QUALIFIER.dotted_string)
                    pqi.d.cpsuri = _encode_asn1_str(backend, qualifier.encode('ascii'))
                else:
                    assert isinstance(qualifier, x509.UserNotice)
                    pqi.pqualid = _txt2obj(backend, x509.OID_CPS_USER_NOTICE.dotted_string)
                    un = backend._lib.USERNOTICE_new()
                    backend.openssl_assert(un != backend._ffi.NULL)
                    pqi.d.usernotice = un
                    if qualifier.explicit_text:
                        un.exptext = _encode_asn1_utf8_str(backend, qualifier.explicit_text)
                    un.noticeref = _encode_notice_reference(backend, qualifier.notice_reference)
...
def _encode_authority_key_identifier(backend, authority_keyid):
    ...
    if authority_keyid.authority_cert_issuer is not None:
        akid.issuer = _encode_general_names(backend, authority_keyid.authority_cert_issuer)
    if authority_keyid.authority_cert_serial_number is not None:
        akid.serial = _encode_asn1_int(backend, authority_keyid.authority_cert_serial_number)
    return akid


def _encode_basic_constraints(backend, basic_constraints):
    constraints = backend._lib.BASIC_CONSTRAINTS_new()
    constraints = backend._ffi.gc(constraints, backend._lib.BASIC_CONSTRAINTS_free)
    constraints.ca = 255 if basic_constraints.ca else 0
    if basic_constraints.ca:
        if basic_constraints.path_length is not None:
            constraints.pathlen = _encode_asn1_int(backend, basic_constraints.path_length)
    return constraints
            ...
            if isinstance(name, x509.DirectoryName):
                ...
                if isinstance(name, x509.IPAddress):
                    ...
                    if isinstance(name.value, ipaddress.IPv4Network):
                        ...
                        if isinstance(name.value, ipaddress.IPv6Network):
...
def _encode_policy_constraints(backend, policy_constraints):
    ...
    if policy_constraints.require_explicit_policy is not None:
        pc.requireExplicitPolicy = _encode_asn1_int(backend, policy_constraints.require_explicit_policy)
    if policy_constraints.inhibit_policy_mapping is not None:
        pc.inhibitPolicyMapping = _encode_asn1_int(backend, policy_constraints.inhibit_policy_mapping)
    return pc
...
_EXTENSION_ENCODE_HANDLERS = {ExtensionOID.BASIC_CONSTRAINTS: _encode_basic_constraints, 
 ...
 ExtensionOID.AUTHORITY_INFORMATION_ACCESS: _encode_authority_information_access, 
