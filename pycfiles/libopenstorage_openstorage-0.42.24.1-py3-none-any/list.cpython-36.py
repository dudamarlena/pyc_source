...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/pip/pip/_internal/commands/list.py
...
class ListCommand(IndexGroupCommand):
    ...
    def output_package_listing(self, packages, options):
        ...
        if options.list_format == 'columns':
            if packages:
                data, header = format_for_columns(packages, options)
                self.output_package_listing_columns(data, header)
        if options.list_format == 'freeze':
            for dist in packages:
                if options.verbose >= 1:
                    write_output('%s==%s (%s)', dist.project_name, dist.version, dist.location)
                else:
