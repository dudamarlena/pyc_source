...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/jeepney/jeepney/bus_messages.py
...
class MatchRule:
    __doc__ = "Construct a match rule to subscribe to DBus messages.\n    \n    e.g.::\n    \n        mr = MatchRule(interface='org.freedesktop.DBus', member='NameOwnerChanged',\n                       type='signal')\n        msg = add_match(mr)\n        # Send this message to subscribe to the signal\n    "

    def __init__(self, *, type=None, sender=None, interface=None, member=None, path=None, path_namespace=None, destination=None, eavesdrop=False):
        self.conditions = c = {}
        if type:
            c['type'] = type
        if sender:
    ...
    def add_arg_condition(self, argno, value, kind='string'):
        ...
        """
        if kind not in frozenset({'path', 'string', 'namespace'}):
    ...
    def serialise(self):
        ...
        for k, v in sorted(self.conditions.items()):
            parts.append('{}={}'.format(k, v.replace("'", "\\'")))
