...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-ed191__6/cryptography/cryptography/x509/extensions.py
# Compiled at: 2020-01-10 16:25:37
# Size of source mod 2**32: 50781 bytes
from __future__ import absolute_import, division, print_function
import abc, datetime, hashlib, ipaddress, warnings
...
class AuthorityKeyIdentifier(object):
    ...
    def from_issuer_subject_key_identifier(cls, ski):
        if isinstance(ski, SubjectKeyIdentifier):
            ...
            warnings.warn('Extension objects are deprecated as arguments to from_issuer_subject_key_identifier and support will be removed soon. Please migrate to passing a SubjectKeyIdentifier directly.',
...
class BasicConstraints(object):
    ...
    def __init__(self, ca, path_length):
        if not isinstance(ca, bool):
            ...
            if path_length is not None:
                if not isinstance(path_length, six.integer_types) or path_length < 0:
        ...
        if full_name:
            if relative_name:
                ...
                if full_name:
                    ...
                    if reasons:
                        if not isinstance(reasons, frozenset) or not all(isinstance(x, ReasonFlags) for x in reasons):
            ...
            if reasons:
                if not crl_issuer:
                    if not (full_name or relative_name):
                        raise ValueError('You must supply crl_issuer, full_name, or relative_name when reasons is not None')
...
class PolicyConstraints(object):
    ...
    def __init__(self, require_explicit_policy, inhibit_policy_mapping):
        if require_explicit_policy is not None:
            if not isinstance(require_explicit_policy, six.integer_types):
                raise TypeError('require_explicit_policy must be a non-negative integer or None')
            if inhibit_policy_mapping is not None:
                if not isinstance(inhibit_policy_mapping, six.integer_types):
...
class PolicyInformation(object):
    ...
    def __init__(self, policy_identifier, policy_qualifiers):
        ...
        if policy_qualifiers:
            ...
            if not all(isinstance(x, (six.text_type, UserNotice)) for x in policy_qualifiers):
...
class UserNotice(object):
    ...
    def __init__(self, notice_reference, explicit_text):
        if notice_reference:
            if not isinstance(notice_reference, NoticeReference):
...
class KeyUsage(object):
    ...
    def __repr__(self):
        ...
        return '<KeyUsage(digital_signature={0.digital_signature}, content_commitment={0.content_commitment}, key_encipherment={0.key_encipherment}, data_encipherment={0.data_encipherment}, key_agreement={0.key_agreement}, key_cert_sign={0.key_cert_sign}, crl_sign={0.crl_sign}, encipher_only={1}, decipher_only={2})>'.format(self, encipher_only, decipher_only)
...
class NameConstraints(object):
    ...
    def __init__(self, permitted_subtrees, excluded_subtrees):
        if permitted_subtrees is not None:
            ...
            if permitted_subtrees is None:
                if excluded_subtrees is None:
...
class IssuingDistributionPoint(object):
    ...
    def __init__(self, full_name, relative_name, only_contains_user_certs, only_contains_ca_certs, only_some_reasons, indirect_crl, only_contains_attribute_certs):
        if only_some_reasons:
            if not isinstance(only_some_reasons, frozenset) or not all(isinstance(x, ReasonFlags) for x in only_some_reasons):
            ...
            if not (isinstance(only_contains_user_certs, bool) and isinstance(only_contains_ca_certs, bool) and isinstance(indirect_crl, bool) and isinstance(only_contains_attribute_certs, bool)):
            ...
            raise any([
             ...
             relative_name, only_some_reasons]) or ValueError('Cannot create empty extension: if only_contains_user_certs, only_contains_ca_certs, indirect_crl, and only_contains_attribute_certs are all False, then either full_name, relative_name, or only_some_reasons must have a value.')
