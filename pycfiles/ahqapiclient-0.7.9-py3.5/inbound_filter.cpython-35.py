...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ahqapiclient/resources/inbound_filter.py
...
class InboundFilter(Resource):
    ...
    def make_doc(self, type, match_type, filter_value, action, type_attr=None, action_attr=None):
        return {'type': type, 
         'type_attr': type_attr, 
    ...
    def create_inbound_filter(self, type, match_type, filter_value, action, type_attr=None, action_attr=None):
        return self.post(path=self.rurl(), data=self.make_doc(type, match_type, filter_value, action, type_attr, action_attr))

    def get_inbound_filter(self, _id):
        return self.get(path=self.rurl(_id))

    def update_inbound_filter(self, _id, type, match_type, filter_value, action, type_attr=None, action_attr=None):
        return self.put(path=self.rurl(_id), data=self.make_doc(type, match_type, filter_value, action, type_attr, action_attr))
        ...
        for f in filters:
            ...
            if action == 'forward':
                if f['action_attr']:
                    ...
                    raise ValueError('Forward action missing the mail recipient.')
                if _type == 'header':
                    if f['type_attr']:
