...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ahqapiclient/http_client.py
# Compiled at: 2017-10-26 06:53:10
# Size of source mod 2**32: 9507 bytes
...
def get_tls_certs(endpoint, tmp_path='/tmp'):
    sum_ca = adler32(endpoint['auth_options']['ca']) & 4294967295
    sum_cert = adler32(endpoint['auth_options']['cert']) & 4294967295
...
class HTTPClient(object):
    ...
    DEFAULT_USER_AGENT = 'abusehq-api-client/0.4.0'

    def __init__(self, default_endpoint, timeout=60.0, session=None):
        ...
        if session:
            ...
            self._HTTPClient__session = Session()
            self._HTTPClient__session.mount('https://', HTTPAdapter(max_retries=2, pool_connections=100, pool_maxsize=100))
    ...
    def __set_auth(self, request, endpoint):
        ...
        if auth_method == 'NONE':
            ...
            if auth_method == 'JWT':
                ...
                if auth_method == 'HTTP':
                    request.headers['Authorization'] = 'Basic %s' % b64encode('%s:%s' % (auth_options['user'], auth_options['password']))
                else:
                    if auth_method == 'HMAC' or auth_method == 'INTERNAL':
    ...
    def __send(self, request, endpoint, raw):
        request = self._HTTPClient__set_default_headers(request)
        request = self._HTTPClient__set_auth(request, endpoint)
        ...
        while True:
            try:
                response = self._HTTPClient__session.send(request.prepare(), **kwargs)
            except ConnectionError as e:
                msg = str(e)
                if 'Connection reset by peer' in msg:
                    max_reset_by_peer_retries -= 1
                    self.log.warning('Connection reset by peer. Retries left %s' % max_reset_by_peer_retries)
                    continue
                else:
                    raise HTTPException('%s. (%s)' % (e, request.url), -1)
            except Timeout as e:
                if max_timeout_retries:
                    max_timeout_retries -= 1
                    self.log.warning('Connection timeout. Retries left %s' % max_timeout_retries)
                else:
                    raise HTTPException('%s. (%s)' % (e, request.url), 0)
            else:
                break
        ...
        if raw:
            ...
            if response.status_code >= 400:
                raise HTTPException('%s. (%s)' % (response.text, request.url), response.status_code)
    ...
    def request(self, method, path, params, data, endpoint, headers, raw):
        ...
        r = Request(method=method, url=self._HTTPClient__url(endpoint, path), params=params, data=data, headers=headers)
        return self._HTTPClient__send(r, endpoint, raw)

    def get(self, path, params=None, endpoint=None, headers=None, raw=False):
        return self.request('GET', path, params, None, endpoint, headers, raw)

    def post(self, path, params=None, data=None, endpoint=None, headers=None, raw=False):
        if data:
            data = self._dumps(data)
        return self.request('POST', path, params, data, endpoint, headers, raw)

    def put(self, path, params=None, data=None, endpoint=None, headers=None, raw=False):
        if data:
            data = self._dumps(data)
        return self.request('PUT', path, params, data, endpoint, headers, raw)

    def delete(self, path, params=None, endpoint=None, headers=None, raw=False):
        return self.request('DELETE', path, params, None, endpoint, headers, raw)