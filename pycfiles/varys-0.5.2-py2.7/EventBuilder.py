...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/varys/EventBuilder.py
# Compiled at: 2014-06-10 20:03:06
import os
...
class EventBuilder(object):
    ...
    def run(self):
        ...
        for sub in self.subjects:
            ...
            forced_concat = 'fidl' in self.output_formats
            event_lists = []
            durations = []
            r = 0
            ...
            if concat_needed:
                for set_name in event_sets.keys():
                    if set_name not in self.concat_sets:
                        continue
            ...
            if not self.skip_output:
                ...
                for set_name in event_sets.keys():
                    ...
                    if 'pickle' not in ofs:
                        ofs.append('pickle')
                    for format in ofs:
                        ev_files = FileWriter.write_set(format, event_sets[set_name], set_dir, forced_concat_events.get(set_name, None), **self.get_write_args())
...
class FixedDuration_EventBuilder(RowWise_EventBuilder):
    ...
    def events_for_row(self, row_dict):
        ...
        if not name or not onset or self.keep_names and name not in self.keep_names:
            return
...
def check_events_for_set_names(dict_lists):
    if not all([ 'set' in d for d_list in dict_lists for d in d_list ]):
        raise ValueError('All events must have a value for key "set". ' + 'Check your event definitions!')
