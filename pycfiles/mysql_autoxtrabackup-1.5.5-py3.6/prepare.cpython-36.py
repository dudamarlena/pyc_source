...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/backup_prepare/prepare.py
# Compiled at: 2020-03-24 02:41:06
# Size of source mod 2**32: 56202 bytes
...
class Prepare(GeneralClass):
    ...
    def __init__(self, config=path_config.config_path_file, dry_run=0, tag=None):
        ...
        if self.tag:
            if not os.path.isfile('{}/backup_tags.txt'.format(self.backupdir)):
                raise RuntimeError('Could not find backup_tags.txt inside backup directory. Please run without --tag option')
            ...
            if self.recent_full_backup_file():
                ...
                if hasattr(self, 'stream'):
                    if hasattr(self, 'encrypt'):
                        if hasattr(self, 'xbs_decrypt'):
                            logger.info('Using xbstream to extract and decrypt from full_backup.stream!')
                            xbstream_command = '{} {} --decrypt={} --encrypt-key={} --encrypt-threads={} < {}/{}/full_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.decrypt, self.encrypt_key, self.encrypt_threads, self.full_dir, self.recent_full_backup_file(), self.full_dir, self.recent_full_backup_file())
                            logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                            if self.dry == 0:
                                if isfile('{}/{}/full_backup.stream'.format(self.full_dir, self.recent_full_backup_file())):
                ...
                if hasattr(self, 'stream'):
                    ...
                    xbstream_command = '{} {} < {}/{}/full_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.full_dir, self.recent_full_backup_file(), self.full_dir, self.recent_full_backup_file())
                    logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                    if self.dry == 0:
                        if isfile('{}/{}/full_backup.stream'.format(self.full_dir, self.recent_full_backup_file())):
                            status, output = subprocess.getstatusoutput(xbstream_command)
                ...
                if hasattr(self, 'decrypt'):
                    if hasattr(self, 'remove_original_enc'):
                        if self.remove_original_enc:
                            decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.full_dir, self.recent_full_backup_file())
                    else:
                        decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{}'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.full_dir, self.recent_full_backup_file())
                ...
                if hasattr(self, 'decompress'):
                    if hasattr(self, 'remove_original_comp'):
                        if self.remove_original_comp:
                            decmp = '{} --decompress={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decompress, self.full_dir, self.recent_full_backup_file())
                    else:
                        decmp = '{} --decompress={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decompress, self.full_dir, self.recent_full_backup_file())
                ...
                xtrabackup_prepare_cmd = '{} --prepare --target-dir={}/{}'.format(self.backup_tool, self.full_dir, self.recent_full_backup_file())
                ...
                if self.dry == 0:
                    ...
                    status or logger.error('FAILED: FULL BACKUP prepare.')
                    raise RuntimeError('FAILED: FULL BACKUP prepare.')
        else:
            if found_backups[1] == 'Inc':
                        ...
                        for i in list_of_dir[:index_num + 1]:
                            if i != found_backups[0]:
                                ...
                                if hasattr(self, 'decrypt'):
                                    if hasattr(self, 'remove_original_enc'):
                                        if self.remove_original_enc:
                                            decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.inc_dir, i)
                                    else:
                                        decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{}'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.inc_dir, i)
                                    logger.info('Trying to decrypt backup')
                                    logger.info('Running decrypt command -> {}'.format(decr))
                                    if self.dry == 0:
                                        status = ProcessRunner.run_command(decr)
                                ...
                                if hasattr(self, 'decompress'):
                                    if hasattr(self, 'remove_original_comp'):
                                        if self.remove_original_comp:
                                            decmp = '{} --decompress={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decompress, self.inc_dir, i)
                                    else:
                                        decmp = '{} --decompress={} --target-dir={}/{}'.format(self.backup_tool, self.decompress, self.inc_dir, i)
                                    logger.info('Trying to decompress backup')
                                    logger.info('Running decompress command -> {}'.format(decmp))
                                    if self.dry == 0:
                                        status = ProcessRunner.run_command(decmp)
                                ...
                                xtrabackup_prepare_cmd = '{} --prepare {} --target-dir={}/{} --incremental-dir={}/{}'.format(self.backup_tool, self.xtrabck_prepare, self.full_dir, self.recent_full_backup_file(), self.inc_dir, i)
                                if hasattr(self, 'xtra_options'):
                                    xtrabackup_prepare_cmd += ' '
                                    xtrabackup_prepare_cmd += self.xtra_options
                                if hasattr(self, 'xtra_prepare_options'):
                                ...
                                if hasattr(self, 'stream'):
                                    ...
                                    xbstream_command = '{} {} < {}/{}/inc_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.inc_dir, i, self.inc_dir, i)
                                    logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                                    if self.dry == 0:
                                        if isfile('{}/{}/inc_backup.stream'.format(self.inc_dir, i)):
                                            status, output = subprocess.getstatusoutput(xbstream_command)
                                            if status == 0:
                                                logger.info('OK: XBSTREAM command succeeded.')
                                            else:
                                ...
                                if hasattr(self, 'decompress'):
                                    if hasattr(self, 'remove_original_comp'):
                                        ...
                                        xtrabackup_prepare_inc_cmd = '{} --prepare --target-dir={}/{} --incremental-dir={}/{}'.format(self.backup_tool, self.full_dir, self.recent_full_backup_file(), self.inc_dir, i)
                                        if hasattr(self, 'xtra_options'):
    ...
    def prepare_only_full_backup(self):
        ...
        if recent_bck:
            if self.check_inc_backups() == 0:
                ...
                if hasattr(self, 'stream'):
                    if self.stream == 'tar':
                        untar_cmd = 'tar -xf {}/{}/full_backup.tar -C {}/{}'.format(self.full_dir, recent_bck, self.full_dir, recent_bck)
                        logger.info('The following tar command will be executed -> {}'.format(untar_cmd))
                        if self.dry == 0:
                            if isfile('{}/{}/full_backup.tar'.format(self.full_dir, recent_bck)):
                ...
                if hasattr(self, 'stream') and self.stream == 'xbstream' and hasattr(self, 'encrypt') and hasattr(self, 'xbs_decrypt'):
                    logger.info('Using xbstream to extract and decrypt from full_backup.stream!')
                    xbstream_command = '{} {} --decrypt={} --encrypt-key={} --encrypt-threads={} < {}/{}/full_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.decrypt, self.encrypt_key, self.encrypt_threads, self.full_dir, recent_bck, self.full_dir, recent_bck)
                    logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                    ...
                    if hasattr(self, 'stream'):
                        if self.stream == 'xbstream':
                            logger.info('Using xbstream to extract from full_backup.stream!')
                            xbstream_command = '{} {} < {}/{}/full_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.full_dir, recent_bck, self.full_dir, recent_bck)
                            logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                            if self.dry == 0:
                                if isfile('{}/{}/full_backup.stream'.format(self.full_dir, recent_bck)):
                                    status, output = subprocess.getstatusoutput(xbstream_command)
                                    if status == 0:
                                        logger.info('OK: XBSTREAM command succeeded.')
                                    else:
                ...
                if hasattr(self, 'decrypt'):
                    if hasattr(self, 'remove_original_enc'):
                        if self.remove_original_enc:
                            decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.full_dir, recent_bck)
                    else:
                        decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{}'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.full_dir, recent_bck)
                    logger.info('Trying to decrypt backup')
                    logger.info('Running decrypt command -> {}'.format(decr))
                    if self.dry == 0:
                        status = ProcessRunner.run_command(decr)
                ...
                if hasattr(self, 'decompress'):
                    if hasattr(self, 'remove_original_comp'):
                        if self.remove_original_comp:
                            decmp = '{} --decompress={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decompress, self.full_dir, recent_bck)
                    else:
                        decmp = '{} --decompress={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decompress, self.full_dir, recent_bck)
                    logger.info('Trying to decompress backup')
                    if self.dry == 0:
                        ...
                        if status:
                            ...
                            logger.error('FAILED: FULL BACKUP decompression')
                            raise RuntimeError('FAILED: FULL BACKUP decompression')
                xtrabackup_prepare_cmd = '{} --prepare --target-dir={}/{}'.format(self.backup_tool, self.full_dir, recent_bck)
                if hasattr(self, 'xtra_options'):
                    xtrabackup_prepare_cmd += ' '
                    xtrabackup_prepare_cmd += self.xtra_options
                if hasattr(self, 'xtra_prepare_options'):
                ...
                if hasattr(self, 'decompress'):
                    if hasattr(self, 'remove_original_comp'):
                        ...
                        decmp = '{} --decompress={} --target-dir={}/{}'.format(self.backup_tool, self.decompress, self.full_dir, self.recent_full_backup_file())
                    logger.info('Trying to decompress backup')
                    logger.info('Running decompress command -> {}'.format(decmp))
                    if self.dry == 0:
                        status = ProcessRunner.run_command(decmp)
                        if status:
                            ...
                            logger.error('FAILED: FULL BACKUP decompression')
                            raise RuntimeError('FAILED: FULL BACKUP decompression')
                xtrabackup_prepare_cmd = '{} --prepare {} --target-dir={}/{}'.format(self.backup_tool, self.xtrabck_prepare, self.full_dir, self.recent_full_backup_file())
                if hasattr(self, 'xtra_options'):
                    xtrabackup_prepare_cmd += ' '
                    xtrabackup_prepare_cmd += self.xtra_options
                if hasattr(self, 'xtra_prepare_options'):
                ...
                for inc_backup_dir in list_of_dir:
                    if inc_backup_dir != max(os.listdir(self.inc_dir)):
                        ...
                        if hasattr(self, 'decrypt'):
                            if hasattr(self, 'remove_original_enc'):
                                if self.remove_original_enc:
                                    decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.inc_dir, inc_backup_dir)
                            else:
                                decr = '{} --decrypt={} --encrypt-key={} --target-dir={}/{}'.format(self.backup_tool, self.decrypt, self.encrypt_key, self.inc_dir, inc_backup_dir)
                            logger.info('Trying to decrypt backup')
                            logger.info('Running decrypt command -> {}'.format(decr))
                            if self.dry == 0:
                                status = ProcessRunner.run_command(decr)
                        ...
                        if hasattr(self, 'decompress'):
                            if hasattr(self, 'remove_original_comp'):
                                if self.remove_original_comp:
                                    decmp = '{} --decompress={} --target-dir={}/{} --remove-original'.format(self.backup_tool, self.decompress, self.inc_dir, inc_backup_dir)
                            else:
                                decmp = '{} --decompress={} --target-dir={}/{}'.format(self.backup_tool, self.decompress, self.inc_dir, inc_backup_dir)
                            logger.info('Trying to decompress backup')
                            logger.info('Running decompress command -> {}'.format(decmp))
                            if self.dry == 0:
                                status = ProcessRunner.run_command(decmp)
                                if status:
                                    ...
                                    logger.error('FAILED: FULL BACKUP decrypt.')
                                    raise RuntimeError('FAILED: FULL BACKUP decrypt.')
                        xtrabackup_prepare_inc_cmd = '{} --prepare {} --target-dir={}/{} --incremental-dir={}/{}'.format(self.backup_tool, self.xtrabck_prepare, self.full_dir, self.recent_full_backup_file(), self.inc_dir, inc_backup_dir)
                        if hasattr(self, 'xtra_options'):
                            xtrabackup_prepare_inc_cmd += ' '
                            xtrabackup_prepare_inc_cmd += self.xtra_options
                        if hasattr(self, 'xtra_prepare_options'):
                        ...
                        if hasattr(self, 'stream'):
                            logger.info('Using xbstream to extract from inc_backup.stream!')
                            xbstream_command = '{} {} < {}/{}/inc_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.inc_dir, inc_backup_dir, self.inc_dir, inc_backup_dir)
                            logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                            if self.dry == 0:
                                if isfile('{}/{}/inc_backup.stream'.format(self.inc_dir, inc_backup_dir)):
                                    status, output = subprocess.getstatusoutput(xbstream_command)
                                    if status == 0:
                                        logger.info('OK: XBSTREAM command succeeded.')
                                    else:
                        ...
                        if hasattr(self, 'decompress'):
                            if hasattr(self, 'remove_original_comp'):
                                ...
                                xtrabackup_prepare_inc_cmd = '{} --prepare --target-dir={}/{} --incremental-dir={}/{}'.format(self.backup_tool, self.full_dir, self.recent_full_backup_file(), self.inc_dir, inc_backup_dir)
                                if hasattr(self, 'xtra_options'):
                                    xtrabackup_prepare_inc_cmd += ' '
                                    xtrabackup_prepare_inc_cmd += self.xtra_options
                                if hasattr(self, 'xtra_prepare_options'):
    ...
    def run_xtra_copyback(self, datadir=None):
        copy_back = '{} --copy-back {} --target-dir={}/{} --datadir={}'.format(self.backup_tool, self.xtra_options, self.full_dir, self.recent_full_backup_file(), self.datadir if datadir is None else datadir)
