...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/master_backup_script/backuper.py
# Compiled at: 2020-03-24 02:41:06
# Size of source mod 2**32: 34807 bytes
import logging, os, subprocess, shlex, shutil, time
...
class Backup(GeneralClass):
    ...
    def add_tag(self, backup_type: str, backup_size: str, backup_status: str):
        ...
        if not self.tag:
            ...
            with open('{}/backup_tags.txt'.format(self.backupdir), 'a') as (backtags_file):
                backtag_final = backtag_str.format(bk_name=backup_name, bk_type=backup_type,
    ...
    def show_tags(backup_dir):
        if os.path.isfile('{}/backup_tags.txt'.format(backup_dir)):
            ...
            column_names = '{0}\t{1}\t{2}\t{3}\t{4}\tTAG\n'.format('Backup'.ljust(19), 'Type'.ljust(4), 'Status'.ljust(2), 'Completion_time'.ljust(19), 'Size')
    ...
    def mysql_connection_flush_logs(self):
        ...
        if hasattr(self, 'mysql_socket'):
            ...
            new_command = command_connection.format(self.mysql, self.mycnf, self.mysql_user, self.mysql_password, self.mysql_socket)
        else:
            command_connection += ' --port={}'
            command_connection += command_execute
            new_command = command_connection.format(self.mysql, self.mycnf, self.mysql_user, self.mysql_password, self.mysql_host, self.mysql_port)
            ...
            if len(os.listdir(self.full_dir)) == 1 or i != max(os.listdir(self.full_dir)):
                ...
                if hasattr(self, 'move_archive') and int(self.move_archive) == 1:
                    ...
                    if status:
                        ...
                        run_tar = 'tar --use-compress-program=pigz -cvf {} {} {}'.format(archive_file, self.full_dir, self.inc_dir)
                    else:
                        logger.warning('pigz executeable not available. Defaulting to singlecore tar')
                        run_tar = 'tar -zcf {} {} {}'.format(archive_file, self.full_dir, self.inc_dir)
    ...
    def clean_old_archives(self):
        ...
        for archive in self.sorted_ls(self.archive_dir):
            ...
            if hasattr(self, 'archive_max_duration') and (now - archive_date).total_seconds() >= self.archive_max_duration:
                ...
                if hasattr(self, 'archive_max_size') and self.get_directory_size(self.archive_dir) > self.archive_max_size:
    ...
    def general_command_builder(self):
        ...
        try:
            args += ' --compress={} --compress-chunk-size={} --compress-threads={}'.format(self.compress, self.compress_chunk_size, self.compress_threads)
        except AttributeError:
            pass

        try:
            args += ' --encrypt={} --encrypt-key={} --encrypt-key-file={} --encrypt-threads={} --encrypt-chunk-size={}'.format(self.encrypt, self.encrypt_key, self.encrypt_key_file, self.encrypt_threads, self.encrypt_chunk_size)
        except AttributeError:
            pass
    ...
    def full_backup(self):
        ...
        xtrabackup_cmd = '{} --defaults-file={} --user={} --password={}  --target-dir={} --backup'.format(self.backup_tool, self.mycnf, self.mysql_user, self.mysql_password, full_backup_dir)
        ...
        if hasattr(self, 'stream') and self.stream == 'tar' and (hasattr(self, 'encrypt') or hasattr(self, 'compress')):
            logger.error("xtrabackup: error: compressed and encrypted backups are incompatible with the 'tar' streaming format. Use --stream=xbstream instead.")
            raise RuntimeError("xtrabackup: error: compressed and encrypted backups are incompatible with the 'tar' streaming format. Use --stream=xbstream instead.")
        else:
        ...
        if self.dry == 1:
            ...
            self.add_tag(backup_type='Full', backup_size=(self.get_folder_size(full_backup_dir)),
    ...
    def inc_backup(self):
        ...
        if recent_inc == 0:
            xtrabackup_inc_cmd = '{} --defaults-file={} --user={} --password={} --target-dir={} --incremental-basedir={}/{} --backup'.format(self.backup_tool, self.mycnf, self.mysql_user, self.mysql_password, inc_backup_dir, self.full_dir, recent_bck)
            xtrabackup_inc_cmd += self.general_command_builder()
            if hasattr(self, 'stream'):
                if self.stream == 'tar':
                    logger.error("xtrabackup: error: streaming incremental backups are incompatible with the 'tar' streaming format. Use --stream=xbstream instead.")
                    raise RuntimeError("xtrabackup: error: streaming incremental backups are incompatible with the 'tar' streaming format. Use --stream=xbstream instead.")
            if hasattr(self, 'stream') and self.stream == 'xbstream' and hasattr(self, 'encrypt') and hasattr(self, 'xbs_decrypt'):
                ...
                if 'encrypt' in xtrabackup_inc_cmd:
                    logger.info('Applying workaround for LP #1444255')
                    xbcrypt_command = '{} -d -k {} -a {} -i {}/{}/xtrabackup_checkpoints.xbcrypt -o {}/{}/xtrabackup_checkpoints'.format(self.xbcrypt, self.encrypt_key, self.encrypt, self.full_dir, recent_bck, self.full_dir, recent_bck)
            ...
            if self.dry == 0:
                ...
                status_str = 'OK' if status is True else 'FAILED'
                self.add_tag(backup_type='Inc', backup_size=(self.get_folder_size(inc_backup_dir)),
                  backup_status=status_str)
                return status
        else:
            xtrabackup_inc_cmd = '{} --defaults-file={} --user={} --password={}  --target-dir={} --incremental-basedir={}/{} --backup'.format(self.backup_tool, self.mycnf, self.mysql_user, self.mysql_password, inc_backup_dir, self.inc_dir, recent_inc)
            xtrabackup_inc_cmd += self.general_command_builder()
        if hasattr(self, 'stream'):
            if self.stream == 'tar':
                logger.error("xtrabackup: error: streaming incremental backups are incompatible with the 'tar' streaming format. Use --stream=xbstream instead.")
        ...
        if hasattr(self, 'stream') and hasattr(self, 'encrypt') and hasattr(self, 'xbs_decrypt'):
            ...
            xbstream_command = '{} {} --decrypt={} --encrypt-key={} --encrypt-threads={} < {}/{}/inc_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.decrypt, self.encrypt_key, self.encrypt_threads, self.inc_dir, recent_inc, self.inc_dir, recent_inc)
            logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
            ...
            if hasattr(self, 'stream'):
                ...
                xbstream_command = '{} {} < {}/{}/inc_backup.stream -C {}/{}'.format(self.xbstream, self.xbstream_options, self.inc_dir, recent_inc, self.inc_dir, recent_inc)
                logger.info('The following xbstream command will be executed {}'.format(xbstream_command))
                ...
                if 'encrypt' in xtrabackup_inc_cmd:
                    logger.info('Applying workaround for LP #1444255')
                    xbcrypt_command = '{} -d -k {} -a {} -i {}/{}/xtrabackup_checkpoints.xbcrypt -o {}/{}/xtrabackup_checkpoints'.format(self.xbcrypt, self.encrypt_key, self.encrypt, self.inc_dir, recent_inc, self.inc_dir, recent_inc)
                    logger.info('The following xbcrypt command will be executed {}'.format(xbcrypt_command))
                    if self.dry == 0:
                        status, output = subprocess.getstatusoutput(xbcrypt_command)
                        if status == 0:
    ...
    def all_backup(self):
        ...
        if self.recent_full_backup_file() == 0:
            ...
            if hasattr(self, 'remote_conn'):
                if hasattr(self, 'remote_dir'):
                    if self.remote_conn:
                        if self.remote_dir:
                            self.copy_backup_to_remote_host()
            return True
        if self.last_full_backup_date() == 1:
            ...
            if hasattr(self, 'archive_dir'):
                ...
                if hasattr(self, 'archive_max_duration') and self.archive_max_duration or hasattr(self, 'archive_max_size') and self.archive_max_size:
            ...
            logger.info('- - - - You have a full backup that is less than {} seconds old. - - - -'.format(self.full_backup_interval))
