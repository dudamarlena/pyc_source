...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/prepare_env_test_mode/runner_test_mode.py
# Compiled at: 2018-12-29 16:15:57
# Size of source mod 2**32: 30834 bytes
...
import socket, os, shutil, logging, subprocess
...
class RunnerTestMode(GeneralClass):
    ...
    def run_pt_table_checksum(basedir, conn_options=None):
        ...
        if conn_options is None:
            command = 'pt-table-checksum --user={} --socket={} --recursion-method dsn=h=localhost,D=test,t=dsns --no-check-binlog-format --no-check-slave-tables'.format('root', sock_file)
        else:
            command = 'pt-table-checksum {} --recursion-method dsn=h=localhost,D=test,t=dsns --no-check-binlog-format --no-check-slave-tables'.format(conn_options)
    ...
    def populate_dsns_table(sql_conn, slave_port):
        ...
        insert_into = "{} -e 'insert into dsns(id, parent_id, dsn) values({}, {}, {})'".format(sql_conn, dsns_id, dsns_parent_id, dsn)
    ...
    def check_slave_status(sql_command):
        ...
        for i, j in enumerate((list_output[2:]), start=1):
            ...
            if 'Last_IO_Error' == splitted[0].lstrip():
                ...
                if 'Last_SQL_Error' == splitted[0].lstrip() and splitted[1].lstrip() != '':
    ...
    def get_log_file_log_pos_slave(full_backup_dir):
        ...
        with open(file_name, 'r') as (slave_info):
            ...
            splitted = parse_me.split(',')
            return (
             splitted[0].split('=')[1].replace("'", ''), splitted[1].split('=')[1])

    def run_change_master(self, basedir, full_backup_dir, mysql_slave_client_cmd, mysql_master_client_cmd, is_slave=None):
        """
        ...
        if '5.5' in basedir:
            sql_change_master = '{} -e "CHANGE MASTER TO MASTER_HOST=\'{}\', MASTER_USER=\'{}\', MASTER_PASSWORD=\'{}\', MASTER_PORT={}, MASTER_LOG_FILE=\'{}\', MASTER_LOG_POS={}"'
        else:
        ...
        if '5.5' not in basedir:
            ...
            if '5.5' in basedir:
                ...
                self.run_sql_command(sql_change_master.format(mysql_slave_client_cmd, '127.0.0.1', 'repl', 'Baku12345', port[7:], file_pos[0], file_pos[1]))
            else:
    ...
    def wipe_backup_prepare_copyback(self, basedir, keyring_vault=0):
        ...
        for options in ConfigGenerator(config=(self.conf)).options_combination_generator(self.mysql_options):
            ...
            if self.clone_obj.wipe_server_all(basedir_path=basedir, options=options):
                ...
                backup_obj = WrapperForBackupTest(config=(self.conf), full_dir=full_dir,
                  inc_dir=inc_dir,
                  basedir=basedir)
                logger.debug('Started to run run_all_backup()')
                if backup_obj.run_all_backup():
                    prepare_obj = WrapperForPrepareTest(config=(self.conf), full_dir=full_dir,
                      inc_dir=inc_dir)
                    ...
                    if prepare_obj.run_prepare_backup():
                        if hasattr(self, 'make_slaves'):
                            ...
                            slave_full_options = self.prepare_start_slave_options(basedir=basedir, slave_number=1,
                              options=options)
                            ...
                            mysql_slave_client_cmd = RunBenchmark(config=(self.conf)).get_mysql_conn(basedir=basedir, file_name=('cl_node{}'.format(1)))
                            self.create_dsns_table(mysql_master_client_cmd)
                            if self.run_change_master(basedir=basedir, full_backup_dir=('{}/{}'.format(full_dir, full_backup_dir)),
                            ...
                            if '5.7' in basedir:
                                if '2_4_ps_5_7' in self.conf:
                                    slave_conf_file = 'xb_2_4_ps_5_7_slave.cnf'
                            if '8.0' in basedir:
                                if '8_0_ps_8_0' in self.conf:
                                    slave_conf_file = 'xb_8_0_ps_8_0_slave.cnf'
                            if '5.6' in basedir:
                                if '2_4_ps_5_6' in self.conf:
                                    slave_conf_file = 'xb_2_4_ps_5_6_slave.cnf'
                            if '5.6' in basedir:
                                if '2_3_ps_5_6' in self.conf:
                                    slave_conf_file = 'xb_2_3_ps_5_6_slave.cnf'
                            if '5.5' in basedir:
                                if '2_3_ps_5_5' in self.conf:
                                    slave_conf_file = 'xb_2_3_ps_5_5_slave.cnf'
                            if '5.5' in basedir:
                                if '2_4_ps_5_5' in self.conf:
                                    slave_conf_file = 'xb_2_4_ps_5_5_slave.cnf'
                            cnf_obj.generate_config_files(test_path=(self.testpath), conf_file=slave_conf_file,
                              basedir=basedir,
                            ...
                            if backup_obj_2.all_backup():
                                ...
                                if prepare_obj_2.run_prepare_backup():
                                    ...
                                    slave_full_options = self.prepare_start_slave_options(basedir=basedir, slave_number=2,
                                      options=options)
                                    prepare_obj_2.start_mysql_func(start_tool=('{}/start_dynamic'.format(basedir)), options=slave_full_options)
                                    ...
                                    mysql_slave_client_cmd_2 = RunBenchmark(config=(self.conf)).get_mysql_conn(basedir=basedir, file_name=('cl_node{}'.format(2)))
                                    full_backup_dir_2 = prepare_obj_2.recent_full_backup_file()
                                    if self.run_change_master(basedir=basedir, full_backup_dir=('{}/{}'.format(full_dir_2, full_backup_dir_2)),
                                      mysql_master_client_cmd=mysql_master_client_cmd,
