...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/prepare_env_test_mode/config_generator.py
# Compiled at: 2020-02-07 09:05:18
# Size of source mod 2**32: 19774 bytes
    ...
    def generate_config_files(test_path, conf_file, basedir, datadir, sock_file, home_path, backup_path=None):
        try:
            ...
            with open(conf_path, 'w+') as (cfgfile):
                ...
                config.set(section2, 'backupdir', join(home_path, 'XB_TEST/backup_dir/ps_{}_x_{}'.format(get_ps_version, get_xb_version)))
                config.set(section2, 'backup_tool', '{}/target/percona-xtrabackup-{}.x-debug/bin/xtrabackup'.format(test_path, get_xb_version_replaced))
                ...
                if '5.7' in basedir or '8.0' in basedir:
                    config.set(section2, 'xtra_options', '--slave-info --no-version-check --core-file --parallel=10 --throttle=40 --check-privileges --ftwrl-wait-timeout=0 --ftwrl-wait-query-type=all --ftwrl-wait-threshold=1 --kill-long-queries-timeout=1 --kill-wait-query-type=all --kill-long-query-type=all --no-backup-locks --keyring-file-data={}/mysql-keyring/keyring '.format(basedir))
                else:
                    config.set(section2, 'xtra_options', '--slave-info --no-version-check --core-file --ftwrl-wait-timeout=0 --ftwrl-wait-query-type=all --ftwrl-wait-threshold=1 --kill-long-queries-timeout=1 --kill-wait-query-type=all --kill-long-query-type=all --no-backup-locks --parallel=10 --throttle=40 --check-privileges ')
                ...
                config.set(section3, '#Enable if you want to remove .qp files after decompression.(Not available yet, will be released with XB 2.3.7 and 2.4.6)')
                ...
                config.set(section4, 'xbcrypt', '{}/target/percona-xtrabackup-{}.x-debug/bin/xbcrypt'.format(test_path, get_xb_version_replaced))
                ...
                config.set(section4, '#Enable if you want to remove .qp files after decompression.(Not available yet, will be released with XB 2.3.7 and 2.4.6)')
                ...
                config.set(section5, 'xbstream', '{}/target/percona-xtrabackup-{}.x-debug/bin/xbstream'.format(test_path, get_xb_version_replaced))
                ...
                config.set(section8, 'gitcmd', '--recursive --depth=1 https://github.com/percona/percona-server.git')
                ...
                config.set(section8, 'xb_configs', 'xb_2_4_ps_5_6.cnf xb_2_4_ps_5_7.cnf xb_2_3_ps_5_6.cnf xb_2_3_ps_5_5.cnf xb_2_4_ps_5_5.cnf xb_8_0_ps_8_0.cnf')
                config.set(section8, 'make_slaves', '1')
                if '5_7' in conf_file:
                    config.set(section8, 'default_mysql_options', '--early-plugin-load=keyring_file.so,--keyring_file_data={}/mysql-keyring/keyring,--log-bin=mysql-bin,--log-slave-updates,--server-id={},--gtid-mode=ON,--enforce-gtid-consistency,--binlog-format=row,--encrypt_binlog=ON,--master_verify_checksum=ON,--binlog_checksum=CRC32,--innodb_encrypt_tables=ON,--innodb_encrypt_online_alter_logs=ON,--innodb_temp_tablespace_encrypt=ON')
                else:
                    if '8_0' in conf_file:
                        config.set(section8, 'default_mysql_options', '--early-plugin-load=keyring_file.so,--keyring_file_data={}/mysql-keyring/keyring,--log-bin=mysql-bin,--log-slave-updates,--server-id={},--gtid-mode=ON,--enforce-gtid-consistency,--binlog-format=row,--encrypt_binlog=ON,--master_verify_checksum=ON,--binlog_checksum=CRC32,--innodb_encrypt_tables=ON,--innodb_encrypt_online_alter_logs=ON,--innodb_temp_tablespace_encrypt=ON')
                    else:
                        if '5_6' in conf_file:
                            config.set(section8, 'default_mysql_options', '--log-bin=mysql-bin,--log-slave-updates,--server-id={},--gtid-mode=ON,--enforce-gtid-consistency,--binlog-format=row')
                        else:
                            if '5_5' in conf_file:
                                config.set(section8, 'default_mysql_options', '--log-bin=mysql-bin,--log-slave-updates,--server-id={},--binlog-format=row')
                            if '5_7' in conf_file or '8_0' in conf_file:
                                config.set(section8, 'mysql_options', '--innodb_buffer_pool_size=1G 2G 3G,--innodb_log_file_size=1G 2G 3G,--innodb_page_size=4K 8K 16K 32K 64K')
                            else:
                                if '5_6' in conf_file:
                                    config.set(section8, 'mysql_options', '--innodb_buffer_pool_size=1G 2G 3G,--innodb_log_file_size=1G 2G 3G,--innodb_page_size=4K 8K 16K')
    ...
    def the_main_generator(self):
        ...
        for basedir in basedirs:
            for conf_file in conf_list:
                if '8.0' in basedir:
                    if '8_0_ps_8_0' in conf_file:
                        self.generate_config_files(test_path=(self.testpath), conf_file=conf_file,
                          basedir=basedir,
                          datadir='data',
                          sock_file=self.benchmark_obj.get_sock(basedir=basedir),
                          home_path=(self.home))
                if '5.7' in basedir:
                    if '2_4_ps_5_7' in conf_file:
                        self.generate_config_files(test_path=(self.testpath), conf_file=conf_file,
                          basedir=basedir,
                          datadir='data',
                          sock_file=self.benchmark_obj.get_sock(basedir=basedir),
                          home_path=(self.home))
                if '5.6' in basedir and '2_4_ps_5_6' in conf_file:
                    self.generate_config_files(test_path=(self.testpath), conf_file=conf_file,
                      basedir=basedir,
                      datadir='data',
                      sock_file=self.benchmark_obj.get_sock(basedir=basedir),
                    ...
                    if '5.6' in basedir and '2_3_ps_5_6' in conf_file:
                        self.generate_config_files(test_path=(self.testpath), conf_file=conf_file,
                          basedir=basedir,
                          datadir='data',
                          sock_file=self.benchmark_obj.get_sock(basedir=basedir),
