...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/general_conf/check_env.py
...
class CheckEnv(GeneralClass):
    ...
    def check_mysql_uptime(self, options=None):
        ...
        """
        if options is None:
            statusargs = "{} --defaults-file={} --user={} --password='{}' status".format(self.mysqladmin, self.mycnf, self.mysql_user, self.mysql_password)
            if hasattr(self, 'mysql_socket'):
                statusargs += ' --socket={}'.format(self.mysql_socket)
            else:
                if hasattr(self, 'mysql_host'):
    ...
    def check_mysql_conf(self):
        ...
        if self.mycnf is None or self.mycnf == '':
            ...
            if not os.path.exists(self.mycnf) and self.mycnf:
