...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/implicit/datasets/movielens.py
...
def _read_dataframes_100k(path):
    ...
    import pandas
    ratings = pandas.read_table((os.path.join(path, 'u.data')), names=[
     'userId', 'movieId', 'rating', 'timestamp'])
    movies = pandas.read_csv((os.path.join(path, 'u.item')), names=[
     'movieId', 'title'],
      usecols=[
     0, 1],
      delimiter='|',
...
def _read_dataframes(path):
    import pandas
    ratings = pandas.read_csv((os.path.join(path, 'ratings.dat')), delimiter='::', names=[
     'userId', 'movieId', 'rating', 'timestamp'])
    movies = pandas.read_table((os.path.join(path, 'movies.dat')), delimiter='::', names=[
