...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/wor/ds_down.py
# Compiled at: 2014-07-12 06:27:16
# Size of source mod 2**32: 12439 bytes
"""Synology Download Station url adder.
"""
import sys, os, logging, requests, json, configparser, subprocess

class NoDefaultHeaderConfigParser(configparser.ConfigParser):
    __doc__ = 'ConfigParser without the need of default section.'
...
def send_url(add_url, config_file):
    ...
    if not add_url.startswith('http:') and not add_url.startswith('magnet:'):
        with open(add_url, 'rb') as (payload):
            args = {'api': 'SYNO.DownloadStation.Task',  'version': '1', 
             'method': 'create', 
             'session': 'DownloadStation', 
             '_sid': auth}
...
def process_cmd_line(inputs=sys.argv[1:], parent_parsers=list()):
    ...
    class Verbose_action(argparse.Action):
        __doc__ = "Argparse action: Cumulative verbose switch '-v' counter"
    ...
    class Quiet_action(argparse.Action):
        __doc__ = "Argparse action: Cumulative quiet switch '-q' counter"
    ...
    parser = argparse.ArgumentParser(parents=parent_parsers, formatter_class=argparse.ArgumentDefaultsHelpFormatter, description='Passes urls and files to Synology DownloadStation for download.')
    parser.add_argument('-c', '--config-file', type=str, default='~/.config/ds-down.conf', help='Config file.')
    parser.add_argument('-v', nargs='?', default=2, action=Verbose_action, dest='verbose', help='Verbosity level specifier.')
    parser.add_argument('-q', nargs='?', action=Quiet_action, dest='verbose', help='Be more quiet, negatively affects verbosity level.')
    parser.add_argument('add_url', metavar='DownloadURL', help='URL to be passed to DownloadStation.')
...
def main():
    ...
    logging.basicConfig(level=convert_int_to_logging_level(args.verbose), format=lformat)
    del args.verbose
    if send_url(**args.__dict__):
        return 0
    return 1