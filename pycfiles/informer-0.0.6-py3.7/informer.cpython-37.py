...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/informer/informer.py
...
class Informer:
    ...
    def __init__(self, robot_id=None, block=True):
        ...
        for key in self.register_keys:
            ...
            if self.robot_id == None:
                ...
                self.data_dict[key] = utils.encode_message(data=('server:' + key),
        ...
        for key in self.register_keys:
            recv_thread = threading.Thread(target=(self.connect),
              args=(key, self.socket_dict[key]))
            recv_thread.start()
        ...
        for key in self.recv_keys:
            if key in self.register_keys:
                ...
                recv_thread = threading.Thread(target=receive_func,
                  args=(self,))
                recv_thread.start()
    ...
    def draw_box(self, lt_x, lt_y, width, height, message='', color='red', **kwargs):
        data = utils.to_json(dtype='box', lt_x=lt_x,
          lt_y=lt_y,
          width=width,
          height=height,
          message=message,
    ...
    def draw_center_box(self, ct_x, ct_y, width, height, message='', color='red'):
        data = utils.to_json(dtype='center_box', ct_x=ct_x,
          ct_y=ct_y,
          width=width,
          height=height,
          message=message,
    ...
    def draw_line(self, s_x, s_y, e_x, e_y, color='red'):
        data = utils.to_json(dtype='line', s_x=s_x,
          s_y=s_y,
          e_x=e_x,
          e_y=e_y,
          color=color)
