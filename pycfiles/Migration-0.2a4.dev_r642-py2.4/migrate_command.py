...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\migration\migrate_command.py
# Compiled at: 2006-11-02 09:05:34
import sys, migration, turbogears
...
class MigrateCommand(CommandWithDB):
    ...
    def run(self):
        ...
        parser = OptionParser()
        parser.add_option('-p', '--package', action='store', type='string', dest='package')
        parser.add_option('-c', '--config', action='store', type='string', dest='config')
        (options, args) = parser.parse_args()
        if not options.package:
            print 'You need to specify the repository package name with tg-admin migrate -p your_app.migrate command'
            sys.exit()
    ...
    def set_version(self, version):
        version = int(version)
        print 'Setting database to version %s (no migration will be done)' % version
        migration.set_version(version=version, package=self.package)

    def get_version(self):
        print 'The current database version is %s' % migration.get_version(package=self.package)
        print 'The lastest database version is %s' % migration.get_lastest_version(package=self.package)