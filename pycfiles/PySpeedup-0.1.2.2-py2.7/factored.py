...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyspeedup/factored.py
# Compiled at: 2017-02-25 12:54:13
import numbers, functools, operator, math
    ...
    def __init__(self, *args):
        ...
        if not args:
            ...
            if args[0] == 0:
                ...
                self._d = {k:v for k, v in args[0]._d.viewitems() if type(k) == int and type(v) == int if type(k) == int and type(v) == int}
                if len(self) != len(args[0]):
                    raise NotImplementedError()
            elif type(args[0]) == dict:
                self._d = {k:v for k, v in args[0].viewitems() if type(k) == int and type(v) == int if type(k) == int and type(v) == int}
