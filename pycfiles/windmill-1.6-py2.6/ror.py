...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill2/castile/ror.py
...
class RemoteObjectDoesNotExist(RemoteObjectException):
    pass
...
def create_remote(client, name, description=None):
    ...
    if description['type'] == 'null':
        ...
        if description['type'] in ('int', 'float', 'string'):
...
class RemoteObject(object):
    ...
    def __len__(self):
        ...
        if description.has_key('length'):
            ...
            if description.has_key('attributes'):
...
ror_type_cases = {'int': RemoteInt, 'float': RemoteFloat, 
   'string': RemoteString}