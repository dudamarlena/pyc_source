...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill2/castile/events.py
...
class EventClient(object):
    ...
    def describe(self, object_id, depth=0):
        if self.event_node_type == 'http-rest-api':
            ...
            (response, content) = self.http_client.request(path, 'GET', headers={'content-type': 'application/json'})
            assert response.status == 200
            return simplejson.loads(content)
        if self.event_node_type == 'streaming-push':
            pass
    ...
    def callFunction(self, object_id, args, kwargs):
        if self.event_node_type == 'http-rest-api':
            ...
            (response, content) = self.http_client.request(path, 'POST', body=simplejson.dumps(body), headers={'content-type': 'application/json'})
            assert response.status == 200
            return simplejson.loads(content)
        else:
            if self.event_node_type == 'streaming-push':
    ...
    def http_push(self, event_type, obj):
        ...
        (response, content) = self.http_client.request(self.http_events_uri + event_type, 'PUT', body=body, headers={'content-type': 'application/json'})
        assert response.status == 200
    ...
    def add_remote_listener(self, event_type, obj):
        ...
        body = simplejson.dumps(obj)
        (response, content) = self.http_client.request(self.http_events_uri + client_ns + '/listeners', 'POST', body=body, headers={'content-type': 'application/json'})
        assert response.status == 200
...
def eToDict(e):
    d = dict([ (x, getattr(e, x)) for x in dir(e) if type(getattr(e, x)) in [int, float, str, unicode, dict, list, tuple]
             ])
    d.pop('__dict__')
...
class CastileNode(object):
    ...
    def register_with_node(self, uri):
        h = httplib2.Http()
        (response, content) = h.request(with_slash(uri), method='POST', body=simplejson.dumps(self.client_map[self.ns].registration_obj))
        assert response.status == 200
    ...
    def get_cherrypy_server(self, host='0.0.0.0', port=None, server_name=None, numthreads=50):
        ...
        self.httpd = wsgiserver.CherryPyWSGIServer((host, port), self.get_application(), server_name=server_name, numthreads=numthreads)
        return self.httpd

    def start_cherrypy_server(self, host='0.0.0.0', port=None, server_name=None, numthreads=50, threaded=True):
    ...
    def POST(self, request, client_ns=None, action=None, ror_action=None, ror_name=None):
        ...
        if client_ns is None:
            ...
            if action == 'listeners':
                ...
                if action == 'ror' and (ror_action == 'callFunction' or ror_action == 'createInstance'):
                    if obj:
                        if obj.has_key('args'):
                            args = obj['args']
                        else:
                            args = []
                        if obj.has_key('kwargs'):
                            kwargs = obj['kwargs']
                        else:
                            kwargs = {}
                    else:
