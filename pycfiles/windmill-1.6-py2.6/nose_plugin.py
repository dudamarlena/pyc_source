...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill/authoring/nose_plugin.py
# Compiled at: 2011-01-13 01:48:00
from nose.plugins import Plugin
from windmill.bin import admin_options
parser_options = {}
for option_class in [ getattr(admin_options, x) for x in dir(admin_options) if hasattr(getattr(admin_options, x), 'option_names')
                    ]:
...
class WindmillNosePlugin(Plugin):
    ...
    def options(self, parser, env):
        for (name, option_class) in parser_options.items():
            parser.add_option('--' + name, dest=name, help='Windmill option: ' + option_class.__doc__, default=None)

        parser.add_option('--wmbrowser', dest='wmbrowser', help='Windmill option: Browser to start before running windmill tests', default=None)
        parser.add_option('--wmtesturl', dest='wmtesturl', help='Windmill option: Test url to run windmill tests against', default=None)
        parser.add_option('--wmssl', dest='wmssl', help='Windmill option: Enable SSL support', default=False, action='store_true')
        return
