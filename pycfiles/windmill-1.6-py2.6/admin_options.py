...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill/bin/admin_options.py
# Compiled at: 2011-04-09 01:50:00
import windmill, os, sys, logging
...
def help(bin_name='windmill'):
    ...
    all_option_names = []
    options_string = []
    for option in [ getattr(module, x) for x in dir(module) if hasattr(getattr(module, x), 'option_names') if getattr(module, x).__doc__ is not None
                  ]:
        ...
        if option.option_names[0] is None:
            if not issubclass(option, GeneralBool):
                options_string.append('    ' + ('').join([str(option.option_names[1]) + '=' + ' :: ', option.__doc__]) + default)
            else:
                options_string.append('    ' + ('').join([str(option.option_names[1]) + ' :: ', option.__doc__]) + default)
        elif not issubclass(option, GeneralBool):
            options_string.append('    ' + ('').join(['-' + str(option.option_names[0]) + ', ' + str(option.option_names[1]) + '=' + ' :: ', option.__doc__]) + default)
        else:
            options_string.append('    ' + ('').join(['-' + str(option.option_names[0]) + ', ' + str(option.option_names[1]) + ' :: ', option.__doc__]) + default)
