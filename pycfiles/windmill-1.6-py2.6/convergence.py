...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill/server/convergence.py
...
class ControllerQueue(object):
    ...
    def __init__(self, command_resolution_suite, test_resolution_suite):
        self.resolution_suites = {'test': test_resolution_suite, 'command': command_resolution_suite}
        self.queue = []
...
class TestResolutionSuite(object):
    ...
    def report_without_resolve(self, result, uuid, starttime, endtime, suite_name, debug=None, output=None):
        test = {'result': result, 'uuid': uuid, 'starttime': starttime, 'endtime': endtime, 'suite_name': suite_name, 
        ...
        if getattr(windmill, 'js_framework_active', False) is True:
            ...
            if tests['testFailureCount'] is not 0:
                for (k, v) in tests.items():
                    if type(v) is dict and v.get('result') is False:
                        print 'Failed: ' + k
