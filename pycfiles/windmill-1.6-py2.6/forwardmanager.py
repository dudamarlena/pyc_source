...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill/server/forwardmanager.py
# Compiled at: 2011-01-13 01:48:00
import proxy, time, sys
...
class ForwardManager(object):
    ...
    def known_hosts(self):
        ...
        result = list(urlparse('%s://%s/' % host) for host in first + forwarded if host not in exclude)
        return result
...
if __name__ == '__main__':
    ...
    class TestManager(unittest.TestCase):
        ...
        def testUnmapEnviron(self):
            testEnv = {'HTTP_ACCEPT': 'text/html,application/xhtml+xml', 'CONTENT_TYPE': 'text/plain', 
               'SCRIPT_NAME': '', 
               'REQUEST_METHOD': 'GET', 
               'HTTP_HOST': 'testurl', 
        ...
        def testForwardToDoesntDropQuery(self):
            ...
            self.assertEquals(urlparse('http://otherurl/forwarded?foo=bar'), mgr.forward_to(query_url, self.aUrl))

        def testForwardMapDoesntDropQuery(self):
            mgr = ForwardManager('http://testurl/')
            query_url = urlparse('https://something/forwarded?foo=bar')
            self.assertEquals(urlparse('http://testurl/forwarded?foo=bar'), mgr.forward_map(query_url))
        ...
        def testParseCookies(self):
            ...
            self.assertEquals('g=Nj; a=42; b=test; e=1; f=0; c=1', mgr.cookies_for('a.b.c'))
            headers = [('set-cookie', 'e=; expires=Thu, 01-Dec-1994 16:00:00 GMT'),
             ('set-cookie', 'a=; expires=Thu, 01 Dec 1994 16:00:00 GMT')]
            mgr.parse_headers(headers, 'a.b.c')
            self.assertEquals('g=Nj; b=test; f=0; c=1', mgr.cookies_for('a.b.c'))
