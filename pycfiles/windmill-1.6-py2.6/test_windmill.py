...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/windmill/management/commands/test_windmill.py
...
class Command(BaseCommand):
    ...
    def handle(self, *labels, **options):
        ...
        for (ttype, mod) in tests:
            if ttype == 'tests':
                for ucls in [ getattr(mod, x) for x in dir(mod) if type(getattr(mod, x, None)) in (types.ClassType,
                 types.TypeType) if issubclass(getattr(mod, x), WindmillDjangoUnitTest)
                            ]:
                    wmtests.append(ucls.test_dir)

            elif mod.__file__.endswith('__init__.py') or mod.__file__.endswith('__init__.pyc'):
