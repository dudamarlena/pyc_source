...
# [GCC 8.4.0]
# Embedded file name: /home/vaab/lib/python/site-packages/sact/epoch/clock.py
# Compiled at: 2015-01-20 02:15:27
"""
.. :doctest:

"""
import datetime, time, warnings, dateutil.parser
...
def round_date(date):
    ...
    if date.second or date.microsecond:
        return date - datetime.timedelta(seconds=date.second, microseconds=date.microsecond)
    return date
...
class Time(datetime.datetime):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) and isinstance(args[0], basestring):
            if len(args) > 1:
                raise SyntaxError('Too much positional arguments when using Time instanciation by string.')
            ...
            return Time(dateutil.parser.parse(args[0], default=default), **kwargs)
        ...
        if check:
            try:
                ...
                raise ValueError('Encountered datetime method limitation: %s' % str(e))
    ...
    @classmethod
    def from_string(cls, date_str, hint_src_tz, relative=True, formats=None):
        ...
        for f in formats:
            try:
                return cls.strptime(date_str, f, hint_src_tz=hint_src_tz, relative=relative)
            except ValueError:
                pass
    ...
    def strptime(cls, value, format, hint_src_tz, relative=False):
        ...
        input_time = (
         relative.timetuple(), relative.microsecond)
        time_struct, microseconds = strptime(value, format, reference=input_time)
