...
# [GCC 8.4.0]
# Embedded file name: /Users/sam/Documents/fall17/mathviz/mathviz_hopper/src/bottle.py
...
try:
    ...
    def getargspec(func):
        ...
        return (
         args, varargs, keywords, tuple(defaults) or None)
...
class Router(object):
    ...
    def __init__(self, strict=False):
        ...
        self.filters = {'re': lambda conf: (
                _re_flatten(conf or self.default_pattern),
                None, None), 
           'int': lambda conf: (
                 '-?\\d+', int, lambda x: str(int(x))), 
           'float': lambda conf: (
                   '-?[\\d.]+', float, lambda x: str(float(x))), 
...
class BaseResponse(object):
    ...
    bad_headers = {204: frozenset(('Content-Type', 'Content-Length')), 
       304: frozenset(('Allow', 'Content-Encoding', 'Content-Language', 'Content-Length', 'Content-Range',
...
class _ImportRedirect(object):
    ...
    def __init__(self, name, impmask):
        ...
        self.module.__dict__.update({'__file__': __file__, 
           '__path__': [], '__all__': [], '__loader__': self})
...
class AiohttpServer(AsyncioServerAdapter):
    ...
    def run(self, handler):
        ...
        protocol_factory = lambda : WSGIServerHttpProtocol(handler, readpayload=True, debug=not self.quiet)
        self.loop.run_until_complete(self.loop.create_server(protocol_factory, self.host, self.port))
        if 'BOTTLE_CHILD' in os.environ:
            import signal
...
class AutoServer(ServerAdapter):
    """ Untested. """
    adapters = [
     WaitressServer, PasteServer, TwistedServer, CherryPyServer,
...
class Jinja2Template(BaseTemplate):
    ...
    def loader(self, name):
        ...
        with open(fname, 'rb') as (f):
            return (
             f.read().decode(self.encoding), fname, lambda : False)
...
class StplParser(object):
    ...
    _re_cache = {}
    _re_tok = _re_inl = '(?mx)(        # verbose and dot-matches-newline mode\n        [urbURB]*\n        (?:  \'\'(?!\')\n            |""(?!")\n            |\'{6}\n            |"{6}\n            |\'(?:[^\\\\\']|\\\\.)+?\'\n            |"(?:[^\\\\"]|\\\\.)+?"\n            |\'{3}(?:[^\\\\]|\\\\.|\\n)+?\'{3}\n            |"{3}(?:[^\\\\]|\\\\.|\\n)+?"{3}\n        )\n    )'
