...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DrAdm1/newdb.py
# Compiled at: 2009-08-20 16:43:05
"""
...
def main():
    ...
    cli = optparse.OptionParser(description='Setup database for the project. Part of DrAdm1 utilities pack. http://dradm.org', version='1.0', usage='%prog [options] <site> <database>')
    cli.add_option('--verbose', '-v', help='verbose output to the console', action='store_true', default=False)
    cli.add_option('--user', '-U', help='setup alternative username', default=False)
    cli.add_option('--password', '-P', help='setup password instead of password autogeneration', default=False)
    cli.add_option('--database', '-D', help='setup alternative database name', default=False)
    cli.add_option('--project', '-p', help='select project for this virtual host', type='string', default='')
    cli.add_option('--type', '-t', help='type of the DBMS, one of: MYSQL (default), POSTGRES', dest='dbtype', type='string', default='MYSQL')
    try:
    ...
    try:
        db = database.Database(config, databasename, opt.project, 'localhost', opt.dbtype)
        if db.exists():
        ...
        user = dbuser.DbUser(config, username, opt.project, 'localhost', opt.dbtype)
        if not user.exists():
