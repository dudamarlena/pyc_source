...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DrAdm1/mod/backup.py
# Compiled at: 2009-07-13 22:12:22
"""
...
class MysqlBackup:
    ...
    def restore(self, with_data=True, destdb=''):
        ...
        for filename in filter(lambda x: x[-4:] == '.sql', arc.getnames()):
            sql = open(filename, 'r')
            mysql = Popen('mysql -u %s -p%s -h %s -D %s' % (self.mysql_user, self.mysql_pass, self.mysql_host, destdb), shell=True, stdin=sql, stdout=PIPE, stderr=PIPE)
            mysql.wait()
        ...
        if with_data:
            for filename in filter(lambda x: x[-4:] == '.txt', arc.getnames()):
                call(['mysqlimport', '-u%s' % self.mysql_user, '-p%s' % self.mysql_pass, '-L', '%s' % destdb, '%s' % filename], stdout=stdout)
