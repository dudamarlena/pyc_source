...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DrAdm1/siteadd.py
# Compiled at: 2009-07-13 22:24:04
"""
...
def main():
    ...
    cli.add_option('--verbose', '-v', help='increase verbosity of this program', action='store_true', default=False)
    cli.add_option('--force', '-f', help='overwrite existing virtual host config and files if it exists', action='store_true', default=False)
    cli.add_option('--no-database', '-d', help='setup virtual host without database', action='store_true', default=False)
    ...
    try:
        ...
        try:
            ...
            if opt.verbose:
                ...
                if created_ok:
                    print '* Connect string: mysql://%s:%s@%s/%s' % (db_user.getname(), db_user.getpasswd(), db_user.gethost(), db.getname())
        except:
            db_user.drop()
            raise
