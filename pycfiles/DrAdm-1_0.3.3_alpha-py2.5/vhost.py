...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DrAdm1/mod/vhost.py
# Compiled at: 2009-08-17 02:34:02
"""
...
class ApacheVhost:
    ...
    def __init__(self, config, name, project=''):
        ...
        if project:
            ...
            self.hostpath = os.path.join(self.prjroot, self.project, self.name)
            self.prjpath = os.path.join(self.prjroot, self.project)
        else:
    ...
    def add(self):
        ...
        if os.path.exists(self.confname) and not os.path.exists('%s.bak-by-dradm' % self.confname):
