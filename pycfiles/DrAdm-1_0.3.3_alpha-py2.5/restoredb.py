...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DrAdm1/restoredb.py
# Compiled at: 2009-07-15 17:56:08
"""
...
def main():
    ...
    cli = optparse.OptionParser(description='Restore MySQL database from backup. Part of DrAdm1 utilities pack. http://dradm.org', version='1.0', usage='%prog [options] <database_name>')
    cli.add_option('--date', '-d', help='find backup for the selected date (set date as YY-MM-DD), instead mostly fresh backup will used', default=False)
    cli.add_option('--no-data', '-n', help='import only structures with no data', action='store_true', default=False)
    cli.add_option('--database', '-D', help='import into alternative database', default=False)
    cli.add_option('--list', '-l', help='list backup files for the selected database and/or date', action='store_true', default=False)
    cli.add_option('--yes', '-y', help='automatically answer YES on all questions asked by the program', action='store_true', default=False)
