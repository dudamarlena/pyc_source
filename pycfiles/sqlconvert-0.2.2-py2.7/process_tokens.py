...
# [GCC 8.4.0]
# Embedded file name: build/lib.linux-i686-2.7/sqlconvert/process_tokens.py
# Compiled at: 2017-03-19 12:16:11
...
def is_comment_or_space(token):
    return isinstance(token, Comment) or token.ttype in (T.Comment, T.Comment.Single, T.Comment.Multiline,
    ...
    def close(self):
        ...
        for token in tokens:
            if not is_comment_or_space(token):
                raise ValueError('Incomplete SQL statement: %s' % tokens)
