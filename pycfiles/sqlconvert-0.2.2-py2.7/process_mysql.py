...
# [GCC 8.4.0]
# Embedded file name: build/lib.linux-i686-2.7/sqlconvert/process_mysql.py
# Compiled at: 2017-04-15 16:17:08
from sqlparse.sql import Comment, Function, Identifier, Parenthesis, Statement, Token
...
def _is_directive_token(token):
    if isinstance(token, Comment):
        ...
        if subtokens:
            comment = subtokens[0]
            if comment.ttype is T.Comment.Multiline and comment.value.startswith('/*!'):
                return True
    return False
...
def remove_directive_tokens(statement):
    r"""Remove /\*! directives \*/ from the first-level"""
    new_tokens = []
        ...
        if is_comment_or_space(token):
            ...
            if isinstance(token, Parenthesis):
                ...
                if token.value == ',':
                    ...
                    expected = 'END'
                    continue
        raise ValueError('SQL syntax error: expected "%s", got %s "%s"' % (
         expected, token.ttype, token.normalized))

    new_line = Token(T.Newline, '\n')
    new_lines = [new_line]
    for i, values in enumerate(values_tokens):
        if i == len(values_tokens) - 1:
            new_lines = []
        statement = Statement(insert_tokens + [values] + end_tokens + new_lines)
        yield statement
