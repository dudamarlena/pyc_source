...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/madiolahb/acting.py
# Compiled at: 2011-12-07 22:49:51
from core import check_action, GUARANTEED_ROLL, INFLUENCES, max_influence, max_recovery, ROLL_EFFECT
import random
...
def act(lifewheel, influence=None, domain=False, profession=None, **kwargs):
    ...
    cando, is_guaranteed = check_action(lifewheel, False, influence, heroic, profession)
    if not cando:
        lifewheel['errors'].append('%s does not have enough will exerted to perform.' % lifewheel['name'])
        return
    if is_guaranteed:
...
def contest(lifewheel, influence=None, heroic=None, profession=None, object=None, **kwargs):
    self.game.lastinfluence = influence
    cando, is_guaranteed = check_action(self.lifewheel, True, influence, heroic, profession)
    if not cando:
        self.errors.append('%s does not have enough will exerted to perform.' % self.lifewheel.name)
        return
    if is_guaranteed:
        self.game.lastroll = GUARANTEED_ROLL
    else:
