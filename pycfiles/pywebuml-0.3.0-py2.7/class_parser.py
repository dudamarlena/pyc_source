...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywebuml\parsers\static_typed\class_parser.py
# Compiled at: 2011-03-31 21:32:50
"""
...
from pywebuml.parsers.constants import CLASS_KEYWORDS, MODIFIERS
from pywebuml.parsers.exceptions import MissingClosingClassDefinition, MissingOpeningClassDefinition
from pywebuml.models import Class, ClassInheritence, ClassImplements, Enum, Interface, Abstract
from pywebuml.parsers.utils import has_any_keyword, remove_keywords, get_visibility
...
class AbstractStaticTypedClassParser(object):
    ...
    def parse(self, filepath, content, index, owner=None):
        ...
        index, current_class = self.parse_class_signature(filepath, content, index, base_class_package, owner)
        res.append(current_class)
        position = index - 1
        current_line = content[position]
        if '{' in current_line and '}' in current_line and has_any_keyword(current_line, CLASS_KEYWORDS):
            return (
             index, res)
        while True:
    ...
    def parse_attribute(self, content, index, current_class):
        ...
        """
        index, attr = self.get_attribute_parser().parse(current_class, content, index, self.package_manager)
        return index
    ...
    def parse_class_signature(self, filepath, content, index, base_class_package, owner=None):
        ...
        class_name, base_classes, implemented_interfaces = self.get_class_definition_data(class_definition, content)
        index = current_position
        LOGGER.debug('Found class: %s', class_name)
        package = '%s.%s' % (base_class_package, class_name)
        if is_other_type:
            klass = self.get_other_type_class_model(package, class_name, filepath, self.language, owner)
        else:
            klass = self.get_class_model(package, class_name, filepath, self.language, owner)
    ...
    def is_method(self, current_line, current_class):
        ...
        if '=' in current_line and '{' not in current_line:
            return False
        if '(' in current_line and '=' in current_line and current_line.find('(') > current_line.find('='):
            return False
    ...
    def get_other_type_class_model(self, package, class_name, filepath, language, owner):
    ...
    def get_class_model(self, package, class_name, filepath, language, owner=None):
        ...
        """
        if self.is_enum:
            klass = Enum(package, class_name, filepath, language, owner)
        elif self.is_interface:
            klass = Interface(package, class_name, filepath, language, owner)
        elif self.is_abstract:
            klass = Abstract(package, class_name, filepath, language, owner)
        else:
            klass = Class(package, class_name, filepath, language, owner)
        return klass