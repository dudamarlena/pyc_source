...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywebuml\parsers\static_typed\method_parser.py
# Compiled at: 2011-04-02 15:28:30
"""
...
from pywebuml.parsers.exceptions import MissingClosingParenthesis, MissingClosingMethod
from pywebuml.parsers.utils import get_visibility, remove_keywords, has_any_keyword, get_type_and_name
from pywebuml.parsers.constants import ABSTRACT_KEYWORDS, MODIFIERS
LOGGER = getLogger(__name__)

class AbstractStaticTypedMethodParser(object):
    ...
    def parse(self, class_model, content, index):
        ...
        is_abstract = has_any_keyword(current_line, ABSTRACT_KEYWORDS) or isinstance(class_model, Interface)
        ...
        if not is_abstract:
            if '{' in content[current_position] and content[current_position].count('{') == content[current_position].count('}'):
                current_position += 1
            else:
                opened_keys = 0
                if '{' in content[current_position]:
        ...
        if self.should_skip_language_methods(signature):
            ...
            return (current_position,
             Method(name, class_model, signature, visibility, is_static, is_abstract))
