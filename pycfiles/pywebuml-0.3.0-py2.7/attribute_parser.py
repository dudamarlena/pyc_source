...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywebuml\parsers\static_typed\attribute_parser.py
# Compiled at: 2011-04-03 15:30:34
"""
...
from pywebuml.parsers.utils import get_visibility, has_any_keyword, remove_keywords, get_type_and_name
from pywebuml.parsers.constants import CONST_KEYWORDS, LANGUAGE_BASE_VALUES, MODIFIERS
from pywebuml.parsers.exceptions import ClossingArrayException
...
class AbstractStaticTypedAttributeParser(object):
    ...
    def parse(self, class_model, content, index, package_manager):
        ...
        if self.should_update_index(current_line, content, current_position, class_model):
            ...
            found_equals = False
            if (current_line.endswith('=') or '=' not in current_line) and '{' not in current_line:
                current_position += 1
            else:
                if '{' not in current_line and content[(current_position + 1)].startswith('{'):
                    current_position += 1
                while True:
                    ...
                    current_position += 1
                    if opened_keys == 0 and opened_parenthesis == 0 and (line.endswith(';') or line.endswith('}')):
                        break
        ...
        if self.should_skip_language_attributes(current_line):
            ...
            variable_type, name = get_type_and_name(current_line, index, class_model)
            ...
            return (current_position,
             Attribute(name, variable_type, class_model, visibility, is_static, is_final, referenced_class_package))
    ...
    def should_update_index(self, current_line, content, current_position, class_model):
        ...
        if isinstance(class_model, Enum) and (current_line.endswith(';') or current_line.endswith(',') or class_model.programming_language == 'C#'):
            should_iterate = False
        elif not current_line.endswith(';') and '{' not in current_line and '(' not in current_line:
            should_iterate = True
    ...
    def get_referenced_type(self, variable_type, package_manager):
        ...
        if variable_type not in LANGUAGE_BASE_VALUES and '[' not in variable_type and '<' not in variable_type:
            ...
            if '[' in variable_type:
                ...
                if array_class in LANGUAGE_BASE_VALUES:
                    referenced_class_package = variable_type
