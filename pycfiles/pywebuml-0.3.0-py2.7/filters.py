...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywebuml\filters.py
# Compiled at: 2011-04-02 16:47:40
"""
...
def get_class_type(context, current_class):
    ...
    if isinstance(current_class, Enum):
        ...
        if isinstance(current_class, Interface):
            return '<< interface >>'
        if isinstance(current_class, Abstract):
            return '<< abstract >>'
        return ''
...
def should_create_edge(context, class_model, related_package):
    ...
    if not isinstance(class_model, Enum):
        ...
        if class_model.package == related_package:
            return False
        return True


@contextfilter
def create_edge(context, current_package, related_package, related_classname=None, reverse=False):
