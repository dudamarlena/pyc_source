...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywebuml\models.py
# Compiled at: 2011-04-02 16:05:26
"""
Defines de model classes and the connection to them
"""
from app import db
CLASS_TYPES = ('normal', 'inner', 'enum', 'interface', 'abstract')
VISIBILITY_VALUES = ('public', 'protected', 'private')
...
class Class(db.Model):
    ...
    owner_package = db.Column(db.String(1024), db.ForeignKey('classes.package'), nullable=True)
    owner = db.relationship('Class', remote_side=package, backref='inner_classes')
    __mapper_args__ = {'polymorphic_on': discriminator, 'polymorphic_identity': 'normal'}

    def __init__(self, package, name, filepath, programming_language, owner=None):
...
class Interface(Class):
    ...
    def __init__(self, package, name, filepath, programming_language, owner=None):
        super(Interface, self).__init__(package, name, filepath, programming_language, owner)
...
class Abstract(Class):
    ...
    def __init__(self, package, name, filepath, programming_language, owner=None):
        super(Abstract, self).__init__(package, name, filepath, programming_language, owner)
...
class Enum(Class):
    ...
    def __init__(self, package, name, filepath, programming_language, owner=None):
        super(Enum, self).__init__(package, name, filepath, programming_language, owner)
...
class ClassInheritence(db.Model):
    ...
    def __eq__(self, other):
        return self.class_package == other.class_package and self.base_class_package == other.base_class_package
...
class ClassImplements(db.Model):
    ...
    def __eq__(self, other):
        return self.class_package == other.class_package and self.interface_class_package == other.interface_class_package
...
class Attribute(db.Model):
    ...
    def __init__(self, name, type, class_model, visibility, is_static, is_final, referenced_class_package=None):
        self.name = name
        self.type = type
        self.class_model = class_model
    ...
    def __eq__(self, other):
        return self.name == other.name and self.class_package == other.class_package
...
class Method(db.Model):
    ...
    def __init__(self, name, class_model, signature, visibility, is_static, is_abstract):
        self.name = name
        self.class_model = class_model
    ...
    def __eq__(self, other):
        return self.signature == other.signature and self.class_package == other.class_package