...
__doc__ = 'loadmicros.py A script to allow a user to load micro-wire data. Taken from: https://github.com/alafuzof/NeuralynxIO'
...
def load_ncs(file_path, load_time=True, rescale_data=True, signal_scaling='mv'):
    ...
    elif signal_scaling in ('uV', 'µV', 'uV', 'µV'):
        signal_scaling = MICROVOLT_SCALING
    elif signal_scaling in ('V', 'V'):
...
def read_header(fid):
    ...
    pos = fid.tell()
    fid.seek(0)
    raw_hdr = fid.read(HEADER_LENGTH).strip('\x00')
    fid.seek(pos)
    return raw_hdr
