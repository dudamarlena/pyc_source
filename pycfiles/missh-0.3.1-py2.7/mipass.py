...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/mipass.py
...
import socket, threading, SocketServer, os, stat, time, sys, random
verbose = False
conf_fn = '~/.missh'
...
def rand():
    return str(random.randrange(4294967296))
...
def get_key_val(line):
    ...
    if pos < 0:
        return ('', '')
    return (
     line[:pos].strip().lower(), line[pos + 1:].strip())
...
class pass_db():
    ...
    def set_pass(self, id, pwd):
        ...
        """
        if not self.master1 != None:
            raise AssertionError
            return self.check_id(id) or False
        else:
...
class client():
    ...
    def kill(self):
        ...
        if self.connected:
            ...
            self.close()
            return (
             not is_resp_err(resp), get_resp_val(resp))
        else:
            return (
             False, 'No password keeping service found.')
