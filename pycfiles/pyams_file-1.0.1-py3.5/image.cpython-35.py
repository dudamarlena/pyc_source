...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_file/image.py
# Compiled at: 2019-12-20 07:27:45
...
class ThumbnailGeometry:
    __doc__ = 'Image thumbnail geometry'
...
class ImageThumbnailer(ContextAdapter):
    __doc__ = 'Image thumbnailer adapter'
    ...
    def create_thumbnail(self, target, format=None):
        ...
        if isinstance(target, str):
            ...
            if IThumbnailGeometry.providedBy(target):
        ...
        with self.context as (blob):
            try:
                if blob is None:
                    ...
                    new_image = BytesIO()
                    image.resize((width, height), Image.ANTIALIAS).filter(ImageFilter.UnsharpMask(radius=0.5, percent=100, threshold=0)).save(new_image, format)
...
class ImageSelectionThumbnailer(ImageThumbnailer):
    __doc__ = 'Image thumbnailer based on user selection'
        ...
        with self.context as (blob):
            try:
                if blob is None:
                    ...
                    image.crop((geometry.x1, geometry.y1, geometry.x2, geometry.y2)).resize(thumb_size, Image.ANTIALIAS).filter(ImageFilter.UnsharpMask(radius=0.5, percent=100, threshold=0)).save(new_image, format)
                    return (new_image, format.lower())
            finally:
                if blob is not None:
                    blob.close()
...
class ImageRatioThumbnailer(ImageSelectionThumbnailer):
    __doc__ = 'Image thumbnailer with specific ratio'
    ratio = (None, None)
...
class ImagePortraitThumbnailer(ImageRatioThumbnailer):
    __doc__ = 'Image portrait thumbnail adapter'
...
class ImageSquareThumbnailer(ImageRatioThumbnailer):
    __doc__ = 'Image square thumbnail adapter'
...
class ImageHorizontalThumbnailer(ImageRatioThumbnailer):
    __doc__ = 'Image horizontal thumbnailer'

    def get_thumb_size(self, width, height, geometry):
        thumb_size = (
         abs(geometry.x2 - geometry.x1), abs(geometry.y2 - geometry.y1))
        w_ratio = 1.0 * width / thumb_size[0]
        h_ratio = 1.0 * height / thumb_size[1]
...
class ImagePanoThumbnailer(ImageHorizontalThumbnailer):
    __doc__ = 'Image panoramic thumbnail adapter'
...
class ImageCardThumbnailer(ImageHorizontalThumbnailer):
    __doc__ = 'Image card thumbnail adapter'
...
class ImageBannerThumbnailer(ImageRatioThumbnailer):
    __doc__ = 'Image banner thumbnail adapter'
...
class ResponsiveImageThumbnailer(ImageSelectionThumbnailer):
    __doc__ = 'Responsive image thumbnailer'
...
class XsImageThumbnailer(ResponsiveImageThumbnailer):
    __doc__ = 'eXtra-Small responsive image thumbnailer'
...
class SmImageThumbnailer(ResponsiveImageThumbnailer):
    __doc__ = 'SMall responsive image thumbnailer'
...
class MdImageThumbnailer(ResponsiveImageThumbnailer):
    __doc__ = 'MeDium responsive image thumbnailer'
...
class LgImageThumbnailer(ResponsiveImageThumbnailer):
    __doc__ = 'LarGe responsive image thumbnailer'
