...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_file/thumbnail.py
# Compiled at: 2019-12-20 06:55:30
# Size of source mod 2**32: 12981 bytes
"""PyAMS_file.thumbnail module

This module is used to generate images thumbnails.
"""
import logging, re, transaction
...
class ImageThumbnailAdapter:
    __doc__ = 'Image thumbnails adapter'

    def __init__(self, image):
        self.image = image
        self.thumbnails = get_annotation_adapter(image, THUMBNAIL_ANNOTATIONS_KEY, OOBTree.OOBTree, notify=False, locate=False)
    ...
    def set_geometry(self, selection_name, geometry):
        ...
        geometries = get_annotation_adapter(self.image, THUMBNAIL_GEOMETRY_KEY, PersistentDict, notify=False, locate=False)
        geometries[selection_name] = geometry
        for current_thumbnail_name in list(self.thumbnails.keys()):
            if current_thumbnail_name == selection_name or current_thumbnail_name.startswith('{0}:'.format(selection_name)):
    ...
    def get_selection(self, selection_name, format=None):
        ...
        if thumbnailer is not None:
            ...
            if selection is not None:
                ...
                locate(selection, self.image, '++thumb++{0}:{1}x{2}.{3}'.format(selection_name, selection_size[0], selection_size[1], format))
                LOGGER.debug('  > Generated thumbnail selection: {}'.format(selection.__name__))
                registry.notify(ObjectAddedEvent(selection))
                return selection

    def get_thumbnail(self, thumbnail_name, format=None, watermark=None, watermark_position='scale'):
            ...
            if thumbnail_image is not None:
                ...
                if watermark is not None:
                    ...
                    if watermarker is not None:
                        ...
                        thumbnail_image, format = watermarker.add_watermark(thumbnail_image, watermark, watermark_position)
                    ...
                    locate(thumbnail_image, self.image, '++thumb++{0}{1}{2}x{3}.{4}'.format(thumbnailer_name, ':' if thumbnailer_name else '', thumbnail_size[0], thumbnail_size[1], format))
...
class ThumbnailTraverser(ContextAdapter):
    __doc__ = '++thumb++ namespace traverser'
