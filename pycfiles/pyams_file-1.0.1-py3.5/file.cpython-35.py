...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_file/file.py
# Compiled at: 2019-12-20 03:12:02
# Size of source mod 2**32: 15358 bytes
...
from pyams_file.interfaces import FileModifiedEvent, IAudioFile, IBlobReferenceManager, IFile, IFileInfo, IImageFile, ISVGImageFile, IVideoFile
from pyams_utils.adapter import ContextAdapter, adapter_config
from pyams_utils.registry import get_utility
from pyams_utils.request import check_request
__docformat__ = 'restructuredtext'
BLOCK_SIZE = 65536
EXTENSIONS_THUMBNAILS = {'.7z': 'application-x-7z-compressed.png', 
 '.ac3': 'audio-ac3.png', 
...
class File(Persistent, Contained):
    __doc__ = 'Generic file persistent object'
...
class ImageFile(File):
    __doc__ = 'Image file persistent object'
    image_size = (-1, -1)
        ...
        with self as (blob):
            try:
                ...
                if width >= image_size[0] and height >= image_size[1]:
                    return
                new_image = BytesIO()
                w_ratio = 1.0 * width / image_size[0]
                h_ratio = 1.0 * height / image_size[1]
                if keep_ratio:
                    ratio = min(w_ratio, h_ratio)
                    image.resize((round(ratio * image_size[0]), round(ratio * image_size[1])), Image.ANTIALIAS).save(new_image, image.format, quality=99)
                else:
                    image.resize((round(w_ratio * image_size[0]), round(h_ratio * image_size[1])), Image.ANTIALIAS).save(new_image, image.format, quality=99)
                self.data = new_image
    ...
    def crop(self, x1, y1, x2, y2):
        ...
        with self as (blob):
            try:
                image = Image.open(blob)
                new_image = BytesIO()
                image.crop((x1, y1, x2, y2)).save(new_image, image.format, quelity=99)
                self.data = new_image
                request = check_request()
                request.registry.notify(FileModifiedEvent(self))
            finally:
    ...
    def rotate(self, angle=-90):
        ...
        with self as (blob):
            try:
                image = Image.open(blob)
                new_image = BytesIO()
                image.rotate(angle, expand=True).save(new_image, image.format, quality=99)
                self.data = new_image
                request = check_request()
                request.registry.notify(FileModifiedEvent(self))
            finally:
...
class SVGImageFile(File):
    __doc__ = 'SVG image file persistent object'


@implementer(IVideoFile)
class VideoFile(File):
    __doc__ = 'Video file persistent object'


@implementer(IAudioFile)
class AudioFile(File):
    __doc__ = 'Audio file persistent object'
