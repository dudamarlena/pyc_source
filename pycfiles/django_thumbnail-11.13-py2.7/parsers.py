...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/sorl/thumbnail/parsers.py
# Compiled at: 2012-12-12 10:05:53
import re
from sorl.thumbnail.helpers import ThumbnailError, toint
bgpos_pat = re.compile('^(?P<value>\\d+)(?P<unit>%|px)$')
geometry_pat = re.compile('^(?P<x>\\d+)?(?:x(?P<y>\\d+))?$')
...
def parse_crop(crop, xy_image, xy_window):
    ...
    x_alias_percent = {'left': '0%', 
       'center': '50%', 
       'right': '100%'}
    y_alias_percent = {'top': '0%', 
       'center': '50%', 
       'bottom': '100%'}
    xy_crop = crop.split(' ')
    if len(xy_crop) == 1:
