...
# Embedded file name: /home/fede/newhome/projects/bottle-cork/tests/cork/cork.py
# Compiled at: 2015-11-07 16:16:48
...
import bottle, hashlib, os, re, sys, uuid
...
class BaseCork(object):
    ...
    def __init__(self, directory=None, backend=None, email_sender=None, initialize=False, session_domain=None, smtp_server=None, smtp_url='localhost', session_key_name=None):
        ...
        self.password_reset_timeout = 86400
        ...
        if backend is None:
            self._store = JsonBackend(directory, users_fname='users', roles_fname='roles', pending_reg_fname='register', initialize=initialize)
        else:
            self._store = backend
        return
    ...
    def require(self, username=None, role=None, fixed_role=False, fail_redirect=None):
        ...
        if fixed_role and role is None:
            raise AAAException('A role must be specified if fixed_role has been set')
    ...
    def create_user(self, username, role, password, email_addr=None, description=None):
        ...
        if self.current_user.level < 100:
            raise AuthException('The current user is not authorized to create users.')
        if username in self._store.users:
        ...
        h = h.decode('ascii')
        self._store.users[username] = {'role': role, 
           'hash': h, 
    ...
    def register(self, username, password, email_addr, role='user', max_level=50, subject='Signup confirmation', email_template='views/registration_email.tpl', description=None, **kwargs):
        ...
        email_text = bottle.template(email_template, username=username, email_addr=email_addr, role=role, creation_date=creation_date, registration_code=registration_code, **kwargs)
        self.mailer.send_email(email_addr, subject, email_text)
        h = self._hash(username, password)
        h = h.decode('ascii')
        self._store.pending_registrations[registration_code] = {'username': username, 
           'role': role, 
           'hash': h, 
           'email_addr': email_addr, 
           'desc': description, 
    ...
    def validate_registration(self, registration_code):
        ...
        if username in self._store.users:
            raise AAAException('User is already existing.')
        self._store.users[username] = {'role': data['role'], 
           'hash': data['hash'], 
    ...
    def send_password_reset_email(self, username=None, email_addr=None, subject='Password reset confirmation', email_template='views/password_reset_email', **kwargs):
        ...
        email_text = bottle.template(email_template, username=username, email_addr=email_addr, reset_code=reset_code, **kwargs)
        self.mailer.send_email(email_addr, subject, email_text)
        return
    ...
    def reset_password(self, reset_code, password):
        ...
        try:
            ...
            if not isinstance(email_addr, type('')):
                raise AssertionError
                h = isinstance(h, type('')) or h.encode('utf-8')
        ...
        if not isinstance(h, type('')):
            raise AssertionError
            raise (self._verify_password(username, email_addr, h) or AuthException)('Invalid reset code.')
    ...
    def make_auth_decorator(self, username=None, role=None, fixed_role=False, fail_redirect='/login'):
        ...
        def auth_require(username=username, role=role, fixed_role=fixed_role, fail_redirect=fail_redirect):
            ...
            def decorator(func):
                ...
                def wrapper(*a, **ka):
                    session_manager.require(username=username, role=role, fixed_role=fixed_role, fail_redirect=fail_redirect)
                    return func(*a, **ka)
    ...
    def _hash_scrypt(username, pwd, salt=None):
        ...
        if not scrypt_available:
            raise Exception('scrypt.hash required. Please install the scrypt library.')
        if salt is None:
            salt = os.urandom(32)
        assert len(salt) == 32, 'Incorrect salt length'
        cleartext = '%s\x00%s' % (username, pwd)
    ...
    def _verify_password(self, username, pwd, salted_hash):
        ...
        :returns: bool
        """
        assert isinstance(salted_hash, type(''))
    ...
    def _reset_code(self, username, email_addr):
        ...
        t = '%d' % time()
        t = t.encode('utf-8')
        reset_code = (':').join((username.encode('utf-8'),
...
class User(object):
    ...
    def update(self, role=None, pwd=None, email_addr=None):
        ...
        if pwd is not None:
            self._cork._store.users[username]['hash'] = self._cork._hash(username, pwd).decode()
