...
# Embedded file name: /anaconda3/lib/python3.6/site-packages/ackeras/classification.py
# Compiled at: 2018-09-05 04:02:02
# Size of source mod 2**32: 4757 bytes
import pandas as pd, numpy as np, time, pdb
...
class Classification:
    ...
    def __init__(self, X_insample, y_insample, X_outsample=None, y_outsample=None, verbose=100):
        ...
        self.svm_space = {'kernel':[
          'rbf'], 
         'gamma':np.logspace(-9, 9, num=25, base=10), 
         'C':np.logspace(-9, 9, num=25, base=10)}
        self.frst_space = {'n_estimators':list(np.arange(10, 30, 2)), 
         'max_features':list(np.round(np.arange(0.1, 1.0, 0.05), 2)), 
         'criterion':[
          'gini', 'entropy']}
    ...
    def ensable_prediction(self):
        ...
        if X_outsample is not None:
            ...
            svm_pred = svm_clf.predict_proba(X_outsample) if svm_clf is not None else frst_pred
            joint_prob = average_prediction(frst_pred, svm_pred)
        if y_outsample is not None:
            score = confusion_matrix(self.y_outsample, np.argmax(joint_prob, axis=1))
    ...
    def fit_predict(self):
        ...
        if joint_prob is not None:
            print('Outputting joint_probability, you can get the classifiers with .opt_svm and .opt_frst')
            return joint_prob
        else:
            print('Careful, no out of sample data, you can still get the classifiers with .opt_svm and .opt_frst')
            return