...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/visualization_utils.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 45246 bytes
...
import abc, collections, matplotlib
matplotlib.use('Agg')
...
def create_visualization_fn(category_index, include_masks=False, include_keypoints=False, include_track_ids=False, **kwargs):
  ...
  """
    if images.shape[3] > 3:
        ...
        if images.shape[3] == 1:
            ...
            visualization_keyword_args = {'use_normalized_coordinates':use_normalized_coordinates, 
        ...
        visualize_boxes_fn = create_visualization_fn(category_index, include_masks=instance_masks is not None, 
         include_keypoints=keypoints is not None, 
         include_track_ids=track_ids is not None, **visualization_keyword_args)
        elems = [
...
def draw_side_by_side_evaluation_image(eval_dict, category_index, max_boxes_to_draw=20, min_score_thresh=0.2, use_normalized_coordinates=True):
  ...
  """
    ...
    for indx in range(eval_dict[input_data_fields.original_image].shape[0]):
        ...
        if detection_fields.detection_masks in eval_dict:
            instance_masks = tf.cast(tf.expand_dims((eval_dict[detection_fields.detection_masks][indx]),
              axis=0), tf.uint8)
        keypoints = None
        if detection_fields.detection_keypoints in eval_dict:
            keypoints = tf.expand_dims((eval_dict[detection_fields.detection_keypoints][indx]),
              axis=0)
        groundtruth_instance_masks = None
        if input_data_fields.groundtruth_instance_masks in eval_dict:
            groundtruth_instance_masks = tf.cast(tf.expand_dims((eval_dict[input_data_fields.groundtruth_instance_masks][indx]),
              axis=0), tf.uint8)
        images_with_detections = draw_bounding_boxes_on_image_tensors(tf.expand_dims((eval_dict[input_data_fields.original_image][indx]),
          axis=0),
          ...
          axis=0),
          category_index,
          original_image_spatial_shape=tf.expand_dims((eval_dict[input_data_fields.original_image_spatial_shape][indx]),
          axis=0),
          true_image_shape=tf.expand_dims((eval_dict[input_data_fields.true_image_shape][indx]),
          axis=0),
        ...
        images_with_groundtruth = draw_bounding_boxes_on_image_tensors(tf.expand_dims((eval_dict[input_data_fields.original_image][indx]),
          axis=0),
          tf.expand_dims((eval_dict[input_data_fields.groundtruth_boxes][indx]),
          axis=0),
          tf.expand_dims((eval_dict[input_data_fields.groundtruth_classes][indx]),
          axis=0),
          tf.expand_dims(tf.ones_like((eval_dict[input_data_fields.groundtruth_classes][indx]),
          dtype=(tf.float32)),
          axis=0),
          category_index,
          original_image_spatial_shape=tf.expand_dims((eval_dict[input_data_fields.original_image_spatial_shape][indx]),
        ...
        if input_data_fields.image_additional_channels in eval_dict:
            images_with_additional_channels_groundtruth = draw_bounding_boxes_on_image_tensors(tf.expand_dims((eval_dict[input_data_fields.image_additional_channels][indx]),
              axis=0),
              tf.expand_dims((eval_dict[input_data_fields.groundtruth_boxes][indx]),
              axis=0),
              tf.expand_dims((eval_dict[input_data_fields.groundtruth_classes][indx]),
...
def draw_mask_on_image_array(image, mask, color='red', alpha=0.4):
  ...
  """
    ...
    if image.shape[:2] != mask.shape:
        raise ValueError('The image has spatial dimensions %s but the mask has dimensions %s' % (
...
class EvalMetricOpsVisualization(six.with_metaclass(abc.ABCMeta, object)):
    __doc__ = "Abstract base class responsible for visualizations during evaluation.\n\n  Currently, summary images are not run during evaluation. One way to produce\n  evaluation images in Tensorboard is to provide tf.summary.image strings as\n  `value_ops` in tf.estimator.EstimatorSpec's `eval_metric_ops`. This class is\n  responsible for accruing images (with overlaid detections and groundtruth)\n  and returning a dictionary that can be passed to `eval_metric_ops`.\n  "

    def __init__(self, category_index, max_examples_to_draw=5, max_boxes_to_draw=20, min_score_thresh=0.2, use_normalized_coordinates=True, summary_name_prefix='evaluation_image'):
    ...
    """
        ...
        def image_summary_or_default_string(summary_name, image):
            ...
            return tf.cond(tf.equal(tf.size(tf.shape(image)), 4), lambda : tf.summary.image(summary_name, image), lambda : tf.constant(''))
...
class VisualizeSingleFrameDetections(EvalMetricOpsVisualization):
    __doc__ = 'Class responsible for single-frame object detection visualizations.'

    def __init__(self, category_index, max_examples_to_draw=5, max_boxes_to_draw=20, min_score_thresh=0.2, use_normalized_coordinates=True, summary_name_prefix='Detections_Left_Groundtruth_Right'):
        super(VisualizeSingleFrameDetections, self).__init__(category_index=category_index,
    ...
    def images_from_evaluation_dict(self, eval_dict):
        return draw_side_by_side_evaluation_image(eval_dict, self._category_index, self._max_boxes_to_draw, self._min_score_thresh, self._use_normalized_coordinates)