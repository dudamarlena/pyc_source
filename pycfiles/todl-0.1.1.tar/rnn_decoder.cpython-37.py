...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/lstm_object_detection/lstm/rnn_decoder.py
    ...
    with tf.variable_scope(scope) if scope else _NoVariableScope():
        ...
        for local_step in range(sequence_length):
            for sequence_index in range(num_sequences):
                ...
                if pre_bottleneck:
                    decoder_inputs[sequence_index][local_step] = cell.pre_bottleneck(inputs=(decoder_inputs[sequence_index][local_step]),
                      state=(state_tuple[1]),
                      input_index=sequence_index)

            action = generate_action(selection_strategy, local_step, sequence_step, [
             batch_size, 1, 1, 1])
            inputs, _ = select_inputs(decoder_inputs, action, local_step, is_training, is_quantized)
...
def select_inputs(decoder_inputs, action, local_step, is_training, is_quantized, get_alt_inputs=False):
  ...
  """
    ...
    stacked_inputs = tf.stack([decoder_inputs[seq_index][local_step] for seq_index in range(num_seqs)],
      axis=(-1))
    action_index = tf.one_hot(action, num_seqs)
    selected_inputs = lstm_utils.quantize_op((stacked_inputs * action_index), is_training, is_quantized,
    ...
    if get_alt_inputs:
        ...
        selected_inputs = lstm_utils.quantize_op((stacked_inputs * action_index_alt), is_training, is_quantized,
