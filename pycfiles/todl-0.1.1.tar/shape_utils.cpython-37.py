...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/shape_utils.py
  ...
  """
    if isinstance(elems, list):
        ...
        for elem_shape in elem_shapes:
            if not elem_shape and elem_shape[0] or elem_shape[0] != elem_shapes[0][0]:
        ...
        if elems_shape:
            if not elems_shape[0]:
                return tf.map_fn(fn, elems, dtype, parallel_iterations, back_prop)
            outputs = [fn(arg) for arg in tf.unstack(elems)]
        else:
            if all([isinstance(output, tf.Tensor) for output in outputs]):
                return tf.stack(outputs)
            if all([isinstance(output, list) for output in outputs]) and all([all([isinstance(entry, tf.Tensor) for entry in output_list]) for output_list in outputs]):
...
def check_min_image_dim(min_dim, image_tensor):
  ...
  """
    ...
    if image_height is None or image_width is None:
        shape_assert = tf.Assert(tf.logical_and(tf.greater_equal(tf.shape(image_tensor)[1], min_dim), tf.greater_equal(tf.shape(image_tensor)[2], min_dim)), [
         'image size must be >= {} in both height and width.'.format(min_dim)])
        with tf.control_dependencies([shape_assert]):
            return tf.identity(image_tensor)
    if image_height < min_dim or image_width < min_dim:
        raise ValueError('image size must be >= %d in both height and width; image dim = %d,%d' % (
...
def assert_shape_equal(shape_a, shape_b):
  ...
  """
    if all((isinstance(dim, int) for dim in shape_a)):
        if all((isinstance(dim, int) for dim in shape_b)):
            if shape_a != shape_b:
...
def assert_shape_equal_along_first_dimension(shape_a, shape_b):
  ...
  Raises:
    ValueError: When shapes are both static and unequal.
  """
    if isinstance(shape_a[0], int):
        if isinstance(shape_b[0], int):
...
def assert_box_normalized(boxes, maximum_normalized_coordinate=1.1):
  ...
  """
    ...
    return tf.Assert(tf.logical_and(tf.less_equal(box_maximum, maximum_normalized_coordinate), tf.greater_equal(box_minimum, 0)), [
     boxes])
...
def flatten_dimensions(inputs, first, last):
  ...
  """
    if first >= inputs.shape.ndims or last > inputs.shape.ndims:
        raise ValueError('`first` and `last` must be less than inputs.shape.ndims. found {} and {} respectively while ndims is {}'.format(first, last, inputs.shape.ndims))
...
def expand_first_dimension(inputs, dims):
  ...
  """
    ...
    assert_op = tf.assert_equal((inputs_shape[0]),
      (tf.reduce_prod(tf.stack(dims))), message='First dimension of `inputs` cannot be expanded into provided `dims`')
...
def resize_images_and_return_shapes(inputs, image_resizer_fn):
  ...
  """
    ...
    outputs = static_or_dynamic_map_fn(image_resizer_fn,
      elems=inputs,
      dtype=[
     tf.float32, tf.int32])
