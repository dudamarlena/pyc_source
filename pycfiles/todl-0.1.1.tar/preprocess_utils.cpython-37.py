...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/core/preprocess_utils.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 19880 bytes
  ...
  """
    ...
    def flip():
        ...
        for tensor in tensor_list:
            if not dim < 0:
                if dim >= len(tensor.get_shape().as_list()):
  ...
  """
    with tf.name_scope(None, 'pad_to_bounding_box', [image]):
        ...
        if original_dtype != tf.float32:
            if original_dtype != tf.float64:
                image = tf.cast(image, tf.int32)
        image_rank_assert = tf.Assert(tf.logical_or(tf.equal(tf.rank(image), 3), tf.equal(tf.rank(image), 4)), [
        ...
        if image_shape.ndims == 3:
            ...
            if image_shape.ndims is None:
                ...
                offset_assert = tf.Assert(tf.logical_and(tf.greater_equal(after_padding_width, 0), tf.greater_equal(after_padding_height, 0)), [
...
def _crop(image, offset_height, offset_width, crop_height, crop_width):
  ...
  """
    ...
    size_assertion = tf.Assert(tf.logical_and(tf.greater_equal(original_shape[0], crop_height), tf.greater_equal(original_shape[1], crop_width)), [
...
def get_random_scale(min_scale_factor, max_scale_factor, step_size):
  ...
  """
    ...
    if step_size == 0:
        return tf.random_uniform([1], minval=min_scale_factor,
...
def resize_to_range(image, label=None, min_size=None, max_size=None, factor=None, keep_aspect_ratio=True, align_corners=True, label_layout_is_chw=False, scope=None, method=tf.image.ResizeMethod.BILINEAR):
  ...
  """
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if max_size is not None:
            ...
            small_height = tf.cast(tf.floor(orig_height * small_scale_factor), tf.int32)
            small_width = tf.cast(tf.floor(orig_width * small_scale_factor), tf.int32)
            small_size = tf.stack([small_height, small_width])
            new_size = tf.cond(tf.cast(tf.reduce_max(large_size), tf.float32) > max_size, lambda : small_size, lambda : large_size)
        if factor is not None:
            new_size += (factor - (new_size - 1) % factor) % factor
        else:
            if not keep_aspect_ratio:
                new_size = [
                 tf.reduce_max(new_size), tf.reduce_max(new_size)]
            new_tensor_list.append(tf.image.resize(image,
              new_size, method=method, align_corners=align_corners))
            if label is not None:
                if label_layout_is_chw:
                    resized_label = tf.expand_dims(label, 3)
                    resized_label = tf.image.resize(resized_label,
                      new_size,
                      method=(get_label_resize_method(label)),
                      align_corners=align_corners)
                    resized_label = tf.squeeze(resized_label, 3)
                else:
                    resized_label = tf.image.resize(label,
                      new_size,
                      method=(get_label_resize_method(label)),
                      align_corners=align_corners)
