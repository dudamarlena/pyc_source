...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/slim/nets/mobilenet/mobilenet.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 18043 bytes
"""Mobilenet Base Class."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import collections, contextlib, copy, os, tensorflow as tf
...
def apply_activation(x, name=None, activation_fn=None):
    if activation_fn:
        return activation_fn(x, name=name)
    return x
...
def _fixed_padding(inputs, kernel_size, rate=1):
  ...
  """
    ...
    padded_inputs = tf.pad(tensor=inputs,
      paddings=[
     [
      0, 0], [pad_beg[0], pad_end[0]], [pad_beg[1], pad_end[1]],
     [
      0, 0]])
...
def safe_arg_scope(funcs, **kwargs):
  ...
  """
    filtered_args = {name:value for name, value in kwargs.items() if value is not None if value is not None}
    if filtered_args:
...
def mobilenet_base(inputs, conv_defs, multiplier=1.0, final_endpoint=None, output_stride=None, use_explicit_padding=False, scope=None, is_training=False):
  ...
  """
    if multiplier <= 0:
        ...
        if output_stride is not None and (output_stride == 0 or output_stride) > 1:
            if output_stride % 2:
                raise ValueError('Output stride must be None, 1 or a multiple of 2.')
    with _scope_all(scope, default_scope='Mobilenet'):
        with safe_arg_scope([slim.batch_norm], is_training=is_training):
            with _set_arg_scope_defaults(conv_defs_defaults):
                with _set_arg_scope_defaults(conv_defs_overrides):
                    current_stride = 1
                    rate = 1
                    net = inputs
                    end_points = {}
...
def global_pool(input_tensor, pool_op=tf.compat.v2.nn.avg_pool2d):
  ...
  """
    ...
    if shape[1] is None or shape[2] is None:
        ...
        kernel_size = [
         1, shape[1], shape[2], 1]
    output = pool_op(input_tensor,
      ksize=kernel_size, strides=[1, 1, 1, 1], padding='VALID')
...
def training_scope(is_training=True, weight_decay=4e-05, stddev=0.09, dropout_keep_prob=0.8, bn_decay=0.997):
  ...
  """
    batch_norm_params = {'decay':bn_decay, 
    ...
    with slim.arg_scope([
     slim.conv2d, slim.fully_connected, slim.separable_conv2d],
      ...
      normalizer_fn=(slim.batch_norm)):
        with slim.arg_scope([mobilenet_base, mobilenet], is_training=is_training):
            with safe_arg_scope([slim.batch_norm], **batch_norm_params):
                with safe_arg_scope([slim.dropout], is_training=is_training, keep_prob=dropout_keep_prob):
                    with slim.arg_scope([slim.conv2d], weights_regularizer=(slim.l2_regularizer(weight_decay))):
                        with slim.arg_scope([slim.separable_conv2d], weights_regularizer=None) as (s):
                            return s