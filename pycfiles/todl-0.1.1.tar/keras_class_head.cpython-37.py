...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/predictors/heads/keras_class_head.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 14761 bytes
"""Class Head.
...
class ConvolutionalClassHead(head.KerasHead):
    ...
    """
        if use_depthwise:
            ...
            if self._use_depthwise:
                self._class_predictor_layers.append((tf.keras.layers.DepthwiseConv2D)(
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
                 depth_multiplier=1, 
                 strides=1, 
                 dilation_rate=1, 
                 name='ClassPredictor_depthwise', **conv_hyperparams.params()))
                self._class_predictor_layers.append(conv_hyperparams.build_batch_norm(training=(is_training and not freeze_batchnorm),
 ...
 num_predictions_per_location * self._num_class_slots, [1, 1], name='ClassPredictor', **conv_hyperparams.params(use_bias=True)))
            else:
                self._class_predictor_layers.append((tf.keras.layers.Conv2D)(
 ...
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
                 name='ClassPredictor', 
                 bias_initializer=tf.constant_initializer(self._class_prediction_bias_init), **conv_hyperparams.params(use_bias=True)))
...
class MaskRCNNClassHead(head.KerasHead):
    ...
    """
        ...
        self._class_predictor_layers.append(fc_hyperparams.build_batch_norm(training=(is_training and not freeze_batchnorm),
    ...
    """
        if use_depthwise:
            ...
            if self._use_dropout:
                self._class_predictor_layers.append(tf.keras.layers.Dropout(rate=(1.0 - self._dropout_keep_prob)))
            if self._use_depthwise:
                self._class_predictor_layers.append((tf.keras.layers.SeparableConv2D)(
 num_predictions_per_location * self._num_class_slots,
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
                 depth_multiplier=1, 
