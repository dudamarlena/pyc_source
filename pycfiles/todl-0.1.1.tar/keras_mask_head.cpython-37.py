...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/predictors/heads/keras_mask_head.py
...
class ConvolutionalMaskHead(head.KerasHead):
    __doc__ = 'Convolutional class prediction head.'
    ...
    """
        ...
        elif self._use_depthwise:
            self._mask_predictor_layers.append((tf.keras.layers.DepthwiseConv2D)(
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
             depth_multiplier=1, 
             strides=1, 
             dilation_rate=1, 
             name='MaskPredictor_depthwise', **conv_hyperparams.params()))
            self._mask_predictor_layers.append(conv_hyperparams.build_batch_norm(training=(is_training and not freeze_batchnorm),
            ...
            self._mask_predictor_layers.append((tf.keras.layers.Conv2D)(
 num_predictions_per_location * num_mask_channels, [1, 1], name='MaskPredictor', **conv_hyperparams.params(use_bias=True)))
        else:
            self._mask_predictor_layers.append((tf.keras.layers.Conv2D)(
 num_predictions_per_location * num_mask_channels,
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
             name='MaskPredictor', **conv_hyperparams.params(use_bias=True)))
...
class MaskRCNNMaskHead(head.KerasHead):
 ...
 [
  3, 3], padding='SAME', 
             name='MaskPredictor_conv2d_{}'.format(i), **self._conv_hyperparams.params()))
            self._mask_predictor_layers.append(self._conv_hyperparams.build_batch_norm(training=(self._is_training and not self._freeze_batchnorm),
        ...
        if self._convolve_then_upsample:
            ...
            self._mask_predictor_layers.append(tf.keras.layers.Lambda(lambda features: ops.nearest_neighbor_upsampling(features,
              height_scale=height_scale, width_scale=width_scale)))
            self._mask_predictor_layers.append((tf.keras.layers.Conv2D)(
 num_conv_channels,
 [
  3, 3], padding='SAME', 
             name='MaskPredictor_upsample_conv2d', **self._conv_hyperparams.params()))
            self._mask_predictor_layers.append(self._conv_hyperparams.build_batch_norm(training=(self._is_training and not self._freeze_batchnorm),
 ...
 [
  3, 3], padding='SAME', 
         name='MaskPredictor_last_conv2d', **self._conv_hyperparams.params(use_bias=True)))
        self.built = True
    ...
    """
        if not self._convolve_then_upsample:
            features = tf.image.resize_bilinear(features,
              [self._mask_height, self._mask_width], align_corners=True)
        mask_predictions = features
        for layer in self._mask_predictor_layers:
            mask_predictions = layer(mask_predictions)

        return tf.expand_dims(tf.transpose(mask_predictions, perm=[0, 3, 1, 2]),
          axis=1,
          name='MaskPredictor')


class WeightSharedConvolutionalMaskHead(head.KerasHead):
    __doc__ = 'Weight shared convolutional mask prediction head based on Keras.'
