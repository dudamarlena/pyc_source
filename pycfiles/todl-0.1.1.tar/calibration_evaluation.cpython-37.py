...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/metrics/calibration_evaluation.py
...
class CalibrationDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    __doc__ = 'Class to evaluate calibration detection metrics.'
    ...
    """
        ...
        matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=iou_threshold,
          unmatched_threshold=iou_threshold)
    ...
    """
        ...
        gt_classes_one_hot = tf.one_hot(groundtruth_classes,
          one_hot_depth, dtype=(tf.float32))
        one_hot_cls_targets, _, _, _, _ = self._target_assigner.assign(det_boxlist,
          gt_boxlist,
          gt_classes_one_hot,
          unmatched_class_label=tf.zeros(shape=one_hot_depth, dtype=(tf.float32)))
        cls_targets = tf.argmax(one_hot_cls_targets, axis=1)
        is_class_matched = tf.cast((tf.equal(tf.cast(cls_targets, tf.int64), detection_classes)),
          dtype=(tf.int64))
