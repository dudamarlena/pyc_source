...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/builders/calibration_builder.py
...
def _find_interval_containing_new_value(x, new_value):
    ...
    compare = tf.cast((tf.reshape(new_value, shape=(new_value_shape, 1)) >= tf.reshape(x, shape=(1, x_shape))),
      dtype=(tf.int32))
...
def _function_approximation_proto_to_tf_tensors(x_y_pairs_message):
  ...
  """
    tf_x = tf.convert_to_tensor([x_y_pair.x for x_y_pair in x_y_pairs_message.x_y_pair],
      dtype=(tf.float32))
    tf_y = tf.convert_to_tensor([x_y_pair.y for x_y_pair in x_y_pairs_message.x_y_pair],
      dtype=(tf.float32))
    return (tf_x, tf_y)
  ...
  """
    if calibration_config.WhichOneof('calibrator') == 'function_approximation':
        ...
        def calibration_fn(class_predictions_with_background):
            flat_class_predictions_with_background = tf.reshape(class_predictions_with_background,
              shape=[-1])
            ...
            calibrated_class_predictions_with_background = tf.reshape(updated_scores,
              shape=original_detections_shape,
              name='calibrate_scores')
            return calibrated_class_predictions_with_background

    else:
        if calibration_config.WhichOneof('calibrator') == 'class_id_function_approximations':
            ...
            def calibration_fn(class_predictions_with_background):
                ...
                for class_id, class_tensor in enumerate(class_tensors):
                    ...
                    if class_id in class_id_function_dict:
                        output_tensor = _tf_linear_interp1d(x_to_interpolate=flat_class_tensor,
            ...
            if calibration_config.WhichOneof('calibrator') == 'temperature_scaling_calibration':
                ...
                def calibration_fn(class_predictions_with_background):
                    ...
                    calibrated_class_predictions_with_background = tf.math.divide(class_predictions_with_background,
                      scaler,
