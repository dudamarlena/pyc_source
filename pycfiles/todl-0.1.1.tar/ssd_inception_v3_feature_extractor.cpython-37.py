...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/models/ssd_inception_v3_feature_extractor.py
...
class SSDInceptionV3FeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
    __doc__ = 'SSD Feature Extractor using InceptionV3 features.'
    ...
    """
        super(SSDInceptionV3FeatureExtractor, self).__init__(is_training=is_training,
        ...
        if not self._override_base_feature_extractor_hyperparams:
            raise ValueError('SSD Inception V3 feature extractor always usesscope returned by `conv_hyperparams_fn` for both the base feature extractor and the additional layers added since there is no arg_scope defined for the base feature extractor.')
    ...
    """
        ...
        feature_map_layout = {'from_layer':[
          'Mixed_5d', 'Mixed_6e', 'Mixed_7c', '', '', ''][:self._num_layers], 
         'layer_depth':[
          -1, -1, -1, 512, 256, 128][:self._num_layers], 
         'use_explicit_padding':self._use_explicit_padding, 
         'use_depthwise':self._use_depthwise}
        with slim.arg_scope(self._conv_hyperparams_fn()):
            with tf.variable_scope('InceptionV3', reuse=(self._reuse_weights)) as (scope):
                _, image_features = inception_v3.inception_v3_base((ops.pad_to_multiple(preprocessed_inputs, self._pad_to_multiple)),
