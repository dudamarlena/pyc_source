...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/core/dense_prediction_cell.py
...
class DensePredictionCell(object):
    __doc__ = "DensePredictionCell class used as a 'layer' in semantic segmentation."
    ...
    """
        if config[_OP] == _PYRAMID_POOLING:
            config[_TARGET_SIZE], config[_KERNEL] = self._get_pyramid_pooling_arguments(crop_size=crop_size,
    ...
    """
        batch_norm_params = {'is_training':is_training and fine_tune_batch_norm, 
         'decay':0.9997, 
         'epsilon':1e-05, 
         'scale':True}
          ...
          reuse=reuse):
            with (slim.arg_scope)([slim.batch_norm], **batch_norm_params):
                with tf.variable_scope(scope, _META_ARCHITECTURE_SCOPE, [features]):
                    ...
                    concat_logits = slim.conv2d(concat_logits, (self.hparams['concat_channels']),
                      1,
