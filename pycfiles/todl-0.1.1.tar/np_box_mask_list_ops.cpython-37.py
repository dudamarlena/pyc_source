...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/np_box_mask_list_ops.py
  ...
  """
    if fields is not None:
        if 'masks' not in fields:
            fields.append('masks')
    return box_list_to_box_mask_list(np_box_list_ops.gather(boxlist=box_mask_list,
      indices=indices,
      fields=fields))
...
def sort_by_field(box_mask_list, field, order=np_box_list_ops.SortOrder.DESCEND):
  ...
  Returns:
    sorted_box_mask_list: A sorted BoxMaskList with the field in the specified
      order.
  """
    return box_list_to_box_mask_list(np_box_list_ops.sort_by_field(boxlist=box_mask_list,
      field=field,
      order=order))
...
def multi_class_non_max_suppression(box_mask_list, score_thresh, iou_thresh, max_output_size):
  ...
  """
    ...
    elif not isinstance(box_mask_list, np_box_mask_list.BoxMaskList):
        raise ValueError('box_mask_list must be a box_mask_list')
    elif not box_mask_list.has_field('scores'):
    ...
    for class_idx in range(num_classes):
        box_mask_list_and_class_scores = np_box_mask_list.BoxMaskList(box_data=(box_mask_list.get()),
