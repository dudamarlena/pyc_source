...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/exporter.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 22619 bytes
"""Functions to export object detection inference graph."""
import os, tempfile, tensorflow as tf
...
def rewrite_nn_resize_op(is_quantized=False):
  ...
  """
    ...
    def remove_nn():
        ...
        stack_1_pattern = graph_matcher.OpTypePattern('Pack',
          inputs=[input_pattern, input_pattern], ordered_inputs=False)
        stack_2_pattern = graph_matcher.OpTypePattern('Pack',
          inputs=[stack_1_pattern, stack_1_pattern], ordered_inputs=False)
        reshape_pattern = graph_matcher.OpTypePattern('Reshape',
          inputs=[stack_2_pattern, 'Const'], ordered_inputs=False)
        consumer_pattern = graph_matcher.OpTypePattern('Add|AddV2|Max|Mul',
          inputs=[reshape_pattern, '*'], ordered_inputs=False)
        match_counter = 0
        ...
        for match in matcher.match_graph(tf.get_default_graph()):
            ...
            nn_resize = tf.image.resize_nearest_neighbor((projection_op.outputs[0]),
...
def replace_variable_values_with_moving_averages(graph, current_checkpoint_file, new_checkpoint_file, no_ema_collection=None):
  ...
  """
    with graph.as_default():
        ...
        ema_variables_to_restore = config_util.remove_unecessary_ema(ema_variables_to_restore, no_ema_collection)
...
def _tf_example_input_placeholder(input_shape=None):
  ...
  """
    ...
    return (batch_tf_example_placeholder,
     shape_utils.static_or_dynamic_map_fn(decode,
...
def add_output_tensor_nodes(postprocessed_tensors, output_collection_name='inference_op'):
  ...
  """
    ...
    outputs = {}
    outputs[detection_fields.detection_boxes] = tf.identity(boxes,
      name=(detection_fields.detection_boxes))
    outputs[detection_fields.detection_scores] = tf.identity(scores,
      name=(detection_fields.detection_scores))
    if multiclass_scores is not None:
        outputs[detection_fields.detection_multiclass_scores] = tf.identity(multiclass_scores,
          name=(detection_fields.detection_multiclass_scores))
    if box_classifier_features is not None:
        outputs[detection_fields.detection_features] = tf.identity(box_classifier_features,
          name=(detection_fields.detection_features))
    outputs[detection_fields.detection_classes] = tf.identity(classes,
      name=(detection_fields.detection_classes))
    outputs[detection_fields.num_detections] = tf.identity(num_detections,
      name=(detection_fields.num_detections))
    if raw_boxes is not None:
        outputs[detection_fields.raw_detection_boxes] = tf.identity(raw_boxes,
          name=(detection_fields.raw_detection_boxes))
    if raw_scores is not None:
        outputs[detection_fields.raw_detection_scores] = tf.identity(raw_scores,
          name=(detection_fields.raw_detection_scores))
    if keypoints is not None:
        outputs[detection_fields.detection_keypoints] = tf.identity(keypoints,
          name=(detection_fields.detection_keypoints))
    if masks is not None:
        outputs[detection_fields.detection_masks] = tf.identity(masks,
          name=(detection_fields.detection_masks))
  ...
  """
    with tf.Graph().as_default():
        with tf.Session() as (sess):
            ...
            detection_signature = tf.saved_model.signature_def_utils.build_signature_def(inputs=tensor_info_inputs,
              outputs=tensor_info_outputs,
              method_name=(tf.saved_model.signature_constants.PREDICT_METHOD_NAME))
            builder.add_meta_graph_and_variables(sess,
              [
             tf.saved_model.tag_constants.SERVING],
              signature_def_map={tf.saved_model.signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY: detection_signature})
            builder.save()
...
def build_detection_graph(input_type, detection_model, input_shape, output_collection_name, graph_hook_fn):
    ...
    if input_shape is not None:
        if input_type != 'image_tensor':
            if input_type != 'encoded_image_string_tensor':
                if input_type != 'tf_example':
                    raise ValueError('Can only specify input shape for `image_tensor`, `encoded_image_string_tensor`, or `tf_example` inputs.')
        placeholder_args['input_shape'] = input_shape
    placeholder_tensor, input_tensors = (input_placeholder_fn_map[input_type])(**placeholder_args)
    outputs = _get_outputs_from_inputs(input_tensors=input_tensors,
...
def _export_inference_graph(input_type, detection_model, use_moving_averages, trained_checkpoint_prefix, output_directory, additional_output_tensor_names=None, input_shape=None, output_collection_name='inference_op', graph_hook_fn=None, write_inference_graph=False, temp_checkpoint_prefix=''):
    """Export helper."""
    tf.gfile.MakeDirs(output_directory)
    frozen_graph_path = os.path.join(output_directory, 'frozen_inference_graph.pb')
    saved_model_path = os.path.join(output_directory, 'saved_model')
    model_path = os.path.join(output_directory, 'model.ckpt')
    outputs, placeholder_tensor = build_detection_graph(input_type=input_type,
      detection_model=detection_model,
    ...
    write_graph_and_checkpoint(inference_graph_def=(tf.get_default_graph().as_graph_def()),
    ...
    elif additional_output_tensor_names is not None:
        output_node_names = ','.join(outputs.keys() + additional_output_tensor_names)
    else:
        output_node_names = ','.join(outputs.keys())
    frozen_graph_def = freeze_graph.freeze_graph_with_def_protos(input_graph_def=(tf.get_default_graph().as_graph_def()),
      input_saver_def=input_saver_def,
...
def profile_inference_graph(graph):
  ...
  """
    ...
    tfprof_vars_option['trim_name_regexes'] = [
     '.*BatchNorm.*']
    tfprof_flops_option['trim_name_regexes'] = [
     '.*BatchNorm.*', '.*Initializer.*', '.*Regularizer.*', '.*BiasAdd.*']
    tf.contrib.tfprof.model_analyzer.print_model_analysis(graph,
