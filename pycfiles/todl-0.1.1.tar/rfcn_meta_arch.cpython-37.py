...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/meta_architectures/rfcn_meta_arch.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 19099 bytes
...
class RFCNMetaArch(faster_rcnn_meta_arch.FasterRCNNMetaArch):
    __doc__ = 'R-FCN Meta-architecture definition.'
    ...
    """
        super(RFCNMetaArch, self).__init__(is_training,
          num_classes,
          image_resizer_fn,
          feature_extractor,
          number_of_stages,
        ...
        if self._rfcn_box_predictor.is_keras_model:
            ...
            box_predictions = self._rfcn_box_predictor.predict([
             ...
             1],
              ...
              proposal_boxes=proposal_boxes_normalized)
        refined_box_encodings = tf.squeeze(tf.concat((box_predictions[box_predictor.BOX_ENCODINGS]), axis=1),
          axis=1)
        class_predictions_with_background = tf.squeeze(tf.concat((box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]),
          axis=1),
          axis=1)
        absolute_proposal_boxes = ops.normalized_to_image_coordinates(proposal_boxes_normalized,
          image_shape, parallel_iterations=(self._parallel_iterations))
