...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/smc.py
...
def _blend_tensor(blending_weights, tensor, num_particles, batch_size):
  ...
  """
    tensor = tf.transpose((tf.reshape(tensor, [num_particles, batch_size, -1])),
      perm=[1, 2, 0])
...
def relaxed_resampling(log_weights, states, num_particles, batch_size, temperature=0.5, random_seed=None):
  ...
  """
    resampling_parameters = tf.transpose(log_weights, perm=[1, 0])
    resampling_dist = tf.contrib.distributions.RelaxedOneHotCategorical(temperature,
...
def smc(transition_fn, num_steps, num_particles=1, resampling_criterion=ess_criterion, resampling_fn=multinomial_resampling, loop_fn=None, parallel_iterations=30, swap_memory=True):
  ...
  """
    ...
    seq_mask = tf.transpose(tf.sequence_mask(num_steps, maxlen=max_num_steps, dtype=(tf.float32)),
      perm=[
     1, 0])
    seq_mask = tf.tile(seq_mask, [1, num_particles])
    mask_ta = tf.TensorArray((seq_mask.dtype), max_num_steps,
      name='mask_ta')
    ...
    def while_step(t, state, tas, log_weights_acc, log_z_hat_acc):
        ...
        log_z_hat_update = tf.reduce_logsumexp(log_weights_acc,
          axis=0) - tf.log(tf.to_float(num_particles))
        log_z_hat_acc += tf.cond(t < max_num_steps - 1, lambda : log_z_hat_update * resampled, lambda : log_z_hat_update)
        ta_updates = [
         log_weights_acc, resampled]
        new_tas = [ta.write(t, x) for ta, x in zip(tas, ta_updates)]
        log_weights_acc *= 1.0 - tf.tile(resampled[tf.newaxis, :], [
         num_particles, 1])
        new_state = (new_particle_state, new_loop_state)
        return (t + 1, new_state, new_tas, log_weights_acc, log_z_hat_acc)

    _, final_state, tas, _, log_z_hat = tf.while_loop(while_predicate,
      while_step,
      loop_vars=(
     t0, init_states, tas, log_weights_acc, log_z_hat_acc),
