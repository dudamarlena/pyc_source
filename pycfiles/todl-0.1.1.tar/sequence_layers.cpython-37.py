...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/attention_ocr/python/sequence_layers.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 15086 bytes
...
import collections, abc, logging, numpy as np, tensorflow as tf
...
class SequenceLayerBase(object):
    __doc__ = 'A base abstruct class for all sequence layers.\n\n  A child class has to define following methods:\n    get_train_input\n    get_eval_input\n    unroll_cell\n  '
    ...
    """
        ...
        self._softmax_w = slim.model_variable('softmax_w',
          [
         self._mparams.num_lstm_units, self._params.num_char_classes],
          initializer=orthogonal_initializer,
          regularizer=regularizer)
        self._softmax_b = slim.model_variable('softmax_b',
          [self._params.num_char_classes], initializer=(tf.zeros_initializer()),
          regularizer=regularizer)
    ...
    """
        with tf.variable_scope('LSTM'):
            ...
            lstm_cell = tf.contrib.rnn.LSTMCell((self._mparams.num_lstm_units),
              use_peepholes=False,
              cell_clip=(self._mparams.lstm_state_clip_value),
              state_is_tuple=True,
              initializer=orthogonal_initializer)
            lstm_outputs, _ = self.unroll_cell(decoder_inputs=decoder_inputs,
...
class NetSlice(SequenceLayerBase):
    __doc__ = 'A layer which uses a subset of image features to predict each character.\n  '
    ...
    def unroll_cell(self, decoder_inputs, initial_state, loop_function, cell):
        ...
        return tf.contrib.legacy_seq2seq.rnn_decoder(decoder_inputs=decoder_inputs,
...
class Attention(SequenceLayerBase):
    __doc__ = 'A layer which uses attention mechanism to select image features.'
    ...
    def unroll_cell(self, decoder_inputs, initial_state, loop_function, cell):
        return tf.contrib.legacy_seq2seq.attention_decoder(decoder_inputs=decoder_inputs,
          initial_state=initial_state,
...
class AttentionWithAutoregression(Attention):
    __doc__ = 'A layer which uses both attention and auto regression.'
...
def get_layer_class(use_attention, use_autoregression):
  ...
  """
    if use_attention and use_autoregression:
        ...
        if use_attention:
            layer_class = use_autoregression or Attention
        else:
            if not use_attention:
                layer_class = use_autoregression or NetSlice
            else:
                if not use_attention:
