...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/per_image_evaluation.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 34228 bytes
...
class PerImageEvaluation(object):
    __doc__ = 'Evaluate detection result of a single image.'
    ...
    """
        ...
        scores, tp_fp_labels = self._compute_tp_fp(detected_boxes=detected_boxes,
        ...
        is_class_correctly_detected_in_image = self._compute_cor_loc(detected_boxes=detected_boxes,
          detected_scores=detected_scores,
          detected_class_labels=detected_class_labels,
          groundtruth_boxes=groundtruth_boxes,
          groundtruth_class_labels=groundtruth_class_labels,
    ...
    """
        if detected_masks is not None and groundtruth_masks is None or detected_masks is None:
            if groundtruth_masks is not None:
                raise ValueError('If `detected_masks` is provided, then `groundtruth_masks` should also be provided.')
        is_class_correctly_detected_in_image = np.zeros((self.num_groundtruth_classes),
          dtype=int)
        for i in range(self.num_groundtruth_classes):
            ...
            is_class_correctly_detected_in_image[i] = self._compute_is_class_correctly_detected_in_image(detected_boxes=detected_boxes_at_ith_class,
            ...
            if groundtruth_boxes.size > 0:
                ...
                if detected_masks is not None:
                    if groundtruth_masks is not None:
                        mask_mode = True
                if mask_mode:
                    detected_boxlist = np_box_mask_list.BoxMaskList(box_data=np.expand_dims((detected_boxes[max_score_id]), axis=0),
                      mask_data=np.expand_dims((detected_masks[max_score_id]), axis=0))
                    gt_boxlist = np_box_mask_list.BoxMaskList(box_data=groundtruth_boxes,
    ...
    """
        ...
        for i in range(self.num_groundtruth_classes):
            ...
            gt_boxes_at_ith_class, gt_masks_at_ith_class, detected_boxes_at_ith_class, detected_scores_at_ith_class, detected_masks_at_ith_class = self._get_ith_class_arrays(detected_boxes, detected_scores, detected_masks, detected_class_labels, groundtruth_boxes, groundtruth_masks, groundtruth_class_labels, i)
            scores, tp_fp_labels = self._compute_tp_fp_for_single_class(detected_boxes=detected_boxes_at_ith_class,
              detected_scores=detected_scores_at_ith_class,
              groundtruth_boxes=gt_boxes_at_ith_class,
        ...
        return (
         result_scores, result_tp_fp_labels)
    ...
    """
        detected_boxlist = np_box_mask_list.BoxMaskList(box_data=detected_boxes,
        ...
        gt_non_group_of_boxlist = np_box_mask_list.BoxMaskList(box_data=(groundtruth_boxes[(~groundtruth_is_group_of_list)]),
          mask_data=(groundtruth_masks[(~groundtruth_is_group_of_list)]))
        gt_group_of_boxlist = np_box_mask_list.BoxMaskList(box_data=(groundtruth_boxes[groundtruth_is_group_of_list]),
          mask_data=(groundtruth_masks[groundtruth_is_group_of_list]))
    ...
    """
        ...
        if mask_mode:
            ...
            iou_mask, ioa_mask, scores, num_detected_boxes = self._get_overlaps_and_scores_mask_mode(detected_boxes=detected_boxes,
              detected_scores=detected_scores,
              detected_masks=detected_masks,
              groundtruth_boxes=(groundtruth_boxes[mask_presence_indicator, :]),
              groundtruth_masks=(groundtruth_masks[mask_presence_indicator, :]),
              groundtruth_is_group_of_list=(groundtruth_is_group_of_list[mask_presence_indicator]))
            if sum(mask_presence_indicator) < len(mask_presence_indicator):
                iou, ioa, _, num_detected_boxes = self._get_overlaps_and_scores_box_mode(detected_boxes=detected_boxes,
                  detected_scores=detected_scores,
                  groundtruth_boxes=(groundtruth_boxes[~mask_presence_indicator, :]),
                  groundtruth_is_group_of_list=(groundtruth_is_group_of_list[(~mask_presence_indicator)]))
            num_detected_boxes = detected_boxes.shape[0]
        else:
            mask_presence_indicator = np.zeros((groundtruth_is_group_of_list.shape),
              dtype=bool)
            iou, ioa, scores, num_detected_boxes = self._get_overlaps_and_scores_box_mode(detected_boxes=detected_boxes,
              detected_scores=detected_scores,
              groundtruth_boxes=groundtruth_boxes,
              groundtruth_is_group_of_list=groundtruth_is_group_of_list)
        if groundtruth_boxes.size == 0:
            return (
             scores, np.zeros(num_detected_boxes, dtype=bool))
        ...
        def compute_match_ioa(ioa, is_box):
            ...
            return (
             scores_group_of, tp_fp_labels_group_of)
        ...
        if iou_mask.shape[1] > 0:
            ...
            compute_match_iou(iou_mask,
              (groundtruth_is_difficult_mask_list[(~groundtruth_is_group_of_mask_list)]),
              is_box=False)
        scores_mask_group_of = np.ndarray([0], dtype=float)
        tp_fp_labels_mask_group_of = np.ndarray([0], dtype=float)
        if ioa_mask.shape[1] > 0:
            scores_mask_group_of, tp_fp_labels_mask_group_of = compute_match_ioa(ioa_mask,
              is_box=False)
        if iou.shape[1] > 0:
        ...
        if mask_mode:
            ...
            return (
             np.concatenate((
              scores[valid_entries], scores_mask_group_of)),
             np.concatenate((
