...
# [GCC 8.4.0]
# Embedded file name: /usr/local/google/home/rikel/bandits_repo/deep-bayesian-contextual-bandits/research/bandits/algorithms/bf_variational_neural_bandit_model.py
# Compiled at: 2018-07-23 14:29:50
# Size of source mod 2**32: 12563 bytes
...
class BfVariationalNeuralBanditModel(BayesianNN):
    __doc__ = 'Implements an approximate Bayesian NN using Variational Inference.'
    ...
    """
        ...
        return (
         output_h, output_h_local, neg_kl)
    ...
    """
        ...
        def weight_prior(dtype, shape, c, d, e):
            ...
            del d
            del e
            return tfd.Independent(tfd.Normal(loc=(tf.zeros(shape, dtype)), scale=(tf.to_float(np.sqrt(2) / shape[0]))),
        ...
        if self.verbose:
            ...
            output_layer = tfl.DenseLocalReparameterization((self.n_out),
            ...
            if getattr(self.hparams, 'infer_noise_sigma', False):
                log_likelihood = log_gaussian((self.y),
                  (self.y_pred), (self.noise_sigma), reduce_sum=False)
            else:
                log_likelihood = log_gaussian((self.y),
                  (self.y_pred), (self.hparams.noise_sigma), reduce_sum=False)
        ...
        self.train_op = tf.train.AdamOptimizer(self.hparams.initial_lr).minimize((self.loss),
