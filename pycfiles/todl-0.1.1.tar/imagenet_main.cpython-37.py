...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/r1/resnet/imagenet_main.py
...
def _parse_example_proto(example_serialized):
  ...
  """
    ...
    feature_map.update({k:sparse_float32 for k in ('image/object/bbox/xmin', 'image/object/bbox/ymin',
    ...
    return (
     features['image/encoded'], label, bbox)
...
def parse_record(raw_record, is_training, dtype):
  ...
  """
    ...
    image = imagenet_preprocessing.preprocess_image(image_buffer=image_buffer,
      bbox=bbox,
...
def get_synth_input_fn(dtype):
    return resnet_run_loop.get_synth_input_fn(DEFAULT_IMAGE_SIZE,
      DEFAULT_IMAGE_SIZE, NUM_CHANNELS, NUM_CLASSES, dtype=dtype)


class ImagenetModel(resnet_model.Model):
    __doc__ = 'Model class with appropriate defaults for Imagenet data.'
    ...
    """
        if resnet_size < 50:
            ...
            bottleneck = True
        super(ImagenetModel, self).__init__(resnet_size=resnet_size,
...
def _get_block_sizes(resnet_size):
  ...
  """
    ...
    try:
        ...
        err = 'Could not find layers for selected Resnet size.\nSize received: {}; sizes allowed: {}.'.format(resnet_size, list(choices.keys()))
...
def define_imagenet_flags():
    resnet_run_loop.define_resnet_flags(resnet_size_choices=[
...
def run_imagenet(flags_obj):
  ...
  """
    ...
    result = resnet_run_loop.resnet_main(flags_obj,
      imagenet_model_fn, input_function, DATASET_NAME, shape=[
     DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, NUM_CHANNELS])
    return result
