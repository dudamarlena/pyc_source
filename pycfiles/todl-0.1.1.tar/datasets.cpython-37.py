...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/data/datasets.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 18927 bytes
...
def create_pianoroll_dataset(path, split, batch_size, num_parallel_calls=DEFAULT_PARALLELISM, shuffle=False, repeat=False, min_note=21, max_note=108):
  ...
  """
    ...
    dataset = tf.data.Dataset.from_generator(pianoroll_generator,
      output_types=(
     tf.float64, tf.int64),
      output_shapes=(
     [
    ...
    def process_pianoroll_batch(data, lengths):
        ...
        targets = data
        inputs = data - tf.constant(mean, dtype=(tf.float32), shape=[
         1, 1, mean.shape[0]])
    ...
    dataset = dataset.map(process_pianoroll_batch, num_parallel_calls=num_parallel_calls)
...
def create_human_pose_dataset(path, split, batch_size, num_parallel_calls=DEFAULT_PARALLELISM, shuffle=False, repeat=False):
  ...
  """
    ...
    def pose_generator():
        for pose_sequence in pose_sequences:
            yield (
             pose_sequence, pose_sequence.shape[0])

    dataset = tf.data.Dataset.from_generator(pose_generator,
      output_types=(
     tf.float64, tf.int64),
      output_shapes=(
     [
    ...
    dataset = dataset.padded_batch(batch_size,
      padded_shapes=([None, num_features], []))

    def process_pose_data(data, lengths):
        data = tf.to_float(tf.transpose(data, perm=[1, 0, 2]))
        lengths = tf.to_int32(lengths)
        targets = data
        inputs = data - tf.constant(mean,
          dtype=(tf.float32), shape=[1, 1, mean.shape[0]])
        inputs = tf.pad(inputs, [[1, 0], [0, 0], [0, 0]], mode='CONSTANT')[:-1]
        inputs *= tf.expand_dims(tf.transpose(tf.sequence_mask(lengths, dtype=(inputs.dtype))), 2)
        return (inputs, targets, lengths)

    dataset = dataset.map(process_pose_data,
...
def create_speech_dataset(path, batch_size, samples_per_timestep=200, num_parallel_calls=DEFAULT_PARALLELISM, prefetch_buffer_size=2048, shuffle=False, repeat=False):
  ...
  """
    ...
    dataset = tf.data.TFRecordDataset(filenames).map(read_speech_example,
      num_parallel_calls=num_parallel_calls)
    ...
    dataset = dataset.padded_batch(batch_size,
      padded_shapes=([None, samples_per_timestep], []))
    ...
    dataset = dataset.map(process_speech_batch, num_parallel_calls=num_parallel_calls)
    dataset = dataset.prefetch(prefetch_buffer_size)
    itr = dataset.make_one_shot_iterator()
    inputs, targets, lengths = itr.get_next()
    return (inputs, targets, lengths)
...
def create_chain_graph_dataset(batch_size, num_timesteps, steps_per_observation=None, state_size=1, transition_variance=1.0, observation_variance=1.0, transition_type=STANDARD_TRANSITION, observation_type=STANDARD_OBSERVATION, fixed_observation=None, prefetch_buffer_size=2048, dtype='float32'):
  ...
  """
    ...
    def data_generator():
        ...
        while True:
            states = []
            observations = []
            states.append(np.random.normal(size=[state_size], scale=observation_std).astype(dtype))
            for t in xrange(1, num_timesteps + 1):
                if transition_type == ROUND_TRANSITION:
                    loc = np.round(states[(-1)])
                else:
                    if transition_type == STANDARD_TRANSITION:
                        loc = states[(-1)]
                ...
                if t % steps_per_observation == 0:
                    if fixed_observation is None:
                        if observation_type == SQUARED_OBSERVATION:
                            ...
                            if observation_type == ABS_OBSERVATION:
                                loc = np.abs(states[(-1)])
                            else:
                                if observation_type == STANDARD_OBSERVATION:
                                    loc = states[(-1)]
                        x_t = np.random.normal(size=[state_size], loc=loc,
    ...
    dataset = tf.data.Dataset.from_generator(data_generator,
      output_types=(
     tf.as_dtype(dtype), tf.as_dtype(dtype)),
      output_shapes=(
     [
      num_timesteps + 1, state_size],
     [
