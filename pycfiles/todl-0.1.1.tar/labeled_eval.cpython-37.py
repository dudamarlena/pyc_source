...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/tcn/labeled_eval.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 12036 bytes
...
import estimators.get_estimator as get_estimator
...
def nearest_cross_sequence_neighbors(data, tasks, n_neighbors=1):
  ...
  """
    ...
    for idx in range(num_data):
        ...
        _, nearest_indices = zip(*sorted(distances,
          key=(lambda x: x[0]))[:n_neighbors])
...
def compute_cross_sequence_recalls_at_k(embeddings, labels, label_attr_keys, tasks, k_list, summary_writer, training_step):
  ...
  """
    ...
    indices = nearest_cross_sequence_neighbors(embeddings,
      tasks, n_neighbors=(max(k_list)))
    ...
    for idx, label_attr in enumerate(label_attr_keys):
        ...
        recall_list = compute_cross_sequence_recall_at_k(retrieved_labels=problem_retrieved,
        ...
        for k, recall in zip(k_list, recall_list):
            ...
            summ = tf.Summary(value=[
             tf.Summary.Value(tag=('validation/classification/%s error@top%d' % (
              label_attr, k)),
    ...
    for i in range(recall_lists.shape[1]):
        ...
        summ = tf.Summary(value=[
         tf.Summary.Value(tag=('validation/classification/average error@top%d' % k_list[i]),
           simple_value=recall_error)])
...
def evaluate_once(estimator, input_fn_by_view, batch_size, checkpoint_path, label_attr_keys, embedding_size, num_views, k_list):
  ...
  """
    ...
    for view_index in range(num_views):
        predictions = estimator.inference((input_fn_by_view(view_index)),
          checkpoint_path, batch_size,
          predict_keys=eval_tensor_keys)
...
def main(_):
    ...
    def input_fn_by_view(view_index):
        ...
        def input_fn():
            preprocessed_images, labels, tasks = data_providers.labeled_data_provider(labeled_tables,
              (estimator.preprocess_data),
              view_index, image_attr_keys, label_attr_keys,
