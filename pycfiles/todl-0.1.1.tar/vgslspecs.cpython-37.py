...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/street/python/vgslspecs.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 19594 bytes
...
import nn_ops, shapes
from six.moves import xrange
import tensorflow as tf
import tensorflow.contrib.slim as slim

class VGSLSpecs(object):
    __doc__ = 'Layers that can be built from a string definition.'
    ...
    """
        ...
        self.reduction_factors = [
         1.0, 1.0, 1.0, 1.0]
        self.valid_ops = [
         self.AddSeries, self.AddParallel, self.AddConvLayer,
    ...
    """
        ...
        final_factors = None
        while index < len(self.model_str):
            if self.model_str[index] != ')':
                ...
                if num_dims == 0:
                    ...
                    if num_dims != len(layer.get_shape()):
        ...
        return (
         tf.concat(axis=(num_dims - 1), values=layers), index + 1)
    ...
    """
        ...
        return (
         slim.conv2d(prev_layer,
           depth, [height, width], activation_fn=fn, scope=name), m.end())
    ...
    """
        ...
        return (
         slim.max_pool2d(prev_layer,
           [height, width], [y_stride, x_stride], padding='SAME',
           scope=name), m.end())
    ...
    """
        ...
        if m is None:
            return (None, None)
        name = self._GetLayerName(m.group(0), index, m.group(2))
        layer = slim.dropout(prev_layer,
          0.5, is_training=(self.is_training), scope=name)
        return (layer, m.end())
    ...
    """
        ...
        layer = shapes.transposing_reshape(prev_layer,
          src_dim, part_a, part_b, dest_dim_a, dest_dim_b, name=name)
    ...
    """
        ...
        shaped = tf.reshape(prev_layer,
          [-1, input_depth], name=(name + '_reshape_in'))
        ...
        return (
         tf.reshape(output,
           [shapes.tensor_dim(prev_layer, 0), 1, 1, depth], name=(name + '_reshape_out')), m.end())
    ...
    """
        ...
        inputs = tf.reshape(inputs,
          [-1, num_steps, input_depth], name=(name + '_reshape_in'))
        ...
        if summarize:
            outputs = tf.slice(outputs,
              [0, num_steps - 1, 0], [-1, 1, -1], name=(name + '_sum_slice'))
            outputs = tf.reshape(outputs,
              [input_batch, num_slices, 1, output_depth], name=(name + '_reshape_out'))
        else:
            outputs = tf.reshape(outputs,
              [input_batch, num_slices, num_steps, output_depth], name=(name + '_reshape_out'))
        if dim == 'y':
