...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/models/base.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 14517 bytes
"""Reusable model classes for FIVO."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import sonnet as snt, tensorflow as tf
from fivo import nested_utils as nested
tfd = tf.contrib.distributions

class ELBOTrainableSequenceModel(object):
    __doc__ = 'An abstract class for ELBO-trainable sequence models to extend.\n\n  Because the ELBO, IWAE, and FIVO bounds all accept the same arguments,\n  any model that is ELBO-trainable is also IWAE- and FIVO-trainable.\n  '
    ...
    """
        ...
        self.observations_ta = nested.tas_for_tensors(observations,
          (self.max_seq_len), clear_after_read=False)
...
class ConditionalNormalDistribution(object):
    __doc__ = 'A Normal distribution conditioned on Tensor inputs via a fc network.'

    def __init__(self, size, hidden_layer_sizes, sigma_min=0.0, raw_sigma_bias=0.25, hidden_activation_fn=tf.nn.relu, initializers=None, name='conditional_normal_distribution'):
    ...
    """
        ...
        self.fcnet = snt.nets.MLP(output_sizes=(hidden_layer_sizes + [2 * size]),
...
class ConditionalBernoulliDistribution(object):
    __doc__ = 'A Bernoulli distribution conditioned on Tensor inputs via a fc net.'

    def __init__(self, size, hidden_layer_sizes, hidden_activation_fn=tf.nn.relu, initializers=None, bias_init=0.0, name='conditional_bernoulli_distribution'):
    ...
    """
        ...
        if initializers is None:
            initializers = DEFAULT_INITIALIZERS
        self.fcnet = snt.nets.MLP(output_sizes=(hidden_layer_sizes + [size]),
          activation=hidden_activation_fn,
          initializers=initializers,
          activate_final=False,
          use_bias=True,
...
class NormalApproximatePosterior(ConditionalNormalDistribution):
    __doc__ = 'A Normally-distributed approx. posterior with res_q parameterization.'

    def __init__(self, size, hidden_layer_sizes, sigma_min=0.0, raw_sigma_bias=0.25, hidden_activation_fn=tf.nn.relu, initializers=None, smoothing=False, name='conditional_normal_distribution'):
        super(NormalApproximatePosterior, self).__init__(size,
          hidden_layer_sizes, sigma_min=sigma_min, raw_sigma_bias=raw_sigma_bias,
    ...
    """
        if not initializers:
            ...
            if not inputs_per_timestep:
                inputs_per_timestep = [
                 1] * num_timesteps
            outputs_per_timestep = outputs_per_timestep or [
             1] * num_timesteps
        ...
        self.log_variances = _get_variables_ta(bias_shapes, 'proposal_log_variance_%d', initializers['b'])
        self.mean_biases = _get_variables_ta(bias_shapes, 'proposal_b_%d', initializers['b'])
        weight_shapes = zip(inputs_per_timestep, outputs_per_timestep)
        self.mean_weights = _get_variables_ta(weight_shapes, 'proposal_w_%d', initializers['w'])
        self.shapes = tf.TensorArray(dtype=(tf.int32),
          size=num_timesteps,
          dynamic_size=False,
          clear_after_read=False).unstack(weight_shapes)
