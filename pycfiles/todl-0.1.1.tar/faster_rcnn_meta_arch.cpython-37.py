...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/meta_architectures/faster_rcnn_meta_arch.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 138151 bytes
...
"""
import abc, functools, tensorflow as tf
...
class FasterRCNNFeatureExtractor(object):
    __doc__ = 'Faster R-CNN Feature Extractor definition.'
...
class FasterRCNNKerasFeatureExtractor(object):
    __doc__ = 'Keras-based Faster R-CNN Feature Extractor definition.'
...
class FasterRCNNMetaArch(model.DetectionModel):
    __doc__ = 'Faster R-CNN Meta-architecture definition.'
    ...
    """
        ...
        if not isinstance(first_stage_anchor_generator, grid_anchor_generator.GridAnchorGenerator):
            ...
            if isinstance(first_stage_box_predictor_arg_scope_fn, hyperparams_builder.KerasLayerHyperparams):
                ...
                self._first_stage_box_predictor_first_conv = tf.keras.Sequential([
                 (tf.keras.layers.Conv2D)(
 self._first_stage_box_predictor_depth, kernel_size=[
 self._first_stage_box_predictor_kernel_size,
 self._first_stage_box_predictor_kernel_size], 
                  dilation_rate=self._first_stage_atrous_rate, 
                  padding='SAME', 
                  name='RPNConv', **conv_hyperparams.params()),
                 conv_hyperparams.build_batch_norm((self._is_training and not freeze_batchnorm),
                   name='RPNBatchNorm'),
                 tf.keras.layers.Lambda((tf.nn.relu6),
                   name='RPNActivation')],
                  name='FirstStageRPNFeatures')
                self._first_stage_box_predictor = box_predictor_builder.build_convolutional_keras_box_predictor(is_training=(self._is_training),
                  num_classes=1,
            ...
            def rpn_box_predictor_feature_extractor(rpn_features_to_crop):
                with slim.arg_scope(self._first_stage_box_predictor_arg_scope_fn()):
                    ...
                    return slim.conv2d(rpn_features_to_crop,
            ...
            self._first_stage_box_predictor = box_predictor_builder.build_convolutional_box_predictor(is_training=(self._is_training),
              num_classes=1,
    ...
    """
        ...
        self._batched_prediction_tensor_names = [x for x in prediction_dict if x not in ('image_shape',
                                                                                         'anchors')]
        ...
        if self._is_training:
            if self.clip_anchors_to_image:
                anchors_boxlist = box_list_ops.clip_to_window(anchors_boxlist,
                  clip_window, filter_nonoverlapping=False)
            else:
            ...
            anchors_boxlist = box_list_ops.clip_to_window(anchors_boxlist,
              clip_window, filter_nonoverlapping=(not self._use_static_shapes))
        self._anchors = anchors_boxlist
        prediction_dict = {'rpn_box_predictor_features': rpn_box_predictor_features, 
         ...
         'rpn_objectness_predictions_with_background': tf.cast(rpn_objectness_predictions_with_background, dtype=(tf.float32)), 
         
         'anchors': anchors_boxlist.data['boxes'], 
    ...
    """
        ...
        if self._mask_rcnn_box_predictor.is_keras_model:
            ...
            box_predictions = self._mask_rcnn_box_predictor.predict([
             ...
             1],
              ...
              prediction_stage=2)
        refined_box_encodings = tf.squeeze((box_predictions[box_predictor.BOX_ENCODINGS]),
          axis=1,
          name='all_refined_box_encodings')
        class_predictions_with_background = tf.squeeze((box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]),
          axis=1,
        ...
        prediction_dict = {'refined_box_encodings':tf.cast(refined_box_encodings, dtype=tf.float32), 
    ...
    """
        box_encodings_batch = tf.reshape(refined_box_encodings, [
        ...
        raw_detection_boxes_normalized = shape_utils.static_or_dynamic_map_fn((self._normalize_and_clip_boxes),
          elems=[
         raw_detection_boxes_absolute, true_image_shapes],
          dtype=(tf.float32))
        detection_feature_map_indices = tf.zeros_like((raw_detection_boxes_normalized[:, :, :, 0]),
          dtype=(tf.int32))
        return {fields.PredictionFields.raw_detection_boxes: raw_detection_boxes_normalized, 
    ...
    def _extract_box_classifier_features(self, flattened_feature_maps):
        ...
        elif self._feature_extractor_for_box_classifier_features:
            box_classifier_features = self._feature_extractor_for_box_classifier_features(flattened_feature_maps)
        else:
            box_classifier_features = self._feature_extractor.extract_box_classifier_features(flattened_feature_maps,
    ...
    """
        if self._is_training:
            ...
            if self._mask_rcnn_box_predictor.is_keras_model:
                ...
                mask_predictions = self._mask_rcnn_box_predictor.predict([
                 ...
                 1],
                  scope=(self.second_stage_box_predictor_scope),
                  prediction_stage=3)
            prediction_dict['mask_predictions'] = tf.squeeze((mask_predictions[box_predictor.MASK_PREDICTIONS]),
              axis=1)
        else:
            ...
            if self._mask_rcnn_box_predictor.is_keras_model:
                ...
                mask_predictions = self._mask_rcnn_box_predictor.predict([
                 ...
                 1],
                  scope=(self.second_stage_box_predictor_scope),
                  prediction_stage=3)
            detection_masks = tf.squeeze((mask_predictions[box_predictor.MASK_PREDICTIONS]),
              axis=1)
            ...
            prediction_dict[fields.DetectionResultFields.detection_masks] = tf.reshape(tf.sigmoid(detection_masks), [
    ...
    """
        ...
        anchors = box_list_ops.concatenate(self._first_stage_anchor_generator.generate([
         (feature_map_shape[1],
          feature_map_shape[2])]))
    ...
    def _extract_proposal_features(self, preprocessed_inputs):
        ...
        elif self._feature_extractor_for_proposal_features:
            proposal_features = (self._feature_extractor_for_proposal_features(preprocessed_inputs), {})
        else:
            proposal_features = self._feature_extractor.extract_proposal_features(preprocessed_inputs,
    ...
    """
        ...
        elif self._first_stage_box_predictor.is_keras_model:
        ...
        box_encodings = tf.concat((box_predictions[box_predictor.BOX_ENCODINGS]),
          axis=1)
        objectness_predictions_with_background = tf.concat((box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]),
          axis=1)
    ...
    """
        ...
        def _batch_gather_kept_indices(predictions_tensor):
            return shape_utils.static_or_dynamic_map_fn(functools.partial((tf.gather), indices=keep_indices),
        ...
        with tf.name_scope('FirstStagePostprocessor'):
            if self._number_of_stages == 1:
                ...
                return {fields.DetectionResultFields.detection_boxes: proposal_boxes, 
                 ...
                 fields.DetectionResultFields.detection_multiclass_scores: proposal_multiclass_scores, 
                 
                 fields.DetectionResultFields.num_detections: tf.cast(num_proposals, dtype=(tf.float32)), 
                 
                 fields.DetectionResultFields.raw_detection_boxes: raw_proposal_boxes, 
                 
                 fields.DetectionResultFields.raw_detection_scores: raw_proposal_scores}
        if (self._number_of_stages == 2 or self._number_of_stages) == 3:
            if self._is_training:
                with tf.name_scope('SecondStagePostprocessor'):
                    mask_predictions = prediction_dict.get(box_predictor.MASK_PREDICTIONS)
                    detections_dict = self._postprocess_box_classifier((prediction_dict['refined_box_encodings']),
                      (prediction_dict['class_predictions_with_background']),
                      (prediction_dict['proposal_boxes']),
                      (prediction_dict['num_proposals']),
                      true_image_shapes,
                      mask_predictions=mask_predictions)
                if 'rpn_features_to_crop' in prediction_dict:
                    if self._initial_crop_size:
    ...
    """
        with tf.name_scope('SecondStageDetectionFeaturesExtract'):
            ...
            detection_features_pool = tf.reduce_mean(detection_features_unpooled,
              axis=[1, 2])
            reshaped_detection_features_pool = tf.reshape(detection_features_pool, [
            ...
            reshaped_detection_features = tf.reshape(detection_features_unpooled, [
    ...
    """
        ...
        proposal_boxes, proposal_scores, _, _, nmsed_additional_fields, num_proposals = self._first_stage_nms_fn(tf.expand_dims(raw_proposal_boxes, axis=2),
        ...
        def normalize_boxes(args):
            ...
            image_shape = args[1]
            normalized_boxes_per_image = box_list_ops.to_normalized_coordinates((box_list.BoxList(proposal_boxes_per_image)),
              (image_shape[0]), (image_shape[1]),
              check_range=False).get()
            return normalized_boxes_per_image

        normalized_proposal_boxes = shape_utils.static_or_dynamic_map_fn(normalize_boxes,
          elems=[proposal_boxes, image_shapes], dtype=(tf.float32))
        raw_normalized_proposal_boxes = shape_utils.static_or_dynamic_map_fn(normalize_boxes,
          elems=[
         raw_proposal_boxes, image_shapes],
          dtype=(tf.float32))
    ...
    """
        ...
        for single_image_proposal_boxes, single_image_proposal_scores, single_image_num_proposals, single_image_groundtruth_boxlist, single_image_groundtruth_classes_with_background, single_image_groundtruth_weights in zip(tf.unstack(proposal_boxes), tf.unstack(proposal_scores), tf.unstack(num_proposals), groundtruth_boxlists, groundtruth_classes_with_background_list, groundtruth_weights_list):
            ...
            sampled_padded_boxlist = box_list_ops.pad_or_clip_box_list(sampled_boxlist,
        ...
        return (
         tf.stack(single_image_proposal_box_sample),
    ...
    """
        ...
        for one_hot_encoding in self.groundtruth_lists(fields.BoxListFields.classes):
            groundtruth_classes_with_background_list.append(tf.cast(tf.pad(one_hot_encoding, [[0, 0], [1, 0]], mode='CONSTANT'),
              dtype=(tf.float32)))

        groundtruth_masks_list = self._groundtruth_lists.get(fields.BoxListFields.masks)
        if groundtruth_masks_list is not None:
            if self._resize_masks:
                resized_masks_list = []
                for mask in groundtruth_masks_list:
                    _, resized_mask, _ = self._image_resizer_fn(image=(tf.zeros(tf.stack([tf.shape(mask)[1],
                     tf.shape(mask)[2], 1]))),
                      masks=mask)
    ...
    """
        ...
        return box_list_ops.boolean_mask(proposal_boxlist,
          selected_positions,
          use_static_shapes=(self._use_static_shapes),
          indicator_sum=(self._second_stage_batch_size if self._use_static_shapes else None))
    ...
    """
        cropped_regions = self._flatten_first_two_dimensions(self._crop_and_resize_fn(features_to_crop, proposal_boxes_normalized, [
    ...
    """
        refined_box_encodings_batch = tf.reshape(refined_box_encodings, [
         ...
         self._box_coder.code_size])
        class_predictions_with_background_batch = tf.reshape(class_predictions_with_background, [
        ...
        class_predictions_batch = tf.reshape(tf.slice(class_predictions_with_background_batch_normalized, [
         0, 0, 1], [-1, -1, -1]), [
        ...
        if mask_predictions is not None:
            ...
            mask_predictions_batch = tf.reshape(mask_predictions, [-1, self.max_num_proposals,
             self.num_classes, mask_height, mask_width])
        else:
            batch_size = shape_utils.combined_static_and_dynamic_shape(refined_box_encodings_batch)[0]
            batch_anchor_indices = tf.tile((tf.expand_dims(tf.range(self.max_num_proposals), 0)),
              multiples=[
             batch_size, 1])
            additional_fields = {'multiclass_scores':class_predictions_with_background_batch_normalized, 
             'anchor_indices':tf.cast(batch_anchor_indices, tf.float32)}
            nmsed_boxes, nmsed_scores, nmsed_classes, nmsed_masks, nmsed_additional_fields, num_detections = self._second_stage_nms_fn(refined_decoded_boxes_batch,
            ...
            if refined_decoded_boxes_batch.shape[2] > 1:
                class_ids = tf.expand_dims(tf.argmax((class_predictions_with_background_batch[:, :, 1:]), axis=2, output_type=(tf.int32)),
                  axis=(-1))
                raw_detection_boxes = tf.squeeze((tf.batch_gather(refined_decoded_boxes_batch, class_ids)),
                  axis=2)
            else:
                raw_detection_boxes = tf.squeeze(refined_decoded_boxes_batch, axis=2)
        raw_normalized_detection_boxes = shape_utils.static_or_dynamic_map_fn((self._normalize_and_clip_boxes),
          elems=[
         raw_detection_boxes, image_shapes],
          dtype=(tf.float32))
        detections = {fields.DetectionResultFields.detection_boxes: nmsed_boxes, 
         
         fields.DetectionResultFields.detection_scores: nmsed_scores, 
         
         fields.DetectionResultFields.detection_classes: nmsed_classes, 
         
         fields.DetectionResultFields.detection_multiclass_scores: nmsed_additional_fields['multiclass_scores'], 
         
         fields.DetectionResultFields.detection_anchor_indices: tf.cast(nmsed_additional_fields['anchor_indices'], tf.int32), 
         
         fields.DetectionResultFields.num_detections: tf.cast(num_detections, dtype=(tf.float32)), 
         
         fields.DetectionResultFields.raw_detection_boxes: raw_normalized_detection_boxes, 
         
         fields.DetectionResultFields.raw_detection_scores: class_predictions_with_background_batch_normalized}
    ...
    """
        ...
        return tf.reshape(decoded_boxes.get(), tf.stack([combined_shape[0], combined_shape[1],
         num_classes, 4]))
    ...
    def _normalize_and_clip_boxes(self, boxes_and_image_shape):
        ...
        normalized_boxes_per_image = box_list_ops.to_normalized_coordinates((box_list.BoxList(boxes_per_image)),
          (image_shape[0]),
          (image_shape[1]),
          check_range=False).get()
        normalized_boxes_per_image = box_list_ops.clip_to_window((box_list.BoxList(normalized_boxes_per_image)),
    ...
    """
        with tf.name_scope('RPNLoss'):
            batch_cls_targets, batch_cls_weights, batch_reg_targets, batch_reg_weights, _ = target_assigner.batch_assign_targets(target_assigner=(self._proposal_target_assigner),
            ...
            batch_sampled_indices = tf.cast(shape_utils.static_or_dynamic_map_fn(_minibatch_subsample_fn,
              [
            ...
            batch_one_hot_targets = tf.one_hot(tf.cast(batch_cls_targets, dtype=(tf.int32)),
              depth=2)
            ...
            localization_losses = self._first_stage_localization_loss(rpn_box_encodings,
              batch_reg_targets, weights=sampled_reg_indices, losses_mask=losses_mask)
            objectness_losses = self._first_stage_objectness_loss(rpn_objectness_predictions_with_background,
            ...
            localization_loss = tf.multiply((self._first_stage_loc_loss_weight), localization_loss,
              name='localization_loss')
            objectness_loss = tf.multiply((self._first_stage_obj_loss_weight), objectness_loss,
    ...
    """
        with tf.name_scope('BoxClassifierLoss'):
            ...
            num_proposals_or_one = tf.cast((tf.expand_dims(tf.maximum(num_proposals, tf.ones_like(num_proposals)), 1)),
              dtype=(tf.float32))
            normalizer = tf.tile(num_proposals_or_one, [
             1, self.max_num_proposals]) * batch_size
            batch_cls_targets_with_background, batch_cls_weights, batch_reg_targets, batch_reg_weights, _ = target_assigner.batch_assign_targets(target_assigner=(self._detector_target_assigner),
            ...
            flat_cls_targets_with_background = tf.reshape(batch_cls_targets_with_background, [
             batch_size * self.max_num_proposals, -1])
            one_hot_flat_cls_targets_with_background = tf.argmax(flat_cls_targets_with_background,
              axis=1)
            ...
            if refined_box_encodings.shape[1] == 1:
                reshaped_refined_box_encodings = tf.reshape(refined_box_encodings, [
            ...
            if self.groundtruth_has_field(fields.InputDataFields.is_annotated):
                losses_mask = tf.stack(self.groundtruth_lists(fields.InputDataFields.is_annotated))
            second_stage_loc_losses = self._second_stage_localization_loss(reshaped_refined_box_encodings,
              batch_reg_targets,
              weights=batch_reg_weights,
              losses_mask=losses_mask) / normalizer
            second_stage_cls_losses = ops.reduce_sum_trailing_dimensions(self._second_stage_classification_loss(class_predictions_with_background,
            ...
            localization_loss = tf.multiply((self._second_stage_loc_loss_weight), second_stage_loc_loss,
              name='localization_loss')
            classification_loss = tf.multiply((self._second_stage_cls_loss_weight), second_stage_cls_loss,
              name='classification_loss')
            loss_dict = {'Loss/BoxClassifierLoss/localization_loss':localization_loss, 
             'Loss/BoxClassifierLoss/classification_loss':classification_loss}
            ...
            if prediction_masks is not None:
                if groundtruth_masks_list is None:
                    ...
                    batch_mask_targets, _, _, batch_mask_target_weights, _ = target_assigner.batch_assign_targets(target_assigner=(self._detector_target_assigner),
                      anchors_batch=proposal_boxlists,
                      gt_box_batch=groundtruth_boxlists,
                ...
                reshaped_prediction_masks = tf.reshape(prediction_masks_masked_by_class_targets, [
                ...
                flat_normalized_proposals = box_list_ops.to_normalized_coordinates((box_list.BoxList(tf.reshape(proposal_boxes, [-1, 4]))),
                  (image_shape[1]),
                  (image_shape[2]), check_range=False).get()
                flat_cropped_gt_mask = self._crop_and_resize_fn(tf.expand_dims(flat_gt_masks, -1), tf.expand_dims(flat_normalized_proposals, axis=1), [
                 mask_height, mask_width])
                flat_cropped_gt_mask = tf.stop_gradient(flat_cropped_gt_mask)
                batch_cropped_gt_mask = tf.reshape(flat_cropped_gt_mask, [
                 batch_size, -1, mask_height * mask_width])
                mask_losses_weights = batch_mask_target_weights * tf.cast(paddings_indicator, dtype=(tf.float32))
                mask_losses = self._second_stage_mask_loss(reshaped_prediction_masks,
            ...
            if second_stage_mask_loss is not None:
                mask_loss = tf.multiply((self._second_stage_mask_loss_weight), second_stage_mask_loss,
                  name='mask_loss')
    ...
    """
        ...
        batch_cls_targets_with_background, _, _, _, _ = target_assigner.batch_assign_targets(target_assigner=(self._detector_target_assigner),
          anchors_batch=proposal_boxlists,
          gt_box_batch=groundtruth_boxlists,
          gt_class_targets_batch=groundtruth_classes_with_background_list,
          unmatched_class_label=tf.constant(([
    ...
    def _get_refined_encodings_for_postitive_class(self, refined_box_encodings, flat_cls_targets_with_background, batch_size):
        refined_box_encodings_with_background = tf.pad(refined_box_encodings, [
         [
          0, 0], [1, 0], [0, 0]])
        refined_box_encodings_masked_by_class_targets = box_list_ops.boolean_mask((box_list.BoxList(tf.reshape(refined_box_encodings_with_background, [
         -1, self._box_coder.code_size]))),
          (tf.reshape(tf.greater(flat_cls_targets_with_background, 0), [-1])),
          use_static_shapes=(self._use_static_shapes),
          indicator_sum=(batch_size * self.max_num_proposals if self._use_static_shapes else None)).get()
        return tf.reshape(refined_box_encodings_masked_by_class_targets, [
    ...
    """
        for proposal_boxlist, single_image_loc_loss, single_image_cls_loss, single_image_num_proposals in zip(proposal_boxlists, tf.unstack(second_stage_loc_losses), tf.unstack(second_stage_cls_losses), tf.unstack(num_proposals)):
            proposal_boxlist = box_list.BoxList(tf.slice(proposal_boxlist.get(), [
            ...
            single_image_cls_loss = tf.slice(single_image_cls_loss, [
             0], [single_image_num_proposals])
            return self._hard_example_miner(location_losses=(tf.expand_dims(single_image_loc_loss, 0)),
    ...
    """
        if fine_tune_checkpoint_type not in ('detection', 'classification'):
            ...
            include_patterns = load_all_detection_checkpoint_vars or [
             self.first_stage_feature_extractor_scope,
             self.second_stage_feature_extractor_scope]
        feature_extractor_variables = contrib_framework.filter_variables(variables_to_restore,
