...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/core/nas_network.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 14892 bytes
...
def config(num_conv_filters=20, total_training_steps=500000, drop_path_keep_prob=1.0):
    return contrib_training.HParams(filter_scaling_rate=2.0,
...
def nas_arg_scope(weight_decay=4e-05, batch_norm_decay=0.9997, batch_norm_epsilon=0.001, sync_batch_norm_method='None'):
    ...
    weights_initializer = contrib_layers.variance_scaling_initializer(factor=0.3333333333333333,
      mode='FAN_IN',
      uniform=True)
    with arg_scope([slim.fully_connected, slim.conv2d, slim.separable_conv2d], weights_regularizer=weights_regularizer,
      weights_initializer=weights_initializer):
        with arg_scope([slim.fully_connected], activation_fn=None,
          scope='FC'):
            with arg_scope([slim.conv2d, slim.separable_conv2d], activation_fn=None,
  ...
  """
    with tf.variable_scope(scope, 'nas', [images], reuse=reuse):
        ...
        for cell_num in range(len(backbone)):
            ...
            if cell_num == 0:
                ...
                if backbone[cell_num] == backbone[(cell_num - 1)] + 1:
                    stride = 2
                    if backbone[cell_num] == 3:
                        if nas_remove_os32_stride:
                            stride = 1
                    filter_scaling *= hparams.filter_scaling_rate
                else:
                    if backbone[cell_num] == backbone[(cell_num - 1)] - 1:
        ...
        if nas_use_classification_head:
            output_stride_to_expanded_filters = {8:256, 
             16:512,  32:1024}
            ...
            if current_output_stride != 32:
                ...
                for i in range(num_downsampling):
                    ...
                    net = resnet_utils.conv2d_same(net,
                      target_filters, 3, stride=stride, rate=rate, scope=(scope + '_conv'))
...
def pnasnet(images, num_classes, is_training=True, global_pool=False, output_stride=16, nas_architecture_options=None, nas_training_hyper_parameters=None, reuse=None, scope='pnasnet', final_endpoint=None, sync_batch_norm_method='None'):
    ...
    batch_norm = utils.get_batch_norm_fn(sync_batch_norm_method)
    cell = nas_genotypes.PNASCell((hparams.num_conv_filters), (hparams.drop_path_keep_prob),
      (len(backbone)),
      (hparams.total_training_steps),
      batch_norm_fn=batch_norm)
    with arg_scope([slim.dropout, batch_norm], is_training=is_training):
        return _build_nas_base(images,
          cell=cell,
          backbone=backbone,
          ...
          nas_use_classification_head=(nas_architecture_options['nas_use_classification_head']),
          reuse=reuse,
          scope=scope,
          final_endpoint=final_endpoint,
          batch_norm_fn=batch_norm,
          nas_remove_os32_stride=(nas_architecture_options['nas_remove_os32_stride']))
...
def hnasnet(images, num_classes, is_training=True, global_pool=False, output_stride=8, nas_architecture_options=None, nas_training_hyper_parameters=None, reuse=None, scope='hnasnet', final_endpoint=None, sync_batch_norm_method='None'):
    ...
    if nas_architecture_options is None:
        ...
        if nas_training_hyper_parameters:
            hparams.set_hparam('drop_path_keep_prob', nas_training_hyper_parameters['drop_path_keep_prob'])
            hparams.set_hparam('total_training_steps', nas_training_hyper_parameters['total_training_steps'])
        is_training or tf.logging.info('During inference, setting drop_path_keep_prob = 1.0.')
        hparams.set_hparam('drop_path_keep_prob', 1.0)
    tf.logging.info(hparams)
    operations = [
    ...
    batch_norm = utils.get_batch_norm_fn(sync_batch_norm_method)
    cell = NASBaseCell((hparams.num_conv_filters), operations,
      used_hiddenstates,
      hiddenstate_indices,
