...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/global_objectives/util.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 14183 bytes
"""Contains utility functions for the global objectives library."""
import tensorflow as tf

def weighted_sigmoid_cross_entropy_with_logits(labels, logits, positive_weights=1.0, negative_weights=1.0, name=None):
...
def weighted_surrogate_loss(labels, logits, surrogate_type='xent', positive_weights=1.0, negative_weights=1.0, name=None):
    ...
    with tf.name_scope(name, 'weighted_loss', [
     ...
     negative_weights]) as (name):
        if surrogate_type == 'xent':
            return weighted_sigmoid_cross_entropy_with_logits(logits=logits,
              labels=labels,
              positive_weights=positive_weights,
              negative_weights=negative_weights,
              name=name)
        if surrogate_type == 'hinge':
            return weighted_hinge_loss(logits=logits,
              labels=labels,
              positive_weights=positive_weights,
              negative_weights=negative_weights,
              name=name)
...
def build_label_priors(labels, weights=None, positive_pseudocount=1.0, negative_pseudocount=1.0, variables_collections=None):
  ...
  """
    ...
    try:
        ...
        weighted_label_counts = tf.contrib.framework.model_variable(name='weighted_label_counts',
          shape=[
         num_labels],
          dtype=dtype,
         ...
         positive_pseudocount] * num_labels),
          ...
          trainable=False)
        weighted_label_counts_update = weighted_label_counts.assign_add(tf.reduce_sum(weights * labels, 0))
        weight_sum = tf.contrib.framework.model_variable(name='weight_sum',
          shape=[
         num_labels],
          dtype=dtype,
          initializer=tf.constant_initializer(([
