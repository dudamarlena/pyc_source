...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/evaluation/parsing_covering.py
...
class ParsingCovering(base_metric.SegmentationMetric):
    ...
    def compare_and_accumulate(self, groundtruth_category_array, groundtruth_instance_array, predicted_category_array, predicted_instance_array):
        ...
        intersection_ids, intersection_areas = np.unique(intersection_id_array,
          return_counts=True)
    ...
    def detailed_results(self, is_thing=None):
        ...
        for category_set_name, in_category_set in six.iteritems(category_sets):
            if np.any(in_category_set):
                ...
                results[category_set_name] = {'pc':0, 
                 'n':0}
    ...
    def reset(self):
        """See base class."""
        self.weighted_iou_per_class = np.zeros((self.num_categories),
          dtype=(np.float64))
