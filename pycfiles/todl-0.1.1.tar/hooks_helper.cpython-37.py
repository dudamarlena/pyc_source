...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/utils/logs/hooks_helper.py
...
def get_train_hooks(name_list, use_tpu=False, **kwargs):
  ...
  """
    if not name_list:
        return []
    if use_tpu:
        logging.warning('hooks_helper received name_list `%s`, but a TPU is specified. No hooks will be used.', name_list)
        return []
    train_hooks = []
...
def get_examples_per_second_hook(every_n_steps=100, batch_size=128, warm_steps=5, **kwargs):
  ...
  Returns:
    ...
    profiling tools like chrome://tracing.
  """
    return hooks.ExamplesPerSecondHook(batch_size=batch_size,
