...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/builders/hyperparams_builder.py
...
class KerasLayerHyperparams(object):
    ...
    """
        ...
        self._op_params = {'kernel_regularizer':_build_keras_regularizer(hyperparams_config.regularizer), 
    ...
    def scope_fn():
        with (slim.arg_scope)([slim.batch_norm], **batch_norm_params) if batch_norm_params is not None else context_manager.IdentityContextManager():
            with slim.arg_scope(affected_ops,
              weights_regularizer=(_build_slim_regularizer(hyperparams_config.regularizer)),
              weights_initializer=(_build_initializer(hyperparams_config.initializer)),
...
def _build_initializer(initializer, build_for_keras=False):
  ...
  """
    ...
    if initializer_oneof == 'truncated_normal_initializer':
        return tf.truncated_normal_initializer(mean=(initializer.truncated_normal_initializer.mean),
          stddev=(initializer.truncated_normal_initializer.stddev))
    if initializer_oneof == 'random_normal_initializer':
        return tf.random_normal_initializer(mean=(initializer.random_normal_initializer.mean),
    ...
    if initializer_oneof == 'variance_scaling_initializer':
        ...
        if build_for_keras:
            if initializer.variance_scaling_initializer.uniform:
                return tf.variance_scaling_initializer(scale=(initializer.variance_scaling_initializer.factor),
                  mode=(mode.lower()),
            ...
            try:
                ...
                return tf.variance_scaling_initializer(scale=truncated_scale,
                  mode=(mode.lower()),
                  distribution='normal')

        else:
            return slim.variance_scaling_initializer(factor=(initializer.variance_scaling_initializer.factor),
              mode=mode,
...
def _build_batch_norm_params(batch_norm, is_training):
  ...
  """
    batch_norm_params = {'decay':batch_norm.decay, 
...
def _build_keras_batch_norm_params(batch_norm):
  ...
  """
    batch_norm_params = {'momentum':batch_norm.decay, 
     'center':batch_norm.center, 
     'scale':batch_norm.scale, 
     'epsilon':batch_norm.epsilon}
    return batch_norm_params