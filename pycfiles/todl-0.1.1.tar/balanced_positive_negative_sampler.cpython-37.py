...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/core/balanced_positive_negative_sampler.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 11876 bytes
...
class BalancedPositiveNegativeSampler(minibatch_sampler.MinibatchSampler):
    __doc__ = 'Subsamples minibatches to a desired balance of positives and negatives.'
    ...
    """
        ...
        one_hot_selector = tf.one_hot((tf.cast(indexed_positions, tf.int32) - 1), total_num_samples,
          dtype=(tf.float32))
        return tf.cast(tf.tensordot((tf.cast(input_tensor, tf.float32)), one_hot_selector,
          axes=[0, 0]), tf.int32)
    ...
    """
        if not indicator.shape.is_fully_defined():
            ...
            assert isinstance(batch_size, int), 'batch_size has to be an integer when is_static isTrue.'
        ...
        sampled_idx_indicator = tf.cast(tf.reduce_sum(tf.one_hot(sampled_idx, depth=input_length),
          axis=0), tf.bool)
