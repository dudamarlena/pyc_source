...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/lstm_object_detection/lstm/lstm_cells.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 28949 bytes
"""BottleneckConvLSTMCell implementation."""
import tensorflow.compat.v1 as tf
import tensorflow.contrib as contrib_layers
import tensorflow.contrib as contrib_rnn
from tensorflow.contrib import slim
import tensorflow.contrib.framework.python.ops as contrib_variables
import lstm_object_detection.lstm.utils as lstm_utils

class BottleneckConvLSTMCell(contrib_rnn.RNNCell):
    __doc__ = 'Basic LSTM recurrent network cell using separable convolutions.\n\n  The implementation is based on:\n  Mobile Video Object Detection with Temporally-Aware Feature Maps\n  https://arxiv.org/abs/1711.06368.\n\n  We add forget_bias (default: 1) to the biases of the forget gate in order to\n  reduce the scale of forgetting in the beginning of the training.\n\n  This LSTM first projects inputs to the size of the output before doing gate\n  computations. This saves params unless the input is less than a third of the\n  state size channel-wise.\n  '
    ...
    @property
    def state_size(self):
        return contrib_rnn.LSTMStateTuple(self._output_size + [self._num_units], self._output_size + [self._num_units])
        ...
        with tf.variable_scope(scope, reuse=(tf.AUTO_REUSE)):
            ...
            if self._flatten_state:
                ...
                if self._pre_bottleneck:
                    ...
                    bottleneck = contrib_layers.separable_conv2d((tf.concat([inputs, h], 3)),
                      (self._num_units),
                      (self._filter_size),
                      depth_multiplier=1,
    ...
    """
        ...
        ret_flat = [contrib_variables.model_variable((state_name + str(i)), shape=s, dtype=dtype, initializer=tf.truncated_normal_initializer(stddev=0.03)) if learned_state else tf.zeros(([batch_size] + s), dtype=dtype, name=state_name) for i, s in enumerate(state_size)]
        if learned_state:
    ...
    """
        ...
        with tf.variable_scope('conv_lstm_cell', reuse=(tf.AUTO_REUSE)):
            ...
            inputs = contrib_layers.separable_conv2d((tf.concat([inputs, state], 3)),
              (self.output_size[(-1)]),
              (self._filter_size),
              depth_multiplier=1,
...
class GroupedConvLSTMCell(contrib_rnn.RNNCell):
    ...
    """
        ...
        with tf.variable_scope(scope, reuse=(tf.AUTO_REUSE)):
            ...
            for k in range(self._groups):
                if self._pre_bottleneck:
                    ...
                    if self._use_batch_norm:
                        b_x = lstm_utils.quantizable_separable_conv2d(inputs,
                        ...
                        b_h = lstm_utils.quantizable_separable_conv2d((h_list[k]),
                          (self._num_units // self._groups),
                          (self._filter_size),
                          is_quantized=(self._is_quantized),
                          depth_multiplier=1,
                        ...
                        bottleneck = lstm_utils.quantizable_separable_conv2d(bottleneck_concat,
                          (self._num_units // self._groups),
                          (self._filter_size),
                          is_quantized=(self._is_quantized),
                          depth_multiplier=1,
                ...
                concat = lstm_utils.quantizable_separable_conv2d(bottleneck,
                ...
                concat = lstm_utils.quantize_op(concat,
                  is_training=(self._is_training),
                  default_min=(-6),
                  default_max=6,
                  is_quantized=(self._is_quantized),
                ...
                f_add = lstm_utils.quantize_op(f_add,
                  is_training=(self._is_training),
                  default_min=(-6),
                  default_max=6,
                  is_quantized=(self._is_quantized),
                ...
                f_act = lstm_utils.fixed_quantize_op(f_act,
                  fixed_min=0.0,
                  fixed_max=1.0,
                  is_quantized=(self._is_quantized),
                ...
                i_act = tf.sigmoid(i)
                i_act = lstm_utils.fixed_quantize_op(i_act,
                  fixed_min=0.0,
                  fixed_max=1.0,
                  is_quantized=(self._is_quantized),
                  scope=('input_gate_%d/act_quant' % k))
                j_act = self._activation(j)
                j_act = lstm_utils.fixed_quantize_op(j_act,
                  fixed_min=0.0,
                  fixed_max=6.0,
                  is_quantized=(self._is_quantized),
                ...
                new_c = a + b
                new_c = lstm_utils.fixed_quantize_op(new_c,
                  fixed_min=0.0,
                  fixed_max=6.0,
                  is_quantized=(self._is_quantized),
                ...
                if not self._is_quantized:
                    if self._scale_state:
                        normalizer = tf.maximum(1.0, tf.reduce_max(new_c, axis=(1,
                                                                                2,
                                                                                3)) / 6)
                        ...
                        new_c_act = lstm_utils.fixed_quantize_op(new_c_act,
                          fixed_min=0.0,
                          fixed_max=6.0,
                          is_quantized=(self._is_quantized),
                          scope=('new_c_%d/act_quant' % k))
                        o_act = tf.sigmoid(o)
                        o_act = lstm_utils.fixed_quantize_op(o_act,
                          fixed_min=0.0,
                          fixed_max=1.0,
                          is_quantized=(self._is_quantized),
                          scope=('output_%d/act_quant' % k))
                        new_h = new_c_act * o_act
                        new_h_act = lstm_utils.fixed_quantize_op(new_h,
                          fixed_min=0.0,
                          fixed_max=6.0,
                          is_quantized=(self._is_quantized),
            ...
            bottleneck = lstm_utils.quantizable_concat(out_bottleneck,
              axis=3,
              is_training=False,
              is_quantized=(self._is_quantized),
    ...
    """
        ...
        with tf.name_scope(None):
            state = tf.identity(state,
              name=('raw_inputs/init_lstm_h_%d' % (input_index + 1)))
        if self._flatten_state:
            batch_size = inputs.shape[0]
            height = inputs.shape[1]
            width = inputs.shape[2]
            ...
            with tf.variable_scope('bottleneck_%d' % input_index):
                ...
                for k in range(self._groups):
                    with tf.variable_scope('group_%d' % k):
                        bottleneck_out.append(lstm_utils.quantizable_separable_conv2d(lstm_utils.quantizable_concat([
                ...
                inputs = lstm_utils.quantizable_concat(bottleneck_out,
                  axis=3,
                  is_training=(self._is_training),
                  is_quantized=(self._is_quantized),
