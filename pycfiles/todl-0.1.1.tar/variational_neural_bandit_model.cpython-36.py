...
# [GCC 8.4.0]
# Embedded file name: /usr/local/google/home/rikel/bandits_repo/deep-bayesian-contextual-bandits/research/bandits/algorithms/variational_neural_bandit_model.py
        ...
        if self.verbose:
            ...
            if getattr(self.hparams, 'infer_noise_sigma', False):
                log_likelihood = log_gaussian((self.y),
                  (self.y_pred_local), (self.noise_sigma), reduce_sum=False)
            else:
                y_hat = self.y_pred_local if use_local_reparameterization else self.y_pred
            log_likelihood = log_gaussian((self.y),
              y_hat, (self.hparams.noise_sigma), reduce_sum=False)
