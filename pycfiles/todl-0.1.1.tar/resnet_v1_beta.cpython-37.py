...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/core/resnet_v1_beta.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 34236 bytes
...
def bottleneck(inputs, depth, depth_bottleneck, stride, unit_rate=1, rate=1, outputs_collections=None, scope=None):
  ...
  """
    with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as (sc):
        ...
        residual = conv2d_ws.conv2d_same(residual, depth_bottleneck, 3, stride, rate=(rate * unit_rate),
          scope='conv2')
        residual = conv2d_ws.conv2d(residual, depth, [1, 1], stride=1, activation_fn=None,
          scope='conv3')
...
def root_block_fn_for_beta_variant(net, depth_multiplier=1.0):
  ...
  """
    net = conv2d_ws.conv2d_same(net,
      (int(64 * depth_multiplier)), 3, stride=2, scope='conv1_1')
    net = conv2d_ws.conv2d_same(net,
      (int(64 * depth_multiplier)), 3, stride=1, scope='conv1_2')
    net = conv2d_ws.conv2d_same(net,
      (int(128 * depth_multiplier)), 3, stride=1, scope='conv1_3')
    return net
...
def resnet_v1_beta(inputs, blocks, num_classes=None, is_training=None, global_pool=True, output_stride=None, root_block_fn=None, reuse=None, scope=None, sync_batch_norm_method='None'):
  ...
  """
    if root_block_fn is None:
        root_block_fn = functools.partial((conv2d_ws.conv2d_same), num_outputs=64,
          kernel_size=7,
          stride=2,
          scope='conv1')
          ...
          outputs_collections=end_points_collection):
            ...
            with arg_scope:
                ...
                if num_classes is not None:
                    net = conv2d_ws.conv2d(net, num_classes, [1, 1], activation_fn=None, normalizer_fn=None,
                      scope='logits',
  ...
  """
    ...
    blocks = [
     resnet_v1_small_beta_block('block1',
       base_depth=64, num_units=2, stride=2),
     resnet_v1_small_beta_block('block2',
       base_depth=128, num_units=2, stride=2),
     resnet_v1_small_beta_block('block3',
       base_depth=256, num_units=2, stride=2),
...
def resnet_v1_18_beta(inputs, num_classes=None, is_training=None, global_pool=False, output_stride=None, multi_grid=None, root_depth_multiplier=0.25, reuse=None, scope='resnet_v1_18', sync_batch_norm_method='None'):
  ...
  """
    ...
    return resnet_v1_beta(inputs,
      ...
      global_pool=global_pool,
      output_stride=output_stride,
      root_block_fn=functools.partial(root_block_fn_for_beta_variant, depth_multiplier=root_depth_multiplier),
      reuse=reuse,
      scope=scope,
      sync_batch_norm_method=sync_batch_norm_method)
  ...
  """
    ...
    blocks = [
     resnet_v1_beta_block('block1',
       base_depth=64, num_units=3, stride=2),
     resnet_v1_beta_block('block2',
       base_depth=128, num_units=4, stride=2),
     resnet_v1_beta_block('block3',
       base_depth=256, num_units=6, stride=2),
     resnet_utils.Block('block4', bottleneck, [{'depth':2048,  'depth_bottleneck':512,  'stride':1,  'unit_rate':rate} for rate in multi_grid])]
    return resnet_v1_beta(inputs,
      blocks=blocks,
      num_classes=num_classes,
      is_training=is_training,
  ...
  """
    ...
    blocks = [
     ...
     resnet_v1_beta_block('block2',
       base_depth=128, num_units=4, stride=2),
     resnet_v1_beta_block('block3',
       base_depth=256, num_units=23, stride=2),
     resnet_utils.Block('block4', bottleneck, [{'depth':2048,  'depth_bottleneck':512,  'stride':1,  'unit_rate':rate} for rate in multi_grid])]
    return resnet_v1_beta(inputs,
      blocks=blocks,
      num_classes=num_classes,
...
def resnet_arg_scope(weight_decay=0.0001, batch_norm_decay=0.997, batch_norm_epsilon=1e-05, batch_norm_scale=True, activation_fn=tf.nn.relu, use_batch_norm=True, sync_batch_norm_method='None', normalization_method='unspecified', use_weight_standardization=False):
  ...
  """
    ...
    if normalization_method == 'batch':
        ...
        if normalization_method == 'none':
            ...
            if normalization_method == 'group':
                ...
                if normalization_method == 'unspecified':
    ...
    with slim.arg_scope([conv2d_ws.conv2d], weights_regularizer=(slim.l2_regularizer(weight_decay)),
