...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/lstm_object_detection/inputs/tf_sequence_example_decoder.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 11262 bytes
...
class BoundingBoxSequence(tfexample_decoder.ItemHandler):
    __doc__ = 'An ItemHandler that concatenates SparseTensors to Bounding Boxes.\n  '
    ...
    """
        ...
        if self._return_dense:
            bounding_boxes = tf.sparse_tensor_to_dense(bounding_boxes,
              default_value=(self._default_value))
        return bounding_boxes


class TFSequenceExampleDecoder(data_decoder.DataDecoder):
    __doc__ = 'Tensorflow Sequence Example proto decoder.'

    def __init__(self):
        """Constructor sets keys_to_features and items_to_handlers."""
        self.keys_to_context_features = {'image/format':tf.FixedLenFeature((), tf.string, default_value='jpeg'), 
         'image/filename':tf.FixedLenFeature((), tf.string, default_value=''), 
         'image/key/sha256':tf.FixedLenFeature((), tf.string, default_value=''), 
         'image/source_id':tf.FixedLenFeature((), tf.string, default_value=''), 
         'image/height':tf.FixedLenFeature((), tf.int64, 1), 
         'image/width':tf.FixedLenFeature((), tf.int64, 1)}
        self.keys_to_features = {'image/encoded':tf.FixedLenSequenceFeature((), tf.string), 
        ...
        self.items_to_handlers = {fields.InputDataFields.image: tfexample_decoder.Image(image_key='image/encoded',
                                         format_key='image/format',
                                         channels=3,
                                         repeated=True), 
         
         fields.InputDataFields.source_id: tfexample_decoder.Tensor('image/source_id'), 
         
         fields.InputDataFields.key: tfexample_decoder.Tensor('image/key/sha256'), 
         
         fields.InputDataFields.filename: tfexample_decoder.Tensor('image/filename'), 
         
         fields.InputDataFields.groundtruth_boxes: BoundingBoxSequence(prefix='bbox/'), 
         
         fields.InputDataFields.groundtruth_classes: tfexample_decoder.Tensor('bbox/label/index'), 
         
         fields.InputDataFields.groundtruth_area: tfexample_decoder.Tensor('area'), 
         
         fields.InputDataFields.groundtruth_is_crowd: tfexample_decoder.Tensor('is_crowd'), 
         
         fields.InputDataFields.groundtruth_difficult: tfexample_decoder.Tensor('difficult'), 
         
         fields.InputDataFields.groundtruth_group_of: tfexample_decoder.Tensor('group_of')}
