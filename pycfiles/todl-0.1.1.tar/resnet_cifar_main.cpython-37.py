...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/benchmark/models/resnet_cifar_main.py
...
class LearningRateBatchScheduler(tf.keras.callbacks.Callback):
    __doc__ = 'Callback to update learning rate on every batch (not epoch boundaries).\n\n  N.B. Only support Keras optimizers, not TF optimizers.\n\n  Attributes:\n      schedule: a function that takes an epoch index and a batch index as input\n          (both integer, indexed from 0) and returns a new learning rate as\n          output (float).\n  '
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        if lr != self.prev_lr:
            ...
            self.prev_lr = lr
            logging.debug('Epoch %05d Batch %05d: LearningRateBatchScheduler change learning rate to %s.', self.epochs, batch, lr)
...
def run(flags_obj):
  ...
  """
    ...
    if flags_obj.tf_gpu_thread_mode:
        ...
        strategy = distribution_utils.get_distribution_strategy(distribution_strategy=(flags_obj.distribution_strategy),
          num_gpus=(flags_obj.num_gpus),
        ...
        if flags_obj.use_synthetic_data:
            ...
            input_fn = common.get_synth_input_fn(height=(cifar_preprocessing.HEIGHT),
    ...
    if not flags_obj.skip_eval:
        eval_input_dataset = input_fn(is_training=False,
          ...
          batch_size=(flags_obj.batch_size),
          parse_record_fn=(cifar_preprocessing.parse_record))
    steps_per_epoch = cifar_preprocessing.NUM_IMAGES['train'] // flags_obj.batch_size
    lr_schedule = 0.1
    if flags_obj.use_tensor_lr:
        initial_learning_rate = common.BASE_LEARNING_RATE * flags_obj.batch_size / 128
        lr_schedule = tf.keras.optimizers.schedules.PiecewiseConstantDecay(boundaries=(list((p[1] * steps_per_epoch for p in LR_SCHEDULE))),
          values=([
         initial_learning_rate] + list((p[0] * initial_learning_rate for p in LR_SCHEDULE))))
    with strategy_scope:
    ...
    if not flags_obj.use_tensor_lr:
        lr_callback = LearningRateBatchScheduler(schedule=learning_rate_schedule,
          batch_size=(flags_obj.batch_size),
    ...
    if not strategy:
        if flags_obj.explicit_gpu_placement:
            no_dist_strat_device = tf.device('/device:GPU:0')
            no_dist_strat_device.__enter__()
    history = model.fit(train_input_dataset, epochs=train_epochs,
      steps_per_epoch=steps_per_epoch,
      callbacks=callbacks,
    ...
    eval_output = None
    if not flags_obj.skip_eval:
        eval_output = model.evaluate(eval_input_dataset, steps=num_eval_steps,
          verbose=2)
    if not strategy:
        if flags_obj.explicit_gpu_placement:
