...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/vrd_evaluation.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 26028 bytes
...
import collections, logging, numpy as np, six
...
class VRDDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    """
        ...
        self._evaluation.add_single_ground_truth_image_info(image_key=image_id,
          groundtruth_box_tuples=(self._process_groundtruth_boxes(groundtruth_box_tuples)),
    ...
    """
        ...
        self._evaluation.add_single_detected_image_info(image_key=image_id,
          detected_box_tuples=(self._process_detection_boxes(detection_box_tuples[selector])),
          detected_scores=(detections_dict[standard_fields.DetectionResultFields.detection_scores][selector]),
    ...
    """
        ...
        vrd_metrics = {self._metric_prefix + 'weightedAP@{}IOU'.format(self._matching_iou_threshold): weighted_average_precision, 
         
         self._metric_prefix + 'mAP@{}IOU'.format(self._matching_iou_threshold): mean_average_precision, 
         
         self._metric_prefix + 'Recall@50@{}IOU'.format(self._matching_iou_threshold): recall_50, 
         
         self._metric_prefix + 'Recall@100@{}IOU'.format(self._matching_iou_threshold): recall_100}
        if relationships:
...
class VRDPhraseDetectionEvaluator(VRDDetectionEvaluator):
    __doc__ = 'A class to evaluate VRD detections in phrase setting.\n\n  Expected groundtruth box datatype is vrd_box_data_type, expected groudtruth\n  labels datatype is label_data_type.\n  Expected detection box datatype is single_box_data_type, expected detection\n  labels datatype is label_data_type.\n  '

    def __init__(self, matching_iou_threshold=0.5):
        super(VRDPhraseDetectionEvaluator, self).__init__(matching_iou_threshold=matching_iou_threshold,
...
class _VRDDetectionEvaluation(object):
    __doc__ = 'Performs metric computation for the VRD task. This class is internal.\n  '
    ...
    """
        ...
        if image_key in self._groundtruth_box_tuples:
            ...
            groundtruth_box_tuples = np.empty(shape=[
             0, 4],
              dtype=(detected_box_tuples.dtype))
            groundtruth_class_tuples = np.array([], dtype=(detected_class_tuples.dtype))
        scores, tp_fp_labels, mapping = self._per_image_eval.compute_detection_tp_fp(detected_box_tuples=detected_box_tuples,
