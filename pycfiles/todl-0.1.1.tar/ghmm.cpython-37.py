...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/models/ghmm.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 20795 bytes
...
class GaussianHMM(object):
    ...
    """
        ...
        def _ta_for_param(param):
            ...
            ta = tf.TensorArray(dtype=(param.dtype), size=size,
              dynamic_size=False,
    ...
    """
        z_variances = [
         self.transition_variances.read(0)]
        ...
        self.obs_dist = tfd.MultivariateNormalFullCovariance(loc=tf.zeros([self.num_timesteps], dtype=(tf.float32)),
    ...
    """
        ...
        mess_mean, mess_prec = tf.cond(tf.less(t, self.num_timesteps - 1), lambda : tf.unstack(self._compute_backwards_messages(xs[1:]).read(0)), lambda : [
    ...
    def _compute_backwards_messages(self, xs):
        ...
        xs = tf.TensorArray(dtype=(xs.dtype), size=num_xs,
          dynamic_size=False,
          clear_after_read=True).unstack(xs)
        messages_ta = tf.TensorArray(dtype=(xs.dtype), size=num_xs,
          dynamic_size=False,
    ...
    """
        ...
        return tfd.MultivariateNormalFullCovariance(loc=mean,
...
class TrainableGaussianHMM(GaussianHMM, base.ELBOTrainableSequenceModel):
    __doc__ = 'An interface between importance-sampling training methods and the GHMM.'
    ...
    Args:
      ...
      dtype: The datatype of the state.
    """
        super(TrainableGaussianHMM, self).__init__(num_timesteps,
          transition_variances, emission_variances, transition_weights,
          emission_weights, dtype=dtype)
        self.random_seed = random_seed
        if not proposal_type in ('filtering', 'smoothing', 'prior', 'true-filtering',
                                 'true-smoothing'):
            raise AssertionError
        elif proposal_type == 'true-filtering':
            self.proposal = self._filtering_proposal
        else:
            if proposal_type == 'true-smoothing':
                self.proposal = self._smoothing_proposal
            else:
                if proposal_type == 'prior':
                    self.proposal = self.transition
                else:
                    if proposal_type == 'filtering':
                        self._learned_proposal_fn = base.NonstationaryLinearDistribution(num_timesteps,
                          inputs_per_timestep=([1] + [2] * (num_timesteps - 1)))
                        self.proposal = self._learned_filtering_proposal
                    else:
                        if proposal_type == 'smoothing':
                            inputs_per_timestep = [
                             num_timesteps] + [num_timesteps - t for t in range(num_timesteps - 1)]
                            self._learned_proposal_fn = base.NonstationaryLinearDistribution(num_timesteps,
                              inputs_per_timestep=inputs_per_timestep)
    ...
    def _smoothing_proposal(self, t, state):
        ...
        mess_mean, mess_prec = tf.cond(tf.less(t, self.num_timesteps - 1), lambda : tf.unstack(self.messages.read(t)), lambda : [
         tf.zeros([self.batch_size]), tf.zeros([self.batch_size])])
