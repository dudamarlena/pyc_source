...
# [GCC 8.4.0]
# Embedded file name: /usr/local/google/home/rikel/bandits_repo/deep-bayesian-contextual-bandits/research/bandits/algorithms/multitask_gp.py
# Compiled at: 2018-07-23 14:29:50
# Size of source mod 2**32: 14968 bytes
...
class MultitaskGP(BayesianNN):
    ...
    def cov(self, x, x2):
        ...
        lin = tf.matmul((x / tf.nn.softplus(ls_lin)), (x2 / tf.nn.softplus(ls_lin)),
          transpose_b=True)
    ...
    """
        ...
        self.x_train = tf.get_variable('training_data',
          initializer=tf.ones([
         self.hparams.batch_size, self.n_in],
          dtype=(tf.float64)),
        ...
        self.y_train = tf.get_variable('training_labels',
          ...
          validate_shape=False,
          trainable=False)
        self.weights_train = tf.get_variable('weights_train',
          initializer=tf.ones([
         self.hparams.batch_size, self.n_out],
          dtype=(tf.float64)),
          validate_shape=False,
          trainable=False)
        self.input_op = tf.assign((self.x_train), (self.x_in), validate_shape=False)
        self.input_w_op = tf.assign((self.weights_train),
          (self.weights), validate_shape=False)
        ...
        self.noise = tf.get_variable('noise',
          initializer=tf.cast(0.0, dtype=(tf.float64)))
        self.amplitude = tf.get_variable('amplitude',
          initializer=tf.cast(1.0, dtype=(tf.float64)))
        ...
        self.length_scales_lin = tf.get_variable('length_scales_linear',
          initializer=tf.zeros([1, self.n_in], dtype=(tf.float64)))
        self.task_vectors = tf.get_variable('latent_task_vectors',
          initializer=tf.random_normal([
         self.n_out, self.task_latent_dim],
          dtype=(tf.float64)))
        ...
        self.var_op = tf.assign(self.input_std, tf.sqrt(0.0001 + tf.reduce_sum((tf.square(self.y - tf.reduce_sum((self.y), axis=0) / index_counts)),
          axis=0) / index_counts))
        ...
        self.input_inv = tf.Variable(tf.eye((self.hparams.batch_size), dtype=(tf.float64)),
          validate_shape=False,
          trainable=False)
    ...
    """
        ...
        inds = tf.squeeze(tf.reshape(tf.tile(tf.reshape(tf.range(self.n_out), (self.n_out, 1)), (
        ...
        y_sample = tf.transpose(tf.reshape(y_sample, (
         self.n_out, -1))) * self.input_std + self.input_mean
        return (
         y_mean, y_sample)
    ...
    """
        ...
        for step in range(num_steps):
            ...
            if numpts >= self.max_num_points:
                if self.keep_fixed_after_max_obs:
            ...
            res = self.sess.run(ops, feed_dict={self.x: x, 
             self.x_in: x, 
             self.y: y, 
