...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/nlp/xlnet/data_utils.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 31166 bytes
...
def file_based_input_fn_builder(input_file, name_to_features, batch_size, is_training):
    ...
    def input_fn():
        ...
        if isinstance(input_file, str):
            ...
            d = d.apply(tf.data.experimental.parallel_interleave((tf.data.TFRecordDataset),
              sloppy=is_training,
...
def create_classification_dataset(file_path, seq_length, batch_size, is_training):
    """Creates input dataset from (tf)records files for pretraining."""
    name_to_features = {'input_ids':tf.io.FixedLenFeature([seq_length], tf.int64), 
...
def get_input_iterator(input_fn, strategy):
    ...
    if callable(input_data):
        iterator = iter(strategy.experimental_distribute_datasets_from_function(input_data))
    else:
        iterator = iter(strategy.experimental_distribute_dataset(input_data))
    return iterator


def get_classification_input_data(batch_size, seq_len, strategy, is_training, file_path):
    ...
    def _dataset_fn(ctx=None):
        del ctx
        train_dataset = create_classification_dataset(file_path=file_path,
          seq_length=seq_len,
          batch_size=batch_size,
...
def get_squad_input_data(batch_size, seq_len, q_len, strategy, is_training, file_path):
    """Returns input dataset from input file string."""
    use_dataset_fn = isinstance(strategy, tf.distribute.experimental.TPUStrategy)
    if use_dataset_fn:
        if batch_size % strategy.num_replicas_in_sync != 0:
        ...
        input_glob = os.path.join(file_path, 'spiece.model.*.slen-{}.qlen-{}.train.tf_record'.format(seq_len, q_len))
    ...
    def _dataset_fn(ctx=None):
        del ctx
        train_dataset = create_squad_dataset(file_path=global_input_paths,
          seq_length=seq_len,
          batch_size=batch_size,
          is_training=is_training)
        return train_dataset
...
def _idx_pair_to_mask(beg_indices, end_indices, inputs, tgt_len, num_predict):
    ...
    non_func_mask = tf.logical_and(tf.not_equal(inputs, SEP_ID), tf.not_equal(inputs, CLS_ID))
    all_indices = tf.where(non_func_mask, tf.range(tgt_len, dtype=(tf.int64)), tf.constant((-1), shape=[tgt_len], dtype=(tf.int64)))
    candidate_matrix = tf.cast(tf.logical_and(all_indices[None, :] >= beg_indices[:, None], all_indices[None, :] < end_indices[:, None]), tf.float32)
    cumsum_matrix = tf.reshape(tf.cumsum(tf.reshape(candidate_matrix, [-1])), [
     -1, tgt_len])
    ...
    return (
     is_masked, target_mask)


def _word_span_mask(inputs, tgt_len, num_predict, min_num_words, max_num_words, boundary):
    ...
    span_lens = tf.random.categorical(logits=(logits[None]),
      num_samples=num_predict,
      dtype=(tf.int64))[0] + min_num_words
    ...
    beg_indices = tf.cumsum(left_ctx_len) + tf.cumsum(right_offset, exclusive=True)
    ...
    return _idx_pair_to_mask(beg_indices, end_indices, inputs, tgt_len, num_predict)


def _token_span_mask(inputs, tgt_len, num_predict, min_num_tokens, max_num_tokens):
    ...
    span_lens = tf.random.categorical(logits=(logits[None]),
      num_samples=num_predict,
      dtype=(tf.int64))[0] + min_num_tokens
    span_lens_float = tf.cast(span_lens, tf.float32)
    left_ratio = tf.random.uniform(shape=[num_predict], minval=0.0, maxval=1.0)
    left_ctx_len = left_ratio * span_lens_float * (mask_alpha - 1)
    left_ctx_len = round_to_int(left_ctx_len)
...
def _online_sample_masks(inputs, tgt_len, num_predict, online_masking_config, boundary=None):
    """Sample target positions to predict."""
    logging.info('Online sample with strategy: `%s`.', online_masking_config.sample_strategy)
    if online_masking_config.sample_strategy == 'single_token':
        return _single_token_mask(inputs, tgt_len, num_predict)
    if online_masking_config.sample_strategy == 'whole_word':
    ...
    if online_masking_config.sample_strategy == 'token_span':
        return _token_span_mask(inputs, tgt_len, num_predict, online_masking_config.min_num_tokens, online_masking_config.max_num_tokens)
    if online_masking_config.sample_strategy == 'word_span':
        assert boundary is not None, 'word span sampling requires `boundary`'
        return _word_span_mask(inputs, tgt_len, num_predict, online_masking_config.min_num_words, online_masking_config.max_num_words, boundary)
    raise NotImplementedError


def create_pretrain_dataset(file_names, bsz_per_core, seq_len, reuse_len, perm_size, leak_ratio, online_masking_config, num_predict=None, input_pipeline_context=None):
    """Creates pretrain dataset."""

    def parser(record):
        record_spec = {'input':tf.io.FixedLenFeature([seq_len], tf.int64), 
         'seg_id':tf.io.FixedLenFeature([seq_len], tf.int64), 
         'label':tf.io.FixedLenFeature([1], tf.int64)}
        if online_masking_config.sample_strategy in ('whole_word', 'word_span'):
            logging.info('Add `boundary` spec for %s', online_masking_config.sample_strategy)
            ...
            if online_masking_config.sample_strategy in ('whole_word', 'word_span'):
                ...
                boundary = None
            is_masked, _ = _online_sample_masks(inputs,
              seq_len, num_predict, online_masking_config, boundary=boundary)
            if reuse_len > 0:
                non_reuse_len = seq_len - reuse_len
                if not (reuse_len % perm_size == 0 and non_reuse_len % perm_size == 0):
                    raise AssertionError
                perm_mask_0, target_mask_0, input_k_0, input_q_0 = _local_perm(inputs[:reuse_len], is_masked[:reuse_len], perm_size, reuse_len, leak_ratio)
                perm_mask_1, target_mask_1, input_k_1, input_q_1 = _local_perm(inputs[reuse_len:], is_masked[reuse_len:], perm_size, non_reuse_len, leak_ratio)
            ...
            perm_mask, target_mask, input_k, input_q = _local_perm(inputs, is_masked, perm_size, seq_len, leak_ratio)
    ...
    dataset = parse_files_to_dataset(parser=parser,
...
def format_filename(prefix, suffix, bsz_per_host, seq_len, reuse_len=None, uncased=False):
    ...
    if reuse_len is not None:
        ...
        case_str = 'uncased.'
    file_name = '{}.seq-{}.{}{}{}{}'.format(prefix, seq_len, reuse_str, bsz_str, case_str, suffix)
    return file_name


def get_pretrain_input_data(batch_size, seq_len, strategy, file_path, reuse_len, perm_size, leak_ratio, num_predict, uncased, online_masking_config, num_hosts=1):
    """Returns input dataset from input file string."""
    use_dataset_fn = isinstance(strategy, tf.distribute.experimental.TPUStrategy)
    split = 'train'
    bsz_per_host = int(batch_size / num_hosts)
    record_glob_base = format_filename(prefix=('meta.{}.pass-*'.format(split)),
      suffix='json*',
    ...
    for idx, record_dir in enumerate(tfrecord_dirs):
        ...
        logging.info('[Dir %d] Number of chosen batches: %s', idx, cur_record_info['num_batch'])
        logging.info('[Dir %d] Number of chosen files: %s', idx, len(cur_record_info['filenames']))
    ...
    def _dataset_fn(ctx=None):
        ...
        train_dataset = create_pretrain_dataset(file_names=(record_info['filenames']),
...
def parse_files_to_dataset(parser, file_paths, bsz_per_core, sequential, input_pipeline_context=None):
...
def _local_perm(inputs, is_masked, perm_size, seq_len, leak_ratio):
  ...
  """
    ...
    if leak_ratio > 0:
        leak_tokens = tf.logical_and(masked_tokens, tf.random.uniform([seq_len], maxval=1.0) < leak_ratio)
    ...
    return (
     perm_mask, target_mask, inputs_k, inputs_q)