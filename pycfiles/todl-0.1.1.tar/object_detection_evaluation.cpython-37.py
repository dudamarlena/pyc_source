...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/object_detection_evaluation.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 58035 bytes
...
import collections, logging, unicodedata, numpy as np, six
...
class DetectionEvaluator(six.with_metaclass(ABCMeta, object)):
    __doc__ = 'Interface for object detection evalution classes.\n\n  Example usage of the Evaluator:\n  ------------------------------\n  evaluator = DetectionEvaluator(categories)\n\n  # Detections and groundtruth for image 1.\n  evaluator.add_single_groundtruth_image_info(...)\n  evaluator.add_single_detected_image_info(...)\n\n  # Detections and groundtruth for image 2.\n  evaluator.add_single_groundtruth_image_info(...)\n  evaluator.add_single_detected_image_info(...)\n\n  metrics_dict = evaluator.evaluate()\n  '
...
class ObjectDetectionEvaluator(DetectionEvaluator):
    __doc__ = 'A class to evaluate detections.'
    ...
    """
        ...
        self._evaluation = ObjectDetectionEvaluation(num_groundtruth_classes=(self._num_classes),
    ...
    """
        return (
         self._evaluation.get_internal_state(), self._image_ids)
    ...
    def _build_metric_names(self):
        ...
        if self._recall_lower_bound > 0.0 or self._recall_upper_bound < 1.0:
            self._metric_names = [
             self._metric_prefix + 'Precision/mAP@{}IOU@[{:.1f},{:.1f}]Recall'.format(self._matching_iou_threshold, self._recall_lower_bound, self._recall_upper_bound)]
        else:
            self._metric_names = [
    ...
    """
        if image_id in self._image_ids:
            ...
            if standard_fields.InputDataFields.groundtruth_difficult in six.viewkeys(groundtruth_dict):
                if not groundtruth_dict[standard_fields.InputDataFields.groundtruth_difficult].size:
                    groundtruth_difficult = groundtruth_classes.size or groundtruth_dict[standard_fields.InputDataFields.groundtruth_difficult]
                else:
                    pass
        ...
        self._evaluation.add_single_ground_truth_image_info(image_key=image_id,
    ...
    """
        ...
        self._evaluation.add_single_detected_image_info(image_key=image_id,
          detected_boxes=(detections_dict[standard_fields.DetectionResultFields.detection_boxes]),
          detected_scores=(detections_dict[standard_fields.DetectionResultFields.detection_scores]),
    ...
    """
        ...
        args = [
         eval_dict_filtered[standard_fields.InputDataFields.key]]
...
class PascalDetectionEvaluator(ObjectDetectionEvaluator):
    __doc__ = 'A class to evaluate detections using PASCAL metrics.'

    def __init__(self, categories, matching_iou_threshold=0.5):
        super(PascalDetectionEvaluator, self).__init__(categories,
...
class PrecisionAtRecallDetectionEvaluator(ObjectDetectionEvaluator):
    __doc__ = 'A class to evaluate detections using precision@recall metrics.'

    def __init__(self, categories, matching_iou_threshold=0.5, recall_lower_bound=0.0, recall_upper_bound=1.0):
        super(PrecisionAtRecallDetectionEvaluator, self).__init__(categories,
          matching_iou_threshold=matching_iou_threshold,
...
class PascalInstanceSegmentationEvaluator(ObjectDetectionEvaluator):
    __doc__ = 'A class to evaluate instance masks using PASCAL metrics.'

    def __init__(self, categories, matching_iou_threshold=0.5):
        super(PascalInstanceSegmentationEvaluator, self).__init__(categories,
          matching_iou_threshold=matching_iou_threshold,
          evaluate_corlocs=False,
...
class WeightedPascalInstanceSegmentationEvaluator(ObjectDetectionEvaluator):
    __doc__ = 'A class to evaluate instance masks using weighted PASCAL metrics.\n\n  Weighted PASCAL metrics computes the mean average precision as the average\n  precision given the scores and tp_fp_labels of all classes. In comparison,\n  PASCAL metrics computes the mean average precision as the mean of the\n  per-class average precisions.\n\n  This definition is very similar to the mean of the per-class average\n  precisions weighted by class frequency. However, they are typically not the\n  same as the average precision is not a linear function of the scores and\n  tp_fp_labels.\n  '

    def __init__(self, categories, matching_iou_threshold=0.5):
        super(WeightedPascalInstanceSegmentationEvaluator, self).__init__(categories,
          matching_iou_threshold=matching_iou_threshold,
          evaluate_corlocs=False,
...
class OpenImagesDetectionEvaluator(ObjectDetectionEvaluator):
    ...
    Args:
      ...
      group_of_weight: Weight of the group-of bounding box. If set to 0 (default
        ...
        to false negatives.
    """
        super(OpenImagesDetectionEvaluator, self).__init__(categories,
    ...
    """
        if image_id in self._image_ids:
            raise ValueError('Image with id {} already added.'.format(image_id))
        groundtruth_classes = groundtruth_dict[standard_fields.InputDataFields.groundtruth_classes] - self._label_id_offset
        if standard_fields.InputDataFields.groundtruth_group_of in six.viewkeys(groundtruth_dict):
            groundtruth_group_of = groundtruth_dict[standard_fields.InputDataFields.groundtruth_group_of].size or groundtruth_classes.size or groundtruth_dict[standard_fields.InputDataFields.groundtruth_group_of]
        else:
            groundtruth_group_of = None
            if not len(self._image_ids) % 1000:
                logging.warning('image %s does not have groundtruth group_of flag specified', image_id)
            elif self._evaluate_masks:
                groundtruth_masks = groundtruth_dict[standard_fields.InputDataFields.groundtruth_instance_masks]
            else:
                groundtruth_masks = None
            self._evaluation.add_single_ground_truth_image_info(image_id,
...
class OpenImagesChallengeEvaluator(OpenImagesDetectionEvaluator):
    ...
    """
        ...
        super(OpenImagesChallengeEvaluator, self).__init__(categories,
          matching_iou_threshold,
          evaluate_masks=evaluate_masks,
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        groundtruth_classes = groundtruth_dict[standard_fields.InputDataFields.groundtruth_classes] - self._label_id_offset
        self._evaluatable_labels[image_id] = np.unique(np.concatenate((
         groundtruth_dict.get(standard_fields.InputDataFields.groundtruth_image_classes, np.array([], dtype=int)) - self._label_id_offset,
         groundtruth_classes)))
...
ObjectDetectionEvalMetrics = collections.namedtuple('ObjectDetectionEvalMetrics', [
...
class OpenImagesDetectionChallengeEvaluator(OpenImagesChallengeEvaluator):
    __doc__ = 'A class implements Open Images Detection Challenge metric.'
    ...
    Args:
      ...
      evaluate_corlocs: if True, additionally evaluates and returns CorLoc.
    """
        super(OpenImagesDetectionChallengeEvaluator, self).__init__(categories=categories,
          evaluate_masks=False,
          matching_iou_threshold=matching_iou_threshold,
          evaluate_corlocs=False,
          group_of_weight=1.0)
...
class OpenImagesInstanceSegmentationChallengeEvaluator(OpenImagesChallengeEvaluator):
    __doc__ = 'A class implements Open Images Instance Segmentation Challenge metric.'
    ...
    Args:
      ...
      evaluate_corlocs: if True, additionally evaluates and returns CorLoc.
    """
        super(OpenImagesInstanceSegmentationChallengeEvaluator, self).__init__(categories=categories,
          evaluate_masks=True,
          matching_iou_threshold=matching_iou_threshold,
          evaluate_corlocs=False,
          group_of_weight=0.0)
...
ObjectDetectionEvaluationState = collections.namedtuple('ObjectDetectionEvaluationState', [
...
class ObjectDetectionEvaluation(object):
    __doc__ = 'Internal implementation of Pascal object detection metrics.'
    ...
    """
        ...
        self.per_image_eval = per_image_eval_class(num_groundtruth_classes=num_groundtruth_classes,
          matching_iou_threshold=matching_iou_threshold,
    ...
    Raises:
      ValueError: if the number of boxes, scores and class labels differ in
        length.
    """
        if not len(detected_boxes) != len(detected_scores):
            if len(detected_boxes) != len(detected_class_labels):
        ...
        scores, tp_fp_labels, is_class_correctly_detected_in_image = self.per_image_eval.compute_object_detection_metrics(detected_boxes=detected_boxes,
          detected_scores=detected_scores,
        ...
        if (self.num_gt_instances_per_class == 0).any():
            ...
            for class_index in range(self.num_class):
                if self.num_gt_instances_per_class[class_index] == 0:
                    continue
                elif not self.scores_per_class[class_index]:
                ...
                recall_within_bound_indices = [index for index, value in enumerate(recall) if value >= self.recall_lower_bound if value <= self.recall_upper_bound]
