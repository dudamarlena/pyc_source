...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/data_decoders/tf_example_decoder.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 21597 bytes
...
class _ClassTensorHandler(slim_example_decoder.Tensor):
    __doc__ = 'An ItemHandler to fetch class ids from class text.'
    ...
    """
        ...
        name_to_id_table = hash_table_class(initializer=lookup.KeyValueTensorInitializer(keys=(tf.constant(list(name_to_id.keys()))),
          values=tf.constant((list(name_to_id.values())), dtype=(tf.int64))),
          default_value=(-1))
        display_name_to_id = label_map_util.get_label_map_dict(label_map_proto_file,
          use_display_name=True)
        display_name_to_id_table = hash_table_class(initializer=lookup.KeyValueTensorInitializer(keys=(tf.constant(list(display_name_to_id.keys()))),
        ...
        super(_ClassTensorHandler, self).__init__(tensor_key, shape_keys, shape, default_value)

    def tensors_to_item(self, keys_to_tensors):
        unmapped_tensor = super(_ClassTensorHandler, self).tensors_to_item(keys_to_tensors)
        return tf.maximum(self._name_to_id_table.lookup(unmapped_tensor), self._display_name_to_id_table.lookup(unmapped_tensor))


class _BackupHandler(slim_example_decoder.ItemHandler):
    __doc__ = 'An ItemHandler that tries two ItemHandlers in order.'
    ...
    def tensors_to_item(self, keys_to_tensors):
        ...
        return tf.cond(pred=(tf.equal(tf.reduce_prod(tf.shape(item)), 0)),
          true_fn=(lambda : self._backup.tensors_to_item(keys_to_tensors)),
          false_fn=(lambda : item))


class TfExampleDecoder(data_decoder.DataDecoder):
    __doc__ = 'Tensorflow Example proto decoder.'

    def __init__(self, load_instance_masks=False, instance_mask_type=input_reader_pb2.NUMERICAL_MASKS, label_map_proto_file=None, use_display_name=False, dct_method='', num_keypoints=0, num_additional_channels=0, load_multiclass_scores=False):
    ...
    """
        ...
        if dct_method:
            ...
            additional_channel_image = slim_example_decoder.Image(image_key='image/additional_channels/encoded',
              format_key='image/format',
              channels=1,
              repeated=True,
              dct_method=dct_method)
            ...
            additional_channel_image = slim_example_decoder.Image(image_key='image/additional_channels/encoded',
              ...
              channels=1,
              repeated=True)
        self.items_to_handlers = {fields.InputDataFields.image: image, 
        ...
        if load_multiclass_scores:
            self.keys_to_features['image/object/class/multiclass_scores'] = tf.VarLenFeature(tf.float32)
            self.items_to_handlers[fields.InputDataFields.multiclass_scores] = slim_example_decoder.Tensor('image/object/class/multiclass_scores')
        else:
            if num_additional_channels > 0:
                self.keys_to_features['image/additional_channels/encoded'] = tf.FixedLenFeature((
                 num_additional_channels,), tf.string)
                self.items_to_handlers[fields.InputDataFields.image_additional_channels] = additional_channel_image
            else:
                self._num_keypoints = num_keypoints
                if num_keypoints > 0:
                    self.keys_to_features['image/object/keypoint/x'] = tf.VarLenFeature(tf.float32)
                    self.keys_to_features['image/object/keypoint/y'] = tf.VarLenFeature(tf.float32)
                    self.items_to_handlers[fields.InputDataFields.groundtruth_keypoints] = slim_example_decoder.ItemHandlerCallback([
                     'image/object/keypoint/y', 'image/object/keypoint/x'], self._reshape_keypoints)
                if load_instance_masks:
                    if instance_mask_type in (input_reader_pb2.DEFAULT,
                     input_reader_pb2.NUMERICAL_MASKS):
                        ...
                        self.items_to_handlers[fields.InputDataFields.groundtruth_instance_masks] = slim_example_decoder.ItemHandlerCallback([
                         'image/object/mask', 'image/height', 'image/width'], self._reshape_instance_masks)
                    else:
                        if instance_mask_type == input_reader_pb2.PNG_MASKS:
                            self.keys_to_features['image/object/mask'] = tf.VarLenFeature(tf.string)
                            self.items_to_handlers[fields.InputDataFields.groundtruth_instance_masks] = slim_example_decoder.ItemHandlerCallback([
                             'image/object/mask', 'image/height', 'image/width'], self._decode_png_instance_masks)
                        else:
                            raise ValueError('Did not recognize the `instance_mask_type` option.')
            if label_map_proto_file:
                label_handler = _BackupHandler(_ClassTensorHandler('image/object/class/text',
                  label_map_proto_file, default_value=''), slim_example_decoder.Tensor('image/object/class/label'))
                image_label_handler = _BackupHandler(_ClassTensorHandler((fields.TfExampleFields.image_class_text),
                  label_map_proto_file,
                  default_value=''), slim_example_decoder.Tensor(fields.TfExampleFields.image_class_label))
            else:
                label_handler = slim_example_decoder.Tensor('image/object/class/label')
            image_label_handler = slim_example_decoder.Tensor(fields.TfExampleFields.image_class_label)
        self.items_to_handlers[fields.InputDataFields.groundtruth_classes] = label_handler
        self.items_to_handlers[fields.InputDataFields.groundtruth_image_classes] = image_label_handler
    ...
    """
        ...
        tensor_dict[fields.InputDataFields.original_image_spatial_shape] = tf.shape(tensor_dict[fields.InputDataFields.image])[:2]
        ...
        tensor_dict[fields.InputDataFields.groundtruth_weights] = tf.cond(tf.greater(tf.shape(tensor_dict[fields.InputDataFields.groundtruth_weights])[0], 0), lambda : tensor_dict[fields.InputDataFields.groundtruth_weights], default_groundtruth_weights)
        return tensor_dict
    ...
    """
        ...
        return tf.cond(tf.greater(tf.size(png_masks), 0), lambda : tf.map_fn(decode_png_mask, png_masks, dtype=(tf.float32)), lambda : tf.zeros(tf.cast((tf.stack([0, height, width])), dtype=(tf.int32))))