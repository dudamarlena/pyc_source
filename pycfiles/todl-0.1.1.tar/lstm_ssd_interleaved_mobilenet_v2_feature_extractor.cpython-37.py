...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/lstm_object_detection/models/lstm_ssd_interleaved_mobilenet_v2_feature_extractor.py
...
import tensorflow.python.framework as tf_ops
...
class LSTMSSDInterleavedMobilenetV2FeatureExtractor(lstm_ssd_meta_arch.LSTMSSDInterleavedFeatureExtractor):
    __doc__ = 'LSTM-SSD Interleaved Feature Extractor using MobilenetV2 features.'
    ...
    Args:
      ...
      override_base_feature_extractor_hyperparams: Whether to override
        ...
        `conv_hyperparams_fn`.
    """
        super(LSTMSSDInterleavedMobilenetV2FeatureExtractor, self).__init__(is_training=is_training,
    ...
    """
        ...
        with tf.variable_scope(scope_name, reuse=(self._reuse_weights)) as (base_scope):
            net, end_points = mobilenet_v2.mobilenet_base(preprocessed_inputs,
              depth_multiplier=(self._depth_multipliers[0]),
              conv_defs=mobilenet_defs.mobilenet_v2_lite_def(is_quantized=(self._is_quantized)),
              use_explicit_padding=(self._use_explicit_padding),
              scope=base_scope)
        ...
        with tf.variable_scope(scope_name, reuse=(self._reuse_weights)) as (base_scope):
            if self._low_res:
                ...
                with tf.name_scope(None):
                    inputs_small = tf.identity(inputs_small,
                      name='normalized_input_image_tensor_small')
            else:
                inputs_small = preprocessed_inputs
            net, end_points = mobilenet_v2.mobilenet_base(inputs_small,
    ...
    Returns:
      ...
      step: the step
    """
        lstm_cell = lstm_cells.GroupedConvLSTMCell(filter_size=(3, 3),
    ...
    """
        ...
        with slim.arg_scope(mobilenet_v2.training_scope(is_training=(self._is_training),
          bn_decay=0.9997)):
            with slim.arg_scope([mobilenet.depth_multiplier], min_depth=(self._min_depth),
              divisible_by=8):
        ...
        with slim.arg_scope(self._conv_hyperparams_fn()):
            with tf.variable_scope('LSTM', reuse=(self._reuse_weights)):
                output_size = (
                 large_base_feature_shape[1], large_base_feature_shape[2])
                ...
                net_seq, states_out = rnn_decoder.multi_input_rnn_decoder(nets_seq,
                  init_state,
                  lstm_cell,
                  step,
            ...
            with tf.variable_scope('FeatureMap'):
                feature_maps = feature_map_generators.multi_resolution_feature_maps(feature_map_layout=(self._feature_map_layout),
