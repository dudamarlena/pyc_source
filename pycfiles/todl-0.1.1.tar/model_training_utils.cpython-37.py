...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/nlp/bert/model_training_utils.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 20411 bytes
    ...
    if _sentinel is not None:
        ...
        if run_eagerly:
            if isinstance(strategy, tf.distribute.experimental.TPUStrategy):
                raise ValueError('TPUStrategy should not run eagerly as it heavily relies on graph optimization for the distributed system.')
        if eval_input_fn:
            if eval_steps is None or metric_fn is None:
    ...
    with distribution_utils.get_strategy_scope(strategy):
        ...
        def _replicated_step(inputs):
            ...
            if explicit_allreduce:
                grad_utils.minimize_using_explicit_allreduce(tape, optimizer, loss, training_vars, pre_allreduce_callbacks, post_allreduce_callbacks)
            else:
            ...
            with eval_summary_writer.as_default():
                for metric in eval_metrics + model.metrics:
                    ...
                    tf.summary.scalar((metric.name),
                      metric_value, step=current_training_step)
        ...
        while current_step < total_training_steps:
            ...
            if current_step % steps_per_epoch == 0:
                if current_step < total_training_steps:
                    _save_checkpoint(strategy, checkpoint, model_dir, checkpoint_name.format(step=current_step))
                    if sub_model_export_name:
                        _save_checkpoint(strategy, sub_model_checkpoint, model_dir, '%s_step_%d.ckpt' % (sub_model_export_name, current_step))
                if eval_input_fn:
        ...
        _save_checkpoint(strategy, checkpoint, model_dir, checkpoint_name.format(step=current_step))
        if sub_model_export_name:
            _save_checkpoint(strategy, sub_model_checkpoint, model_dir, '%s.ckpt' % sub_model_export_name)
        if eval_input_fn:
            logging.info('Running final evaluation after training is complete.')
            _run_evaluation(current_step, _get_input_iterator(eval_input_fn, strategy))
        training_summary = {'total_training_steps':total_training_steps, 
         'train_loss':_float_metric_value(train_loss_metric)}
        if eval_metrics:
