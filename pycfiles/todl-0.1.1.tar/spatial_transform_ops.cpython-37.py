...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/spatial_transform_ops.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 21618 bytes
...
def _valid_indicator(feature_grid_y, feature_grid_x, true_feature_shapes):
  ...
  """
    ...
    valid_indicator = tf.logical_and(tf.expand_dims(tf.logical_and(feature_grid_y >= 0, tf.less(feature_grid_y, height)), 3), tf.expand_dims(tf.logical_and(feature_grid_x >= 0, tf.less(feature_grid_x, width)), 2))
...
def ravel_indices(feature_grid_y, feature_grid_x, num_levels, height, width, box_levels):
  ...
  """
    ...
    batch_dim_indices = tf.reshape(tf.range(batch_size) * batch_dim_offset, [batch_size, 1, 1, 1]) * tf.ones([1, num_boxes, size_y, size_x], dtype=(tf.int32))
    box_level_indices = tf.reshape(box_levels * level_dim_offset, [batch_size, num_boxes, 1, 1]) * tf.ones([1, 1, size_y, size_x], dtype=(tf.int32))
    height_indices = tf.reshape(feature_grid_y * height_dim_offset, [
     batch_size, num_boxes, size_y, 1]) * tf.ones([1, 1, 1, size_x], dtype=(tf.int32))
    width_indices = tf.reshape(feature_grid_x, [batch_size, num_boxes, 1, size_x]) * tf.ones([1, 1, size_y, 1], dtype=(tf.int32))
...
def pad_to_max_size(features):
  ...
  """
    ...
    features_all = [tf.image.pad_to_bounding_box(feature, 0, 0, max_height, max_width) for feature in features]
...
def _gather_valid_indices(tensor, indices, padding_value=0.0):
  ...
  """
    padded_tensor = tf.concat([
     padding_value * tf.ones([1, tf.shape(tensor)[(-1)]], dtype=(tensor.dtype)), tensor],
      axis=0)
    padded_tensor *= 1.0
    return tf.gather(padded_tensor, indices + 1)


def multilevel_roi_align(features, boxes, box_levels, output_size, num_samples_per_cell_y=1, num_samples_per_cell_x=1, align_corners=False, extrapolation_value=0.0, scope=None):
  ...
  """
    with tf.name_scope(scope, 'MultiLevelRoIAlign'):
        ...
        box_grid_y, box_grid_x = box_grid_coordinate_vectors(boxes,
          size_y=size_y, size_x=size_x, align_corners=align_corners)
        feature_grid_y0, feature_grid_x0, feature_grid_y1, feature_grid_x1 = feature_grid_coordinate_vectors(box_grid_y, box_grid_x)
        feature_grid_y = tf.reshape(tf.stack([feature_grid_y0, feature_grid_y1], axis=3), [
         batch_size, num_boxes, -1])
        feature_grid_x = tf.reshape(tf.stack([feature_grid_x0, feature_grid_x1], axis=3), [
         batch_size, num_boxes, -1])
        feature_coordinates = ravel_indices(feature_grid_y, feature_grid_x, num_levels, max_feature_height, max_feature_width, box_levels)
        valid_indices = _valid_indicator(feature_grid_y, feature_grid_x, true_feature_shapes)
        feature_coordinates = tf.where(valid_indices, feature_coordinates, -1 * tf.ones_like(feature_coordinates))
        flattened_features = tf.reshape(features, [-1, num_filters])
        flattened_feature_values = _gather_valid_indices(flattened_features, feature_coordinates, extrapolation_value)
        features_per_box = tf.reshape(flattened_feature_values, [
        ...
        features_per_box = tf.reshape(features_per_box, [
         batch_size * num_boxes, size_y * 2, size_x * 2, num_filters])
        features_per_box = tf.nn.avg_pool(features_per_box, [
...
def native_crop_and_resize(image, boxes, crop_size, scope=None):
    ...
    with tf.name_scope(scope, 'CropAndResize'):
        cropped_regions = tf.image.crop_and_resize(image, tf.reshape(boxes, [-1] + boxes.shape.as_list()[2:]), get_box_inds(boxes), crop_size)
        final_shape = tf.concat([tf.shape(boxes)[:2],
         tf.shape(cropped_regions)[1:]],
          axis=0)
...
def matmul_crop_and_resize(image, boxes, crop_size, extrapolation_value=0.0, scope=None):
  ...
  """
    with tf.name_scope(scope, 'MatMulCropAndResize'):
        ...
        return multilevel_roi_align([image], boxes,
          box_levels,
          crop_size,
          align_corners=True,
