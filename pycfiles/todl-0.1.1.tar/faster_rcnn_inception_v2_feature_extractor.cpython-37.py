...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/models/faster_rcnn_inception_v2_feature_extractor.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 10666 bytes
...
def _batch_norm_arg_scope(list_ops, use_batch_norm=True, batch_norm_decay=0.9997, batch_norm_epsilon=0.001, batch_norm_scale=False, train_batch_norm=False):
    ...
    if use_batch_norm:
        batch_norm_params = {'is_training':train_batch_norm,  'scale':batch_norm_scale, 
    ...
    return slim.arg_scope(list_ops, normalizer_fn=normalizer_fn,
      normalizer_params=batch_norm_params)


class FasterRCNNInceptionV2FeatureExtractor(faster_rcnn_meta_arch.FasterRCNNFeatureExtractor):
    __doc__ = 'Faster R-CNN Inception V2 feature extractor implementation.'
    ...
    """
        if first_stage_features_stride != 8:
    ...
    """
        ...
        shape_assert = tf.Assert(tf.logical_and(tf.greater_equal(tf.shape(preprocessed_inputs)[1], 33), tf.greater_equal(tf.shape(preprocessed_inputs)[2], 33)), [
         'image size must at least be 33 in both height and width.'])
        with tf.control_dependencies([shape_assert]):
            with tf.variable_scope('InceptionV2', reuse=(self._reuse_weights)) as (scope):
                with _batch_norm_arg_scope([slim.conv2d, slim.separable_conv2d], batch_norm_scale=True,
                  train_batch_norm=(self._train_batch_norm)):
                    _, activations = inception_v2.inception_v2_base(preprocessed_inputs,
                      final_endpoint='Mixed_4e',
                      min_depth=(self._min_depth),
                      depth_multiplier=(self._depth_multiplier),
                      scope=scope)
        return (
         activations['Mixed_4e'], activations)
                    ...
                    with tf.variable_scope('Mixed_5a'):
                        with tf.variable_scope('Branch_0'):
                            branch_0 = slim.conv2d(net,
                              (depth(128)), [1, 1], weights_initializer=(trunc_normal(0.09)),
                              scope='Conv2d_0a_1x1')
                            branch_0 = slim.conv2d(branch_0, (depth(192)), [3, 3], stride=2, scope='Conv2d_1a_3x3')
                        with tf.variable_scope('Branch_1'):
                            branch_1 = slim.conv2d(net,
                              (depth(192)), [1, 1], weights_initializer=(trunc_normal(0.09)),
                              scope='Conv2d_0a_1x1')
                    ...
                    with tf.variable_scope('Mixed_5b'):
                        ...
                        with tf.variable_scope('Branch_2'):
                            branch_2 = slim.conv2d(net,
                              (depth(160)), [1, 1], weights_initializer=(trunc_normal(0.09)),
                              scope='Conv2d_0a_1x1')
                        ...
                        with tf.variable_scope('Branch_3'):
                            ...
                            branch_3 = slim.conv2d(branch_3,
                              (depth(128)), [1, 1], weights_initializer=(trunc_normal(0.1)),
                              scope='Conv2d_0b_1x1')
