...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/staging/training/grad_utils.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 6284 bytes
...
def _filter_and_allreduce_gradients(grads_and_vars, allreduce_precision='float32'):
  ...
  """
    ...
    allreduced_grads = tf.distribute.get_replica_context().all_reduce(tf.distribute.ReduceOp.SUM, grads)
    if allreduce_precision == 'float16':
        allreduced_grads = [tf.cast(grad, 'float32') for grad in allreduced_grads]
    return (
     allreduced_grads, variables)
...
def minimize_using_explicit_allreduce(tape, optimizer, loss, trainable_variables, pre_allreduce_callbacks=None, post_allreduce_callbacks=None):
  ...
  """
    if isinstance(optimizer, tf.keras.mixed_precision.experimental.LossScaleOptimizer):
        ...
        allreduced_scaled_grads, filtered_training_vars = _filter_and_allreduce_gradients(grads_and_vars,
        ...
        if pre_allreduce_callbacks:
            grads_and_vars = _run_callbacks(pre_allreduce_callbacks, grads_and_vars)
        allreduced_grads, filtered_training_vars = _filter_and_allreduce_gradients(grads_and_vars,
