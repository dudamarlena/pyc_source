...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/feelvos/utils/embedding_utils.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 46115 bytes
...
def pairwise_distances2(x, y):
  ...
  """
    return tf.reduce_sum((tf.squared_difference(x[:, tf.newaxis], y[tf.newaxis, :])),
      axis=(-1))
...
def cross_correlate(x, y, max_distance=9):
  ...
  """
    with tf.name_scope('cross_correlation'):
        corr = correlation_cost_op.correlation_cost((x[tf.newaxis]),
          (y[tf.newaxis]), kernel_size=1, max_displacement=max_distance,
          stride_1=1,
          stride_2=1,
          pad=max_distance)
...
def nearest_neighbor_features_per_object(reference_embeddings, query_embeddings, reference_labels, max_neighbors_per_object, k_nearest_neighbors, gt_ids=None, n_chunks=100):
  ...
  """
    with tf.name_scope('nn_features_per_object'):
        ...
        nn_features_reshaped = tf.reshape(nn_features, tf.stack(shape[:3] + [tf.size(gt_ids),
         nn_features_dim]))
...
def _nn_features_per_object_for_chunk(reference_embeddings, query_embeddings, wrong_label_mask, k_nearest_neighbors):
  ...
  """
    ...
    if k_nearest_neighbors == 1:
        ...
        pad_dist = tf.tile((tf.reduce_max(masked_dists, axis=2)[(..., tf.newaxis)]), multiples=[
         1, 1, k_nearest_neighbors])
...
def create_embedding_segmentation_features(features, feature_dimension, n_layers, kernel_size, reuse, atrous_rates=None):
  ...
  """
    if not (atrous_rates is None or atrous_rates):
    ...
    with tf.variable_scope('embedding_seg', reuse=reuse):
        for n in range(n_layers):
            features = model.split_separable_conv2d(features,
              feature_dimension, kernel_size=kernel_size, rate=(atrous_rates[n]),
              scope=('split_separable_conv2d_{}'.format(n)))
...
def add_image_summaries(images, nn_features, logits, batch_size, prev_frame_nn_features=None):
  ...
  """
    ...
    if prev_frame_nn_features is not None:
        tf.summary.image('nn_bg_features_prev', (prev_frame_nn_features[..., 1:2, 0]),
...
def get_embeddings(images, model_options, embedding_dimension):
  ...
  """
    ...
    if model_options.decoder_output_stride is not None:
        ...
        features = model.refine_by_decoder(features,
          end_points,
          crop_size=[
         height, width],
    ...
    with tf.variable_scope('embedding'):
        embeddings = split_separable_conv2d_with_identity_initializer(features,
          embedding_dimension, scope='split_separable_conv2d')
    return embeddings
...
def get_logits_with_matching(images, model_options, weight_decay=0.0001, reuse=None, is_training=False, fine_tune_batch_norm=False, reference_labels=None, batch_size=None, num_frames_per_video=None, embedding_dimension=None, max_neighbors_per_object=0, k_nearest_neighbors=1, use_softmax_feedback=True, initial_softmax_feedback=None, embedding_seg_feature_dimension=256, embedding_seg_n_layers=4, embedding_seg_kernel_size=7, embedding_seg_atrous_rates=None, normalize_nearest_neighbor_distances=True, also_attend_to_previous_frame=True, damage_initial_previous_frame_mask=False, use_local_previous_frame_attention=True, previous_frame_attention_window_size=15, use_first_frame_matching=True, also_return_embeddings=False, ref_embeddings=None):
  ...
  """
    ...
    scaled_reference_labels = tf.image.resize_nearest_neighbor(reference_labels,
    ...
    if num_frames_per_video is None:
        ...
        for n in range(batch_size):
            ...
            nn_features_n, ref_obj_ids = nearest_neighbor_features_per_object(reference_embedding,
              queries_embedding, reference_labels, max_neighbors_per_object,
              k_nearest_neighbors, n_chunks=n_chunks)
        ...
        if not ref_embeddings is None or also_attend_to_previous_frame or use_softmax_feedback:
            features = features[:, 2:]
        else:
            features = features[:, 1:]
    ...
    for n in range(batch_size):
        ...
        features_n_tiled = tf.tile((features_n[tf.newaxis]), multiples=[
         n_objs, 1, 1, 1, 1])
        ...
        if also_attend_to_previous_frame:
            ...
            if is_training:
                ...
                initial_softmax_feedback_n = create_initial_softmax_from_labels(prev_frame_labels,
                  (reshaped_reference_labels[n][0]), decoder_output_stride=None,
                  reduce_labels=True)
            else:
                if initial_softmax_feedback is not None:
                    initial_softmax_feedback_n = initial_softmax_feedback[n]
                else:
                    initial_softmax_feedback_n = None
        elif initial_softmax_feedback_n is None:
        ...
        if not also_attend_to_previous_frame:
            ...
            for t in range(num_frames_per_video - n_ref_frames):
                ...
                if also_attend_to_previous_frame:
                    ...
                    if use_local_previous_frame_attention:
                        ...
                        prev_frame_nn_features_n = local_previous_frame_nearest_neighbor_features_per_object(last_frame_embedding,
                          (query_embeddings[0]),
                          prev_frame_labels,
                          (all_ref_obj_ids[n]),
                          max_distance=previous_frame_attention_window_size)
                    else:
                        prev_frame_nn_features_n, _ = nearest_neighbor_features_per_object(last_frame_embedding,
                          query_embeddings, prev_frame_labels, max_neighbors_per_object,
                          k_nearest_neighbors, gt_ids=(all_ref_obj_ids[n]))
                ...
                embedding_seg_features_n_t = create_embedding_segmentation_features(features_n_concat_t,
                  embedding_seg_feature_dimension, embedding_seg_n_layers,
                  embedding_seg_kernel_size, (reuse or n > 0),
                  atrous_rates=embedding_seg_atrous_rates)
                logits_t = model.get_branch_logits(embedding_seg_features_n_t,
                  1,
            ...
            logits_shape = tf.stack([
             n_objs, num_frames_per_video - n_ref_frames] + resolve_shape(logits)[2:-1])
            ...
            add_image_summaries((images[n * num_frames_per_video:(n + 1) * num_frames_per_video]),
              nn_features_n,
              logits_transposed,
              batch_size=1,
...
def subsample_reference_embeddings_and_labels(reference_embeddings_flat, reference_labels_flat, ref_obj_ids, max_neighbors_per_object):
  ...
  """
    ...
    max_neighbors_per_object_repeated = tf.tile((tf.constant(max_neighbors_per_object)[tf.newaxis]),
      multiples=[
     tf.size(ref_obj_ids)])
...
def conv2d_identity_initializer(scale=1.0, mean=0, stddev=0.03):
  ...
  """
    ...
    """
        ...
        return tf.constant(weights, dtype=dtype) + tf.truncated_normal(shape,
...
def create_initial_softmax_from_labels(last_frame_labels, reference_labels, decoder_output_stride, reduce_labels):
  ...
  """
    if decoder_output_stride is None:
        ...
        labels_output_size = tf.image.resize_nearest_neighbor(last_frame_labels,
          [h_sub, w_sub], align_corners=True)
        reference_labels_output_size = tf.image.resize_nearest_neighbor(reference_labels,
          [h_sub, w_sub], align_corners=True)
    if reduce_labels:
...
def local_previous_frame_nearest_neighbor_features_per_object(prev_frame_embedding, query_embedding, prev_frame_labels, gt_ids, max_distance=9):
  ...
  """
    with tf.name_scope('local_previous_frame_nearest_neighbor_features_per_object'):
        ...
        if USE_CORRELATION_COST:
            offset_labels = correlation_cost_op.correlation_cost((tf.ones((1, height, width, 1))),
