...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/runners.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 20929 bytes
...
def create_dataset_and_model(config, split, shuffle, repeat):
  ...
  """
    ...
    if config.dataset_type == 'pianoroll':
        inputs, targets, lengths, mean = datasets.create_pianoroll_dataset((config.dataset_path),
          split, (config.batch_size), shuffle=shuffle, repeat=repeat)
        emission_bias_init = -tf.log(1.0 / tf.clip_by_value(mean, 0.0001, 0.9999) - 1)
        emission_distribution_class = base.ConditionalBernoulliDistribution
    else:
        if config.dataset_type == 'speech':
            inputs, targets, lengths = datasets.create_speech_dataset((config.dataset_path),
              (config.batch_size), samples_per_timestep=(config.data_dimension),
            ...
            model = vrnn.create_vrnn((inputs.get_shape().as_list()[2]), (config.latent_size),
            ...
            if config.model == 'srnn':
                model = srnn.create_srnn((inputs.get_shape().as_list()[2]), (config.latent_size),
                  emission_distribution_class,
                  emission_bias_init=emission_bias_init,
                  proposal_type=(config.proposal_type),
                  sigma_min=sigma_min,
...
def run_train(config, create_dataset_and_model_fn=create_dataset_and_model):
  ...
  """
    ...
    def create_logging_hook(step, bound_value):
        ...
        logging_hook = tf.train.LoggingTensorHook({'step':step, 
    ...
    def create_loss():
        inputs, targets, lengths, model, _ = create_dataset_and_model_fn(config,
          split='train', shuffle=True, repeat=True)
        if config.bound == 'elbo':
            ll_per_seq, _, _ = bounds.iwae(model,
              (inputs, targets), lengths, num_samples=1, parallel_iterations=(config.parallel_iterations))
        else:
            if config.bound == 'iwae':
                ll_per_seq, _, _ = bounds.iwae(model,
                  (inputs, targets), lengths, num_samples=(config.num_samples), parallel_iterations=(config.parallel_iterations))
            else:
                if config.bound in ('fivo', 'fivo-aux'):
                    if config.resampling_type == 'relaxed':
                        ll_per_seq, _, _, _ = bounds.fivo(model,
                        ...
                        ll_per_seq, _, _, _ = bounds.fivo(model,
                          (inputs, targets), lengths, num_samples=(config.num_samples), resampling_criterion=(smc.ess_criterion),
                          resampling_type=(config.resampling_type),
                          random_seed=(config.random_seed),
                          parallel_iterations=(config.parallel_iterations))
              ...
              log_step_count_steps=(config.summarize_every)) as (sess):
                ...
                while not sess.should_stop():
                    if cur_step <= config.max_steps:
                        if config.task > 0:
                            cur_step = start_training or sess.run(global_step)
...
def run_eval(config, create_dataset_and_model_fn=create_dataset_and_model):
  ...
  """
    ...
    def create_graph():
        global_step = tf.train.get_or_create_global_step()
        inputs, targets, lengths, model, _ = create_dataset_and_model_fn(config,
          split=(config.split), shuffle=False, repeat=False)
        elbo_ll_per_seq, _, _ = bounds.iwae(model,
          (inputs, targets), lengths, num_samples=1, parallel_iterations=(config.parallel_iterations))
        iwae_ll_per_seq, _, _ = bounds.iwae(model,
          (inputs, targets), lengths, num_samples=(config.num_samples), parallel_iterations=(config.parallel_iterations))
        fivo_ll_per_seq, _, _, _ = bounds.fivo(model,
          (inputs, targets), lengths, num_samples=(config.num_samples), resampling_criterion=(smc.ess_criterion),
          random_seed=(config.random_seed),
          parallel_iterations=(config.parallel_iterations))
    ...
    def average_bounds_over_dataset(lower_bounds, total_batch_length, batch_size, sess):
    ...
    def summarize_lls(lls_per_t, lls_per_seq, summary_writer, step):
        ...
        for i, bound in enumerate(['elbo', 'iwae', 'fivo']):
            per_t_summary = scalar_summary('%s/%s_ll_per_t' % (config.split, bound), lls_per_t[i])
            per_seq_summary = scalar_summary('%s/%s_ll_per_seq' % (
             config.split, bound), lls_per_seq[i])
    ...
    with tf.Graph().as_default():
        ...
        summary_writer = tf.summary.FileWriter(summary_dir,
          flush_secs=15, max_queue=100)
        saver = tf.train.Saver()
        with tf.train.SingularMonitoredSession() as (sess):
            wait_for_checkpoint(saver, sess, config.logdir)
            step = sess.run(global_step)
            tf.logging.info('Model restored from step %d, evaluating.' % step)
            ll_per_t, ll_per_seq = average_bounds_over_dataset(lower_bounds, total_batch_length, batch_size, sess)
            summarize_lls(ll_per_t, ll_per_seq, summary_writer, step)
            tf.logging.info('%s elbo ll/t: %f, iwae ll/t: %f fivo ll/t: %f', config.split, ll_per_t[0], ll_per_t[1], ll_per_t[2])
            tf.logging.info('%s elbo ll/seq: %f, iwae ll/seq: %f fivo ll/seq: %f', config.split, ll_per_seq[0], ll_per_seq[1], ll_per_seq[2])
...
def run_sample(config, create_dataset_and_model_fn=create_dataset_and_model):
    ...
    def sample_from_model(model, initial_state, initial_inputs, mean):
        initial_state, initial_output = model.sample_step(initial_state, initial_inputs, 0)
        output_ta = tf.TensorArray(size=(config.sample_length), dtype=(tf.float32),
          dynamic_size=False,
        ...
        _, _, _, output_ta = tf.while_loop(sample_predicate,
          sample_step,
          loop_vars=(
         t0, initial_state, initial_output, output_ta),
          parallel_iterations=(config.parallel_iterations))
    ...
    def create_graph():
        inputs, targets, lengths, model, mean = create_dataset_and_model_fn(config,
          split=(config.split), shuffle=True, repeat=True)
        ...
        if config.bound == 'elbo':
            _, _, state = bounds.iwae(model,
              (input_prefixes, target_prefixes), prefix_lengths,
              num_samples=1)
        else:
            if config.bound == 'iwae':
                _, _, state = bounds.iwae(model,
                ...
                if config.bound == 'fivo':
                    _, _, _, state = bounds.fivo(model,
                      (input_prefixes, target_prefixes), prefix_lengths, num_samples=(config.num_samples),
                      resampling_criterion=(smc.ess_criterion),
                      random_seed=(config.random_seed))
