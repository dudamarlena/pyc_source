...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/ops.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 45569 bytes
"""A module for helper tensorflow ops."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import collections, math, six
from six.moves import range
from six.moves import zip
import tensorflow as tf
import object_detection.core as fields
from object_detection.utils import shape_utils
import object_detection.utils as spatial_ops
...
def expanded_shape(orig_shape, start_dim, num_dims):
  ...
  """
    with tf.name_scope('ExpandedShape'):
        start_dim = tf.expand_dims(start_dim, 0)
...
def normalized_to_image_coordinates(normalized_boxes, image_shape, parallel_iterations=32):
  ...
  """
    ...
    absolute_boxes = shape_utils.static_or_dynamic_map_fn(_to_absolute_coordinates,
...
def pad_to_multiple(tensor, multiple):
  ...
  """
    if multiple == 1:
        ...
        elif tensor_height is None:
            tensor_height = tf.shape(tensor)[1]
            padded_tensor_height = tf.cast((tf.ceil(tf.cast(tensor_height, dtype=(tf.float32)) / tf.cast(multiple, dtype=(tf.float32)))),
              dtype=(tf.int32)) * multiple
        else:
            padded_tensor_height = int(math.ceil(float(tensor_height) / multiple) * multiple)
        if tensor_width is None:
            tensor_width = tf.shape(tensor)[2]
            padded_tensor_width = tf.cast((tf.ceil(tf.cast(tensor_width, dtype=(tf.float32)) / tf.cast(multiple, dtype=(tf.float32)))),
              dtype=(tf.int32)) * multiple
        else:
            padded_tensor_width = int(math.ceil(float(tensor_width) / multiple) * multiple)
...
def padded_one_hot_encoding(indices, depth, left_pad):
  ...
  """
    if not (depth < 0 or isinstance(depth, six.integer_types)):
        raise ValueError('`depth` must be a non-negative integer.')
    if not (left_pad < 0 or isinstance(left_pad, six.integer_types)):
    ...
    def one_hot_and_pad():
        one_hot = tf.cast(tf.one_hot((tf.cast(indices, tf.int64)), depth, on_value=1,
          off_value=0), tf.float32)
        return tf.pad(one_hot, [[0, 0], [left_pad, 0]], mode='CONSTANT')

    result = tf.cond(tf.greater(tf.size(indices), 0), one_hot_and_pad, lambda : tf.zeros((depth + left_pad, 0)))
...
def dense_to_sparse_boxes(dense_locations, dense_num_boxes, num_classes):
  ...
  """
    ...
    tiled_classes = [tf.tile([i], tf.expand_dims(dense_num_boxes[i], 0)) for i in range(num_classes)]
...
def retain_groundtruth(tensor_dict, valid_indices):
  ...
  """
    ...
    if not len(input_shape) == 1:
        raise len(input_shape) == 2 and input_shape[1] == 1 or ValueError('The shape of valid_indices is invalid.')
    valid_indices = tf.reshape(valid_indices, [-1])
    valid_dict = {}
    if fields.InputDataFields.groundtruth_boxes in tensor_dict:
        num_boxes = tf.maximum(tf.shape(tensor_dict[fields.InputDataFields.groundtruth_boxes])[0], 1)
        for key in tensor_dict:
            if key in [fields.InputDataFields.groundtruth_boxes,
             ...
             fields.InputDataFields.groundtruth_difficult,
             fields.InputDataFields.groundtruth_label_types]:
                valid_dict[key] = tf.reshape(tf.gather(tf.reshape(tensor_dict[key], [num_boxes, -1]), valid_indices), [-1])
            else:
                valid_dict[key] = tensor_dict[key]

    else:
        raise ValueError('%s not present in input tensor dict.' % fields.InputDataFields.groundtruth_boxes)
    return valid_dict
...
def replace_nan_groundtruth_label_scores_with_ones(label_scores):
  ...
  """
    return tf.where(tf.is_nan(label_scores), tf.ones(tf.shape(label_scores)), label_scores)
...
def filter_groundtruth_with_nan_box_coordinates(tensor_dict):
  ...
  """
    ...
    nan_indicator_vector = tf.greater(tf.reduce_sum(tf.cast((tf.is_nan(groundtruth_boxes)),
      dtype=(tf.int32)),
...
def filter_unrecognized_classes(tensor_dict):
  ...
  """
    ...
    recognized_indices = tf.where(tf.greater(tensor_dict[fields.InputDataFields.groundtruth_classes], unrecognized_label))
  ...
  """
    with tf.variable_scope(scope, 'NormalizeToTarget', [inputs]):
        if not inputs.get_shape():
            ...
            if not dim < 0:
                if dim >= input_rank:
                    raise ValueError('dim must be non-negative but smaller than the input rank.')
                if not input_shape[dim]:
                    raise ValueError('input shape should be statically defined along the specified dimension.')
                depth = input_shape[dim]
                if not isinstance(target_norm_value, float):
                    if isinstance(target_norm_value, list):
                        if not (len(target_norm_value) == depth and all([isinstance(val, float) for val in target_norm_value])):
                            raise ValueError('target_norm_value must be a float or a list of floats with length equal to the depth along the dimension to be normalized.')
                if isinstance(target_norm_value, float):
                    initial_norm = depth * [target_norm_value]
            else:
                initial_norm = target_norm_value
        target_norm = tf.contrib.framework.model_variable(name='weights',
          dtype=(tf.float32),
...
def batch_position_sensitive_crop_regions(images, boxes, crop_size, num_spatial_bins, global_pool, parallel_iterations=64):
  ...
  """
    ...
    def _position_sensitive_crop_fn(inputs):
        images, boxes = inputs
        return position_sensitive_crop_regions(images,
          boxes,
          crop_size=crop_size,
          num_spatial_bins=num_spatial_bins,
          global_pool=global_pool)

    return shape_utils.static_or_dynamic_map_fn(_position_sensitive_crop_fn,
      elems=[
     images, boxes],
      dtype=(tf.float32),
      parallel_iterations=parallel_iterations)
...
def position_sensitive_crop_regions(image, boxes, crop_size, num_spatial_bins, global_pool):
  ...
  """
    ...
    for split, box in zip(image_splits, position_sensitive_boxes):
        if split.shape.is_fully_defined() and box.shape.is_fully_defined():
            crop = tf.squeeze((matmul_crop_and_resize(tf.expand_dims(split, axis=0), tf.expand_dims(box, axis=0), bin_crop_size)),
              axis=0)
        else:
            crop = tf.image.crop_and_resize(tf.expand_dims(split, 0), box, tf.zeros((tf.shape(boxes)[0]), dtype=(tf.int32)), bin_crop_size)
        image_crops.append(crop)

    if global_pool:
        position_sensitive_features = tf.add_n(image_crops) / len(image_crops)
        position_sensitive_features = tf.reduce_mean(position_sensitive_features,
          [1, 2], keepdims=True)
    else:
        block_size = bin_crop_size[0]
        if block_size >= 2:
            image_crops = [tf.space_to_depth(crop, block_size=block_size) for crop in image_crops]
        position_sensitive_features = tf.stack(image_crops, axis=0)
        position_sensitive_features = tf.squeeze(tf.batch_to_space_nd(position_sensitive_features, block_shape=([
         1] + num_spatial_bins),
          crops=tf.zeros((3, 2), dtype=(tf.int32))),
          axis=[
         0])
        if block_size >= 2:
            position_sensitive_features = tf.depth_to_space(position_sensitive_features,
  ...
  """
    ...
    def reframe_box_masks_to_image_masks_default():
        ...
        unit_boxes = tf.concat([
         tf.zeros([num_boxes, 2]), tf.ones([num_boxes, 2])],
          axis=1)
    ...
    image_masks = tf.cond(tf.shape(box_masks)[0] > 0, reframe_box_masks_to_image_masks_default, lambda : tf.zeros([0, image_height, image_width, 1], dtype=(tf.float32)))
    return tf.squeeze(image_masks, axis=3)


def merge_boxes_with_multiple_labels(boxes, classes, confidences, num_classes, quantization_bins=10000):
  ...
  """
    ...
    box_class_shape_assert = shape_utils.assert_shape_equal_along_first_dimension(boxes_shape, classes_shape)
    box_confidence_shape_assert = shape_utils.assert_shape_equal_along_first_dimension(boxes_shape, confidences_shape)
    ...
    with tf.control_dependencies([
     ...
     box_dimension_assert, box_normalized_assert]):
        ...
        hashcodes = ymin + xmin * quantization_bins + ymax * quantization_bins * quantization_bins + xmax * quantization_bins * quantization_bins * quantization_bins
        unique_hashcodes, unique_indices = tf.unique(hashcodes)
        num_boxes = tf.shape(boxes)[0]
        num_unique_boxes = tf.shape(unique_hashcodes)[0]
        merged_box_indices = tf.unsorted_segment_min(tf.range(num_boxes), unique_indices, num_unique_boxes)
        ...
        def map_box_encodings(i):
            ...
            box_class_encodings = tf.sparse_to_dense(box_indices,
              [num_classes], tf.constant(1, dtype=(tf.int64)), validate_indices=False)
            box_confidence_encodings = tf.sparse_to_dense(box_indices,
              [num_classes], box_confidences, validate_indices=False)
            return (box_class_encodings, box_confidence_encodings)

        class_encodings, confidence_encodings = tf.map_fn(map_box_encodings,
...
def nearest_neighbor_upsampling(input_tensor, scale=None, height_scale=None, width_scale=None):
  ...
  """
    if not scale:
        if height_scale is None or width_scale is None:
            raise ValueError('Provide either `scale` or `height_scale` and `width_scale`.')
    with tf.name_scope('nearest_neighbor_upsampling'):
        h_scale = scale if height_scale is None else height_scale
        w_scale = scale if width_scale is None else width_scale
        batch_size, height, width, channels = shape_utils.combined_static_and_dynamic_shape(input_tensor)
...
def fpn_feature_levels(num_levels, unit_scale_index, image_ratio, boxes):
  ...
  """
    assert num_levels > 0, '`num_levels` must be > 0. Found {}'.format(num_levels)
    if not (unit_scale_index < num_levels and unit_scale_index >= 0):
        raise AssertionError('`unit_scale_index` must be in [0, {}). Found {}.'.format(num_levels, unit_scale_index))
    ...
    levels = tf.cast((tf.floordiv(tf.log(areas_sqrt * image_ratio), log_2) + unit_scale_index),
      dtype=(tf.int32))
...
def bfloat16_to_float32_nested(tensor_nested):
  ...
  """
    if isinstance(tensor_nested, tf.Tensor):
        ...
        if isinstance(tensor_nested, (list, tuple)):
            out_tensor_dict = [bfloat16_to_float32_nested(t) for t in tensor_nested]
        else:
            if isinstance(tensor_nested, dict):
                out_tensor_dict = {k:bfloat16_to_float32_nested(v) for k, v in tensor_nested.items()}
