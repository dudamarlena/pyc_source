...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/predictors/convolutional_box_predictor.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 18273 bytes
                ...
                with slim.arg_scope(self._conv_hyperparams_fn()):
                    with slim.arg_scope([slim.dropout], is_training=(self._is_training)):
                        ...
                        for head_name in sorted_keys:
                            ...
                            prediction = head_obj.predict(features=net,
...
class WeightSharedConvolutionalBoxPredictor(box_predictor.BoxPredictor):
    ...
    def _insert_additional_projection_layer(self, image_feature, inserted_layer_counter, target_channel):
        ...
        image_feature = slim.conv2d(image_feature,
          ...
          scope=('ProjectionLayer/conv2d_{}'.format(inserted_layer_counter)))
        if self._apply_batch_norm:
            image_feature = slim.batch_norm(image_feature,
              scope=('ProjectionLayer/conv2d_{}/BatchNorm'.format(inserted_layer_counter)))
    ...
    def _compute_base_tower(self, tower_name_scope, image_feature, feature_index):
        ...
        for i in range(self._num_layers_before_predictor):
            ...
            net = conv_op(net,
              (self._depth),
              [self._kernel_size, self._kernel_size], stride=1,
              padding='SAME',
              activation_fn=None,
              normalizer_fn=(tf.identity if self._apply_batch_norm else None),
            ...
            if self._apply_batch_norm:
                net = slim.batch_norm(net,
                  scope=('{}/conv2d_{}/BatchNorm/feature_{}'.format(tower_name_scope, i, feature_index)))
            net = tf.nn.relu6(net)
    ...
    def _predict_head(self, head_name, head_obj, image_feature, box_tower_feature, feature_index, num_predictions_per_location):
        ...
        if self._share_prediction_tower:
            ...
            head_tower_feature = self._compute_base_tower(tower_name_scope=tower_name_scope,
              image_feature=image_feature,
              feature_index=feature_index)
        return head_obj.predict(features=head_tower_feature,
          num_predictions_per_location=num_predictions_per_location)
            ...
            with tf.variable_scope('WeightSharedConvolutionalBoxPredictor', reuse=(tf.AUTO_REUSE)):
                with slim.arg_scope(self._conv_hyperparams_fn()):
                    with slim.arg_scope([slim.dropout], is_training=(self._is_training)):
                        ...
                        box_tower_feature = self._compute_base_tower(tower_name_scope=box_tower_scope,
                          image_feature=image_feature,
                          feature_index=feature_index)
                        box_encodings = self._box_prediction_head.predict(features=box_tower_feature,
                          num_predictions_per_location=num_predictions_per_location)
                        ...
                        for head_name in sorted_keys:
                            if head_name == CLASS_PREDICTIONS_WITH_BACKGROUND:
                                ...
                                head_obj = self._other_heads[head_name]
                            prediction = self._predict_head(head_name=head_name,
                              head_obj=head_obj,
                              image_feature=image_feature,
