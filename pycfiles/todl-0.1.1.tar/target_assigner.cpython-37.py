...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/core/target_assigner.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 32989 bytes
...
import object_detection.core as mat
import object_detection.core as sim_calc
...
class TargetAssigner(object):
    __doc__ = 'Target assigner to compute classification and regression targets.'

    def __init__(self, similarity_calc, matcher, box_coder_instance, negative_class_weight=1.0):
    ...
    """
        if not isinstance(similarity_calc, sim_calc.RegionSimilarityCalculator):
            ...
            assert isinstance(box_coder_instance, box_coder.BoxCoder), 'box_coder must be a BoxCoder'
    ...
    def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None, unmatched_class_label=None, groundtruth_weights=None):
    ...
    """
        ...
        unmatched_shape_assert = shape_utils.assert_shape_equal(shape_utils.combined_static_and_dynamic_shape(groundtruth_labels)[1:], shape_utils.combined_static_and_dynamic_shape(unmatched_class_label))
        ...
        with tf.control_dependencies([
         unmatched_shape_assert, labels_and_box_shapes_assert]):
            ...
            match = self._matcher.match(match_quality_matrix, valid_rows=(tf.greater(groundtruth_weights, 0)))
            reg_targets = self._create_regression_targets(anchors, groundtruth_boxes, match)
            cls_targets = self._create_classification_targets(groundtruth_labels, unmatched_class_label, match)
    ...
    """
        ...
        if groundtruth_boxes.has_field(fields.BoxListFields.keypoints):
            groundtruth_keypoints = groundtruth_boxes.get_field(fields.BoxListFields.keypoints)
            matched_keypoints = match.gather_based_on_match(groundtruth_keypoints,
              unmatched_value=(tf.zeros(groundtruth_keypoints.get_shape()[1:])),
              ignored_value=(tf.zeros(groundtruth_keypoints.get_shape()[1:])))
            matched_gt_boxlist.add_field(fields.BoxListFields.keypoints, matched_keypoints)
    ...
    """
        return match.gather_based_on_match(groundtruth_weights,
          ignored_value=0.0, unmatched_value=0.0)
...
def create_target_assigner(reference, stage=None, negative_class_weight=1.0, use_matmul_gather=False):
  ...
  """
    if reference == 'Multibox' and stage == 'proposal':
        ...
        if reference == 'FasterRCNN' and stage == 'proposal':
            ...
            matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=0.7, unmatched_threshold=0.3,
              force_match_for_each_row=True,
              use_matmul_gather=use_matmul_gather)
            box_coder_instance = faster_rcnn_box_coder.FasterRcnnBoxCoder(scale_factors=[
             10.0, 10.0, 5.0, 5.0])
        else:
            if reference == 'FasterRCNN' and stage == 'detection':
                similarity_calc = sim_calc.IouSimilarity()
                matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=0.5, negatives_lower_than_unmatched=True,
                  use_matmul_gather=use_matmul_gather)
                box_coder_instance = faster_rcnn_box_coder.FasterRcnnBoxCoder(scale_factors=[
                 10.0, 10.0, 5.0, 5.0])
            else:
                if reference == 'FastRCNN':
                    similarity_calc = sim_calc.IouSimilarity()
                    matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=0.5, unmatched_threshold=0.1,
    ...
    return TargetAssigner(similarity_calc, matcher, box_coder_instance, negative_class_weight=negative_class_weight)


def batch_assign(target_assigner, anchors_batch, gt_box_batch, gt_class_targets_batch, unmatched_class_label=None, gt_weights_batch=None):
  ...
  """
    ...
    if not len(anchors_batch) == len(gt_box_batch) == len(gt_class_targets_batch):
        raise ValueError('batch size incompatible with lengths of anchors_batch, gt_box_batch and gt_class_targets_batch.')
    cls_targets_list = []
    cls_weights_list = []
    reg_targets_list = []
    reg_weights_list = []
    ...
    if gt_weights_batch is None:
        gt_weights_batch = [
         None] * len(gt_class_targets_batch)
    for anchors, gt_boxes, gt_class_targets, gt_weights in zip(anchors_batch, gt_box_batch, gt_class_targets_batch, gt_weights_batch):
        cls_targets, cls_weights, reg_targets, reg_weights, match = target_assigner.assign(anchors, gt_boxes, gt_class_targets, unmatched_class_label, gt_weights)
...
def batch_get_targets(batch_match, groundtruth_tensor_list, groundtruth_weights_list, unmatched_value, unmatched_weight):
  ...
  """
    ...
    targets_list = []
    weights_list = []
    for match_tensor, groundtruth_tensor, groundtruth_weight in zip(match_list, groundtruth_tensor_list, groundtruth_weights_list):
        match_object = mat.Match(match_tensor)
        targets = match_object.gather_based_on_match(groundtruth_tensor,
          unmatched_value=unmatched_value,
          ignored_value=unmatched_value)
        targets_list.append(targets)
        weights = match_object.gather_based_on_match(groundtruth_weight,
          unmatched_value=unmatched_weight,
          ignored_value=(tf.zeros_like(unmatched_weight)))
        weights_list.append(weights)

    return (
     tf.stack(targets_list), tf.stack(weights_list))


def batch_assign_confidences(target_assigner, anchors_batch, gt_box_batch, gt_class_confidences_batch, gt_weights_batch=None, unmatched_class_label=None, include_background_class=True, implicit_class_weight=1.0):
    """Batched assignment of classification and regression targets.
  ...
  """
    ...
    if not all((isinstance(anchors, box_list.BoxList) for anchors in anchors_batch)):
        raise ValueError('anchors_batch must be a BoxList or list of BoxLists.')
    if not len(anchors_batch) == len(gt_box_batch) == len(gt_class_confidences_batch):
        raise ValueError('batch size incompatible with lengths of anchors_batch, gt_box_batch and gt_class_confidences_batch.')
    cls_targets_list = []
    cls_weights_list = []
    reg_targets_list = []
    reg_weights_list = []
    ...
    if gt_weights_batch is None:
        gt_weights_batch = [
         None] * len(gt_class_confidences_batch)
    for anchors, gt_boxes, gt_class_confidences, gt_weights in zip(anchors_batch, gt_box_batch, gt_class_confidences_batch, gt_weights_batch):
        if gt_class_confidences is not None:
            if len(gt_class_confidences.get_shape().as_list()) > 2:
                raise ValueError('The shape of the class target is not supported. ', gt_class_confidences.get_shape())
        else:
            cls_targets, _, reg_targets, _, match = target_assigner.assign(anchors,
              gt_boxes, gt_class_confidences, unmatched_class_label, groundtruth_weights=gt_weights)
            if include_background_class:
                cls_targets_without_background = tf.slice(cls_targets, [0, 1], [-1, -1])
            else:
        ...
        regression_targets = reg_targets * tf.expand_dims(regression_weights, axis=(-1))
        regression_weights_expanded = tf.expand_dims(regression_weights, axis=(-1))
        cls_targets_without_background = cls_targets_without_background * (1 - tf.cast(negative_mask, dtype=(tf.float32)))
        cls_weights_without_background = (1 - implicit_class_weight) * tf.cast(explicit_example_mask,
          dtype=(tf.float32)) + implicit_class_weight
        if include_background_class:
            cls_weights_background = (1 - implicit_class_weight) * regression_weights_expanded + implicit_class_weight
            classification_weights = tf.concat([
             cls_weights_background, cls_weights_without_background],
              axis=(-1))
            cls_targets_background = 1 - regression_weights_expanded
            classification_targets = tf.concat([
             cls_targets_background, cls_targets_without_background],
              axis=(-1))
        else:
