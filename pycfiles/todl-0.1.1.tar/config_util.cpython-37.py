...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/utils/config_util.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 38931 bytes
  ...
  """
    ...
    if image_resizer_config.HasField('keep_aspect_ratio_resizer'):
        if image_resizer_config.keep_aspect_ratio_resizer.pad_to_max_dimension:
            return [
             image_resizer_config.keep_aspect_ratio_resizer.max_dimension] * 2
        return [-1, -1]
...
def _is_generic_key(key):
    ...
    for prefix in ('graph_rewriter_config', 'model', 'train_input_config', 'train_config',
                   'eval_config'):
...
def _check_and_convert_legacy_input_config_key(key):
  ...
  """
    ...
    if field_name == 'train_shuffle':
        ...
        if field_name == 'eval_shuffle':
            key_name = 'eval_input_configs'
            field_name = 'shuffle'
        else:
            if field_name == 'train_input_path':
                key_name = 'train_input_config'
                field_name = 'input_path'
            else:
                if field_name == 'eval_input_path':
                    key_name = 'eval_input_configs'
                    field_name = 'input_path'
                else:
                    if field_name == 'append_train_input_path':
                        key_name = 'train_input_config'
                        field_name = 'input_path'
                    else:
                        if field_name == 'append_eval_input_path':
                            key_name = 'eval_input_configs'
                            field_name = 'input_path'
                        else:
...
def check_and_parse_input_config_key(configs, key):
  ...
  """
    ...
    elif isinstance(configs[key_name], input_reader_pb2.InputReader):
  ...
  """
    if isinstance(configs[key_name], input_reader_pb2.InputReader):
        ...
        if input_name is None:
            ...
            if input_name is not None and len(configs[key_name]):
                update_count = 0
