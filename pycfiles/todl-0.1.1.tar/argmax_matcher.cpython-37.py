...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/matchers/argmax_matcher.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 9514 bytes
...
class ArgMaxMatcher(matcher.Matcher):
    __doc__ = 'Matcher based on highest value.\n\n  This class computes matches from a similarity matrix. Each column is matched\n  to a single row.\n\n  To support object detection target assignment this class enables setting both\n  matched_threshold (upper threshold) and unmatched_threshold (lower thresholds)\n  defining three categories of similarity which define whether examples are\n  positive, negative, or ignored:\n  (1) similarity >= matched_threshold: Highest similarity. Matched/Positive!\n  (2) matched_threshold > similarity >= unmatched_threshold: Medium similarity.\n          Depending on negatives_lower_than_unmatched, this is either\n          Unmatched/Negative OR Ignore.\n  (3) unmatched_threshold > similarity: Lowest similarity. Depending on flag\n          negatives_lower_than_unmatched, either Unmatched/Negative OR Ignore.\n  For ignored matches this class sets the values in the Match object to -2.\n  '

    def __init__(self, matched_threshold, unmatched_threshold=None, negatives_lower_than_unmatched=True, force_match_for_each_row=False, use_matmul_gather=False):
    ...
    """
        ...
        if matched_threshold is None:
            if unmatched_threshold is not None:
        ...
        if not negatives_lower_than_unmatched:
            if self._unmatched_threshold == self._matched_threshold:
                raise ValueError('When negatives are in between matched and unmatched thresholds, these cannot be of equal value. matched: {}, unmatched: {}'.format(self._matched_threshold, self._unmatched_threshold))
    ...
    """
        ...
        def _match_when_rows_are_non_empty():
            ...
            if self._force_match_for_each_row:
                ...
                force_match_column_indicators = tf.one_hot(force_match_column_ids,
                  depth=(similarity_matrix_shape[1])) * tf.cast(tf.expand_dims(valid_rows, axis=(-1)), dtype=(tf.float32))
        ...
        if similarity_matrix.shape.is_fully_defined():
            ...
            return tf.cond(tf.greater(tf.shape(similarity_matrix)[0], 0), _match_when_rows_are_non_empty, _match_when_rows_are_empty)
