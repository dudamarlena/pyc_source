...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/predictors/heads/keypoint_head.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 4468 bytes
"""Keypoint Head.
...
class MaskRCNNKeypointHead(head.Head):
    __doc__ = 'Mask RCNN keypoint prediction head.\n\n  Please refer to Mask RCNN paper:\n  https://arxiv.org/abs/1703.06870\n  '
    ...
    """
        ...
        with slim.arg_scope(self._conv_hyperparams_fn()):
            net = slim.conv2d(features,
              (self._keypoint_prediction_conv_depth),
              [3, 3], scope='conv_1')
            for i in range(1, self._keypoint_prediction_num_conv_layers):
                net = slim.conv2d(net,
                  (self._keypoint_prediction_conv_depth),
                  [3, 3], scope=('conv_%d' % (i + 1)))

            net = slim.conv2d_transpose(net,
              (self._num_keypoints), [2, 2], scope='deconv1')
            heatmaps_mask = tf.image.resize_bilinear(net,
              [self._keypoint_heatmap_height, self._keypoint_heatmap_width], align_corners=True,
              name='upsample')
            return tf.expand_dims(tf.transpose(heatmaps_mask, perm=[0, 3, 1, 2]),
              axis=1,
