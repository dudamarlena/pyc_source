...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/nlp/transformer/transformer.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 22223 bytes
...
class Transformer(tf.keras.Model):
        ...
        if len(inputs) == 2:
            ...
            if self.params['padded_decode']:
                if not self.params['num_replicas']:
                    raise NotImplementedError('Padded decoding on CPU/GPUs is not supported.')
                decode_batch_size = int(self.params['decode_batch_size'] / self.params['num_replicas'])
                inputs.set_shape([
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length, training):
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            if self.params['padded_decode']:
                ...
                self_attention_bias = tf.slice(decoder_self_attention_bias, [0, 0, i, 0], [
                 bias_shape[0], bias_shape[1], 1, bias_shape[3]])
            else:
                decoder_input += timing_signal[i:i + 1]
                self_attention_bias = decoder_self_attention_bias[:, :, i:i + 1, :i + 1]
            decoder_outputs = self.decoder_stack(decoder_input,
    ...
    def predict(self, encoder_outputs, encoder_decoder_attention_bias, training):
        ...
        encoder_decoder_attention_bias = tf.cast(encoder_decoder_attention_bias, self.params['dtype'])
        symbols_to_logits_fn = self._get_symbols_to_logits_fn(max_decode_length, training)
        initial_ids = tf.zeros([batch_size], dtype=(tf.int32))
        init_decode_length = max_decode_length if self.params['padded_decode'] else 0
        num_heads = self.params['num_heads']
        dim_per_head = self.params['hidden_size'] // num_heads
        cache = {'layer_%d' % layer:{'k':tf.zeros([batch_size, init_decode_length, num_heads, dim_per_head], dtype=self.params['dtype']),  'v':tf.zeros([batch_size, init_decode_length, num_heads, dim_per_head], dtype=self.params['dtype'])} for layer in range(self.params['num_hidden_layers'])}
        ...
        top_scores = scores[:, 0]
        return {'outputs':top_decoded_ids, 
         'scores':top_scores}


class PrePostProcessingWrapper(tf.keras.layers.Layer):
    __doc__ = 'Wrapper class that applies layer pre-processing and post-processing.'
...
class EncoderStack(tf.keras.layers.Layer):
    __doc__ = 'Transformer encoder stack.\n\n  The encoder stack is made up of N identical layers. Each layer is composed\n  of the sublayers:\n    1. Self-attention layer\n    2. Feedforward network (which is 2 fully-connected layers)\n  '
    ...
    def build(self, input_shape):
        ...
        for _ in range(params['num_hidden_layers']):
            self_attention_layer = attention_layer.SelfAttention(params['hidden_size'], params['num_heads'], params['attention_dropout'])
            feed_forward_network = ffn_layer.FeedForwardNetwork(params['hidden_size'], params['filter_size'], params['relu_dropout'])
            self.layers.append([
             PrePostProcessingWrapper(self_attention_layer, params),
             PrePostProcessingWrapper(feed_forward_network, params)])

        self.output_normalization = tf.keras.layers.LayerNormalization(epsilon=1e-06,
          dtype='float32')
...
class DecoderStack(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for _ in range(params['num_hidden_layers']):
            self_attention_layer = attention_layer.SelfAttention(params['hidden_size'], params['num_heads'], params['attention_dropout'])
            enc_dec_attention_layer = attention_layer.Attention(params['hidden_size'], params['num_heads'], params['attention_dropout'])
            feed_forward_network = ffn_layer.FeedForwardNetwork(params['hidden_size'], params['filter_size'], params['relu_dropout'])
            self.layers.append([
             PrePostProcessingWrapper(self_attention_layer, params),
    ...
    def call(self, decoder_inputs, encoder_outputs, decoder_self_attention_bias, attention_bias, training, cache=None, decode_loop_step=None):
    ...
    """
        for n, layer in enumerate(self.layers):
            ...
            with tf.name_scope(layer_name):
                with tf.name_scope('self_attention'):
                    decoder_inputs = self_attention_layer(decoder_inputs,
                      decoder_self_attention_bias,
                      training=training,
                      cache=layer_cache,
                      decode_loop_step=decode_loop_step)
                with tf.name_scope('encdec_attention'):
                    decoder_inputs = enc_dec_attention_layer(decoder_inputs,
                      encoder_outputs,
                      attention_bias,
                      training=training)
                with tf.name_scope('ffn'):
