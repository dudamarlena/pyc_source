...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/anchor_generators/grid_anchor_generator.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 9202 bytes
...
class GridAnchorGenerator(anchor_generator.AnchorGenerator):
    __doc__ = 'Generates a grid of anchors at given scales and aspect ratios.'
    ...
    """
        return [
         len(self._scales) * len(self._aspect_ratios)]
    ...
    """
        ...
        with tf.init_scope():
            self._base_anchor_size = tf.cast((tf.convert_to_tensor(self._base_anchor_size)),
              dtype=(tf.float32))
            self._anchor_stride = tf.cast((tf.convert_to_tensor(self._anchor_stride)),
              dtype=(tf.float32))
            self._anchor_offset = tf.cast((tf.convert_to_tensor(self._anchor_offset)),
              dtype=(tf.float32))
