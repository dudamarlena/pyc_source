...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/anchor_generators/multiple_grid_anchor_generator.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 16677 bytes
...
class MultipleGridAnchorGenerator(anchor_generator.AnchorGenerator):
    __doc__ = 'Generate a grid of anchors for multiple CNN layers.'
    ...
    """
        if isinstance(box_specs_list, list):
            if all([isinstance(list_item, list) for list_item in box_specs_list]):
    ...
    Raises:
      ...
      ValueError: if feature_map_shape_list does not consist of pairs of
        integers
    """
        if isinstance(feature_map_shape_list, list):
            raise len(feature_map_shape_list) == len(self._box_specs) or ValueError('feature_map_shape_list must be a list with the same length as self._box_specs')
        else:
            ...
            for arg, arg_name in zip([anchor_strides, anchor_offsets], ['anchor_strides', 'anchor_offsets']):
                if isinstance(arg, list):
                    if not len(arg) == len(self._box_specs):
                        raise ValueError('%s must be a list with the same length as self._box_specs' % arg_name)
                    assert all([isinstance(list_item, tuple) and len(list_item) == 2 for list_item in arg]), '%s must be a list of pairs.' % arg_name
            ...
            if not tf.contrib.framework.is_tensor(self._base_anchor_size):
                base_anchor_size = [
                 scale_height * tf.constant((self._base_anchor_size[0]), dtype=(tf.float32)),
                 scale_width * tf.constant((self._base_anchor_size[1]), dtype=(tf.float32))]
            else:
        ...
        for feature_map_index, (grid_size, scales, aspect_ratios, stride, offset) in enumerate(zip(feature_map_shape_list, self._scales, self._aspect_ratios, anchor_strides, anchor_offsets)):
            tiled_anchors = grid_anchor_generator.tile_anchors(grid_height=(grid_size[0]),
            ...
            if self._clip_window is not None:
                tiled_anchors = box_list_ops.clip_to_window(tiled_anchors,
                  (self._clip_window), filter_nonoverlapping=False)
...
def create_ssd_anchors(num_layers=6, min_scale=0.2, max_scale=0.95, scales=None, aspect_ratios=(1.0, 2.0, 3.0, 0.5, 0.3333333333333333), interpolated_scale_aspect_ratio=1.0, base_anchor_size=None, anchor_strides=None, anchor_offsets=None, reduce_boxes_in_lowest_layer=True):
  ...
  """
    if base_anchor_size is None:
        ...
        if not scales is None:
            scales = scales or [min_scale + (max_scale - min_scale) * i / (num_layers - 1) for i in range(num_layers)] + [1.0]
        else:
            pass
        scales += [1.0]
