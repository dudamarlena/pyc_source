...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/export_tflite_ssd_graph_lib.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 13963 bytes
"""Exports an SSD detection model to use with tf-lite.

See export_tflite_ssd_graph.py for usage.
"""
import os, tempfile, numpy as np, tensorflow as tf
...
def get_const_center_size_encoded_anchors(anchors):
  ...
  """
    ...
    encoded_anchors = tf.constant((np.transpose(np.stack((y_out, x_out, h_out, w_out)))),
      dtype=(tf.float32),
      shape=[
     num_anchors[0], _DEFAULT_NUM_COORD_BOX],
      name='anchors')
...
def append_postprocessing_op(frozen_graph_def, max_detections, max_classes_per_detection, nms_score_threshold, nms_iou_threshold, num_classes, scale_values, detections_per_class=100, use_regular_nms=False, additional_output_tensors=()):
  ...
  """
    ...
    input_names = []
    output_names = [
     'TFLite_Detection_PostProcess'] + list(additional_output_tensors)
  ...
  """
    ...
    if pipeline_config.model.WhichOneof('model') != 'ssd':
        ...
        image = tf.placeholder((tf.float32),
          shape=shape, name='normalized_input_image_tensor')
        ...
        tf.identity((get_const_center_size_encoded_anchors(predicted_tensors['anchors'])),
          name='anchors')
        ...
        frozen_graph_def = exporter.freeze_graph_with_def_protos(input_graph_def=(tf.get_default_graph().as_graph_def()),
          input_saver_def=input_saver_def,
          input_checkpoint=checkpoint_to_use,
          output_node_names=(','.join([
         'raw_outputs/box_encodings', 'raw_outputs/class_predictions',
         'anchors'] + list(additional_output_tensors))),
        ...
        if add_postprocessing_op:
            transformed_graph_def = append_postprocessing_op(frozen_graph_def,
              max_detections,
