...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/core/losses.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 29846 bytes
...
import abc, six, tensorflow as tf
...
class WeightedL2LocalizationLoss(Loss):
    __doc__ = 'L2 localization loss function with anchorwise output support.\n\n  Loss[b,a] = .5 * ||weights[b,a] * (prediction[b,a,:] - target[b,a,:])||^2\n  '
...
class WeightedSmoothL1LocalizationLoss(Loss):
    ...
    Returns:
      loss: a float tensor of shape [batch_size, num_anchors] tensor
        representing the value of the loss function.
    """
        return tf.reduce_sum(tf.losses.huber_loss(target_tensor,
          prediction_tensor,
          delta=(self._delta),
...
class WeightedSigmoidClassificationLoss(Loss):
    __doc__ = 'Sigmoid cross entropy classification loss function.'
    ...
    """
        if class_indices is not None:
            weights *= tf.reshape(ops.indices_to_dense_vector(class_indices, tf.shape(prediction_tensor)[2]), [
             1, 1, -1])
        per_entry_cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(labels=target_tensor,
          logits=prediction_tensor)
        return per_entry_cross_ent * weights


class SigmoidFocalClassificationLoss(Loss):
    __doc__ = 'Sigmoid focal cross entropy loss.\n\n  Focal loss down-weights well classified examples and focusses on the hard\n  examples. See https://arxiv.org/pdf/1708.02002.pdf for the loss definition.\n  '
...
class WeightedSoftmaxClassificationLoss(Loss):
    ...
    """
        ...
        prediction_tensor = tf.divide(prediction_tensor,
          (self._logit_scale), name='scale_logit')
        per_row_cross_ent = tf.nn.softmax_cross_entropy_with_logits(labels=(tf.reshape(target_tensor, [-1, num_classes])),
          logits=(tf.reshape(prediction_tensor, [-1, num_classes])))
        return tf.reshape(per_row_cross_ent, tf.shape(weights)) * weights


class WeightedSoftmaxClassificationAgainstLogitsLoss(Loss):
    __doc__ = 'Softmax loss function against logits.\n\n   Targets are expected to be provided in logits space instead of "one hot" or\n   "probability distribution" space.\n  '
...
class BootstrappedSigmoidClassificationLoss(Loss):
    ...
    """
        ...
        per_entry_cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(labels=bootstrap_target_tensor,
          logits=prediction_tensor)
        return per_entry_cross_ent * weights


class HardExampleMiner(object):
    __doc__ = 'Hard example mining for regions in a list of images.\n\n  Implements hard example mining to select a subset of regions to be\n  back-propagated. For each image, selects the regions with highest losses,\n  subject to the condition that a newly selected region cannot have\n  an IOU > iou_threshold with any of the previously selected regions.\n  This can be achieved by re-using a greedy non-maximum suppression algorithm.\n  A constraint on the number of negatives mined per positive region can also be\n  enforced.\n\n  Reference papers: "Training Region-based Object Detectors with Online\n  Hard Example Mining" (CVPR 2016) by Srivastava et al., and\n  "SSD: Single Shot MultiBox Detector" (ECCV 2016) by Liu et al.\n  '
    ...
    """
        ...
        if match:
            if self._max_negatives_per_positive:
                self._num_positives_list = num_positives_list
                self._num_negatives_list = num_negatives_list
        return (
         location_loss, cls_loss)

    def summarize(self):
        """Summarize the number of positives and negatives after mining."""
        if self._num_positives_list:
            if self._num_negatives_list:
                avg_num_positives = tf.reduce_mean(tf.cast((self._num_positives_list), dtype=(tf.float32)))
                avg_num_negatives = tf.reduce_mean(tf.cast((self._num_negatives_list), dtype=(tf.float32)))
    ...
    """
        ...
        max_negatives = tf.maximum(min_negatives_per_image, tf.cast((max_negatives_per_positive * tf.cast(num_positives, dtype=(tf.float32))),
          dtype=(tf.int32)))
        topk_negatives_indicator = tf.less_equal(tf.cumsum(tf.cast(negatives_indicator, dtype=(tf.int32))), max_negatives)
