...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/seq2species/run_training.py
...
import build_model, configuration, input as seq2species_input
...
def run_training(model, hparams, training_dataset, logdir, batch_size):
  ...
  """
    ...
    scaffold = tf.train.Scaffold(saver=tf.train.Saver((tf.global_variables()),
      max_to_keep=5,
    ...
    with tf.train.MonitoredTrainingSession(master=(FLAGS.master),
      ...
      max_wait_secs=(FLAGS.recovery_wait_secs)) as (sess):
        ...
        while not sess.should_stop():
            if global_step < hparams.train_steps:
                accuracy, weighted_accuracy, loss, global_step, _ = sess.run(ops)

                def gather_measures():
                    new_measures = {'train_loss':loss, 
                     'global_step':global_step}
                    for target in FLAGS.targets:
                        new_measures.update({'train_accuracy/%s' % target: accuracy[target], 
                         'train_weighted_accuracy/%s' % target: weighted_accuracy[target]})

                    update_measures(measures,
                      new_measures, loss, max_loss=(FLAGS.max_task_loss))
                ...
                if np.isfinite(loss):
                    if not loss >= FLAGS.max_task_loss or global_step > FLAGS.min_train_steps:
                        break
...
def write_measures(measures, checkpoint_file, init_time):
  ...
  """
    ...
    experiment_measures = seq2label_pb2.Seq2LabelExperimentMeasures(checkpoint_path=checkpoint_file,
...
def main(unused_argv):
    ...
    input_dataset = seq2species_input.InputDataset.from_tfrecord_files((FLAGS.train_files),
      'train',
      (FLAGS.targets),
      dataset_info,
    ...
    with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
        model = build_model.ConvolutionalNet(hparams,
          dataset_info, targets=(FLAGS.targets))
    measures, checkpoint_file = (None, None)
    print('Starting model training.')
    for cur_measures, cur_file in run_training(model,
      hparams, input_dataset, (FLAGS.logdir), batch_size=(FLAGS.batch_size)):
