...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/core/post_processing.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 58322 bytes
  ...
  """
    with tf.name_scope('BatchIOU'):
        y1_min, x1_min, y1_max, x1_max = tf.split(value=boxes1,
          ...
          axis=2)
        y2_min, x2_min, y2_max, x2_max = tf.split(value=boxes2,
          num_or_size_splits=4,
          axis=2)
...
def _suppression_loop_body(boxes, iou_threshold, output_size, idx):
  ...
  """
    ...
    box_slice = tf.slice(boxes, [0, idx * _NMS_TILE_SIZE, 0], [
     1, _NMS_TILE_SIZE, 4])
    _, box_slice, _, _ = tf.while_loop(lambda _boxes, _box_slice, _threshold, inner_idx: inner_idx < idx, _cross_suppression, [boxes, box_slice, iou_threshold,
     tf.constant(0)])
    ...
    suppressed_iou, _, _, _ = tf.while_loop(lambda _iou, _threshold, loop_condition, _iou_sum: loop_condition, _self_suppression, [
     iou, iou_threshold,
     tf.constant(True),
    ...
    boxes = tf.tile(tf.expand_dims(box_slice, [1]), [
     1, num_tiles, 1, 1]) * mask + tf.reshape(boxes, [1, num_tiles, _NMS_TILE_SIZE, 4]) * (1 - mask)
...
def partitioned_non_max_suppression_padded(boxes, scores, max_output_size, iou_threshold=0.5, score_threshold=float('-inf')):
  ...
  """
    ...
    num_boxes = tf.shape(boxes)[0]
    num_boxes += pad
    boxes = tf.pad((tf.cast(boxes, tf.float32)),
      [[0, pad], [0, 0]], constant_values=(-1))
    scores = tf.pad(tf.cast(scores, tf.float32), [[0, pad]])
    scores_mask = tf.expand_dims((tf.cast(scores > score_threshold, boxes.dtype)),
      axis=1)
...
def _validate_boxes_scores_iou_thresh(boxes, scores, iou_thresh, change_coordinate_frame, clip_window):
  ...
  """
    if not 0 <= iou_thresh <= 1.0:
        ...
        elif shape_utils.get_dim_as_int(scores.shape[1]) is None:
            raise ValueError('scores must have statically defined second dimension')
        else:
            if boxes.shape.ndims != 3:
                raise ValueError('boxes must be of rank 3.')
            assert shape_utils.get_dim_as_int(boxes.shape[1]) == shape_utils.get_dim_as_int(scores.shape[1]) or shape_utils.get_dim_as_int(boxes.shape[1]) == 1, 'second dimension of boxes must be either 1 or equal to the second dimension of scores'
...
def _clip_window_prune_boxes(sorted_boxes, clip_window, pad_to_max_output_size, change_coordinate_frame):
  ...
  """
    sorted_boxes = box_list_ops.clip_to_window(sorted_boxes,
      clip_window,
    ...
    return (
     sorted_boxes, num_valid_nms_boxes_cumulative)
        ...
        for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
            ...
            if pad_to_max_output_size:
                ...
                if use_partitioned_nms:
                    selected_indices, num_valid_nms_boxes, boxlist_and_class_scores.data['boxes'], boxlist_and_class_scores.data['scores'], _ = partitioned_non_max_suppression_padded((boxlist_and_class_scores.get()),
                      (boxlist_and_class_scores.get_field(fields.BoxListFields.scores)),
                      max_selection_size,
                      iou_threshold=iou_thresh,
                      score_threshold=score_thresh)
                else:
                    selected_indices, num_valid_nms_boxes = tf.image.non_max_suppression_padded((boxlist_and_class_scores.get()),
                      (boxlist_and_class_scores.get_field(fields.BoxListFields.scores)),
                      max_selection_size,
                      iou_threshold=iou_thresh,
                      score_threshold=score_thresh,
                ...
                if hasattr(tf.image, 'non_max_suppression_with_scores') and tf.compat.forward_compatible(2019, 6, 6):
                    selected_indices, selected_scores = tf.image.non_max_suppression_with_scores((boxlist_and_class_scores.get()),
                      (boxlist_and_class_scores.get_field(fields.BoxListFields.scores)),
                      max_selection_size,
                      iou_threshold=iou_thresh,
                      score_threshold=score_thresh,
                    ...
                    selected_indices = tf.image.non_max_suppression((boxlist_and_class_scores.get()),
                      (boxlist_and_class_scores.get_field(fields.BoxListFields.scores)),
                      max_selection_size,
                      iou_threshold=iou_thresh,
                      score_threshold=score_thresh)
...
def class_agnostic_non_max_suppression(boxes, scores, score_thresh, iou_thresh, max_classes_per_detection=1, max_total_size=0, clip_window=None, change_coordinate_frame=False, masks=None, boundaries=None, pad_to_max_output_size=False, use_partitioned_nms=False, additional_fields=None, soft_nms_sigma=0.0, scope=None):
  ...
  """
    ...
    if q > 1:
        class_ids = tf.expand_dims(tf.argmax(scores, axis=1, output_type=(tf.int32)),
          axis=1)
    ...
    with tf.name_scope(scope, 'ClassAgnosticNonMaxSuppression'):
        ...
        if pad_to_max_output_size:
            ...
            if use_partitioned_nms:
                selected_indices, num_valid_nms_boxes, boxlist_and_class_scores.data['boxes'], boxlist_and_class_scores.data['scores'], argsort_ids = partitioned_non_max_suppression_padded((boxlist_and_class_scores.get()),
                  (boxlist_and_class_scores.get_field(fields.BoxListFields.scores)),
                  max_selection_size,
                  iou_threshold=iou_thresh,
                  score_threshold=score_thresh)
            ...
            if hasattr(tf.image, 'non_max_suppression_with_scores'):
                if tf.compat.forward_compatible(2019, 6, 6):
                    selected_indices, selected_scores = tf.image.non_max_suppression_with_scores((boxlist_and_class_scores.get()),
                      (boxlist_and_class_scores.get_field(fields.BoxListFields.scores)),
                      max_selection_size,
                      iou_threshold=iou_thresh,
                ...
                sorted_boxes = pad_to_max_output_size or box_list_ops.gather(sorted_boxes, tf.range(num_valid_nms_boxes))
  ...
  """
    if use_combined_nms:
        ...
        with tf.name_scope(scope, 'CombinedNonMaxSuppression'):
            batch_nmsed_boxes, batch_nmsed_scores, batch_nmsed_classes, batch_num_detections = tf.image.combined_non_max_suppression(boxes=boxes,
              scores=scores,
    ...
    with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
        ...
        if batch_size is None:
            ...
            def _single_image_nms_fn(args):
                ...
                if use_static_shapes:
                    ...
                    per_image_scores = tf.reshape(tf.slice(per_image_scores, [0, 0], tf.stack([per_image_num_valid_boxes, -1])), [
                     -1, num_classes])
                    per_image_masks = tf.reshape(tf.slice(per_image_masks, 4 * [0], tf.stack([per_image_num_valid_boxes, -1, -1, -1])), [
                    ...
                    if per_image_additional_fields is not None:
                        ...
                        nmsed_boxlist, num_valid_nms_boxes = multiclass_non_max_suppression(per_image_boxes,
                          per_image_scores,
                          score_thresh,
                          iou_thresh,
                          max_size_per_class,
        ...
        batch_outputs = map_fn(_single_image_nms_fn,
          elems=([
         boxes, scores, masks, clip_window] + list(ordered_additional_fields.values()) + [num_valid_boxes]),
