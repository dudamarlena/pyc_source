...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/seq2species/input.py
...
class _InputEncoding(object):
    ...
    def __init__(self, dataset_info, mode, targets, noise_rate=0.0, fixed_read_length=None):
        ...
        self.character_encodings = np.concatenate([[self._character_to_base_distribution(char)] for char in self.all_characters],
          axis=0)
        ...
        with tf.variable_scope('parse_single_tfexample'):
            ...
            if self.noise_rate > 0.0:
                read_sequence = tf.py_func(seq2label_utils.add_read_noise, [
                 features['sequence'], self.noise_rate], tf.string)
            else:
...
class InputDataset(object):
    ...
    """
        ...
        encoding = _InputEncoding((self.dataset_info),
          (self.mode),
          (self.targets),
        ...
        dataset = dataset.map((encoding.parse_single_tfexample),
          num_parallel_calls=(self._num_parallel_calls))
        dataset = dataset.repeat(self._train_epochs if self.is_train else 1)
        if randomize_input:
            dataset = dataset.shuffle(buffer_size=(max(1000, batch_size)),
              seed=(self._random_seed))
        elif self._ensure_constant_batch_size:
