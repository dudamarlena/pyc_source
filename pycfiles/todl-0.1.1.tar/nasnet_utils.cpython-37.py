...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/slim/nets/nasnet/nasnet_utils.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 20826 bytes
...
def factorized_reduction(net, output_filters, stride, data_format=INVALID):
    ...
    if not data_format != INVALID:
        ...
        if data_format == 'NHWC':
            ...
            stride_spec = [
             1, 1, stride, stride]
        path1 = tf.compat.v2.nn.avg_pool2d(input=net,
          ksize=[
         1, 1, 1, 1],
          strides=stride_spec,
          padding='VALID',
        ...
        path2 = tf.pad(tensor=net, paddings=pad_arr)[:, :, 1:, 1:]
        concat_axis = 1
    path2 = tf.compat.v2.nn.avg_pool2d(input=path2,
      ksize=[
     1, 1, 1, 1],
      strides=stride_spec,
      padding='VALID',
...
def _stacked_separable_conv(net, stride, operation, filter_size, use_bounded_activation):
    ...
    net = slim.separable_conv2d(net,
      ...
      stride=stride)
    net = slim.batch_norm(net,
      scope=('bn_sep_{0}x{0}_{1}'.format(kernel_size, num_layers)))
    return net
    ...
    def _reduce_prev_layer(self, prev_layer, curr_layer):
        ...
        if prev_layer is None:
            ...
            if curr_filter_shape != prev_filter_shape:
                ...
                prev_layer = factorized_reduction(prev_layer,
                  curr_num_filters, stride=2)
            else:
                if curr_num_filters != prev_num_filters:
                    prev_layer = activation_fn(prev_layer)
                    prev_layer = slim.conv2d(prev_layer,
                      curr_num_filters, 1, scope='prev_1x1')
...
class NasNetAReductionCell(NasNetABaseCell):
    __doc__ = 'NASNetA Reduction Cell.'
