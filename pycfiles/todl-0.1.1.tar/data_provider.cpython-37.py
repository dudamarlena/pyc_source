...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/attention_ocr/python/data_provider.py
...
InputEndpoints = collections.namedtuple('InputEndpoints', ['images', 'images_orig', 'labels', 'labels_one_hot'])
ShuffleBatchConfig = collections.namedtuple('ShuffleBatchConfig', [
 'num_batching_threads', 'queue_capacity', 'min_after_dequeue'])
DEFAULT_SHUFFLE_CONFIG = ShuffleBatchConfig(num_batching_threads=8,
  queue_capacity=3000,
...
def augment_image(image):
  ...
  """
    with tf.variable_scope('AugmentImage'):
        ...
        distorted_image = inception_preprocessing.apply_with_random_selector(distorted_image,
          (lambda x, method: tf.image.resize_images(x, [height, width], method)),
          num_cases=4)
        distorted_image.set_shape([height, width, 3])
        distorted_image = inception_preprocessing.apply_with_random_selector(distorted_image,
          functools.partial((inception_preprocessing.distort_color),
          fast_mode=False),
          num_cases=4)
...
def central_crop(image, crop_size):
  ...
  """
    with tf.variable_scope('CentralCrop'):
        ...
        assert_op1 = tf.Assert(tf.greater_equal(image_height, target_height), [
         'image_height < target_height', image_height, target_height])
        assert_op2 = tf.Assert(tf.greater_equal(image_width, target_width), [
    ...
    with tf.variable_scope('PreprocessImage'):
        ...
        if augment or central_crop_size:
            ...
            if central_crop_size:
                view_crop_size = (
                 int(central_crop_size[0] / num_towers),
                 central_crop_size[1])
...
def get_data(dataset, batch_size, augment=False, central_crop_size=None, shuffle_config=None, shuffle=True):
  ...
  """
    ...
    image = preprocess_image(image_orig,
      augment, central_crop_size, num_towers=(dataset.num_of_views))
