...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/model_lib_v2.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 33277 bytes
"""Constructs model, inputs, and training environment."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import copy, time, tensorflow as tf
...
def eager_train_step(detection_model, features, labels, unpad_groundtruth_tensors, optimizer, learning_rate, add_regularization_loss=True, clip_gradients_value=None, global_step=None, num_replicas=1.0):
  ...
  """
    ...
    labels = model_lib.unstack_batch(labels,
    ...
    for loss_type in losses_dict:
        tf.compat.v2.summary.scalar(loss_type,
          (losses_dict[loss_type]), step=global_step)
...
def load_fine_tune_checkpoint(model, checkpoint_path, checkpoint_type, load_all_detection_checkpoint_vars, input_dataset, unpad_groundtruth_tensors):
  ...
  """
    ...
    strategy.experimental_run_v2(_dummy_computation_fn,
      args=(
     features,
     labels))
    var_map = model.restore_map(fine_tune_checkpoint_type=checkpoint_type,
      load_all_detection_checkpoint_vars=load_all_detection_checkpoint_vars)
    available_var_map = variables_helper.get_variables_available_in_checkpoint(var_map,
      checkpoint_path,
...
def train_loop(hparams, pipeline_config_path, model_dir, config_override=None, train_steps=None, use_tpu=False, save_final_config=False, export_to_tpu=None, checkpoint_every_n=1000, **kwargs):
  ...
  """
    ...
    kwargs.update({'train_steps':train_steps, 
     'use_bfloat16':configs['train_config'].use_bfloat16 and use_tpu})
    configs = merge_external_params_with_configs(configs,
      hparams, kwargs_dict=kwargs)
    ...
    if export_to_tpu is None:
        ...
        if (train_config.fine_tune_checkpoint_type or train_config).from_detection_checkpoint:
    ...
    with strategy.scope():
        ...
        global_step = tf.compat.v2.Variable(0,
          trainable=False, dtype=(tf.compat.v2.dtypes.int64), name='global_step')
    ...
    with summary_writer.as_default():
        with strategy.scope():
            ...
            manager = tf.compat.v2.train.CheckpointManager(ckpt,
              model_dir, max_to_keep=7)
            ckpt.restore(manager.latest_checkpoint)

            def train_step_fn(features, labels):
                return eager_train_step(detection_model,
                  features,
                  labels,
            ...
            for _ in range(train_steps - global_step.value()):
                ...
                tf.compat.v2.summary.scalar('steps_per_sec',
                  (1.0 / (end_time - start_time)), step=global_step)
  ...
  """
    ...
    def compute_eval_dict(features, labels):
        ...
        if use_tpu and postprocess_on_cpu:
            detections = tf.contrib.tpu.outside_compilation(postprocess_wrapper, (
        ...
        eval_dict = eval_util.result_dict_for_batched_example(eval_images,
        ...
        return (
         eval_dict, losses_dict, class_agnostic)
...
def eval_continuously(hparams, pipeline_config_path, config_override=None, train_steps=None, sample_1_of_n_eval_examples=1, sample_1_of_n_eval_on_train_examples=1, use_tpu=False, override_eval_num_epochs=True, postprocess_on_cpu=False, export_to_tpu=None, model_dir=None, checkpoint_dir=None, wait_interval=180, **kwargs):
  ...
  """
    ...
    if override_eval_num_epochs:
        if eval_on_train_input_config.num_epochs != 1:
    ...
    for eval_input_config in eval_input_configs:
        next_eval_input = inputs.eval_input(eval_config=eval_config,
    ...
    global_step = tf.compat.v2.Variable(0,
      trainable=False, dtype=(tf.compat.v2.dtypes.int64))
