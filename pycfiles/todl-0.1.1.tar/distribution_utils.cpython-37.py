...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/utils/misc/distribution_utils.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 11050 bytes
...
def _collective_communication(all_reduce_alg):
  ...
  """
    collective_communication_options = {None:tf.distribute.experimental.CollectiveCommunication.AUTO, 
...
def _mirrored_cross_device_ops(all_reduce_alg, num_packs):
  ...
  """
    if all_reduce_alg is None:
        return
    mirrored_all_reduce_options = {'nccl':tf.distribute.NcclAllReduce, 
...
def get_distribution_strategy(distribution_strategy='mirrored', num_gpus=0, all_reduce_alg=None, num_packs=1, tpu_address=None):
...
class SyntheticDataset(object):
    __doc__ = 'A dataset that generates synthetic data on each device.'
  ...
  """
    ...
    if tf_config:
        ...
        if worker_hosts:
            ...
            os.environ['TF_CONFIG'] = json.dumps({'cluster':{'worker': workers}, 
             'task':{'type':'worker', 
              'index':task_index}})
        else:
            num_workers = 1
