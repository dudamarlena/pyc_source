...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/core/nas_cell.py
...
import deeplab.core as xception_utils
from deeplab.core.utils import resize_bilinear
from deeplab.core.utils import scale_dimension
from tensorflow.contrib.slim.nets import resnet_utils
arg_scope = contrib_framework.arg_scope
slim = contrib_slim
separable_conv2d_same = functools.partial((xception_utils.separable_conv2d_same), regularize_depthwise=True)
            ...
            if 'atrous' in operation:
                ...
                if stride == 2:
                    ...
                    net = resnet_utils.conv2d_same(net,
                      filter_size, kernel_size, rate=1, stride=1, scope=('atrous_{0}x{0}'.format(kernel_size)))
                else:
                    net = resnet_utils.conv2d_same(net,
                      filter_size, kernel_size, rate=2, stride=1, scope=('atrous_{0}x{0}'.format(kernel_size)))
                net = self._batch_norm_fn(net, scope=('bn_atr_{0}x{0}'.format(kernel_size)))
            else:
                if not operation in ('none', ) or stride > 1 or input_filters != filter_size:
                    net = tf.nn.relu(net)
