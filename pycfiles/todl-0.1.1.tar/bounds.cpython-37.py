...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/bounds.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 14393 bytes
...
def iwae(model, observations, seq_lengths, num_samples=1, parallel_iterations=30, swap_memory=True):
  ...
  """
    log_p_hat, log_weights, _, final_state = fivo(model,
      observations,
      seq_lengths,
...
def fivo(model, observations, seq_lengths, num_samples=1, resampling_criterion=smc.ess_criterion, resampling_type='multinomial', relaxed_resampling_temperature=0.5, parallel_iterations=30, swap_memory=True, random_seed=None):
  ...
  """
    ...
    if resampling_type == 'multinomial':
        ...
        if resampling_type == 'relaxed':
            resampling_fn = functools.partial((smc.relaxed_resampling),
              temperature=relaxed_resampling_temperature)
    ...
    log_p_hat, log_weights, resampled, final_state, _ = smc.smc(transition_fn,
      seq_lengths,
      ...
      swap_memory=swap_memory)
    return (
     log_p_hat, log_weights, resampled, final_state)


def fivo_aux_td(model, observations, seq_lengths, num_samples=1, resampling_criterion=smc.ess_criterion, resampling_type='multinomial', relaxed_resampling_temperature=0.5, parallel_iterations=30, swap_memory=True, random_seed=None):
    """Experimental."""
    batch_size = tf.shape(seq_lengths)[0]
    ...
    def transition_fn(prev_state, t):
        ...
        new_model_state, zt, log_q_zt, log_p_zt, log_p_x_given_z, log_r_tilde, p_ztplus1 = model(prev_model_state, t)
        ...
        log_r = tf.log(r_tilde_sigma_sq) - tf.log(r_tilde_sigma_sq + p_sigma_sq) - tf.square(r_tilde_mu - p_mu) / (r_tilde_sigma_sq + p_sigma_sq)
        log_r = 0.5 * tf.reduce_sum(log_r, axis=(-1))
        prev_r_tilde_mu, prev_r_tilde_sigma_sq = prev_log_r_tilde
        prev_log_r_tilde = -0.5 * tf.reduce_sum((tf.square(tf.stop_gradient(zt) - prev_r_tilde_mu) / prev_r_tilde_sigma_sq),
          axis=(-1))
        last_timestep = t >= tiled_seq_lengths - 1
        log_r = tf.where(last_timestep, tf.zeros_like(log_r), log_r)
        prev_log_r_tilde = tf.where(last_timestep, tf.zeros_like(prev_log_r_tilde), prev_log_r_tilde)
    ...
    def loop_fn(loop_state, loop_args, unused_model_state, log_weights, resampled, mask, t):
        ...
        log_p_hat_update = tf.reduce_logsumexp(log_weights,
          axis=0) - tf.log(tf.to_float(num_samples))
        log_p_hat_acc += tf.cond(t >= max_seq_len - 1, lambda : log_p_hat_update, lambda : log_p_hat_update * resampled)
        ...
        mask = tf.cond(tf.equal(t, 0), lambda : tf.zeros_like(mask), lambda : mask)
        prev_log_r_tilde = tf.where(tf.is_inf(prev_log_r_tilde), tf.zeros_like(prev_log_r_tilde), prev_log_r_tilde)
        log_lambda = tf.reduce_mean((prev_log_r_tilde - log_p_x_given_z - log_r), axis=0,
          keepdims=True)
        bellman_error = mask * tf.square(prev_log_r_tilde - tf.stop_gradient(log_lambda + log_p_x_given_z + log_r))
    ...
    log_weights, resampled, accs = smc.smc(transition_fn,
      seq_lengths,
      num_particles=num_samples,
      resampling_criterion=resampling_criterion,
      resampling_fn=resampling_fn,
    ...
    tf.summary.scalar('bellman_loss', tf.reduce_mean(bellman_loss / tf.to_float(seq_lengths)))
    tf.summary.scalar('log_r_diff', tf.reduce_mean(tf.reduce_mean(log_r_diff, axis=0) / tf.to_float(seq_lengths)))
