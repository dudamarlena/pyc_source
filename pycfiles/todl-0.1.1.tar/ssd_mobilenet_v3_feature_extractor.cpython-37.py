...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/models/ssd_mobilenet_v3_feature_extractor.py
...
class SSDMobileNetV3FeatureExtractorBase(ssd_meta_arch.SSDFeatureExtractor):
    __doc__ = 'Base class of SSD feature extractor using MobilenetV3 features.'
    ...
    """
        super(SSDMobileNetV3FeatureExtractorBase, self).__init__(is_training=is_training,
    ...
    """
        ...
        with tf.variable_scope((self._scope_name),
          reuse=(self._reuse_weights)) as (scope):
            with slim.arg_scope(mobilenet_v3.training_scope(is_training=None, bn_decay=0.9997)):
                with slim.arg_scope([
                 mobilenet.depth_multiplier],
                  min_depth=(self._min_depth)):
                    with slim.arg_scope(self._conv_hyperparams_fn()) if self._override_base_feature_extractor_hyperparams else context_manager.IdentityContextManager():
                        _, image_features = mobilenet_v3.mobilenet_base((ops.pad_to_multiple(preprocessed_inputs, self._pad_to_multiple)),
...
class SSDMobileNetV3LargeFeatureExtractor(SSDMobileNetV3FeatureExtractorBase):
    __doc__ = 'Mobilenet V3-Large feature extractor.'
    ...
    def __init__(self, is_training, depth_multiplier, min_depth, pad_to_multiple, conv_hyperparams_fn, reuse_weights=None, use_explicit_padding=False, use_depthwise=False, override_base_feature_extractor_hyperparams=False, scope_name='MobilenetV3'):
        super(SSDMobileNetV3LargeFeatureExtractor, self).__init__(conv_defs=(mobilenet_v3.V3_LARGE_DETECTION),
          from_layer=[
         'layer_14/expansion_output', 'layer_17'],
...
class SSDMobileNetV3SmallFeatureExtractor(SSDMobileNetV3FeatureExtractorBase):
    __doc__ = 'Mobilenet V3-Small feature extractor.'

    def __init__(self, is_training, depth_multiplier, min_depth, pad_to_multiple, conv_hyperparams_fn, reuse_weights=None, use_explicit_padding=False, use_depthwise=False, override_base_feature_extractor_hyperparams=False, scope_name='MobilenetV3'):
        super(SSDMobileNetV3SmallFeatureExtractor, self).__init__(conv_defs=(mobilenet_v3.V3_SMALL_DETECTION),
          from_layer=[
         'layer_10/expansion_output', 'layer_13'],
          is_training=is_training,
          depth_multiplier=depth_multiplier,
          min_depth=min_depth,
          pad_to_multiple=pad_to_multiple,
