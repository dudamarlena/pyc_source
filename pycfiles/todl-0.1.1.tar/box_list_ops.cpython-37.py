...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/core/box_list_ops.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 43889 bytes
...
class SortOrder(object):
    __doc__ = 'Enum class for sort order.\n\n  Attributes:\n    ascend: ascend order.\n    descend: descend order.\n  '
    ascend = 1
    descend = 2
  ...
  """
    with tf.name_scope(scope, 'PruneOutsideWindow'):
        ...
        coordinate_violations = tf.concat([
         ...
         tf.greater(y_max, win_y_max), tf.greater(x_max, win_x_max)], 1)
  ...
  """
    with tf.name_scope(scope, 'PruneCompleteleyOutsideWindow'):
        ...
        coordinate_violations = tf.concat([
         ...
         tf.less_equal(y_max, win_y_min), tf.less_equal(x_max, win_x_min)], 1)
        valid_indices = tf.reshape(tf.where(tf.logical_not(tf.reduce_any(coordinate_violations, 1))), [-1])
        return (gather(boxlist, valid_indices), valid_indices)
...
def sq_dist(boxlist1, boxlist2, scope=None):
  ...
  """
    with tf.name_scope(scope, 'SqDist'):
        ...
        innerprod = tf.matmul((boxlist1.get()), (boxlist2.get()), transpose_a=False,
          transpose_b=True)
  ...
  """
    with tf.name_scope(scope, 'BooleanMask'):
        ...
        if use_static_shapes:
            ...
            indexed_positions = tf.cast((tf.multiply(tf.cumsum(selected_positions), selected_positions)),
              dtype=(tf.int32))
            one_hot_selector = tf.one_hot((indexed_positions - 1),
              indicator_sum, dtype=(tf.float32))
            sampled_indices = tf.cast(tf.tensordot(tf.cast((tf.range(tf.shape(indicator)[0])), dtype=(tf.float32)),
              one_hot_selector,
              axes=[
             0, 0]),
              dtype=(tf.int32))
...
def filter_greater_than(boxlist, thresh, scope=None):
  ...
  """
    with tf.name_scope(scope, 'FilterGreaterThan'):
        ...
        high_score_indices = tf.cast(tf.reshape(tf.where(tf.greater(scores, thresh)), [
         -1]), tf.int32)
  ...
  """
    with tf.name_scope(scope, 'NonMaxSuppression'):
        if not 0 <= thresh <= 1.0:
            ...
            if not isinstance(boxlist, box_list.BoxList):
                raise ValueError('boxlist must be a BoxList')
            assert boxlist.has_field('scores'), "input boxlist must have 'scores' field"
        selected_indices = tf.image.non_max_suppression((boxlist.get()),
          (boxlist.get_field('scores')), max_output_size,
  ...
  """
    with tf.name_scope(scope, 'ToAbsoluteCoordinates'):
        ...
        if check_range:
            ...
            max_assert = tf.Assert(tf.greater_equal(maximum_normalized_coordinate, box_maximum), [
             'maximum box coordinate value is larger than %f: ' % maximum_normalized_coordinate, box_maximum])
            with tf.control_dependencies([max_assert]):
                width = tf.identity(width)
  ...
  """
    if not 0.0 <= nms_iou_thresh <= 1.0:
        ...
        if not 0.0 <= voting_iou_thresh <= 1.0:
            ...
            assert pool_boxes.has_field('scores'), "pool_boxes must have a 'scores' field"
        assert pool_boxes.has_field('classes'), "pool_boxes must have a 'classes' field"
    refined_boxes = []
...
def select_random_box(boxlist, default_box=None, seed=None, scope=None):
  ...
  """
    with tf.name_scope(scope, 'SelectRandomBox'):
        ...
        def select_box():
            random_index = tf.random_uniform([], maxval=number_of_boxes,
              dtype=(tf.int32),
              seed=seed)
