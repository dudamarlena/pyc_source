...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/core/xception.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 40279 bytes
r"""Xception model.
...
import nets.mobilenet as mobilenet_v3_ops
slim = contrib_slim
_DEFAULT_MULTI_GRID = [
 1, 1, 1]
_CLIP_CAP = 6
...
def separable_conv2d_same(inputs, num_outputs, kernel_size, depth_multiplier, stride, rate=1, use_explicit_padding=True, regularize_depthwise=False, scope=None, **kwargs):
 ...
 num_outputs,
 kernel_size, depth_multiplier=depth_multiplier, 
         stride=stride, 
         rate=rate, 
         padding=padding, 
         scope=scope, **kwargs)
 ...
 kernel_size, depth_multiplier=depth_multiplier, 
         ...
         rate=rate, 
         padding=padding, 
         scope=scope + '_depthwise', **kwargs)
        return (slim.conv2d)(outputs,
 num_outputs,
 1, scope=scope + '_pointwise', **kwargs)

    if not stride == 1:
        if not use_explicit_padding:
  ...
  """
    ...
    with tf.variable_scope(scope, 'xception_module', [inputs]) as (sc):
        ...
        def _separable_conv(features, depth, kernel_size, depth_multiplier, regularize_depthwise, rate, stride, scope):
            ...
            return separable_conv2d_same(features, depth,
              kernel_size,
        ...
        if use_squeeze_excite:
            residual = mobilenet_v3_ops.squeeze_excite(input_tensor=residual,
              squeeze_factor=16,
              inner_activation_fn=(tf.nn.relu),
              gating_fn=(lambda x: tf.nn.relu6(x + 3) * 0.16667),
              pool=se_pool_size)
        elif skip_connection_type == 'conv':
            shortcut = slim.conv2d(inputs, (depth_list[(-1)]),
              [
             1, 1],
              stride=stride,
              activation_fn=None,
...
def xception(inputs, blocks, num_classes=None, is_training=True, global_pool=True, keep_prob=0.5, output_stride=None, reuse=None, scope=None, sync_batch_norm_method='None'):
  ...
  """
    with tf.variable_scope(scope,
      'xception', [inputs], reuse=reuse) as (sc):
  ...
  """
    ...
    return Block(scope, xception_module, [
     {'depth_list':depth_list, 
      ...
      'se_pool_size':se_pool_size}] * num_units)
...
def xception_41(inputs, num_classes=None, is_training=True, global_pool=True, keep_prob=0.5, output_stride=None, regularize_depthwise=False, multi_grid=None, reuse=None, scope='xception_41', sync_batch_norm_method='None'):
    """Xception-41 model."""
    blocks = [
     xception_block('entry_flow/block1', depth_list=[
      128, 128, 128],
       ...
       num_units=1,
       stride=2),
     xception_block('entry_flow/block2', depth_list=[
      256, 256, 256],
       skip_connection_type='conv',
       activation_fn_in_separable_conv=False,
       regularize_depthwise=regularize_depthwise,
       num_units=1,
       stride=2),
     xception_block('entry_flow/block3', depth_list=[
      728, 728, 728],
       skip_connection_type='conv',
       activation_fn_in_separable_conv=False,
       regularize_depthwise=regularize_depthwise,
       num_units=1,
       stride=2),
     xception_block('middle_flow/block1', depth_list=[
      728, 728, 728],
       ...
       num_units=8,
       stride=1),
     xception_block('exit_flow/block1', depth_list=[
      728, 1024, 1024],
       skip_connection_type='conv',
       activation_fn_in_separable_conv=False,
       regularize_depthwise=regularize_depthwise,
       num_units=1,
       stride=2),
     xception_block('exit_flow/block2', depth_list=[
      1536, 1536, 2048],
    ...
    return xception(inputs, blocks=blocks,
...
def xception_65(inputs, num_classes=None, is_training=True, global_pool=True, keep_prob=0.5, output_stride=None, regularize_depthwise=False, multi_grid=None, reuse=None, scope='xception_65', sync_batch_norm_method='None'):
    """Xception-65 model."""
    return xception_65_factory(inputs=inputs,
      num_classes=num_classes,
      is_training=is_training,
      global_pool=global_pool,
      keep_prob=keep_prob,
    ...
    blocks = [
     ...
     xception_block('entry_flow/block3', depth_list=[
      256, 256, 256],
       ...
       use_squeeze_excite=False,
       se_pool_size=se_pool_size),
     xception_block('entry_flow/block4', depth_list=[
      728, 728, 728],
       skip_connection_type='conv',
       activation_fn_in_separable_conv=False,
       regularize_depthwise=regularize_depthwise,
       ...
       use_squeeze_excite=use_squeeze_excite,
       se_pool_size=se_pool_size),
     xception_block('entry_flow/block5', depth_list=[
      728, 728, 728],
       skip_connection_type='conv',
       activation_fn_in_separable_conv=False,
       regularize_depthwise=regularize_depthwise,
...
def xception_71(inputs, num_classes=None, is_training=True, global_pool=True, keep_prob=0.5, output_stride=None, regularize_depthwise=False, multi_grid=None, reuse=None, scope='xception_71', sync_batch_norm_method='None'):
    """Xception-71 model."""
    return xception_71_factory(inputs=inputs,
      num_classes=num_classes,
      is_training=is_training,
      global_pool=global_pool,
      keep_prob=keep_prob,
