...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/legacy/trainer.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 17725 bytes
  ...
  """
    ...
    if data_augmentation_options:
        tensor_dict = preprocessor.preprocess(tensor_dict,
          data_augmentation_options, func_arg_map=preprocessor.get_default_func_arg_map(include_label_weights=True,
          include_multiclass_scores=include_multiclass_scores,
          include_instance_masks=include_instance_masks,
          include_keypoints=include_keypoints))
    input_queue = batcher.BatchQueue(tensor_dict,
  ...
  """
    ...
    def extract_images_and_targets(read_data):
        ...
        if fields.InputDataFields.source_id in read_data:
            ...
            elif merge_multiple_label_boxes:
        ...
        if merge_multiple_label_boxes:
            if masks_gt is not None or keypoints_gt is not None:
...
def train(create_tensor_dict_fn, create_model_fn, train_config, master, task, num_clones, worker_replicas, clone_on_cpu, ps_tasks, worker_job_name, is_chief, train_dir, graph_hook_fn=None):
  ...
  """
    ...
    with tf.Graph().as_default():
        ...
        sync_optimizer = None
        if train_config.sync_replicas:
            training_optimizer = tf.train.SyncReplicasOptimizer(training_optimizer,
        ...
        with tf.device(deploy_config.optimizer_device()):
            ...
            total_loss, grads_and_vars = model_deploy.optimize_clones(clones,
              training_optimizer, regularization_losses=regularization_losses)
            total_loss = tf.check_numerics(total_loss, 'LossTensor is inf or nan.')
            if train_config.bias_grad_multiplier:
                biases_regex_list = [
                 '.*/biases']
                grads_and_vars = variables_helper.multiply_gradients_matching_regex(grads_and_vars,
                  biases_regex_list,
        ...
        if train_config.fine_tune_checkpoint:
            ...
            var_map = detection_model.restore_map(fine_tune_checkpoint_type=(train_config.fine_tune_checkpoint_type),
            ...
            available_var_map = variables_helper.get_variables_available_in_checkpoint(var_map,
              (train_config.fine_tune_checkpoint), include_global_step=False)
