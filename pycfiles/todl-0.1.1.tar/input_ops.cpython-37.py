...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/skip_thoughts/skip_thoughts/ops/input_ops.py
...
def parse_example_batch(serialized):
  ...
  """
    features = tf.parse_example(serialized,
      features={'encode':tf.VarLenFeature(dtype=tf.int64), 
...
def prefetch_input_data(reader, file_pattern, shuffle, capacity, num_reader_threads=1):
  ...
  """
    ...
    filename_queue = tf.train.string_input_producer(data_files,
      shuffle=shuffle, capacity=16, name='filename_queue')
    if shuffle:
    ...
    tf.summary.scalar('queue/%s/fraction_of_%d_full' % (values_queue.name,
     capacity), tf.cast(values_queue.size(), tf.float32) * (1.0 / capacity))
    return values_queue