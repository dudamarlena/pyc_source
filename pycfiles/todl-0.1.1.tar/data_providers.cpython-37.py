...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/tcn/data_providers.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 20799 bytes
"""Defines data providers used in training and evaluating TCNs."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import functools, random, numpy as np, preprocessing, tensorflow as tf
...
def labeled_data_provider(filenames, preprocess_fn, view_index, image_attr_keys, label_attr_keys, batch_size=32, num_epochs=1):
  ...
  """
    ...
    return (
     batch_images, batch_labels, batch_tasks)
...
def parse_sequence_example(serialized_example, num_views):
    ...
    context_features = {'task':tf.FixedLenFeature(shape=[], dtype=tf.string), 
    ...
    context_parse, sequence_parse = tf.parse_single_sequence_example(serialized=serialized_example,
...
def parse_sequence_to_pairs_batch(serialized_example, preprocess_fn, is_training, num_views, batch_size, window):
  ...
  """
    ...
    anchor_images = tf.map_fn((preprocessing.decode_image),
      anchor_images, dtype=(tf.float32))
    pos_images = tf.map_fn((preprocessing.decode_image),
      pos_images, dtype=(tf.float32))
    ...
    ims = [
     anchor_prepro, positive_prepro, anchor_images, pos_images]
...
def multiview_pairs_provider(file_list, preprocess_fn, num_views, window, is_training, batch_size, examples_per_seq=2, num_parallel_calls=12, sequence_prefetch_size=12, batch_prefetch_size=12):
  ...
  """
    ...
    dataset = dataset.map(_parse_sequence,
    ...
    return (
     batch_images, anchor_labels, positive_labels, anchor_images, pos_images)
