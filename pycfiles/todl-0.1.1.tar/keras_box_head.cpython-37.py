...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/predictors/heads/keras_box_head.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 13670 bytes
"""Box Head.
...
class ConvolutionalBoxHead(head.KerasHead):
    ...
    """
        if use_depthwise:
            ...
            if self._use_depthwise:
                self._box_encoder_layers.append((tf.keras.layers.DepthwiseConv2D)(
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
                 depth_multiplier=1, 
                 strides=1, 
                 dilation_rate=1, 
                 name='BoxEncodingPredictor_depthwise', **conv_hyperparams.params()))
                self._box_encoder_layers.append(conv_hyperparams.build_batch_norm(training=(is_training and not freeze_batchnorm),
 ...
 num_predictions_per_location * self._box_code_size, [1, 1], name='BoxEncodingPredictor', **conv_hyperparams.params(use_bias=True)))
            else:
                self._box_encoder_layers.append((tf.keras.layers.Conv2D)(
 num_predictions_per_location * self._box_code_size,
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
                 name='BoxEncodingPredictor', **conv_hyperparams.params(use_bias=True)))
...
class MaskRCNNBoxHead(head.KerasHead):
    ...
    """
        ...
        self._box_encoder_layers.append(fc_hyperparams.build_batch_norm(training=(is_training and not freeze_batchnorm),
...
class WeightSharedConvolutionalBoxHead(head.KerasHead):
    ...
    """
        if use_depthwise:
            ...
            self._box_encoder_layers = []
            if self._use_depthwise:
                self._box_encoder_layers.append((tf.keras.layers.SeparableConv2D)(
 num_predictions_per_location * self._box_code_size,
 [
  self._kernel_size, self._kernel_size], padding='SAME', 
                 name='BoxPredictor', **conv_hyperparams.params(use_bias=True)))
            else:
                self._box_encoder_layers.append((tf.keras.layers.Conv2D)(
 num_predictions_per_location * self._box_code_size,
 [
