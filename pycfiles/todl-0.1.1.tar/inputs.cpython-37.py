...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/inputs.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 34002 bytes
...
HASH_KEY = 'hash'
HASH_BINS = 2147483648
SERVING_FED_EXAMPLE_KEY = 'serialized_example'
INPUT_BUILDER_UTIL_MAP = {'dataset_build':dataset_builder.build, 
 'model_build':model_builder.build}

def _multiclass_scores_or_one_hot_labels(multiclass_scores, groundtruth_boxes, groundtruth_classes, num_classes):
...
def transform_input_data(tensor_dict, model_preprocess_fn, image_resizer_fn, num_classes, data_augmentation_fn=None, merge_multiple_boxes=False, retain_original_image=False, use_multiclass_scores=False, use_bfloat16=False, retain_original_image_additional_channels=False):
  ...
  """
    ...
    if fields.InputDataFields.multiclass_scores in out_tensor_dict:
        out_tensor_dict[fields.InputDataFields.multiclass_scores] = _multiclass_scores_or_one_hot_labels(out_tensor_dict[fields.InputDataFields.multiclass_scores], out_tensor_dict[fields.InputDataFields.groundtruth_boxes], out_tensor_dict[fields.InputDataFields.groundtruth_classes], num_classes)
    if fields.InputDataFields.groundtruth_boxes in out_tensor_dict:
        out_tensor_dict = util_ops.filter_groundtruth_with_nan_box_coordinates(out_tensor_dict)
        out_tensor_dict = util_ops.filter_unrecognized_classes(out_tensor_dict)
    if retain_original_image:
        out_tensor_dict[fields.InputDataFields.original_image] = tf.cast(image_resizer_fn(out_tensor_dict[fields.InputDataFields.image], None)[0], tf.uint8)
    else:
        if fields.InputDataFields.image_additional_channels in out_tensor_dict:
            ...
            out_tensor_dict[fields.InputDataFields.image] = tf.concat([
             out_tensor_dict[fields.InputDataFields.image], channels],
              axis=2)
            if retain_original_image_additional_channels:
                out_tensor_dict[fields.InputDataFields.image_additional_channels] = tf.cast(image_resizer_fn(channels, None)[0], tf.uint8)
        ...
        if fields.InputDataFields.groundtruth_boxes in tensor_dict:
            ...
            out_tensor_dict[fields.InputDataFields.groundtruth_boxes] = realigned_bboxes.get()
        if fields.InputDataFields.groundtruth_keypoints in tensor_dict:
            keypoints = out_tensor_dict[fields.InputDataFields.groundtruth_keypoints]
            realigned_keypoints = keypoint_ops.change_coordinate_frame(keypoints, im_box)
            out_tensor_dict[fields.InputDataFields.groundtruth_keypoints] = realigned_keypoints
        if use_bfloat16:
            preprocessed_resized_image = tf.cast(preprocessed_resized_image, tf.bfloat16)
        out_tensor_dict[fields.InputDataFields.image] = tf.squeeze(preprocessed_resized_image,
          axis=0)
        out_tensor_dict[fields.InputDataFields.true_image_shape] = tf.squeeze(true_image_shape,
          axis=0)
        if fields.InputDataFields.groundtruth_instance_masks in out_tensor_dict:
            ...
            out_tensor_dict[fields.InputDataFields.groundtruth_instance_masks] = resized_masks
        label_offset = 1
        zero_indexed_groundtruth_classes = out_tensor_dict[fields.InputDataFields.groundtruth_classes] - label_offset
        if use_multiclass_scores:
            out_tensor_dict[fields.InputDataFields.groundtruth_classes] = out_tensor_dict[fields.InputDataFields.multiclass_scores]
        else:
            out_tensor_dict[fields.InputDataFields.groundtruth_classes] = tf.one_hot(zero_indexed_groundtruth_classes, num_classes)
    ...
    if fields.InputDataFields.groundtruth_confidences in out_tensor_dict:
        groundtruth_confidences = out_tensor_dict[fields.InputDataFields.groundtruth_confidences]
        out_tensor_dict[fields.InputDataFields.groundtruth_confidences] = tf.reshape(groundtruth_confidences, [-1, 1]) * out_tensor_dict[fields.InputDataFields.groundtruth_classes]
    else:
        groundtruth_confidences = tf.ones_like(zero_indexed_groundtruth_classes,
          dtype=(tf.float32))
    ...
    if merge_multiple_boxes:
        merged_boxes, merged_classes, merged_confidences, _ = util_ops.merge_boxes_with_multiple_labels(out_tensor_dict[fields.InputDataFields.groundtruth_boxes], zero_indexed_groundtruth_classes, groundtruth_confidences, num_classes)
        ...
        out_tensor_dict[fields.InputDataFields.groundtruth_confidences] = merged_confidences
    if fields.InputDataFields.groundtruth_boxes in out_tensor_dict:
        out_tensor_dict[fields.InputDataFields.num_groundtruth_boxes] = tf.shape(out_tensor_dict[fields.InputDataFields.groundtruth_boxes])[0]
    return out_tensor_dict


def pad_input_data_to_static_shapes(tensor_dict, max_num_boxes, num_classes, spatial_image_shape=None):
  ...
  """
    ...
    if fields.InputDataFields.image_additional_channels in tensor_dict:
        num_additional_channels = shape_utils.get_dim_as_int(tensor_dict[fields.InputDataFields.image_additional_channels].shape[2])
    num_channels = 3
    if fields.InputDataFields.image in tensor_dict:
        num_channels = shape_utils.get_dim_as_int(tensor_dict[fields.InputDataFields.image].shape[2])
    if num_additional_channels:
        if num_additional_channels >= num_channels:
            raise ValueError('Image must be already concatenated with additional channels.')
        if fields.InputDataFields.original_image in tensor_dict:
            if shape_utils.get_dim_as_int(tensor_dict[fields.InputDataFields.original_image].shape[2]) == num_channels:
                raise ValueError('Image must be already concatenated with additional channels.')
    padding_shapes = {fields.InputDataFields.image: [
    ...
    if fields.InputDataFields.original_image in tensor_dict:
        padding_shapes[fields.InputDataFields.original_image] = [height, width,
         shape_utils.get_dim_as_int(tensor_dict[fields.InputDataFields.original_image].shape[2])]
    if fields.InputDataFields.groundtruth_keypoints in tensor_dict:
        tensor_shape = tensor_dict[fields.InputDataFields.groundtruth_keypoints].shape
    ...
    if fields.InputDataFields.groundtruth_keypoint_visibilities in tensor_dict:
        tensor_shape = tensor_dict[fields.InputDataFields.groundtruth_keypoint_visibilities].shape
        padding_shape = [
         max_num_boxes, shape_utils.get_dim_as_int(tensor_shape[1])]
        padding_shapes[fields.InputDataFields.groundtruth_keypoint_visibilities] = padding_shape
    padded_tensor_dict = {}
    for tensor_name in tensor_dict:
        padded_tensor_dict[tensor_name] = shape_utils.pad_or_clip_nd(tensor_dict[tensor_name], padding_shapes[tensor_name])

    if fields.InputDataFields.num_groundtruth_boxes in padded_tensor_dict:
        padded_tensor_dict[fields.InputDataFields.num_groundtruth_boxes] = tf.minimum(padded_tensor_dict[fields.InputDataFields.num_groundtruth_boxes], max_num_boxes)
...
def augment_input_data(tensor_dict, data_augmentation_options):
  ...
  """
    tensor_dict[fields.InputDataFields.image] = tf.expand_dims(tf.cast((tensor_dict[fields.InputDataFields.image]), dtype=(tf.float32)), 0)
    ...
    include_label_weights = fields.InputDataFields.groundtruth_weights in tensor_dict
    include_label_confidences = fields.InputDataFields.groundtruth_confidences in tensor_dict
    include_multiclass_scores = fields.InputDataFields.multiclass_scores in tensor_dict
    tensor_dict = preprocessor.preprocess(tensor_dict,
      data_augmentation_options, func_arg_map=preprocessor.get_default_func_arg_map(include_label_weights=include_label_weights,
...
def _get_labels_dict(input_dict):
    ...
    if fields.InputDataFields.groundtruth_difficult in labels_dict:
        labels_dict[fields.InputDataFields.groundtruth_difficult] = tf.cast(labels_dict[fields.InputDataFields.groundtruth_difficult], tf.int32)
    return labels_dict
...
def _replace_empty_string_with_random_number(string_tensor):
  ...
  """
    ...
    random_source_id = tf.as_string(tf.random_uniform(shape=[], maxval=9223372036854775807, dtype=(tf.int64)))
    out_string = tf.cond((tf.equal(string_tensor, empty_string)),
...
def _get_features_dict(input_dict):
    ...
    source_id = _replace_empty_string_with_random_number(input_dict[fields.InputDataFields.source_id])
    hash_from_source_id = tf.string_to_hash_bucket_fast(source_id, HASH_BINS)
    features = {fields.InputDataFields.image: input_dict[fields.InputDataFields.image], 
     
     HASH_KEY: tf.cast(hash_from_source_id, tf.int32), 
     fields.InputDataFields.true_image_shape: input_dict[fields.InputDataFields.true_image_shape], 
     
     fields.InputDataFields.original_image_spatial_shape: input_dict[fields.InputDataFields.original_image_spatial_shape]}
    if fields.InputDataFields.original_image in input_dict:
        features[fields.InputDataFields.original_image] = input_dict[fields.InputDataFields.original_image]
    if fields.InputDataFields.image_additional_channels in input_dict:
        features[fields.InputDataFields.image_additional_channels] = input_dict[fields.InputDataFields.image_additional_channels]
    return features
...
def create_train_input_fn(train_config, train_input_config, model_config):
  ...
  """
    ...
    def _train_input_fn(params=None):
        return train_input(train_config, train_input_config, model_config, params=params)

    return _train_input_fn


def train_input(train_config, train_input_config, model_config, model=None, params=None):
  ...
  """
    if not isinstance(train_config, train_pb2.TrainConfig):
        ...
        if not isinstance(train_input_config, input_reader_pb2.InputReader):
            raise TypeError('The `train_input_config` must be a input_reader_pb2.InputReader.')
        if not isinstance(model_config, model_pb2.DetectionModel):
            raise TypeError('The `model_config` must be a model_pb2.DetectionModel.')
        if model is None:
            model_preprocess_fn = INPUT_BUILDER_UTIL_MAP['model_build'](model_config,
              is_training=True).preprocess
        else:
    ...
    def transform_and_pad_input_data_fn(tensor_dict):
        ...
        data_augmentation_fn = functools.partial(augment_input_data,
          data_augmentation_options=data_augmentation_options)
        image_resizer_config = config_util.get_image_resizer_config(model_config)
        image_resizer_fn = image_resizer_builder.build(image_resizer_config)
        transform_data_fn = functools.partial(transform_input_data,
          model_preprocess_fn=model_preprocess_fn, image_resizer_fn=image_resizer_fn,
        ...
        tensor_dict = pad_input_data_to_static_shapes(tensor_dict=(transform_data_fn(tensor_dict)),
          max_num_boxes=(train_input_config.max_number_of_boxes),
          num_classes=(config_util.get_number_of_classes(model_config)),
          spatial_image_shape=(config_util.get_spatial_image_size(image_resizer_config)))
        return (_get_features_dict(tensor_dict), _get_labels_dict(tensor_dict))

    dataset = INPUT_BUILDER_UTIL_MAP['dataset_build'](train_input_config,
...
def eval_input(eval_config, eval_input_config, model_config, model=None, params=None):
  ...
  """
    ...
    if not isinstance(eval_input_config, input_reader_pb2.InputReader):
        raise TypeError('The `eval_input_config` must be a input_reader_pb2.InputReader.')
    else:
        if not isinstance(model_config, model_pb2.DetectionModel):
            raise TypeError('The `model_config` must be a model_pb2.DetectionModel.')
        if eval_config.force_no_resize:
            ...
            image_resizer_proto.identity_resizer.CopyFrom(image_resizer_pb2.IdentityResizer())
            arch_config.image_resizer.CopyFrom(image_resizer_proto)
        if model is None:
            model_preprocess_fn = INPUT_BUILDER_UTIL_MAP['model_build'](model_config,
              is_training=False).preprocess
        else:
            model_preprocess_fn = model.preprocess
    ...
    dataset = INPUT_BUILDER_UTIL_MAP['dataset_build'](eval_input_config,
...
def create_predict_input_fn(model_config, predict_input_config):
  ...
  """
    ...
    """
        ...
        true_image_shape = tf.expand_dims((input_dict[fields.InputDataFields.true_image_shape]),
          axis=0)
        return tf.estimator.export.ServingInputReceiver(features={fields.InputDataFields.image: images, 
