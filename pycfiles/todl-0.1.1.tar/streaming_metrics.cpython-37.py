...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/deeplab/evaluation/streaming_metrics.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 10413 bytes
...
def _running_total(value, shape, name=None):
    ...
    with tf.variable_scope(name, 'running_total', [value]):
        total_var = tf.get_variable('total',
          shape,
          (value.dtype),
...
def streaming_panoptic_quality(groundtruth_categories, groundtruth_instances, predicted_categories, predicted_instances, num_classes, max_instances_per_category, ignored_label, offset, name=None):
  ...
  """
    ...
    with tf.variable_scope(name, 'streaming_panoptic_quality', input_args):
        panoptic_results = tf.py_func(_panoptic_quality_helper,
          input_args, return_types, stateful=False)
        iou, tp, fn, fp = tuple(panoptic_results)
        total_iou, updated_iou = _running_total(iou,
          [num_classes], name='iou_total')
...
def streaming_parsing_covering(groundtruth_categories, groundtruth_instances, predicted_categories, predicted_instances, num_classes, max_instances_per_category, ignored_label, offset, normalize_by_image_size=True, name=None):
  ...
  """
    ...
    with tf.variable_scope(name, 'streaming_parsing_covering', input_args):
        covering_results = tf.py_func(_parsing_covering_helper,
          input_args, return_types, stateful=False)
        weighted_iou_per_class, gt_area_per_class = tuple(covering_results)
        total_weighted_iou_per_class, updated_weighted_iou_per_class = _running_total(weighted_iou_per_class,
          [num_classes], name='weighted_iou_per_class_total')
        total_gt_area_per_class, updated_gt_area_per_class = _running_total(gt_area_per_class,
          [num_classes], name='gt_area_per_class_total')
