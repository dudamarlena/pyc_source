...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/lstm_object_detection/meta_architectures/lstm_ssd_meta_arch.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 17390 bytes
...
class LSTMSSDMetaArch(ssd_meta_arch.SSDMetaArch):
    __doc__ = 'LSTM Meta-architecture definition.'
    ...
    def __init__(self, is_training, anchor_generator, box_predictor, box_coder, feature_extractor, encode_background_as_zeros, image_resizer_fn, non_max_suppression_fn, score_conversion_fn, classification_loss, localization_loss, classification_loss_weight, localization_loss_weight, normalize_loss_by_num_matches, hard_example_miner, unroll_length, target_assigner_instance, add_summaries=True):
        super(LSTMSSDMetaArch, self).__init__(is_training=is_training,
    ...
    def predict(self, preprocessed_inputs, true_image_shapes, states=None, state_name='lstm_state', feature_scope=None):
        with tf.variable_scope((self._extract_features_scope), values=[
         preprocessed_inputs],
          reuse=(tf.AUTO_REUSE)):
            feature_maps = self._feature_extractor.extract_features(preprocessed_inputs,
              states, state_name, unroll_length=(self._unroll_length),
              scope=feature_scope)
        ...
        self._states = states
        anchors = self._anchor_generator.generate(feature_map_spatial_dims, im_height=(image_shape[1]),
        ...
        with tf.variable_scope('Loss', reuse=(tf.AUTO_REUSE)):
            ...
            if box_encodings.shape.ndims == 4:
                if box_encodings.shape[2] == 1:
                    box_encodings = tf.squeeze(box_encodings, axis=2)
            class_predictions_with_background = tf.concat((prediction_dict['class_predictions_with_background']),
              axis=1)
        predictions_dict = {'preprocessed_inputs':preprocessed_inputs,  'box_encodings':box_encodings, 
        ...
        with tf.name_scope(scope, 'Loss', prediction_dict.values()):
            ...
            if self.groundtruth_has_field(fields.BoxListFields.keypoints):
                ...
                if self.groundtruth_has_field(fields.BoxListFields.weights):
                    ...
                    cls_losses = ops.reduce_sum_trailing_dimensions(self._classification_loss((prediction_dict['class_predictions_with_background']),
                      batch_cls_targets,
    ...
    """
        if fine_tune_checkpoint_type not in ('classification', 'detection', 'interleaved',
                                             'lstm', 'interleaved_pretrain'):
        ...
        for variable in tf.global_variables():
            ...
            if 'global_step' in var_name:
                continue
            if not fine_tune_checkpoint_type == 'classification':
                if fine_tune_checkpoint_type == 'interleaved_pretrain':
                    var_name = re.split('^' + self._extract_features_scope + '/', var_name)[(-1)]
                if 'FeatureMaps' in var_name:
                ...
                if interleaved_model:
                    ...
                    if scope_to_replace in var_name:
                        ...
                        variables_to_restore[var_name] = variable
                    elif 'interleaved' in fine_tune_checkpoint_type:
...
class LSTMSSDFeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
    __doc__ = 'LSTM SSD Meta-architecture Feature Extractor definition.'
...
class LSTMSSDInterleavedFeatureExtractor(LSTMSSDFeatureExtractor):
    __doc__ = 'LSTM SSD Meta-architecture Interleaved Feature Extractor definition.'
    __metaclass__ = abc.ABCMeta
