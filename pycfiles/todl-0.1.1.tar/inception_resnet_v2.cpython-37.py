...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/slim/nets/inception_resnet_v2.py
# Compiled at: 2020-04-05 19:50:58
# Size of source mod 2**32: 18785 bytes
...
def block35(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, 'Block35', [net], reuse=reuse):
        ...
        up = slim.conv2d(mixed, (net.get_shape()[3]), 1, normalizer_fn=None, activation_fn=None,
          scope='Conv2d_1x1')
        scaled_up = up * scale
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1,
          padding='SAME'):
            ...
            with tf.compat.v1.variable_scope('Mixed_6a'):
                with tf.compat.v1.variable_scope('Branch_0'):
                    tower_conv = slim.conv2d(net, 384, 3, stride=(1 if use_atrous else 2), padding=padding,
                      scope='Conv2d_1a_3x3')
                with tf.compat.v1.variable_scope('Branch_1'):
                    tower_conv1_0 = slim.conv2d(net, 256, 1, scope='Conv2d_0a_1x1')
                    tower_conv1_1 = slim.conv2d(tower_conv1_0, 256, 3, scope='Conv2d_0b_3x3')
                    tower_conv1_2 = slim.conv2d(tower_conv1_1, 384, 3, stride=(1 if use_atrous else 2),
                      padding=padding,
                      scope='Conv2d_1a_3x3')
                with tf.compat.v1.variable_scope('Branch_2'):
                    tower_pool = slim.max_pool2d(net, 3, stride=(1 if use_atrous else 2), padding=padding,
            ...
            with tf.compat.v1.variable_scope('Mixed_7a'):
                with tf.compat.v1.variable_scope('Branch_0'):
                    ...
                    tower_conv_1 = slim.conv2d(tower_conv, 384, 3, stride=2, padding=padding,
                      scope='Conv2d_1a_3x3')
                with tf.compat.v1.variable_scope('Branch_1'):
                    tower_conv1 = slim.conv2d(net, 256, 1, scope='Conv2d_0a_1x1')
                    tower_conv1_1 = slim.conv2d(tower_conv1, 288, 3, stride=2, padding=padding,
                      scope='Conv2d_1a_3x3')
                with tf.compat.v1.variable_scope('Branch_2'):
                    tower_conv2 = slim.conv2d(net, 256, 1, scope='Conv2d_0a_1x1')
                    tower_conv2_1 = slim.conv2d(tower_conv2, 288, 3, scope='Conv2d_0b_3x3')
                    tower_conv2_2 = slim.conv2d(tower_conv2_1, 320, 3, stride=2, padding=padding,
                      scope='Conv2d_1a_3x3')
                with tf.compat.v1.variable_scope('Branch_3'):
                    tower_pool = slim.max_pool2d(net, 3, stride=2, padding=padding,
                      scope='MaxPool_1a_3x3')
                net = tf.concat([
...
def inception_resnet_v2(inputs, num_classes=1001, is_training=True, dropout_keep_prob=0.8, reuse=None, scope='InceptionResnetV2', create_aux_logits=True, activation_fn=tf.nn.relu):
  ...
  """
    ...
    with tf.compat.v1.variable_scope(scope,
      'InceptionResnetV2', [inputs], reuse=reuse) as (scope):
            ...
            if create_aux_logits:
                if num_classes:
                    with tf.compat.v1.variable_scope('AuxLogits'):
                        ...
                        aux = slim.conv2d(aux, 768, (aux.get_shape()[1:3]), padding='VALID',
                          scope='Conv2d_2a_5x5')
...
def inception_resnet_v2_arg_scope(weight_decay=4e-05, batch_norm_decay=0.9997, batch_norm_epsilon=0.001, activation_fn=tf.nn.relu, batch_norm_updates_collections=tf.compat.v1.GraphKeys.UPDATE_OPS, batch_norm_scale=False):
  ...
  """
    with slim.arg_scope([slim.conv2d, slim.fully_connected], weights_regularizer=(slim.l2_regularizer(weight_decay)),
      biases_regularizer=(slim.l2_regularizer(weight_decay))):
        ...
        with slim.arg_scope([slim.conv2d], activation_fn=activation_fn, normalizer_fn=(slim.batch_norm),
