...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/fivo/fivo/ghmm_runners.py
...
def run_train(config):
    ...
    def create_logging_hook(step, bound_value, likelihood, bound_gap):
        ...
        logging_hook = tf.train.LoggingTensorHook({'step':step, 
         'value':bound_value,  'll':likelihood, 
         'gap':bound_gap},
    ...
    def create_losses(model, observations, lengths):
        if config.bound == 'elbo':
            ll_per_seq, _, _ = bounds.iwae(model,
              observations, lengths, num_samples=1, parallel_iterations=(config.parallel_iterations))
        else:
            if config.bound == 'iwae':
                ll_per_seq, _, _ = bounds.iwae(model,
                  observations, lengths, num_samples=(config.num_samples), parallel_iterations=(config.parallel_iterations))
            else:
                if config.bound == 'fivo':
                    if config.resampling_type == 'relaxed':
                        ...
                        ll_per_seq, _, _, _ = bounds.fivo(model,
                          observations, lengths, num_samples=(config.num_samples),
                          resampling_criterion=(smc.ess_criterion),
                          resampling_type=(config.resampling_type),
                          random_seed=(config.random_seed),
                          parallel_iterations=(config.parallel_iterations))
    ...
    def create_graph():
        ...
        xs, lengths = datasets.create_chain_graph_dataset((config.batch_size),
         ...
         log_hook],
          ...
          log_step_count_steps=(config.summarize_every * 20)) as (sess):
            ...
            while cur_step <= config.max_steps:
                cur_step = sess.should_stop() or sess.run(global_step)
...
def run_eval(config):
    ...
    def create_bound(model, xs, lengths):
        if config.bound == 'elbo':
            ll_per_seq, log_weights, _ = bounds.iwae(model,
              xs, lengths, num_samples=1, parallel_iterations=(config.parallel_iterations))
        else:
            if config.bound == 'iwae':
                ll_per_seq, log_weights, _ = bounds.iwae(model,
                  xs, lengths, num_samples=(config.num_samples), parallel_iterations=(config.parallel_iterations))
            else:
                if config.bound == 'fivo':
                    ll_per_seq, log_weights, resampled, _ = bounds.fivo(model,
                      xs, lengths, num_samples=(config.num_samples),
                      resampling_criterion=(smc.ess_criterion),
                      resampling_type=(config.resampling_type),
                      random_seed=(config.random_seed),
                      parallel_iterations=(config.parallel_iterations))
        ...
        if config.bound == 'fivo':
            return (
             bound_per_t, log_weights, resampled)
    ...
    with tf.Graph().as_default():
        ...
        with tf.train.SingularMonitoredSession(checkpoint_dir=(config.logdir)) as (sess):
            ...
            data = {'mean':avg_bound,  'std':std,  'log_weights':log_weights,  'log_weight_means':avg_log_weight, 
