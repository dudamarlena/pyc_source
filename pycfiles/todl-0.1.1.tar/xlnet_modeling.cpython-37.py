...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/official/nlp/xlnet/xlnet_modeling.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 46038 bytes
...
def _get_initializer(flags):
    ...
    if flags.init_method == 'uniform':
        initializer = tf.keras.initializers.RandomUniform(minval=(-flags.init_range),
...
class PositionalEmbedding(tf.keras.layers.Layer):
    __doc__ = 'Generates relative positional embeddings used in Transformer-XL and XLNet.'
...
class RelativeAttention(tf.keras.layers.Layer):
    __doc__ = 'Core calculations for relative attention.'
...
class PositionwiseFF(tf.keras.layers.Layer):
    ...
    def build(self, unused_input_shapes):
        ...
        self.inner_projection_layer = tf.keras.layers.Dense(units=(self.d_inner),
          activation=activation,
          kernel_initializer=(self.kernel_initializer),
          name='layer_1')
        self.output_projection_layer = tf.keras.layers.Dense(units=(self.d_model),
          kernel_initializer=(self.kernel_initializer),
          name='layer_2')
        self.output_dropout = tf.keras.layers.Dropout(rate=(self.dropout),
          name='drop_2')
        self.output_layer_norm = tf.keras.layers.LayerNormalization(name='LayerNorm',
          axis=(-1),
          epsilon=1e-12)
...
class EmbeddingLookup(tf.keras.layers.Layer):
    __doc__ = 'Looks up words embeddings for id tensor.'
...
class RelativeMultiheadAttention(tf.keras.layers.Layer):
    __doc__ = 'Multi-head attention with relative embedding.'
    ...
    def build(self, unused_input_shapes):
        ...
        self.output_layer_norm = tf.keras.layers.LayerNormalization(name='LayerNorm',
          axis=(-1),
          epsilon=1e-12)
        self.kh_projection_layer = self.add_weight('k/kernel',
          shape=[
         self.d_model, self.n_head, self.d_head],
          initializer=(self.initializer))
        self.vh_projection_layer = self.add_weight('v/kernel',
          shape=[
         self.d_model, self.n_head, self.d_head],
          initializer=(self.initializer))
        self.kr_projection_layer = self.add_weight('r/kernel',
          shape=[
         self.d_model, self.n_head, self.d_head],
          initializer=(self.initializer))
        self.qh_projection_layer = self.add_weight('q/kernel',
          shape=[
         self.d_model, self.n_head, self.d_head],
          initializer=(self.initializer))
        self.relative_attention_layer = RelativeAttention(dropout_att=(self.dropout_att),
          scale=(self.scale))
        self.proj_o = self.add_weight('o/kernel',
          shape=[
         self.d_model, self.n_head, self.d_head],
          initializer=(self.initializer))
    ...
    def call(self, h, g, r, r_w_bias, r_r_bias, seg_mat, r_s_bias, seg_embed, attn_mask_h, attn_mask_g, mems, target_mapping):
        """Implements call() for the layer."""
...
class TransformerXLModel(tf.keras.layers.Layer):
    __doc__ = 'Defines a Transformer-XL computation graph with additional support for XLNet.'
    ...
    def build(self, unused_input_shapes):
        ...
        if self.untie_r:
            ...
            self.r_r_bias = self.add_weight('r_r_bias',
             ...
             self.n_layer, self.n_head, self.d_head],
              dtype=(self.tf_float),
              initializer=(self.initializer))
            self.r_s_bias = self.add_weight('r_s_bias',
              shape=[
             self.n_layer, self.n_head, self.d_head],
              dtype=(self.tf_float),
              initializer=(self.initializer))
            ...
            self.r_r_bias = self.add_weight('r_r_bias',
             ...
             self.n_head, self.d_head],
              ...
              initializer=(self.initializer))
            self.r_s_bias = self.add_weight('r_s_bias',
              [self.n_head, self.d_head], dtype=(self.tf_float),
              initializer=(self.initializer))
        self.seg_embed = self.add_weight('seg_embed',
          [self.n_layer, 2, self.n_head, self.d_head], dtype=(self.tf_float),
          initializer=(self.initializer))
        self.mask_emb = self.add_weight('mask_emb/mask_emb',
          shape=[1, 1, self.d_model], dtype=(self.tf_float))
        ...
        for i in range(self.n_layer):
            self.rel_multihead_layers.append(RelativeMultiheadAttention(d_model=(self.d_model),
            ...
            self.h_positionwise_ffn_layers.append(PositionwiseFF(d_model=(self.d_model),
    ...
    def call(self, inputs):
        ...
        if input_mask is not None and perm_mask is not None:
            ...
            if input_mask is not None and perm_mask is None:
        ...
        if self.bi_data:
            ...
            if dtype is not None:
                ...
                for i in range(self.n_layer):
                    ...
                    output_h, output_g = attention_layer(h=output_h,
                      g=output_g,
                      r=pos_emb,
...
class PretrainingXLNetModel(tf.keras.Model):
    ...
    def __init__(self, use_proj, xlnet_config, run_config, **kwargs):
        ...
        self.transformerxl_model = TransformerXLModel(n_token=(self.xlnet_config.n_token),
          initializer=(self.initializer),
          attn_type='bi',
    ...
    def call(self, features):
        ...
        transformerxl_output, self.new_mems, self.lookup_table = self.transformerxl_model(input_ids,
          seg_id=seg_ids,
          input_mask=None,
          mems=mems,
          perm_mask=perm_mask,
          target_mapping=target_mapping,
          inp_q=inp_q)
        lm_loss, _ = self.lmloss_layer(hidden=transformerxl_output,
          target=target,
...
class ClassificationXLNetModel(tf.keras.Model):
    __doc__ = 'XLNet keras model combined with classification loss layer.\n\n  See the original paper: https://arxiv.org/pdf/1906.08237.pdf\n\n  '
    ...
    def __init__(self, xlnet_config, run_config, n_class, summary_type, **kwargs):
        ...
        self.transformerxl_model = TransformerXLModel(n_token=(self.xlnet_config.n_token),
          ...
          same_length=(self.run_config.same_length),
          name='transformer')
        self.summarization_layer = Summarization(d_model=(self.xlnet_config.d_model),
          n_head=(self.xlnet_config.n_head),
          d_head=(self.xlnet_config.d_head),
          dropout=(self.run_config.dropout),
          dropout_att=(self.run_config.dropout_att),
        ...
        self.cl_loss_layer = ClassificationLossLayer(n_class=n_class,
          initializer=(self.initializer),
          name='classification')
    ...
    def call(self, features):
        ...
        mems = features.get('mems', None)
        transformerxl_output, new_mems, self.lookup_table = self.transformerxl_model(input_ids, seg_ids, input_mask, mems)
...
class LMLossLayer(tf.keras.layers.Layer):
    __doc__ = 'Layer computing cross entropy loss for language modeling.'
    ...
    def build(self, unused_input_shapes):
        if self.use_proj:
            ...
            self.proj_layer_norm = tf.keras.layers.LayerNormalization(axis=(-1),
              epsilon=1e-12,
              name='lm_projection/LayerNorm')
        if not self.tie_weight:
            self.softmax_w = self.add_weight('weight',
              shape=[
             self.n_token, self.d_model],
              initializer=(self.initializer))
        self.softmax_b = self.add_weight('bias',
          shape=[self.n_token], initializer=(tf.zeros_initializer()))
...
class Summarization(tf.keras.layers.Layer):
    __doc__ = 'The layer to pool the output from XLNet model into a vector.'
...
class ClassificationLossLayer(tf.keras.layers.Layer):
    __doc__ = 'Layer computing cross entropy loss for classification task.'
    ...
    def build(self, unused_input_shapes):
        self.proj_layer = tf.keras.layers.Dense(units=(self.n_class),
          kernel_initializer=(self.initializer),
          name='logit')
...
class QAXLNetModel(tf.keras.Model):
    __doc__ = 'XLNet keras model combined with question answering loss layer.\n\n  See the original paper: https://arxiv.org/pdf/1906.08237.pdf\n\n  '
    ...
    def __init__(self, xlnet_config, run_config, start_n_top, end_n_top, **kwargs):
        ...
        self.transformerxl_model = TransformerXLModel(n_token=(self.xlnet_config.n_token),
          ...
          same_length=(self.run_config.same_length),
          name='transformer')
        self.qa_loss_layer = QALossLayer(d_model=(self.xlnet_config.d_model),
    ...
    def call(self, features, training=False):
        ...
        transformerxl_output, new_mems, self.lookup_table = self.transformerxl_model(input_ids, seg_ids, input_mask)
        if training:
            loss, logits = self.qa_loss_layer(hidden=transformerxl_output,
              p_mask=p_mask,
              cls_index=cls_index,
            ...
            self.add_loss(loss)
            return (new_mems, logits)
        results = self.qa_loss_layer(hidden=transformerxl_output,
          p_mask=p_mask,
          cls_index=cls_index)
        return results
...
class QALossLayer(tf.keras.layers.Layer):
    __doc__ = 'Layer computing position and regression loss for question answering task.'
    ...
    def build(self, unused_input_shapes):
        self.start_logits_proj_layer = tf.keras.layers.Dense(units=1,
          kernel_initializer=(self.initializer),
          name='start_logits/dense')
        self.end_logits_proj_layer0 = tf.keras.layers.Dense(units=(self.d_model),
          kernel_initializer=(self.initializer),
          activation=(tf.nn.tanh),
          name='end_logits/dense_0')
        self.end_logits_proj_layer1 = tf.keras.layers.Dense(units=1,
          kernel_initializer=(self.initializer),
          name='end_logits/dense_1')
        self.end_logits_layer_norm = tf.keras.layers.LayerNormalization(axis=(-1),
          epsilon=1e-12,
          name='end_logits/LayerNorm')
        self.answer_class_proj_layer0 = tf.keras.layers.Dense(units=(self.d_model),
          kernel_initializer=(self.initializer),
          activation=(tf.nn.tanh),
          name='answer_class/dense_0')
        self.answer_class_proj_layer1 = tf.keras.layers.Dense(units=1,
          kernel_initializer=(self.initializer),
          use_bias=False,
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            start_top_log_probs, start_top_index = tf.nn.top_k(start_log_probs,
              k=(self.start_n_top))
        ...
        regression_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=is_impossible,
          logits=cls_logits)
