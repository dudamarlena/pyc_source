...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/eval_util.py
# Compiled at: 2020-04-05 19:50:57
# Size of source mod 2**32: 43787 bytes
"""Common utility functions for evaluation."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import collections, os, re, time, numpy as np
...
EVAL_METRICS_CLASS_DICT = {'coco_detection_metrics':coco_evaluation.CocoDetectionEvaluator, 
 'coco_mask_metrics':coco_evaluation.CocoMaskEvaluator, 
 'oid_challenge_detection_metrics':object_detection_evaluation.OpenImagesDetectionChallengeEvaluator, 
 'oid_challenge_segmentation_metrics':object_detection_evaluation.OpenImagesInstanceSegmentationChallengeEvaluator, 
 'pascal_voc_detection_metrics':object_detection_evaluation.PascalDetectionEvaluator, 
 'weighted_pascal_voc_detection_metrics':object_detection_evaluation.WeightedPascalDetectionEvaluator, 
 'precision_at_recall_detection_metrics':object_detection_evaluation.PrecisionAtRecallDetectionEvaluator, 
 'pascal_voc_instance_segmentation_metrics':object_detection_evaluation.PascalInstanceSegmentationEvaluator, 
 'weighted_pascal_voc_instance_segmentation_metrics':object_detection_evaluation.WeightedPascalInstanceSegmentationEvaluator, 
...
def visualize_detection_results(result_dict, tag, global_step, categories, summary_dir='', export_dir='', agnostic_mode=False, show_groundtruth=False, groundtruth_box_visualization_color='black', min_score_thresh=0.5, max_num_predictions=20, skip_scores=False, skip_labels=False, keep_image_id_for_visualization_export=False):
  ...
  """
    ...
    if not set([
     ...
     detection_fields.detection_classes]).issubset(set(result_dict.keys())):
        raise ValueError('result_dict does not contain all expected keys.')
    if show_groundtruth:
        if input_fields.groundtruth_boxes not in result_dict:
    ...
    if show_groundtruth:
        ...
        vis_utils.visualize_boxes_and_labels_on_image_array(image=image,
    ...
    summary = tf.Summary(value=[
     tf.Summary.Value(tag=tag,
       image=tf.Summary.Image(encoded_image_string=(vis_utils.encode_image_array_as_png_str(image))))])
    summary_writer = tf.summary.FileWriterCache.get(summary_dir)
    summary_writer.add_summary(summary, global_step)
  ...
  """
    ...
    with tf.contrib.slim.queues.QueueRunners(sess):
        try:
            try:
                for batch in range(int(num_batches)):
                    if (batch + 1) % 100 == 0:
                        ...
                        result_dict, result_losses_dict = batch_processor(tensor_dict,
                          sess, batch, counters, losses_dict=losses_dict)
                    if not result_dict:
                        continue
            ...
            if eval_export_path:
                if eval_export_path is not None:
                    for evaluator in evaluators:
                        if not isinstance(evaluator, coco_evaluation.CocoDetectionEvaluator):
                            if isinstance(evaluator, coco_evaluation.CocoMaskEvaluator):
                                pass
...
def repeated_checkpoint_run(tensor_dict, summary_dir, evaluators, batch_processor=None, checkpoint_dirs=None, variables_to_restore=None, restore_fn=None, num_batches=1, eval_interval_secs=120, max_number_of_evaluations=None, max_evaluation_global_step=None, master='', save_graph=False, save_graph_dir='', losses_dict=None, eval_export_path=None, process_metrics_fn=None):
  ...
  """
    if max_number_of_evaluations:
        ...
        if max_evaluation_global_step:
            if max_evaluation_global_step <= 0:
                raise ValueError('`max_evaluation_global_step` must be either None or positive.')
        assert checkpoint_dirs, '`checkpoint_dirs` must have at least one entry.'
    ...
    while 1:
        ...
        if not model_path:
            ...
            if model_path == last_evaluated_model_path:
                ...
                if max_evaluation_global_step:
                    if global_step >= max_evaluation_global_step:
...
def result_dict_for_single_example(image, key, detections, groundtruth=None, class_agnostic=False, scale_to_absolute=False):
  ...
  """
    ...
    for detection_key in detections:
        detections[detection_key] = tf.expand_dims((detections[detection_key][0]),
          axis=0)

    batched_output_dict = result_dict_for_batched_example(image,
...
def result_dict_for_batched_example(images, keys, detections, groundtruth=None, class_agnostic=False, scale_to_absolute=False, original_image_spatial_shapes=None, true_image_shapes=None, max_gt_boxes=None):
  ...
  """
    ...
    if original_image_spatial_shapes is None:
        original_image_spatial_shapes = tf.tile(tf.expand_dims((tf.shape(images)[1:3]), axis=0),
          multiples=[
         tf.shape(images)[0], 1])
    else:
        if len(original_image_spatial_shapes.shape) != 2:
            if original_image_spatial_shapes.shape[1] != 2:
                raise ValueError('`original_image_spatial_shape` should be a 2D tensor of shape [batch_size, 2].')
    if true_image_shapes is None:
        true_image_shapes = tf.tile(tf.expand_dims((tf.shape(images)[1:4]), axis=0),
          multiples=[
         tf.shape(images)[0], 1])
    else:
        if len(true_image_shapes.shape) != 2:
            if true_image_shapes.shape[1] != 3:
                raise ValueError('`true_image_shapes` should be a 2D tensor of shape [batch_size, 3].')
        else:
            output_dict = {input_data_fields.original_image: images, 
            ...
            if scale_to_absolute:
                output_dict[detection_fields.detection_boxes] = shape_utils.static_or_dynamic_map_fn(_scale_box_to_absolute,
                  elems=[
                 detection_boxes, original_image_spatial_shapes],
                  dtype=(tf.float32))
            else:
            ...
            if detection_fields.detection_masks in detections:
                ...
                output_dict[detection_fields.detection_masks] = shape_utils.static_or_dynamic_map_fn(_resize_detection_masks,
                  elems=[
            ...
            if detection_fields.detection_keypoints in detections:
                ...
                if scale_to_absolute:
                    output_dict[detection_fields.detection_keypoints] = shape_utils.static_or_dynamic_map_fn(_scale_keypoint_to_absolute,
                      elems=[
                     detection_keypoints, original_image_spatial_shapes],
                      dtype=(tf.float32))
        if groundtruth:
            ...
            if input_data_fields.groundtruth_instance_masks in groundtruth:
                ...
                groundtruth[input_data_fields.groundtruth_instance_masks] = shape_utils.static_or_dynamic_map_fn(_resize_groundtruth_masks,
                  elems=[
                 masks, original_image_spatial_shapes],
                  dtype=(tf.uint8))
            ...
            groundtruth_boxes = shape_utils.static_or_dynamic_map_fn(_scale_box_to_normalized_true_image,
              elems=[
             groundtruth_boxes, true_image_shapes],
              dtype=(tf.float32))
            ...
            if scale_to_absolute:
                ...
                output_dict[input_data_fields.groundtruth_boxes] = shape_utils.static_or_dynamic_map_fn(_scale_box_to_absolute,
                  elems=[
                 groundtruth_boxes, original_image_spatial_shapes],
                  dtype=(tf.float32))
            if class_agnostic:
