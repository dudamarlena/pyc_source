...
# [GCC 8.4.0]
# Embedded file name: /Users/svpino/dev/tensorflow-object-detection-sagemaker/todl/tensorflow-object-detection/research/object_detection/builders/preprocessor_builder.py
...
PREPROCESSING_FUNCTION_MAP = {'normalize_image':preprocessor.normalize_image, 
 ...
 'random_jpeg_quality':preprocessor.random_jpeg_quality, 
 'random_downscale_to_target_pixels':preprocessor.random_downscale_to_target_pixels, 
 'random_patch_gaussian':preprocessor.random_patch_gaussian, 
 'rgb_to_gray':preprocessor.rgb_to_gray, 
 'scale_boxes_to_pixel_coordinates':preprocessor.scale_boxes_to_pixel_coordinates, 
 'subtract_channel_mean':preprocessor.subtract_channel_mean, 
 'convert_class_logits_to_softmax':preprocessor.convert_class_logits_to_softmax}
RESIZE_METHOD_MAP = {preprocessor_pb2.ResizeImage.AREA: tf.image.ResizeMethod.AREA, 
 ...
 preprocessor_pb2.ResizeImage.NEAREST_NEIGHBOR: tf.image.ResizeMethod.NEAREST_NEIGHBOR}
  ...
  """
    ...
    if step_type == 'random_pad_image':
        ...
        if config.HasField('min_image_height'):
            min_image_size = (
             config.min_image_height, config.min_image_width)
        max_image_size = None
        ...
        if config.HasField('max_image_height'):
            max_image_size = (
             config.max_image_height, config.max_image_width)
    ...
    if step_type == 'random_crop_pad_image':
        ...
        if pad_color:
            if len(pad_color) != 3:
                tf.logging.warn('pad_color should have 3 elements (RGB) if set!')
            pad_color = tf.cast([x for x in config.pad_color], dtype=(tf.float32))
        kwargs = {'min_object_covered':config.min_object_covered, 
         'aspect_ratio_range':(
          config.min_aspect_ratio,
          config.max_aspect_ratio), 
         'area_range':(
        ...
        return (
         preprocessor.random_crop_pad_image, kwargs)
    ...
    if step_type == 'ssd_random_crop_pad_fixed_aspect_ratio':
        ...
        return (
         preprocessor.ssd_random_crop_pad_fixed_aspect_ratio, kwargs)
