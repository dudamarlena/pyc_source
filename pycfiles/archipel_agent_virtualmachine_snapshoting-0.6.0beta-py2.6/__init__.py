...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvirtualmachinesnapshoting/__init__.py
...
def make_archipel_plugin(configuration, entity, group):
    ...
    """
    return [
     {'info': snapshoting.TNSnapshoting.plugin_info(), 'plugin': snapshoting.TNSnapshoting(configuration, entity, group)}]
...
def version():
    ...
    return (
     __name__, pkg_resources.get_distribution('archipel-agent-virtualmachine-snapshoting').version, [snapshoting.TNSnapshoting.plugin_info()])