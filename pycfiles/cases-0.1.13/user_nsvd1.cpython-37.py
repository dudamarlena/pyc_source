...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/user_nsvd1.py
# Compiled at: 2019-08-20 09:47:28
# Size of source mod 2**32: 10321 bytes
...
class UserNSVD1(BaseNSVD1):
    ...
    def __init__(self, train_file=None, test_file=None, metadata_file=None, output_file=None, epochs=30, learn_rate=0.01, delta=0.015, factors=10, init_mean=0, init_stdev=0.1, stop_criteria=0.001, batch=False, n2=10, learn_rate2=0.01, delta2=0.015, sep='\t', output_sep='\t', metadata_sep='\t', metadata_as_binary=False, random_seed=None):
        """
        ...
        """
        super(UserNSVD1, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, factors=factors,
          init_mean=init_mean,
    ...
    def init_model(self):
        ...
        self.non_zero_x = []
        self.d = []
        self.metadata = ReadFile((self.metadata_file), sep=(self.metadata_sep), as_binary=(self.metadata_as_binary)).read_metadata_or_similarity()
        ...
        sparsity = (1 - self.metadata['number_interactions'] / (len(self.metadata['col_1']) * len(self.metadata['col_2']))) * 100
                ...
                for _ in range(self.n2):
                    for u, user in enumerate(self.users):
                        ...
                        for l in self.non_zero_x[u]:
                            self.w[l] += self.learn_rate2 * (self.d[u] * np.dot(self.x[u][l], e.T) - self.w[l] * self.delta2)
