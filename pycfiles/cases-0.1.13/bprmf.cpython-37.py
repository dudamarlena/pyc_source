...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/item_recommendation/bprmf.py
...
class BprMF(BaseItemRecommendation):
    ...
    def __init__(self, train_file=None, test_file=None, output_file=None, factors=10, learn_rate=0.05, epochs=30, batch_size=0, rank_length=10, init_mean=0, init_stdev=0.1, reg_u=0.0025, reg_i=0.0025, reg_j=0.00025, reg_bias=0, sep='\t', output_sep='\t', random_seed=None, items_test=False):
        ...
        """
        super(BprMF, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, rank_length=rank_length,
          sep=sep,
    ...
    def init_model(self):
        ...
        if self.items_test:
            for u, user in enumerate(self.users):
                self.train_set['items_unobserved'][user] = list(set(self.items) - set(self.train_set['items_seen_by_user'][user]))
    ...
    def sample_pair(self, user):
        ...
        """
        return (
         random.choice(list(self.train_set['items_seen_by_user'][user])),
         random.choice(self.train_set['items_unobserved'][user]))
