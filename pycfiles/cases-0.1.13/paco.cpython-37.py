...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/clustering/paco.py
...
class PaCo(object):
    ...
    def __init__(self, train_file, k_row=None, l_col=None, density_low=0.008, as_binary=True, sep='\t', random_seed=None):
        """
        ...
        self.train_set = ReadFile(train_file,
          as_binary=as_binary, sep=sep).read()
    ...
    def update_information(self, first_iteration=False):
        ...
        if first_iteration:
            ...
            self.density = np.matrix(np.divide(self.count_ones, self.count_total))
            self.density[self.density < self.density_low] = 0.0
    ...
    def merge(self, min_value_row, min_value_col):
        ...
        if min_value_row[0] > min_value_col[0]:
            ...
            self.count_total = np.delete((self.count_total),
              (pair[0], pair[1]), axis=1)
            self.count_total = np.insert((self.count_total),
              (self.count_total.shape[1]), (new_count_total.T), axis=1)
            new_count_ones = self.count_ones[:,
             pair[0]] + self.count_ones[:, pair[1]]
            self.count_ones = np.delete((self.count_ones),
              (pair[0], pair[1]), axis=1)
            self.count_ones = np.insert((self.count_ones),
              (self.count_ones.shape[1]), (new_count_ones.T), axis=1)
        else:
            ...
            self.count_total = np.delete((self.count_total),
              (pair[0], pair[1]), axis=0)
            self.count_total = np.insert((self.count_total),
              (self.count_total.shape[0]), new_count_total, axis=0)
            new_count_ones = self.count_ones[pair[0], :] + self.count_ones[pair[1], :]
            self.count_ones = np.delete((self.count_ones),
              (pair[0], pair[1]), axis=0)
            self.count_ones = np.insert((self.count_ones),
              (self.count_ones.shape[0]), new_count_ones, axis=0)
    ...
    def fit(self):
        ...
        while criteria:
            ...
            if len(self.list_row) == 1:
                if len(self.list_col) == 1:
                    break
    ...
    def compute(self, verbose=True):
        ...
        if verbose:
            ...
            print('Number of bi-groups needing recommendations:: ', self.density[np.logical_and(self.density != 1, self.density != 0)].size)
        else:
            self.fit()