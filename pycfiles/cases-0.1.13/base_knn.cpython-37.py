...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/base_knn.py
...
class BaseKNN(BaseRatingPrediction):
    ...
    def __init__(self, train_file, test_file, output_file=None, reg_bi=10, reg_bu=15, similarity_metric='cosine', sep='\t', output_sep='\t'):
        """
        ...
        """
        super(BaseKNN, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, similarity_metric=similarity_metric,
          sep=sep,
    ...
    def compute_bi(self):
        ...
        for item in self.items:
            ...
            for user in self.train_set['users_viewed_item'].get(item, []):
                self.bi[item] = self.bi.get(item, 0) + float(self.train_set['feedback'][user].get(item, 0)) - self.train_set['mean_value'] - self.bu.get(user, 0)
                count += 1
    ...
    def compute_bu(self):
        ...
        for user in self.users:
            ...
            for item in self.train_set['items_seen_by_user'].get(user, []):
                self.bu[user] = self.bu.get(user, 0) + float(self.train_set['feedback'][user].get(item, 0)) - self.train_set['mean_value'] - self.bi.get(item, 0)
                count += 1
    ...
    def compute_bui(self):
        ...
        for user in self.users:
            for item in self.items:
                self.bui.setdefault(user, {}).update({item: self.train_set['mean_value'] + self.bu.get(user, 0) + self.bi.get(item, 0)})
