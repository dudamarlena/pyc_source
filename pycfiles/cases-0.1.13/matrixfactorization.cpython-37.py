...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/matrixfactorization.py
...
class MatrixFactorization(BaseRatingPrediction):
    ...
    def __init__(self, train_file=None, test_file=None, output_file=None, factors=10, learn_rate=0.01, epochs=30, delta=0.015, init_mean=0.1, init_stdev=0.1, baseline=False, bias_learn_rate=0.005, delta_bias=0.002, stop_criteria=0.009, sep='\t', output_sep='\t', random_seed=None):
        ...
        """
        super(MatrixFactorization, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, sep=sep,
            ...
            for user in self.test_set['users']:
                for item in self.test_set['feedback'][user]:
                    self.predictions.append((user, item,
                     self._predict_score(self.user_to_user_id[user], self.item_to_item_id[item], True)))
