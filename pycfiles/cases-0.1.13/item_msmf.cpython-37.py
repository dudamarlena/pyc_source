...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/item_msmf.py
...
class ItemMSMF(MatrixFactorization):
    ...
    def __init__(self, train_file=None, test_file=None, output_file=None, similarity_file=None, neighbors=20, factors=10, learn_rate=0.01, epochs=30, delta=0.015, init_mean=0.1, init_stdev=0.1, baseline=True, bias_learn_rate=0.005, delta_bias=0.002, stop_criteria=0.009, sep='\t', output_sep='\t', similarity_sep='\t', random_seed=None, verbose=True):
        super(ItemMSMF, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, sep=sep, learn_rate=learn_rate,
          factors=factors,
          epochs=epochs,
          delta=delta,
          init_mean=init_mean,
    ...
    def search_similar_items(self, item):
        ...
        for i, sim in list_similar:
            if i != item_index and self.item_id_to_item[i] in self.train_set['items']:
