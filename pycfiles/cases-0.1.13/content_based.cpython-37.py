...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/item_recommendation/content_based.py
...
class ContentBased(BaseItemRecommendation):
    ...
    def __init__(self, train_file=None, test_file=None, output_file=None, similarity_file=None, similarity_sep='\t', rank_length=10, as_binary=True, sep='\t', output_sep='\t'):
        """
        ...
        """
        super(ContentBased, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, as_binary=as_binary,
    ...
    def init_model(self):
        ...
        if self.similarity_file is not None:
            similarity = ReadFile((self.similarity_file), sep=(self.similarity_sep), as_binary=False).read_metadata_or_similarity()
    ...
    def predict_item_score(self, user, item):
        ...
        return [
         user, item, sum_sim / len(self.users_profile[user])]
