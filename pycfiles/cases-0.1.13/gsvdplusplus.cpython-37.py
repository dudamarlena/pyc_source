...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/gsvdplusplus.py
# Compiled at: 2019-08-20 09:47:28
# Size of source mod 2**32: 11160 bytes
...
class GSVDPlusPlus(MatrixFactorization):
    ...
    def __init__(self, train_file=None, test_file=None, output_file=None, metadata_file=None, factors=10, learn_rate=0.01, epochs=10, delta=0.015, init_mean=0.1, init_stdev=0.1, bias_learn_rate=0.005, delta_bias=0.002, stop_criteria=0.009, sep='\t', sep_metadata='\t', output_sep='\t', random_seed=None, update_delta=False):
        """
        ...
        """
        super(GSVDPlusPlus, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, factors=factors,
        ...
        self.meta_dict_category, self.meta_list_item_category, self.meta_set_items, self.meta_dict_item_category = ReadFile(metadata_file, sep_metadata).read_item_category()
        self.g_to_gid = {}
        self.gid_to_g = {}
    ...
    def init_model(self):
        ...
        for item in self.items:
            ...
            if item in self.train_set['items']:
                self.n_i[self.item_to_item_id[item]] = np.sqrt(len(self.users_id_seen_by_item[self.item_to_item_id[item]]))
            else:
            ...
            for user in self.test_set['users']:
                ...
                for item in self.test_set['feedback'][user]:
                    ...
                    self.predictions.append((
                     user, item,
                     self._predict_gsvd(self.user_to_user_id[user], self.item_to_item_id[item], pu, pi, True)))
