...
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/item_recommendation/group_based_recommender.py
...
import caserec.clustering.kmedoids as kmedoids
...
class GroupBasedRecommender(BaseItemRecommendation):
    ...
    def __init__(self, train_files=None, test_file=None, output_file=None, similarity_metric='cosine', rank_length=10, k_groups=3, recommender='UserKNN', as_binary=False, sep='\t', output_sep='\t', max_int_kmedoids=1000, parser='', user_weights=False):
        """
        ...
        """
        super(GroupBasedRecommender, self).__init__(train_file='', test_file=test_file, output_file=output_file,
          as_binary=as_binary,
    ...
    def run_kmedoids(self):
        set_train_tuple = []
        support_matrix, clusters = kmedoids((self.distance_matrix), (self.k_groups), max_interactions=(self.max_int_kmedoids),
          random_seed=123)
    ...
    def generate_recommendation(self):
        ...
        for n, train_file in enumerate(self.gb_train_files):
            if self.recommender == 'UserKNN':
                rec = UserKNN(train_file=train_file, similarity_metric=(self.similarity_metric), as_binary=True,
                ...
                rec = ItemKNN(train_file=train_file, test_file=(self.test_file), similarity_metric=(self.similarity_metric),
                  as_binary=True)
                rec.compute(verbose=False, verbose_evaluation=False)
                self.ranking += rec.ranking
