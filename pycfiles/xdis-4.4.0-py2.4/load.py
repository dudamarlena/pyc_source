...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xdis/load.py
# Compiled at: 2020-04-20 10:25:46
import marshal, py_compile, sys, tempfile, time
from struct import unpack, pack
import os.path as osp, xdis.unmarshal
from xdis.version_info import PYTHON3, PYTHON_VERSION
from xdis.magics import IS_PYPY3, PYTHON_MAGIC_INT, int2magic, magic_int2float, magic2int, magicint2version, versions
...
def load_module_from_file_object(fp, filename='<unknown>', code_objects=None, fast_load=False, get_code=True):
    ...
    try:
        ...
        try:
            ...
            if len(magic) >= 2:
                raise ImportError('Unknown magic number %s in %s' % (ord(magic[0:1]) + 256 * ord(magic[1:2]), filename))
            else:
                raise ImportError("Bad magic number: '%s'" % magic)

        if magic_int in (3010, 3020, 3030, 3040, 3050, 3060, 3061, 3071, 3361, 3091,
                         3101, 3103, 3141, 3270, 3280, 3290, 3300, 3320, 3330, 3371,
                         62071, 62071, 62081, 62091, 62092, 62111):
            raise ImportError('%s is interim Python %s (%d) bytecode which is not supported.\nFinal released versions are supported.' % (filename, versions[magic], magic2int(magic)))
        elif magic_int == 62135:
            fp.seek(0)
            return fix_dropbox_pyc(fp)
        elif magic_int == 62215:
            raise ImportError('%s is a dropbox-hacked Python %s (bytecode %d).\nSee https://github.com/kholia/dedrop for how to decrypt.' % (filename, versions[magic], magic2int(magic)))
        try:
            ...
            if version >= 3.7:
                ...
                if pep_bits & 1 or magic_int == 3393:
                    sip_hash = unpack('<Q', fp.read(8))[0]
                else:
                    timestamp = unpack('<I', fp.read(4))[0]
                    source_size = unpack('<I', fp.read(4))[0]
            else:
                timestamp = unpack('<I', ts)[0]
                if 3200 <= magic_int < 20121 and version >= 1.5 or magic_int in IS_PYPY3:
                    source_size = unpack('<I', fp.read(4))[0]
            if get_code:
