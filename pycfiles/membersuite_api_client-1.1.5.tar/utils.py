...
# [GCC 8.4.0]
# Embedded file name: /usr/local/var/pyenv/versions/hub/lib/python2.7/site-packages/membersuite_api_client/utils.py
...
def membersuite_object_factory(membersuite_object_data):
    ...
    klasses = {'Individual': security_models.Individual, 'PortalUser': security_models.PortalUser}
    try:
        klass = klasses[membersuite_object_data['ClassType']]
    except KeyError:
        return MemberSuiteObject(membersuite_object_data=membersuite_object_data)
...
def get_new_client(request_session=False):
    ...
    client = ConciergeClient(access_key=os.environ['MS_ACCESS_KEY'], secret_key=os.environ['MS_SECRET_KEY'], association_id=os.environ['MS_ASSOCIATION_ID'])
    if request_session:
...
def value_for_key(membersuite_object_data, key):
    ...
    """
    key_value_dicts = {d['Key']:d['Value'] for d in membersuite_object_data['Fields']['KeyValueOfstringanyType']}
