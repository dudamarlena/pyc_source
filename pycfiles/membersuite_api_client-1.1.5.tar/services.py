...
# [GCC 8.4.0]
# Embedded file name: /usr/local/var/pyenv/versions/hub/lib/python2.7/site-packages/membersuite_api_client/security/services.py
...
def login_to_portal(username, password, client, retries=2, delay=0):
    ...
    while attempts < retries:
        if attempts:
            time.sleep(delay)
        result = client.client.service.LoginToPortal(_soapheaders=[
         concierge_request_header], portalUserName=username, portalPassword=password)
        ...
        if login_to_portal_result['Success']:
            ...
            return PortalUser(membersuite_object_data=portal_user, session_id=session_id)
        attempts += 1
        try:
            error_code = login_to_portal_result['Errors']['ConciergeError'][0]['Code']
        except IndexError:
            continue
        else:
...
def logout(client):
    ...
    if not client.session_id:
        client.request_session()
    concierge_request_header = client.construct_concierge_header(url='http://membersuite.com/contracts/IConciergeAPIService/Logout')
    logout_result = client.client.service.Logout(_soapheaders=[
     concierge_request_header])
    result = logout_result['body']['LogoutResult']
    if result['SessionID'] is None:
...
def get_user_for_membersuite_entity(membersuite_entity):
    ...
    if not user:
        try:
            user = User.objects.filter(email=membersuite_entity.email_address)[0]
        except IndexError:
            pass

    if not user:
        user = User.objects.create(username=user_username, email=membersuite_entity.email_address, first_name=membersuite_entity.first_name, last_name=membersuite_entity.last_name)
        user_created = True
