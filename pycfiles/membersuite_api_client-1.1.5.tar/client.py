...
# [GCC 8.4.0]
# Embedded file name: /usr/local/var/pyenv/versions/hub/lib/python2.7/site-packages/membersuite_api_client/client.py
...
class ConciergeClient(object):
    ...
    def request_session(self):
        ...
        """
        concierge_request_header = self.construct_concierge_header(url='http://membersuite.com/contracts/IConciergeAPIService/WhoAmI')
        result = self.client.service.WhoAmI(_soapheaders=[
         concierge_request_header])
        self.session_id = get_session_id(result=result)
        if not self.session_id:
            raise MembersuiteLoginError(result['body']['WhoAmIResult']['Errors'])
    ...
    def construct_concierge_header(self, url):
        ...
        """
        concierge_request_header = etree.Element(etree.QName(XHTML_NAMESPACE, 'ConciergeRequestHeader'), nsmap={'sch': XHTML_NAMESPACE})
        if self.session_id:
            session = etree.SubElement(concierge_request_header, etree.QName(XHTML_NAMESPACE, 'SessionId'))
            session.text = self.session_id
        access_key = etree.SubElement(concierge_request_header, etree.QName(XHTML_NAMESPACE, 'AccessKeyId'))
        access_key.text = self.access_key
        association_id = etree.SubElement(concierge_request_header, etree.QName(XHTML_NAMESPACE, 'AssociationId'))
        association_id.text = self.association_id
        signature = etree.SubElement(concierge_request_header, etree.QName(XHTML_NAMESPACE, 'Signature'))
        signature.text = self.get_hashed_signature(url=url)
        return concierge_request_header

    def execute_object_query(self, object_query, start_record=0, limit_to=400):
        concierge_request_header = self.construct_concierge_header(url='http://membersuite.com/contracts/IConciergeAPIService/ExecuteMSQL')
        result = self.client.service.ExecuteMSQL(_soapheaders=[
         concierge_request_header], msqlStatement=object_query, startRecord=start_record, maximumNumberOfRecordsToReturn=limit_to)
        return result