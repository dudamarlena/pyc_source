...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokerengine/engine/data_encoder.py
...
class DataEncoder:
    ...
    def encode_seats(self, seats):
        return {'seats': [ self.encode_player(player) for player in seats.players ]}

    @classmethod
    def encode_pot(self, players):
        pots = GameEvaluator.create_pot(players)
        main = {'amount': pots[0]['amount']}
        gen_hsh = lambda sidepot: {'amount': sidepot['amount'], 'eligibles': [ p.uuid for p in sidepot['eligibles'] ]}
        side = [ gen_hsh(sidepot) for sidepot in pots[1:] ]
        return {'main': main, 'side': side}

    @classmethod
    def encode_game_information(self, config, seats):
        hsh = {'player_num': len(seats.players), 
           'rule': {'initial_stack': config['initial_stack'], 
    ...
    def encode_valid_actions(self, call_amount, min_bet_amount, max_bet_amount):
        return {'valid_actions': [{'action': 'fold', 'amount': 0}, {'action': 'call', 'amount': call_amount}, {'action': 'raise', 'amount': {'min': min_bet_amount, 'max': max_bet_amount}}]}

    @classmethod
    def encode_action(self, player, action, amount):
        return {'player_uuid': player.uuid, 
           'action': action, 
           'amount': amount}

    @classmethod
    def encode_street(self, street):
        return {'street': self.__street_to_str(street)}
    ...
    def encode_round_state(self, state):
        hsh = {'street': self.__street_to_str(state['street']), 
           'pot': self.encode_pot(state['table'].seats.players), 
           'community_card': [ str(card) for card in state['table'].get_community_card() ], 'dealer_btn': state['table'].dealer_btn, 
