...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokerengine/engine/player.py
...
class Player:
    ...
    def paid_sum(self):
        pay_history = [ h for h in self.action_histories if h['action'] not in ('FOLD',
                                                                                'ANTE') ]
        last_pay_history = pay_history[(-1)] if len(pay_history) != 0 else None
        if last_pay_history:
            return last_pay_history['amount']
        else:
            return 0
    ...
    def __ante_history(self, pay_amount):
        assert pay_amount > 0
        return {'action': self.ACTION_ANTE, 
           'amount': pay_amount}
