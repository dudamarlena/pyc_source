...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/examples/players/emulator_player.py
...
class EmulatorPlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        for action in try_actions:
            ...
            log('average stack after simulation when declares %s : %s' % ({0: 'FOLD', 1: 'CALL', 2: 'MIN_RAISE', 3: 'MAX_RAISE'}[action], action_results[action]))
...
class MyModel(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if self.CALL == self.action:
            return (valid_actions[1]['action'], valid_actions[1]['amount'])
        if self.MIN_RAISE == self.action:
            return (valid_actions[2]['action'], valid_actions[2]['amount']['min'])
        if self.MAX_RAISE == self.action:
