...
# [GCC 8.4.0]
# Embedded file name: /home/chris/git/alphabuyer/.env/local/lib/python2.7/site-packages/ajax_forms/templatetags/daf_help.py
# Compiled at: 2017-07-05 11:32:54
import re
...
def sort_link(request, param_name, field_name, default='', label=''):
    ...
    if not order_by:
        ...
        return '<a class="sort-link" href="?%(query_string)s">%(name_visible)s %(icon)s</a>' % dict(icon=icon, name_visible=label or field_name.title().replace('_', ' '), query_string=new_path)
...
def daf_result_list(cl):
    ...
    return {'cl': cl, 'result_hidden_fields': list(result_hidden_fields(cl)), 
...
def paginator_number(cl, i):
    ...
    return format_html('<li><a href="{0}"{1}>{2}</a></li> ', cl.get_query_string({PAGE_VAR: i}), mark_safe(' class="end"' if i == cl.paginator.num_pages - 1 else ''), i + 1)
