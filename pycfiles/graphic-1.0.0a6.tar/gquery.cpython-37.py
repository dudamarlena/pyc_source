...
# [GCC 8.4.0]
# Embedded file name: e:\kanzhun\projects\graph\graphic\src\graphic\gquery.py
# Compiled at: 2018-11-04 22:09:03
...
class Context:
    ...
    def add_ref(self, name, val):
        if name in self._name_2_ent:
            raise KeyError('{} has already to use to ref to {}'.format(name, self._name_2_ent.get(name).__repr__()))
...
class GQuery:
    _LIMIT = 20
    __slots__ = ('_where', '_entities', '_context', '_select', '_limit', '_order_by')
