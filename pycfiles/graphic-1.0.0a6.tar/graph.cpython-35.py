...
# [GCC 8.4.0]
# Embedded file name: e:\kanzhun\projects\graph\graphic\src\graphic\graph.py
# Compiled at: 2018-11-13 05:13:18
# Size of source mod 2**32: 9854 bytes
...
class RemoteGraph(with_metaclass(RemoteGraphMeta)):
    ...
    def __new__(cls, engine, *args, **config):
        ...
        if target_cls is None:
            raise AttributeError('Not surpport engine for {}, please make sure the target engine can be import'.format(engine))
...
class GraphEntity(Mapping, Hashable):
    ...
    def __subclasshook__(cls, C):
        if cls is GraphEntity:
            return check_methods(C, 'is_node', 'is_path', 'is_edge')
...
class Relationship(GraphEntity):
    __slots__ = GraphEntity.__slots__ + ('_node_from', '_node_to', '_type', '_with_direction')

    def __init__(self, node_from, node_to, type=None, with_direction=True, **properties):
        super().__init__(**properties)
...
class Path(GraphEntity):
    __slots__ = GraphEntity.__slots__ + ('_start_node', '_end_node', '_min_len', '_max_len')
...
class SubGraph:
    __doc__ = 'Arbitrary, unordered collection of nodes and relationships.'
    ...
    def __or__(self, other):
        return type(self)(nodes=self.nodes | other.nodes, relationships=self.relationships | other.relationships)
