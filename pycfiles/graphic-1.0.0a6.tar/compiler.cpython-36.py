...
# [GCC 8.4.0]
# Embedded file name: e:\kanzhun\projects\graph\graphic\src\graphic\query\cypher\compiler.py
# Compiled at: 2018-11-05 00:42:31
    ...
    def compile_q(q):
        if isinstance(q, Func):
            ...
            if len(q) > 1:
                ...
                return '({})'.format(join_by.join([compile_q(child) for child in q.children]))
...
def compile_match_clause(*entities):
    ...
    return ' '.join([hydrage_entity(entity) for entity in entities])
...
def _build_node(node):
    key_vals = ','.join(['{}:{}'.format(key, _encode_cypher_value(val)) for key, val in node])
    if len(key_vals) == 0:
        return 'CREATE ({}:{})'.format(node.alias, ':'.join(node.labels))
    else:
        return 'CREATE ({}:{} {{{}}})'.format(node.alias, ':'.join(node.labels), key_vals)


def _build_relationship(relationship):
    if relationship.type is None:
        raise KeyError('New relationship must with type')
    key_vals = ' '.join(['{}:{}'.format(key, _encode_cypher_value(val)) for key, val in relationship])
    if len(key_vals) == 0:
        return 'CREATE ({})-[{}:{}]->({})'.format(relationship.node_from.alias, relationship.alias, relationship.type, relationship.node_to.alias)
    else:
        return 'CREATE ({})-[{}:{} {{{}}}]->({})'.format(relationship.node_from.alias, relationship.alias, relationship.type, key_vals, relationship.node_to.alias)
...
def build(iterable_nodes, iterable_relationships=[]) -> Iterable:
    ...
    create_parts.extend([_build_relationship(rel) for rel in relationships_set])
    return '\n'.join(create_parts)