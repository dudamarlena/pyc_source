...
# [GCC 8.4.0]
# Embedded file name: e:\kanzhun\projects\graph\graphic\src\graphic\query\func.py
# Compiled at: 2018-11-05 01:19:50
...
class Func:
    ...
    def __call__(self, lookup_field=None) -> str:
        ...
        return '{}{}{!r}'.format(_field, self.exp, self._val)
    ...
    def __repr__(self):
        return '{}({}, {!r})'.format(self.__class__.__name__, self._field, self._val)


__funcs__ = (('eq', '='), ('gt', '>'), ('lt', '<'), ('gte', '>='), ('lte', '<='), ('neq', '<>'),
             ('in', ' IN '), ('startswith', ' STARTS WITH '), ('endswith', ' ENDS WITH '),
             ('contains', ' CONTAINS '))
for name, exp in __funcs__:
    _ = type(name, (Func,), {'__slots__': ('_field', '_val'), 
     'exp': exp})
...
class Aggregation:
    ...
    def __call__(self, lookup_field=None) -> str:
        ...
        if lookup_field is not None:
            _field = lookup_field(_field)
        return '{}({})'.format(self.__class__.__name__, _field)
    ...
    def __repr__(self):
        return '{}({})'.format(self.__class__.__name__, self._field)


__aggregations__ = ('avg', 'min', 'max', 'sum', 'count')
for name in __aggregations__:
    _ = type(name, (Aggregation,), {'__slots__': ('_field', )})
