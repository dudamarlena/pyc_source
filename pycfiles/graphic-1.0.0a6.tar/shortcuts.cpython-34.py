...
# [GCC 8.4.0]
# Embedded file name: e:\kanzhun\projects\graph\graphic\src\graphic\shortcuts.py
# Compiled at: 2018-11-13 05:29:42
...
def relationship(node_from, node_to, type=None,
    ...
    """
    return Relationship(node_from, node_to, type=type, with_direction=with_direction, **properties)


def link(node_from, node_to, type=None, **properties) -> Relationship:
    """Create new relationship without derection of two nodes
    """
    return relationship(node_from, node_to, type=type, with_direction=False, **properties)
...
def use(engine='graphic.engine.neo4j', **config) -> RemoteGraph:
    ...
    try:
        ...
        warnings.warn('can not import engine({}) module'.format(engine), ImportWarning)
