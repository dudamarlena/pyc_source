...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/arf_tools/src/classifiers/decision_tree.py
# Compiled at: 2018-04-13 18:36:04
...
def p_log_p(freq):
    r""" fonction pour calculer \sum p_i log(p_i) """
...
class Split(object):
    __doc__ = ' Permet de coder un split pour une variable continue. '
    ...
    def predict(self, data):
        """ Prediction pour une matrice d'exemples, -1 si <= threshold, +1
        sinon.
    ...
    @staticmethod
    def best_gain(x, y):
        """  calcul le meilleur seuil pour la colonne x (1-dimension) et les
        labels y.
        ...
        for i in range(y.size):
            if x[idx_sorted[i]] != xlast:
                ...
                if htmp < hmin:
                    hmin = htmp
                    split_val = (xlast + x[idx_sorted[i]]) / 2.0
                xlast = x[idx_sorted[i]]

        return (
         h - hmin / ylen, split_val)
    ...
    def __str__(self):
        return 'var %s, thresh %f (gain %f)' % (self.idvar,
         self.threshold,
         self.gain)


class Node(Classifier):
    __doc__ = " Noeud d'un arbre. "
    ...
    def predict(self, data):
        ...
        return [self.left.predict(data[i, :])[0] if res < 0 else self.right.predict(data[i, :])[0] for i, res in enumerate(self.split.predict(data))]
...
class DecisionTree(Classifier):
    __doc__ = ' Arbre de decision. '

    def __init__(self, max_depth=None, min_samples_split=2):
        """
        :param max_depth: profondeur max
        :param min_samples_split:  nombre d'exemples minimal pour pouvoir
        spliter le noeud
        :return:
        """
    ...
    def fit(self, data, y):
        ...
        on apprend un noeud, puis on cree les deux enfants de ce noeud, que
        l'on ajoute a la pile des noeuds a traiter par la suite
        (nodes_to_treat), ainsi que les index des exemples associes (dic_idx)
        """
        ...
        while len(nodes_to_treat) > 0:
            ...
            if entropy(y[idx_train]) == 0 or curnode.depth >= self.max_depth or len(l_idx) < self.min_samples_split or len(r_idx) < self.min_samples_split:
                curnode.leaf = True
                continue
                ...
                nodes_to_treat = [
                 curnode.left, curnode.right] + nodes_to_treat
    ...
    def to_dot(self, dic_var=None):
        ...
        while len(nodes) > 0:
            ...
            if not curnode.leaf:
                s += '%d [label="%s <= %f\n IG=%f\n ' % (idx,
                 curnode.split.idvar if not dic_var else dic_var[curnode.split.idvar], curnode.split.threshold, curnode.split.gain)
                s += ' %s \n ",shape="box" ];\n' % (labinfo,)
                lidx = cpt + 1
                ridx = cpt + 2
                s += '%d -> %d; %d -> %d;\n' % (idx, lidx, idx, ridx)
