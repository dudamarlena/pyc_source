...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/arf_tools/src/classifiers/utils/gradient_descent.py
# Compiled at: 2018-04-10 17:29:35
...
class UnknownDescenteDeGradient(Exception):
    ...
    def __init__(self, descente: str):
        Exception.__init__(self, 'La descente de gradient : "{descente}" n\'existe pas'.format(descente=descente))


def gradient_descent(datax, datay, type_descente, w_init, loss_g, max_iter=10000, pas=0.01, batch_size=None):
    ...
    if type_descente == DescenteDeGradient.MINI_BATCH:
        return mini_batch(datax, datay, w_init, max_iter, pas, loss_g, batch_size)
    if type_descente == DescenteDeGradient.STOCHASTIQUE:
...
def mini_batch(datax, datay, w_init, max_iter, pas, loss_g, batch_size):
...
def gradient_descent_h(datax, datay, w_init, loss, loss_g, max_iter=10000, pas=0.01):
    """ Effectue une descente de gradient pour optimiser la valeur d'un
    vecteur de poids d'un perceptron.
    ...
    calculées.
    """
    w_histo, f_histo, grad_histo = w_init, np.array([loss(datax, datay, w_init)]), np.array([loss_g(datax, datay, w_init)])
    for _ in range(max_iter):
...
def optimize_h(fonc, dfonc, x_init, pas=0.001, max_iter=1000):
    ...
    calculées.
    """
    x_histo, f_histo, grad_histo = np.array([x_init]), np.array([fonc(*x_init)]), np.array([np.array(dfonc(*x_init))])
    for _ in range(max_iter):
