...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/arf_tools/src/parse_data.py
# Compiled at: 2018-04-13 18:33:09
...
def load_imdb(path=PATH_DATA_IMDB, encoding='latin1', threshold_good_rating=6.5, label_good_rating=1, label_bad_rating=-1):
    with open(path, 'rb') as (file):
        data, id2titles, fields = pickle.load(file, encoding=encoding)
    datax = data[:, :32]
    datay = np.array([label_good_rating if x[33] > threshold_good_rating else label_bad_rating for x in data])
    return (
     datax, datay, fields)
...
def load_usps(class_neg, class_pos, filename_train=PATH_DATA_USPS_TRAIN, filename_test=PATH_DATA_USPS_TEST, label_neg=0, label_pos=1):
...
def load_usps_1_vs_all(class_pos, filename_train=PATH_DATA_USPS_TRAIN, filename_test=PATH_DATA_USPS_TEST, label_neg=-1, label_pos=1):
    with open(filename_train, 'r') as (f):
        f.readline()
        data = [[float(x) for x in l.split()] for l in f if len(l.split()) > 2]
    tmp = np.array(data)
