...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/arf_tools/src/classifiers/perceptron.py
# Compiled at: 2018-04-13 18:35:40
# Size of source mod 2**32: 5387 bytes
""" Ce module contient toutes les fonctions nécessaires pour utiliser un
perceptron (un réseau de une neurone).
"""
...
class UnknownInitialisation(Exception):
    ...
    def __init__(self, initialisation: str):
        Exception.__init__(self, 'L\'initialisation : "{initialisation}" n\'existe pas'.format(initialisation=initialisation))


class Perceptron:

    def __init__(self, loss=hinge, loss_g=hinge_g, max_iter=10000, eps=0.01, biais=True, activation=np.sign, type_descente=DescenteDeGradient.MINI_BATCH, taille_batch=50, initialisation=Initialisation.RANDOM, projection=identite):
    ...
    def fit(self, datax, datay):
        ...
        self.w = gradient_descent(datax, datay, type_descente=self.type_descente, w_init=self.w, loss_g=self.loss_g, max_iter=self.max_iter, pas=self.eps, batch_size=self.taille_batch)
    ...
    def fit_h(self, datax, datay):
        ...
        if self.biais:
            datax = np.c_[(datax, np.ones(len(datax)))]
        self._init_w(taille=datax.shape[1])
        w_histo, f_histo, grad_histo = gradient_descent_h(datax, datay, w_init=self.w, loss=self.loss, loss_g=self.loss_g, max_iter=self.max_iter, pas=self.eps)
        self.w = w_histo[(-1)]
    ...
    def score(self, datax, datay):
        return np.array([np.equal(self.predict(x), y) for x, y in zip(datax, datay)]).sum() / len(datax)
    ...
    def _init_w(self, taille):
        ...
        if self.initialisation == Initialisation.ZERO:
            ...
            if self.initialisation == Initialisation.RANDOM:
