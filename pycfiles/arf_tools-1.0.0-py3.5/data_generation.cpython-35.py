...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/arf_tools/src/data_generation.py
...
def gen_arti(centerx=1, centery=1, sigma=0.1, nbex=1000, data_type=0, epsilon=0.02):
    ...
    if data_type == 1:
        xpos = np.vstack((
         np.random.multivariate_normal([centerx, centerx], np.diag([
          sigma, sigma]), nbex // 4),
         np.random.multivariate_normal([-centerx, -centerx], np.diag([
          sigma, sigma]), nbex // 4)))
        xneg = np.vstack((
...
def make_grid(data=None, xmin=-5, xmax=5, ymin=-5, ymax=5, step=20):
    ...
    if data is not None:
        xmax, xmin, ymax, ymin = (
         np.max(data[:, 0]),
         np.min(data[:, 0]), np.max(data[:, 1]), np.min(data[:, 1]))
    x, y = np.meshgrid(np.arange(xmin, xmax, (xmax - xmin) * 1.0 / step), np.arange(ymin, ymax, (ymax - ymin) * 1.0 / step))
