...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spatial_access/NetworkInterface.py
...
class NetworkInterface:
    ...
    def __init__(self, network_type, logger=None, disable_area_threshold=False):
        ...
        self.nodes = None
        self.edges = None
        self.area_threshold = None if disable_area_threshold else 5000
    ...
    def _get_bbox(self, primary_data, secondary_data, secondary_input, epsilon):
        """
        ...
        """
        if secondary_input:
            composite_lon = list(primary_data['lon']) + list(secondary_data['lon'])
            composite_lat = list(primary_data['lat']) + list(secondary_data['lat'])
        else:
    ...
    def load_network(self, primary_data, secondary_data, secondary_input, epsilon):
        """
        ...
        if not isinstance(primary_data, pd.DataFrame):
        ...
        elif not isinstance(secondary_data, pd.DataFrame):
            assert secondary_data is None
            ...
            if not isinstance(epsilon, float):
                if not isinstance(epsilon, int):
                    raise AssertionError
            self._try_create_cache()
            self._get_bbox(primary_data, secondary_data, secondary_input, epsilon)
    ...
    def _request_network(self):
        ...
        try:
            if self.network_type == 'bike':
                ...
                self.nodes, self.edges = network_from_bbox(lat_min=(self.bbox[0]), lng_min=(self.bbox[1]), lat_max=(self.bbox[2]),
                  lng_max=(self.bbox[3]),
                  custom_osm_filter=osm_bike_filter)
            else:
                self.nodes, self.edges = network_from_bbox(lat_min=(self.bbox[0]),
                  lng_min=(self.bbox[1]),
                  lat_max=(self.bbox[2]),
                  lng_max=(self.bbox[3]),
    ...
    def _remove_disconnected_components(self):
        ...
        if self.logger:
            ...
            self.logger.debug('Removed {}/{} ({:,.2f}%) edges and {}/{} ({:,.2f}%) nodes which were disconnected components in {:,.2f} seconds'.format(edges_diff, len_edges_before, edges_diff_percent, nodes_diff, len_nodes_before, nodes_diff_percent, time_diff))