...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spatial_access/BaseModel.py
# Compiled at: 2019-06-09 10:59:34
# Size of source mod 2**32: 37856 bytes
import pandas as pd, geopandas as gpd
from shapely.geometry import Point
import matplotlib as mpl, matplotlib.patches as mpatches, matplotlib.pyplot, json
...
class ModelData:
    ...
    def __init__(self, network_type, sources_filename, destinations_filename, source_column_names=None, dest_column_names=None, configs=None, debug=False):
        """
        Args:
    ...
    def load_transit_matrix(self, read_from_file=None):
        ...
        """
        if read_from_file:
            self.transit_matrix = TransitMatrix((self.network_type), read_from_file=read_from_file,
              debug=(self.debug))
        else:
            self.transit_matrix = TransitMatrix((self.network_type), primary_input=(self.sources_filename),
        ...
        if filename:
            ...
            if self.source_column_names is None:
                ...
                self.source_column_names = {'lat':lat, 
                 'lon':lon,  'idx':idx,  'population':population}
            try:
        ...
        if filename:
            ...
            if self.dest_column_names is None:
                ...
                self.dest_column_names = {'lat':lat, 
                 'lon':lon,  'idx':idx,  'category':category, 
                 'capacity':capacity}
            try:
    ...
    def count_dests_in_range_by_categories(self, source_id, category, upper_threshold):
        ...
        """
        if category == 'all_categories':
            return self.transit_matrix.matrix_interface.count_dests_in_range(source_id, upper_threshold, None)
        else:
            return self.transit_matrix.matrix_interface.count_dests_in_range(source_id, upper_threshold, category)
    ...
    def _spatial_join_community_index(self, dataframe, shapefile='data/chicago_boundaries/chicago_boundaries.shp', spatial_index='community', projection='epsg:4326'):
        """
        ...
        geo_result = gpd.sjoin(boundaries_gdf, geo_original, how='right', op='intersects')
    ...
    def _build_aggregate(self, data_frame, aggregation_args, shapefile, spatial_index, projection):
        ...
        if 'lat' not in data_frame.columns or 'lon' not in data_frame.columns or 'spatial_index' not in data_frame.columns:
            data_frame = self._spatial_join_community_index(dataframe=data_frame, shapefile=shapefile,
    ...
    def aggregate(self, aggregation_type=None, shapefile='data/chicago_boundaries/chicago_boundaries.shp', spatial_index='community', projection='epsg:4326'):
        """
        ...
        if self.model_results is None:
            ...
            if self._requires_user_aggregation_type:
                if aggregation_type is None:
                    ...
                    if aggregation_type not in frozenset({'max', 'min', 'mean'}):
                        raise UnexpectedAggregationTypeException(aggregation_type)
                    else:
                        self._aggregation_args = {}
        ...
        self.aggregated_results = self._build_aggregate(data_frame=results_with_coordinates, aggregation_args=(self._aggregation_args),
          shapefile=shapefile,
          spatial_index=spatial_index,
          projection=projection)
    ...
    def write_aggregated_results(self, filename=None, output_type='csv'):
        ...
        if filename is not None:
            ...
            filename = self._get_output_filename(keyword='aggregate', extension=output_type,
              file_path='data/')
    ...
    def write_results(self, filename=None):
        ...
        if self.model_results is None:
            raise ModelNotCalculatedException()
        if filename is None:
            filename = self._get_output_filename(keyword='model', extension='csv',
              file_path='data/')
        self.model_results.to_csv(filename)

    @staticmethod
    def _join_aggregated_data_with_boundaries(aggregated_results, spatial_index, shapefile='data/chicago_boundaries/chicago_boundaries.shp'):
        """
        Args:
        ...
        results = boundaries_gdf.merge(aggregated_results, left_on=spatial_index, right_index=True,
          how='outer')
        results.fillna(value=0, inplace=True)
        return results[columns_to_keep]

    def plot_cdf(self, plot_type=None, xlabel='xlabel', ylabel='ylabel', title='title', bins=100, is_density=False, filename=None):
        """
        Args:
        ...
        ax.grid(zorder=0)
        available_colors = [
         'black', 'magenta', 'lime', 'red', 'black', 'orange', 'grey', 'yellow', 'brown', 'teal']
        color_keys = []
        for column in cdf_eligible.columns:
            if plot_type not in column:
                ...
                n, bins, blah = ax.hist(x, bins, density=is_density, histtype='step', cumulative=True,
                  label=column,
                  color=color,
                  zorder=3)
        ...
        if filename is None:
            filename = self._get_output_filename(keyword='figure', extension='png', file_path='figures/')
        mpl.pyplot.savefig(filename, dpi=400)
        self.logger.info('Plot was saved to: {}'.format(filename))

    def plot_choropleth(self, column, include_destinations=True, title='Title', color_map='Greens', shapefile='data/chicago_boundaries/chicago_boundaries.shp', spatial_index='community', filename=None):
        """
        Args:
        ...
        if self.aggregated_results is None:
            ...
            if include_destinations:
                ...
                categories = None
            results_with_geometry = self._join_aggregated_data_with_boundaries(aggregated_results=(self.aggregated_results), spatial_index=spatial_index,
              shapefile=shapefile)
            ...
            if categories is not None:
                available_colors = [
                 'magenta', 'lime', 'red', 'black', 'orange', 'grey', 'yellow', 'brown', 'teal']
                ...
                for category in categories:
                    ...
                    mpl.pyplot.scatter(y=(dest_subset['lat']), x=(dest_subset['lon']), color=color, marker='o', s=(50 * (dest_subset['capacity'] / max_dest_capacity)),
                      label=category)
                    if not monochrome:
