...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spatial_access/p2p.py
# Compiled at: 2019-06-09 10:59:34
# Size of source mod 2**32: 19706 bytes
import time, logging, os, scipy.spatial
...
class TransitMatrix:
    ...
    def __init__(self, network_type, primary_input=None, secondary_input=None, read_from_file=None, primary_hints=None, secondary_hints=None, debug=False, configs=None):
        """
        Args:
        ...
        if isinstance(configs, Configs):
            ...
            self.configs = Configs()
        self._network_interface = NetworkInterface(network_type, logger=(self.logger), disable_area_threshold=(self.configs.disable_area_threshold))
        self.matrix_interface = MatrixInterface(logger=(self.logger), require_extended_range=(self.configs.require_extended_range))
        if network_type not in frozenset({'bike', 'walk', 'otp', 'drive'}):
            raise UnknownModeException(network_type)
        if self.primary_input == self.secondary_input:
    ...
    def _get_type_of_series(series):
        ...
        if type(series[0]) == str:
            return str
        if issubdtype(series.dtype, integer) or issubdtype(series.dtype, signedinteger):
            return integer
    ...
    def _parse_csv(self, primary):
        ...
        if not skip_user_input:
            ...
            self.logger.debug('Total number of {} in the dataset: {}'.format(keyword, pre_drop))
            if dropped_lines > 0:
                self.logger.warning('Rows dropped due to missing latitude or longitude values: %d', dropped_lines)
            if primary:
    ...
    def _parse_network(self):
        ...
        if self.configs.use_meters:
            ...
            if self.network_type == 'walk':
                edges['edge_weight'] = edges['distance'] / self.configs._get_walk_speed() + self.configs.walk_node_penalty
            else:
                if self.network_type == 'bike':
                    edges['edge_weight'] = edges['distance'] / self.configs._get_bike_speed() + self.configs.bike_node_penalty
                else:
        ...
        self.matrix_interface.add_edges_to_graph(from_column, to_column, edge_weight_column, is_bidirectional_column)
    ...
    def _match_to_nearest_neighbor(self, is_primary=True, is_also_secondary=False):
        ...
        for row in data.itertuples():
            ...
            if is_primary:
                self.matrix_interface.add_user_source_data(network_id=node_loc, user_id=origin_id,
                  weight=edge_weight,
                  is_also_dest=is_also_secondary)
            else:
                self.matrix_interface.add_user_dest_data(network_id=node_loc, user_id=origin_id,
                  weight=edge_weight)

        time_delta = time.time() - start_time
        self.logger.debug('Nearest Neighbor matching completed in {:,.2f} seconds'.format(time_delta))
    ...
    def prefetch_network(self):
        ...
        """
        self._load_inputs()
        self.logger.debug('Fetching network (%s) with epsilon: %f', self.network_type, self.configs.epsilon)
        self._network_interface.load_network(self.primary_data, self.secondary_data, self.secondary_input is not None, self.configs.epsilon)
    ...
    def _is_compressible(self):
        ...
        """
        return self._is_symmetric() and self.network_type in frozenset({'bike', 'walk'})
    ...
    def process(self):
        ...
        """
        if not self.network_type != 'otp':
            raise AssertionError('no need to call process for an otp matrix')
        else:
            start_time = time.time()
            ...
            self.matrix_interface.prepare_matrix(is_symmetric=(self._is_symmetric()), is_compressible=(self._is_compressible()),
              rows=rows,
              columns=cols,
