...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spatial_access/MatrixInterface.py
# Compiled at: 2019-06-09 10:59:34
# Size of source mod 2**32: 17815 bytes
import multiprocessing, time, os, csv
...
class MatrixInterface:
    ...
    def __init__(self, logger=None, require_extended_range=False):
        ...
        self._parser = None
        self._map_id_type_enum_to_is_string_boolean = {0:False, 
         1:True}
        self._map_value_type_enum_to_is_extended_boolean = {0:False, 
         1:True}
    ...
    def _read_tmx(self, filename):
        ...
        self.primary_ids_are_string = self._map_id_type_enum_to_is_string_boolean[tmx_type_reader.get_row_type_enum()]
        self.secondary_ids_are_string = self._map_id_type_enum_to_is_string_boolean[tmx_type_reader.get_col_type_enum()]
        self.is_extended = self._map_value_type_enum_to_is_extended_boolean[tmx_type_reader.get_value_type_enum()]
        self._load_parser()
    ...
    def add_edges_to_graph(self, from_column, to_column, edge_weight_column, is_bidirectional_column):
        """
        Args:
    ...
    def get_values_by_source(self, source_id, sort=False):
        ...
        """
        return self._parser.decode_vector_of_dest_tuples(self.transit_matrix.getValuesBySource(self._parser.encode_source_id(source_id), sort))
    ...
    def get_values_by_dest(self, dest_id, sort=False):
        ...
        """
        return self._parser.decode_vector_of_source_tuples(self.transit_matrix.getValuesByDest(self._parser.encode_dest_id(dest_id), sort))
    ...
    def add_user_source_data(self, network_id, user_id, weight, is_also_dest):
        ...
        """
        self.transit_matrix.addToUserSourceDataContainer(network_id, self._parser.encode_source_id(user_id), weight)
        if is_also_dest:
            self.transit_matrix.addToUserDestDataContainer(network_id, self._parser.encode_dest_id(user_id), weight)
    ...
    def _load_parser(self):
        ...
        """
        if self.primary_ids_are_string:
            if self.secondary_ids_are_string:
                self._parser = StringStringParser
        if self.primary_ids_are_string and not self.secondary_ids_are_string:
        ...
        elif not self.primary_ids_are_string:
            if not self.secondary_ids_are_string:
        ...
        if self.is_extended:
            if self.primary_ids_are_string:
                ...
                if self.primary_ids_are_string:
                    ...
                    if not self.primary_ids_are_string:
                        if self.secondary_ids_are_string:
                            return _p2pExtension.pyTransitMatrixIxSxUI
                        if not self.primary_ids_are_string and not self.secondary_ids_are_string:
                            return _p2pExtension.pyTransitMatrixIxIxUI
        elif self.primary_ids_are_string:
            if self.secondary_ids_are_string:
    ...
    def build_matrix(self):
        ...
        if self.logger:
            self.logger.debug('Shortest path matrix computed in {:,.2f} seconds'.format(logger_vars))
    ...
    def _get_value_by_id(self, source_id, dest_id):
        ...
        """
        try:
            return self.transit_matrix.getValueById(self._parser.encode_source_id(source_id), self._parser.encode_dest_id(dest_id))
        except BaseException:
    ...
    def add_to_category_map(self, dest_id, category):
        ...
        transit matrix.
        """
        self.transit_matrix.addToCategoryMap(self._parser.encode_dest_id(dest_id), self._parser.encode_category(category))
    ...
    def time_to_nearest_dest(self, source_id, category=None):
        ...
        if category is None:
            ...
            return self.transit_matrix.timeToNearestDestPerCategory(self._parser.encode_source_id(source_id), self._parser.encode_category(category))
    ...
    def count_dests_in_range(self, source_id, threshold, category=None):
        ...
        """
        if category is None:
            return self.transit_matrix.countDestsInRange(self._parser.encode_source_id(source_id), threshold)
        else:
            return self.transit_matrix.countDestsInRangePerCategory(self._parser.encode_source_id(source_id), self._parser.encode_category(category), threshold)

    def _set_mock_data_frame(self, dataset, source_ids, dest_ids):
        """
        Warning: Not for use in production.
        """
        self.transit_matrix.setMockDataFrame(dataset, self._parser.encode_vector_source_ids(source_ids), self._parser.encode_vector_dest_ids(dest_ids))