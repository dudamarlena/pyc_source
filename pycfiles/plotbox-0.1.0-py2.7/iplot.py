...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/plotbox/iplot.py
# Compiled at: 2015-12-03 14:40:05
...
def plotly_plot(x=None, y=None, df=None, style='scatter', title=None, filename='plotly_plot_test', xlabel=None, ylabel=None, vlines=None, hlines=None, xlim=(None, None), ylim=(None, None), dropna=True, dateextractX=False, dateextractY=False, figsize=(16, 9), plotlyplot=True, saveAs=None, **kwargs):
    """
...
def plotly_date_frequency_plot(df=None, array=None, series=None, title=None, filename='plotly_date_frequency_plot_test', weekend_bool=True, weekend_height=20, xlabel=None, ylabel=None, vlines=None, hlines=None, xlim=None, ylim=None, **kwargs):
    ...
    if weekend_bool:
        for i in weekend_tuples:
            plt.plot([i[0], i[1], i[0], i[1]], [0.1, weekend_height,
             weekend_height, 0.1], alpha=0.6, color='grey')
...
def histogram(df, title, xlabel='', ylabel='', colorscale='Portland'):
    data = Data([
     Heatmap(z=list(np.array(df)), colorscale=colorscale, x=df.index, y=df.columns)])
    layout = Layout(title=title, xaxis=XAxis(title=xlabel), yaxis=YAxis(title=ylabel))
    fig = Figure(data=data, layout=layout)
    plot_url = py.plot(fig, filename=title)
    return plot_url


def hkr(all_hist_data, hist_labels, rug_text, bin_size=0.2, curve_type='kde', show_hist=True, show_curve=True, show_rug=True, **kwargs):
    """
    Plots multiple
    """
    ...
    for index in range(blnk_length):
        ...
        start[index] = min(all_hist_data[index]) * 1.0
        end[index] = max(all_hist_data[index]) * 1.0

    def make_hists(all_hist_data, bin_size, start, end, hist_labels, colors):
        hist = [None] * blnk_length
        for index in range(blnk_length):
            hist[index] = Histogram(x=hist_x[index], histnorm='probability', name=hist_labels[index], marker=Marker(color=colors[index]), autobinx=False, xbins=XBins(start=start[index], end=end[index], size=bin_size), opacity=0.7)
    ...
    def make_kde(hist_x, start, end, hist_labels, colors, bin_size):
        ...
        for index in range(blnk_length):
            curve[index] = Scatter(x=curve_x[index], y=curve_y[index], mode='lines', name=hist_labels[index], marker=Marker(color=colors[index]))
    ...
    def make_rug(hist_x, rug_text, colors):
        rug = [
         None] * blnk_length
        for index in range(blnk_length):
            rug[index] = Scatter(x=hist_x[index], y=[
             hist_labels[index]] * len(hist_x[0]), text=rug_text, mode='markers', name=hist_labels[index], marker=Marker(color=colors[index], symbol='line-ns-open'))
    ...
    if show_rug == True:
        ...
        fig['layout']['yaxis1']['domain'] = [0.35, 1]
        fig['layout']['yaxis2']['domain'] = [0, 0.25]
    else:
