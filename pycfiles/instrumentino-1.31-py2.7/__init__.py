...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/instrumentino/__init__.py
...
from instrumentino.action import SysAction, SysActionParamTime, SysActionParamInt
from instrumentino.controllers.arduino import Arduino, SysVarAnalogArduinoUnipolar, SysVarAnalogArduinoBipolarWithExternalPolarity, SysVarDigitalArduino
...
class InstrumentinoApp(wx.App):
    ...
    def OnLogUpdate(self, event):
        ...
        cfg.Log(text)
        if critical:
            dlg = wx.MessageDialog(self.mainFrame, text, 'Error', wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnPopMessage(self, event):
        ...
        wx.Bell()
        if waitForUser:
            dlg = wx.MessageDialog(self.mainFrame, text + '\nResume operation?', 'Waiting for user', wx.OK | wx.ICON_QUESTION)
        else:
            dlg = wx.MessageDialog(self.mainFrame, text, 'Notification', wx.OK | wx.ICON_QUESTION)
        dlg.ShowModal()
        dlg.Destroy()
        e.set()
    ...
    def loadFile(self, event, message, wildcard, listCtrl):
        ...
        """
        dlg = wx.FileDialog(self.mainFrame, message=message, defaultDir=cfg.UserFilesPath(), defaultFile='', wildcard=wildcard, style=wx.OPEN | wx.CHANGE_DIR)
            ...
            try:
                ...
                with open(filePath, 'r') as (fp):
                    ...
                    if savedFile.systemUid == self.system.GetSystemUid():
                        ...
                        errDlg = wx.MessageDialog(self.mainFrame, 'File incompatible with this system', 'Error', wx.OK | wx.ICON_ERROR)
                        errDlg.ShowModal()
                        errDlg.Destroy()
            except (IOError, EOFError):
                errDlg = wx.MessageDialog(self.mainFrame, 'Error loading the file', 'Error', wx.OK | wx.ICON_ERROR)
                errDlg.ShowModal()
                errDlg.Destroy()
    ...
    def saveFile(self, event, message, wildcard, extension, listCtrl):
        ...
        """
        dlg = wx.FileDialog(self.mainFrame, message=message, defaultDir=cfg.UserFilesPath(), defaultFile='', wildcard=wildcard, style=wx.SAVE)
        if dlg.ShowModal() == wx.ID_OK:
            try:
                path = dlg.GetPath()
                ...
                errDlg = wx.MessageDialog(self.mainFrame, 'Error saving the file', 'Error', wx.OK | wx.ICON_ERROR)
                errDlg.ShowModal()
                errDlg.Destroy()
    ...
    def OnClose(self, event):
        ...
        """
        dlg = wx.MessageDialog(self.mainFrame, 'Do you really want to Close this application?', 'Confirm Exit', wx.OK | wx.CANCEL | wx.ICON_QUESTION)
    ...
    def OnComm(self, controllerName, connectionSetupFunc):
        ...
        """
        dlg = wx.SingleChoiceDialog(self.mainFrame, 'Select the serial port', 'Connect ' + controllerName, SerialUtil().getSerialPortsList(), wx.CHOICEDLG_STYLE)
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnStopButton(self, evt):
        ...
        """
        cfg.userStopped = True
        dlg = wx.MessageDialog(self.mainFrame, 'Operation stopped!', 'Notification', wx.OK | wx.ICON_INFORMATION)
        result = dlg.ShowModal()
        dlg.Destroy()
    ...
    def OnAbout(self, evt):
        ...
        info.Description = wx.lib.wordwrap.wordwrap(self.system.description + '\r\nThis software is based on the instrumentino framework.', 350, wx.ClientDC(self.mainFrame))
        info.WebSite = 'http://www.chemie.unibas.ch/~hauser/open-source-lab/instrumentino/index.html'
        info.Developers = ['Joel Koenka']
        info.License = wx.lib.wordwrap.wordwrap('This software is released under GPLv3. The code is hosted on GitHub: https://github.com/yoelk/instrumentino\n' + 'When using Instrumentino for scientific publications, please cite the release article: http://www.sciencedirect.com/science/article/pii/S0010465514002112', 500, wx.ClientDC(self.mainFrame))
        wx.AboutBox(info)
