...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/instrumentino/action.py
...
class SysActionParamInt(SysActionParam):
    ...
    def SetTextCtrl(self, parent):
        ...
        """
        self.maskedTextCtrl = wx.lib.masked.NumCtrl(parent, -1, self.value, integerWidth=cfg.numIntegerPartWidth, fractionWidth=0, min=self.range[0], max=self.range[1])
...
class SysActionParamFloat(SysActionParam):
    ...
    def SetTextCtrl(self, parent):
        ...
        Set the text controls for the parameter's value
        """
        self.maskedTextCtrl = wx.lib.masked.NumCtrl(parent, -1, self.value, integerWidth=cfg.numIntegerPartWidth, fractionWidth=cfg.numFractionPartWidth, min=self.range[0], max=self.range[1])
        self.maskedTextCtrl.Bind(wx.EVT_KEY_UP, self.onKeyPress)
        return self.maskedTextCtrl
...
class SysActionParamTime(SysActionParam):
    ...
    def SetTextCtrl(self, parent):
        ...
        Set the text controls for the parameter's value
        """
        self.maskedTextCtrl = wx.lib.masked.TextCtrl(parent, -1, '', mask='##:##:##.###', formatcodes='DF!', defaultValue=str(self.value))
        self.maskedTextCtrl.Bind(wx.EVT_KEY_UP, self.onKeyPress)
        return self.maskedTextCtrl
...
class SysAction(RunnableItem):
    ...
    def __repr__(self):
        ...
        """
        retVal = 'sysAction:\rname: ' + self.name + '\rparams:'
