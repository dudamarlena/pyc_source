...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/instrumentino/comp.py
...
import wx
from wx import xrc
import wx.lib.masked as masked, time
...
class SysVarAnalog(SysVar):
    ...
    def CreatePanel(self, parent):
        ...
        self.monitorTextCtrl = masked.NumCtrl(panel, value=0, integerWidth=cfg.numIntegerPartWidth, fractionWidth=cfg.numFractionPartWidth, min=self.range[0], max=self.range[1], style=wx.TE_CENTRE | wx.TE_READONLY)
        sizer.Add(self.monitorTextCtrl)
        if self.editable:
            sizer.Add(wx.StaticText(panel, label='Set to:'))
            editTextCtrl = masked.NumCtrl(panel, value=0, integerWidth=cfg.numIntegerPartWidth, fractionWidth=cfg.numFractionPartWidth, min=self.range[0], max=self.range[1], style=wx.TE_CENTRE | wx.WANTS_CHARS)
