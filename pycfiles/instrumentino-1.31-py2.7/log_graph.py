...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/instrumentino/log_graph.py
...
from matplotlib.dates import DateFormatter, MinuteLocator, SecondLocator, AutoDateFormatter, AutoDateLocator
...
class LogGraphPanel(wx.Panel):
            ...
            for var in comp.vars.values():
                ...
                if isinstance(var, SysVarAnalog):
                    ...
                    if not self.hasBipolarRange(name):
                        ...
                        self.plottedLines[name] = self.axes.plot(self.time, self.plottedAnalogData[name].data, '-', lw=lineWidth, color=color, label=nameOnLegend, visible=graphVisible)[0]
                    else:
                        self.plottedAnalogData[name + '_POS'] = AnalogData([0, var.range[1]])
                        self.plottedAnalogData[name + '_NEG'] = AnalogData([var.range[0], 0])
                        self.plottedLines[name + '_POS'] = self.axes.plot(self.time, self.plottedAnalogData[(name + '_POS')].data, '-', lw=lineWidth, color=color, label=nameOnLegend, visible=graphVisible)[0]
                        self.plottedLines[name + '_NEG'] = self.axes.plot(self.time, self.plottedAnalogData[(name + '_NEG')].data, '--', lw=lineWidth, color=color, visible=graphVisible)[0]
    ...
    def HideVariableFromLog(self, name):
        if not self.hasBipolarRange(name):
            plottedLines = [
             self.plottedLines[name]]
        else:
            plottedLines = [
             self.plottedLines[(name + '_POS')],
    ...
    def Redraw(self, firstTime=False):
        ...
        if not self.cb_freeze.IsChecked():
            self.axes.set_xbound(lower=self.time[max(0, len(self.time) - int(self.slider_zoom.GetValue() * 60 * cfg.app.updateFrequency))], upper=self.time[(-1)])
