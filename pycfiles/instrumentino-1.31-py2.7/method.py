...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/instrumentino/method.py
...
import pickle, wx.lib.masked as masked
...
class SysMethod(RunnableItem):
    ...
    def setRepeatPanel(self, parent):
        ...
        self.maskedTextCtrl = masked.NumCtrl(panel, value=self.repeat, integerWidth=cfg.numIntegerPartWidth, fractionWidth=0, min=0)
        ...
        if filename != '':
            try:
                ...
                if savedFile.systemUid != cfg.systemUid:
                    filename = ''
                    errDlg = wx.MessageDialog(cfg.mainFrame, 'File incompatible with this system', 'Error', wx.OK | wx.ICON_ERROR)
                    errDlg.ShowModal()
                    errDlg.Destroy()
                    return False
            except (IOError, EOFError):
                errDlg = wx.MessageDialog(cfg.mainFrame, 'Error loading the file', 'Error', wx.OK | wx.ICON_ERROR)
                errDlg.ShowModal()
                errDlg.Destroy()
                return False
...
class ActionsListCtrl(ExecutableListCtrl):
    ...
    def getDefaultDataItem(self):
        ...
        if len(self.availableActions) > 0:
            return copy.deepcopy(self.availableActions[0])
        else:
            return
