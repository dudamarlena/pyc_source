...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\base.py
...
booltoolbar.actionSubtract_with_coefficient = QtGui.QAction(QtGui.QIcon('xicam/gui/icons_15.png'), 'Subtract data with coefficient', w)
...
class plugin(QtCore.QObject):
    ...
    def setup(self, placeholders=None):
        ...
        for widget, placeholder in zip([
         self.centerwidget, self.rightwidget, self.bottomwidget, self.toolbar, self.leftwidget], self.placeholders):
...
class EZplugin(plugin):
    ...
    def __init__(self, name='TestPlugin', toolbuttons=[], parameters=[], openfileshandler=None, centerwidget=pg.ImageView, bottomwidget=pg.PlotWidget, panels=[]):
        self.name = name
        ...
        if openfileshandler:
            self.openfiles = openfileshandler
