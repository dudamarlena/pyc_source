...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pipeline\spacegrp_peaks.py
...
def angles_to_pixels(angles, center, sdd, pixel_size=None):
    ...
    if np.NaN in angles:
        ...
        x = tan_2t * sdd
        px = sdd * tan_2t / pixel_size[0]
        py = np.sqrt(sdd ** 2 + x ** 2) * tan_al / pixel_size[1]
...
class peak(object):
    ...
    def position(self, center, sdd, pixels):
        ...
        x = tan_2t * sdd
        self.x = sdd * tan_2t / pixels
        self.y = np.sqrt(sdd ** 2 + x ** 2) * tan_al / pixels

    def isAt(self, pos):
        if np.isnan(self.twotheta):
            return False
        return int(self.x) == int(pos.x()) and int(self.y) == int(pos.y())

    def __str__(self):
        s = ('Peak type: {}\n').format(self.mode)
        s += ('Lattice vector (h,k,l): {}\n').format(self.hkl)
        if self.twotheta is not None:
            s += ('2θ: {}\n').format(self.twotheta)
        if self.alphaf is not None:
            s += ('αf: {}\n').format(self.alphaf)
        if self.qpar is not None:
            s += ('q∥: {}\n').format(self.qpar / 10000000000.0)
        if self.qz is not None:
            s += ('q⟂: {}\n').format(self.qz / 10000000000.0)
        return s
...
def find_peaks(a, b, c, alpha=None, beta=None, gamma=None, normal=None, norm_type='uvw', wavelen=1.23984e-10, refdelta=2.236e-06, refbeta=-1.879e-09, order=5, unitcell=None, space_grp=None):
    ...
    if norm_type == 'xyz':
        ...
        if norm_type == 'hkl':
            if normal.dtype.type is not np.int_:
        ...
        peaks = list()
        for hkl in HKL:
            if not sgexclusions.check(hkl, space_grp):
                continue
...
if __name__ == '__main__':
    ang1 = np.deg2rad(90.0)
    ang2 = np.deg2rad(90.0)
    ang3 = np.deg2rad(90.0)
    n = np.array([0, 0, 1], dtype=float)
    peaks = find_peaks(1e-08, 1e-08, 1e-08, alpha=ang1, beta=ang2, gamma=ang3, normal=n, norm_type='uvw', wavelen=1.23984e-10, order=2)
    trans_peaks = []
    refle_peaks = []
    for key in peaks:
