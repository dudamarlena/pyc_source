...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\tomography\pipelinefunctions.py
...
__status__ = 'Beta'
import numpy as np, numexpr as ne, concurrent.futures as cf, tomopy
from tomopy.util import mproc
import scipy.ndimage.filters as snf, skimage.transform as st
DTYPE_RANGE = {'uint8': (0, 255), 'uint16': (0, 65535), 
   'int8': (-128, 127), 
   'int16': (-32768, 32767), 
   'float32': (-1, 1), 
   'float64': (-1, 1)}
...
def reader(start_sinogram=0, end_sinogram=0, step_sinogram=1, start_projection=0, end_projection=0, step_projection=1, sinograms_per_chunk=0, projections_per_chunk=0):
    """
    ...
    return (
     (
      start_projection, end_projection, step_projection), (start_sinogram, end_sinogram, step_sinogram),
...
if __name__ == '__main__':
    ...
    d = tomopy.read_als_832h5('/home/lbluque/TestDatasetsLocal/dleucopodia.h5', ind_tomo=(500,
                                                                                          501,
                                                                                          502,
                                                                                          503))
    d = d[0]
