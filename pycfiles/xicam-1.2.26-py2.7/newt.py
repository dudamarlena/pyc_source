...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\client\newt.py
...
class NewtClient(User):
    ...
    def get_scratch_dir(self, system):
        ...
        if system == 'edison':
            for i in range(1, 10):
                root_folder = '/scratch' + str(i) + '/scratchdirs/' + self.username
    ...
    def get_home_dir(self, system):
        ...
        home_dir = None
        try:
            root_home = '/global/homes/' + self.username[0] + '/' + self.username
    ...
    def execute_command(self, command, system, bin_path='/bin'):
        ...
        self.check_login()
        self.check_system(system)
        r = self.post(self.BASE_URL + '/command/' + system, data={'executable': bin_path + '/' + command})
        return self.check_response(r)
    ...
    def download_file_generator(self, path, system, save_path=None, chunk_size=65536):
        """
