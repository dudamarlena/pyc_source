...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\widgets\customwidgets.py
...
__status__ = 'Beta'
import numpy as np, pyqtgraph as pg, explorer
    ...
    def mouseMoved(self, ev):
        ...
        try:
            if viewBox.sceneBoundingRect().contains(pos):
                ...
                if (0 <= x < self.imageItem.image.shape[0]) & (0 <= y < self.imageItem.image.shape[1]):
                    self.coordsLabel.setText(("<div style='font-size: 12pt;background-color:#111111;'>x={0},   <span style=''>y={1}</span>,   <span style=''>I={2}</span>").format(x, y, self.imageItem.image[(x, y)]))
                else:
                    self.coordsLabel.setText("<div style='font-size: 12pt;background-color:#111111;'>x= ,   <span style=''>y= </span>,   <span style=''>I= </span>")
...
class histDialogButton(QtGui.QPushButton):
    ...
    def return_vals(self):
        self.value = [
         self.field2.value(), self.field1.value()]
        self.dialog.accept()
