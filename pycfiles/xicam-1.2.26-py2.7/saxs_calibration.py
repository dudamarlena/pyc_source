...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pipeline\saxs_calibration.py
...
def fit_geometry(geometry, maxima, d_spacings, selected_parameter):
    args = [
     geometry, d_spacings, maxima[::-1], selected_parameter]
...
class PyFAIGeometry(pyFAI.geometry.Geometry):
    ...
    def get_fit2d(self):
        ...
        return [self.get_wavelength() * 10000000000.0,
...
def dpdak_saxs_calibration(img, geometry, d_spacings, fit_param, step_size):
    ...
    return (params[1] / 1000.0, [params[2], params[3]])
