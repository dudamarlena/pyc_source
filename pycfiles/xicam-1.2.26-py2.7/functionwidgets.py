...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\tomography\functionwidgets.py
...
class FunctionWidget(fw.FeatureWidget):
    ...
    def __init__(self, name, subname, package, input_functions=None, checkable=True, closeable=True, parent=None):
        ...
        self.params = Parameter.create(name=self.name, children=config.parameters[self.subfunc_name], type='group', readonly=False)
    ...
    def exposed_param_dict(self):
        ...
        """
        param_dict = {key:val for key, val in self.updated_param_dict.iteritems() if key in [ param.name() for param in self.params.children() ]}
        return param_dict
    ...
    def func_signature(self):
        ...
        for param, value in self.param_dict.iteritems():
            signature += (isinstance(value, str) or ('{0}={1},').format)(param, value) if 1 else ("{0}='{1}',").format(param, value)
    ...
    def addInputFunction(self, parameter, functionwidget):
        ...
        """
        if parameter in self.input_functions:
            if functionwidget.subfunc_name == self.input_functions[parameter].subfunc_name:
                raise AttributeError('Input function already exists')
            self.removeInputFunction(parameter)
...
class NormalizeFunctionWidget(FunctionWidget):
    ...
    def __init__(self, name, subname, package, input_functions=None, checkable=True, closeable=True, parent=None):
        super(NormalizeFunctionWidget, self).__init__(name=name, subname=subname, package=package, input_functions=input_functions, checkable=checkable, closeable=closeable, parent=parent)
        if subname == 'ROI':
...
class TomoPyReconFunctionWidget(FunctionWidget):
    ...
    def __init__(self, name, subname, package):
        ...
        self.input_functions = {'theta': AnglesFunctionWidget('Projection Angles', 'Projection Angles', closeable=False, package=reconpkg.packages['tomopy'], checkable=False), 
           'center': FunctionWidget('Center Detection', 'Phase Correlation', closeable=True, package=reconpkg.packages['tomopy'])}
        super(TomoPyReconFunctionWidget, self).__init__(name, subname, package, input_functions=self.input_functions, checkable=False)
        ...
        ui.build_function_menu(self.submenu, config.funcs['Input Functions'][name], config.names, self.addCenterDetectFunction)
...
class AstraReconFuncWidget(TomoPyReconFunctionWidget):
    ...
    def __init__(self, name, subname, package):
        ...
        self.tomopy_args = [
         'center', 'sinogram_order', 'nchunk', 'init_recon', 'tomo', 'theta',
         'ncore', 'algorithm']
...
class MaskFunctionWidget(FunctionWidget):
    ...
    def __init__(self, name, subname, package, checkable=True, closeable=True, parent=None):
        super(MaskFunctionWidget, self).__init__(name, subname, package, checkable=checkable, closeable=closeable, parent=parent)
...
class WriteFunctionWidget(FunctionWidget):
    ...
    def pathChanged(self):
        ...
        """
        self.params.param('fname').setValue(os.path.join(self.parent.value(), self.folder.value(), self.file.value()))

    def updateParamsDict(self):
        """
        Overrides parent method of updateParamsDict to avoid adding the 'Browse' action as a parameter
        """
        self.param_dict.update({param.name():param.value() for param in self.params.children() if param.name() != 'Browse' if param.name() != 'Browse'})
        for p, ipf in self.input_functions.iteritems():
            ipf.updateParamsDict()
...
class CORSelectionWidget(QtGui.QWidget):
    cor_detection_funcs = [
     'Phase Correlation', 'Vo', 'Nelder-Mead']
    ...
    def __init__(self, subname='Phase Correlation', parent=None):
        ...
        self.function = FunctionWidget(name='Center Detection', subname=subname, package=reconpkg.packages[config.names[subname][1]])
        self.params = pg.parametertree.Parameter.create(name=self.function.name, children=config.parameters[self.function.subfunc_name], type='group')
