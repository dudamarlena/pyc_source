...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\client\spot.py
...
class SpotClient(NewtClient):
    ...
    def search(self, query, **kwargs):
        ...
        """
        self.check_login()
        allowed_kwargs = {'sortterm': ['fs.stage_date', 'appmetadata.sdate'], 'sorttype': [
                      'desc', 'asc'], 
           'end_station': [
                         'bl832', 'bl733']}
        ...
        for key in kwargs:
            if key not in allowed_kwargs.keys() and key not in generic_kwargs:
                raise ValueError('%s keyword not in allowed keywords %s' % (
                 key,
                 list(allowed_kwargs.keys() + generic_kwargs)))
    ...
    def get_image_as(self, dataset, stage, ext='tif', image=None, index=None):
        ...
        url = r['pnglocaion'] if ext == 'png' else r['tiflocaion']
        r = self.get(url)
    ...
    def download_dataset_generator(self, dataset, stage, save_path=None, chunk_size=65536):
        """
