...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pipeline\detectors.py
...
class PhotonicScience(pyFAI.detectors.Detector):
    ...
    force_pixel = True
    MAX_SHAPE = (1042, 1042)
    DEFAULT_PIXEL1 = DEFAULT_PIXEL2 = 0.0002

    def __init__(self, pixel1=0.0002, pixel2=0.0002):
        ...
        if pixel1 != self.DEFAULT_PIXEL1 or pixel2 != self.DEFAULT_PIXEL2:
            self._binning = (
             int(2 * pixel1 / self.DEFAULT_PIXEL1), int(2 * pixel2 / self.DEFAULT_PIXEL2))
...
class Maxipix_BM05(pyFAI.detectors.Maxipix5x1):
    ...
    def calc_mask(self):
        ...
        """
        if self.max_shape is None:
            raise NotImplementedError('Generic Pilatus detector does not know its max size ...')
        mask = np.zeros(self.max_shape, dtype=np.int8)
