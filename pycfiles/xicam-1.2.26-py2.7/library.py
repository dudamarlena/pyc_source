...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\library.py
# Compiled at: 2018-08-27 17:21:07
from PySide import QtGui
import sys, base, viewer
...
import os, pipeline, numpy as np
...
class FlowLayout(QtGui.QLayout):
    ...
    def doLayout(self, rect, testOnly):
        ...
        for item in self.itemList:
            wid = item.widget()
            spaceX = self.spacing() + wid.style().layoutSpacing(QtGui.QSizePolicy.PushButton, QtGui.QSizePolicy.PushButton, Qt.Horizontal)
            spaceY = self.spacing() + wid.style().layoutSpacing(QtGui.QSizePolicy.PushButton, QtGui.QSizePolicy.PushButton, Qt.Vertical)
    ...
    def populate(self, path):
        ...
        for entry in entries:
            if not (entry == '..' and os.path.normpath(path) == pipeline.pathtools.getRoot()) and not entry == '.':
                ...
                w = thumbwidgetitem(os.path.join(path, entry), parentwindow=self.parentwindow, nameoverride=nameoverride)
                self.addWidget(w)
    ...
    def __init__(self, path, parentwindow, nameoverride=None):
        ...
        if os.path.isdir(path):
            ...
            try:
                ...
                self.thumb *= 255.0 / np.max(self.thumb)
                self.image = QtGui.QImage(self.thumb.astype(np.uint8), self.thumb.shape[1], self.thumb.shape[0], self.thumb.shape[1], QtGui.QImage.Format_Indexed8)
