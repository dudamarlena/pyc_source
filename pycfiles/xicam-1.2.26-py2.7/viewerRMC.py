...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\viewerRMC.py
...
import pyqtgraph as pg, numpy as np, subprocess, xicam.RmcView as rmc, time
from xicam import threads
from daemon.daemon import daemon
import multiprocessing, Queue, glob, platform
...
class plugin(base.plugin):
    ...
    def opendirectory(self, folder, operation=None):
        ...
        if input:
            ...
            if len(view_widget.orig_image.shape) > 2:
            ...
            view_widget.drawROI(0, 0, view_widget.orig_image.shape[0], view_widget.orig_image.shape[1], 'r', view_widget.orig_view.getImageItem().getViewBox())
...
class LogViewer(pg.ImageView):
    ...
    def setImage(self, *args, **kwargs):
        ...
        if np.isnan(levelmin):
            levelmin = 0
        if np.isnan(levelmax):
            levelmax = 1
        if np.isinf(levelmin):
            levelmin = 0
...
class inOutViewer(QtGui.QWidget):
    ...
    def __init__(self, paths, worker, parent=None):
        ...
        if paths is not None:
            ...
            self.drawROI(0, 0, self.orig_image.shape[0], self.orig_image.shape[1], 'r', self.orig_view.getImageItem().getViewBox())
    ...
    def center(self, sample=True):
        ...
        if sample:
            ...
            img = tifimage.tifimage(np.rot90((self.edited_image.astype(float) / self.edited_image.max() * 65536).astype(np.int16)))
        else:
    ...
    def drawCameraLocation(self, box, location):
        ...
        cameraPlot.setData([location[0]], [location[1]], pen=None, symbol='o', brush=pg.mkBrush('#FFA500'))
        return cameraBox
    ...
    def runRMC(self):
        ...
        if self.edited_image is None:
            ...
            hig_info = {'hipRMCInput': {'instrumentation': {'inputimage': ('{}').format(self.write_path_sample), 'imagesize': [
                                                               self.new_dim, self.new_dim], 
                                                   'numtiles': params.child('Num tiles').value(), 
                                                   'loadingfactors': [
                                                                    params.child('Loading factor').value()]}, 
            ...
            process = threads.RunnableMethod(method=self.run_RMCthread, finished_slot=self.RMC_done, except_slot=self.hiprmc_not_found)
