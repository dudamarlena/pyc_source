...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\xas.py
...
class XASPlugin(base.plugin):
    ...
    def addwithcoefmode(self):
        ...
        """
        coef, ok = QtGui.QInputDialog.getDouble(self.ui, 'Enter scaling coefficient x (A+xB):', 'Enter coefficient')
        if coef and ok:
    ...
    def subtractwithcoefmode(self):
        ...
        """
        coef, ok = QtGui.QInputDialog.getDouble(None, 'Enter scaling coefficient x (A-xB):', 'Enter coefficient')
        if coef and ok:
            operation = lambda m: m[0] - coef * np.sum(m[1:], 0)
    ...
    def opendata(self, data=None, operation=None, operationname=None):
        ...
        widget = widgets.OOMTabItem(itemclass=widgets.dimgViewer, dimg=dimg, operation=operation, operationname=operationname, plotwidget=self.bottomwidget, toolbar=self.toolbar)
    ...
    def exportimage(self):
        ...
        dialog = QtGui.QFileDialog(parent=None, caption='Export image as EDF', directory=unicode(os.path.dirname(self.getCurrentTab().paths[0])), filter='EDF (*.edf)')
