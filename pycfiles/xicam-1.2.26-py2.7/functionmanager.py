...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\tomography\functionmanager.py
...
class FunctionManager(fw.FeatureManager):
    ...
    mask_functions = [
     'F3D Mask Filter', 'F3D MM Erosion', 'F3D MM Dilation', 'F3D MM Opening', 'F3D MM Closing']
    slice_dir = {'Remove Outliers': 'proj', 
       'Remove NAN': 'proj', 
       ...
       'Gridrec': 'sino', 
       'FBP': 'sino', 'ART': 'sino', 'BART': 'sino', 'MLEM': 'sino', 'OSEm': 'sino', 'OSPML Hybrid': 'sino', 
       'OSPML Quad': 'sino', 'PML Hybrid': 'sino', 'PML Quad': 'sino', 'SIRT': 'sino', 'BP': 'sino', 
       'FBP_astra': 'sino', 'SIRT_astra': 'sino', 'SART': 'sino', 'ART_astra': 'sino', 'CGLS': 'sino', 'BP_CUDA': 'sino', 'FBP_CUDA': 'sino', 
       'SIRT_CUDA': 'sino', 'SART_CUDA': 'sino', 'CGLS_CUDA': 'sino', 'Gridrec Tomocam': 'sino', 'SIRT TomoCam': 'sino', 
       'MBIR TomoCam': 'sino', 'Polar Mean Filter': 'sino', 
       ...
       'Circular Mask': 'sino', 
       'Add': 'both', 
       'Subtract': 'both', 'Multiply': 'both', 'Divide': 'both', 'Maximum': 'both'}

    def __init__(self, list_layout, form_layout, function_widgets=None, blank_form=None):
        super(FunctionManager, self).__init__(list_layout, form_layout, feature_widgets=function_widgets, blank_form=blank_form)
        self.cor_offset = lambda x: x
        self.cor_scale = lambda x: x
    ...
    def connectCropPad(self):
        ...
        if 'Padding' in names and 'Crop' in names:
            ...
            children = [
             crop.params.child('p11'), crop.params.child('p21'), crop.params.child('p12'), crop.params.child('p22')]
            for p in children:
    ...
    def adjustReaderParams(self, roi_widget):
        ...
        for feature in self.features:
            if 'Reader' in feature.name:
                ...
                x2 = int(pos[0] + size[0]) if pos[0] + size[0] < feature.params.child('end_width').defaultValue() else feature.params.child('end_width').defaultValue()
                y1 = int(pos[1]) if pos[1] > 0 else 0
                y2 = int(pos[1] + size[1]) if pos[1] + size[1] < feature.params.child('end_sinogram').defaultValue() else feature.params.child('end_sinogram').defaultValue()
    ...
    def saveState(self, datawidget):
        ...
        for function in self.features:
            ...
            if 'Reconstruction' in function.name:
                for param, ipf in function.input_functions.iteritems():
                    ...
                    if 'center' in param:
                        ...
                        if ipf.subfunc_name in FunctionManager.center_func_slc:
                            map(args.append, map(datawidget.data.fabimage.__getitem__, FunctionManager.center_func_slc[ipf.subfunc_name]))
                        else:
        ...
        return [
         d, theta, center, config.extract_pipeline_dict(self.features),
    ...
    def loadDataDictionary(self, datawidget, theta, center, slc=None):
        ...
        try:
            if slc is not None and slc[0].start is not None:
                slc_ = slice(slc[0].start, datawidget.data.shape[0] - 1, slc[0].step) if slc[0].stop is None else slc[0]
    ...
    def updateDataDict(self, data_dict, function, name):
        ...
        if 'Sino 360 to 180' in name:
            data_dict['keepvalues'] = [
             data_dict['proj'], data_dict['num_proj_per_chunk'], data_dict['numprojchunks'],
             data_dict['numprojused'], data_dict['width']]
            proj = data_dict['proj']
            numangles = int(proj[1] - proj[0]) / 2
            data_dict['proj'] = (0, numangles - 1, proj[2])
            data_dict['num_proj_per_chunk'] = np.minimum(data_dict['num_proj_per_chunk'], data_dict['proj'][1] - data_dict['proj'][0])
    ...
    def loadPreviewData(self, datawidget, dims=None, ncore=None, skip_names=['Write', 'Reader'], fixed_func=None, prange=None):
        """
        ...
        for func in self.features:
            ...
            if not func.enabled:
                ...
                if func.func_name in skip_names:
    ...
    def reconGenerator(self, datawidget, func_dict, theta, center, yaml_pipe, run_dict, dims, ncore=1, reconType='Full'):
        """
        ...
        d = {'num_proj_per_chunk': num_proj_per_chunk, 'num_sino_per_chunk': num_sino_per_chunk, 
           'numprojchunks': numprojchunks, 'numsinochunks': numsinochunks, 'numprojused': numprojused, 
           'numsinoused': numsinoused, 'proj': proj, 
           'sino': sino, 'width': width, 'keepvalues': None}
        while True:
            if axis == 'proj':
            ...
            for y in range(niter):
                ...
                if curfunc == 0:
                    if axis == 'proj':
                        slc = (slice(y * num_proj_per_chunk + proj[0], np.minimum((y + 1) * num_proj_per_chunk + proj[0], proj[1]), proj[2]), slice(*sino), slice(*width))
                    else:
                        slc = (
                         slice(*proj),
                         slice(y * num_sino_per_chunk + sino[0], np.minimum((y + 1) * num_sino_per_chunk + sino[0], sino[1]), sino[2]), slice(*width))
                    ...
                    if axis == 'proj':
                        ...
                        tomo = dxchange.reader.read_hdf5(tempfilenames[curtemp], '/tmp/tmp', slc=(
                         (
                          start, end, 1), (0, d['numsinoused'], 1),
                         (
                ...
                while True:
                    ...
                    if newaxis != 'both' and newaxis != axis:
                        ...
                        dxchange.write_hdf5(d['tomo'], fname=tempfilenames[(1 - curtemp)], gname='tmp', dname='tmp', overwrite=False, appendaxis=appendaxis)
                        break
                    yield ('{}: ').format(func_name)
                    curtime = time.time()
                    function, write = self.updatePartial(func_dict[func_name], func_name, d, params_dict[func_name], slc)
                    d[write] = function()
    ...
    def testParameterRange(self, function, parameter, prange):
        ...
        for i in prange:
            ...
            fixed_func = type('FixedFunc', (), {'func_name': function.func_name, 'subfunc_name': function.subfunc_name, 'missing_args': function.missing_args, 
            ...
            self.sigTestRange.emit(('Computing preview for {} parameter {}={}...').format(function.name, parameter, i), fixed_func, {'function': function.func_name, parameter: prange})
                        ...
                        for ipf, sipfs in ipfs.iteritems():
                            for sipf in sipfs:
                                if param in funcWidget.input_functions:
                                    ...
                                    ifwidget = self.addInputFunction(funcWidget, param, ipf, sipf, package=reconpkg.packages[config_dict[sipf][1]])
                    ...
                    if param == 'Package':
                        ...
                        for param, ipfs in value.iteritems():
                            for ipf, sipf in ipfs.iteritems():
                                ifwidget = self.addInputFunction(funcWidget, param, ipf, sipf.keys()[0], package=reconpkg.packages[config_dict[sipf.keys()[0]][1]])
    ...
    def extractPipelineRunnable(self, runnable_pipe, center, params, proj, sino, sino_p_chunk, width, path, ncore=1):
        """
