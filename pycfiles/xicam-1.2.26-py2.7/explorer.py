...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\widgets\explorer.py
...
class GlobusFileView(RemoteFileView):
    ...
    def handleDownloadAction(self, save_path=None, fslot=None):
        ...
        for path in paths:
            ...
            args = [self.endpoint, path, 'LOCAL ENDPOINT']
            kwargs = {}
    ...
    def handleTransferAction(self):
        ...
        for path in paths:
            ...
            args = [self.endpoint, path, 'DEST ENDPOINT']
            kwargs = {}
...
class SFTPFileView(QtGui.QTreeWidget):
    ...
    def refresh(self, path=None):
        ...
        if path is not None:
            self.path = path
            self.client.cd(path)
        self.client.walktree(self.path, lambda x: self.sigAddTopLevelItem.emit(x, 'file'), lambda x: self.sigAddTopLevelItem.emit(x, 'dir'), lambda : None, recurse=False)
        return self
...
class SpotDatasetView(QtGui.QTreeWidget):
    ...
    def __init__(self, spot_client, parent=None):
        ...
        self.search_params = {'skipnum': '0', 'sortterm': 'fs.stage_date', 'sortterm': 'appmetadata.sdate', 'sorttype': 'desc', 
           'end_station': 'bl832'}
        self.getDatasets('')
        self.setHeaderHidden(True)
        self.menu = QtGui.QMenu()
    ...
    def getDatasets(self, query):
        ...
        search = threads.method(callback_slot=self.createDatasetDictionary, finished_slot=msg.clearMessage)(self.client.search)
    ...
    def createDatasetDictionary(self, data):
        ...
        for index in range(len(data)):
            derived_data = {data[index]['fs']['stage']: data[index]['name']}
    ...
    def currentChanged(self, current, previous):
        ...
        try:
            if item.childCount() == 0:
                ...
                bg_get_preview = threads.method(callback_slot=self.sigItemPreview.emit, except_slot=self.handleException)(self.client.get_image_as)
                bg_get_preview(dataset, stage, index=0)
        except AttributeError:
            pass

    def getStagesAndDatasets(self):
        dsets_stages = [ (item.parent().parent().text(0), item.parent().text(0)) for item in self.selectedItems() if item.childCount() == 0
                       ]
        return dsets_stages
...
class FileExplorer(QtGui.QWidget):
    ...
    def __init__(self, file_view, parent=None):
        ...
        for button, icon_file in zip((self.back_button, self.refresh_button), ('xicam/gui/icons_44.png',
                                                                               'xicam/gui/icons_57.png')):
            icon = QtGui.QIcon()
...
class MultipleFileExplorer(QtGui.QTabWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.standard_actions = OrderedDict({'SPOT': addspot, 'Cori': addcori, 'Edison': addedison, 'Bragg': addbragg, 
           'SFTP': addsftp})
    ...
    def addSFTPTab(self):
        add_sftp_explorer = lambda client: self.addFileExplorer(client.host.split('.')[0], FileExplorer(SFTPFileView(client, self)))
        add_sftp_callback = lambda client: self.loginSuccess(client, add_explorer=add_sftp_explorer)
    ...
    def handleDeleteActions(self, paths):
        r = QtGui.QMessageBox.warning(self, 'Delete file', ('Are you sure you want to delete\n{}?').format((',\n').join(paths)), QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
...
class JobTable(QtGui.QTableWidget):
    ...
    def addProgJob(self, job_desc, generator, args, kwargs, finish_slot=None):
        ...
        bg_generator = threads.iterator(callback_slot=job_entry.progress, finished_slot=finish_slot, interrupt_signal=job_entry.sigCancel)(generator)
...
class SFTPDirTreeItem(LazyTreeItem, QtCore.QObject):
    ...
    @threads.method()
    def getChildren(self):
        self.client.walktree(self.path, self.sigAddChildFile.emit, self.sigAddChildDir.emit, self.handleUnknown, recurse=False)
