...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\threads.py
...
__status__ = 'Beta'
import sys, types, traceback, functools, Queue, multiprocessing as mp
...
class RunnableMethod(QtCore.QRunnable):
    ...
    def __init__(self, method, method_args=(), method_kwargs={}, callback_slot=None, finished_slot=None, except_slot=None, default_exhandle=True, priority=0, lock=None):
        ...
        if default_exhandle:
            self.default_handler = functools.partial(default_exception_handler)
        ...
        try:
            try:
                ...
                try:
                    ...
                    msg.logMessage('Runnable method tried to return value, but signal was already disconnected.', msg.WARNING)
                    if self.lock is not None:
                        self.lock.unlock()
                    return
    ...
    def emit(self, slot, value):
        if slot is None:
            ...
            if type(value) is not tuple:
                value = (value,)
            if str(type(slot)) == "<type 'PySide.QtCore.SignalInstance'>":
                if slot is None:
                    return
                ...
                guiinvoker.invoke_in_main_thread(slot, *value)
            return
...
class RunnableIterator(RunnableMethod):
    ...
    def __init__(self, iterator, iterator_args=(), iterator_kwargs={}, callback_slot=None, finished_slot=None, interrupt_signal=None, except_slot=None, default_exhandle=True, priority=0, lock=None, parent=None):
        super(RunnableIterator, self).__init__(method=iterator, method_args=iterator_args, method_kwargs=iterator_kwargs, callback_slot=callback_slot, finished_slot=finished_slot, except_slot=except_slot, default_exhandle=default_exhandle, priority=priority, lock=lock)
        ...
        try:
            try:
                for status in self._method(*self.method_args, **self.method_kwargs):
                    if self._interrupt:
                        raise StopIteration(('{0} running in background thread {1} interrupted').format(self._method.__name__, QtCore.QThread.currentThreadId()))
                    try:
...
def method(callback_slot=None, finished_slot=None, except_slot=None, default_exhandle=True, lock=None):
    ...
    def wrap_runnable_method(function):
        ...
        def _runnable_method(*args, **kwargs):
            runnable = RunnableMethod(function, method_args=args, method_kwargs=kwargs, callback_slot=callback_slot, finished_slot=finished_slot, except_slot=except_slot, default_exhandle=default_exception_handler, lock=lock)
...
def iterator(callback_slot=None, finished_slot=None, interrupt_signal=None, except_slot=None, lock=None, parent=None):
    ...
    def wrap_runnable_iterator(generator):
        ...
        def _runnable_iterator(*args, **kwargs):
            runnable = RunnableIterator(generator, iterator_args=args, iterator_kwargs=kwargs, callback_slot=callback_slot, finished_slot=finished_slot, interrupt_signal=interrupt_signal, except_slot=except_slot, lock=lock, parent=parent)
            add_to_queue(runnable)
...
def default_exception_handler(etype, ex, tb):
    ...
    None
    """
    exmessage = ('').join(traceback.format_exception(etype, ex, tb))
