...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\tomography\viewers.py
# Compiled at: 2018-08-27 17:21:07
from collections import deque
import numpy as np, tomopy, pyqtgraph as pg
...
class TomoViewer(QtGui.QWidget):
    ...
    def wireupCenterSelection(self, recon_function):
        ...
        if recon_function is not None:
            ...
            self.projectionViewer.sigCenterChanged.connect(lambda x: center_param.setValue(x))
            self.projectionViewer.centerBox.valueChanged.connect(lambda x: center_param.setValue(x))
            self.projectionViewer.setCenterButton.clicked.connect(lambda : center_param.setValue(self.projectionViewer.centerBox.value()))
    ...
    def getflats(self, slc=None):
        ...
        if hasattr(flats, 'values'):
            flats = np.array(flats.values())
        if slc is None:
    ...
    def getdarks(self, slc=None):
        ...
        if hasattr(darks, 'values'):
            darks = np.array(darks.values())
        if slc is None:
...
class MBIRViewer(QtGui.QWidget):
    ...
    def __init__(self, data, path, *args, **kwargs):
        ...
        self.cor_function = functionwidgets.FunctionWidget(name='Center Detection', subname='Phase Correlation', package=reconpkg.packages[config.names['Phase Correlation'][1]])
        self.cor_params = pg.parametertree.Parameter.create(name=self.cor_function.name, children=config.parameters[self.cor_function.subfunc_name], type='group')
        ...
        params = [{'name': 'Dataset path', 'type': 'str'}, {'name': 'Z start', 'type': 'int', 'value': 0, 'default': 0},
         {'name': 'Z num elts', 'type': 'int', 'value': int(data.shape[(-1)]), 'default': int(data.shape[(-1)])}, {'name': 'Smoothness', 'type': 'float', 'value': 0.15, 'default': 0.15}, {'name': 'Zinger thresh', 'type': 'float', 'value': 5, 'default': 5}, {'name': 'View subsample factor', 'type': 'int', 'value': 2, 'default': 2}, {'name': 'Output folder', 'type': 'str', 'value': 'Results', 'default': 'Results'}]
    ...
    def changeCORfunction(self, index):
        ...
        self.cor_function = functionwidgets.FunctionWidget(name='Center Detection', subname=subname, package=reconpkg.packages[config.names[subname][1]])
        self.cor_params = pg.parametertree.Parameter.create(name=self.cor_function.name, children=config.parameters[self.cor_function.subfunc_name], type='group')
        self.cor_param_tree = pg.parametertree.ParameterTree()
        self.cor_param_tree.setMinimumHeight(200)
        self.cor_param_tree.setMinimumWidth(200)
    ...
    def find_COR(self, cor_function):
        ...
        if cor_function not in self.cor_detection_funcs:
            return -1
        else:
            if cor_function == 'Phase Correlation':
                ...
                if cor_function == 'Vo':
                    ...
                    if cor_function == 'Nelder-Mead':
                        kwargs = {'tomo': np.ascontiguousarray(self.data.fabimage[:, :, :]), 'theta': tomopy.angles(int(self.data.shape[0]), ang1=90, ang2=270)}
                    else:
                        return -1
    ...
    def write_slurm(self):
        ...
        if not self.center > 0 or self.center > self.data.shape[0]:
            ...
            slurm += (' --smoothness {} --zinger_thresh {}').format(self.mbir_params.child('Smoothness').value(), self.mbir_params.child('Zinger thresh').value())
...
class ProjectionViewer(QtGui.QWidget):
    ...
    def __init__(self, data, toolbar=None, view_label=None, center=None, paths=None, *args, **kwargs):
        ...
        self.roi_histogram.vb.enableAutoRange(self.roi_histogram.vb.YAxis, False)
    ...
    def writeCOR(self):
        ...
        """
        cor = QtGui.QInputDialog.getDouble(self.cor_box, 'Write COR value to file', 'Write COR value to file', self.data.shape[1] / 2)
        if cor[1]:
    ...
    def setCenter(self, x, y):
        ...
        y : float
            x-coordinate of overlay image in the background images coordinates
        """
        center = (self.data.shape[1] + x - 1) / 2.0
        self.centerBox.setValue(center)
    ...
    def updateROIFromCenter(self, center):
        ...
        self.imgoverlay_roi.translate(pg.Point((2 * center + 1 - self.data.shape[1] - s, 0)))
    ...
    def normalize(self, val):
        ...
        if val and not self.normalized:
            ...
            self.roi_histogram.setLevels(-1, 1)
...
class PreviewViewer(QtGui.QSplitter):
    ...
    def __init__(self, dim, maxpreviews=None, *args, **kwargs):
        ...
        self.view_number.setMaximum(5000)
