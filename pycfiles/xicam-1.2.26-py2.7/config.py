...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\tomography\config.py
            ...
            for p in f.params.children():
                if p.name() in als832defaults[f.subfunc_name]:
                    try:
                        ...
                        v = t(v) if t is not int else t(float(v))
                        if 'conversion' in als832defaults[f.subfunc_name][p.name()]:
                            v *= als832defaults[f.subfunc_name][p.name()]['conversion']
                        p.setDefault(v)
                        p.setValue(v)
                    except KeyError as e:
                        msg.logMessage(('Key {} not found in metadata. Error: {}').format(p.name(), e.message), level=40)

        elif f.func_name == 'Reader':
...
def extract_pipeline_dict(funwidget_list):
    ...
    """
    ex_lst = [
     'file name', 'fname', 'folder name', 'parent folder']
    d = OrderedDict()
    count = 1
...
def extract_runnable_dict(funwidget_list):
    ...
    """
    center_functions = {'find_center_pc': {'proj1': 'tomo[0]', 'proj2': 'tomo[-1]'}, 'find_center': {'tomo': 'tomo', 'theta': 'theta'}, 'find_center_vo': {'tomo': 'tomo'}}
    d = OrderedDict()
            ...
            for param, ipf in f.input_functions.iteritems():
                if 'theta' in param or 'center' in param:
                    ...
                    for key, val in ipf.partial.keywords.iteritems():
                        subfunc += (isinstance(val, str) or ('{}={},').format)(key, val) if 1 else ("{}='{}',").format(key, val)
