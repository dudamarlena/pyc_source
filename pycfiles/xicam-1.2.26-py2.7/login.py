...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\widgets\login.py
...
class LoginDialog(QtGui.QWidget):
    ...
    def handleLogin(self):
        ...
        if usr == '':
            ...
            QtGui.QMessageBox.warning(self, 'Password missing', ('You need to provide proof that you really are {}!').format(usr))
        elif host == '' and not self.ui.host_box.isHidden():
            QtGui.QMessageBox.warning(self, 'Host missing', 'Xi-cam can not guess what host you want to connect to yet!')
        else:
            self.setCurrentWidget(self.ui.progress_page)
            self.progressMessage('Logging in...')
            self.startProgress()
            credentials = {'username': usr, 'password': pwd} if self.ui.host_box.isHidden() else {'username': usr, 'password': pwd, 'host': host}
