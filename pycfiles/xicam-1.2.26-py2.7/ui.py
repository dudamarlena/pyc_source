...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\tomography\ui.py
...
class UIform(object):
    ...
    def setupUi(self):
        ...
        icon_properties = QtGui.QIcon(QtGui.QPixmap('xicam/gui/icons_61.png'))
            ...
            for subfunc in subfuncs:
                if isinstance(subfuncs, dict) and len(subfuncs[subfunc]) > 0:
                    ...
                    for opt in subfuncs[subfunc]:
                        ...
                        try:
                            funcaction.triggered.connect(partial(actionslot, func, opt, reconpkg.packages[functiondata[opt][1]]))
                    ...
                    try:
                        funcaction.triggered.connect(partial(actionslot, func, subfunc, reconpkg.packages[functiondata[subfunc][1]]))
...
class Toolbar(QtGui.QToolBar):
    ...
    def connectTriggers(self, slicepreview, multislicepreview, preview3D, fullrecon, center, roiselection, mbir, openflats, opendarks):
        """
        Connect toolbar action signals to give slots
