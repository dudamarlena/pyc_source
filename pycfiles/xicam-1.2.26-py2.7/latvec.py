...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\hipgisaxs\latvec.py
...
def latticevectors(a, b, c, zoffset, maxreps=100, repetitions=None, scaling=1.0):
    ...
    mi = np.vstack([a, b, c])
    combs = combs_generator(*repetitions)
...
def latticelines(a, b, c, zoffset, maxreps=100, repetitions=None, scaling=1.0):
...
def vecs2angles(v):
    ...
    vproj = v.copy()
    vproj[:, 2] = 0
    return np.arctan2(vproj[:, 1], vproj[:, 0]) / np.pi * 180.0
