...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\safeimporter.py
...
def import_module(modname, packagename=None):
    ...
    try:
        ...
        if response == QtGui.QMessageBox.Yes:
            ...
            if not failure:
                ...
                msgBox.exec_()
                exit(0)
            elif modname.strip('.') == 'MOTD':
                from xicam import debugtools
                debugtools.frustration()
                msgBox = QtGui.QMessageBox()
                msgBox.setText('Sorry, ' + missingpackage + ' could not be installed. This is a Xi-cam critical library.')
            ...
            if config.settings['Ignored Modules']:
                ...
                config.settings['Ignored Modules'] = [
                 missingpackage]
            msgBox = QtGui.QMessageBox()
        ...
        if modname.strip('.') == 'MOTD':
            ...
            debugtools.frustration()
            msgBox = QtGui.QMessageBox()
            msgBox.setText('Sorry, ' + missingpackage + ' is a Xi-cam critical library. This must be installed to run Xi-cam!')
    ...
    if not module:
        msg.logMessage('Failed to import ' + modname, msg.CRITICAL)
    return module