...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/bashlex/parser.py
...
def p_word_list(p):
    ...
    if len(p) == 2:
        p[0] = [
         _expandword(parserobj, p.slice[1])]
    else:
        p[0] = p[1]
...
def _expandword(parser, tokenword):
    if parser._expansionlimit == -1:
        node = ast.node(kind='word', word=tokenword, pos=(
         tokenword.lexpos, tokenword.endlexpos), parts=[])
        return node
    else:
        ...
        node = ast.node(kind='word', word=expandedword, pos=(
         tokenword.lexpos, tokenword.endlexpos), parts=parts)
        return node
...
def p_shell_command(p):
    ...
    if len(p) == 2:
        ...
        p[0] = ast.node(kind='compound', redirects=[], list=[
         ast.node(kind=kind, parts=parts, pos=_partsspan(parts))], pos=_partsspan(parts))
    assert p[0].kind == 'compound'
...
def p_for_command(p):
    ...
    p[0] = ast.node(kind='compound', redirects=[], list=[
     ast.node(kind='for', parts=parts, pos=_partsspan(parts))], pos=_partsspan(parts))
...
def p_if_command(p):
    ...
    parts = _makeparts(p)
    p[0] = ast.node(kind='compound', redirects=[], list=[
     ast.node(kind='if', parts=parts, pos=_partsspan(parts))], pos=_partsspan(parts))
...
def p_pipeline_command(p):
    ...
    if len(p) == 2:
        if len(p[1]) == 1:
            ...
            p[0] = ast.node(kind='pipeline', parts=p[1], pos=(
             p[1][0].pos[0], p[1][(-1)].pos[1]))
    else:
        ...
        if p[2].kind == 'pipeline':
            ...
            p[0] = ast.node(kind='pipeline', parts=[node, p[2]], pos=(
             node.pos[0], p[2].pos[1]))
...
yaccparser = yacc.yacc(tabmodule='bashlex.parsetab', outputdir=os.path.dirname(__file__), debug=False)
...
def split(s):
    r"""a utility function that mimics shlex.split but handles more
