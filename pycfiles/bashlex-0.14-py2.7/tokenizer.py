...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/bashlex/tokenizer.py
...
valid_reserved_first_command = {'if': tokentype.IF, 
   'then': tokentype.THEN, 
   'else': tokentype.ELSE, 
   'elif': tokentype.ELIF, 
   'fi': tokentype.FI, 
...
class MatchedPairError(errors.ParsingError):
    ...
    def __init__(self, startline, message, tokenizer):
        super(MatchedPairError, self).__init__(message, tokenizer.source, tokenizer._shell_input_line_index - 1)
...
class token(object):
    ...
    def __init__(self, type_, value, pos=None, flags=None):
        assert type_ is not None and isinstance(type_, tokentype)
        if flags is None:
            flags = set()
        self.ttype = type_
        self.value = value
    ...
    def __eq__(self, other):
        return isinstance(other, token) and self.type == other.type and self.value == other.value and self.lexpos == other.lexpos and self.endlexpos == other.endlexpos and self.flags == other.flags
...
class tokenizer(object):
    ...
    def __init__(self, s, parserstate, strictmode=True, eoftoken=None, lastreadtoken=None, tokenbeforethat=None, twotokensago=None):
    ...
    def __iter__(self):
        while True:
            t = self.token()
            if t is eoftoken or self._added_newline and t.lexpos + 1 == len(self._shell_input_line):
                break
            yield t
    ...
    def token(self):
        self._two_tokens_ago, self._token_before_that, self._last_read_token = self._token_before_that, self._last_read_token, self._current_token
        self._current_token = self._readtoken()
        if isinstance(self._current_token, tokentype):
            self._recordpos()
            self._current_token = self._createtoken(self._current_token, self._current_token.value)
        if self._parserstate & parserflags.EOFTOKEN and self._current_token.ttype == self._shell_eof_token:
    ...
    def _readtokenword(self, c):
        ...
        def handleshellexp():
            ...
            if peek_char == '(' or c == '$' and peek_char in '{[':
                ...
                self._push_delimiter(peek_char)
                try:
                    ttok = self._parse_matched_pair(peek_char, peek_char, peek_char, allowesc=peek_char == "'")
                finally:
                    self._pop_delimiter()
            ...
            if d['pass_next_character']:
                ...
                if c == '\\':
                    ...
                    if peek_char == '\n':
                        ...
                        self._ungetc(peek_char)
                        if cd is None or cd == '`' or cd == '"' and peek_char is not None and 'dquote' in sh_syntaxtab[peek_char]:
    ...
    def _parse_comsub(self, doublequotes, open, close, parsingcommand=False, dquote=False, firstclose=False):
        ...
        if peekc == '(':
            ...
            while count:
                ...
                if reservedwordok:
                    ...
                    elif checkcomment and c == '#' and (lexrwlen == 0 or insideword and lexwlen == 0):
                        pass
                ...
                if not insidecomment and checkcase and c == '<':
                    ...
                    if peekc == c:
                        ...
                        c = peekc
                elif checkcomment and not insidecomment and c == '#' and (reservedwordok and lexrwlen == 0 or insideword or lexwlen == 0):
                    insidecomment = True
                ...
                if _shellquote(c):
                    ...
                    try:
                        if wasdollar and c == "'":
                            nestret = self._parse_matched_pair(c, c, c, allowesc=True, dquote=True)
                        else:
                            nestret = self._parse_matched_pair(c, c, c, dquote=True)
                    finally:
                        self._pop_delimiter()
                    ...
                    if not insidecase and open == c:
                        count -= 1
                    if c == '(':
                        nestret = self._parse_comsub(None, '(', ')', parsingcommand=True, dquote=False)
                    elif c == '{':
                        nestret = self._parse_matched_pair(None, '{', '}', firstclose=True, dolbrace=True, dquote=True)
                    elif c == '[':
                        nestret = self._parse_matched_pair(None, '[', ']', dquote=True)
                    ret += nestret
                wasdollar = c == '$'
    ...
    def _parse_matched_pair(self, doublequotes, open, close, parsingcommand=False, allowesc=False, dquote=False, firstclose=False, dolbrace=False, arraysub=False):
        ...
        def handledollarword():
            if open == c:
                count -= 1
            if c == '(':
                return self._parse_comsub(None, '(', ')', parsingcommand=True, dquote=False)
            else:
                if c == '{':
                    return self._parse_matched_pair(None, '{', '}', firstclose=True, dquote=rdquote, dolbrace=True)
                if c == '[':
        ...
        while count:
            ...
            if insidecomment:
                ...
                if c == '\n':
                    insidecomment = False
                continue
            elif lookforcomments and not insidecomment and c == '#' and (not ret or ret[(-1)] == '\n' or _shellblank(ret[(-1)])):
                insidecomment = True
            if passnextchar:
                passnextchar = False
                ret += c
            ...
            if dolbrace:
                if dolbracestate == 'param':
                    if len(ret) > 1:
                        dd = {'%': 'quote', '#': 'quote', '/': 'quote2', '^': 'quote', ',': 'quote'}
                        if c in dd:
                            dolbracestate = dd[c]
                    elif c in '#%^,~:-=?+/':
                        dolbracestate = 'op'
    ...
    def _command_token_position(self, token):
        return token.ttype == tokentype.ASSIGNMENT_WORD or self._parserstate & parserflags.REDIRLIST or token.ttype not in (tokentype.SEMI_SEMI, tokentype.SEMI_AND, tokentype.SEMI_SEMI_AND) and self._reserved_word_acceptable(token)
    ...
    def _reserved_word_acceptable(self, tok):
        if not tok or tok.ttype in _reserved or tok.value in _reserved:
            return True
        if self._last_read_token.ttype == tokentype.WORD and self._token_before_that.ttype == tokentype.FUNCTION:
            return True
        return False
    ...
    def _ungetc(self, c):
        if self._shell_input_line and self._shell_input_line_index and self._shell_input_line_index <= len(self._shell_input_line):
    ...
    def _specialcasetokens(self, tokstr):
        if self._last_read_token.ttype == tokentype.WORD and self._token_before_that.ttype in (tokentype.FOR,
         tokentype.CASE,
         tokentype.SELECT) and tokstr == 'in':
            if self._token_before_that.ttype == tokentype.CASE:
                self._parserstate.add(parserflags.CASEPAT)
                self._esacs_needed_count += 1
            return tokentype.IN
        if self._last_read_token.ttype == tokentype.WORD and self._token_before_that.ttype in (tokentype.FOR, tokentype.SELECT) and tokstr == 'do':
            return tokentype.DO
        ...
        if self._last_read_token.ttype == tokentype.ARITH_FOR_EXPRS and tokstr == 'do':
            return tokentype.DO
        if self._last_read_token.ttype == tokentype.ARITH_FOR_EXPRS and tokstr == '{':
            self._open_brace_count += 1
            return tokentype.LEFT_CURLY
        if self._open_brace_count and self._reserved_word_acceptable(self._last_read_token) and tokstr == '}':
            self._open_brace_count -= 1
