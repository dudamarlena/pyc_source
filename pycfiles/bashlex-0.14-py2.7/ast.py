...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/bashlex/ast.py
    ...
    def _format(n, level=0):
        if isinstance(n, node):
            ...
            if isinstance(n, list):
                lines = [
                 '[']
                lines.extend(indent * (level + 1) + _format(x, level + 1) + ',' for x in n)
                if len(lines) > 1:
...
class posshifter(nodevisitor):
    ...
    def visitnode(self, node):
        node.pos = (
         node.pos[0] + self.count, node.pos[1] + self.count)