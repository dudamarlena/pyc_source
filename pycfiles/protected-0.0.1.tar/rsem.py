...
# Embedded file name: build/bdist.linux-x86_64/egg/protect/expression_profiling/rsem.py
...
def rsem_disk(star_bams, rsem_index):
    ...
    return int(3 * ceil(star_transcriptome_bam.size + 524288) + 4 * ceil(rsem_index.size + 524288))
...
def wrap_rsem(job, star_bams, univ_options, rsem_options):
    ...
    :rtype: dict
    """
    rsem = job.addChildJobFn(run_rsem, star_bams['rna_transcriptome.bam'], univ_options, rsem_options, cores=rsem_options['n'], disk=PromisedRequirement(rsem_disk, star_bams, rsem_options['index']))
    return rsem.rv()
...
def run_rsem(job, rna_bam, univ_options, rsem_options):
    ...
    """
    work_dir = os.getcwd()
    input_files = {'star_transcriptome.bam': rna_bam, 
    ...
    parameters = [
     ...
     'rsem']
    docker_call(tool='rsem', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=rsem_options['version'])
    output_files = {}
    for filename in ('rsem.genes.results', 'rsem.isoforms.results'):
        ...
        export_results(job, output_files[filename], ('/').join([work_dir, filename]), univ_options, subfolder='expression')
