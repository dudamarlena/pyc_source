...
# Embedded file name: build/bdist.linux-x86_64/egg/protect/test/unit/test_mutation_callers.py
...
class TestMutationCallers(ProtectTest):
    ...
    def test_star_fusion(self):
        ...
        c = Job.wrapJobFn(run_fusion, a1.rv(), a2.rv(), univ_options, b.rv('star_fusion'), b.rv('fusion_inspector')).encapsulate()
        a1.addChild(a2)
        a1.addChild(b)
        b.addChild(c)
        Job.Runner.startToil(a1, self.options)
    ...
    def _test_mixed_callers(self, mut_caller, mut_caller_fn):
        ...
        b1 = Job.wrapJobFn(self._get_all_tools, config_file).encapsulate()
        b2 = Job.wrapJobFn(self._get_tool, b1.rv(), mut_caller)
        c = Job.wrapJobFn(mut_caller_fn, a1.rv(), a2.rv(), a3.rv(), univ_options, b2.rv()).encapsulate()
        a1.addChild(a2)
        a2.addChild(a3)
        a3.addChild(b1)
        b1.addChild(b2)
    ...
    def _get_test_dna_alignments(job, sample_type):
        ...
        subprocess.check_call(final_call.split(' '))
        return {bamfile: job.fileStore.writeGlobalFile(bamfile), bamfile + '.bai': job.fileStore.writeGlobalFile(bamfile + '.bai')}
    ...
    def _get_test_rna_alignments(job):
        ...
        final_call = base_call + bamfile + ' ' + bamfile
        subprocess.check_call(final_call.split(' '))
        return {'rna_genome': {bamfile: job.fileStore.writeGlobalFile(bamfile), 
