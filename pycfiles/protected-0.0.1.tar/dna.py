...
# Embedded file name: build/bdist.linux-x86_64/egg/protect/alignment/dna.py
...
def bwa_disk(dna_fastqs, bwa_index):
    return int(6 * ceil(sum([ f.size for f in dna_fastqs ]) + 524288) + 2.5 * ceil(bwa_index.size + 524288))
...
def align_dna(job, fastqs, sample_type, univ_options, bwa_options):
    ...
    :rtype: dict
    """
    bwa = job.wrapJobFn(run_bwa, fastqs, sample_type, univ_options, bwa_options, disk=PromisedRequirement(bwa_disk, fastqs, bwa_options['index']), memory=univ_options['java_Xmx'], cores=bwa_options['n'])
    sam2bam = job.wrapJobFn(bam_conversion, bwa.rv(), sample_type, univ_options, bwa_options['samtools'], disk=PromisedRequirement(sam2bam_disk, bwa.rv()), memory=univ_options['java_Xmx'])
    reheader = job.wrapJobFn(fix_bam_header, sam2bam.rv(), sample_type, univ_options, bwa_options['samtools'], disk=PromisedRequirement(sam2bam_disk, bwa.rv()), memory=univ_options['java_Xmx'])
    regroup = job.wrapJobFn(add_readgroups, reheader.rv(), sample_type, univ_options, bwa_options['picard'], disk=PromisedRequirement(regroup_disk, reheader.rv()), memory=univ_options['java_Xmx'])
    mkdup = job.wrapJobFn(mark_duplicates, regroup.rv(), sample_type, univ_options, bwa_options['picard'], disk=PromisedRequirement(mkdup_disk, regroup.rv()), memory=univ_options['java_Xmx'])
    index = job.wrapJobFn(index_bamfile, mkdup.rv(), sample_type, univ_options, bwa_options['samtools'], sample_info='fix_pg_sorted', disk=PromisedRequirement(index_disk, mkdup.rv()), memory=univ_options['java_Xmx'])
    job.addChild(bwa)
...
def run_bwa(job, fastqs, sample_type, univ_options, bwa_options):
    ...
    """
    work_dir = os.getcwd()
    input_files = {'dna_1.fastq': fastqs[0], 
    ...
    with open(('').join([work_dir, '/', sample_type, '.sam']), 'w') as (samfile):
        docker_call(tool='bwa', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], outfile=samfile, tool_version=bwa_options['version'])
...
def bam_conversion(job, samfile, sample_type, univ_options, samtools_options):
    ...
    """
    work_dir = os.getcwd()
    input_files = {sample_type + '.sam': samfile}
    ...
    docker_call(tool='samtools', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=samtools_options['version'])
...
def fix_bam_header(job, bamfile, sample_type, univ_options, samtools_options, retained_chroms=None):
    ...
    with open(headerfile.name, 'r') as (headerfile):
        with open(('/').join([work_dir, sample_type + '_output_bam.header']), 'w') as (outheaderfile):
...
def add_readgroups(job, bamfile, sample_type, univ_options, picard_options):
    ...
    docker_call(tool='picard', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], java_xmx=univ_options['java_Xmx'], tool_version=picard_options['version'])
    output_file = job.fileStore.writeGlobalFile(('/').join([work_dir, sample_type + '_reheader.bam']))
    job.fileStore.deleteGlobalFile(bamfile)
...
def mark_duplicates(job, bamfile, sample_type, univ_options, picard_options):
    ...
    parameters = [
     ...
     'CREATE_INDEX=true']
    docker_call(tool='picard', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], java_xmx=univ_options['java_Xmx'], tool_version=picard_options['version'])
    output_file = job.fileStore.writeGlobalFile(('/').join([work_dir, sample_type + '_mkdup.bam']))
    job.fileStore.deleteGlobalFile(bamfile)
