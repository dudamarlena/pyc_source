...
# Embedded file name: build/bdist.linux-x86_64/egg/protect/pipeline/ProTECT.py
...
from protect.common import delete_bams, delete_fastqs, email_report, get_file_from_gdc, get_file_from_s3, get_file_from_url, ParameterError, parse_chromosome_string, untargz, is_gzipfile, gunzip, dummy_job
...
import argparse, os, pkg_resources, re, shutil, sys, yaml
...
def _ensure_patient_group_is_ok(patient_object, patient_name=None):
    ...
    if 'tumor_type' not in patient_object:
        raise ParameterError('The patient entry for sample %s ' % patient_name + 'does not contain a Tumor type.')
    elif patient_object['tumor_type'] not in TCGAToGTEx:
        raise ParameterError('The patient entry for sample %s ' % patient_name + 'does contains an invalid Tumor type. Please use one of the valid TCGA tumor types.')
    if {'tumor_dna_fastq_1', 'normal_dna_fastq_1', 'tumor_rna_fastq_1'}.issubset(test_set):
        pass
    else:
        if 'hla_haplotype_files' not in test_set:
            raise ParameterError('The patient entry for sample %s ' % patient_name + 'does not contain a hla_haplotype_files entry.\nCannot haplotype patient if all the input sequence files are not fastqs.')
        if ({re.search('tumor_dna_((bam)|(fastq_1)).*', x) for x in test_set} == {None} or {re.search('normal_dna_((bam)|(fastq_1)).*', x) for x in test_set} == {None}) and 'mutation_vcf' not in test_set and 'fusion_bedpe' not in test_set:
            raise ParameterError('The patient entry for sample %s ' % patient_name + 'does not contain a mutation_vcf or fusion_bedpe entry. If both tumor and normal DNA sequences (fastqs or bam) are not provided, a pre-computed vcf and/or bedpe must be provided.')
        if {re.search('tumor_rna_((bam)|(fastq_1)).*', x) for x in test_set} == {None}:
            if 'mutation_vcf' not in test_set and 'fusion_bedpe' in test_set:
                pass
            else:
                raise ParameterError('The patient entry for sample %s ' % patient_name + 'does not contain a tumor rna sequence data entry. We require either tumor_rna_fastq_1 or tumor_rna_bam.')
        if 'tumor_rna_bam' in test_set and 'tumor_rna_transcriptome_bam' not in test_set:
            if 'expression_files' not in test_set:
                raise ParameterError('The patient entry for sample %s ' % patient_name + 'was provided a tumor rna bam with sequences mapped to the genome but was not provided a matching rna bam for the transcriptome or a tar containing expression values. We require either a matching transcriptome bam to estimateexpression, or the precomputed expression values.')
    return
...
def get_fastq_2(job, patient_id, sample_type, fastq_1):
    ...
    while extn:
        ...
        raise ParameterError('Could not determine prefix from provided fastq (%s). Is it of the form <fastq_prefix>1.[fq/fastq][.gz]?' % fastq_1)

    if final_extn not in ('.fastq', '.fastq.gz', '.fq', '.fq.gz'):
        raise ParameterError('If and _2 fastq path is not specified, only .fastq, .fq or their gzippped extensions are accepted. Could not process %s:%s.' % (
         patient_id, sample_type + '_fastq_1'))
...
def parse_patients(job, patient_dict, skip_fusions=False):
    ...
    :rtype: dict
    """
    output_dict = {'ssec_encrypted': patient_dict.get('ssec_encrypted') in (True, 'True', 'true'), 'patient_id': patient_dict['patient_id'], 
    ...
    if 'mutation_vcf' in patient_dict:
        ...
        for f in fastq1s:
            ...
            if f + '_fastq_2' not in output_dict:
                output_dict[f + '_fastq_2'] = get_fastq_2(job, patient_dict['patient_id'], f, output_dict[(f + '_fastq_1')])
...
def _parse_config_file(job, config_file, max_cores=None):
    ...
    for key in input_config.keys():
        if key == 'patients':
            for sample_name in input_config[key]:
                ...
                sample_set[sample_name] = parse_patients(job, input_config['patients'][sample_name], not input_config['mutation_calling']['star_fusion']['run'])
                ...
                if sample_set[sample_name]['gdc_inputs']:
                    if 'tumor_rna_bam' in sample_set[sample_name]['gdc_inputs']:
                        raise ParameterError('Cannot run ProTECT using GDC RNA bams. Please fix sample %s' % sample_name)
                    gdc_inputs = True
            ...
            if key == 'Universal_Options':
                ...
                assert univ_options['storage_location'].startswith(('Local', 'local',
                                                                    'aws'))
                if univ_options['storage_location'] in ('Local', 'local'):
                    assert os.path.isabs(univ_options['output_folder']), 'Needs to be absolute if storage_location is Local.'
                    assert univ_options['output_folder'] != 'NA', 'Cannot have NA as output folder if storage location is Local.'
                ...
                if key == 'alignment':
                    ...
                    append_subgroup = []
                tool_options.update(_process_group(input_config[key], required_keys[key], key, append_subgroup))
    ...
    if ssec_encrypted:
        ...
        assert gdc_inputs and univ_options['gdc_download_token'] is not None, 'Cannot read from GDC without a token.'
    job.fileStore.logToMaster('Obtaining tool inputs')
    process_tool_inputs = job.addChildJobFn(get_all_tool_inputs, tool_options, mutation_caller_list=mutation_caller_list)
    job.fileStore.logToMaster('Obtained tool inputs')
    return (
     sample_set, univ_options, process_tool_inputs.rv())
...
def parse_config_file(job, config_file, max_cores=None):
    ...
    :param int max_cores: The maximum cores to use for any single high-compute job.
    """
    sample_set, univ_options, processed_tool_inputs = _parse_config_file(job, config_file, max_cores)
...
def launch_protect(job, patient_data, univ_options, tool_options):
    ...
    for sample_type in ('tumor_dna', 'normal_dna', 'tumor_rna'):
        if sample_type + '_fastq_1' in patient_data:
            fastq_files[sample_type] = job.wrapJobFn(get_patient_fastqs, sample_prep.rv(), sample_type, disk='10M')
            ...
            bam_files[sample_type] = job.wrapJobFn(get_patient_bams, sample_prep.rv(), sample_type, univ_options, tool_options['bwa'], tool_options['mutect'], disk='10M').encapsulate()
    ...
    if 'hla_haplotype_files' in patient_data:
        ...
        for sample_type in ('tumor_dna', 'normal_dna', 'tumor_rna'):
            phlat_files[sample_type] = job.wrapJobFn(run_phlat, fastq_files[sample_type].rv(), sample_type, univ_options, tool_options['phlat'], cores=tool_options['phlat']['n'], disk=PromisedRequirement(phlat_disk, fastq_files[sample_type].rv()))
            fastq_files[sample_type].addChild(phlat_files[sample_type])
            phlat_files[sample_type].addChild(fastq_deletion_1)

        haplotype_patient = job.wrapJobFn(merge_phlat_calls, phlat_files['tumor_dna'].rv(), phlat_files['normal_dna'].rv(), phlat_files['tumor_rna'].rv(), univ_options, disk='100M', memory='100M', cores=1)
    ...
    if bam_files['tumor_rna'] is None:
        if 'fusion_bedpe' in patient_data and 'expression_files' in patient_data and 'mutation_vcf' not in patient_data:
            ...
            cutadapt = job.wrapJobFn(run_cutadapt, fastq_files['tumor_rna'].rv(), univ_options, tool_options['cutadapt'], cores=1, disk=PromisedRequirement(cutadapt_disk, fastq_files['tumor_rna'].rv()))
            bam_files['tumor_rna'] = job.wrapJobFn(align_rna, cutadapt.rv(), univ_options, tool_options['star'], cores=1, disk='100M').encapsulate()
            fastq_deletion_2 = job.wrapJobFn(delete_fastqs, {'cutadapted_rnas': cutadapt.rv()}, disk='100M', memory='100M')
            ...
            if 'fusion_bedpe' not in patient_data:
                ...
                fusions = job.wrapJobFn(wrap_fusion, cutadapt.rv(), bam_files['tumor_rna'].rv(), univ_options, tool_options['star_fusion'], tool_options['fusion_inspector'], disk='100M', memory='100M', cores=1).encapsulate()
            else:
                fusions = job.wrapJobFn(get_patient_bedpe, sample_prep.rv())
                sample_prep.addChild(fusions)
        ...
        if 'fusion_bedpe' in patient_data:
            ...
            if tool_options['star_fusion']['run'] is True:
                job.fileStore.logToMaster('Input RNA bams were provided for sample %s. Fusion detection can only be run with input fastqs.' % univ_options['patient'])
            fusions = None
        if 'expression_files' in patient_data:
            rsem = job.wrapJobFn(get_patient_expression, sample_prep.rv())
            sample_prep.addChild(rsem)
        else:
            rsem = job.wrapJobFn(wrap_rsem, bam_files['tumor_rna'].rv(), univ_options, tool_options['rsem'], cores=1, disk='100M').encapsulate()
            bam_files['tumor_rna'].addChild(rsem)
        delete_bam_files['tumor_rna'] = job.wrapJobFn(delete_bams, bam_files['tumor_rna'].rv(), univ_options['patient'], disk='100M', memory='100M')
        ...
        if phlat_files['tumor_rna'] is not None:
            mhc_pathway_assessment = job.wrapJobFn(run_mhc_gene_assessment, rsem.rv(), phlat_files['tumor_rna'].rv(), univ_options, tool_options['reports'], disk='100M', memory='100M', cores=1)
            rsem.addChild(mhc_pathway_assessment)
            phlat_files['tumor_rna'].addChild(mhc_pathway_assessment)
        else:
            mhc_pathway_assessment = job.wrapJobFn(run_mhc_gene_assessment, rsem.rv(), None, univ_options, tool_options['reports'], disk='100M', memory='100M', cores=1)
            rsem.addChild(mhc_pathway_assessment)
        itx_resistance_assessment = job.wrapJobFn(run_itx_resistance_assessment, rsem.rv(), univ_options, tool_options['reports'], disk='100M', memory='100M', cores=1)
        rsem.addChild(itx_resistance_assessment)
        car_t_validity_assessment = job.wrapJobFn(run_car_t_validity_assessment, rsem.rv(), univ_options, tool_options['reports'], disk='100M', memory='100M', cores=1)
            ...
            if 'fusion_bedpe' in patient_data:
                ...
                for sample_type in ('tumor_dna', 'normal_dna'):
                    if bam_files[sample_type] is None:
                        ...
                        bam_files[sample_type] = job.wrapJobFn(align_dna, fastq_files[sample_type].rv(), sample_type, univ_options, tool_options['bwa'], cores=1, disk='100M').encapsulate()
                        fastq_files[sample_type].addChild(bam_files[sample_type])
                        bam_files[sample_type].addChild(fastq_deletion_1)
                    delete_bam_files[sample_type] = job.wrapJobFn(delete_bams, bam_files[sample_type].rv(), univ_options['patient'], disk='100M', memory='100M')
                    bam_files[sample_type].addChild(delete_bam_files[sample_type])

            mutations = {'radia': job.wrapJobFn(run_radia, bam_files['tumor_rna'].rv(), bam_files['tumor_dna'].rv(), bam_files['normal_dna'].rv(), univ_options, tool_options['radia'], disk='100M').encapsulate(), 
               'mutect': job.wrapJobFn(run_mutect, bam_files['tumor_dna'].rv(), bam_files['normal_dna'].rv(), univ_options, tool_options['mutect'], disk='100M').encapsulate(), 
               'muse': job.wrapJobFn(run_muse, bam_files['tumor_dna'].rv(), bam_files['normal_dna'].rv(), univ_options, tool_options['muse']).encapsulate(), 
               'somaticsniper': job.wrapJobFn(run_somaticsniper, bam_files['tumor_dna'].rv(), bam_files['normal_dna'].rv(), univ_options, tool_options['somaticsniper']).encapsulate(), 
               'strelka': job.wrapJobFn(run_strelka, bam_files['tumor_dna'].rv(), bam_files['normal_dna'].rv(), univ_options, tool_options['strelka']).encapsulate(), 
               'indels': job.wrapJobFn(run_indel_caller, bam_files['tumor_dna'].rv(), bam_files['normal_dna'].rv(), univ_options, 'indel_options', disk='100M', memory='100M', cores=1)}
            for sample_type in ('tumor_dna', 'normal_dna'):
                for caller in mutations:
                    bam_files[sample_type].addChild(mutations[caller])

        bam_files['tumor_rna'].addChild(mutations['radia'])
        get_mutations = job.wrapJobFn(run_mutation_aggregator, {caller:cjob.rv() for caller, cjob in mutations.items()}, univ_options, disk='100M', memory='100M', cores=1).encapsulate()
        for caller in mutations:
    ...
    if get_mutations is not None:
        snpeff = job.wrapJobFn(run_snpeff, get_mutations.rv(), univ_options, tool_options['snpeff'], disk=PromisedRequirement(snpeff_disk, tool_options['snpeff']['index']))
    ...
    transgene = job.wrapJobFn(run_transgene, snpeffed_calls, bam_files['tumor_rna'].rv(), univ_options, tool_options['transgene'], disk=PromisedRequirement(transgene_disk, bam_files['tumor_rna'].rv()), memory='100M', cores=1, tumor_dna_bam=tumor_dna_bam, fusion_calls=fusion_calls)
    if snpeff:
    ...
    spawn_mhc = job.wrapJobFn(spawn_antigen_predictors, transgene.rv(), haplotype_patient.rv(), univ_options, (tool_options['mhci'], tool_options['mhcii']), disk='100M', memory='100M', cores=1).encapsulate()
    haplotype_patient.addChild(spawn_mhc)
    transgene.addChild(spawn_mhc)
    merge_mhc = job.wrapJobFn(merge_mhc_peptide_calls, spawn_mhc.rv(), transgene.rv(), univ_options, disk='100M', memory='100M', cores=1)
    spawn_mhc.addFollowOn(merge_mhc)
    transgene.addChild(merge_mhc)
    rankboost = job.wrapJobFn(wrap_rankboost, rsem.rv(), merge_mhc.rv(), transgene.rv(), univ_options, tool_options['rankboost'], disk='100M', memory='100M', cores=1)
    ...
    for tool in tools:
        for option in tools[tool]:
            if isinstance(tools[tool][option], dict):
                tools[tool][option] = get_all_tool_inputs(job, {option: tools[tool][option]}, outer_key=(':').join([outer_key, tool]).lstrip(':'))[option]
            elif option.split('_')[(-1)] in ('file', 'vcf', 'index', 'fasta', 'fai',
                                             'idx', 'dict', 'tbi', 'beds', 'gtf',
                                             'config'):
                tools[tool][option] = job.addChildJobFn(get_pipeline_inputs, (':').join([outer_key, tool, option]).lstrip(':'), tools[tool][option]).rv()
            elif option == 'version':
...
def get_pipeline_inputs(job, input_flag, input_file, encryption_key=None, per_file_encryption=False, gdc_download_token=None):
    """
    ...
    if input_file.startswith(('http', 'https', 'ftp')):
        input_file = get_file_from_url(job, input_file, encryption_key=encryption_key, per_file_encryption=per_file_encryption, write_to_jobstore=True)
    elif input_file.startswith(('S3', 's3')):
        input_file = get_file_from_s3(job, input_file, encryption_key=encryption_key, per_file_encryption=per_file_encryption, write_to_jobstore=True)
    elif input_file.startswith(('GDC', 'gdc')):
        input_file = get_file_from_gdc(job, input_file, gdc_download_token=gdc_download_token, write_to_jobstore=True)
    else:
...
def prepare_samples(job, patient_dict, univ_options):
    ...
    for input_file in patient_dict:
        if not input_file.endswith(('bam', 'bai', '_1', '_2', 'files', 'vcf', 'bedpe')):
            ...
            continue
        output_dict[input_file] = get_pipeline_inputs(job, (':').join([univ_options['patient'], input_file]), patient_dict[input_file], encryption_key=univ_options['sse_key'] if patient_dict['ssec_encrypted'] else None, per_file_encryption=univ_options['sse_key_is_master'], gdc_download_token=univ_options['gdc_download_token'])
...
def get_patient_fastqs(job, patient_dict, sample_type):
    ...
    """
    return [
     patient_dict[(sample_type + '_fastq_1')], patient_dict[(sample_type + '_fastq_2')]]
...
def get_patient_bams(job, patient_dict, sample_type, univ_options, bwa_options, mutect_options):
    ...
    if sample_type + '_bam' in patient_dict['gdc_inputs']:
        ...
        output_job = job.wrapJobFn(index_bamfile, patient_dict[(sample_type + '_bam')], 'rna' if sample_type == 'tumor_rna' else sample_type, univ_options, bwa_options['samtools'], sample_info=sample_info, export=False, disk=PromisedRequirement(index_disk, patient_dict[(sample_type + '_bam')]))
    ...
    if sample_type == 'tumor_rna':
        ...
        return {'rna_genome': output_dict, 'rna_transcriptome.bam': patient_dict['tumor_rna_transcriptome_bam']}
    else:
        return output_dict
        return
...
def get_patient_vcf(job, patient_dict):
    ...
    :rtype: toil.fileStore.FileID
    """
    temp = job.fileStore.readGlobalFile(patient_dict['mutation_vcf'], os.path.join(os.getcwd(), 'temp.gz'))
    if is_gzipfile(temp):
...
def get_patient_bedpe(job, patient_dict):
    ...
    :rtype: toil.fileStore.FileID
    """
    temp = job.fileStore.readGlobalFile(patient_dict['fusion_bedpe'], os.path.join(os.getcwd(), 'temp.gz'))
    if is_gzipfile(temp):
        outfile = job.fileStore.writeGlobalFile(gunzip(temp))
...
def get_patient_mhc_haplotype(job, patient_dict):
    ...
    for filename in ('mhci_alleles.list', 'mhcii_alleles.list'):
        output_dict[filename] = job.fileStore.writeGlobalFile(os.path.join(haplotype_archive, filename))
...
def get_patient_expression(job, patient_dict):
    ...
    for filename in ('rsem.genes.results', 'rsem.isoforms.results'):
        output_dict[filename] = job.fileStore.writeGlobalFile(os.path.join(expression_archive, filename))
...
def generate_config_file():
    ...
    :return: None
    """
    shutil.copy(os.path.join(os.path.dirname(__file__), 'input_parameters.yaml'), os.path.join(os.getcwd(), 'ProTECT_config.yaml'))
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(prog='ProTECT', description='Prediction of T-Cell Epitopes for Cancer Therapy', epilog='Contact Arjun Rao (aarao@ucsc.edu) if you encounter any problems while running ProTECT')
    inputs = parser.add_mutually_exclusive_group(required=True)
    inputs.add_argument('--config_file', dest='config_file', help='Config file to be used in the run.', type=str, default=None)
    inputs.add_argument('--generate_config', dest='generate_config', help='Generate a config file in the current directory that is pre-filled with references and flags for an hg19 run.', action='store_true', default=False)
    parser.add_argument('--max-cores-per-job', dest='max_cores', help='Maximum cores to use per job. Aligners and Haplotypers ask for cores dependent on the machine that the launchpad gets assigned to -- In a heterogeneous cluster, this can lead to problems. This value should be set to the number of cpus on the smallest node in a cluster.', type=int, required=False, default=None)
    ...
    if params.generate_config:
        ...
        if params.maxCores:
            ...
            elif params.max_cores > int(params.maxCores):
                print('The value provided to max-cores-per-job (%s) was greater than that provided to maxCores (%s). Setting max-cores-per-job = maxCores.' % (
