...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fynance/algorithms/allocation.py
# Compiled at: 2019-09-25 08:11:18
# Size of source mod 2**32: 19291 bytes
...
__all__ = [
 'ERC', 'HRP', 'IVP', 'MDP', 'MVP', 'MVP_uc', 'rolling_allocation']

def ERC(X, w0=None, up_bound=1.0, low_bound=0.0):
...
def _get_cluster(mat_cov, cItems):
    ...
    """
    cov_ = mat_cov.loc[(cItems, cItems)]
...
def _get_IVP(mat_cov):
    ...
    """
    ivp = 1.0 / np.diag(mat_cov)
    ivp /= np.sum(ivp)
    return ivp


def HRP(X, method='single', metric='euclidean', low_bound=0.0, up_bound=1.0):
...
def IVP(X, normalize=False, low_bound=0.0, up_bound=1.0):
...
def MVP_uc(X, w0=None, up_bound=1.0, low_bound=0.0):
...
def MDP(X, w0=None, up_bound=1.0, low_bound=0.0):
...
def rolling_allocation(f, X, n=252, s=63, ret=True, drift=True, **kwargs):
    ...
    **kwargs
    ...
    portfolio = pd.Series(100.0, index=idx, name='portfolio')
    if ret:
        X_ = X.pct_change()
    else:
        X_ = X
    ...
    for slice_n, slice_s in roll():
        ...
        w_mat.loc[roll.d, :] = w_mat.loc[roll.d, :].fillna(0.0)
        perf = _perf_alloc((X.loc[(slice_s, assets)].fillna(method='bfill').values),
          w=w,
          drift=drift)
        portfolio.loc[slice_s] = portfolio.loc[roll.d] * perf.flatten()

    w_mat = w_mat.fillna(method='ffill').fillna(0.0)
    return (
     portfolio, w_mat)
