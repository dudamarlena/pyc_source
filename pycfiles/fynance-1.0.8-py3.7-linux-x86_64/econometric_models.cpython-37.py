...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fynance/models/econometric_models.py
# Compiled at: 2019-05-23 11:31:40
...
def ARMA_GARCH(y, phi, theta, alpha, beta, c, omega, p, q, Q, P):
    ...
    u, h = ARMA_GARCH_cy(y, phi, theta, alpha, beta, float(c), float(omega), int(p), int(q), int(Q), int(P))
    return (
     u, h)
...
def ARMAX_GARCH(y, x, phi, psi, theta, alpha, beta, c, omega, p, q, Q, P):
    ...
    alpha = np.asarray(alpha, dtype=(np.float64))
    beta = np.asarray(beta, dtype=(np.float64))
    u, h = ARMAX_GARCH_cy(y, x, phi, theta, psi, alpha, beta, float(c), float(omega), int(p), int(q), int(Q), int(P))
    return (
     u, h)