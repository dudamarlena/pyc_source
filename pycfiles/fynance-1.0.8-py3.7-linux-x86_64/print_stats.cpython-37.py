...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fynance/backtest/print_stats.py
# Compiled at: 2019-03-21 06:02:08
...
def set_text_stats(underly, period=252, accur=True, perf=True, vol=True, sharp=True, calma=True, underlying='Underlying', fees=0, **kwpred):
    ...
    if perf:
        ...
        perf_targ = np.sign(perf[(-1)] / perf[0]) * np.float_power(np.abs(perf[(-1)] / perf[0]), period / perf.size) - 1.0
        ...
        for key, pred in kwpred.items():
            ...
            perf_pred = np.sign(perf[(-1)] / perf[0]) * np.float_power(np.abs(perf[(-1)] / perf[0]), period / perf.size) - 1.0
