...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fynance/tools/indicators.py
# Compiled at: 2019-08-21 03:15:06
# Size of source mod 2**32: 12016 bytes
...
def macd_hist(series, lags=9, fast_ma=12, slow_ma=26, kind_ma='ema'):
    ...
    """
    macd_lin = macd_line(series,
      fast_ma=fast_ma, slow_ma=slow_ma, kind_ma=kind_ma)
    sig_lin = signal_line(series,
      lags=lags, fast_ma=fast_ma, slow_ma=slow_ma, kind_ma=kind_ma)
    ...
    if kind_ma.lower() == 'sma':
        ...
        if kind_ma.lower() == 'ema':
            ...
            if kind_ma.lower() == 'wma':
                ...
                raise ValueError('Unknown kind_ma: {}'.format(kind_ma))
    RSI = np.zeros([T])
    RSI[1:] = 100 * ma_U / (ma_U + ma_D + 1e-08)
    return RSI
