...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fynance/tools/metrics.py
# Compiled at: 2019-08-20 10:15:33
# Size of source mod 2**32: 22675 bytes
...
def annual_return(series, period=252):
    ...
    return np.sign(ret) * np.float_power((np.abs(ret)),
      (period / T),
...
def annual_volatility(series, period=252):
    ...
    """
    return np.sqrt(period) * np.std(series[1:] / series[:-1] - 1.0)
...
def perf_index(series, base=100.0):
...
def perf_returns(returns, log=False, base=100.0):
...
def perf_strat(underlying, signals=None, log=False, base=100.0, reinvest=False):
...
def roll_calmar(series, period=252.0):
    ...
    series = np.asarray(series, dtype=(np.float64)).flatten()
    T = series.size
    t = np.arange(1.0, (T + 1.0), dtype=(np.float64))
    ret = series / series[0]
    annual_return = np.sign(ret) * np.float_power((np.abs(ret)),
      (period / t), dtype=(np.float64)) - 1.0
    roll_maxdd = roll_mdd_cy(series)
    roll_cal = np.zeros([T])
    not_null = roll_maxdd != 0.0
...
def roll_sharpe(series, period=252, win=0, cap=True):
    ...
    ret[1:] = series[1:] / series[:-1] - 1.0
    ma = series / series[0]
    ma[win:] = series[win:] / series[:-win]
    annual_return = np.sign(ma) * np.float_power((np.abs(ma)),
      (period / t), dtype=(np.float64)) - 1.0
