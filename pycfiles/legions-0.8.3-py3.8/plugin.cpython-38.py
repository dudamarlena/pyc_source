...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/legions/plugin.py
...
class LegionPlugin(PluginInterface):
    ...
    def get_opts_parser(self, add_help=True):
        ...
        """
        opts_parser = argparse.ArgumentParser(description='Legion - EVM Node Security Toolkit',
          formatter_class=(argparse.ArgumentDefaultsHelpFormatter),
          add_help=add_help)
        opts_parser.add_argument('--config',
          '-c', default='', type=str, help='Configuration File')
        opts_parser.add_argument('--verbose',
          '-v',
          action='count',
          default=0,
          help='Increase verbosity, can be specified multiple times')
        opts_parser.add_argument('--stderr',
          '-s',
          action='store_true',
          help='By default the logging output goes to a temporary file. This disables this feature by sending the logging output to stderr')
        return opts_parser
