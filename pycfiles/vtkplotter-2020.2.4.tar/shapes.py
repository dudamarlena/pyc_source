...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/shapes.py
# Compiled at: 2020-04-15 17:46:25
...
class Points(Mesh):
    ...
    def __init__(self, plist, r=5, c=(0.3, 0.3, 0.3), alpha=1):
        ...
        if n and len(plist[0]) == 2:
            plist = np.c_[(np.array(plist), np.zeros(len(plist)))]
        if utils.isSequence(c) and (len(c) > 3 or utils.isSequence(c[0]) and len(c[0]) == 4) or utils.isSequence(alpha):
            cols = c
            n = len(plist)
            if n != len(cols):
...
class Glyph(Mesh):
    ...
    def __init__(self, mesh, glyphObj, orientationArray=None, scaleByScalar=False, scaleByVectorSize=False, scaleByVectorComponents=False, colorByScalar=False, colorByVectorSize=False, tol=0, c='white', alpha=1):
        ...
        if orientationArray is not None:
            ...
            if isinstance(orientationArray, str):
                if orientationArray.lower() == 'normals':
                    ...
                    gly.SetVectorModeToUseVector()
            elif utils.isSequence(orientationArray) and not tol:
...
class Tensors(Mesh):
    ...
    def __init__(self, domain, source='ellipsoid', useEigenValues=True, isSymmetric=True, threeAxes=False, scale=1, maxScale=None, length=None, c=None, alpha=1):
...
class Lines(Mesh):
    ...
    def __init__(self, startPoints, endPoints=None, c='gray', alpha=1, lw=1, dotted=False, scale=1):
        ...
        self.lw(lw)
        if dotted:
            self.GetProperty().SetLineStipplePattern(61680)
...
class Spline(Line):
    ...
    def __init__(self, points, smooth=0.5, degree=2, closed=False, s=2, res=None):
        ...
        smooth *= maxb / 2
        tckp, _ = splprep(points.T, task=0, s=smooth, k=degree)
...
def Arrows(startPoints, endPoints=None, s=None, scale=1, c=None, alpha=1, res=12):
    ...
    """
    if isinstance(startPoints, Mesh):
        startPoints = startPoints.points()
    if isinstance(endPoints, Mesh):
    ...
    arrg = Glyph(startPoints, arr.GetOutput(), orientationArray=orients, scaleByVectorSize=True, colorByVectorSize=True, c=c, alpha=alpha).flat()
...
class Arrow2D(Mesh):
    ...
    def __init__(self, startPoint, endPoint, shaftLength=0.8, shaftWidth=0.05, headLength=0.25, headWidth=0.2, fill=True, c='r', alpha=1):
        if isinstance(startPoint, vtk.vtkActor):
            startPoint = startPoint.GetPosition()
        if isinstance(endPoint, vtk.vtkActor):
            endPoint = endPoint.GetPosition()
...
def Arrows2D(startPoints, endPoints=None, shaftLength=0.8, shaftWidth=0.09, headLength=None, headWidth=0.2, fill=True, scale=1, c=None, cmap=None, alpha=1):
    """
    ...
    arr = Arrow2D((0, 0, 0), (1, 0, 0), shaftLength, shaftWidth, headLength, headWidth, fill)
    pts = Points(startPoints, r=0.001, c=c, alpha=alpha).off()
    orients = (endPoints - startPoints) * scale
    if orients.shape[1] == 2:
        orients = np.c_[(np.array(orients), np.zeros(len(orients)))]
    arrg = Glyph(pts, arr.polydata(False), orientationArray=orients, scaleByVectorSize=True, c=c, alpha=alpha).flat().lighting('ambient')
    if c is not None:
        arrg.color(c)
    settings.collectable_actors.append(arrg)
    arrg.name = 'Arrows2D'
...
class Polygon(Mesh):
    ...
    def __init__(self, pos=(0, 0, 0), nsides=6, r=1, c='coral', alpha=1):
        if len(pos) == 2:
            pos = (
             pos[0], pos[1], 0)
        t = np.linspace(np.pi / 2, 2.5 * np.pi, num=nsides, endpoint=False)
...
class Star(Mesh):
    ...
    def __init__(self, pos=(0, 0, 0), n=5, r1=0.7, r2=1.0, line=False, c='m', alpha=1):
        if len(pos) == 2:
            pos = (
             pos[0], pos[1], 0)
        t = np.linspace(np.pi / 2, 2.5 * np.pi, num=n, endpoint=False)
...
class Disc(Mesh):
    ...
    def __init__(self, pos=(0, 0, 0), r1=0.5, r2=1, c='coral', alpha=1, res=12, resphi=None):
        if len(pos) == 2:
            pos = (
             pos[0], pos[1], 0)
...
class Arc(Mesh):
    ...
    def __init__(self, center, point1, point2=None, normal=None, angle=None, invert=False, c='grey', alpha=1, res=48):
        if len(point1) == 2:
            point1 = (
             point1[0], point1[1], 0)
...
class Ellipsoid(Mesh):
    ...
    def __init__(self, pos=(0, 0, 0), axis1=(1, 0, 0), axis2=(0, 2, 0), axis3=(0, 0, 3), c='c', alpha=1, res=24):
        self.name = 'Ellipsoid'
        self.center = pos
        self.va_error = 0
        self.vb_error = 0
    ...
    def asphericity(self):
        ...
        asp = (((a - b) / (a + b)) ** 2 + ((a - c) / (a + c)) ** 2 + ((b - c) / (b + c)) ** 2) / 3.0 * 4.0
        return asp
    ...
    def asphericity_error(self):
        ...
        dL2 = ea ** 2 * (-8 * (a - b) ** 2 / (3 * (a + b) ** 3) - 8 * (a - c) ** 2 / (3 * (a + c) ** 3) + 4 * (2 * a - 2 * c) / (3 * (a + c) ** 2) + 4 * (2 * a - 2 * b) / (3 * (a + b) ** 2)) ** 2 + eb ** 2 * (4 * (-2 * a + 2 * b) / (3 * (a + b) ** 2) - 8 * (a - b) ** 2 / (3 * (a + b) ** 3) - 8 * (-b + c) ** 2 / (3 * (b + c) ** 3) + 4 * (2 * b - 2 * c) / (3 * (b + c) ** 2)) ** 2 + ec ** 2 * (4 * (-2 * a + 2 * c) / (3 * (a + c) ** 2) - 8 * (a - c) ** 2 / (3 * (a + c) ** 3) + 4 * (-2 * b + 2 * c) / (3 * (b + c) ** 2) - 8 * (-b + c) ** 2 / (3 * (b + c) ** 3)) ** 2
        err = np.sqrt(dL2)
        self.va_error = ea
        self.vb_error = eb
        self.vc_error = ec
...
class Grid(Mesh):
    ...
    def __init__(self, pos=(0, 0, 0), normal=(0, 0, 1), sx=1, sy=1, sz=(0, ), c='gray', alpha=1, lw=1, resx=10, resy=10):
        if len(pos) == 2:
            pos = (
             pos[0], pos[1], 0)
...
def CubicGrid(pos=(0, 0, 0), n=(10, 10, 10), spacing=(), c='lightgrey', alpha=0.1):
    ...
    if len(spacing) == 3:
        ...
        img.SetSpacing(1.0 / n[0], 1.0 / n[1], 1.0 / n[2])
...
class Spring(Mesh):
    ...
    def __init__(self, startPoint=(0, 0, 0), endPoint=(1, 0, 0), coils=20, r=0.1, r2=None, thickness=None, c='grey', alpha=1):
...
class Text(Mesh):
    ...
    def __init__(self, txt, pos=(0, 0, 0), s=1, depth=0, justify='bottom-left', c=None, alpha=1, bc=None):
...
def Text2D(txt, pos=3, s=1, c=None, alpha=1, bg=None, font='courier', justify='bottom-left'):
...
class Latex(Picture):
    ...
    def __init__(self, formula, pos=(0, 0, 0), c='k', s=1, bg=None, alpha=1, res=30, usetex=False, fromweb=False):
