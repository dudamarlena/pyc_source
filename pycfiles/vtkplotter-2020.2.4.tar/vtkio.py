...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/vtkio.py
# Compiled at: 2020-04-13 12:59:34
from __future__ import division, print_function
import vtk
from vtk.util.numpy_support import vtk_to_numpy
import os, numpy as np, vtkplotter.utils as utils, vtkplotter.colors as colors
...
import vtkplotter.docs as docs, vtkplotter.settings as settings
...
def _load_file(filename, c, alpha, threshold, spacing, unpack):
    ...
    if fl.endswith('.xml') or fl.endswith('.xml.gz') or fl.endswith('.xdmf'):
    ...
    elif fl.endswith('.neutral') or fl.endswith('.neu'):
        actor = loadNeutral(filename)
    elif fl.endswith('.gmsh'):
        actor = loadGmesh(filename)
    elif fl.endswith('.pcd'):
        ...
        if fl.endswith('.3ds'):
            ...
            if fl.endswith('.wrl'):
                ...
                if fl.endswith('.tif') or fl.endswith('.slc') or fl.endswith('.vti') or fl.endswith('.mhd') or fl.endswith('.nrrd') or fl.endswith('.nii') or fl.endswith('.dem'):
                    ...
                    if fl.endswith('.png') or fl.endswith('.jpg') or fl.endswith('.bmp') or fl.endswith('.jpeg'):
                        if '.png' in fl:
                        ...
                        if fl.endswith('.vtm') or fl.endswith('.vtmb'):
...
def gunzip(filename):
    ...
    tmp_file.name = os.path.join(os.path.dirname(tmp_file.name), os.path.basename(filename).replace('.gz', ''))
...
def loadNumpy(inobj):
    ...
    def _buildactor(d):
        ...
        if not len(vertices):
            ...
            if 'representation' in keys:
                prp.SetRepresentation(d['representation'])
            if 'color' in keys and d['color']:
                act.color(d['color'])
...
def _np_dump(obj):
    ...
    def _doactor(obj, adict):
        ...
        if poly.GetCellData().GetScalars():
            adict['activedata'] = [
             'celldata', poly.GetCellData().GetScalars().GetName()]
        if poly.GetPointData().GetScalars():
            adict['activedata'] = [
             'pointdata', poly.GetPointData().GetScalars().GetName()]
...
def write(objct, fileoutput, binary=True):
    ...
    if isinstance(obj, Mesh):
        ...
        if isinstance(obj, (vtk.vtkActor, vtk.vtkVolume)):
            obj = objct.GetMapper().GetInput()
        else:
            if isinstance(obj, (vtk.vtkPolyData, vtk.vtkImageData)):
                obj = objct
            fr = fileoutput.lower()
            if fr.endswith('.vtk'):
                        ...
                        if fr.endswith('.xml'):
                            ...
                            if len(faces[0]) == 4:
                                ...
                                for i in range(ntets):
                                    v0, v1, v2, v3 = faces[i]
                                    outF.write('     <tetrahedron index="' + str(i) + '" v0="' + v0 + '" v1="' + v1 + '" v2="' + v2 + '" v3="' + v3 + '"/>\n')
...
def exportWindow(fileoutput, binary=False, speed=None, html=True):
    ...
    if fr.endswith('.obj'):
        ...
        outF.write(x3d_html)
        outF.close()
        colors.printc('~save Saved files:', fileoutput, fileoutput.replace('.x3d', '.html'), c='g')
    elif fr.endswith('.npy'):
        sdict = dict()
        vp = settings.plotter_instance
        sdict['shape'] = vp.shape
        sdict['sharecam'] = vp.sharecam
        sdict['camera'] = None
...
def importWindow(fileinput, mtlFile=None, texturePath=None):
    ...
    if '.npy' in fileinput:
        ...
        vp = Plotter(size=data['size'], axes=axes, title=title, bg=backgrcol)
...
class Video:
    ...
    def __init__(self, name='movie.mp4', duration=None, fps=24, backend='ffmpeg'):
    ...
    def action(self, elevation_range=(0, 80), azimuth_range=(0, 359), zoom=None, cam1=None, cam2=None):
    ...
    def close(self):
        ...
        if self.backend == 'ffmpeg':
            out = os.system(self.command + ' ' + str(self.fps) + ' -i ' + self.tmp_dir.name + os.sep + '%01d.png ' + self.name)
            if out:
