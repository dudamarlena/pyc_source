...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/mesh.py
# Compiled at: 2020-04-15 08:30:45
from __future__ import division, print_function
import numpy as np, os, vtk, vtkplotter.colors as colors, vtkplotter.docs as docs, vtkplotter.settings as settings, vtkplotter.utils as utils
from vtkplotter.base import ActorBase
from vtk.util.numpy_support import numpy_to_vtk, vtk_to_numpy
__doc__ = 'Submodule extending the ``vtkActor`` object functionality.' + docs._defs
__all__ = [
 'Mesh', 'merge']
...
class Mesh(vtk.vtkFollower, ActorBase):
    ...
    def __init__(self, inputobj=None, c=None, alpha=1, computeNormals=False):
        ...
        elif hasattr(inputobj, 'GetOutput'):
    ...
    def points(self, pts=None, transformed=True, copy=False):
        ...
        if pts is None:
            ...
            if utils.isSequence(pts) and not utils.isSequence(pts[0]) or isinstance(pts, (int, np.integer)):
    ...
    def addScalarBar(self, pos=(0.8, 0.05), title='', titleXOffset=0, titleYOffset=15, titleFontSize=12, nlabels=10, c=None, horizontal=False, vmin=None, vmax=None):
        """
        ...
        self.scalarbar = addons.addScalarBar(self, pos, title, titleXOffset, titleYOffset, titleFontSize, nlabels, c, horizontal, vmin, vmax)
        return self

    def addScalarBar3D(self, pos=(0, 0, 0), normal=(0, 0, 1), sx=0.1, sy=2, title='', titleXOffset=-1.4, titleYOffset=0.0, titleSize=1.5, titleRotation=0.0, nlabels=9, precision=3, labelOffset=0.4, c=None, alpha=1, cmap=None):
        """
        ...
        """
        import vtkplotter.addons as addons
        self.scalarbar = addons.addScalarBar3D(self, pos, normal, sx, sy, title, titleXOffset, titleYOffset, titleSize, titleRotation, nlabels, precision, labelOffset, c, alpha, cmap)
        return self.scalarbar

    def texture(self, tname, tcoords=None, interpolate=True, repeat=True, edgeClamp=False):
            ...
            if isinstance(tname, vtk.vtkTexture):
                ...
                if tcoords is not None:
                    ...
                    if os.path.exists(tname):
                        ...
                        colors.printc('File does not exist or texture', tname, 'not found in', settings.textures_path, c='r')
    ...
    def clone(self):
        ...
        if self.trail:
            ...
            cloned.addTrail(self.trailOffset, self.trailSegmentSize * n, n, None, None, self.trail.GetProperty().GetLineWidth())
        if self.shadow:
            cloned.addShadow(self.shadowX, self.shadowY, self.shadowZ, self.shadow.GetProperty().GetColor(), self.shadow.GetProperty().GetOpacity())
        return cloned
    ...
    def stretch(self, q1, q2):
        ...
        if self.trail:
            self.updateTrail()
        if self.shadow:
            self.addShadow(self.shadowX, self.shadowY, self.shadowZ, self.shadow.GetProperty().GetColor(), self.shadow.GetProperty().GetOpacity())
        return self
    ...
    def cutWithPlane(self, origin=(0, 0, 0), normal=(1, 0, 0), returnCut=False):
        ...
        clipper = vtk.vtkClipPolyData()
        clipper.SetInputData(self.polydata(True))
    ...
    def triangulate(self, verts=True, lines=True):
        ...
        if self._polydata.GetNumberOfPolys() or self._polydata.GetNumberOfStrips():
            ...
            if self._polydata.GetNumberOfLines():
    ...
    def pointColors(self, scalars_or_colors, cmap='jet', alpha=1, mode='scalars', bands=None, vmin=None, vmax=None):
        """
        ...
        poly = self.polydata(False)
        if isinstance(scalars_or_colors, str):
        ...
        if n != poly.GetNumberOfPoints():
            colors.printc('Error in pointColors(): nr. of scalars != nr. of points', n, poly.GetNumberOfPoints(), c=1)
        if utils.isSequence(alpha):
            useAlpha = True
            if len(alpha) > n:
                colors.printc('Error in pointColors(): nr. of scalars < nr. of alpha values', n, len(alpha), c=1)
                raise RuntimeError()
        if bands:
    ...
    def cellColors(self, scalars_or_colors, cmap='jet', alpha=1, alphaPerCell=False, mode='scalars', bands=None, vmin=None, vmax=None):
        """
        ...
        if 'color' in mode:
            ...
            if n != poly.GetNumberOfCells():
                colors.printc('Error in cellColors(): nr. of scalars != nr. of cells', n, poly.GetNumberOfCells(), c=1)
            if utils.isSequence(alpha):
                useAlpha = True
                if len(alpha) > n:
                    colors.printc('Error in cellColors(): nr. of scalars != nr. of alpha values', n, len(alpha), c=1)
                    raise RuntimeError()
            if bands:
                scalars_or_colors = utils.makeBands(scalars_or_colors, bands)
            if vmin is None:
    ...
    def _cellColors1By1(self, acolors, alphas, alphaPerCell):
        ...
        if len(acolors) != n or utils.isSequence(alphas) and len(alphas) != n:
            colors.printc('Error in cellColors(): mismatch in input list sizes.', len(acolors), n, c=1)
            return self
        lut = vtk.vtkLookupTable()
        if alphaPerCell:
            lut.SetNumberOfTableValues(n)
            ...
            ucolors, uids, inds = np.unique(acolors, axis=0, return_index=True, return_inverse=True)
            nc = len(ucolors)
            if nc == 1:
    ...
    def updateTrail(self):
        ...
        if lastpos is None:
            self.trailPoints = [
             currentpos] * len(self.trailPoints)
            return
        else:
            if np.linalg.norm(currentpos - lastpos) < self.trailSegmentSize:
                return
            self.trailPoints.append(currentpos)
    ...
    def boundaries(self, boundaryEdges=True, featureAngle=65, nonManifoldEdges=True, returnPointIds=False, returnCellIds=False):
        """
    ...
    def labels(self, content=None, cells=False, scale=None, ratio=1, precision=3):
        ...
        if content is None:
            ...
            if len(arr) != len(content):
                colors.printc('Error in labels(): array length mismatch', len(arr), len(content), c=1)
                return
        if arr is None and mode == 0:
            ...
            tapp.Update()
            ids = Mesh(tapp.GetOutput(), c=[0.5, 0.5, 0.5]).pickable(0)
