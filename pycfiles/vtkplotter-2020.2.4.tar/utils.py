...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/utils.py
# Compiled at: 2020-04-15 17:48:15
from __future__ import division, print_function
import vtk, sys
from vtk.util.numpy_support import numpy_to_vtk, numpy_to_vtkIdTypeArray
import numpy as np, vtkplotter.colors as colors, vtkplotter.docs as docs, time
__doc__ = '\nUtilities submodule.\n' + docs._defs
__all__ = [
 'ProgressBar',
 'geometry',
...
class ProgressBar():
    ...
    def print(self, txt='', counts=None):
        ...
        if self.bar != self._oldbar:
            ...
            eraser = [' '] * self._lentxt + ['\x08'] * self._lentxt
            eraser = ('').join(eraser)
            if self.ETA:
...
def findDistanceToLines2D(P0, P1, pts):
    ...
    def distance(P0, P1, p):
        T = P1 - P0
        L = (T ** 2).sum(axis=1)
        U = -((P0[:, 0] - p[(Ellipsis, 0)]) * T[:, 0] + (P0[:, 1] - p[(Ellipsis, 1)]) * T[:, 1]) / L
        U = U.reshape(len(U), 1)
        D = P0 + U * T - p
...
def printInfo(obj):
    ...
    def printvtkactor(actor, tab=''):
        if not actor.GetPickable():
            ...
            if not actor.GetMapper().GetScalarVisibility():
                ...
                colors.printc(colors.getColorName(col) + ', rgb=(' + colr + ', ' + colg + ', ' + colb + '), alpha=' + str(alpha), c='g', bold=0)
                if actor.GetBackfaceProperty():
                    ...
                    colors.printc(colors.getColorName(bcol) + ', rgb=(' + bcolr + ', ' + bcolg + ', ' + bcolb + ')', c='g', bold=0)
            ...
            if ptdata.GetNumberOfArrays() + cldata.GetNumberOfArrays():
                ...
                colors.printc(mapper.GetScalarModeAsString(), '  coloring =', mapper.GetColorModeAsString(), c='g', bold=0)
    ...
    if not obj:
        ...
        if isinstance(obj, vtk.vtkActor):
            ...
            printHistogram(obj, horizontal=True, logscale=True, bins=8, height=15, c='b', bold=0)
        elif hasattr(obj, 'interactor'):
            axtype = {0: '(no axes)', 1: '(three customizable gray grid walls)', 
            ...
            for a in obj.getVolumes():
                if a.GetBounds() is not None:
                    ...
                    colors.printc('      scalar range:', np.round(img.GetScalarRange(), 4), c='b', bold=0)
                    bnds = a.GetBounds()
...
def printHistogram(data, bins=10, height=10, logscale=False, minbin=0, horizontal=False, char='▉', c=None, bold=True, title='Histogram'):
    """
    ...
    if minbin:
        ...
        hi = h[0]
    if sys.version_info[0] < 3 and char == '▉':
        char = '*'
...
def orientedCamera(center=(
 0, 0, 0), upVector=(0, 1, 0), backoffVector=(0, 0, 1), backoff=1):
    """
...
def make_ticks(x0, x1, N, labels=None):
    ...
    if np.sign(x0) < 0:
        lowBound *= 10
    if lowBound > 0 and lowBound - dstep < 0:
        lowBound = 0
    else:
        if lowBound < 0 and lowBound + dstep > 0:
    ...
    if 'Trimesh' in inputobj_type or 'primitives' in inputobj_type:
        ...
        if isSequence(trim_c):
            if isSequence(trim_c[0]):
                sameColor = len(np.unique(trim_c, axis=0)) < 2
                trim_c = trim_c / 255
                if sameColor:
                    tact.c(trim_c[(0, [0, 1, 2])]).alpha(trim_c[(0, 3)])
                elif inputobj.visual.kind == 'face':
                    tact.cellColors(trim_c[:, [0, 1, 2]], mode='colors', alpha=trim_c[:, 3], alphaPerCell=alphaPerCell)
                else:
                    tact.pointColors(trim_c[:, [0, 1, 2]], mode='colors', alpha=trim_c[3])
        return tact
    if 'PointCloud' in inputobj_type:
        ...
        if hasattr(inputobj, 'vertices_color'):
            ...
            if len(trim_c):
                ...
                trim_al = np.sum(trim_al) / len(trim_al)
