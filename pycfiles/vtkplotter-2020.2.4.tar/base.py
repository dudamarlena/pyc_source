...
# Compiled at: 2020-04-06 15:46:10
...
class ActorBase(object):
    ...
    def addPointScalars(self, scalars, name):
        ...
        if len(scalars) != data.GetNumberOfPoints():
            colors.printc('~times addPointScalars(): Number of scalars != nr. of points', len(scalars), data.GetNumberOfPoints(), c=1)
            raise RuntimeError()
    ...
    def addCellScalars(self, scalars, name):
        ...
        if len(scalars) != data.GetNumberOfCells():
            colors.printc('addCellScalars() Error: Number of scalars != nr. of cells', len(scalars), data.GetNumberOfCells(), c=1)
            raise RuntimeError()
        arr = numpy_to_vtk(np.ascontiguousarray(scalars), deep=True)
        arr.SetName(name)
    ...
    def addPointVectors(self, vectors, name):
        ...
        if len(vectors) != data.GetNumberOfPoints():
            colors.printc('addPointVectors Error: Number of vectors != nr. of points', len(vectors), data.GetNumberOfPoints(), c=1)
            raise RuntimeError()
    ...
    def addCellVectors(self, vectors, name):
        ...
        if len(vectors) != data.GetNumberOfCells():
            colors.printc('addCellVectors Error: Number of vectors != nr. of cells', len(vectors), data.GetNumberOfCells(), c=1)
            raise RuntimeError()
        arr = vtk.vtkFloatArray()
        arr.SetNumberOfComponents(3)
        arr.SetName(name)
