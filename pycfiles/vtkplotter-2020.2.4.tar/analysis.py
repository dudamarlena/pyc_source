...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/analysis.py
# Compiled at: 2020-04-15 08:04:31
from __future__ import division, print_function
import vtkplotter.docs as docs, vtk, numpy as np
from vtk.util.numpy_support import numpy_to_vtk, vtk_to_numpy
import vtkplotter.utils as utils, vtkplotter.colors as colors, vtkplotter.shapes as shapes
...
def alignLandmarks(source, target, rigid=False):
    ...
    if source.N() != target.N():
        colors.printc('~times Error in alignLandmarks(): Source and Target with != nr of points!', source.N(), target.N(), c=1)
        raise RuntimeError()
...
def pcaEllipsoid(points, pvalue=0.95):
    ...
    if len(coords) < 4:
        ...
        U, s, R = np.linalg.svd(cov)
        p, n = s.size, P.shape[0]
        fppf = f.ppf(pvalue, p, n - p) * (n - 1) * p * (n + 1) / n / (n - p)
        cfac = 1 + 6 / (n - 1)
        ua, ub, uc = np.sqrt(s * fppf) / cfac
...
def smoothMLS3D(meshs, neighbours=10):
    ...
    for i in pb.range():
        ...
        if nc >= neighbours and nc > 5:
            m = np.linalg.lstsq(closest, [1.0] * nc)[0]
            vers = m / np.linalg.norm(m)
            hpcenter = np.mean(closest, axis=0)
    ...
    ccoords3d = np.delete(newcoords4d, 3, axis=1)
    act = shapes.Points(ccoords3d)
    act.pointColors(ctimes, cmap='jet')
    return act
...
def recoSurface(pts, dims=(250, 250, 250), radius=None, sampleSize=None, holeFilling=True, bounds=(), pad=0.1):
    """
    ...
    if len(bounds) == 6:
        ...
        sdf.SetBounds(x0 - (x1 - x0) * pad, x1 + (x1 - x0) * pad, y0 - (y1 - y0) * pad, y1 + (y1 - y0) * pad, z0 - (z1 - z0) * pad, z1 + (z1 - z0) * pad)
...
def removeOutliers(points, radius):
    ...
    print('# of removed outlier points: ', removal.GetNumberOfPointsRemoved(), '/', poly.GetNumberOfPoints())
    outpts = []
...
def probePlane(vol, origin=(0, 0, 0), normal=(1, 0, 0)):
    ...
    planeCut.Update()
    cutmesh = Mesh(planeCut.GetOutput(), c=None)
...
def interpolateToVolume(mesh, kernel='shepard', radius=None, bounds=None, nullValue=None, dims=(20, 20, 20)):
    """
    ...
    probe.SetSpacing((bounds[1] - bounds[0]) / (dims[0] - 1), (bounds[3] - bounds[2]) / (dims[1] - 1), (bounds[5] - bounds[4]) / (dims[2] - 1))
    if radius is None:
...
def interpolateToStructuredGrid(mesh, kernel=None, radius=None, bounds=None, nullValue=None, dims=None):
    """
...
def streamLines(domain, probe, activeVectors='', integrator='rk4', direction='forward', initialStepSize=None, maxPropagation=None, maxSteps=10000, stepLength=None, extrapolateToBoundingBox=(), surfaceConstrain=False, computeVorticity=True, ribbons=None, tubes={}, scalarRange=None, lw=None):
    """
    ...
    if len(tubes):
        ...
        streamTube.SetInputArrayToProcess(1, 0, 0, vtk.vtkDataObject.FIELD_ASSOCIATION_POINTS, vname)
        streamTube.Update()
...
def signedDistanceFromPointCloud(mesh, maxradius=None, bounds=None, dims=(20, 20, 20)):
    ...
    if maxradius is None:
        maxradius = mesh.diagonalSize() / 10.0
