...
# Compiled at: 2020-04-10 09:21:06
"""
...
import vtkplotter.shapes as shapes, vtkplotter.settings as settings, vtkplotter.docs as docs, numpy as np, vtk
__doc__ = '\nAdditional objects like axes, legends etc..\n' + docs._defs
__all__ = [
 'addLight',
 'addScalarBar',
...
def addScalarBar(mesh, pos=(
 0.8, 0.05), title='', titleXOffset=0, titleYOffset=15, titleFontSize=12, nlabels=10, c=None, horizontal=False, vmin=None, vmax=None):
    ...
    if not hasattr(mesh, 'mapper'):
        ...
        if horizontal:
            ...
            sb.SetNumberOfLabels(int((nlabels - 1) / 2.0))
...
def addScalarBar3D(obj, pos=(
 0, 0, 0), normal=(
 0, 0, 1), sx=0.1, sy=2, title='', titleXOffset=-1.5, titleYOffset=0.0, titleSize=1.5, titleRotation=0.0, nlabels=9, prec=2, labelOffset=0.4, c=None, alpha=1, cmap=None):
    ...
    if isinstance(obj, Mesh):
        ...
        if isSequence(obj):
            ...
            raise RuntimeError()
        scale = shapes.Grid([-sx * labelOffset, 0, 0], c=c, alpha=alpha, sx=sx, sy=sy, resx=1, resy=256)
...
def addSlider2D(sliderfunc, xmin, xmax, value=None, pos=4, title='', c=None, showValue=True):
...
def addSlider3D(sliderfunc, pos1, pos2, xmin, xmax, value=None, s=0.03, title='', rotation=0, c=None, showValue=True):
    ...
    if value is None or value < xmin:
        value = xmin
    t = 1.5 / np.sqrt(mag(np.array(pos2) - pos1))
 ...
 1, 0, 0), yHighlightZeroColor=(0, 1, 0), zHighlightZeroColor=(0, 0, 1), showTicks=True, xTickLength=0.015, yTickLength=0.015, zTickLength=0.015, xTickThickness=0.0025, yTickThickness=0.0025, zTickThickness=0.0025, xTickColor=None, yTickColor=None, zTickColor=None, xMinorTicks=1, yMinorTicks=1, zMinorTicks=1, tipSize=None, xLabelSize=0.0175, yLabelSize=0.0175, zLabelSize=0.0175, xLabelOffset=0.015, yLabelOffset=0.015, zLabelOffset=0.01, xPositionsAndLabels=None, yPositionsAndLabels=None, zPositionsAndLabels=None, useGlobal=False, tol=0.0001):
    ...
    if not ssmax:
        ...
        if xtitle:
            ...
            titles.append(xt)
        if ytitle:
            yt = shapes.Text(ytitle, pos=(0, 0, 0), s=yTitleSize, bc=yTitleBackfaceColor, c=yTitleColor, justify=yTitleJustify, depth=titleDepth)
            if reorientShortTitle and len(ytitle) < 3:
                wpos = [
            ...
            titles.append(yt)
        if ztitle:
            zt = shapes.Text(ztitle, pos=(0, 0, 0), s=zTitleSize, bc=zTitleBackfaceColor, c=zTitleColor, justify=zTitleJustify, depth=titleDepth)
