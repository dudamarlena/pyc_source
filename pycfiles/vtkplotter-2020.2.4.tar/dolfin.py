...
# [GCC 8.4.0]
# Embedded file name: /home/musy/Projects/vtkplotter/vtkplotter/dolfin.py
# Compiled at: 2020-03-29 19:13:06
from __future__ import division, print_function
import vtk
from vtk.util.numpy_support import numpy_to_vtk
import numpy as np, vtkplotter.utils as utils
...
def _inputsort_dolfinx(obj):
    ...
    if u:
        print('u.value_rank', u.value_rank)
    if u and u.value_rank:
        print('u.value_dimension', u.value_dimension(0))
    if u:
        print('u.value_shape', u.value_shape())
    return (
     mesh, u)
...
def _compute_uvalues(u, mesh):
    if not u:
        return
    if hasattr(u, 'compute_vertex_values'):
    ...
    elif hasattr(u, 'compute_point_values'):
   ...
   :param float warpZfactor: elevate z-axis by scalar value (useful for 2D geometries)
    ...
    if len(inputobj) == 0:
        ...
        if style == 0 or style == 'vtk':
            ...
            if style == 1 or style == 'matplotlib':
                font = 'courier'
                ...
                if mesh and ('mesh' in mode or 'color' in mode or 'displace' in mode):
                    ...
                    if cmap and actor.u_values is not None and c is None:
                        if u.value_rank() > 0:
                            actor.pointColors(utils.mag(actor.u_values), cmap=cmap, bands=bands, vmin=vmin, vmax=vmax)
                        else:
                            actor.pointColors(actor.u_values, cmap=cmap, bands=bands, vmin=vmin, vmax=vmax)
                    if 'displace' in mode:
                        actor.move(u)
...
class MeshActor(Mesh):
    ...
    def __init__(self, *inputobj, **options):
        ...
        if not mesh:
            ...
            Mesh.__init__(self, poly, c=c, alpha=alpha, computeNormals=computeNormals)
            self.mesh = mesh
            self.u = u
    ...
    def move(self, u=None, deltas=None):
        ...
        if coords.shape != deltas.shape:
            printc('ERROR: Try to move mesh with wrong solution type shape:', coords.shape, 'vs', deltas.shape, c=1)
            printc("Mesh is not moved. Try mode='color' in plot().", c=1)
            return
        else:
            movedpts = coords + deltas
            if movedpts.shape[1] == 2:
...
def MeshPoints(*inputobj, **options):
    ...
    if not mesh:
        ...
        if len(plist[0]) == 1:
            plist = np.insert(plist, 1, 0, axis=1)
            plist = np.insert(plist, 2, 0, axis=1)
...
def MeshArrows(*inputobj, **options):
    ...
    if not mesh:
        ...
        if u_values.shape[1] == 2:
            ...
            startPoints = np.insert(startPoints, 2, 0, axis=1)
            endPoints = np.insert(endPoints, 2, 0, axis=1)
        actor = shapes.Arrows(startPoints, endPoints, s=s, scale=scale, alpha=alpha, res=res)
        actor.color(c)
        actor.mesh = mesh
        actor.u = u
        actor.u_values = u_values
...
def MeshStreamLines(*inputobj, **options):
    ...
    if not mesh:
        ...
        str_lns = streamLines(meshact, probes, direction=direction, maxPropagation=maxPropagation, tubes=tubes, scalarRange=scalarRange, activeVectors='u_values')
        if lw:
            str_lns.lw(lw)
        return str_lns