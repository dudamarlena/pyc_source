...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/quebert/test/test_mediator.py
# Compiled at: 2009-08-21 14:12:21
try:
    import json
except:
...
class TestMediator(unittest.TestCase):
    ...
    def test_makeRequest(self):
        ...
        t.makeRequest('http://localhost:8000/foo', ARG).addCallback(self.assertEquals, RESULT)
        task = dict(url='/bar', params=json.dumps('hello'))
    ...
    def test_taskReceived(self):
        ...
        RESULT = dict(res='ok')
        task = dict(url='/bar', params=json.dumps(ARG))
        l = []
        ...
        t.addPosthook(hook)
        t.addPosthook(hook)
        t.taskReceived(dict(task)).addCallback(lambda _: self.assertEquals(l, [1, 1, 1, 1]))
        l = []
        t = m.Mediator(getPage=getPage)
        t.taskReceived(dict(task)).addCallback(lambda _: self.assertEquals(l, []))
        t = m.Mediator(getPage=getPage)
