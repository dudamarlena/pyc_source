...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/quebert/client.py
# Compiled at: 2011-03-28 13:15:50
try:
    import json
except ImportError:
...
def add_task(url, params=None, kind='tasks', ttl=0, run_at=None, uuid=None, priority=MEDIUM, encode=json.dumps, meta_encode=json.dumps, **kw):
    """
    ...
    payload = {'url': url, 'params': encode(params), 
    ...
    payload.update(kw)
    add_raw_task(meta_encode(payload), kind, priority, rmq_username, rmq_password, rmq_host, rmq_exchange)
    return uuid


def add_raw_task(body, kind='tasks', priority=MEDIUM, user='guest', pwd='guest', host='localhost', exchange='flow'):
    """
