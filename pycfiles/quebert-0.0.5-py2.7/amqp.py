...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/quebert/amqp.py
...
gen_id = itertools.count(1).next

class Watcher(object):

    def __init__(self, queueName, queue, channel, exchange, routingKey, reactor=reactor, decode=json.loads, encode=json.dumps):
    ...
    def requeue(self, reason, mediator, message, original):
        ...
        def _cb(newtask):
            ...
            content = Content(newcontent, original_content.children, original_content.properties)
            return self.channel.basic_publish(exchange=self.exchange, routing_key=self.routingKey, content=content)
    ...
    def wrappedCallback(self, mediator, msg):
        ...
        * I check for new messages again.
        ...
        return defer.maybeDeferred(mediator.taskReceived, message).addErrback(self.requeue, mediator, message, msg).addCallback(lambda _: self.channel.basic_ack(msg.delivery_tag)).addErrback(_eb)
...
class TxAMQPClient(object):
    ...
    def __init__(self, host, username, password, reactor=reactor, watcherFactory=None):
        self.host = host
    ...
    def connect(self):
        ...
        return ClientCreator(reactor, AMQClient, delegate, '/', amqp08).connectTCP(self.host, 5672)
    ...
    def setup_connection(self, exchange, exchangeType='topic'):
        ...
        yield channel.exchange_declare(exchange=exchange, type=exchangeType, durable=True, auto_delete=False)
        defer.returnValue((client, channel))

    @defer.inlineCallbacks
    def watch(self, exchange, queue, routingKey, exchangeType='topic', decode=json.loads, encode=json.dumps):
        """
        ...
        yield channel.queue_bind(queue=queue, exchange=exchange, routing_key=routingKey)
        reply = yield channel.basic_consume(queue=queue)
        q = yield client.queue(reply.consumer_tag)
        log.msg('Waiting for messages...')
        w = self.watcherFactory(queue, q, channel, exchange, routingKey, self.reactor, decode, encode)
        defer.returnValue(w)