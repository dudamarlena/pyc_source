...
# Compiled at: 2015-11-05 10:24:24
...
class Server(RemoteServer):
    ...
    def __init__(self, path='browsermob-proxy', options={}):
        ...
        if not os.path.isfile(path) and exec_not_on_path:
            raise Exception("Browsermob-Proxy binary couldn't be found in path provided: %s" % path)
        self.path = path
        self.host = 'localhost'
    ...
    def start(self):
        ...
        self.process = subprocess.Popen(self.command, shell=True, stdout=self.log_file, stderr=subprocess.STDOUT)
        count = 0
    ...
    def stop(self):
        ...
        try:
            ...
            subprocess.Popen(self.command, shell=True, stdout=self.log_file, stderr=subprocess.STDOUT)
