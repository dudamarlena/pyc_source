...
# [GCC 8.4.0]
# Embedded file name: C:\Users\baven_000\Documents\GitHub\tmmurali\halp\build\lib\halp\undirected_hypergraph.py
...
class UndirectedHypergraph(object):
    ...
    def add_hyperedge(self, nodes, attr_dict=None, **attr):
        ...
        if is_new_hyperedge:
            ...
            self._hyperedge_attributes[hyperedge_id] = {'nodes': nodes, '__frozen_nodes': frozen_nodes, 'weight': 1}
        else:
    ...
    def read(self, file_name, delim=',', sep='\t'):
        ...
        for line in in_file.readlines():
            ...
            if not 1 <= len(words) <= 2:
                raise IOError(('Line {} ').format(line_number) + ('contains {} ').format(len(words)) + 'columns -- must contain only 1 or 2.')
