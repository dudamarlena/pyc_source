...
# Embedded file name: /home/alonso/Science/Codes/ReformCodes/NaMaster/test/test_nmt_field.py
# Compiled at: 2020-03-08 16:30:02
...
class TestFieldCAR(unittest.TestCase):
    ...
    def setUp(self):
        ...
        sth = np.sin(th)
        cth = np.cos(th)
        self.mps[0, :, :] = np.sqrt(7.5 / np.pi) * sth ** 2 * np.cos(2 * ph)
        self.mps[1, :, :] = -np.sqrt(7.5 / np.pi) * sth ** 2 / 4.0
        self.mps[2, :, :] = -np.sqrt(52.5 / np.pi) * cth * sth ** 2 / 2.0
        for i in range(self.ntemp):
            self.tmp[i][0, :, :] = np.sqrt(7.5 / np.pi) * sth ** 2 * np.cos(2 * ph)
            self.tmp[i][1, :, :] = -np.sqrt(7.5 / np.pi) * sth ** 2 / 4.0
            self.tmp[i][2, :, :] = -np.sqrt(52.5 / np.pi) * cth * sth ** 2 / 2.0
    ...
    def test_field_alloc(self):
        ...
        self.assertTrue(1e-05 * np.mean(np.fabs(f2p.get_maps()[0])) > np.mean(np.fabs(f2p.get_maps()[0] - (self.mps[1] * self.msk).flatten())))
        self.assertTrue(1e-05 * np.mean(np.fabs(f2p.get_maps()[1])) > np.mean(np.fabs(f2p.get_maps()[1] - (self.mps[2] * self.msk).flatten())))
        self.assertEqual(len(f0.get_templates()), 0)
        self.assertEqual(len(f2.get_templates()), 0)
        self.assertEqual(len(f2p.get_templates()), 0)
        f0 = nmt.NmtField((self.msk), [self.mps[0]], templates=(np.array([[t[0]] for t in self.tmp])),
          beam=(self.beam),
          wcs=(self.wcs))
        f2 = nmt.NmtField((self.msk), [self.mps[1], self.mps[2]], templates=(np.array([[t[1], t[2]] for t in self.tmp])),
          beam=(self.beam),
          wcs=(self.wcs))
    ...
    def test_field_error(self):
        ...
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], beam=(self.beam),
              wcs=wcs)
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk[:90]), [self.mps[0]], beam=(self.beam),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0, :90]], beam=(self.beam),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), (self.mps), wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=(self.tmp),
              beam=(self.beam),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=[[t[0], t[1]] for t in self.tmp],
              beam=(self.beam),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=[[t[0, :90]] for t in self.tmp],
              beam=(self.beam),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=1,
              beam=(self.beam),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=[[t[0]] for t in self.tmp],
              beam=(self.beam[:90]),
              wcs=(self.wcs))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=[[t[0]] for t in self.tmp],
...
class TestFieldHPX(unittest.TestCase):
    ...
    def test_field_masked(self):
        ...
        f2_msk = nmt.NmtField(msk, [mps_msk[1], mps_msk[2]], masked_on_input=True)
    ...
    def test_field_alloc(self):
        ...
        self.assertTrue(1e-05 * np.mean(np.fabs(f2p.get_maps()[0])) > np.mean(np.fabs(f2p.get_maps()[0] - self.mps[1] * self.msk)))
        self.assertTrue(1e-05 * np.mean(np.fabs(f2p.get_maps()[1])) > np.mean(np.fabs(f2p.get_maps()[1] - self.mps[2] * self.msk)))
        self.assertEqual(len(f0.get_templates()), 0)
        self.assertEqual(len(f2.get_templates()), 0)
        self.assertEqual(len(f2p.get_templates()), 0)
        f0 = nmt.NmtField((self.msk), [self.mps[0]], templates=(np.array([[t[0]] for t in self.tmp])),
    ...
    def test_field_error(self):
        ...
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[0]], templates=[
             [
              self.tmp[0, 0, :15]]])
        with self.assertRaises(ValueError):
            nmt.NmtField(self.msk, self.mps)
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[1], self.mps[2]], templates=(self.tmp))
        with self.assertRaises(ValueError):
            nmt.NmtField((self.msk), [self.mps[1], self.mps[2]], templates=1)
        with self.assertRaises(ValueError):
...
class TestFieldFsk(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.beam = np.array([np.arange(self.nell) * self.lmax / (self.nell - 1.0),
         np.ones(self.nell)])
    ...
    def test_field_masked(self):
        ...
        f0_msk = nmt.NmtFieldFlat(lx, ly, msk, [mps_msk[0]], masked_on_input=True)
    ...
    def test_field_flat_alloc(self):
        ...
        f0 = nmt.NmtFieldFlat((self.lx), (self.ly), (self.msk), [
         self.mps[0]],
          beam=(self.beam), templates=(np.array([[t[0]] for t in self.tmp])))
        f2 = nmt.NmtFieldFlat((self.lx), (self.ly), (self.msk), [
         self.mps[1], self.mps[2]],
          beam=(self.beam), templates=(np.array([[t[1], t[2]] for t in self.tmp])))
        self.assertTrue(normdiff(f0.get_maps()[0], 0 * self.msk) < 1e-10)
        self.assertTrue(normdiff(f2.get_maps()[0], 0 * self.msk) < 1e-10)
        self.assertTrue(normdiff(f2.get_maps()[1], 0 * self.msk) < 1e-10)
        self.assertEqual(len(f0.get_templates()), 5)
    ...
    def test_field_flat_errors(self):
        ...
        with self.assertRaises(ValueError):
            nmt.NmtFieldFlat((self.lx), (self.ly), (self.msk), [self.mps[0]], templates=(np.array([[t[0, :self.ny // 2]] for t in self.tmp])))
        with self.assertRaises(ValueError):
        ...
        with self.assertRaises(ValueError):
            nmt.NmtFieldFlat((self.lx), (self.ly), (self.msk), [
             self.mps[0]],
              templates=(self.tmp))
        with self.assertRaises(ValueError):
            nmt.NmtFieldFlat((self.lx), (self.ly), (self.msk), [self.mps[0]], templates=1)
        with self.assertRaises(ValueError):
