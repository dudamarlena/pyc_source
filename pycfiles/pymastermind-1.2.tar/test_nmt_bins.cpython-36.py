...
# Embedded file name: /home/alonso/Science/Codes/ReformCodes/NaMaster/test/test_nmt_bins.py
# Compiled at: 2019-12-25 17:21:57
# Size of source mod 2**32: 6615 bytes
import unittest, numpy as np, pymaster as nmt
...
class TestBinsSph(unittest.TestCase):
    ...
    def setUp(self):
        ...
        fell = ells * (ells + 1.0) / (2 * np.pi)
        self.bv = nmt.NmtBin(nside=(self.nside), bpws=bpws,
          ells=ells,
          weights=weights,
          lmax=(self.lmax))
        self.bcf = nmt.NmtBin(nside=(self.nside), nlb=4,
          lmax=(self.lmax),
          is_Dell=True)
        self.bvf1 = nmt.NmtBin(nside=(self.nside), bpws=bpws,
          ells=ells,
          weights=weights,
          lmax=(self.lmax),
          is_Dell=True)
        self.bvf2 = nmt.NmtBin(nside=(self.nside), bpws=bpws,
          ells=ells,
          weights=weights,
          lmax=(self.lmax),
          f_ell=fell)
    ...
    def test_bins_errors(self):
        ...
        with self.assertRaises(RuntimeError):
            nmt.NmtBin(nside=(self.nside), bpws=bpws,
              ells=ells,
              weights=weights,
              lmax=(self.lmax))
    ...
    def test_bins_edges(self):
        self.assertEqual(self.bc.get_n_bands(), self.be.get_n_bands())
        self.assertTrue(np.sum(np.fabs(self.bc.get_effective_ells() - self.be.get_effective_ells())) < 1e-10)
    ...
    def test_bins_variable(self):
        ...
        for i in range(self.bv.get_n_bands()):
            ...
            self.assertTrue(np.fabs(np.sum(wl1) - 1.0) < 1e-05)

        nbarr = np.arange(self.bv.get_n_bands())
        self.assertTrue(normdiff(self.bv.get_effective_ells(), 2 + self.nlb * nbarr + 0.5 * (self.nlb - 1)) < 1e-05)
    ...
    def test_bins_binning(self):
        ...
        self.assertTrue(normdiff(cl_b_p, cl_b) < 1e-05)
        cl_u_p = (cl_b[:, None] * np.ones([len(cl_b), self.nlb])).flatten()
        self.assertTrue(normdiff(cl_u_p, cl_u[2:iend]) < 1e-05)

    def test_bins_binning_f_ell(self):
        cls = np.arange((self.lmax + 1), dtype=float)
        fell = cls * (cls + 1.0) / 2 / np.pi
        cl_b = self.bcf.bin_cell(cls)
        self.assertTrue(normdiff(cl_b, self.bvf1.bin_cell(cls)) < 1e-05)
        self.assertTrue(normdiff(cl_b, self.bvf2.bin_cell(cls)) < 1e-05)
        cl_u = self.bcf.unbin_cell(cl_b)
        self.assertTrue(normdiff(cl_u, self.bvf1.unbin_cell(cl_b)) < 1e-05)
        self.assertTrue(normdiff(cl_u, self.bvf2.unbin_cell(cl_b)) < 1e-05)
        iend = 2 + self.nlb * ((self.lmax - 2) // self.nlb)
...
class TestBinsFsk(unittest.TestCase):
    ...
    def test_bins_flat_alloc(self):
        ...
        self.assertTrue(normdiff((np.arange(self.nbands) + 0.5) * self.nlb + 2, self.b.get_effective_ells()) < 1e-05)
    ...
    def test_bins_flat_binning(self):
        ...
        self.assertTrue(normdiff(cl_b, self.b.get_effective_ells()) < 1e-05)
        i_bin = (cl.astype(int) - 2) // self.nlb
        igood = (i_bin >= 0) & (i_bin < self.nbands)
        self.assertTrue(normdiff(cl_u[igood], cl_b[i_bin[igood]]) < 1e-05)
