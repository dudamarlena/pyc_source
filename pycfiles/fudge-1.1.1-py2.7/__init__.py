...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fudge/__init__.py
# Compiled at: 2017-12-12 10:17:01
...
def start():
    ...
    """
    warnings.warn('fudge.start() has been deprecated.  Use fudge.clear_calls() instead', DeprecationWarning, 3)
    clear_calls()
...
def stop():
    ...
    """
    warnings.warn('fudge.stop() has been deprecated.  Use fudge.verify() instead', DeprecationWarning, 3)
    verify()
...
class Call(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.expected_times_called is not None and self.actual_times_called > self.expected_times_called:
            raise AssertionError('%s was called %s time(s). Expected %s.' % (
        ...
        if with_args:
            ...
            if self.expected_kwargs != kwargs:
                raise AssertionError('%s was called unexpectedly with args %s' % (
                 self,
            ...
            for expected_arg, expected_value in self.expected_matching_kwargs.items():
                if expected_arg in kwargs:
                    if expected_value != kwargs[expected_arg]:
                        raise AssertionError('%s was called unexpectedly with args %s' % (
                         self,
                         self._repr_call(args, {expected_arg: kwargs[expected_arg]}, shorten_long_vals=False)))
        ...
        if self.expected_matching_args:
            if self.expected_matching_args != args:
                raise AssertionError('%s was called unexpectedly with args %s' % (
                 self,
                 self._repr_call(args, kwargs, shorten_long_vals=False)))
        with_arg_counts = self.expected_arg_count is not None or self.expected_kwarg_count is not None
        if with_arg_counts:
            if self.expected_arg_count is None:
                self.expected_arg_count = 0
            if len(args) != self.expected_arg_count:
                raise AssertionError('%s was called with %s arg(s) but expected %s' % (
            ...
            if len(kwargs.keys()) != self.expected_kwarg_count:
                raise AssertionError('%s was called with %s keyword arg(s) but expected %s' % (
        ...
        if self.unexpected_kwargs:
            for un_key, un_val in self.unexpected_kwargs.items():
                if un_key in kwargs and kwargs[un_key] == un_val:
                    raise AssertionError('%s was called unexpectedly with kwarg %s=%s' % (
                     self, un_key, un_val))

        if self.unexpected_args:
            for un_arg in self.unexpected_args:
                if un_arg in args:
                    raise AssertionError('%s was called unexpectedly with arg %s' % (
                     self, un_arg))
...
class ExpectedCall(Call):
    ...
    def assert_called(self):
        assert self.was_called, '%s was not called' % self
...
class CallStack(object):
    ...
    def __call__(self, *args, **kw):
        try:
            ...
            raise AssertionError('This attribute of %s can only be called %s time(s).  Call reset() if necessary or fudge.clear_calls().' % (
...
class Fake(object):
    ...
    **expect_call=True**
    ...
    def __init__(self, name=None, allows_any_call=False, callable=False, expect_call=False):
        ...
        if allows_any_call:
            warnings.warn('Fake(allows_any_call=True) is deprecated; use Fake.is_a_stub()', DeprecationWarning, 3)
            self.is_a_stub()
    ...
    def __getattribute__(self, name):
        ...
        if name in g('_attributes'):
            return g('_attributes')[name]
        if name in g('_properties'):
        ...
        raise AttributeError("%s object does not allow call or attribute '%s' (maybe you want %s.is_a_stub() ?)" % (
    ...
    def __call__(self, *args, **kwargs):
        if '__init__' in self._declared_calls:
            ...
            raise RuntimeError('%s object cannot be called (maybe you want %s.is_callable() ?)' % (
    ...
    _assignment = re.compile('\\s*(?P<name>[a-zA-Z0-9_]+)\\s*=\\s*(fudge\\.)?Fake\\(.*')
    ...
    def _get_current_call(self):
        if not self._last_declared_call_name:
            if not self._callable:
                raise FakeDeclarationError("Call to a method that expects a predefined call but no such call exists.  Maybe you forgot expects('method') or provides('method') ?")
    ...
    def next_call(self, for_method=None):
        ...
        if for_method:
            if for_method not in self._declared_calls:
                raise FakeDeclarationError('next_call(for_method=%r) is not possible; declare expects(%r) or provides(%r) first' % (
        ...
        if last_call_name:
            ...
            raise FakeDeclarationError('next_call() must follow provides(), expects() or is_callable()')
        ...
        if not isinstance(exp, CallStack):
            stack = CallStack(self, initial_calls=[exp], expected=isinstance(exp, ExpectedCall), call_name=exp.call_name)
            if last_call_name:
    ...
    def remember_order(self):
        ...
        """
        if self._callable:
            raise FakeDeclarationError('remember_order() cannot be used for Fake(callable=True) or Fake(expect_call=True)')
