...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fudge/tests/test_fudge.py
# Compiled at: 2015-06-08 15:36:17
...
class TestFake(unittest.TestCase):
    ...
    def test_repr_shortens_long_values(self):
        ...
        try:
            ...
            eq_(str(exc), "fake:widget.set_bits('123456789101112131415161718192021222324252627...') was called unexpectedly with args ()")
        else:
...
class TestLongArgValues(unittest.TestCase):
    ...
    def test_arg_diffs_are_not_shortened(self):
        ...
        try:
            ...
            eq_(str(exc), "fake:widget.set_bits('123456789101112131415161718192021222324252627...') was called unexpectedly with args ('99999999999999999999999999999999999999999999999999999999')")
        else:
            raise RuntimeError('expected AssertionError')

    def test_kwarg_diffs_are_not_shortened(self):
        fake = Fake('widget').provides('set_bits').with_args(newbits='12345678910111213141516171819202122232425262728293031')
        try:
            fake.set_bits(newbits='99999999999999999999999999999999999999999999999999999999')
        except AssertionError as exc:
            eq_(str(exc), "fake:widget.set_bits(newbits='123456789101112131415161718192021222324252627...') was called unexpectedly with args (newbits='99999999999999999999999999999999999999999999999999999999')")
        else:
            raise RuntimeError('expected AssertionError')
...
class TestArguments(unittest.TestCase):
    ...
    def test_wrong_args(self):
        exp = self.fake.expects('theCall').with_args(1, "ho ho ho ho ho ho ho, it's Santa", {'ditto': False})
        exp.theCall()
    ...
    def test_too_many_args(self):
        ...
        db.execute('select foozilate()', bind={'one': 1})
    ...
    def test_zero_args_ok(self):
        ...
        mail.send(to='you', from_='me', msg='hi')
...
class TestNextCall(unittest.TestCase):
    ...
    def test_next_call_with_callables(self):
        login = fudge.Fake('login').is_callable().returns('yes').next_call().returns('maybe').next_call().returns('no')
        eq_(login(), 'yes')
        eq_(login(), 'maybe')
        eq_(login(), 'no')

    def test_returns(self):
        db = Fake('db').provides('get_id').returns(1).provides('set_id').next_call(for_method='get_id').returns(2)
        eq_(db.get_id(), 1)
        eq_(db.set_id(), None)
        eq_(db.get_id(), 2)
        return
...
class TestExpectsAndProvides(unittest.TestCase):
    ...
    def test_multiple_expects_for_sep_methods(self):
        self.fake = fudge.Fake().expects('marco').returns(1).expects('polo').returns('A').expects('marco').returns(2).expects('polo').returns('B')
    ...
    def test_multiple_provides_for_sep_methods(self):
        self.fake = fudge.Fake().provides('marco').returns(1).provides('polo').returns('A').provides('marco').returns(2).provides('polo').returns('B')
        eq_(self.fake.marco(), 1)
        eq_(self.fake.marco(), 2)
        eq_(self.fake.polo(), 'A')
        eq_(self.fake.polo(), 'B')
...
class TestOrderedCalls(unittest.TestCase):
    ...
    def test_multiple_returns_affect_order(self):
        db = Fake('db').remember_order().expects('get_id').returns(1).expects('set_id').next_call(for_method='get_id').returns(2)
        eq_(db.get_id(), 1)
        eq_(db.set_id(), None)
        eq_(db.get_id(), 2)
        fudge.verify()
