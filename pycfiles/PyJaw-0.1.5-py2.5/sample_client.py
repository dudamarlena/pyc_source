...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/pyjaw/sample_client.py
...
USER_TOPIC_PATTERN = re.compile('/user/(.+?)/(profile|followers|followees|conversations)/?$')
CONVERSATION_TOPIC_PATTERN = re.compile('/conversation/(\\w{11})/?$')
...
class SampleClient(object):
    ...
    def __init__(self, key, username, password, hostname='localhost:3000'):
        self.client = RejawClient(key, hostname)
        ...
        for event in events:
            if USER_TOPIC_PATTERN.match(event['topic']):
                ...
                if subtopic == 'profile':
                    ...
                    if event.get('joined'):
                        if event['joined']['cid'] not in self.conversations.keys():
                ...
                if conversation not in self.conversations.keys():
