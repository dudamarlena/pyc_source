...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/pyogp/apps/examples/smoke_test.py
...
$/LicenseInfo$
"""
import unittest, getpass, logging
from optparse import OptionParser
import time
from pyogp.lib.client.agent import Agent
from pyogp.lib.client.enums import DeRezDestination, AssetType, InventoryType, WearablesIndex
...
class TestServer(unittest.TestCase):
    ...
    def test_im(self):
        ...
        def im_received(message_info):
            if str(message_info.payload['FromAgentID']) == str(client.agent_id):
                s.signal()
    ...
    def test_create_object(self):
        ...
        def object_created(object_info):
            ...
            matches = client.inventory.search_inventory(client.inventory.folders, name='Objects')
            folder = matches.pop()
            transaction_id = UUID()
            transaction_id.random()
            prim.derez(client, DeRezDestination.TakeIntoAgentInventory, folder.FolderID, transaction_id, client.active_group_id)
            s.signal()
    ...
    def test_create_and_remove_item_in_inventory(self):
        ...
        """
        s = Semaphore()
        matches = client.inventory.search_inventory(client.inventory.folders, name='Notecards')
        folder = matches.pop()
        ...
        client.inventory.create_new_item(folder, item_name, item_desc, AssetType.Notecard, InventoryType.Notecard, WearablesIndex.WT_SHAPE, 0, inventory_item_created)
        s.wait(30)
        self.assertFalse(s.timed_out)
        matches = client.inventory.search_inventory(client.inventory.folders, name=item_name)
        self.assertTrue(len(matches) > 0)
        item = matches.pop()
        api.sleep(5)
        client.inventory.remove_inventory_item(item, folder, client.inventory.folders)
    ...
    def test_can_walk(self):
        ...
        """
        old_pos = Vector3(X=client.Position.X, Y=client.Position.Y, Z=client.Position.Z)
        client.walk()
        api.sleep(5)
        client.walk(False)
        api.sleep(5)
        self.assertFalse(client.Position.X == old_pos.X and client.Position.Y == old_pos.Y and client.Position.Z == old_pos.Z)
    ...
    def test_can_teleport(self):
        ...
        """
        old_pos = Vector3(X=client.Position.X, Y=client.Position.Y, Z=client.Position.Z)
        new_pos = Vector3(X=client.Position.X + 5, Y=client.Position.Y + 5, Z=client.Position.Z)
        client.teleport(region_handle=client.region.RegionHandle, position=new_pos)
        api.sleep(5)
        self.assertFalse(client.Position.X == old_pos.X and client.Position.Y == old_pos.Y and client.Position.Z == old_pos.Z)
        client.teleport(region_handle=client.region.RegionHandle, position=old_pos)
    ...
    def test_physics(self):
        ...
        client.fly(False)
        api.sleep(5)
        new_pos = Vector3(X=client.Position.X, Y=client.Position.Y, Z=client.Position.Z)
        self.assertFalse(new_pos.X == old_pos.X and new_pos.Y == old_pos.Y and new_pos.Z == old_pos.Z)
