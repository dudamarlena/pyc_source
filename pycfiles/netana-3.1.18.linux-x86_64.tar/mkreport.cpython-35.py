...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/netana/mkreport.py
# Compiled at: 2014-10-05 16:24:54
...
class MkReport:
    ...
    def DoACAnalysis(self, source='VG'):
        ...
        try:
            ...
            two_pij = 2 * math.pi * complex(0.0, 1.0)
            for freq in self.genfreq(fstart, fstop, finc):
                ...
                for n in range(self.Nodes):
                    ...
                    self.matvalues = self.EvalEqu(self.Mat)
                    npmat = np.array(self.matvalues, dtype=np.complex128)
                    determ = npmat.copy()
                    determ[:, n] = m
                ...
                try:
                    ...
                    if abs(tf) < 1e-06:
                        tf = complex(1e-30, 0.0)
                    mag = 20.0 * math.log10(abs(tf))
                    angle = math.degrees(cmath.phase(tf))
                    print('{:+20.4f}{:+20.4f}'.format(mag, angle), file=self.ofile)
    ...
    def DoDCAnalysis(self, source='VG'):
        ...
        if 'GOALS' in self.NetDict:
            ...
            for goalnb in range(self.NetDict['GOALS']):
                ...
                print('#{:s} = {:g} {:s}\t{}'.format(goalkey, goal_result, gUnit, self.NetDict[goalkey]), file=self.ofile)
