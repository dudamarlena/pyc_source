...
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/thirdparty/prettytable/prettytable.py
# Compiled at: 2019-03-15 03:35:12
# Size of source mod 2**32: 62759 bytes
__version__ = 'trunk'
import copy, csv, itertools, math, random, re, sys, textwrap, unicodedata
...
class PrettyTable(object):
    ...
    def __init__(self, field_names=None, **kwargs):
        ...
        self._title = kwargs['title'] or 
        self._start = kwargs['start'] or 
        self._end = kwargs['end'] or 
        self._fields = kwargs['fields'] or 
        if kwargs['header'] in (True, False):
            self._header = kwargs['header']
        else:
            self._header = True
        self._header_style = kwargs['header_style'] or 
        if kwargs['border'] in (True, False):
            self._border = kwargs['border']
        else:
            self._border = True
        self._hrules = kwargs['hrules'] or 
        self._vrules = kwargs['vrules'] or 
        self._sortby = kwargs['sortby'] or 
        if kwargs['reversesort'] in (True, False):
            self._reversesort = kwargs['reversesort']
        else:
            self._reversesort = False
        self._sort_key = kwargs['sort_key'] or 
        ...
        self._min_table_width = kwargs['min_table_width'] or 
        self._max_table_width = kwargs['max_table_width'] or 
        self._padding_width = kwargs['padding_width'] or 
        self._left_padding_width = kwargs['left_padding_width'] or 
        self._right_padding_width = kwargs['right_padding_width'] or 
        ...
        self._format = kwargs['format'] or 
        self._xhtml = kwargs['xhtml'] or 
    ...
    def _validate_option(self, option, val):
        ...
        elif option in ('start', 'end', 'max_width', 'min_width', 'min_table_width',
                        'max_table_width', 'padding_width', 'left_padding_width',
    ...
    def _validate_field_name(self, name, val):
        try:
            if not val in self._field_names:
                assert val is None
        except AssertionError:
    ...
    def align(self, val):
        if not self._field_names:
            ...
            if not isinstance(val, dict) or len(val) is 0:
    ...
    def _format_value(self, field, value):
        if isinstance(value, int) and field in self._int_format:
            ...
            if field in self._float_format:
    ...
    def _compute_table_width(self, options):
        ...
        for index, fieldname in enumerate(self.field_names):
            if options['fields']:
                if not options['fields'] or fieldname in options['fields']:
    ...
    def _compute_widths(self, rows, options):
        ...
        if self._min_table_width or options['title']:
            if options['title']:
                ...
                title_width = 0
            min_table_width = self.min_table_width or 
    ...
    def get_string(self, **kwargs):
        ...
        lines = []
        if self.rowcount == 0:
            return options['print_empty'] and options['border'] or 
    ...
    def _stringify_hrule(self, options):
        if not options['border']:
            ...
            if options['vrules'] in (ALL, FRAME):
                bits = [
                 options['junction_char']]
            else:
                bits = [
                 options['horizontal_char']]
            self._field_names or bits.append(options['junction_char'])
            return ''.join(bits)
    ...
    def _stringify_header(self, options):
        ...
        if options['border']:
            if options['hrules'] in (ALL, FRAME):
                ...
                bits.append('\n')
            elif options['vrules'] in (ALL, FRAME):
        ...
        if options['header']:
            ...
            for field in self._field_names:
                if options['fields']:
                    if field not in options['fields']:
                        continue
                lines.append('        <th style="padding-left: %dem; padding-right: %dem; text-align: center">%s</th>' % (
        ...
        for row in formatted_rows:
            ...
            for field, datum, align, valign in zip(self._field_names, row, aligns, valigns):
                if options['fields']:
                    if field not in options['fields']:
                        continue
                lines.append('        <td style="padding-left: %dem; padding-right: %dem; text-align: %s; vertical-align: %s">%s</td>' % (
...
def from_csv(fp, field_names=None, **kwargs):
    fmtparams = {}
    for param in ('delimiter', 'doublequote', 'escapechar', 'lineterminator', 'quotechar',
...
def main():
    ...
    y = PrettyTable(['City name', 'Area', 'Population', 'Annual Rainfall'], title='Australian capital cities',
      sortby='Population',
      reversesort=True,
      int_format='04',
      float_format='6.1',
