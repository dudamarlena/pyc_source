...
# Embedded file name: /Users/boyhack/programs/pocsuite3/pocsuite3/lib/core/poc.py
# Compiled at: 2019-11-11 22:57:25
# Size of source mod 2**32: 12061 bytes
import re
...
import pocsuite3.lib.core.register as register
...
class POCBase(object):
    ...
    def __init__(self):
        ...
        if self.current_protocol == POC_CATEGORY.PROTOCOL.HTTP:
            self.global_options['target'] = OptString('', 'Target HTTP, IPv4, IPv6 address or file with ip:port (file://)',
              require=True)
    ...
    def get_infos(self):
        ...
        """
        fields = [
         'name', 'VulID', 'version', 'author', 'vulDate', 'createDate', 'updateDate', 'references',
        ...
        for option in args:
            for k, v in option.items():
                if v.require and v.value == '':
                    raise PocsuiteValidationException("'{key}' must be set,please using command 'set {key}'".format(key=k))
