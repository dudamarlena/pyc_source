...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flask_tlsauth/__init__.py
# Compiled at: 2013-04-03 11:53:53
...
def renderUserForm(ca):
    ...
    def wrapped():
        ...
        if form.validate_on_submit():
            return Response(ca.gencert(str(form.name.data), str(form.email.data), str(form.org.data)), mimetype='application/x-pkcs12')
        ...
        if form.validate_on_submit():
            if blindsign:
                if not scrutinizer or scrutinizer(str(form.csr.data)):
                    return Response(ca.signcsr(str(form.csr.data)), mimetype='text/plain')
            else:
                ca.submit(str(form.csr.data))
                return Response('Thank you. If all goes well you should soon receive your signed certificate.')
...
def showcsrs(ca, groups=None):
    ...
    def wrapped():
        try:
            ...
            if err:
                return err
            return render_template('csrs.html', certs=[ (todn(cert.get_subject()), datetime.datetime.fromtimestamp(os.stat(path).st_mtime), os.path.basename(path)) for cert, path in ca.incoming()
                                                      ])
        except:
            import traceback
...
def tlsauth_init(app, ca, groups=None):
    ...
    """
    app.add_url_rule('/tlsauth/register/', 'register', renderUserForm(ca), methods=('GET',
                                                                                    'POST'))
    app.add_url_rule('/tlsauth/certify/', 'certify', renderCSRForm(ca, blindsign=False), methods=('GET',
                                                                                                  'POST'))
