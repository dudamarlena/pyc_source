...
# Embedded file name: build/bdist.linux-x86_64/egg/aioetcd3/maintenance.py
...
class Maintenance(StubMixin):
    ...
    @call_grpc(lambda : rpc.StatusRequest(), lambda r: Status(r), lambda s: s._maintenance_stub.Status)
    async def status(self):
        pass

    @call_grpc(lambda action, type, mid: rpc.AlarmRequest(action=action, memberID=mid, alarm=type), lambda r: [m for m in r.alarms], lambda s: s._maintenance_stub.Alarm)
    async def alarm(self, action, type, mid=0):
        pass

    @call_grpc(lambda : rpc.SnapshotRequest(), lambda r: (r.remaining_bytes, r.blob), lambda s: s._maintenance_stub.Snapshot)
    async def snapshot(self):
        pass

    @call_grpc(lambda : rpc.HashRequest(), lambda r: r.hash, lambda s: s._maintenance_stub.Hash)
    async def hash(self):
        pass

    @call_grpc(lambda : rpc.DefragmentRequest(), lambda r: None, lambda s: s._maintenance_stub.Defragment)
