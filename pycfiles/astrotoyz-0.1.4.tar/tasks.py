...
# [GCC 8.4.0]
# Embedded file name: astrotoyz/tasks.py
# Compiled at: 2015-04-02 22:29:37
"""
...
def create_catalog(toyz_settings, tid, params):
    ...
    dataframe = {'columns': index + catalog.columns.values.tolist(), 
       'index': index, 
       'data': catalog.astype(object).fillna('NaN').values.tolist()}
    cat_info = {'cid': catalog.cid, 
       'name': catalog.name, 
       'settings': catalog.settings, 
       'dataframe': dataframe}
    response = {'id': 'create_catalog', 
       'cat_info': cat_info}
    return response
...
def select_src(toyz_settings, tid, params):
    ...
    response = {'id': 'select_src', 'status': status, 
       'src': src}
    return response
...
def run_sextractor(toyz_settings, tid, params):
    ...
    """
    import astrotoyz.astromatic.sex as sex, toyz.utils.db, shutil
    ...
    if 'temp' not in shortcuts:
        raise ToyJobError("You removed your 'temp' shortcut, DON'T DO THAT!" + 'Refresh your browser and it will be restored')
