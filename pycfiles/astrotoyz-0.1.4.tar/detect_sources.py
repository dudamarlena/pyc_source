...
# [GCC 8.4.0]
# Embedded file name: astrotoyz/detect_sources.py
# Compiled at: 2015-03-08 10:40:06
from __future__ import division, print_function
import sys, numpy as np, numpy.lib.recfunctions as rfn, scipy.ndimage.filters as filters, scipy.ndimage as ndimage
from scipy.optimize import curve_fit
import multiprocessing
from toyz.web import session_vars
import toyz.utils.core as core, astrotoyz.core
fit_dtypes = {'circular_moffat': [
                     (
                      'amplitude', float),
                     (
                      'x', float),
...
def detect_sources(img_data, threshold, aperture_type='radius', size=5, footprint=None, bin_struct=None, sigma=2, saturate=None, margin=None):
    """
...
def fit_circular_moffat(data, init_params={}):
    ...
    fwhm = np.sqrt(np.sum((data > floor + amplitude / 2.0).flatten()))
    beta = 3.5
    alpha = 0.5 * fwhm / np.sqrt(2.0 ** (1.0 / beta) - 1.0)
    ...
    fit_result[3] = np.sqrt(2.0 ** (1.0 / beta) - 1.0) * fit_result[3] * 2
...
def elliptical_moffat((x, y), amplitude, x_mean, y_mean, alpha1, alpha2, beta, angle, floor):
    ...
    moff.ravel(): 1D numpy array
        ...
        so that it can be used with the curve_fit function
    """
    A = (np.cos(angle) / alpha1) ** 2.0 + (np.sin(angle) / alpha2) ** 2.0
    B = (np.sin(angle) / alpha1) ** 2.0 + (np.cos(angle) / alpha2) ** 2.0
    C = 2.0 * np.sin(angle) * np.cos(angle) * (1.0 / alpha1 ** 2.0 - 1.0 / alpha2 ** 2.0)
    moff = floor + amplitude / (1.0 + A * (x - x_mean) ** 2 + B * (y - y_mean) ** 2 + C * (x - x_mean) * (y - y_mean)) ** beta
    return moff.ravel()
...
def fit_elliptical_moffat(data):
    ...
    fwhm = np.sqrt(np.sum((data > floor + amplitude / 2.0).flatten()))
    beta = 3.5
    alpha1 = 0.5 * fwhm / np.sqrt(2.0 ** (1.0 / beta) - 1.0)
    alpha2 = 0.5 * fwhm / np.sqrt(2.0 ** (1.0 / beta) - 1.0)
    angle = 0
    ...
    fit_result[3] = np.sqrt(2.0 ** (1.0 / beta) - 1.0) * fit_result[3] * 2
    fit_result[4] = np.sqrt(2.0 ** (1.0 / beta) - 1.0) * fit_result[4] * 2
    return (fit_result, pcov)
...
def elliptical_gaussian((x, y), amplitude, x_mean, y_mean, std_x, std_y, theta, floor):
    a = 0.5 * (np.cos(theta) / std_x) ** 2 + 0.5 * (np.sin(theta) / std_y) ** 2
    b = -np.sin(2 * theta) / (4 * std_x ** 2) + np.sin(2 * theta) / (4 * std_x ** 2)
    c = 0.5 * (np.sin(theta) / std_x) ** 2 + 0.5 * (np.cos(theta) / std_y) ** 2
...
fit_types = {'circular_moffat': fit_circular_moffat, 
...
def find_stars(img_data, aperture_type='radius', maxima_size=5, maxima_sigma=2, maxima_footprint=None, aperture_radii=[], threshold=None, saturate=None, margin=None, bin_struct=None, fit_method='elliptical moffat', wcs=None):
    """
    ...
    if fit_method not in fit_types.keys():
        raise astrotoyz.core.AstroToyzError("Invalid fit method, please choose from '" + ("','").join(fit_types))
    ...
    processes = [ FitWorker(tasks, results, img_data, radius, fit_method) for i in xrange(num_processes)
                ]
    for p in processes:
        p.start()
