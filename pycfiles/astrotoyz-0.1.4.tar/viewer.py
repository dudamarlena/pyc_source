...
# [GCC 8.4.0]
# Embedded file name: /media/data-beta/users/fmooleka/git_projects/astro-toyz/astrotoyz/viewer.py
# Compiled at: 2015-06-10 19:28:07
"""
...
import astropy.wcs as pywcs, astropy.io.fits as pyfits, numpy as np, astrotoyz as astro, astrotoyz.core
...
def get_wcs(file_info, hdulist):
    ...
    if not hasattr(session_vars, 'wcs_info'):
        ...
        set_wcs_info = True
    elif file_info['filepath'] != session_vars.wcs_info['filepath'] or int(file_info['frame']) != session_vars.wcs_info['frame']:
        set_wcs_info = True
...
def get_img_data(data_type, file_info, img_info, **kwargs):
    ...
    if 'fit_type' in kwargs:
        fit_response = get_2d_fit(file_info, kwargs['fit_type'], kwargs['x'], kwargs['y'], kwargs['width'], kwargs['height'])
...
def get_img_info(file_info, img_info, **kwargs):
    if file_info['ext'] == 'fits':
        ...
        try:
            coord_ranges = [ map(int, coord_range.split(':')) for coord_range in hdu.header['DETSEC'][1:-1].split(',') ]
        except KeyError:
            coord_ranges = [
             [
              1, hdu.data.shape[1]], [1, hdu.data.shape[0]]]

        img_info['coord_range'] = {'x': coord_ranges[0], 'y': coord_ranges[1]}
    return img_info
...
def get_2d_fit(file_info, fit_type, x, y, width, height, **kwargs):
    ...
    if len(fit) > 0:
        ...
        response = {'id': 'get_2d_fit', 'status': 'success', 
           'fit': params}
    else:
        response = {'id': 'get_2d_fit', 'status': 'fit failed', 
           'fit': {}}
    return response