...
# [GCC 8.4.0]
# Embedded file name: astrotoyz/catalog.py
# Compiled at: 2015-04-24 01:09:04
"""
...
import os, pandas, numpy as np, json
from datetime import datetime
from toyz.web import session_vars
import toyz.utils.core, astrotoyz.core
...
def get_catalog(cid, create='yes', **kwargs):
    ...
    try:
        ...
        raise astrotoyz.core.AstroToyzError('You must supply a dict of catalog settings with a filepath to ' + 'load or create a catalog')
...
def load_catalog(file_info, load_log=False):
    ...
    if load_log:
        log = pandas.read_sql_query(("SELECT * FROM log WHERE cid='{0}'").format(cid), engine)
    else:
        log = None
...
class Catalog(pandas.DataFrame):
    ...
    def __init__(self, cid, file_info, settings={}, name=None, data=None, log=None, **df_settings):
        ...
        if name is not None:
            ...
            self.name = self.cid
        default_settings = {'creation': {'creation_time': str(datetime.now()), 
                        'fits_file': file_info, 
                        'software_version': {'toyz': 'alpha', 
                                             'astrotoyz': 'alpha'}}, 
           'file_info': {'filepath': '', 
                         'file_type': '', 
                         'file_settings': {}}, 
           'data': {'fit_method': 'elliptical_moffat', 
                    'ra_name': 'ra', 
                    'dec_name': 'dec', 
                    'id_name': 'id', 
                    'min_sep': {'wcs': [
                                      1, 'arcsec'], 
                                'px': 5}, 
                    'build_src_info': {'module': 'astrotoyz.catalog', 
                                       'func': 'build_src_info'}}}
        self.settings = toyz.utils.core.merge_dict(default_settings, settings, True)
        if self.index.name is None and self.index.names[0] is None and self.settings['data']['id_name'] in self.columns:
    ...
    def save(self, filepath=None):
        ...
        if meta_record.first() is None:
            meta = CatalogMeta(cid=self.cid, name=self.name, settings=json.dumps(self.settings))
            session.add(meta)
        else:
            meta = meta_record
            meta.update({'cid': self.cid, 
               'name': self.name, 
    ...
    def log(self, action, log):
        ...
        """
        self.new_log.loc[self.new_log.shape[0]] = pandas.Series({'cid': self.cid, 
           'action': action, 
           'log': log})
    ...
    def get_log(self, include_new=True):
        ...
        """
        engine = create_engine(self.settings['file_info']['filepath'])
        Base.metadata.bind = engine
        DBSession = sessionmaker(bind=engine)
        session = DBSession()
    ...
    def get_min_sep(self):
        ...
        if 'min_sep' in self.settings['data']:
            ...
            elif 'px' in self.settings['data']['min_sep']:
    ...
    def get_src_info(self, **kwargs):
        if 'ra' in kwargs and 'dec' in kwargs or self.settings['data']['ra_name'] in kwargs and self.settings['data']['dec_name'] in kwargs:
            src_info = kwargs
        else:
            import astrotoyz.viewer, copy
    ...
    def add_src(self, src_info, file_info, **kwargs):
        ...
        if self.shape[0] == 0 or d2d > sep:
            import importlib
            build_module = importlib.import_module(self.settings['data']['build_src_info']['module'])
    ...
    def get_nearest_neighbors(self, coord1=None, coord2=None, coord_type=None, coord_unit=None, coords=None):
        """
        ...
        if coords is None:
            if coord1 is None or coord2 is None or coord_type is None:
                raise astrotoyz.core.AstroToyzError("Must supply either 'coords' or 'coord1' and 'coord2' and 'coord_type")
        elif self.settings['data']['ra_name'] in coords and self.settings['data']['dec_name'] in coords:
            coord1 = coords[self.settings['data']['ra_name']]
            coord2 = coords[self.settings['data']['dec_name']]
            coord_type = 'wcs'
        elif 'ra' in src_info and 'dec' in src_info and (coord_type is None or coord_type == 'wcs'):
            coord1 = src_info['ra']
            coord2 = src_info['dec']
            coord_type = 'wcs'
        elif 'x' in src_info and 'y' in src_info and (coord_type is None or coord_type == 'px'):
            coord1 = src_info['x']
            coord2 = src_info['y']
            coord_type = 'px'
        else:
            raise astrotoyz.core.AstroToyzError("Adding a source requires a pair of coordinates from 'ra' and 'dec', " + "'x' and 'y', or the catlogs 'ra' and 'dec' fields")
        if coord_type == 'wcs':
        ...
        if idx.shape == ():
            idx = idx.max()
        return (
         idx, d2d, self.loc[self.index[idx]])
