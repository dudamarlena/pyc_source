...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddalg/test_itree.py
...
class TestItree(unittest.TestCase):
    ...
    def test_jaccard_query(self):
        overlaps = self.tree.jaccard_query(5, 40, min_jaccard=0.8)
        self.assertEqual(1, len(overlaps))
        self.assertListEqual([IntervalImpl.of(0, 40)], overlaps)

    @fail_if_not_removed
    def test_fuzzy_query(self):
        overlaps = self.tree.fuzzy_query(2, 38, coverage=0.85)
        self.assertEqual(1, len(overlaps))
        self.assertListEqual([IntervalImpl.of(0, 40)], overlaps)
