...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/panya/admin.py
# Compiled at: 2011-05-26 03:46:53
...
class ModelBaseAdmin(admin.ModelAdmin):
    actions = [
     make_published, make_staging, make_unpublished]
    ...
    fieldsets = (
     ...
     (
      'Publishing',
      {'fields': ('state', 'publish_on', 'retract_on', 'sites', 'publishers'), 'classes': ('collapse', )}),
     (
      'Meta',
      {'fields': ('categories', 'tags', 'created', 'owner'), 'classes': ('collapse', )}),
     (
      'Image',
      {'fields': ('image', 'crop_from', 'effect'), 'classes': ('collapse', )}),
     (
      'Commenting',
      {'fields': ('comments_enabled', 'anonymous_comments', 'comments_closed'), 'classes': ('collapse', )}),
     (
      'Liking',
      {'fields': ('likes_enabled', 'anonymous_likes', 'likes_closed'), 'classes': ('collapse', )}))
