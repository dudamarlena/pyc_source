...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xpython/byteop/byteop25.py
# Compiled at: 2020-05-07 19:39:58
...
class ByteOp25(object):
    ...
    def call_function_with_args_resolved(self, func, posargs, namedargs):
        if hasattr(func, 'im_func'):
            ...
            if not isinstance(posargs[0], func.im_class):
                raise TypeError('unbound method %s() must be called with %s instance as first argument (got %s instance instead)' % (
        ...
        if isinstance(func, types.BuiltinFunctionType):
            ...
            if func == locals:
                self.vm.push(self.vm.frame.f_globals)
                return
            if self.version != PYTHON_VERSION and PYTHON_VERSION >= 3.4 and func == __build_class__:
    ...
    def IMPORT_FROM(self, name):
        ...
        if not hasattr(mod, name):
            value = ImportError("cannot import name '%s' from '%s' (%s)" % (
    ...
    def MAKE_FUNCTION(self, argc):
        ...
        fn = Function(name=None, code=code, globs=globs, argdefs=defaults, closure=None, vm=self.vm)
