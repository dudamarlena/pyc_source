...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/Products/eCards/tests/test_installation.py
...
class TestProductInstallation(base.eCardTestCase):
    ...
    def testCoreProductsInstalled(self):
        ...
        for prod in CORE_PRODUCTS:
            self.failUnless(self.portal.portal_quickinstaller.isProductInstalled(prod), 'Core product %s is not already installed' % prod)

    def testPortalFactorySetup(self):
        for t in self.eCardTypes:
            self.failUnless(t in self.factory.getFactoryTypes(), 'Type %s is not a factory types' % t)
    ...
    def testTypesNotListed(self):
        metaTypesNotToList = self.properties.navtree_properties.getProperty('metaTypesNotToList')
        for t in self.eCardTypes:
            self.failUnless(t in metaTypesNotToList, "Type %s will show up in the nav and shouldn't" % t)
    ...
    def testCorePloneLayersStillPresent(self):
        ...
        for (selection, layers) in self.skins.getSkinPaths():
            for specific_layer in core_layers:
                self.failUnless(specific_layer in layers, "The %s layer                     does not appear in the layers of Plone's available skins" % specific_layer)
    ...
    def testECardTypesFTI(self):
        ...
        """
        eCardFTIMapping = [{'eCard': {'title': 'eCard', 'content_meta_type': 'eCard', 'product': 'eCards', 'global_allow': False, 'filter_content_types': False, 'content_icon': 'ecard_icon.gif', 'factory': 'addeCard'}}, {'eCardCollection': {'title': 'eCard Collection', 'content_meta_type': 'ECardCollection', 'product': 'eCards', 'global_allow': True, 'filter_content_types': True, 'allowed_content_types': ('eCard', ), 'content_icon': 'ecardcollection_icon.gif', 'factory': 'addeCardCollection'}}]
        for fti_type in eCardFTIMapping:
            for (k, v) in fti_type.items():
                ...
                for (type_attr, type_attr_value) in v.items():
                    self.assertEquals(eval('type_obj.%s' % type_attr), type_attr_value, "For type %s, the FTI value should be %s for the %s field                         types tool, but it's %s" % (k, type_attr_value, type_attr, eval('type_obj.%s' % type_attr)))
    ...
    def testBrowseToObjectViaViewAlias(self):
        ...
        self.failUnless('/view' in ecard_fti.getActionObject('object/view').getActionExpression())
    ...
    def testFSDVInSkinsTool(self):
        ...
        for prodSkin in self.eCardSkins:
            self.failUnless(prodSkin in self.skins.objectIds(), 'The skin %s was not registered with the skins tool' % prodSkin)
