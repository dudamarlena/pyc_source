...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/raven_cron/runner.py
# Compiled at: 2013-12-18 16:36:00
...
parser = ArgumentParser(description='Wraps commands and reports failing ones to sentry.', epilog='SENTRY_DSN can also be passed as an environment variable.')
parser.add_argument('--dsn', metavar='SENTRY_DSN', default=getenv('SENTRY_DSN'), help='Sentry server address')
...
def update_dsn(opts):
    ...
    conf_precedence = [
     home_conf_file, system_conf_file]
...
class CommandReporter(object):
    ...
    def report_fail(self, exit_status, buf, elapsed):
        if self.dsn is None:
            ...
            self.client.captureMessage(message, data={'logger': 'cron'}, extra={'command': self.command, 
               'exit_status': exit_status, 
               'last_lines': last_lines}, time_spent=elapsed)
            return