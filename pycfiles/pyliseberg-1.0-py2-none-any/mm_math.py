...
# Embedded file name: d:\shared\tim_working\development\github\pylirious\pylirious\mm-api\python\mm\mm_math.py
...
def lerpv3(a, b, t):
    ...
    return (
     (1 - t) * a[0] + t * b[0], (1 - t) * a[1] + t * b[1], (1 - t) * a[2] + t * b[2])
...
def make_axis_angle_matrix(axis, angle):
    ...
    fZSin = axis[2] * fSin
    return (
     fX2 * (1 - fCos) + fCos, fXYM - fZSin, fXZM + fYSin, fXYM + fZSin, fY2 * (1 - fCos) + fCos, fYZM - fXSin, fXZM - fYSin, fYZM + fXSin, fZ2 * (1 - fCos) + fCos)
...
def make_align_axis_matrix(initialAxis, alignWithAxis):
    ...
    axisDot = dotv3(alignWithAxis, initialAxis)
    axisCross = crossv3(initialAxis, alignWithAxis)
