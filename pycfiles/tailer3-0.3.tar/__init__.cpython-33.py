...
# [GCC 8.4.0]
# Embedded file name: /home/dnarvaez/pytailer/src/tailer/__init__.py
# Compiled at: 2013-03-02 10:42:53
# Size of source mod 2**32: 8507 bytes
import re, time

class Tailer(object):
    __doc__ = '    Implements tailing and heading functionality like GNU tail and head\n    commands.\n    '
    line_terminators = ('\r\n', '\n', '\r')
    ...
    def seek_line_forward(self):
        ...
        start = 0
        if bytes_read:
            if read_str[0] in self.line_terminators:
                start += 1
        while bytes_read > 0:
            i = start
            while i < bytes_read:
    ...
    def seek_line(self):
        ...
        if bytes_read:
            if read_str[(-1)] in self.line_terminators:
                bytes_read -= 1
    ...
    def follow(self, delay=1.0):
        ...
        while True:
            ...
            if line:
                if trailing:
                    if line in self.line_terminators:
                        trailing = False
                        continue
...
def tail(file, lines=10):
    r"""    Return the last lines of the file.
...
def head(file, lines=10):
    r"""    Return the top lines of the file.
    ...
    try:
        try:
            if options.lines > 0:
                if options.head:
                    if options.follow:
                        print('Cannot follow from top of file.')
                        sys.exit(1)
...
def main():
    ...
    parser.add_option('-n', '--lines', dest='lines', default=10, type='int', help='output the last N lines, instead of the last 10')
    parser.add_option('-t', '--top', dest='head', default=False, action='store_true', help='output lines from the top instead of the bottom. Does not work with follow')
    parser.add_option('-s', '--sleep-interval', dest='sleep', default=1.0, metavar='S', type='float', help='with  -f,  sleep  for  approximately  S  seconds between iterations')
    parser.add_option('', '--test', dest='test', default=False, action='store_true', help='Run some basic tests')
