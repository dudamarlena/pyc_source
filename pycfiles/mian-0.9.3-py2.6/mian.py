...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mian/mian.py
...
import struct, sys, warnings, zlib
...
def print_block_types():
    ...
    for (block_hex, block_names) in sorted(BLOCK_TYPES.iteritems(), key=itemgetter(0)):
    ...
    if o.plot_mode == 'normal':
        ...
        if o.log:
            for (index, block_counts) in enumerate(counts):
                plt.semilogy(block_counts, label=BLOCK_TYPES[block_type_hexes[index]][0], linewidth=1, nonposy='clip', picker=3)

        else:
            for (index, block_counts) in enumerate(counts):
                plt.plot(block_counts, label=BLOCK_TYPES[block_type_hexes[index]][0], linewidth=1, picker=3)
        ...
        if o.plot_mode == 'colormap':
            ...
            im = plt.imshow(Data, cmap=cm.jet, extent=(
             max_chunk_z, min_chunk_z, max_chunk_x, min_chunk_x))
...
def mian(world_dir, block_type_hexes, options):
    ...
    if o.plot_mode == 'colormap' or o.plot_mode == 'wirframe':
        title += (' - map for block {0}').format(BLOCK_TYPES[block_type_hexes[0]][0])
...
def generate_graph_data(world_dir, mcr_files, block_type_hexes, plot_mode):
    if plot_mode == 'normal':
        ...
        print 'Done!'
        return total_counts
    if plot_mode == 'colormap' or plot_mode == 'wireframe':
        min_x = 0
        min_z = 0
        max_x = 0
        max_z = 0
        ...
        for index_x in xrange(abs(min_chunk_x) + abs(max_chunk_x)):
            for index_z in xrange(abs(min_chunk_z) + abs(max_chunk_z)):
                counts = count_chunk_blocks(world_dir, (
                 X[index_z][index_x], Z[index_z][index_x]), block_type_hexes[0])
                if counts < 0:
        ...
        print '100%... Done!'
        return (
         X, Z, min_block_x, min_block_z, max_block_x, max_block_z, Data)
...
def extract_region_chunk_blocks(mcr_file, coordsXZ):
    ...
    location = struct.unpack(LOCATION_FORMAT, LOCATION_PADDING + location_raw)[0]
    if location == 0:
        return
    else:
        file_pointer.seek(location * SECTOR_BYTES)
        chunk_length = struct.unpack(UNSIGNED_LONG_FORMAT, file_pointer.read(CHUNK_LENGTH_BYTES))[0]
        chunk_compression = struct.unpack(UNSIGNED_CHAR_FORMAT, file_pointer.read(COMPRESSION_BYTES))[0]
        chunk_raw = file_pointer.read(chunk_length)
        chunk = decompress(chunk_raw, chunk_compression)
        index = chunk.find(BLOCKS_NBT_TAG)
        blocks = chunk[index + len(BLOCKS_NBT_TAG):index + len(BLOCKS_NBT_TAG) + 32768]
        return blocks
...
def count_chunk_blocks(world_dir, chunkXZ, block_type):
    ...
    """
    rXZ = (
     chunkXZ[0] / 32, chunkXZ[1] / 32)
    mcr_file = world_dir.rstrip('/') + '/region/r.' + str(rXZ[0]) + '.' + str(rXZ[1]) + '.mcr'
    local_chunkXZ = (
     divmod(chunkXZ[0], 32)[1], +divmod(chunkXZ[1], 32)[1])
...
def extract_region_blocks(mcr_file):
    ...
    for offset in locations:
        ...
        chunk = decompress(chunk_raw, chunk_compression)
        index = chunk.find(BLOCKS_NBT_TAG)
        blocks = chunk[index + len(BLOCKS_NBT_TAG) + 4:index + len(BLOCKS_NBT_TAG) + 4 + 32768]
...
def main(argv=None):
    ...
    parser.add_option('-b', '--blocks', dest='block_type_names', default=None, help='Specify block types to include as a comma-separated list, using either the block types or hex values from the list.')
    parser.add_option('-l', '--list', action='store_true', dest='print_blocks', help='List available block types and their names (from <http://www.minecraftwiki.net/wiki/Data_values>)')
    parser.add_option('-n', '--nether', action='store_true', default=False, dest='nether', help='Graph The Nether instead of the ordinary world.')
    parser.add_option('--log', action='store_true', default=False, dest='log', help='Render logarithmic output.')
    parser.add_option('-o', '--output', default=None, dest='save_path', help='Save the result to file instead of showing an interactive GUI.')
    parser.add_option('--dpi', type='int', default=100, dest='dpi', help='The resolution in dots per inch for the --output option. Default = 100 (800x600).')
    parser.add_option('--plot-mode', '-p', type='string', default='normal', dest='plot_mode', help='The plot modes are: normal, colormap and wireframe (3D). Warning! Wireframe can be really resource hungry with big maps')
    ...
    if options.print_blocks:
        ...
        for block_type_name in block_type_names:
            ...
            for found_hex in found_hexes:
                if found_hex not in block_type_hexes:
