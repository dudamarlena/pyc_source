...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\underverse\__init__.py
# Compiled at: 2012-03-06 15:00:22
r"""
...
__all__ = [
 'NecRow', 'Underverse', 'Verse', 'SubVerse', 'and_', 'or_', 'adapter', 'converter']
and_ = ' and '
or_ = ' or '
        ...
        try:
            ...
            if '__data__' in self:
                ...
                if 'value' in self:
                    ...
                    if type(self['value']) == dict and attr in self['value']:
    ...
    """
        ...
        for k, v in mapped_results.items():
            ...
            if hasattr(k, '__iter__') and type(k) != str:
                ...
                tmp.append(k)
            elif len(k) > 0 and type(k) != str and type(k) != unicode:
                for l in k:
                    tmp.append(l)
  ...
  Please look at the *groupby* functionality for a Verse for a more comprehensive usage explanation.
    ...
    """
        ...
        if len(attrs) == 0:
            ...
            attr = attrs[0]
            for a in self:
                if attr in a and a[attr] not in names:
                    names[a[attr]] = 0
                    yield a[attr]
                elif hasattr(a, attr):
                    val = getattr(a, attr)
                ...
                def generator(new_converts):
                    for n in new_converts:
                        if type(n) is not NecRow:
                            if not hasattr(n, '__dict__') and type(n) is not dict:
                                ...
                                necro = NecRow(dict(n))
                            yield (
                             str(necro.uuid), adapter(necro))
                        else:
                            yield (
                             str(n.uuid), adapter(n))
