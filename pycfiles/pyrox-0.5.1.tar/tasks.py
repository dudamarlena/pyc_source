...
# Embedded file name: /private/tmp/test/lib/python2.7/site-packages/pyrowire/tasks/tasks.py
    ...
    while True:
        try:
            ...
            if job_data:
                ...
                redis.hdel('%s.%s' % (topic, 'pending'), job_sid)
                redis.hset('%s.%s' % (topic, 'complete'), job_sid, json.dumps(final_job_data))
        except (ConnectionError, TimeoutError, IndexError, TypeError, KeyError) as e:
