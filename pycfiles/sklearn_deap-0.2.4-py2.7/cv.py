...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/evolutionary_search/cv.py
# Compiled at: 2019-07-15 05:08:09
from __future__ import division
import os, warnings, numpy as np, random
...
def _evalFunction(individual, name_values, X, y, scorer, cv, iid, fit_params, verbose=0, error_score='raise', score_cache={}):
    """ Developer Note:
        --------------------
    ...
    if paramkey in score_cache:
        ...
        for train, test in cv.split(X, y):
            ...
            _score = _fit_and_score(estimator=individual.est, X=X, y=y, scorer=scorer, train=train, test=test, verbose=verbose, parameters=parameters, fit_params=fit_params, error_score=error_score)[0]
            if iid:
...
class EvolutionaryAlgorithmSearchCV(BaseSearchCV):
    ...
    def __init__(self, estimator, params, scoring=None, cv=4, refit=True, verbose=False, population_size=50, gene_mutation_prob=0.1, gene_crossover_prob=0.5, tournament_size=3, generations_number=10, gene_type=None, n_jobs=1, iid=True, error_score='raise', fit_params={}):
        super(EvolutionaryAlgorithmSearchCV, self).__init__(estimator=estimator, scoring=scoring, iid=iid, refit=refit, cv=cv, verbose=verbose, error_score=error_score)
        self.params = params
    ...
    def cv_results_(self):
        if self._cv_results is None:
            ...
            for p, gen in enumerate(self.all_history_):
                idxs, individuals, each_scores = zip(*[ (idx, indiv, np.mean(indiv.fitness.values)) for idx, indiv in list(gen.genealogy_history.items()) if indiv.fitness.valid and not np.all(np.isnan(indiv.fitness.values))
                                                      ])
    ...
    def _fit(self, X, y, parameter_dict):
        ...
        toolbox.register('evaluate', _evalFunction, name_values=name_values, X=X, y=y, scorer=self.scorer_, cv=cv, iid=self.iid, verbose=self.verbose, error_score=self.error_score, fit_params=self.fit_params, score_cache=self.score_cache)
        ...
        pop, logbook = algorithms.eaSimple(pop, toolbox, cxpb=0.5, mutpb=0.2, ngen=self.generations_number, stats=stats, halloffame=hof, verbose=self.verbose)
