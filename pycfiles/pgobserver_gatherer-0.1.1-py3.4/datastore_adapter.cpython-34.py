...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pgobserver_gatherer/datastore_adapter.py
...
def check_all_hosts_for_connectivity(hosts_data: list):
    ...
    for hd in hosts_data:
        logging.debug('checking host_id %s (%s:%s/%s) for DB connectivity...', hd['host_id'], hd['host_name'], hd['host_port'], hd['host_db'])
        try:
            datadb.executeOnHost(hd['host_name'], hd['host_port'], hd['host_db'], hd['host_user'], hd['host_password'], 'select * from pg_stat_database limit 1')
...
def dataset_to_delimited_stringio(data, delimiter='\t'):
    ...
    for row in data:
        row_as_str = [str(x).replace(delimiter, delimiter_escaped) if x is not None else '\\N' for x in row]
...
if __name__ == '__main__':
    ...
    print(generate_insert_with_returning(data=[{'c1': 't1',  'c2': 't2'}, {'c1': 't11',  'c2': 't22'}], table_name='mytbl', insert_column_names=['c1', 'c2'], return_column_names=['id']))
    exit()
