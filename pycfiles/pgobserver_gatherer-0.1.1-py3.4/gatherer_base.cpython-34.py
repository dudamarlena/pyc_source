...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pgobserver_gatherer/gatherer_base.py
# Compiled at: 2016-04-20 09:16:45
# Size of source mod 2**32: 7819 bytes
from multiprocessing import Process
import logging, random, time, traceback, pgobserver_gatherer.datastore_adapter as datastore_adapter
...
class GathererBase(Process):
    __doc__ = 'Each gatherer is a separate process'
    ...
    def __init__(self, host_data, settings, dataset_name):
        ...
        if globalconfig.config.get('features', {}).get('calculate_deltas', True):
            self.delta_engine = DeltaEngine(self.host_name, self.dataset_name, self.dataset_keys, globalconfig.config.get('features', {}).get('simple_deltas', False))

    def get_and_store_postgres_version(self):
        sql = "\n            select current_setting('server_version_num')::int as server_version_num\n        "
        data = datadb.executeOnHost(self.host_data['host_name'], self.host_data['host_port'], self.host_data['host_db'], self.host_data['host_user'], self.host_data['host_password'], sql, (self.host_id,))
        if data:
    ...
    def process_retry_queue_if_any(self):
        if self.retry_queue:
            if len(self.retry_queue) > 0:
                logging.info('[%s][%s] processing retry queue with %s datasets from', self.host_name, self.gatherer_name, len(self.retry_queue))
        ...
        while True:
            ...
            try:
                try:
                    ...
                    logging.info('[%s][%s] gather_data finished in %s s. rows=%s', self.host_name, self.gatherer_name, round(duration, 2), len(data))
                    ...
                    if data:
                        if globalconfig.config.get('database', {}).get('store_raw_metrics'):
                            ...
                            duration = time.time() - start_time
                            logging.info('[%s][%s] store_data finished in %s s', self.host_name, self.gatherer_name, round(duration, 2))
                        transformed = data
                        if self.delta_engine:
                            if len(output_handlers) > 0:
                                try:
                    ...
                    logging.error('[%s][%s] gather/store loop failed!: %s', self.host_name, self.gatherer_name, traceback.format_exc())
