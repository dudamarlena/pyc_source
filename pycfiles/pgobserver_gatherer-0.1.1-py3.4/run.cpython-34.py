...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pgobserver_gatherer/run.py
# Compiled at: 2016-04-28 10:34:30
# Size of source mod 2**32: 18900 bytes
import importlib
from argparse import ArgumentParser
import json, logging, os, time, collections, psycopg2, pgobserver_gatherer.globalconfig as globalconfig, pgobserver_gatherer.datadb as datadb, pgobserver_gatherer.datastore_adapter as datastore_adapter, pgobserver_gatherer.gatherer_base as gatherer_base, pgobserver_gatherer.gatherer_load as gatherer_load, pgobserver_gatherer.gatherer_schemas as gatherer_schemas, pgobserver_gatherer.gatherer_sproc as gatherer_sproc, pgobserver_gatherer.gatherer_database as gatherer_database, pgobserver_gatherer.gatherer_statements as gatherer_statements, pgobserver_gatherer.gatherer_bgwriter as gatherer_bgwriter, pgobserver_gatherer.gatherer_locks as gatherer_locks, pgobserver_gatherer.gatherer_index as gatherer_index, pgobserver_gatherer.gatherer_table as gatherer_table, pgobserver_gatherer.gatherer_table_io as gatherer_table_io, pgobserver_gatherer.gatherer_connection as gatherer_connection, pgobserver_gatherer.gatherer_kpi as gatherer_kpi
...
gatherers_by_host_id = collections.defaultdict(dict)
gatherer_intervals_snapshot = collections.defaultdict(dict)
...
def main():
    ...
    group_console.add_argument('-g', '--gatherer', default='kpi', help='gatherer interval name. one of: [ {} ]'.format(' | '.join(sorted(list(globalconfig.SUPPORTED_DATASETS.keys())))))
    group_console.add_argument('-i', '--interval', help='interval in seconds between metric gatherings. default 5', default=5, type=int)
    group_console.add_argument('-n', '--no-deltas', action='store_true', help='dont calculate diffs for predefined columns')
    group_console.add_argument('-f', '--filters', nargs='*', help='filter(s) for "key columns" (if defined for a metric) to enable monitoring of a specific table')
    group_console.add_argument('-r', '--human-readable', action='store_true', help='Human readable units for big numbers/sizes')
    ...
    logging.basicConfig(format='%(asctime)s %(levelname)s %(process)d %(message)s', level=logging.DEBUG if int(args.verbose) >= 2 else logging.INFO if int(args.verbose) == 1 else logging.ERROR)
    logging.debug(args)
    if args.init_config:
        ...
        if args.config:
            ...
            if args.ds_host and args.ds_dbname and args.ds_user and args.ds_pass and args.ds_gather_group:
                logging.info('initializing datastore config from command line params...')
                globalconfig.set_config_values('database', {'host': args.ds_host,  'name': args.ds_dbname,  'port': args.ds_port,  'pool_size': args.ds_pool_size, 
                 'backend_user': args.ds_user,  'backend_password': args.ds_pass, 
                 'gather_group': args.ds_gather_group})
            else:
                if args.console:
                    ...
                    globalconfig.set_config_values('features', {'calculate_deltas': False if args.no_deltas else True,  'simple_deltas': True})
                else:
                    parser.print_help()
                    exit(1)
        ...
        if not args.console:
            datadb.setConnectionString(host=globalconfig.config['database']['host'], port=globalconfig.config['database']['port'], dbname=globalconfig.config['database']['name'], username=globalconfig.config['database']['backend_user'], password=globalconfig.config['database']['backend_password'])
    ...
    while True:
        ...
        try:
            ...
            if args.console:
                ...
                host_data = [
                 {'host_name': args.host,  'host_port': args.port,  'host_user': args.username,  'host_db': args.dbname,  'host_settings': json.dumps(test_settings),  'host_password': args.password,  'host_ui_shortname': args.host, 
                  'host_id': 0}]
            else:
            ...
            if len(host_data) == 0:
                ...
                if len(gatherers_by_host_id) > 0:
                    logging.warning('all workers will be terminated...')
            ...
            if new_hosts:
                ...
                for host_id in new_hosts:
                    ...
                    for gatherer_name in active_gatherers:
                        settings = json.loads(host_settings)
                        logging.info('[main] loading worker "%s"."%s" with settings: %s', host_ui_shortname, gatherer_name, settings)
                        ...
                        if p:
                            ...
                            logging.info('[main] "%s" gatherer processes started for host %s (host_id = %s)', p.gatherer_name, host_ui_shortname, host_id)
