...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/tap_strava/__init__.py
...
def oauth_auth(args):
    if not set(OAUTH_CONFIG_KEYS).issubset(args.config.keys()):
        ...
        return
    creds = {'access_token': args.config['access_token']}
    client = Client(**creds)
    url = client.authorization_url(client_id=(args.config['client_id']), redirect_uri='http://127.0.0.1:5000/authorization')
    LOGGER.info(url)
