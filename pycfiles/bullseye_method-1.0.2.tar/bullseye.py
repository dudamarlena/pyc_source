...
# Embedded file name: build/bdist.linux-x86_64/egg/bullseye/bullseye.py
# Compiled at: 2012-03-03 20:47:01
from traits.api import HasTraits, Enum, Bool, Instance, Delegate, on_trait_change
from traitsui.api import View, Item, UItem, HGroup, VGroup, DefaultOverride
from chaco.api import Plot, ArrayPlotData, color_map_name_dict, GridPlotContainer, VPlotContainer, PlotLabel
from chaco.tools.api import ZoomTool, SaveTool, ImageInspectorTool, ImageInspectorOverlay, PanTool
...
class Bullseye(HasTraits):
    ...
    gridm = None
    grid = None
    traits_view = View(HGroup(VGroup(HGroup(VGroup(Item('object.process.x', label='Centroid x', format_str='%.4g µm', tooltip='horizontal beam position relative to chip center'), Item('object.process.a', label='Major 4sig', format_str='%.4g µm', tooltip='major axis beam width 4 sigma ~ 1/e^2 width'), Item('object.process.t', label='Rotation', format_str='%.4g°', tooltip='angle between horizontal an major axis')), VGroup(Item('object.process.y', label='Centroid y', format_str='%.4g µm', tooltip='vertical beam position relative to chip center'), Item('object.process.b', label='Minor 4sig', format_str='%.4g µm', tooltip='major axis beam width 4 sigma ~ 1/e^2 width'), Item('object.process.include_radius', label='Include radius', format_str='%.4g µm', tooltip='energy inclusion radius according to ignore level, used to crop before taking moments')), style='readonly'), VGroup(Item('object.process.capture.shutter', tooltip='exposure time per frame in seconds'), Item('object.process.capture.gain', tooltip='analog camera gain in dB'), Item('object.process.capture.framerate', tooltip='frames per second to attempt, may be limited by shutter time and processing speed'), Item('object.process.capture.average', tooltip='number of subsequent images to boxcar average'), Item('object.process.background', tooltip='background intensity percentile to subtract from image'), Item('object.process.ignore', tooltip='fraction of total intensity to ignore for cropping, determines include radius')), HGroup(Item('object.process.active', tooltip='capture and processing running'), Item('object.process.capture.auto_shutter', tooltip='adjust the shutter time to yield acceptably exposed frames with peak values between .25 and .75'), Item('object.process.track', tooltip='adjust the region of interest to track the beam center, the size is not adjusted'), Item('object.process.capture.dark', tooltip='capture a dark image and subtract it from subsequent images, reset if gain or shutter change')), HGroup(UItem('colormap', tooltip='image colormap'), Item('invert', tooltip='invert the colormap')), UItem('abplots', editor=ComponentEditor(), width=-200, height=-300, resizable=False, tooltip='line sums (red), moments (blue) and 2-sigma markers (green) along the major and minor axes')), UItem('plots', editor=ComponentEditor(), width=800, tooltip='top right: beam image with 2-sigma and 6-sigma radius ellipses and axis markers (green). top left and bottom right: vertial and horizontal line sums (red), moments (blue) and 2-sigma markers (green). bottom left: beam data from moments'), layout='split'), resizable=True, title='Bullseye ― Beam Profiler', width=1000)
    ...
    def __init__(self, **k):
        ...
        self.update_data()
        self.populate_plots()
        self.on_trait_change(self.update_data, 'process.new_data', dispatch='fast_ui')

    def setup_plots(self):
        self.screen = Plot(self.data, resizable='hv', padding=0, bgcolor='lightgray', border_visible=False)
        ...
        self.horiz = Plot(self.data, resizable='h', padding=0, height=100, bgcolor='lightgray', border_visible=False)
        self.horiz.value_mapper.range.low_setting = -0.1 * self.process.capture.maxval
        self.horiz.index_range = self.screen.index_range
        self.vert = Plot(self.data, orientation='v', resizable='v', padding=0, width=100, bgcolor='lightgray', border_visible=False)
        ...
        self.vert.value_mapper.range.low_setting = -0.1 * self.process.capture.maxval
        self.vert.index_range = self.screen.value_range
        self.mini = Plot(self.data, width=100, height=100, resizable='', padding=0, bgcolor='lightgray', border_visible=False)
        self.mini.index_axis.visible = False
        self.mini.value_axis.visible = False
        self.label = PlotLabel(component=self.mini, overlay_position='inside left', font='modern 10', text=self.process.text)
        self.mini.overlays.append(self.label)
        self.plots = GridPlotContainer(shape=(2, 2), padding=0, spacing=(5, 5), use_backbuffer=True, bgcolor='lightgray')
        self.plots.component_grid = [[self.vert, self.screen],
         [
          self.mini, self.horiz]]
        self.screen.overlays.append(ZoomTool(self.screen, x_max_zoom_factor=100.0, y_max_zoom_factor=100.0, x_min_zoom_factor=0.5, y_min_zoom_factor=0.5, zoom_factor=1.2))
        self.screen.tools.append(PanTool(self.screen))
        self.plots.tools.append(SaveTool(self.plots, filename='bullseye.pdf'))
        self.asum = Plot(self.data, padding=0, height=100, bgcolor='lightgray', title='major axis', border_visible=False)
        self.bsum = Plot(self.data, padding=0, height=100, bgcolor='lightgray', title='minor axis', border_visible=False)
        ...
        self.abplots = VPlotContainer(padding=20, spacing=20, use_backbuffer=True, bgcolor='lightgray', fill_padding=True)
        self.abplots.add(self.bsum, self.asum)

    def populate_plots(self):
        self.screenplot = self.screen.img_plot('img', xbounds='xbounds', ybounds='ybounds', interpolation='nearest', colormap=color_map_name_dict[self.colormap])[0]
        self.set_invert()
        ...
        self.screenplot.overlays.append(ImageInspectorOverlay(component=self.screenplot, image_inspector=t, border_size=0, bgcolor='transparent', align='ur', tooltip_mode=False, font='modern 10'))
        ...
        for p in [('ell1_x', 'ell1_y'), ('ell3_x', 'ell3_y'),
         ('a_x', 'a_y'), ('b_x', 'b_y')]:
            self.screen.plot(p, type='line', color='green', alpha=0.5)
