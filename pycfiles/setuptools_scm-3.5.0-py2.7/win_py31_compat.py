...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/setuptools_scm/win_py31_compat.py
...
"""
import os, nt, posixpath, ctypes.wintypes, sys, __builtin__ as builtins
...
def compat_stat(path):
    ...
    return nt.stat_result((
     stat.st_mode,) + (info.file_index, info.volume_serial_number, info.number_of_links) + stat[4:])
...
def get_file_info(path):
    ...
    flags_and_attributes = FILE_ATTRIBUTE_NORMAL | FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT
    template_file = None
    handle = CreateFile(path, desired_access, share_mode, security_attributes, creation_disposition, flags_and_attributes, template_file)
