...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/setuptools_scm/hg.py
...
def _hg_tagdist_normalize_tagcommit(config, tag, dist, node, branch):
    dirty = node.endswith('+')
    node = 'h' + node.strip('+')
    revset = ("(branch(.) and tag({tag!r})::. and (merge() or file('re:^(?!\\.hgtags).*$')) and not tag({tag!r}))").format(tag=tag)
    if tag != '0.0':
        commits = do([
         'hg', 'log', '-r', revset, '--template', '{node|short}'], config.absolute_root)
    else:
        commits = True
    trace('normalize', locals())
    if commits or dirty:
        return meta(tag, distance=dist, node=node, dirty=dirty, branch=branch, config=config)
    else:
...
def archival_to_version(data, config=None):
    ...
    if 'tag' in data:
        ...
        if 'latesttag' in data:
            return meta(data['latesttag'], distance=data['latesttagdistance'], node=node, config=config)
