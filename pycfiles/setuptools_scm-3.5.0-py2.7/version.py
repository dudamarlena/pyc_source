...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/setuptools_scm/version.py
# Compiled at: 2020-02-13 15:05:29
from __future__ import print_function
import datetime, warnings, re
...
def _parse_version_tag(tag, config):
    ...
    if match:
        if len(match.groups()) == 1:
            ...
            key = 'version'
        result = {'version': match.group(key), 
...
def tag_to_version(tag, config=None):
    ...
    if not isinstance(tagdict, dict) or not tagdict.get('version', None):
        ...
        if tagdict.get('suffix', ''):
            warnings.warn(("tag {!r} will be stripped of its suffix '{}'").format(tag, tagdict['suffix']))
...
class ScmVersion(object):
    ...
    def __init__(self, tag_version, distance=None, node=None, dirty=False, preformatted=False, branch=None, **kw):
        if kw:
    ...
    @property
    def extra(self):
        warnings.warn('ScmVersion.extra is deprecated and will be removed in future', category=DeprecationWarning, stacklevel=2)
        return self._extra
    ...
    def __repr__(self):
        return self.format_with('<ScmVersion {tag} d={distance} n={node} d={dirty} b={branch}>')

    def format_with(self, fmt, **kw):
        return fmt.format(time=self.time, tag=self.tag, distance=self.distance, node=self.node, dirty=self.dirty, branch=self.branch, **kw)
...
def _format_local_with_time(version, time_format):
    if version.exact or version.node is None:
        ...
        return version.format_choice('+{node}', '+{node}.d{time:{time_format}}', time_format=time_format)
        return
...
def format_version(version, **config):
    ...
    if version.preformatted:
        return version.tag
    version_scheme = callable_or_entrypoint('setuptools_scm.version_scheme', config['version_scheme'])
    local_scheme = callable_or_entrypoint('setuptools_scm.local_scheme', config['local_scheme'])
