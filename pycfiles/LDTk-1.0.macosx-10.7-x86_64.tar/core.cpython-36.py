...
# [GCC 8.4.0]
# Embedded file name: /Users/hannu/soft/anaconda/envs/p36/lib/python3.6/site-packages/ldtk/core.py
...
import warnings, astropy.io.fits as pf
from glob import glob
from os.path import exists, join, basename
from pickle import dump, load
from numpy import array, asarray, arange, linspace, zeros, zeros_like, ones, ones_like, delete, append, diag, poly1d, polyfit, vstack, diff, cov, exp, log, sqrt, clip, pi, percentile
...
if not exists(ldtk_root):
    os.mkdir(ldtk_root)
TWO_PI = 2 * pi
TEFF_POINTS = append(arange(2300, 7001, 100), arange(7200, 12001, 200))
TEFF_POINTS = delete(TEFF_POINTS, TEFF_POINTS.searchsorted(5000))
...
def dxdy(f, x, y, h=1e-05):
    return (f([x + h, y + h]) - f([x + h, y - h]) - (f([x - h, y + h]) - f([x - h, y - h]))) / (4 * h ** 2)


def v_from_poly(lf, x0, dx=0.001, nx=5):
...
class SIS(object):
    __doc__ = 'Simple wrapper for a specific intensity spectrum file.'

    def __init__(self, fname):
        self.filename = fname
        with pf.open(fname) as (hdul):
            self.wl0 = hdul[0].header['crval1'] * 0.1
            self.dwl = hdul[0].header['cdelt1'] * 0.1
            self.nwl = hdul[0].header['naxis1']
            self.data = hdul[0].data
            self.mu = hdul[1].data
            self.z = sqrt(1 - self.mu ** 2)
            self.wl = self.wl0 + arange(self.nwl) * self.dwl

    def intensity_profile(self, l0=0, l1=100000.0):
