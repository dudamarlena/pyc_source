...
# [GCC 8.4.0]
# Embedded file name: /Users/hannu/soft/anaconda/envs/p36/lib/python3.6/site-packages/ldtk/ldtk.py
# Compiled at: 2017-09-18 08:19:31
# Size of source mod 2**32: 14764 bytes
"""
...
class LDPSet(object):
    ...
    def __init__(self, filters, mu, ldp_samples):
        ...
        self._limb_mu = sqrt(1.0 - self._z_orig[self._limb_i] ** 2)
        self.redefine_limb()
        self._lnl = zeros(self._nfilters)
        self.set_uncertainty_multiplier(1.0)
        self._update()
    ...
    def _update(self):
        ...
        self._lnc1 = -0.5 * self._nmu * log(TWO_PI)
        self._lnc2 = [-log(self._em * e).sum() for e in self._std]
        self._err2 = [(self._em * e) ** 2 for e in self._std]

    def set_limb_z(self, z):
        self._limb_z = z
        self._limb_i = argmin(abs(self._z_orig - z))
        self._limb_mu = sqrt(1.0 - z ** 2)
        self.reset_sampling()

    def set_limb_mu(self, mu):
        self._limb_mu = mu
        self._limb_i = argmin(abs(self._mu_orig - mu))
        self._limb_z = sqrt(1.0 - mu ** 2)
        self.reset_sampling()

    def redefine_limb(self):
        self._z = self._z_orig[self._limb_i:] / self._limb_z
        self._mu = sqrt(1.0 - self._z ** 2)
    ...
    def _coeffs(self, return_cm=False, do_mc=False, n_mc_samples=20000, mc_thin=25, mc_burn=25, ldmodel=QuadraticModel, ngc=4):
        """
        ...
        for iflt, qc in enumerate(qcs):
            ...
            for ic in range(npar):
                s[ic] = 1.0 / sqrt(-dx2((lambda x: self._lnlike(x, flt=iflt, ldmodel=ldmodel)), qc, 1e-05, dim=ic))
...
class LDPSetCreator(object):
    ...
    def __init__(self, teff, logg, z, filters, qe=None, limits=None, offline_mode=False, force_download=False, verbose=False, cache=None):
        ...
        def set_lims(ms_or_samples, pts, plims=[
 0.135, 99.865]):
        ...
        with pf.open(self.files[0]) as (hdul):
            wl0 = hdul[0].header['crval1'] * 0.1
            dwl = hdul[0].header['cdelt1'] * 0.1
            nwl = hdul[0].header['naxis1']
