...
class JSBuild(build_ext):
    def run(self):
        self.yarn = shutil.which('yarn')

        try:
            subprocess.check_output([self.yarn, '--version'])
        except OSError:
            raise RuntimeError(
                "Yarn must be installed to build the following extensions: " +
    ...
    def build_extension(self):
        ...
        subprocess.check_call([self.yarn], cwd=self.build_temp, env=env)
        subprocess.check_call([self.yarn, 'build'], cwd=self.build_temp, env=env)
