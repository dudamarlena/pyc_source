...
"Yevhen Kuzmovych's python client to httpbin.org, open sourced under the MIT license"
...
def read_version():
    ...
    finder.visit(ast.parse(local_file('kuzmoyev_httpbin', 'version.py')))
...
setup(
    ...
    author_email='kuzmovich.goog@gmail.com',
    description=read_version(),
    include_package_data=True,
    install_requires=read_requirements(),
    long_description=read_readme(),
    name='kuzmoyev_httpbin',
    packages=find_packages(exclude=['*tests*']),
    test_suite='nose.collector',
    version=read_version(),
    entry_points={
        'console_scripts': ['kuzmoyev-httpbin = kuzmoyev_httpbin.console:entrypoint'],
    },
    package_data={
        'kuzmoyev_httpbin': ' '.join([
            '*.cfg',
            '*.py',
            '*.rst',
            '*.txt',
