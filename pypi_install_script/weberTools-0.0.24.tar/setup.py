...
def loadLongDesc(fname):
    ...
    # WeiYF.20170605 为避免 pip install 时出现 UnicodeDecodeError 暂时直接使用简单串
    return "weberTools"


def main_setup():
    setup(
        name="weberTools",  # 包的名字
        packages=[  # 包含的包，可以多个，这是一个列表
            'weberTools',
            'weberTools/SftpTools',
            'weberTools/FtpTools'
        ],
        version="0.0.24",  # 当前包的版本，这个按你自己需要的版本控制方式来

        keywords="public weberTools weber",  # 关于当前包的一些关键字，方便PyPI进行分类。
        author="Weber Juche",  # 包的作者
        author_email="weber.juche@gmail.com",  # 作者的邮件地址
        url="https://github.com/Weber-JC/PythonEggs/tree/master/weberTools",  # 这个包的项目地址
        license="MIT",  # 授权方式,

        # zip_safe=True,
        zip_safe=False,

        description='Public tools library wrote by Weber Juche.',
        # u"包含了常用的简单函数的共享库", #关于这个包的描述,这个会出现 PyPI 的列表页面

        long_description=loadLongDesc("README.md"),  # 参见loadLongDesc方法说明
