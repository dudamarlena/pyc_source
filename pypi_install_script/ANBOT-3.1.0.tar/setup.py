...
def parse_cli_args():
    parser = argparse.ArgumentParser(description="ANBOT - Discord Bot's instance manager (V3)")
    parser.add_argument(
...
def basic_setup():
    ...
    print()
    print(
        "Your basic configuration has been saved. Please run `anbot <name>` to"
...
async def create_backup(selected, instance_data):
    if confirm("Would you like to make a backup of the data for this instance? (y/n)"):
        ...
        backup_filename = "anbotv3-{}-{}.tar.gz".format(
        ...
        if pth.exists():
            ...
            from ANBOT.downloader.repo_manager import RepoManager
